odoo.define("website_twitter_wall.views", function (require) {
  ...
  var TweetWall = Widget.extend({
    ...
    init: function (parent, wallID) {
      ...
      var zoomLevel = 1 / (window.devicePixelRatio * 0.8);
    ...
    _toggleShuffle: function () {
      this.shuffle = this.shuffle === false;
    },
    /**
     * @private
     */
    ...
    _processTweet: function () {
      ...
      var tweets = _.filter(self.pool_cache, function (f) {
        ...
        if (
          f.round <= leastRound &&
          (!el.length || el.offset().top > $(window).height())
        ) {
          return f;
        }
      });
      ...
      if (tweets.length) {
        ...
        self.prependTweetsTo = nextPrepend.length
          ? nextPrepend.first()
          : $(".o-tw-walls-col").first();
      }
    },
    /**
     * @private
  ...
  publicWidget.registry.websiteTwitterWall = publicWidget.Widget.extend({
    ...
    start: function () {
      ...
      var observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (
            ...
            mutation.attributeName === "data-tweet-id"
          ) {
            $(mutation.target.contentDocument)
              .find(".Tweet-header .Tweet-brand, .Tweet-body .Tweet-actions")
              .remove();
            $(mutation.target.contentDocument)
              .find("body")
              .css("zoom", $("body").css("zoom"));
            $(mutation.target.contentDocument)
              .find(".EmbeddedTweet")
              .removeClass("js-clickToOpenTarget");
          }
        });
      });
      ...
      // Initialize widgets
      this.twitterWall = new TweetWall(
        this,
        parseInt($(".o-tw-walls").data("wall-id"))
      );
    ...
    _setColumns: function (number, single) {
      ...
      while (i < number) {
        newCols.push(
          $('<div class="o-tw-walls-col col-' + 12 / number + '"></div>').appendTo(
            ".o-tw-walls"
          )
        );
        i++;
      ...
      $(".o-tw-walls-col")
        .removeClass("col-4 col-6 col-12")
        .addClass("col-" + 12 / number);
      if (single) {
