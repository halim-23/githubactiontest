...
class WebsiteTwitterWall(models.Model):
    ...
    mode = fields.Selection(
        [("recent", "Recent"), ("popular", "Popular"), ("mixed", "Mixed")],
        default="recent",
        string="Type of tweets",
        help="Most recent tweets, most popular tweets, or both",
    )
    image = fields.Binary()
    ...
    def _compute_website_url(self):
        ...
        for wall in self:
            if wall.id:
                wall.website_url = "%s/twitter_wall/view/%s" % (
                    wall.get_base_url(),
                    slug(wall),
                )
    ...
    def fetch_tweets(self):
        ...
        try:
            if fields.Datetime.from_string(
                ...
                self._cr.execute(
                    "SELECT value FROM ir_config_parameter WHERE key='twitter_wall_search' FOR UPDATE NOWAIT"
                )
    ...
    def search_tweets(self):
        ...
        website = self.env["website"].search(
            [("twitter_api_key", "!=", False), ("twitter_api_secret", "!=", False)],
            limit=1,
        )
        ...
        is_favorite_search = (
            "favorites:" in self.search_pattern
        ...
        try:
            ...
            raise UserError(
                _("The tweets search failed. Check your credentials and access token")
            )
        ...
        if tweet:
            tweet.write(
                {
                    "wall_ids": [
                        ...
                        if wall_id not in tweet.wall_ids.ids
                    ]
                }
            )
        else:
            try:
                if not author_id:
                    ...
                    card_url = (
                        "https://api.twitter.com/1/statuses/oembed.json?id=%s&omit_script=true"
                        % (tweet_id)
                    )
                else:
                ...
                response = requests.get(
                    card_url, headers={"Content-Type": "application/json"}
                )
    ...
    def _website_url(self, name, arg):
        ...
        res.update(
            {
                (wall.id, "%s/twitter_wall/view/%s" % (wall.get_base_url(), slug(wall)))
                for wall in self
            }
        )
        return res
