...
class SaleReport(models.Model):
    ...
    def read_group(
        ...
        """
        avg_days_to_confirm = next(
            (field for field in fields if re.search(r"\bavg_days_to_confirm\b", field)),
            False,
        )
        ...
        if avg_days_to_confirm:
            ...
            if any(
                ...
                for field in [avg_days_to_confirm]
                if field
            ):
                raise UserError(
                    "Value: 'avg_days_to_confirm' should only be used to show an average. If you are seeing this message then it is being accessed incorrectly."
                )
