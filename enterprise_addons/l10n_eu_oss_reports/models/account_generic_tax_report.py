...
class AccountGenericTaxReport(models.AbstractModel):
    ...
    def _init_filter_tax_report(self, options, previous_options=None):
        ...
        oss_rep_ln = self.env["account.tax.repartition.line"].search(
            [("tag_ids", "in", oss_tag.ids), ("company_id", "in", company_ids)], limit=1
        )

        if oss_rep_ln:
            options["available_oss_reports"] = {
                "generic_oss_import",
                "generic_oss_no_import",
            }
        else:
        ...
        if previous_options_report in {"generic_oss_import", "generic_oss_no_import"}:
            options["tax_report"] = (
                previous_options_report
                if previous_options_report in options["available_oss_reports"]
                else "generic"
            )
        ...
        if options["tax_report"] == "generic_oss_no_import":
            domain += [
                ...
                (
                    "tax_tag_ids",
                    "not in",
                    self.env.ref("l10n_eu_oss.tag_eu_import").ids,
                ),
            ]
            ...
            isinstance(options["tax_report"], int)
    ...
    def _process_generic_lines_for_oss_report(self, generic_lines):
        ...
        def append_country_and_taxes_lines(rslt, tax_lines_by_country):
            for country, tax_lines in sorted(
                tax_lines_by_country.items(), key=lambda elem: elem[0].display_name
            ):
                col_number = len(tax_lines[0]["columns"]) if tax_lines else 0
                tax_sums = [
                    sum(
                        tax_lines[line_index]["columns"][col_index]["no_format"]
                        for line_index in range(len(tax_lines))
                    )
                ...
                for tax_sum in tax_sums:
                    country_columns += [
                        {"name": ""},
                        {"no_format": tax_sum, "name": self.format_value(tax_sum)},
                    ]

                country_line = {
                    "id": self._get_generic_line_id(
                        "res.country", country.id, parent_line_id=line["id"]
                    ),
        ...
        for line in generic_lines:
            ...
            if model_info[0] != "account.tax":
                ...
                if not tax_oss_country:
                    raise UserError(
                        _(
                            "OSS tax %s is not mapped in any fiscal position with a country set.",
                            tax.display_name,
                        )
                    )
                    ...
                    raise UserError(
                        _(
                            "Inconsistent setup: OSS tax %s is mapped in fiscal positions from different countries.",
                            tax.display_name,
                        )
                    )
    ...
    def _get_reports_buttons(self, options):
        ...
        if options["tax_report"] in {"generic_oss_import", "generic_oss_no_import"}:
            ...
            res = [
                button for button in res if button["action"] != "periodic_vat_entries"
            ]
    ...
    def get_xml(self, options):
        ...
        eu_standard_rates = {
            source_code: rate
            for source_code, rate, target_code in sorted(EU_TAX_MAP.keys())
        }
        tax_scopes = dict(
            self.env["account.tax"].fields_get()["tax_scope"]["selection"]
        )
                ...
                data[current_country].append(
                    {
                        ...
                        "supply_type": tax_scopes[tax.tax_scope].upper()
                        if tax.tax_scope
                        else "GOODS",
                        "rate_type": "STANDARD"
                        if tax.amount == eu_standard_rates[current_country.code]
                        else "REDUCED",
                    }
                )

        values = {
            "VATNumber": sender_company.vat
            if sender_company.account_fiscal_country_id in eu_countries
            else None,
            "VoesNumber": sender_company.voes
            if sender_company.account_fiscal_country_id not in eu_countries
            else None,
            "IOSSNumber": sender_company.ioss
            if options["tax_report"] == "generic_oss_import"
            else None,
            "IntNumber": sender_company.intermediary_no
            if options["tax_report"] == "generic_oss_import"
            else None,
            "Year": date_to.year,
            "Quarter": quarter,
            "Month": month,
            "country_taxes": data,
            "creation_timestamp": fields.Datetime.context_timestamp(
    ...
    def _get_oss_xml_template(self, options):
        ...
        country_code = self._get_sender_company_for_export(
            options
