odoo.define("mrp_workorder_navigation.tests", function (require) {
  ...
  QUnit.module(
    "mrp_workorder_tabletview_navigation",
    {
      beforeEach: function () {
        const models = {
          "mrp.workorder": {
        ...
        const views = {
          ...
          "mrp.workorder,1,form":
            ...
            "</group>" +
            "</form>",
          "mrp.workorder,false,search":
            '<search><field name="name" string="Foo"/></search>',
        };
    ...
    function () {
      QUnit.test("workorder navigation", async function (assert) {
        ...
        assert.containsOnce(
          webClient,
          ".o_workorder_tablet",
          "tablet view should be opened"
        );

        await testUtils.dom.click($(".btn.o_workorder_icon_btn"));
        await legacyExtraNextTick();
        assert.strictEqual(
          $(webClient.el).find(".breadcrumb-item").length,
          1,
          "there should be only one controller in actionManager"
        );
      });
    }
  );
