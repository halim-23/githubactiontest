odoo.define("mrp_embed_viewer_widget_no_reload.tests", function (require) {
  ...
  QUnit.module("mrp_embed_viewer_widget_no_reload", {
    beforeEach: function () {
      ...
      this.formArch =
        "<form>" +
        '<field name="worksheet" widget="mrp_embed_viewer_no_reload" attrs="' +
        attrs +
        '"/>' +
        "</form>";
          ...
          records: [
            {
              id: 1,
              worksheet:
                "https://docs.google.com/presentation/d/1yVipMA_MFXDnxl8G_LyBPdmB7sHsLXoTT-Pn6dx6Em0/edit#slide=id.p",
            },
            {
              id: 2,
              worksheet: false,
  ...
  QUnit.test("Embedded Viewer Widget No Reload : Visible Slide", async function (
    assert
  ) {
    assert.expect(4);
    ...
    assert.strictEqual(
      Boolean(renderer),
      true,
      "There should be a renderer linked to the form"
    );

    var recordWidgets = renderer.allFieldWidgets[form.handle];
    assert.strictEqual(
      Boolean(recordWidgets),
      true,
      "There should be widgets for this record"
    );
  ...
  QUnit.test("Embedded Viewer Widget No Reload : Invisible Slide", async function (
    assert
  ) {
    assert.expect(4);
