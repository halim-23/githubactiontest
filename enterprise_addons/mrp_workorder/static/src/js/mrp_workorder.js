odoo.define("mrp_workorder.update_kanban", function (require) {
  ...
  var BackArrow = FieldInteger.extend({
    ...
    _render: function () {
      this.$el.html(
        '<button class="btn btn-secondary o_workorder_icon_btn o_workorder_icon_back"><i class="fa fa-arrow-left"/></button>'
      );
    },
    _onClick: function () {
      var self = this;
  ...
  function tabletRenderButtons($node) {
    ...
    this.$buttons = $("<div/>");
    this.$buttons.html(
      '<button class="btn btn-secondary back-button"><i class="fa fa-arrow-left"/></button>'
    );
