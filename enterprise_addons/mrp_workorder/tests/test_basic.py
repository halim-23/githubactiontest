...
class TestWorkOrderProcessCommon(TestMrpCommon):
    ...
    def test_cancel_mo_with_routing(self):
        ...
        self.assertEqual(
            backorder.move_raw_ids[0].state,
            "cancel",
            "The other move raw are cancelled like their MO.",
        )
        self.assertEqual(
            backorder.move_raw_ids[1].state,
            "cancel",
            "The other move raw are cancelled like their MO.",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            backorder.move_finished_ids[0].state,
            "cancel",
    ...
    def test_putaway_after_manufacturing_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.graphics_card, self.stock_location, 20
        )
...
class TestWorkOrderProcess(TestWorkOrderProcessCommon):
    ...
    def test_00_workorder_process(self):
        ...
        self.env["stock.move"].search(
            [("product_id", "in", [product_bolt.id, product_screw.id])]
        )._do_unreserve()
        ...
        lot_sheet = self.env["stock.production.lot"].create(
            {"product_id": product_table_sheet.id, "company_id": self.env.company.id}
        )
        lot_leg = self.env["stock.production.lot"].create(
            {"product_id": product_table_leg.id, "company_id": self.env.company.id}
        )
        lot_bolt = self.env["stock.production.lot"].create(
            {"product_id": product_bolt.id, "company_id": self.env.company.id}
        )
        ...
        self.assertEqual(
            workorder.state, "waiting", "workorder state should be waiting."
        )
        ...
        finished_lot = self.env["stock.production.lot"].create(
            {
                "product_id": production_table.product_id.id,
        ...
        move_table_sheet = production_table.move_raw_ids.filtered(
            lambda x: x.product_id == product_table_sheet
        )
    ...
    def test_00b_workorder_process(self):
        ...
        bom.bom_line_ids.filtered(
            lambda p: p.product_id == product_table_sheet
        ).operation_id = bom.operation_ids[0]
        bom.bom_line_ids.filtered(
            lambda p: p.product_id == product_table_leg
        ).operation_id = bom.operation_ids[1]
        bom.bom_line_ids.filtered(
            lambda p: p.product_id == product_bolt
        ).operation_id = bom.operation_ids[2]
        ...
        self.assertEqual(
            workorders[0].state, "waiting", "First workorder state should be waiting."
        )
        ...
        move_leg = production_table.move_raw_ids.filtered(
            lambda p: p.product_id == product_table_leg
        )
        ...
        workorders[2]._next()
        move_table_bolt = production_table.move_raw_ids.filtered(
            lambda p: p.product_id.id == product_bolt.id
        )
        ...
        wiz = self.env["change.production.qty"].create(
            {"mo_id": production_table.id, "product_qty": 1.0}
        )
        wiz.change_prod_qty()
        # ---------------------------------------------------------------
        # Check consume quants and produce quants after posting inventory
    ...
    def test_explode_from_order(self):
        ...
        self.workcenter_1.write(
            {"capacity": 1, "time_start": 0, "time_stop": 0, "time_efficiency": 100}
        )
        ...
        self.assertEqual(
            ...
            4,
            "Consume material lines are not generated properly.",
        )
        ...
        self.assertEqual(
            len(product_4_consume_moves), 2, "Consume move are not generated properly."
        )
        self.assertEqual(
            sum(product_4_consume_moves.mapped("product_uom_qty")),
            24,
            "Consume material quantity of Stick should be 24.",
        )
        self.assertFalse(
            product_5_consume_moves, "Move should not create for phantom bom"
        )

        # create required lots
        lot_product_2 = self.env["stock.production.lot"].create(
            {"product_id": self.product_2.id, "company_id": self.env.company.id}
        )
        ...
        kit_wo = man_order.workorder_ids.filtered(
            lambda wo: wo.operation_id.name == "Gift Wrap Maching"
        )
        door_wo_1 = man_order.workorder_ids.filtered(
            lambda wo: wo.operation_id.name == "Cutting Machine"
        )
        door_wo_2 = man_order.workorder_ids.filtered(
            lambda wo: wo.operation_id.name == "Weld Machine"
        )
        workorders._compute_state()
        for workorder in workorders:
            self.assertEqual(
                workorder.workcenter_id, self.workcenter_1, "Workcenter does not match."
            )
        self.assertEqual(kit_wo.state, "ready", "Workorder should be in ready state.")
        self.assertEqual(
            door_wo_1.state, "ready", "Workorder should be in ready state."
        )
        self.assertEqual(
            door_wo_2.state, "pending", "Workorder should be in pending state."
        )
        self.assertEqual(
            kit_wo.duration_expected,
            960,
            "Workorder duration should be 960 instead of %s."
            % str(kit_wo.duration_expected),
        )
        self.assertEqual(
            door_wo_1.duration_expected,
            480,
            "Workorder duration should be 480 instead of %s."
        ...
        kit_wo.button_start()
        finished_lot = self.env["stock.production.lot"].create(
            {"product_id": man_order.product_id.id, "company_id": self.env.company.id}
        )
        kit_wo.write({"finished_lot_id": finished_lot.id, "qty_producing": 48})
        ...
        door_wo_1.write({"finished_lot_id": finished_lot.id, "qty_producing": 48})
        ...
        self.assertEqual(
            door_wo_2.state, "ready", "Workorder should be in ready state."
        )
    ...
    def test_01_without_workorder(self):
        ...
        lot_charger = self.env["stock.production.lot"].create(
            {"product_id": product_charger.id, "company_id": self.env.company.id}
        )
        lot_keybord = self.env["stock.production.lot"].create(
            {"product_id": product_keybord.id, "company_id": self.env.company.id}
        )
        ...
        laptop_lot_001 = self.env["stock.production.lot"].create(
            {"product_id": custom_laptop.id, "company_id": self.env.company.id}
        )
        ...
        details_operation_form = Form(
            mo_custom_laptop.move_raw_ids[0],
        ...
        details_operation_form = Form(
            mo_custom_laptop.move_raw_ids[1],
            view=self.env.ref("stock.view_stock_move_operations"),
        )
        with details_operation_form.move_line_ids.edit(0) as ml:
            ml.qty_done = 12
        ...
        for move in mo_custom_laptop.move_raw_ids:
            self.assertEqual(
                move.quantity_done,
                12,
                "Wrong produced quantity on raw material %s" % (move.product_id.name),
            )
    ...
    def test_03_test_serial_number_defaults(self):
        ...
        serial_a = self.env["stock.production.lot"].create(
            {"product_id": laptop.id, "company_id": self.env.company.id}
        )
        workorder.finished_lot_id = serial_a
        workorder = self.env["mrp.workorder"].browse(
            workorder.record_production()["res_id"]
        )
        serial_b = self.env["stock.production.lot"].create(
            {"product_id": laptop.id, "company_id": self.env.company.id}
        )
        ...
        serial_c = self.env["stock.production.lot"].create(
            {"product_id": laptop.id, "company_id": self.env.company.id}
        )
    ...
    def test_03b_test_serial_number_defaults(self):
        ...
        workorder_0 = self.env["mrp.workorder"].browse(
            workorder_0.record_production()["res_id"]
        )
        ...
        workorder_1 = self.env["mrp.workorder"].browse(
            workorder_1.record_production()["res_id"]
        )
        ...
        self.assertEqual(
            sum(productions.move_finished_ids.move_line_ids.mapped("qty_done")), 2
        )
        self.assertEqual(
            productions.move_finished_ids.move_line_ids.mapped("lot_id"), lot_1 | lot_2
        )
    ...
    def test_04_test_planning_date(self):
        ...
        self.assertAlmostEqual(
            ...
            msg="Workorder should be planned now.",
        )
        self.assertAlmostEqual(
            workorder.date_planned_finished,
            datetime.now() + timedelta(hours=1),
            delta=timedelta(seconds=10),
            msg="Workorder should be done in an hour.",
        )
    ...
    def test_04b_test_planning_date(self):
        ...
        self.assertAlmostEqual(
            ...
            msg="Workorder should be planned tomorrow.",
        )
        self.assertAlmostEqual(
            workorder.date_planned_finished,
            date_start + timedelta(hours=1),
            delta=timedelta(seconds=1),
            msg="Workorder should be done one hour later.",
        )
    ...
    def test_planning_overlaps_wo(self):
        ...
        # Check that the workorder is planned now and that it lasts one hour
        self.assertAlmostEqual(
            workorder_prev.date_planned_start,
            date_start,
            delta=timedelta(seconds=10),
            msg="Workorder should be planned in +30min",
        )
        self.assertAlmostEqual(
            workorder_prev.date_planned_finished,
            date_start + timedelta(hours=1),
            delta=timedelta(seconds=10),
            msg="Workorder should be done in +90min",
        )
        ...
        self.assertAlmostEqual(
            ...
            workorder_prev.date_planned_finished,
            delta=timedelta(seconds=10),
            msg="Workorder should be planned after the first one",
        )
        self.assertAlmostEqual(
            workorder.date_planned_finished,
            workorder_prev.date_planned_finished + timedelta(hours=1),
            delta=timedelta(seconds=10),
            msg="Workorder should be done one hour later.",
        )
    ...
    def test_planning_overlaps_wo_02(self):
        ...
        self.assertAlmostEqual(
            workorder_a.date_planned_start,
            (date_start + timedelta(days=1, minutes=24)),
            delta=timedelta(seconds=10),
            msg="The workorder should be planned for the first available interval",
        )
        self.assertAlmostEqual(
            workorder_a.date_planned_finished,
            (date_start + timedelta(days=1, minutes=48)),
            delta=timedelta(seconds=10),
            msg="Workorder should be done in 24 minutes",
        )
        self.assertAlmostEqual(
            workorder_b.date_planned_start,
            (date_start + timedelta(days=1, minutes=48)),
            delta=timedelta(seconds=10),
            msg="The workorder should be planned for the first available interval",
        )
        self.assertAlmostEqual(
            workorder_b.date_planned_finished,
            (date_start + timedelta(days=2, minutes=12)),
            delta=timedelta(seconds=10),
            msg="Workorder should be done in 24 minutes",
        )
    ...
    def test_change_production_1(self):
        ...
        self.assertEqual(
            production_table.workorder_ids[0].qty_producing,
            2,
            "Quantity to produce not updated",
        )
    ...
    def test_planning_0(self):
        ...
        self.assertEqual(
            ...
            "Alternative workcenter is not reciprocal",
        )
        self.assertEqual(
    ...
    def test_planning_3(self):
        ...
        self.workcenter_1.alternative_workcenter_ids = self.wc_alt_1 | self.wc_alt_2
        self.env["mrp.workcenter"].search([]).resource_calendar_id.write(
            {"tz": "UTC"}
        )  # compute all date in UTC
        ...
        mo.button_plan()
        self.assertEqual(
            mo.workorder_ids[0].workcenter_id, self.wc_alt_2, "wrong workcenter"
        )
        wo1_start = mo.workorder_ids[0].date_planned_start
        wo1_stop = mo.workorder_ids[0].date_planned_finished
        self.assertAlmostEqual(
            wo1_start, start, delta=timedelta(seconds=10), msg="Wrong plannification"
        )
        self.assertAlmostEqual(
            wo1_stop,
            start + timedelta(minutes=85.58),
            delta=timedelta(seconds=10),
            msg="Wrong plannification",
        )
        ...
        self.assertAlmostEqual(
            wo1_stop,
            planned_date + timedelta(minutes=85.59),
            delta=timedelta(seconds=10),
            msg="Wrong plannification",
        )
    ...
    def test_planning_6(self):
        ...
        wo = mo.workorder_ids
        self.assertAlmostEqual(
            wo.date_planned_start, planned_date, delta=timedelta(seconds=10)
        )
        self.assertAlmostEqual(
            wo.date_planned_finished,
            planned_date + timedelta(minutes=60),
            delta=timedelta(seconds=10),
        )
        wo.button_start()
        ...
        self.assertAlmostEqual(
            wo.date_finished, datetime.now(), delta=timedelta(seconds=10)
        )
        self.assertAlmostEqual(
            wo.date_planned_finished, datetime.now(), delta=timedelta(seconds=10)
        )
        ...
        wo.button_start()
        self.assertAlmostEqual(
            wo.date_start, datetime.now(), delta=timedelta(seconds=10)
        )
        self.assertAlmostEqual(
            wo.date_planned_start, datetime.now(), delta=timedelta(seconds=10)
        )
    ...
    def test_conflict_and_replan(self):
        ...
        bom.bom_line_ids.filtered(
            lambda p: p.product_id == self.product_table_sheet
        ).operation_id = bom.operation_ids[0].id
        bom.bom_line_ids.filtered(
            lambda p: p.product_id == self.product_table_leg
        ).operation_id = bom.operation_ids[1].id
        bom.bom_line_ids.filtered(
            lambda p: p.product_id == self.product_bolt
        ...
        self.assertFalse(
            wo1.id in wo1._get_conflicted_workorder_ids(), "Shouldn't conflict"
        )
        self.assertFalse(
            wo2.id in wo2._get_conflicted_workorder_ids(), "Shouldn't conflict"
        )
        self.assertFalse(
            wo3.id in wo3._get_conflicted_workorder_ids(), "Shouldn't conflict"
        )

        # Conflicted with wo1
        wo2.write(
            {
                "date_planned_start": wo1.date_planned_start,
        ...
        wo3.write(
            {
                "date_planned_start": wo1.date_planned_start - timedelta(weeks=1),
                "date_planned_finished": wo1.date_planned_finished - timedelta(weeks=1),
            }
        )

        self.assertTrue(
            wo2.id in wo2._get_conflicted_workorder_ids(), "Should conflict with wo1"
        )
        self.assertTrue(
            wo1.id in wo1._get_conflicted_workorder_ids(), "Should conflict with wo2"
        )

        self.assertTrue(
            "text-danger" in wo2.json_popover,
            "Popover should in be in red (due to conflict)",
        )
        self.assertTrue(
            "text-danger" in wo3.json_popover,
            "Popover should in be in red (due to bad order of wo)",
        )
        self.assertTrue(
            "text-warning" in wo3.json_popover, "Popover contains of warning (late)"
        )

        wo1.button_start()
        self.assertEqual(wo1.state, "progress")
        self.assertEqual(
            wo2.id in wo2._get_conflicted_workorder_ids(),
            False,
            "Shouldn't have a conflict because wo1 is in progress",
        )
        ...
        self.assertEqual(
            ...
            "Planned date of Workorder 1 shouldn't change (because it is in progress)",
        )
        self.assertNotEqual(
            wo2.date_planned_start,
            wo2_date_planned_start,
            "Planned date of Workorder 2 should be updated",
        )
        self.assertNotEqual(
            wo3.date_planned_start,
            wo3_date_planned_start,
            "Planned date of Workorder 3 should be updated",
        )
        self.assertTrue(
            wo3.date_planned_start > wo2.date_planned_start,
            "Workorder 2 should be before the 3",
        )
...
class TestRoutingAndKits(TransactionCase):
    ...
    def test_1(self):
        ...
        self.assertEqual(
            mo.workorder_ids[1].move_raw_ids.product_id, self.compfinished1
        )
    ...
    def test_3(self):
        ...
        self.bom_finished1.bom_line_ids[
            0
        ].operation_id = self.bom_finished1.operation_ids[0]
        self.bom_finished1.bom_line_ids[
            1
        ].operation_id = self.bom_finished1.operation_ids[1]
        ...
        self.assertEqual(len(mo.workorder_ids), 3)
        self.assertEqual(
            mo.workorder_ids[0].move_raw_ids.product_id, self.compfinished1
        )
    ...
    def test_4(self):
        ...
        self.bom_finished1.write(
            {"bom_line_ids": [(0, 0, {"product_id": kit2.id, "product_qty": 1})]}
        )
        ...
        self.assertEqual(
            set(mo.workorder_ids[1].move_raw_ids.product_id.ids),
            {self.compfinished1.id, compkit2.id},
        )
        self.assertEqual(mo.workorder_ids[2].move_raw_ids.product_id, self.compkit1)

    def test_5(self):
        # Main bom: set the normal component to the first of the two operations of the routing.
        bomline_compfinished = self.bom_finished1.bom_line_ids.filtered(
            lambda bl: bl.product_id == self.compfinished1
        )
