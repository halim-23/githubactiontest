...
class TestWorkOrder(common.TestMrpCommon):
    ...
    def setUpClass(cls):
        ...
        cls.submarine_pod = cls.env["product.product"].create(
            {"name": "Submarine pod", "type": "product", "tracking": "serial"}
        )
        ...
        cls.elon_musk = cls.env["product.product"].create(
            {"name": "Elon Musk", "type": "product", "tracking": "serial"}
        )
        ...
        cls.metal_cylinder = cls.env["product.product"].create(
            {"name": "Metal cylinder", "type": "product", "tracking": "lot"}
        )
        ...
        cls.trapped_child = cls.env["product.product"].create(
            {"name": "Trapped child", "type": "product", "tracking": "none"}
        )
        # Bill of material
        ...
        Quant._update_available_quantity(
            cls.elon_musk, cls.location_1, 1.0, lot_id=cls.elon1
        )
        Quant._update_available_quantity(
            cls.elon_musk, cls.location_1, 1.0, lot_id=cls.elon2
        )
        Quant._update_available_quantity(
            cls.elon_musk, cls.location_1, 1.0, lot_id=cls.elon3
        )
        Quant._update_available_quantity(
            cls.metal_cylinder, cls.location_1, 6.0, lot_id=cls.mc1
        )
    ...
    def test_assign_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_charger, self.stock_location, 5
        )
        self.env["stock.quant"]._update_available_quantity(
            product_keybord, self.stock_location, 5
        )
    ...
    def test_flexible_consumption_1b(self):
        ...
        self.bom_submarine.bom_line_ids.filtered(
            lambda line: line.product_id == self.trapped_child
        ).operation_id = self.bom_submarine.operation_ids[0]
        ...
        self.assertEqual(
            wo_form.component_id, self.trapped_child, "The suggested component is wrong"
        )
        self.assertEqual(
            wo_form.qty_done, 24, "The suggested component qty_done is wrong"
        )
        self.assertEqual(
            wo_form.component_remaining_qty, 24, "The remaining quantity is wrong"
        )
        ...
        self.assertEqual(
            wo_form.component_remaining_qty, 12, "The remaining quantity is wrong"
        )
        wo_form.qty_producing = 2
        ...
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
        self.assertEqual(
            wo_form.qty_done, 12, "The suggested component qty_done is wrong"
        )
        self.assertEqual(
            wo_form.component_remaining_qty, 12, "The remaining quantity is wrong"
        )
    ...
    def test_flexible_consumption_1c(self):
        ...
        self.assertEqual(
            wo_form.qty_done, 6, "The suggested component qty_done is wrong"
        )
        wo = wo_form.save()
        wo._next()
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
        self.assertEqual(
            wo_form.qty_done, 2, "The suggested component qty_done is wrong"
        )
        ...
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
        self.assertEqual(
            wo_form.component_id,
            self.metal_cylinder,
            "The suggested component is wrong",
        )
        self.assertEqual(
            wo_form.qty_done, 1, "The suggested component qty_done is wrong"
        )
        self.assertEqual(wo_form.lot_id, self.mc1, "The suggested lot is wrong")
        wo = wo_form.save()
        wo._next()
        ...
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
        self.assertEqual(
            wo_form.finished_lot_id, self.sp1, "The suggested final product is wrong"
        )
        self.assertEqual(
            wo_form.qty_done,
            1,
            "The suggested qty_done should be one as the component is a serial number",
        )
        ...
        wo.button_start()
        self.assertEqual(
            wo.finished_lot_id, self.sp1, "The suggested final product is wrong"
        )
        wo.do_finish()

        # Verify the reserved quantity has been split correctly
        self.assertEqual(
            mo.move_raw_ids.move_line_ids[1].product_uom_qty,
            6.0,
            "The reserved quantity is wrong",
        )
        self.assertEqual(
            mo.move_raw_ids.move_line_ids[2].product_uom_qty,
            6.0,
            "The reserved quantity is wrong",
        )
        self.assertEqual(
            mo.move_raw_ids.move_line_ids[3].product_uom_qty,
            1.0,
            "The reserved quantity is wrong",
        )
        self.assertEqual(
            mo.move_raw_ids.move_line_ids[4].product_uom_qty,
            1.0,
            "The reserved quantity is wrong",
        )
    ...
    def test_flexible_consumption_2(self):
        ...
        mo.button_mark_done()
        move_1 = mo.move_raw_ids.filtered(
            lambda move: move.product_id == self.metal_cylinder and move.state == "done"
        )
        self.assertEqual(
            sum(move_1.mapped("quantity_done")), 1, "Only one cylinder was consumed"
        )
        move_2 = mo.move_raw_ids.filtered(
            lambda move: move.product_id == self.elon_musk and move.state == "done"
        )
        self.assertEqual(
            sum(move_2.mapped("quantity_done")), 2, "2 Elon Musk was consumed"
        )
        move_3 = mo.move_raw_ids.filtered(
            lambda move: move.product_id == self.trapped_child and move.state == "done"
        )
        self.assertEqual(
            sum(move_3.mapped("quantity_done")), 12, "12 child was consumed"
        )
        self.assertEqual(mo.state, "done", 'Final state of the MO should be "done"')
    ...
    def test_workorder_reservation_1(self):
        ...
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
        self.assertEqual(
            wo_form.lot_id, self.elon1, "component lot should be prefilled"
        )
        ...
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
        self.assertEqual(
            wo_form.finished_lot_id, self.sp1, "final lot should be prefilled"
        )
        wo = wo_form.save()
        wo.do_finish()
        production.button_mark_done()

        move_elon = production.move_raw_ids.filtered(
            lambda move: move.product_id == self.elon_musk
        )
        ...
        self.assertEqual(
            len(move_elon.move_line_ids), 1, "their should be 2 move lines"
        )
        self.assertEqual(
            move_elon.move_line_ids.lot_id, self.elon1, "Wrong serial number used"
        )
        move_cylinder = production.move_raw_ids.filtered(
            lambda move: move.product_id == self.metal_cylinder
        )
        self.assertEqual(move_cylinder.state, "done", "Move should be done")
        self.assertEqual(
            move_cylinder.quantity_done, 2, "Consumed quantity should be 4"
        )
        move_child = production.move_raw_ids.filtered(
            lambda move: move.product_id == self.trapped_child
        )
    ...
    def test_workorder_reservation_2(self):
        ...
        self.assertEqual(
            wo_form.qty_done,
            1,
            "component quantity should be 1 as final product is tracked",
        )
        self.assertEqual(wo_form.component_remaining_qty, 1, "It needs 2 component")
        self.assertEqual(
            wo_form.qty_producing,
            1,
            "Quantity to produce should prefilled with 1 (serial tracked product)",
        )
        wo = wo_form.save()
        wo._next()
        wo_form = Form(
        ...
        self.assertEqual(
            wo_form.lot_id, self.mc1, "workorder should consume the second product"
        )
        self.assertEqual(
            wo_form.qty_done, 2, "Quantity to consume should prefilled with 2"
        )
        self.assertEqual(
            wo_form.component_id,
            self.metal_cylinder,
            "workorder should be consume the second product",
        )
        wo = wo_form.save()
        wo._next()
        action = wo.record_production()
        wo_form = Form(
            self.env["mrp.workorder"].browse(action["res_id"]),
            view="mrp_workorder.mrp_workorder_view_form_tablet",
        )
        wo_form.finished_lot_id = self.sp2
        self.assertEqual(
            wo_form.lot_id, self.elon2, "component lot should be prefilled"
        )
        self.assertEqual(
            wo_form.qty_done,
            1,
        ...
        move_elon = productions.move_raw_ids.filtered(
            lambda move: move.product_id == self.elon_musk
        )
        self.assertEqual(move_elon.mapped("state"), ["done"] * 2, "Move should be done")
        self.assertEqual(
            sum(move_elon.mapped("quantity_done")), 2, "Consumed quantity should be 2"
        )
        self.assertEqual(
            len(move_elon.move_line_ids), 2, "their should be 2 move lines"
        )
        self.assertEqual(
            move_elon.move_line_ids.mapped("lot_id"),
            self.elon1 | self.elon2,
            "Wrong serial numbers used",
        )
        move_cylinder = productions.move_raw_ids.filtered(
            lambda move: move.product_id == self.metal_cylinder
        )
        self.assertEqual(
            move_cylinder.mapped("state"), ["done"] * 2, "Move should be done"
        )
        self.assertEqual(
            sum(move_cylinder.mapped("quantity_done")),
            4,
            "Consumed quantity should be 4",
        )
        move_child = productions.move_raw_ids.filtered(
            lambda move: move.product_id == self.trapped_child
        )
        self.assertEqual(
            move_child.mapped("state"), ["done"] * 2, "Move should be done"
        )
        self.assertEqual(
            sum(move_child.mapped("quantity_done")),
            24,
            "Consumed quantity should be 24",
        )
    ...
    def test_workorder_reservation_3(self):
        ...
        self.bom_submarine.bom_line_ids.write(
            {"operation_id": self.bom_submarine.operation_ids.id}
        )
        self.bom_submarine.bom_line_ids.filtered(
            lambda line: line.product_id == self.elon_musk
        ).product_qty = 2
        self.bom_submarine.bom_line_ids.filtered(
            lambda line: line.product_id == self.metal_cylinder
        ).product_qty = 3
        self.bom_submarine.bom_line_ids.filtered(
            lambda line: line.product_id == self.trapped_child
        ).unlink()
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.metal_cylinder, self.location_1, -5.0, lot_id=self.mc1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.metal_cylinder, self.location_1, 2.0, lot_id=self.mc2
        )
        ...
        self.assertEqual(
            production.workorder_ids[0].state,
            "ready",
            "workorder state should be 'ready'",
        )

        production.workorder_ids[0].button_start()
        wo_form = Form(
            production.workorder_ids[0],
            view="mrp_workorder.mrp_workorder_view_form_tablet",
        )
        self.assertEqual(wo_form.qty_producing, 1, "Wrong quantity to produce")
        wo_form.qty_producing = 1
        self.assertEqual(
            wo_form.component_id, self.elon_musk, "The component should be changed"
        )
        ...
        self.assertEqual(
            wo_form.component_id, self.metal_cylinder, "The component should be changed"
        )
        ...
        self.assertEqual(move_cylinder.state, "done", "Move should be done")
        self.assertEqual(
            move_cylinder.quantity_done, 3, "Consumed quantity should be 4"
        )
        self.assertEqual(
            move_cylinder.move_line_ids.mapped("lot_id"),
            self.mc1 | self.mc2,
            "Wrong serial numbers used",
        )
    ...
    def test_workorder_2(self):
        ...
        self.assertEqual(
            sorted_workorder_ids[0].state, "ready", "workorder state should be 'ready'"
        )
        self.assertEqual(
            sorted_workorder_ids[1].state,
            "pending",
            "workorder state should be 'pending'",
        )

        sorted_workorder_ids[0].button_start()
        wo_form = Form(
            sorted_workorder_ids[0], view="mrp_workorder.mrp_workorder_view_form_tablet"
        )
        self.assertEqual(wo_form.finished_lot_id.id, False, "final lot should be empty")
        self.assertEqual(
            wo_form.qty_producing, 1, "Wrong quantity to produce (serial tracked)"
        )
        ...
        sorted_workorder_ids_2 = production.procurement_group_id.mrp_production_ids[
            1
        ].workorder_ids.sorted()
        sorted_workorder_ids_3 = production.procurement_group_id.mrp_production_ids[
            2
        ].workorder_ids.sorted()
        sorted_workorder_ids[1].button_start()
        wo_form = Form(
            sorted_workorder_ids[1], view="mrp_workorder.mrp_workorder_view_form_tablet"
        )
        self.assertEqual(
            wo_form.finished_lot_id, self.sp1, "final lot should be prefilled"
        )
        ...
        wo_form = Form(
            sorted_workorder_ids_2[1],
            view="mrp_workorder.mrp_workorder_view_form_tablet",
        )
        self.assertEqual(
            wo_form.finished_lot_id, self.sp2, "final lot should be prefilled"
        )
        ...
        wo_form = Form(
            sorted_workorder_ids_3[1],
            view="mrp_workorder.mrp_workorder_view_form_tablet",
        )
        self.assertEqual(
            wo_form.finished_lot_id, self.sp3, "final lot should be prefilled"
        )
        ...
        wo_form = Form(
            sorted_workorder_ids[2], view="mrp_workorder.mrp_workorder_view_form_tablet"
        )
        self.assertEqual(
            wo_form.finished_lot_id, self.sp1, "final lot should be prefilled"
        )
        ...
        self.assertEqual(
            sum(move_elon.mapped("quantity_done")), 3, "Consumed quantity should be 2"
        )
        self.assertEqual(
            len(move_elon.move_line_ids), 3, "their should be 2 move lines"
        )
        self.assertEqual(
            move_elon.move_line_ids.mapped("lot_id"),
            self.elon1 | self.elon2 | self.elon3,
            "Wrong serial numbers used",
        )
        self.assertEqual(
            move_elon.move_line_ids.produce_line_ids.lot_id,
            self.sp1 | self.sp2 | self.sp3,
            "Wrong produced serial numbers",
        )
        move_cylinder = productions.move_raw_ids.filtered(
            lambda move: move.product_id == self.metal_cylinder
        )
        self.assertEqual(
            move_cylinder.mapped("state"), ["done"] * 3, "Move should be done"
        )
        self.assertEqual(
            sum(move_cylinder.mapped("quantity_done")),
            6,
        ...
        self.assertEqual(
            move_cylinder.move_line_ids.produce_line_ids.lot_id,
            self.sp1 | self.sp2 | self.sp3,
            "Wrong produced serial numbers",
        )
        move_child = productions.move_raw_ids.filtered(
        ...
        self.assertEqual(
            move_child.mapped("state"), ["done"] * 3, "Move should be done"
        )
        self.assertEqual(
            sum(move_child.mapped("quantity_done")),
            36,
        ...
        self.assertEqual(
            len(move_child.move_line_ids),
            3,
            "Their should be 3 move line as production was made in 3 steps",
        )
        self.assertEqual(
            move_child.move_line_ids.produce_line_ids.lot_id,
            self.sp1 | self.sp2 | self.sp3,
            "Wrong produced serial numbers",
        )
    ...
    def test_workorder_3(self):
        ...
        self.assertEqual(
            len(move_child.move_line_ids),
            2,
            "Their should be 2 move lines as their are two production",
        )
        move_byproduct_angry_british_diver = productions.move_finished_ids.filtered(
            lambda move: move.product_id == self.angry_british_diver
        )
        self.assertEqual(
            move_byproduct_angry_british_diver.mapped("state"),
            ["done"] * 2,
            "Move should be done",
        )
        self.assertEqual(
            sum(move_byproduct_angry_british_diver.mapped("quantity_done")),
            2,
            "Consumed quantity should be 2",
        )
        self.assertEqual(
            len(move_byproduct_angry_british_diver.move_line_ids),
            2,
            "Their should be 2 move lines",
        )
        self.assertEqual(
            move_byproduct_angry_british_diver.move_line_ids.mapped("lot_id"),
            self.abd_1 | self.abd_2,
            "Wrong serial numbers used",
        )
        move_byproduct_advertising = productions.move_finished_ids.filtered(
            lambda move: move.product_id == self.advertising
        )
        self.assertEqual(
            move_byproduct_advertising.mapped("state"),
            ["done"] * 2,
            "Move should be done",
        )
        self.assertEqual(
            sum(move_byproduct_advertising.mapped("quantity_done")),
            4,
            "Consumed quantity should be 2",
        )
        self.assertEqual(
            len(move_byproduct_advertising.move_line_ids),
            2,
            "Their should be 2 move lines",
        )
        self.assertEqual(
            move_byproduct_advertising.move_line_ids.mapped("lot_id"),
            self.advertise_1 | self.advertise_2,
            "Wrong serial numbers used",
        )
    ...
    def test_workorder_4(self):
        ...
        self.assertEqual(
            wo_form.finished_lot_id,
            lot1,
            "Final lot should be the one entered in previous wo",
        )
        wo_form.finished_lot_id = lot1
        wo_form.qty_producing = 2
        wo = wo_form.save()
    ...
    def test_workorder_5(self):
        ...
        self.assertEqual(
            wo_form.component_id, self.elon_musk, "Should be a register component check"
        )
        ...
        self.assertEqual(
            wo_form.component_id,
            self.metal_cylinder,
            "Should be a register component check",
        )
    ...
    def test_post_inventory(self):
        ...
        done_raw_moves = production.move_raw_ids.filtered(
            lambda move: move.state == "done"
        )
        self.assertEqual(len(done_raw_moves), 3, "wrong number of done raw moves")
        self.assertEqual(
            done_raw_moves[0].quantity_done, 1, "Components are not consumed"
        )
        self.assertEqual(
            done_raw_moves[1].quantity_done, 12, "Components are not consumed"
        )
        self.assertEqual(
            done_raw_moves[2].quantity_done, 2, "Components are not consumed"
        )
        ...
        assigned_raw_moves = backorder.move_raw_ids.filtered(
            lambda move: move.state == "assigned"
        )
        self.assertEqual(
            len(assigned_raw_moves), 3, "wrong number of reserved raw moves"
        )

        done_finished_move = production.move_finished_ids.filtered(
            lambda move: move.state == "done"
        )
        self.assertEqual(
            len(done_finished_move), 1, "wrong number of done finished moves"
        )
        self.assertEqual(
            done_finished_move.quantity_done, 1, "finished product are not produced"
        )
        ...
        done_raw_moves = (production | backorder).move_raw_ids.filtered(
            lambda move: move.state == "done"
        )
        self.assertEqual(len(done_raw_moves), 6, "wrong number of done raw moves")
        drm_elon = done_raw_moves.filtered(
            lambda move: move.product_id == self.elon_musk
        )
        drm_metal = done_raw_moves.filtered(
            lambda move: move.product_id == self.metal_cylinder
        )
        drm_child = done_raw_moves.filtered(
            lambda move: move.product_id == self.trapped_child
        )
        ...
        done_finished_move = (production | backorder).move_finished_ids.filtered(
            lambda move: move.state == "done"
        )
        self.assertEqual(
            len(done_finished_move), 2, "wrong number of done finished moves"
        )
        self.assertEqual(
            done_finished_move[0].quantity_done, 1, "finished product are not produced"
        )
        self.assertEqual(
            done_finished_move[1].quantity_done, 1, "finished product are not produced"
        )
    ...
    def test_add_component(self):
        ...
        additional_form = Form(
            self.env["mrp_workorder.additional.product"].with_context(
                {
    ...
    def test_add_component_2(self):
        ...
        table = self.env["product.product"].create(
            {"name": "Table", "type": "product", "tracking": "none"}
        )

        table_leg = self.env["product.product"].create(
            {"name": "Table Leg", "type": "product", "tracking": "none"}
        )
    ...
    def test_produce_more_than_planned(self):
        ...
        wo_form.finished_lot_id = self.sp1
        self.assertEqual(
            wo_form.qty_done, 60, "The suggested component qty_done is wrong"
        )
        self.assertEqual(
            wo_form.component_remaining_qty,
            60,
            "The component remaining quantity is wrong",
        )
        wo = wo_form.save()
        wo._next()
        wo_form = Form(wo, view="mrp_workorder.mrp_workorder_view_form_tablet")
    ...
    def test_suggested_lot_in_multi_step(self):
        ...
        self.submarine_pod.tracking = "none"
        self.bom_submarine.bom_line_ids.filtered(
            lambda l: l.product_id.id != self.elon_musk.id
        ).unlink()
        ...
        wo.button_start()
        self.assertEqual(
            production.move_raw_ids.move_line_ids.lot_id,
            self.elon2,
    ...
    def test_step_by_product_variant(self):
        ...
        a1 = self.env["product.attribute.value"].create(
            {"name": "V0hFCg==", "attribute_id": who_attr.id}
        )
        a2 = self.env["product.attribute.value"].create(
            {"name": "QVJN", "attribute_id": who_attr.id}
        )
        a3 = self.env["product.attribute.value"].create(
            {"name": "UllW", "attribute_id": who_attr.id}
        )
        ...
        product_a1 = product_who.product_variant_ids.filtered(
            lambda v: a1
            in v.product_template_attribute_value_ids.product_attribute_value_id
        )
        product_a2 = product_who.product_variant_ids.filtered(
            lambda v: a2
            in v.product_template_attribute_value_ids.product_attribute_value_id
        )
        product_a3 = product_who.product_variant_ids.filtered(
            lambda v: a3
            in v.product_template_attribute_value_ids.product_attribute_value_id
        )
