...
class TestDuplicateProducts(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.painted_boat = cls.env["product.product"].create(
            {"name": "Painted boat", "type": "product", "tracking": "serial"}
        )
        ...
        cls.blank_boat = cls.env["product.product"].create(
            {"name": "Blank Boat", "type": "product", "tracking": "serial"}
        )
        ...
        cls.painting = cls.env["product.product"].create(
            {"name": "Color Painting", "type": "product", "tracking": "lot"}
        )
        ...
        cls.bom_boat = cls.env["mrp.bom"].create(
            {"product_tmpl_id": cls.painted_boat.product_tmpl_id.id, "product_qty": 1.0}
        )
        ...
        Quant._update_available_quantity(
            cls.blank_boat, cls.location_1, 1.0, lot_id=cls.bb1
        )
        Quant._update_available_quantity(
            cls.painting, cls.location_1, 10.0, lot_id=cls.p1
        )
    ...
    def test_duplicate_without_point(self):
        ...
        self.assertEqual(
            production.workorder_ids.state, "ready", "workorder state should be 'ready'"
        )
        self.assertEqual(
            len(production.workorder_ids.check_ids),
            3,
            "Same components are not merged, should be 3 quality checks",
        )
        painting_checks = production.workorder_ids.check_ids.filtered(
            lambda check: check.component_id == self.painting
        )
        self.assertEqual(
            len(painting_checks), 2, "should be 2 quality checks for painting"
        )
    ...
    def test_assignation_1(self):
        ...
        self.assertEqual(
            len(production.workorder_ids.check_ids),
            3,
            "Same components merged, should be 3 quality checks",
        )
        painting_checks = production.workorder_ids.check_ids.filtered(
            lambda check: check.component_id == self.painting
        )
    ...
    def test_byproduct_1(self):
        ...
        move_paint_raw = production.move_raw_ids.filtered(
            lambda move: move.product_id == self.painting
        )
        self.assertEqual(
            len(move_paint_raw),
            2,
            "there should be 2 moves after merge same components",
        )
        self.assertEqual(
            move_paint_raw.mapped("state"), ["done", "done"], "Moves should be done"
        )
        self.assertEqual(
            move_paint_raw.mapped("quantity_done"),
            [1, 1],
            "Consumed quantity should be 2",
        )
        self.assertEqual(
            len(move_paint_raw.move_line_ids), 2, "their should be 2 move lines"
        )
        self.assertEqual(
            move_paint_raw.mapped("move_line_ids").mapped("lot_id"),
            self.p1,
            "Wrong lot numbers used",
        )
        move_paint_finished = production.move_finished_ids.filtered(
            lambda move: move.product_id == self.painting
        )
        self.assertEqual(move_paint_finished.state, "done", "Move should be done")
        self.assertEqual(
            move_paint_finished.quantity_done, 1, "Consumed quantity should be 1"
        )
        self.assertEqual(
            len(move_paint_finished.move_line_ids), 1, "their should be 1 move line"
        )
        self.assertEqual(
            move_paint_finished.move_line_ids.lot_id, self.p2, "Wrong lot numbers used"
        )
