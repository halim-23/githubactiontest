...
class MrpWorkorderAdditionalProduct(models.TransientModel):
    ...
    product_uom_id = fields.Many2one(
        "uom.uom", domain="[('category_id', '=', product_uom_category_id)]"
    )
    product_uom_category_id = fields.Many2one(related="product_id.uom_id.category_id")
    type = fields.Selection([("component", "Component"), ("byproduct", "By-Product")])
    ...
    def add_product(self):
        ...
        if wo.current_quality_check_id:
            ...
            last_check = wo.check_ids.filtered(
                lambda c: not c.next_check_id
                and c.finished_product_sequence == wo.qty_produced
                and c != additional_check
            )
