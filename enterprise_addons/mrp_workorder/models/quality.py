...
class MrpRouting(models.Model):
    ...
    def _compute_quality_point_count(self):
        ...
        data = {
            res["operation_id"][0]: res["operation_id_count"]
    ...
    def action_mrp_workorder_show_steps(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "mrp_workorder.action_mrp_workorder_show_steps"
        )
        ctx = {
...
class QualityPoint(models.Model):
    ...
    worksheet = fields.Selection(
        [("noupdate", "Do not update page"), ("scroll", "Scroll to specific page")],
        string="Worksheet",
        default="noupdate",
    )
    ...
    def _compute_is_workorder_step(self):
        for quality_point in self:
            quality_point.is_workorder_step = (
                quality_point.operation_id
                or quality_point.picking_type_ids
                and all(
...
class QualityCheck(models.Model):
    ...
    def _compute_title(self):
        ...
        for check in self:
            if not check.point_id or check.component_id:
                check.title = '{} "{}"'.format(
                    check.test_type_id.display_name, check.component_id.name
                )
    ...
    def _compute_result(self):
        for check in self:
            ...
            check.quality_state_for_summary = (
                _("Done") if state != "none" else _("To Do")
            )
