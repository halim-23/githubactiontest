    ...
    def action_work_order(self):
        if not self.env.context.get("desktop_list_view", False):
            action = self.env["ir.actions.actions"]._for_xml_id(
                "mrp_workorder.mrp_workorder_action_tablet"
            )
            return action
        else:
...
class MrpProductionWorkcenterLine(models.Model):
    ...
    skipped_check_ids = fields.One2many(
        "quality.check", "workorder_id", domain=[("quality_state", "=", "none")]
    )
    ...
    component_id = fields.Many2one(
        "product.product", related="current_quality_check_id.component_id"
    )
    ...
    component_uom_id = fields.Many2one(
        "uom.uom", compute="_compute_component_data", string="Component UoM"
    )
    control_date = fields.Datetime(
        related="current_quality_check_id.control_date", readonly=False
    )
        ...
        for wo in self.filtered(lambda w: w.state not in ("done", "cancel")):
            if wo.test_type in ("register_byproducts", "register_consumed_materials"):
                if wo.quality_state == "none":
                    completed_lines = (
                        wo.move_line_ids.filtered(lambda l: l.lot_id)
                        if wo.component_id.tracking != "none"
                        else wo.move_line_ids
                    )
                    if not self.move_id.additional:
                        wo.component_remaining_qty = self._prepare_component_quantity(
                            wo.move_id, wo.qty_producing
                        ) - sum(completed_lines.mapped("qty_done"))
                    else:
                        wo.component_remaining_qty = self._prepare_component_quantity(
                            wo.move_id, wo.qty_remaining
                        ) - sum(completed_lines.mapped("qty_done"))
    ...
    def _next(self, continue_production=False):
        ...
        if float_compare(self.qty_producing, 0, precision_rounding=rounding) <= 0:
            raise UserError(
                _("Please ensure the quantity to produce is greater than 0.")
            )
            ...
            self.component_remaining_qty -= float_round(
                self.qty_done,
            ...
            if self.move_line_id:
                ...
                if (
                    float_compare(
                        self.qty_done,
                        self.move_line_id.product_uom_qty,
                    ...
                    self.move_line_id.with_context(
    ...
    def _change_quality_check(self, position, skipped=False):
        ...
        checks_to_consider = self.check_ids.filtered(
            lambda c: c.finished_product_sequence == self.qty_produced
        )
        if position == "first":
        ...
        self.write(
            {
                "allow_producing_quantity_change": not check.previous_check_id
                and all(c.quality_state == "none" for c in checks_to_consider)
                ...
                if change_worksheet_page
                else self.worksheet_page,
            }
        )
    ...
    def action_menu(self):
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "mrp_workorder.mrp_workorder_view_form_tablet_menu"
                    ).id,
                    "form",
                ]
            ],
    ...
    def action_add_component(self):
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "mrp_workorder.view_mrp_workorder_additional_product_wizard"
                    ).id,
                    "form",
                ]
            ],
    ...
    def _create_checks(self):
        for wo in self:
            ...
            ).filtered(lambda move: move.has_tracking != "none" or move.operation_id)
            quality_team_id = (
                self.env["quality.alert.team"]
                .search(
                    [
                        "|",
                        ("company_id", "=", wo.company_id.id),
    ...
    def record_production(self):
        ...
        if (
            ...
            and self.product_tracking != "none"
        ...
        if (
            ...
            backorder = self.production_id._generate_backorder_productions(
                close_mo=False
            )
    ...
    def _create_extra_move_lines(self):
        ...
        for quant in quants:
            ...
            if (
                float_compare(
                    self.qty_done, 0, precision_rounding=self.product_id.uom_id.rounding
                )
                <= 0
            ):
                break
    ...
    def _check_remaining_quality_checks(self):
        checks_not_process = self.check_ids.filtered(
            lambda c: c.quality_state == "none"
            and c.test_type
            not in ("register_consumed_materials", "register_byproducts")
        )
    ...
    def open_tablet_view(self):
        self.ensure_one()
        if (
            not self.is_user_working
            and self.working_state != "blocked"
            and self.state in ("ready", "waiting", "progress", "pending")
        ):
            self.button_start()
        return {
            "type": "ir.actions.act_window",
                ...
                [
                    ...
                    [
                        (
                            "production_id",
                            "in",
                            self.production_id.procurement_group_id.mrp_production_ids.ids,
                        )
                    ],
                ]
            )
    ...
    def on_barcode_scanned(self, barcode):
        ...
        if self.component_tracking:
            lot = self.env["stock.production.lot"].search(
                [("name", "=", barcode), ("product_id", "=", self.component_id.id)]
            )
            if not lot:
                # create a new lot
                ...
                lot = (
                    ...
                    .with_context(active_mo_id=self.production_id.id)
                    .create(
                        {
                            "name": barcode,
            ...
            self.production_id.product_id.tracking
    ...
    def _update_component_quantity(self):
        if self.component_tracking == "serial":
            self.qty_done = self.product_id.uom_id._compute_quantity(
