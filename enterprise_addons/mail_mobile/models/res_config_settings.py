...
class ResConfigSettings(models.TransientModel):
    ...
    enable_ocn = fields.Boolean(
        "Push Notifications", config_parameter="mail_mobile.enable_ocn"
    )
    ...
    def _get_endpoint(self):
        return (
            ...
            .sudo()
            .get_param("odoo_ocn.endpoint", DEFAULT_ENDPOINT)
        )
    ...
    def get_fcm_project_id(self):
        ...
        if not project_id:
            ...
            try:
                ...
                project_id = iap_tools.iap_jsonrpc(
                    self._get_endpoint() + "/iap/ocn/enable_service", params=params
                )
                # Storing project id for generate token
                ir_params_sudo.set_param("odoo_ocn.project_id", project_id)
            except Exception as e:
                _logger.error(
                    "An error occurred while contacting the ocn server: %s", e.args[0]
                )
        return project_id
    ...
    def register_device(self, device_key, device_name):
        ...
        try:
            result = iap_tools.iap_jsonrpc(
                self._get_endpoint() + "/iap/ocn/register_device", params=values
            )
        except Exception as e:
            _logger.error(
                "An error occurred while contacting the ocn server: %s", e.args[0]
