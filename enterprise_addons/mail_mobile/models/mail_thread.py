...
class MailThread(models.AbstractModel):
    ...
    def _notify_thread(self, message, msg_vals=False, notify_by_email=True, **kwargs):
        rdata = super(MailThread, self)._notify_thread(
            message, msg_vals, notify_by_email, **kwargs
        )
    ...
    def _notify_record_by_ocn(self, message, rdata, msg_vals=False, **kwargs):
        ...
        if not icp_sudo.get_param("odoo_ocn.project_id") or not icp_sudo.get_param(
            "mail_mobile.enable_ocn"
        ):
            return
    ...
    def _notify_by_ocn_send(self, message, partner_ids, msg_vals=False):
        ...
        receiver_ids = (
            ...
            .sudo()
            .search([("id", "in", partner_ids), ("ocn_token", "!=", False)])
        )
        if receiver_ids:
            endpoint = self.env["res.config.settings"]._get_endpoint()
            payload = self._notify_by_ocn_send_prepare_payload(
            ...
            at_mention_analyser_id_list = self._at_mention_analyser(
            ...
            for chunk in chunks:
                try:
                    ...
                    _logger.error(
                        "An error occurred while contacting the ocn server: %s", e
                    )

    def _notify_by_ocn_send_prepare_payload(
        self, message, receiver_ids, msg_vals=False
    ):
    ...
    def _notify_get_action_link(self, link_type, **kwargs):
        original_link = super(MailThread, self)._notify_get_action_link(
            link_type, **kwargs
        )
