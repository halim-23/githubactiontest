...
class TestAccountAsset(TestAccountReportsCommon):
    ...
    def test_00_account_asset(self):
        ...
        with self.assertRaises(
            UserError,
            msg="You shouldn't be able to close if there are posted entries in the future",
        ):
    ...
    def test_01_account_asset(self):
        ...
        """
        account_asset_model_sale_test0 = (
            self.env["account.asset"]
            .with_context(asset_type="purchase")
            .create(
                {
        ...
        self.company_data[
            "default_account_assets"
        ].asset_model = account_asset_model_sale_test0
        ...
        self.assertEqual(
            len(recognition),
            1,
            "One and only one recognition should have been created from invoice.",
        )

        self.assertTrue(
            recognition.state == "open", "Recognition should be in Open state"
        )
        first_invoice_line = invoice.invoice_line_ids[0]
        self.assertEqual(
            recognition.original_value,
        ...
        first_installment_line = recognition.depreciation_move_ids.sorted(
            lambda r: r.id
        )[0]
        self.assertAlmostEqual(
            first_installment_line.asset_remaining_value,
            recognition.original_value - first_installment_line.amount_total,
            msg="Remaining value is incorrect.",
        )
        self.assertAlmostEqual(
            first_installment_line.asset_depreciated_value,
            first_installment_line.amount_total,
            msg="Depreciated value is incorrect.",
        )

        # I check next installment date.
        last_installment_date = first_installment_line.date
        installment_date = last_installment_date + relativedelta(
            months=+int(recognition.method_period)
        )
        self.assertEqual(
            recognition.depreciation_move_ids.sorted(lambda r: r.id)[1].date,
            installment_date,
    ...
    def test_asset_from_move_line_form(self):
        ...
        asset = self.env["account.asset"].new(
            {"original_move_line_ids": [(6, 0, move_line_ids.ids)]}
        )
        asset_form = Form(
    ...
    def test_asset_modify_depreciation(self):
        ...
        self.assertEqual(
            10,
            len(
                self.truck.depreciation_move_ids.filtered(lambda x: x.state == "draft")
            ),
        )
    ...
    def test_asset_modify_report(self):
        ...
        self.assertEqual(
            "10 y", lines[0]["columns"][3]["name"], "Depreciation Rate = 10%"
        )
        ...
        self.assertListEqual(
            [10000.0, 1500.0, 0.0, 11500.0, 4500.0, 1000.0, 0.0, 5500.0, 6000.0],
            [x["no_format_name"] for x in lines[0]["columns"][4:]],
        )
        ...
        self.assertListEqual(
            [10000.0, 1500.0, 0.0, 11500.0, 4500.0, 3250.0, 0.0, 7750.0, 3750.0],
            [x["no_format_name"] for x in lines[0]["columns"][4:]],
        )
    ...
    def test_asset_reverse_depreciation(self):
        ...
        max_date_posted_before = max(
            self.truck.depreciation_move_ids.filtered(
                lambda m: m.state == "posted" and m.date < move_to_reverse.date
            ),
            key=lambda m: m.date,
        )
        self.assertEqual(
            move_to_reverse.asset_remaining_value,
            max_date_posted_before.asset_remaining_value,
        )
        self.assertEqual(
            move_to_reverse.asset_depreciated_value,
            max_date_posted_before.asset_depreciated_value,
        )

        # Check that the depreciation has been reported on the next move
        min_date_draft = min(
            self.truck.depreciation_move_ids.filtered(
                lambda m: m.state == "draft" and m.date > move_to_reverse.date
            ),
            key=lambda m: m.date,
        )
        self.assertEqual(
            move_to_reverse.asset_remaining_value - min_date_draft.amount_total,
            min_date_draft.asset_remaining_value,
        )
        self.assertEqual(
            move_to_reverse.asset_depreciated_value + min_date_draft.amount_total,
    ...
    def test_asset_credit_note(self):
        ...
        self.company_data[
            "default_account_assets"
        ].asset_model = account_asset_model_fixedassets_test0
        ...
        depreciation_lines = self.env["account.move.line"].search(
            [
                (
                    "account_id",
                    "=",
                    account_asset_model_fixedassets_test0.account_depreciation_id.id,
                ),
        ...
        asset = self.env["account.asset"].create(
            {
                ...
                - relativedelta(years=1, month=6, day=1),
                "original_value": 10000,
                "method_number": 5,
                "method_period": "1",
                "method": "linear",
