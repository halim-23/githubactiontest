...
class AssetSell(models.TransientModel):
    ...
    action = fields.Selection(
        [("sell", "Sell"), ("dispose", "Dispose")], required=True, default="sell"
    )
    ...
    def _compute_select_invoice_line_id(self):
        for record in self:
            record.select_invoice_line_id = (
                record.action == "sell" and len(record.invoice_id.invoice_line_ids) > 1
            )
    ...
    def _compute_gain_or_loss(self):
        for record in self:
            line = (
                ...
                or len(record.invoice_id.invoice_line_ids) == 1
                and record.invoice_id.invoice_line_ids
    ...
    def do_action(self):
        ...
        invoice_line = (
            ...
            else self.invoice_line_id or self.invoice_id.invoice_line_ids
        )
        return self.asset_id.set_to_close(
            invoice_line_id=invoice_line, date=invoice_line.move_id.invoice_date
        )
