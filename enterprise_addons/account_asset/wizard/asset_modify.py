...
class AssetModify(models.TransientModel):
    ...
    value_residual = fields.Monetary(
        string="Depreciable Amount", help="New residual amount for the asset"
    )
    salvage_value = fields.Monetary(
        string="Not Depreciable Amount", help="New salvage amount for the asset"
    )
    ...
    account_asset_id = fields.Many2one(
        "account.account", string="Asset Gross Increase Account"
    )
                    ...
                    "original_move_line_ids": [
                        (
                            ...
                            move.line_ids.filtered(
                                lambda r: r.account_id == self.account_asset_id
                            ).ids,
                        )
                    ],
                }
    ...
    def _compute_need_date(self):
        for record in self:
            ...
            record.need_date = (
                self.env.context.get("resume_after_pause") and record.asset_id.prorata
            ) or value_changed
