...
class AccountMove(models.Model):
    ...
    asset_depreciated_value = fields.Monetary(
        string="Cumulative Depreciation", copy=False
    )
    ...
    asset_ids = fields.One2many(
        "account.asset", string="Assets", compute="_compute_asset_ids"
    )
    asset_ids_display_name = fields.Char(
        compute="_compute_asset_ids"
    )  # just a button label. That's to avoid a plethora of different buttons defined in xml
                        ...
                        self._prepare_move_for_asset_depreciation(
                            {
                                ...
                                "move_ref": _("Report of reversal for {name}").format(
                                    name=move.asset_id.name
                                ),
                                ...
                                "asset_depreciated_value": move.amount_total
                                + max(
                                    move.asset_id.depreciation_move_ids.mapped(
        ...
        for move in self:
            ...
            for move_line in move.line_ids.filtered(
                lambda line: not (
                    ...
                    and line.account_id.user_type_id.internal_group == "asset"
                )
            ):
                if (
    ...
    def _prepare_move_for_asset_depreciation(self, vals):
        ...
        if missing_fields:
            raise UserError(
                _("Some fields are missing {}").format(", ".join(missing_fields))
            )
        asset = vals["asset_id"]
        ...
        if asset.original_move_line_ids and asset.original_move_line_ids[
            0
        ...
        move_line_2 = {
            ...
            "analytic_account_id": account_analytic_id.id
            if asset.asset_type in ("purchase", "expense")
            else False,
            "analytic_tag_ids": [(6, 0, analytic_tag_ids.ids)]
            if asset.asset_type in ("purchase", "expense")
            else False,
    ...
    def _get_depreciation(self):
        ...
        if asset:
            account = (
                ...
                if asset.asset_type == "sale"
                else asset.account_depreciation_id
            )
            ...
            if any(
                (line.account_id, line[field])
                == (asset.account_asset_id, asset.original_value)
        ...
        for record in self:
            ...
            if record.number_asset_ids:
                ...
                record.asset_ids_display_name = "%s %s" % (
                    len(record.asset_ids),
                    asset_type.get(record.asset_ids[0].asset_type),
                )
            else:
                record.asset_ids_display_name = ""
            record.asset_id_display_name = {
                "sale": _("Revenue"),
                "purchase": _("Asset"),
                "expense": _("Expense"),
            }.get(record.asset_id.asset_type)
            record.draft_asset_ids = bool(
                record.asset_ids.filtered(lambda x: x.state == "draft")
            )
    ...
    def open_asset_view(self):
        ret = {
            ...
            "view_id": [
                v[0]
                for v in self.env["account.asset"]._get_views(self.asset_asset_type)
                if v[1] == "form"
            ][0],
    ...
    def _delete_reversed_entry_assets(self):
        ...
        for move in self.filtered(lambda m: m.reversed_entry_id):
            ...
            for line in move.reversed_entry_id.line_ids.filtered(
                lambda l: (
                    l.asset_ids
            ...
            for line in move.reversed_entry_id.line_ids.filtered(
                ...
                for asset in line.asset_ids:
                    if rp_count[(line.product_id.id, line.price_unit)] > 0 and (
                        asset.state == "draft"
                        or all(
                            state == "draft"
                            for state in asset.depreciation_move_ids.mapped("state")
    ...
    def _close_assets(self):
        for asset in self.asset_id:
            if asset.asset_type in ("expense", "sale") and all(
                m.state == "posted" for m in asset.depreciation_move_ids
            ):
...
class AccountMoveLine(models.Model):
    ...
    asset_ids = fields.Many2many(
        ...
        "asset_id",
        string="Asset Linked",
        help="Asset created from this Journal Item",
        copy=False,
    )
    ...
    def _turn_as_asset(self, asset_type, view_name, view):
        ...
        ctx.update(
            {
                "default_original_move_line_ids": [
                    (6, False, self.env.context["active_ids"])
                ],
    ...
    def turn_as_asset(self):
        return self._turn_as_asset(
            "purchase",
            _("Turn as an asset"),
            self.env.ref("account_asset.view_account_asset_form"),
        )
    ...
    def turn_as_deferred(self):
        ...
        if balance > 0:
            return self._turn_as_asset(
                "expense",
                _("Turn as a deferred expense"),
                self.env.ref("account_asset.view_account_asset_expense_form"),
            )
        else:
            return self._turn_as_asset(
                "sale",
                _("Turn as a deferred revenue"),
                self.env.ref("account_asset.view_account_asset_revenue_form"),
            )
