...
class AccountAsset(models.Model):
    ...
    depreciation_entries_count = fields.Integer(
        compute="_entry_count", string="# Posted Depreciation Entries"
    )
    ...
    prorata_date = fields.Date(
        string="Prorata Date", readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    gross_increase_value = fields.Monetary(
        string="Gross Increase Value", compute="_compute_book_value", compute_sudo=True
    )
    ...
    first_depreciation_date = fields.Date(
        ...
        compute="_compute_first_depreciation_date",
        store=True,
        readonly=False,
    ...
    user_type_id = fields.Many2one(
        ...
        related="account_asset_id.user_type_id",
    ...
    def _compute_disposal_date(self):
        for asset in self:
            if asset.state == "close":
                dates = asset.depreciation_move_ids.filtered(lambda m: m.date).mapped(
                    "date"
                )
    ...
    @api.depends(
        "original_move_line_ids", "original_move_line_ids.account_id", "asset_type"
    )
    def _compute_value(self):
        for record in self:
            ...
            if not record.original_move_line_ids:
                ...
                continue
            if any(
                line.move_id.state == "draft" for line in record.original_move_line_ids
            ):
                raise UserError(_("All the lines should be posted"))
            if any(
                type != record.original_move_line_ids[0].move_id.move_type
                for type in record.original_move_line_ids.mapped("move_id.move_type")
            ):
            ...
            if (
                record.account_asset_id.multiple_assets_per_line
                and len(record.original_move_line_ids) == 1
            ):
                record.original_value /= max(
                    1, int(record.original_move_line_ids.quantity)
                )
    ...
    def _compute_display_model_choice(self):
        for record in self:
            ...
            record.display_model_choice = record.state == "draft" and bool(
                self.env["account.asset"].search(domain, limit=1)
            )
    ...
    def _onchange_account_depreciation_expense_id(self):
        if not self.original_move_line_ids and self.asset_type not in (
            "purchase",
            "expense",
        ):
    ...
    def _compute_acquisition_date(self):
        for record in self:
            record.acquisition_date = record.acquisition_date or min(
                record.original_move_line_ids.mapped("date")
                + [
                    record.prorata
                    and record.prorata_date
                    or record.first_depreciation_date
    ...
    def _onchange_depreciation_move_ids(self):
        ...
        asset_remaining_value = (
            self.original_value - self.already_depreciated_amount_import
        )
    ...
    def _onchange_account_asset_id(self):
        self.display_model_choice = self.state == "draft" and self.env[
            "account.asset"
        ].search_count(
        ...
        if self.asset_type in ("purchase", "expense"):
            ...
            self.account_depreciation_expense_id = (
                self.account_depreciation_expense_id or self.account_asset_id
            )
    ...
    @api.depends(
        "acquisition_date", "original_move_line_ids", "method_period", "company_id"
    )
    def _compute_first_depreciation_date(self):
        for record in self:
            pre_depreciation_date = (
                record.acquisition_date
                or min(
                    record.original_move_line_ids.mapped("date"),
                    default=record.acquisition_date,
                )
                or fields.Date.today()
            )
            ...
            if record.method_period == "12":
                ...
                depreciation_date = depreciation_date + relativedelta(
                    day=record.company_id.fiscalyear_last_day
                )
    ...
    def unlink(self):
        for asset in self:
            for line in asset.original_move_line_ids:
                body = _("A document linked to %s has been deleted: ") % (
                    line.name or _("this move")
                )
                body += "<a href=# data-oe-model=account.asset data-oe-id=%d>%s</a>" % (
                    asset.id,
                    asset.name,
                )
        ...
        if computation_sequence == max_depreciation_nb:
            ...
            if self.method in ("linear", "degressive_then_linear"):
                ...
                if self.prorata:
                    nb_depreciation -= 1
                linear_amount = min(
                    total_amount_to_depr / nb_depreciation, residual_amount
                )
    ...
    def compute_depreciation_board(self):
        ...
        if posted_depreciation_move_ids and posted_depreciation_move_ids[-1].date:
            ...
            if (
                last_depreciation_date > depreciation_date
            ):  # in case we unpause the asset
                depreciation_date = last_depreciation_date + relativedelta(
                    months=+int(self.method_period)
                )
        commands = [
            (2, line_id.id, False)
            for line_id in self.depreciation_move_ids.filtered(
                lambda x: x.state == "draft"
            )
        ]
        newlines = self._recompute_board(
            depreciation_number,
            starting_sequence,
    ...
    def _recompute_board(
        ...
        if amount_to_depreciate != 0.0:
            for asset_sequence in range(starting_sequence + 1, depreciation_number + 1):
                while (
                    amount_change_ids and amount_change_ids[0].date <= depreciation_date
                ):
                    ...
                    amount_change_ids[0].write(
                        {
                            ...
                            "asset_depreciated_value": amount_to_depreciate
                            - residual_amount
                            + already_depreciated_amount,
                        }
                    )
                ...
                if prorata and asset_sequence == 1:
                    ...
                    if int(self.method_period) % 12 != 0:
                        ...
                        days = (
                            self.company_id.compute_fiscalyear_dates(first_date)[
                                "date_to"
                            ]
                            - first_date
                        ).days + 1
    ...
    def _get_disposal_moves(self, invoice_line_ids, disposal_date):
        ...
        for asset, invoice_line_id in zip(self, invoice_line_ids):
            ...
            commands = [
                (2, line_id.id, False) for line_id in unposted_depreciation_move_ids
            ]

            # Create a new depr. line with the residual amount and post it
            asset_sequence = (
                len(asset.depreciation_move_ids)
                - len(unposted_depreciation_move_ids)
                + 1
            )
            ...
            depreciation_moves = asset.depreciation_move_ids.filtered(
                ...
                and not (r.reversal_move_id and r.reversal_move_id[0].state == "posted")
            )
            ...
            asset.write(
                {"depreciation_move_ids": commands, "method_number": asset_sequence}
            )
            ...
            move_ids += (
                ...
                .search([("asset_id", "=", asset.id), ("state", "=", "draft")])
                .ids
            )
    ...
    def set_to_close(self, invoice_line_id, date=None):
        ...
        move_ids = full_asset._get_disposal_moves(
            [invoice_line_id] * len(full_asset), disposal_date
        )
    ...
    def set_to_running(self):
        if (
            ...
            and not max(
                self.depreciation_move_ids, key=lambda m: m.date
            ).asset_remaining_value
            == 0
        ):
    ...
    def pause(self, pause_date):
        ...
        all_lines_before_pause = self.depreciation_move_ids.filtered(
            lambda x: x.date <= pause_date
        )
        line_before_pause = all_lines_before_pause and max(
            all_lines_before_pause, key=lambda x: x.date
        )
        following_lines = self.depreciation_move_ids.filtered(
            lambda x: x.date > pause_date
        )
        if following_lines:
            ...
            if self.prorata:
                ...
                depreciation_period_start = (
                    line_before_pause
                    and line_before_pause.date
                    or self.prorata_date
                    or self.first_depreciation_date
                )
                try:
                    time_ratio = ((pause_date - depreciation_period_start).days) / (
                        first_following.date - depreciation_period_start
                    ).days
            ...
            raise UserError(
                _("Trying to pause an asset without any future depreciation line")
            )
    ...
    def _insert_depreciation_line(self, line_before, amount, label, depreciation_date):
        ...
        new_line = moveObj.create(
            moveObj._prepare_move_for_asset_depreciation(
                {
                    "amount": amount,
                    "asset_id": self,
                    ...
                    "asset_depreciated_value": line_before
                    and (line_before.asset_depreciated_value + amount)
                    or amount,
                }
            )
        )
    ...
    def _check_depreciations(self):
        for record in self:
            if (
                record.state == "open"
                and record.depreciation_move_ids
                and not record.currency_id.is_zero(
                    record.depreciation_move_ids.filtered(
                        lambda x: not x.reversal_move_id
                    )
                    .sorted(lambda x: (x.date, x.id))[-1]
                    .asset_remaining_value
                )
            ):
                raise UserError(
