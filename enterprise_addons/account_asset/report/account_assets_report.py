...
class assets_report(models.AbstractModel):
    ...
    def get_account_codes(self, account):
        return [
            (name, name)
            for name in self._get_account_group_with_company(
                account.code, account.company_id.id
            )[1:]
        ]
    ...
    def _get_account_group(self, account_code, parent_group=None, group_dict=None):
        ...
        return self._get_account_group_with_company(
            account_code, self.env.company.id, parent_group, group_dict
        )
    ...
    def _get_account_group_with_company(
        self, account_code, company_id, parent_group=None, group_dict=None
    ):
        """Get the list of parent groups for this account
        return: list containing the main group key, then the name of every group
                for this account, beginning by the more general, until the
        ...
        group_dict = (
            group_dict
            or self.env["account.report"]._get_account_groups_for_asset_report()
        )
        self = self._with_context_company2code2account()
        account_id = (
            self.env.context["company2code2account"]
            .get(company_id, {})
            .get(account_code)
        )
        account_suffix = (
            []
            if parent_group
            else [account_id.display_name if account_id else _("No asset account")]
        )
        for k, v in group_dict.items():
            ...
            if not v.get("children") and account_code_short == k:
                ...
                v.get("children") and key_split[0] <= account_code_short <= key_split[1]
            ):
                return (
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for company_id, company_asset_lines in groupby(
            ...
            for al in parent_lines:
                ...
                depreciation_opening = company_currency.round(
                    al["depreciated_start"] * al_rate
                ) - company_currency.round(al["depreciation"] * al_rate)
                ...
                asset_opening = (
                    company_currency.round(al["asset_original_value"] * al_rate)
                    if opening
                    else 0.0
                )
                asset_add = (
                    0.0
                    if opening
                    else company_currency.round(al["asset_original_value"] * al_rate)
                )
                asset_minus = 0.0
                ...
                for child in children_lines[al["asset_id"]]:
                    ...
                    child_rate = self._get_rate_cached(
                        child_currency,
                        company_currency,
                        company,
                        child["asset_acquisition_date"],
                        curr_cache,
                    )

                    depreciation_opening += company_currency.round(
                        child["depreciated_start"] * child_rate
                    ) - company_currency.round(child["depreciation"] * child_rate)
                    depreciation_closing += company_currency.round(
                ...
                line = {
                    ...
                    "columns": [
                        {
                            "name": al["asset_acquisition_date"]
                        ...
                        {
                            "name": (al["asset_method"] == "linear" and _("Linear"))
    ...
    def _get_assets_lines(self, options):
        ...
        sql = """
            ...
            """.format(
            where_account_move=where_account_move
        )
        ...
        self.env.cr.execute(
            sql,
            {"date_to": date_to, "date_from": date_from, "company_ids": company_ids},
        )
        results = self.env.cr.dictfetchall()
        self.env.cr.execute(
            "DROP TABLE temp_account_move"
        )  # Because tests are run in the same transaction, we need to clean here the SQL INHERITS
        return results
