...
class Providerdhl(models.Model):
    ...
    dhl_account_number = fields.Char(
        string="DHL Account Number", groups="base.group_system"
    )
    dhl_package_dimension_unit = fields.Selection(
        [("I", "Inches"), ("C", "Centimeters")],
        default="C",
        string="Package Dimension Unit",
    )
    dhl_package_weight_unit = fields.Selection(
        [("L", "Pounds"), ("K", "Kilograms")], default="K", string="Package Weight Unit"
    )
    dhl_default_package_type_id = fields.Many2one(
        "stock.package.type", string="DHL Package Type"
    )
    dhl_region_code = fields.Selection(
        [("AP", "Asia Pacific"), ("AM", "America"), ("EU", "Europe")],
        default="AM",
        string="Region",
    )
    ...
    dhl_dutiable = fields.Boolean(
        string="Dutiable Material", help="Check this if your package is dutiable."
    )
    dhl_duty_payment = fields.Selection(
        [("S", "Sender"), ("R", "Recipient")], required=True, default="S"
    )
    ...
    def _rate_shipment_vals(self, order=False, picking=False):
        ...
        rating_request = {}
        srm = DHLProvider(
            self.log_xml, request_type="rate", prod_environment=self.prod_environment
        )
        ...
        if picking:
            rating_request["BkgDetails"] = srm._set_dct_bkg_details_from_picking(
                picking
            )
        else:
            rating_request["BkgDetails"] = srm._set_dct_bkg_details(
                order._get_estimated_weight(), self, order.company_id.partner_id
            )
        rating_request["To"] = srm._set_dct_to(destination_partner_id)
        if self.dhl_dutiable:
            rating_request["Dutiable"] = srm._set_dct_dutiable(
                total_value, currency_id.name
            )
        real_rating_request = {}
                ...
                if condition_code == "410301":
                    return {
                        ...
                        % (
                            ...
                            _(
                                "Hint: The destination may not require the dutiable option."
                            ),
                        ),
                        "warning_message": False,
                    }
        ...
        if shipping_charge:
            ...
            if not shipping_currency or order_currency.name == shipping_currency:
                ...
                price = quote_currency._convert(
                    ...
                    order_currency,
                    order.company_id or picking.company_id,
    ...
    def dhl_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            shipment_request["RequestedPickupTime"] = srm._set_requested_pickup_time(
                True
            )
            shipment_request["Billing"] = srm._set_billing(
                account_number, "S", self.dhl_duty_payment, self.dhl_dutiable
            )
            shipment_request["Consignee"] = srm._set_consignee(picking.partner_id)
            shipment_request["Shipper"] = srm._set_shipper(
                account_number,
            ...
            if self.dhl_dutiable:
                ...
                shipment_request["Dutiable"] = srm._set_dutiable(
                    total_value, currency_name, incoterm
                )
            shipment_request["ShipmentDetails"] = srm._set_shipment_details(picking)
            shipment_request["LabelImageFormat"] = srm._set_label_image_format(
                self.dhl_label_image_format
            )
            ...
            logmessage = _(
                "Shipment created into DHL <br/> <b>Tracking Number : </b>%s"
            ) % (traking_number)
            dhl_labels = [
                (
                    "LabelDHL-%s.%s" % (traking_number, self.dhl_label_image_format),
                    dhl_response.LabelImage[0].OutputImage,
                )
            ]
            if picking.sale_id:
    ...
    def dhl_get_return_label(self, picking, tracking_number=None, origin_date=None):
        shipment_request = {}
        srm = DHLProvider(
            self.log_xml, request_type="ship", prod_environment=self.prod_environment
        )
        site_id = self.sudo().dhl_SiteID
        password = self.sudo().dhl_password
        account_number = self.sudo().dhl_account_number
        ...
        shipment_request["Shipper"] = srm._set_shipper(
            account_number, picking.partner_id, picking.partner_id
        )
        total_value = sum(
            [
                line.product_id.lst_price * line.product_uom_qty
    ...
    def dhl_get_tracking_link(self, picking):
        return (
            "http://www.dhl.com/en/express/tracking.html?AWB=%s"
    ...
    def dhl_cancel_shipment(self, picking):
        ...
        picking.message_post(
            body=_("You can't cancel DHL shipping without pickup date.")
        )
