odoo.define("l10n_de_pos_cert.pos", function (require) {
  ...
  models.PosModel = models.PosModel.extend({
    ...
    isCountryGermanyAndFiskaly() {
      return this.isCountryGermany() && Boolean(this.getTssId());
    },
    ...
    async _flush_orders(orders, options) {
      ...
      for (const orderJson of orders) {
        orderObjectMap[orderJson.id] = new models.Order(
          {},
      ...
      if (sentToFiskaly.length > 0) {
        ...
        try {
          result = await _super_posmodel._flush_orders.apply(this, [
            sentToFiskaly,
            options,
          ]);
        } catch (error) {
    ...
    htmlToImgLetterRendering() {
      return (
        ...
        _super_posmodel.htmlToImgLetterRendering.apply(this, arguments)
      );
    },
  });
  ...
  models.Order = models.Order.extend({
    ...
    isTransactionFinished() {
      return (
        this.transactionState === "finished" || this.tssInformation.time_start.value
      );
    },
    // @Override
    export_for_printing() {
    ...
    _createAmountPerVatRateArray() {
      ...
      return Object.keys(amountPerVatRate)
        .filter((rate) => Boolean(amountPerVatRate[rate]))
        .map((rate) => ({
          vat_rate: rate,
    ...
    async finishShortTransaction() {
      ...
      return $.ajax({
        headers: {Authorization: `Bearer ${this.pos.getApiToken()}`},
        url: `${this.pos.getApiUrl()}/tss/${this.pos.getTssId()}/tx/${
          this.fiskalyUuid
