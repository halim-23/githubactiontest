...
class PosDsfinvkExport(models.Model):
    ...
    l10n_de_fiskaly_export_uuid = fields.Char(
        ...
        help="The uuid of the export at Fiskaly",
    )
    start_datetime = fields.Datetime(
        string="Start Datetime",
        required=True,
        help="Only export data with dates larger than or equal to the given start date",
    )
    end_datetime = fields.Datetime(
        string="End Datetime",
        required=True,
        help="Only export data with dates smaller than or equal to the given start date",
    )
        ...
        for export in self:
            if export.start_datetime > export.end_datetime:
                raise ValidationError(
                    _("The start datetime should be smaller than the end datetime")
                )

    @api.constrains("config_id")
    def _check_fiskaly_client_tss(self):
        for export in self:
            if export.config_id and (
                not export.config_id.l10n_de_fiskaly_client_id
                or not export.config_id.l10n_de_fiskaly_tss_id
            ):
                raise ValidationError(
                    _(
                        "You can only export the data of a point of sale linked to a TSS."
                    )
                )
    ...
    def _l10n_de_trigger_fiskaly_export(self):
        ...
        trigger_resp = self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
            "PUT", "/exports/%s" % self.l10n_de_fiskaly_export_uuid, json
        )
    ...
    def l10n_de_action_refresh_state(self):
        export_data = self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
            "GET", "/exports/%s" % self.l10n_de_fiskaly_export_uuid
        ).json()
    ...
    def l10n_de_action_download_export(self):
        ...
        attachment = self.env["ir.attachment"].search(
            [("res_model", "=", "l10n_de_pos.dsfinvk_export"), ("res_id", "=", self.id)]
        )
        if attachment:
            return {
                "target": "new",
        ...
        if export_data["state"] == "COMPLETED":
            download_resp = self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
                "GET", "/exports/%s/download" % self.l10n_de_fiskaly_export_uuid
            )
            download_resp.raise_for_status()

            attachment = self.env["ir.attachment"].create(
                {
                    "name": "dsfinvk-%s-%s.tar"
                    % (
