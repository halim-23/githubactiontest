...
class ResCompany(models.Model):
    ...
    l10n_de_fiskaly_api_key = fields.Char(
        string="Fiskaly API Key", groups="base.group_erp_manager"
    )
    l10n_de_fiskaly_api_secret = fields.Char(
        string="Fiskaly API Secret", groups="base.group_erp_manager"
    )
    l10n_de_fiskaly_organization_id = fields.Char(string="Fiskaly Organization ID")
    is_country_germany = fields.Boolean(
        string="Company located in Germany", compute="_compute_is_country_germany"
    )
    l10n_de_fiskaly_kassensichv_token = fields.Char(
        string="Fiskaly Kassensichv Token",
        groups="base.group_erp_manager",
        help="Store the temporary token used for the Kassensichv API",
    )
    l10n_de_fiskaly_dsfinvk_token = fields.Char(
        string="Fiskaly DSFinV-K Token",
        groups="base.group_erp_manager",
        help="Store the temporary token used for the DSFinV-K API",
    )
    ...
    def _l10n_de_fiskaly_kassensichv_url(self):
        return (
            ...
            .sudo()
            .get_param(
                "l10n_de_fiskaly_kassensichv_url",
    ...
    def _l10n_de_fiskaly_dsfinvk_api_url(self):
        return (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("l10n_de_fiskaly_dsfinvk_url", "https://dsfinvk.fiskaly.com")
        )
        ...
        if not self.sudo().l10n_de_fiskaly_kassensichv_token:
            ...
            if auth_response.status_code == 401:
                raise ValidationError(
                    _(
                        "The combination of your Fiskaly API key and secret is incorrect. "
                        "Please update them in your company settings"
                    )
                )
            self.sudo().l10n_de_fiskaly_kassensichv_token = auth_response.json()[
                "access_token"
            ]
        headers = {
            "Authorization": "Bearer " + self.sudo().l10n_de_fiskaly_kassensichv_token
        }
        return url, headers

    def _l10n_de_fiskaly_kassensichv_rpc(
        self, method, path, json=None, version=2, recursive=False
    ):
        try:
            timeout = 300
            ...
            if method == "GET":
                ...
                res = requests.post(
                    url + path, headers=headers, json=json, timeout=timeout
                )
            elif method == "PUT":
                res = requests.put(
                    url + path, headers=headers, json=json, timeout=timeout
            ...
            if res.status_code == 401 and not recursive:
                ...
                res = self._l10n_de_fiskaly_kassensichv_rpc(
                    method, path, json, version, True
                )
            ...
            raise UserError(
                _(
                    "There are some connection issues between us and Fiskaly, try again later."
                )
            )
    ...
    def _l10n_de_fiskaly_dsfinvk_auth(self, version):
        ...
        if not self.sudo().l10n_de_fiskaly_dsfinvk_token:
            ...
            self.sudo().l10n_de_fiskaly_dsfinvk_token = auth_response.json()[
                "access_token"
            ]
        headers = {
            "Authorization": "Bearer " + self.sudo().l10n_de_fiskaly_dsfinvk_token
        }
        return url, headers

    def _l10n_de_fiskaly_dsfinvk_rpc(
        self, method, path, json=None, version=0, recursive=False
    ):
        try:
            timeout = 300
            ...
            if res.status_code == 401 and not recursive:
                ...
                res = self._l10n_de_fiskaly_dsfinvk_rpc(
                    method, path, json, version, True
                )
            return res
        except ConnectionError:
    ...
    def _l10n_de_fiskaly_iap_rpc(self, path, params, version=1):
        endpoint = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("l10n_de_fiskaly_iap.endpoint", DEFAULT_ENDPOINT)
        )
        ...
        if not self.l10n_de_fiskaly_organization_id:
            ...
            if not response.get("ignored"):
                if response.get("api_key"):
                    self.write(
                        {
                            "l10n_de_fiskaly_organization_id": response[
                                "organization_id"
                            ],
