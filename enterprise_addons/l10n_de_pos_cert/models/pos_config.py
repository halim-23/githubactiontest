...
class PosConfig(models.Model):
    ...
    l10n_de_fiskaly_tss_id = fields.Char(
        string="TSS ID",
        readonly=True,
        copy=False,
        help="The TSS ID at Fiskaly side linked to the pos.config. "
        "Due to some new broken changes, this field also includes the tss puk and pin separated by `|`",
    )
    l10n_de_fiskaly_client_id = fields.Char(
        string="Client ID",
        readonly=True,
        copy=False,
        help="The Client ID refers to the client at Fiskaly which is mapped to a pos.config.",
    )
    l10n_de_create_tss_flag = fields.Boolean(
        string="Create TSS",
        help="This allows to send a request to Fiskaly in order to create and link a TSS to the pos.config",
    )
    is_company_country_germany = fields.Boolean(
        string="Company located in Germany", related="company_id.is_country_germany"
    )

    def _l10n_de_check_fiskaly_api_key_secret(self):
        if (
            not self.company_id.sudo().l10n_de_fiskaly_api_key
            or not self.company_id.sudo().l10n_de_fiskaly_api_secret
        ):
            raise UserError(
                _(
                    "You have to set your Fiskaly key and secret in your company settings."
                )
            )
    ...
    def _l10n_de_check_fiskaly_tss_client_ids(self):
        if not self.l10n_de_fiskaly_tss_id or not self.l10n_de_fiskaly_client_id:
            raise UserError(
                _(
                    "You have to set your Fiskaly TSS ID and Client ID in your PoS settings."
                )
            )
    ...
    def l10n_de_get_fiskaly_urls_and_keys(self, config_id):
        ...
        return {
            "kassensichv_url": self.env[
                "res.company"
            ]._l10n_de_fiskaly_kassensichv_url(),
    ...
    def unlink(self):
        ...
        tss_to_disable_data = [
            (config.company_id, config.l10n_de_fiskaly_tss_id)
            for config in self
            if config.l10n_de_create_tss_flag
        ]
        res = super().unlink()
        ...
        for tss_data in tss_to_disable_data:
            ...
            if "|" in tss_id_admin_puk_pin:
                ...
                pos_config._l10n_de_fiskaly_kassensichv_rpc(
                    "POST", "/tss/%s/admin/auth" % tss_id, {"admin_pin": admin_pin}
                )
                pos_config._l10n_de_fiskaly_kassensichv_rpc(
                    "PATCH", "/tss/%s" % tss_id, {"state": "DISABLED"}
                )
            else:
                pos_config._l10n_de_fiskaly_kassensichv_rpc(
                    "PUT", "/tss/%s" % tss_id, {"state": "DISABLED"}, version=1
                )
    ...
    def _l10n_de_create_tss_process(self):
        ...
        local_tss = self.search(
            [
                ("company_id", "=", self.company_id.id),
                ("l10n_de_fiskaly_tss_id", "!=", False),
            ]
        )
        db_uuid = self.env["ir.config_parameter"].sudo().get_param("database.uuid")
        self.company_id._l10n_de_fiskaly_iap_rpc(
            "/tss", {"tss_id": tss_id, "db_uuid": db_uuid, "tss": len(local_tss)}
        )
    ...
    def open_session_cb(self):
        ...
        if (
            self.is_company_country_germany
            and self.l10n_de_create_tss_flag
            and "|" not in self.l10n_de_fiskaly_tss_id
        ):
            # Disable the TSS created under v1
            self.company_id._l10n_de_fiskaly_kassensichv_rpc(
                "PUT",
                "/tss/%s" % self._l10n_de_get_tss_id(),
                {"state": "DISABLED"},
                version=1,
            )
            self._create_tss_v2()
    ...
    def _create_tss_v2(self, tss_id=None):
        ...
        tss_creation_resp = self.company_id._l10n_de_fiskaly_kassensichv_rpc(
            "PUT", "/tss/%s" % tss_id, {}
        )  # Yes, Fiskaly is asking for a empty object...
        tss_puk = tss_creation_resp.json()["admin_puk"]
        self.company_id._l10n_de_fiskaly_kassensichv_rpc(
            "PATCH", "/tss/%s" % tss_id, {"state": "UNINITIALIZED"}
        )
        ...
        self.company_id._l10n_de_fiskaly_kassensichv_rpc(
            "POST", "/tss/%s/admin/auth" % tss_id, {"admin_pin": tss_pin}
        )
        self.company_id._l10n_de_fiskaly_kassensichv_rpc(
            "PATCH", "/tss/%s" % tss_id, {"state": "INITIALIZED"}
        )
        ...
        self.company_id._l10n_de_fiskaly_kassensichv_rpc(
            "PUT",
            "/tss/%s/client/%s" % (tss_id, client_id),
            {"serial_number": self.uuid},
        )
        self.company_id._l10n_de_fiskaly_kassensichv_rpc(
            "POST", "/tss/%s/admin/logout" % tss_id, {"admin_pin": tss_pin}
        )

        self.write(
            {
                "l10n_de_fiskaly_tss_id": "|".join([tss_id, tss_puk, tss_pin]),
                "l10n_de_fiskaly_client_id": client_id,
            }
        )
