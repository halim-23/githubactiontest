...
class PosSession(models.Model):
    ...
    l10n_de_fiskaly_cash_point_closing_uuid = fields.Char(
        string="Fiskaly Cash Point Closing Uuid",
        readonly=True,
    ...
    def _validate_session(
        ...
        if (
            self.config_id.is_company_country_germany
            and self.config_id.l10n_de_fiskaly_tss_id
            and self.order_ids
        ):
    ...
    def _l10n_de_send_fiskaly_cash_point_closing(self, json):
        ...
        cash_register_resp = self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
            "GET", "/cash_registers/%s" % self.config_id.l10n_de_fiskaly_client_id
        )
        if cash_register_resp.status_code == 404:  # register the cash register
            self._l10n_de_create_fiskaly_cash_register()
        cash_point_closing_resp = self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
            "PUT", "/cash_point_closings/%s" % cash_point_closing_uuid, json
        )
        if cash_point_closing_resp.status_code != 200:
            raise UserError(
                _(
                    "Cash point closing error with Fiskaly: \n %s",
                    cash_point_closing_resp.json(),
                )
            )
    ...
    def _l10n_de_create_fiskaly_cash_register(self):
        ...
        self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
            "PUT", "/cash_registers/%s" % self.config_id.l10n_de_fiskaly_client_id, json
        )

    def _l10n_de_fiskaly_get_vat_definitions(self):
        vat_definitions_resp = self.company_id._l10n_de_fiskaly_dsfinvk_rpc(
            "GET", "/vat_definitions"
        )
        vat_definitions = {}
