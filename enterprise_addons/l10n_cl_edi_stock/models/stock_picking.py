...
from odoo.tools import BytesIO, float_repr, html_escape
...
class Picking(models.Model):
    ...
    l10n_cl_delivery_guide_reason = fields.Selection(
        ...
        string="Reason of the Transfer",
        default="1",
    )

    l10n_cl_draft_status = fields.Boolean(
        help="Technical field making it possible to have a draft status for entering "
    ...
    l10n_latam_document_type_id = fields.Many2one(
        "l10n_latam.document.type", string="Document Type", readonly=True, copy=False
    )
    ...
    _sql_constraints = [
        (
            "unique_document_number_in_company",
    ...
    def action_cancel(self):
        for record in self.filtered(
            lambda x: x.company_id.country_id == self.env.ref("base.cl")
            and x.l10n_cl_dte_status
        ):
            # The move cannot be modified once the DTE has been accepted by the SII
            if record.l10n_cl_dte_status == "accepted":
                raise UserError(
                    _("%s is accepted by SII. It cannot be cancelled.", self.name)
                )
    ...
    def _get_next_document_number(self):
        return self.env["ir.sequence"].next_by_code(
            "l10n_cl_edi_stock.stock_picking_caf_sequence"
        )

    def create_delivery_guide(self):
        self.ensure_one()
        if (
            self.company_id.country_id.code != "CL"
            or not self.company_id.l10n_cl_dte_service_provider
        ):
            raise UserError(
                _("This company has no connection with the SII configured.  ")
            )
        ...
        if not document_type:
            raise UserError(
                _(
                    "Document type with code 52 active not found. You can update the module to solve this problem. "
                )
            )
    ...
    def print_delivery_guide_pdf(self):
        return self.env.ref(
            "l10n_cl_edi_stock.action_delivery_guide_report_pdf"
        ).report_action(self)
    ...
    def _l10n_cl_create_delivery_guide_validation(self):
        if not self.partner_id:
            raise UserError(
                _("Please set a Delivery Address as the delivery guide needs one.")
            )
        if not self.partner_id.l10n_cl_delivery_guide_price:
            raise UserError(
                _("Please, configure the Delivery Guide Price in the partner.")
            )
        if self.partner_id._l10n_cl_is_foreign():
            raise UserError(
                _("Delivery Guide for foreign partner is not implemented yet")
            )
        ...
        if not caf_file:
            raise UserError(
                _(
                    "CAF file for the document type %s not found. Please, upload a caf file before to "
                    "create the delivery guide",
    ...
    def _l10n_cl_edi_prepare_values(self):
        ...
        values = {
            ...
            "caf": self.l10n_latam_document_type_id.sudo()._get_caf_file(
                self.company_id.id, int(self.l10n_latam_document_number)
            ),
            "fe_value": self.scheduled_date.date(),
            "rr_value": "55555555-5"
            if self.partner_id._l10n_cl_is_foreign()
            else self._l10n_cl_format_vat(self.partner_id.vat),
            "rsr_value": self._format_length(self.partner_id.name, 40),
            "mnt_value": float_repr(
                self._l10n_cl_get_tax_amounts()[0]["total_amount"], 0
            ),
            "picking": self,
            "it1_value": self._format_length(move_lines[0].product_id.name, 40)
            if move_lines
            else "",
        ...
        for move in self.move_lines:
            ...
            line_amounts[move] = {
                ...
                "price_unit": self.company_id.currency_id.round(
                    tax_res["total_excluded"] / move.product_uom_qty
                ),
    ...
    def _prepare_pdf_values(self):
        ...
        result = {
            ...
            "has_unit_price": any(
                [l["price_unit"] != 0.0 for l in total_line_amounts.values()]
            ),
            "has_discount": any(
                l.get("total_discount", "0") != "0" for l in total_line_amounts.values()
            ),
        }
        return result
    ...
    def _prepare_dte_values(self):
        ...
        caf_file = self.l10n_latam_document_type_id.sudo()._get_caf_file(
            self.company_id.id, int(self.l10n_latam_document_number)
        )
    ...
    def _l10n_cl_get_comuna_recep(self):
        if self.partner_id._l10n_cl_is_foreign():
            return self._format_length(
                ...
                or (
                    ...
                    and self.partner_id.commercial_partner_id.state_id.name
                )
                or "N-A",
                20,
            )
        if self.l10n_latam_document_type_id._is_doc_type_voucher():
            return "N-A"
        return (
            self.partner_id.city or self.partner_id.commercial_partner_id.city or False
        )
    ...
    def _l10n_cl_get_dte_barcode_xml(self, caf_file):
        ...
        dd = self.env.ref("l10n_cl_edi_stock.dd_template")._render(
            self._l10n_cl_edi_prepare_values()
        )
    ...
    def _l10n_cl_send_dte_to_sii(self, retry_send=True):
        ...
        self.message_post(
            body=html_escape(
                _(
                    "DTE has been sent to SII with response: %s.",
                    self._l10n_cl_get_sii_reception_status_message(sii_response_status),
                )
            )
        )
    ...
    def _l10n_cl_verify_dte_status(self, send_dte_to_partner=True):
        ...
        self.message_post(
            ...
            % (
                ...
                html_escape(
                    response_parsed.findtext(
                        "{http://www.sii.cl/XMLSchema}RESP_HDR/GLOSA"
                    )
                ),
                html_escape(
                    response_parsed.findtext(
