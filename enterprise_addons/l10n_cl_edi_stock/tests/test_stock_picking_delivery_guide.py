...
class TestL10nClEdiStock(TestL10nClEdiStockCommon):
    @patch(
        "odoo.addons.l10n_cl_edi.models.l10n_cl_edi_util.L10nClEdiUtilMixin._get_cl_current_strftime"
    )
    def test_l10n_cl_edi_delivery_with_taxes_from_inventory(
        self, get_cl_current_strftime
    ):
        ...
        xml_expected_dte = misc.file_open(
            "l10n_cl_edi_stock/tests/expected_dtes/delivery_guide_products_with_taxes.xml"
        ).read()

        self.assertXmlTreeEqual(
            etree.fromstring(
                base64.b64decode(
                    picking.l10n_cl_sii_send_file.with_context(bin_size=False).datas
                )
            ),
    ...
    def test_l10n_cl_edi_delivery_with_taxes_from_sale_order(
        self, get_cl_current_strftime
    ):
        get_cl_current_strftime.return_value = "2019-10-24T20:00:00"
    ...
    def test_l10n_cl_edi_delivery_without_taxes_from_sale_order(
        self, get_cl_current_strftime
    ):
        get_cl_current_strftime.return_value = "2019-10-24T20:00:00"
        ...
        xml_expected_dte = misc.file_open(
            "l10n_cl_edi_stock/tests/expected_dtes/delivery_guide_products_without_taxes.xml"
        ).read()
    ...
    def test_l10n_cl_edi_delivery_guide_no_price(self, get_cl_current_strftime):
        ...
        xml_expected_dte = misc.file_open(
            "l10n_cl_edi_stock/tests/expected_dtes/delivery_guide_no_price.xml"
        ).read()
