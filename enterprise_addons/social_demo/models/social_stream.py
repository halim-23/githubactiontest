...
class DemoSocialStream(models.Model):
    ...
    def create(self, vals_list):
        ...
        demo_streams = [
            ...
            self.env.ref(
                "social_demo.social_stream_twitter_search_competitor",
            ...
            self.env.ref(
                "social_demo.social_stream_youtube_account", raise_if_not_found=False
            ),
            self.env.ref(
    ...
    def _add_default_stream_posts(self):
        ...
        author_image = (
            ("/web/image/res.partner/%s/avatar_128" % res_partner_10.id)
            if res_partner_10
            else ""
        )
        ...
        for stream in self:
            ...
            elif stream.media_id == self.env.ref(
                "social_twitter.social_media_twitter", raise_if_not_found=False
            ):
                author_name = "Twitter Account"
            elif stream.media_id == self.env.ref(
                "social_facebook.social_media_facebook", raise_if_not_found=False
            ):
                author_name = "My Page"
            else:
