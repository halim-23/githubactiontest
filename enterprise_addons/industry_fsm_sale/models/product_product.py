...
class ProductProduct(models.Model):
    ...
    fsm_quantity = fields.Integer(
        "Material Quantity",
        compute="_compute_fsm_quantity",
        inverse="_inverse_fsm_quantity",
        search="_search_fsm_quantity",
    )
    ...
    def _compute_fsm_quantity(self):
        ...
        if task:
            ...
            qty_dict = {
                    x["product_id"][0]: x["product_uom_qty"]
                    for x in products_qties
                    if x["product_id"]
            }
            for product in self:
    ...
    def _inverse_fsm_quantity(self):
        ...
        if task:
            ...
            for sol in sale_lines_read_group:
                sale_lines_per_product[
                    sol["product_id"][0]
                ] |= SaleOrderLine_sudo.browse(sol["ids"])
            for product in self:
                sale_lines = sale_lines_per_product.get(
                    product.id, self.env["sale.order.line"]
                )
                ...
                diff_qty = product.fsm_quantity - sum(
                    sale_lines.mapped("product_uom_qty")
                )
                if (
                    all_editable_lines
                ):  # existing line: change ordered qty (and delivered, if delivered method)
                    if diff_qty > 0:
                        vals = {
                            "product_uom_qty": all_editable_lines[0].product_uom_qty
                            + diff_qty,
                        }
                        if product.service_type == "manual":
                            vals["qty_delivered"] = (
                                all_editable_lines[0].product_uom_qty + diff_qty
                            )
                        all_editable_lines[0].with_context(
                    ...
                    if (
                        task.sale_order_id.pricelist_id.discount_policy
                        == "without_discount"
                    ):
                        sol = SaleOrderLine_sudo.new(vals)
                        sol._onchange_discount()
                        vals.update({"discount": sol.discount or 0.0})
                    SaleOrderLine_sudo.create(vals)
    ...
    def set_fsm_quantity(self, quantity):
        ...
        if (
            ...
            or quantity
            and quantity < 0
            or not self.user_has_groups("project.group_project_user")
        ):
            return
        self = self.sudo()
