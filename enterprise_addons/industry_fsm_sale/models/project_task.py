...
class Task(models.Model):
    ...
    material_line_product_count = fields.Integer(
        compute="_compute_material_line_totals"
    )
    ...
    display_create_invoice_primary = fields.Boolean(
        compute="_compute_display_create_invoice_buttons"
    )
    ...
    def _compute_display_conditions_count(self):
        ...
        for task in self:
            ...
            satisfied += (
                1 if task.allow_material and task.material_line_product_count else 0
            )
            task.update(
                {
    ...
    def _compute_quotation_count(self):
        quotation_data = (
            self.sudo()
            .env["sale.order"]
            .read_group([("task_id", "in", self.ids)], ["task_id"], ["task_id"])
        )
        mapped_data = {
            q["task_id"][0]: q["task_id_count"] for q in quotation_data
        }
        for task in self:
    ...
    def _compute_material_line_totals(self):
        def if_fsm_material_line(sale_line_id, task, employee_mapping_product_ids=None):
            ...
            if (
                employee_mapping_product_ids
            ):  # Then we need to search the product in the employee mappings
                is_not_timesheet_line = (
                    is_not_timesheet_line
                    and sale_line_id.product_id.id not in employee_mapping_product_ids
                )
            ...
            return all(
                [
                    is_not_timesheet_line,
                    is_not_empty,
                    is_not_service_from_so,
                    is_task_related,
                ]
            )

        employee_mapping_timesheet_product_ids = (
            {}
        )  # keys = project_id, value = list of timesheet_product_id
        fsm_tasks = self.filtered("is_fsm")
        if fsm_tasks:
            employee_mapping_read_group = (
                ...
                .read_group(
                    ...
                    [
                        "project_id",
                        "timesheet_product_ids:array_agg(timesheet_product_id)",
                    ],
                    ["project_id"],
                )
            )
            employee_mapping_timesheet_product_ids = {
                res["project_id"]: res["timesheet_product_ids"]
                for res in employee_mapping_read_group
            }
        sols = (
            self.env["sale.order.line"]
            .sudo()
            .search([("order_id", "in", self.sudo().sale_order_id.ids)])
        )
        ...
        for task in self:
            material_sale_lines = (
                sols_by_so[task.sudo().sale_order_id.id]
                .sudo()
                .filtered(
                    lambda sol: if_fsm_material_line(
                        sol,
                        task,
            ...
            task.material_line_total_price = sum(
                material_sale_lines.mapped("price_total")
            )
            task.material_line_product_count = sum(
                material_sale_lines.mapped("product_uom_qty")
            )
    ...
    def _compute_display_create_invoice_buttons(self):
        for task in self:
            ...
            if (
                not task.is_fsm
                or not task.fsm_done
                or not task.allow_billable
                or task.timer_start
                or not task.sale_order_id
                or task.invoice_status == "invoiced"
                or task.sale_order_id.state in ["cancel"]
            ):
    ...
    def _compute_warning_message(self):
        employee_rate_fsm_tasks = self.filtered(
            lambda task: task.pricing_type == "employee_rate"
    ...
    def action_create_invoice(self):
        ...
        so_to_confirm = self.filtered(
            lambda task: task.sale_order_id
            and task.sale_order_id.state in ["draft", "sent"]
    ...
    def action_fsm_view_quotations(self):
        ...
        if self.quotation_count == 1:
            action["res_id"] = (
                self.env["sale.order"].search([("task_id", "=", self.id)]).id
            )
    ...
    def action_fsm_view_material(self):
        if not self.partner_id:
            raise UserError(
                _("A customer should be set on the task to generate a worksheet.")
            )
        ...
        if self.project_id and self.timesheet_product_id:
            domain = expression.AND(
                [domain, [("id", "!=", self.timesheet_product_id.id)]]
            )
        deposit_product = (
        ...
        kanban_view = self.env.ref(
            "industry_fsm_sale.view_product_product_kanban_material"
        )
        search_view = self.env.ref(
            "industry_fsm_sale.product_search_form_view_inherit_fsm_sale"
        )
        return {
            "type": "ir.actions.act_window",
            ...
            "context": {
                "fsm_mode": True,
                "create": self.env["product.template"].check_access_rights(
    ...
    def action_fsm_validate(self):
        ...
        billable_tasks = self.filtered(
            lambda task: task.allow_billable
            and (task.allow_timesheets or task.allow_material)
        )
        timesheets_read_group = (
            self.env["account.analytic.line"]
            .sudo()
            .read_group(
                [("task_id", "in", billable_tasks.ids), ("project_id", "!=", False)],
                ["task_id", "id"],
                ["task_id"],
            )
        )
        timesheet_count_by_task_dict = {
            timesheet["task_id"][0]: timesheet["task_id_count"]
            for timesheet in timesheets_read_group
        }
        for task in billable_tasks:
            timesheet_count = timesheet_count_by_task_dict.get(task.id)
            if (
                not task.sale_order_id and not timesheet_count
    ...
    def _fsm_create_sale_order(self):
        ...
        team = (
            self.env["crm.team"]
            .sudo()
            ._get_default_team_id(user_id=user_id.id, domain=None)
        )
    ...
    def _fsm_create_sale_order_line(self):
        ...
        not_billed_timesheets = (
            self.env["account.analytic.line"]
            .sudo()
            .search([("task_id", "=", self.id), ("project_id", "!=", False)])
        ...
        if self.pricing_type == "employee_rate":
            ...
            timesheets_by_employee_dict = defaultdict(
                lambda: self.env["account.analytic.line"]
            )  # key: employee_id, value: timesheets
            ...
            product_timesheets_dict = defaultdict(
                lambda: self.env["account.analytic.line"]
            )  # key: (timesheet_product_id.id, price_unit, uom_id.id), value: list of timesheets
            for mapping in employee_mappings:
                employee_timesheets = timesheets_by_employee_dict[
                    mapping.employee_id.id
                ]
                product_timesheets_dict[
                    mapping.timesheet_product_id.id,
                    mapping.price_unit,
                    mapping.timesheet_product_id.uom_id.id,
                ] |= employee_timesheets
            ...
            if (
                not sol_in_task
            ):  # Then, add the default product of the project and remaining timesheets in the dict (step 5)
                ...
                if not_billed_timesheets:
                    ...
                    product_timesheets_dict[
                        product.id, product.lst_price, product.uom_id.id
                    ] |= not_billed_timesheets
                elif (
                    default_product.id,
                    default_product.lst_price,
                    default_product.uom_id.id,
                ) in product_timesheets_dict:
                    product = default_product

            # Get all existing service sales order items in the sales order (step 6)
            existing_service_sols = self.sudo().sale_order_id.order_line.filtered(
                "is_service"
            )
            sols_by_product_and_price_dict = defaultdict(
                lambda: self.env["sale.order.line"]
            )  # key: (product_id, price_unit), value: sales order items
            for (
                sol
            ) in (
                existing_service_sols
            ):  # classify the SOLs to easily find the ones that we want.
            ...
            update_timesheet_commands = (
                []
            )  # used to update the so_line field of each timesheet in the current task.

            if (
                not sol_in_task and sols_by_product_and_price_dict
            ):  # Then check in the existing sol if a SOL has the default product defined in the project to set the SOL of the task (step 6.1)
                sol = sols_by_product_and_price_dict.get(
                    (
                        self.project_id.timesheet_product_id.id,
                        self.project_id.timesheet_product_id.lst_price,
                    )
                )
                if sol:
            ...
            for (
                ...
                price_unit,
                uom_id,
            ), timesheets in product_timesheets_dict.items():
                sol = sols_by_product_and_price_dict.get(
                    (timesheet_product_id, price_unit)
                )  # get the existing SOL with the product and the correct price unit
                ...
                update_timesheet_commands.extend(
                    [
                        fields.Command.update(timesheet_id, {"so_line": sol.id})
                        for timesheet_id in timesheets.ids
                    ]
                )
                if not sol_in_task and (
                    not product
                    or (
                        product.id == timesheet_product_id
                        and product.lst_price == price_unit
                    )
                ):
            ...
            sale_order_line = (
                self.sale_order_id
                and self.sudo().sale_order_id.order_line.filtered(
                    lambda sol: sol.product_id == self.project_id.timesheet_product_id
                )[:1]
            )
            ...
            self.sudo().write(
                {  # We need to sudo in case the user cannot see all timesheets in the current task.
                    "sale_line_id": sale_order_line.id,
                    # assign SOL to timesheets
                    "timesheet_ids": [
                        fields.Command.update(
                            timesheet_id, {"so_line": sale_order_line.id}
                        )
                        for timesheet_id in not_billed_timesheets.ids
                    ],
                }
            )
    ...
    def _prepare_materials_delivery(self):
        ...
        timesheet_sol_ids = [
            timesheet["so_line"][0] for timesheet in read_group_timesheets
        ]
        sale_order_lines = (
            self.env["sale.order.line"]
            .sudo()
            .search(
                [
                    ...
                    (
                        "order_id",
                        "in",
                        self.sale_order_id.sudo()
                        .filtered(lambda so: so.state == "sale")
                        .ids,
                    ),
                ]
            )
