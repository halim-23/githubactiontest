...
class TestIndustryFsmEmployeeRate(TestFsmFlowSaleCommon):
    ...
    def test_fsm_employee_rate(self):
        ...
        """
        self.assertTrue(
            self.fsm_project_employee_rate.is_fsm,
            "The project should be a fsm project.",
        )
        self.assertEqual(
            self.fsm_project_employee_rate.pricing_type,
            "employee_rate",
            "The pricing of this fsm project should be employee rate since there are some employee mappings into it.",
        )
        self.assertEqual(
            len(self.fsm_project_employee_rate.sale_line_employee_ids),
            3,
            "The number of employee mappings in this fsm project should be equal to 3 mappings.",
        )
        ...
        self.assertEqual(
            len(task.timesheet_ids), 3, "The task should have 3 timesheets."
        )
        ...
        self.assertEqual(
            len(task.timesheet_ids.so_line),
            3,
            "Each timesheet should have a different SOL.",
        )
        self.assertEqual(
            task.sale_order_id.order_line.mapped("qty_delivered"),
            [1.0] * 3,
            "The generated SO should has 3 SOLs in which the quantity delivered should be equal to 1 hour.",
        )
        self.assertEqual(
            task.sale_line_id.product_id,
            self.fsm_project_employee_rate.timesheet_product_id,
            "The SOL linked to the task should have the default service product of the product.",
        )
        ...
        self.assertEqual(
            len(task.timesheet_ids), 1, "The task should have 1 timesheet."
        )
        self.assertFalse(task.sale_line_id, "The task should have no SOL.")
        ...
        self.assertEqual(
            task.sale_line_id,
            task.timesheet_ids.so_line,
            "The SOL in task and timesheet should be the same.",
        )
        self.assertEqual(
            task.sale_line_id.product_id,
            self.fsm_project_employee_rate.timesheet_product_id,
            "The product in the SOL should be the default service product defined in the project.",
        )
        ...
        self.assertEqual(
            len(task.timesheet_ids), 2, "The task should have 2 timesheets."
        )
        self.assertFalse(task.sale_line_id, "The task should have no SOL.")
        ...
        first_employee_mapping = (
            self.fsm_project_employee_rate.sale_line_employee_ids.filtered(
                lambda mapping: mapping.employee_id in task.timesheet_ids.employee_id
            )[:1]
        )
        self.assertEqual(
            task.sale_line_id.product_id,
            first_employee_mapping.timesheet_product_id,
            "The product choose for the SOL in the task should be the product in the first mapping found based on the timesheets in the task.",
        )
        self.assertEqual(
            first_employee_mapping.employee_id,
            self.employee_user2,
            "The mapping found should be the one for the Employee User 2.",
        )
        self.assertEqual(
            first_employee_mapping.timesheet_product_id,
            self.product_delivery_timesheet1,
            "The product in this mapping should be the one defined the Employee User 2.",
        )
        self.assertEqual(
            task.sale_order_id.order_line.mapped("qty_delivered"),
            [2.0] * 2,
            "Each SOL generated should have 2 hours as quantity delivered.",
        )
    ...
    def test_fsm_employee_rate_with_same_product_in_two_mappings(self):
        ...
        ProjectSaleLineEmployeeMap = self.env[
            "project.sale.line.employee.map"
        ].with_context(default_project_id=self.fsm_project_employee_rate.id)
        ...
        self.assertEqual(
            employee_manager_mapping.timesheet_product_id,
            employee_user2_mapping.timesheet_product_id,
            "Both mappings should have the same service product.",
        )
        self.assertNotEqual(
            employee_manager_mapping.price_unit,
            employee_user2_mapping.price_unit,
            "The price unit of both mappings with the same service product should be different.",
        )
        ...
        self.assertEqual(
            ...
            self.product_order_timesheet1,
            "The both SOLs of this generated SO should have the same service product.",
        )
        self.assertNotEqual(
            so.order_line[0].price_unit,
            so.order_line[1].price_unit,
            "The only different in the both SOLs is the price unit.",
        )
