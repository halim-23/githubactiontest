...
class TestFsmFlowSale(TestFsmFlowSaleCommon):
    def test_fsm_flow(self):
        # material
        self.assertFalse(
            self.task.material_line_product_count,
            "No product should be linked to a new task",
        )
        with self.assertRaises(
            UserError, msg="Should not be able to get to material without customer set"
        ):
            self.task.action_fsm_view_material()
        self.task.write({"partner_id": self.partner_1.id})
        self.assertFalse(
            self.task.task_to_invoice, "Nothing should be invoiceable on task"
        )
        self.task.with_user(self.project_user).action_fsm_view_material()
        self.product_ordered.with_user(self.project_user).with_context(
            {"fsm_task_id": self.task.id}
        ).fsm_add_quantity()
        self.assertEqual(
            self.task.material_line_product_count,
            1,
            "1 product should be linked to the task",
        )
        self.product_ordered.with_user(self.project_user).with_context(
            {"fsm_task_id": self.task.id}
        ).fsm_add_quantity()
        self.assertEqual(
            self.task.material_line_product_count,
            2,
            "2 product should be linked to the task",
        )
        self.product_delivered.with_user(self.project_user).with_context(
            {"fsm_task_id": self.task.id}
        ).fsm_add_quantity()
        self.assertEqual(
            self.task.material_line_product_count,
            3,
            "3 products should be linked to the task",
        )
        self.product_delivered.with_user(self.project_user).with_context(
            {"fsm_task_id": self.task.id}
        ).fsm_remove_quantity()
        ...
        self.assertEqual(
            self.task.material_line_product_count,
            3,
            "3 product should be linked to the task",
        )
        ...
        invoice_wizard = (
            self.env["sale.advance.payment.inv"].with_context(invoice_ctx).create({})
        )
        ...
        # quotation
        self.assertEqual(
            self.task.quotation_count, 1, "1 quotation should be linked to the task"
        )
        quotation = self.env["sale.order"].search(
            [("state", "!=", "cancel"), ("task_id", "=", self.task.id)]
        )
        self.assertEqual(
            self.task.action_fsm_view_quotations()["res_id"],
            quotation.id,
            "Created quotation id should be in the action",
        )
    ...
    def test_fsm_sale_pricelist(self):
        ...
        order_line = self.task.sale_order_id.order_line.filtered(
            lambda l: l.name == "product_a"
        )
