...
class TestMultiCompanyCommon(TestCommonSaleTimesheet):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.company_data["default_user_employee"].write(
            {
                "groups_id": [
                    (6, 0, [user_group_employee.id, user_project_group_employee.id])
                ],
            }
        )
        ...
        Project = (
            ...
            .sudo()
            .with_context({"mail_create_nolog": True, "tracking_disable": True})
        )
    ...
    def test_task(self):
        ...
        self.assertEqual(
            self.task_1.sale_order_id.state,
            "draft",
            "Sale order should not be confirmed",
        )
        ...
        self.assertEqual(
            self.task_1.sale_order_id.state, "sale", "Sale order should be confirmed"
        )
        self.assertEqual(
            self.task_1.sale_order_id.company_id.id,
            self.task_1.company_id.id,
            "The company of the sale order should be the same as the one from the task",
        )
