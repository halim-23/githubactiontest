...
class EasypostRequest:
                ...
                if error_detail:
                    error_message += "".join(
                        [
                            "\n - %s: %s"
                            % (
                                err.get("field", _("Unspecified field")),
                                err.get("message", _("Unknown error")),
                            )
    ...
    def fetch_easypost_carrier(self):
        ...
        if carriers:
            ...
            raise UserError(
                _(
                    "You have no carrier linked to your Easypost Account.\
                Please connect to Easypost, link your account to carriers and then retry."
                )
            )

    def _check_required_value(
        self, carrier, recipient, shipper, order=False, picking=False
    ):
    ...
    def _prepare_address(self, addr_type, addr_obj):
        ...
        address = {
            "order[%s][%s]" % (addr_type, field_name): addr_obj[addr_obj_field]
            for field_name, addr_obj_field in addr_fields.items()
            if addr_obj[addr_obj_field]
        }
        address["order[%s][name]" % addr_type] = (
            addr_obj.name or addr_obj.display_name
        )[:25]
        ...
        if addr_obj.commercial_company_name:
            address[
                "order[%s][company]" % addr_type
            ] = addr_obj.commercial_company_name[:25]
        return address
    ...
    def _prepare_order_shipments(self, carrier, order):
        ...
        max_weight = carrier._easypost_convert_weight(
            carrier.easypost_default_package_type_id.max_weight
        )
        # Order weight
        ...
        if max_weight and total_weight > max_weight:
            ...
            last_shipment_weight = float_round(
                total_weight % max_weight, precision_digits=1
            )
            for shp_id in range(0, total_shipment):
                shipments.update(
                    self._prepare_parcel(
                        shp_id,
    ...
    def _prepare_picking_shipments(self, carrier, picking, is_return=False):
        ...
        if move_lines_without_package:
            ...
            shipment.update(
                self._customs_info(
                    0,
                    move_lines_without_package.filtered(
                        lambda ml: ml.product_id.type in ["product", "consu"]
                    ),
                )
            )
            ...
            for package in picking.package_ids:
                ...
                shipment.update(
                    self._prepare_parcel(
                        ...
                        weight=weight,
                        label_format=carrier.easypost_label_file_type,
                    )
                )
    ...
    def _prepare_parcel(self, shipment_id, package, weight=False, label_format="pdf"):
        ...
        shipment = {
            ...
            "order[shipments][%d][options][label_date]"
            % shipment_id: datetime.datetime.now().isoformat(),
        }
        if package.package_carrier_type == "easypost":
            if package.shipper_package_code:
                shipment.update(
                    {
                        "order[shipments][%d][parcel][predefined_package]"
                        % shipment_id: package.shipper_package_code
                    }
                )
            if not package.shipper_package_code or (
                package.packaging_length > 0
                and package.width > 0
                and package.height > 0
            ):
                shipment.update(
                    {
                        "order[shipments][%d][parcel][length]"
                        % shipment_id: package.packaging_length,
                        "order[shipments][%d][parcel][width]"
                        % shipment_id: package.width,
                        "order[shipments][%d][parcel][height]"
                        % shipment_id: package.height,
                    }
                )
        else:
            raise UserError(
                _(
                    "Package type used in pack %s is not configured for easypost.",
    ...
    def _customs_info(self, shipment_id, lines):
        ...
        for line in lines:
            ...
            rounded_qty = max(
                1,
                float_round(
                    unit_quantity, precision_digits=0, rounding_method="HALF-UP"
                ),
            )
            ...
            customs_info.update(
                {
                    ...
                    % (shipment_id, customs_item_id): rounded_qty,
                    "order[shipments][%d][customs_info][customs_items][%d][value]"
                    % (shipment_id, customs_item_id): line.sale_price,
                    "order[shipments][%d][customs_info][customs_items][%d][currency]"
                    % (
                        shipment_id,
                        customs_item_id,
                    ): line.picking_id.company_id.currency_id.name,
                    "order[shipments][%d][customs_info][customs_items][%d][weight]"
                    % (shipment_id, customs_item_id): line.env[
                        "delivery.carrier"
                    ]._easypost_convert_weight(line.product_id.weight * unit_quantity),
    ...
    def _options(self, shipment_id, carrier):
        ...
        if carrier.easypost_default_service_id:
            service_otpions = (
                carrier.easypost_default_service_id._get_service_specific_options()
            )
            for option_name, option_value in service_otpions.items():
                options[
                    "order[shipments][%d][options][%s]" % (shipment_id, option_name)
                ] = option_value
        return options

    def rate_request(
        self, carrier, recipient, shipper, order=False, picking=False, is_return=False
    ):
        ...
        self._check_required_value(
            carrier, recipient, shipper, order=order, picking=picking
        )
        ...
        if picking:
            order_payload.update(
                self._prepare_picking_shipments(carrier, picking, is_return=is_return)
            )
        else:
        ...
        if not rates:
            error_message = _(
                "It seems Easypost do not provide shipments for this order.\
        ...
        # If the user ask for a specific service level on its carrier.
            ...
            rate = [
                rate
                for rate in rates
                if rate["service"] == carrier.easypost_default_service_id.name
            ]
            if not rate:
                error_message = _(
                    "There is no rate available for the selected service level for one of your package. Please choose another service level."
                )
            else:
                rate = rate[0]
    ...
    def send_shipping(self, carrier, recipient, shipper, picking, is_return=False):
        ...
        """
        # create an order
        result = self.rate_request(
            carrier, recipient, shipper, picking=picking, is_return=is_return
        )
        ...
        response = self._post_process_ship_response(
            response, carrier=carrier, picking=picking
        )
        # explicitly check response for any messages
        ...
        result["track_shipments_url"] = {
            res["tracking_code"]: res["tracker"]["public_url"]
            for res in response["shipments"]
        }
        result["track_label_data"] = {
            res["tracking_code"]: res["postage_label"]["label_url"]
            for res in response["shipments"]
        }
        return result
    ...
    def get_tracking_link(self, order_id):
        ...
        for shipment in response.get("shipments"):
            tracking_public_urls.append(
                [shipment["tracking_code"], shipment["tracker"]["public_url"]]
            )
    ...
    def get_tracking_link_from_code(self, code):
        ...
        for tracker in response.get("trackers"):
            tracking_public_urls.append(
                [tracker["tracking_code"], tracker["public_url"]]
            )
        return tracking_public_urls
    ...
    def _post_process_ship_response(self, response, carrier=False, picking=False):
        ...
        if (
            ...
            and carrier.easypost_delivery_type == "UPS"
            and len(response.get("shipments", [])) > 1
        ):
            if len(response.get("shipments")[0].get("rates", [])) > 0 and all(
                s.get("messages") for s in response["shipments"][1:]
            ):
                if picking:
