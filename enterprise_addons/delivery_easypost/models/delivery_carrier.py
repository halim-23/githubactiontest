...
class DeliverCarrier(models.Model):
    ...
    delivery_type = fields.Selection(
        ...
        ondelete={
            "easypost": lambda recs: recs.write(
                {"delivery_type": "fixed", "fixed_price": 0}
            )
        },
    )
    easypost_test_api_key = fields.Char(
        "Test API Key",
        groups="base.group_system",
        help="Enter your API test key from Easypost account.",
    )
    easypost_production_api_key = fields.Char(
        "Production API Key",
        groups="base.group_system",
        help="Enter your API production key from Easypost account",
    )
    easypost_delivery_type = fields.Char("Easypost Carrier Type")
    easypost_delivery_type_id = fields.Char(
        "Easypost Carrier Type ID, technical for API request"
    )
    easypost_default_package_type_id = fields.Many2one(
        "stock.package.type",
        string="Default Package Type for Easypost",
        domain="[('easypost_carrier', '=', easypost_delivery_type)]",
    )
    easypost_default_service_id = fields.Many2one(
        "easypost.service",
    ...
    easypost_label_file_type = fields.Selection(
        ...
        string="Easypost Label File Type",
        default="PDF",
    )
        ...
        if self.delivery_type == "easypost" and self.sudo().easypost_production_api_key:
            ...
            if carriers:
                action = self.env["ir.actions.actions"]._for_xml_id(
                    "delivery_easypost.act_delivery_easypost_carrier_type"
                )
                action["context"] = {
            ...
            raise UserError(
                "A production key is required in order to load your easypost carriers."
            )

    def easypost_rate_shipment(self, order):
        """Return the rates for a quotation/SO."""
        ep = EasypostRequest(
            self.sudo().easypost_production_api_key
            if self.prod_environment
            else self.sudo().easypost_test_api_key,
            self.log_xml,
        )
        response = ep.rate_request(
            self, order.partner_shipping_id, order.warehouse_id.partner_id, order
        )
    ...
    def easypost_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            for track_number, tracker_url in result.get("track_shipments_url").items():
                carrier_tracking_link += (
                    "<a href=" + tracker_url + ">" + track_number + "</a><br/>"
                )
            ...
            for track_number, label_url in result.get("track_label_data").items():
                ...
                labels.append(
                    (
                        "LabelEasypost-%s.%s"
                        % (track_number, self.easypost_label_file_type),
                        label.content,
                    )
                )

            logmessage = _(
                "Shipment created into Easypost<br/>" "<b>Tracking Numbers:</b> %s<br/>"
            ) % (carrier_tracking_link)
            if picking.sale_id:
    ...
    def easypost_get_return_label(
        ...
        if rate["currency"] == pickings.company_id.currency_id.name:
            ...
            quote_currency._convert(
                float(rate["rate"]),
                pickings.company_id.currency_id,
                self.env.company,
                fields.Date.today(),
            )
        ...
        " + ".join(result.get("track_shipments_url").keys())
    ...
    def easypost_get_tracking_link(self, picking):
        ...
        return (
            len(tracking_urls) == 1 and tracking_urls[0][1] or json.dumps(tracking_urls)
        )
    ...
    def _easypost_get_services_and_package_types(self):
        ...
        response_package = requests.get(
            url_join(
                base_url,
                "/delivery_easypost/static/data/package_types_by_carriers.json",
            )
        )
        response_service = requests.get(
            url_join(
                base_url, "/delivery_easypost/static/data/services_by_carriers.json"
            )
        )
    ...
    def _onchange_delivery_type(self):
        if self.delivery_type == "easypost":
            ...
            if not self.easypost_test_api_key or not self.easypost_production_api_key:
                ...
                if (
                    carrier.easypost_production_api_key
                    and not self.easypost_production_api_key
                ):
                    self.easypost_production_api_key = (
                        carrier.easypost_production_api_key
                    )
    ...
    def _generate_services(self, rates):
        ...
        for service_name in {
            service["name"] for service in existing_services
        } ^ set(services_name):
            self.env["easypost.service"].create(
                {"name": service_name, "easypost_carrier": self.easypost_delivery_type}
            )
