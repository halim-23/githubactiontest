...
class TestDeliveryEasypost(EasypostTestCommon):
    ...
    def test_easypost_one_package_shipping(self):
        ...
        self.assertGreater(
            ...
            0.00,
            "Could't get rate for this order from easypost fedex",
        )
        choose_delivery_carrier.button_confirm()
        sale_order_fedex.action_confirm()

        self.assertEqual(
            len(sale_order_fedex.picking_ids),
            1,
            "The Sales Order did not generate a picking for ep-fedex.",
        )
        ...
        self.assertGreater(
            picking_fedex.weight, 0.0, "Picking weight should be positive.(ep-fedex)"
        )

        # Set a service in order to test rate request for a specific service.
        self.easypost_fedex_carrier.easypost_default_service_id = (
            self.env["easypost.service"]
            .search([("name", "=", "STANDARD_OVERNIGHT")])
            .id
        )

        if not self.easypost_fedex_carrier.easypost_default_service_id:
            _logger.warning(
                '"STANDARD_OVERNIGHT" is not anymore a fedex service, easypost default service is not tested.'
            )

        picking_fedex._action_done()
        self.assertGreater(
            picking_fedex.carrier_price,
            0.0,
            "Easypost carrying price is probably incorrect(fedex)",
        )
        self.assertIsNot(
            picking_fedex.carrier_tracking_ref,
            False,
    ...
    def test_easypost_multiple_packages_shipping(self):
        ...
        picking_fedex = sale_order_fedex.picking_ids[0]
        self.assertEqual(
            picking_fedex.carrier_id.id,
            sale_order_fedex.carrier_id.id,
        ...
        picking_fedex.move_lines[
            0
        ].move_line_ids.result_package_id.package_type_id = (
            self.fedex_default_package_type.id
        )
        picking_fedex.move_lines[
            0
        ].move_line_ids.result_package_id.shipping_weight = 10.0
