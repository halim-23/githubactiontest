...
class TestEasypostRequest(EasypostTestCommon):
    ...
    def test_prepare_order_shipments(self):
        ...
        shipment = self.easypost._prepare_order_shipments(
            self.easypost_fedex_carrier, sale_order_fedex
        )

        self.assertEqual(shipment["order[shipments][0][parcel][weight]"], 80)
        self.assertFalse(
            "order[shipments][1][parcel][weight]" in shipment,
            "Should have only 1 shipment",
        )
    ...
    def test_prepare_order_shipments_multiple(self):
        ...
        self.assertEqual(
            shipment["order[shipments][0][parcel][weight]"],
            3 * 16,
            "First package weight",
        )
        self.assertTrue(
            "order[shipments][1][parcel][weight]" in shipment, "Should have 2 shipments"
        )
        self.assertEqual(
            shipment["order[shipments][1][parcel][weight]"], 2 * 16, "Leftover weight"
        )
        self.assertFalse(
            "order[shipments][2][parcel][weight]" in shipment, "Should have 2 shipments"
        )
