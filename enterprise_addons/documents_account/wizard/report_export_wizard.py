...
class ReportExportWizard(models.TransientModel):
    ...
    def _get_default_folder(self):
        ...
        return (
            company.documents_account_settings
            and company.account_folder
            or self.env.ref("documents.documents_finance_folder")
        )

    folder_id = fields.Many2one(
        string="Folder",
        comodel_name="documents.folder",
        help="Folder where to save the generated file",
        required=True,
        default=_get_default_folder,
    )
    tag_ids = fields.Many2many(
        "documents.tag",
        "export_wiz_document_tag_rel",
        string="Tags",
    ...
    def get_attachment_vals(self, file_name, file_content, mimetype, log_options_dict):
        rslt = super(ReportExportWizard, self).get_attachment_vals(
            file_name, file_content, mimetype, log_options_dict
        )
