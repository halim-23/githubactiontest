...
class AccountMove(models.Model):
    ...
    document_request_line_id = fields.Many2one(
        "account.move.line", string="Reconciliation Journal Entry Line"
    )
    ...
    def _get_domain_matching_suspense_moves(self):
        ...
        return expression.OR([domain, [("reconciliation_invoice_id", "=", self.id)]])
        ...
        if (
            ...
            if previous_attachment_id:
                document = (
                    ...
                    .sudo()
                    .search([("attachment_id", "=", previous_attachment_id)], limit=1)
                )
            if document:
    ...
    def _update_or_create_document(self, attachment_id):
        if self.company_id.documents_account_settings:
            setting = (
                self.env["documents.account.folder.setting"]
                .sudo()
                .search(
                    [
            ...
            if setting:
                ...
                Documents = (
                    self.env["documents.document"]
                    .with_context(default_type="empty")
                    .sudo()
                )
...
class AccountMoveLine(models.Model):
    ...
    reconciliation_invoice_id = fields.One2many(
        "account.move", "document_request_line_id"
    )
