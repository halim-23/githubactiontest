...
class TestCaseDocumentsBridgeAccount(TransactionCase):
    ...
    def test_bridge_folder_workflow(self):
        ...
        """
        self.assertEqual(
            self.document_txt.res_model,
            "documents.document",
            "failed at default res model",
        )
        multi_return = self.workflow_rule_vendor_bill.apply_actions(
            [self.document_txt.id, self.document_gif.id]
        )
        self.assertEqual(
            multi_return.get("type"),
            "ir.actions.act_window",
            "failed at invoice workflow return value type",
        )
        self.assertEqual(
            multi_return.get("res_model"),
            "account.move",
        ...
        self.assertEqual(
            self.document_txt.res_model,
            "account.move",
            "failed at workflow_bridge_dms_account" " new res_model",
        )
        vendor_bill_txt = self.env["account.move"].search(
            [("id", "=", self.document_txt.res_id)]
        )
        self.assertTrue(
            vendor_bill_txt.exists(),
            "failed at workflow_bridge_dms_account vendor_bill",
        )
        self.assertEqual(
            self.document_txt.res_id,
            vendor_bill_txt.id,
            "failed at workflow_bridge_dms_account res_id",
        )
        self.assertEqual(
            vendor_bill_txt.move_type,
            "in_invoice",
            "failed at workflow_bridge_dms_account vendor_bill type",
        )
        vendor_bill_gif = self.env["account.move"].search(
            [("id", "=", self.document_gif.res_id)]
        )
        self.assertEqual(
        ...
        single_return = self.workflow_rule_vendor_bill.apply_actions(
            [self.document_txt.id]
        )
        self.assertEqual(
            single_return.get("res_model"),
            "account.move",
            "failed at invoice res_model action from workflow create model",
        )
        invoice = self.env[single_return["res_model"]].browse(
            single_return.get("res_id")
        )
        attachments = self.env["ir.attachment"].search(
            [("res_model", "=", "account.move"), ("res_id", "=", invoice.id)]
        )
        self.assertEqual(
            len(attachments), 1, "there should only be one ir attachment matching"
        )
    ...
    def test_bridge_account_account_settings_on_write(self):
        ...
        for invoice_type in ["in_invoice", "out_invoice", "in_refund", "out_refund"]:
            ...
            txt_doc = self.env["documents.document"].search(
                [("attachment_id", "=", attachment_txt_test.id)]
            )
            self.assertEqual(
                txt_doc.folder_id, folder_test, "the text test document have a folder"
            )
            invoice_test.write(
                {"message_main_attachment_id": attachment_txt_alternative_test.id}
            )
            self.assertEqual(
                txt_doc.attachment_id.id,
                attachment_txt_alternative_test.id,
                "the attachment of the document should have swapped",
            )
            attachment_txt_main_attachment_test.register_as_main_attachment()
            self.assertEqual(
                txt_doc.attachment_id.id,
                attachment_txt_main_attachment_test.id,
                "the attachment of the document should have swapped",
            )
    ...
    def test_reconciliation_request(self):
        account_type_test = self.env["account.account.type"].create(
            {"name": "account type test", "type": "other", "internal_group": "asset"}
        )
        account_test = self.env["account.account"].create(
            {
                "name": "Receivable",
                "code": "0000222",
                "user_type_id": account_type_test.id,
                "reconcile": True,
            }
        )
        journal_test = self.env["account.journal"].create(
            {"name": "journal test", "type": "bank", "code": "BNK67"}
        )
        ...
        self.assertEqual(
            action["res_model"], "account.move", "a new invoice should be generated"
        )
        invoice = self.env["account.move"].browse(action["res_id"])
        self.assertEqual(
            invoice.document_request_line_id.id,
            account_move_line_test.id,
