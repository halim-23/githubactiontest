...
class ApprovalRequest(models.Model):
    ...
    def action_confirm(self):
        ...
        if self.category_id.manager_approval == "required":
            ...
            if not employee.parent_id:
                raise UserError(
                    _(
                        "This request needs to be approved by your manager. There is no manager linked to your employee profile."
                    )
                )
            if not employee.parent_id.user_id:
                raise UserError(
                    _(
                        "This request needs to be approved by your manager. There is no user linked to your manager."
                    )
                )
            if not self.approver_ids.filtered(
                ...
                raise UserError(
                    _(
                        "This request needs to be approved by your manager. Your manager is not in the approvers list."
                    )
                )
    ...
    def _compute_request_status(self):
        for request in self:
            ...
            required_statuses = request.approver_ids.filtered("required").mapped(
                "status"
            )
            required_approved = required_statuses.count("approved") == len(
                required_statuses
            )
            minimal_approver = (
    ...
    def _compute_approver_ids(self):
        for request in self:
            ...
            users_to_category_approver = defaultdict(
                lambda: self.env["approval.category.approver"]
            )
            ...
            for user in new_users:
                ...
                if current_approver and current_approver.required != required:
                    approver_id_vals.append(
                        Command.update(current_approver.id, {"required": required})
                    )
...
class ApprovalApprover(models.Model):
    ...
    request_id = fields.Many2one(
        "approval.request", string="Request", ondelete="cascade", check_company=True
    )
    company_id = fields.Many2one(
        string="Company",
    ...
    def _compute_can_edit(self):
        self.update(
            {"can_edit": self.env.user.has_group("approvals.group_approval_user")}
        )
