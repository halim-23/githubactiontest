...
class TransferModel(models.Model):
    ...
    def _get_default_date_start(self):
        ...
        return (
            company.compute_fiscalyear_dates(date.today())["date_from"]
            if company
            else None
        )
    ...
    journal_id = fields.Many2one(
        "account.journal", required=True, string="Destination Journal"
    )
    company_id = fields.Many2one(
        "res.company", readonly=True, related="journal_id.company_id"
    )
    date_start = fields.Date(
        string="Start Date", required=True, default=_get_default_date_start
    )
    ...
    move_ids = fields.One2many(
        "account.move", "transfer_model_id", string="Generated Moves"
    )
    move_ids_count = fields.Integer(compute="_compute_move_ids_count")
    total_percent = fields.Float(
        compute="_compute_total_percent", string="Total Percent", readonly=True
    )
        ...
        for record in self:
            if not (0 < record.total_percent <= 100.0):
                raise ValidationError(
                    _(
                        "The total percentage (%s) should be less or equal to 100 !",
            ...
            for line in record.line_ids:
                if line.partner_ids and line.analytic_account_ids:
                    for p in line.partner_ids:
                        for a in line.analytic_account_ids:
                            ...
                            if combination in combinations:
                                raise ValidationError(
                                    _(
                                        "The partner filter %s in combination with the analytic filter %s is duplicated",
                                        p.display_name,
                                        a.display_name,
                                    )
                                )
                    ...
                    for p in line.partner_ids:
                        ...
                        if combination in combinations:
                            raise ValidationError(
                                _("The partner filter %s is duplicated", p.display_name)
                            )
                        combinations.append(combination)
                    ...
                    for a in line.analytic_account_ids:
                        ...
                        if combination in combinations:
                            raise ValidationError(
                                _(
                                    "The analytic filter %s is duplicated",
                                    a.display_name,
                                )
                            )
                        combinations.append(combination)
    ...
    def _compute_total_percent(self):
        ...
        for record in self:
            non_filtered_lines = record.line_ids.filtered(
                lambda l: not l.partner_ids and not l.analytic_account_ids
            )
        ...
        for record in self:
            ...
            if record.account_ids and record.line_ids:
                ...
                if not record.date_stop:
                    ...
                    record._create_or_update_move_for_period(
                        start_date, min(next_move_date, record.date_stop)
                    )
        return False
    ...
    def _create_or_update_move_for_period(self, start_date, end_date):
        ...
        if current_move is None:
            current_move = self.env["account.move"].create(
                {
                    "ref": "%s: %s --> %s"
                    % (self.name, str(start_date), str(end_date)),
                    "date": end_date,
    ...
    def _determine_start_date(self):
        ...
        move_ids = self.env["account.move"].search(
            last_move_domain, order="date desc", limit=1
        )
    ...
    def _get_auto_transfer_move_line_values(self, start_date, end_date):
        ...
        filtered_lines = self.line_ids.filtered(
            lambda x: x.analytic_account_ids or x.partner_ids
        )
        if filtered_lines:
            values += filtered_lines._get_transfer_move_lines_values(
                start_date, end_date
            )

        non_filtered_lines = self.line_ids - filtered_lines
        if non_filtered_lines:
            values += self._get_non_filtered_auto_transfer_move_line_values(
                non_filtered_lines, start_date, end_date
            )
            ...
            [
                ...
                [
                    (
                        "analytic_account_id",
                        "not in",
                        self.line_ids.analytic_account_ids.ids,
                    ),
                    ("partner_id", "not in", self.line_ids.partner_ids.ids),
                ],
            ]
        )
        total_balance_by_accounts = self.env["account.move.line"].read_group(
        ...
        for total_balance_account in total_balance_by_accounts:
            ...
            if not float_is_zero(initial_amount, precision_digits=9):
                ...
                source_move_line = {
                    ...
                    "credit"
                    if source_account_is_debit
                    else "debit": substracted_amount,
                }
                values_list += move_lines_values
                values_list.append(source_move_line)
        return values_list

    def _get_non_analytic_transfer_values(
        self, account, lines, write_date, amount, is_debit
    ):
        """
        ...
        for i, line in enumerate(lines):
            ...
            move_line = line._get_destination_account_transfer_move_line_values(
                account, line_amount, is_debit, write_date
            )
...
class TransferModelLine(models.Model):
    ...
    transfer_model_id = fields.Many2one(
        "account.transfer.model", string="Transfer Model", required=True
    )
    ...
    percent = fields.Float(
        ...
        required=True,
        default=100,
        help="Percentage of the sum of lines from the origin accounts will be transferred to the destination account",
    )
    analytic_account_ids = fields.Many2many(
        "account.analytic.account",
        string="Analytic Filter",
        help="Adds a condition to only transfer the sum of the lines from the origin accounts that match these analytic accounts to the destination account",
    )
    partner_ids = fields.Many2many(
        "res.partner",
        string="Partner Filter",
        help="Adds a condition to only transfer the sum of the lines from the origin accounts that match these partners to the destination account",
    )
    ...
    def _get_transfer_move_lines_values(self, start_date, end_date):
        ...
        for transfer_model_line in self:
            domain = transfer_model_line._get_move_lines_domain(
                start_date, end_date, already_handled_move_line_ids
            )
            total_balances_by_account = self.env["account.move.line"].read_group(
                domain, ["ids:array_agg(id)", "balance", "account_id"], ["account_id"]
            )
            for total_balance_account in total_balances_by_account:
                ...
                if not float_is_zero(balance, precision_digits=9):
                    ...
                    account_id = total_balance_account["account_id"][0]
                    account = self.env["account.account"].browse(account_id)
                    transfer_values += transfer_model_line._get_transfer_values(
                        account, amount, source_account_is_debit, end_date
                    )
    ...
    def _get_move_lines_domain(self, start_date, end_date, avoid_move_line_ids=None):
        ...
        """
        self.ensure_one()
        move_lines_domain = self.transfer_model_id._get_move_lines_base_domain(
            start_date, end_date
        )
        if avoid_move_line_ids:
            move_lines_domain.append(("id", "not in", avoid_move_line_ids))
        if self.analytic_account_ids:
            move_lines_domain.append(
    ...
    def _get_transfer_values(self, account, amount, is_debit, write_date):
        ...
        self.ensure_one()
        return [
            self._get_destination_account_transfer_move_line_values(
                account, amount, is_debit, write_date
            ),
            self._get_origin_account_transfer_move_line_values(
                account, amount, is_debit, write_date
            ),
        ]

    def _get_origin_account_transfer_move_line_values(
        self, origin_account, amount, is_debit, write_date
    ):
        """
        ...
        :rtype: dict
        """
        anal_accounts = self.analytic_account_ids and ", ".join(
            self.analytic_account_ids.mapped("name")
        )
        ...
        if anal_accounts and partners:
            ...
            ) % (anal_accounts, partners)
        elif anal_accounts:
            name = _("Automatic Transfer (entries with analytic account(s): %s)") % (
                anal_accounts,
            )
        elif partners:
    ...
    def _get_destination_account_transfer_move_line_values(
        self, origin_account, amount, is_debit, write_date
    ):
        """
        ...
        if anal_accounts and partners:
            name = _(
                "Automatic Transfer (from account %s with analytic account(s): %s and partner(s): %s)"
            ) % (origin_account.code, anal_accounts, partners)
        elif anal_accounts:
            name = _(
                "Automatic Transfer (from account %s with analytic account(s): %s)"
            ) % (origin_account.code, anal_accounts)
        elif partners:
            name = _("Automatic Transfer (from account %s with partner(s): %s)") % (
                origin_account.code,
                partners,
            )
        else:
            name = _("Automatic Transfer (%s%% from account %s)") % (
                self.percent,
                origin_account.code,
            )
        return {
    ...
    def _compute_percent_is_readonly(self):
        for record in self:
            record.percent_is_readonly = (
                record.analytic_account_ids or record.partner_ids
            )
