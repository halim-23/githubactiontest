...
class MoveModelLineTestCase(AccountAutoTransferTestCase):
    ...
    def test__get_transfer_move_lines_values_same_aaccounts(self):
        ...
        transfer_model_line_1 = self._add_transfer_model_line(
            self.destination_accounts[0].id,
            analytic_account_ids=[aaccounts[0].id, aaccounts[1].id],
        )
        transfer_model_line_2 = self._add_transfer_model_line(
            self.destination_accounts[1].id, analytic_account_ids=[aaccounts[0].id]
        )
    ...
    def test__get_transfer_move_lines_values(self):
        ...
        transfer_model_line_2 = self._add_transfer_model_line(
            self.destination_accounts[1].id, analytic_account_ids=[aaccounts[2].id]
        )
    ...
    @patch(
        "odoo.addons.account_auto_transfer.models.transfer_model.TransferModel._get_move_lines_base_domain"
    )
    def test__get_move_lines_domain(self, patched):
        ...
        percent = 42.42
        analytic_transfer_model_line = self._add_transfer_model_line(
            self.destination_accounts[0].id,
            analytic_account_ids=[aaccount_1.id, aaccount_2.id],
        )
        percent_transfer_model_line = self._add_transfer_model_line(
            self.destination_accounts[1].id, percent=percent
        )

        anal_res = analytic_transfer_model_line._get_move_lines_domain(*args)
        anal_expected = return_val + [
            ("analytic_account_id", "in", [aaccount_1.id, aaccount_2.id])
        ]
    ...
    def test__get_origin_account_transfer_move_line_values(self):
        percent = 92.42
        transfer_model_line = self._add_transfer_model_line(
            self.destination_accounts[0].id, percent=percent
        )
        ...
        result = transfer_model_line._get_origin_account_transfer_move_line_values(
            *params
        )
        expected = {
            "name": "Automatic Transfer (to account %s)"
            % self.destination_accounts[0].code,
    ...
    def test__get_destination_account_transfer_move_line_values(self):
        ...
        write_date = fields.Date.to_date("2019-12-19")
        params = [origin_account, amount, is_debit, write_date]
        anal_result = analytic_transfer_model_line._get_destination_account_transfer_move_line_values(
            *params
        )
        ...
        percent_result = percent_transfer_model_line._get_destination_account_transfer_move_line_values(
            *params
        )
        percent_expected_result = {
            "name": "Automatic Transfer (%s%% from account %s)"
            % (percent, self.origin_accounts[0].code),
            "account_id": self.destination_accounts[1].id,
            "date_maturity": write_date,
            "debit" if is_debit else "credit": amount,
        }
    ...
    def test__get_transfer_move_lines_values_same_partner_ids(self):
        ...
        transfer_model_line_2 = self._add_transfer_model_line(
            self.destination_accounts[1].id, partner_ids=[partner_ids[0].id]
        )
    ...
    def test__get_transfer_move_lines_values_partner(self):
        ...
        transfer_model_line_1 = self._add_transfer_model_line(
            self.destination_accounts[3].id,
            analytic_account_ids=[aaccounts[0].id],
            partner_ids=[partner_ids[1].id],
        )
        transfer_model_line_2 = self._add_transfer_model_line(
            self.destination_accounts[2].id, partner_ids=[partner_ids[0].id]
        )
