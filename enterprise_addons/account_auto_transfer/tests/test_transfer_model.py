...
from odoo.addons.account_auto_transfer.tests.account_auto_transfer_test_classes import (
    AccountAutoTransferTestCase,
)
...
class TransferModelTestFunctionalCase(AccountAutoTransferTestCase):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.analytic_accounts = reduce(
            lambda x, y: x + y,
            (
                cls._create_analytic_account(cls, name)
                for name in ("ANA1", "ANA2", "ANA3")
            ),
        )
        ...
        for date in cls.dates:
            ...
            for an_account in chain(
                cls.analytic_accounts, [cls.env["account.analytic.account"]]
            ):
    ...
    def test_analytics(self):
        ...
        self.functional_transfer.line_ids[
            0
        ].analytic_account_ids = self.analytic_accounts[0:2]
        self.functional_transfer.line_ids[
            1
        ].analytic_account_ids = self.analytic_accounts[2]
        ...
        self.assertEqual(
            sum(
                self.env["account.move.line"]
                .search([("account_id", "=", self.destination_accounts[0].id)])
                .mapped("balance")
            ),
            8000,
        )
        ...
        self.assertEqual(
            sum(
                self.env["account.move.line"]
                .search([("account_id", "=", self.destination_accounts[1].id)])
                .mapped("balance")
            ),
            4000,
        )
        ...
        self.assertEqual(
            sum(
                self.env["account.move.line"]
                .search([("account_id", "=", self.destination_accounts[2].id)])
                .mapped("balance")
            ),
            800,
        )
        self.assertEqual(
            sum(
                self.env["account.move.line"]
                .search([("account_id", "=", self.destination_accounts[3].id)])
                .mapped("balance")
            ),
            800,
        )
...
class TransferModelTestCase(AccountAutoTransferTestCase):
    @patch(
        "odoo.addons.account_auto_transfer.models.transfer_model.TransferModel.action_perform_auto_transfer"
    )
    ...
    @patch(
        "odoo.addons.account_auto_transfer.models.transfer_model.TransferModel._create_or_update_move_for_period"
    )
    def test_action_perform_auto_transfer(self, patched):
        self.transfer_model.date_start = datetime.strftime(
        ...
        self.transfer_model.write(
            {
                "line_ids": [
                    (0, 0, {"percent": 50.0, "account_id": slave_ids[0].id}),
                    (0, 0, {"percent": 50.0, "account_id": slave_ids[1].id}),
                ]
            }
        )
        ...
        self.assertEqual(
            initial_call_count + 13,
            patched.call_count,
            "13 more calls should have been done",
        )

    @patch(
        "odoo.addons.account_auto_transfer.models.transfer_model.TransferModel._get_auto_transfer_move_line_values"
    )
    def test__create_or_update_move_for_period(
        self, patched_get_auto_transfer_move_line_values
    ):
        # PREPARATION
        master_ids, slave_ids = self._create_accounts(2, 0)
        next_move_date = self.transfer_model._get_next_move_date(
            self.transfer_model.date_start
        )
        ...
        created_move = self.transfer_model._create_or_update_move_for_period(
            self.transfer_model.date_start, next_move_date
        )
        ...
        self.assertRecordValues(
            created_move.line_ids.filtered(lambda l: l.credit),
            [
                {
                    "account_id": master_ids[0].id,
                    "date_maturity": next_move_date,
        ...
        self.assertRecordValues(
            created_move.line_ids.filtered(lambda l: l.debit),
            [
                {
                    "account_id": master_ids[1].id,
                    "date_maturity": next_move_date,
        ...
        updated_move = self.transfer_model._create_or_update_move_for_period(
            self.transfer_model.date_start, next_move_date
        )
        self.assertEqual(
            amount_of_moves,
            self.env["account.move"].search_count([]),
            "No move have been created",
        )
        self.assertEqual(
            amount_of_move_lines,
        ...
        self.assertRecordValues(
            updated_move.line_ids.filtered(lambda l: l.credit),
            [
                {
                    "account_id": master_ids[0].id,
                    "date_maturity": next_move_date,
        ...
        self.assertRecordValues(
            updated_move.line_ids.filtered(lambda l: l.debit),
            [
                {
                    "account_id": master_ids[1].id,
                    "date_maturity": next_move_date,
    ...
    def test__get_move_for_period(self):
        ...
        # Generate a move
        move_date = self.transfer_model._get_next_move_date(
            self.transfer_model.date_start
        )
        ...
        self.assertEqual(
            move_for_period,
            already_generated_move,
            "Should be equal to the already generated move",
        )
        ...
        move_for_period = self.transfer_model._get_move_for_period(date_to_test)
        self.assertIsNone(
            move_for_period, "No move is generated yet for the next period"
        )
    ...
    def test__determine_start_date(self):
        ...
        self.assertEqual(
            start_date,
            self.transfer_model.date_start,
            "No moves generated yet, start date should be the start date of the transfer model",
        )
        ...
        self.assertEqual(
            start_date,
            self.transfer_model.date_start,
            "A move generated but not posted, start date should be the start date of the transfer model",
        )
        ...
        self.assertEqual(
            start_date,
            move.date,
            "A move posted, start date should be the date of that move",
        )

        second_move = self._create_basic_move(
            date_str="2019-08-01",
            journal_id=self.journal.id,
            transfer_model_id=self.transfer_model.id,
            posted=False,
        ...
        self.assertEqual(
            start_date,
            move.date,
            "Two moves generated, start date should be the date of the last posted one",
        )

        second_move.action_post()
        random_move = self._create_basic_move(
            date_str="2019-08-01", journal_id=self.journal.id
        )
        start_date = self.transfer_model._determine_start_date()
        self.assertEqual(
            start_date,
            second_move.date,
            "Random move generated not linked to transfer model, start date should be the date of the last one linked to it",
        )
    ...
    def test__get_next_move_date(self):
        ...
        for frequency in experimentations:
            ...
            for start_date, expected_date_str in experimentations[frequency]:
                ...
                self.assertEqual(
                    next_date,
                    fields.Date.to_date(expected_date_str),
                    "Next date from %s should be %s"
                    % (str(next_date), expected_date_str),
                )

    @patch(
        "odoo.addons.account_auto_transfer.models.transfer_model.TransferModel._get_non_analytic_transfer_values"
    )
    @patch("odoo.models.BaseModel.read_group")
    def test__get_non_filtered_auto_transfer_move_line_values(
        self, patched_read_group, patched_get_values
    ):
        ...
        exp = [
            ...
            {
                "name": "Automatic Transfer (-%s%%)"
                % self.transfer_model.total_percent,
        ...
        exp_res_len = (
            len([x for x in patched_read_group.return_value if x["balance"] != 0.0]) * 2
        )
        res = self.transfer_model._get_non_filtered_auto_transfer_move_line_values(
            [], start_date, end_date
        )
    ...
    def test__get_non_analytic_transfer_values(self, patched):
        ...
        res = self.transfer_model._get_non_analytic_transfer_values(
            account, lines, write_date, amount, is_debit
        )
        ...
        exp_calls = [
            call(
                account, round(amount * (line.percent / 100.0), 1), is_debit, write_date
            )
            for line in lines
        ]
