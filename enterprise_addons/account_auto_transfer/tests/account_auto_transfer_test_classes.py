...
class AccountAutoTransferTestCase(AccountTestInvoicingCommon):
    ...
    def _assign_origin_accounts(self):
        self.transfer_model.write({"account_ids": [(6, 0, self.origin_accounts.ids)]})
    ...
    def _add_transfer_model_line(
        self,
        account_id: int = False,
        percent: float = 100.0,
        analytic_account_ids: list = False,
        partner_ids: list = False,
    ):
        ...
        return self.env["account.transfer.model.line"].create(
            {
                ...
                and [(4, aa) for aa in analytic_account_ids],
