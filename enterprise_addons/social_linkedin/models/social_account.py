...
class SocialAccountLinkedin(models.Model):
    ...
    linkedin_account_urn = fields.Char(
        "LinkedIn Account URN", readonly=True, help="LinkedIn Account URN"
    )
    linkedin_account_id = fields.Char(
        "LinkedIn Account ID", compute="_compute_linkedin_account_id"
    )
    ...
    def _compute_stats_link(self):
        ...
        for account in linkedin_accounts:
            account.stats_link = (
                "https://www.linkedin.com/company/%s/admin/analytics/visitors/"
    ...
    def _compute_statistics(self):
        ...
        for account in linkedin_accounts:
            ...
            for stat_name in list(all_stats_dict.keys()):
                all_stats_dict["%s_trend" % stat_name] = self._compute_trend(
                    all_stats_dict.get(stat_name, 0), month_stats_dict.get(stat_name, 0)
                )
            # store statistics
    ...
    def _compute_statistics_linkedin(self, last_30d=False):
        ...
        if last_30d:
            ...
            endpoint += (
                "?timeIntervals=%s"
                % "(timeRange:(start:%i,end:%i),timeGranularityType:MONTH)"
                % (start, end)
            )

        response = requests.get(
            endpoint, params=params, headers=self._linkedin_bearer_headers(), timeout=5
        )
        ...
        return {
            ...
            "engagement": data.get("clickCount", 0)
    ...
    def create(self, vals_list):
        ...
        linkedin_accounts = res.filtered(
    ...
    def _get_linkedin_accounts(self, linkedin_access_token):
        ...
        response = requests.get(
            ...
            params={
                "q": "roleAssignee",
                "role": "ADMINISTRATOR",
                "projection": "(elements*(*,organization~(%s)))"
                % self.env["social.media"]._LINKEDIN_ORGANIZATION_PROJECTION,
            },
        ...
        if "elements" in response and isinstance(response.get("elements"), list):
            for organization in response.get("elements"):
                if organization.get("state") != "APPROVED":
                    continue
                image_url = self._extract_linkedin_picture_url(
                    organization.get("organization~")
                )
                image_data = (
                    requests.get(image_url, timeout=10).content if image_url else None
                )
                ...
                if account_urn not in accounts_urn:
                    ...
                    accounts.append(
                        {
                            "name": organization.get("organization~", {}).get(
                                "localizedName"
                            ),
            ...
            .search(
                [
                    ...
                    (
                        ...
                        "in",
                        [l.get("linkedin_account_urn") for l in linkedin_accounts],
                    ),
                ]
            )
        )
    ...
    def _create_default_stream_linkedin(self):
        ...
        page_posts_stream_type = self.env.ref(
            "social_linkedin.stream_type_linkedin_company_post"
        )
