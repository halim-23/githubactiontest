...
class SocialMediaLinkedin(models.Model):
    ...
    _LINKEDIN_ORGANIZATION_PROJECTION = (
    ...
    _LINKEDIN_COMMENT_PROJECTION = (
        "id,comments,$URN,content,message(text,attributes*(%s)),likesSummary,created(time, actor~person(%s)~organization(%s)),commentsSummary(totalFirstLevelComments,selectedComments(~comment(id,$URN,created)) )"
        % (
            _LINKEDIN_TAG_PROJECTION,
            _LINKEDIN_PERSON_PROJECTION,
            _LINKEDIN_ORGANIZATION_PROJECTION,
        )
    )
    _LINKEDIN_STREAM_POST_PROJECTION = (
        "id,totalShareStatistics,created(time), author~person(%s)~organization(%s), specificContent(com.linkedin.ugc.ShareContent(shareCommentary(text), media(originalUrl)))"
    ...
    def _action_add_account(self):
        ...
        linkedin_use_own_account = (
            ...
            .sudo()
            .get_param("social.linkedin_use_own_account")
        )
        linkedin_app_id = (
            self.env["ir.config_parameter"].sudo().get_param("social.linkedin_app_id")
        )
        linkedin_client_secret = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("social.linkedin_client_secret")
        )
    ...
    def _add_linkedin_accounts_from_configuration(self, linkedin_app_id):
        ...
        return {
            ...
            "url": "https://www.linkedin.com/oauth/v2/authorization?%s"
            % url_encode(params),
            "target": "self",
        }
    ...
    def _add_linkedin_accounts_from_iap(self):
        ...
        if iap_add_accounts_url == "unauthorized":
            ...
            iap_add_accounts_url == "linkedin_missing_configuration"
            or iap_add_accounts_url == "missing_parameters"
        ):
            raise UserError(
                _(
