...
class SocialStreamPostLinkedIn(models.Model):
    ...
    linkedin_author_id = fields.Char(
        "LinkedIn author ID", compute="_compute_linkedin_author_urn"
    )
    linkedin_author_vanity_name = fields.Char(
        "LinkedIn Vanity Name",
        help="Vanity name, used to generate a link to the author",
    )
    linkedin_author_image_url = fields.Char("LinkedIn author image URL")

    linkedin_comments_count = fields.Integer(
        "LinkedIn Comments", help="Number of comments"
    )
    ...
    def _compute_author_link(self):
        ...
        for post in linkedin_posts:
            if post.linkedin_author_urn:
                post.author_link = (
                    "https://linkedin.com/company/%s" % post.linkedin_author_id
                )
            else:
    ...
    def _compute_post_link(self):
        ...
        for post in linkedin_posts:
            if post.linkedin_post_urn:
                post.post_link = (
                    "https://www.linkedin.com/feed/update/%s" % post.linkedin_post_urn
                )
            else:
    ...
    def _linkedin_comment_add(self, message, comment_urn=None):
        ...
        response = requests.post(
            ...
            params={
                "projection": "(%s)"
                % self.env["social.media"]._LINKEDIN_COMMENT_PROJECTION
            },
            json=data,
    ...
    def _linkedin_comment_delete(self, comment_urn):
        comment_id = re.search(
            r"urn:li:comment:\(urn:li:activity:\w+,(\w+)\)", comment_urn
        ).group(1)
        endpoint = url_join(
            self.env["social.media"]._LINKEDIN_ENDPOINT,
            "socialActions/%s/comments/%s"
            % (quote(self.linkedin_post_urn), quote(comment_id)),
        )
    ...
    def _linkedin_comment_fetch(self, comment_urn=None, offset=0, count=20):
        ...
        comments = [
            self._linkedin_format_comment(comment)
            for comment in response.get("elements", [])
        ]
    ...
    def _linkedin_format_comment(self, json_data):
        ...
        data = {
            ...
            "from": {
                ...
                "name": self.stream_id._format_linkedin_name(
                    json_data.get("created", {}).get("actor~")
                ),
                "authorUrn": json_data.get("created", {}).get("actor"),
                "picture": self.account_id._extract_linkedin_picture_url(
                    json_data.get("created", {}).get("actor~")
                ),
                "vanityName": json_data.get("created", {})
                .get("actor~")
                .get("vanityName"),
                "isOrganization": "organization"
                in json_data.get("created", {}).get("actor", ""),
            },
            ...
            "formatted_created_time": self.env[
                ...
                datetime.fromtimestamp(
                    json_data.get("created", {}).get("time", 0) / 1000
                )
            ),
            "likes": {
                "summary": {
                    "total_count": json_data.get("likesSummary", {}).get(
                        "totalLikes", 0
                    ),
                    "can_like": False,
                    "has_liked": json_data.get("likesSummary", {}).get(
        ...
        sub_comments_count = json_data.get("commentsSummary", {}).get(
