...
class SocialStreamLinkedIn(models.Model):
    ...
    def _fetch_stream_data(self):
        ...
        projection = (
            "(paging,elements*(%s))"
            % self.env["social.media"]._LINKEDIN_STREAM_POST_PROJECTION
        )
        ...
        posts_response = requests.get(
            posts_endpoint,
            params={"q": "authors", "projection": projection, "count": 100},
        ...
        linkedin_post_data = {
            stream_post_data.get("id"): self._prepare_linkedin_stream_post_values(
                stream_post_data
            )
        ...
        stats_endpoint = url_join(
            self.env["social.media"]._LINKEDIN_ENDPOINT,
            "socialActions?ids=List(%s)"
            % ",".join([quote(urn) for urn in linkedin_post_data]),
        )
        ...
        if "results" in stats_response:
            for post_urn, post_data in stats_response["results"].items():
                linkedin_post_data[post_urn].update(
                    {
                        "linkedin_comments_count": post_data.get(
                            "commentsSummary", {}
                        ).get("totalFirstLevelComments", 0),
                        "linkedin_likes_count": post_data.get("likesSummary", {}).get(
                            "totalLikes", 0
                        ),
                    }
                )
    ...
    def _format_linkedin_name(self, json_data):
        user_name = "%s %s" % (
            json_data.get("localizedLastName", ""),
            json_data.get("localizedFirstName", ""),
        )
    ...
    def _prepare_linkedin_stream_post_values(self, data):
        ...
        post_values = {
            ...
            "published_date": datetime.fromtimestamp(
                data.get("created", {}).get("time", 0) / 1000
            ),
            "linkedin_post_urn": data.get("id"),
            "linkedin_author_urn": data.get("author"),
            "message": data.get("specificContent", {})
            .get("com.linkedin.ugc.ShareContent", {})
            .get("shareCommentary", {})
            .get("text"),
            "linkedin_author_image_url": self.account_id._extract_linkedin_picture_url(
                data.get("author~")
            ),
            "stream_post_image_ids": [(5, 0)]
            + [
                (0, 0, image_value)
                for image_value in self._extract_linkedin_image(medias)
            ],
        }
    ...
    def _extract_linkedin_image(self, medias):
        return [
            ...
            for media in medias
            if media.get("originalUrl")
            and "digitalmediaAsset" in media.get("media", "")
        ]
    ...
    def _extract_linkedin_article(self, medias):
        ...
        return {
            ...
            "link_image_url": self._enforce_url_scheme(
                (medias[0].get("thumbnails") or [{}])[0].get("url")
            ),
