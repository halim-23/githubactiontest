...
class CrossoveredBudgetLines(models.Model):
            ...
            for group_line in result:
                ...
                for budget_line_of_group in all_budget_lines_that_compose_group:
                    if "practical_amount" in fields or "percentage" in fields:
                        group_line[
                            "practical_amount"
                        ] += budget_line_of_group.practical_amount

                    if "theoritical_amount" in fields or "percentage" in fields:
                        group_line[
                            "theoritical_amount"
                        ] += budget_line_of_group.theoritical_amount

                    if "percentage" in fields:
                        if group_line["theoritical_amount"]:
                            # use a weighted average
                            group_line["percentage"] = float(
                                (group_line["practical_amount"] or 0.0)
                                / group_line["theoritical_amount"]
                            )
    ...
    def _onchange_dates(self):
        ...
        if domain_list:
            self.crossovered_budget_id = self.env["crossovered.budget"].search(
                AND(domain_list), limit=1
            )
