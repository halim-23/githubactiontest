...
class TestAccountBudgetCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.analytic_group_projects = cls.env["account.analytic.group"].create(
            {"name": "Projects"}
        )
        cls.analytic_group_departments = cls.env["account.analytic.group"].create(
            {"name": "Departments"}
        )
        ...
        cls.crossovered_budget_budgetpessimistic0 = cls.env[
            "crossovered.budget"
        ].create(
            {
        ...
        cls.account_budget_post_sales0 = cls.env["account.budget.post"].create(
            {
                "name": "Sales",
                "account_ids": [
                    (6, None, cls.company_data["default_account_revenue"].ids)
                ],
            }
        )
        ...
        cls.account_budget_post_purchase0 = cls.env["account.budget.post"].create(
            {
                "name": "Purchases",
                "account_ids": [
                    (6, None, cls.company_data["default_account_expense"].ids)
                ],
            }
        )
