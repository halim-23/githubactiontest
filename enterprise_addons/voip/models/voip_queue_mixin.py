...
class VoipQueueMixin(models.AbstractModel):
    ...
    has_call_in_queue = fields.Boolean(
        "Is in the Call Queue", compute="_compute_has_call_in_queue"
    )

    def _compute_has_call_in_queue(self):
        domain = self._linked_phone_call_domain()
        call_per_id = {
            call.activity_id.res_id: True
            for call in self.env["voip.phonecall"].search(domain)
        }
        for rec in self:
    ...
    def create_call_in_queue(self):
        ...
        phonecall_activity_type = self.env.ref(
            "mail.mail_activity_data_call", raise_if_not_found=False
        )
        if not phonecall_activity_type:
            phonecall_activity_type = self.env["mail.activity.type"].search(
                [("category", "=", "phonecall")], limit=1
            )
            if not phonecall_activity_type:
                phonecall_activity_type = self.env.ref(
                    "mail.mail_activity_todo", raise_if_not_found=False
                ) or self.env["mail.activity.type"].search(
                    [("category", "=", False)], limit=1
                )
                if phonecall_activity_type:
                    _logger.warning(
                        "No phonecall activity type found. VOIP activities aren't guaranteed to work as expected. Fallback on %s",
                        phonecall_activity_type.name,
                    )
                else:
                    _logger.warning(
                        "No phonecall or fallback activity type found. VOIP activities aren't guaranteed to work as expected."
                    )
        ...
        values_list = [
            {
                ...
                "activity_type_id": phonecall_activity_type
                and phonecall_activity_type.id,
                "user_id": self.env.user.id,
