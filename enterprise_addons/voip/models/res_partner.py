...
class Contact(models.Model):
    ...
    sanitized_phone = fields.Char(
        "Phone number sanitized", compute="_compute_sanitized_phone", store=True
    )
    sanitized_mobile = fields.Char(
        "Mobile number sanitized", compute="_compute_sanitized_mobile", store=True
    )
    ...
    def _compute_sanitized_phone(self):
        for partner in self.with_context(prefetch_fields=False):
            partner.sanitized_phone = (
                partner.phone and partner._voip_sanitization(partner.phone) or ""
            )

    @api.depends("mobile", "country_id")
    def _compute_sanitized_mobile(self):
        for partner in self.with_context(prefetch_fields=False):
            partner.sanitized_mobile = (
                partner.mobile and partner._voip_sanitization(partner.mobile) or ""
            )
