odoo.define("voip.UserAgent", function (require) {
  ...
  const UserAgent = Class.extend(mixins.EventDispatcherMixin, ServicesMixin, {
     ...
     *
     * @returns {Object} result user and pbx configuration return by the rpc
     */
    getPbxConfiguration() {
      ...
      if (this._callState !== CALL_STATE.NO_CALL) {
        if (this._callState === CALL_STATE.RINGING_CALL) {
          ...
          try {
            ...
            console.error(_.str.sprintf(_t("Cancel failed: %s"), err));
          }
        } else {
    ...
    makeCall(number) {
      ...
      if (this._mode === "demo") {
        ...
        this._timerAcceptedTimeout = this._demoTimeout(() => this._onAccepted());
     ...
     * @returns {Object} the initialised ua
     */
     ...
     * @private
     * @param {Object} params user and pbx configuration parameters
     * @returns {Object} the ua configuration parameters
     */
    _getUaConfig(params) {
    ...
    _initUserAgent(result) {
      ...
      if (this._mode === "prod") {
        ...
        this._userAgent.transport.ws.onerror = () =>
          this._triggerError(
    ...
    _makeCall(number) {
      ...
      try {
        ...
        this._triggerError(
          _.string.sprintf(
            _t(
              "The connection cannot be made.</br> Please check your configuration.</br> (Reason received: %s)"
            ),
            err.reason_phrase
          )
        );
    ...
    _updateCallState(newState) {
      ...
      switch (this._callState) {
        ...
        case CALL_STATE.REJECTING_CALL:
          ...
          break;
        default:
          // Don't update if call state set with an unknown value
          mode = false;
      }
      if (mode) {
     ...
     */
    _onErrorMicrophone() {
      this._triggerError(_t("Please Allow the use of the microphone"), {
        isTemporary: true,
      });
      this.hangup();
    },
    ...
    async _onInvite(inviteSession) {
      ...
      const numberSanitized = sanitizedPhone(
        inviteSession.remoteIdentity.uri.type,
        number
      );
    ...
    _onRejected(response) {
      ...
      if (
        ...
        this._triggerError(
          _.str.sprintf(_t(`Call rejected (reason: "%s")`), response.reasonPhrase),
          {isTemporary: true}
        );
        this.trigger_up("sip_cancel_outgoing");
      }
