  ...
  const PhoneCallContactsTab = PhoneCallTab.extend({
    ...
    async refreshPhonecallsStatus() {
      ...
      const contactsData = await this._rpc({
        model: "res.partner",
        method: "search_read",
        fields: ["display_name", "email", "id", "avatar_128", "mobile", "phone"],
    ...
    async searchPhoneCall(search) {
      if (search) {
        ...
        this._isLazyLoadFinished = false;
        const contactsData = await this._rpc({
          domain: ["|", ["phone", "!=", false], ["mobile", "!=", false]].concat(
            this._searchDomain
          ),
          fields: ["email", "display_name", "id", "avatar_128", "mobile", "phone"],
          limit: this._limit,
          method: "search_read",
          model: "res.partner",
          offset: this._offset,
    ...
    async _lazyLoadPhonecalls() {
      ...
      const dom = ["|", ["phone", "!=", false], ["mobile", "!=", false]].concat(
        this._searchDomain ? this._searchDomain : []
      );
      const contactsData = await this._rpc({
        model: "res.partner",
        method: "search_read",
