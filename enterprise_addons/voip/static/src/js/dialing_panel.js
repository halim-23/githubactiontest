  ...
  const DialingPanel = Widget.extend({
    ...
    async start() {
      ...
      this.call(
        ...
        "onNotification",
        this,
        this._onLongpollingNotifications
      );
    ...
    async _fetchMissedCallFromServer() {
      ...
      if (this._missedCounter > 0) {
        ...
        this.$(".o_dial_recent_tab .o_dial_tab, .tab-pane.o_dial_recent").addClass(
          "active"
        );
        this._activeTab = this._tabs.recent;
      }
    ...
    _resetMissedCalls(data = {data: {forceReset: false}}) {
      if (
        ...
        (data.data.forceReset || (this._isFolded && this._isShow))
      ) {
        this._missedCounter = 0;
        this._rpc({
          model: "res.users",
      ...
      const useVOIP = await new Promise((resolve) => {
        ...
        const processChoice = (useVoip) => {
          if ($checkbox.find('input[type="checkbox"]').is(":checked")) {
            this._userAgent.updateCallPreference(
              realSession.uid,
              useVoip ? "voip" : "phone"
            );
          }
        ...
        if (!hasMicroPhonePermission) {
          await new Promise((resolve) =>
            Dialog.alert(
              this,
              _t(
                "You must allow the access to the microphone on your device. Otherwise, the VoIP call receiver will not hear you."
              ),
              {
    ...
    _onSipCustomerUnavailable() {
      this.displayNotification({
        message: _t("Customer unavailable. Please try later."),
      });
    },
    /**
     * @private
