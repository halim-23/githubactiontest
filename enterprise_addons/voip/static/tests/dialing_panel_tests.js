  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("autocall flow", async function (assert) {
            ...
            if (args.model === "voip.phonecall") {
              ...
              switch (args.method) {
                case "get_next_activities_list":
                  ...
                  return self.phoneCallDetailsData.filter(
                    (phoneCallDetailData) =>
                ...
                case "create_from_rejected_call":
                  (self.phoneCallDetailsData.find((d) => d.id === id) || {}).state =
                    "pending";
        ...
        await testUtils.dom.click(dialingPanel.$(".o_dial_call_button"));
        assert.isVisible(
          dialingPanel.$(".o_phonecall_in_call").first(),
        ...
        assert.ok(dialingPanel._isInCall, "Should be in call");

        // Hangup call
        await testUtils.dom.click(dialingPanel.$(".o_dial_hangup_button"));
        assert.notOk(dialingPanel._isInCall, "Call should hang up");
        assert.containsOnce(
          dialingPanel,
          ".o_phonecall_details",
      ...
      QUnit.test("keyboard navigation on dial keypad input", async function (assert) {
        ...
        assert.strictEqual(
          dialingPanel.$(".o_phonecall_details .o_phonecall_info_name").text().trim(),
          "Call to 987654321",
          "Details should have been shown"
        );
        assert.ok(
          dialingPanel._isInCall,
          "should be in call on pressing ENTER after dialing a phone number"
        );
        ...
        assert.notOk(
          dialingPanel._isInCall,
          "should no longer be in call after hangup"
        );
      ...
      QUnit.test(
        "DialingPanel is closable with the BackButton in the mobile app",
