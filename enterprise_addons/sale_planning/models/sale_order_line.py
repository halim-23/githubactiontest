...
from odoo.tools import float_compare, float_is_zero, float_round, format_duration
...
class SaleOrderLine(models.Model):
    ...
    planning_hours_planned = fields.Float(
        compute="_compute_planning_hours_planned", store=True, compute_sudo=True
    )
    planning_hours_to_plan = fields.Float(
        compute="_compute_planning_hours_to_plan", store=True, compute_sudo=True
    )

    @api.depends(
        "product_uom", "product_uom_qty", "product_id.planning_enabled", "state"
    )
    def _compute_planning_hours_to_plan(self):
        sol_planning = self.filtered_domain(
            [
                ...
                if sol.product_uom == uom_hour or sol.product_uom == uom_unit:
                    ...
                    sol.planning_hours_to_plan = float_round(
                        sol.product_uom._compute_quantity(
                            sol.product_uom_qty, uom_hour, raise_if_failure=False
                        ),
                        precision_digits=2,
                    )
    ...
    def _compute_planning_hours_planned(self):
        ...
        if sol_planning:
            ...
            group_data = PlanningSlot.with_context(
                sale_planning_prevent_recompute=True
            ).read_group(
                [
            ...
            mapped_data = {
                data["sale_line_id"][0]: data["allocated_hours"] for data in group_data
            }
        ...
        self.env.add_to_compute(
            PlanningSlot._fields["allocated_hours"],
            PlanningSlot.search(
                [
                    ("start_datetime", "=", False),
    ...
    def name_get(self):
        res = super().name_get()
        with_planning_remaining_hours = self.env.context.get(
            "with_planning_remaining_hours"
        )
        ...
        for line in self:
            ...
            if line.product_id.planning_enabled:
                remaining_hours = (
                    line.planning_hours_to_plan - line.planning_hours_planned
                )
    ...
    def _planning_slot_generation(self):
        ...
        for so_line in self:
            if (
                so_line.product_id.type == "service"
    ...
    def _planning_slot_values(self):
        return {
            ...
            "allocated_hours": self.planning_hours_to_plan
