...
class SaleOrder(models.Model):
    ...
    planning_first_sale_line_id = fields.Many2one(
        "sale.order.line", compute="_compute_planning_first_sale_line_id"
    )
    planning_initial_date = fields.Date(compute="_compute_planning_initial_date")

    @api.depends(
        "order_line.planning_hours_to_plan", "order_line.planning_hours_planned"
    )
    def _compute_planning_hours(self):
        group_data = self.env["sale.order.line"].read_group(
            ...
            ["order_id", "planning_hours_to_plan", "planning_hours_planned"],
            ["order_id"],
        )
        mapped_data = defaultdict(
            lambda: {"planning_hours_to_plan": 0.0, "planning_hours_planned": 0.0}
        )
        mapped_data.update(
            {
                data["order_id"][0]: {
                    "planning_hours_to_plan": data["planning_hours_to_plan"],
                    "planning_hours_planned": data["planning_hours_planned"],
                }
                for data in group_data
            }
        )
        for order in self:
            order.planning_hours_planned = mapped_data[order.id][
                "planning_hours_planned"
            ]
            order.planning_hours_to_plan = (
                mapped_data[order.id]["planning_hours_to_plan"]
                - mapped_data[order.id]["planning_hours_planned"]
            )

    @api.depends(
        "order_line.product_id.planning_enabled",
        "order_line.planning_hours_to_plan",
        "order_line.planning_hours_planned",
    )
    ...
    def _compute_planning_initial_date(self):
        ...
        mapped_data = {
            data["sale_order_id"][0]: data["start_datetime"] for data in group_data
        }
        for order in self:
