...
class ProductTemplate(models.Model):
    ...
    def _check_planning_product_uom_is_time(self):
        ...
        if self.filtered(
            lambda product: product.planning_enabled
            and product.uom_id.category_id != time_uom_category
            and product.uom_id != unit_uom
        ):
            raise ValidationError(
                _(
                    "Plannable services should use an UoM within the %s category.",
                    time_uom_category.name,
                )
            )
    ...
    def _check_planning_product_is_service(self):
        invalid_products = self.filtered(
            lambda product: product.planning_enabled and product.type != "service"
        )
        if invalid_products:
            raise ValidationError(
                _(
