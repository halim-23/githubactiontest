...
class PosOrder(models.Model):
    ...
    def _check_config_germany_floor(self, session_id=None, order_id=None):
        ...
        return bool(
            config
            and config.company_id.l10n_de_is_germany_and_fiskaly()
            and config.floor_ids
        )
    ...
    def _order_fields(self, ui_order):
        ...
        if (
            self._check_config_germany_floor(session_id=ui_order["pos_session_id"])
            and "l10n_de_fiskaly_time_start" not in fields
        ):
            fields["l10n_de_fiskaly_time_start"] = ui_order["creation_date"].replace(
                "T", " "
            )[:19]
        return fields
    ...
    def create_from_ui(self, orders, draft=False):
        if (
            self._check_config_germany_floor(
            ...
            for ui_order in orders:
                ...
                if ui_order["data"].get("server_id"):
                    existing_order = (
                        self.env["pos.order"]
                        .browse(ui_order["data"]["server_id"])
                        .exists()
                    )
                if not existing_order or existing_order.state == "draft":
                    differences = self._line_differences(
                        existing_order, ui_order["data"]
                    )
                    if differences:
            ...
            for json_record in res:
                if json_record["pos_reference"] in order_differences:
                    json_record["differences"] = order_differences[
                        json_record["pos_reference"]
                    ]
            return res
        else:
    ...
    def _line_differences(self, existing_order, ui_order=None):
        ...
        new_line_dict = (
            {}
            if not ui_order
            else self._merge_order_lines([line[2] for line in ui_order["lines"]])
        )
        old_lines = []
        if existing_order:
            old_lines = existing_order.lines.read(
                [
                    "qty",
                    "product_id",
                    "full_product_name",
    ...
    def retrieve_line_difference(self, ui_orders):
        ...
        for order in ui_orders:
            existing_order = (
                None if not order.get("server_id") else self.browse(order["server_id"])
            )
