...
class TestPickingBarcodeClientAction(TestBarcodeClientAction):
    def test_internal_picking_from_scratch_1(self):
        ...
        with patch(
            ...
            new=picking_write_mock,
        ):
            self.start_tour(
                url, "test_internal_picking_from_scratch_1", login="admin", timeout=180
            )
    ...
    def test_internal_picking_from_scratch_2(self):
        ...
        self.start_tour(
            url, "test_internal_picking_from_scratch_2", login="admin", timeout=180
        )

        self.assertEqual(len(internal_picking.move_line_ids), 4)
        prod1_ml = internal_picking.move_line_ids.filtered(
            lambda ml: ml.product_id.id == self.product1.id
        )
        prod2_ml = internal_picking.move_line_ids.filtered(
            lambda ml: ml.product_id.id == self.product2.id
        )
    ...
    def test_internal_picking_reserved_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.shelf1, 1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.shelf1, 1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.shelf3, 1
        )
        ...
        with patch(
            ...
            self.start_tour(
                url, "test_internal_picking_reserved_1", login="admin", timeout=180
            )
    ...
    def test_internal_picking_change_location(self):
        ...
        # Creates some quants.
        self.env["stock.quant"]._update_available_quantity(
            self.product1, sub_location_1, 1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, sub_location_2, 1
        )
        ...
        self.start_tour(
            url, "test_internal_change_location", login="admin", timeout=180
        )
    ...
    def test_receipt_from_scratch_with_lots_1(self):
        ...
        self.start_tour(
            url, "test_receipt_from_scratch_with_lots_1", login="admin", timeout=180
        )
        self.assertEqual(
            receipt_picking.move_line_ids.mapped("lot_name"), ["lot1", "lot2"]
        )
    ...
    def test_receipt_from_scratch_with_lots_2(self):
        ...
        url = self._get_client_action_url(receipt_picking.id)
        self.start_tour(
            url, "test_receipt_from_scratch_with_lots_2", login="admin", timeout=180
        )
        self.assertEqual(
            receipt_picking.move_line_ids.mapped("lot_name"), ["lot1", "lot2"]
        )
    ...
    def test_receipt_from_scratch_with_lots_3(self):
        ...
        url = self._get_client_action_url(receipt_picking.id)
        self.start_tour(
            url, "test_receipt_from_scratch_with_lots_3", login="admin", timeout=180
        )
    ...
    def test_receipt_from_scratch_with_lots_4(self):
        ...
        url = self._get_client_action_url(receipt_picking.id)
        self.start_tour(
            url, "test_receipt_from_scratch_with_lots_4", login="admin", timeout=180
        )
        move_lines = receipt_picking.move_line_ids
    ...
    def test_receipt_from_scratch_with_lots_5(self):
        ...
        url = self._get_client_action_url(receipt_picking.id)
        self.start_tour(
            url, "test_receipt_from_scratch_with_sn_1", login="admin", timeout=180
        )
        move_lines = receipt_picking.move_line_ids
        self.assertEqual(move_lines[0].product_id.id, self.productserial1.id)
    ...
    def test_receipt_reserved_1(self):
        ...
        with patch(
            ...
            self.assertEqual(
                receipt_picking.move_line_ids[0].location_dest_id.id, shelf1.id
            )
            self.assertEqual(
                receipt_picking.move_line_ids[1].location_dest_id.id, shelf1.id
            )
    ...
    def test_delivery_lot_with_package(self):
        ...
        self.start_tour(
            url, "test_delivery_lot_with_package", login="admin", timeout=180
        )
    ...
    def test_delivery_reserved_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.stock_location, 4
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.stock_location, 4
        )
    ...
    def test_delivery_reserved_3(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.stock_location, 2
        )
    ...
    def test_delivery_from_scratch_1(self):
        ...
        self.start_tour(
            url, "test_delivery_from_scratch_with_lots_1", login="admin", timeout=180
        )
    ...
    def test_delivery_reserved_lots_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productlot1, self.stock_location, 1, lot_id=lot1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productlot1, self.stock_location, 2, lot_id=lot2
        )
        ...
        self.start_tour(
            url, "test_delivery_reserved_lots_1", login="admin", timeout=180
        )
    ...
    def test_delivery_different_products_with_same_lot_name(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productlot1, self.stock_location, 2, lot_id=lot1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productlot2, self.stock_location, 2, lot_id=lot2
        )
        ...
        self.start_tour(
            url,
            "test_delivery_different_products_with_same_lot_name",
            login="admin",
            timeout=180,
        )
    ...
    def test_delivery_from_scratch_sn_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productserial1, self.stock_location, 1, lot_id=sn1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productserial1, self.stock_location, 1, lot_id=sn2
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productserial1, self.stock_location, 1, lot_id=sn3
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productserial1, self.stock_location, 1, lot_id=sn4
        )
        ...
        self.start_tour(
            url, "test_delivery_reserved_with_sn_1", login="admin", timeout=180
        )
    ...
    def test_delivery_using_buttons(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.stock_location, 3
        )
        self.env["stock.quant"]._update_available_quantity(
            product3, self.stock_location, 4
        )
        ...
        self.assertEqual(
            delivery_picking.move_line_ids.mapped("qty_done"), [2, 3, 4, 2]
        )
    ...
    def test_receipt_reserved_lots_multiloc_1(self):
        ...
        self.start_tour(
            url, "test_receipt_reserved_lots_multiloc_1", login="admin", timeout=180
        )
    ...
    def test_pack_multiple_scan(self):
        ...
        package = self.env["stock.quant.package"].search([("name", "=", "PACK0001000")])
    ...
    def test_pack_common_content_scan(self):
        ...
        self.start_tour(
            url, "test_pack_common_content_scan", login="admin", timeout=180
        )
    ...
    def test_pack_multiple_location_02(self):
        ...
        self.start_tour(
            url, "test_pack_multiple_location_02", login="admin", timeout=180
        )
    ...
    def test_put_in_pack_from_multiple_pages(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.shelf2, 1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.shelf2, 1
        )
        ...
        self.start_tour(
            url, "test_put_in_pack_from_multiple_pages", login="admin", timeout=180
        )
    ...
    def test_duplicate_serial_number(self):
        ...
        self.start_tour(
            url, "test_receipt_duplicate_serial_number", login="admin", timeout=180
        )

        self.start_tour(
            url, "test_delivery_duplicate_serial_number", login="admin", timeout=180
        )
    ...
    def test_bypass_source_scan(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productlot1, self.shelf1, 2, lot_id=lot1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productserial1, self.shelf2, 1, lot_id=lot2
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.shelf2, 4, package_id=pack1
        )
    ...
    def test_put_in_pack_from_different_location(self):
        ...
        self.start_tour(
            url, "test_put_in_pack_from_different_location", login="admin", timeout=180
        )
        pack = self.env["stock.quant.package"].search([])[-1]
        self.assertEqual(len(pack.quant_ids), 2)
    ...
    def test_put_in_pack_scan_package(self):
        ...
        self.start_tour(
            url, "test_put_in_pack_scan_package", login="admin", timeout=180
        )
    ...
    def test_highlight_packs(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.stock_location, 4, package_id=pack1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.stock_location, 4, package_id=pack1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.stock_location, 2, package_id=pack2
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.stock_location, 2, package_id=pack2
        )
    ...
    def test_picking_owner_scan_package(self):
        ...
        self.start_tour(
            url, "test_picking_owner_scan_package", login="admin", timeout=180
        )
    ...
    def test_gs1_reserved_delivery(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_gtin_8, self.stock_location, 99
        )
    ...
    def test_gs1_receipt_conflicting_barcodes(self):
        ...
        self.start_tour(
            url, "test_gs1_receipt_conflicting_barcodes_1", login="admin", timeout=180
        )
        ...
        self.start_tour(
            url, "test_gs1_receipt_conflicting_barcodes_2", login="admin", timeout=180
        )
        ...
        self.start_tour(
            url, "test_gs1_receipt_conflicting_barcodes_3", login="admin", timeout=180
        )
    ...
    def test_gs1_receipt_quantity_with_uom(self):
        ...
        url = self._get_client_action_url(receipt.id)
        self.start_tour(
            url, "test_gs1_receipt_quantity_with_uom", login="admin", timeout=180
        )
    ...
    def test_gs1_package_receipt_and_delivery(self):
        ...
        sequence = self.env["ir.sequence"].search(
            [("code", "=", "stock.quant.package")], limit=1
        )
        ...
        package = self.env["stock.quant.package"].search(
            [("name", "=", "546879213579461324")]
        )
        package2 = self.env["stock.quant.package"].search(
            [("name", "=", "130406658041178543")]
        )
        package3 = self.env["stock.quant.package"].search(
            [("name", "=", "PACK0000123")]
        )
...
class TestInventoryAdjustmentBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_inventory_adjustment_multi_location(self):
        ...
        self.start_tour(
            url, "test_inventory_adjustment_multi_location", login="admin", timeout=180
        )
        ...
        self.assertEqual(
            inventory_moves.mapped("state"), ["done", "done", "done", "done"]
        )
        inventory_move_in_WH_stock = inventory_moves.filtered(
            lambda l: l.location_dest_id == self.stock_location
        )
        self.assertEqual(
            set(inventory_move_in_WH_stock.mapped("product_id")),
            {self.product1, self.product2},
        )
        self.assertEqual(
            inventory_move_in_WH_stock.filtered(
        ...
        inventory_move_in_shelf1 = inventory_moves.filtered(
            lambda l: l.location_dest_id == self.shelf1
        )
        ...
        inventory_move_in_shelf2 = inventory_moves.filtered(
            lambda l: l.location_dest_id == self.shelf2
        )
    ...
    def test_inventory_adjustment_tracked_product(self):
        ...
        self.start_tour(
            url, "test_inventory_adjustment_tracked_product", login="admin", timeout=180
        )
        ...
        self.assertEqual(
            inventory_moves.mapped("state"),
            ["done", "done", "done", "done", "done", "done"],
        )

        moves_with_lot = inventory_moves.filtered(
            lambda l: l.product_id == self.productlot1
        )
        ...
        moves_with_sn = inventory_moves.filtered(
            lambda l: l.product_id == self.productserial1
        )
        ...
        self.assertEqual(
            mls_with_lot.filtered(lambda ml: ml.lot_id.name == "lot1").qty_done, 3
        )
        self.assertEqual(
            mls_with_lot.filtered(lambda ml: ml.lot_id.name == "lot2").qty_done, 1
        )
        self.assertEqual(
            mls_with_lot.filtered(lambda ml: ml.lot_id.name == "lot3").qty_done, 1
        )
        self.assertEqual(
            set(mls_with_sn.mapped("lot_id.name")),
    ...
    def test_inventory_package(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.stock_location, 7, package_id=pack
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.stock_location, 3, package_id=pack
        )
    ...
    def test_inventory_owner_scan_package(self):
        ...
        self.start_tour(
            url, "test_inventory_owner_scan_package", login="admin", timeout=180
        )

        inventory_moves = self.env["stock.move"].search(
            [("product_id", "=", self.product1.id), ("is_inventory", "=", True)]
        )
    ...
    def test_inventory_using_buttons(self):
        ...
        productlot1_quant = self.env["stock.quant"].search(
            [("product_id", "=", self.productlot1.id), ("quantity", ">", 0)]
        )
        self.assertEqual(len(product1_quant), 1)
    ...
    def test_gs1_inventory_product_units(self):
        ...
        self.start_tour(
            url, "test_gs1_inventory_product_units", login="admin", timeout=180
        )
    ...
    def test_gs1_inventory_package(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product1, self.shelf1, 8, package_id=pack_1
        )
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.shelf2, 6, package_id=pack_2
        )
        ...
        pack_3 = self.env["stock.quant.package"].search(
            [("name", "=", "122333444455555670")]
        )
    ...
    def test_gs1_inventory_lot_serial(self):
        ...
        url = "/web#action=" + str(action_id.id)
        self.start_tour(
            url, "test_gs1_inventory_lot_serial", login="admin", timeout=180
        )
        ...
        self.assertEqual(
            smls_lot.lot_id.mapped("name"), ["LOT-AAA", "LOT-AAB", "LOT-AAC"]
        )
