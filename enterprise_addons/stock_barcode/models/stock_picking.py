...
class StockPicking(models.Model):
    ...
    def _get_stock_barcode_data(self):
        ...
        data = {
            "records": {
                "stock.picking": self.read(
                    self._get_fields_stock_barcode(), load=False
                ),
                ...
                "res.partner": owners.read(
                    owners._get_fields_stock_barcode(), load=False
                ),
                "stock.location": locations.read(
                    locations._get_fields_stock_barcode(), load=False
                ),
                ...
                "stock.production.lot": lots.read(
                    lots._get_fields_stock_barcode(), load=False
                ),
        ...
        for picking in data["records"]["stock.picking"]:
            picking["note"] = (
                False
                if is_html_empty(picking["note"])
                else html2plaintext(picking["note"])
            )
        return data
    ...
    def _check_source_package(self, package):
        corresponding_po = self.move_line_ids.filtered(
            lambda r: r.package_id.id == package.id
            and r.result_package_id.id == package.id
        )
        for po in corresponding_po:
            po.qty_done = po.product_uom_qty
        if corresponding_po:
            self.package_level_ids_details.filtered(
                lambda p: p.package_id == package
            ).is_done = True
            return True
        else:
            return False
    ...
    def _check_destination_location(self, location):
        ...
        corresponding_ml = self.move_line_ids.filtered(
            lambda ml: not ml.location_processed
            and float_compare(
                ml.qty_done, 0, precision_rounding=ml.product_uom_id.rounding
            )
            == 1
    ...
    def _create_new_picking(self, picking_type):
        ...
        (
            location_dest_id,
            location_id,
        ) = picking_type.warehouse_id._get_partner_locations()
    ...
    def on_barcode_scanned(self, barcode):
        if not self.env.company.nomenclature_id:
            ...
            if product_packaging:
                if self._check_product(
                    product_packaging.product_id, product_packaging.qty
                ):
                    return
            ...
            location = self.env["stock.location"].search(
                ["|", ("name", "=", barcode), ("barcode", "=", barcode)], limit=1
            )
            ...
            product_packaging = self.env["product.packaging"].search(
                [("barcode", "=", parsed_result["code"])], limit=1
            )
            if product_packaging:
                if self._check_product(
                    product_packaging.product_id, product_packaging.qty
        ...
        return {
            "warning": {
                ...
                "message": _(
                    'The barcode "%(barcode)s" doesn\'t correspond to a proper product, package or location.'
                )
                % {"barcode": barcode},
            }
        }
        ...
        if product:
            ...
            if not self.search_count(
                ...
                return {
                    "warning": {
                        "title": _(
                            "No %s ready for this product", picking_type["name"]
                        ),
                    }
                }
            action = self.env["ir.actions.actions"]._for_xml_id(
                "stock_barcode.stock_picking_action_kanban"
            )
