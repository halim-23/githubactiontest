odoo.define("stock_barcode.tourHelper", function (require) {
   ...
   */
  function assertLineButtonsAreVisible(
    lineIndex,
    isVisible,
    cssSelector = ".o_line_button"
  ) {
  ...
  function assertPreviousVisible(expected) {
    ...
    var current =
      (!$previousButton.length && !expected) || $previousButton.hasClass("o_hidden");
  ...
  function assertPreviousEnabled(expected) {
    var $previousButton = $(".o_previous_page");
    var current =
      (!$previousButton.length && !expected) || $previousButton.prop("disabled");
  ...
  function assertNextVisible(expected) {
    ...
    var current =
      (!$nextButton.length && !expected) || $nextButton.hasClass("o_hidden");
  ...
  function assertLinesCount(expected) {
    const current = document.querySelectorAll(".o_barcode_lines > .o_barcode_line")
      .length;
  ...
  function assertDestinationLocationHighlight(expected) {
    ...
    assert(
      $locationElem.hasClass("o_strong"),
      expected,
      "Location destination is not bold"
    );
  }
  ...
  function assertLineQty($line, qty) {
    assert(
      $line[0].querySelector(".qty-done,.inventory_quantity").innerText,
      qty,
      "line quantity is wrong"
    );
  }
  ...
  function pressShift() {
    document
      .querySelector(".o_barcode_client_action")
      .dispatchEvent(
        new window.KeyboardEvent("keydown", {bubbles: true, key: "Shift"})
      );
  }
