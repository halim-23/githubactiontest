...
tour.register("test_gs1_inventory_lot_serial", {test: true}, [
  ...
  },
  {
    ...
    run: function () {
      ...
      helper.assert(
        line.querySelector(".product-label").textContent,
        "PRO_GTIN_12_lot"
      );
  ...
  },
  {
    ...
    run: function () {
      ...
      helper.assert(
        parentLine.querySelector(".product-label").textContent,
        "PRO_GTIN_12_lot"
      );
...
tour.register("test_gs1_package_receipt", {test: true}, [
  ...
  {
    trigger:
      '.o_barcode_line[data-barcode="584687955629"]:contains(546879213579461324)',
    run: function () {
      helper.assertLinesCount(2);
      const $line1 = helper.getLine({barcode: "82655853"});
      helper.assertLineIsHighlighted($line1, false);
