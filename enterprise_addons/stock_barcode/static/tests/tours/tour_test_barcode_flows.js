odoo.define("test_barcode_flows.tour", function (require) {
  ...
  tour.register("test_internal_picking_from_scratch_1", {test: true}, [
...

    {
      ...
      run: function () {
        ...
        helper.assert(
          $line.find(
            ".o_barcode_line_details > .o_barcode_line_title > .o_barcode_product_ref"
          ).length,
          1
        );
  ...
  tour.register("test_internal_change_location", {test: true}, [
    {
      ...
      run: function () {
        helper.assertPageSummary(
          "From Stock House/Abandonned Ground Floor To Stock House"
        );
        helper.assertPreviousVisible(true);
        helper.assertPreviousEnabled(true);
        helper.assertNextVisible(true);
    ...
    },
    {
      ...
      run: function () {
        const srcLocationList = document.querySelector(
          ".o_barcode_list_locations.o_source_locations"
        );
        ...
        const destLocationList = document.querySelector(
          ".o_barcode_list_locations.o_destination_locations"
        );
    ...
    },
    {
      ...
      run: function () {
        helper.assertPageSummary(
          "From Stock House/Abandonned Ground Floor To Stock House/Poorly lit floor"
        );
        helper.assertPreviousVisible(true);
        helper.assertPreviousEnabled(true);
        helper.assertNextVisible(true);
  ...
  tour.register("test_delivery_lot_with_package", {test: true}, [
    ...
    {trigger: ".o_line_button.o_toggle_sublines"},
    {
      ...
      run: function () {
        ...
        helper.assert(
          $line1.find(".fa-archive").parent().text().includes("pack_sn_1"),
          true
        );
        helper.assert($line2.find(".o_line_lot_name").text(), "sn2");
        helper.assert(
          $line2.find(".fa-archive").parent().text().includes("pack_sn_1"),
          true
        );
      },
    },
  ...
  tour.register("test_delivery_using_buttons", {test: true}, [
    {
      ...
      run: function () {
        ...
        helper.assertValidateEnabled(true);
        helper.assert(
          $(".o_line_button[name=incrementButton]").length,
          3,
  ...
  tour.register("test_inventory_adjustment_tracked_product", {test: true}, [
    ...
    {
      trigger:
        '.o_barcode_line:contains("productlot1") .o_sublines .o_barcode_line:nth-child(3)',
      run: function () {
        helper.assertLinesCount(2);
        helper.assertSublinesCount(3);
      },
  ...
  tour.register("test_put_in_pack_from_different_location", {test: true}, [
...

    {
      ...
      run: function () {
        ...
        helper.assert(
          $line.find(".fa-archive").length,
          1,
          "Expected a 'fa-archive' icon for assigned pack"
        );
      },
    },
    // Scans dest. location.
  ...
  tour.register("test_inventory_using_buttons", {test: true}, [
    ...
    {trigger: ".o_barcode_client_action", run: "scan productserial1"},
    {
      ...
      run: function () {
        ...
        const setButton = document.querySelector(
          ".o_selected .o_line_button.o_set > .fa-check"
        );
    ...
    {trigger: ".o_barcode_client_action", run: "scan BNG-118"},
    {
      ...
      run: function () {
        ...
        helper.assertButtonIsNotVisible($line, "remove_unit");
        const setButton = document.querySelector(
          ".o_selected .o_line_button.o_set.o_difference"
        );
        helper.assert(Boolean(setButton), true);
      },
    },
    ...
    {trigger: '.o_barcode_line:contains("productserial1") .o_line_button.o_set'},
    {
      ...
      run: function () {
        ...
        helper.assertButtonIsNotVisible($line, "add_quantity");
        helper.assertButtonIsNotVisible($line, "remove_unit");
        const goodQuantitySetButton = document.querySelector(
          ".o_selected .o_line_button.o_set > .fa-check"
        );
        helper.assert(Boolean(goodQuantitySetButton), true);
        const differenceSetButton = document.querySelector(
          ".o_selected .o_line_button.o_set.o_difference"
        );
    ...
    {trigger: '.o_barcode_line:contains("productserial1") .o_line_button.o_set'},
    {
      ...
      run: function () {
        ...
        helper.assert(Boolean(differenceSetButton), false);
        const emptySetButton = document.querySelector(
          ".o_selected .o_line_button.o_set"
        );
