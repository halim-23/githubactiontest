...
export default class BarcodeModel extends owl.core.EventBus {
  ...
  get barcodeInfo() {
    ...
    if (!line && this.lastScannedPackage) {
      ...
      line = lines.find(
        (l) => l.package_id && l.package_id.id === this.lastScannedPackage
      );
    }
    ...
    if (this.pageIndex === 0 && !line) {
      ...
      // Message depends of the selected line.
      ...
      if (this._lineIsNotComplete(line)) {
        ...
        if (tracking !== "none" && !trackingNumber) {
          // Line is waiting a tracking number.
          if (tracking === "lot") {
            this.messageType = "scan_lot";
          } else {
            this.messageType = "scan_serial";
      ...
      // Message depends of the operation.
    ...
    switch (this.messageType) {
      ...
      case "scan_product_or_src":
        barcodeInformations.message = _t(
          "Scan more products, or scan a new source location"
        );
        break;
      case "scan_product_or_dest":
        barcodeInformations.message = _t(
          "Scan more products, or scan the destination location"
        );
        break;
      case "scan_lot":
        barcodeInformations.message = _t("Scan the lot number of the product");
        break;
      case "scan_serial":
        barcodeInformations.message = _t("Scan the serial number of the product");
        break;
    }
  ...
  get lastScannedLine() {
    if (this.scannedLinesVirtualId.length) {
      const virtualId = this.scannedLinesVirtualId[
        this.scannedLinesVirtualId.length - 1
      ];
  ...
  get previousScannedLines() {
    ...
    if (this.lastScannedPackage) {
      lines.push(
        ...this.currentState.lines.filter(
          (l) => l.package_id.id === this.lastScannedPackage
        )
      );
    }
    return lines;
  ...
  get viewsWidgetData() {
    // shouldn't be override by the child model instead of set lineModel and viewReference
    return {
  ...
  async changeSourceLocation(id, applyChangeToPageLines = false) {
    ...
    const refDestLocationId = applyChangeToPageLines
      ? this.currentDestLocationId
      : this._defaultDestLocationId();
    ...
    if (currentPage && this.currentLocationId === currentPage.sourceLocationId) {
      ...
      // Otherwise, searches for a page with these src./dest. locations.
      for (let i = 0; i < this.pages.length; i++) {
        const page = this.pages[i];
        if (
          page.sourceLocationId === this.currentLocationId &&
  ...
  displayBarcodeLines(recordId) {
    ...
    if (recordId) {
      // If we pass a record id...
      if (this.pages.length > 1) {
        // ... looks to go to this record's page...
        for (let i = 0; i < this.pages.length; i++) {
  ...
  async updateLotName(line, lotName) {
    ...
    for (const l of this.pageLines) {
      ...
      if (lotName === l.lot_name || (l.lot_id && lotName === l.lot_id.name)) {
        this.notification.add(_t("This serial number is already used."), {
          type: "warning",
        });
  ...
  async validate() {
    await this.save();
    const action = await this.orm.call(this.params.model, this.validateMethod, [
      this.recordIds,
    ]);
    const options = {
      on_close: (ev) => {
  ...
  async _createNewLine(params) {
    if (
      params.fieldsParams &&
      params.fieldsParams.uom &&
      params.fieldsParams.product_id
    ) {
      let productUOM = this.cache.getRecord(
        "uom.uom",
  ...
  _groupLinesByPage(state) {
    ...
    for (const line of state.lines) {
      // Groups the barcode lines by src/dest locations.
    ...
    if (pages.length === 0) {
      // If no pages, creates a default one.
      const page = {
        index: pages.length,
        lines: [],
        sourceLocationId: this.currentLocationId,
  ...
  async _parseBarcode(barcode, filters) {
      ...
      if (parsedBarcode.length) {
        ...
        for (const data of parsedBarcode) {
          ...
          if (["location", "location_dest"].includes(rule.type)) {
            const location = await this.cache.getRecordByBarcode(
              value,
              "stock.location"
            );
            if (!location) {
            ...
            if (
              rule.type === "location_dest" ||
              this.messageType === "scan_product_or_dest"
            ) {
            ...
            if (this.useExistingLots) {
              result.lot = await this.cache.getRecordByBarcode(
                value,
            ...
            const stockPackage = await this.cache.getRecordByBarcode(
              value,
              "stock.quant.package"
            );
            if (stockPackage) {
            ...
            const packageType = await this.cache.getRecordByBarcode(
              value,
              "stock.package.type"
            );
            if (packageType) {
            ...
            const product = await this.cache.getRecordByBarcode(
              value,
              "product.product"
            );
            if (product) {
            ...
            if (this.groups.group_uom) {
              result.uom = await this.cache.getRecord(
                "uom.uom",
                rule.associated_uom_id
              );
            }
    ...
    const recordByData = await this.cache.getRecordByBarcode(
      barcode,
      false,
      false,
      filters
  ...
  async _processBarcode(barcode) {
    ...
    if (barcodeData.packaging) {
      barcodeData.product = this.cache.getRecord(
        "product.product",
        barcodeData.packaging.product_id
      );
    ...
    if (currentLine) {
      ...
      // No line found. If it's possible, creates a new line.
      const fieldsParams = this._convertDataToFieldsParams({
        product,
        qty: barcodeData.quantity,
        lotName: barcodeData.lotName,
  ...
  _findLine(barcodeData) {
    ...
    for (const line of this.pageLines) {
      ...
      foundLine =
        this.selectedLine && this.selectedLine.virtual_id === line.virtual_id
          ? line
          : foundLine || line;
    }
    return foundLine;
  ...
  _shouldSearchForAnotherLine(line, barcodeData) {
    ...
    const parentLine = groupLines.find(
      (gl) => gl.virtual_ids.indexOf(line.virtual_id) !== -1
    );
    if (parentLine) {
