...
export default class BarcodePickingModel extends BarcodeModel {
    ...
    if (moveScannedLineOnly && this.previousScannedLines.length) {
      ...
      for (const line of this.previousScannedLines) {
        ...
        if (!line.product_uom_qty || line.qty_done >= line.product_uom_qty) {
          ...
          // ... otherwise, we split it to a new line.
  ...
  async updateLine(line, args) {
    ...
    if (result_package_id) {
      if (typeof result_package_id === "number") {
        result_package_id = this.cache.getRecord(
          "stock.quant.package",
          result_package_id
        );
        if (
          result_package_id.package_type_id &&
          typeof result_package_id === "number"
        ) {
  ...
  async _cancel() {
    await this.save();
    await this.orm.call(this.params.model, "action_cancel", [[this.params.id]]);
  ...
  _createLinesState() {
    ...
    for (const id of picking.move_line_ids) {
      ...
      smlData.virtual_id =
        Number(smlData.dummy_id) || previousVirtualId || this._uniqueVirtualId;
      ...
      smlData.lot_id =
        smlData.lot_id && this.cache.getRecord("stock.production.lot", smlData.lot_id);
      smlData.owner_id =
        smlData.owner_id && this.cache.getRecord("res.partner", smlData.owner_id);
      smlData.package_id =
        smlData.package_id &&
        this.cache.getRecord("stock.quant.package", smlData.package_id);
      const resultPackage =
