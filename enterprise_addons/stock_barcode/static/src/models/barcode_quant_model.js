...
export default class BarcodeQuantModel extends BarcodeModel {
  ...
  async apply() {
    ...
    const notifyAndGoAhead = (res) => {
      ...
      if (this.pages.length > 1) {
        // Stay in inventory if there is multiple pages.
        this.notification.add(_t("Inventory adjustment was saved"), {type: "success"});
        this.trigger("refresh");
      } else {
        // Go back to the menu if it's only one page.
        this.notification.add(_t("The inventory adjustment has been validated"), {
          type: "success",
        });
  ...
  setOnHandQuantity(line) {
    if (line.product_id.tracking === "serial") {
      // Special case for product tracked by SN.
  ...
  async _createNewLine(params) {
    ...
    if (product.detailed_type != "product") {
      ...
      const message = sprintf(
        _t("%s can't be inventoried. Only storable products can be inventoried."),
        productName
      );
    ...
    if (product.tracking !== "none") {
      if (params.fieldsParams.lot_name) {
        ...
        // Search for a quant with no lot.
  ...
  async _updateLotName(line, lotName) {
    ...
    if (existingQuant.length) {
      ...
      if (line.lot_id) {
        line.lot_id = await this.cache.getRecordByBarcode(
          lotName,
  ...
  _createLinesState() {
    ...
    const lines = [];
    for (const id of Object.keys(this.cache.dbIdCache["stock.quant"]).map((id) =>
      Number(id)
    )) {
      ...
      const prevLine =
        this.currentState && this.currentState.lines.find((l) => l.id === id);
      ...
      quant.lot_id =
        quant.lot_id && this.cache.getRecord("stock.production.lot", quant.lot_id);
      quant.package_id =
        quant.package_id &&
        this.cache.getRecord("stock.quant.package", quant.package_id);
      quant.owner_id =
        quant.owner_id && this.cache.getRecord("res.partner", quant.owner_id);
  ...
  _getPrintOptions() {
    ...
    const quantsToPrint = this.pageLines.filter(
      (quant) => quant.inventory_quantity_set
    );
