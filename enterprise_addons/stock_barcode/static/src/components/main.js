...
class MainComponent extends Component {
  ...
  async render() {
    ...
    let selectedLine = document.querySelector(
      ".o_sublines .o_barcode_line.o_highlight"
    );
    if (!selectedLine) {
    ...
    if (selectedLine) {
      ...
      if (scrollCoordY !== false) {
        // Scrolls to the line only if it's not entirely visible.
  ...
  async cancel() {
    ...
    const action = await this.orm.call(this.props.model, "action_cancel_from_barcode", [
      [this.props.id],
    ]);
    const onClose = (res) => {
  ...
  async print(action, method) {
    ...
    if (!action && method) {
      action = await this.orm.call(this.props.model, method, [[this.props.id]]);
    }
