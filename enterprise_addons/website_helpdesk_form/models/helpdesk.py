...
class HelpdeskTeam(models.Model):
    ...
    def unlink(self):
        teams_with_submit_form = self.filtered(
            lambda t: t.website_form_view_id is not False
        )
    ...
    def _ensure_submit_form_view(self):
        for team in self:
            if not team.website_form_view_id:
                default_form = etree.fromstring(
                    self.env.ref("website_helpdesk_form.ticket_submit_form").arch
                )
