...
class TestLeadConvertToTicket(crm_common.TestCrmCommon):
    ...
    def assertTicketLeadConvertData(self, ticket, lead, team, ticket_type, partner):
        ...
        self.assertEqual(
            ticket.email, lead.email_from if lead.email_from else partner.email
        )
        self.assertIn(partner, ticket.message_partner_ids)
        self.assertEqual(
            ticket.partner_email, lead.email_from if lead.email_from else partner.email
        )
        self.assertEqual(
            ticket.partner_phone,
            lead.phone
            if lead.phone
            else partner.phone or lead.mobile or partner.mobile,
        )
    ...
    def test_lead_convert_to_ticket_corner_cases(self):
        ...
        self.user_sales_salesman.write(
        ...
        convert = (
            self.env["crm.lead.convert2ticket"]
        ...
        self.user_sales_salesman.write(
            {"groups_id": [(3, self.env.ref("sales_team.group_sale_salesman").id)]}
        )
        ...
        with self.assertRaises(AccessError):
            convert = (
                self.env["crm.lead.convert2ticket"]
                .with_context({"active_model": "crm.lead", "active_id": self.lead_1.id})
                .create(
                    {
                        "team_id": self.test_team.id,
                        "ticket_type_id": self.test_ticket_type.id,
    ...
    def test_lead_convert_to_ticket_w_email_rights(self):
        ...
        msg = lead.message_post(
            body="Youplaboum", subtype_xmlid="mail.mt_comment", message_type="comment"
        )
        ...
        test_ticket_type = self.env["helpdesk.ticket.type"].search(
            [("id", "=", self.test_ticket_type.id)]
        )
        ...
        convert = (
            self.env["crm.lead.convert2ticket"]
            .with_context({"active_model": "crm.lead", "active_id": self.lead_1.id})
            .create({"team_id": test_team.id, "ticket_type_id": test_ticket_type.id})
        )
        action = convert.action_lead_to_helpdesk_ticket()
        ...
        self.assertTicketLeadConvertData(
            ticket, lead, test_team, test_ticket_type, new_partner
        )

        # check discussion thread transfer
        self.assertTrue(
            all(message in ticket.message_ids for message in lead_message_ids)
        )
    ...
    def test_lead_convert_to_ticket_w_name(self):
        ...
        ticket = self.env["helpdesk.ticket"].sudo().search([("name", "=", lead.name)])
        self.assertTicketLeadConvertData(
            ticket, lead, self.test_team, self.test_ticket_type, self.contact_company_1
        )
    ...
    def test_lead_convert_to_ticket_w_partner(self):
        ...
        ticket = self.env["helpdesk.ticket"].sudo().search([("name", "=", lead.name)])
        self.assertTicketLeadConvertData(
            ticket, lead, self.test_team, self.test_ticket_type, self.contact_1
        )
    ...
    def test_lead_convert_to_ticket_w_contact_name(self):
        ...
        ticket = self.env["helpdesk.ticket"].sudo().search([("name", "=", lead.name)])
        self.assertTicketLeadConvertData(
            ticket, lead, self.test_team, self.test_ticket_type, self.contact_2
        )
