...
class HelpdeskTicketConvert2Lead(models.TransientModel):
    ...
    def _compute_user_id(self):
        for convert in self:
            ...
            convert.user_id = (
                user if user and user in convert.team_id.member_ids else False
            )
    ...
    def action_convert_to_lead(self):
        ...
        if self.action == "create":
            self.partner_id = self.ticket_id._find_matching_partner(
                force_create=True
            ).id
        ...
        lead_sudo.message_post_with_view(
            ...
            values={"self": lead_sudo, "origin": self.ticket_id},
