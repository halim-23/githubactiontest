...
class HrAppraisal(models.Model):
    ...
    def action_open_survey_inputs(self):
        self.ensure_one()
        if (
            self.user_has_groups("hr_appraisal.group_hr_appraisal_manager")
            or self.env.user.employee_id in self.manager_ids
        ) and len(self.survey_ids) > 1:
            return {
                ...
                "views": [
                    [
                        self.env.ref(
                            "hr_appraisal_survey.survey_user_input_view_tree"
                        ).id,
                        "tree",
                    ]
                ],
