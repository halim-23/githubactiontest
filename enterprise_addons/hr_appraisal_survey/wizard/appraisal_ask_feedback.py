...
class AppraisalAskFeedback(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if (
            ...
            result["survey_template_id"] = (
                appraisal.department_id.appraisal_survey_template_id.id
                or appraisal.company_id.appraisal_survey_template_id.id
            )
        return result

    appraisal_id = fields.Many2one(
        "hr.appraisal", default=lambda self: self.env.context.get("active_id", None)
    )
    employee_id = fields.Many2one(
        related="appraisal_id.employee_id", string="Appraisal Employee"
    )
    ...
    attachment_ids = fields.Many2many(
        "ir.attachment",
        "hr_appraisal_survey_mail_compose_message_ir_attachments_rel",
        "wizard_id",
        "attachment_id",
        string="Attachments",
    )
    ...
    survey_template_id = fields.Many2one(
        "survey.survey", required=True, domain=[("is_appraisal", "=", True)]
    )
    ...
    @api.depends(
        "survey_template_id"
    )  # fake trigger otherwise not computed in new mode
        ...
        for wizard in self.filtered("employee_id"):
            if wizard.template_id:
                wizard.subject = self.sudo()._render_template(
                    ...
                    "hr.appraisal",
                    wizard.appraisal_id.ids,
                    post_process=True,
                )[wizard.appraisal_id.id]
    ...
    def _compute_deadline(self):
        ...
        for wizard in self:
            wizard.deadline = min(
                date_in_month, wizard.appraisal_id.date_close + relativedelta(days=-1)
            )
    ...
    def _send_mail(self, answer):
        ...
        user_body = (
            ...
            if not is_html_empty(
                html_sanitize(user_body, strip_style=True, strip_classes=True)
            )
            else False
        )
        ctx = {"user_body": user_body}
        body = self.with_context(**ctx)._render_field(
            "body", answer.ids, post_process=True
        )[answer.id]
        mail_values = {
    ...
    def action_send(self):
        ...
        for employee in self.employee_ids.filtered(
            lambda e: e.user_id.has_group("hr_appraisal.group_hr_appraisal_user")
        ):
            answer = answers.filtered(
                lambda l: l.partner_id == employee.user_id.partner_id
            )
            self.appraisal_id.with_context(
