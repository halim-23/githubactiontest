...
class TestMrpWorkorderPlm(TestPlmCommon):
    ...
    def test_operation_change(self):
        ...
        eco1.new_bom_id.operation_ids[0].quality_point_ids[
            0
        ].test_type_id = self.env.ref("quality.test_type_picture")
        ...
        # Check correctness
        self.assertEqual(
            eco1.routing_change_ids[0].change_type,
            "update",
            "Wrong type on opration change line.",
        )
        self.assertEqual(
            eco1.routing_change_ids[1].change_type,
            "remove",
            "Wrong type on opration change line.",
        )
        self.assertEqual(
            eco1.routing_change_ids[2].change_type,
            "add",
            "Wrong type on opration change line.",
        )
