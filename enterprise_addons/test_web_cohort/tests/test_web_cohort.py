...
class TestCohortForward(TestCohortCommon):
    ...
    def test_forward(self):
        ...
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "year", [], "retention", "forward"
        )["rows"]
        ...
        self.assertEqual(
            result[0]["value"] + result[1]["value"], NB_START_DAY * NB_END_DAY
        )

        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "month", [], "retention", "forward"
        )["rows"]
        self.assertEqual(len(result), 3)
        self.assertEqual(result[0]["columns"][0]["percentage"], 81)
        self.assertEqual(result[0]["columns"][1]["percentage"], 46.7)
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "week", [], "retention", "forward"
        )["rows"]
        self.assertEqual(len(result), 11)
        self.assertEqual(result[0]["columns"][0]["percentage"], 96.7)
        self.assertEqual(result[0]["columns"][1]["percentage"], 90.0)
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "day", [], "retention", "forward"
        )["rows"]
        ...
        self.assertEqual(
            sum(v["value"] for v in result), (29 * 30 / 2) * 10 * NB_START_DAY
        )
        ...
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "type_id", "week", [], "retention", "forward"
        )["rows"]
        # There is 8 type so value should be 8
        self.assertEqual(result[0]["value"], TYPE_SIZE)
        # Test Churn mode
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "year", [], "churn", "forward"
        )["rows"]
...
class TestCohortBackward(TestCohortCommon):
    ...
    def test_backward(self):
        # Test interval
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "year", [], "retention", "backward"
        )["rows"]
        self.assertEqual(len(result), 2)
        self.assertEqual(result[0]["columns"][0]["percentage"], 100.0)
        self.assertEqual(result[0]["columns"][-1]["percentage"], 51)
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "month", [], "retention", "backward"
        )["rows"]
        self.assertEqual(len(result), 3)
        ...
        self.assertEqual(result[0]["columns"][-1]["percentage"], 51)
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "week", [], "retention", "backward"
        )["rows"]
        self.assertEqual(len(result), 11)
        self.assertEqual(result[0]["columns"][0]["percentage"], 100)
        ...
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "day", [], "retention", "backward"
        )["rows"]
        self.assertEqual(len(result), NB_START_DAY)
        ...
        # Test measure
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "revenue", "week", [], "retention", "backward"
        )["rows"]
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "type_id", "week", [], "retention", "backward"
        )["rows"]
        # There is 8 type so value should be 8
        self.assertEqual(result[0]["value"], TYPE_SIZE)
        # Test Churn mode
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "__count", "month", [], "churn", "backward"
        )["rows"]
...
class TestCohortEmptyFuturBackward(TestCohortCommon):
    ...
    def test_empty_backward(self):
        ...
        """
        result = self.WebCohortSimpleModel.get_cohort_data(
            "date_start", "date_stop", "revenue", "day", [], "retention", "backward"
        )["rows"]
