...
class WebsiteHelpdesk(http.Controller):
    @http.route(
        ["/helpdesk/rating", '/helpdesk/rating/<model("helpdesk.team"):team>'],
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    def page(self, team=False, **kw):
        ...
        if user.has_group("helpdesk.group_heldpesk_manager"):
            ...
            domain = AND(
                [
                    [("use_rating", "=", True), ("portal_show_rating", "=", True)],
                    team_domain,
                ]
            )
        ...
        for team in teams:
            ...
            ratings = (
                ...
                .sudo()
                .search(domain, order="id desc", limit=100)
            )
            ...
            any_rating = False
            for x in (7, 30, 90):
                todate = (datetime.date.today() - datetime.timedelta(days=x)).strftime(
                    "%Y-%m-%d 00:00:00"
                )
                domdate = domain + [
        ...
        return request.render(
            "helpdesk.team_rating_page", {"page_name": "rating", "teams": team_values}
        )
