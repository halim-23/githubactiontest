odoo.define("helpdesk.dashboard_tests", function (require) {
  ...
  QUnit.module("Views", {}, function () {
    ...
    QUnit.test("dashboard basic rendering", async function (assert) {
      ...
      assert.containsOnce(
        kanban,
        "div.o_helpdesk_dashboard",
        "should render the dashboard"
      );
      assert.strictEqual(
        kanban.$(".o_kanban_view_wrapper > .o_helpdesk_dashboard").length,
        1,
        "dashboard should be sibling of renderer element"
      );
      assert.strictEqual(
        kanban.$(".o_target_to_set").text().trim(),
        "12.00",
        "should have written correct target"
      );
      assert.hasAttrValue(
        kanban.$(".o_target_to_set"),
        "value",
        "12.00",
      ...
      var kanban = await createView({
        ...
        mockRPC: function (route, args) {
          ...
          if (args.model === "res.users" && args.method === "write") {
            ...
            dashboard_data.helpdesk_target_closed =
              args.args[1].helpdesk_target_closed;
    ...
    QUnit.test("dashboard rendering with empty many2one", async function (assert) {
      ...
      this.data.partner.fields.partner_id = {
        string: "Partner",
        type: "many2one",
        relation: "partner",
      };
