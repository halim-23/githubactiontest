odoo.define("helpdesk.dashboard", function (require) {
  ...
  KanbanRecord.include({
    ...
    _openRecord() {
      ...
      if (
        ...
        this.modelName === "helpdesk.team" &&
        kanbanTicketElement.length
      ) {
  ...
  var HelpdeskDashboardRenderer = KanbanRenderer.extend({
    ...
    _render: function () {
      ...
      return this._super.apply(this, arguments).then(function () {
        ...
        if (!self.$el.parent(".o_kanban_view_wrapper").length) {
          self.$el.wrap(
            '<div class="o_kanban_view_wrapper d-flex flex-column align-items-start"></div>'
          );
        }
    ...
    _onDashboardTargetClicked: function (e) {
      ...
      $input.replaceAll($target).focus().select();
    },
  });
  ...
  var HelpdeskDashboardController = KanbanController.extend({
    ...
    _onDashboardEditTarget: function (e) {
      ...
      if (isNaN(target_value)) {
        this.displayNotification({
          message: _t("Please enter an integer value"),
          type: "danger",
        });
      } else {
        var values = {};
    ...
    _onDashboardOpenAction: function (e) {
      ...
      if (
        _.contains(
          ["action_view_rating_today", "action_view_rating_7days"],
          action_name
        )
      ) {
        return this._rpc({model: this.modelName, method: action_name}).then(function (
          data
        ) {
          if (data) {
