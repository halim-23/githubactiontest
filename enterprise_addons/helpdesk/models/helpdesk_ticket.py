...
class HelpdeskSLAStatus(models.Model):
    ...
    exceeded_days = fields.Float(
        "Excedeed Working Days",
        compute="_compute_exceeded_days",
        compute_sudo=True,
        store=True,
    ...
    def _compute_deadline(self):
        for status in self:
            ...
            avg_hour = (
                working_calendar.hours_per_day or 8
            )  # default to 8 working hours/day
    ...
    def _compute_exceeded_days(self):
        for status in self:
            if (
                status.reached_datetime
                and status.deadline
                and status.ticket_id.team_id.resource_calendar_id
            ):
...
class HelpdeskTicket(models.Model):
    ...
    def default_get(self, fields):
        ...
        if result.get("team_id") and fields:
            ...
            if (
                "user_id" in fields and "user_id" not in result
            ):  # if no user given, deduce it from the team
                result["user_id"] = team._determine_user_to_assign()[team.id].id
            if (
                "stage_id" in fields and "stage_id" not in result
            ):  # if no stage given, deduce it from the team
    ...
    team_id = fields.Many2one(
        "helpdesk.team", string="Helpdesk Team", default=_default_team_id, index=True
    )
    ...
    kanban_state_label = fields.Char(
        compute="_compute_kanban_state_label", string="Column Status", tracking=True
    )
    ...
    email = fields.Char(
        related="partner_email", string="Email on Customer", readonly=False
    )
    ...
    close_hours = fields.Integer(
        "Time to close (hours)",
        compute="_compute_close_hours",
        store=True,
        help="This duration is based on the working calendar of the team",
    )
    open_hours = fields.Integer(
        "Open Time (hours)",
    ...
    sla_reached_late = fields.Boolean(
    ...
    sla_fail = fields.Boolean(
        "Failed SLA Policy", compute="_compute_sla_fail", search="_search_sla_fail"
    )
    sla_success = fields.Boolean(
        "Success SLA Policy",
        compute="_compute_sla_success",
        search="_search_sla_success",
    )

    use_credit_notes = fields.Boolean(
        related="team_id.use_credit_notes", string="Use Credit Notes"
    )
    use_coupons = fields.Boolean(related="team_id.use_coupons", string="Use Coupons")
    use_product_returns = fields.Boolean(
        related="team_id.use_product_returns", string="Use Returns"
    )
    use_product_repairs = fields.Boolean(
        related="team_id.use_product_repairs", string="Use Repairs"
    )
    use_rating = fields.Boolean(
        related="team_id.use_rating", string="Use Customer Ratings"
    )
    ...
    def _compute_domain_user_ids(self):
        ...
        for ticket in self:
            if (
                ticket.team_id
                and ticket.team_id.privacy == "invite"
                and ticket.team_id.visibility_member_ids
            ):
                manager_ids = mapped_data.get(helpdesk_manager_group_id, [])
                ticket.domain_user_ids = [
                    Command.set(manager_ids + ticket.team_id.visibility_member_ids.ids)
                ]
            else:
    ...
    def _compute_sla_deadline(self):
        ...
        for ticket in self:
            ...
            ticket.sla_deadline = (
                min(status_not_reached.mapped("deadline"))
    ...
    def _search_sla_fail(self, operator, value):
        ...
        if (value and operator in expression.NEGATIVE_TERM_OPERATORS) or (
            not value and operator not in expression.NEGATIVE_TERM_OPERATORS
        ):  # is not failed
            return [
                "&",
    ...
    def _search_sla_success(self, operator, value):
        ...
        if (value and operator in expression.NEGATIVE_TERM_OPERATORS) or (
            ...
            return [
                ("sla_status_ids.reached_datetime", ">", datetime_now),
                ("sla_reached_late", "!=", False),
            ]
        return [
            ("sla_status_ids.reached_datetime", "<", datetime_now),
    ...
    def _compute_partner_ticket_count(self):
        def _get_email_to_search(email):
            ...
            return (
                ("@" + domain)
                if domain and domain not in iap_tools._MAIL_DOMAIN_BLACKLIST
                else email
            )
        ...
        for ticket in self:
            ...
            if ticket.partner_email:
                ...
                domain = expression.OR(
                    [domain, [("partner_email", "ilike", email_search)]]
                )
            if ticket.partner_phone:
                domain = expression.OR(
                    [domain, [("partner_phone", "ilike", ticket.partner_phone)]]
                )
            if ticket.partner_id:
                domain = expression.OR(
                    [
                        ...
                        [
                            (
                                "partner_id",
                                "child_of",
                                ticket.partner_id.commercial_partner_id.id,
                            )
                        ],
                    ]
                )
            ...
            ticket.partner_ticket_count = (
                len(partner_ticket) - 1 if partner_ticket else 0
            )
    ...
    def _compute_assign_hours(self):
        for ticket in self:
            ...
            if (
                create_date
                and ticket.assign_date
                and ticket.team_id.resource_calendar_id
            ):
                duration_data = (
                    ticket.team_id.resource_calendar_id.get_work_duration_data(
                        create_date,
                        fields.Datetime.from_string(ticket.assign_date),
                        compute_leaves=True,
                    )
                )
        ...
        for ticket in self:
            ...
            if create_date and ticket.close_date:
                duration_data = (
                    ticket.team_id.resource_calendar_id.get_work_duration_data(
                        create_date,
                        fields.Datetime.from_string(ticket.close_date),
                        compute_leaves=True,
                    )
                )
    ...
    def _compute_open_hours(self):
        for ticket in self:
            if (
                ticket.create_date
            ):  # fix from https://github.com/odoo/enterprise/commit/928fbd1a16e9837190e9c172fa50828fae2a44f7
                if ticket.close_date:
                    ...
                    time_difference = (
                        fields.Datetime.now()
                        - fields.Datetime.from_string(ticket.create_date)
                    )
                ticket.open_hours = (
            ...
            if partner_name and partner_email and not partner_id:
                ...
                try:
                    vals["partner_id"] = (
                        ...
                        .find_or_create(tools.formataddr((partner_name, parsed_email)))
                        .id
                    )
    ...
    def write(self, vals):
        ...
        res = super(HelpdeskTicket, self - assigned_tickets - closed_tickets).write(
            vals
        )
        res &= super(HelpdeskTicket, assigned_tickets - closed_tickets).write(
            dict(
                vals,
                **{
                    "assign_date": now,
        ...
        res &= super(HelpdeskTicket, closed_tickets - assigned_tickets).write(
            dict(
                vals,
                **{
                    "close_date": now,
    ...
    def _sla_apply(self, keep_reached=False):
        ...
        for tickets, slas in sla_per_tickets.items():
            sla_status_value_list += tickets._sla_generate_status_values(
                slas, keep_reached=keep_reached
            )
    ...
    def _sla_find(self):
        ...
        for key, tickets in tickets_map.items():  # only one search per ticket group
            ...
            result[tickets] = slas.filtered(
                lambda s: s.tag_ids <= tickets.tag_ids
            )  # SLA to apply on ticket subset
        return result
                    ...
                    result.append(
                        {
                            ...
                            if ticket.stage_id == sla.stage_id
                            else False,  # in case of SLA on first stage
                        }
                    )
        ...
        action.update(
            {
                "domain": [
                    ("id", "!=", self.id),
                    ("id", "in", self.partner_ticket_ids.ids),
                ],
    ...
    def message_new(self, msg, custom_values=None):
        ...
        ticket = super(
            HelpdeskTicket, self.with_context(mail_notify_author=True)
        ).message_new(msg, custom_values=values)
        partner_ids = [
            x.id
            for x in self.env["mail.thread"]._mail_find_partner_from_emails(
                self._ticket_email_split(msg), records=ticket
            )
            if x
        ]
        customer_ids = [
            ...
            for p in self.env["mail.thread"]._mail_find_partner_from_emails(
                tools.email_split(values["partner_email"]), records=ticket
            )
            if p
        ]
    ...
    def message_update(self, msg, update_vals=None):
        partner_ids = [
            x.id
            for x in self.env["mail.thread"]._mail_find_partner_from_emails(
                self._ticket_email_split(msg), records=self
            )
            if x
        ]
        if partner_ids:
    ...
    def _notify_get_groups(self, msg_vals=None):
        ...
        new_groups = [
            (
                ...
                and helpdesk_user_group_id in pdata["groups"],
    ...
    def _mail_get_message_subtypes(self):
        ...
        if len(self) == 1 and self.team_id:
            team = self.team_id
            optional_subtypes = [
                ("use_credit_notes", self.env.ref("helpdesk.mt_ticket_refund_posted")),
