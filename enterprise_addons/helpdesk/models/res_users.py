    ...
    def write(self, vals):
        if "active" in vals and not vals.get("active"):
            teams = self.env["helpdesk.team"].search(
                [("visibility_member_ids", "in", self.ids)]
            )
            for team in teams:
                unlinks = [
                    Command.unlink(user.id) for user in teams.member_ids if user in self
                ]
