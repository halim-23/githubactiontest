...
class HelpdeskTeam(models.Model):
    ...
    _sql_constraints = [
        (
            "not_portal_show_rating_if_not_use_rating",
    ...
    privacy = fields.Selection(
        [("user", "All Users"), ("invite", "Invited Users")],
        string="Users Assign",
        default="user",
    )
    ...
    assign_method = fields.Selection(
        [("manual", "Manual"), ("randomly", "Random"), ("balanced", "Balanced")],
    ...
    visibility_member_ids = fields.Many2many(
        "res.users",
        "helpdesk_visibility_team",
        string="Team Visibility",
        domain=lambda self: self._default_domain_member_ids(),
    ...
    allow_portal_ticket_closing = fields.Boolean(
        "Closure by Customers", help="Allow customers to close their tickets"
    )
    use_website_helpdesk_form = fields.Boolean("Website Form")
    use_website_helpdesk_livechat = fields.Boolean(
        "Live Chat",
    ...
    use_helpdesk_timesheet = fields.Boolean(
        "Timesheets",
        compute="_compute_use_helpdesk_timesheet",
        store=True,
        readonly=False,
        help="This requires to have project module installed.",
    )
    use_helpdesk_sale_timesheet = fields.Boolean(
        "Time Billing",
        compute="_compute_use_helpdesk_sale_timesheet",
        store=True,
        readonly=False,
        help="Reinvoice the time spent on ticket through tasks.",
    )
    use_credit_notes = fields.Boolean("Refunds")
    use_coupons = fields.Boolean("Coupons")
    use_fsm = fields.Boolean(
        "Field Service", help="Convert tickets into Field Service tasks"
    )
    ...
    portal_show_rating = fields.Boolean(
        "Public Rating",
        compute="_compute_portal_show_rating",
        store=True,
        readonly=False,
    )
    use_sla = fields.Boolean("SLA Policies")
    upcoming_sla_fail_tickets = fields.Integer(
        string="Upcoming SLA Fail Tickets", compute="_compute_upcoming_sla_fail_tickets"
    )
    unassigned_tickets = fields.Integer(
        string="Unassigned Tickets", compute="_compute_unassigned_tickets"
    )
    ...
    open_ticket_count = fields.Integer(
        "# Open Tickets", compute="_compute_open_ticket_count"
    )
    sla_policy_count = fields.Integer(
        "# SLA Policy", compute="_compute_sla_policy_count"
    )
    # auto close ticket
    auto_close_ticket = fields.Boolean("Automatic Closing")
    auto_close_day = fields.Integer(
    ...
    from_stage_ids = fields.Many2many(
        "helpdesk.stage",
        relation="team_stage_auto_close_from_rel",
        string="In Stages",
    ...
    to_stage_id = fields.Many2one(
        ...
        compute="_compute_assign_stage_id",
        readonly=False,
        store=True,
        domain="[('id', 'in', stage_ids)]",
        help="Stage to which inactive tickets will be automatically moved once the period of inactivity is reached.",
    )
    display_alias_name = fields.Char(
        string="Alias email", compute="_compute_display_alias_name"
    )

    @api.depends("auto_close_ticket", "stage_ids")
    def _compute_assign_stage_id(self):
        stages_dict = {
            stage["id"]: 1 if stage["is_close"] else 2
            for stage in self.env["helpdesk.stage"].search_read(
                [
                    ("id", "in", self.stage_ids.ids),
                    "|",
                    ("is_close", "=", True),
                    ("fold", "=", True),
                ],
        ...
        for team in self:
            stage_ids = sorted(
                [
                    (val, stage_id)
                    for stage_id, val in stages_dict.items()
                    if stage_id in team.stage_ids.ids
                ]
            )
    ...
    def _compute_sla_policy_count(self):
        sla_data = self.env["helpdesk.sla"].read_group(
            [("team_id", "in", self.ids)], ["team_id"], ["team_id"]
        )
        mapped_data = {
            data["team_id"][0]: data["team_id_count"] for data in sla_data
        }
        for team in self:
    ...
    def create(self, vals_list):
        teams = super(
            HelpdeskTeam, self.with_context(mail_create_nosubscribe=True)
        ).create(vals_list)
    ...
    def unlink(self):
        ...
        )  # remove stages that only belong to team in self
        stages.unlink()
    ...
    def _update_cron(self):
        cron = self.env.ref(
            "helpdesk.ir_cron_auto_close_ticket", raise_if_not_found=False
        )
    ...
    def _check_sla_group(self):
        ...
        if non_sla_teams:
            self.env["helpdesk.sla"].search(
                [("team_id", "in", non_sla_teams.ids)]
            ).write({"active": False})
    ...
    def retrieve_dashboard(self):
        ...
        user_uses_sla = self.user_has_groups("helpdesk.group_use_sla") and bool(
            self.env["helpdesk.team"].search([("use_sla", "=", True)], limit=1)
        )
        ...
        tickets = HelpdeskTicket.search_read(
            expression.AND([domain, [("stage_id.is_close", "=", False)]]),
        ...
        result["my_all"]["hours"] = fields.Float.round(
            result["my_all"]["hours"] / (result["my_all"]["count"] or 1), 2
        )
        result["my_high"]["hours"] = fields.Float.round(
            result["my_high"]["hours"] / (result["my_high"]["count"] or 1), 2
        )
        ...
        if self.env["helpdesk.team"].search([("use_rating", "=", True)], limit=1):
            ...
            team_satisfaction = (
                fields.Float.round(
                    (
                        total_rating / total_activity_values
    ...
    def _action_view_rating(self, period=False, only_my_closed=False):
        ...
        if period == "seven_days":
            ...
            domain += [
                ("close_date", ">=", fields.Datetime.to_string(datetime.date.today()))
            ]
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "helpdesk.rating_rating_action_helpdesk"
        )
    ...
    def action_view_rating_today(self):
        ...
        return self.search([("member_ids", "in", self._uid)])._action_view_rating(
            period="today", only_my_closed=True
        )
    ...
    def action_view_rating_7days(self):
        ...
        return self.search([("member_ids", "in", self._uid)])._action_view_rating(
            period="seven_days", only_my_closed=True
        )
    ...
    def action_view_team_rating(self):
        ...
        rating_ids = self.rating_ids.filtered(
            lambda x: x.rating >= 1 and x.consumed
        ).ids
        if len(rating_ids) == 1:
            action.update(
                {
    ...
    def action_view_sla_policy(self):
        ...
        if self.sla_policy_count == 1:
            action.update(
                {
                    "view_mode": "form",
                    "res_id": self.env["helpdesk.sla"]
                    .search([("team_id", "=", self.id)], limit=1)
                    .id,
                    "views": [(False, "form")],
                }
            )
    ...
    def _determine_user_to_assign(self):
        ...
        for team in self:
            ...
            if (
                team.assign_method == "randomly"
            ):  # randomly means new tickets get uniformly distributed
                last_assigned_user = (
                ...
                team.assign_method == "balanced"
            ):  # find the member with the least open ticket
                ...
                open_ticket_per_user_map = dict.fromkeys(
                    member_ids, 0
                )  # dict: user_id -> open ticket count
    ...
    def _determine_stage(self):
        ...
        for team in self:
            result[team.id] = self.env["helpdesk.stage"].search(
                [("team_ids", "in", team.id)], order="sequence", limit=1
            )
        return result
    ...
    def _cron_auto_close_tickets(self):
        teams = self.env["helpdesk.team"].search_read(
            ...
            fields=["id", "auto_close_day", "from_stage_ids", "to_stage_id"],
        )
        teams_dict = defaultdict(
            dict
        )  # key: team_id, values: the remaining result of the search_group
        today = fields.datetime.today()
        for team in teams:
            # Compute the threshold_date
            team["threshold_date"] = today - relativedelta.relativedelta(
                days=team["auto_close_day"]
            )
            teams_dict[team["id"]] = team
        tickets_domain = [
            ("stage_id.is_close", "=", False),
            ("team_id", "in", list(teams_dict.keys())),
        ]
        ...
        for ticket in inactive_tickets:
            ...
            if teams_dict[ticket.team_id.id]["to_stage_id"]:
                ticket.write(
                    {"stage_id": teams_dict[ticket.team_id.id]["to_stage_id"][0]}
                )
    ...
    def _mail_get_message_subtypes(self):
        ...
        if len(self) == 1:
            optional_subtypes = [
                (
                    "use_credit_notes",
                    self.env.ref("helpdesk.mt_team_ticket_refund_posted"),
                ),
                (
                    "use_product_returns",
                    self.env.ref("helpdesk.mt_team_ticket_return_done"),
                ),
                (
                    "use_product_repairs",
                    self.env.ref("helpdesk.mt_team_ticket_repair_done"),
                ),
            ]
...
class HelpdeskStage(models.Model):
    ...
    def write(self, vals):
        if "active" in vals and not vals["active"]:
            self.env["helpdesk.ticket"].search([("stage_id", "in", self.ids)]).write(
                {"active": False}
            )
    ...
    def unlink(self):
        ...
        if default_team_id:
            shared_stages = self.filtered(
                lambda x: len(x.team_ids) > 1 and default_team_id in x.team_ids.ids
            )
            tickets = (
                self.env["helpdesk.ticket"]
...
class HelpdeskSLA(models.Model):
    ...
    time = fields.Float(
        "In",
        help="Time to reach given stage based on ticket creation date",
        default=0,
        required=True,
    )
    ticket_count = fields.Integer(compute="_compute_ticket_count")
