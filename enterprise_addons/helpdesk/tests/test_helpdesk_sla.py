...
class HelpdeskSLA(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.helpdesk_manager = Users.create(
            {
                ...
                "groups_id": [
                    (6, 0, [cls.env.ref("helpdesk.group_helpdesk_manager").id])
                ],
            }
        )
        ...
        cls.test_team = (
            cls.env["helpdesk.team"]
            .with_user(cls.helpdesk_manager)
            .create({"name": "Test Team", "use_sla": True})
            .sudo()
        )
    ...
    def _ticket_patch_now(self, datetime_str):
        ...
        def new_today():
            return fields.Datetime.from_string(datetime_str).replace(
    ...
    def test_sla_no_tag(self):
        ...
        self.assertEqual(
            ticket.sla_status_ids.sla_id, self.sla, "SLA should have been applied"
        )
    ...
    def test_sla_waiting(self):
        ...
        status = ticket.sla_status_ids.filtered(
            lambda sla: sla.sla_id.id == self.sla_2.id
        )
        self.assertEqual(
            status.deadline,
            datetime(2019, 1, 9, 12, 2, 0),
            "No waiting time, deadline = creation date + 1 day + 2 hours + 2 minutes",
        )
        ...
        self._utils_set_create_date(
            ticket.message_ids.tracking_value_ids, "2019-01-08 11:09:50", ticket
        )
        self.assertEqual(
            status.deadline,
            datetime(2019, 1, 9, 12, 2, 0),
        ...
        ticket.message_track(["stage_id"], initial_values)
        self._utils_set_create_date(
            ticket.message_ids.tracking_value_ids[0], "2019-01-08 12:15:00", ticket
        )
        ...
        ticket.message_track(["stage_id"], initial_values)
        self._utils_set_create_date(
            ticket.message_ids.tracking_value_ids[0], "2019-01-12 10:35:58", ticket
        )
        ...
        self.assertEqual(
            status.deadline,
            datetime(2019, 1, 14, 15, 47),
            "We have waiting time: deadline = old_deadline + 3 full working days (waiting) + 3.75 hours (waiting) + 2 days (weekend)",
        )
        ...
        ticket.message_track(["stage_id"], initial_values)
        self._utils_set_create_date(
            ticket.message_ids.tracking_value_ids[0], "2019-01-14 15:30:00", ticket
        )
        self.assertFalse(status.deadline, "In waiting stage: no more deadline")
        ...
        with patch.object(fields.Datetime, "now", lambda: datetime(2019, 1, 16, 15, 0)):
            ...
            ticket.message_track(["stage_id"], initial_values)
            self._utils_set_create_date(
                ticket.message_ids.tracking_value_ids[0], "2019-01-16 15:00:00", ticket
            )
            self.assertEqual(
                status.deadline,
                datetime(2019, 1, 16, 15, 17),
                "We have waiting time: deadline = old_deadline +  7.5 hours (waiting)",
            )

    @patch.object(fields.Date, "today", lambda: NOW.date())
    @patch.object(
        fields.Datetime, "today", lambda: NOW.replace(hour=0, minute=0, second=0)
    )
    @patch.object(fields.Datetime, "now", lambda: NOW)
    def test_failed_tickets(self):
        self.sla.time = 3
        # Failed ticket
        failed_ticket = self.create_ticket(
            user_id=self.env.user.id,
            create_date=NOW - relativedelta(hours=3, minutes=2),
        )

        # Not failed ticket
        ticket = self.create_ticket(
            user_id=self.env.user.id,
            create_date=NOW - relativedelta(hours=2, minutes=2),
        )
    ...
    @patch.object(
        fields.Datetime,
        "now",
        lambda: NOW + relativedelta(hour=20, minute=0, microsecond=0),
    )
    def test_deadlines_after_work(self):
        ...
        self.assertEqual(
            ...
            "Day0:20h + 3h = Day1:8h + 3h = Day1:11h",
        )
        ...
        self.assertEqual(
            ticket.sla_deadline,
            fields.Datetime.now() + relativedelta(days=2, hour=11),
            "Day0:20h + 11h = Day0:20h + 1day:3h = Day1:8h + 1day:3h = Day2:8h + 3h = Day2:11h",
        )
    ...
    @patch.object(
        fields.Datetime,
        "now",
        lambda: NOW + relativedelta(hour=8, minute=0, microsecond=0),
    )
    def test_deadlines_during_work(self):
        ...
        self.assertEqual(
            ticket.sla_deadline,
            fields.Datetime.now() + relativedelta(days=0, hour=11),
            "Day0:8h + 3h = Day0:11h",
        )
        ...
        self.assertEqual(
            ticket.sla_deadline,
            fields.Datetime.now() + relativedelta(days=1, hour=11),
            "Day0:8h + 11h = Day0:8h + 1day:3h = Day1:8h + 3h = Day1:11h",
        )
