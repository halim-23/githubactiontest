...
class TestHelpdeskFlow(HelpdeskCommon):
    ...
    def test_access_rights(self):
        ...
        team = (
            ...
            .with_user(self.helpdesk_manager)
            .create({"name": "test"})
        )
        ...
        tag = (
            self.env["helpdesk.tag"]
            .with_user(self.helpdesk_user)
            .create({"name": "test with unique name please"})
        )
        tag.read()
    ...
    def test_assign_close_dates(self):
        ...
        with self._ticket_patch_now("2019-01-10 13:00:00"):
            ...
            self.assertEqual(
                ticket1.user_id.id, ticket1._uid, "Assignation for ticket not correct"
            )
            self.assertEqual(
                ticket1.assign_hours, 17, "Assignation time for ticket not correct"
            )
        with self._ticket_patch_now("2019-01-10 15:00:00"):
            ...
            self.assertEqual(
                ticket1.close_hours, 19, "Close time for ticket not correct"
            )

    def test_ticket_partners(self):
        # we create a partner
        partner = self.env["res.partner"].create({"name": "Freddy Krueger"})
        ...
        self.assertTrue(
            ticket1.partner_ticket_count == 1,
            "Incorrect number of tickets from the same partner.",
        )

    def test_team_assignation_randomly(self):
        # we put the helpdesk user and manager in the test_team's members
        self.test_team.member_ids = [
            (6, 0, [self.helpdesk_user.id, self.helpdesk_manager.id])
        ]
    ...
    def test_team_assignation_balanced(self):
        ...
        self.env["helpdesk.ticket"].search(
            [("user_id", "=", self.helpdesk_user.id)]
        ).write({"stage_id": self.stage_done.id})
        ...
        self.assertEqual(
            self.env["helpdesk.ticket"].search_count(
                [("user_id", "=", self.helpdesk_user.id), ("close_date", "=", False)]
            ),
            3,
        )
        self.assertEqual(
            self.env["helpdesk.ticket"].search_count(
                [("user_id", "=", self.helpdesk_manager.id), ("close_date", "=", False)]
            ),
            3,
        )
    ...
    def test_create_from_email_multicompany(self):
        ...
        helpdesk_team_model = self.env["ir.model"].search(
            [("model", "=", "helpdesk_team")]
        )
...
--000000000000a47519057e029630--
"""
        partners_exist = Partner.search(
            [
                (
                    "email",
                    "in",
                    ["client_a@someprovider.com", "client_b@someprovider.com"],
                )
            ]
        )
        self.assertFalse(partners_exist)

        helpdesk_ticket0_id = self.env["mail.thread"].message_process(
            "helpdesk.ticket", new_message0
        )
        helpdesk_ticket1_id = self.env["mail.thread"].message_process(
            "helpdesk.ticket", new_message1
        )
        ...
        self.assertTrue(
            partner0 in helpdesk_ticket0.message_follower_ids.mapped("partner_id")
        )
        self.assertTrue(
            partner1 in helpdesk_ticket1.message_follower_ids.mapped("partner_id")
        )
