...
class MrpProductionWorkcenterLine(models.Model):
    ...
    measure_success = fields.Selection(
        related="current_quality_check_id.measure_success", readonly=False
    )
    norm_unit = fields.Char(
        related="current_quality_check_id.norm_unit", readonly=False
    )
    ...
    def button_finish(self):
        ...
        for check in self.check_ids:
            ...
            if check.test_type in [
                ...
                raise UserError(
                    _(
                        "You first need to complete the Quality Check using the Tablet View before marking the Operation as Done."
                    )
                )
