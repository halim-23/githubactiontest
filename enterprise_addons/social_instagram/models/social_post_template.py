...
class SocialPostTemplate(models.Model):
    ...
    instagram_image_id = fields.Many2one(
        "ir.attachment", compute="_compute_instagram_image_id"
    )
    ...
    display_instagram_preview = fields.Boolean(
        "Display Instagram Preview", compute="_compute_display_instagram_preview"
    )
    instagram_preview = fields.Html(
        "Instagram Preview", compute="_compute_instagram_preview"
    )

    @api.depends("message", "account_ids.media_id.media_type")
    def _compute_display_instagram_preview(self):
        for post in self:
            post.display_instagram_preview = post.message and (
    ...
    def _compute_instagram_image_id(self):
        for post in self:
            jpeg_images = post.image_ids.filtered(
                lambda image: image.mimetype == "image/jpeg"
            )
            post.instagram_image_id = jpeg_images[0] if jpeg_images else False

    @api.depends(
        lambda self: ["message", "image_ids"]
        + self._get_post_message_modifying_fields()
    )
    def _compute_instagram_preview(self):
        ...
        for post in self:
            post.instagram_preview = self.env.ref(
                "social_instagram.instagram_preview"
            )._render(
                {
                    "error_code": post._get_instagram_image_error(),
                    "image": post.instagram_image_id.with_context(bin_size=False).datas
                    if post.instagram_image_id
                    else False,
                    ...
                    "message": post._prepare_post_content(
                        ...
                        **{
                            field: post[field]
                            for field in post._get_post_message_modifying_fields()
                        }
                    ),
    ...
    def _get_instagram_image_error(self):
        ...
        if not self.image_ids:
            ...
            if not self.instagram_image_id:
                ...
                try:
                    image_base64 = self.instagram_image_id.with_context(
                        bin_size=False
                    ).datas
