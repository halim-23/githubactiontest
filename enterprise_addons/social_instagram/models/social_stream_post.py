...
class SocialStreamPostInstagram(models.Model):
    ...
    instagram_facebook_author_id = fields.Char(
    ...
    def _compute_author_link(self):
        ...
        super(
            SocialStreamPostInstagram, (self - instagram_posts)
        )._compute_author_link()

        for post in instagram_posts:
            post.author_link = "https://www.instagram.com/%s" % url_quote(
                post.author_name
            )
    ...
    def _instagram_comment_fetch(self, next_records_token=False, count=20):
        ...
        return {
            "comments": [
                self._instagram_format_comment(comment)
                for comment in response.get("data", [])
            ],
            "nextRecordsToken": response.get("paging").get("cursors").get("after")
    ...
    def _instagram_comment_delete(self, comment_id):
        ...
        comments_endpoint_url = url_join(
            self.env["social.media"]._INSTAGRAM_ENDPOINT, "/v10.0/%s" % comment_id
        )
        requests.delete(
    ...
    def _instagram_format_comment(self, comment):
        return {
            ...
            "from": {
                ...
                "id": comment.get("user").get("id") if "user" in comment else -1,
            },
            ...
            "formatted_created_time": self.env[
                ...
                fields.Datetime.from_string(
                    dateutil.parser.parse(comment.get("timestamp")).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                )
            ),
            "likes": {"summary": {"total_count": comment.get("like_count")}},
            ...
            "comments": {
                ...
                "data": [
                    self._instagram_format_comment(comment)
                    for comment in comment["replies"].get("data", [])
                ][::-1]
            }
