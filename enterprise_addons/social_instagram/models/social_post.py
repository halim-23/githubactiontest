...
class SocialPostInstagram(models.Model):
            ...
            if "instagram" in post.media_ids.mapped("media_type"):
                ...
                if image_error_code == "missing":
                    raise ValidationError(
                        _("An image is required when posting on Instagram.")
                    )
                elif image_error_code == "wrong_extension":
                    raise ValidationError(
                        _("Only .jpg/.jpeg images can be posted on Instagram.")
                    )
                elif image_error_code == "incorrect_ratio":
                    raise ValidationError(
                        _(
                            "Your image has to be within the 4:5 and the 1.91:1 aspect ratio as required by Instagram."
                        )
                    )
    ...
    def _get_stream_post_domain(self):
        ...
        instagram_post_ids = [
            instagram_post_id
            for instagram_post_id in self.live_post_ids.mapped("instagram_post_id")
            if instagram_post_id
        ]
        if instagram_post_ids:
            return expression.OR(
                [domain, [("instagram_post_id", "in", instagram_post_ids)]]
            )
        else:
            return domain
