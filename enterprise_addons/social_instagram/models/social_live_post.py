    ...
    def _refresh_statistics(self):
        ...
        for account in accounts:
            ...
            existing_live_posts = (
                ...
                .sudo()
                .search([("instagram_post_id", "in", instagram_post_ids)])
            )

            existing_live_posts_by_instagram_post_id = {
                live_post.instagram_post_id: live_post
                for live_post in existing_live_posts
            }

            for post in response["data"]:
                existing_live_post = existing_live_posts_by_instagram_post_id.get(
                    post.get("id")
                )
                if existing_live_post:
                    existing_live_post.write(
                        {
                            "engagement": post.get("comments_count", 0)
                            + post.get("like_count", 0)
                        }
                    )

    def _post(self):
        instagram_live_posts = self.filtered(
            lambda post: post.account_id.media_type == "instagram"
        )
    ...
    def _post_instagram(self):
        ...
        if media_result.status_code != 200 or not media_result.json().get("id"):
            self.write(
                {
                    "state": "failed",
                    "failure_reason": json.loads(media_result.text or "{}")
                    .get("error", {})
                    .get("message", ""),
                }
            )
        ...
        if publish_result.status_code == 200:
            ...
            values = {
                "state": "failed",
                "failure_reason": json.loads(publish_result.text or "{}")
                .get("error", {})
                .get("message", ""),
            }
