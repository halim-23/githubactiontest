...
class ReportBomStructure(models.AbstractModel):
    ...
    def _get_bom(
        ...
        product_tmpl_id = (res["product"] and res["product"].product_tmpl_id.id) or (
            res["bom"] and res["product"].product_tmpl_id.id
        )
        res["ecos"] = (
            self.env["mrp.eco"].search_count(
                [("product_tmpl_id", "=", product_tmpl_id), ("state", "!=", "done")]
            )
            or ""
        )
        return res
