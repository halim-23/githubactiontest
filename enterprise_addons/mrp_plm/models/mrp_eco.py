    ...
    def _compute_nb(self):
        ...
        for eco_type in self:
            ...
            eco_type.nb_approvals = MrpEco.search_count(
                [("type_id", "=", eco_type.id), ("approval_ids.status", "=", "none")]
            )
...
class MrpEcoApprovalTemplate(models.Model):
    ...
    user_ids = fields.Many2many(
        ...
        string="Users",
        domain=lambda self: [
            ("groups_id", "in", self.env.ref("mrp_plm.group_plm_user").id)
        ],
        required=True,
    )
...
class MrpEcoApproval(models.Model):
    ...
    eco_id = fields.Many2one("mrp.eco", "ECO", ondelete="cascade", required=True)
    approval_template_id = fields.Many2one(
        "mrp.eco.approval.template", "Template", ondelete="cascade", required=True
    )
    name = fields.Char(
        "Role", related="approval_template_id.name", store=True, readonly=False
    )
    user_id = fields.Many2one("res.users", "Approved by")
    required_user_ids = fields.Many2many(
        "res.users",
        string="Requested Users",
...
class MrpEcoStage(models.Model):
    ...
    allow_apply_change = fields.Boolean(
        string="Allow to apply changes", help="Allow to apply changes from this stage."
    )
    final_stage = fields.Boolean(
        string="Final Stage",
        help="Once the changes are applied, the ECOs will be moved to this stage.",
    )
    ...
    def _compute_is_blocking(self):
        for rec in self:
            rec.is_blocking = any(
                ...
                for template in rec.approval_template_ids
            )
...
class MrpEco(models.Model):
    ...
    def _get_type_selection(self):
        return [("bom", _("Bill of Materials")), ("product", _("Product Only"))]
    ...
    stage_id = fields.Many2one(
        ...
        default=lambda self: self.env["mrp.eco.stage"].search(
            [("type_ids", "in", self._context.get("default_type_id"))], limit=1
        ),
    )
    ...
    priority = fields.Selection(
        [("0", "Normal"), ("1", "High")], string="Priority", tracking=True, index=True
    )
    note = fields.Html("Note")
    effectivity = fields.Selection(
        [("asap", "As soon as possible"), ("date", "At Date")],
        string="Effective",  # Is this English ?
        compute="_compute_effectivity",
        inverse="_set_effectivity",
        store=True,
        help="Date on which the changes should be applied. For reference only.",
    )
    effectivity_date = fields.Datetime(
        "Effective Date", tracking=True, help="For reference only."
    )
    approval_ids = fields.One2many(
        "mrp.eco.approval", "eco_id", "Approvals", help="Approvals by stage"
    )
    ...
    user_can_approve = fields.Boolean(
        "Can Approve",
        compute="_compute_user_approval",
    ...
    kanban_state = fields.Selection(
        [("normal", "In Progress"), ("done", "Approved"), ("blocked", "Blocked")],
        string="Kanban State",
        copy=False,
    ...
    type = fields.Selection(
        selection=_get_type_selection, string="Apply on", default="bom", required=True
    )
    ...
    new_bom_id = fields.Many2one("mrp.bom", "New Bill of Materials", copy=False)
    new_bom_revision = fields.Integer(
        "BoM Revision", related="new_bom_id.version", store=True, readonly=False
    )
    bom_change_ids = fields.One2many(
        "mrp.eco.bom.change",
        "eco_id",
        string="ECO BoM Changes",
        compute="_compute_bom_change_ids",
        help="Difference between old BoM and new BoM revision",
        store=True,
    )
    bom_rebase_ids = fields.One2many(
        "mrp.eco.bom.change", "rebase_id", string="BoM Rebase"
    )
    routing_change_ids = fields.One2many(
        "mrp.eco.routing.change",
        "eco_id",
        string="ECO Routing Changes",
        compute="_compute_routing_change_ids",
    ...
    displayed_image_attachment_id = fields.Many2one(
        "ir.attachment", related="displayed_image_id.ir_attachment_id", readonly=False
    )
    color = fields.Integer("Color")
    active = fields.Boolean(
        "Active",
        default=True,
        help="If the active field is set to False, it will allow you to hide the engineering change order without removing it.",
    )
    current_bom_id = fields.Many2one("mrp.bom", string="New Bom")
    previous_change_ids = fields.One2many(
        "mrp.eco.bom.change",
        "eco_rebase_id",
        string="Previous ECO Changes",
        compute="_compute_previous_bom_change",
        store=True,
    )
    ...
    def _is_conflict(self, new_bom_lines, changes=None):
        ...
        for reb_line in changes:
            ...
            if new_line and (
                reb_line.old_operation_id,
                reb_line.old_uom_id,
                reb_line.old_product_qty,
            ) != (new_line.operation_id, new_line.product_uom_id, new_line.product_qty):
        ...
        if self.new_bom_id:
            for line in new_bom.bom_line_ids:
                ...
                if old_line and (
                    ...
                    old_line.product_uom_id,
                    old_line.product_qty,
                    old_line.operation_id,
                ):
                    new_bom_commands += [
                        (
                            0,
        ...
        for reb_line in rebase_lines:
            ...
            if new_bom_line:
                if new_bom_line.product_qty + reb_line.upd_product_qty > 0.0:
                    ...
                    new_bom_line.write(
                        {
                            "product_qty": new_bom_line.product_qty
                            + reb_line.upd_product_qty,
    ...
    def apply_rebase(self):
        ...
        if self.bom_rebase_ids:
            new_bom_lines = {
                (line.product_id): line for line in self.new_bom_id.bom_line_ids
            }
            if self._is_conflict(new_bom_lines, self.bom_rebase_ids):
                return self.write({"state": "conflict"})
            else:
                old_bom_lines = {
                    (line.product_id): line for line in self.bom_id.bom_line_ids
                }
        ...
        if self.previous_change_ids:
            ...
            if self._is_conflict(new_bom_lines, self.previous_change_ids):
                ...
                new_activated_bom_lines = {
                    (line.product_id): line
                    for line in self.current_bom_id.bom_line_ids
                }
                self.rebase(
                    new_activated_bom_lines, new_bom_lines, self.previous_change_ids
                )
                # Remove all rebase line of current eco.
        ...
        if self.current_bom_id:
            self.new_bom_id.write(
                {
                    "version": self.current_bom_id.version + 1,
                    "previous_bom_id": self.current_bom_id.id,
                }
            )
    ...
    @api.depends(
        "bom_id.bom_line_ids",
        "new_bom_id.bom_line_ids",
        "new_bom_id.bom_line_ids.product_qty",
        "new_bom_id.bom_line_ids.product_uom_id",
        "new_bom_id.bom_line_ids.operation_id",
    )
    def _compute_bom_change_ids(self):
        # Compute difference between old bom and new bom revision.
        for eco in self:
            eco.bom_change_ids = eco._get_difference_bom_lines(
                eco.bom_id, eco.new_bom_id
            )

    @api.depends(
        "bom_id.bom_line_ids",
        "current_bom_id.bom_line_ids",
        "current_bom_id.bom_line_ids.product_qty",
        "current_bom_id.bom_line_ids.product_uom_id",
        "current_bom_id.bom_line_ids.operation_id",
    )
    def _compute_previous_bom_change(self):
        for eco in self:
            if eco.current_bom_id:
                # Compute difference between old bom and newly activated bom.
                eco.previous_change_ids = eco._get_difference_bom_lines(
                    eco.bom_id, eco.current_bom_id
                )
            else:
    ...
    @api.depends(
        ...
        "bom_id.operation_ids.active",
        "new_bom_id.operation_ids",
        "new_bom_id.operation_ids.active",
    )
            ...
            if rec.new_bom_id and rec.bom_id:
                for operation in rec.new_bom_id.operation_ids:
                    ...
                    if old_ops:
                        ...
                        if (
                            tools.float_compare(
                                old_op.time_cycle_manual, operation.time_cycle_manual, 2
                            )
                            != 0
                        ):
                        ...
                        new_routing_commands += self._prepare_detailed_change_commands(
                            operation, old_op
                        )
                    else:
                        new_routing_commands += [
    ...
    def _compute_user_approval(self):
        for eco in self:
            is_required_approval = eco.stage_id.approval_template_ids.filtered(
                lambda x: x.approval_type in ("mandatory", "optional")
                and self.env.user in x.user_ids
            )
            user_approvals = eco.approval_ids.filtered(
                lambda x: x.template_stage_id == eco.stage_id
                and x.user_id == self.env.user
                and not x.is_closed
            )
            last_approval = user_approvals.sorted(
                lambda a: a.create_date, reverse=True
            )[:1]
            eco.user_can_approve = (
                is_required_approval and not last_approval.is_approved
            )
    ...
    def _compute_kanban_state(self):
        ...
        for rec in self:
            approvals = rec.approval_ids.filtered(
                lambda app: app.template_stage_id == rec.stage_id and not app.is_closed
            )
            if not approvals:
    ...
    def _compute_allow_change_stage(self):
        for rec in self:
            approvals = rec.approval_ids.filtered(
                lambda app: app.template_stage_id == rec.stage_id
            )
            if approvals:
    ...
    def _compute_allow_apply_change(self):
        for rec in self:
            rec.allow_apply_change = rec.stage_id.allow_apply_change and rec.state in (
                "confirmed",
                "progress",
            )
    ...
    def _compute_allow_change_kanban_state(self):
        for rec in self:
            rec.allow_change_kanban_state = (
                False if rec.stage_id.approval_template_ids else True
            )
    ...
    def onchange_type_id(self):
        self.stage_id = (
            ...
            .search([("type_ids", "in", self.type_id.id)], limit=1)
            .id
        )
    ...
    def write(self, vals):
        if vals.get("stage_id"):
            ...
            for eco in self:
                if eco.stage_id and (
                    (newstage.sequence, newstage.id)
                    > (eco.stage_id.sequence, eco.stage_id.id)
                ):
                    if not eco.allow_change_stage:
                        raise UserError(
                            _(
                                "You cannot change the stage, as approvals are still required."
                            )
                        )
                ...
                if eco.stage_id != newstage:
                    eco.approval_ids.filtered(lambda x: x.status != "none").write(
                        {"is_closed": True}
                    )
    ...
    def message_post(self, **kwargs):
        ...
        ):  # should use message_values to avoid a read
            for eco in self:
                for approval in eco.approval_ids.filtered(
                    lambda app: app.template_stage_id == eco.stage_id
                    and app.status == "none"
                    and app.approval_template_id.approval_type == "comment"
                ):
                    if self.env.user in approval.approval_template_id.user_ids:
                        approval.write({"status": "comment", "user_id": self.env.uid})
        return message
    ...
    def _create_or_update_approval(self, status):
        for eco in self:
            for approval_template in eco.stage_id.approval_template_ids.filtered(
                lambda a: self.env.user in a.user_ids
            ):
                approvals = eco.approval_ids.filtered(
                    lambda x: x.approval_template_id == approval_template
                    and not x.is_closed
                )
                none_approvals = approvals.filtered(lambda a: a.status == "none")
                confirmed_approvals = approvals - none_approvals
                if none_approvals:
                    none_approvals.write(
                        {
                            "status": status,
                ...
                eco.message_post_with_view(
                    "mrp_plm.message_approval", values={"approval": approval}
                )
    ...
    def conflict_resolve(self):
        ...
        self.new_bom_id.write(
            {"version": self.bom_id.version + 1, "previous_bom_id": self.bom_id.id}
        )
    ...
    def action_new_revision(self):
        ...
        for eco in self:
            if eco.type == "bom":
                eco.new_bom_id = eco.bom_id.copy(
                    default={
                ...
                attachments = IrAttachment.search(
                    [("res_model", "=", "mrp.bom"), ("res_id", "=", eco.bom_id.id)]
                )
            else:
                attachments = IrAttachment.search(
                    [
                ...
                if eco.type == "product":
                    for attach in eco.with_context(active_test=False).mrp_document_ids:
                        ...
                        if not attach.active:
                            origin.unlink()
                            continue
                        if origin._compute_checksum(
                            origin.raw
                        ) == origin._compute_checksum(attach.raw):
                            if attach.origin_attachment_id.name != attach.name:
                                attach.origin_attachment_id.name = attach.name
                            if (
                                attach.origin_attachment_id.company_id
                                != attach.company_id
                            ):
                                attach.origin_attachment_id.company_id = (
                                    attach.company_id
...
class MrpEcoBomChange(models.Model):
    ...
    old_operation_id = fields.Many2one(
        "mrp.routing.workcenter", "Previous Consumed in Operation"
    )
    new_operation_id = fields.Many2one(
        "mrp.routing.workcenter", "New Consumed in Operation"
    )
    upd_product_qty = fields.Float("Quantity", compute="_compute_change", store=True)
    uom_change = fields.Char(
        "Unit of Measure", compute="_compute_change", compute_sudo=True
    )
    operation_change = fields.Char(
        compute="_compute_change", string="Consumed in Operation", compute_sudo=True
    )
    ...
    def _compute_change(self):
        for rec in self:
            ...
            rec.operation_change = (
                rec.new_operation_id.name
                if rec.change_type == "add"
                else rec.old_operation_id.name
            )
            ...
            if (
                rec.old_operation_id != rec.new_operation_id
            ) and rec.change_type == "update":
                rec.operation_change = (
                    (rec.old_operation_id.name or "")
                    + " -> "
                    + (rec.new_operation_id.name or "")
                )
...
class MrpEcoRoutingChange(models.Model):
    ...
    eco_id = fields.Many2one(
        "mrp.eco", "Engineering Change", ondelete="cascade", required=True
    )
    change_type = fields.Selection(
        [("add", "Add"), ("remove", "Remove"), ("update", "Update")],
        string="Type",
    ...
    upd_time_cycle_manual = fields.Float(
        "Manual Duration Change", compute="_compute_upd_time_cycle_manual", store=True
    )
    operation_id = fields.Many2one(
        "mrp.routing.workcenter", "New or Previous Operation"
    )
    ...
    def _compute_upd_time_cycle_manual(self):
        for rec in self:
            rec.upd_time_cycle_manual = (
                rec.new_time_cycle_manual - rec.old_time_cycle_manual
            )
