...
class SocialStreamYoutube(models.Model):
    ...
    def _fetch_channel_videos(self):
        ...
        playlist_items_endpoint = url_join(
            self.env["social.media"]._YOUTUBE_ENDPOINT, "youtube/v3/playlistItems"
        )
        ...
        existing_posts = self.env["social.stream.post"].search(
            [("stream_id", "=", self.id), ("youtube_video_id", "in", youtube_video_ids)]
        )
        ...
        for video in video_items_response["items"]:
            ...
            values = {
                ...
                "published_date": fields.Datetime.from_string(
                    dateutil.parser.parse(video_info["publishedAt"]).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                ),
            ...
            if youtube_duration:
                ...
                values["youtube_video_duration"] = (
                    seconds / 60 + minutes + (hours * 60) + (days * 1440)
                )
