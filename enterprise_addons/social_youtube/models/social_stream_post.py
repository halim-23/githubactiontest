...
class SocialStreamPostYoutube(models.Model):
    ...
    def _compute_author_link(self):
        ...
        for post in youtube_posts:
            post.author_link = "http://www.youtube.com/channel/%s" % (
                post.stream_id.account_id.youtube_channel_id
            )
    ...
    def _compute_post_link(self):
        ...
        for post in youtube_posts:
            post.post_link = (
                "https://www.youtube.com/watch?v=%s" % post.youtube_video_id
            )
        ...
        if comment_id:
            if is_edit:
                ...
                result_comment = requests.put(
                    url_join(
                        self.env["social.media"]._YOUTUBE_ENDPOINT,
                        "youtube/v3/comments",
                    ),
                ...
                result_comment = requests.post(
                    ...
                    params=common_params,
                    json={"snippet": {"textOriginal": message, "parentId": comment_id}},
                    timeout=5,
                ).json()
        else:
    ...
    def _youtube_comment_delete(self, comment_id):
        ...
        response = requests.delete(
            url=url_join(
                self.env["social.media"]._YOUTUBE_ENDPOINT, "youtube/v3/comments"
            ),
            params={
                "id": comment_id,
                "access_token": self.account_id.youtube_access_token,
    ...
    def _youtube_comment_fetch(self, next_page_token=False, count=20):
        ...
        comments_endpoint_url = url_join(
            self.env["social.media"]._YOUTUBE_ENDPOINT, "youtube/v3/commentThreads"
        )
        params = {
        ...
        if not result.ok:
            ...
            if result_json.get("error"):
                ...
                if error_code == 404 and error_reason == "videoNotFound":
                    error_message = _(
                        "Video not found. It could have been removed from Youtube."
                    )
                elif error_code == 403 and error_reason == "commentsDisabled":
                    error_message = _(
                        "Comments are marked as 'disabled' for this video. It could have been set as 'private'."
                    )
        ...
        for comment in result_json.get("items", []):
            ...
            youtube_comment_replies = [
                ...
                for reply in list(
                    reversed(comment.get("replies", {}).get("comments", []))
                )
            ]
            if youtube_comment_replies:
                youtube_comment["comments"] = {"data": youtube_comment_replies}

            comments.append(youtube_comment)

        return {"comments": comments, "nextPageToken": result_json.get("nextPageToken")}
    ...
    def _gc_youtube_data(self):
        ...
        youtube_stream = self.env.ref(
            "social_youtube.stream_type_youtube_channel_videos"
        )
