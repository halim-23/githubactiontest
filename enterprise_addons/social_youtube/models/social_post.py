...
class SocialPostYoutube(models.Model):
    ...
    youtube_accounts_count = fields.Integer(
        "Selected YouTube Accounts", compute="_compute_youtube_accounts_count"
    )
    youtube_accounts_other_count = fields.Integer(
        "Selected Other Accounts", compute="_compute_youtube_accounts_count"
    )
    ...
    def _check_message_not_empty(self):
        ...
        for social_post in self:
            ...
            elif not social_post.message and [
                "youtube"
            ] != social_post.media_ids.mapped("media_type"):
                raise UserError(
                    _("The 'message' field is required for post ID %s", social_post.id)
                )
    ...
    def _compute_youtube_access_token(self):
        for post in self:
            youtube_account = post.account_ids.filtered(
    ...
    @api.depends(
        "youtube_title", "youtube_description", "youtube_video_id", "scheduled_date"
    )
    def _compute_youtube_preview(self):
        for post in self:
            post.youtube_preview = self.env.ref(
                "social_youtube.youtube_preview"
            )._render(
                {
                    ...
                    "published_date": post.scheduled_date
    ...
    def _compute_youtube_accounts_count(self):
        for post in self:
            post.youtube_accounts_count = len(
                post.account_ids.filtered(
                    lambda account: account.media_type == "youtube"
                )
            )
            post.youtube_accounts_other_count = (
                len(post.account_ids) - post.youtube_accounts_count
            )
    ...
    def _check_post_access(self):
        ...
        for social_post in self:
            ...
            if (
                not social_post.youtube_video_id
                and "youtube" in social_post.media_ids.mapped("media_type")
            ):
                raise UserError(
                    _("You have to upload a video when posting on YouTube.")
                )
    ...
    def _get_stream_post_domain(self):
        ...
        youtube_video_ids = [
            youtube_video_id
            for youtube_video_id in self.mapped("youtube_video_id")
            if youtube_video_id
        ]
        if youtube_video_ids:
            return expression.OR(
                [domain, [("youtube_video_id", "in", youtube_video_ids)]]
            )
        else:
            return domain
    ...
    def _prepare_post_content(self, message, media_type, **kw):
        message = super(SocialPostYoutube, self)._prepare_post_content(
            message, media_type, **kw
        )
    ...
    def _get_post_message_modifying_fields(self):
        return super(SocialPostYoutube, self)._get_post_message_modifying_fields() + [
            "youtube_video_id"
        ]
