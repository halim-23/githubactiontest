...
class SocialYoutubeController(SocialController):
    ...
    def youtube_account_callback(
        self,
        code=None,
        iap_access_token=None,
        iap_refresh_token=None,
        iap_expires_in=0,
        **kw
    ):
        ...
        if (not iap_access_token or not iap_access_token) and not code:
            return request.render(
                ...
                {
                    "error_message": _(
                        "YouTube did not provide a valid authorization code."
                    )
                },
            )
        ...
        youtube_oauth_client_id = (
            ...
            .sudo()
            .get_param("social.youtube_oauth_client_id")
        )
        youtube_oauth_client_secret = (
            request.env["ir.config_parameter"]
            .sudo()
            .get_param("social.youtube_oauth_client_secret")
        )
        ...
        if iap_access_token and iap_refresh_token:
            ...
            if not token_exchange_response.get("refresh_token"):
                return request.render(
                    ...
                    {
                        "error_message": _(
                            "Auth endpoint did not provide a refresh token. Please try again."
                        )
                    },
                )
    ...
    def social_youtube_comment(
        ...
        **kwargs
        ...
        return json.dumps(
            stream_post._youtube_comment_add(comment_id, message, is_edit=is_edit)
        )
    ...
    def social_youtube_get_comments(
        self, stream_post_id, next_page_token=False, comments_count=20
    ):
        stream_post = self._get_social_stream_post(stream_post_id, "youtube")
    ...
    def _youtube_create_accounts(self, access_token, refresh_token, expires_in):
        youtube_channels_endpoint = url_join(
            request.env["social.media"]._YOUTUBE_ENDPOINT, "youtube/v3/channels"
        )
        youtube_channels = requests.get(
        ...
        if "error" in youtube_channels:
            raise SocialValidationException(
                _(
                    "YouTube did not provide a valid access token or it may have expired."
                )
            )
        ...
        for channel in youtube_channels.get("items"):
            ...
            base_values = {
                ...
                "youtube_token_expiration_date": fields.Datetime.now()
                + timedelta(seconds=int(expires_in)),
                "youtube_upload_playlist_id": channel["contentDetails"][
                    "relatedPlaylists"
                ]["uploads"],
                "is_media_disconnected": False,
                "image": base64.b64encode(
                    requests.get(
                        channel["snippet"]["thumbnails"]["medium"]["url"], timeout=10
                    ).content
                ),
            }
    ...
    def _youtube_get_existing_accounts(self, youtube_channels):
        youtube_accounts_ids = [
            account["id"] for account in youtube_channels.get("items", [])
        ]
                ...
                .search(
                    [
                        (
                            "media_id",
                            "=",
                            request.env.ref("social_youtube.social_media_youtube").id,
                        ),
