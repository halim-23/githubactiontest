...
class HrLeave(models.Model):
    ...
    payslip_state = fields.Selection(
        [
            ...
            ("blocked", "To defer to next payslip"),
        ],
    ...
    def action_validate(self):
        ...
        for leave in self:
            if any(
                ...
                for payslip in waiting_payslips
            ):
    ...
    def _recompute_payslips(self):
        ...
        for leave in self:
            for payslip in all_payslips:
                if payslip.employee_id == leave.employee_id and (
                    payslip.date_from <= leave.date_to.date()
                    and payslip.date_to >= leave.date_from.date()
                ):
    ...
    def _cancel_work_entry_conflict(self):
        ...
        for leave in leaves_to_defer:
            leave.activity_schedule(
                ...
                note=_(
                    'Please create manually the work entry for <a href="#" data-oe-model="%s" data-oe-id="%s">%s</a>'
                )
                % (
                    leave.employee_id._name,
                    leave.employee_id.id,
                    leave.employee_id.display_name,
                ),
                user_id=leave.employee_id.company_id.deferred_time_off_manager.id
