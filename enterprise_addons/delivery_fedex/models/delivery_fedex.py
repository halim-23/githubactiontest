...
class ProviderFedex(models.Model):
    ...
    delivery_type = fields.Selection(
        ...
        ondelete={
            "fedex": lambda recs: recs.write(
                {"delivery_type": "fixed", "fixed_price": 0}
            )
        },
    )

    fedex_developer_key = fields.Char(
        string="Developer Key", groups="base.group_system"
    )
    fedex_developer_password = fields.Char(
        string="Password", groups="base.group_system"
    )
    fedex_account_number = fields.Char(
        string="FedEx Account Number", groups="base.group_system"
    )
    fedex_meter_number = fields.Char(string="Meter Number", groups="base.group_system")
    fedex_droppoff_type = fields.Selection(
        [
    ...
    fedex_default_package_type_id = fields.Many2one(
        "stock.package.type", string="Fedex Package Type"
    )
    ...
    fedex_label_stock_type = fields.Selection(
        FEDEX_STOCK_TYPE, string="Label Type", default="PAPER_LETTER"
    )
    fedex_label_file_type = fields.Selection(
        [("PDF", "PDF"), ("EPL2", "EPL2"), ("PNG", "PNG"), ("ZPLII", "ZPLII")],
        default="PDF",
        string="FEDEX Label File Type",
    )
    fedex_document_stock_type = fields.Selection(
        FEDEX_STOCK_TYPE, string="Commercial Invoice Type", default="PAPER_LETTER"
    )
    ...
    def fedex_rate_shipment(self, order):
        max_weight = self._fedex_convert_weight(
            self.fedex_default_package_type_id.max_weight, self.fedex_weight_unit
        )
        price = 0.0
        is_india = (
            order.partner_shipping_id.country_id.code == "IN"
            and order.company_id.partner_id.country_id.code == "IN"
        )

        weight_value = self._fedex_convert_weight(
            order._get_estimated_weight(), self.fedex_weight_unit
        )
        ...
        srm = FedexRequest(
            self.log_xml, request_type="rating", prod_environment=self.prod_environment
        )
        ...
        if (
            self.fedex_service_type
            in ["INTERNATIONAL_ECONOMY", "INTERNATIONAL_PRIORITY"]
            or is_india
        ):
            total_commodities_amount = 0.0
            commodity_country_of_manufacture = (
                order.warehouse_id.partner_id.country_id.code
            )
    ...
    def fedex_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            srm.client_detail(
                superself.fedex_account_number, superself.fedex_meter_number
            )
            ...
            package_type = (
                picking.package_ids
                and picking.package_ids[0].package_type_id.shipper_package_code
                or self.fedex_default_package_type_id.shipper_package_code
            )
            srm.shipment_request(
                self.fedex_droppoff_type,
                self.fedex_service_type,
            ...
            if self.fedex_service_type in [
                ...
                for operation in picking.move_line_ids:
                    ...
                    commodity_amount = (
                        round(operation.sale_price / commodity_quantity, 2)
                        if commodity_quantity
                        else operation.sale_price
                    )
            ...
            if (
                ...
                po_number = (
                    "B2B"
                    if picking.partner_id.commercial_partner_id.is_company
                    else "B2C"
                )
            ...
            if package_count > 1:
                ...
                for sequence, package in enumerate(picking.package_ids, start=1):
                    ...
                    srm.set_master_package(
                        net_weight, package_count, master_tracking_id=master_tracking_id
                    )
            ...
            ###############
                ...
                package_type = (
                    picking.package_ids[:1].package_type_id
                    or picking.carrier_id.fedex_default_package_type_id
                )
                srm._add_package(
                    net_weight,
                    package_code=package_type.shipper_package_code,
                ...
                if not request.get("errors_message"):
                    ...
                    if _convert_curr_iso_fdx(order_currency.name) in request["price"]:
                        ...
                        if (
                            _convert_curr_iso_fdx(company_currency.name)
                            in request["price"]
                        ):
                    ...
                    fedex_labels = [
                        (
                            "LabelFedex-%s-%s.%s"
                            % (carrier_tracking_ref, index, self.fedex_label_file_type),
                            label,
                        )
            ...
            if commercial_invoice:
                ...
                picking.message_post(
                    body="Fedex Documents", attachments=fedex_documents
                )
        return res
    ...
    def fedex_cancel_shipment(self, picking):
        request = FedexRequest(
            self.log_xml,
            request_type="shipping",
            prod_environment=self.prod_environment,
        )
        superself = self.sudo()
        request.web_authentication_detail(
            superself.fedex_developer_key, superself.fedex_developer_password
        )
        ...
        if result.get("delete_success") and not result.get("errors_message"):
            picking.message_post(
                body=_("Shipment #%s has been cancelled", master_tracking_id)
            )
    ...
    def _get_request_price(self, req_price, order, order_currency=None):
        ...
        currency_instances = self.env["res.currency"].search(
            [("name", "in", currency_codes)]
        )
