...
class FedexRequest:
    ...
    def __init__(
        ...
        if request_type == "shipping":
            wsdl_path = get_resource_path(
                "delivery_fedex", "api", wsdl_folder, "ShipService_v28.wsdl"
            )
            self.start_shipping_transaction(wsdl_path)
        elif request_type == "rating":
            wsdl_path = get_resource_path(
                "delivery_fedex", "api", wsdl_folder, "RateService_v31.wsdl"
            )
    ...
    def set_shipper(self, company_partner, warehouse_partner):
        ...
        Contact.PersonName = (
            remove_accents(company_partner.name)
            if not company_partner.is_company
            else ""
        )
        Contact.CompanyName = (
            remove_accents(company_partner.commercial_company_name) or ""
        )
    ...
    def set_recipient(self, recipient_partner):
        ...
        if recipient_partner.is_company:
            ...
            Contact.CompanyName = (
                remove_accents(recipient_partner.commercial_company_name) or ""
            )
    ...
    def shipment_request(
        ...
        self.RequestedShipment.SpecialServicesRequested = (
            self.factory.ShipmentSpecialServicesRequested()
        )
        ...
        if saturday_delivery:
            ...
            if (service_type == "FEDEX_2_DAY" and timestamp_day == "Thursday") or (
                service_type
                in ["PRIORITY_OVERNIGHT", "FIRST_OVERNIGHT", "INTERNATIONAL_PRIORITY"]
                and timestamp_day == "Friday"
            ):
    ...
    def _add_package(
        ...
        if package_code == "YOUR_PACKAGING":
            ...
            package.Dimensions.Units = (
                "IN" if self.RequestedShipment.TotalWeight.Units == "LB" else "CM"
            )
        if po_number:
    ...
    def rate(self):
        ...
        if self.hasCommodities:
            self.RequestedShipment.CustomsClearanceDetail.Commodities = (
                self.listCommodities
            )

        try:
            self.response = self.client.service.getRates(
            ...
            if (
                self.response.HighestSeverity != "ERROR"
                and self.response.HighestSeverity != "FAILURE"
            ):
                ...
                for rating in self.response.RateReplyDetails[0].RatedShipmentDetails:
                    formatted_response["price"][
                        rating.ShipmentRateDetail.TotalNetFedExCharge.Currency
                    ] = float(rating.ShipmentRateDetail.TotalNetFedExCharge.Amount)
                if len(self.response.RateReplyDetails[0].RatedShipmentDetails) == 1:
                    if (
                        "CurrencyExchangeRate"
                        in self.response.RateReplyDetails[0]
                        .RatedShipmentDetails[0]
                        .ShipmentRateDetail
                        and self.response.RateReplyDetails[0]
                        .RatedShipmentDetails[0]
                        .ShipmentRateDetail["CurrencyExchangeRate"]
                    ):
                        formatted_response["price"][
                            self.response.RateReplyDetails[0]
                            .RatedShipmentDetails[0]
                            .ShipmentRateDetail.CurrencyExchangeRate.FromCurrency
                        ] = float(
                            self.response.RateReplyDetails[0]
                            .RatedShipmentDetails[0]
                            ...
                            self.response.RateReplyDetails[0]
                            .RatedShipmentDetails[0]
                            .ShipmentRateDetail.CurrencyExchangeRate.Rate
                        )
            else:
                errors_message = "\n".join(
                    [
    ...
    def start_shipping_transaction(self, wsdl_path):
        self.client = Client(
            "file:///%s" % wsdl_path.lstrip("/"), plugins=[LogPlugin(self.debug_logger)]
        )
        self.factory = self.client.type_factory("ns0")
        self.VersionId = self.factory.VersionId()
    ...
    def duties_payment(self, sender_party, responsible_account_number, payment_type):
        self.RequestedShipment.CustomsClearanceDetail.DutiesPayment = (
            self.factory.Payment()
        )
        self.RequestedShipment.CustomsClearanceDetail.DutiesPayment.PaymentType = (
            payment_type
        )
    ...
    def customs_value(
        self, customs_value_currency, customs_value_amount, document_content
    ):
        self.RequestedShipment.CustomsClearanceDetail = (
            self.factory.CustomsClearanceDetail()
        )
        self.RequestedShipment.CustomsClearanceDetail.CustomsValue = (
            self.factory.Money()
        )
        self.RequestedShipment.CustomsClearanceDetail.CustomsValue.Currency = (
            customs_value_currency
        )
        self.RequestedShipment.CustomsClearanceDetail.CustomsValue.Amount = (
            customs_value_amount
        )
        if (
            self.RequestedShipment.Shipper.Address.CountryCode == "IN"
            and self.RequestedShipment.Recipient.Address.CountryCode == "IN"
        ):
            if not self.RequestedShipment.CustomsClearanceDetail.CommercialInvoice:
                self.RequestedShipment.CustomsClearanceDetail.CommercialInvoice = (
                    self.factory.CommercialInvoice()
                )
            else:
                del (
                    self.RequestedShipment.CustomsClearanceDetail.CommercialInvoice.TaxesOrMiscellaneousChargeType
                )
            self.RequestedShipment.CustomsClearanceDetail.CommercialInvoice.Purpose = (
                "SOLD"
            )
    ...
    def return_label(self, tracking_number, origin_date):
        ...
        self.RequestedShipment.SpecialServicesRequested.ReturnShipmentDetail = (
            return_details
        )
        if self.hasCommodities:
    ...
    def process_shipment(self):
        ...
        try:
            self.response = self.client.service.processShipment(
                WebAuthenticationDetail=self.WebAuthenticationDetail,
                ClientDetail=self.ClientDetail,
                TransactionDetail=self.TransactionDetail,
                Version=self.VersionId,
            ...
            if (
                ...
                formatted_response["tracking_number"] = (
                    self.response.CompletedShipmentDetail.CompletedPackageDetails[0]
                    .TrackingIds[0]
                    .TrackingNumber
                )
                if (
                    "CommitDate"
                    in self.response.CompletedShipmentDetail.OperationalDetail
                ):
                    formatted_response[
                        "date"
                    ] = (
                        self.response.CompletedShipmentDetail.OperationalDetail.CommitDate
                    )
                else:
                    formatted_response["date"] = date.today()

                if (
                    self.RequestedShipment.RequestedPackageLineItems.SequenceNumber
                    == self.RequestedShipment.PackageCount
                ) or self.hasOnePackage:
                    if (
                        "ShipmentRating" in self.response.CompletedShipmentDetail
                        and self.response.CompletedShipmentDetail.ShipmentRating
                    ):
                        for (
                            rating
                        ) in (
                            self.response.CompletedShipmentDetail.ShipmentRating.ShipmentRateDetails
                        ):
                            formatted_response["price"][
                                rating.TotalNetFedExCharge.Currency
                            ] = float(rating.TotalNetFedExCharge.Amount)
                            if (
                                "CurrencyExchangeRate" in rating
                                and rating.CurrencyExchangeRate
                            ):
                                formatted_response["price"][
                                    rating.CurrencyExchangeRate.FromCurrency
                                ] = float(
                                    rating.TotalNetFedExCharge.Amount
                                    / rating.CurrencyExchangeRate.Rate
                                )
                    else:
                ...
                if "MasterTrackingId" in self.response.CompletedShipmentDetail:
                    formatted_response[
                        ...
                        self.response.CompletedShipmentDetail.MasterTrackingId.TrackingNumber
                    )
    ...
    def _get_labels(self, file_type):
        ...
        if (
            file_type.upper() in ["PNG"]
            and self.response.CompletedShipmentDetail.CompletedPackageDetails[
                0
            ].PackageDocuments
        ):
            for auxiliary in (
    ...
    def get_document(self):
        if self.response.CompletedShipmentDetail.ShipmentDocuments:
            return (
                self.response.CompletedShipmentDetail.ShipmentDocuments[0]
                .Parts[0]
                .Image
            )
        else:
    ...
    def delete_shipment(self):
        ...
        try:
            ...
            self.response = self.client.service.deleteShipment(
                WebAuthenticationDetail=self.WebAuthenticationDetail,
                ClientDetail=self.ClientDetail,
                TransactionDetail=self.TransactionDetail,
                Version=self.VersionId,
