...
class TestDeliveryFedexInternal(TransactionCase):
    ...
    def setUp(self):
        ...
        self.env["res.currency.rate"].create(
            {"currency_id": self.currency_gbp_id.id, "rate": 0.5, "name": "2001-01-01"}
        )
        self.env["res.currency.rate"].create(
            {"currency_id": self.currency_eur_id.id, "rate": 0.8, "name": "2001-01-01"}
        )
        self.cr.execute(
            "UPDATE res_company SET currency_id = %s", [self.currency_gbp_id.id]
        )
        self.cr.execute(
            "UPDATE res_currency SET active = true where name in ('EUR', 'GBP', 'USD')"
        )
    ...
    def test_get_request_price_order_currency(self):
        price = self.carrier._get_request_price(
            {"USD": 1.234, "UKL": 999}, self.sale_order, self.currency_usd_id
        )
    ...
    def test_get_request_price_company_currency(self):
        ...
        price = self.carrier._get_request_price(
            {"UKL": 4, "EUR": 999}, self.sale_order, self.currency_usd_id
        )
    ...
    def test_get_request_price_other_currency(self):
        ...
        price = self.carrier._get_request_price(
            {"EUR": 4}, self.sale_order, self.currency_usd_id
        )
        self.assertEqual(4 / 0.8, price)

    def test_get_request_price_other_currency_iso(self):
        # case where Fedex returns GBP instead of UKL
        price = self.carrier._get_request_price(
            {"GBP": 4}, self.sale_order, self.currency_usd_id
        )
        self.assertEqual(4 * 2, price)
