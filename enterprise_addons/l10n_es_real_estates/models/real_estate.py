...
class RealEstate(models.Model):
    ...
    name = fields.Char(
        string="Name",
        required=True,
        help="Name to display ot identify this real estate.",
    )
    invoice_ids = fields.One2many(
        string="Related Invoices",
        inverse_name="l10n_es_real_estate_id",
        comodel_name="account.move",
        required=True,
    )

    cadastral_reference = fields.Selection(
        selection=[
            ("1", "Spanish Territory"),
    ...
    street_type = fields.Char(
        ...
        size=5,
        required=True,
        help="Type of street, normalized according to INE.",
    )
    street_name = fields.Char(
        string="Street Name",
        size=50,
        required=True,
        help="Name of the street where the building is located.",
    )
    street_number_type = fields.Selection(
        string="Type of Street Number",
        selection=[("NUM", "Number"), ("KM", "Kilometer"), ("S/N", "Without Number")],
        required=True,
        default="NUM",
        help="Type of the data contained in the street_number field.",
    )
    street_number = fields.Integer(string="Street Number")
    street_number_km_qualifier = fields.Selection(
        string="Street Number Qualifier",
        selection=[("BIS", "Bis"), ("MOD", "Mod"), ("DUP", "Dup"), ("ANT", "Ant")],
        help="Qualifier for KM-typed street number",
    )
    street_block = fields.Char(
        string="Block Number", size=3, help="Number of the building block in the street"
    )
    ...
    address_complement = fields.Char(
        string="Address Complement",
        size=40,
        help="Any data necessary to complete the address",
    )
    city = fields.Char(
        string="City",
        size=30,
        help="The city, if it is different from the municipality.",
    )
    municipality = fields.Char(
        string="Municipality", size=30, required=True, help="Name of the Municipality"
    )
    municipality_code = fields.Char(
        string="Municipality Code",
        size=5,
        required=True,
        help="Municipality code, as given by INE",
    )
