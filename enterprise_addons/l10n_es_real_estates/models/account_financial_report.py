...
class AccountFinancialReport(models.Model):
    ...
    def _mod347_get_real_estates_data(self, boe_report_options, currency_id):
        ...
        count = self.env.ref(
            "l10n_es_real_estates.mod_347_statistics_real_estates_count"
        )
        count_line_data = self._get_lines(
            boe_report_options,
            line_id=self._build_line_id([("", count._name, count.id)]),
        )[0]
        count = sum(i["no_format"] for i in count_line_data["columns"])

        total = self.env.ref("l10n_es_real_estates.mod_347_real_estates")
        total_line_data = self._get_lines(
            boe_report_options,
            line_id=self._build_line_id([("", total._name, total.id)]),
        )[-1]
        total = currency_id.round(
            sum(i["no_format"] for i in total_line_data["columns"])
        )
    ...
    def _boe_export_mod347(self, options, current_company, period, year):
        ...
        """
        rslt = super(AccountFinancialReport, self)._boe_export_mod347(
        ...
        negocio_required_a = self._mod347_get_required_partner_ids_for_boe(
            "real_estates",
            year + "-01-01",
            year + "-12-31",
            boe_wizard,
        ...
        rslt += self._call_on_sublines(
            boe_report_options,
            "l10n_es_real_estates.mod_347_operations_real_estates_sold",
        ...
        negocio_required_b = self._mod347_get_required_partner_ids_for_boe(
            "real_estates",
            year + "-01-01",
            year + "-12-31",
            boe_wizard,
        ...
        rslt += self._call_on_sublines(
            boe_report_options,
            "l10n_es_real_estates.mod_347_operations_real_estates_bought",
            lambda report_data: self._mod347_write_type2_partner_record(
                report_data,
                year,
                current_company,
        ...
        rslt += self._call_on_sublines(
            boe_report_options,
            "l10n_es_real_estates.mod_347_real_estates",
            lambda report_data: self._mod347_write_type2_real_estates_records(
                report_data, year, current_company
            ),
        )
    ...
    def _mod347_get_required_partner_ids_for_boe(
        ...
        rslt = super(
            AccountFinancialReport, self
        )._mod347_get_required_partner_ids_for_boe(
            mod_invoice_type,
            date_from,
            date_to,
            boe_wizard,
        ...
        real_estates_vat_data = boe_wizard.real_estates_vat_mod347_data.filtered(
    ...
    def _mod347_write_type2_real_estates_records(
        self, report_data, year, current_company
    ):
        line_real_estate = self.env["l10n_es_reports.real.estate"].browse(
            report_data["line_data"]["id"]
        )
        ...
        # time in the loop.
        address = self._boe_format_string(
            line_real_estate.cadastral_reference, length=1
        )
        ...
        address += self._boe_format_string(
            line_real_estate.street_number_type, length=3
        )
        address += self._boe_format_string(
            line_real_estate.street_number
            and str(line_real_estate.street_number)
            or "",
            length=5,
        )
        address += self._boe_format_string(
            line_real_estate.street_number_km_qualifier or "", length=3
        )
        address += self._boe_format_string(
            line_real_estate.street_block or "", length=3
        )
        ...
        address += self._boe_format_string(
            line_real_estate.address_complement or "", length=40
        )
        ...
        for (partner, partner_invoices) in invoice_partner_map.items():
            ...
            rslt += self._boe_format_string(
                self._extract_spanish_tin(partner, except_if_foreign=False), length=9
            )
            rslt += self._boe_format_string(
                " " * 9
            )  # TIN of the legal representative (only useful if under 14 years of age)
            ...
            convert = lambda i: i.currency_id._convert(
            ...
            year_amount_sum = currency_id.round(
                sum(convert(i) for i in partner_invoices)
            )
