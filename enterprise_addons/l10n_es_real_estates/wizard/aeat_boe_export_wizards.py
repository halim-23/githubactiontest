        ...
        for data in self.real_estates_vat_mod347_data:
            if not real_estates_vat_dict.get(data.partner_id.id):
                ...
                for trimester in range(1, 5):
                    real_estates_vat_dict[data.partner_id.id][str(trimester)] = {
                        "local_negocio": {"A": 0, "B": 0},
                        "seguros": {"B": 0},
                        "otras": {"A": 0, "B": 0},
                    }

            real_estates_vat_dict[data.partner_id.id][data.trimester][
                data.operation_class
            ][data.operation_key] = data.perceived_amount
...
class Mod347BOERealEstatesVATData(models.TransientModel):
    ...
    trimester = fields.Selection(
        selection=[("1", "1st"), ("2", "2nd"), ("3", "3rd"), ("4", "4th")],
        required=True,
    )
