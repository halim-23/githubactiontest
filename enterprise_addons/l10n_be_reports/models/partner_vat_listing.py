...
class ReportL10nBePartnerVatListing(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        tag_ids = [
            self.env["ir.model.data"]._xmlid_to_res_id(k)
            for k in [
                "l10n_be.tax_report_line_00",
                "l10n_be.tax_report_line_01",
                "l10n_be.tax_report_line_02",
                "l10n_be.tax_report_line_03",
                "l10n_be.tax_report_line_45",
                "l10n_be.tax_report_line_49",
            ]
        ]
        tag_ids_2 = [
            self.env["ir.model.data"]._xmlid_to_res_id(k)
            for k in ["l10n_be.tax_report_line_54", "l10n_be.tax_report_line_64"]
        ]
        query = """
        ...
        for record in self.env.cr.dictfetchall():
            ...
            columns = [
                record["vat"].replace(" ", "").upper(),
                record["turnover"],
                record["vat_amount"],
            ]
            if not context.get("no_format", False):
                columns[1] = formatLang(
                    self.env, columns[1] or 0.0, currency_obj=currency_id
                )
                columns[2] = formatLang(
                    self.env, columns[2] or 0.0, currency_obj=currency_id
                )
            lines.append(
                {
  ...
  """
        ...
        ctx.update(
            {
                "no_format": True,
                "date_from": ctx["date_from"][0:4] + "-01-01",
                "date_to": ctx["date_from"][0:4] + "-12-31",
            }
        )
