...
class AccountReport(models.AbstractModel):
    ...
    def _get_belgian_xml_export_representative_node(self):
        ...
        if representative:
            vat_no, country_from_vat = self.env[
                "account.generic.tax.report"
            ]._split_vat_number_and_country_code(representative.vat or "")
            country = self.env["res.country"].search(
                [("code", "=", country_from_vat)], limit=1
            )
            phone = representative.phone or representative.mobile
            node_values = {
                "vat": stdnum.get_cc_module("be", "vat").compact(
                    vat_no
                ),  # Sanitize VAT number
                "name": representative.name,
                "street": "%s %s"
                % (representative.street or "", representative.street2 or ""),
                "zip": representative.zip,
                "city": representative.city,
                "country_code": (country or representative.country_id).code,
                "email": representative.email,
                "phone": phone
                and re.sub(r"[./()\s]", "", phone),  # Exclude what's not a number
            }

            missing_fields = [k for k, v in node_values.items() if not v or v == " "]
            if missing_fields:
                message = _(
                    "Some fields required for the export are missing. Please specify them."
                )
                action = {
