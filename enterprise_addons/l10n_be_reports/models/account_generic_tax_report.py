...
class AccountGenericTaxReport(models.AbstractModel):
    ...
    def l10n_be_print_xml(self, options):
        ...
        view_id = self.env.ref(
            "l10n_be_reports.view_account_financial_report_export"
        ).id
        return {
    ...
    def get_xml(self, options):
        ...
        vat_no, country_from_vat = self._split_vat_number_and_country_code(
            self.get_vat_for_export(options)
        )

        sender_company = self._get_sender_company_for_export(options)
        default_address = sender_company.partner_id.address_get()
        address = (
            self.env["res.partner"].browse(default_address.get("default"))
            or sender_company.partner_id
        )
        if not address.email:
            raise UserError(
                _("No email address associated with company %s.", sender_company.name)
            )
        if not address.phone:
            raise UserError(
                _("No phone associated with company %s.", sender_company.name)
            )
        ...
        send_ref = (
            str(sender_company.partner_id.id) + str(dt_from[5:7]) + str(dt_to[:4])
        )
        ...
        date_to = (
            dt_to[0:7]
            + "-"
            + str(calendar.monthrange(int(dt_to[0:4]), int(ending_month))[1])
        )

        data = {
            "client_nihil": options.get("client_nihil"),
            "ask_restitution": options.get("ask_restitution", False),
            "ask_payment": options.get("ask_payment", False),
        }

        complete_vat = (
            country_from_vat or (address.country_id and address.country_id.code or "")
        ) + vat_no
        file_data = {
            "issued_by": issued_by,
            "vat_no": complete_vat,
...
"""
    ...
    """
        ...
        non_compound_rep_lines = self.env["account.tax.report.line"].search(
            [
                ("tag_name", "not in", ("48s44", "48s46L", "48s46T", "46L", "46T")),
                ("report_id.country_id.code", "=", "BE"),
            ]
        )
        lines_grids_map = {line.id: line.tag_name for line in non_compound_rep_lines}
        lines_grids_map[
            self.env.ref("l10n_be.tax_report_title_operations_sortie_46").id
        ] = "46"
        lines_grids_map[
            self.env.ref("l10n_be.tax_report_title_operations_sortie_48").id
        ] = "48"
        ...
        if (
            len([item for item in grids_list if item[0] == "71" or item[0] == "72"])
            == 0
        ):
        ...
        for code, amount, carryover_bounds, tax_line in grids_list:
            if carryover_bounds:
                amount, dummy = self.get_amounts_after_carryover(
                    tax_line, amount, carryover_bounds, options, 0
                )
            ...
            rslt += (
                '\n\t\t\t<ns2:Amount GridNumber="%(code)s">%(amount)s</ns2:Amount'
                ">" % (grid_amount_data)
            )
    ...
    def _get_popup_messages(
        ...
        country_id = (
            self.env["account.tax.report"].browse(options["tax_report"]).country_id
        )
        if country_id.code != "BE":
            return super()._get_popup_messages(
                line_balance, carryover_balance, options, tax_report_line
            )
        return {
            "positive": {
                "description1": _(
                    "This amount in the XML file will be increased by the positive amount from"
                ),
                "description2": _(
                    " past period(s), previously stored on the corresponding tax line."
                ),
                "description3": _(
                    "The amount in the xml will be : %s",
                    self.format_value(line_balance),
                ),
            },
            "negative": {
                "description1": _(
                    "This amount in the XML file will be reduced by the negative amount from"
                ),
                "description2": _(
                    " past period(s), previously stored on the corresponding tax line."
                ),
            ...
            "out_of_bounds": {
                "description1": _(
                    "This amount in the XML file will be set to %s.",
                    self.format_value(line_balance),
                ),
                "description2": _(
