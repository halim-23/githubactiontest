...
class ResPartner(models.Model):
    ...
    form_file = fields.Binary(
        readonly=True, help="Technical field to store all forms file."
    )
    ...
    def _generate_281_50_form(self, file_type, wizard_values):
        ...
        """
        tag_281_50_commissions = self.env.ref(
            "l10n_be_reports.account_tag_281_50_commissions"
        )
        ...
        tag_281_50_exposed_expenses = self.env.ref(
            "l10n_be_reports.account_tag_281_50_exposed_expenses"
        )
        tags = (
            self.env["account.account.tag"]
            + tag_281_50_commissions
            + tag_281_50_fees
            + tag_281_50_atn
            + tag_281_50_exposed_expenses
        )

        reference_year = wizard_values["reference_year"]

        commissions_per_partner = self._get_balance_per_partner(
            tag_281_50_commissions, reference_year
        )
        fees_per_partner = self._get_balance_per_partner(
            tag_281_50_fees, reference_year
        )
        atn_per_partner = self._get_balance_per_partner(tag_281_50_atn, reference_year)
        exposed_expenses_per_partner = self._get_balance_per_partner(
            tag_281_50_exposed_expenses, reference_year
        )
        paid_amount_per_partner = self._get_paid_amount_per_partner(
            reference_year, tags
        )
        ...
        partners = self._get_partners(
            ...
            exposed_expenses_per_partner,
            paid_amount_per_partner,
        )  # Get only partner with a balance > 0
        ...
        for partner in partners:
            ...
            partner_information = partner._get_partner_information(
                partner_remunerations, paid_amount
            )
            values_dict = partner._generate_codes_values(
                wizard_values, partner_information
            )
            ...
            if "xml" in file_type:
                attachments.append(
                    (file_name + ".xml", partner._generate_281_50_xml(values_dict))
                )
            if "pdf" in file_type:
                attachments.append(
                    (file_name + ".pdf", partner._generate_281_50_pdf(values_dict))
                )

        downloaded_filename = ""
        if (
            len(attachments) > 1
        ):  # If there are more than one file, we zip all these files.
            ...
            with tempfile.SpooledTemporaryFile() as tmp_file:  # We store the zip into a temporary file.
                ...
                ) as archive:  # We create the zip archive.
        ...
        return {
            ...
            "url": "/web/content/res.partner/%s/form_file/%s?download=true"
            % (partners[0].id, downloaded_filename),
        }
    ...
    def _generate_281_50_xml(self, values_dict):
        ...
        xml, dummy = self.env.ref(
            "l10n_be_reports.action_report_partner_281_50_xml"
        )._render_qweb_text(partner_id, values_dict)
        xml_element = etree.fromstring(xml)
        xml_file = etree.tostring(
            xml_element, xml_declaration=True, encoding="utf-8"
        )  # Well format the xml and add the xml_declaration
        return xml_file
    ...
    def _generate_281_50_pdf(self, values_dict):
        ...
        partner_id = self.parent_id and self.parent_id.id or self.id
        pdf_file, dummy = (
            self.env.ref("l10n_be_reports.action_report_partner_281_50_pdf")
            .sudo()
            ._render_qweb_pdf(partner_id, values_dict)
        )
        return pdf_file
    ...
    def _check_required_values(self):
        ...
        if not (
            ...
            and current_company.zip
            and current_company.city
            and current_company.phone
            and current_company.vat
        ):
            raise UserError(
                _(
                    "Your company is not correctly configured. Please be sure that the following pieces of information are set: street, zip, city, phone and vat"
                )
            )
        if not self.parent_id:
            if not (
                self.street and self.zip and (self.citizen_identification or self.vat)
            ):
                raise UserError(
                    _(
                        "The partner %s is not correctly configured. Plsea be sure that the following pieces of information are set: street, zip code and vat.",
                        self.name,
                    )
                )
            ...
            raise UserError(
                _(
                    "Partner %s is not correctly configured. Please be sure that the following pieces of information are set: street, zip code and vat.",
                    self.parent_id.name,
                )
            )
    ...
    def _generate_codes_values(self, wizard_values, partner_information):
        ...
        return {
            ...
            "F2008": 28150,
            "F2009": 0,
            "F2011": partner_information.get("nature") == "1"
            and partner_information.get("citizen_identification")
            or partner_information.get("vat"),
            ...
            "F50_2064": partner_information.get(
                "total_amount"
            ),  # Total from 2060 to 2063
            ...
            "F50_2110": partner_information.get("nature") == "2"
            and partner_information.get("vat")
            or "",
    ...
    def _get_partner_information(self, partner_remuneration, paid_amount):
        self.ensure_one()
        is_company_partner = (
            not self.is_company and self.commercial_partner_id.id != self.id
        )
        company_partner = self.commercial_partner_id
        return {
            "name": is_company_partner and company_partner.name or self.name,
            "address": is_company_partner
            and (company_partner.street + (company_partner.street2 or ""))
            or (self.street + (self.street2 or "")),
            ...
            "vat": (is_company_partner or self.is_company)
            and company_partner.vat[2:]
            or "",
            "remunerations": partner_remuneration,
            "paid_amount": paid_amount,
            "total_amount": sum(partner_remuneration.values()),
            "job_position": (is_company_partner or self.is_company)
            and ""
            or self.function,
            "citizen_identification": (is_company_partner or self.is_company)
            and ""
            or self.citizen_identification,
        }
    ...
    def _get_balance_per_partner(self, tag, reference_year):
        ...
        for bpp in balance_per_partner_list:
            bpp["balance"] = self.filtered(
                lambda p: p.id == bpp["partner_id"]
            ).currency_id.round(bpp["balance"])
            tmp.update({bpp["partner_id"]: bpp["balance"]})

        return tmp

    def _get_partners(
        self, commissions, fees, atn, exposed_expenses, paid_amount_per_partner
    ):
        """
    ...
    def _get_paid_amount_per_partner(self, reference_year, tags):
        ...
        for app in amount_per_partner:
            app["paid_amount"] = self.filtered(
                lambda p: p.id == app["partner_id"]
            ).currency_id.round(app["paid_amount"])
        ...
        for app in amount_for_previous_year:
            ...
            dict_amount_for_previous_year.update(
                {app["partner_id"]: app["paid_amount"]}
            )
