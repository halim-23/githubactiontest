...
class AccountTaxReport(models.Model):
    ...
    def get_checks_to_perform(self, amounts, carried_over):
        if self.id == self.env["ir.model.data"]._xmlid_to_res_id(
            "l10n_be.tax_report_vat"
        ):
            return [
                ...
                (
                    ...
                    not all(
                        v >= 0
                        for v in [v for k, v in amounts.items() if not carried_over[k]]
                    ),
                ),
                # Code C
                (
                    ...
                    min(0.0, amounts["c55"])
                    if amounts["c86"] > 0 or amounts["c88"] > 0
                    else False,
                ),
                # Code D
                (
                ...
                (
                    ...
                    min(
                        0.0,
                        (amounts["c84"] + amounts["c86"] + amounts["c88"]) * 0.21
                        - amounts["c55"],
                    )
                    if min(
                        0.0,
                ...
                (
                    ...
                    min(
                        0.0,
                        (amounts["c85"] + amounts["c87"]) * 0.21
                        - (amounts["c56"] + amounts["c57"]),
                    )
                    if min(
                        0.0,
                        (amounts["c85"] + amounts["c87"]) * 0.21
                ...
                (
                    "[85] * 21% >= [63]",
                    min(0.0, amounts["c85"] * 0.21 - amounts["c63"])
                    if min(0.0, amounts["c85"] * 0.21 - amounts["c63"]) < -62
                    else False,
                ),
                # Code U
                (
                    "[49] * 21% >= [64]",
                    min(0.0, amounts["c49"] * 0.21 - amounts["c64"])
                    if min(0.0, amounts["c49"] * 0.21 - amounts["c64"]) < -62
                    else False,
                ),
                # Code AC
                (
                ...
                (
                    ...
                    if amounts["c44"] > 99999
                    else False,
                ),
            ]
        return super(AccountTaxReport, self).get_checks_to_perform(
            amounts, carried_over
        )
