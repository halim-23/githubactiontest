...
class L10nBe28150Wizard(models.Model):
    ...
    def _onchange_reference_year(self):
        if len(self.reference_year) == 4:
            try:
                ...
                pass
            if reference_year_int and (
                reference_year_int >= fields.Date.today().year
                or reference_year_int <= 0
            ):
    ...
    def _check_reference_year(self):
        for record in self.filtered(
            lambda r: not r.is_test
        ):  # No constrains when it's a test.
            if len(record.reference_year) != 4:
                raise UserError(
                    _(
                        "Please make sure the reference year is written with four characters (e.g. 2019)"
                    )
                )
            ...
            try:
                ...
                raise UserError(
                    _("Please make sure that the reference year only contains numbers.")
                )
            if (
                not reference_year_int
                or reference_year_int < 0
                or reference_year_int >= fields.Date.today().year
            ):
                raise UserError(
                    _(
                        "Wrong reference year. Please make sure the given value corresponds to an actual year and is not superior or equal to the current one."
                    )
                )
