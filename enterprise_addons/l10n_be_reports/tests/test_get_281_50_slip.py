...
class TestResPartner(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_be.l10nbe_chart_template"):
        ...
        cls.partner_a.write(
            {
                ...
                "is_company": True,
                "category_id": [
                    (4, cls.env.ref("l10n_be_reports.res_partner_tag_281_50").id)
                ],
            }
        )
        ...
        cls.tag_281_50_commissions = cls.env.ref(
        ...
        cls.tag_281_50_exposed_expenses = cls.env.ref(
    ...
    def test_res_partner_get_paid_amount(self):
        ...
        payable_move_lines = move.mapped("line_ids").filtered(
            lambda x: x.account_internal_type == "payable"
        )
        payable_move_lines += payments.line_ids.filtered(
            lambda x: x.account_internal_type == "payable"
        )
        ...
        tags = (
            ...
            + self.tag_281_50_commissions
            + self.tag_281_50_fees
            + self.tag_281_50_atn
            + self.tag_281_50_exposed_expenses
        )
        paid_amount_per_partner = self.partner_a._get_paid_amount_per_partner(
            "2000", tags
        )
        paid_amount_for_partner_a = paid_amount_per_partner.get(self.partner_a.id, 0.0)
        self.assertEqual(
            paid_amount_for_partner_a, self.partner_a.currency_id.round(413.22)
        )
    ...
    def test_res_partner_get_partner_information(self):
        ...
        payable_move_lines += payment.line_ids.filtered(
            lambda x: x.account_internal_type == "payable"
        )
        payable_move_lines.reconcile()
        ...
        commissions_amounts = self.partner_a._get_balance_per_partner(
            self.tag_281_50_commissions, "2000"
        )
        commissions_balance_for_partner_a = commissions_amounts.get(
            self.partner_a.id, 0.0
        )

        partner_information = self.partner_a._get_partner_information(
            {"commissions": commissions_balance_for_partner_a},
            paid_amount_for_partner_a,
        )
