...
class product_template(models.Model):
        ...
        if self.type == "product" and self.recurring_invoice:
            ...
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "A 'Subscription Product' cannot be a 'Storable Product' !"
                    ),
                }
            }
        ...
        if self.type == "product" and self.recurring_invoice:
            ...
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "A 'Storable Product' cannot be a 'Subscription Product' !"
                    ),
                }
            }
    ...
    def _check_subscription_product(self):
        if any(
            product.recurring_invoice and product.type == "product" for product in self
        ):
