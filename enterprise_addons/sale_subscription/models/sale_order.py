...
class SaleOrder(models.Model):
    ...
    def action_open_subscriptions(self):
        ...
        if len(subscriptions) > 1:
            ...
            form_view = [
                (
                    self.env.ref("sale_subscription.sale_subscription_view_form").id,
                    "form",
                )
            ]
            if "views" in action:
    ...
    def action_draft(self):
        if any(
            order.state == "cancel"
            and any(line.subscription_id for line in order.order_line)
            for order in self
        ):
            raise UserError(
                _(
                    "You cannot set to draft a canceled quotation linked to subscriptions. Please create a new quotation."
                )
            )
    ...
    def _prepare_subscription_data(self, template):
        ...
        values = {
            ...
            "payment_token_id": self.transaction_ids._get_last().token_id.id
            if template.payment_mode == "success_payment"
            else False,
        ...
        for order in self:
            ...
            for subscription in subscriptions:
                subscription_lines = order.order_line.filtered(
                    lambda l: l.subscription_id == subscription
                    and l.product_id.recurring_invoice
                )
                line_values = subscription_lines._update_subscription_line_data(
                    subscription
                )
    ...
    def create_subscriptions(self):
        ...
        for order in self:
            ...
            for template in to_create:
                ...
                values["recurring_invoice_line_ids"] = to_create[
                    template
                ...
                subscription.message_post_with_view(
                    ...
                    values={"self": subscription, "origin": order},
    ...
    def _split_subscription_lines(self):
        ...
        self.ensure_one()
        res = dict()
        new_sub_lines = self.order_line.filtered(
            lambda l: not l.subscription_id
            and l.product_id.subscription_template_id
            and l.product_id.recurring_invoice
        )
        templates = new_sub_lines.mapped("product_id").mapped(
            "subscription_template_id"
        )
        for template in templates:
            lines = self.order_line.filtered(
                lambda l: l.product_id.subscription_template_id == template
                and l.product_id.recurring_invoice
            )
            res[template] = lines
        return res
...
class SaleOrderLine(models.Model):
    ...
    subscription_id = fields.Many2one(
        "sale.subscription", "Subscription", copy=False, check_company=True
    )
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if self.subscription_id:
            ...
            is_already_period_msg = (
                True if _("Invoicing period") in self.name else False
            )
            if (
                self.order_id.subscription_management != "upsell"
            ):  # renewal or creation: one entire period
                date_start = previous_date
                date_start_display = previous_date
                date_end = next_date - relativedelta(
                    days=1
                )  # the period does not include the next renewal date
            ...
            if not is_already_period_msg:
                ...
                format_date = (
                    self.env["ir.qweb.field.date"].with_context(lang=lang).value_to_html
                )
                # Ugly workaround to display the description in the correct language
                if lang:
                    self = self.with_context(lang=lang)
                period_msg = _("Invoicing period") + ": %s - %s" % (
                    format_date(fields.Date.to_string(date_start_display), {}),
    ...
    def create(self, vals):
        ...
        if vals.get("order_id"):
            ...
            if (
                order.origin
                and order.subscription_management in ("upsell", "renew")
                and Product.browse(vals["product_id"]).recurring_invoice
            ):
                vals["subscription_id"] = (
                    ...
                    .id
                    or self.env["sale.subscription"]
                    .search([("code", "=", order.origin)], limit=1)
                    .id
                )
