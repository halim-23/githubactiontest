...
class SaleSubscriptionWizard(models.TransientModel):
    ...
    option_lines = fields.One2many(
        "sale.subscription.wizard.option", "wizard_id", string="Options"
    )
    ...
    def create_sale_order(self):
        ...
        if self.subscription_id.partner_id.property_payment_term_id:
            new_order_vals[
                "payment_term_id"
            ] = self.subscription_id.partner_id.property_payment_term_id.id
        order = sale_order_obj.create(new_order_vals)
        order.message_post(
            body=(
                _("This upsell order has been created from the subscription ")
                + " <a href=# data-oe-model=sale.subscription data-oe-id=%d>%s</a>"
                % (self.subscription_id.id, self.subscription_id.display_name)
            )
        )
        for line in self.option_lines:
            self.subscription_id.partial_invoice_line(
                order, line, date_from=self.date_from
            )
...
class SaleSubscriptionWizardOption(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        "sale.subscription.wizard", required=True, ondelete="cascade"
    )
