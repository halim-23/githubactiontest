...
class SaleSubscriptionWizard(models.TransientModel):
    ...
    def _default_products(self):
        ...
        subscription_id = self.env["sale.subscription"].browse(
    ...
    kept_archived_product_ids = fields.One2many(
        "sale.subscription.renew.wizard.keep_option",
        "wizard_id",
    ...
    replacement_line_ids = fields.One2many(
        "sale.subscription.renew.wizard.replace_option",
        "wizard_id",
        string="Replaced by",
    )
    ...
    def _compute_dismissed(self):
        for wizard in self:
            if wizard.subscription_id.archived_product_count == sum(
                wizard.kept_archived_product_ids.mapped("renew_product")
            ):
    ...
    def create_renewal_order(self):
        ...
        not_renewed_products = self.kept_archived_product_ids.filtered(
            lambda rec: rec.renew_product is False
        )
        ...
        return self.subscription_id.prepare_renewal_order(
            discard_product_ids=discard_ids, new_lines_ids=self.replacement_line_ids
        )
...
class SaleSubscriptionRenewWizardKeepOption(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        "sale.subscription.renew.wizard", required=True, ondelete="cascade"
    )
    ...
    def _compute_product_attributes(self):
        for option in self:
            if option.product_id:
                ...
                if (
                    not option.uom_id
                    or option.product_id.uom_id.category_id.id
                    != option.uom_id.category_id.id
                ):
