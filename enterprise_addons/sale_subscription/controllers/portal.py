...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "subscription_count" in counters:
            ...
            values["subscription_count"] = (
                request.env["sale.subscription"].search_count(
                    self._get_subscription_domain(partner)
                )
                if request.env["sale.subscription"].check_access_rights(
    ...
    @http.route(
        ["/my/subscription", "/my/subscription/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def my_subscription(
        ...
        searchbar_sortings = {
            ...
            "stage_id": {
                "label": _("Status"),
                "order": "stage_id asc, to_renew desc, id desc",
            },
        }
        searchbar_filters = {
        ...
        accounts = SaleSubscription.sudo().search(
...
class SaleSubscription(http.Controller):
    ...
    def subscription(
        self, subscription_id, access_token="", message="", message_class="", **kw
    ):
        ...
        display_close = (
            active_plan_sudo.user_closable and subscription.stage_category == "progress"
        )
        is_follower = (
            request.env.user.partner_id in subscription.message_follower_ids.partner_id
        )
        periods = {
            "daily": "days",
            "weekly": "weeks",
        ...
        if subscription.recurring_rule_type != "weekly":
            rel_period = relativedelta(
                datetime.datetime.today(), subscription.recurring_next_date
            )
            missing_periods = (
                getattr(rel_period, periods[subscription.recurring_rule_type]) + 1
            )
        else:
    ...
    @http.route(
        ["/my/subscription/<int:account_id>/close"],
        type="http",
        methods=["POST"],
        auth="public",
        website=True,
    ...
    def close_account(self, account_id, uuid=None, **kw):
        ...
        if account.sudo().template_id.user_closable:
            close_reason = request.env["sale.subscription.close.reason"].browse(
                int(kw.get("close_reason_id"))
            )
...
class PaymentPortal(payment_portal.PaymentPortal):
    @http.route(
        "/my/subscription/transaction/<int:subscription_id>", type="json", auth="public"
    )
    def subscription_transaction(
        ...
        if not is_validation:  # Renewal transaction
            ...
            invoice_values = (
                subscription.sudo()
                .with_context(lang=subscription.partner_id.lang)
                ._prepare_invoice()
            )  # In sudo mode to read on account.fiscal.position fields
    ...
    @http.route(
        "/my/subscription/assign_token/<int:subscription_id>", type="json", auth="user"
    )
