...
class Project(models.Model):
    ...
    allow_task_dependencies = fields.Boolean(
        compute="_compute_allow_task_dependencies", store=True, readonly=False
    )
    ...
    def default_get(self, fields_list):
        ...
        if "allow_subtasks" in fields_list:
            defaults["allow_subtasks"] = defaults.get(
                "allow_subtasks", False
            ) and not defaults.get("is_fsm")
        if "allow_task_dependencies" in fields_list:
            defaults["allow_task_dependencies"] = defaults.get(
...
class Task(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "project_id" in fields_list and not result.get("project_id") and is_fsm_mode:
            ...
            fsm_project = self.env["project.project"].search(
                [("is_fsm", "=", True), ("company_id", "=", company_id)],
                order="sequence",
                limit=1,
            )
            if fsm_project:
                result["stage_id"] = self.stage_find(
                    fsm_project.id, [("fold", "=", False), ("is_closed", "=", False)]
                )
        ...
        if is_fsm_mode and (date_begin or date_end):
            ...
            if date_diff.seconds / 3600 > 23.5:
                ...
                user_ids_list = [
                    res[2]
                    for res in result.get("user_ids", [])
                    if len(res) == 3 and res[0] == Command.SET
                ]  # user_ids = [(Command.SET, 0, <user_ids>)]
                user_ids = user_ids_list[-1] if user_ids_list else []
                users = self.env["res.users"].sudo().browse(user_ids)
                user = len(users) == 1 and users
                if (
                    user and user.employee_id
                ):  # then the default start/end hours correspond to what is configured on the employee calendar
                    ...
                    company = (
                        self.env["res.company"].sudo().browse(result.get("company_id"))
                        if result.get("company_id")
                        else self.env.user.company_id
                    )
                    resource_calendar = company.resource_calendar_id
                if resource_calendar:
                    resources_work_intervals = resource_calendar._work_intervals_batch(
                        date_begin, date_end
                    )
                    work_intervals = [
                        (start, stop)
                        for start, stop, meta in resources_work_intervals[False]
                    ]
                    if work_intervals:
                        planned_date_begin = (
                            work_intervals[0][0]
                    ...
                    result["planned_date_begin"] = (
                        date_begin.replace(hour=9, minute=0, second=1)
                        .astimezone(pytz.utc)
                        .replace(tzinfo=None)
                    )
                    result["planned_date_end"] = date_end.astimezone(pytz.utc).replace(
                        tzinfo=None
                    )
        return result
    ...
    fsm_done = fields.Boolean(
        "Task Done", compute="_compute_fsm_done", readonly=False, store=True, copy=False
    )
    ...
    display_enabled_conditions_count = fields.Integer(
        compute="_compute_display_conditions_count"
    )
    display_satisfied_conditions_count = fields.Integer(
        compute="_compute_display_conditions_count"
    )
    display_mark_as_done_primary = fields.Boolean(
        compute="_compute_mark_as_done_buttons"
    )
    display_mark_as_done_secondary = fields.Boolean(
        compute="_compute_mark_as_done_buttons"
    )
    has_complete_partner_address = fields.Boolean(
        compute="_compute_has_complete_partner_address"
    )
    ...
    @api.depends(
        ...
        "is_fsm",
        "timer_start",
        "display_enabled_conditions_count",
        "display_satisfied_conditions_count",
    )
    def _compute_mark_as_done_buttons(self):
        for task in self:
            ...
            if task.fsm_done or not task.is_fsm or task.timer_start:
                ...
                if (
                    task.display_enabled_conditions_count
                    == task.display_satisfied_conditions_count
                ):
                    secondary = False
                else:
                    primary = False
    ...
    @api.depends(
        "fsm_done", "display_timesheet_timer", "timer_start", "total_hours_spent"
    )
    ...
    def _compute_has_complete_partner_address(self):
        for task in self:
            task.has_complete_partner_address = (
                task.partner_id.city and task.partner_id.country_id
            )
    ...
    def _read_group_user_ids(self, users, domain, order):
        if self.env.context.get("fsm_mode"):
            ...
            search_domain = [
                ...
                ("id", "in", recently_created_tasks.mapped("user_ids.id")),
            ]
    ...
    def action_fsm_validate(self):
        ...
        closed_stage_by_project = {
            project.id: project.type_ids.filtered(lambda stage: stage.is_closed)[:1]
    ...
    def _stop_all_timers_and_create_timesheets(self):
        ...
        tasks_running_timer_ids = Timer.search(
        ...
        minimum_duration = int(
            ConfigParameter.get_param("hr_timesheet.timesheet_min_duration", 0)
        )
        ...
        if tasks_running_timer_ids:
            ...
            for timer in tasks_running_timer_ids:
                ...
                time_spent = (
                    self._timer_rounding(minutes_spent, minimum_duration, rounding) / 60
                )
    ...
    def action_fsm_navigate(self):
        if (
            ...
            and not self.partner_id.partner_longitude
        ):
        ...
        url = "https://www.google.com/maps/dir/?api=1&destination=%s,%s" % (
            self.partner_id.partner_latitude,
            self.partner_id.partner_longitude,
        )
