...
AttendeeCalendarRenderer.include({
  events: Object.assign({}, AttendeeCalendarRenderer.prototype.events, {
    ...
    "click .o_appointment_search_create_work_hours_appointment":
      "_onSearchCreateWorkHoursAppointment",
  ...
  _initSidebar() {
    ...
    if (this.state.employeeId) {
      ...
      this.$sidebar.prepend(
        qweb.render("Calendar.calendar_link_buttons", {
          appointments: appointments,
          work_hours_appointment: appointments.find(
            (appointment) => appointment.category == "work_hours"
          ),
  ...
  _getFullCalendarOptions(fcOptions) {
    ...
    options.eventClick = (eventClickInfo) => {
      if (this.state.calendarMode === "default") {
        ...
        if (eventClickInfo.event.extendedProps.slot) {
          ...
          const hasSlots = this.calendar
            .getEvents()
            .filter((ev) => ev.extendedProps.slot).length;
          if (!hasSlots) {
            this.$sidebar
              .find(".o_appointment_create_custom_appointment")
              .addClass("disabled");
          }
        }
      }
    ...
    options.eventRender = (info) => {
      ...
      if (this.state.calendarMode === "slots-creation") {
        ...
        if ($element.hasClass("o_calendar_slot")) {
          ...
          const iconSize =
            duration < 1 || info.event.allDay || this.state.scale === "month"
              ? ""
              : "h1";
          const $button = $("<button/>", {
    ...
    options.yearDateClick = (info) => {
      if (this.state.calendarMode === "slots-creation") {
        ...
        if (!hasSlot && info.selectable) {
          ...
          const slotEvents = this.calendar
            .getEvents()
            .filter((ev) => ev.extendedProps.slot);
    ...
    options.select = (selectionInfo) => {
      if (this.state.calendarMode === "default") {
        ...
        const title =
          duration <= 1
            ? moment(selectionInfo.start).format("LL")
            : `${moment(selectionInfo.start).format("LL")} to ${moment(
                selectionInfo.end
              ).format("LL")}`;
        this.calendar.addEvent({
  ...
  _onChangeDisplay(ev) {
    this.$sidebar
      .find(".o_appointment_appointment_group_buttons")
      .toggleClass("o_hidden");
    this.$sidebar.find("#copy_text").toggleClass("o_hidden");
    this.$sidebar
      .find("#scheduling_box")
      .toggleClass("o_appointment_scheduling_message_box");
  ...
  _switchMode() {
    this.state.calendarMode =
      this.state.calendarMode === "default" ? "slots-creation" : "default";
    this.$sidebar
      .find(".o_appointment_create_custom_appointment")
      .toggleClass("o_hidden");
    this.$sidebar
