...
CalendarController.include({
  ...
  _onClickAppointmentLink(ev) {
    ...
    const appointmentURL = window.encodeURI(
      `${this.getSession()["web.base.url"]}${$currentTarget.attr("data-url")}`
    );
    ...
    const slots = slotEvents.map((slot) => {
      return {
        start: legacy.field_utils.parse.datetime(slot.start).toJSON(),
          ...
          ? legacy.field_utils.parse.datetime(slot.end)
          : legacy.field_utils.parse.datetime(moment(slot.start).add(1, "days"))
        ).toJSON(),
        allday: slot.allDay,
      };
    });
