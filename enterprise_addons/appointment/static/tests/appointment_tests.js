...
QUnit.module(
  ...
  function () {
    QUnit.test("verify appointment links button are displayed", async function (
      assert
    ) {
      assert.expect(3);
    ...
    QUnit.test("discard slot in calendar", async function (assert) {
      ...
      const calendar = await createCalendarView(
        {
          ...
          mockRPC: async function (route, args) {
            if (route === "/microsoft_calendar/sync_data") {
              ...
              route === "/web/dataset/call_kw/res.partner/get_attendee_detail"
            ) {
              return Promise.resolve([]);
            }
            return this._super.apply(this, arguments);
      ...
      await testUtils.dom.click(
        calendar.$(".o_calendar_filter_item[data-value=all] input")
      );
      await testUtils.dom.click(calendar.$('button:contains("Share Availabilities")'));
      await testUtils.nextTick();

      assert.strictEqual(
        calendar.model.calendarMode,
        "slots-creation",
    ...
    QUnit.test("cannot move real event in slots-creation mode", async function (
      assert
    ) {
      assert.expect(4);
    ...
    QUnit.test("create slots for custom appointment type", async function (assert) {
      ...
      await testUtils.dom.click(
        calendar.$("button.o_appointment_create_custom_appointment")
      );
      ...
      assert.containsNone(
        calendar,
        ".o_calendar_slot",
        "The slot has been cleared after the creation"
      );
    ...
    QUnit.test("filter works in slots-creation mode", async function (assert) {
      ...
      assert.containsN(
        ...
        ".fc-event",
        2,
        "The slot is successfully created in the calendar"
      );
      ...
      assert.containsOnce(
        calendar,
        ".fc-event",
        "There is now only 1 events displayed"
      );
      assert.containsOnce(
        calendar,
        ".o_calendar_slot",
        "The slot created is still displayed"
      );
      ...
      assert.containsOnce(
        calendar,
        ".fc-event",
        "There is now only 1 calendar event displayed"
      );
      assert.containsNone(
        calendar,
        ".o_calendar_slot",
        "There is no more slots in the calendar view"
      );
    ...
    QUnit.test("click & copy appointment type url", async function (assert) {
      ...
      await testUtils.dom.click(calendar.$("#dropdownAppointmentLink"));
      await testUtils.dom.click(
        calendar.$(".o_appointment_appointment_link_clipboard:first")
      );
    ...
    QUnit.test("create/search work hours appointment type", async function (assert) {
      ...
      await testUtils.dom.click(
        calendar.$(".o_appointment_search_create_work_hours_appointment")
      );
      await testUtils.nextTick();
      ...
      assert.strictEqual(
        3,
        this.data["calendar.appointment.type"].records.length,
