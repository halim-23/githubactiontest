...
class CalendarAppointmentShare(models.TransientModel):
    ...
    appointment_type_ids = fields.Many2many(
        ...
        domain=_domain_appointment_type_ids,
        string="Appointments",
    )
    appointment_type_count = fields.Integer(
        "Selected Appointments Count", compute="_compute_appointment_type_count"
    )
    suggested_employee_ids = fields.Many2many(
        "hr.employee",
        related="appointment_type_ids.employee_ids",
        string="Possible employees",
    ...
    def _compute_appointment_type_count(self):
        for appointment_link in self:
            appointment_link.appointment_type_count = len(
                appointment_link.appointment_type_ids
            )
    ...
    def _compute_employee_ids(self):
        for appointment_link in self:
            ...
            if len(employees) == 1:
                ...
                appointment_link.employee_ids = (
                    self.env.user.employee_id
                    if self.env.user.employee_id in employees
                    else False
                )
    ...
    def _compute_share_link(self):
        ...
        for appointment_link in self:
            ...
            if len(appointment_link.appointment_type_ids) == 1:
                ...
                appt_link = url_join(
                    "%s/" % calendar_url,
                    slug(appointment_link.appointment_type_ids._origin),
                )
                ...
                url_param.update(
                    {
                        "filter_appointment_type_ids": str(
                            appointment_link.appointment_type_ids.ids
                        )
                    }
                )
