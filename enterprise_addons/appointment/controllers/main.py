...
class Appointment(http.Controller):
    ...
    def calendar_appointment_type(
        ...
        **kwargs
        ...
        try:
            filter_employee_ids = (
                json.loads(filter_employee_ids) if filter_employee_ids else []
            )
        ...
        if (
            not suggested_employees
            and employee_id
            and int(employee_id) in appointment_type.employee_ids.ids
        ):
            suggested_employees = (
                request.env["hr.employee"].sudo().browse(int(employee_id))
            )

        default_employee = (
            suggested_employees[0]
            if suggested_employees
            else request.env["hr.employee"]
        )
        ...
        formated_days = [
            format_date(
                fields.Date.from_string("2021-03-0%s" % str(day + 1)),
                "EEE",
                get_lang(request.env).code,
            )
            for day in range(7)
        ]
    ...
    def calendar_appointment(
        ...
        **kwargs
        ...
        return request.redirect(
            "/calendar/%s?%s" % (slug(appointment_type), keep_query("*"))
        )

    @http.route(
        ['/calendar/<model("calendar.appointment.type"):appointment_type>/info'],
        type="http",
        auth="public",
        website=True,
        sitemap=True,
    )
    def calendar_appointment_form(
        self, appointment_type, employee_id, date_time, duration, **kwargs
    ):
        """
        ...
        partner_data = (
            partner.read(fields=["name", "mobile", "email"])[0] if partner else {}
        )
        day_name = format_datetime(
            datetime.strptime(date_time, dtf), "EEE", locale=get_lang(request.env).code
        )
        date_formated = format_datetime(
            datetime.strptime(date_time, dtf), locale=get_lang(request.env).code
        )
        return request.render(
    ...
    @http.route(
        ['/calendar/<model("calendar.appointment.type"):appointment_type>/submit'],
        type="http",
        auth="public",
        website=True,
        methods=["POST"],
    )
    def calendar_appointment_submit(
        self,
        appointment_type,
        datetime_str,
        duration_str,
        ...
        **kwargs
        ...
        date_start = tz_session.localize(
            fields.Datetime.from_string(datetime_str)
        ).astimezone(pytz.utc)
        ...
        if employee.user_id and employee.user_id.partner_id:
            if not employee.user_id.partner_id.calendar_verify_availability(
                date_start, date_end
            ):
                return request.redirect(
                    "/calendar/%s/appointment?state=failed-employee"
        ...
        if Partner:
            if not Partner.calendar_verify_availability(date_start, date_end):
                return request.redirect(
                    "/calendar/%s/appointment?state=failed-partner"
                    % appointment_type.id
                )
        ...
        for question in appointment_type.question_ids:
            ...
            if question.question_type == "checkbox":
                answers = question.answer_ids.filtered(
                    lambda x: (key + "_answer_" + str(x.id)) in kwargs
                )
                if answers:
                    description_bits.append(
                        "%s: %s" % (question.name, ", ".join(answers.mapped("name")))
                    )
            elif question.question_type == "text" and kwargs.get(key):
                answers = [line for line in kwargs[key].split("\n") if line.strip()]
                description_bits.append(
                    "%s:<br/>%s"
                    % (question.name, plaintext2html(kwargs.get(key).strip()))
                )
            elif kwargs.get(key):
                description_bits.append(
                    "%s: %s" % (question.name, kwargs.get(key).strip())
                )
        if description_bits:
            description = (
                "<ul>"
                + "".join(["<li>%s</li>" % bit for bit in description_bits])
                + "</ul>"
            )
    ...
    def _prepare_calendar_values(
        ...
        alarm_ids = (
            appointment_type.reminder_ids
            and [(6, 0, appointment_type.reminder_ids.ids)]
            or []
        )
    ...
    def calendar_appointment_view(
        self, access_token, partner_id, state=False, **kwargs
    ):
        """
        ...
        date_start = (
            day_name + " " + format_func(date_start, locale=locale) + date_start_suffix
        )
        # convert_online_event_desc_to_text method for correct data formatting in external calendars
        details = (
            event.appointment_type_id
            and event.appointment_type_id.message_confirmation
            or event.convert_online_event_desc_to_text(event.description)
            or ""
        )
        params = {
            "action": "TEMPLATE",
    ...
    def calendar_appointment_cancel(self, access_token, partner_id, **kwargs):
        ...
        if fields.Datetime.from_string(
            ...
            return request.redirect(
                "/calendar/view/"
                + access_token
                + "?state=no-cancel&partner_id=%s" % partner_id
            )
        event.sudo().action_cancel_meeting([int(partner_id)])
        return request.redirect(
            "/calendar/%s/appointment?state=cancel" % slug(appointment_type)
        )
    ...
    @http.route(
        ["/calendar/<int:appointment_type_id>/get_message_intro"],
        type="json",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def get_appointment_message_intro(self, appointment_type_id, **kwargs):
        appointment_type = (
            ...
            .browse(int(appointment_type_id))
            .exists()
        )
    ...
    @http.route(
        ["/calendar/<int:appointment_type_id>/update_available_slots"],
        type="json",
        auth="public",
        website=True,
    )
    def calendar_appointment_update_available_slots(
        self, appointment_type_id, employee_id=None, timezone=None, **kwargs
    ):
        """
        Route called when the employee or the timezone is modified to adapt the possible slots accordingly
        """
        appointment_type = request.env["calendar.appointment.type"].browse(
        ...
        slots = appointment_type.sudo()._get_appointment_slots(
            request.session["timezone"], employee
        )
