...
class AppointmentController(CalendarController):
    ...
    def view_meeting(self, token, id):
        ...
        if request.env.user.has_group("base.group_user"):
            ...
            return request.redirect(
                "/web?db=%s#%s" % (request.env.cr.dbname, url_params)
            )
        ...
        return request.redirect(
            "/calendar/view/%s?partner_id=%s"
            % (attendee.event_id.access_token, attendee.partner_id.id)
        )

    @route(
        "/appointment/calendar_appointment_type/create_custom", type="json", auth="user"
    )
    ...
    @route(
        "/appointment/calendar_appointment_type/get_employee_appointment_types",
        type="json",
        auth="user",
    )
    def appointment_get_employee_appointment_types(self):
        ...
        if employee_id:
            ...
            appointment_types_info = request.env[
                "calendar.appointment.type"
            ].search_read(domain, ["name", "category"])
        return {
    ...
    def _get_employee_appointment_info(self, appointment_type):
        ...
        return {"id": appointment_type.id, "url": appointment_employee_url}
