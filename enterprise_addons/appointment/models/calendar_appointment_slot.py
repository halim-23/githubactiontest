...
class CalendarAppointmentSlot(models.Model):
    ...
    appointment_type_id = fields.Many2one(
        "calendar.appointment.type", "Appointment Type", ondelete="cascade"
    )
    ...
    def _check_hour(self):
        if any(slot.start_hour < 0.00 or slot.start_hour >= 24.00 for slot in self):
            raise ValidationError(
                _("Please enter a valid hour between 0:00 and 24:00 for your slots.")
            )
    ...
    def _check_delta_hours(self):
        if any(
            self.filtered(
                lambda slot: slot.start_hour >= slot.end_hour
    ...
    def name_get(self):
        ...
        for slot in self:
            if slot.slot_type == "recurring":
                ...
                result.append(
                    (slot.id, "%s - %s" % (slot.start_datetime, slot.end_datetime))
                )
        return result
