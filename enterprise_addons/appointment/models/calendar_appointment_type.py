...
class CalendarAppointmentType(models.Model):
    ...
    def default_get(self, default_fields):
        ...
        if "category" not in default_fields or result.get("category") in [
            "custom",
            "work_hours",
        ]:
    ...
    category = fields.Selection(
        [("website", "Website"), ("custom", "Custom"), ("work_hours", "Work Hours")],
        string="Category",
        default="website",
    ...
    appointment_duration = fields.Float(
        "Appointment Duration", required=True, default=1.0
    )
    ...
    slot_ids = fields.One2many(
        "calendar.appointment.slot", "appointment_type_id", "Availabilities", copy=True
    )
    ...
    assign_method = fields.Selection(
        [("random", "Random"), ("chosen", "Chosen by the Customer")],
    ...
    def _compute_appointment_count(self):
        ...
        mapped_data = {
            m["appointment_type_id"][0]: m["appointment_type_id_count"]
    ...
    def _check_employee_configuration(self):
        for appointment_type in self:
            if (
                appointment_type.category != "website"
                and len(appointment_type.employee_ids) != 1
            ):
                raise ValidationError(
                    _(
                        "This category of appointment type should only have one employee but got %s employees",
                        len(appointment_type.employee_ids),
                    )
                )
            if appointment_type.category == "work_hours":
                appointment_domain = [
                    ("category", "=", "work_hours"),
                    ("employee_ids", "in", appointment_type.employee_ids.ids),
                ]
                if appointment_type.ids:
                    appointment_domain = AND(
                        [appointment_domain, [("id", "not in", appointment_type.ids)]]
                    )
    ...
    def create(self, values):
        ...
        return super(
            CalendarAppointmentType, self.with_context(mail_create_nosubscribe=True)
        ).create(values)
    ...
    def action_share(self):
        ...
        return {
            ...
            "context": {
                ...
                "default_employee_ids": self.employee_ids.filtered(
                    lambda employee: employee.user_id.id == self.env.user.id
                ).ids,
            },
        }
    ...
    def _slots_generate(self, first_day, last_day, timezone):
        ...
        def append_slot(day, slot):
            ...
            ) <= slot.end_hour - self.appointment_duration:
                slots.append(
                    {
                        self.appointment_tz: (
                            local_start,
        ...
        if self.category != "custom":
            ...
            for slot in self.slot_ids.filtered(
                lambda x: int(x.weekday) == first_day.isoweekday()
            ):
            ...
            unique_slots = self.slot_ids.filtered(
                ...
                and slot.end_datetime > datetime.utcnow()
            )
            ...
            for slot in unique_slots:
                ...
                if employee.sudo().user_partner_id.calendar_verify_availability(
                    startUTC, endUTC
                ):
                    slots.append(
                        {
    ...
    def _slots_available(self, slots, first_day, last_day, employee=None):
            ...
            if start_index != -1:
                for index in range(start_index, len(intervals)):
                    if intervals[index][1] >= end_dt - tolerance:
                        return True
                    if (
                        len(intervals) == index + 1
                        or intervals[index + 1][0] - intervals[index][1] > tolerance
                    ):
                        return False
            return False
        ...
        available_employees = [
            emp.with_context(tz=emp.user_id.tz)
        ...
        for slot in slots:
            for emp_pos, emp in enumerate(available_employees):
                ...
                if is_work_available(
                    slot["UTC"][0], slot["UTC"][1], workhours[emp_pos]
                ):
                    ...
                    if emp.user_partner_id.calendar_verify_availability(
    ...
    def _get_appointment_slots(self, timezone, employee=None):
        ...
        if self.category == "custom" and unique_slots:
            appointment_duration_days = (
                unique_slots[-1].end_datetime - datetime.utcnow()
            ).days
        last_day = requested_tz.fromutc(
            datetime.utcnow() + relativedelta(days=appointment_duration_days)
        )

        # Compute available slots (ordered)
        slots = self._slots_generate(
            first_day.astimezone(appt_tz), last_day.astimezone(appt_tz), timezone
        )
                                    ...
                                    today_slots.append(
                                        {
                                            ...
                                            "datetime": slots[0][timezone][0].strftime(
                                                "%Y-%m-%d %H:%M:%S"
                                            ),
                                            "hours": _("All day"),
                                            "duration": 24,
