...
class AppointmentTest(common.HttpCase):
    def setUp(self):
        ...
        self.resource_calendar = self.env["resource.calendar"].create(
            {"name": "Small Day", "company_id": self.company.id}
        )

        self.resource_calendar.write(
            {"attendance_ids": [(5, False, False)]}
        )  # Wipe out all attendances
        ...
        self.first_user_in_brussel = self.env["res.users"].create(
            {"name": "Grace Slick", "login": "grace"}
        )
        self.first_user_in_brussel.write({"tz": "Europe/Brussels"})

        self.second_user_in_australia = self.env["res.users"].create(
            {"name": "Australian guy", "login": "australian"}
        )
        ...
        self.appointment_in_brussel = self.env["calendar.appointment.type"].create(
            {
                ...
                "slot_ids": [
                    (0, False, {"weekday": "1", "start_hour": 9, "end_hour": 10})
                ],  # Yes, monday has either 0 or 1 as weekday number depending on the object it's in
            }
        )

    def test_extreme_timezone_delta(self):
        context_australia = {
            "uid": self.second_user_in_australia.id,
        ...
        for month in months:
            for week in month["weeks"]:
                for day in week:
                    ...
                    if (
                        day["day"] > utc_now.date()
                        and day["day"]
                        < (
                            utc_now + relativedelta(days=appointment.max_schedule_days)
                        ).date()
                        and day["day"].weekday() == 0
                        and day["day"] not in already_checked
                    ):
                        ...
                        self.assertEqual(
                            len(day["slots"]),
                            1,
                            "Each monday should have only one slot",
                        )
                        slot = day["slots"][0]
                        self.assertEqual(
                            slot["employee_id"],
                            self.employee_in_brussel.id,
                            "The right employee should be available on each slot",
                        )
                        self.assertEqual(
                            slot["hours"], "09:00", "Slots hours has to be 09:00"
                        )  # We asked to display the slots as Europe/Brussels
        ...
        self.assertGreaterEqual(
            mondays_count, 1, "There should be at least one monday in the time range"
        )
            ...
            for week in month["weeks"]:
                for day in week:
                    if (
                        ...
                        < now
                        + relativedelta(
                            days=self.appointment_in_brussel.max_schedule_days
                        )
                        and day["day"].month == week[-1]["day"].month
                        and day["day"].isoweekday() == 1
                    ):

                        self.assertEqual(
                            len(day["slots"]), 1, "There should be 1 slot each monday"
                        )
                    elif day["day"] < now:
                        self.assertEqual(
                            len(day["slots"]), 0, "There should be no slot in the past"
                        )
    ...
    def test_generate_unique_slots(self):
        ...
        self.assertEqual(
            custom_appointment_type.category,
            "custom",
            "It should be a custom appointment type",
        )
        self.assertEqual(
            len(custom_appointment_type.slot_ids),
            2,
        ...
        for week in slots[0]["weeks"]:
            for day in week:
                ...
                if (now + timedelta(hours=1)).date() == day["day"]:
                    ...
                    if (now + timedelta(hours=1)).date().month > week[0][
                        "day"
                    ].month and len(slots) > 1:
                        # Take the slots of the day in the next month (happens when we are in the end of the month)
                        slot = next(
                            d["slots"]
                            for d in slots[1]["weeks"][0]
                            if d["day"] == day["day"]
                        )

                    self.assertEqual(
                        len(slot), 1, "There should be 1 slot for this date"
                    )
                elif (now + timedelta(days=2)).date() == day["day"]:
                    # Check if the month we are is greater than the one we are in if there are mutltiple months
                    if (now + timedelta(days=2)).date().month != week[0][
                        "day"
                    ].month and len(slots) > 1:
                        # Take the slots of the day in the next month (happens when we are in the end of the month)
                        slot = next(
                    ...
                    self.assertEqual(
                        len(slot), 1, "There should be 1 all day slot for this date"
                    )
                    self.assertEqual(slot[0]["hours"], "All day")
                else:
                    self.assertEqual(
                        len(slot), 0, "There should be no slot for this date"
                    )
    ...
    def test_create_custom_appointment(self):
        ...
        self.assertTrue(
            all(slot.slot_type == "unique" for slot in appointment_type.slot_ids),
    ...
    def test_search_create_work_hours(self):
        ...
        self.assertEqual(
            len(appointment_type.slot_ids), 14, "Two slots have been created"
        )
        self.assertTrue(
