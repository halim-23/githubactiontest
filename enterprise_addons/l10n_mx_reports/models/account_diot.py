...
class MxReportPartnerLedger(models.AbstractModel):
    ...
    def _do_query_group_by_account(self, options, line_id):
        ...
        line_clause = (
            line_id and ' AND "account_move_line".partner_id = ' + str(line_id) or ""
        )
    ...
    def _group_by_partner_id(self, options, line_id):
        ...
        for partner_id, result in results.items():
            ...
            if not self._context.get("print_mode"):
                ...
                partners[partner]["lines"] = self.env["account.move.line"].search(
            ...
            if (
                partner.country_id.code == "MX"
                and not partner.vat
                and partners[partner]["lines"]
            ):
    ...
    def get_taxes_with_financial_tag(self, tag_xmlid, allowed_tax_ids):
        rep_lines = self.env["account.tax.repartition.line"].search(
            [
                "|",
                ("invoice_tax_id", "in", allowed_tax_ids),
                ("refund_tax_id", "in", allowed_tax_ids),
                (
                    "tag_ids",
                    "in",
                    self.env["ir.model.data"]._xmlid_to_res_id(tag_xmlid),
                ),
            ]
        )
    ...
    def _get_lines(self, options, line_id=None):
        ...
        unfold_all = self._context.get("print_mode") and not options.get(
            "unfolded_lines"
        )
        tag_16 = self.env.ref("l10n_mx.tag_diot_16")
        tag_non_cre = (
            self.env.ref("l10n_mx.tag_diot_16_non_cre", raise_if_not_found=False)
            or self.env["account.account.tag"]
        )
        tag_8 = (
            self.env.ref("l10n_mx.tag_diot_8", raise_if_not_found=False)
            or self.env["account.account.tag"]
        )
        tag_8_non_cre = (
            self.env.ref("l10n_mx.tag_diot_8_non_cre", raise_if_not_found=False)
            or self.env["account.account.tag"]
        )
        ...
        rep_line_obj = self.env["account.tax.repartition.line"].with_context(
            active_test=False
        )
        ...
        tax16 = rep_line_obj.search(
            [("tag_ids", "in", tag_16.ids), ("company_id", "=", company)]
            + diot_common_domain
        ).mapped("tax_id")
        taxnoncre = self.env["account.tax"]
        if tag_non_cre:
            taxnoncre = rep_line_obj.search(
                [("tag_ids", "in", tag_non_cre.ids), ("company_id", "=", company)]
                + diot_common_domain
            ).mapped("tax_id")
        tax8 = self.env["account.tax"]
        if tag_8:
            tax8 = rep_line_obj.search(
                [("tag_ids", "in", tag_8.ids), ("company_id", "=", company)]
                + diot_common_domain
            ).mapped("tax_id")
        tax8_noncre = self.env["account.tax"]
        if tag_8_non_cre:
            tax8_noncre = rep_line_obj.search(
                [("tag_ids", "in", tag_8_non_cre.ids), ("company_id", "=", company)]
                + diot_common_domain
            ).mapped("tax_id")
        taximp = rep_line_obj.search(
            [("tag_ids", "in", tag_imp.ids), ("company_id", "=", company)]
            + diot_common_domain
        ).mapped("tax_id")
        tax0 = rep_line_obj.search(
            [("tag_ids", "in", tag_0.ids), ("company_id", "=", company)]
            + diot_common_domain
        ).mapped("tax_id")
        tax_ret = rep_line_obj.search(
            [("tag_ids", "in", tag_ret.ids), ("company_id", "=", company)]
            + diot_common_domain
        ).mapped("tax_id")
        tax_exe = rep_line_obj.search(
            [("tag_ids", "in", tag_exe.ids), ("company_id", "=", company)]
            + diot_common_domain
        ).mapped("tax_id")
        grouped_taxes = (
            self.env["account.tax"]
        ...
        if grouped_taxes:
            ...
            taxes_in_groups = taxes_in_groups.browse(
        ...
        for partner in sorted_partners:
            ...
            withh += sum(
                [
                    abs(partner_data.get(ret.id, 0) / (100 / ret.amount))
                    for ret in tax_ret
                ]
            )
            p_columns.append(withh)
            unfolded = (
                "partner_" + str(partner.id) in options.get("unfolded_lines")
                or unfold_all
            )
            lines.append(
                {
                    ...
                    "columns": [
                        {"name": v if index < 5 else int(round(v, 0))}
                        for index, v in enumerate(p_columns)
                    ],
                    "level": 2,
                    "unfoldable": True,
                    "unfolded": unfolded,
            ...
            domain_lines.append(
                {
                    ...
                    "columns": [
                        {"name": v if index < 5 else self.format_value(v)}
                        for index, v in enumerate(p_columns)
                    ],
                    "level": 1,
                }
                ...
                domain_lines.append(
                    {
                        ...
                        "parent_id": "partner_" + str(partner.id),
                        "name": _(
                            "There are more than 80 items in this list, "
    ...
    __diot_supplier_re = re.compile("""[^A-Za-z0-9 Ññ&]""")
    __diot_nationality_re = re.compile("""[^rA-Za-z0-9 Ññ]""")
        ...
        buttons += [
            {
                "name": _("DPIVA (txt)"),
                "sequence": 4,
                "action": "print_dpiva_txt",
                "file_export_type": _("DPIVA"),
            }
        ]
        return buttons
    ...
    def _custom_setlocale(self):
        ...
        try:
            ...
            _logger.info(
                'Error when try to set locale "es_MX". Please '
        ...
        for line in txt_data:
            ...
            data[8] = len(
                [
                    x
                    for x in txt_data
                    if x.get("parent_id") == line.get("id")
                    and "total" not in x.get("id", "")
                ]
            ...
            data[30] = (
                "".join(line.get("name", "")).encode("utf-8").strip().decode("utf-8")
                if columns[0]["name"] != "04"
                else ""
            )
            data[31] = columns[3]["name"] if columns[0]["name"] != "04" else ""
            data[32] = (
                "".join(columns[4]["name"]).encode("utf-8").strip().decode("utf-8")
                if columns[0]["name"] != "04"
                else ""
            )
