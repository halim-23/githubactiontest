...
CFDIBCE_XSLT_CADENA = "l10n_mx_reports/data/xslt/%s" "/BalanzaComprobacion_1_2.xslt"
...
class MxReportAccountTrial(models.AbstractModel):
    ...
    def _post_process(
        self, grouped_accounts, initial_balances, options, comparison_table
    ):
        ...
        afr_lines = afrl_obj.search(
            [("parent_id", "=", False), ("code", "ilike", "MX_COA_%")], order="code"
        )
        for line in afr_lines:
            ...
            if not options.get("coa_only"):
                ...
                for child in childs:
                    child["columns"] = [
                        {"name": self.format_value(v)} for v in child["columns"]
                    ]
            lines.append(
                {
                    "id": self._get_generic_line_id(
                        None, line.code, markup="hierarchy"
                    ),
                    "name": line.name,
        ...
        if not options.get("coa_only"):
            lines.append(
                {
                    "id": self._get_generic_line_id(
                        None, None, markup="hierarchy_total"
                    ),
                    "name": _("Total"),
                    "level": 0,
                    "class": "hierarchy_total",
    ...
    def _get_lines_second_level(
        self, lines_child, grouped_accounts, initial_balances, options, comparison_table
    ):
        ...
        for child in sorted_childs:
            account_lines = self._get_lines_third_level(
                child, grouped_accounts, initial_balances, options, comparison_table
            )
            ...
            if not options.get("coa_only"):
                n_cols = len(comparison_table) * 2 + 2
                child_cols = [
                    c["columns"] for c in account_lines if c.get("level") == 3
                ]
                cols = []
                for col in range(n_cols):
    ...
    @api.model
    def _get_lines_third_level(
        self, line, grouped_accounts, initial_balances, options, comparison_table
    ):
        ...
        domain.append(
                (
                    "id",
                    "not in",
                    self.env.companies.account_cash_basis_base_account_id.ids,
                )
        )
        account_ids = self.env["account.account"].search(domain, order="code")
        tags = (
            account_ids.mapped("tag_ids")
            .filtered(lambda r: r.color == 4)
            .sorted(key=lambda a: a.name)
        )
        for tag in tags:
            ...
            cols = [{"name": ""}]
            childs = self._get_lines_fourth_level(
                accounts, grouped_accounts, initial_balances, options, comparison_table
            )
            if not childs:
                continue
            if not options.get("coa_only"):
            ...
            lines.append(
                {
                    "id": self._get_generic_line_id(
                        "account.account.tag", tag.id, markup="level_two"
                    ),
                    "name": name,
                    "columns": cols,
                    "level": 3,
    ...
    def _get_lines_fourth_level(
        self, accounts, grouped_accounts, initial_balances, options, comparison_table
    ):
        lines = []
        ...
        for account in accounts:
            ...
            if not options.get("coa_only"):
                ...
                for period in range(len(comparison_table)):
                    if account in grouped_accounts and (
                        not is_zero(initial_balances.get(account, 0))
                        or not is_zero(grouped_accounts[account][period]["debit"])
                        or not is_zero(grouped_accounts[account][period]["credit"])
                    ):
                        non_zero = True
                        break
                if not non_zero:
            ...
            if not options.get("coa_only"):
                cols = self._get_cols(
                    initial_balances, account, comparison_table, grouped_accounts
                )
            lines.append(
                {
                    "id": self._get_generic_line_id(
                        "account.account", account.id, markup="level_four"
                    ),
                    "name": name,
                    "level": 4,
                    "columns": cols,
    ...
    def _get_cols(self, initial_balances, account, comparison_table, grouped_accounts):
        ...
        for period in range(len(comparison_table)):
            ...
            cols += [
                grouped_accounts[account][period]["debit"],
    ...
    def get_bce_dict(self, options):
        ...
        accounts = []
        account_lines = [
            l for l in xml_data if l.get("level") in [2, 3] and l.get("show", True)
        ]
    ...
    def _get_lines(self, options, line_id=None):
        ...
        comparison_table += (
            options.get("comparison") and options["comparison"].get("periods") or []
        )
        for account, periods_results in accounts_results:
            ...
            for i, res in enumerate(periods_results):
                ...
                if i == 0:
                    initial_balances[account] = res.get("initial_balance", {}).get(
                        "balance", 0.0
                    ) + res.get("unaffected_earnings", {}).get("balance", 0.0)
                sums = [
                    res.get("sum", {}).get("debit", 0.0)
                    - account_init_bal.get("debit", 0.0),
                    res.get("sum", {}).get("credit", 0.0)
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        return super(
            MxReportAccountTrial, self.with_context(self._set_context(options))
        ).get_html(options, line_id, additional_context)

    def get_report_filename(self, options):
        return (
            super(MxReportAccountTrial, self.with_context(self._set_context(options)))
            .get_report_filename(options)
            .upper()
        )
    ...
    def _get_report_name(self):
        ...
        date_report = (
            fields.Date.from_string(context["date_from"])
    ...
    def open_journal_items(self, options, params):
        ...
        return super(MxReportAccountTrial, self).open_journal_items(options, new_params)
