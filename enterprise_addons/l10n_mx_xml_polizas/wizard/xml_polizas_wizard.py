...
class MoveExportData(defaultdict):
    ...
    def append(self, move_date, journal_name, move_name, move_data):
        ...
        move_key = self.Key(
            date.strftime(move_date, "%Y-%m-%d"), journal_name, move_name
        )
...
class XmlPolizasExportWizard(models.TransientModel):
    ...
    def _compute_filter_partial_journals(self):
        ...
        if journals_options:
            selected_journals = [
                x.get("selected", False) for x in journals_options if "code" in x
            ]
        ...
        for record in self:
            if not record.order_number:
                if self.export_type in ("AF", "FC"):
                    selection = dict(
                        self._fields["export_type"]._description_selection(self.env)
                    )
                    raise ValidationError(
                        _(
                            "Order number is required for Export Type %r or %r",
                            selection["AF"],
                            selection["FC"],
                        )
                    )
                ...
                raise ValidationError(
                    _(
                        "Order number (%s) is invalid, must be like: %s",
                        record.order_number,
        ...
        for record in self:
            if not record.process_number:
                if self.export_type in ("CO", "DE"):
                    ...
                    raise ValidationError(
                        _(
                            "Process number is required for Export Type %r or %r",
                            selection["CO"],
                            selection["DE"],
                        )
                    )
                ...
                raise ValidationError(
                    _(
                        "Process number (%s) is invalid, must be like: %s",
                        record.process_number,
                        PROCESS_NUMBER_EXAMPLE,
                    )
                )
    ...
    def _get_moves_data(self):
        ...
        for period, moves in move_data.items():
            for move_key, lines in moves.items():
                move_data[period][move_key] = sorted(
                    lines, key=lambda x: x["line_label"]
                )
    ...
    def _get_xml_data(self):
        ...
        for period, moves in self._get_moves_data().items():
            ...
            xml_content = self.env["ir.qweb"]._render(
                "l10n_mx_xml_polizas.xml_polizas",
                values={
                    "period": period,
                    "moves": moves,
            ...
            with tools.file_open(
                "l10n_mx_xml_polizas/data/xsd/1.3/PolizasPeriodo_1_3.xsd", "rb"
            ) as xsd:
            ...
            records.append(
                {
                    "year": period.year,
                    "month": period.month,
                    "filename": "%s%s%sPL.XML"
                    % (self.env.company.vat, period.year, period.month),
                    "content": xml_content,
                }
            )
    ...
    def export_xml(self):
        ...
        if not vat:
            ...
            raise RedirectWarning(
                _("Please define the VAT on your company."),
                action.id,
                _("Company Settings"),
            )
