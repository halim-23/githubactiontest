...
class IrAttachment(models.Model):
    ...
    def _load_xsd_lu_electronic_files(self):
        ...
        attachment = self.env.ref(
            "l10n_lu_reports.xsd_cached_eCDF_file_v2_0-XML_schema_xsd", False
        )
        if attachment:
            return
        try:
            ...
            _logger.info(
                "Cannot connect with the given URL for the Luxembourg electronic reports xsd."
            )
            return
        try:
            ...
            _logger.info(
                "You are trying to load an invalid xsd file for the Luxembourg electronic reports.\n%s",
                e,
            )
            return
        attachment = self.create(
            {
                ...
                "datas": base64.encodebytes(
                    response.content.replace(b'<xsd:pattern value="[\\P{Cc}]+" />', b"")
                ),
            }
        )
