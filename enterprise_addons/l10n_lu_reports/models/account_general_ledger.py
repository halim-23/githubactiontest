...
class AccountGeneralLedger(models.AbstractModel):
    ...
    def _fill_l10n_lu_saft_report_invoices_values(self, options, values):
        ...
        for move_vals in values["move_vals_list"]:
            ...
            for line_vals in move_vals["line_vals_list"]:
                if line_vals["tax_line_id"]:
                    ...
                    not line_vals["account_internal_type"] in ("receivable", "payable")
                    and not line_vals["exclude_from_invoice_tab"]
                ):
            ...
            move_vals["total_invoice_balance"] = (
                move_vals["total_invoice_untaxed_balance"]
                + move_vals["total_invoice_tax_balance"]
            )
        ...
        if non_ref_uoms:
            ...
            uoms |= self.env["uom.uom"].search(
                [
                    ("category_id", "in", non_ref_uoms.category_id.ids),
                    ("uom_type", "=", "reference"),
                ]
            )
        res["uoms"] = uoms
        ...
        for product_code, grouped_products in groupby(
            res["product_vals_list"], key=lambda product: product["default_code"]
        ):
    ...
    def get_xml(self, options):
        ...
        xsd_attachment = self.env["ir.attachment"].search(
            [("name", "=", "xsd_cached_FAIA_v_2_01_reduced_version_A_xsd")]
        )
        if xsd_attachment:
            with io.BytesIO(
                base64.b64decode(xsd_attachment.with_context(bin_size=False).datas)
            ) as xsd:
