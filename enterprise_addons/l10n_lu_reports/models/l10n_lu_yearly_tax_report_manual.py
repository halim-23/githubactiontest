...
class L10nLuYearlyTaxReportManual(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            _("Only one tax report data record per year (per company) is allowed!"),
        )
    ]

    company_id = fields.Many2one("res.company", required=True)
    year = fields.Char(required=True)
    avg_nb_employees = fields.Float(
        string="Average number of employees for the fiscal year",
        compute="_compute_avg_nb_employees",
    )
    avg_nb_employees_with_salary = fields.Float("  - with salary or wage")
    avg_nb_employees_with_no_salary = fields.Float(
        "  - with no salary (family members)"
    )
    phone_number = fields.Char(
        string="Phone number for contacting the declaring person", size=30
    )
    ...
    submitted_rcs = fields.Boolean(
        string="Annual accounts submitted to the Trade and Companies Register (RCS)"
    )
    ...
    def name_get(self):
        ...
        for r in self:
            result.append(
                (
                    r.id,
                    _("Yearly Tax Report Manual Data")
                    + " ("
                    + str(r.company_id.name)
                    + ") "
                    + r.year,
                )
            )
    ...
    def _compute_avg_nb_employees(self):
        self.avg_nb_employees = (
            self.avg_nb_employees_with_no_salary + self.avg_nb_employees_with_salary
        )
    ...
    def _compute_totals(self):
        self.report_section_472_rest = (
            self.report_section_472
            - self.report_section_001
            - self.report_section_002
            - self.report_section_003
            - self.report_section_004
            - self.report_section_005
            - self.report_section_007
        )
        self.report_section_455_rest = (
            self.report_section_455 - self.report_section_008 - self.report_section_009
        )
        self.report_section_456_rest = (
            self.report_section_456 - self.report_section_010 - self.report_section_011
        )
        self.report_section_457_rest = (
            self.report_section_457 - self.report_section_013 - self.report_section_202
        )
        self.report_section_458_rest = (
            self.report_section_458
            - self.report_section_077
            - self.report_section_081
            - self.report_section_085
        )
        self.report_section_459_rest = (
            self.report_section_459
            - self.report_section_078
            - self.report_section_082
            - self.report_section_086
        )
        self.report_section_460_rest = (
            self.report_section_460
            - self.report_section_079
            - self.report_section_083
            - self.report_section_087
        )
        self.report_section_461_rest = (
            self.report_section_461
            - self.report_section_404
            - self.report_section_405
            - self.report_section_406
        )

    def print_xml(self):
        generating_wizard = self.env.context["calling_wizard_id"]
        return (
            self.env["l10n_lu.generate.tax.report"]
            .browse(generating_wizard)
            .with_context(tax_report_data_id=self.id)
            .get_xml()
        )
