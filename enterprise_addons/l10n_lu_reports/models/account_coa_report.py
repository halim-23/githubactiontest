...
from ..models.coa_data import ACCOUNTS_2019, ACCOUNTS_2020


class AccountChartOfAccountReport(models.AbstractModel):
    _inherit = "account.coa.report"

    def _get_lu_xml_2_0_report_values(
        self,
        options,
        avg_nb_employees=1,
        size="small",
        ...
        if date_from.year >= 2020:
            ...
            raise ValidationError(
                _("CoA report for Luxembourg not supported for years previous to 2019.")
            )
        ...
        if date_from.year <= 2019:
            update_fields, net142 = self._assemble_results_financial_year(
                account_fields
            )
            account_fields.update(update_fields)
            # Update class total 1-5 and set net debit or credit only
            net1to5 = (
                account_fields.get("1111", 0.00)
                - account_fields.get("1112", 0.00)
                + net142
            )
        ...
        # 2020 new totals
        if date_from.year >= 2020:
            account_fields["2957"] = account_fields.get(
                "1111", 0.00
            ) + account_fields.get("2257", 0.00)
            account_fields["2958"] = account_fields.get(
                "1112", 0.00
            ) + account_fields.get("2258", 0.00)
            account_fields["0161"] = account_fields.get(
                "1111", 0.00
            ) - account_fields.get("1112", 0.00)
        ...
        for val in account_dict.values():
            if (
                val.get("required", False)
                and val["debit"] not in account_fields
                and val["credit"] not in account_fields
            ):
        ...
        if (
            account_fields.get("2261")
            or account_fields.get("2262")
            or account_fields.get("2347")
            or account_fields.get("2348")
        ):
            net106 = (
                account_fields.get("2261", 0.00)
                - account_fields.get("2262", 0.00)
                + account_fields.get("2347", 0.00)
                - account_fields.get("2348", 0.00)
            )
    ...
    def _get_account_fields(self, options, account_dict, year):
        ...
        for line in [
            ln
            for ln in lines
            if self._get_model_info_from_id(ln.get("id"))[0] == "account.account"
        ]:
            ...
            for i in range(len(code)):
                ...
                if acc:
                    if balance > 0.00:
                        account_fields[acc["debit"]] = (
                            account_fields.get(acc["debit"], 0.00) + balance
                        )
                    # Accounts at 0.00 not counted because they might be accounts from an old CoA
                    # being moved to another account
                    elif balance < 0.00:
                        account_fields[acc["credit"]] = (
                            account_fields.get(acc["credit"], 0.00) - balance
                        )
        ...
        for account_code in account_dict:
            ...
            if (
                account_fields.get(account_dict[account_code]["debit"])
                or account_fields.get(account_dict[account_code]["credit"])
            ) and not (
                account_dict[account_code]["debit"] == "2533"
                and account_dict[account_code]["credit"] == "2536"
            ):
                net = account_fields.get(
                    account_dict[account_code]["debit"], 0.00
                ) - account_fields.get(account_dict[account_code]["credit"], 0.00)
                ...
                if len(account_code) == 2:
                    if account_code[0] in ["1", "2", "3", "4", "5"]:
                        if net > 0.00:
                            account_fields["1111"] = (
                                account_fields.get("1111", 0.00) + net
                            )
                        else:
                            account_fields["1112"] = (
                                account_fields.get("1112", 0.00) - net
                            )
                    elif account_code[0] in ["6", "7"]:
                        if net > 0.00:
                            account_fields["2257"] = (
                                account_fields.get("2257", 0.00) + net
                            )
                        else:
                            account_fields["2258"] = (
                                account_fields.get("2258", 0.00) - net
                            )
        ...
        if year <= 2019:
            net6to7 = account_fields.get("2257", 0.00) - account_fields.get(
                "2258", 0.00
            )
    ...
    def _assemble_results_financial_year(self, account_fields):
        ...
        if float_compare(net142, 0.0, 2) > 0:
            ...
            net142 = account_fields.get("2257", 0.00) - account_fields.get("2258", 0.00)
        net14 = (account_fields.get("0159", 0.00) + update_fields.get("0161", 0.00)) - (
            account_fields.get("0160", 0.00) + update_fields.get("0162", 0.00)
        )
