...
class AssetsReport(models.AbstractModel):
    ...
    def print_xml(self, options):
        if self._get_report_country_code(options) == "LU":
            ...
            generate_xml = (
                self.env["l10n_lu.generate.xml"].with_context(new_context).create({})
            )
    ...
    def _get_lu_xml_2_0_report_values(self, options):
        ...
        def _get_assets_data(lines):
            ...
            asset_ids = {
                self._get_model_info_from_id(line["id"])[1]: line["id"]
                for line in lines
                if "|" in line["id"]
            }
            assets = self.env["account.asset"].search(
                [("id", "in", list(asset_ids.keys()))]
            )
            ...
            if any(
                [
                    ...
                    for curr in list(asset_currencies) + [self.env.company.currency_id]
                ]
            ):
                raise ValidationError(
                    _(
                        "Only assets having EUR currency for companies using EUR currency can be reported."
                    )
                )
            depreciable_values = {
            ...
            for asset in assets:
                ...
                for ml in asset.original_move_line_ids:
                    ...
                    for tax in ml.tax_ids:
                        ...
                        tax_line = ml.move_id.line_ids.filtered(
                            lambda r: r.tax_line_id == tax
                            and r.tax_repartition_line_id
                            and r.debit
                        )
                        if tax_line:
                            total_tax += (
                                tax_line.balance * balance / tax_line.tax_base_amount
                            )
        ...
        (
            ...
            expenditures_table,
            depreciations_table,
        ) = self._l10n_lu_get_expenditures_and_depreciations_tables(
    ...
    def _l10n_lu_get_expenditures_and_depreciations_tables(
        self, lines, tax_amounts, depreciable_values
    ):
        """
        ...
        for line in lines:
            ...
            acquisition_date = parse_date(
                self.env, line["columns"][0]["name"]
            ).strftime("%d/%m/%Y")
            name = line["name"]
            acquisition_cost_no_vat = float(
                line["columns"][4]["no_format_name"]
            ) + float(line["columns"][5]["no_format_name"])
                ...
                expenditures_line.update(
                    {
                        "504": {
                            "field_type": "float",
                            "value": float_round(acquisition_cost_no_vat + vat, 2),
                        },
                        "505": {"field_type": "float", "value": float_round(vat, 2)},
                    }
                )
            expenditures_line["506"] = {
                "field_type": "float",
                "value": float_round(acquisition_cost_no_vat, 2),
            }
            expenditures_line["508"] = {
                "field_type": "float",
                "value": float_round(value_to_be_depreciated, 2),
            }
            expenditures_table.append(expenditures_line)

            # Update Depreciation/amortisation table
            depreciation_or_amortisation = float(
                line["columns"][3]["name"][:-2]
            )  # remove ' %' sign
            ...
            book_value_beginning = float(line["columns"][4]["no_format_name"]) - float(
                line["columns"][8]["no_format_name"]
            )
            ...
            depreciation = float(line["columns"][9]["no_format_name"]) - float(
                line["columns"][10]["no_format_name"]
            )
            ...
            depreciations_line = {
                ...
                "604": {
                    "field_type": "float",
                    "value": float_round(depreciation_or_amortisation, 2),
                },
                "605": {
                    "field_type": "float",
                    "value": float_round(book_value_beginning, 2),
                },
            ...
            for key in range(604, 610):
                if (
                    depreciations_line.get(str(key))
                    and depreciations_line[str(key)]["value"] == 0.00
                ):
        ...
        total_vat = sum(
            [i.get("505", {"value": 0.00})["value"] for i in expenditures_table]
        )
        ...
        total_book_value_beginning = sum(
            [i.get("605") and i["605"]["value"] or 0.0 for i in depreciations_table]
        )
        total_acquisitions = sum(
            [i.get("606") and i["606"]["value"] or 0.0 for i in depreciations_table]
        )
        total_sales = sum(
            [i.get("607") and i["607"]["value"] or 0.0 for i in depreciations_table]
        )
        total_depreciation = sum(
            [i.get("608") and i["608"]["value"] or 0.0 for i in depreciations_table]
        )
        total_book_value_end = sum(
            [i.get("609") and i["609"]["value"] or 0.0 for i in depreciations_table]
        )
        ...
        if totals_depreciations_table["613"]["value"] != 0.00:
            totals_depreciations_table["616"] = {
                "field_type": "float",
                "value": totals_depreciations_table["613"]["value"],
            }
