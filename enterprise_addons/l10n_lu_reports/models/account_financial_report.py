...
class ReportAccountFinancialReport(models.Model):
    ...
    def _get_lu_reports(self):
        return {
            ...
            self.env.ref(
                "l10n_lu_reports.account_financial_report_l10n_lu_pl"
            ).id: "CA_COMPP",
            self.env.ref(
                "l10n_lu_reports.account_financial_report_l10n_lu_pl_abr"
            ).id: "CA_COMPPABR",
        }
    ...
    def _get_lu_electronic_report_values(self, options):
        ...
        values.update(
            {
                ...
                "03": {
                    "value": self.env.company.currency_id.name,
                    "field_type": "char",
                },
            }
        )
        ...
        for line in filter(
            lambda l: "financial_group_line_id" not in l
            and isinstance(l.get("id"), int),
            lines,
        ):
            ...
            split_line_code = (ReportLine.browse(line["id"]).code or "").split(
                "_"
            ) or []
            ...
            if len(split_line_code) > 2:
                ...
                _report_useful_fields(
                    columns[0]["no_format"], split_line_code[2], parent_code, required
                )
                # previous year balance
                _report_useful_fields(
                    columns[1]["no_format"],
                    str(int(split_line_code[2]) + 1),
                    parent_code and str(int(parent_code) + 1),
                    required,
                )
    ...
    def _get_lu_xml_2_0_report_values(self, options, references=False):
        ...
        def _get_references():
            ...
            names = {}
            notes = (
                self.env["account.report.manager"]
                .search(
                    [
            ...
            for note in notes:
                ...
                if len(split) > 1 and split[-2].isnumeric() and split[-1].isnumeric():
                    line = self.env["account.financial.html.report.line"].search(
                        [("id", "=", split[-2])], limit=1
                    )
        ...
        for form in lu_template_values["forms"]:
            if references:
                ...
                references = {
                    r: references[r]
                    for r in references
                    if str(int(r) - 1000) in form["field_values"]
                    or str(int(r) - 999) in form["field_values"]
                }
                names = {
                    r: names[r]
                    for r in references
    ...
    def l10n_lu_open_report_export_wizard(self, options):
        ...
        if options["date"]["mode"] == "single":
            ...
            new_context["account_report_generation_options"]["date"][
                "date_from"
        ...
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "l10n_lu_reports.view_l10n_lu_generate_accounts_report"
                    ).id,
                    "form",
                ]
            ],
