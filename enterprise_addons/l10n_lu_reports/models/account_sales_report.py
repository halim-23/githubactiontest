...
class ECSalesReport(models.AbstractModel):
    ...
    def _get_ec_sale_code_options_data(self, options):
        ...
        return {
            "goods": {
                "name": "L",
                "tax_report_line_ids": self.env.ref(
                    "l10n_lu.account_tax_report_line_1b_1_intra_community_goods_pi_vat"
                ).ids,
            },
            "triangular": {
                "name": "T",
                "tax_report_line_ids": self.env.ref(
                    "l10n_lu.account_tax_report_line_1b_6_a_subsequent_to_intra_community"
                ).ids,
            },
            "services": {
                "name": "S",
                "tax_report_line_ids": self.env.ref(
                    "l10n_lu.account_tax_report_line_1b_6_b1_non_exempt_customer_vat"
                ).ids,
            },
        }
            ...
            res.append(
                {
                    "name": _("XML"),
                    "sequence": 3,
                    "action": "l10n_lu_open_report_export_wizard",
                }
            )
        return res
    ...
    def _process_query_result(self, options, query_result):
        if self._get_report_country_code(options) != "LU":
            return super(ECSalesReport, self)._process_query_result(
    ...
    def _get_report_data(self, options):
        ...
        if (
            dt_from.day == 1
            and dt_from.month % 3 == 1
            and dt_to == dt_from + relativedelta(day=31, month=dt_from.month + 2)
        ):
        ...
        # dt_from is 1st day & dt_to is last day of same month
            ...
            raise UserError(
                _("Check from/to dates. XML must cover 1 full month or 1 full quarter.")
            )
        ...
        xml_data.update(
            {
                ...
                "type_labes": month
                and ["TVA_LICM", "TVA_PSIM"]
                or ["TVA_LICT", "TVA_PSIT"],
            }
        )

        rendered_content = self.env["ir.qweb"]._render(
            "l10n_lu_reports.EcSalesLuXMLReport", xml_data
        )
    ...
    def _get_lu_xml_2_0_report_values(self, options, comparison_files):
        ...
        intrastat_codes = [
            c["id"] for c in options["ec_sale_code"] if c["selected"]
        ] or ["L", "T", "S"]
        corrections, compared_declarations = self._get_correction_data(
            options, comparison_files
        )
        forms = self._format_export_values(
            intrastat_codes,
            xml_data,
            corrections,
        ...
        return (
            forms,
            year,
            month and "M" + str(month) or quarter and "Q" + str(quarter),
            "".join(intrastat_codes),
        )
    ...
    def l10n_lu_open_report_export_wizard(self, options):
        ...
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "l10n_lu_reports.view_l10n_lu_generate_vat_intra_report"
                    ).id,
                    "form",
                ]
            ],
    ...
    def _format_export_values(
                    ...
                    corr_table.append(
                        {
                            ...
                            "11": {
                                "field_type": "number",
                                "value": correction_time[1],
                            },  # Year
                            period_field: {
                                "field_type": "number",
                                "value": correction_time[2],
                            },  # Period
        ...
        for dummy, ctype, cyear, cperiod in compared_declarations:
            ...
            if (
                int(cyear) > int(year)
                or cperiodmonth >= dperiod
                and int(cyear) == int(year)
            ):
                raise ValidationError(
                    _(
                        "Some problems with the comparison declarations occurred. "
        ...
        if "L" in intrastat_codes or "T" in intrastat_codes:
            ...
            l_table, l_tot = _get_decl_table(
                xml_data["l_lines"], xml_data["l_sum"], mapping
            )
            if xml_data["l_lines"]:
            ...
            t_table, t_tot = _get_decl_table(
                xml_data["t_lines"], xml_data["t_sum"], mapping
            )
            if xml_data["t_lines"]:
            ...
            if corrections:
                ...
                total_corrections = corrections["l_sum"] * int(
                    "L" in intrastat_codes
                ) + corrections["t_sum"] * int("T" in intrastat_codes)
            form["field_values"]["16"] = {
                "field_type": "float",
        ...
        if "S" in intrastat_codes:
            ...
            mapping = {"total": "04", "country": "01", "vat": "02", "amount": "03"}
            s_table, s_tot = _get_decl_table(
                xml_data["s_lines"], xml_data["s_sum"], mapping
            )
            if xml_data["s_lines"]:
            ...
            # Corrections
            if corrections:
                s_corr_table = _get_corrective_table(
                    corrections["s_lines"], is_triangular=False, language=language
                )
                total_corrections = corrections["s_sum"] if corrections else 0.00
                form["field_values"]["16"] = {
                    "field_type": "float",
                    "value": corrections["s_sum"],
                }
                if s_corr_table:
    ...
    def _get_correction_data(self, options, comparison_files):
        ...
        for name, dec in comparison_files:
            try:
                attached_declarations.append(
                    (name, self._get_data_from_xml(dec, self.env.company.matr_number))
                )
            except ValidationError as err:
                raise ValidationError(_("Error in file ") + name + ": " + str(err))
        summarized_data, original_declarations = self._summarize_data(
            attached_declarations
        )
    ...
    def _get_data_from_xml(self, string, company_matr):
        ...
        fields_map = {
            ("03", "07", "14"): "amount",
            ("07", "15"): "triangular",
            ("01", "05", "09"): "acquirer_country",
            ("02", "06", "10"): "acquirer_vat",
            ("11"): "year",
            ("12", "18"): "period",
            ("12"): "quarterly",
            ("18"): "monthly",
        ...
        try:
            ...
            raise ValidationError(
                _("The provided comparison file is not a properly formatted XML.")
            )
        ...
        try:
            ...
            declarer = [
                ...
                for declarer in d.iterchildren(tag=prfx + "Declarer")
            ]
            ...
            for decl in declarations:
                ...
                tables = [
                    table for table in decl.FormData.iterchildren(tag=prfx + "Table")
                ]
                for line in [
                    line
                    for table in tables
                    for line in table.iterchildren(tag=prfx + "Line")
                ]:
                    line_data = {}
                    for field_type in ("TextField", "NumericField"):
                        for field in line.iterchildren(tag=prfx + field_type):
                            for key in [
                                key for key in fields_map if field.attrib["id"] in key
                            ]:
                    ...
                    if not corrective:
                        ...
                        line_data["type"] = dtype[:-1] + (
                            "T" if line_data.get("quarterly", False) else "M"
                        )
                    line_processed = {
                        k: line_data[k]
                        for k in (
            ...
            raise ValidationError(
                _("The provided comparison file is not a proper eCDF XML declaration!")
            )
        ...
        if not data["corrective"] and not data["original"] and not data["declared"]:
            raise ValidationError(
                _(
                    "There are no Intracommunity VAT declarations for the declaring company in the provided file!"
                )
            )
        return data
    ...
    def _summarize_data(self, declarations):
        ...
        original_declarations = [
            (d["type"], d["year"], d["period"])
            for decl in declarations
            for d in decl[1]["declared"]
        ]
        summarized_data = {
            key: {"s_lines": {}, "t_lines": {}, "l_lines": {}}
            for key in original_declarations
        }
        ...
        for line in [
            ...
            for ln in (data[1]["original"] + data[1]["corrective"])
        ]:
            ...
            if line_decl in original_declarations:
                line_type = (
                    "s_lines"
                    if line["type"][4] == "P"
                    else line["triangular"] and "t_lines" or "l_lines"
                )
                acq = (line["acquirer_country"], line["acquirer_vat"])
                summarized_data[line_decl][line_type][acq] = summarized_data[line_decl][
                    line_type
                ].get(acq, 0.00) + float(line["amount"].replace(",", "."))
    ...
    def _compare_declarations(self, summarized_data, options):
        ...
        for key in summarized_data:  # key: (declaration_type, year, period)
            ...
            new_data = {
                ln_type: {(k[0], k[1]): k[3] for k in decl}
                for ln_type, decl in new_lines.items()
                if ln_type.endswith("lines")
            }
            for k in new_data:
                if (
                    k[0] in ["l", "t"]
                    ...
                    corrections[k][key] = _compare_lines(
                        summarized_data[key].get(k, {}), new_data[k]
                    )
                else:
                    corrections[k][key] = {}
        ...
        for k in corrections:
            correction_sums[k.replace("lines", "sum")] = sum(
                [sum(corrections[k][key].values()) for key in corrections[k]]
            )
...
class L10n_luStoredSalesReport(models.Model):
    ...
    codes = fields.Selection(
        [
            (
                "LT",
                "Supply of goods and supply of goods made in the context of triangular operations",
            ),
            ("S", "Supply of services"),
            (
                "LTS",
                "Supply of goods (normal/in the context of triangular operations) and supply of services",
            ),
        ]
    )
