...
class L10nLuGenerateAccountsReport(models.TransientModel):
    ...
    size = fields.Selection(
        [
            ("large", "Large-Sized Undertaking"),
            ("medium", "Medium-Sized Undertaking"),
            ("small", "Small-Sized Undertaking"),
        ],
        required=True,
        default="large",
    )
    pl = fields.Selection(
        [("abr", "Abridged"), ("full", "Full")], required=True, default="full"
    )
    bs = fields.Selection(
        [("abr", "Abridged"), ("full", "Full")], required=True, default="full"
    )
    ...
    def _lu_get_declarations(self, declaration_template_values):
        ...
        if not self.coa_only:
            ...
            bs_declaration = bs_report._get_lu_xml_2_0_report_values(
                bs_report_options, self.import_notes_as_references
            )[0]
        ...
        if not self.coa_only:
            ...
            pl_declaration = pl_report._get_lu_xml_2_0_report_values(
                pl_report_options, self.import_notes_as_references
            )[0]
        ...
        options["date"] = {
            "date_from": datetime.strptime(
                report_options["date"]["date_from"], "%Y-%m-%d"
            ),
        ...
        coa_declaration = self.env["account.coa.report"]._get_lu_xml_2_0_report_values(
            ...
            self.bs,
            self.coa_only,
            self.optional_remarks,
        )

        forms = [pl_declaration, bs_declaration] if not self.coa_only else []
        forms.append(coa_declaration)
        if (
            datetime.strptime(report_options["date"]["date_from"], "%Y-%m-%d").year
            != 2019
            and not self.coa_only
        ):
        ...
        return {
            ...
            "year": fields.Datetime.from_string(
                report_options["date"]["date_from"]
            ).year,
            "period": "1",
        }
