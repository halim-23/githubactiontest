...
from ..models.l10n_lu_tax_report_data import (
...
class L10nLuGenerateTaxReport(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if not rec["period"]:
            raise ValidationError(
                _(
                    "The fiscal period you have selected is invalid, please verify. Valid periods are : Month, Quarter and Year."
                )
            )
    ...
    def _get_export_vat(self):
        if self.env.context.get("tax_report_options"):
            ...
            options = self.env["account.generic.tax.report"]._get_options(
                previous_options={"tax_report": self.env.ref("l10n_lu.tax_report").id}
            )
    ...
    def open_repartition_model(self):
        ...
        options = self.env.context["tax_report_options"]
        decl = self.env["account.generic.tax.report"]._get_lu_electronic_report_values(
            options
        )["forms"][0]
        ...
        if not yearly_tax_report_manual:
            yearly_tax_report_manual = self.env[
                "l10n_lu.yearly.tax.report.manual"
            ].create(
                {
                    "year": str(decl["year"]),
                    "company_id": self.env.company.id,
        ...
        view_id = self.env.ref(
            "l10n_lu_reports.view_l10n_lu_yearly_tax_report_manual_export"
        ).id
        return {
    ...
    def _lu_get_declarations(self, declaration_template_values):
        ...
        form = self.env["account.generic.tax.report"]._get_lu_electronic_report_values(
            options
        )["forms"][0]
        ...
        form["model"] = 1
        declaration = {
            "declaration_singles": {"forms": [form]},
            "declaration_groups": [],
        }
    ...
    def _add_yearly_fields(self, data, form):
        numeric_fields = {
            "001": data.report_section_001,
            "002": data.report_section_002,
            "003": data.report_section_003,
            "004": data.report_section_004,
            "005": data.report_section_005,
            "007": data.report_section_007,
            "008": data.report_section_008,
            "009": data.report_section_009,
            "010": data.report_section_010,
            # field 010 reports to the annex
            "389": data.report_section_010,
            "388": data.report_section_010,
            "011": data.report_section_011,
            "013": data.report_section_013,
            "202": data.report_section_202,
            "077": data.report_section_077,
            "078": data.report_section_078,
            "079": data.report_section_079,
            "404": data.report_section_404,
            "081": data.report_section_081,
            "082": data.report_section_082,
            "083": data.report_section_083,
            "405": data.report_section_405,
            "085": data.report_section_085,
            "086": data.report_section_086,
            "087": data.report_section_087,
            "406": data.report_section_406,
        }
    ...
    def _adapt_to_full_annual_declaration(self, form, report_id=None):
        ...
        if report_id:
            ...
            data = self.env["l10n_lu.yearly.tax.report.manual"].browse(
                self.env.context["tax_report_data_id"]
            )
        ...
        if rest:
            raise ValidationError(
                _(
                    "The following monthly fields haven't been completely allocated yet: "
                )
                + str(rest)
            )

        if data.phone_number:
            form["field_values"]["237"] = {
                "value": data.phone_number,
                "field_type": "char",
            }
        if data.books_records_documents:
            form["field_values"]["238"] = {
                "value": data.books_records_documents,
                "field_type": "char",
            }
        if data.avg_nb_employees_with_salary:
            form["field_values"]["108"] = {
                "value": data.avg_nb_employees_with_salary,
                "field_type": "float",
            }
        if data.avg_nb_employees_with_no_salary:
            form["field_values"]["109"] = {
                "value": data.avg_nb_employees_with_no_salary,
                "field_type": "float",
            }
        if data.avg_nb_employees:
            form["field_values"]["110"] = {
                "value": data.avg_nb_employees,
                "field_type": "float",
            }
        ...
        if data.report_section_007:
            ...
            form["field_values"]["206"] = {
                "value": data.report_section_206,
                "field_type": "char",
            }
        ...
        if data.report_section_010:
            form["field_values"]["387"] = {
                "value": "Report from 010",
                "field_type": "char",
            }
        # Appendix part F: Names and addresses to be specified (accountant/lessor)
        for k, v in {
            "397": data.report_section_397,
            "398": data.report_section_398,
            "399": data.report_section_399,
            "400": data.report_section_400,
            "401": data.report_section_401,
            "402": data.report_section_402,
        }.items():
            if v:
        ...
        for total, f in YEARLY_NEW_TOTALS.items():
            form["field_values"][total] = {
                "value": (
                    sum(
                        [
                            form["field_values"].get(a)
                            and float(
                                str(form["field_values"][a]["value"]).replace(",", ".")
                            )
                            or 0.00
                            for a in f.get("add", [])
                        ]
                    ...
                    - sum(
                        [
                            ...
                            or 0.00
                            for a in f.get("subtract", [])
                        ]
                    )
                ),
                "field_type": "float",
            }
        form["field_values"]["998"] = {
            "value": "1" if data.submitted_rcs else "0",
            "field_type": "boolean",
        }
        form["field_values"]["999"] = {
            "value": "0" if data.submitted_rcs else "1",
            "field_type": "boolean",
        }
        # Add annex
        if self.env.context.get("tax_report_options"):
            annex_fields, expenditures_table = self._add_annex(
                self.env.context["tax_report_options"]
            )
    ...
    def _adapt_to_simplified_annual_declaration(self, form):
        ...
        form["field_values"] = {
            k: v
            for k, v in form["field_values"].items()
            if k in YEARLY_SIMPLIFIED_FIELDS
        }

    def _get_account_code(self, ln):
        if ln.get("caret_options") == "account.account":
            account_code = (
                self.env["account.account"].browse(ln["id"]).mapped("code")[0]
            )
            return account_code
        return False
    ...
    def _get_annex_data_from_lines(self, lines):
        ...
        for ln in lines:
            ...
            if account_code:
                ...
                matching = [
                    code
                    for domain, code in YEARLY_ANNEX_MAPPING.items()
                    if int(domain[0]) <= int(account_code)
                    and int(domain[1]) > int(account_code)
                ]
                for code in matching:
                    annex_lines.setdefault(code, {})
                    annex_lines[code]["%"] = 100.00  # business portion always 100%
                    annex_lines[code]["base_amount"] = (
                        annex_lines[code].get("base_amount", 0.00)
                        + ln["columns"][0]["no_format"]
                    )
                    annex_lines[code]["tot_VAT"] = (
                        annex_lines[code].get("tot_VAT", 0.00)
                        + ln["columns"][1]["no_format"]
                    )
                    annex_lines[code]["total"] = (
                        annex_lines[code].get("total", 0.00)
                        + ln["columns"][0]["no_format"]
                        + ln["columns"][1]["no_format"]
                    )
                    ...
                    if code == "A43":
                        ...
                        annex_lines[code]["detailed_lines"] = annex_lines[code].get(
                            "detailed_lines", []
                        ) + [detail_line]
        return annex_lines
    ...
    def _add_expenditures(self, data, lu_annual_report=False):
        ...
        for data_dict in data.get("detailed_lines", []):
            ...
            report_line["412"] = {
                "value": data_dict["bus_base_amount"],
                "field_type": "float",
            }
    ...
    def _add_annex_fields_expenditures(
        self, annex_fields, lines, lu_annual_report=False
    ):
        ...
        for code, data in annex_lines.items():
            if code and (
                data.get("base_amount") or data.get("tot_VAT") or data.get("total")
            ):
                ...
                if YEARLY_ANNEX_FIELDS[code].get("base_amount"):
                    annex_fields[YEARLY_ANNEX_FIELDS[code]["base_amount"]] = data[
                        "base_amount"
                    ]
    ...
    def _add_annex(self, options):
        ...
        lines = (
            ...
            .with_context(
                self.env["account.generic.tax.report"]._set_context(annex_options)
            )
            ._get_lines(annex_options)
        )
        (
            annex_fields,
            expenditures_table,
            total_base_amount,
            total_vat,
        ) = self._add_annex_fields_expenditures(self, annex_fields, lines)
        # Annex totals
        if annex_fields:
    ...
    def _remove_zero_fields(self, field_values):
        ...
        parents = (
            ...
            .search([])
            .mapped(lambda r: (r.code, r.parent_id.code))
        )
        parents_dict = {p[0]: p[1] for p in parents}
        new_field_values = {}
        for f in field_values:
            if (
                f in VAT_MANDATORY_FIELDS
                or field_values[f]["field_type"] not in ("float", "number")
                or (
                    field_values[f]["field_type"] == "number"
                    and field_values[f]["value"] != "0,00"
                )
                or (
                    field_values[f]["field_type"] == "float"
                    and float_compare(field_values[f]["value"], 0.0, 2) != 0
                )
            ):
