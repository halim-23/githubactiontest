...
class L10nLuGenerateXML(models.TransientModel):
    ...
    def _lu_validate_xml_content(self, content):
        attachment = self.env.ref(
            "l10n_lu_reports.xsd_cached_eCDF_file_v2_0-XML_schema_xsd",
    ...
    def get_xml(self, lu_annual_report=False):
        ...
        if agent:
            ecdf_not_ok = not agent.l10n_lu_agent_ecdf_prefix or not re.match(
                "[0-9A-Z]{6}", agent.l10n_lu_agent_ecdf_prefix
            )
            matr_not_ok = not agent.l10n_lu_agent_matr_number or not re.match(
                "[0-9]{11,13}", agent.l10n_lu_agent_matr_number
            )
            if ecdf_not_ok or matr_not_ok:
                raise RedirectWarning(
                    message=_(
                        "Some fields required for the export are missing or invalid. Please verify them."
                    ),
                    action={
                    ...
                    additional_context={
                        "required_fields": [
                            ecdf_not_ok and "l10n_lu_agent_ecdf_prefix",
        ...
        lu_template_values = {
            ...
            "interface": "MODL5",
            "agent_vat": vat or "NE",
            "agent_matr_number": agent.l10n_lu_agent_matr_number
            or company.matr_number
            or "NE",
            "agent_rcs_number": agent.l10n_lu_agent_rcs_number
            or company.company_registry
            or "NE",
            "declarations": [],
        }
        ...
        if lu_annual_report:
            declarations_data = lu_annual_report._lu_get_declarations(
                declaration_template_values
            )
            self._save_xml_report(
                declarations_data, lu_template_values, filename, lu_annual_report
            )
            url = (
                "web/content/?model="
                + lu_annual_report._name
                + "&id="
                + str(lu_annual_report.id)
                + "&filename_field=filename&field=report_data&download=true&filename="
    ...
    def _save_xml_report(
        self, declarations_data, lu_template_values, filename, lu_annual_report=False
    ):
        lu_template_values["declarations"] = declarations_data["declarations"]

        # Add function to format floats
        lu_template_values["format_float"] = lambda f: tools.float_utils.float_repr(
            f, 2
        ).replace(".", ",")
        rendered_content = self.env.ref(
            "l10n_lu_reports.l10n_lu_electronic_report_template_2_0"
        )._render(lu_template_values)
    ...
    def _lu_get_declarations(self, declaration_template_values):
        values = self.env[self.env.context["model"]]._get_lu_xml_2_0_report_values(
            self.env.context["account_report_generation_options"]
        )
        declarations = {
            "declaration_singles": {"forms": values["forms"]},
            "declaration_groups": [],
        }
