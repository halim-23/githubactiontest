...
class L10nLuGenerateSalesReport(models.TransientModel):
    ...
    def _lu_get_declarations(self, declaration_template_values):
        ...
        selected = [
            code.get("name")
            for code in report_generation_options.get("intrastat_code", [])
            if code.get("selected")
        ]
        if ("L" in selected) != ("T" in selected) and self.save_report:
            raise UserError(
                _(
                    "The report can't be saved, because it isn't a valid eCDF declaration. "
                    "Either both 'L' and 'T' codes should be selected, or none of them"
                )
            )
        comparison_files = [
            (d.attachment_id.name, base64.b64decode(d.attachment_id.datas))
            for d in self.l10n_lu_stored_report_ids
        ]
        ec_sales_report = self.env["account.sales.report"]
        forms, year, period, codes = ec_sales_report._get_lu_xml_2_0_report_values(
            report_generation_options, comparison_files
        )
        declarations = {
            "declaration_singles": {"forms": forms},
            "declaration_groups": [],
        }
