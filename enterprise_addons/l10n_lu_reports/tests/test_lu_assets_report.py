...
class LuxembourgAssetsReportTaxesTest(TestAccountReportsCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_lu.lu_2011_chart_1"):
        ...
        cls.tax_17 = cls.env["account.tax"].search(
            [
                ("company_id", "=", cls.company_data["company"].id),
                ("name", "=", "17-P-G"),
            ]
        )
        cls.tax_3 = cls.env["account.tax"].search(
            [
                ("company_id", "=", cls.company_data["company"].id),
                ("name", "=", "3-P-G"),
            ]
        ...
        cls.product = cls.env["product.product"].create(
            {"name": "product_1", "lst_price": 100.0}
        )
    ...
    def test_asset_no_invoice(self):
        ...
        report, options = self._get_report_and_options(
            report, datetime.today() - relativedelta(days=365), datetime.today()
        )
    ...
    def test_asset_multiple_lines_and_taxes(self):
        # Basic test case with one original_move_line_ids with one tax on it
        data = self._get_report_data_linked_assets(
            [self.inv1.invoice_line_ids[1], self.inv1.invoice_line_ids[2]]
        )
    ...
    def _get_report_data_linked_assets(cls, invoice_lines):
        ...
        asset_form.account_depreciation_expense_id = cls.company_data[
            "default_account_assets"
        ]
        ...
        report, options = cls._get_report_and_options(
            report, datetime.today() - relativedelta(days=365), datetime.today()
        )
        data = report._get_lu_xml_2_0_report_values(options)["forms"][0]["tables"][0]
        return data
    ...
    def _post_invoice(cls, lines):
        move_form = Form(
            cls.env["account.move"].with_context(default_move_type="in_invoice")
        )
        move_form.invoice_date = datetime.today() - relativedelta(days=500)
