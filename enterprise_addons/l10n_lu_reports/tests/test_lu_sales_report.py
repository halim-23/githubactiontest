...
class LuxembourgSalesReportTest(AccountSalesReportCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_lu.lu_2011_chart_1"):
        ...
        cls.l_tax = cls.env["account.tax"].search(
            [
        ...
        cls.t_tax = cls.env["account.tax"].search(
            [
                ("company_id", "=", cls.company_data["company"].id),
                ("name", "=", "0-ICT-S-G"),
                "|",
        ...
        cls.s_tax = cls.env["account.tax"].search(
            [
                ("company_id", "=", cls.company_data["company"].id),
                ("name", "=", "0-IC-S-S"),
                "|",
        ...
        cls.product_1 = cls.env["product.product"].create(
            {"name": "product_1", "lst_price": 1.0}
        )
        cls.product_2 = cls.env["product.product"].create(
            {"name": "product_2", "lst_price": 10.0}
        )
        cls.product_3 = cls.env["product.product"].create(
            {"name": "product_3", "lst_price": 100.0}
        )
    ...
    def test_ec_sales_report(self):
        ...
        self.assertEqual(
            report._get_report_country_code(options),
            "LU",
            "The country chosen for EC Sales list should be Luxembourg",
        )
    ...
    def test_empty_comparisons(self):
        report = self.env["account.sales.report"]
        report = report.with_context(date_from="2020-04-01", date_to="2020-04-30")
        options = report._get_options(None)
    ...
    def test_multiple_comparison(self):
        ...
        report, options = self._get_report_and_options(
            report, "2020-04-01", "2020-04-30"
        )
        wizard = self.env["l10n_lu.generate.vat.intra.report"].create({})
        wizard.save_report = False
        wizard.with_context(
            report.l10n_lu_open_report_export_wizard(options)["context"]
        ).get_xml()
        ...
        self.assertXmlTreeEqual(
            self.get_xml_tree_from_string(declaration_to_compare_2), expected_xml_tree
        )
        ...
        report, options = self._get_report_and_options(
            report, "2020-06-01", "2020-06-30"
        )
        corrections, compared_declarations = report._get_correction_data(
            options, comparison_files=[("", declaration_to_compare_2)]
        )  # problem with the options here
        self.assertListEqual(
            compared_declarations,
            [("", "TVA_LICM", "2020", "5"), ("", "TVA_PSIM", "2020", "5")],
        )
        expected = {
            "l_sum": 2.0,
            "t_sum": 20.0,
        ...
        self.assertDictEqual(
            expected,
            corrections,
            "Wrong correction values for Luxembourg EC Sales report.",
        )
        ...
        corrections, compared_declarations = report._get_correction_data(
            options,
            comparison_files=[
                ("", declaration_to_compare),
                ("", declaration_to_compare_2),
            ],
        )
        self.assertListEqual(
            compared_declarations,
    ...
    def test_wrong_files(self):
        ...
        asset_report = self.env["account.assets.report"].with_context(
        ...
        declaration_to_compare = base64.b64decode(wizard.report_data.decode("utf-8"))
        with self.assertRaises(ValidationError):
            report._get_correction_data(
                options, comparison_files=[("", declaration_to_compare)]
            )
