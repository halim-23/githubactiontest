  ...
  var GanttRenderer = AbstractRenderer.extend({
    ...
    _prepareViewInfo: function () {
      return {
        ...
        activeScaleInfo: {
          ...
          interval: this.SCALES[this.state.scale].cellPrecisions[
            this.cellPrecisions[this.state.scale]
          ],
    ...
    async _renderView() {
      ...
      return Promise.all(proms).then(function () {
        ...
        self._replaceElement(
          QWeb.render(self.template_to_use, {
            widget: self,
    ...
    _renderRows: function (rows, groupedBy) {
      ...
      rows.forEach(function (row) {
        ...
        var params = {
          ...
          consolidate:
            groupLevel === 0 &&
            self.state.groupedBy[0] === self.consolidationParams.maxField,
        ...
        if (
          self.thumbnails &&
          row.groupedByField &&
          row.groupedByField in self.thumbnails
        ) {
          params.thumbnail = {
