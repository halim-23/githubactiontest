odoo.define("web_gantt.GanttController", function (require) {
  ...
  var GanttController = AbstractController.extend({
    ...
    _copy: function (id, schedule) {
      return this._executeAsyncOperation(this.model.copy.bind(this.model), [
        id,
        schedule,
      ]);
    },
    ...
    _getDialogContext: function (date, rowId) {
      ...
      context[state.dateStopField] = date
        .clone()
        .endOf(this.SCALES[state.scale].interval);
      if (rowId) {
    ...
    _reschedule: function (ids, schedule, isUTC, callback) {
      return this._executeAsyncOperation(this.model.reschedule.bind(this.model), [
        ids,
        schedule,
        isUTC,
        callback,
    ...
    _onAddClicked: function (ev) {
      ...
      var context = {};
      var state = this.model.get();
      context[state.dateStartField] = this.model.convertToServerTime(
        state.focusDate.clone().startOf(state.scale)
      );
      context[state.dateStopField] = this.model.convertToServerTime(
        state.focusDate.clone().endOf(state.scale)
      );
      for (var k in context) {
        context[_.str.sprintf("default_%s", k)] = context[k];
      }
    ...
    _onPillDropped: function (ev) {
      ...
      if (diff) {
        ...
        schedule[state.dateStartField] = this.model.dateAdd(
          pill[state.dateStartField],
          diff,
          this.SCALES[state.scale].time
        );
        schedule[state.dateStopField] = this.model.dateAdd(
          pill[state.dateStopField],
          diff,
          this.SCALES[state.scale].time
        );
