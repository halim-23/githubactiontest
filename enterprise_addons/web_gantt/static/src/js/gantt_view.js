odoo.define("web_gantt.GanttView", function (require) {
  ...
  var GanttView = AbstractView.extend({
    ...
    jsLibs: ["/web/static/lib/nearest/jquery.nearest.js"],
    viewType: "gantt",
    ...
    init: function (viewInfo, params) {
      ...
      var collapseFirstLevel = Boolean(arch.attrs.collapse_first_level);

      // Unavailability
      var displayUnavailability = Boolean(arch.attrs.display_unavailability);
      ...
      var consolidationMax = arch.attrs.consolidation_max
      ...
      if (Object.keys(consolidationMax).length > 0) {
        ...
        collapseFirstLevel = Boolean(consolidationMaxField) || collapseFirstLevel;
      }
      ...
      var formViewId = arch.attrs.form_view_id
        ? parseInt(arch.attrs.form_view_id, 10)
        : false;
      if (params.action && !formViewId) {
        // Fallback on form view action, or 'false'
      ...
      var initialDate = moment(
        params.context.initialDate ||
          params.initialDate ||
          arch.attrs.initial_date ||
          new Date()
      );
      ...
      var thumbnails = this.arch.attrs.thumbnails
        ? pyUtils.py_eval(this.arch.attrs.thumbnails)
        : {};
      // Plan option
      var canPlan = this.arch.attrs.plan ? Boolean(JSON.parse(this.arch.attrs.plan)) : true;
      // Cell create option
      const canCellCreate = this.arch.attrs.cell_create
        ? Boolean(JSON.parse(this.arch.attrs.cell_create))
        : true;
      ...
      this.rendererParams.disableDragdrop = arch.attrs.disable_drag_drop
        ? Boolean(JSON.parse(arch.attrs.disable_drag_drop))
        : false;
