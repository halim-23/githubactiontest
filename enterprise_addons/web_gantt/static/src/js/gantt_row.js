odoo.define("web_gantt.GanttRow", function (require) {
  ...
  var GanttRow = Widget.extend({
    ...
    init: function (parent, pillsInfo, viewInfo, options) {
      ...
      this.cellHeight =
        this.level * (this.LEVEL_TOP_OFFSET + 3) + (this.level > 0 ? this.level : 0);
    ...
    setDroppable: function (firstCell) {
      ...
      this.$el.droppable({
        drop: function (event, ui) {
          ...
          if (diff || self.rowId !== oldRowId) {
            // Do not perform write if nothing change
    ...
    _bindPillPopover: function (target) {
      ...
      $target
        .popover({
          ...
          placement: "auto",
          content: function () {
            return self.viewInfo.popoverQWeb.render(
              "gantt-popover",
              self._getPopoverContext($(this).data("id"))
            );
          },
        })
        .popover("show");
    ...
    _aggregateGroupedPills: function () {
      ...
      var intervals = _.reduce(
        this.viewInfo.slots,
        function (intervals, slotStart) {
        ...
        function (pills, intervalStart) {
          ...
          if (pillsInThisInterval.length) {
            ...
            if (isContinuous && previousPill.count === pillsInThisInterval.length) {
              ...
              previousPill.aggregatedPills = previousPill.aggregatedPills.concat(
                pillsInThisInterval
              );
            } else {
              var newPill = {
                ...
                startDate: moment.max(
                  _.min(pillsInThisInterval, "startDate").startDate,
                  intervalStart
                ),
                stopDate: moment.min(
                  _.max(pillsInThisInterval, "stopDate").stopDate,
                  intervalStop
                ),
              };
              ...
              if (self.consolidate && self.consolidationParams.field) {
                newPill.consolidationValue = pillsInThisInterval.reduce(function (
                  sum,
                  pill
                ) {
      ...
      this.pills.forEach(function (pill) {
        ...
        if (self.consolidate && self.consolidationParams.maxValue) {
          ...
          pill.style = _.str.sprintf(
            "background-color: rgba(%s, %s, %s, 0.6)",
            color,
            color,
            color
          );
        ...
        switch (self.state.scale) {
          case "day":
            left = pill.startDate.diff(
              pill.startDate.clone().startOf("hour"),
              "minutes"
            );
            ...
            pill.width =
              gapSize > 0
                ? `calc(${widthPill}% + ${gapSize}px - ${margin}px)`
                : `calc(${widthPill}% - ${margin}px)`;
            break;
          case "week":
          case "month":
          ...
          case "year":
            ...
            if (monthsDiff < 1) {
              ...
              diff = Math.max(
                Math.ceil(pill.stopDate.diff(pill.startDate, "days", true)),
                2
              );
              pill.width = `calc(${
                (diff / pill.startDate.daysInMonth()) * 100
              }% - ${margin}px)`;
            } else {
    ...
    _getDiff: function (resizeSnappingWidth, gridOffset) {
      return (
        Math.round(gridOffset / resizeSnappingWidth) *
        this.viewInfo.activeScaleInfo.interval
      );
    },
    /**
    ...
    _getPopoverContext: function (pillID) {
      ...
      data.userTimezoneStartDate = this._convertToUserTime(
        data[this.state.dateStartField]
      );
      data.userTimezoneStopDate = this._convertToUserTime(
        data[this.state.dateStopField]
      );
      return data;
    },
    /**
    ...
    _getResizeSnappingWidth: function (firstCell) {
      if (!this.firstCell) {
        this.firstCell =
          firstCell ||
          $(".o_gantt_view .o_gantt_header_scale .o_gantt_header_cell:first")[0];
      }
      ...
      for (const date of this.viewInfo.slots) {
        ...
        if (!this.isGroup && this.unavailabilities.slice(index).length) {
          ...
          for (let j = 0; j < this.cellPart; j++) {
            ...
            const subSlotStop = date
              .clone()
              .add((j + 1) * cellTime, time)
              .subtract(1, "seconds");
            let subSlotUnavailable = 0;
            for (let i = index; i < this.unavailabilities.length; i++) {
              const u = this.unavailabilities[i];
    ...
    _saveResizeChanges: function (pillID, resizeDiff, direction) {
      ...
      if (direction === "left") {
        ...
        data.date = pill[this.state.dateStartField]
          .clone()
          .subtract(resizeDiff, this.viewInfo.activeScaleInfo.time);
      } else {
        data.field = this.state.dateStopField;
        data.date = pill[this.state.dateStopField]
          .clone()
          .add(resizeDiff, this.viewInfo.activeScaleInfo.time);
      }
    ...
    _setDraggable: function ($pill) {
      ...
      if (
        ...
        !pill.disableStartResize &&
        !pill.disableStopResize &&
        !this.isGroup
      ) {
      ...
      if (
        ...
        $pill.resizable({
          ...
          resize: function (event, ui) {
            var diff = Math.round(
              ((ui.size.width - ui.originalSize.width) / resizeSnappingWidth) *
                self.viewInfo.activeScaleInfo.interval
            );
      ...
      switch (this.state.scale) {
        ...
        case "month":
          timeSpans.forEach(function (span) {
            ...
            if (hourDiff === 0) {
              if (
                snappedStartDate.diff(span.startDate, "hours") > 2 &&
                span.stopDate.diff(snappedStopDate, "hours") > 2
              ) {
    ...
    _onMouseMove: function (ev) {
      if (
        (this.options.canCreate || this.options.canEdit) &&
        ...
        var hoveredCell;
        if (
          ev.target.classList.contains("o_gantt_pill") ||
          ev.target.parentNode.classList.contains("o_gantt_pill")
        ) {
          elementsFromPoint(ev.pageX, ev.pageY).some(function (element) {
            return element.classList.contains("o_gantt_cell")
              ? ((hoveredCell = element), true)
              : false;
          });
        } else {
