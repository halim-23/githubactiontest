...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        taxcloud_orders = self.filtered("fiscal_position_id.is_taxcloud")
        taxcloud_coupon_orders = taxcloud_orders.filtered(
            "order_line.coupon_program_id"
        )
        partial_taxcloud_coupon_orders = taxcloud_coupon_orders.filtered(
            not_totally_invoiceable
        )
        if partial_taxcloud_coupon_orders:
            bad_orders = str(partial_taxcloud_coupon_orders.mapped("display_name"))[
                1:-1
            ]
            bad_orders = (
                bad_orders if len(bad_orders) < 80 else bad_orders[:80] + ", ..."
            )
            raise UserError(
                _(
