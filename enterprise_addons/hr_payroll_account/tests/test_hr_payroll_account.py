...
class TestHrPayrollAccountCommon(TestPayslipContractBase):
    ...
    def setUpClass(cls):
        ...
        cls.hr_structure_softwaredeveloper = cls.env["hr.payroll.structure"].create(
            {
                ...
                "type_id": cls.env["hr.payroll.structure.type"]
        ...
        # YTI Clean that brol
        cls.hr_payslip_john.date_to = str(
...
class TestHrPayrollAccount(TestHrPayrollAccountCommon):
    def test_00_hr_payslip_run(self):
        ...
        payslip_employee = self.env["hr.payslip.employees"].create(
            {
                "employee_ids": [
                    (4, self.hr_employee_john.id),
                    (4, self.hr_employee_mark.id),
                ]
            }
        )
    ...
    def test_05_hr_payslip(self):
        ...
        self.assertFalse(
            self.hr_payslip_john.payslip_run_id, "There is already a payslip run!"
        )
        ...
        # I verify the payslip run is in close state.
        self.assertEqual(
            self.hr_payslip_john.payslip_run_id.state, "close", "State not changed!"
        )

        # I verify that the Accounting Entry is created.
        self.assertTrue(
            self.hr_payslip_john.move_id, "Accounting entry has not been created!"
        )
    ...
    def test_07_hr_payslip(self):
        ...
        self.assertFalse(
            self.hr_payslip_john.move_id, "Accounting entry has been created!"
        )
