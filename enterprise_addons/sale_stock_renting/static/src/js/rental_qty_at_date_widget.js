odoo.define("sale_stock_renting.QtyAtDateWidget", function (require) {
  ...
  QtyAtDateWidget.include({
    ...
    async _onRentalGanttView(ev) {
      ev.stopPropagation();
      const action = await data_manager.load_action(
      ...
      action.domain = [["product_id", "=", this.data.product_id.data.id]];
    ...
    _getContent() {
      ...
      this.data.end_date = this.data.return_date
        .clone()
        .add(this.getSession().getTZOffset(this.data.return_date), "minutes")
        .format(time.getLangDateFormat());
      this.data.start_date = this.data.pickup_date
        .clone()
        .add(this.getSession().getTZOffset(this.data.pickup_date), "minutes")
        .format(time.getLangDateFormat());
      const $content = $(
        QWeb.render("sale_stock_renting.QtyDetailPopOver", {
          data: this.data,
        })
      );
