...
class Product(models.Model):
        ...
        if self.env.context.get("warehouse_id", False):
            return super(Product, self)._get_qty_in_rent_domain() + [
                (
                    ...
                    "=",
                    int(self.env.context.get("warehouse_id")),
                )
            ]
        else:
            return super(Product, self)._get_qty_in_rent_domain()
    ...
    def _get_unavailable_qty(self, fro, to=None, **kwargs):
        ...
        qty_always_in_rent_during_period = (
            sum(covers_period.mapped(unavailable_qty)) if covers_period else 0
        )
    ...
    def _get_active_rental_lines(self, fro, to, **kwargs):
        ...
        if not to or fro == to:
            active_lines_at_time_fro = Reservation.search(
                domain + [("reservation_begin", "<=", fro), ("return_date", ">=", fro)]
            )
            return [], [], active_lines_at_time_fro
        else:
            begins_during_period = Reservation.search(
                domain
                + [("reservation_begin", ">", fro), ("reservation_begin", "<", to)]
            )
            ends_during_period = Reservation.search(
                domain
                + [
                    ("return_date", ">", fro),
                    ("return_date", "<", to),
            ...
            covers_period = Reservation.search(
                domain + [("reservation_begin", "<=", fro), ("return_date", ">=", to)]
            )
    ...
    def _get_unavailable_lots(self, fro=fields.Datetime.now(), to=None, **kwargs):
        ...
        return (begins_during_period + ends_during_period + covers_period).mapped(
            "unavailable_lot_ids"
        )
