...
class RentalOrderLine(models.Model):
    ...
    def _compute_qty_at_date(self):
        ...
        for line in self - non_rental:
            ...
            if (
                ...
                and line.pickup_date
                and line.return_date
            ):
                (
                    reservation_begin,
                    reservation_end,
                ) = line.product_id._unavailability_period(
                    line.pickup_date, line.return_date
                )
                ...
                virtual_available_at_date = max(
                    rentable_qty - rented_qty_during_period, 0
                )
    ...
    def write(self, vals):
        ...
        for sol in movable_confirmed_rental_lines:
            old_vals[sol.id] = (
                (sol.pickedup_lot_ids, sol.returned_lot_ids)
                if sol.product_id.tracking == "serial"
                else (sol.qty_delivered, sol.qty_returned)
            )
            if (
                vals.get("pickedup_lot_ids", False)
                and vals["pickedup_lot_ids"][0][0] == 6
            ):
                pickedup_lot_ids = vals["pickedup_lot_ids"][0][2]
                if (
                    sol.product_uom_qty == len(pickedup_lot_ids)
                    and pickedup_lot_ids != sol.reserved_lot_ids.ids
                ):
        ...
        movable_confirmed_rental_lines.mapped("company_id").filtered(
            lambda company: not company.rental_loc_id
        ...
        for sol in movable_confirmed_rental_lines:
            ...
            if sol.product_id.tracking == "serial" and (
                vals.get("pickedup_lot_ids", False)
                or vals.get("returned_lot_ids", False)
            ):
                ...
                if vals.get("pickedup_lot_ids", False):
                    ...
                    sol._return_serials(
                        removed_pickedup_lots, rented_location, stock_location
                    )
                if vals.get("returned_lot_ids", False):
                    ...
                    sol._return_serials(
                        removed_returned_lots, stock_location, rented_location
                    )
            elif sol.product_id.tracking != "serial" and (
                vals.get("qty_delivered", False) or vals.get("qty_returned", False)
            ):
                ...
                if qty_delivered_change > 0:
                    ...
                    sol._return_qty(
                        -qty_delivered_change, stock_location, rented_location
                    )
                ...
                if qty_returned_change > 0.0:
                    ...
                    sol._return_qty(
                        -qty_returned_change, rented_location, stock_location
                    )
    ...
    def _move_serials(self, lot_ids, location_id, location_dest_id):
        ...
        for lot_id in lot_ids:
            lot_quant = self.env["stock.quant"]._gather(
                self.product_id, location_id, lot_id
            )
            lot_quant = lot_quant.filtered(lambda quant: quant.quantity == 1.0)
            if not lot_quant:
                raise ValidationError(
                    _(
                        "No valid quant has been found in location %s for serial number %s !"
                    )
            ...
            ml = self.env["stock.move.line"].create(
                rental_stock_move._prepare_move_line_vals(reserved_quant=lot_quant)
            )
            ml["qty_done"] = 1
    ...
    def _stock_consistency(self):
        for line in self.filtered("is_rental"):
            ...
            if moves and moves.mapped("product_id") != line.product_id:
                raise ValidationError(
                    "You cannot change the product of lines linked to stock moves."
                )
    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        ...
        super(RentalOrderLine, other_lines)._action_launch_stock_rule(
    ...
    def _compute_unavailable_lots(self):
        ...
        for line in self:
            line.unavailable_lot_ids = (
                line.reserved_lot_ids | line.pickedup_lot_ids
            ) - line.returned_lot_ids
