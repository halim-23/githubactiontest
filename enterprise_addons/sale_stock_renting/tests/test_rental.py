...
class TestRentalCommon(common.SingleTransactionCase):
        ...
        cls.product_id = cls.env["product.product"].create(
            {
                ...
                ).id,  # remove category if possible?
        ...
        quants = cls.env["stock.quant"].create(
            {
                ...
                "location_id": cls.env["sale.order"]
    ...
    def test_rental_product_flow(self):
        ...
        self.order_line_id1.write({"product_uom_qty": 3})
        ...
        self.assertEqual(self.product_id.qty_available, 1)

        """ In company stock valuation """
        self.assertEqual(self.product_id.quantity_svl, 4)

        """
            Partial Return
        """

        self.order_line_id1.write({"qty_returned": 2})
        ...
        """ In company warehouses """
        self.assertEqual(self.product_id.qty_available, 3)
        ...
        self.order_line_id1.write({"qty_returned": 3})

        self.assertEqual(self.product_id.qty_available, 4.0)
    ...
    def test_rental_lot_flow(self):
        ...
        rentable_lots = self.env["stock.production.lot"]._get_available_lots(
            self.tracked_product_id
        )
        self.assertEqual(
            set(lots.ids), set(rentable_lots.ids)
        )  # set is here to ensure that order wont break test
        ...
        self.assertEqual(
            self.order_line_id2.reserved_lot_ids, self.order_line_id2.pickedup_lot_ids
        )
    ...
    def test_schedule_report(self):
        ...
        self.assertEqual(
            self.env["sale.rental.schedule"].search_count(
