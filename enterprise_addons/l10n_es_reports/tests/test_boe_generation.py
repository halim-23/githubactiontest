...
class TestBOEGeneration(TestAccountReportsCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        super(TestBOEGeneration, cls).setUpClass(
        ...
        base_tags = (
            cls.env.ref("l10n_es.mod_111_02")
            + cls.env.ref("l10n_es.mod_115_02")
            + cls.env.ref("l10n_es.mod_303_01")
        )
        tax_tags = (
            cls.env.ref("l10n_es.mod_111_03")
            + cls.env.ref("l10n_es.mod_115_03")
            + cls.env.ref("l10n_es.mod_303_03")
        )
    ...
    def _check_boe_export(self, report, options):
        ...
        self.assertEqual(
            "l10n_es_reports.aeat.boe.mod%s.export.wizard"
            % report.l10n_es_reports_modelo_number,
            wizard_action["res_model"],
            "Wrong BOE export wizard returned",
        )
    ...
    def _check_boe_111_to_303(self, modelo_number):
        ...
        options["sorted_groupby_keys"] = [
            (0,)
        ]  # Artificially set here; 'in real life', always set by _get_table when opening the report
    ...
    def test_boe_mod_347(self):
        ...
        options = self._init_options(
            report,
            fields.Date.from_string("2020-01-01"),
            fields.Date.from_string("2020-12-31"),
        )
        if "multi_company" in options:
            del options["multi_company"]
