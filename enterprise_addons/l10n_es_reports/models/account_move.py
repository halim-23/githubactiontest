...
class AccountMove(models.Model):
    ...
    def _auto_init(self):
        if not column_exists(
            self.env.cr, "account_move", "l10n_es_reports_mod349_available"
        ):
            create_column(
                self.env.cr, "account_move", "l10n_es_reports_mod349_available", "bool"
            )
            mod349_countries = self.env.ref("base.europe").country_ids - self.env.ref(
                "base.es"
            )
            if mod349_countries:
                self.env.cr.execute(
    ...
    def _mod_349_selection_values(self):
        ...
        if context.get("move_type") in self.get_sale_types():
            return [
                ...
                (
                    "H",
                    _("H - Supply without taxes delivered by a legal representative"),
                ),
            ]
    ...
    l10n_es_reports_mod347_invoice_type = fields.Selection(
        ...
        selection=[
            ...
            ("insurance", "Insurance operation"),
        ],
        default="regular",
        help="Defines the category into which this invoice falls for mod 347 report.",
    )
    l10n_es_reports_mod347_available = fields.Boolean(
        string="Available for Mod347",
        compute="_compute_l10n_es_reports_mod347_available",
    ...
    l10n_es_reports_mod349_invoice_type = fields.Selection(
        string="Type for mod 349",
        selection="_mod_349_selection_values",
        help="Defines the category into which this invoice falls for mod 349 report",
        default=_default_mod_349_invoice_type,
    )
    l10n_es_reports_mod349_available = fields.Boolean(
        string="Available for Mod349",
        store=True,
        compute="_compute_l10n_es_reports_mod349_available",
    ...
    def _compute_l10n_es_reports_mod347_available(self):
        for record in self:
            ...
            record.l10n_es_reports_mod347_available = (
                record.company_id.country_id.code == "ES"
            )
    ...
    def _compute_l10n_es_reports_mod349_available(self):
        ...
        mod349_countries = self.env.ref("base.europe").country_ids.filtered_domain(
            [("code", "!=", "ES")]
        )
        for record in self:
            record.l10n_es_reports_mod349_available = (
                record.partner_id.country_id in mod349_countries
            )
    ...
    def _onchange_partner_id_set_347_invoice_type(self):
        ...
        for record in self:
            ...
            record.l10n_es_reports_mod347_invoice_type = (
                False if record.partner_id.country_id in eu_countries else "regular"
            )
    ...
    def _post(self, soft=True):
        ...
        for record in posted.filtered(lambda move: move.is_invoice()):
            if (
                record.company_id.chart_template_id in spanish_coa_list
                and record.partner_id.country_id.code == "ES"
                and (
                    not record.l10n_es_reports_mod347_invoice_type
                    or (
                        record.l10n_es_reports_mod349_available
                        and not record.l10n_es_reports_mod349_invoice_type
                    )
                )
            ):
                raise UserError(
                    _("Please select a Spanish invoice type for this invoice.")
                )
        return posted
    ...
    def create(self, vals):
        ...
        if not res.l10n_es_reports_mod349_invoice_type:
            res.l10n_es_reports_mod349_invoice_type = res.with_context(
                move_type=res.move_type
            )._default_mod_349_invoice_type()
        return res
