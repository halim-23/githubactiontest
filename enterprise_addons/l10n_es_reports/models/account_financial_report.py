...
class AccountFinancialReportLine(models.Model):
    ...
    l10n_es_mod347_threshold = fields.Float(
        "Mod.347 Partner Threshold",
        help="""
...
This feature is only supported/useful in spanish MOD347 report.""",
    ...
    def _parse_threshold_parameter(self, date):
        ...
        if self.l10n_es_mod347_threshold:
            threshold_currency = self.env["res.currency"].search(
                [("name", "=", "EUR")], limit=1
            )
            ...
            if not threshold_currency or not threshold_currency.active:
                raise UserError(
                    _(
                        "Currency %s, used for a threshold in this report, is either nonexistent or inactive. Please create or activate it.",
                        threshold_currency.name,
                    )
                )

            company_currency = self.env.company.currency_id
            return threshold_currency._convert(
                self.l10n_es_mod347_threshold, company_currency, self.env.company, date
            )
    ...
    def _get_options_with_threshold(self, options_list):
        ...
        for i, options in enumerate(options_list):
            from_fy_dates = self.env.company.compute_fiscalyear_dates(
                fields.Date.from_string(options["date"]["date_from"])
            )
            to_fy_dates = self.env.company.compute_fiscalyear_dates(
            ...
            if ids_to_exclude:
                new_options_list.append(
                    {
                        **options,
                        "l10n_es_mod347_add_domain": [
                            (self.groupby, "not in", tuple(ids_to_exclude))
                        ],
                    }
                )
            else:
    ...
    def _compute_amls_results(self, options_list, calling_financial_report, sign=1):
        ...
        return super()._compute_amls_results(
            options_list, calling_financial_report, sign=sign
        )
