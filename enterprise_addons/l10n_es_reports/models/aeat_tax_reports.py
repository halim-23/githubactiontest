...
class AEATAccountFinancialReport(models.Model):
    ...
    l10n_es_reports_modelo_number = fields.Char(
        string="Spanish Modelo Number",
        help="The modelo number of this report. Non-Spanish (or non-modelo) reports must leave this field to None.",
    )
    ...
    def _get_options(self, previous_options=None):
        ...
        if self.l10n_es_reports_modelo_number:
            ...
            if (
                aeat_wizard_id and aeat_modelo
            ):  # If we do have these, it means an AEAT wizard was used to generate this report
                aeat_wizard = self.env[
                    "l10n_es_reports.mod" + aeat_modelo + ".wizard"
                ].browse(aeat_wizard_id)
                ...
                casilla_fields = [
                    x for x in dir(aeat_wizard) if x.startswith(casilla_prefix)
                ]
                context_line_values = {}
                for attr in casilla_fields:
                    ...
                    line_code = (
                        ...
                        + aeat_wizard._modelo
                        + "_"
                        + attr.replace(self.CASILLA_FIELD_PREFIX, "")
                    )
                    context_line_values[line_code] = attr_value

                self = self.with_context(
                    financial_report_line_values=context_line_values
                )
    ...
    def print_boe(self, options):
        ...
        if boe_wizard_model != None:
            ...
            view_id = self.env.ref(
                "l10n_es_reports.mod"
                + self.l10n_es_reports_modelo_number
                + "_boe_wizard"
            ).id
            context = self.env.context.copy()
            context.update(
                {
                    "l10n_es_reports_report_options": {
                        **options,
                        "l10n_es_generation_context": self.env.context,
                    }
                }
            )
            return {
                ...
                "view_mode": "form",
                "views": [[view_id, "form"]],
                "res_model": "l10n_es_reports.aeat.boe.mod"
                + self.l10n_es_reports_modelo_number
                + ".export.wizard",
    ...
    def _get_mod_period_and_year(self, options):
        ...
        if date_from.day != 1 or date_to.day != (date_to + relativedelta(day=31)).day:
            raise UserError(
                _(
                    "Your date range does not cover entire months, please use a start and end date matching respectively the first and last day of a month."
                )
            )
        ...
        rslt_year = str(
            date_from.year
        )  # Identical to date_to.year thanks to the previous conditions
        if date_from.month == date_to.month:
            rslt_period = "%02d" % (date_from.month)
        elif (
            date_from.month == date_to.month - 2
            and self._retrieve_period_and_year(date_from, trimester=True)[0]
            == self._retrieve_period_and_year(date_to, trimester=True)[0]
        ):
    ...
    def _convert_period_to_dates(self, period, year):
        ...
        if period[-1] == "T":
            ...
            return datetime(day=1, month=1 + (quarter - 1) * 3, year=int(year)), (
                datetime(day=1, month=quarter * 3, year=int(year))
                + relativedelta(day=31)
            )  # relativedelta used to force last day of the month without triggering ValueError for months with less than 31 days
        else:
            return datetime(day=1, month=int(period), year=int(year)), datetime(
                day=1, month=int(period), year=int(year)
            ) + relativedelta(day=31)
    ...
    def get_txt(self, options):
        ...
        if options.get("multi_company", False):
            raise UserError(
                _(
                    "Cannot generate a BOE file for multiple companies at once. Please select only one."
                )
            )
        ...
        return getattr(self, "_boe_export_mod" + self.l10n_es_reports_modelo_number)(
            options, current_company, period, year
        )
    ...
    def _boe_format_string(self, string, length=-1, align="left", fill_char=b" "):
        ...
        for char in unicodedata.normalize(
            "NFKC", string
        ):  # We use a normalized version of the string here so that we are sure accentuated charcaters are each time encoded with only one character (and not a regular character followed by a combining one)
            if not char in ("Ñ", "Ç"):
                normalized_char = unicodedata.normalize(
                    "NFD", char
                )  # Not NFKD, as the NFKC normalization in the loop already ensures good treatment of compatibility characters. NFD splits accentuated characters in two parts: the original character, and the accent to combine it with
                rslt += normalized_char.encode(
                    "ISO-8859-1", "ignore"
                )  # Combinable accentuation characters are not supported by this encoding, and disappear when transcoding.
            else:
    ...
    def _boe_format_number(
        ...
        if in_currency:
            ...
            conversion_date = self.env.context[
                "l10n_es_reports_boe_conversion_date"
            ]  # This context key is set in _set_context
            curr_eur = self.env["res.currency"].search([("name", "=", "EUR")], limit=1)
            number = company.currency_id._convert(
                number, curr_eur, company, conversion_date
            )
        ...
        negative_amount = (
            in_currency
            and company.currency_id.compare_amounts(number, 0.0) == -1
            or number < 0
        )
        ...
        return self._boe_format_string(sign_str) + self._boe_format_string(
            str_number, length=length - len(sign_str), align="right", fill_char=b"0"
        )
    ...
    def _retrieve_casilla_lines(self, report_lines):
        ...
        for line in report_lines:
            ...
            if matcher:
                ...
                casilla_value = line["columns"][0][
                    "no_format"
                ]  # Element [0] is the current period, in case we are comparing
    ...
    def _retrieve_report_line(self, options, xmlid):
        ...
        line_id = self._get_generic_line_id(
            "account.financial.html.report.line", self.env.ref(xmlid).id
        )
    ...
    def _retrieve_boe_manual_wizard(self, options):
        ...
        return self.env[
            ...
            + self.l10n_es_reports_modelo_number
            + ".export.wizard"
        ].browse(options["l10n_es_reports_boe_wizard_id"])

    def _call_on_sublines(
        self, report_options, line_xml_id, fun_to_call, required_ids_set=set()
    ):
        ...
        report_line_id = self._get_generic_line_id(
            "account.financial.html.report.line", report_line.id
        )
        for generated_line in self._get_lines(report_options, line_id=report_line_id):
            ...
            if (
                parsed_generated_id[-1][0] != "total"
                and generated_line["level"] >= report_line.level
                and generated_line.get("caret_options") == "partner_id"
            ):
                rslt += fun_to_call(
                    {
                        "line_data": generated_line,
        ...
        for (
            ...
            required_ids_set
        ):  # These elements are the ones for wich no line was generated, but that were into the original required ids set. So, we still treat them.
            rslt += fun_to_call(
                {
                    "line_data": {"id": element},
                    "line_xml_id": line_xml_id,
                    "report_options": report_options,
                }
            )
    ...
    def _get_subline_data(self, report_options, line_xml_id, sub_line_id):
        ...
        for generated_line in self._get_lines(report_options, line_id=report_line_id):
            ...
            if (
                ...
                and generated_line["level"] >= report_line.level
                and generated_line.get("caret_options") == "partner_id"
                and generated_line["id"] == sub_line_id
            ):
    ...
    def _extract_tin(self, partner, error_if_no_tin=True):
        if not partner.vat:
            if error_if_no_tin:
                raise UserError(
                    _("No TIN set for partner %s (id %d). Please define one.")
    ...
    def _generate_111_115_common_header(self, options, current_company, period, year):
        ...
        rslt += self._boe_format_string(
            "<T" + self.l10n_es_reports_modelo_number + "01000>"
        )
        rslt += self._boe_format_string(" ")
        ...
        rslt += self._boe_format_string(
            " " * 20
        )  # We keep the name of the declaring party blank here, as it is a company
    ...
    def _boe_export_mod111(self, options, current_company, period, year):
        if not period:
            raise UserError(
                _(
                    "Wrong report dates for BOE generation : please select a range of one month or a trimester."
                )
            )

        rslt = self._generate_111_115_common_header(
            options, current_company, period, year
        )
        ...
        rslt += self._boe_format_number(
            casilla_lines_map["02"],
            length=17,
            decimal_places=2,
            signed=True,
            in_currency=True,
        ...
        rslt += self._boe_format_string(
            boe_wizard.complementary_declaration and "X" or " "
        )
        rslt += self._boe_format_string(
            boe_wizard.complementary_declaration
            and boe_wizard.previous_report_number
            or "",
            length=13,
        )
    ...
    def _boe_export_mod303(self, options, current_company, period, year):
        ...
        rslt += self._generate_mod_303_page1(
            options, current_company, period, year, casilla_lines_map
        )
        rslt += self._generate_mod_303_page3(
            options, current_company, period, year, casilla_lines_map
        )
    ...
    def _generate_mod_303_page1(
        self, options, current_company, period, year, casilla_lines_map
    ):
        # Wizard with manually-entered data
        boe_wizard = self._retrieve_boe_manual_wizard(options)
        ...
        rslt += self._boe_format_number(
            2
        )  # Tributación exclusivamente foral => Always "no", for simplicity
        ...
        exonerated_from_mod_390 = boe_wizard._get_exonerated_from_mod_390_2021_value(
            period
        )
        ...
        if exonerated_from_mod_390 == 1:
            profit_and_loss_report = self.env.ref(
                "l10n_es_reports.financial_report_es_profit_and_loss"
            )
            ...
            transactions_volume_options["sorted_groupby_keys"] = (
                options["sorted_groupby_keys"],
            )
            transactions_volume_line = self.env.ref(
                "l10n_es_reports.es_profit_and_loss_line_1"
            )
            transactions_volume_vals = profit_and_loss_report._get_lines(
                transactions_volume_options,
                line_id=self._build_line_id(
                    [("", transactions_volume_line._name, transactions_volume_line.id)]
                ),
            )
            annual_volume_indicator = (
                current_company.currency_id.is_zero(
                    transactions_volume_vals[0]["columns"][0]["no_format"]
                )
                and 2
                or 1
            )
        ...
        # Casillas
        rslt += self._boe_format_number(
            casilla_lines_map["01"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(400, length=5)  # Casilla 02 is constant
        rslt += self._boe_format_number(
            casilla_lines_map["03"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["04"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(1000, length=5)  # Casilla 05 is constant
        rslt += self._boe_format_number(
            casilla_lines_map["06"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["07"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(2100, length=5)  # Casilla 08 is constant
        rslt += self._boe_format_number(
            casilla_lines_map["09"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["10"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["11"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["12"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["13"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["14"],
            length=17,
        ...
        rslt += self._boe_format_number(
            casilla_lines_map["16"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(50, length=5)  # Casilla 17 is constant
        rslt += self._boe_format_number(
            casilla_lines_map["18"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["19"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(140, length=5)  # Casilla 20 is constant
        rslt += self._boe_format_number(
            casilla_lines_map["21"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["22"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(520, length=5)  # Casilla 23 is constant
        rslt += self._boe_format_number(
            casilla_lines_map["24"], length=17, decimal_places=2, in_currency=True
        )
        rslt += self._boe_format_number(
            casilla_lines_map["25"],
            length=17,
            decimal_places=2,
            signed=True,
            in_currency=True,
    ...
    def _generate_mod_303_page3(
        ...
        if options["date"]["date_from"] < "2022-01-01":
            ...
            if "61" not in casilla_lines_map:
                ...
                tables, where_clause, where_params = self._query_get(
                    options, [("tax_tag_ids", "in", tag_61.ids)]
                )
                query = (
                    """
        ...
        rslt += self._boe_format_number(0, length=17)
        rslt += self._boe_format_number(
            casilla_lines_map["46"],
            length=17,
            decimal_places=2,
            signed=True,
            in_currency=True,
        )  # Should normally be casilla 64 (= sum of casillas 46, 58 and 76), but only casilla 46 is in our version of the report
        rslt += self._boe_format_number(
            casilla_lines_map["65"], length=5, decimal_places=2
        )
        rslt += self._boe_format_number(
            casilla_lines_map["66"],
            length=17,
            decimal_places=2,
            signed=True,
            in_currency=True,
        ...
        rslt += self._boe_format_number(
            casilla_lines_map.get("110", 0),
            length=17,
            decimal_places=2,
            in_currency=True,
        )
        rslt += self._boe_format_number(
            casilla_lines_map.get("78", 0),
            length=17,
            decimal_places=2,
            in_currency=True,
        )
        rslt += self._boe_format_number(
            casilla_lines_map.get("87", 0),
            length=17,
            decimal_places=2,
            in_currency=True,
        )
        rslt += self._boe_format_number(
            casilla_lines_map["68"],
            length=17,
            decimal_places=2,
            signed=True,
            in_currency=True,
        ...
        gov_giving_back = (
            current_company.currency_id.compare_amounts(casilla_lines_map["71"], 0)
            == -1
        )
        ...
        rslt += self._boe_format_string(
            bic if gov_giving_back and iban and iban[:2] != "ES" else "", length=11
        )
        rslt += self._boe_format_string(iban, length=34)
        ...
        if gov_giving_back:
            ...
            rslt += self._boe_format_string(
                " ".join([bank.street or "", bank.street2 or ""]), length=35
            )
            ...
            if iban and boe_wizard.declaration_type != "N":
                ...
                if iban_country_code == "ES":
                    marca = "1"
                elif iban_country_code in self.env.ref("base.sepa_zone").mapped(
                    "country_ids.code"
                ):
                    marca = "2"
                else:
    ...
    def _boe_export_mod347(self, options, current_company, period, year):
        ...
        rslt = self._mod347_write_type2_header_record(
            current_company, boe_wizard, boe_report_options
        )
        seguros_required_b = self._mod347_get_required_partner_ids_for_boe(
            "insurance", year + "-01-01", year + "-12-31", boe_wizard, "B", "seguros"
        )
        rslt += self._call_on_sublines(
            boe_report_options,
        ...
        otras_required_a = self._mod347_get_required_partner_ids_for_boe(
            "regular", year + "-01-01", year + "-12-31", boe_wizard, "A", "otras"
        )
        rslt += self._call_on_sublines(
            boe_report_options,
        ...
        otras_required_b = self._mod347_get_required_partner_ids_for_boe(
            "regular", year + "-01-01", year + "-12-31", boe_wizard, "B", "otras"
        )
        rslt += self._call_on_sublines(
            boe_report_options,
    ...
    def _mod347_get_required_partner_ids_for_boe(
        self,
        mod_invoice_type,
        date_from,
        date_to,
        ...
        cash_basis_manual_data = boe_wizard.cash_basis_mod347_data.filtered(
            lambda x: x.operation_key == operation_key
            and x.operation_class == operation_class
        )
        ...
        if operation_key == "A":  # Only for perceived amounts
            ...
            cash_payments_aml = self.env["account.partial.reconcile"].search(
                [
                    ("credit_move_id.date", "<=", date_to),
                    ("credit_move_id.date", ">=", date_from),
                    ("credit_move_id.journal_id.type", "=", "cash"),
                    (
                        "debit_move_id.move_id.l10n_es_reports_mod347_invoice_type",
                        "=",
                        mod_invoice_type,
                    ),
                    (
                        "credit_move_id.account_id.user_type_id",
                        "=",
    ...
    def _mod347_write_type2_header_record(
        self, current_company, boe_wizard, boe_report_options
    ):
        ...
        rslt += self._boe_format_string(
            boe_wizard.get_formatted_contact_phone(), length=9
        )
        rslt += self._boe_format_string(boe_wizard.contact_person_name, length=40)
        mod_347_boe_sequence = self.env["ir.sequence"].search(
            [
                ("company_id", "=", current_company.id),
                ("code", "=", "l10n_es.boe.mod_347"),
            ]
        )
        rslt += self._boe_format_number(347) + self._boe_format_string(
            mod_347_boe_sequence.next_by_id(), length=10
        )
        rslt += self._boe_format_string(
            boe_wizard.complementary_declaration and "X" or " "
        )
        rslt += self._boe_format_string(
            boe_wizard.substitutive_declaration and "X" or " "
        )
        rslt += self._boe_format_string(
            boe_wizard.previous_report_number or "",
            length=13,
            fill_char=b"0",
            align="right",
        )

        declarados_count_line_id = self._get_generic_line_id(
            "account.financial.html.report.line",
            self.env.ref("l10n_es_reports.mod_347_statistics_operations_count").id,
        )
        declarados_count_line_data = self._get_lines(
            boe_report_options, line_id=declarados_count_line_id
        )[0]
        rslt += self._boe_format_number(
            sum(i["no_format"] for i in declarados_count_line_data["columns"]), length=9
        )

        declarados_total_line_id = self._get_generic_line_id(
            "account.financial.html.report.line",
            self.env.ref("l10n_es_reports.mod_347_operations_title").id,
        )
        declarados_total_line_data = self._get_lines(
            boe_report_options, line_id=declarados_total_line_id
        )[
            -1
        ]  # Index -1 to get the line containing the total
        declarados_total = sum(
            i["no_format"] for i in declarados_total_line_data["columns"]
        )
        rslt += self._boe_format_number(
            declarados_total,
            length=16,
        ...
        real_estates_data = self._mod347_get_real_estates_data(
            boe_report_options, current_company.currency_id
        )
        rslt += self._boe_format_number(real_estates_data["count"], length=9)

        rslt += self._boe_format_number(
            real_estates_data["total"],
            length=15,
            decimal_places=2,
            signed=True,
            sign_pos=" ",
        ...
        rslt += self._boe_format_string(
            " " * 9
        )  # TIN of the legal representant; blank if 14 years or older
        rslt += self._boe_format_string(" " * 88)
        rslt += self._boe_format_string(
            " " * 13
        )  # "Sello Electronico" => for administration
        rslt += b"\r\n"
    ...
    def _mod347_write_type2_partner_record(
        self,
        report_data,
        year,
        current_company,
        ...
        line_partner = self.env["res.partner"].browse(
            self._get_caret_option_target_id(report_data["line_data"]["id"])
        )
        ...
        rslt += self._boe_format_string(
            line_partner.country_id.code == "ES"
            and self._extract_spanish_tin(line_partner)
            or "",
            length=9,
        )
        ...
        province_code = (
            line_partner.state_id
            and SPANISH_PROVINCES_REPORT_CODES.get(line_partner.state_id.code)
            or "99"
        )
        ...
        if province_code == "99" and (
            not line_partner.country_id or not line_partner.country_id.code
        ):
            raise UserError(
                _(
                    "Partner with %s (id %d) is not associated to any Spanish province, and should hence have a country code. For this, fill in its 'country' field."
                )
                % (line_partner.name, line_partner.id)
            )
        ...
        year_operations_sum = currency_id.round(
            sum(i["no_format"] for i in report_data["line_data"].get("columns", []))
        )
        rslt += self._boe_format_number(
            year_operations_sum,
            length=16,
        ...
        for domain_tuple in evaluated_domain:
            if (
                domain_tuple[0] == "move_id.l10n_es_reports_mod347_invoice_type"
                and domain_tuple[1] == "="
            ):
        ...
        user_type_id = (
            operation_key == "A"
            and self.env.ref("account.data_account_type_receivable").id
            or self.env.ref("account.data_account_type_payable").id
        )
        matching_field = operation_key == "A" and "debit" or "credit"
        cash_payments_lines_in_period = self.env["account.move.line"].search(
            [
                ("date", "<=", year + "-12-31"),
                ("date", ">=", year + "-01-01"),
        ...
        for cash_payment_aml in cash_payments_lines_in_period:
            partial_reconcile_ids = getattr(
                cash_payment_aml, "matched_" + matching_field + "_ids"
            )
            partial_rec_on_inv_type = partial_reconcile_ids.filtered(
                lambda x: getattr(
                    x, matching_field + "_move_id"
                ).move_id.l10n_es_reports_mod347_invoice_type
        ...
        curr_eur = self.env["res.currency"].search([("name", "=", "EUR")], limit=1)
        threshold = curr_eur._convert(
            6000,
            currency_id,
            current_company,
            self.env.context["l10n_es_reports_boe_conversion_date"],
        )
        if (
            currency_id.compare_amounts(metalico_amount, threshold) == 1
        ):  # We only must report this amount if it is above 6000 €
            rslt += self._boe_format_number(
                metalico_amount, length=15, decimal_places=2, in_currency=True
            )
        else:
        ...
        operation_class = (
            insurance and "seguros" or local_negocio and "local_negocio" or "otras"
        )
        ...
        for trimester in range(1, 5):
            ...
            real_estates_vat_partner_dict = manual_parameters_map.get(
                "real_estates_vat", {}
            ).get(line_partner.id)
            real_estates_vat_amount = (
                real_estates_vat_partner_dict
                and real_estates_vat_partner_dict[str(trimester)][operation_class][
                    operation_key
                ]
                or 0
            )
        ...
        for trimester in range(1, 4):
            trimester_total = (
                report_data["line_data"]
                .get("columns", [{} for i in range(1, 4)])[-trimester]
                .get("no_format", 0)
            )
            rslt += self._boe_format_number(
                trimester_total,
            ...
            rslt += self._boe_format_number(
                real_estates_vat_by_trimester[trimester],
                length=16,
                decimal_places=2,
                signed=True,
                sign_pos=" ",
        ...
        if line_partner.country_id in europe_countries:
            ...
            intracom_tin = (
                (partner_tin[:2] != "es") and partner_tin or ""
            )  # We write an empty string if the partner has a Spanish TIN (because it then has already been written previously)
        ...
        cash_basis_data = (
            cash_basis_partner
            and cash_basis_partner[operation_class][operation_key]
            or None
        )
        rslt += self._boe_format_string(cash_basis_data != None and "X" or " ")

        rslt += self._boe_format_string(
            line_partner == current_company.partner_id and "X" or " "
        )

        rslt += self._boe_format_string(
            " "
        )  # Not supported by Odoo; according to the partners, too few people need this option

        rslt += self._boe_format_number(
            cash_basis_data or 0,
            length=16,
            decimal_places=2,
            signed=True,
            sign_pos=" ",
    ...
    def _boe_export_mod349(self, options, current_company, period, year):
        ...
        if boe_wizard.trimester_2months_report:
            if period[-1] == "T":
                ...
                options["date"]["date_to"] = (
                    end_date + relativedelta(day=31, months=-1)
                ).strftime("%Y-%m-%d")
            else:
                raise UserError(
                    _(
                        "You cannot generate a BOE file for the first two months of a trimester if only one month is selected!"
                    )
                )

        # Header
        rslt = self._mod_349_write_type1_header_record(
            options, period, year, current_company, boe_wizard
        )

        # Invoices lines
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_supplies",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "E", current_company
            ),
        )
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_acquisitions",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "A", current_company
            ),
        )
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_triangular",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "T", current_company
            ),
        )
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_services_sold",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "S", current_company
            ),
        )
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_services_acquired",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "I", current_company
            ),
        )
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_supplies_without_taxes",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "M", current_company
            ),
        )
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_supplies_without_taxes_legal_representative",
            lambda report_data: self._mod_349_write_type2_invoice_record(
                report_data, year, "H", current_company
            ),
        )
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_supplies_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
                "E",
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_acquisitions_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_triangular_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_services_sold_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_services_acquired_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_supplies_without_taxes_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
        ...
        rslt += self._call_on_sublines(
            options,
            "l10n_es_reports.mod_349_supplies_without_taxes_legal_representative_refunds",
            lambda report_data: self._mod_349_write_type2_refund_records(
                options,
                report_data,
                current_company,
    ...
    def _mod_349_write_type1_header_record(
        self, options, period, year, current_company, boe_wizard
    ):
        ...
        rslt += self._boe_format_string(boe_wizard.contact_person_name, length=40)
        mod_349_boe_sequence = self.env["ir.sequence"].search(
            [
                ("company_id", "=", current_company.id),
                ("code", "=", "l10n_es.boe.mod_349"),
            ]
        )
        rslt += self._boe_format_number(349) + self._boe_format_string(
            mod_349_boe_sequence.next_by_id(), length=10
        )
        rslt += self._boe_format_string(
            boe_wizard.complementary_declaration and "X" or " "
        )
        ...
        rslt += self._boe_format_number(
            self._retrieve_report_line(
                options, "l10n_es_reports.mod_349_statistics_invoices_partners_count"
            ),
            length=9,
        )
        rslt += self._boe_format_number(
            self._retrieve_report_line(
                options, "l10n_es_reports.mod_349_statistics_invoices_total_amount"
            ),
            length=15,
            in_currency=True,
            decimal_places=2,
        ...
        rslt += self._boe_format_number(
            self._retrieve_report_line(
                options, "l10n_es_reports.mod_349_statistics_refunds_partners_count"
            ),
            length=9,
        )
        rslt += self._boe_format_number(
            self._retrieve_report_line(
                options, "l10n_es_reports.mod_349_statistics_refunds_total_amount"
            ),
            length=15,
            in_currency=True,
            decimal_places=2,
        ...
        rslt += self._boe_format_string(
            boe_wizard.trimester_2months_report and "X" or " "
        )
        rslt += self._boe_format_string(" " * 204)
        rslt += self._boe_format_string(
            " " * 9
        )  # TIN of the legal representative, if under 14 years old
        rslt += self._boe_format_string(" " * 101)  # Constant
        rslt += b"\r\n"
        return rslt

    def _mod_349_write_type2_invoice_record(
        self, report_data, year, key, current_company
    ):
        line_partner = self.env["res.partner"].browse(
            self._get_caret_option_target_id(report_data["line_data"]["id"])
        )
        ...
        rslt += self._boe_format_number(
            report_data["line_data"]["columns"][0]["no_format"],
            length=13,
            decimal_places=2,
            in_currency=True,
        )
    ...
    def _mod_349_write_type2_refund_records(
        self,
        options,
        report_data,
        current_company,
        mod_349_type,
        invoice_report_line_xml_id,
    ):
        line_partner = self.env["res.partner"].browse(
            self._get_caret_option_target_id(report_data["line_data"]["id"])
        )
        ...
        for refund_invoice in self.env["account.move"].search(
            ...
            if not original_invoice:
                raise UserError(
                    _(
                        "Refund Invoice %s was created without a link to the original invoice that was credited, "
                        "while we need that information for this report. "
                    )
                    % (refund_invoice.display_name,)
                )

            invoice_period, invoice_year = self._retrieve_period_and_year(
                original_invoice.date, trimester=report_period[-1] == "T"
            )
            (
                invoice_period,
                invoice_year,
                refund_invoice.l10n_es_reports_mod349_invoice_type,
            )

            # We compute the total refund for this invoice until the current period
            all_previous_refunds = self.env["account.move"].search(
                [
            ...
            line_date_from, line_date_to = self._convert_period_to_dates(
                invoice_period, invoice_year
            )
            line_options["date"]["date_from"] = datetime.strftime(
                line_date_from, "%Y-%m-%d"
            )
            line_options["date"]["date_to"] = datetime.strftime(
                line_date_to, "%Y-%m-%d"
            )

            subline_id = "financial_report_group_%s_%s" % (
                self.env.ref(invoice_report_line_xml_id).id,
                line_partner.id,
            )
            invoice_line_data = self._get_subline_data(
                line_options, invoice_report_line_xml_id, subline_id
            )
            ...
            rslt += self._boe_format_number(
                current_company.currency_id.round(
                    previous_report_amount - total_refund
                ),
                length=13,
                decimal_places=2,
                in_currency=True,
            ...
            rslt += self._boe_format_number(
                previous_report_amount, length=13, decimal_places=2, in_currency=True
            )
