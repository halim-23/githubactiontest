...
class AEATBOEExportWizard(models.TransientModel):
    ...
    calling_export_wizard_id = fields.Many2one(
        string="Calling Export Wizard",
        comodel_name="account_reports.export.wizard",
        help="Optional field containing the report export wizard calling this BOE wizard, if there is one.",
    )

    def download_boe_action(self):
        if (
            self.calling_export_wizard_id
            and not self.calling_export_wizard_id.l10n_es_reports_boe_wizard_model
        ):
...
class Mod111And115And303CommonBOEWizard(models.TransientModel):
    ...
    partner_bank_id = fields.Many2one(
        string="Direct Debit Account",
        comodel_name="res.partner.bank",
        help="The IBAN account number to use for direct debit. Leave blank if you don't use direct debit.",
        domain="[('partner_id','=',company_partner_id)]",
    )
    complementary_declaration = fields.Boolean(
        string="Complementary Declaration",
        help="Whether or not this BOE file is a complementary declaration.",
    )
    ...
    previous_report_number = fields.Char(
        ...
        size=13,
        help="Number of the report previously submitted",
    )
...
class Mod347And349CommonBOEWizard(models.TransientModel):
    ...
    contact_person_name = fields.Char(
        ...
        default=_default_contact_name,
        required=True,
        help="Name of the contact person fot this BOE file's submission",
    )
    contact_person_phone = fields.Char(
        string="Contact phone number",
        default=_default_contact_phone,
        help="Phone number where to join the contact person",
    )
    complementary_declaration = fields.Boolean(
        string="Complementary Declaration",
        help="Whether or not this BOE file corresponds to a complementary declaration",
    )
    substitutive_declaration = fields.Boolean(
        string="Substitutive Declaration",
        help="Whether or not this BOE file corresponds to a substitutive declaration",
    )
    previous_report_number = fields.Char(
        string="Previous Report Number",
        size=13,
        help="Number of the previous report, corrected or replaced by this one, if any",
    )
        ...
        for record in self:
            if not record.partner_bank_id.bank_bic:
                raise ValidationError(
                    _(
                        "Please first assign a BIC number to the bank related to this account."
                    )
                )
    ...
    def get_partners_manual_parameters_map(self):
        ...
        for data in self.cash_basis_mod347_data:
            if not cash_basis_dict.get(data.partner_id.id):
                cash_basis_dict[data.partner_id.id] = {
                    "local_negocio": {"A": None, "B": None},
                    "seguros": {"B": None},
                    "otras": {"A": None, "B": None},
                }

            cash_basis_dict[data.partner_id.id][data.operation_class][
                data.operation_key
            ] = data.perceived_amount
...
class Mod347BOEManuaPartnerData(models.TransientModel):
    ...
    parent_wizard_id = fields.Many2one(
        comodel_name="l10n_es_reports.aeat.boe.mod347.export.wizard"
    )
    ...
    )  # required by the monetary field
    ...
    operation_class = fields.Selection(
        selection=[
            ("local_negocio", "Arrendamiento Local Negocio"),
            ("seguros", "Operaciones de Seguros"),
            ("otras", "Otras operaciones"),
        ],
        required=True,
...
class Mod349BOEWizard(models.TransientModel):
    ...
    trimester_2months_report = fields.Boolean(
        string="Trimester monthly report",
        help="Whether or not this BOE file must be generated with the data of the first two months of the trimester (in case its total amount of operation is above the threshold fixed by the law)",
    )
