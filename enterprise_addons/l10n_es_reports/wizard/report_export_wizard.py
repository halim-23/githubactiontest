...
class ReportExportWizard(models.TransientModel):
    ...
    l10n_es_reports_boe_wizard_id = fields.Integer(
        help="Technical field storing the id of the related BOE generation wizard when exporting a Spanish report"
    )
    l10n_es_reports_boe_wizard_model = fields.Char(
        help="Technical field storing the model of the related BOE generation wizard when exporting a Spanish report"
    )

    def export_report(self):
        self.ensure_one()
        if (
            any(format.name == "BOE" for format in self.export_format_ids)
            and not self.l10n_es_reports_boe_wizard_model
        ):
            report = self._get_report_obj()
            boe_action = report.print_boe(
                self.env.context.get("account_report_generation_options")
            )
            ...
            if boe_action["type"] == "ir.actions.act_window":
                boe_wizard = self.env[boe_action["res_model"]].browse(
                    boe_action["res_id"]
                )
...
class ReportExportWizardOption(models.TransientModel):
    ...
    def apply_export(self, report_options):
        ...
        if self.name == "BOE":
            ...
            if (
                self.export_wizard_id.l10n_es_reports_boe_wizard_id
                and self.export_wizard_id.l10n_es_reports_boe_wizard_model
            ):
                boe_wizard = self.env[
                    self.export_wizard_id.l10n_es_reports_boe_wizard_model
                ].browse(self.export_wizard_id.l10n_es_reports_boe_wizard_id)
