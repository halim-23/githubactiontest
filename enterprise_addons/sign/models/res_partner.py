...
class ResPartner(models.Model):
    ...
    signature_count = fields.Integer(
        compute="_compute_signature_count", string="# Signatures"
    )
    ...
    def _compute_signature_count(self):
        ...
        signature_data_mapped = {
            data["partner_id"][0]: data["partner_id_count"] for data in signature_data
        }
        for partner in self:
    ...
    def write(self, vals):
        partners_email_changed = (
            self.filtered(lambda r: r.email != vals["email"])
            if "email" in vals
            else None
        )
