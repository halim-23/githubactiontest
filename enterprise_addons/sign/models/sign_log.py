...
class SignLog(models.Model):
    ...
    ip = fields.Char(
        "IP address of the visitor", required=True, groups="sign.group_sign_manager"
    )
    ...
    request_state = fields.Selection(
        ...
        required=True,
        string="State of the request on action log",
    ...
    def _get_or_check_hash(self, vals):
        ...
        domain = [
            ("sign_request_id", "=", vals["sign_request_id"]),
            ("action", "in", ["create", "sign"]),
        ]
        if "id" in vals:
        ...
        if not prev_activity:
            ...
            body = sign_request.template_id.with_context(
                bin_size=False
            ).attachment_id.datas
        else:
    ...
    def _compute_string_to_hash(self, vals):
        ...
        item_values = (
            self.env["sign.request.item.value"]
            .search([("sign_request_id", "=", vals["sign_request_id"])])
            .filtered(
    ...
    def _check_document_integrity(self):
        ...
        for log in logs:
            vals = {
                ...
                for key, value in log.read()[0].items()
            }
    ...
    def _update_vals_with_http_request(self, vals):
        ...
        if "geoip" in request.session and not (
            vals.get("latitude") and vals.get("longitude")
        ):
            vals.update(
                {
    ...
    def _create_log(self, record, action, is_request=False, **kwargs):
        ...
        vals = (
            Log._prepare_vals_from_request(record)
            if is_request
            else Log._prepare_vals_from_item(record)
        )
        vals["action"] = action
        vals.update(kwargs)
