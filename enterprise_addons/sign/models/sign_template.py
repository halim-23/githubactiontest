...
class SignTemplate(models.Model):
    ...
    sign_item_ids = fields.One2many(
        "sign.item", "template_id", string="Signature Items", copy=True
    )
    responsible_count = fields.Integer(
        compute="_compute_responsible_count", string="Responsible Count"
    )

    active = fields.Boolean(default=True, string="Active")
    privacy = fields.Selection(
        [("employee", "All Users"), ("invite", "On Invitation")],
        string="Who can Sign",
        default="invite",
    ...
    favorited_ids = fields.Many2many(
        ...
        default=lambda s: s._default_favorited_ids(),
    )
    ...
    sign_request_ids = fields.One2many(
        "sign.request", "template_id", string="Signature Requests"
    )
    ...
    def create(self, vals):
        if (
            "active" in vals
            and vals["active"]
            and not self.env.user.has_group("sign.group_sign_user")
        ):
    ...
    def _compute_responsible_count(self):
        for template in self:
            template.responsible_count = len(
                template.sign_item_ids.mapped("responsible_id")
            )

    def _compute_signed_in_progress_template(self):
        sign_requests = self.env["sign.request"].read_group(
            [("state", "!=", "canceled")],
            ["state", "template_id"],
            ["state", "template_id"],
            lazy=False,
    ...
    def get_empty_list_help(self, help):
        if not self.env.ref(
            "sign.template_sign_tour", raise_if_not_found=False
        ) or not self.env.user.has_group("sign.group_sign_user"):
    ...
    def upload_template(self, name=None, dataURL=None, active=True):
        ...
        try:
            file_pdf = PdfFileReader(
                io.BytesIO(base64.b64decode(datas)),
                strict=False,
        ...
        attachment = self.env["ir.attachment"].create(
            {"name": name, "datas": datas, "mimetype": mimetype}
        )
    ...
    @api.model
    def update_from_pdfviewer(
        self, template_id=None, duplicate=None, sign_items=None, name=None
    ):
        ...
        item_ids = {it for it in map(int, sign_items) if it > 0}
        ...
        for item in template.sign_item_ids:
            ...
            values["option_ids"] = [
                (6, False, [int(op) for op in values.get("option_ids", [])])
            ]
            item.write(values)
        for item in sign_items.values():
            item["template_id"] = template.id
            item["option_ids"] = [
                (6, False, [int(op) for op in item.get("option_ids", [])])
            ]
    ...
    def rotate_pdf(self, template_id=None):
        ...
        template.datas = base64.b64encode(
            pdf.rotate_pdf(base64.b64decode(template.datas))
        )
    ...
    def add_option(self, value):
        ...
        option_id = (
            option if option else self.env["sign.item.option"].create({"value": value})
        )
        return option_id.id
...
class SignItem(models.Model):
    ...
    template_id = fields.Many2one(
        "sign.template", string="Document Template", required=True, ondelete="cascade"
    )

    type_id = fields.Many2one(
        "sign.item.type", string="Type", required=True, ondelete="cascade"
    )

    required = fields.Boolean(default=True)
    responsible_id = fields.Many2one(
        "sign.item.role", string="Responsible", ondelete="restrict"
    )
...
class SignItemType(models.Model):
    ...
    default_width = fields.Float(
        string="Default Width", digits=(4, 3), required=True, default=0.150
    )
    default_height = fields.Float(
        string="Default Height", digits=(4, 3), required=True, default=0.015
    )
    auto_field = fields.Char(
        string="Auto-fill Partner Field",
...
class SignItemParty(models.Model):
    ...
    sms_authentification = fields.Boolean(
        "SMS Authentication",
        default=False,
    )
