...
TTFSearchPath.append(
    os.path.join(config["root_path"], "..", "addons", "web", "static", "fonts", "sign")
)
...
class SignRequest(models.Model):
    ...
    reference = fields.Char(
        ...
        help="This is how the document will be named in the mail",
    )

    access_token = fields.Char(
        "Security Token", required=True, default=_default_access_token, readonly=True
    )

    request_item_ids = fields.One2many(
        "sign.request.item", "sign_request_id", string="Signers"
    )
    state = fields.Selection(
        [("sent", "Sent"), ("signed", "Fully Signed"), ("canceled", "Canceled")],
        default="sent",
        tracking=True,
        group_expand="_expand_states",
    )

    completed_document = fields.Binary(
        readonly=True, string="Completed Document", attachment=True
    )

    nb_wait = fields.Integer(
        string="Sent Requests", compute="_compute_count", store=True
    )
    nb_closed = fields.Integer(
        string="Completed Signatures", compute="_compute_count", store=True
    )
    nb_total = fields.Integer(
        string="Requested Signatures", compute="_compute_count", store=True
    )
    progress = fields.Char(
        string="Progress", compute="_compute_count", compute_sudo=True
    )
    start_sign = fields.Boolean(
        string="Signature Started",
        help="At least one signer has signed the document.",
        compute="_compute_count",
        compute_sudo=True,
    )
    integrity = fields.Boolean(
        string="Integrity of the Sign request",
        compute="_compute_hashes",
        compute_sudo=True,
    )
    ...
    last_action_date = fields.Datetime(
        related="message_ids.create_date", readonly=True, string="Last Action Date"
    )
    completion_date = fields.Date(
        string="Completion Date", compute="_compute_count", compute_sudo=True
    )

    sign_log_ids = fields.One2many(
        "sign.log",
        "sign_request_id",
        string="Logs",
        help="Activity logs linked to this request",
    )
    template_tags = fields.Many2many(
        "sign.template.tag", string="Template Tags", related="template_id.tag_ids"
    )
    message = fields.Html("sign.message")
    message_cc = fields.Html("sign.message_cc")
    attachment_ids = fields.Many2many(
        "ir.attachment", string="Attachments", readonly=True, ondelete="restrict"
    )

    need_my_signature = fields.Boolean(
        compute="_compute_need_my_signature", search="_search_need_my_signature"
    )

    def _compute_need_my_signature(self):
        my_partner_id = self.env.user.partner_id
        for sign_request in self:
            sign_request.need_my_signature = any(
                sri.partner_id.id == my_partner_id.id and sri.state == "sent"
                for sri in sign_request.request_item_ids
            )
    ...
    def _search_need_my_signature(self, operator, value):
        ...
        documents_ids = (
            self.env["sign.request.item"]
            .search([("partner_id.id", "=", my_partner_id.id), ("state", "=", "sent")])
    ...
    def _compute_count(self):
        for rec in self:
            ...
            if wait == 0 and closed and signed_requests:
                last_completed_request = signed_requests.sorted(
                    key=lambda i: i.signing_date, reverse=True
                )[0]
    ...
    def _check_after_compute(self):
        for rec in self:
            if (
                rec.state == "sent"
                and rec.nb_closed == len(rec.request_item_ids)
                and len(rec.request_item_ids) > 0
            ):  # All signed
                rec.action_signed()
    ...
    def go_to_document(self):
        ...
        request_item = self.request_item_ids.filtered(
            lambda r: r.partner_id and r.partner_id.id == self.env.user.partner_id.id
        )[:1]
        return {
            "name": self.reference,
            ...
            "context": {
                ...
                "sign_token": request_item.access_token
                if request_item and request_item.state == "sent"
                else None,
                "create_uid": self.create_uid.id,
                "state": self.state,
                "request_item_states": {
                    str(item.id): item.is_mail_sent for item in self.request_item_ids
                },
            },
        }
    ...
    def go_to_signable_document(self):
        ...
        return {
            ...
            "context": {
                ...
                "state": self.state,
                "request_item_states": {
                    item.id: item.is_mail_sent for item in self.request_item_ids
                },
            },
        }
    ...
    def open_template(self):
        ...
        return {
            ...
            "domain": [
                ...
                ["active", "=", self.template_id.active],
            ],
            "views": [[False, "kanban"]],
        }
    ...
    def get_completed_document(self):
        ...
        return {
            ...
            "url": "/sign/download/%(request_id)s/%(access_token)s/completed"
            % {"request_id": self.id, "access_token": self.access_token},
        }
    ...
    def toggle_favorited(self):
        ...
        self.write(
            {
                "favorited_ids": [
                    (3 if self.env.user in self.favorited_ids else 4, self.env.user.id)
                ]
            }
        )
    ...
    def action_sent(self):
        ...
        for sign_request in self:
            ...
            included_request_items = sign_request.request_item_ids.filtered(
            ...
            if sign_request.send_signature_accesses(ignored_partners=ignored_partners):
                self.env["sign.log"]._create_log(
                    sign_request, "create", is_request=True
                )
                included_request_items.action_sent()
                body = _("The signature mail has been sent to: ")
                for signer, role in included_request_items.mapped(
                    lambda sri: (sri.partner_id, sri.role_id)
                ):
                ...
                sign_request.message_post(
                    body=body, attachment_ids=sign_request.attachment_ids.ids
                )
            else:
    ...
    def action_send(self):
        for sign_request in self.filtered(lambda sr: sr.state == "sent"):
            request_items = sign_request.request_item_ids.filtered(
                lambda sri: sri.state == "sent"
            )
            request_items.send_signature_accesses()
            body = _("The signature mail has been sent to: ")
            for signer, role in request_items.mapped(
                lambda sri: (sri.partner_id, sri.role_id)
            ):
                body += " %s(%s)," % (signer.name, role.name)
            body = body.strip(",")
    ...
    def set_signers(self, signers):
        ...
        for rec in self:
            rec.request_item_ids.filtered(
                lambda r: not r.partner_id or not r.role_id
            ).unlink()
            ids_to_remove = []
            for request_item in rec.request_item_ids:
                for i in range(0, len(signers)):
                    if (
                        signers[i]["partner_id"] == request_item.partner_id.id
    ...
    def check_request_edit_during_sign(self, request_id):
        ...
        return (
            request_sudo.exists()
            and request_sudo.nb_closed == 0
            and self.env.user.has_group("base.group_user")
        )

    def send_signature_accesses(self, ignored_partners=None):
        self.ensure_one()
        roles_and_responsible_dont_match = not self.request_item_ids.ids or (
            set(self.request_item_ids.mapped("role_id"))
            != set(self.template_id.sign_item_ids.mapped("responsible_id"))
        )
        # if sign request has default role and 0 items, it should be sent
        request_is_not_default_without_items = not (
            len(self.request_item_ids) == 1
            and self.request_item_ids.role_id
            == self.env.ref("sign.sign_item_role_default")
            and not (self.template_id.sign_item_ids)
        )
        ...
        self.request_item_ids.filtered(
            ...
            or not ignored_partners
            or r.partner_id.id not in ignored_partners
    ...
    def send_completed_document(self):
        ...
        pdf_content, __ = (
            report_action.with_user(public_user).sudo()._render_qweb_pdf(self.id)
        )
        ...
        self.attachment_ids = [
            ...
            Command.link(attachment_log.id),
        ]
        ...
        for signer in self.request_item_ids:
            ...
            body = tpl._render(
                {
                    "record": self,
                    "link": url_join(
                        base_url, "sign/document/%s/%s" % (self.id, signer.access_token)
                    ),
            ...
            self.env["sign.request"]._message_send_mail(
                ...
                },
                {
                    ...
                    "subject": _("%s has been edited and signed", self.reference)
                    if request_edited
                    else _("%s has been signed", self.reference),
        ...
        tpl = self.env.ref("sign.sign_template_mail_completed")
        followers = self.mapped(
            "message_follower_ids.partner_id"
        ) - self.request_item_ids.mapped("partner_id")
            ...
            body = tpl._render(
                {
                    "record": self,
                    "link": url_join(
                        base_url, "sign/document/%s/%s" % (self.id, self.access_token)
                    ),
                    "subject": "%s signed" % self.reference,
    ...
    def _get_font(self):
        custom_font = (
            self.env["ir.config_parameter"].sudo().get_param("sign.use_custom_font")
        )
        ...
        for p in range(0, old_pdf.getNumPages()):
            ...
            for item in items:
                value = SignItemValue.search(
                    [("sign_item_id", "=", item.id), ("sign_request_id", "=", self.id)],
                    limit=1,
                )
                ...
                if item.type_id.item_type == "text":
                    ...
                    image_reader = ImageReader(
                        io.BytesIO(base64.b64decode(value[value.find(",") + 1 :]))
                    )
    ...
    def _message_send_mail(
        ...
        **kwargs
        ...
        msg = (
            sign_request.env["mail.message"]
            .sudo()
            .new(dict(body=body, **message_values))
        )
        notif_layout = sign_request.env.ref(notif_template_xmlid)
        body_html = notif_layout._render(
            dict(message=msg, **notif_values), engine="ir.qweb", minimal_qcontext=True
        )
        ...
        mail = (
            sign_request.env["mail.mail"]
            .sudo()
            .create(dict(body_html=body_html, **mail_values))
        )
        if force_send:
            mail.send()
        return mail
    ...
    def initialize_new(
        ...
        sign_users = (
            self.env["res.users"]
            .search(
                [("partner_id", "in", [signer["partner_id"] for signer in signers])]
            )
        ...
        if attachment_ids:
            attachment_ids.write(
                {"res_model": sign_request._name, "res_id": sign_request.id}
            )
        ...
        return {
            ...
            "sign_token": sign_request.request_item_ids.filtered(
...
class SignRequestItem(models.Model):
    ...
    sign_request_id = fields.Many2one(
        "sign.request", string="Signature Request", ondelete="cascade", required=True
    )
    sign_item_value_ids = fields.One2many(
        "sign.request.item.value", "sign_request_item_id", string="Value"
    )
    reference = fields.Char(related="sign_request_id.reference", string="Document Name")

    access_token = fields.Char(
        required=True, default=_default_access_token, readonly=True
    )
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("sent", "To Sign"), ("completed", "Completed")],
        readonly=True,
        default="draft",
    )
    ...
    is_mail_sent = fields.Boolean(
        readonly=True, copy=False, help="The signature mail has been sent."
    )
    ...
    def action_draft(self):
        for request_item in self:
            ...
            itemsToClean = (
                request_item.sign_request_id.template_id.sign_item_ids.filtered(
                    lambda r: r.responsible_id == request_item.role_id
                    or not r.responsible_id
                )
            )
            self.env["sign.request.item.value"].search(
                [
                    ("sign_item_id", "in", itemsToClean.mapped("id")),
        ...
        for signer in self:
            ...
            body = tpl._render(
                {
                    ...
                    "body": self.sign_request_id.message
                    if not is_html_empty(self.sign_request_id.message)
                    else False,
                    "use_sign_terms": self.env["ir.config_parameter"]
                    .sudo()
                    .get_param("sign.use_sign_terms"),
                },
                engine="ir.qweb",
                minimal_qcontext=True,
            )
            ...
            self.env["sign.request"]._message_send_mail(
                ...
                },
                {
                    ...
                    "email_to": formataddr(
                        (signer.partner_id.name, signer.signer_email)
                    ),
    ...
    def sign(self, signature, new_sign_items=None):
        ...
        if not isinstance(signature, dict):
            ...
            signer_items = request.template_id.sign_item_ids.filtered(
                lambda r: not r.responsible_id or r.responsible_id.id == self.role_id.id
            )
            ...
            if not (
                item_ids <= authorised_ids
                and required_ids <= item_ids
                and (signature_ids - authorised_ids) == new_sign_ids
            ):  # Security check
                return False
            ...
            if (
                new_sign_ids
                and self.env["sign.request"].check_request_edit_during_sign(request.id)
                and check_new_sign_item_types(new_sign_items)
            ):
                old_template = request.template_id
                request.template_id = self.env["sign.template"]._copy_edited_template(
                    old_template.id, request.create_uid.id
                )

                new_items_signature = dict(
                    filter(
                        lambda item: int(item[0]) not in authorised_ids,
                        signature.items(),
                    )
                )
                old_items_signature = dict(
                    filter(
                        lambda item: int(item[0]) in authorised_ids, signature.items()
                    )
                )

                old_items_signature = self._get_existing_items_signature(
                    old_template, old_items_signature
                )
                new_items_signature = self._get_new_items_signature(
                    new_items_signature, new_sign_items
                )

                signature = dict(new_items_signature, **old_items_signature)

                self.env["sign.log"]._create_log(
                    request, "update", True, partner_id=self.env.user.partner_id.id
                )
                body = _(
                    "The signature request was edited by: %s.",
            ...
            for itemId in signature:
                ...
                if item_value.sign_item_id.type_id.item_type == "signature":
                    self.signature = signature[itemId][
                        signature[itemId].find(",") + 1 :
                    ]
    ...
    def _get_new_items_signature(self, signature, new_sign_items):
        ...
        for new_sign_item_id, new_sign_item_value in new_sign_items.items():
            ...
            new_sign_item_value["option_ids"] = [
                (
                    6,
                    False,
                    [int(op) for op in new_sign_item_value.get("option_ids", [])],
                )
            ]
    ...
    def resend_access(self, id):
        ...
        body = _(
            "The signature mail has been sent to: %s(%s)",
            sign_request_item.partner_id.name,
            sign_request_item.role_id.name,
        )
        ...
        sign_request_item.sign_request_id.message_post(
            body=body,
            attachment_ids=sign_request_item.sign_request_id.attachment_ids.ids,
        )
    ...
    def _send_sms(self):
        for rec in self:
            ...
            self.env["sms.api"]._send_sms(
                [rec.sms_number], _("Your confirmation code is %s", rec.sms_token)
            )
    ...
    def _compute_email(self):
        for sign_request_item in self:
            sign_request_item.signer_email = (
                sign_request_item.partner_id.email_normalized
            )
    ...
    def _update_email(self):
        for sign_request_item in self:
            ...
            sign_request_item.access_token = self.env[
                "sign.request"
            ]._default_access_token()
            if sign_request_item.is_mail_sent:
                sign_request_item.sign_request_id.message_post(
                    body=_(
                        "The mail address of %s has been updated. The request will be automatically resent.",
                        sign_request_item.partner_id.name,
                    )
                )
                self.env["sign.log"]._create_log(
                    sign_request_item, "update_mail", is_request=False
                )
...
class SignRequestItemValue(models.Model):
    ...
    sign_request_item_id = fields.Many2one(
        "sign.request.item",
        string="Signature Request item",
        required=True,
        ondelete="cascade",
    )
    sign_item_id = fields.Many2one(
        "sign.item", string="Signature Item", required=True, ondelete="cascade"
    )
    sign_request_id = fields.Many2one(
        string="Signature Request",
        required=True,
        ondelete="cascade",
        related="sign_request_item_id.sign_request_id",
    )
