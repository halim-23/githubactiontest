    ...
    $upload_input.on("change", function (e) {
      ...
      Promise.all(
        ...
        .then((uploadedTemplateData) => {
          const templates = uploadedTemplateData.map((template, index) => {
            return {
...
odoo.define("sign.template", function (require) {
  ...
  PDFIframe.include({
    ...
    handleControlClick: function (e) {
      ...
      $dropdown.find(".o_edit_mode_dropdown_item").on("click", function (e) {
        ...
        const posX =
          ($(e.target).offset().left - $pageElement.find(".textLayer").offset().left) /
          $pageElement.innerWidth();
        const posY =
          ($(e.target).offset().top - $pageElement.find(".textLayer").offset().top) /
        ...
        signItem.off("dragstop").on("dragstop", function (e, ui) {
          signItem.data({
            posx:
              Math.round((ui.position.left / signItem.parent().innerWidth()) * 1000) /
              1000,
            posy:
              Math.round((ui.position.top / signItem.parent().innerHeight()) * 1000) /
              1000,
          });
        ...
        signItem.off("resizestop").on("resizestop", function (e, ui) {
          signItem.data({
            width:
              Math.round((ui.size.width / signItem.parent().innerWidth()) * 1000) /
              1000,
            height:
              Math.round((ui.size.height / signItem.parent().innerHeight()) * 1000) /
              1000,
          });
  ...
  var SignItemCustomPopover = Widget.extend({
    ...
    start: function () {
      ...
      return this._super().then(function () {
          ...
          self
            .$(
              '.o_sign_field_align_group button[data-align="' +
                self.$currentTarget.data("alignment") +
                '"]'
            )
        ...
        sign_utils.setAsResponsibleSelect(
          self.$responsibleSelect.find("select"),
          self.$currentTarget.data("responsible"),
          self.parties
        );
        sign_utils.setAsOptionsSelect(
          self.$optionsSelect.find("input"),
          self.$currentTarget.data("itemId"),
          self.$currentTarget.data("option_ids"),
          self.select_options
        );
        self
          .$('input[type="checkbox"]')
          .prop("checked", self.$currentTarget.data("required"));
    ...
    hide: function () {
      ...
      var selected_options = this.$optionsSelect
  ...
  var EditablePDFIframe = PDFIframe.extend({
      ...
      this.fullyLoaded.then(function () {
        if (self.editMode) {
          if (self.$iframe.prop("disabled")) {
            ...
            var rotateButton = $(
              "<button id='rotateCw' class='toolbarButton o_sign_rotate rotateCw' title='" +
                rotateText +
                "'/>"
            );
            ...
            var $fieldTypeButtons = $(
              core.qweb.render("sign.type_buttons", {sign_item_types: typesArr})
            );
            ...
            var $fieldTypeButtonItems = $fieldTypeButtons.children(
    ...
    enableCustom: function ($signatureItem) {
      ...
      $signatureItem.off("resizestop").on("resizestop", function (e, ui) {
        $signatureItem.data({
          width:
            Math.round((ui.size.width / $signatureItem.parent().innerWidth()) * 1000) /
            1000,
          height:
            Math.round(
  ...
  var Template = AbstractAction.extend(StandaloneFieldManagerMixin, {
    ...
    createTemplateTagsField: function () {
      ...
      return this.model.load(params).then(function (recordId) {
        ...
        self.tag_idsMany2Many = new FormFieldMany2ManyTags(
          ...
          "tag_ids",
          self.record,
          {mode: "edit", create: true, attrs: {options: {color_field: "color"}}}
        );
        ...
        if (config.isDebug()) {
          ...
          self.group_idsMany2many = new FormFieldMany2ManyTags(
            ...
            "group_ids",
            self.record,
            {mode: "edit", create: false, attrs: {options: {color_field: "color"}}}
          );
    ...
    _onFieldChanged: function (event) {
      var self = this;
      var $majInfo = this.$(event.target.$el)
        .parent()
  ...
  var DocumentBackend = AbstractAction.extend({
    ...
    start: function () {
      ...
      var def = this._rpc({
        ...
        var url = $buttonsContainer
          .find(".o_sign_download_document_button")
          .attr("href");
        var logUrl = $buttonsContainer.find(".o_sign_download_log_button").attr("href");
        self.$buttons =
          self.cp_content && self.cp_content.$buttons && self.cp_content.$buttons.length
            ? self.cp_content.$buttons
            : $("");
        if (url) {
          self.$downloadButton = $("<a/>", {html: _t("Download Document")}).addClass(
        ...
        if (logUrl) {
          self.$downloadLogButton = $("<a/>", {html: _t("Certificate")}).addClass(
            url ? "btn btn-secondary" : "btn btn-primary"
          );
          self.$downloadLogButton.attr("href", logUrl);
          self.$buttons = self.$buttons.add(self.$downloadLogButton);
        }
        if ($signDocumentButton) self.$buttons = $signDocumentButton.add(self.$buttons);
            ...
            .each(function (i, el) {
              $(el).append(
                $("<button/>", {
                  type: "button",
                  title:
                    self.requestStates && self.requestStates[this.dataset.id]
