  ...
  var PDFIframe = Widget.extend(
    Object.assign({}, PinchItemMixin, {
      ...
      start: function () {
        ...
        var viewerURL =
          "/web/static/lib/pdfjs/web/viewer.html?unique=" + Number(new Date()) + "&file=";
        viewerURL +=
          encodeURIComponent(this.attachmentLocation)
            .replace(/'/g, "%27")
            .replace(/"/g, "%22") + "#page=1";
        ...
        $(
          ...
          .each(function (i, el) {
            var $signatureItem = self.createSignItem(
              ...
              Boolean(el.required),
              parseInt(el.responsible || (el.responsible_id && el.responsible_id[0])) ||
                0,
              parseFloat(el.posX),
              parseFloat(el.posY),
              parseFloat(el.width),
      ...
      display_select_options: function (
        ...
        function select_option($container, option_id) {
          var $selected_op = $container.find(":data(id)").filter(function () {
            return $(this).data("id") === option_id;
          });
          var $other_options = $container.find(":data(id)").filter(function () {
            return $(this).data("id") !== option_id;
          });
      ...
      updateSignItemFontSize: function ($signItem, normalSize) {
        ...
        if (
          $.inArray(this.types[$signItem.data("type")].item_type, [
            "signature",
            "initial",
            "textarea",
            "selection",
      ...
      createSignItem: function (
        ...
        return $signatureItem
          ...
          .toggle(Boolean(value) || this.requestState != "signed");
      },
      ...
      detachSignItem: function ($signItem) {
        ...
        for (let i = 0; i < this.configuration[pageNo].length; i++) {
          if (
            this.configuration[pageNo][i].data("posx") === $signItem.data("posx") &&
            this.configuration[pageNo][i].data("posy") === $signItem.data("posy")
          ) {
            this.configuration[pageNo].splice(i, 1);
          }
        }
      ...
      updateSignItem: function ($signatureItem) {
        var posX = $signatureItem.data("posx"),
          posY = $signatureItem.data("posy");
        var width = $signatureItem.data("width"),
        ...
        $signatureItem
          ...
          .toggleClass(
            ...
            this.pdfView ||
              Boolean($signatureItem.data("hasValue")) ||
...
odoo.define("sign.utils", function (require) {
  ...
  function setAsOptionsSelect($select, item_id, selected, select_options) {
    var configuration = getOptionsSelectConfiguration(
      item_id,
      select_options,
      selected
    );
...
odoo.define("sign.document_signing", function (require) {
          ...
          click: function (e) {
            ...
            if (!name || !mail || mail.indexOf("@") < 0) {
              this.$inputs
                .eq(0)
                .closest(".form-group")
                .toggleClass("o_has_error", !name)
                .find(".form-control, .custom-select")
                .toggleClass("is-invalid", !name);
              this.$inputs
                .eq(1)
                .closest(".form-group")
                .toggleClass("o_has_error", !mail || mail.indexOf("@") < 0)
                .find(".form-control, .custom-select")
                .toggleClass("is-invalid", !mail || mail.indexOf("@") < 0);
              return false;
            }
  ...
  var SMSSignerDialog = Dialog.extend({
    ...
    events: {
      "click button.o_sign_resend_sms": function (e) {
        ...
        var route =
          ...
          this.requestToken +
          "/" +
          this.$("#o_sign_phone_number_input").val();
        session
          .rpc(route, {})
          ...
          .guardedCatch(function (error) {
            ...
            Dialog.alert(
              this,
              _t("Unable to send the SMS, please contact the sender of the document."),
              {
                title: _t("Error"),
              }
            );
    ...
    _onValidateSMS: function () {
      ...
      var route =
        "/sign/sign/" + this.requestID + "/" + this.requestToken + "/" + input.val();
      var params = {
      ...
      session.rpc(route, params).then(function (response) {
        ...
        if (response === true) {
          new (self.get_thankyoudialog_class())(
            self,
            self.RedirectURL,
            self.RedirectURLText,
            self.requestID,
            {nextSign: (self.name_list || []).length}
          ).open();
          self.do_hide();
        }
      });
    ...
    start: async function () {
      ...
      if (nextTemplate && nextTemplate.template) {
        ...
        this.next_document = result.reduce(function (prev, curr) {
          return Math.abs(curr.sign_request_id[0] - self.requestID) <=
            Math.abs(prev.sign_request_id[0] - self.requestID)
            ? curr
            : prev;
        });
        this.options.buttons.push({
    ...
    init: function (parent, RedirectURL, requestID, options) {
      ...
      options.subtitle =
        ...
        _t("Your signature has been saved.") +
          " " +
          _.str.sprintf(_t("Next signatory is %s"), this.name_list[0]);
      options.size = options.size || "medium";
      options.technical = false;
      if (config.device.isMobile) {
        options.technical = true;
      ...
      (options.buttons = [
        {
          text: _.str.sprintf(_t("Next signatory (%s)"), this.name_list[0]),
  ...
  var SignablePDFIframe = PDFIframe.extend({
    ...
    fetchSignature: function (signatureType = "signature") {
      var self = this;
      const shouldRequestSignature = Object.values(self.signatureItems).some(
        (currentSignature) => {
      ...
      if (shouldRequestSignature) {
        return self
          ._rpc({
            route:
              "/sign/get_signature/" +
              self.getParent().requestID +
              "/" +
              self.getParent().accessToken,
            params: {
    ...
    doPDFPostLoad: function () {
      ...
      Promise.all([
        ...
        return self.signatureItemNav
          .prependTo(self.$("#viewerContainer"))
          .then(function () {
    ...
    createSignItem: function (
      ...
      if (!readonly) {
        ...
        if (
          [_t("Text"), _t("Multiline Text")].includes(type.name) &&
          ...
          $signatureItem.on(
            ...
            _.debounce(function (e) {
              ...
              if (
                ...
                self
                  .adjustSignatureSize(self.nextSignature, $signatureItem)
                  .then((data) => {
                    $signatureItem
                      .data("signature", data)
                      .empty()
                ...
                self
                  .adjustSignatureSize(self.nextInitial, $signatureItem)
                  .then((data) => {
                    $signatureItem
                      .data("signature", data)
                      .empty()
                    ...
                    await Promise.all(
                      page.reduce((promise, item) => {
                        if (
                          ...
                          item.data("responsible") === self.role
                        ) {
                          promise.push(
                            self.adjustSignatureSize(signature, item).then((data) => {
                              item
                                .data("signature", data)
                                .empty()
                                .append(
  ...
  var SignableDocument = Document.extend({
    events: {
      ...
      "pdfCompleted .o_sign_pdf_iframe": function (e) {
        if (this.name_list && this.name_list.length > 0) {
          ...
          var next_signatory = _.str.sprintf(
        ...
        for (var page in this.iframeWidget.configuration) {
          for (var i = 0; i < this.iframeWidget.configuration[page].length; i++) {
            ...
            if ($elem[0].type === "checkbox") {
              ...
              if ($elem[0].checked) {
                value = "on";
              } else if (!$elem.data("required")) value = "off";
        ...
        session.rpc(route, params).then(function (response) {
          ...
          if (response === true) {
            ...
            if (self.name_list && self.name_list.length > 0) {
              new (self.get_nextdirectsigndialog_class())(
                self,
                self.RedirectURL,
                self.requestID,
                {nextSign: self.name_list.length}
    ...
    signDocument: function (e) {
      ...
      if (
        self.iframeWidget &&
        self.iframeWidget.signatureItems &&
        Object.keys(self.iframeWidget.signatureItems).length > 0
      ) {
      ...
      signDialog.open().onConfirm(function () {
        ...
        function _sign() {
          ajax
            .jsonRpc("/sign/sign/" + self.requestID + "/" + self.accessToken, "call", {
              signature: signature,
            })
            .then(function (success) {
              if (!success) {
                setTimeout(function () {
                  // To be sure this dialog opens after the thank you dialog below
                  Dialog.alert(
                    self,
                    _t(
