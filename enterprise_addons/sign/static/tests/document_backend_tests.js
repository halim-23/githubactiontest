  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("simple rendering", async function (assert) {
        ...
        assert.strictEqual(
          $(webClient.el).find(".o_sign_document").text().trim(),
          "def",
      ...
      QUnit.test(
        "search more in many2one pointing to sign.template model",
        async function (assert) {
          // Addon sign patches the ListController for some models, like 'sign.template'.
          assert.expect(1);

          this.data["sign.template"].records = this.data[
            "sign.template"
          ].records.concat([
          ...
          assert.strictEqual(
            form.$(".o_field_widget[name=template_id] input").val(),
            "some template"
          );
