...
class TestSignCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.template_without_sign_items = cls.env["sign.template"].create(
            {"attachment_id": cls.attachment.id, "sign_item_ids": [(6, 0, [])]}
        )
        ...
        cls.single_role_sign_request = cls.create_sign_request(
            cls, cls.template, [cls.partner_id.id]
        )
        cls.multi_role_sign_request = cls.create_sign_request(
            cls, cls.template_multi_role, [cls.partner_id.id, cls.partner_2_id.id]
        )
        cls.default_role_sign_request = cls.create_sign_request(
            cls, cls.template_without_sign_items, [cls.partner_id.id]
        )
