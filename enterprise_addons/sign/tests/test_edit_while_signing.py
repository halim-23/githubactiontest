...
class TestEditModeWhileSigning(HttpCase, TestSignCommon):
    ...
    def _build_jsonrpc_payload(self, params):
        ...
        return {"jsonrpc": "2.0", "method": "call", "params": params, "id": None}
    ...
    def test_sign_without_new_items(self):
        ...
        sign_values = self.create_sign_values(
            sign_request.template_id.sign_item_ids, sign_request_item.role_id.id
        )
        ...
        sign_result = self.opener.post(
            self._build_url(url), json=self._build_jsonrpc_payload(data)
        )
