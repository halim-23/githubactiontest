...
class CustomerPortal(portal.CustomerPortal):
    ...
    @http.route(
        ["/my/signatures", "/my/signatures/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_signatures(
        self,
        page=1,
        date_begin=None,
        date_end=None,
    ...
    @http.route(
        ["/my/signature/<int:item_id>"], type="http", auth="public", website=True
    )
    def portal_my_signature(self, item_id, access_token=None, **kwargs):
        ...
        try:
            if (
                sign_item_sudo.state != "draft"
                and not sign_item_sudo.partner_id == partner_id
            ):
            ...
            values = self._get_page_view_values(
                sign_item_sudo,
                sign_item_sudo.access_token,
                values,
                "my_signatures_history",
                False,
                **kwargs,
            )
