...
class Sign(http.Controller):
    def get_document_qweb_context(self, id, token):
        ...
        if token:
            current_request_item = sign_request.request_item_ids.filtered(
                lambda r: r.access_token == token
            )
            if (
                not current_request_item
                and sign_request.access_token != token
                and http.request.env.user.id != sign_request.create_uid.id
            ):
                return http.request.render("sign.deleted_sign_request")
        ...
        item_values = {}
        sr_values = (
            http.request.env["sign.request.item.value"]
            .sudo()
            .search(
                [
        ...
        is_request, record = (
            (False, current_request_item)
            if current_request_item
            else (True, sign_request)
        )
        ...
        return {
            ...
            "token": token,
            "nbComments": len(
                sign_request.message_ids.filtered(lambda m: m.message_type == "comment")
            ),
            "isPDF": (sign_request.template_id.attachment_id.mimetype.find("pdf") > -1),
            "webimage": re.match(
                "image.*(gif|jpe|jpg|png)",
            ...
            "readonly": not (
                current_request_item and current_request_item.state == "sent"
            ),
    ...
    @http.route(
        ["/sign/document/<int:id>/<token>"], type="http", auth="public", website=True
    )
    ...
    @http.route(
        ["/sign/download/<int:id>/<token>/<download_type>"], type="http", auth="public"
    )
    def download_document(self, id, token, download_type, **post):
        ...
        if download_type == "log":
            report_action = http.request.env.ref(
            ...
            if not document:
                if (
                    sign_request.check_is_encrypted()
                ):  # if the document is completed but the document is encrypted
                    return request.redirect(
                        "/sign/password/%(request_id)s/%(access_token)s"
                        % {"request_id": id, "access_token": token}
                    )
    ...
    def share_link(self, link, **post):
        template = (
            http.request.env["sign.template"]
            .sudo()
            .search([("share_link", "=", link)], limit=1)
        )
        if not template:
            return http.request.not_found()

        sign_request = (
            http.request.env["sign.request"]
            .with_user(template.create_uid)
            .create(
                {
                    "template_id": template.id,
                    "reference": "%(template_name)s-public"
        ...
        request_item = (
            ...
            .create(
                {
                    "sign_request_id": sign_request.id,
                    "role_id": template.sign_item_ids.mapped("responsible_id").id,
                }
            )
        )
        ...
        return request.redirect(
            "/sign/document/%(request_id)s/%(access_token)s"
            % {"request_id": sign_request.id, "access_token": request_item.access_token}
        )

    @http.route(
        ["/sign/password/<int:sign_request_id>/<token>"], type="http", auth="public"
    )
    def check_password_page(self, sign_request_id, token, **post):
        ...
        request_item = (
            ...
            .search(
                [
                    ...
                    ("state", "=", "completed"),
                    ("sign_request_id.access_token", "=", token),
                ],
                limit=1,
            )
        )
        ...
        old_pdf = PdfFileReader(
            io.BytesIO(base64.b64decode(template_id.attachment_id.datas)),
            strict=False,
        ...
        return request.redirect(
            "/sign/document/%(request_id)s/%(access_token)s"
            % {"request_id": sign_request_id, "access_token": token}
        )
    ...
    def get_document(self, id, token):
        return http.Response(
            template="sign._doc_sign",
            qcontext=self.get_document_qweb_context(id, token),
        ).render()
    ...
    def update_signature(self, sign_request_id, role, signature_type=None, datas=None):
        ...
        if (
            not allowed
            or signature_type not in ["sign_signature", "sign_initials"]
            or not user
        ):
            return False
    ...
    def new_partners(self, partners=[]):
        ...
        for p in partners:
            ...
            pIDs.append(
                existing.id
                if existing
                else ResPartner.create({"name": p[0], "email": p[1]}).id
            )
        return pIDs

    @http.route(
        ["/sign/get_signature/<int:request_id>/<item_access_token>"],
        type="json",
        auth="public",
    )
    def sign_get_user_signature(
        self, request_id, item_access_token, signature_type="signature"
    ):
        sign_request_item = (
            http.request.env["sign.request.item"]
            .sudo()
        ...
        sign_request_user = (
            ...
            .sudo()
            .search([("partner_id", "=", sign_request_item.partner_id.id)], limit=1)
        )
    ...
    def make_public_user(self, id, token, name=None, mail=None):
        ...
        if (
            not sign_request
            or len(sign_request.request_item_ids) != 1
            or sign_request.request_item_ids.partner_id
        ):
            return False
        ...
        if request_item.role_id and request_item.role_id.sms_authentification:
            ...
            try:
                ...
                request_item.sign_request_id.activity_schedule(
                    ...
                    note=_(
                        "%s couldn't sign the document due to an insufficient credit error.",
    ...
    def sign(
        ...
        if request_item.role_id and request_item.role_id.sms_authentification:
            ...
            if sms_token != request_item.sms_token:
                return False
            if sms_token == request_item.sms_token:
                request_item.sign_request_id._message_log(
                    body=_(
                        "%s validated the signature by SMS with the phone number %s."
                    )
                    % (request_item.partner_id.display_name, request_item.sms_number)
                )
        ...
        sign_users = user_ids.filtered(
            lambda u: u.has_group("sign.group_sign_employee")
        )
        for sign_user in sign_users:
            request_item.sign_request_id.activity_feedback(
                ["mail.mail_activity_data_todo"], user_id=sign_user.id
            )

        request.env["sign.log"]._create_log(
            request_item, "sign", is_request=False, token=token
        )
    ...
    def check_encrypted(self, sign_request_id):
        request_item = (
            http.request.env["sign.request.item"]
            .sudo()
            .search([("sign_request_id", "=", sign_request_id)], limit=1)
        )
        if not request_item:
            return False
        ...
        if (
            request_item.sign_request_id.nb_total
            != request_item.sign_request_id.nb_closed
        ):
            return False
        template_id = request_item.sign_request_id.template_id
    ...
    def save_location(self, id, token, latitude=0, longitude=0):
        sign_request_item = (
            ...
            .sudo()
            .search(
                [("sign_request_id", "=", id), ("access_token", "=", token)], limit=1
            )
        )
    ...
    def render_assets_pdf_iframe(self, **kw):
        ...
        return (
            ...
            .sudo()
            ._render_template("sign.compiled_assets_pdf_iframe", context)
        )
