...
class SignSendRequest(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if "subject" in fields:
            res["subject"] = _(
                "Signature Request - %(file_name)s",
                file_name=template.attachment_id.name,
            )
    ...
    signer_ids = fields.One2many(
        "sign.send.request.signer", "sign_send_request_id", string="Signers"
    )
    ...
    message = fields.Html(
        "Message", help="Message to be sent to signers of the specified document"
    )
    message_cc = fields.Html(
        "CC Message", help="Message to be sent to followers of the signed document"
    )
    ...
    def _onchange_template_id(self):
        ...
        self.subject = _("Signature Request - %s") % (
            self.template_id.attachment_id.name or ""
        )
    ...
    def _compute_is_user_signer(self):
        if self.signers_count and self.env.user.partner_id in self.signer_ids.mapped(
            "partner_id"
        ):
    ...
    def _activity_done(self):
        signatories = self.signer_id.name or ", ".join(
            self.signer_ids.partner_id.mapped("name")
        )
    ...
    def create_request(self, send=True, without_mail=False):
        ...
        if self.signers_count:
            signers = [
                {"partner_id": signer.partner_id.id, "role": signer.role_id.id}
            ...
            signers = [
                {
                    ...
                    "role": self.env.ref("sign.sign_item_role_default").id,
                }
            ]
    ...
    def sign_directly_without_mail(self):
        ...
        return {
            ...
            "context": {
                ...
                "token_list": [
                    item.access_token for item in request.request_item_ids[1:]
                ],
                "current_signor_name": user_item.partner_id.name,
                "name_list": [
                    item.partner_id.name for item in request.request_item_ids[1:]
                ],
            },
        }
