...
class SignDuplicateTemplatePDF(models.TransientModel):
    ...
    def duplicate_template_with_pdf(self):
        if not self._compare_page_templates(
            self.original_template_id.datas, self.new_pdf
        ):
            raise UserError(
    ...
    def _compare_page_templates(self, original_file, new_file):
        pages_original_file = PdfFileReader(
            io.BytesIO(base64.b64decode(original_file)),
            strict=False,
            overwriteWarnings=False,
        ).getNumPages()
        pages_new_file = PdfFileReader(
            io.BytesIO(base64.b64decode(new_file)),
            strict=False,
            overwriteWarnings=False,
        ).getNumPages()
