...
class TestTimer(TransactionCase):
    ...
    def test_timer_from_self_user(self):
        ...
        self.assertEqual(
            len(self.test_timer.user_timer_id), 1, "It should have created one timer"
        )

        self.test_timer.action_timer_stop()
        self.assertEqual(
            len(self.test_timer.user_timer_id), 0, "It should have delete the timer"
        )
        ...
        self.assertEqual(
            len(self.env["timer.timer"].search([])),
            2,
            "It should have created 2 timers for the same user",
        )
        self.assertFalse(
            self.test_timer.is_timer_running,
            "The first timer should be in pause because the second one interrupt it",
        )
        self.assertTrue(
            self.test_timer_bis.is_timer_running, "The second timer should be running"
        )
        ...
        self.assertTrue(
            self.test_timer.is_timer_running,
            "The first timer should be running after being resumed",
        )
        self.assertFalse(
            self.test_timer_bis.is_timer_running,
            "The second timer should be in pause when another timer has been started",
        )
        ...
        self.assertEqual(
            len(self.env["timer.timer"].search([])),
            3,
            "A third timer should be created",
        )
        self.assertFalse(
            self.test_timer.is_timer_running,
            "The first timer has been interrupted and should be in pause",
        )
        self.assertFalse(
            self.test_timer_bis.is_timer_running,
            "The second timer has been interrupted and should be in pause",
        )
        self.assertTrue(
            override_test_timer.is_timer_running, "The third timer should be running"
        )
        ...
        self.assertEqual(
            len(self.env["timer.timer"].search([])),
            2,
            "It should remains only 2 timers",
        )
        self.assertEqual(
            len(override_test_timer.user_timer_id),
            0,
            "The third timer should be deleted because of his override method",
        )
    ...
    def test_timer_with_many_users(self):
        ...
        self.assertEqual(
            len(self.env["timer.timer"].search([])),
            2,
            "It should have created two timers",
        )
        self.assertEqual(
            len(self.test_timer.with_user(self.usr1).user_timer_id),
            1,
            "It should exist only one timer for this user, model and record",
        )
        self.assertEqual(
            len(self.test_timer.with_user(self.usr2).user_timer_id),
            1,
            "It should exist only one timer for this user, model and record",
        )
        ...
        self.assertEqual(
            len(self.env["timer.timer"].search([])),
            1,
            "It should have deleted one timer",
        )
        self.assertEqual(
            len(self.test_timer.with_user(self.usr1).user_timer_id),
            1,
        ...
        self.assertEqual(
            len(self.test_timer.with_user(self.usr2).user_timer_id),
            0,
            "It shouldn't exit one timer for this user, model and record",
        )
    ...
    def test_timer_rounding(self):
        ...
        result = self.test_timer._timer_rounding(minutes_spent, minimum, rounding)
        self.assertEqual(
            result, 15, "It should have been round to the next multiple of 15"
        )
