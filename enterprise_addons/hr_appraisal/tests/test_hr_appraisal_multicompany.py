...
class TestHrAppraisal(TransactionCase):
    ...
    def setUp(self):
        ...
        self.hr_employee2 = self.HrEmployee.create(
            dict(
                user_id=self.user.id,
    ...
    def test_hr_appraisal(self):
        ...
        self.assertEqual(
            appraisal_count([("employee_id", "=", self.hr_employee.id)]), 0
        )
        self.assertEqual(
            appraisal_count([("employee_id", "=", self.hr_employee2.id)]), 0
        )
        ...
        self.assertEqual(
            appraisal_count([("employee_id", "=", self.hr_employee2.id)]), 1
        )

        self.assertEqual(
            self.env["hr.appraisal"]
            .search([("employee_id", "=", self.hr_employee2.id)])
            .company_id.id,
            self.other_company.id,
        )
        self.assertEqual(
            self.user.with_company(company=self.other_company.id).next_appraisal_date,
