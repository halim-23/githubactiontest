...
class RequestAppraisal(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if not set(fields) & {"employee_id", "template_id", "recipient_ids"}:
            return result
        if self.env.context.get("default_appraisal_id"):
            appraisal = self.env["hr.appraisal"].browse(
                self.env.context["default_appraisal_id"]
            )
    ...
    def _compute_body(self):
        for wizard in self:
            ...
            if wizard.template_id:
                ctx = {
                    "employee_to_name": ", ".join(
                        wizard.recipient_ids.sorted("name").mapped("name")
                    ),
    ...
    def action_invite(self):
        ...
        subject = context_self._render_field(
            "subject", appraisal.ids, post_process=False
        )[appraisal.id]
        body = context_self._render_field(
            "body", appraisal.ids, engine="qweb", post_process=True
        )[appraisal.id]
