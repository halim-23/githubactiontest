...
class hr_department(models.Model):
    ...
    def _compute_appraisals_to_process(self):
        appraisals = self.env["hr.appraisal"].read_group(
            [("department_id", "in", self.ids), ("state", "in", ["new", "pending"])],
            ["department_id"],
            ["department_id"],
        )
        result = {
    ...
    def _compute_appraisal_feedbacks(self):
        for department in self:
            department.employee_feedback_template = (
                department.company_id.appraisal_employee_feedback_template
                or self.env.company.appraisal_employee_feedback_template
            )
            department.manager_feedback_template = (
                department.company_id.appraisal_manager_feedback_template
