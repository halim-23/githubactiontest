...
class HrAppraisal(models.Model):
    ...
    def _get_default_employee(self):
        if (
            ...
            in ("hr.employee", "hr.employee.public")
            ...
            return (
                self.env["res.users"].browse(self.env.context["active_id"]).employee_id
            )
    ...
    employee_user_id = fields.Many2one(
        "res.users", string="Employee User", related="employee_id.user_id"
    )
    ...
    previous_appraisal_date = fields.Date(
        related="previous_appraisal_id.date_close", string="Previous Appraisal Date"
    )
    uncomplete_goals_count = fields.Integer(
        related="employee_id.uncomplete_goals_count"
    )
    ...
    date_close = fields.Date(
        ...
        help="Date of the appraisal, automatically updated when the appraisal is Done or Cancelled.",
        required=True,
        index=True,
    ...
    manager_user_ids = fields.Many2many(
        "res.users", string="Manager Users", compute="_compute_user_manager_rights"
    )
    meeting_ids = fields.Many2many("calendar.event", string="Meetings")
    meeting_count_display = fields.Char(
        string="Meeting Count", compute="_compute_meeting_count"
    )
    date_final_interview = fields.Date(
        string="Final Interview", compute="_compute_final_interview"
    )
    ...
    employee_autocomplete_ids = fields.Many2many(
        "hr.employee", compute="_compute_user_manager_rights"
    )
    ...
    show_employee_feedback_full = fields.Boolean(
        compute="_compute_show_employee_feedback_full"
    )
    manager_feedback = fields.Html(
        compute="_compute_manager_feedback", store=True, readonly=False
    )
    show_manager_feedback_full = fields.Boolean(
        compute="_compute_show_manager_feedback_full"
    )
    ...
    def _compute_buttons_display(self):
        ...
        for appraisal in self:
            ...
            appraisal.can_see_manager_publish = (
                user_employee.id in appraisal.manager_ids.ids
            )
        for appraisal in self - new_appraisals:
            if (
                is_manager
                and not appraisal.can_see_employee_publish
                and not appraisal.can_see_manager_publish
            ):
                (
                    appraisal.can_see_employee_publish,
                    appraisal.can_see_manager_publish,
                ) = (True, True)
    ...
    def _compute_user_manager_rights(self):
        ...
        self.is_appraisal_manager = self.user_has_groups(
            "hr_appraisal.group_hr_appraisal_user"
        )
        if self.is_appraisal_manager:
            self.is_implicit_manager = False
            self.employee_autocomplete_ids = self.env["hr.employee"].search(
                [("company_id", "=", self.env.company.id)]
            )
        else:
            child_ids = self.env.user.employee_id.child_ids
            appraisal_child_ids = self.env.user.employee_id.appraisal_child_ids
            self.employee_autocomplete_ids = (
                child_ids + appraisal_child_ids + self.env.user.employee_id
            )
    ...
    def _compute_show_employee_feedback_full(self):
        for appraisal in self:
            ...
            appraisal.show_employee_feedback_full = (
                is_appraisee and not appraisal.employee_feedback_published
            )
    ...
    def _compute_show_manager_feedback_full(self):
        for appraisal in self:
            ...
            appraisal.show_manager_feedback_full = (
                is_appraiser and not appraisal.manager_feedback_published
            )
    ...
    def _compute_employee_feedback(self):
        for appraisal in self.filtered(lambda a: a.state == "new"):
            appraisal.employee_feedback = (
                appraisal.department_id.employee_feedback_template
                if appraisal.department_id.custom_appraisal_templates
    ...
    def _compute_manager_feedback(self):
        for appraisal in self.filtered(lambda a: a.state == "new"):
            appraisal.manager_feedback = (
                appraisal.department_id.manager_feedback_template
                if appraisal.department_id.custom_appraisal_templates
    ...
    def _compute_waiting_feedback(self):
        for appraisal in self:
            appraisal.waiting_feedback = (
                not appraisal.employee_feedback_published
                or not appraisal.manager_feedback_published
            )
    ...
    def subscribe_employees(self):
        for appraisal in self:
            partners = (
                appraisal.manager_ids.mapped("related_partner_id")
    ...
    def send_appraisal(self):
        for appraisal in self:
            confirmation_mail_template = (
                appraisal.company_id.appraisal_confirm_mail_template
            )
            mapped_data = {
                **{appraisal.employee_id: confirmation_mail_template},
                **{
                    manager: confirmation_mail_template
            ...
            for employee, mail_template in mapped_data.items():
                ...
                if employee.user_id:
                    appraisal.activity_schedule(
                        ...
                        note=_(
                            'Fill appraisal for <a href="#" data-oe-model="%s" data-oe-id="%s">%s</a>'
                        )
                        % (
                            appraisal.employee_id._name,
                            appraisal.employee_id.id,
    ...
    def _update_previous_appraisal(self):
        for employee in set(self.mapped("employee_id")):
            appraisals = employee.appraisal_ids.filtered(
    ...
    def _update_next_appraisal_date(self):
        for employee in set(self.mapped("employee_id")):
            ...
            employee.sudo().write(
                {
                    "next_appraisal_date": appraisals[0].date_close
                    if appraisals
                    else False
                }
            )
                ...
                employee.sudo().write(
                    {
                        ...
                        "last_appraisal_date": appraisals[0].date_close
                        if appraisals
                        else False,
                    }
                )
    ...
    def write(self, vals):
        ...
        if "manager_ids" in vals:
            previous_managers = {
                x: y for x, y in self.mapped(lambda a: (a.id, a.manager_ids))
            }
    ...
    def _appraisal_plan_post(self):
        ...
        days = int(
            ...
            .sudo()
            .get_param("hr_appraisal.appraisal_create_in_advance_days", 8)
        )
        for appraisal in self:
            if (
                not appraisal.appraisal_plan_posted
                and appraisal.company_id.appraisal_plan
                and not appraisal.employee_id.next_appraisal_date
            ):
                month = (
                    appraisal.company_id.duration_first_appraisal
                    if appraisal.employee_id.sudo().appraisal_count == 1
                    else appraisal.company_id.duration_next_appraisal
                )
                date = datetime.date.today() + relativedelta(months=month, days=-days)
                formated_date = format_date(self.env, date, date_format="MMM d y")
                body = _(
                    "Thanks to your Appraisal Plan, without any new manual Appraisal, the new Appraisal will be automatically created on %s.",
                    formated_date,
                )
    ...
    def read(self, fields=None, load="_classic_read"):
        ...
        if check_feedback:
            fields = fields + [
                "can_see_employee_publish",
                "can_see_manager_publish",
        ...
        if check_feedback:
            for appraisal in records:
                if (
                    not appraisal["can_see_employee_publish"]
                    and not appraisal["employee_feedback_published"]
                ):
                    appraisal["employee_feedback"] = _("Unpublished")
                if (
                    not appraisal["can_see_manager_publish"]
                    and not appraisal["manager_feedback_published"]
                ):
    ...
    def action_calendar_event(self):
        self.ensure_one()
        partners = (
            self.manager_ids.mapped("related_partner_id")
            | self.employee_id.related_partner_id
