...
class ResCompany(models.Model):
    ...
    def _get_default_appraisal_confirm_mail_template(self):
        return self.env.ref(
            "hr_appraisal.mail_template_appraisal_confirm", raise_if_not_found=False
        )

    appraisal_plan = fields.Boolean(
        string="Automatically Generate Appraisals", default=True
    )
    assessment_note_ids = fields.One2many("hr.appraisal.note", "company_id")
    appraisal_employee_feedback_template = fields.Html(
        default=_get_default_employee_feedback_template
    )
    appraisal_manager_feedback_template = fields.Html(
        default=_get_default_manager_feedback_template
    )
    ...
    duration_after_recruitment = fields.Integer(
        string="Create an Appraisal after recruitment", default=6
    )
    duration_first_appraisal = fields.Integer(
        string="Create a first Appraisal after", default=6
    )
    duration_next_appraisal = fields.Integer(
        string="Create a second Appraisal after", default=12
    )
        ...
        for company in companies:
            domain = [
                ...
                (
                    ...
                    ">",
                    current_date
                    - relativedelta(months=company.duration_after_recruitment + 1),
                ),
                (
                    "create_date",
                    "<=",
                    current_date
                    - relativedelta(months=company.duration_after_recruitment),
                ),
                "|",  # First Appraisal
                "&",
                ("appraisal_count", "=", 1),
                (
                    ...
                    "<=",
                    current_date
                    - relativedelta(months=company.duration_first_appraisal),
                ),
                # Next Appraisal
                "&",
                ("appraisal_count", ">", 1),
                (
                    ...
                    "<=",
                    current_date
                    - relativedelta(months=company.duration_next_appraisal),
                ),
            ]
    ...
    def _generate_activities(self, appraisals):
        ...
        for appraisal in appraisals:
            ...
            last_appraisal_months = employee.last_appraisal_date and (
                today.year - employee.last_appraisal_date.year
            ) * 12 + (today.month - employee.last_appraisal_date.month)
            if employee.user_id:
                # an appraisal has been just created
                if employee.appraisal_count == 1:
                    months = (
                        appraisal.date_close.year - employee.create_date.year
                    ) * 12 + (appraisal.date_close.month - employee.create_date.month)
                    note = _(
                        "You arrived %s months ago. Your appraisal is created you can assess yourself here. Your manager will determinate the date for your '1to1' meeting."
                    ) % (months)
                else:
                    note = _(
                        "Your last appraisal was %s months ago. Your appraisal is created you can assess yourself here. Your manager will determinate the date for your '1to1' meeting."
                ...
                for manager in managers.filtered("user_id"):
                    formated_date = format_date(
                        self.env, appraisal.date_close, date_format="MMM d y"
                    )
                    if employee.appraisal_count == 1:
                        note = _(
                            "The employee %s arrived %s months ago. An appraisal for %s is created. You can assess %s & determinate the date for '1to1' meeting before %s."
                        ) % (
                            employee.name,
                            months,
                            employee.name,
                        ...
                        note = _(
                            "Your employee's last appraisal was %s months ago. An appraisal for %s is created. You can assess %s & determinate the date for '1to1' meeting before %s."
                        ) % (
                            last_appraisal_months,
                            employee.name,
                            employee.name,
