...
class AccountTestFecImport(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_fr.l10n_fr_pcg_chart_template"):
        ...
        # Wizard --------------------------------------
        cls.wizard = cls.env["account.fec.import.wizard"].create(
        ...
        matrix = [
            line.lstrip().split("\t")
            for line in cls.test_content.lstrip().split("\n")
            if line.lstrip()
        ]
    ...
    def test_import_fec_moves(self):
        ...
        self.wizard._import_files(
            ["account.account", "account.journal", "res.partner", "account.move"]
        )
        ...
        self.assertEqual(
            1,
            len(
                moves[2].line_ids.filtered(lambda x: x.partner_id.name == "PARTNER 01")
            ),
        )
    ...
    def test_import_fec_demo_file(self):
        ...
        with tools.file_open(
            "l10n_fr_fec_import/demo/123459254FEC20190430.txt", mode="rb"
        ) as test_file:
    ...
    def test_import_fec_export(self):
        ...
        new_moves = self.env["account.move"].search(
            [
                ...
                (
                    ...
                    "=",
                    False,
                ),  # exclude automatic tax closing  entries
            ],
            order="name",
        )
        ...
        moves_data = [
            (
                self.company_export.id,
                "INV/001/123456",
                self.company_data_2["default_journal_sale"].id,
                self.partner_a.id,
            ...
            (
                self.company_export.id,
                "INV/001/123457",
                self.company_data_2["default_journal_purchase"].id,
                self.partner_b.id,
                datetime.date(2010, 1, 1),
            ),
        ]
        ...
        lines_data = [
            (
                ...
                100.30,
                "1",
                self.company_data_2["default_account_receivable"].id,
            ),
            (
                self.company_export.id,
                "line-2",
                ...
                0.00,
                "2",
                self.company_data_2["default_account_tax_sale"].id,
            ),
            (
                self.company_export.id,
                "line-3",
                ...
                0.00,
                "3",
                self.company_data_2["default_account_payable"].id,
            ),
            (
                self.company_export.id,
                "line-4",
    ...
    def test_balance_moves_by_day(self):
        ...
        domain = [
            ("company_id", "=", self.company.id),
            ("move_name", "in", ("ACH/20180808", "ACH/20180910")),
        ]
        move_lines = self.env["account.move.line"].search(
            domain, order="move_name,name"
        )
    ...
    def test_balance_moves_by_month(self):
        ...
        domain = [
            ("company_id", "=", self.company.id),
            ("move_name", "in", ("ACH/201808", "ACH/201809")),
        ]
        move_lines = self.env["account.move.line"].search(
            domain, order="move_name,name"
        )
