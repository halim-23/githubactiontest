...
class FecImportWizard(models.TransientModel):
    ...
    attachment_id = fields.Binary(
        string="File", required=True, help="Accounting FEC data file to be imported"
    )
    company_id = fields.Many2one(
        comodel_name="res.company",
        string="Company",
        help="Company used for the import",
    ...
    def _get_rows(self, attachment, attachment_name):
        ...
        if bytes_data.startswith(codecs.BOM_UTF8):
            ...
            if not string_data:
                raise UserError(
                    _("Cannot determine the encoding for the attached file.")
                )
    ...
    def _generator_fec_account_account(self, rows, cache):
        ...
        for record in rows:
            ...
            if account_code and account_code not in accounts_set:
                ...
                if not existing:
                    account_code = account_code[:digits] + account_code[digits:].rstrip(
                        "0"
                    )
                    data = {
        ...
        for record in rows:
            ...
            if partner_name and (partner_name, partner_ref) not in partners_set:
                ...
                if not existing or (partner_ref and partner_ref != existing.ref):
                    ...
                    account = account_code and cache["account.account"].get(
                        account_code.rstrip("0"), None
                    )
                    if account:
                        ...
                        elif account.user_type_id == self.env.ref(
    ...
    def _check_rounding_issues(self, moves_dict, balance_dict):
        ...
        domain = [
            ("code", "in", ("658000", "758000")),
            ("company_id", "=", self.company_id.id),
        ]
        debit_account, credit_account = self.env["account.account"].search(
            domain, order="code"
        )
    ...
    def _try_balance_moves(
        self, key_function, currency, journal_id, journal_code, lines_grouped_by_date
    ):
    ...
    def _check_imbalanced_journals(
        self, cache, moves_dict, balance_dict, imbalanced_journals, imbalances
    ):
        ...
        journal_codes = {
            journal.id: journal_code
            for journal_code, journal in cache["account.journal"].items()
        }
        for journal_id in imbalanced_journals:
            ...
            # then alert the user that the file cannot be understood
                ...
                for move in imbalanced_moves:
                    ...
                    if not float_is_zero(balance, precision_rounding=currency.rounding):
                        balance_issues += _(
                            "Move with name '%s' has a balance of %s\n",
                            move["name"],
                            float_repr(balance, currency.decimal_places),
                        )
                raise UserError(
                    _("Moves report incorrect balances:\n%s", balance_issues)
                )
    ...
    def _generator_fec_account_move(self, rows, cache):
        ...
        for idx, record in enumerate(rows):
            ...
            move_date = (
                move_date and datetime.datetime.strptime(move_date, "%Y%m%d")
            ) or piece_date
            partner_ref = record.get("CompAuxNum", "")
            partner_name = record.get("CompAuxLib", "")
            journal_code = record.get("JournalCode", "")
            ...
            if not journal:
                ...
                journal_code = cache.get("mapping_journal_code", {}).get(
                    journal_code, None
                )
                if journal_code:
                    journal = cache["account.journal"].get(journal_code, None)
            ...
            balance_data = balance_dict.get(
                move_key, {"balance": 0.0, "matching": False}
            )
            ...
            if not account:
                raise UserError(
                    _("Line %s has an invalid account %r", idx, account_code)
                )
            ...
            line_data = {
                ...
                "exclude_from_invoice_tab": account.user_type_id.type
        ...
        if imbalanced_journals:
            self._check_imbalanced_journals(
                cache, moves_dict, balance_dict, imbalanced_journals, imbalances
            )
    ...
    def _gather_templates(self):
        ...
        account_templates = self.env["account.account.template"].search_read(
            domain, ["code", "display_name", "user_type_id", "reconcile"]
        )
            ...
            for limit in [999, 3, 2]:
                ...
                template = next(
                    (
                        ...
                        if normalize_code(k) == normalize_code(record["code"])
                    ),
                    {},
                )
                if template:
    ...
    def _setup_bank_journal(self, journal):
        ...
        journal.suspense_account_id = (
            self.company_id.account_journal_suspense_account_id
        )
    ...
    def _reconcile_imported_move_lines(self, moves):
        ...
        for record in self.env.cr.fetchall():
            ...
            self.env["account.move.line"].browse(matched_move_line_ids).with_context(
    ...
    def _import_files(self, models=None):
        ...
        if (
            ...
            or not self.company_id.chart_template_id
        ):
            action = self.env.ref("account.action_account_config")
            raise RedirectWarning(
                _("You should install a Fiscal Localization first."),
                action.id,
        ...
        if "account.move" in models:
            ...
            start_date = (
                self.env["account.move"].search(domain, limit=1, order="date asc").date
                or fields.Date.today()
            )
            start_date_str = start_date.strftime("%Y-%m-%d")
            self.env["ir.config_parameter"].sudo().set_param(
                "sequence.mixin.constraint_start_date", start_date_str
            )
        ...
        if moves:
            ...
            if journals:
                journals_dict = {
                    journal.id: journal for journal in cache["account.journal"].values()
                }
                for journal_id, journal_type in self._get_journal_type(
                    journals, ratio=0.7, min_moves=3
                ):
