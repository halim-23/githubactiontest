...
class Generate1099Wizard(models.TransientModel):
    ...
    def _generate_row(self, company, vendor, total, boxes_1099):
        ...
        for box_1099 in boxes_1099:
            if box_1099 == vendor.box_1099_id:
                row.append(
                    -total
                )  # payments to vendors will be negative, so flip the sign
            else:
                row.append(0)
    ...
    def action_generate(self):
        ...
        for line in lines:
            if curr_vendor != line.partner_id and curr_total != 0:
                ...
                new_row = self._generate_row(
                    line.company_id, curr_vendor, curr_total, boxes_1099
                )
        ...
        if curr_total != 0:
            ...
            writer.writerow(
                self._generate_row(
                    lines[-1].company_id, curr_vendor, curr_total, boxes_1099
                )
            )
        ...
        return {
            ...
            "url": "/web/content?model=l10n_us_1099.wizard&download=true&field=generated_csv_file&filename=1099 report {} - {}.csv&id={}".format(
                self.start_date.strftime(us_format),
                self.end_date.strftime(us_format),
                self.id,
            ),
        }
