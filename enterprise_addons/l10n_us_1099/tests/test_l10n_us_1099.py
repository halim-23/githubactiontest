...
class Test1099(AccountTestInvoicingCommon):
        ...
        cls.vendor_1099 = cls.env["res.partner"].create(
            {
                ...
                "city": "San Francisco",
                "state_id": cls.env["res.country.state"]
                .search([("name", "=", "California")], limit=1)
                .id,
                "zip": "94134",
                "country_id": cls.env.ref("base.us").id,
                "phone": "(650) 691-3277",
    ...
    def test1099Wizard(self):
        ...
        self.assertEqual(
            wizard.lines_to_export,
            expected_lines,
            "Wizard should contain the credit part of the 1099 vendor entry.",
        )
        ...
        self.assertEqual(
            csv_content[0], header, "Wizard did not generate the expected header."
        )
        ...
        self.assertEqual(
            csv_content[1],
            expected_line,
            "Wizard did not generate the expected line for the 1099 vendor.",
        )
        self.assertEqual(
            len(csv_content), 2, "Wizard should exactly generate the two lines above."
        )
