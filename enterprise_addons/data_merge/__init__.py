...
def post_init(cr, registry):
    ...
    if not odoo.modules.db.has_unaccent(cr):
        _logger.warning(
            'pg extension "unaccent" not loaded, deduplication rules of type "accent" will be treated as "exaxt"'
        )
...
def uninstall_hook(cr, registry):
    ...
    models_to_clean = env["ir.model"].search(
        [("ref_merge_ir_act_server_id", "!=", False)]
    )
