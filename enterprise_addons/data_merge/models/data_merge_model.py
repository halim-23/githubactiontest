...
class DataMergeModel(models.Model):
    ...
    domain = fields.Char(
        string="Domain", help="Records eligible for the deduplication process"
    )
    ...
    rule_ids = fields.One2many(
        "data_merge.rule",
        "model_id",
        string="Deduplication Rules",
        help="Suggest to merge records matching at least one of these rules",
    )
    records_to_merge_count = fields.Integer(compute="_compute_records_to_merge_count")

    mix_by_company = fields.Boolean(
        "Cross-Company",
        default=False,
        help="When enabled, duplicates across different companies will be suggested",
    )
    ...
    merge_threshold = fields.Integer(
        string="Similarity Threshold",
        default=75,
        help="Records with a similarity percentage above this threshold will be automatically merged",
    )
    create_threshold = fields.Integer(
        string="Suggestion Threshold",
        default=0,
    ...
    is_contextual_merge_action = fields.Boolean(
        string="Merge action attached",
        help='If True, this record is used for contextual menu action "Merge" on the target model.',
    )
    ...
    def _compute_records_to_merge_count(self):
        count_data = self.env["data_merge.record"].read_group(
            [("model_id", "in", self.ids)], ["model_id"], ["model_id"]
        )
        counts = {cd["model_id"][0]: cd["model_id_count"] for cd in count_data}
        for dm_model in self:
            dm_model.records_to_merge_count = (
                counts[dm_model.id] if dm_model.id in counts else 0
            )
    ...
    def _compute_custom_merge_method(self):
        for dm_model in self:
            if dm_model.res_model_name:
                dm_model.custom_merge_method = hasattr(
                    self.env[dm_model.res_model_name], "_merge_method"
                )
            else:
    ...
    def _notify_new_duplicates(self):
        ...
        """
        for dm_model in self.env["data_merge.model"].search(
            [("merge_mode", "=", "manual")]
        ):
            ...
            if (
                not dm_model.last_notification
                or (dm_model.last_notification + delta) < fields.Datetime.now()
            ):
    ...
    def _cron_find_duplicates(self):
        ...
        """
        self.env["data_merge.model"].sudo().search([]).find_duplicates(
            batch_commits=True
        )
    ...
    def find_duplicates(self, batch_commits=False):
        ...
        for dm_model in self:
            ...
            for rule in dm_model.rule_ids:
                ...
                field_name = res_model._inherits_join_calc(
                    table, rule.field_id.name, query
                )
                if rule.field_id.relation:
                    ...
                    rhs_alias = query.join(
                        lhs_alias, lhs_column, related_model._table, "id", lhs_column
                    )
                    field_name = related_model._inherits_join_calc(
                        rhs_alias, related_model._rec_name, query
                    )
                ...
                if company_field and not dm_model.mix_by_company:
                    group_by = ", %s" % res_model._inherits_join_calc(
                        table, "company_id", query
                    )
            ...
            _logger.info(
                "Query identification done after %s" % str(timeit.default_timer() - t1)
            )
            t1 = timeit.default_timer()
            if ast.literal_eval(
            ...
            _logger.info(
                "Merging lists done after %s" % str(timeit.default_timer() - t1)
            )
            t1 = timeit.default_timer()
            ...
            for group_to_create in groups_to_create:
                ...
                group = (
                    self.env["data_merge.group"]
                    .with_context(prefetch_fields=False)
                    .create({"model_id": dm_model.id})
                )
                ...
                if (
                    dm_model.create_threshold > 0
                    and group.similarity * 100 <= dm_model.create_threshold
                ):
                    group.unlink()
                    continue
            ...
            _logger.info(
                "Record creation done after %s" % str(timeit.default_timer() - t1)
            )
    ...
    def _check_prevent_merge(self):
        ...
        for model_name in models:
            if (
                model_name
                and hasattr(self.env[model_name], "_prevent_merge")
                and self.env[model_name]._prevent_merge
            ):
                raise ValidationError(
                    "Deduplication is forbidden on the model: %s" % model_name
                )
    ...
    def open_records(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "data_merge.action_data_merge_record"
        )
        action["context"] = dict(
            ast.literal_eval(action.get("context")),
            searchpanel_default_model_id=self.id,
        )
        return action
