...
class DataMergeGroup(models.Model):
    ...
    model_id = fields.Many2one(
        "data_merge.model",
        string="Deduplication Model",
        ondelete="cascade",
        required=True,
    )
    res_model_id = fields.Many2one(
        related="model_id.res_model_id", store=True, readonly=True
    )
    res_model_name = fields.Char(
        related="model_id.res_model_name", store=True, readonly=True
    )
    ...
    divergent_fields = fields.Char(compute="_compute_similarity", store=True)
    ...
    def _compute_similarity(self):
        for group in self:
            ...
            records = (
                self.env[group.res_model_name].browse(record_ids).read(read_fields)
            )
            ...
            diff_fields = set(read_fields) - {
                k for k, v in data
            }  # fields of the model minus the identical fields
    ...
    def discard_records(self, records=[]):
        ...
        self.env["data_merge.record"].search(domain).write(
            {"is_discarded": True, "is_master": False}
        )
    ...
    def _elect_master_record(self):
        ...
        for group in self:
            ...
            if master:
                master_record = group.record_ids.filtered(
                    lambda r: r.res_id == master.id
                )
    ...
    def _log_merge(self, master_record, merged_records, chatter_data):
        ...
        for rec in merged_records:
            ...
            if self.model_id.removal_mode == "archive":
                rec._original_records()._message_log_with_view(
                    "data_merge.data_merge_merged", values=values
                )
            master_record._original_records()._message_log_with_view(
                "data_merge.data_merge_main", values=master_values
            )
    ...
    def _merge_method(self, master, records):
        ...
        :return dict
        """
        self.env["data_merge.record"]._update_foreign_keys(
            destination=master, source=records
        )
    ...
    def _cleanup(self):
        ...
        for group in self:
            ...
            for rec in group.record_ids:
                ...
                origin_inactive = (
                    original_record._active_name
                    and not original_record[original_record._active_name]
                )
            ...
            if records_discarded and records_kept == 1:
                records_to_delete += group.record_ids.filtered(
                    lambda r: not r.is_discarded
                )
