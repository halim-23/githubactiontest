...
class TestDeduplication(test_common.TestCommon):
    def test_deduplication_exact(self):
        ...
        self.assertEqual(
            self.MyModel.records_to_merge_count, 0, "0 record should have been found"
        )
        ...
        self.assertEqual(
            self.MyModel.records_to_merge_count, 2, "2 records should have been found"
        )
    ...
    def test_deduplication_multiple(self):
        ...
        self._create_record(
            "x_dm_test_model", x_name="toto", x_email="toto@example.com"
        )
        self._create_record("x_dm_test_model", x_name="bob", x_email="bob@example.com")
        self._create_record(
            "x_dm_test_model", x_name="alfred", x_email="alfred@example.com"
        )
        self.MyModel.find_duplicates()
        self.MyModel._compute_records_to_merge_count()
        ...
        self._create_record(
            "x_dm_test_model", x_name="toto", x_email="real_toto@example.com"
        )
        ...
        self.assertEqual(
            self.MyModel.records_to_merge_count, 4, "4 records should have been found"
        )

        self.DMGroup.search([("model_id", "=", self.MyModel.id)]).unlink()

        self._create_record(
            "x_dm_test_model", x_name="titi", x_email="real_toto@example.com"
        )
        self.MyModel.find_duplicates()
        self.MyModel._compute_records_to_merge_count()

        self.assertEqual(
            self.MyModel.records_to_merge_count, 5, "5 records should have been found"
        )
        self.assertEqual(
            self.DMGroup.search_count([("model_id", "=", self.MyModel.id)]),
            1,
            "1 group should have been created",
        )

        self.DMGroup.search([("model_id", "=", self.MyModel.id)]).unlink()

        self._create_record(
            "x_dm_test_model", x_name="robert", x_email="alfred@example.com"
        )
        self._create_record(
            "x_dm_test_model", x_name="jacky", x_email="jacky@example.com"
        )
        self.MyModel.find_duplicates()
        self.MyModel._compute_records_to_merge_count()

        self.assertEqual(
            self.MyModel.records_to_merge_count, 7, "7 records should have been found"
        )
        self.assertEqual(
            self.DMGroup.search_count([("model_id", "=", self.MyModel.id)]),
            2,
            "2 groups should have been created",
        )
    ...
    def test_record_references(self):
        ...
        records = self.env["data_merge.record"].search(
            [("res_id", "=", rec.id), ("model_id", "=", self.MyModel.id)]
        )
    ...
    def test_record_active(self):
        ...
        record = records[0]
        self.assertEqual(
            record._original_records(), rec, "Both records should be equal"
        )
