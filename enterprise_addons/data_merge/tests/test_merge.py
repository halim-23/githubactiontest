...
class TestMerge(test_common.TestCommon):
    def test_generic_merge(self):
        ...
        ref = self._create_record(
            "x_dm_test_model_ref", x_name="ref toto", x_test_id=rec2.id
        )
        ...
        self.assertEqual(
            master_record._original_records(),
            rec,
            "the 1st record created should be the master",
        )
    ...
    def test_generic_insensitive_rule(self):
        ...
        self.assertEqual(len(groups), 1, "Should have found 1 group")
        self.assertEqual(
            len(groups.record_ids),
            3,
            'First group must contains three records: ("accentuée", "accentue", "Accentuée")',
        )
        self.assertNotIn(
            "Accentué",
            groups[0].record_ids.mapped("display_name"),
            'Group must not contains "Accentué"',
        )
