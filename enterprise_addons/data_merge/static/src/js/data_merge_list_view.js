odoo.define("data_merge.ListView", function (require) {
  ...
  var DataMergeListController = DataCommonListController.extend({
    ...
    renderButtons: function ($node) {
      ...
      this.$buttons.on(
        "click",
        ".o_data_merge_merge_button",
        this._onMergeClick.bind(this)
      );
      this.$buttons.on(
        "click",
        ".o_data_merge_unselect_button",
        this._onUnselectClick.bind(this)
      );
    },
    ...
    _getGroupRecords: function (group_id) {
      ...
      const group_records = records.filter(
        (record) => record.data.group_id === group_id
      );
     ...
     * @param {Boolean} show_discarded Flag to indicate if the "show discarded" filter is active
     */
    ...
    _onMergeClick: function (ev) {
      ...
      const discard_view =
        context && context.show_discarded ? context.show_discarded : false;
      ...
      const message = _.str.sprintf(
        _t(
          "Are you sure that you want to merge the selected records in their respective group?"
        )
      );
      ...
      Dialog.confirm(self, message, {
        confirm_callback: function () {
          self
            ._callAction("merge_multiple_records", null, group_ids, discard_view)
            .then(function () {
    ...
    _onMergeDiscardRecords: function (ev) {
      ...
      const discard_view =
        record.context && record.context.show_discarded
          ? record.context.show_discarded
          : false;
    ...
    _doActionMergeDiscard: function (action, group_id, record_ids, discard_view) {
      ...
      this._callAction(action, group_id, record_ids, discard_view).then(function (res) {
        if (res && "type" in res && res.type.startsWith("ir.actions")) {
          ...
          if (action === "merge_records") {
            const records_merged =
              res && "records_merged" in res ? res.records_merged : "The selected";
    ...
    _showMergeNotification: function (records_merged) {
      this.displayNotification({
        message: _.str.sprintf(
          _t("%s records have been merged"),
          records_merged ? records_merged : _t("The selected")
        ),
        type: "info",
      });
    },
