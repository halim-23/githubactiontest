...
class AccountAnalyticLine(models.Model):
    ...
    def _check_no_link_task_and_ticket(self):
        ...
        if any(
            ...
            raise ValidationError(
                _(
                    "A timesheet cannot be linked to a task and a ticket at the same time."
                )
            )
    ...
    def _timesheet_get_portal_domain(self):
        ...
        if not self.env.user.has_group("hr_timesheet.group_hr_timesheet_user"):
            domain = expression.OR(
                [domain, self._timesheet_in_helpdesk_get_portal_domain()]
            )
        return domain
    ...
    def _timesheet_in_helpdesk_get_portal_domain(self):
        return [
            ...
            (
                "helpdesk_ticket_id.message_partner_ids",
                "child_of",
                [self.env.user.partner_id.commercial_partner_id.id],
            ),
