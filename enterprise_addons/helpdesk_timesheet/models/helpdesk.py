...
class HelpdeskTeam(models.Model):
    ...
    def _compute_total_timesheet_time(self):
        ...
        timesheets_read_group = self.env["account.analytic.line"].read_group(
            [
                (
                    "helpdesk_ticket_id",
                    "in",
                    helpdesk_timesheet_teams.ticket_ids.filtered(
                        lambda x: not x.stage_id.is_close
                    ).ids,
                )
            ],
        ...
        for result in timesheets_read_group:
            ...
            if uom_id:
                uom_ids.add(uom_id)
            timesheet_data_dict[result["helpdesk_ticket_id"][0]].append(
                ...
                [
                    sum(
                        [
                            unit_amount
                            * uoms_dict.get(
                                product_uom_id, team.timesheet_encode_uom_id
                            ).factor_inv
            ...
            self.env["timer.timer"].search(
                [
                    ...
                    (
                        "res_id",
                        "in",
                        self.with_context(active_test=False).ticket_ids.ids,
                    ),
                ]
            ).unlink()
        result = super(HelpdeskTeam, self).write(vals)
        for team in self.filtered(
            lambda team: team.use_helpdesk_timesheet and not team.project_id
        ):
            team.project_id = team._create_project(
                team.name, team.use_helpdesk_sale_timesheet, {}
            )
        return result
    ...
    def action_view_timesheets(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "helpdesk_timesheet.act_hr_timesheet_line_helpdesk"
        )
        action.update(
            {
                "domain": [
                    (
                        "helpdesk_ticket_id",
                        "in",
                        self.ticket_ids.filtered(lambda x: not x.stage_id.is_close).ids,
                    )
                ],
                "context": {
...
class HelpdeskTicket(models.Model):
    ...
    timesheet_ids = fields.One2many(
        "account.analytic.line", "helpdesk_ticket_id", "Timesheets"
    )
    use_helpdesk_timesheet = fields.Boolean(
        "Timesheet activated on Team",
        related="team_id.use_helpdesk_timesheet",
        readonly=True,
    )
    display_timesheet_timer = fields.Boolean(
        "Display Timesheet Time", compute="_compute_display_timesheet_timer"
    )
        ...
        for ticket in self:
            if not ticket.display_timesheet_timer:
                ...
                ticket.display_timer_start_secondary = (
                    ticket.display_timer_start_primary
                )
    ...
    def _compute_display_timesheet_timer(self):
        for ticket in self:
            ticket.display_timesheet_timer = (
                ticket.use_helpdesk_timesheet and not ticket.encode_uom_in_days
            )

    @api.depends("timesheet_ids")
    def _compute_total_hours_spent(self):
        for ticket in self:
            ticket.total_hours_spent = round(
                sum(ticket.timesheet_ids.mapped("unit_amount")), 2
            )
    ...
    def _fields_view_get(
        ...
        result = super(HelpdeskTicket, self)._fields_view_get(
