...
class RentalProcessing(models.TransientModel):
    ...
    def _get_wizard_lines(self):
        ...
        if rental_lines_to_process:
            ...
            self.rental_wizard_line_ids = [(6, 0, [])] + [
                (0, 0, vals) for vals in lines_values
            ]

    @api.depends("rental_wizard_line_ids")
    def _compute_has_late_lines(self):
        for wizard in self:
            wizard.has_late_lines = wizard.rental_wizard_line_ids and any(
                line.is_late for line in wizard.rental_wizard_line_ids
            )
...
class RentalProcessingLine(models.TransientModel):
    ...
    def _default_wizard_line_vals(self, line, status):
        delay_price = line.product_id._compute_delay_price(
            fields.Datetime.now() - line.return_date
        )
        return {
    ...
    rental_order_wizard_id = fields.Many2one(
        "rental.order.wizard", "Rental Order Wizard", required=True, ondelete="cascade"
    )
    ...
    def _only_pickedup_can_be_returned(self):
        for wizard_line in self:
            if (
                wizard_line.status == "return"
                and wizard_line.qty_returned > wizard_line.qty_delivered
            ):
