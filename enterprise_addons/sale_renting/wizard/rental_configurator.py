...
class RentalWizard(models.TransientModel):
    ...
    pickup_date = fields.Datetime(
        ...
        + relativedelta(minute=0, second=0, hours=1),
    )
    return_date = fields.Datetime(
        string="Return",
        required=True,
        default=lambda s: fields.Datetime.now()
        + relativedelta(minute=0, second=0, hours=1, days=1),
    )
    ...
    currency_id = fields.Many2one(
        "res.currency", string="Currency", compute="_compute_displayed_currency"
    )
    ...
    def _compute_displayed_currency(self):
        for wizard in self:
            wizard.currency_id = (
                wizard.pricelist_id.currency_id or wizard.pricing_id.currency_id
            )
    ...
    def _compute_pricing_explanation(self):
        ...
        for key, value in self.pricing_id._fields["unit"]._description_selection(
            self.env
        ):
