...
class TestRentalCommon(odoo.tests.common.SingleTransactionCase):
    ...
    def test_pricelists(self):
        ...
        self.assertEqual(
            sol.price_unit, 3.5, "Pricing should take into account pricelist A"
        )
        sale_order.write({"pricelist_id": pricelist_B.id})
        sale_order.update_prices()
        self.assertEqual(
            sol.price_unit, 15, "Pricing should take into account pricelist B"
        )
    ...
    def test_delay_pricing(self):
        ...
        self.assertEqual(
            self.product_id._compute_delay_price(timedelta(hours=5.0)), 12.5
        )

        self.assertEqual(
            self.product_id._compute_delay_price(timedelta(hours=5.0, days=6)), 102.5
        )
    ...
    def test_discount(self):
        ...
        sale_order.update_prices()
        self.assertEqual(
            sol.discount, 0, "Discount should always been 0 on pricelist change"
        )
        sale_order.write({"pricelist_id": pricelist_B.id})
        sale_order.update_prices()
        self.assertEqual(
