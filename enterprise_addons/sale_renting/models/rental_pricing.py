...
class RentalPricing(models.Model):
        ...
        if unit != self.unit:
            if unit == "month" or self.unit == "month":
                raise ValidationError(
                    _(
                        "Conversion between Months and another duration unit are not supported!"
                    )
                )
            converted_duration = math.ceil(
                (duration * PERIOD_RATIO[unit])
                / (self.duration * PERIOD_RATIO[self.unit])
            )
        else:
    ...
    def _compute_duration_vals(self, pickup_date, return_date):
        ...
        months = (
            1
            if duration_diff.days or duration_diff.hours or duration_diff.minutes
            else 0
        )
