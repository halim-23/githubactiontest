...
class ProductTemplate(models.Model):
    ...
    extra_hourly = fields.Float(
        "Extra Hour", help="Fine by hour overdue", company_dependent=True
    )
    extra_daily = fields.Float(
        "Extra Day", help="Fine by day overdue", company_dependent=True
    )
    ...
    def name_get(self):
        ...
        for res in res_names:
            result.append(
                (
                    res[0],
                    res[0] in rental_product_ids
                    and "%s %s" % (res[1], _("(Rental)"))
                    or res[1],
                )
            )
        return result
...
class ProductProduct(models.Model):
    ...
    def _get_qty_in_rent(self):
        ...
        res = {
            data["product_id"][0]: data["qty_delivered"] - data["qty_returned"]
            for data in active_rental_lines
        }
        for product in self:
    ...
    def _get_best_pricing_rule(self, **kwargs):
        ...
        pickup_date, return_date = kwargs.get("pickup_date", False), kwargs.get(
            "return_date", False
        )
            ...
            if pricing.applies_to(self):
                if duration and unit:
                    ...
                    price = pricing._compute_price(
                        duration_dict[pricing.unit], pricing.unit
                    )
