...
class TestReportsCommon(TestMrpAccount):
    def test_mrp_cost_structure(self):
        ...
        total_operation_cost = sum(
            wo.costs_hour * sum(wo.time_ids.mapped("duration")) / 60.0
            for wo in production_table.workorder_ids
        )
        ...
        report_values = report._get_report_values(docids=production_table.id)["lines"][
            0
        ]
        self.assertEqual(
            report_values["component_cost_by_product"][self.dining_table],
            total_component_cost * (100 - byproduct_cost_share) / 100,
        )
        self.assertEqual(
            report_values["operation_cost_by_product"][self.dining_table],
            total_operation_cost * (100 - byproduct_cost_share) / 100,
        )
        self.assertEqual(
            report_values["component_cost_by_product"][byproduct],
            total_component_cost * byproduct_cost_share / 100,
        )
        self.assertEqual(
            report_values["operation_cost_by_product"][byproduct],
            total_operation_cost * byproduct_cost_share / 100,
        )
        ...
        company_p = self.env["res.company"].create(
            {"name": "Pirates R Us", "currency_id": currency_p.id}
        )
        ...
        report_values = report.with_user(user_p)._get_report_values(
            docids=production_table.id
        )["lines"][0]
        self.assertEqual(
            report_values["component_cost_by_product"][self.dining_table],
            total_component_cost * (100 - byproduct_cost_share) / 100 / exchange_rate,
        )
        self.assertEqual(
            report_values["operation_cost_by_product"][self.dining_table],
            total_operation_cost * (100 - byproduct_cost_share) / 100 / exchange_rate,
        )
        self.assertEqual(
            report_values["component_cost_by_product"][byproduct],
            total_component_cost * byproduct_cost_share / 100 / exchange_rate,
        )
        self.assertEqual(
            report_values["operation_cost_by_product"][byproduct],
            total_operation_cost * byproduct_cost_share / 100 / exchange_rate,
        )
