...
class TestItalianTaxReport(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_it.l10n_it_chart_template_generic"):
        ...
        cls.tax_4v = cls.env["account.tax"].search(
            [
                ("company_id", "=", cls.company_data["company"].id),
                ("name", "=", "Iva al 4% (debito)"),
            ]
        )
        cls.tax_4a = cls.env["account.tax"].search(
            [
                ("company_id", "=", cls.company_data["company"].id),
        ...
        cls.tax_4v.tax_group_id.property_tax_payable_account_id = cls.company_data[
    ...
    def _test_line_report_carryover(
        self,
        invoice_date,
        invoice_amount,
        tax_line,
        first_month_options,
        second_month_options,
        target_line_code,
        target_line_value,
    ):
        ...
        with patch.object(
            ...
            line = [
                line for line in report_lines if line["line_code"] == target_line_code
            ][0]
