  ...
  const MapModel = AbstractModel.extend({
    ...
    async _fetchRecordData() {
      ...
      const results = await this._rpc({
        ...
        fields: groupBy ? this.fields.concat(groupBy) : this.fields,
        domain: this.domain,
      ...
      if (
        results &&
        groupBy &&
        ["one2many", "many2many"].includes(this.fieldsInfo[groupBy].type)
      ) {
        ...
        for (const record of results.records) {
          record[groupBy] = record[groupBy].map((val) =>
            val && val in labelPerRecordId ? [val, labelPerRecordId[val]] : false
          );
        }
      }
      return results;
    ...
    _fetchRoute: function () {
      const coordinatesParam = this.data.records
        ...
        .map(
          (record) =>
            record.partner.partner_longitude + "," + record.partner.partner_latitude
        );
    ...
    _maxBoxAPI: function () {
      ...
      this.data.partners.forEach((partner) => {
        if (
          ...
          promises.push(
            this._fetchCoordinatesFromAddressMB(partner).then((coordinates) => {
        ...
        for (let i = 0; i < partnersList.length; i++) {
          ...
          try {
            ...
            await new Promise((resolve) => {
              this.coordinateFetchingTimeoutHandle = setTimeout(
                resolve,
                this.COORDINATE_FETCH_DELAY
              );
            });
          }
        }
    ...
    _partnerFetching: async function (partnerIds) {
      this.data.partners = partnerIds.length
        ? await this._fetchRecordsPartner(partnerIds)
        : [];
