  ...
  const MapView = AbstractView.extend({
    ...
    init: function (viewInfo, params) {
      ...
      if (this.arch.attrs.default_order) {
        this.loadParams.orderBy = [
          {name: this.arch.attrs.default_order || "display_name", asc: true},
        ];
      }
      ...
      this.rendererParams.hideTitle = utils.toBoolElse(
        this.arch.attrs.hide_title || "",
        false
      );
      ...
      this.rendererParams.hideAddress = utils.toBoolElse(
        this.arch.attrs.hide_address || "",
        false
      );
      ...
      this.rendererParams.hasFormView = params.actionViews.some(
        (view) => view.type === "form"
      );

      this.controllerParams.actionName = params.action
        ? params.action.name
        : _t("Untitled");
    },
    /**
