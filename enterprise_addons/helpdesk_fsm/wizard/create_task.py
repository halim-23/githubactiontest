...
class CreateTask(models.TransientModel):
    ...
    project_id = fields.Many2one(
        "project.project",
        string="Project",
        help="Project in which to create the task",
        required=True,
        domain="[('company_id', '=', company_id), ('is_fsm', '=', True)]",
    )
    partner_id = fields.Many2one(
        "res.partner",
        string="Customer",
        help="Ticket's customer, will be linked to the task",
        required=True,
    ...
    def default_get(self, fields_list):
        ...
        if "project_id" in fields_list and not defaults.get("project_id"):
            task_default = (
                ...
                .with_context(fsm_mode=True)
                .default_get(["project_id"])
            )
