...
ERROR_MESSAGES = {
    ...
    ERROR_FILE_NAMES_NOT_MATCHING: _(
        "You must send the same quantity of documents and file names"
    ),
    ERROR_NO_CONNECTION: _("Server not available. Please retry later"),
    ERROR_SERVER_IN_MAINTENANCE: _(
        "Server is currently under maintenance. Please retry later"
    ),
}
...
class HrExpense(models.Model):
    ...
    def _compute_error_message(self):
        for record in self:
            if (
                record.extract_status_code != SUCCESS
                and record.extract_status_code != NOT_READY
            ):
    ...
    def _compute_can_show_send_resend(self, record):
        can_show = True
        if (
            not self.env.company.expense_extract_show_ocr_option_selection
            or self.env.company.expense_extract_show_ocr_option_selection == "no_send"
        ):
            can_show = False
        if record.state != "draft":
            can_show = False
        if (
            record.message_main_attachment_id is None
            or len(record.message_main_attachment_id) == 0
        ):
            can_show = False
        return can_show
    ...
    def _compute_show_resend_button(self):
        for record in self:
            record.extract_can_show_resend_button = self._compute_can_show_send_resend(
                record
            )
            if record.extract_state not in [
                "error_status",
                "not_enough_credit",
    ...
    def _compute_show_send_button(self):
        for record in self:
            record.extract_can_show_send_button = self._compute_can_show_send_resend(
                record
            )
    ...
    extract_remote_id = fields.Integer(
        "Id of the request to IAP-OCR",
        default="-1",
        help="Expense extract id",
        copy=False,
        readonly=True,
    )
    extract_word_ids = fields.One2many(
        "hr.expense.extract.words", inverse_name="expense_id", copy=False
    )
    extract_can_show_resend_button = fields.Boolean(
        "Can show the ocr resend button", compute=_compute_show_resend_button
    )
    extract_can_show_send_button = fields.Boolean(
        "Can show the ocr send button", compute=_compute_show_send_button
    )
    ...
    def check_status(self):
        ...
        records_to_update = self.filtered(
            lambda exp: exp.extract_state in ["waiting_extraction", "extract_not_ready"]
        )
            ...
            records_to_preupdate = self.search(
                [
                    (
                        "extract_state",
                        "in",
                        ["waiting_extraction", "extract_not_ready"],
                    ),
                    ("id", "not in", records_to_update.ids),
                    ("state", "=", "draft"),
                ],
                limit=limit,
            )
    ...
    def _check_status(self):
        ...
        endpoint = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("hr_expense_extract_endpoint", "https://iap-extract.odoo.com")
            + "/iap/expense_extract/get_result"
        )
        params = {"version": CLIENT_OCR_VERSION, "document_id": self.extract_remote_id}
        ...
        if result["status_code"] == SUCCESS:
            ...
            description_ocr = (
                ocr_results["description"]["selected_value"]["content"]
                if "description" in ocr_results
                else ""
            )
            total_ocr = (
                ocr_results["total"]["selected_value"]["content"]
                if "total" in ocr_results
                else ""
            )
            date_ocr = (
                ocr_results["date"]["selected_value"]["content"]
                if "date" in ocr_results
                else ""
            )
            currency_ocr = (
                ocr_results["currency"]["selected_value"]["content"]
                if "currency" in ocr_results
                else ""
            )
            bill_reference_ocr = (
                ocr_results["bill_reference"]["selected_value"]["content"]
                if "bill_reference" in ocr_results
                else ""
            )
            ...
            self.product_id = (
                predicted_product_id if predicted_product_id else self.product_id
            )
    ...
    def retry_ocr(self):
        ...
        if (
            attachments
            and attachments.exists()
            and self.extract_state
            in [
    ...
    def buy_credits(self):
        url = self.env["iap.account"].get_credits_url(
            base_url="", service_name="invoice_ocr"
        )
        return {
    ...
    def get_empty_list_help(self, help):
        ...
        if self.env.user.has_group("hr_expense.group_hr_expense_manager") and (
            ...
            action_id = self.env.ref(
                "hr_expense_extract.action_expense_sample_receipt"
            ).id
            html_to_return = """
...
<p>
    <a type="action" name="%(action_id)s" class="btn btn-primary text-white">Try Sample Receipt</a>
                    ...
                    "action_id": action_id,
                    "mail_alias": self._get_empty_list_mail_alias(),
                }
    ...
    def action_register_payment(self):
        ...
        if samples.count(True):
            action = self.env["ir.actions.actions"]._for_xml_id(
                "hr_expense_extract.action_expense_sample_register"
            )
