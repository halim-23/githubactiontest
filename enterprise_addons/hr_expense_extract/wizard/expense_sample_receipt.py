...
class ExpenseSampleReceipt(models.Model):
    ...
    def _action_create_expense(self, values, sample_number):
        fallback_employee = self.env["hr.employee"].search([], limit=1) or self.env[
            "hr.employee"
        ].create(
            {
        ...
        journal_id = (
            self.env["hr.expense.sheet"]._default_journal_id()
        ...
        image_path = get_module_resource(
            "hr_expense_extract", "static/img", "sample_%s.jpeg" % sample_number
        )
    ...
    def action_choose_sample_2(self):
        return self._action_create_expense(
            {"name": "Restaurant", "amount": 17.02, "date": datetime.date(2020, 6, 29)},
            2,
        )
    ...
    def action_choose_sample_4(self):
        return self._action_create_expense(
            {"name": "Travel", "amount": 700, "date": datetime.date(2020, 6, 29)}, 4
        )
