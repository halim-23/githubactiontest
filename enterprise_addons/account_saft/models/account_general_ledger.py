...
class AccountGeneralLedger(models.AbstractModel):
    ...
    def _fill_saft_report_general_ledger_values(self, options, values):
        ...
        for account, periods_results in accounts_results:
            ...
            closing_balance = account_balance.get("debit", 0.0) - account_balance.get(
                "credit", 0.0
            )
        ...
        for line_vals in self._cr.dictfetchall():
            line_vals["rate"] = (
                abs(line_vals["amount_currency"]) / abs(line_vals["balance"])
                if line_vals["balance"]
                else 1.0
            )
            line_vals["tax_detail_vals_list"] = []

            journal_vals_map.setdefault(
                line_vals["journal_id"],
                {
        ...
        for journal_vals in journal_vals_map.values():
            journal_vals["move_vals_list"] = list(
                journal_vals.pop("move_vals_map").values()
            )
    ...
    def _fill_saft_report_partner_ledger_values(self, options, values):
        res = {
            "customer_vals_list": [],
            "supplier_vals_list": [],
            "partner_detail_map": defaultdict(
                lambda: {
                    "type": False,
                    "addresses": [],
                    "contacts": [],
        ...
        for partner, results in partners_results:
            ...
            opening_balance = partner_init_bal.get("debit", 0.0) - partner_init_bal.get(
                "credit", 0.0
            )
            closing_balance = (
                partner_init_bal.get("debit", 0.0)
        ...
        if all_partners:
            ...
            for partner_id, balance in self._cr.fetchall():
                res["partner_detail_map"][partner_id]["type"] = (
                    "customer" if balance >= 0.0 else "supplier"
                )
