odoo.define("web_enterprise.kanban_mobile_tests", function (require) {
  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("kanban with searchpanel: rendering in mobile", async function (
        assert
      ) {
        assert.expect(34);
        ...
        assert.containsNone(
          document.body,
          "div.o_search_panel.o_searchview.o_mobile_search"
        );
        assert.verifySteps([
        ...
        // Select category
        await dom.click(
          $sp.find(".o_search_panel_category_value:contains(hello) header")
        );
        ...
        assert.containsOnce(
          $sp,
          ".o_search_panel_category_value > header.active:contains(hello)"
        );
        assert.containsOnce(
          $sp,
          ".o_search_panel_filter_value:contains(DEF) input:checked"
        );
      ...
      QUnit.test("mobile grouped rendering", async function (assert) {
        ...
        var kanban = await createView({
          ...
          data: this.data,
          arch:
            '<kanban class="o_kanban_test o_kanban_small_column" on_create="quick_create">' +
            '<templates><t t-name="kanban-box">' +
        ...
        assert.hasClass(
          kanban.$(".o_kanban_mobile_tab:first"),
          "o_current",
        ...
        assert.containsN(
          kanban,
          ".o_kanban_group:first > div.o_kanban_record",
          2,
          "there are 2 records in active tab"
        );
        assert.strictEqual(
          kanban.$(".o_kanban_group:nth(1) > div.o_kanban_record").length,
          2,
        ...
        assert.hasClass(
          kanban.$(".o_kanban_group:nth(0) > div:nth(1)"),
        ...
        assert.hasClass(
          kanban.$(".o_kanban_mobile_tab:nth(1)"),
          "o_current",
          "second tab is now active with class 'o_current'"
        );
        assert.hasClass(
          kanban.$(".o_kanban_group:nth(1)"),
          "o_current",
      ...
      QUnit.test("mobile grouped rendering in rtl direction", async function (assert) {
        ...
        assert.strictEqual(
          kanban.$(".o_kanban_group:first")[0].style.right,
          "0%",
          "first tab should have 50% right"
        );
        assert.strictEqual(
          kanban.$(".o_kanban_group:nth(1)")[0].style.right,
          "100%",
      ...
      QUnit.test("mobile grouped with undefined column", async function (assert) {
        ...
        var kanban = await createView({
          ...
          data: this.data,
          arch:
            '<kanban class="o_kanban_test o_kanban_small_column">' +
            '<templates><t t-name="kanban-box">' +
            '<div><field name="foo"/></div>' +
            "</t></templates>" +
            "</kanban>",
        ...
        assert.containsOnce(
          kanban,
          '.o_kanban_columns_content .o_kanban_group:first-child[data-id^="partner_"]',
      ...
      QUnit.test("mobile grouped on many2one rendering", async function (assert) {
        ...
        assert.containsN(
          kanban,
          '.o_kanban_group[data-id^="partner_"]',
          4,
      ...
      QUnit.test("mobile quick create column view rendering", async function (assert) {
        ...
        assert.hasClass(
          kanban.$(
            ".o_kanban_view > .o_kanban_mobile_tabs_container > .o_kanban_mobile_tabs > div:last"
          ),
        ...
        assert.hasClass(
          kanban.$('.o_kanban_group[data-id="3"].o_current > div:nth(1)'),
          "o_kanban_quick_create",
        ...
        await dom.click(kanban.$(".o-kanban-button-new"));
        assert.hasClass(
          kanban.$('.o_kanban_group[data-id="5"].o_current > div:nth(1)'),
          "o_kanban_quick_create",
        ...
        assert.isVisible(
          kanban.$(".o_column_quick_create input"),
          "the quick create column input should be visible"
        );
        assert.containsNone(
          kanban,
          ".o_kanban_examples",
          "Should not have See Examples link in mobile"
        );
        assert.containsNone(
          kanban,
          ".o_discard_msg",
          "Should not have Esc to Discard in mobile kanban"
        );
        kanban.$(".o_column_quick_create input").val("msh");
        await dom.click(kanban.$(".o_column_quick_create button.o_kanban_add"));

        assert.strictEqual(
          kanban.$(".o_kanban_group:last span:contains(msh)").length,
          1,
      ...
      QUnit.test(
        "mobile no quick create column when grouping on non m2o field",
        async function (assert) {
          ...
          assert.containsNone(
            kanban,
            ".o_kanban_mobile_add_column",
            "should not have the add column button"
          );
          assert.containsNone(
            kanban.$(".o_column_quick_create"),
      ...
      QUnit.test(
        "mobile kanban view: preserve active column on grouped kanban view",
        async function (assert) {
          assert.expect(9);
          ...
          const views = {
            "partner,false,kanban":
              ...
              "</t></templates>" +
              "</kanban>",
            "partner,form_view,form":
              '<form string="Partner"><field name="foo"/></form>',
            "partner,false,search":
              '<search><filter name="product" string="product" context="{\'group_by\': \'product_id\'}"/></search>',
          };
          ...
          assert.hasClass(
            $(webClient.el).find(".o_kanban_mobile_tab:first"),
            "o_current",
            "by default, first tab should be active"
          );
          assert.hasClass(
            $(webClient.el).find(".o_kanban_group:first"),
            "o_current",
          ...
          assert.hasClass(
            $(webClient.el).find(".o_kanban_mobile_tab:nth(1)"),
            "o_current",
            "second tab should be active"
          );
          assert.hasClass(
            $(webClient.el).find(".o_kanban_group:nth(1)"),
            "o_current",
          ...
          await dom.click(
            $(webClient.el).find(".o_kanban_group.o_current > .o_kanban_record:first")
          );
