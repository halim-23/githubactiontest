odoo.define("web_enterprise.list_mobile_tests", function (require) {
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("selection is properly displayed (single page)", async function (
        assert
      ) {
        assert.expect(10);
      ...
      QUnit.test("selection box is properly displayed (multi pages)", async function (
        assert
      ) {
        assert.expect(13);
        const list = await createView({
        ...
        assert.containsNone(list.$(".o_list_selection_box"), ".o_list_select_domain");
        assert.strictEqual(
          list.$(".o_list_selection_box").text().replace(/\s+/g, " "),
          " × 1 selected "
        );
        assert.containsOnce(list, ".o_list_selection_box");
        assert.containsOnce(list.el, "div.o_control_panel .o_cp_action_menus");
      ...
      QUnit.test(
        ...
        async function (assert) {
          ...
          await testUtils.dom.click(
            list.$("table .o_optional_columns_dropdown_toggle")
          );
