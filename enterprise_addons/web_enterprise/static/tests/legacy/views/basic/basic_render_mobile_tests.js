odoo.define("web_enterprise.BasicRenderMobileTests", function (require) {
  ...
  QUnit.module(
    "web_enterprise > basic > basic_render_mobile",
    {
      beforeEach: function () {
        this.data = {
          partner: {
    ...
    function () {
      ...
      QUnit.test(`field tooltip shouldn't remain displayed in mobile`, async function (
        assert
      ) {
        assert.expect(2);
        ...
        assert.strictEqual(
          ...
          1,
          "should have a tooltip displayed"
        );
        await testUtils.dom.triggerEvent(label, "touchend");
        assert.strictEqual(
          document.querySelectorAll(".tooltip .oe_tooltip_string").length,
          0,
          "shouldn't have a tooltip displayed"
        );
        form.destroy();
