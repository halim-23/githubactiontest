...
QUnit.test("uses a mobile-friendly view by default (if possible)", async function (
  assert
) {
  assert.expect(4);
...
QUnit.test(
  ...
  async function (assert) {
    ...
    assert.containsOnce(
      webClient.el.querySelector(".o_control_panel"),
    ...
    assert.hasClass(
      webClient.el.querySelector(
        ".o_control_panel .o_cp_switch_buttons > button > span"
      ),
      "fa-th-large"
    );
    ...
    assert.hasClass(
      webClient.el.querySelector(".o_cp_switch_buttons button.o_switch_view.o_kanban"),
      "active"
    );
    assert.doesNotHaveClass(
      webClient.el.querySelector(".o_cp_switch_buttons button.o_switch_view.o_list"),
      "active"
    );
    assert.hasClass(
      webClient.el.querySelector(".o_cp_switch_buttons button.o_switch_view.o_kanban"),
