  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("basic rendering", async function (assert) {
        ...
        assert.containsNone(
          document.body,
          ".o_control_panel .o_mobile_search",
          "search options are hidden by default"
        );
        assert.containsOnce(
          webClient,
          ".o_control_panel .o_enable_searchview",
          "should display a button to toggle the searchview"
        );
      });

      QUnit.test("control panel appears at top on scroll event", async function (
        assert
      ) {
        assert.expect(12);
        ...
        await scrollAndAssert(
          MAX_HEIGHT - DELTA_TEST,
          -(controlPanelHeight - DELTA_TEST),
          true
        );
      ...
      QUnit.test("mobile search: basic display", async function (assert) {
        ...
        await testUtils.dom.click(
          controlPanel.el.querySelector("button.o_enable_searchview")
        );
        // Open search view
        await testUtils.dom.click(
          controlPanel.el.querySelector("button.o_toggle_searchview_full")
        );
      ...
      QUnit.test(
        "mobile search: activate a filter through quick search",
        async function (assert) {
          ...
          assert.containsOnce(
            document.body,
            "button.o_enable_searchview.fa-search",
          ...
          await testUtils.dom.click(
            document.querySelector("button.o_enable_searchview")
          );
          ...
          // Close quick search
          await testUtils.dom.click(
            document.querySelector("button.o_enable_searchview.fa-arrow-left")
          );
      ...
      QUnit.test(
        "mobile search: activate a filter in full screen search view",
        async function (assert) {
          assert.expect(3);
