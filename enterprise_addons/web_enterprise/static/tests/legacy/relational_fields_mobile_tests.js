  ...
  QUnit.module("web_enterprise", {}, function () {
    QUnit.module(
      ...
      function () {
        ...
        QUnit.test("statusbar is rendered correclty on small devices", async function (
          assert
        ) {
          assert.expect(7);
          ...
          assert.strictEqual(
            form.$(".o_statusbar_status > button:contains(aaa)").length,
            1,
          ...
          assert.containsNone(
            form,
            ".o_statusbar_status .dropdown-menu:visible",
          ...
          assert.containsN(
            form,
            ".o_statusbar_status .dropdown-menu button",
            3,
          ...
          var $activeStatus = form.$(
            ".o_statusbar_status .dropdown-menu button[data-value=4]"
          );
          assert.hasClass(
            $activeStatus,
            "btn-primary",
        ...
        QUnit.test("statusbar with no status on extra small screens", async function (
          assert
        ) {
          assert.expect(9);
          ...
          assert.strictEqual(
            form.$(".o_statusbar_status button.dropdown-toggle").text().trim(),
            "",
          ...
          assert.strictEqual(
            form.$(".o_statusbar_status .dropdown-menu button").eq(0).text().trim(),
            "first record",
            "statusbar widget dropdown first button should display the first record display_name"
          );
          assert.strictEqual(
            form.$(".o_statusbar_status .dropdown-menu button").eq(1).text().trim(),
            "second record",
            "statusbar widget dropdown second button should display the second record display_name"
          );
          assert.strictEqual(
            form.$(".o_statusbar_status .dropdown-menu button").eq(2).text().trim(),
            "aaa",
        ...
        QUnit.test("clickable statusbar widget on mobile view", async function (
          assert
        ) {
          assert.expect(5);
        ...
        QUnit.test("many2one in a enterprise environment", async function (assert) {
          ...
          var form = await createView({
            View: FormView,
            arch:
              "<form>" + "<sheet>" + '<field name="trululu"/>' + "</sheet>" + "</form>",
            archs: {
          ...
          assert.equal(
            $modal.length,
            1,
            "there should be one modal opened in full screen"
          );
          assert.containsOnce(
            $modal,
            ".o_kanban_view",
          ...
          assert.containsNone(
            $modal.find(".o_control_panel .o_cp_buttons"),
            ".o-kanban-button-new",
            "kanban view in SelectCreateDialog should not have Create button"
          );
          assert.strictEqual(
            $modal.find(".o_kanban_view .o_kanban_record:not(.o_kanban_ghost)").length,
            3,
            "popup should load 3 records in kanban"
          );

          await testUtils.dom.click(
            $modal.find(".o_kanban_view .o_kanban_record:first")
          );
        ...
        QUnit.test(
          "hide/show element using selection_mode in kanban view in a enterprise environment",
          async function (assert) {
            assert.expect(5);

            this.data.partner.fields.foo = {
              string: "Foo",
              type: "char",
        ...
        QUnit.test(
          "kanban_view_ref attribute opens specific kanban view given as a reference in a mobile environment",
          async function (assert) {
            assert.expect(5);
            ...
            assert.hasClass(
              $modal.find(".o_kanban_view"),
              "kanban2",
              "kanban view with id 2 should be opened as it is given as kanban_view_ref"
            );
            assert.strictEqual(
              $modal.find(".o_kanban_view .o_kanban_record:first").text(),
              "first recordaaa",
        ...
        QUnit.test("many2one dialog on mobile: clear button header", async function (
          assert
        ) {
          assert.expect(7);
        ...
        QUnit.test("many2many_tags in a mobile environment", async function (assert) {
              ...
              if (args.method === "read" && args.model === "partner") {
                if (rpcReadCount === 0) {
                  assert.deepEqual(
                    args.args[0],
                    [2],
                    "form should initially show partner 2"
                  );
                } else if (rpcReadCount === 1) {
                  assert.deepEqual(
                    args.args[0],
                    [1],
                    "partner with id 1 should be selected"
                  );
                }
                rpcReadCount++;
              }
          ...
          assert.strictEqual(
            rpcReadCount,
            2,
            "there should be a read for current form record and selected sibling"
          );
          assert.strictEqual(
            form.$(".o_field_widget.o_input .badge").length,
            1,
