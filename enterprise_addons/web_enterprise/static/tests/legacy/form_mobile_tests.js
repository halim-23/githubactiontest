odoo.define("web_enterprise.form_tests", function (require) {
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("statusbar buttons are correctly rendered in mobile", async function (
        assert
      ) {
        assert.expect(5);
        ...
        assert.strictEqual(
          form.$(".o_statusbar_buttons a:contains(Action)").length,
          1,
      ...
      QUnit.test(
        ...
        async function (assert) {
          ...
          assert.containsN(
            form,
            ".o_statusbar_buttons > button.o_invisible_modifier",
            2,
          ...
          assert.containsOnce(
            form,
            ".o_statusbar_buttons a:contains(Action)",
            "statusbar should contain a button 'Action'"
          );
          assert.containsOnce(
            form,
      ...
      QUnit.test(
        ...
        async function (assert) {
          ...
          assert.containsOnce(
            form,
            ".o_statusbar_buttons > button > span:contains(Ciao)",
            "should have a simple statusbar button 'Ciao'"
          );
      ...
      QUnit.test(
        ...
        async function (assert) {
          ...
          assert.doesNotHaveClass(
            form.el.querySelector(dropdownMenuSelector),
            "show",
          ...
          await testUtils.dom.click(
            form.el.querySelector(".o_statusbar_buttons .dropdown-toggle")
          );
          assert.hasClass(
            form.el.querySelector(dropdownMenuSelector),
            "show",
        ...
        async function (assert) {
          ...
          const form = await createView({
            ...
            async mockRPC(route, {method, args: [, , changedField]}) {
              return method === "onchange" && changedField === "display_name"
                ? onchangePromise
          ...
          assert.strictEqual(
            form.el.querySelector('input[name="display_name"]').value,
            "after onchange"
          );
          assert.hasClass(
            form.el.querySelector(dropdownMenuSelector),
