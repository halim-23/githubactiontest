...
QUnit.module("WebClient Enterprise", (hooks) => {
  ...
  QUnit.module("basic flow with home menu", (hooks) => {
    ...
    QUnit.test("6 -- back to underlying action with many breadcrumbs", async function (
      assert
    ) {
      assert.expect(20);
      const webClient = await createEnterpriseWebClient({serverData, mockRPC});
    ...
    QUnit.test(
      "restore the newly created record in form view (legacy)",
      async (assert) => {
        assert.expect(7);
  ...
  QUnit.test("clear unCommittedChanges when toggling home menu", async function (
    assert
  ) {
    assert.expect(7);
  ...
  QUnit.test("loadState back and forth keeps relevant keys in state", async function (
    assert
  ) {
    assert.expect(9);
