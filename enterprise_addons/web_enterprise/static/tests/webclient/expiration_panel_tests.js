...
QUnit.module("web_enterprise", {}, function () {
  ...
  QUnit.test("Expiration Panel one app installed, buy subscription", async function (
    assert
  ) {
    assert.expect(6);
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      await testUtils.dom.click(
        panel.el.querySelector(".oe_instance_register_form button")
      );
  ...
  QUnit.test("One app installed, check status and get success", async function (
    assert
  ) {
    assert.expect(8);
  ...
  QUnit.test("One app installed, check status and get page reload", async function (
    assert
  ) {
    assert.expect(5);
