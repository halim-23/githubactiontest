...
QUnit.module("MobileMobileSwitchCompanyMenu", (hooks) => {
  ...
  QUnit.test("basic rendering", async (assert) => {
    ...
    assert.hasClass(
      scMenu.el.querySelectorAll(".menu_companies_item i")[0],
      "fa-check-square"
    );
    assert.hasClass(
      scMenu.el.querySelectorAll(".menu_companies_item i")[1],
      "fa-square-o"
    );
    assert.hasClass(
      scMenu.el.querySelectorAll(".menu_companies_item i")[2],
      "fa-square-o"
    );

    assert.strictEqual(
      scMenu.el.textContent,
      "CompaniesHermit(current)Herman'sHeroes TM"
    );
  });
  ...
  QUnit.test(
    "single company selected: toggling it off will keep it",
    async (assert) => {
      assert.expect(11);
  ...
  QUnit.test(
    "multi company mode: log into an already selected company",
    async (assert) => {
      assert.expect(7);
