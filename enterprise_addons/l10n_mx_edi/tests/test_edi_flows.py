...
class TestEdiFlows(TestMxEdiCommon):
    def test_invoice_flow_not_sent(self):
        with freeze_time(self.frozen_today), patch(
            ...
            document = self.invoice.edi_document_ids.filtered(
                lambda d: d.edi_format_id == self.env.ref("l10n_mx_edi.edi_cfdi_3_3")
            )
    ...
    def test_invoice_flow_sent_twice(self):
        with freeze_time(self.frozen_today), patch(
            ...
            "odoo.addons.l10n_mx_edi.models.account_edi_format.AccountEdiFormat._cancel_invoice_edi",
    ...
    def test_payment_flow_PUE_invoice(self):
        with freeze_time(self.frozen_today), patch(
            ...
            # default.
            (self.invoice + self.payment.move_id).line_ids.filtered(
            ...
            document = self.payment.edi_document_ids.filtered(
                lambda d: d.edi_format_id == self.env.ref("l10n_mx_edi.edi_cfdi_3_3")
            )
    ...
    def test_payment_flow_unreconcile(self):
        with freeze_time(self.frozen_today), patch(
            ...
            "odoo.addons.l10n_mx_edi.models.account_edi_format.AccountEdiFormat._cancel_payment_edi",
            new=_mocked_cancel_success,
        ):
            self.payment.action_post()
            self.invoice.action_post()
    ...
    def test_statement_flow_PUE_invoice(self):
        with freeze_time(self.frozen_today), patch(
            ...
            document = self.statement_line.edi_document_ids.filtered(
                lambda d: d.edi_format_id == self.env.ref("l10n_mx_edi.edi_cfdi_3_3")
            )
