...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_get_common_cfdi_values(self, move):
        ...
        if move.l10n_mx_edi_origin:
            origin_type, origin_uuids = move._l10n_mx_edi_read_cfdi_origin(
                move.l10n_mx_edi_origin
            )
        else:
            origin_type = None
            origin_uuids = []
    ...
    def _l10n_mx_edi_get_invoice_cfdi_values(self, invoice):
        ...
        cfdi_values = {
            ...
            **self._l10n_mx_edi_get_common_cfdi_values(invoice),
            ...
            "payment_method_code": (
                invoice.l10n_mx_edi_payment_method_id.code or ""
            ).replace("NA", "99"),
        ...
        if invoice.currency_id.name == "MXN":
            ...
            cfdi_values["currency_conversion_rate"] = abs(
        ...
        cfdi_values.update(
            {
                ...
                "tax_details_transferred": invoice._prepare_edi_tax_details(
                    filter_to_apply=filter_tax_transferred
                ),
                "tax_details_withholding": invoice._prepare_edi_tax_details(
                    filter_to_apply=filter_tax_withholding
                ),
            }
        )
        ...
        cfdi_values.update(
            {
                "has_tax_details_transferred_no_exento": any(
                    x["tax"].l10n_mx_tax_type != "Exento"
                    for x in cfdi_values["tax_details_transferred"][
                        "tax_details"
                    ].values()
                ),
                "has_tax_details_withholding_no_exento": any(
                    x["tax"].l10n_mx_tax_type != "Exento"
                    for x in cfdi_values["tax_details_withholding"][
                        "tax_details"
                    ].values()
                ),
            }
        ...
        def put_discount_on(cfdi_values, discount_vals, other_line_vals):
            ...
            remaining_discount = (
                discount_vals["price_discount"]
                - discount_vals["price_subtotal_before_discount"]
            )
            remaining_price_subtotal = (
                other_line_vals["price_subtotal_before_discount"]
                - other_line_vals["price_discount"]
            )
            ...
            for tax_key in ("tax_details_transferred", "tax_details_withholding"):
                discount_line_tax_details = cfdi_values[tax_key][
                    "invoice_line_tax_details"
                ][discount_line]["tax_details"]
                other_line_tax_details = cfdi_values[tax_key][
                    "invoice_line_tax_details"
                ][other_line]["tax_details"]
                for k, tax_values in discount_line_tax_details.items():
                    if discount_line.currency_id.is_zero(
                    ...
                    tax_amount_to_allow = copysign(
                        min(
                            abs(tax_values["tax_amount_currency"]),
                            abs(other_tax_values["tax_amount_currency"]),
                        ),
                    ...
                    base_amount_to_allow = copysign(
                        min(
                            abs(tax_values["base_amount_currency"]),
                            abs(other_tax_values["base_amount_currency"]),
                        ),
            ...
            return discount_line.currency_id.is_zero(
                remaining_discount - discount_to_allow
            )
        ...
        for line_vals in cfdi_values["invoice_line_vals_list"]:
            ...
            candidate_vals_list = [
                x
                for x in cfdi_values["invoice_line_vals_list"]
            ...
            candidate_vals_list = sorted(
                candidate_vals_list,
                key=lambda x: x["line"].price_subtotal,
                reverse=True,
            )
        ...
        for line_vals in cfdi_values["invoice_line_vals_list"]:
            ...
            if line.currency_id.is_zero(
                line_vals["price_subtotal_before_discount"]
    ...
    def _l10n_mx_edi_export_invoice_cfdi(self, invoice):
        ...
        * error:        An error if the cfdi was not successfuly generated.
        ...
        cfdi_cadena_crypted = (
            ...
            .sudo()
            .get_encrypted_cadena(decoded_cfdi_values["cadena"])
        )
    ...
    def _l10n_mx_edi_export_payment_cfdi(self, move):
        ...
        * error:        An error if the cfdi was not successfully generated.
        ...
        if move.currency_id == mxn_currency:
            ...
            rate_payment_curr_mxn = move.currency_id._convert(
                1.0, mxn_currency, move.company_id, move.date, round=False
            )
        ...
        for field1, field2 in (("debit", "credit"), ("credit", "debit")):
            for partial in pay_rec_lines[f"matched_{field1}_ids"]:
                ...
                if exchange_move:
                    exchange_partial = invoice_line[f"matched_{field2}_ids"].filtered(
                        lambda x: x[f"{field2}_move_id"].move_id == exchange_move
                    )
                    if exchange_partial:
                ...
                if invoice_line.currency_id == payment_line.currency_id:
                    ...
                    exchange_rate = (
                        amount_paid_invoice_curr / amount_paid_invoice_comp_curr
                    )
                    exchange_rate = float_round(
                        exchange_rate, precision_digits=6, rounding_method="UP"
                    )
                ...
                invoice_vals_list.append(
                    {
                        ...
                        + len(invoice._get_reconciled_statement_lines()),
        ...
        payment_account_ord = (
            re.sub(r"\s+", "", bank_accounts[:1].acc_number or "") or None
        )
        payment_account_receiver = (
            re.sub(r"\s+", "", move.journal_id.bank_account_id.acc_number or "") or None
        )
        ...
        cfdi_values = {
            **self._l10n_mx_edi_get_common_cfdi_values(move),
            ...
            "receiver_vat_ord": is_payment_code_receiver_ok
            and move.journal_id.bank_account_id.bank_id.l10n_mx_edi_vat,
            "payment_account_receiver": is_payment_code_receiver_ok
            and payment_account_receiver,
            "cfdi_date": move.l10n_mx_edi_post_time.strftime("%Y-%m-%dT%H:%M:%S"),
        }

        cfdi_payment_datetime = datetime.combine(
            fields.Datetime.from_string(move.date),
            datetime.strptime("12:00:00", "%H:%M:%S").time(),
        )
        cfdi_values["cfdi_payment_date"] = cfdi_payment_datetime.strftime(
            "%Y-%m-%dT%H:%M:%S"
        )
    ...
    def _l10n_mx_edi_post_invoice_pac(self, invoice, exported):
        ...
        credentials = getattr(self, "_l10n_mx_edi_get_%s_credentials" % pac_name)(
            invoice
        )
        ...
        res = getattr(self, "_l10n_mx_edi_%s_sign_invoice" % pac_name)(
            invoice, credentials, exported["cfdi_str"]
        )
        if res.get("errors"):
            return {
                "error": self._l10n_mx_edi_format_error_message(
    ...
    def _l10n_mx_edi_post_payment_pac(self, move, exported):
        ...
        res = getattr(self, "_l10n_mx_edi_%s_sign_payment" % pac_name)(
            move, credentials, exported["cfdi_str"]
        )
        if res.get("errors"):
            return {
                "error": self._l10n_mx_edi_format_error_message(
    ...
    def _l10n_mx_edi_get_finkok_credentials_company(self, company):
        ...
        if company.l10n_mx_edi_pac_test_env:
            ...
            if (
                not company.l10n_mx_edi_pac_username
                or not company.l10n_mx_edi_pac_password
            ):
    ...
    def _l10n_mx_edi_finkok_cancel_service(
    ...
    def _l10n_mx_edi_solfact_cancel_service(
        self, uuid, company, credentials, uuid_replace=None
    ):
...
""" % {
        ...
        response_json = self._l10n_mx_edi_sw_call(
    ...
    def _l10n_mx_edi_sw_cancel(self, move, credentials, cfdi):
        uuid_replace = move.l10n_mx_edi_cancel_invoice_id.l10n_mx_edi_cfdi_uuid
        return self._l10n_mx_edi_sw_cancel_service(
            move.l10n_mx_edi_cfdi_uuid,
            move.company_id,
            credentials,
            uuid_replace=uuid_replace,
    ...
    def _l10n_mx_edi_sw_cancel_service(
        self, uuid, company, credentials, uuid_replace=None
    ):
    ...
    def _is_required_for_invoice(self, invoice):
        ...
        return (
            ...
            and invoice.country_code == "MX"
            and invoice.company_id.currency_id.name == "MXN"
        )
    ...
    def _post_invoice_edi(self, invoices):
        ...
        for invoice in invoices:
            ...
            addenda = (
                invoice.partner_id.l10n_mx_edi_addenda
                or invoice.partner_id.commercial_partner_id.l10n_mx_edi_addenda
            )
            if addenda:
                ...
                res["cfdi_signed"] = self._l10n_mx_edi_cfdi_append_addenda(
                    invoice, res["cfdi_signed"], addenda
                )
            ...
            cfdi_attachment = self.env["ir.attachment"].create(
                {
                    ...
                    "description": _(
                        "Mexican invoice CFDI generated for the %s document."
                    )
                    % invoice.name,
                }
            )
    ...
    def _cancel_invoice_edi(self, invoices):
        ...
        for invoice in invoices:
            ...
            res = getattr(self, "_l10n_mx_edi_%s_cancel_invoice" % pac_name)(
                invoice, credentials, cfdi_data
            )
            if res.get("errors"):
                edi_result[invoice] = {
                    "error": self._l10n_mx_edi_format_error_message(
                        _("PAC failed to cancel the CFDI:"), res["errors"]
                    )
                }
                continue
    ...
    def _post_payment_edi(self, payments):
        ...
        for move in payments:
            ...
            cfdi_signed = (
                res["cfdi_signed"]
                if res["cfdi_encoding"] == "base64"
                else base64.encodebytes(res["cfdi_signed"])
            )
            cfdi_filename = (
                "%s-%s-MX-Payment-10.xml" % (move.journal_id.code, move.name)
            ).replace("/", "")
            cfdi_attachment = self.env["ir.attachment"].create(
                {
                    "name": cfdi_filename,
                    ...
                    "mimetype": "application/xml",
                    "description": _(
                        "Mexican payment CFDI generated for the %s document."
                    )
                    % move.name,
                }
            )
            ...
            if move.payment_id:
                move.payment_id.message_post(
                    body=message, attachment_ids=cfdi_attachment.ids
                )

        return edi_result

    def _cancel_payment_edi(
        self,
        moves,
    ):
        # OVERRIDE
        ...
        for move in moves:
            ...
            res = getattr(self, "_l10n_mx_edi_%s_cancel_payment" % pac_name)(
                move, credentials, cfdi_data
            )
            if res.get("errors"):
                edi_result[move] = {
                    "error": self._l10n_mx_edi_format_error_message(
