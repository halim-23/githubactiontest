...
class AccountMove(models.Model):
    ...
    l10n_mx_edi_certificate_id = fields.Many2one(
        comodel_name="l10n_mx_edi.certificate", string="Source Certificate"
    )
    ...
    def _get_l10n_mx_edi_signed_edi_document(self):
        ...
        return self.edi_document_ids.filtered(
            lambda document: document.edi_format_id == cfdi_3_3_edi
    ...
    def _l10n_mx_edi_decode_cfdi(self, cfdi_data=None):
        ...
        return {
            ...
            and get_cadena(tfd_node, CFDI_XSLT_CADENA_TFD)
            or get_cadena(cfdi_node, CFDI_XSLT_CADENA),
    ...
    def _compute_l10n_mx_edi_cfdi_request(self):
        for move in self:
            if move.country_code != "MX":
                ...
                move.payment_id
                and move.payment_id.payment_type == "inbound"
                and "PPD"
                in move._get_reconciled_invoices().mapped("l10n_mx_edi_payment_policy")
            ):
    ...
    def _compute_l10n_mx_edi_payment_method_id(self):
        for move in self:
            if move.l10n_mx_edi_payment_method_id:
                ...
                move.l10n_mx_edi_payment_method_id = self.env.ref(
    ...
    def _l10n_mx_edi_cron_update_sat_status(self):
        ...
        to_process = self.env["account.edi.document"].search(
            [
                ...
                (
                    "move_id.l10n_mx_edi_sat_status",
                    "in",
        ...
        to_process.filtered(
            ...
            and doc.move_id.l10n_mx_edi_sat_status == "cancelled"
    ...
    def _update_payments_edi_documents(self):
        ...
        for payment in self:
            if payment.l10n_mx_edi_cfdi_uuid:
                payment.l10n_mx_edi_origin = payment._l10n_mx_edi_write_cfdi_origin(
                    "04", [payment.l10n_mx_edi_cfdi_uuid]
                )
    ...
    def _post(self, soft=True):
        ...
        for move in self:
            ...
            tz_force = (
                ...
                .sudo()
                .get_param("l10n_mx_edi_tz_%s" % move.journal_id.id, default=None)
            )
            if tz_force:
    ...
    def button_draft(self):
        ...
        for move in self:
            if move.l10n_mx_edi_cfdi_uuid:
                move.l10n_mx_edi_origin = move._l10n_mx_edi_write_cfdi_origin(
                    "04", [move.l10n_mx_edi_cfdi_uuid]
                )
            if move.payment_id:
    ...
    def _reverse_moves(self, default_values_list, cancel=False):
        ...
        for default_vals, move in zip(default_values_list, self):
            if move.l10n_mx_edi_cfdi_uuid:
                default_vals[
                    "l10n_mx_edi_origin"
                ] = move._l10n_mx_edi_write_cfdi_origin(
                    "01", [move.l10n_mx_edi_cfdi_uuid]
                )
