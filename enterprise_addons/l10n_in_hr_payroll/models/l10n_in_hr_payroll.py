...
class HrContract(models.Model):
    ...
    tds = fields.Float(
        string="TDS", digits="Payroll", help="Amount for Tax Deduction at Source"
    )
    ...
    supplementary_allowance = fields.Float(
        string="Supplementary Allowance", digits="Payroll"
    )
...
class HrPayrollAdvice(models.Model):
        ...
        for advice in self:
            ...
            for slip in payslips:
                if (
                    ...
                    and not slip.employee_id.bank_account_id.acc_number
                ):
                    raise UserError(
                    ...
                    self.env["hr.payroll.advice.line"].create(
                        {
                            ...
                            "ifsc_code": slip.employee_id.bank_account_id.bank_bic
                            or "",
    ...
    def _onchange_company_id(self):
        self.bank_id = (
            ...
            and self.company_id.partner_id.bank_ids[0].bank_id.id
            or False
        )
            ...
            advice = self.env["hr.payroll.advice"].create(
                {
                    ...
                    "name": run.name,
                    "date": run.date_end,
                    "bank_id": company.partner_id.bank_ids
                    and company.partner_id.bank_ids[0].bank_id.id
                    or False,
                }
            )
