...
class HelpdeskTicket(models.Model):
    ...
    repairs_count = fields.Integer(
        "Repairs Count", compute="_compute_repairs_count", compute_sudo=True
    )
    repair_ids = fields.One2many(
        "repair.order", "ticket_id", string="Repairs", copy=False
    )
    ...
    def _compute_repairs_count(self):
        repair_data = (
            ...
            .sudo()
            .read_group([("ticket_id", "in", self.ids)], ["ticket_id"], ["ticket_id"])
        )
        mapped_data = {
            r["ticket_id"][0]: r["ticket_id_count"] for r in repair_data
        }
        for ticket in self:
...
class HelpdeskTeam(models.Model):
    ...
    def _compute_repairs_count(self):
        ...
        for team in self:
            team.repairs_count = sum(
                [val for key, val in mapped_data.items() if key in team.ticket_ids.ids]
            )
    ...
    def action_view_repairs(self):
        ...
        repair_ids = self.ticket_ids.repair_ids.filtered(
