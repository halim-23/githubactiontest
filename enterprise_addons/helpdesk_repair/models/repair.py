...
class Repair(models.Model):
    ...
    ticket_id = fields.Many2one(
        "helpdesk.ticket", string="Ticket", help="Related Helpdesk Ticket"
    )
    ...
    def write(self, vals):
        ...
        if "state" in vals:
            tracked_repairs = self.filtered(
                lambda r: r.ticket_id.use_product_repairs
                and r.state == "done"
                and previous_states[r] != r.state
            )
            ...
            for repair in tracked_repairs:
                body = (
                    ...
                    % (repair.id, repair.display_name)
                )
                repair.ticket_id.sudo().message_post(
                    subtype_id=subtype_id.id, body=body
                )
        return res
