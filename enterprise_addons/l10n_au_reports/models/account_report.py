...
class TaxReport(models.AbstractModel):
    ...
    def _get_data(self, options, raise_warning=False):
        ...
        for p in partner_values:
            if p.get("total_gst", 0) > p.get("gross_paid", 0) and raise_warning:
                raise UserError(
                    _("The total GST is higher than the Gross Paid for %s.", p["name"])
                )
    ...
    def _get_columns_name(self, options):
        return [
            {},
            {"name": _("ABN")},
            {"name": _("Total GST"), "class": "number"},
            {"name": _("Gross Paid"), "class": "number"},
            ...
            lines += [
                {
                    ...
                    "columns": [
                        {"name": row["vat"]},
                        {
                            "name": self.format_value(row["total_gst"]),
                            "no_format_name": row["total_gst"],
                        },
                        {
                            "name": self.format_value(row["gross_paid"]),
                            "no_format_name": row["gross_paid"],
                        },
                        {
                            "name": self.format_value(row["tax_withheld"]),
                            "no_format_name": row["tax_withheld"],
                        },
                    ],
                }
            ]
    ...
    def get_txt(self, options):
        ...
        lines = [
            self._sender_data_record_1(options, sender_data),
            self._sender_data_record_2(sender_data),
            self._sender_data_record_3(sender_data),
        ]
        lines += [
            self._payer_identity_data_record(options, sender_data),
            self._software_data_record(),
        ]
        ...
        for line in lines:
            if len(line) != 996:
                raise UserError(
                    _(
                        "There was an error while writing the file (line length not 996)."
    ...
    def _sender_data_record_1(self, options, data):
        return "%03d%-14s%-11s%-1s%-8s%-1s%-1s%-1s%-10s%-946s" % (
            ...
            int(data["vat"]),  # 6.3  M
            RUN_TYPE,  # 6.4  M
            fields.Date.to_date(options["date"]["date_to"]).strftime(
                "%d%m%Y"
            ),  # 6.5  M
            "P",  # 6.6  M
            "C",  # 6.7  M
    ...
    def _sender_data_record_3(self, data):
        return (
            "%03d%-14s%-38s%-38s%-27s%-3s%-4s%-20s%-38s%-38s%-27s%-3s%-4s%-20s%-76s%-643s"
            % (
                996,  # 6.1  M
    ...
    def _payer_identity_data_record(self, options, data):
        return (
            "%03d%-8s%011d%03d%-4s%-200s%-200s%-38s%-38s%-27s%-3s%-4s%-20s%-38s%-15s%-15s%-76s%-293s"
            % (
                996,  # 6.1  M
                "IDENTITY",  # 6.29 M
                int(data["vat"]),  # 6.30 M
    ...
    def _payee_data_record(self, data):
        ...
        return (
            "%03d%-6s%011d%-30s%-15s%-15s%-200s%-200s%-38s%-38s%-27s%-3s%-4s%-20s%-15s%-6s%-9s%011d%011d%011d%-1s%08d%-200s%-76s%-1s%-1s%-36s"
            % (
                996,  # 6.1  M
                "DPAIVS",  # 6.46 M
                int(data["vat"]),  # 6.47 M
    ...
    def _validate_partner(self, data, without_abn=False):
        ...
        if data["zip"] and (
            not (0 <= int(data["zip"]) <= 9999) or (len(data["zip"]) != 4)
        ):
        ...
        if data["state_code"] == "OTH" and data["zip"] != "9999":
            errors += [
                _("The postalcode must be 9999 because it is in overseas addresses")
            ]
    ...
    def open_invoices(self, options, params=None):
        ...
        tags = self.env.ref("l10n_au.service_tag") + self.env.ref(
            "l10n_au.tax_withheld_tag"
        )
        return {
            ...
            "domain": [
                ...
                ("line_ids.tax_tag_ids", "in", tags.ids),
            ],
        }
