...
class pos_config(models.Model):
    ...
    blackbox_pos_production_id = fields.Char(
        "Registered IoT Box serial number",
    ...
    @api.constrains(
        "module_pos_discount", "module_pos_loyalty", "blackbox_pos_production_id"
    )
    def _check_blackbox_config(self):
        for config in self:
            if config.blackbox_pos_production_id and (
                config.module_pos_discount or config.module_pos_loyalty
            ):
                raise UserError(
                    _(
                        "Loyalty programs, reprint and global discounts cannot be used on a PoS associated with a blackbox."
                    )
                )
            ...
            if config.blackbox_pos_production_id:
                if len(config.blackbox_pos_production_id) != 14:
                    raise ValidationError(
                        _("Serial number must consist of 14 characters.")
                    )
    ...
    @api.constrains(
        "blackbox_pos_production_id", "iface_fiscal_data_module", "is_posbox"
    )
    def _check_iot_and_blackbox_state(self):
        for config in self:
            if (
                (
                    config.blackbox_pos_production_id
                    and not config.iface_fiscal_data_module
                )
                or (
                    not config.blackbox_pos_production_id
                    and config.iface_fiscal_data_module
                )
                or (
                    config.blackbox_pos_production_id
                    and config.iface_fiscal_data_module
                    and not config.is_posbox
                )
            ):
                raise ValidationError(
                    _(
                        "The certified PosBox Ref and the fiscal data module fields have to be set together or ignored."
                    )
                )
...
class res_users(models.Model):
    ...
    def _check_insz_or_bis_number(self):
        for rec in self:
            if rec.insz_or_bis_number and (
                len(rec.insz_or_bis_number) != 11
                or not rec.insz_or_bis_number.isdigit()
            ):
                raise ValidationError(
                    _("The INSZ or BIS number has to consist of 11 numerical digits.")
                )
...
class pos_session(models.Model):
            ...
            for order in rec.order_ids:
                for line in order.lines:
                    if line.discount > 0:
                        ...
                        price_without_discount = line.currency_id.round(
                            line.price_subtotal_incl / (1 - (line.discount / 100))
                        )
                        rec.total_discount += (
    ...
    def action_pos_session_closing_control(self):
        ...
        pro_forma_orders = self.env["pos.order_pro_forma"].search(
            [("session_id", "=", self.id)]
        )
        ...
        if non_finalized_orders:
            raise UserError(
                _(
                    "Your session still contains open orders (%s). Please close all of them first."
                )
                % ", ".join(non_finalized_orders)
            )
                    ...
                    if order.plu_hash == "cf44878a":
                        ...
                        for line in order.lines:
                            ...
                            if key in total_sold_per_category:
                                ...
                                total_sold_per_category[key] = line.price_subtotal_incl
                        current["revenue_per_category"] = list(
                            total_sold_per_category.items()
                        )
        return data
...
class pos_order(models.Model):
    ...
    def create(self, values):
        ...
        if pos_session.config_id.blackbox_pos_production_id and not values.get(
            "blackbox_signature"
        ):
        ...
        if order.blackbox_signature:
            ...
            self.env["pos_blackbox_be.log"].sudo().create(
                description, "create", self._name, order.pos_reference
            )
            ...
            if order.config_id.blackbox_pos_production_id and order.state != "draft":
                ...
                for field in values:
                    if field not in white_listed_fields:
                        raise UserError(
                            _("Modifying registered orders is not allowed.")
                        )
        ...
        fields.update(
            {
                ...
                "blackbox_unique_fdm_production_number": ui_order.get(
                    "blackbox_unique_fdm_production_number"
                ),
                "blackbox_vsc_identification_number": ui_order.get(
    ...
    def create_from_ui(self, orders, draft=False):
        ...
        pro_forma_orders = [
            order["data"] for order in orders if order["data"].get("blackbox_pro_forma")
        ]

        # filter the pro_forma orders out of the orders list
        regular_orders = [
            order for order in orders if not order["data"].get("blackbox_pro_forma")
        ]
...
class pos_order_pro_forma(models.Model):
    ...
    pricelist_id = fields.Many2one(
        "product.pricelist", "Pricelist", default=_default_pricelist, readonly=True
    )
    ...
    pos_version = fields.Char(
        help="Version of Odoo that created the order", readonly=True
    )
    pos_production_id = fields.Char(
        help="Unique ID of Odoo that created this order", readonly=True
    )
    terminal_id = fields.Char(
        help="Unique ID of the POS that created this order", readonly=True
    )
    hash_chain = fields.Char()
    ...
    def set_values(self, ui_order):
        return {
            ...
            "blackbox_date": ui_order.get("blackbox_date"),
            "blackbox_time": ui_order.get("blackbox_time"),
            "blackbox_pos_receipt_time": parser.parse(
...
class product_template(models.Model):
    ...
    def create(self, values):
        self.env["pos_blackbox_be.log"].sudo().create(
            values, "create", self._name, values.get("name")
        )
    ...
    def write(self, values):
        ...
        for product in self:
            self.env["pos_blackbox_be.log"].sudo().create(
                values, "modify", product._name, product.name
            )
    ...
    def _unlink_except_master_data(self):
        work_in = self.env.ref(
            "pos_blackbox_be.product_product_work_in"
        ).product_tmpl_id.id
        work_out = self.env.ref(
    ...
    def unlink(self):
        for product in self:
            self.env["pos_blackbox_be.log"].sudo().create(
                {}, "delete", product._name, product.name
            )
        return super().unlink()
...
class AccountChartTemplate(models.Model):
    ...
    def _load(self, sale_tax_rate, purchase_tax_rate, company):
        super(AccountChartTemplate, self)._load(
        ...
        if self == self.env.ref("l10n_be.l10nbe_chart_template"):
            ...
            if not taxes.active:
                taxes.active = True
            work_in.with_context(install_mode=True).with_company(company.id).write(
                {"taxes_id": [(4, taxes.id)]}
            )
            work_out.with_context(install_mode=True).with_company(company.id).write(
                {"taxes_id": [(4, taxes.id)]}
            )
