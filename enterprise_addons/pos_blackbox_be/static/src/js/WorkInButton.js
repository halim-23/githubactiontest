odoo.define("pos_blackbox_be.WorkInButton", function (require) {
  ...
  class WorkInButton extends PosComponent {
    ...
    async willStart() {
      this.state.status = await this.get_user_session_status(
        this.env.pos.pos_session.id,
        this.env.pos.pos_session.user_id[0]
      );
    }
    async onClick() {
      const clocked = await this.get_user_session_status(
        this.env.pos.pos_session.id,
        this.env.pos.pos_session.user_id[0]
      );
      if (!this.state.status && !clocked) this.ClockIn();
      if (this.state.status && clocked) this.ClockOut();
    }
    async ClockIn() {
      const users_logged = await this.set_user_session_status(
        this.env.pos.pos_session.id,
        this.env.pos.pos_session.user_id[0],
        true
      );
    ...
    async ClockOut() {
      const unpaid_tables = this.env.pos.db
        .load("unpaid_orders", [])
        .filter(function (order) {
          return order.data.amount_total > 0;
        })
        .map(function (order) {
          return order.data.table;
        });
      if (unpaid_tables.length > 0) {
        await this.showPopup("ErrorPopup", {
          title: this.env._t("Fiscal Data Module error"),
          body: this.env._t(
            "Tables %s still have unpaid orders. You will not be able to clock out until all orders have been paid."
          ),
        });
        return;
      }

      const userLogOut = await this.set_user_session_status(
        this.env.pos.pos_session.id,
        this.env.pos.pos_session.user_id[0],
        false
      );
