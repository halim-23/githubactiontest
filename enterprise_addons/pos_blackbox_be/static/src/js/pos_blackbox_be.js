...
odoo.define("pos_blackbox_be.pos_blackbox_be", function (require) {
  ...
  models.Orderline = models.Orderline.extend({
    ...
    _generate_translation_table: function () {
      ...
      var lowercase_to_uppercase = _.range(
        "a".charCodeAt(0),
        "z".charCodeAt(0) + 1
      ).map(function (lowercase_ascii_code) {
        return [
          String.fromCharCode(lowercase_ascii_code),
          String.fromCharCode(lowercase_ascii_code).toUpperCase(),
        ];
      });
    ...
    can_be_merged_with: function (orderline) {
      ...
      var last_id = Object.keys(order.orderlines._byId)[
        Object.keys(order.orderlines._byId).length - 1
      ];

      if (
        this.pos.useBlackBoxBe() &&
        (order.orderlines._byId[last_id].product.id !== orderline.product.id ||
          order.orderlines._byId[last_id].quantity < 0) &&
        this.blackbox_pro_forma_finalized
      ) {
        return false;
      }
    ...
    _show_finalized_error: async function () {
      await Gui.showPopup("ErrorPopup", {
        ...
        body: _t(
          "This orderline has already been finalized in a pro forma order and can no longer be modified. Please create a new line with eg. a negative quantity."
        ),
      });
    },
  ...
  models.PosModel = models.PosModel.extend({
    ...
    _check_iotbox_serial: function (data) {
      ...
      if (!data.value) {
        ...
        "BODO001" + data.value.toUpperCase() !=
        this.config.blackbox_pos_production_id.toUpperCase()
      ) {
        this.proxy._show_could_not_connect_error(
          _t("Incorrect PosBox serial") +
            " " +
            this.config.blackbox_pos_production_id.toUpperCase()
        );
      } else {
        this.chrome.ready.then(function () {
          $(self.chrome.$el)
            .find(".placeholder-posVersion")
            .text(" Ver: " + self.version.server_version + "1807BE_FDM");
          var current = $(self.chrome.$el).find(".placeholder-posID").text();
          $(self.chrome.$el)
            .find(".placeholder-posID")
            .text(" ID: " + self.config.blackbox_pos_production_id);
        });
      }
    },
    ...
    push_order_to_blackbox: function (order) {
      ...
      var prom = this.proxy
        .request_fdm_hash_and_sign(packet)
        .then(function (parsed_response) {
          return new Promise(function (resolve, reject) {
            if (parsed_response) {
              // Put fields that we need on tickets on order
              order.blackbox_order_name =
                self.config.name + "/" + self.config.backend_sequence_number;
              ...
              order.blackbox_ticket_counters = self._prepare_ticket_counter_for_ticket(
                parsed_response.vsc_ticket_counter,
                parsed_response.vsc_total_ticket_counter,
                parsed_response.event_label
              );
              order.blackbox_signature = parsed_response.signature;
              order.blackbox_vsc_identification_number =
                parsed_response.vsc_identification_number;
              order.blackbox_unique_fdm_production_number =
                parsed_response.fdm_unique_production_number;
              order.blackbox_plu_hash = self._prepare_hash_for_ticket(
                packet.fields[packet.fields.length - 1].content
              );
              order.blackbox_pos_version =
                "Odoo " + self.version.server_version + "1807BE_FDM";
              ...
              self.config.blackbox_most_recent_hash = self._prepare_hash_for_ticket(
                Sha1.hash(
                  self.config.blackbox_most_recent_hash + order.blackbox_plu_hash
                )
              );
    ...
    _push_pro_forma: function () {
      ...
      if (
        old_order &&
        old_order.get_orderlines().length &&
        old_order.blackbox_pro_forma !== false
      ) {
    ...
    _split_discount_lines: function () {
      ...
      order.get_orderlines().forEach(function (line) {
        ...
        if (line.reward_id && line.get_price_with_tax() < 0) {
          ...
          var discount_percentage_on_order = Math.abs(
            discount_line.get_price_with_tax() / order_total
          );
          ...
          _.forEach(_.pairs(price_per_tax_letter), function (tax) {
            tax[1] /= 100; // Was in eurocents
            if (tax[1] > 0.00001) {
              var percentage_of_this_tax_in_total = round_pr(
                tax[1] / order_total,
                0.01
              );
      ...
      _.map(lines_to_delete, function (line) {
        self.get_order().remove_orderline(line);
      });
      _.map(lines_to_add, function (line) {
        self.get_order().add_product.apply(self.get_order(), line);
      });
    },
  ...
  models.Order = models.Order.extend({
    export_as_JSON: function () {
      ...
      var to_return = _.extend(json, {
        ...
        blackbox_unique_fdm_production_number: this
      ...
      if (this.blackbox_base_price_in_euro_per_tax_letter) {
        to_return = _.extend(to_return, {
          blackbox_tax_category_a: this.blackbox_base_price_in_euro_per_tax_letter[0]
            .amount,
          blackbox_tax_category_b: this.blackbox_base_price_in_euro_per_tax_letter[1]
            .amount,
          blackbox_tax_category_c: this.blackbox_base_price_in_euro_per_tax_letter[2]
            .amount,
          blackbox_tax_category_d: this.blackbox_base_price_in_euro_per_tax_letter[3]
            .amount,
        });
      }
      ...
      if (this.blackbox_pos_receipt_time) {
        ...
        to_return.blackbox_pos_receipt_time = this.blackbox_pos_receipt_time.format(
    ...
    add_product: async function (product, options) {
      if (
        this.pos.useBlackBoxBe() &&
        !this.pos.check_if_user_clocked() &&
        product !== this.pos.work_in_product
      ) {
        await Gui.showPopup("ErrorPopup", {
          title: _t("POS error"),
    ...
    get_total_with_tax_without_discounts: function () {
      ...
      var total_without_tax = round_pr(
        positive_orderlines.reduce(function (sum, orderLine) {
          return sum + orderLine.get_price_without_tax();
        }, 0),
        this.pos.currency.rounding
      );

      var total_tax = round_pr(
        positive_orderlines.reduce(function (sum, orderLine) {
    ...
    get_price_in_eurocent_per_tax_letter: function (base) {
      ...
      this.get_orderlines().forEach(function (current, index, array) {
        ...
        if (tax_letter) {
          if (base) {
            price_per_tax_letter[tax_letter] += Math.round(
              current.get_price_without_tax() * 100
            );
          } else {
            price_per_tax_letter[tax_letter] += Math.round(
              current.get_price_with_tax() * 100
            );
          }
        }
      });
    ...
    get_base_price_in_euro_per_tax_letter_list: function () {
      var base_price_per_tax_letter = this.get_price_in_eurocent_per_tax_letter(
        "base price"
      );
      var base_price_per_tax_letter_list = _.map(
        _.keys(base_price_per_tax_letter),
        function (key) {
          return {
            letter: key,
  ...
  devices.ProxyDevice.include({
    ...
    build_request: function (id) {
      ...
      packet.add_field(
        new FDMPacketField(
          "sequence number",
          2,
          this._get_sequence_number().toString(),
          "0"
        )
      );
    ...
    _handle_fdm_errors: async function (parsed_response, ignore_non_critical) {
      ...
      if (error_1 === 0) {
        ...
        return true;
      } else if (error_1 === 1 && !ignore_non_critical) {
        // Warnings
        if (error_2 === 1) {
          await Gui.showPopup("ErrorPopup", {
    ...
    _build_fdm_hash_and_sign_request: function (order) {
      ...
      packet.add_field(
        new FDMPacketField(
          "ticket date",
          8,
          order.blackbox_pos_receipt_time.format("YYYYMMDD")
        )
      );
      packet.add_field(
        new FDMPacketField(
          "ticket time",
          6,
          order.blackbox_pos_receipt_time.format("HHmmss")
        )
      );
      packet.add_field(
        new FDMPacketField("insz or bis number", 11, insz_or_bis_number)
      );
      packet.add_field(
        new FDMPacketField(
          "production number POS",
          14,
          this.pos.config.blackbox_pos_production_id
        )
      );
      packet.add_field(
        new FDMPacketField(
          "ticket number",
          6,
          (++this.pos.config.backend_sequence_number).toString(),
          " "
        )
      );
      ...
      packet.add_field(
        new FDMPacketField(
          "total amount to pay in eurocent",
          11,
          this._amount_to_fdm_amount_string(order.blackbox_amount_total),
          " "
        )
      );
      ...
      packet.add_field(
        new FDMPacketField(
          "amount at tax percentage 1 in eurocent",
          11,
          this._amount_to_fdm_amount_string(
            order.blackbox_base_price_in_euro_per_tax_letter[0].amount
          ),
          " "
        )
      );
      ...
      packet.add_field(
        new FDMPacketField(
          "amount at tax percentage 2 in eurocent",
          11,
          this._amount_to_fdm_amount_string(
            order.blackbox_base_price_in_euro_per_tax_letter[1].amount
          ),
          " "
        )
      );
      ...
      packet.add_field(
        new FDMPacketField(
          "amount at tax percentage 3 in eurocent",
          11,
          this._amount_to_fdm_amount_string(
            order.blackbox_base_price_in_euro_per_tax_letter[2].amount
          ),
          " "
        )
      );
      ...
      packet.add_field(
        new FDMPacketField(
          "amount at tax percentage 4 in eurocent",
          11,
          this._amount_to_fdm_amount_string(
            order.blackbox_base_price_in_euro_per_tax_letter[3].amount
          ),
          " "
        )
      );
    ...
    request_fdm_identification: function () {
      ...
      return new Promise(function (resolve, reject) {
        fdm.add_listener(
          self._check_and_parse_fdm_identification_response.bind(self, resolve, reject)
        );
        fdm.action({
          action: "request",
    ...
    _check_and_parse_fdm_hash_and_sign_response: function (
      resolve,
      reject,
      hide_error,
      data
