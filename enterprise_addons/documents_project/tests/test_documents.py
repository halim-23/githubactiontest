...
class TestCaseDocumentsBridgeProject(TransactionCase):
    ...
    def test_bridge_folder_workflow(self):
        ...
        self.assertEqual(
            self.attachment_txt.res_model,
            "project.task",
            "failed at workflow_bridge_documents_project" " new res_model",
        )
        task = self.env["project.task"].search(
            [("id", "=", self.attachment_txt.res_id)]
        )
        self.assertTrue(
            task.exists(), "failed at workflow_bridge_documents_project task"
        )
        self.assertEqual(
            self.attachment_txt.res_id,
            task.id,
            "failed at workflow_bridge_documents_project res_id",
        )
    ...
    def test_bridge_project_project_settings_on_write(self):
        ...
        task_test = self.env["project.task"].create(
            {"name": "task_test", "company_id": company_test.id}
        )
        project_test = self.env["project.project"].create(
            {"name": "project_test", "company_id": company_test.id}
        )
        ...
        attachment_txt_test.write(
            {"res_model": "project.project", "res_id": project_test.id}
        )
        attachment_gif_test.write({"res_model": "project.task", "res_id": task_test.id})

        txt_doc = self.env["documents.document"].search(
            [("attachment_id", "=", attachment_txt_test.id)]
        )
        gif_doc = self.env["documents.document"].search(
            [("attachment_id", "=", attachment_gif_test.id)]
        )
        ...
        self.assertEqual(
            gif_doc.folder_id, folder_test, "the gif test document have a folder"
        )
