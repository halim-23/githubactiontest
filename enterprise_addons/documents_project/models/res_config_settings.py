...
class ResConfigSettings(models.TransientModel):
    ...
    documents_project_settings = fields.Boolean(
        related="company_id.documents_project_settings",
        readonly=False,
        string="Project",
    )
    project_folder = fields.Many2one(
        "documents.folder",
        related="company_id.project_folder",
        readonly=False,
        string="project default workspace",
    )
    project_tags = fields.Many2many(
        "documents.tag",
        "project_tags_table",
        related="company_id.project_tags",
        readonly=False,
        string="Project Tags",
    )
