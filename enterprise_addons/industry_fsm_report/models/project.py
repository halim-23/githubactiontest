...
class Project(models.Model):
    ...
    allow_worksheets = fields.Boolean(
        "Worksheets",
        compute="_compute_allow_worksheets",
        store=True,
        readonly=False,
    ...
    def _compute_worksheet_template_id(self):
        default_worksheet = self.env.ref(
            "industry_fsm_report.fsm_worksheet_template", False
        )
        for project in self:
...
class Task(models.Model):
    ...
    worksheet_signed_by = fields.Char(
        "Signed By", help="Name of the person that signed the task.", copy=False
    )
    worksheet_color = fields.Integer(related="worksheet_template_id.color")
    display_sign_report_primary = fields.Boolean(
        compute="_compute_display_sign_report_buttons"
    )
    display_sign_report_secondary = fields.Boolean(
        compute="_compute_display_sign_report_buttons"
    )
    display_send_report_primary = fields.Boolean(
        compute="_compute_display_send_report_buttons"
    )
    display_send_report_secondary = fields.Boolean(
        compute="_compute_display_send_report_buttons"
    )
    ...
    def _compute_worksheet_count(self):
        ...
        for record in self:
            ...
            if record.worksheet_template_id:
                ...
                worksheet_count = Worksheet.search_count(
                    [("x_project_task_id", "=", record.id)]
                )
    ...
    def action_fsm_worksheet(self):
        ...
        worksheet = self.env[self.worksheet_template_id.sudo().model_id.model].search(
            [("x_project_task_id", "=", self.id)]
        )
    ...
    def action_preview_worksheet(self):
        ...
        if not self.worksheet_template_id:
            raise UserError(
                _("To send the report, you need to select a worksheet template.")
            )
    ...
    def action_send_report(self):
        ...
        return {
            ...
            "context": {
                ...
                if len(tasks_with_report.ids) > 1
                else "comment",
...
class ProjectTaskRecurrence(models.Model):
    ...
    def _get_recurring_fields(self):
        return ["worksheet_template_id"] + super(
