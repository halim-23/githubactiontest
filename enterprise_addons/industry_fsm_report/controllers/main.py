...
class WebStudioController(main.WebStudioController):
    ...
    def edit_view(self, view_id, studio_view_arch, operations=None):
        action = super(WebStudioController, self).edit_view(
            view_id, studio_view_arch, operations
        )
        model = request.env["ir.ui.view"].browse(view_id).model
        worksheet_template_to_change = (
            request.env["worksheet.template"].sudo().search([("model_id", "=", model)])
        )
