...
class CustomerPortal(portal.CustomerPortal):
        ...
        **kw
        ...
        if task_sudo.worksheet_template_id:
            ...
            worksheet = (
                ...
                .sudo()
                .search(
                    [("x_project_task_id", "=", task_sudo.id)],
                    limit=1,
        ...
        return request.render(
            "industry_fsm_report.portal_my_worksheet",
            {
    ...
    @http.route(
        ["/my/task/<int:task_id>/worksheet/sign/<string:source>"],
        type="json",
        auth="public",
        website=True,
    )
    def portal_worksheet_sign(
        ...
        if not task_sudo.has_to_be_signed():
            return {
                "error": _(
                    "The worksheet is not in a state requiring customer signature."
                )
            }
        if not signature:
        ...
        pdf = (
            request.env.ref("industry_fsm_report.task_custom_report")
            .with_user(SUPERUSER_ID)
            ._render_qweb_pdf([task_sudo.id])[0]
        )
        task_sudo.message_post(
            body=_("The worksheet has been signed"),
        ...
        return {
            "force_refresh": True,
            "redirect_url": task_sudo.get_portal_url(
                suffix="/worksheet/%s" % source, query_string=query_string
            ),
        }
