...
class ResourceCalendar(models.Model):
    ...
    hours_per_week = fields.Float(
        compute="_compute_hours_per_week", string="Hours per Week", store=True
    )
    full_time_required_hours = fields.Float(
        string="Fulltime Hours",
        help="Number of hours to work to be considered as fulltime.",
    )
    ...
    @api.depends(
        ...
        "attendance_ids.work_entry_type_id.is_leave",
    )
    def _compute_hours_per_week(self):
        for calendar in self:
            ...
            calendar.hours_per_week = (
                sum_hours / 2 if calendar.two_weeks_calendar else sum_hours
            )
        ...
        return len(
            set(
                self.attendance_ids.filtered(
                    lambda a: not a.work_entry_type_id.is_leave
                ).mapped("dayofweek")
            )
        )
    ...
    def _compute_is_fulltime(self):
        for calendar in self:
            calendar.is_fulltime = not float_compare(
    ...
    @api.depends(
        "hours_per_week",
        "full_time_required_hours",
        "attendance_ids.work_entry_type_id",
    )
        ...
        for calendar in self:
            if not calendar.hours_per_week:
                ...
                if calendar.full_time_required_hours:
                    ...
                    calendar.work_time_rate = (
                        calendar.hours_per_week / calendar.hours_per_week * 100
                    )
