...
class HrPayslip(models.Model):
    ...
    struct_type_id = fields.Many2one(
        "hr.payroll.structure.type", related="struct_id.type_id"
    )
    ...
    worked_days_line_ids = fields.One2many(
        ...
        copy=True,
        compute="_compute_worked_days_line_ids",
        store=True,
        readonly=False,
        states={
            "done": [("readonly", True)],
    ...
    warning_message = fields.Char(
        compute="_compute_warning_message", store=True, readonly=False
    )
    is_regular = fields.Boolean(compute="_compute_is_regular")
    has_negative_net_to_report = fields.Boolean()
    negative_net_to_report_display = fields.Boolean(
        compute="_compute_negative_net_to_report_display"
    )
    negative_net_to_report_message = fields.Char(
        compute="_compute_negative_net_to_report_display"
    )
    negative_net_to_report_amount = fields.Float(
        compute="_compute_negative_net_to_report_display"
    )
    ...
    salary_attachment_count = fields.Integer(
        "Salary Attachment count", compute="_compute_salary_attachment_count"
    )

    @api.depends(
        "employee_id", "contract_id", "struct_id", "date_from", "date_to", "struct_id"
    )
    def _compute_input_line_ids(self):
        ...
        for slip in self:
            if (
                not slip.employee_id
                or not slip.employee_id.salary_attachment_ids
                or not slip.struct_id
            ):
            ...
            if slip.employee_id.salary_attachment_ids:
                lines_to_keep = slip.input_line_ids.filtered(
                    lambda x: x.input_type_id.id not in attachment_type_ids
                )
                input_line_vals = [Command.clear()] + [
                    Command.link(line.id) for line in lines_to_keep
                ]
                ...
                struct_deduction_lines = list(
                    set(slip.struct_id.rule_ids.mapped("code"))
                )
                included_deduction_types = [
                    f
                    for f in deduction_types
                    if attachment_types[f].code in struct_deduction_lines
                ]
                for deduction_type in included_deduction_types:
                    if not slip.struct_id.rule_ids.filtered(
                        lambda r: r.active
                        and r.code == attachment_types[deduction_type].code
                    ):
                        continue
                    attachments = valid_attachments.filtered(
                        lambda a: a.deduction_type == deduction_type
                    )
    ...
    def _compute_salary_attachment_ids(self):
        ...
        for slip in self:
            ...
            if slip.employee_id and slip.input_line_ids:
                ...
                deduction_types = [
                    f
                    for f in attachment_types
                    if attachment_types[f].id in input_line_type_ids
                ]
        ...
        for payslip in self:
            if payslip.state in ["draft", "verify"]:
                ...
                payslip.negative_net_to_report_amount = (
                    payslips_to_report._get_line_values(["NET"], compute_sum=True)[
                        "NET"
                    ]["sum"]["total"]
                )
                ...
                if (
                    payslips_to_report
                    and payslip.state == "verify"
                    and payslip.contract_id
                    and not payslip.activity_ids.filtered(
                    ...
                    payslip.activity_schedule(
                        ...
                        note=_(
                            'At least one previous negative net could be reported on this payslip for <a href="#" data-oe-model="%s" data-oe-id="%s">%s</a>'
                        )
                        % (
                            payslip.employee_id._name,
                            payslip.employee_id.id,
                            payslip.employee_id.display_name,
                        ),
                        user_id=payslip.contract_id.hr_responsible_id.id
    ...
    def action_report_negative_amount(self):
        ...
        deduction_input_line = self.input_line_ids.filtered(
            lambda l: l.input_type_id == deduction_input_type
        )
    ...
    def _compute_is_regular(self):
        for payslip in self:
            payslip.is_regular = (
                payslip.struct_id.type_id.default_struct_id == payslip.struct_id
            )
    ...
    def _compute_line_ids(self):
        ...
        for payslip in self.filtered(
            lambda p: p.line_ids and p.state in ["draft", "verify"]
        ):
            payslip.line_ids = [(5, 0, 0)] + [
                (0, 0, line_vals) for line_vals in payslip._get_payslip_lines()
            ]
    ...
    def _compute_is_superuser(self):
        self.update(
            {
                "is_superuser": self.env.user._is_superuser()
        ...
        if "state" in vals and vals["state"] == "paid":
            ...
            for slip in self.filtered(lambda r: r.salary_attachment_ids):
                for deduction_type, input_type_id in attachment_types.items():
                    attachments = slip.salary_attachment_ids.filtered(
                        lambda r: r.deduction_type == deduction_type
                    )
                    input_lines = slip.input_line_ids.filtered(
                        lambda r: r.input_type_id.id == input_type_id.id
                    )
                    # Use the amount from the computed value in the payslip lines not the input
                    salary_lines = slip.line_ids.filtered(
                        lambda r: r.code in input_lines.mapped("code")
                    )
    ...
    def _generate_pdf(self):
        ...
        for report, payslips in mapped_reports.items():
            for payslip in payslips:
                ...
                if template:
                    template.send_mail(
                        payslip.id, notif_layout="mail.mail_notification_light"
                    )
        ...
        if contract.resource_calendar_id:
            ...
            if out_days or out_hours:
                work_entry_type = self.env.ref(
                    "hr_payroll.hr_work_entry_type_out_of_contract"
                )
                res.append(
                    {
                        "sequence": work_entry_type.sequence,
    ...
    def _compute_worked_days_line_ids(self):
        ...
        generate_to = max(
            min(fields.Date.to_date(p.date_to), current_month_end) for p in self
        )
    ...
    def _get_salary_line_total(self, code):
        _logger.warning(
            "The method _get_salary_line_total is deprecated in favor of _get_line_values"
        )
    ...
    def _get_salary_line_quantity(self, code):
        _logger.warning(
            "The method _get_salary_line_quantity is deprecated in favor of _get_line_values"
        )
        lines = self.line_ids.filtered(lambda line: line.code == code)
    ...
    def _get_line_values(self, code_list, vals_list=None, compute_sum=False):
        ...
        """
            % (selected_fields, "%s", "%s"),
            (tuple(self.ids), tuple(code_list)),
        )
        # self = hr.payslip(1, 2)
        # request_rows = [
    ...
    def action_print_payslip(self):
        return {
            ...
            "url": "/print/payslips?list_ids=%(list_ids)s"
    ...
    def _is_outside_contract_dates(self):
        ...
        payslip = self
        contract = self.contract_id
        return contract.date_start > payslip.date_to or (
            contract.date_end and contract.date_end < payslip.date_from
        )
