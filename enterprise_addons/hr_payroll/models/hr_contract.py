...
class HrContract(models.Model):
    ...
    schedule_pay = fields.Selection(
        related="structure_type_id.default_struct_id.schedule_pay", depends=()
    )
    ...
    calendar_changed = fields.Boolean(
        help="Whether the previous or next contract has a different schedule or not"
    )

    def _compute_payslips_count(self):
        count_data = self.env["hr.payslip"].read_group(
            [("contract_id", "in", self.ids)], ["contract_id"], ["contract_id"]
        )
        mapped_counts = {
            cd["contract_id"][0]: cd["contract_id_count"] for cd in count_data
        }
    ...
    def _is_same_occupation(self, contract):
        ...
        return (
            ...
            and work_time_rate == contract.resource_calendar_id.work_time_rate
        )
    ...
    def _get_occupation_dates(self, include_future_contracts=False):
        ...
        for contract in self:
            ...
            history = self.env["hr.contract.history"].search(
            ...
            all_contracts = history.contract_ids.filtered(
                lambda c: (
                    ...
                    and c != contract
                    and (
                        c.state in ["open", "close"]
                        or (
                            include_future_contracts
                            and c.state == "draft"
                            and c.date_start >= date_today
                        )
                    )
                )
            )  # hr.contract(29, 37, 38, 39, 41) -> hr.contract(29, 37, 39, 41)
            before_contracts = all_contracts.filtered(
                lambda c: c.date_start < contract.date_start
            )  # hr.contract(39, 41)
            after_contracts = all_contracts.filtered(
                lambda c: c.date_start > contract.date_start
            ).sorted(
                key="date_start"
            )  # hr.contract(37, 29)
    ...
    def _compute_calendar_changed(self):
        ...
        contract_resets = self.filtered(
            lambda c: (
                not c.date_start
                or not c.employee_id
                or not c.resource_calendar_id
                or not c.active
                or not (
                    c.state in ("open", "close")
                    or (c.state == "draft" and c.date_start >= date_today)
                )  # make sure to include futur contracts
            )
        )
        contract_resets.filtered(lambda c: c.calendar_changed).write(
        ...
        contract_changed.filtered(lambda c: not c.calendar_changed).write(
            {"calendar_changed": True}
        )
        (self - contract_changed).filtered(lambda c: c.calendar_changed).write(
            {"calendar_changed": False}
        )
    ...
    def _recompute_calendar_changed(self, employee_ids):
        contract_ids = self.search(
            [("employee_id", "in", employee_ids.ids)], order="date_start asc"
        )
        if not contract_ids:
            return
    ...
    def _index_contracts(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "hr_payroll.action_hr_payroll_index"
        )
    ...
    def _get_work_hours(self, date_from, date_to, domain=None):
        ...
        for work_entry in work_entries:
            ...
            if work_entry.work_entry_type_id.is_leave:
                ...
                work_data[work_entry.work_entry_type_id.id] += (
    ...
    def _get_default_work_entry_type(self):
        return (
            self.structure_type_id.default_work_entry_type_id
            or super(HrContract, self)._get_default_work_entry_type()
        )
    ...
    def write(self, vals):
            ...
            self.env["hr.payslip"].search(
                [
                    (
                        "contract_id",
                        "in",
                        self.filtered(lambda c: c.state != "cancel").ids,
                    ),
        ...
        if any(
            key in vals
            for key in ("state", "date_start", "resource_calendar_id", "employee_id")
        ):
