...
class HrPayrollStructureType(models.Model):
    ...
    default_struct_id = fields.Many2one(
        "hr.payroll.structure", string="Regular Pay Structure"
    )
    ...
    struct_type_count = fields.Integer(
        compute="_compute_struct_type_count", string="Structure Type Count"
    )
    ...
    def _check_country(self, vals):
        ...
        if country_id and country_id not in self.env.companies.mapped("country_id").ids:
            raise UserError(
                _(
                    "You should also be logged into a company in %s to set this country.",
