...
class HrPayrollEditPayslipLinesWizard(models.TransientModel):
    ...
    def recompute_following_lines(self, line_id):
        ...
        for line in sorted(self.line_ids, key=lambda x: x.sequence):
            if remove_lines and line.code in self.payslip_id.line_ids.mapped("code"):
                ...
                result_rules_dict[line.code] = {
                    "total": line.total,
                    "amount": line.amount,
                    "quantity": line.quantity,
                }
                localdict = line.salary_rule_id.category_id._sum_salary_rule_category(
                    localdict, line.total
                )

        payslip = self.payslip_id.with_context(
            force_payslip_localdict=localdict,
            prevent_payslip_computation_line_ids=blacklisted_rule_ids,
        )
    ...
    def action_validate_edition(self):
        ...
        for wizard in self:
            ...
            worked_days_lines = [
                (0, 0, line)
                for line in wizard.worked_days_line_ids._export_to_worked_days_line()
            ]
            ...
            wizard.payslip_id.message_post(
                body=_(
                    "This payslip has been manually edited by %s.", self.env.user.name
                )
            )
...
class HrPayrollEditPayslipLine(models.TransientModel):
    ...
    salary_rule_id = fields.Many2one(
        "hr.salary.rule", string="Rule", domain="[('struct_id', '=', struct_id)]"
    )
    ...
    slip_id = fields.Many2one(
        related="edit_payslip_lines_wizard_id.payslip_id", string="Pay Slip"
    )
    struct_id = fields.Many2one(related="slip_id.struct_id")
    category_id = fields.Many2one(
        related="salary_rule_id.category_id", readonly=True, store=True
    )

    edit_payslip_lines_wizard_id = fields.Many2one(
        "hr.payroll.edit.payslip.lines.wizard", required=True, ondelete="cascade"
    )
...
class HrPayrollEditPayslipWorkedDaysLine(models.TransientModel):
    ...
    slip_id = fields.Many2one(
        related="edit_payslip_lines_wizard_id.payslip_id", string="PaySlip"
    )
    sequence = fields.Integer("Sequence")
