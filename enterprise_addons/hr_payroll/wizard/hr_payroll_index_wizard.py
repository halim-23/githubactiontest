...
class HrPayrollIndex(models.TransientModel):
    ...
    def _compute_display_warning(self):
        for index in self:
            ...
            index.display_warning = any(
                contract.state != "open" for contract in contracts
            )
    ...
    def action_confirm(self):
        ...
        if self.display_warning:
            raise UserError(
                _(
                    "You have selected non running contracts, if you really need to index them, please do it by hand"
                )
            )
        ...
        if self.percentage:
            for contract in self.contract_ids:
                ...
                contract.with_context(mail_create_nosubscribe=True).message_post(
