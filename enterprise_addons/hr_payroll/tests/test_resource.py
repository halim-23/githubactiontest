...
class TestResource(TransactionCase):
    ...
    def test_compute_work_time_rate_with_two_weeks_calendar(self):
        """Test Case: check if the computation of the work time rate in the resource.calendar is correct."""
        create_attendance_ids = lambda attendance_list: [
    ...
    def test_compute_work_time_rate_with_credit_time_in_calendar(self):
        ...
        credit_time = self.env["hr.work.entry.type"].create(
            {"name": "Credit Time", "code": "Leave", "is_leave": True}
        )
        ...
        attendances = self.env["resource.calendar.attendance"].create(
            [
                ...
                },
                {
                    ...
                    "hour_to": 17,
                    "day_period": "afternoon",
                    "work_entry_type_id": credit_time.id,
                    "calendar_id": resource_calendar.id,
                },
                {
                    "name": "Thursday Morning",
        ...
        # Define a 4/5
        attendances_to_update = attendances.filtered(
            lambda attendance: (
                attendance.dayofweek == "2" and attendance.day_period == "afternoon"
            )
            or attendance.dayofweek == "3"
        )
        resource_calendar.write(
            {
                "name": "Calendar (4 / 5)",
        ...
        attendances_to_update = attendances.filtered(
            lambda attendance: attendance.dayofweek == "4"
            and attendance.day_period == "afternoon"
        )
        resource_calendar.write(
            {
                "name": "Calendar Full-Time",
