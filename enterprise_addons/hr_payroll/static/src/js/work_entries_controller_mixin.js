odoo.define("hr_payroll.WorkEntryPayrollControllerMixin", function (require) {
  ...
  var WorkEntryPayrollControllerMixin = {
    ...
    updateButtons: function () {
      ...
      var hasConflicts = records.some(function (record) {
        return record.state === "conflict";
      });
      var allValidated = records.every(function (record) {
    ...
    _renderGeneratePayslipButton: function (disabled) {
      return $(
        QWeb.render("hr_work_entry.work_entry_button", {
    ...
    _getActiveEmployeeIds: function () {
      ...
      const employee_ids = _.map(
        employees_datas,
        (employee_datas) => employee_datas[0]
      );
