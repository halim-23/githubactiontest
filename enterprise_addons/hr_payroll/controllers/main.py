...
class HrPayroll(Controller):
    ...
    def get_payroll_report_print(self, list_ids="", **post):
        ...
        for payslip in payslips:
            ...
            report = report.with_context(
                lang=payslip.employee_id.sudo().address_home_id.lang
            )
            pdf_content, _ = report.sudo()._render_qweb_pdf(
                payslip.id, data={"company_id": payslip.company_id}
            )
        ...
        if len(payslips) == 1 and payslips.struct_id.report_id.print_report_name:
            report_name = safe_eval(
                payslips.struct_id.report_id.print_report_name, {"object": payslips}
            )
        else:
    ...
    def get_debug_script_for_incorrect_payslip(self, payslip_id):
        ...
        start = datetime.datetime(
            payslip.date_from.year, payslip.date_from.month, payslip.date_from.day
        )
        end = datetime.datetime(
            payslip.date_to.year, payslip.date_to.month, payslip.date_to.day, 23, 59, 59
        )
            ...
            for record in records:
                ...
                for field_name, value in record.fields_get().items():
                    if (
                        not value["store"]
                        or field_name == "id"
                        or field_name not in whitelisted_fields
                    ):
                        continue
                    if field_name in forced_values.keys():
                        ...
                        record_vals[field_name] = (
                            _record_to_xmlid_ref(record[field_name])
                            or record[field_name].id
                        )
                    else:
            ...
            for vals in vals_list:
                ...
                for key, value in vals.items():
                    if key.endswith("_id") and isinstance(value, str):
                        ...
                        value = "datetime.datetime(%s, %s, %s, %s, %s, %s)" % (
                            value.year,
                            value.month,
                            value.day,
                            value.hour,
        ...
        global_attendances = (
            payslip.employee_id.resource_calendar_id.attendance_ids.filtered(
        ...
        if "car_id" in payslip.contract_id:
            ...
            # CAR CONTRACTS
            car_contracts = payslip.contract_id.car_id.log_contracts.filtered(
        ...
        if "time_credit" in payslip.contract_id and payslip.contract_id.time_credit:
            ...
            standard_calendar_attendances = standard_calendar.attendance_ids.filtered(
                lambda a: not a.resource_id
            )
            ...
            (
                ...
                standard_calendar_fields,
                standard_calendar_forced_values,
            ) = (None, None, None, None)
            (
                standard_calendar_attendances,
                standard_calendar_attendances_name,
                standard_calendar_attendances_fields,
                standard_calendar_attendances_forced_values,
            ) = (None, None, None, None)
        ...
        for records, name, whitelisted_fields, forced_values in data_to_export:
            if not records:
                continue
            records_values = _records_to_values(
                records, whitelisted_fields, forced_values
            )
            content_py_file += [
                "%(offset)scls.%(name)s = cls.env['%(model)s'].create([{"
                % {"offset": "\t" * offset, "name": name, "model": records._name},
        ...
        content_py_file += (
            [
                "\t" * offset
                + "work_entries = self.contract._generate_work_entries(datetime.date(%s, %s, %s), datetime.date(%s, %s, %s))"
            ...
            + [
                "\t" * offset
                + "self.assertEqual(len(self.payslip.worked_days_line_ids), %s)"
                % (len(payslip.worked_days_line_ids)),
                "\t" * offset
                + "self.assertEqual(len(self.payslip.input_line_ids), %s)"
                % (len(payslip.input_line_ids)),
                "\t" * offset
                + "self.assertEqual(len(self.payslip.line_ids), %s)\n"
                % (len(payslip.line_ids)),
            ]
            + [
                "\t" * offset
                + "self.assertAlmostEqual(self.payslip._get_worked_days_line_amount('%s'), %s, places=2)"
                % (wd.code, wd.amount)
            ...
            + [
                "\t" * offset
                + "self.assertAlmostEqual(self.payslip._get_worked_days_line_number_of_days('%s'), %s, places=2)"
                % (wd.code, wd.number_of_days)
                for wd in payslip.worked_days_line_ids
            ]
            + [""]
            + [
                "\t" * offset
                + "self.assertAlmostEqual(self.payslip._get_worked_days_line_number_of_hours('%s'), %s, places=2)"
                % (wd.code, wd.number_of_hours)
                for wd in payslip.worked_days_line_ids
            ]
            + [""]
            + [
                "\t" * offset
                + "self.assertAlmostEqual(self.payslip._get_salary_line_total('%s'), %s, places=2)"
