...
class ContractHistory(models.Model):
    ...
    salary_attachment_count = fields.Char(
    ...
    def _compute_payslips_count(self):
        for history in self:
            history.payslips_count = sum(
                contract.payslips_count for contract in history.contract_ids
            )
    ...
    def _compute_salary_attachment_count(self):
        for history in self:
            if history.employee_id.salary_attachment_count > 0:
                history.salary_attachment_count = str(
                    history.employee_id.salary_attachment_count
                )
            else:
    ...
    def action_open_salary_attachments(self):
        ...
        if self.employee_id.salary_attachment_count == 0:
            ...
            action = self.env["ir.actions.actions"]._for_xml_id(
                "hr_payroll.hr_salary_attachment_action"
            )
            action.update(
                {
                    "context": {
