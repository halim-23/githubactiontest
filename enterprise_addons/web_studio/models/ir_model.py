...
class Base(models.AbstractModel):
    ...
    def create_studio_model_data(self, name):
        ...
        if data:
            data.write(
                {}
            )  # force a write to set the 'studio' and 'noupdate' flags to True
        else:
...
class IrModel(models.Model):
    ...
    def _search_abstract(self, operator, value):
        abstract_models = [
            model._name for model in self.env.values() if model._abstract
        ]
        dom_operator = (
            "in" if (operator, value) in [("=", True), ("!=", False)] else "not in"
        )
        ...
        for model in self:
            ...
            for company in self.env["res.company"].sudo().search([]):
                self.env["ir.default"].set(
                    model.model, company_field.name, company.id, company_id=company.id
                )
        return self.env["ir.model"]
    ...
    def _setup_stages(self):
        ...
        for model in self:
            ...
            stage_model_vals["field_id"].append(
                (
                    0,
                    0,
                    {
            ...
            stage_model = self.with_context(list_editable="bottom").studio_model_create(
                "%s Stages" % model.name, vals=stage_model_vals, options=stage_options
            )[0]
            _logger.info(
                "created stage model %s (%s) for main model %s",
                stage_model.model,
                stage_model.name,
                model.model,
            )
            ...
            self.env[stage_model.model].create(
                [{"x_name": _("In Progress")}, {"x_name": _("Done")}]
            )
            ...
            kanban_state_field = self.env["ir.model.fields"].create(
                {
                    ...
                    "selection_ids": [
                        ...
                        (
                            0,
                            0,
                            {"value": "blocked", "name": _("Blocked"), "sequence": 30},
                        ),
                    ],
                    "relation": stage_model.model,
    ...
    def _setup_tags(self):
        ...
        for model in self:
            ...
            tag_model_vals["field_id"].append(
                (
                    0,
                    0,
                    {
            ...
            tag_model = self.with_context(list_editable="bottom").studio_model_create(
                "%s Tags" % model.name, vals=tag_model_vals
            )[0]
            _logger.info(
                "created tag model %s (%s) for main model %s",
                tag_model.model,
                tag_model.name,
                model.model,
            )
    ...
    def _create_default_action(self, name):
        ...
        self.ensure_one()
        model_views = self.env["ir.ui.view"].search_read(
            [("model", "=", self.model), ("type", "!=", "search")], fields=["type"]
        )
        ...
        sorted_view_types = list(
            sorted(available_view_types, key=lambda vt: VIEWS_ORDER.get(vt, 10))
        )
...
class IrModelField(models.Model):
    ...
    def name_get(self):
        if self.env.context.get("studio"):
            return [
                (field.id, "%s (%s)" % (field.field_description, field.model_id.name))
                for field in self
            ]
