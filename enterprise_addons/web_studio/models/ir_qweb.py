...
class IrQWeb(models.AbstractModel, QWeb):
    ...
    def _get_template(self, template, options):
        ...
        if options.get("full_branding"):
            ...
            basepath = len(
                "/".join(root.getpath(root.xpath("//*[@t-name]")[0]).split("/")[0:-1])
            )
    ...
    def _is_static_node(self, el, options):
        return not options.get("full_branding") and super(IrQWeb, self)._is_static_node(
            el, options
        )
    ...
    def _compile_all_attributes(self, el, options, indent, attr_already_created=False):
        ...
        return code + super(IrQWeb, self)._compile_all_attributes(
            el, options, indent, attr_already_created=attr_already_created
        )
