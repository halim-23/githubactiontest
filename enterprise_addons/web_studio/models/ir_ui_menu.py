...
class IrUiMenu(models.Model):
    ...
    def _get_studio_configuration_menu(self):
        ...
        config_menu = self.with_context(new_context).search(
            [
                ("parent_path", "like", parent_path),
                ("is_studio_configuration", "=", True),
            ]
        )
        if not config_menu:
            is_studio_app = root_xmlids and any(
                map(lambda xmlid: xmlid["studio"], root_xmlids)
            )
            menu_name = (
                _("Configuration") if is_studio_app else _("Custom Configuration")
            )
