...
class View(models.Model):
    ...
    def set_studio_map_popup_fields(self, model, node):
        ...
        field_ids = (
            ...
            .search([("model", "=", model), ("name", "in", field_names)])
            .ids
        )
        if field_ids:
    ...
    def create_automatic_views(self, res_model):
        ...
        if (
            "x_studio_date_start" in model._fields
            and "x_studio_date_stop" in model._fields
        ):
    ...
    def auto_list_view(self, res_model):
        ...
        fields = list()
        if (
            "x_studio_sequence" in model._fields
            and not "x_studio_priority" in model._fields
        ):
        ...
        if "x_studio_user_id" in model._fields:
            fields.append(
                E.field(name="x_studio_user_id", widget="many2one_avatar_user")
            )
        if "x_studio_company_id" in model._fields:
            fields.append(
                E.field(name="x_studio_company_id", groups="base.group_multi_company")
            )
        if (
            "x_studio_currency_id" in model._fields
            and "x_studio_value" in model._fields
        ):
            ...
            fields.append(
                E.field(
                    name="x_studio_value",
                    widget="monetary",
                    options="{'currency_field': 'x_studio_currency_id'}",
                    sum=_("Total"),
                )
            )
        ...
        tree_params = (
            {}
            if not self._context.get("list_editable")
            else {"editable": self._context.get("list_editable")}
        )
    ...
    def auto_form_view(self, res_model):
        ...
        if "x_studio_stage_id" in model._fields:
            header_content.append(
                E.field(name="x_studio_stage_id", widget="statusbar", clickable="1")
            )
            sheet_content.append(
                E.field(name="x_studio_kanban_state", widget="state_selection")
            )
        ...
        if "x_studio_image" in model._fields:
            sheet_content.append(
                E.field(
                    {"class": "oe_avatar", "widget": "image", "name": "x_studio_image"}
                )
            )
        ...
        if "x_studio_partner_id" in model._fields:
            ...
            left_group_content.append(
                E.field(name="x_studio_partner_email", widget="email")
            )
        if (
            "x_studio_currency_id" in model._fields
            and "x_studio_value" in model._fields
            ...
            right_group_content.append(
                E.field(name="x_studio_currency_id", invisible="1")
            )
            right_group_content.append(
                E.field(
                    name="x_studio_value",
        ...
        if (
            ...
            left_group_content.append(
                E.label({"for": "x_studio_date_start"}, string="Dates")
            )
            ...
            daterangeDiv.append(
                E.field(
                    ...
                    widget="daterange",
                    options='{"related_end_date": "x_studio_date_stop"}',
                )
            )
            ...
            daterangeDiv.append(
                E.field(
                    ...
                    widget="daterange",
                    options='{"related_start_date": "x_studio_date_start"}',
                )
            )
        ...
        if "x_studio_notes" in model._fields:
            sheet_content.append(
                E.group(
                    E.field(
                        name="x_studio_notes",
                        placeholder=_("Type down your notes here..."),
                        nolabel="1",
                    )
                )
            )
        ...
        form = E.form(
            E.header(*header_content),
            E.sheet(*sheet_content, string=model._description),
        )
    ...
    def auto_search_view(self, res_model):
        ...
        if "x_studio_partner_id" in model._fields:
            ...
            groupbys.append(
                E.filter(
                    name="groupby_x_partner",
                    string=_("Partner"),
                    context="{'group_by': 'x_studio_partner_id'}",
                    domain="[]",
                )
            )
        if "x_studio_user_id" in model._fields:
            ...
            filters.append(
                E.filter(
                    string=_("My %s", model._description),
                    name="my_%s" % res_model,
            ...
            groupbys.append(
                E.filter(
                    ...
                    context="{'group_by': 'x_studio_user_id'}",
                    domain="[]",
                )
            )
        date_filters = []
        if "x_studio_date" in model._fields:
            date_filters.append(
                E.filter(
                    date="x_studio_date", name="studio_filter_date", string=_("Date")
                )
            )
        if (
            "x_studio_date_start" in model._fields
            ...
            date_filters.append(
                E.filter(
                    date="x_studio_date_start",
                    name="studio_filter_date_start",
                    string=_("Start Date"),
                )
            )
            date_filters.append(
                E.filter(
                    date="x_studio_date_stop",
                    name="studio_filter_date_stop",
                    string=_("End Date"),
                )
            )
        if date_filters:
        ...
        if "x_studio_stage_id" in model._fields:
            groupbys.append(
                E.filter(
                    ...
                    string=_("Stage"),
                    context="{'group_by': 'x_studio_stage_id'}",
                    domain="[]",
                )
            )
    ...
    def auto_gantt_view(self, res_model):
        gantt = E.gantt(
            date_start="x_studio_date_start", date_stop="x_studio_date_stop"
        )
    ...
    def auto_kanban_view(self, res_model):
        ...
        title = E.strong(
            {"class": "o_kanban_record_title", "name": "studio_auto_kanban_title"}
        )
        title.append(E.field(name=model._rec_name_fallback()))
        headers_div = E.div(
            {"class": "o_kanban_record_headings", "name": "studio_auto_kanban_headings"}
        )
        headers_div.append(
            E.field(name="x_studio_priority", widget="boolean_favorite", nolabel="1")
        )
        ...
        top_div = E.div(
            {"class": "o_kanban_record_top", "name": "studio_auto_kanban_top"}
        )
        top_div.extend([headers_div, dropdown_div])
        body_div = E.div(
            {"class": "o_kanban_record_body", "name": "studio_auto_kanban_body"}
        )
        bottom_div = E.div(
            {"class": "o_kanban_record_bottom", "name": "studio_auto_kanban_bottom"}
        )
        bottom_left_div = E.div(
            {"class": "oe_kanban_bottom_left", "name": "studio_auto_kanban_bottom_left"}
        )
        bottom_right_div = E.div(
            {
                "class": "oe_kanban_bottom_right",
                "name": "studio_auto_kanban_bottom_right",
            }
        )
        bottom_div.extend([bottom_left_div, bottom_right_div])
        bottom_right_div.append(
            E.field(name="x_studio_kanban_state", widget="state_selection")
        )
        if "x_studio_user_id" in model._fields:
            pre_fields.append(
            ...
            img = E.img(
                {
                    "t-att-src": "kanban_image('res.users', 'avatar_128', record.x_studio_user_id.raw_value)",
        ...
        if "x_studio_tag_ids" in model._fields:
            body_div.append(
                E.field(name="x_studio_tag_ids", options="{'color_field': 'x_color'}")
            )
        if "x_studio_image" in model._fields:
            image_field = E.field(
                {
    ...
    def apply_inheritance_specs(self, source, specs_tree):
        ...
        if self and self._is_studio_view():
            ...
            return super(View, self).apply_inheritance_specs(
    ...
    def normalize(self):
        ...
        for line in diff:
            if line.strip() and not line.startswith("?"):
                if line.startswith("-") or line.startswith("+"):
                    ...
                    if (
                        line.endswith("[@closed]")
                        and nodes
                        and nodes[-1] + "[@closed]" == line
                    ):
        ...
        for line in diff:
            ...
            if (
                ...
                if line.startswith("-"):
                    ...
                    if is_moved(node) or any(is_moved(x) for x in node.iterancestors()):
                    ...
                    if xpath.get("expr"):
                        ...
                        current_xpath_target = next(
                            iter(old_view_tree.xpath("." + xpath.get("expr"))), None
                        )
                        if (
                            xpath.get("position") == "replace"
                            and current_xpath_target in node.iterancestors()
                        ):
                            continue
                        ...
                        # in the attributes section of a given node
                            ...
                            node.tag != "attributes"
                            and xpath.get("position") != "after"
                        ) or (
                            node.tag == "attributes"
                            and xpath.get("position") != "attributes"
                        ):
                    ...
                    if (
                        node.tag == "attribute"
                        and self._get_node_from_xpath(
                            xpath, node.getparent().getparent(), moved_fields
                        )
                        is not None
                    ):
                        continue
                    ...
                    if anchor_node.tag == "xpath" and not anchor_node.get("expr"):
                        ...
                        (
                            xpath.attrib["expr"],
                            xpath.attrib["position"],
                        ) = self._closest_node_to_xpath(
                            node, old_view_tree, moved_fields
                        )
                    ...
                    if node.tag == "field" and node.get("name") in moved_fields:
                        ...
                        node = etree.Element(
                            "xpath",
                            {
                                "expr": self._node_to_xpath(
                                    moved_fields[node.get("name")]["old"]
                                ),
                                "position": "move",
                            },
                        )
        ...
        normalized_arch = (
            etree.tostring(self._indent_tree(arch), encoding="unicode")
            if len(arch)
            else ""
        )
    ...
    def _get_anchor_node(self, arch, xpath, node, moved_fields):
        ...
        if anchor_node is not None and anchor_node.tag not in [
            "attribute",
            "attributes",
        ]:
            studio_previous_node = self._get_node_from_xpath(
                xpath, anchor_node, moved_fields
            )
            ...
            studio_parent_node = self._get_node_from_xpath(
                xpath, anchor_node, moved_fields
            )
    ...
    def _identify_node(self, node, node_context=None):
        ...
        if not node.get("name") or node.tag not in named_tags:
            ...
            colliding_prev_siblings = [
                sibling
                for sibling in same_tag_prev_siblings
    ...
    def _stringify_node(self, ancestor, node, moved_fields=None):
        ...
        if node.tag == "field" and moved_fields and node.get("name") in moved_fields:
            ...
            if self._node_to_xpath(node) == self._node_to_xpath(
                moved_fields[node.get("name")]["new"]
            ):
    ...
    def _generate_node_attributes(self, node):
        ...
        if node.tag not in ("attribute", "attributes"):
            ...
            node_attributes = sorted(
                node.items(), key=lambda i: i[0], reverse=True
            )  # inverse alphabetically sort attributes by name
            if len(node_attributes):
                for attr in node_attributes:
                    attributes = etree.Element(
                        "attributes",
                        {
                            "name": attr[0],
                        },
    ...
    def copy_qweb_template(self):
        ...
        nos = [
            int(old_copy.get("key").split("_copy_").pop()) for old_copy in old_copies
        ]
        ...
        for node in arch_tree.findall(".//t[@t-call]"):
            ...
            if tcall not in cloned_templates:
                callview = self.search(
                    [("type", "=", "qweb"), ("key", "=", tcall)], limit=1
                )
                if not callview:
    ...
    def _validate_tag_button(self, node, name_manager, node_info):
        ...
        if studio_approval and self.type != "form":
            self._raise_view_error(
                _("studio_approval attribute can only be set in form views"), node
            )
        if studio_approval and studio_approval not in ["True", "False"]:
            self._raise_view_error(
                _("Invalid studio_approval %s in button", studio_approval), node
            )
