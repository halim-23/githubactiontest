...
QUnit.module("Studio", (hooks) => {
  ...
  QUnit.test("add custom field button with other optional columns", async function (
    assert
  ) {
    ...
    await click(
      webClient.el.querySelector("div.o_optional_columns button.dropdown-item-studio")
    );
    assert.containsNone(document.body, ".modal-studio");
    assert.containsOnce(webClient, ".o_studio .o_web_studio_editor .o_list_view");
  });

  QUnit.test("add custom field button without other optional columns", async function (
    assert
  ) {
