  ...
  var loadIframeCss = function (callback) {
    return function WrapLoadIframeCss(assert) {
      ...
      session.rpc("/web_studio/edit_report/test_load_assets").then(function (assets) {
        ...
        loadIframeCss.head += _.map(loadIframeCss.assets.css, function (
          cssCode,
          cssFileName
        ) {
          cssCode = cssCode
          ...
          var style =
            '<style data-href="' + cssFileName + '">\n' + cssCode + "\n</style>";
          var htmlForValidation =
            ...
            "model.test.child": {
              fields: {
                ...
                grandchild: {
                  string: "Grandchild",
                  type: "many2one",
                  relation: "model.test.grandchild",
                  searchable: true,
                },
              },
              records: [],
      ...
      function () {
        QUnit.test("empty editor rendering", async function (assert) {
          ...
          this.templates.push({
            key: "template1",
            view_id: 55,
            arch: "<kikou>" + '<t t-name="template1">' + "</t>" + "</kikou>",
          });
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            // No content helper
            assert.strictEqual(
              rem.$("iframe").contents().find(".page .o_no_content_helper").length,
              1,
            ...
            await testUtils.dom.click(
              rem.$("iframe").contents().find(".page .o_no_content_helper")
            );

            // Printing the report
            assert.containsOnce(
              rem,
              ".o_web_studio_report_print",
        ...
        QUnit.test("basic editor rendering", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            assert.strictEqual(
              rem.$("iframe").contents().find(".page").text(),
              "First spanSecond span",
              "the iframe should be rendered"
            );

            var iframeContainerwidth = getFloatSizeFromPropertyInPixels(
              rem.$(".o_web_studio_report_iframe_container"),
              "width"
            );
            assert.ok(
              Math.abs(iframeContainerwidth - 794) <= 1,
              "the default width should be A4 (794px = 210mm) +/- 1px"
            );

            var iframeMinHeight = getFloatSizeFromPropertyInPixels(
              rem.$(".o_web_studio_report_iframe_container"),
              "min-height"
            );
            ...
            assert.ok(
              heightDifference <= 1,
              "the default height should be A4 (1122.52px = 297mm) at +/- 1 px because of decimals"
            );

            // Click to edit a span
            await testUtils.dom.click(
              rem.$("iframe").contents().find("span:contains(Second)")
            );
            ...
            assert.hasClass(
              rem.$(
                '.o_web_studio_sidebar .o_web_studio_sidebar_header div[name="options"]'
              ),
              "active",
            ...
            assert.containsN(
              rem,
              ".o_web_studio_sidebar .o_web_studio_sidebar_content .card",
              2,
            ...
            await testUtils.dom.click(
              rem.$("iframe").contents().find("span:contains(First span)")
            );
            assert.hasClass(
              rem.$("iframe").contents().find("span:contains(First span)"),
              "o_web_studio_report_selected",
            ...
            await testUtils.dom.click(
              rem.$(
                ".o_web_studio_sidebar .o_web_studio_sidebar_content_properties div.card:eq(1)"
              )
            );
            assert.hasClass(
              rem.$("iframe").contents().find("div.class1"),
              "o_web_studio_report_selected",
              "the corresponding nodes should be selected"
            );
        ...
        QUnit.test("editor rendering with paperformat", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            var iframeWidth = getFloatSizeFromPropertyInPixels(
              rem.$(".o_web_studio_report_iframe_container"),
              "width"
            );
            assert.ok(
              Math.abs(iframeWidth - 756) <= 1,
              "the width should be taken from the paperFormat +/- 1px"
            );

            var iframeHeight = getFloatSizeFromPropertyInPixels(
              rem.$(".o_web_studio_report_iframe_container"),
              "min-height"
            );
            ...
            assert.ok(
              heightDifference <= 1,
              "the height should be taken from the paperFormat +/- 1 px"
            );
        ...
        QUnit.test(
          "preview zoomed by paperformat DPI or smart-shrinking",
          async function (assert) {
            ...
            var containerStyles = rem.editor.$iframe
              .parent()
              .css(["paddingTop", "paddingLeft", "paddingRight", "width"]);

            var diffTopMargin = Math.abs(
              topMargin - parseFloat(containerStyles.paddingTop)
            );
            assert.ok(diffTopMargin < 1, "preview top margin same as paperformat");

            var diffLeftMargin = Math.abs(
              leftMargin - parseFloat(containerStyles.paddingLeft)
            );
            assert.ok(diffLeftMargin < 1, "preview left margin same as paperformat");

            var diffRightMargin = Math.abs(
              rightMargin - parseFloat(containerStyles.paddingRight)
            );
            ...
            assert.ok(
              diffContentZoom < 0.01,
              "zoom value to have body content match DPI"
            );
        ...
        QUnit.test("use pager", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            assert.containsOnce(
              rem,
              ".o_web_studio_report_pager",
              "there should be a pager"
            );
            assert.strictEqual(
              rem.$(".o_web_studio_report_pager").text().trim(),
              "1 / 2",
            ...
            await testUtils.dom.click(
              rem.$(".o_web_studio_report_pager .o_pager_next")
            );
        ...
        QUnit.test("components edition", async function (assert) {
          ...
          // Click to edit a span
          await testUtils.dom.click(
            rem.$("iframe").contents().find("span:contains(First)")
          );

          var $textarea = rem.$(
            '.o_web_studio_sidebar .o_web_studio_active textarea[name="text"]'
          );
          assert.strictEqual(
            $textarea.length,
            1,
        ...
        QUnit.test("components edition 2", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            assert.hasAttrValue(
              rem.$(".o_web_studio_sidebar_header .active"),
              "name",
              "new",
              "the 'Add' tab should be active"
            );
            assert.strictEqual(
              rem.$("iframe").contents().find(".o_web_studio_report_selected").length,
              0,
            ...
            // Switch tab
            await testUtils.dom.click(
              rem.$('.o_web_studio_sidebar_header [name="report"]')
            );
            assert.hasAttrValue(
              rem.$(".o_web_studio_sidebar_header .active"),
              "name",
        ...
        QUnit.test(
          "remove components - when no node is available to select, the add tab is activated",
          async function (assert) {
            var self = this;
            assert.expect(1);
            ...
            var rem = await studioTestUtils.createReportEditorManager({
              ...
              mockRPC: function (route, args) {
                if (route === "/web_studio/edit_report_view") {
                  self.templates[1].arch =
                    "<kikou>" + '<t t-name="template1">' + "</t>" + "</kikou>";
                  return Promise.resolve({
                    report_html: studioTestUtils.getReportHTML(self.templates),
                    views: studioTestUtils.getReportViews(self.templates),
                  });
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              await testUtils.dom.click(
                rem.$(".o_web_studio_active .o_web_studio_remove")
              );
        ...
        QUnit.test("drag & drop text component", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            await testUtils.dom.click(
              rem.$(
                '.o_web_studio_sidebar .o_web_studio_sidebar_header div[name="new"]'
              )
            );

            // Drag and drop a Text component, which should trigger a view edition
            var $text = rem.$(
              ".o_web_studio_sidebar .o_web_studio_field_type_container:eq(1) .o_web_studio_component:contains(Text)"
            );
        ...
        QUnit.test(
          "drag & drop text component in existing col",
          loadIframeCss(async function (assert, done) {
            assert.expect(1);
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              // Drag and drop a Text component, which should trigger a view edition
              var $text = rem.$(
                ".o_web_studio_sidebar .o_web_studio_component:contains(Text):eq(1)"
              );
              await testUtils.dom.dragAndDrop(
                $text,
                rem.$("iframe").contents().find(".col-6:eq(1)")
              );
        ...
        QUnit.test("drag & drop components and cancel", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            var $text = rem.$(
              ".o_web_studio_sidebar .o_web_studio_component:contains(Field):eq(1)"
            );
            await testUtils.dom.dragAndDrop(
              $text,
              rem.$("iframe").contents().find(".col-3:last")
            );
            assert.strictEqual(
              $(".o_web_studio_field_modal").length,
              1,
              "a field modal should be opened"
            );

            // Cancel the field selection
            await testUtils.dom.click($(".o_web_studio_field_modal .btn-secondary"));
            assert.strictEqual(
              rem.$("iframe").contents().find(".o_web_studio_hook").length,
              0,
              "Must cancel the dragAndDrop"
            );

            // Drag and drop an Address component
            var $address = rem.$(
              ".o_web_studio_sidebar .o_web_studio_component:contains(Address)"
            );
            await testUtils.dom.dragAndDrop(
              $address,
              rem.$("iframe").contents().find(".col-3:last")
        ...
        QUnit.test("drag & drop field block", async function (assert) {
          ...
          var rem = await studioTestUtils.createReportEditorManager({
            ...
            mockRPC: function (route, args) {
              if (route === "/web_studio/edit_report_view") {
                ...
                assert.deepEqual(
                  operation.inheritance[0].content,
                  "<div class='row'><div class='col'><span t-field=\"o.child.name\"></span></div></div>",
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            var $field = rem.$(
              ".o_web_studio_sidebar .o_web_studio_field_type_container:eq(0) .o_web_studio_component:contains(Field):eq(0)"
            );
            ...
            assert.strictEqual(
              $(".o_web_studio_field_modal .o_field_selector_chain_part").text().trim(),
              "o (Model Test)",
            ...
            await testUtils.dom.click(
              $(".o_web_studio_field_modal .o_field_selector_prev_page")
            );

            assert.strictEqual(
              $(".o_web_studio_field_modal .o_field_selector_item").text().trim(),
              "o (Model Test)",
              'Only "o" should be selectable, not "docs"'
            );

            await testUtils.dom.click(
              $('.o_web_studio_field_modal .o_field_selector_item[data-name="o"]')
            );

            var allAvailableFields = $(
              ".o_web_studio_field_modal .o_field_selector_item"
            )
              .text()
              .trim();

            assert.ok(allAvailableFields.includes("Name"), "Char field is present");
            assert.ok(
              allAvailableFields.includes("Child"),
              "many2one fields are present"
            );

            assert.notOk(
              allAvailableFields.includes("Children"),
              "many2many fields should not be present"
            );
            assert.notOk(
              allAvailableFields.includes("Attachments"),
              "one2many fields should not be present"
            );

            await testUtils.dom.click(
              $('.o_web_studio_field_modal .o_field_selector_item[data-name="child"]')
            );
            await testUtils.dom.click(
              $('.o_web_studio_field_modal .o_field_selector_item[data-name="name"]')
            );
        ...
        QUnit.test(
          "preselect first available field in the field selector",
          async function (assert) {
            assert.expect(1);
            var done = assert.async();
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              var selectedField = $(
                ".o_web_studio_field_modal .o_field_selector .o_field_selector_chain_part"
              )
                .text()
                .trim();
              assert.strictEqual(
        ...
        QUnit.test(
          ...
          loadIframeCss(async function (assert, done) {
              ...
              mockRPC: function (route, args) {
                if (route === "/web_studio/edit_report_view") {
                  ...
                  assert.deepEqual(
                    ...
                    tests[testIndex].inheritance,
                    tests[testIndex].text
                  );
                  return Promise.reject();
                }
                return this._super.apply(this, arguments);
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              var $field = rem.$(
                ".o_web_studio_sidebar .o_web_studio_field_type_container:eq(1) .o_web_studio_component:contains(Field)"
              );
              ...
              for (testIndex; testIndex < tests.length; testIndex++) {
                ...
                // Drag and drop a Field component, which should trigger a view edition
                await testUtils.dom.dragAndDrop($field, $target, {
                  position: test.position,
                });
                var $nearestHook = rem
                  .$("iframe")
                  .contents()
                  .find(".o_web_studio_nearest_hook");
                assert.strictEqual(
                  $nearestHook.length,
                  test.nearestHookNumber,
                  test.text + " (nearestHook number)"
                );
          ...
          loadIframeCss(async function (assert, done) {
            ...
            var tests = [
              ...
              },
              {
                ...
                buildingBlockSelector:
                  ".o_web_studio_sidebar .o_web_studio_field_type_container:eq(2) .o_web_studio_component:contains(Field Column)",
                ...
                onDragAndDrop: function ($table) {
                  assert.strictEqual(
                    $table.find("tr th:first-child.o_web_studio_nearest_hook").length,
                    1,
                    "Should select the first title cell"
                  );
                  assert.strictEqual(
                    $table.find("tr td:first-child.o_web_studio_nearest_hook").length,
                    4,
              ...
              {
                text:
                  "Should select column without the header because there are two colspan=2",
                buildingBlockSelector:
                  ".o_web_studio_sidebar .o_web_studio_field_type_container:eq(2) .o_web_studio_component:contains(Field Column)",
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              for (testIndex; testIndex < tests.length; testIndex++) {
                ...
                await testUtils.dom.dragAndDrop($buildingBlock, $target, {
                  position: test.position,
                });
        ...
        QUnit.test(
          "drag & drop field in table without loop",
          loadIframeCss(async function (assert, done) {
            assert.expect(4);
        ...
        QUnit.test(
          'drag & drop block "Accounting Total"',
          loadIframeCss(async function (assert, done) {
            assert.expect(1);
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              var $text = rem.$(
                ".o_web_studio_sidebar .o_web_studio_field_type_container:eq(2) .o_web_studio_component:contains(Subtotal & Total)"
              );
              await testUtils.dom.dragAndDrop($text, $main, {
        ...
        QUnit.test(
          ...
          loadIframeCss(async function (assert, done) {
                ...
                if (route === "/web_studio/edit_report_view") {
                  ...
                  assert.deepEqual(
                    ...
                    [
                      {
                        content:
                          '<attribute name="t-value">o.child.grandchild</attribute>',
                        position: "attributes",
                        view_id: 55,
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              assert.strictEqual(
                $card
                  .find(".o_web_studio_report_currency_id .o_field_selector_chain_part")
                  .text()
                  .replace(/\s+/g, " "),
                " o (Model Test) Child ",
                "Should display the t-foreach value"
              );

              rem
                .$(".o_web_studio_report_currency_id .o_field_selector")
                .trigger("focusin");
              await testUtils.nextTick();
              await testUtils.dom.click(
                rem.$(
                  '.o_web_studio_report_currency_id .o_field_selector_item[data-name="grandchild"]'
                )
              );
              await testUtils.dom.click(
                rem.$(".o_web_studio_report_currency_id .o_field_selector_close")
              );
        ...
        QUnit.test(
          ...
          loadIframeCss(async function (assert, done) {
            ...
            await rem.editorIframeDef.then(async function () {
              ...
              var $text = rem.$(
                ".o_web_studio_sidebar .o_web_studio_component:contains(Data table)"
              );
              await testUtils.dom.dragAndDrop($text, $main, {
              ...
              assert.strictEqual(
                $(".o_technical_modal h4:contains(Alert)").length,
                1,
                "Should display an alert because the selected field is wrong"
              );

              await testUtils.dom.click(
                $(".o_technical_modal:contains(Alert) .btn-primary")
              );
              await testUtils.dom.click(
                $(
                  '.o_web_studio_field_modal .o_field_selector_item[data-name="children"]'
                )
              );
              await testUtils.dom.click($(".o_web_studio_field_modal .btn-primary"));
        ...
        QUnit.test('drag & drop block "Address"', async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            $(".o_web_studio_field_modal .o_field_selector").trigger("focusin");
            await testUtils.dom.click(
              $('.o_web_studio_field_modal .o_field_selector_item[data-name="partner"]')
            );
            await testUtils.dom.click($(".o_web_studio_field_modal .btn-primary"));
        ...
        QUnit.test('drag & drop block "Image"', async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            $("body").one("shown.bs.modal", function () {
              assert.containsOnce(
                $("body"),
                ".modal-dialog.o_select_media_dialog",
            ...
            defMediaDialogInit.then(async function () {
              ...
              await testUtilsDom.click(
                $modal.find(".o_existing_attachment_cell").removeClass("d-none")
              );
            ...
            var $page = rem.$("iframe").contents().find(".page");
            var $imageBlock = rem.$(
              ".o_web_studio_sidebar .o_web_studio_component:contains(Image)"
            );
        ...
        QUnit.test("edit text", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            var $editable = rem.$(
              ".o_web_studio_sidebar .card.o_web_studio_active .note-editable"
            );

            assert.strictEqual(
              $editable.html(),
              "taratata <strong>bo</strong>",
        ...
        QUnit.test("open XML editor after modification", async function (assert) {
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            await testUtils.fields.editInput(
              rem.$('.o_web_studio_sidebar .o_web_studio_active textarea[name="text"]'),
              "hello"
            );
            ...
            // Open the XML editor
            await testUtils.dom.click(
              rem.$(".o_web_studio_sidebar .o_web_studio_xml_editor")
            );
        ...
        QUnit.test("automatic undo of correct operation", async function (assert) {
          ...
          this.templates.push({
            key: "template1",
            view_id: 55,
            arch: "<kikou>" + '<t t-name="template1"><div>First</div></t>' + "</kikou>",
          });
                ...
                switch (nbEdit) {
                  ...
                  case 2:
                    ...
                    assert.deepEqual(args.operations[1].inheritance, [
                      {
                        content:
                          '<attribute name="class" separator=" " add="o_underline"/>',
                        position: "attributes",
                        view_id: 55,
                        xpath: "/t/div",
                      },
          ...
          rem.editorIframeDef.then(async function () {
            await testUtils.dom.click(
              rem.$("iframe").contents().find("div:contains(First):last")
            );

            // Trigger a modification
            await testUtils.dom.click(
              rem.$(
                '.o_web_studio_sidebar .card:eq(1) .o_web_studio_text_decoration button[data-property="bold"]'
              )
            );
            ...
            await testUtils.dom.click(
              rem.$(
                '.o_web_studio_sidebar .card:eq(1) .o_web_studio_text_decoration button[data-property="italic"]'
              )
            );
            ...
            await testUtils.dom.click(
              rem.$(
                '.o_web_studio_sidebar .card:eq(1) .o_web_studio_text_decoration button[data-property="underline"]'
              )
            );
        ...
        QUnit.test("automatic undo on AST error", async function (assert) {
              ...
              if (route === "/web_studio/edit_report_view") {
                ...
                if (nbEdit === 1) {
                  assert.strictEqual(
                    args.operations.length,
                    1,
                    "the operation is correctly applied"
                  );
                ...
                if (nbEdit === 2) {
                  assert.strictEqual(
                    args.operations.length,
                    0,
                    "the operation should be undone"
                  );
                  return Promise.resolve({
                    report_html: studioTestUtils.getReportHTML(self.templates),
                    views: studioTestUtils.getReportViews(self.templates),
          ...
          await rem.editorIframeDef.then(async function () {
            await testUtils.dom.click(
              rem.$("iframe").contents().find("div:contains(Kikou):last")
            );
        ...
        QUnit.test("preselect appropriate value in domain dialog", async function (
          assert
        ) {
          var done = assert.async();
          assert.expect(4);
          ...
          await rem.editorIframeDef.then(async function () {
            ...
            await testUtils.dom.click(
              rem.$(".card.o_web_studio_active .o_field_domain_dialog_button")
            );
            assert.strictEqual(
              $(
                "body.modal-open .modal-body .o_domain_selector .o_domain_add_first_node_button"
              ).length,
              1,
            ...
            var selectedField = $(
              ".o_domain_leaf_edition .o_field_selector .o_field_selector_value .o_field_selector_chain_part"
            )
              .text()
              .trim();
            assert.strictEqual(
              selectedField,
              "o (Model Test)",
              "field selector should display first available field 'o (Model Test)'"
            );
