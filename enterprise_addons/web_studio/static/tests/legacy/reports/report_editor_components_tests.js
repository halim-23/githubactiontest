          ...
          this.widgetsOptions = {
            monetary: {
              company_id: {
                ...
                string: "Company",
                description:
                  "Company used for the original currency (only used for t-esc)",
              ...
              date: {
                ...
                string: "Date",
                description:
                  "Date used for the original currency (only used for t-esc)",
            ...
            contact: {
              separator: {
                ...
                description:
                  "Separator use to split the addresse from the display_name.",
                default_value: false,
              },
              no_marker: {
                type: "boolean",
              ...
              fields: {
                ...
                default_value: ["name", "address", "phone", "mobile", "email"],
                params: {
                  type: "selection",
                  params: [
            ...
            duration: {
              ...
              round: {
                ...
                description:
                  "Date unit used for the rounding. If the value is given, this must be smaller than the unit",
      ...
      function () {
        ...
        QUnit.test("field", async function (assert) {
          ...
          tOptions
            ...
            .then(function (res) {
              assert.deepEqual(
                ...
                [
                  {
                    content: '<span t-field="toto.child"></span>',
                    xpath: "/my/node/path/",
                    view_id: 99,
                    position: undefined,
                  },
          ...
          await testUtils.dom.triggerEvents(
            $(".o_web_studio_field_modal .o_field_selector"),
            ["focus"]
          );
          await testUtils.dom.click(
            $(".o_web_studio_field_modal .o_field_selector_close")
          );
          await testUtils.dom.click($(".o_web_studio_field_modal .btn-primary"));

          assert.strictEqual(
            $('.modal main[role="alert"]').length,
            1,
        ...
        QUnit.test("add a binary field", async function (assert) {
            ...
            .then(function (res) {
              assert.deepEqual(
                ...
                [
                  {
                    content:
                      '<span t-field="toto.image" t-options-widget="&quot;image&quot;"></span>',
                    xpath: "/my/node/path/",
                    view_id: 99,
                    position: undefined,
                  },
          ...
          await testUtils.dom.click(
            $('.o_web_studio_field_modal .o_field_selector_item[data-name="image"]')
          );
          await testUtils.dom.click($(".o_web_studio_field_modal .btn-primary"));
        ...
        QUnit.test("column component with valid classes", async function (assert) {
          ...
          assert.strictEqual(
            column.$('input[name="size"]').val(),
            "5",
            "the size should be correctly set"
          );
          assert.strictEqual(
            column.$('input[name="offset"]').val(),
            "3",
        ...
        QUnit.test(
          'hidden "width" for layout component with col nodes',
        ...
        QUnit.test("tOptions component parse expression", async function (assert) {
          ...
          var tOptions = new (editComponentsRegistry.get("tOptions"))(parent, {
            ...
            node: {
              attrs: {
                "t-options":
                  "dict(from_currency=o.child.currency_id, date=o.child.date)",
          ...
          assert.strictEqual(
            tOptions
              .$(
                ".o_web_studio_toption_option_monetary_from_currency .o_field_selector_value"
              )
              .text()
              .replace(/\s+/g, ""),
          ...
          assert.strictEqual(
            tOptions
              .$(".o_web_studio_toption_option_monetary_date .o_field_selector_value")
              .text()
              .replace(/\s+/g, ""),
          ...
          assert.strictEqual(
            tOptions
              .$(
                ".o_web_studio_toption_option_monetary_company_id .o_field_selector_value"
              )
              .text()
              .replace(/\s+/g, ""),
        ...
        QUnit.test("tEsc component with parsable expression", async function (assert) {
          ...
          assert.strictEqual(
            tOptions.$(".o_field_selector_value").text().replace(/\s+/g, ""),
            "o(ModelTest)ChildCompany",
            "Should display the company field"
          );
        ...
        QUnit.test("tEsc component with non-parsable expression", async function (
          assert
        ) {
          assert.expect(1);
          var parent = new Widget();
        ...
        QUnit.test("contact: many2many_select", async function (assert) {
          ...
          await testUtils.mock.addMockEnvironment(parent, {
            intercepts: {
              view_change: function (ev) {
                assert.deepEqual(
                  ev.data.operation.new_attrs["t-options-fields"],
                  optionsFields,
                  "Should save the contact options"
                );

                params.node.attrs["t-options-fields"] = JSON.stringify(
                  ev.data.operation.new_attrs["t-options-fields"]
                );
              },
            },
          ...
          assert.strictEqual(
            tOptions.$(".o_badge_text").text(),
            "NameAddressPhoneMobileEmail",
            "Should display default value"
          );
          await testUtils.dom.click(tOptions.$(".o_input_dropdown input"));
          assert.strictEqual(
            $("ul.ui-autocomplete .ui-menu-item").length,
            1,
            "Should not display the unselected items"
          );
          assert.strictEqual(
            $("ul.ui-autocomplete .o_m2o_dropdown_option").length,
            0,
          ...
          await testUtils.dom.click(
            $("ul.ui-autocomplete .ui-menu-item:contains(VAT)")
          );
          tOptions.destroy();
          ...
          assert.strictEqual(
            tOptions.$(".o_badge_text").text(),
            "NameAddressPhoneMobileEmailVAT",
          ...
          await testUtils.dom.click(
            tOptions.$(".o_field_many2manytags .o_delete:first")
          );
          assert.strictEqual(
            tOptions.$(".o_badge_text").text(),
            "AddressPhoneMobileEmailVAT",
            'Should display the new value without "name"'
          );
          ...
          assert.strictEqual(
            tOptions.$(".o_badge_text").text(),
            "PhoneMobileEmailVAT",
            'Should display the new value without "address"'
          );
        ...
        QUnit.test("contact: address separator", async function (assert) {
          ...
          await testUtils.mock.addMockEnvironment(parent, {
            intercepts: {
              view_change: function (ev) {
                assert.strictEqual(
                  ev.data.operation.new_attrs["t-options-separator"],
                  addressSeparator,
          ...
          var tOptions = new (editComponentsRegistry.get("tOptions"))(parent, params);
          await tOptions.appendTo(parent.$el);
          var separators = _.map(
            tOptions.$(
              ".o_web_studio_toption_option_contact_separator .o_field_widget option"
            ),
            function (option) {
          ...
          assert.deepEqual(
            separators,
            [false, " ", ",", "-", "|", "/"],
            "There should be a selection field with proper values"
          );
          assert.strictEqual(
            tOptions
              .$(
                ".o_web_studio_toption_option_contact_separator .o_field_widget option:selected"
              )
              .text(),
            "Linebreak",
          ...
          await testUtils.fields.editSelect(
            tOptions.$(
              ".o_web_studio_toption_option_contact_separator .o_field_widget"
            ),
            addressSeparator
          );
          parent.destroy();
        ...
        QUnit.test("contact: no_marker boolean field", async function (assert) {
          ...
          await testUtils.mock.addMockEnvironment(parent, {
            intercepts: {
              view_change: function (ev) {
                assert.strictEqual(
                  ev.data.operation.new_attrs["t-options-no_marker"],
                  false,
          ...
          assert.containsOnce(
            tOptions,
            ".o_web_studio_toption_option_contact_no_marker input:checked",
            "no_marker checkbox is checked initially"
          );
          await testUtils.dom.click(
            tOptions.$(".o_web_studio_toption_option_contact_no_marker input")
          );
        ...
        QUnit.test("no search more in many2many_select", async function (assert) {
          ...
          assert.strictEqual(
            $("ul.ui-autocomplete .o_m2o_dropdown_option").length,
            0,
            "Should not display create button nor the search more"
          );
        ...
        QUnit.test("groups component", async function (assert) {
          ...
          assert.containsN(
            groups,
            ".o_field_many2manytags .o_badge_text",
            2,
            "there should be displayed two groups"
          );
          assert.strictEqual(
            groups.$(".o_field_many2manytags").text().replace(/\s/g, ""),
            "MyAwesomeGroupKikou",
