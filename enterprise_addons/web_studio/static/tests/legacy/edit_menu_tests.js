  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("edit menu behavior", async function (assert) {
        ...
        assert.strictEqual(
          $(".o_web_studio_edit_menu_modal").length,
          0,
      ...
      QUnit.test("edit menu dialog", async function (assert) {
        ...
        assert.strictEqual(
          dialog.$("ul.oe_menu_editor > li").data("menu-id"),
          2,
          "the main menu should have the menu-id 2"
        );
        assert.containsOnce(
          dialog,
          "ul.oe_menu_editor > li > div button.js_edit_menu",
          "there should be a button to edit the menu"
        );
        assert.containsOnce(
          dialog,
          "ul.oe_menu_editor > li > div button.js_delete_menu",
        ...
        assert.strictEqual(
          $(".o_web_studio_add_menu_modal").length,
          1,
          "there should be a modal in the dom"
        );
        assert.strictEqual(
          $('.o_web_studio_add_menu_modal input[name="name"]').length,
          1,
          "there should be an input for the name in the dialog"
        );
        await testUtils.dom.click(
          $(
            '.o_web_studio_add_menu_modal .o_field_widget[name="model_choice"] [data-value="new"]'
          )
        );
        assert.isNotVisible(
          $(".o_web_studio_add_menu_modal .o_field_many2one"),
        ...
        await testUtils.dom.click(
          $(
            '.o_web_studio_add_menu_modal .o_field_widget[name="model_choice"] [data-value="existing"]'
          )
        );
        assert.strictEqual(
          $(".o_web_studio_add_menu_modal .o_field_many2one").filter(":visible").length,
          1,
        ...
        await testUtils.fields.editInput(
          $('.o_web_studio_add_menu_modal input[name="name"]'),
          "AA"
        );
        await testUtils.dom.click($(".o_web_studio_add_menu_modal .btn-primary"));
        ...
        await testUtils.dom.dragAndDrop(
          dialog.$("li li .input-group:first"),
          dialog.$(".input-group:first")
        );
        assert.strictEqual(
          dialog.to_move[2].sequence,
          dialog.to_move[21].sequence + 1,
        ...
        assert.strictEqual(
          $('.o_act_window input.o_field_widget[name="name"]').val(),
          "Menu 2",
        ...
        await testUtils.dom.click(
          $(".o_act_window").closest(".modal").find(".btn-primary")
        );
        assert.strictEqual(
          customizeCalls,
          1,
          "current changes are saved after editing a menu"
        );
        ...
        assert.containsN($(document), ".modal", 2, "should have 2 dialogsm");
        assert.strictEqual(
          $(".modal-title:last").text(),
          "Alert",
