  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("disable the view from studio", async function (assert) {
        ...
        await testUtils.dom.click(
          webClient.el.querySelector(".o_web_studio_menu_item a")
        );

        // Make list view disable and form view only will be there in studio view
        await testUtils.dom.click(
          $(webClient.el).find('div[data-type="list"] .o_web_studio_more')
        );
        await testUtils.dom.click(
          $(webClient.el).find('div[data-type="list"] a[data-action="disable_view"]')
        );
        // ReloadAction = false;
        assert.hasClass(
        ...
        await testUtils.dom.click(
          $(webClient.el).find('div[data-type="form"] .o_web_studio_more')
        );
        await testUtils.dom.click(
          $(webClient.el).find('div[data-type="form"] a[data-action="disable_view"]')
        );
        assert.containsOnce(
          $,
          ".o_technical_modal",
