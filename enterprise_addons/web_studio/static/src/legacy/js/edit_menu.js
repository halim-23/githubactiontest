odoo.define("web_studio.EditMenu", function (require) {
  ...
  var EditMenuDialog = Dialog.extend({
    ...
    moveMenu: function (ev, ui) {
      ...
      this.to_move[menu_id] = {
        parent_menu_id:
          $menu.parents("[data-menu-id]:first").data("menu-id") ||
          this.current_primary_menu,
        sequence: $menu.index(),
      };
    ...
    _onSave: function () {
      ...
      if (!_.isEmpty(this.to_move) || !_.isEmpty(this.to_delete)) {
  ...
  const NewMenuDialog = CommonMenuDialog.extend({
    ...
    start: function () {
      ...
      defs.push(
        this.model
          .makeRecord("ir.actions.act_window", [
            {
              name: "model",
              relation: "ir.model",
              type: "many2one",
      ...
      this.set_buttons([
        {
          ...
          classes: "btn-primary confirm_button",
          click: new_model
            ? this._onConfigureModel.bind(this)
    ...
    _onFieldChanged: async function (ev) {
      const res = await StandaloneFieldManagerMixin._onFieldChanged.apply(
        this,
        arguments
      );
