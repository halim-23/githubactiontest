odoo.define("web_studio.ViewEditorSidebar", function (require) {
  ...
  var OPTIONS_BY_WIDGET = {
    ...
    radio: [{name: "horizontal", type: "boolean", string: _lt("Display horizontally")}],
    signature: [
      {
        name: "full_name",
        type: "selection",
    ...
    phone: [{name: "enable_sms", type: "boolean", string: _lt("Enable SMS")}],
  };
  ...
  return Widget.extend(StandaloneFieldManagerMixin, {
    ...
    events: {
      ...
      'focus .o_display_field input[data-type="attributes"][name="domain"]':
        "_onDomainEditor",
      ...
      'change .o_display_chatter input[data-type="email_alias"]':
        "_onEmailAliasChanged",
    ...
    init: function (parent, params) {
      ...
      if (
        this.state.node &&
        (this.state.node.tag === "field" || this.state.node.tag === "filter")
      ) {
        ...
        const fieldRegistryMap = Object.assign(
          {},
          field_registry.map,
          fieldRegistryOwl.map
        );
        field.field_widgets = _.chain(fieldRegistryMap)
          ...
          .filter(function (arr) {
            ...
            var isSupported =
              _.contains(supportedFieldTypes, field.type) &&
              arr[0].indexOf(".") < 0 &&
              unsupportedWidgets.indexOf(arr[0]) < 0;
            return config.isDebug()
              ? isSupported
              : (isSupported && description) || isWidgetKeyDescription;
          })
          .sortBy(function (arr) {
            const description = self.getFieldInfo(arr[1], "description");
          ...
          var signFields = _.chain(_.sortBy(_.values(this.fields_in_view), "string"))
            ...
            .map(function (val, key) {
              return [
                val.name,
                config.isDebug()
                  ? _.str.sprintf("%s (%s)", val.string, val.name)
                  : val.string,
              ];
            })
            .value();
          _.findWhere(OPTIONS_BY_WIDGET[this.widgetKey], {
            name: "full_name",
          }).selection = selection.concat(signFields);
        }
        ...
        this.hasAggregate =
          _.contains(["integer", "float", "monetary"], field.type) &&
          !_.contains(["progressbar", "handle"], this.state.attrs.widget);

        if (this.view_type === "kanban") {
          this.showDisplay =
            this.state.$node &&
            !this.state.$node.parentsUntil(".o_kanban_record").filter(function () {
      ...
      if (this.state.mode === "view" && this.view_type === "gantt") {
        ...
        this.state.attrs.ganttPrecision = this.state.attrs.precision
          ? pyUtils.py_eval(this.state.attrs.precision)
          : {};
      }
    ...
    _changeMapPopupFields: function (ev, oldMapPopupField) {
      ...
      if (ev.data.changes.map_popup.operation === "ADD_M2M") {
        ...
        options.type = "remove";
        options.field_ids = [
          oldMapPopupField.data.find((i) => i.id === ev.data.changes.map_popup.ids[0])
            .res_id,
        ];
      }
    ...
    _changePivotMeasuresFields(ev, oldPivotMeasuresField) {
      ...
      if (ev.data.changes.pivot_popup.operation === "ADD_M2M") {
        ...
        options.type = "remove";
        options.field_ids = [
          oldPivotMeasuresField.data.find(
            (i) => i.id === ev.data.changes.pivot_popup.ids[0]
          ).res_id,
        ];
      }
      this.trigger_up("view_change", options);
    ...
    _computeFieldAttrs: function () {
       ...
       */
      this.state.attrs.invisible =
        this.state.modifiers.invisible || this.state.modifiers.column_invisible;
      ...
      this.state.attrs.placeholder =
        this.state.attrs.placeholder || this.state.field.placeholder;
      this.state.attrs.required =
        this.state.field.required || this.state.modifiers.required;
      ...
      this.state.attrs.related = this.state.field.related
        ? this.state.field.related
        : false;
    },
    /**
     * @private
    ...
    _getNewAttrsFromModifiers: function (modifiers) {
      ...
      var originSubAttrs = pyUtils.py_eval(
      ...
      _.each(modifiers, function (value, key) {
        ...
        if (keyInNodeAndAttrs && !isOriginNodeAttr && trueValue) {
          // Modifier always applied, use modifier attribute
          newAttributes[key] = "1";
        } else if (keyFromView || !trueValue) {
          // Modifier not applied or under certain condition, remove modifier attribute and use attrs if any
          newAttributes[key] = "";
          if (value !== false) {
            attrs.push(
              _.str.sprintf('"%s": %s', key, Domain.prototype.arrayToString(value))
            );
          }
        }
      });
    ...
    _render: function () {
      ...
      if (this.state.mode === "new") {
        ...
        return Promise.all(this.defs).then(() => {
          delete this.defs;
          this.$(".o_web_studio_component").on(
            "drag",
            _.throttle((event, ui) => {
              this.trigger_up("drag_component", {
    ...
    _renderComponentsSection: function () {
      const widgetClasses = form_component_widget_registry.get(
        this.view_type + "_components"
      );
    ...
    _renderExistingFieldsSection: function () {
      ...
      if (this.view_type === "search") {
        formWidgets = Object.values(this.fields).map(
          (field) =>
      ...
      if (this._searchValue) {
        formWidgets = formWidgets.filter((result) => {
          ...
          if (this.debug) {
            return (
              result.label.toLowerCase().includes(searchValue) ||
      ...
      const $sidebarContent = this.$(".o_web_studio_sidebar_content");
      const $existingFieldsSection = $("<div/>", {
        class: `o_web_studio_existing_fields_section`,
      });
      const $section = this._renderSection(formWidgets);
      ...
      if ($existingFields.length) {
        ...
        const $sectionTitle = $("<h3>", {
          ...
          $("<i/>", {class: `o_web_studio_existing_fields_icon fa fa-caret-right ml-2`})
        );
        const $sectionSubtitle = $("<h6>", {
          class: "small text-white",
          text: _t("The following fields are currently not in the view."),
        });
        const $sectionSearchDiv = core.qweb.render(
          "web_studio.ExistingFieldsInputSearch"
        );
    ...
    _renderSection: function (form_widgets) {
      var self = this;
      var $components_container = $("<div>").addClass(
    ...
    _renderWidgetsM2MGroups: function () {
      var self = this;
      var studio_groups =
        this.state.attrs.studio_groups && JSON.parse(this.state.attrs.studio_groups);
      return this.model
        .makeRecord("ir.model.fields", [
          {
            name: "groups",
            fields: [
              {
    ...
    _renderWidgetsMapPopupFields: function () {
      ...
      return this.model
        ...
        .then((recordID) => {
          ...
          const many2many = new Many2ManyTags(this, "map_popup", record, {
            mode: "edit",
          });
    ...
    _renderWidgetsPivotMeasuresFields() {
      ...
      const fieldIDs = JSON.parse(
        this.state.attrs.studio_pivot_measure_field_ids || "[]"
      );
      return this.model
        .makeRecord(
          "ir.model",
        ...
        .then((recordID) => {
          ...
          const many2many = new Many2ManyTags(this, "pivot_popup", record, {
            mode: "edit",
          });
    ...
    _onApprovalDomain: function (ev) {
      ...
      const dialog = new DomainSelectorDialog(
        ...
        rule.domain || [],
        {
          ...
          $content: $("<div>").append("<p>", {
            text: _t(
              "The approval rule is only applied to records matching the following condition:"
            ),
          }),
        }
      ).open();
    ...
    _onApprovalChange: function (ev) {
      ...
      switch (input) {
        ...
        default:
          console.debug(
            "unsupported operation for approval modification",
            ev.target.name
          );
          return false;
      }
    ...
    _onDomainAttrs: function (ev) {
      ...
      var dialog = new DomainSelectorDialog(
        this,
        this.model_name,
        _.isArray(this.state.modifiers[modifier]) ? this.state.modifiers[modifier] : [],
        {
          readonly: false,
          fields: fields,
          size: "medium",
          ...
          $content: $(
            _.str.sprintf(
              _t(
                "<div><p>The <strong>%s</strong> property is only applied to records matching this filter.</p></div>"
              ),
              modifier
            )
          ),
    ...
    _onDomainEditor: function (ev) {
      ...
      var model =
        this.state.node.tag === "filter" ? this.model_name : this.state.field.relation;
      ...
      if (attribute && $input.attr("type") !== "file") {
        ...
        if (attribute.match("^rainbow")) {
          if (this.$("input#rainbow").is(":checked")) {
            new_attrs.effect = JSON.stringify(
              _.pick(
                {
          ...
          if (!_.contains(this.MODIFIERS_IN_NODE_AND_ATTRS, attribute)) {
            if ($input.is(":checked")) {
              new_attrs[attribute] =
                $input.data("leave-empty") === "checked" ? "" : "True";
            } else {
              new_attrs[attribute] =
                $input.data("leave-empty") === "unchecked" ? "" : "False";
            }
          } else {
            var newModifiers = _.extend({}, this.state.modifiers);
    ...
    _onElementRemoved: function () {
      ...
      var message = _.str.sprintf(
        _t("Are you sure you want to remove this %s from the view?"),
        elementName
      );
    ...
    _onFieldChanged: async function (ev) {
      const approvalChanges = Object.keys(ev.data.changes).filter((f) =>
        f.startsWith("rule_group_")
      );
      const isApprovalChange = approvalChanges.length;
      const isMapChange = Object.keys(ev.data.changes).filter((f) => f === "map_popup")
        .length;
      const isPivotChange = Object.keys(ev.data.changes).filter(
        (f) => f === "pivot_popup"
      ).length;
      const approvalField = isApprovalChange && approvalChanges[0];
      const oldMapPopupField =
        this.mapPopupFieldHandle &&
        this.model.get(this.mapPopupFieldHandle).data.map_popup;
      const oldPivotMeasureField =
        this.pivotPopupFieldHandle &&
        this.model.get(this.pivotPopupFieldHandle).data.pivot_popup;
      const result = await StandaloneFieldManagerMixin._onFieldChanged.apply(
        this,
        arguments
      );
      if (isMapChange) {
    ...
    _onFieldNameChanged: function (ev) {
      ...
      if (newName.match(/[^a-z0-9_]/g) || newName.length >= 54) {
        message = _.str.sprintf(
          _t(
            "The new name can contain only a to z lower letters, numbers and _, with " +
    ...
    _onOptionsChanged: function (ev) {
      ...
      if ($input.attr("type") === "checkbox") {
        ...
        if (
          (optionValue && $input.data("leave-empty") !== "checked") ||
    ...
    _onShowInvisibleToggled: function (ev) {
      this.state.show_invisible = Boolean($(ev.currentTarget).is(":checked"));
      this.trigger_up("toggle_form_invisible", {
        show_invisible: this.state.show_invisible,
      });
    },
    /**
     * @private
    ...
    _onChangedGroupBys(attribute, input, newAttrs) {
      ...
      if (
        !newAttrs.length ||
        (["measure"].includes(attribute) &&
          newAttrs.length === 1 &&
          newAttrs[0] === "__count__") ||
        (["second_groupby", "second_row_groupby"].includes(attribute) &&
          newAttrs.length < 2)
      ) {
        ...
        options.oldname = ["second_groupby", "second_row_groupby"].includes(attribute)
          ? newAttrs[1]
          : newAttrs[0];
        options.name = [input];
      } else {
        options.operationType = "remove";
        options.name = ["second_groupby", "second_row_groupby"].includes(attribute)
          ? [newAttrs[1]]
          : [newAttrs[0]];
      }
      return options;
    ...
    _onViewChanged: function (ev) {
      ...
      if (this.view_type === "gantt" && _.str.include(attribute, "precision_")) {
        ...
        this.view_type === "list" &&
        ["sort_field", "sort_order"].includes(attribute)
      ) {
        const new_attrs = {};
        if (attribute === "sort_field" && !$input.val()) {
          ...
          new_attrs.default_order =
            this.$("#sort_field").val() + " " + this.$("#sort_order").val();
        }
        this.trigger_up("view_change", {
          type: "attributes",
          structure: "view_attribute",
        ...
        ["stacked", "first_groupby", "second_groupby", "measure"].includes(attribute)
      ) {
        if (attribute === "stacked") {
          ...
          if (attribute === "first_groupby") {
            const newoptions = this._onChangedGroupBys(
              attribute,
              $input.val(),
              this.state.attrs.groupBys
            );
          ...
          if (attribute === "measure") {
            const newoptions = this._onChangedGroupBys(attribute, $input.val(), [
              this.state.attrs.measure,
            ]);
            options = {...options, ...newoptions};
          }
          this.trigger_up("view_change", {
        ...
        this.view_type === "pivot" &&
        ["column_groupby", "first_row_groupby", "second_row_groupby"].includes(
          attribute
        )
      ) {
        let options = {};
        ...
        if ($input.attr("type") === "checkbox") {
          if (
            ($input.is(":checked") && !$input.data("inverse")) ||
            (!$input.is(":checked") && $input.data("inverse"))
          ) {
            new_attrs[attribute] =
              $input.data("leave-empty") === "checked" ? "" : "true";
