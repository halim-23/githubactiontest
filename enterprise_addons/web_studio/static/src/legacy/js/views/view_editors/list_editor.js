odoo.define("web_studio.ListEditor", function (require) {
  ...
  return ListRenderer.extend(EditorMixin, {
    ...
    events: _.extend({}, ListRenderer.prototype.events, {
      "click th:not(.o_web_studio_hook), td:not(.o_web_studio_hook)":
        "_onExistingColumn",
    }),
    ...
    setLocalState: function (state) {
      if (state.selected_node_id) {
        var $selected_node = this.$(
          'th[data-node-id="' + state.selected_node_id + '"]'
        );
        if ($selected_node) {
          $selected_node.click();
        }
    ...
    _handleDrop: function (ev, ui) {
      ...
      if ($hook.length) {
        var position =
          $hook.closest("table").find("th").eq($hook.index()).data("position") ||
          "after";
        var hookedFieldIndex =
          (position === "before" && $hook.index() + 1) || $hook.index() - 1;
        var fieldName = $hook
          .closest("table")
          .find("th")
          .eq(hookedFieldIndex)
    ...
    _renderBodyCell(record, node, ...args) {
      ...
      const invisibleTechnicalNames = this.invisible_columns.map(
        (column) => column.attrs.name
      );
    ...
    _addStudioHooksOnBodyRow: function ($row) {
      ...
      _.each($row.find("td, th"), function (cell) {
        $("<td>").addClass("o_web_studio_hook").insertAfter($(cell));
      });

      // Insert a hook before the first column
      $("<td>").addClass("o_web_studio_hook").prependTo($row);
    },
    /**
     * @override
     * @private
    ...
    _renderFooter: function () {
      ...
      _.each($footer.find("td"), function (td) {
        $("<td>").addClass("o_web_studio_hook").insertAfter($(td));
      });

      // Insert a hook before the first column
      $("<td>").addClass("o_web_studio_hook").prependTo($footer.find("tr"));
