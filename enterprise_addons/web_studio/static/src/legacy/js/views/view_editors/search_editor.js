  ...
  var SearchEditor = SearchRenderer.extend(EditorMixin, {
    ...
    highlightNearestHook: function ($helper, position) {
      ...
      if ($nearest_form_hook.length) {
        ...
        if (
          tableType === "group_by" &&
          isField &&
          _.contains(this.GROUPABLE_TYPES, type) &&
          store === "true"
        ) {
          addHook = true;
        } else if (
          tableType === "filters" &&
          ((isField && _.contains(this.FILTER_TYPES, type) && store === "true") ||
            isSeparator ||
            isFilter)
        ) {
          addHook = true;
    ...
    _render: function () {
        ...
        drop: function (event, ui) {
          ...
          if ($hook.length) {
            ...
            if (structure === "filter") {
              ...
              self.$domain_label_input = $(
                "<input type='text' id='domain_label' class='o_input mb8'/>"
              );
              $domain_div.append(self.$domain_label_input);
              var domain_dialog = self._openDomainDialog(self.model, [["id", "=", 1]], {
                title: _t("New Filter"),
                size: "medium",
                readonly: false,
            ...
            if (
              hook.type === "filter" &&
              structure === "field" &&
              _.contains(self.FILTER_TYPES, new_attrs.type)
            ) {
              structure = "filter";
      ...
      this.$el.droppable({
        ...
        tolerance: "touch",
        over: function (ev, ui) {
          var $autocompletionFields = self.$(
            ".o_web_studio_search_autocompletion_fields"
          );
