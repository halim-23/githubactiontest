  ...
  var FormEditor = FormRenderer.extend(EditorMixin, {
    ...
    highlightNearestHook: function ($helper, position) {
      ...
      $nearest_form_hooks.each(function () {
        ...
        if (
          _.intersection(fieldClasses, whitelist).length === 0 &&
          hookTypeBlacklist.indexOf(hook.type) > -1
        ) {
    ...
    _render: function () {
      ...
      return this._super.apply(this, arguments).then(async function () {
        ...
        if (!self._hasChatter() && self.chatter_allowed) {
          ...
          const $studioChatterContainer = $("<div>").addClass(
        ...
        if (!self.$(".oe_button_box").length) {
          ...
          var $buttonbox = $("<div>").addClass("oe_button_box").append($buttonbox_hook);
        ...
        // Add avtar
        if (
          self.$(".oe_title").length &
            !self.$(".oe_title").siblings(".oe_avatar").length &&
          !self.$(".oe_title > h1 > .oe_avatar").length
        ) {
          const $avatar = $("<div>", {
            text: _t("Add Picture"),
            class: self.$(".oe_title > h1.d-flex.flex-row").length
              ? "oe_avatar ml-3 p-3 o_web_studio_avatar h4"
              : "oe_avatar ml-3 mr-3 o_web_studio_avatar",
          }).click(function () {
        ...
        self.$("i.o_optional_columns_dropdown_toggle").addClass("text-muted");
      });
    },
    /**
     * @private
    ...
    _renderButtonBox: function () {
      ...
      var $buttonhook = $("<button>").addClass(
        "btn oe_stat_button o_web_studio_button_hook"
      );
            ...
            if (!$widget.is(".o_invisible_modifier")) {
              self._renderAddingContentLine(child).then(function (element) {
                ...
                if ($widget.hasClass("o_web_studio_show_invisible")) {
                  ...
                  $tr
                    .find('label[for="' + $widget.attr("id") + '"]')
                    .removeClass("o_web_studio_show_invisible");
    ...
    _renderTagNotebook: function (node) {
      ...
      var $addTag = $("<li>", {class: "nav-item"}).append(
        '<a href="#" class="nav-link"><i class="fa fa-plus-square"/></a>'
      );
