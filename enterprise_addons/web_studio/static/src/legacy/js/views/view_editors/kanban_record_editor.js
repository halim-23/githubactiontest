      ...
      if (_.isUndefined(tagsWidget)) {
        ...
        $kanban_tags_hook.click(function () {
          ...
          if (_.isEmpty(compatible_fields)) {
            Dialog.alert(
              self,
              _t("You first need to create a many2many field in the form view.")
            );
            return;
          }
          ...
          dialog.on("confirm", self, function (field_name) {
            self.trigger_up("view_change", {
              ...
              node: {
                tag: has_kanban_body
                  ? 'div[hasclass("o_kanban_record_body")]'
                  : "div/*[1]",
              },
              position: "inside",
            });
      ...
      if (avatarNode) {
        ...
        $kanban_image_hook.click(function () {
          var compatible_fields = _.pick(self.state.fields, function (e) {
            return (
              e.type === "many2one" &&
              (e.relation === "res.partner" || e.relation === "res.users")
            );
          });
          if (_.isEmpty(compatible_fields)) {
            Dialog.alert(
              self,
              _t(
                "You first need to create a many2one field to Partner or User in the form view."
              )
            );
            return;
          }
