odoo.define("web_studio.ViewEditorManager", function (require) {
  ...
  class EditorWrapper extends ComponentWrapper {
    handleDrop() {
      return this.componentRef.comp && this.componentRef.comp.handleDrop(...arguments);
    }
    ...
    setSelectable() {
      return (
        this.componentRef.comp && this.componentRef.comp.setSelectable(...arguments)
      );
    }
    unselectedElements() {
      return (
  ...
  var ViewEditorManager = AbstractEditorManager.extend({
    ...
    _addField: function (
          ...
          def_field_values = new Promise(function (resolve, reject) {
            ...
            prom.then(function (openFieldDialog) {
              if (!openFieldDialog) {
                ...
                dialog = new NewFieldDialog(
                  self,
                  modelName,
                  field_description,
                  _.filter(self.fields, {type: "many2one"})
                ).open();
                dialog.on("field_default_values_saved", self, function (values) {
                  if (values.related && values.type === "monetary") {
                    if (self._hasCurrencyField()) {
                      ...
                      var currencyDialog = openCurrencyCreationDialog(
                        relatedCurrency,
                        resolve
                      );
      ...
      Promise.resolve(def_field_values)
    ...
    _editKanbanCover: function (type) {
      ...
      if (type === "remove") {
        var fieldToRemove = _.pick(this.view.fieldsInfo[this.view_type], function (
          field
        ) {
    ...
    _getShowInvisible() {
      return Boolean(
        this.sidebar && this.sidebar.state && this.sidebar.state.show_invisible
      );
    },
    /**
    ...
    _getSidebarState: function (mode, params) {
      ...
      switch (mode) {
        ...
        case "properties":
          ...
          if (
            node.tag === "button" &&
            node.attrs.studio_approval &&
            node.attrs.studio_approval !== "False"
          ) {
            let method, action;
     ...
     * @returns {Object} the fields_view of the x2m field
     */
        ...
        if (this.mode === "edition") {
          ...
          if (!Editor) {
            ...
            if (isComponent(View.prototype.config.Renderer)) {
              const Renderer = class extends EditorMixinOwl(
    ...
    _instantiateSidebar: function (state, previousState) {
      ...
      var params = {
        ...
        isEditingX2m: Boolean(this.x2mField),
    ...
    _onDragComponent: function (ev) {
      var is_nearest_hook = this.editor.highlightNearestHook(
        ev.data.$helper,
        ev.data.position
      );
    ...
    _onDefaultValueChange: function (event) {
      var data = event.data;
      var modelName = this.x2mModel ? this.x2mModel : this.model_name;
      this._setDefaultValue(modelName, data.field_name, data.value).guardedCatch(
        function () {
          if (data.on_fail) {
            data.on_fail();
          }
    ...
    _onNodeClicked: function (ev) {
      ...
      if (this.view_type === "form" && node.tag === "field") {
        var field = this.fields[node.attrs.name];
        var attrs = this.editor.state.fieldsInfo[this.editor.state.viewType][
          node.attrs.name
        ];
        ...
        if (isX2Many && !_.contains(notEditableWidgets, attrs.widget)) {
          ...
          if (
            $node.hasClass("o_field_one2many") ||
            $node.hasClass("o_field_many2many")
          ) {
            $node.block(options);
          } else {
          ...
          $node.find(".o_web_studio_editX2Many").click(function (e) {
            ...
            bus.trigger(
              "STUDIO_ENTER_X2M",
              self._computeX2mPath(
                x2mFieldName,
                x2mViewType,
                null,
                self.editor.state.data[x2mFieldName]
              )
            );
          });
        }
  ...
  function _findParentsPositions(parent, node, positions, indice) {
    ...
    if (parent === node) {
      ...
      _.each(parent.children, function (child) {
        ...
        current_indices[child.tag] = current_indices[child.tag]
          ? current_indices[child.tag] + 1
          : 1;
        var res = _findParentsPositions(
          child,
          node,
          positions,
          current_indices[child.tag]
        );
        if (res) {
          result = res;
        } else {
