odoo.define("web_studio.reportEditComponents", function (require) {
  ...
  var AbstractEditComponent = Abstract.extend(
    WidgetAdapterMixin,
    StandaloneFieldManagerMixin,
    {
      ...
      willStart: function () {
        ...
        _.each(this.directiveFields, function (options, directiveKey) {
          ...
          if (options.type === "related") {
            directiveModel.push({
              name: directiveKey,
              type: "char",
              value: options.freecode
                ? value
                : self._splitRelatedValue(value).chain.join("."),
            });
          } else {
            directiveModel.push(
              _.extend(
        ...
        var defDirective = this.model
          .makeRecord("ir.model.fields", directiveModel)
      ...
      createFieldSelector: function (directiveKey, options) {
        ...
        this.fieldSelector[directiveKey] = new ModelFieldSelector(
          this,
          "record_fake_model",
          split.chain,
          _.extend(
      ...
      _tSetAttributes: function (newAttrs) {
        ...
        _.each(newAttrs, function (tvalue, tset) {
          ...
          op.push({
            ...
            position: "attributes",
            view_id: Number(node.attrs["data-oe-id"]),
            ...
            inheritance: [
              {
                content: attribute,
                position: "attributes",
                view_id: Number(this.node.attrs["data-oe-id"]),
  ...
  var LayoutEditable = AbstractEditComponent.extend({
    ...
    init: function (parent, params) {
      ...
      var fontSizeRegExp = new RegExp(
        /^\s*(h[123456]{1})|(small)|(display-[1234]{1})\s*$/gim
      );
      ...
      this.displayWidth = !(
        params.node.tag === "div" &&
        _.find(this.classesArray, function (item) {
      ...
      this.alignment = _.intersection(this.classesArray, [
        "text-left",
        "text-center",
        "text-right",
      ])[0];
      this.displayAlignment = !_.contains(
        ["inline", "float"],
        this.node.$nodes.css("display")
      );
    ...
    renderElement: function () {
      var self = this;
      this._super.apply(this, arguments);
      this.$(".o_web_studio_background_colorpicker .o_web_studio_color_palette").append(
        this._createPalette()
      );
      this.$(".o_web_studio_background_colorpicker").on(
        "mousedown",
        "button[data-color]",
        function (e) {
          self._onColorChange(
            $(e.currentTarget).data("value").replace("text-", "bg-"),
            "background"
          );
        }
      );
      this.$(".o_web_studio_font_colorpicker .o_web_studio_color_palette").append(
        this._createPalette()
      );
      this.$(".o_web_studio_font_colorpicker").on(
        "mousedown",
        "button[data-color]",
        function (e) {
      ...
      if (this.node.attrs.style) {
        var margin = this.node.attrs.style
          ...
          .filter(function (item) {
            return !item.indexOf(marginName);
          });
        if (margin.length) {
    ...
    _onColorChange: function (value, type) {
      ...
      if (isClass) {
        this._editDomAttribute(
          "class",
          value,
          type === "background"
            ? this["background-color-class"]
            : this["font-color-class"]
        );
      } else {
    ...
    _onCustomColor: function (e) {
      ...
      colorpicker.on("colorpicker:saved", this, (ev) => {
        ...
        $(e.target)
          ...
          .find(".o_web_studio_custom_colors")
          .append($button);
        $button.mousedown();
      });
      colorpicker.open();
    ...
    _onWidthInputChange: function (e) {
      ...
      this._editDomAttribute(
        "style",
        e.target.value && "width:" + e.target.value + "px" + addDisplayInlineBlock,
        this.originalWidth
      );
    },
  });
  ...
  var TIf = AbstractEditComponent.extend({
    ...
    start: function () {
      ...
      try {
        ...
        console.warn(
          "Can't convert the condition in an Odoo domain",
          this.node.attrs["t-if"],
          e
        );
        this.$(".o_field_domain_dialog_button").hide();
      }
      return this._super.apply(this, arguments).then(function () {
        return self.fieldSelector["t-if"].appendTo(
          self.$(".o_web_studio_tif_ifexpression")
        );
      });
    },
    ...
    _onDialogEditButtonClick: function (e) {
      ...
      var defaultDoc = _.findWhere(availableKeys, {
        relation: this.context.docs,
        type: "many2one",
      });
  ...
  var TElse = AbstractEditComponent.extend({
    ...
    init: function () {
      this._super.apply(this, arguments);
      this.tIf = this.node.parent.children[
        this.node.parent.children.indexOf(this.node) - 1
      ].attrs["t-if"];
    ...
    start: function () {
      var self = this;
      return this._super.apply(this, arguments).then(function () {
        return self.fieldSelector["t-else"].appendTo(
          self.$(".o_web_studio_telse_elseexpression")
        );
      });
    },
    /**
  ...
  var TEsc = AbstractEditComponent.extend({
    ...
    start: function () {
      var self = this;
      return this._super.apply(this, arguments).then(function () {
        return self.fieldSelector["t-esc"].appendTo(
          self.$(".o_web_studio_tesc_escexpression")
        );
      });
    },
  });
  ...
  var TSet = AbstractEditComponent.extend({
    ...
    start: function () {
      ...
      return this._super.apply(this, arguments).then(function () {
        return Promise.all([
          self.fieldSelector["t-set"].appendTo(
            self.$(".o_web_studio_tset_setexpression")
          ),
          self.fieldSelector["t-value"].appendTo(
            self.$(".o_web_studio_tset_valueexpression")
          ),
        ]);
      });
    },
    ...
    start: function () {
      ...
      return this._super.apply(this, arguments).then(function () {
        return Promise.all([
          ...
          self.fieldSelector["t-foreach"].appendTo(
            self.$(".o_web_studio_tforeach_foreachexpression")
          ),
        ]);
      });
    },
  ...
  var BlockTotal = AbstractEditComponent.extend({
    ...
    start: function () {
      ...
      return this._super.apply(this, arguments).then(function () {
        return Promise.all([
          self.fieldSelector.total_currency_id.appendTo(
            self.$(".o_web_studio_report_currency_id")
          ),
          self.fieldSelector.total_amount_untaxed.appendTo(
            self.$(".o_web_studio_report_amount_untaxed")
          ),
          self.fieldSelector.total_amount_total.appendTo(
            self.$(".o_web_studio_report_amount_total")
          ),
          self.fieldSelector.total_amount_by_groups.appendTo(
            self.$(".o_web_studio_report_amount_by_groups")
          ),
        ]);
      });
    },
  ...
  var Column = AbstractEditComponent.extend({
    ...
    init: function () {
      ...
      this.sizeClass =
        _.find(this.classes, function (item) {
      ...
      this.size = Number(this.sizeClass.split("col-")[1]);
      this.offset = Number(this.offsetClass.split("offset-")[1]);
  ...
  var TextSelectorTags =
    "span, p, h1, h2, h3, h4, h5, h6, blockquote, pre, small, u, i, b, font, strong, ul, li, dl, dt, ol, th, td";
  var filter =
    ":not([t-field]):not(:has(t, [t-" +
    QWeb2.ACTIONS_PRECEDENCE.join("], [t-field], [t-") +
    "]))";
  var Text = AbstractEditComponent.extend({
    ...
    _startWysiwygEditor: function () {
      ...
      this.$textarea.off().on("input", function (e) {
        // To test simple
        e.preventDefault();
  ...
  var Groups = AbstractEditComponent.extend({
    ...
    init: function () {
      ...
      var groups =
        this.node.attrs.studio_groups && JSON.parse(this.node.attrs.studio_groups);
  ...
  var TOptions = AbstractEditComponent.extend({
    ...
    willStart: function () {
      ...
      this.widgets = _.map(this.widgetsOptions, function (widgetConf, widgetKey) {
        ...
        var options = _.map(widgetConf, function (option, optionKey) {
          ...
          switch (option.type) {
            ...
            case "selection":
              if (
                option.params &&
                option.params.type === "selection" &&
                option.params.selection
              ) {
        ...
        options.sort(function (a, b) {
          return a.type === "boolean" && b.type === "boolean"
            ...
            : a.type === "boolean" && a.key !== "no_marker"
            ? -1
            : 1;
        });
      ...
      // Selected widget
      this.widget = _.findWhere(this.widgets, {
        key: this.values.widget && this.values.widget,
      });
    ...
    _updateWidgetOptions: function () {
      ...
      this.$("button").toggleClass(
        "hidden",
        !this.widget || !this.widget.options.length
      );
      ...
      var defs = _.map(this.widget.options, function (option) {
        var $option = $options.find(
          ".o_web_studio_toption_option_" + self.widget.key + "_" + option.key
        );
