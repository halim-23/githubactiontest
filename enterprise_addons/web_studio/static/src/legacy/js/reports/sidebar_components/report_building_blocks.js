odoo.define("web_studio.reportNewComponents", function (require) {
  ...
  var AbstractNewBuildingBlock = Abstract.extend({
    ...
    _createReportTableColumn: function (options) {
      ...
      _.each(this.targets, function (target) {
        ...
        if (node.tag === "th" || node.tag === "td") {
          var loop = Boolean(self._findParentWithTForeach(node));
      ...
      _.each(this.targets, function (target) {
        ...
        var nodeIndex = 0;
        var nodeRow = self._getParentNode(node, function (node) {
          return node.tag === "tr";
        });
        var cells = self._getChildrenNode(nodeRow, function (node) {
          return node.tag === "td" || node.tag === "th";
        });
        for (var k = 0; k < cells.length; k++) {
          nodeIndex += Number(cells[k].attrs.colspan || 1);
          if (cells[k] === node) {
            break;
          }
        }
        ...
        var table = self._getParentNode(node, function (node) {
          return node.tag === "table";
        });
        var rows = self._getChildrenNode(table, function (node) {
          return node.tag === "tr";
        });
        _.each(rows, function (row) {
          ...
          var cells = self._getChildrenNode(row, function (node) {
            return node.tag === "td" || node.tag === "th";
          });
          ...
          for (var k = 0; k < cells.length; k++) {
            var cell = cells[k];
            cellIndex += Number(cell.attrs.colspan || 1);
            if (cellIndex >= nodeIndex) {
              if (Number(cell.attrs.colspan) > 1 && cellsToGrow.indexOf(cell) === -1) {
      ...
      _.each(cellsToGrow, function (node) {
        inheritance.push({
          content:
            '<attribute name="colspan">' + (Number(node.attrs.colspan) + 1) + "</attribute>",
          position: "attributes",
          xpath: node.attrs["data-oe-xpath"],
          view_id: Number(node.attrs["data-oe-id"]),
        });
    ...
    _createContent: function (options) {
      if (this.dropColumns && typeof this.index === "number") {
        ...
        return _.map(this.targets, function (target) {
          var isCol = (target.node.attrs.class || "").match(
            /(^|\s)(col(-[0-9]+)?)(\s|$)/
          );
          return {
            content: isCol
              ? options.contentInStructure || options.content
              : options.content,
            position: target.position,
            xpath: target.node.attrs["data-oe-xpath"],
            view_id: Number(target.node.attrs["data-oe-id"]),
          };
        });
    ...
    _findParentWithTForeach: function (node) {
      if (
        !node ||
        !node.parent ||
        (node.tag === "div" && node.attrs.class === "page")
      ) {
        return;
      }
  ...
  var TextSelectorTags =
    "span, p, h1, h2, h3, h4, h5, h6, blockquote, pre, small, u, i, b, font, strong, ul, li, dl, dt, ol, .page > .row > div:empty";
  var filter =
    ":not([t-field]):not(:has(t, [t-" + QWeb2.ACTIONS_PRECEDENCE.join("], [t-") + "]))";
      ...
      return this._super.apply(this, arguments).then(function () {
        return Promise.resolve({
          inheritance: self._createContent({
            content:
              '<div class="row"><div class="col"><span>New Text Block</span></div></div>',
          }),
        });
      });
    },
    ...
    add: function () {
      ...
      return self._super.apply(this, arguments).then(function () {
        return new Promise(function (resolve, reject) {
          ...
          var availableKeys = _.filter(self._getContextKeys(target.node), function (
            field
          ) {
            ...
            return Boolean(field.relation) && field.name !== "docs";
          });
          var fieldChain = [];
  ...
  var InlineField = AbstractFieldBlock.extend({
    ...
    _filterTargets: function () {
      ...
      if (
        ...
        (target.node.tag === "td" || target.node.tag === "th")
      ) {
        target = _.find(this.targets, function (target) {
          return Boolean(self._findParentWithTForeach(target.node));
        });
      }
      return target;
    },
  ...
  var TableColumnField = AbstractFieldBlock.extend({
    ...
    _dataInheritance: function (values) {
      ...
      if (this.node.tag === "td" || this.node.tag === "th") {
        ...
        var targetInLoop = Boolean(_.find(
          this.targets,
          function (target) {
        ...
        return this._createContent({
          contentInStructure:
            "<span><strong>" + values.string + ":</strong><br/></span>" + fieldHTML,
          content: fieldHTML,
        });
    ...
    add: function () {
      ...
      return this._super.apply(this, arguments).then(function () {
        var def = new Promise(function (resolve, reject) {
          var $image = $("<img/>");
          var dialog = new weWidgets.MediaDialog(
            self,
            {
              onlyImages: true,
            },
      ...
      return this._super.apply(this, arguments).then(function () {
        return Promise.resolve({
          inheritance: [
            {
              content:
                '<div class="row"><div class="col h2"><span>New Title</span></div></div>',
              position: self.position,
              xpath: self.node.attrs["data-oe-xpath"],
              view_id: Number(self.node.attrs["data-oe-id"]),
            },
          ],
        });
      });
        ...
        self._super.apply(self, callersArguments).then(function () {
          ...
          dialog.on("field_default_values_saved", self, function (values) {
            if (!_.contains(values.related, ".")) {
              Dialog.alert(
                self,
                _t("Please specify a field name for the selected model.")
              );
              return;
            }
  ...
  var TableBlockTotal = AbstractNewBuildingBlock.extend({
    ...
    _dataInheritance: function (values) {
      ...
      return this._createContent({
        contentInStructure:
          ...
          '<t t-set="total_currency_id" t-value="' +
          data.currency_id +
          '"/>' +
          '<t t-set="total_amount_total" t-value="' +
          data.amount_total +
          '"/>' +
          '<t t-set="total_amount_untaxed" t-value="' +
          data.amount_untaxed +
          '"/>' +
          '<t t-set="total_amount_by_groups" t-value="' +
          data.amount_by_groups +
          '"/>' +
    ...
    _dataInheritanceValues: function (values) {
      ...
      if (
        values.relation === "account.move" ||
        values.relation === "sale.order" ||
        values.relation === "purchase.order"
      ) {
