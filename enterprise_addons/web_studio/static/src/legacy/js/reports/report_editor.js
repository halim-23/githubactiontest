  ...
  var ReportEditor = Widget.extend(EditorMixin, {
    ...
    _prepareHooksOnIframeBeforeDrag: function (component) {
      ...
      if (component.dropColumns) {
        ...
        if ($children.length) {
          $gridHooks
            .find(".o_web_studio_hook")
            .data("oe-node", $children.first())
            .data("oe-position", "before");
          ...
          $gridHooks
            .find(".o_web_studio_hook")
            .data("oe-node", $page)
            .data("oe-position", "inside");
          $page.prepend($gridHooks);
        }

        this.$content
          .find(".o_web_studio_structure_hook + .o_web_studio_hook")
          .remove();
        this.$content
          .find(".o_web_studio_structure_hook")
          .prev(".o_web_studio_hook")
          .remove();
      }
      ...
      this.$dropZoneStructure.removeClass(".o_web_studio_nearest").each(function () {
        $(this)
          .children()
          .children(".o_web_studio_hook:only-child")
      ...
      this.$dropZone.filter("th, td").each(function (_, item) {
        ...
        var colspan = Number($node.data("colspan"));
        if (colspan > 1) {
    ...
    dragComponent: function (component, x, y) {
      ...
      _.each(this.dropPosition, function (box) {
        box.dist = Math.sqrt(
          Math.pow(box.centerY - (y - bound.top), 2) +
            Math.pow(box.centerX - (x - bound.left), 2)
        );
      });
      ...
      if (
        !this.dropPosition[0] ||
        this.dropPosition[0].dist > this.nearest_hook_tolerance
      ) {
        return;
      }
      ...
      if (
        !$nearestHook.data("oe-node") ||
        !$nearestHook.data("oe-node").data("oe-id")
      ) {
        return;
      }
      ...
      var $nearestHooks = this.$dropZone.filter(function () {
        ...
        return (
          $hook.data("oe-position") === position &&
          $hook.data("oe-index") === index &&
          $node.data("oe-id") === id &&
          (td
            ? $node.data("oe-xpath").replace(reg, replace)
            : $node.data("oe-xpath")) === xpath
        );
      });
      ...
      if (td) {
        var pos = $nearestHook
          .data("oe-node")
          .data("td-position-" + (position === "before" ? "before" : "after"));
        $nearestHooks = $nearestHooks.filter(function () {
          var $node = $(this).data("oe-node");
          return (
            $node.data(
              "td-position-" + (position === "before" ? "before" : "after")
            ) === pos
          );
        });
      }
    ...
    dropComponent: function (component) {
      ...
      $nearestHooks.get().forEach(function (nearHook) {
        ...
        for (var i = 0; i < targets.length; i++) {
          if (
            targets[i].node.attrs["data-oe-id"] === nodeData.attrs["data-oe-id"] &&
    ...
    _createHookOnNodeAndChildren: function ($node, component) {
      ...
      if ($children.length) {
        ...
        $node.before(
          $newHook.clone().data("oe-node", $node).data("oe-position", "before")
        );
        $node.after(
          $newHook.clone().data("oe-node", $node).data("oe-position", "after")
        );
      } else {
    ...
    _connectNodes: function () {
      ...
      function connectContextOrder(dom, contextOrder) {
        ...
        if (node) {
          ...
          newOrder = node.contextOrder = _.uniq(
            contextOrder.concat(_.keys(node.context))
          );
        }
    ...
    _findAssociatedDOMNodes: function (node) {
      if (node) {
        return this.$content.find(
          '[data-oe-id="' +
            node.attrs["data-oe-id"] +
            '"][data-oe-xpath="' +
            node.attrs["data-oe-xpath"] +
            '"]'
        );
      }
        return $();
    ...
    _resizeIframe: function () {
      ...
      self.$iframe[0].style.height =
        self.$iframe[0].contentWindow.document.body.scrollHeight + "px";
    ...
    _updateContent: function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (reportHTML.error) {
          self.displayNotification({
            message: _.str.sprintf(
              _t(
                "This report could not be previewed or edited because it could not be rendered with this message: %s. This could be happening because this is a custom report type that needs custom data to be rendered and so is not editable by studio."
              ),
              reportHTML.message
            ),
            type: "danger",
        ...
        cwindow.document.open("text/html", "replace").write(reportHTML);
      });
    },
    // --------------------------------------------------------------------------
    // Handlers
