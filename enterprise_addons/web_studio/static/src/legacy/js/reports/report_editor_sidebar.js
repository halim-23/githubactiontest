odoo.define("web_studio.ReportEditorSidebar", function (require) {
  ...
  var ReportEditorSidebar = Widget.extend(StandaloneFieldManagerMixin, {
    ...
    _computeUniqueNodeName: function (node) {
      return (
        node.attrs["data-oe-id"] + node.attrs["data-oe-xpath"].replace(/\[\]\//g, "_")
      );
    },
    /**
    ...
    _getNodeDisplayName: function (node) {
      ...
      if (node.attrs) {
        ...
        if (node.attrs["t-field"] || node.attrs["t-esc"]) {
          displayName.attr +=
            "[" + (node.attrs["t-field"] || node.attrs["t-esc"]) + "]";
        }
    ...
    _getNodeEditableComponents: function (node) {
      ...
      _.each(editComponentsRegistry.map, function (Component) {
        ...
        if ($node.is(selector)) {
          // Use last because DOM eject t tag in table tag
    ...
    _startModeNew: function () {
      ...
      _.each(newComponentsRegistry.map, function (components, title) {
        ...
        _.each(components, function (Component) {
          defs.push(
            new Component(self, {models: self.models}).appendTo($componentsContainer)
          );
        });
    ...
    _startModeProperties: function () {
      ...
      if (!this.debug) {
        ...
        var pageNodeIndex = _.findIndex(this.state.nodes, function (node) {
          return (
            node.node.tag === "div" && _.str.include(node.node.attrs.class, "page")
          );
        });
        if (pageNodeIndex !== -1) {
          this.state.nodes.splice(
            pageNodeIndex + 1,
            this.state.nodes.length - (pageNodeIndex + 1)
          );
        }
      }
      ...
      this.nodes.forEach(function (node) {
        var $accordionSection = $(
          qweb.render("web_studio.AccordionSection", {
        ...
        var renderingProms = self
          ._getComponentsObject(node.components)
          .map(function (Component) {
            if (!Component) {
              self.displayNotification({
                title: "Missing component",
                message: self.state.directive,
                type: "danger",
              });
              return;
            }
            var previousWidgetState =
              self.previousState[self._computeUniqueNodeName(node.node)] &&
              self.previousState[self._computeUniqueNodeName(node.node)][
        ...
        componentsAppendedPromise = Promise.all(renderingProms).then(function () {
          ...
          $accordionSection.find(".collapse").append($("<hr>")).append($removeButton);
        });
        ...
        $accordionSection
          ...
          .find(".collapse")
          .on("show.bs.collapse hide.bs.collapse", function (ev) {
            $(this)
              .parent(".card")
              .toggleClass("o_web_studio_active", ev.type === "show");
          });
      });
    ...
    _onFieldChanged: function (ev) {
      var self = this;
      StandaloneFieldManagerMixin._onFieldChanged
        ...
        .then(function () {
          ...
          if (fieldName === "groups_id") {
            ...
            newAttrs[fieldName] =
              record.data.paperformat_id && record.data.paperformat_id.res_id;
          }
