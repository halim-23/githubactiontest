...
class TestStudioApproval(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user.write({"groups_id": [(4, cls.group_user.id)]})
        cls.manager.write({"groups_id": [(4, cls.group_manager.id)]})
    ...
    def test_00_constraints(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Shouldn't have approval on non-existing method"
        ):
            self.rule.method = "atomize"
        # check that there cannot be 2 entries for the same rule+record
        self.rule.with_user(self.manager).set_approval(
            res_id=self.record.id, approved=False
        )
        with mute_logger("odoo.sql_db"):
            with self.assertRaises(
                IntegrityError, msg="Shouldn't have 2 entries for the same rule+record"
            ):
    ...
    def test_01_single_rule(self):
        ...
        rule.active = True
        with self.assertRaises(
            UserError, msg="Should'nt validate without required group"
        ):
        ...
        self.assertFalse(
            approval_result.get("approved"), "Shouldn't have approved automatically"
        )
        ...
        self.assertTrue(
            ...
            "Should be able to proceed after manager approval",
        )
    ...
    def test_03_single_rule_with_domain(self):
        ...
        self.assertFalse(
            approval_result.get("approved"),
            "Shouldn't be able to proceed on record that matches the rule's domain",
        )
        approval_result = (
            self.env["studio.approval.rule"]
            .with_user(self.user)
        ...
        self.assertTrue(
            approval_result.get("approved"),
            "Should be able to proceed on record that doesn't match the rule's domain",
        )
    ...
    def test_04_rule_rejection(self):
        ...
        self.assertFalse(
            approval_result.get("approved"),
            "User shouldn't be able to proceed following manager rejection",
        )
        approval_result = (
            self.env["studio.approval.rule"]
        ...
        self.assertFalse(
            approval_result.get("approved"),
            "Manager shouldn't be able to proceed following own rejection",
        )
        ...
        self.assertTrue(
            approval_result.get("approved"),
            "Should be able to proceed after manager changed their mind",
        )
    ...
    def test_05_different_users(self):
        ...
        self.assertFalse(
            approval_result.get("approved"), "User shouldn't be able to proceed"
        )
        ...
        self.assertFalse(
            approval_result.get("approved"), "Manager shouldn't be able to proceed"
        )
        approval_info = (
            self.env["studio.approval.rule"]
            .with_user(self.manager)
        ...
        manager_entry = list(
            filter(
                lambda e: e["user_id"][0] == self.manager.id, approval_info["entries"]
            )
        )
        self.assertEqual(
            len(manager_entry),
            1,
            "Only one rule should have been validated by the manager",
        )
        approval_result = (
            self.env["studio.approval.rule"]
            .with_user(self.user)
        ...
        self.assertTrue(
            approval_result.get("approved"),
            "Should be able to proceed after everybody approved",
        )
    ...
    def test_06_security(self):
        ...
        user_rule.with_user(self.user).set_approval(
            res_id=self.record.id, approved=False
        )
        manager_rule.with_user(self.manager).set_approval(
            res_id=self.record.id, approved=False
        )
        with self.assertRaises(
            UserError, msg="Shouldn't be able to cancel approval of someone else"
        ):
            user_rule.with_user(self.manager).delete_approval(res_id=self.record.id)
        with self.assertRaises(
            UserError,
            msg="Shouldn't be able to create a second entry for the same record+rule",
        ):
            user_rule.with_user(self.manager).set_approval(
                res_id=self.record.id, approved=True
            )
        ...
        with self.assertRaises(
            ...
            manager_rule.with_user(self.user).set_approval(
                res_id=self.record.id, approved=True
            )
    ...
    def test_07_forbidden_record(self):
        ...
        self.env["studio.approval.rule"].with_user(self.manager).get_approval_spec(
            model=MODEL, res_id=main_company.id, method=METHOD, action_id=False
        )
        with self.assertRaises(
            AccessError,
            msg="Shouldn't be able to get approval spec on record I can't read",
        ):
            self.env["studio.approval.rule"].with_user(self.manager).get_approval_spec(
                model=MODEL, res_id=alternate_company.id, method=METHOD, action_id=False
            )
        with self.assertRaises(
            AccessError,
            msg="Shouldn't be able to set approval on record I can't write on",
        ):
            self.rule.with_user(self.manager).set_approval(
                res_id=main_company.id, approved=True
            )
    ...
    def test_08_archive(self):
        ...
        self.assertTrue(
            approval_result.get("approved"),
            "Manager should be able to proceed with archived rule even with active_test disabled",
        )
    ...
    def test_09_archive_reverse(self):
        ...
        non_exlusive_rule.with_user(self.manager).set_approval(
            res_id=self.record.id, approved=True
        )
        ...
        self.assertTrue(
            approval_result.get("approved"),
            "should be able to proceed with an exclusive rule if another entry was archived",
        )
    ...
    def test_10_exclusive_collision(self):
        ...
        self.assertTrue(
            approval_result.get("approved"), "User should be able to unlink"
        )
