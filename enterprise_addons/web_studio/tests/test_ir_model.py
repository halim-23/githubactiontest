...
class TestStudioIrModel(TransactionCase):
    ...
    def test_00_model_creation(self):
        ...
        (model, extra_models) = self.env["ir.model"].studio_model_create(
            "Rockets", options=model_options
        )
        self.assertEqual(
            len(extra_models), 1, "one extra model should have been created for stages"
        )

        line_model = self.env["ir.model"].search(
            [("model", "like", model.model + "_line")]
        )
        self.assertEqual(
            len(line_model), 1, "one extra model should have been created for lines"
        )
        ...
        expected_fields = [
            ...
            "x_studio_image",
            "x_studio_user_id",
            model.model + "_line_ids",
        ]

        self.assertTrue(
            all(
                list(filter(lambda x: item in x, created_fields))
    ...
    def test_01_mail_inheritance(self):
        ...
        self.assertEqual(
            len(extra_models), 0, "no extra model should have been created"
        )
        self.assertTrue(model.is_mail_thread, "model should inherit from mail.thread")
        # create a record
        ...
        self.assertIn(
            (self.partner_elon.id, '"Elon Tusk" <elon@spacex.com>', "Contact"),
    ...
    def test_02_model_option_active(self):
        ...
        self.assertTrue(
            default, "the default value for the x_active field should be True"
        )
        active_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_active"), ("model_id", "=", model.id)]
        )
    ...
    def test_03_model_option_sequence(self):
        ...
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_sequence", fields, "a custom sequence field should be set up"
        )
        default = self.env["ir.default"].get(model.model, "x_studio_sequence")
        self.assertEqual(
            default,
            10,
            "the default value for the x_studio_sequence field should be 10",
        )
    ...
    def test_04_model_option_responsible(self):
        ...
        self.assertIn(
            "x_studio_user_id",
            fields,
            "a custom responsible (res.users) field should be set up",
        )
        resp_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_studio_user_id"), ("model_id", "=", model.id)]
        )
        self.assertTrue(
            resp_field.tracking, "the x_studio_user_id field should be tracked"
        )
    ...
    def test_05_model_option_partner(self):
        ...
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_partner_id", fields, "a custom partner field should be set up"
        )
        self.assertIn(
            "x_studio_partner_phone",
            fields,
            "a related field x_studio_partner_phone should be set up",
        )
        self.assertIn(
            "x_studio_partner_email",
            fields,
            "a related field x_studio_partner_email should be set up",
        )
        partner_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_studio_partner_id"), ("model_id", "=", model.id)]
        )
        self.assertTrue(
            partner_field.tracking, "the x_studio_partner_id field should be tracked"
        )
    ...
    def test_06_model_option_company(self):
        ...
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_company_id", fields, "a custom company field should be set up"
        )
        ...
        self.assertEqual(
            len(mc_rule), 1, "there should be a multi-company rule for the model"
        )
        comp_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_studio_company_id"), ("model_id", "=", model.id)]
        )
        self.assertTrue(
            comp_field.tracking, "the x_studio_company_id field should be tracked"
        )
        main_company = self.env.ref("base.main_company")
        default = self.env["ir.default"].get(
            model.model, "x_studio_company_id", company_id=main_company.id
        )
        self.assertEqual(
            default,
            main_company.id,
            "the default value for the x_studio_company_id should be set",
        )
        new_company = self.env["res.company"].create({"name": "SpaceY"})
        new_default = self.env["ir.default"].get(
            model.model, "x_studio_company_id", company_id=new_company.id
        )
        self.assertEqual(
            new_default,
            new_company.id,
            "default values for new companies should be created with the company",
        )
    ...
    def test_08_model_option_date(self):
        ...
        date_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_studio_date"), ("model_id", "=", model.id)]
        )
        self.assertFalse(
            date_field.tracking, "the x_studio_date field should not be tracked"
        )
    ...
    def test_09_model_option_double_dates(self):
        ...
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_date_start", fields, "a custom start date field should be set up"
        )
        self.assertIn(
            "x_studio_date_stop", fields, "a custom stop date field should be set up"
        )
        date_fields = self.env["ir.model.fields"].search(
            [("name", "like", "x_studio_date"), ("model_id", "=", model.id)]
        )
        for date_field in date_fields:
            self.assertFalse(
                date_field.tracking, "start/stop date fields should not be tracked"
            )
    ...
    def test_10_model_option_value(self):
        ...
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_currency_id", fields, "a custom currency field should be set up"
        )
        self.assertIn(
            "x_studio_currency_id", fields, "a custom value field should be set up"
        )
        value_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_studio_value"), ("model_id", "=", model.id)]
        )
        self.assertTrue(
            value_field.tracking, "the x_studio_value field should be tracked"
        )
        main_company = self.env.ref("base.main_company")
        default = self.env["ir.default"].get(
            model.model, "x_studio_currency_id", company_id=main_company.id
        )
        self.assertEqual(
            default,
            main_company.id,
            "the default value for the x_studio_currency_id should be set",
        )
        new_company = self.env["res.company"].create(
            {"name": "SpaceY", "currency_id": self.env.ref("base.INR").id}
        )
        new_default = self.env["ir.default"].get(
            model.model, "x_studio_currency_id", company_id=new_company.id
        )
        self.assertEqual(
            new_default,
            new_company.currency_id.id,
            "default currency for new companies should be create with the company",
        )
    ...
    def test_12_model_option_stages(self):
        ...
        (model, extra_model) = self.env["ir.model"].studio_model_create(
            "Rockets", options=model_options
        )
        self.assertEqual(
            len(extra_model), 1, "an extra model should have been created for stages"
        )
        stage_fields = self.env[extra_model.model]._fields
        self.assertIn(
            "x_studio_sequence", stage_fields, "stages should have a sequence"
        )
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_stage_id", fields, "a custom stage field should be set up"
        )
        self.assertIn(
            "x_studio_priority", fields, "a custom priority field should be set up"
        )
        self.assertIn("x_color", fields, "a custom color field should be set up")
        self.assertIn(
            "x_studio_kanban_state",
            fields,
            "a custom kanban state field should be set up",
        )
        ...
        stage_field = self.env["ir.model.fields"].search(
            [("name", "=", "x_studio_stage_id"), ("model_id", "=", model.id)]
        )
        self.assertTrue(
            stage_field.tracking, "the x_studio_stage_id field should be tracked"
        )
    ...
    def test_13_model_option_tags(self):
        ...
        self.assertEqual(
            len(extra_model), 1, "an extra model should have been created for tags"
        )
        stage_fields = self.env[extra_model.model]._fields
        self.assertIn("x_color", stage_fields, "tags should have a color")
        fields = self.env[model.model]._fields
        self.assertIn(
            "x_studio_tag_ids", fields, "a custom tags field should be set up"
        )
    ...
    def test_15_custom_model_security(self):
        ...
        model_options = []
        (model, _) = self.env["ir.model"].studio_model_create(
            "Rockets", options=model_options
        )
        ...
        self.assertTrue(
            acl_admin.perm_read, "admin should have read access on custom models"
        )
        self.assertTrue(
            acl_admin.perm_write, "admin should have write access on custom models"
        )
        self.assertTrue(
            acl_admin.perm_create, "admin should have create access on custom models"
        )
        self.assertTrue(
            acl_admin.perm_unlink, "admin should have unlink access on custom models"
        )
        ...
        self.assertTrue(
            acl_user.perm_read, "user should have read access on custom models"
        )
        self.assertTrue(
            acl_user.perm_write, "user should have write access on custom models"
        )
        self.assertTrue(
            acl_user.perm_create, "user should have create access on custom models"
        )
        self.assertFalse(
            acl_user.perm_unlink, "user should not have unlink access on custom models"
        )
    ...
    def test_19_custom_model_security(self):
        ...
        model_id, name = (
            self.env["ir.model"].with_context(studio=True).name_create("X_Rockets")
        )
        acl_admin = self.env["ir.model.access"].search(
            [
                ("model_id", "=", model_id),
    ...
    def test_20_prevent_double_underscore(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Custom field names cannot contain double underscores."
        ):
            IrModelFields.create(
                {
                    "ttype": "char",
    ...
    def test_21_set_view_mode_new_window_action(self):
        ...
        self.assertEqual(
            action.view_mode,
            "tree,form",
            "tree and form should be set as a default view mode on window action",
        )
    ...
    def test_22_rename_window_action(self):
        ...
        self.assertEqual(
            action.name, new_menu.name, "action and menu name should be same"
        )
        ...
        self.assertEqual(
            action.name,
            new_menu.name,
            "rename the menu name should rename the window action name",
        )
