...
class TestReportEditor(TransactionCase):
    ...
    def test_copy_inherit_report(self):
        ...
        copy_report_html = copy_report._render_template(
            copy_report.report_name
        ).decode()
        ...
        self.assertFalse(
            copy_view.inherit_id, "copied view does not inherit another one"
        )
    ...
    def test_duplicate(self):
        ...
        duplicate_domain = [
            ("report_name", "=like", "web_studio.test_duplicate_foo_copy_%")
        ]
        ...
        copy1_view = self.env["ir.ui.view"].search([("key", "=", copy1.report_name)])
    ...
    def test_copy_custom_model_rendering(self):
        ...
        report.copy_report_and_template()
        copy = self.env["ir.actions.report"].search(
            [("report_name", "=", "base.report_irmodulereference_copy_1")]
        )
    ...
    def test_duplicate_keep_translations(self):
        ...
        self.assertEqual(set(translations.mapped("src")), {"a_", "ab", "aba"})
    ...
    def test_report_action_translations(self):
        ...
        view_ids = next(
            (leaf[2] for leaf in action["domain"] if leaf[0] == "res_id"), []
        )
        self.assertIn(view.id, view_ids)

        translations = self.env["ir.translation"].search_read(action["domain"], ["src"])
        translation = next(
            trans for trans in translations if trans["src"] == "hello test"
        )
        self.assertTrue(
            translation, 'report translations should shown in "Translations" action'
        )
