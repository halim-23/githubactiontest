...
class TestUi(odoo.tests.HttpCase):
    def test_new_app_and_report(self):
        ...
        self.start_tour(
            "/web?debug=tests", "web_studio_new_report_basic_layout_tour", login="admin"
        )

    def test_optional_fields(self):
        self.start_tour(
            "/web?debug=tests", "web_studio_hide_fields_tour", login="admin"
        )

    def test_model_option_value(self):
        self.start_tour(
            "/web?debug=tests", "web_studio_model_option_value_tour", login="admin"
        )

    def test_rename(self):
        self.start_tour(
            "/web?debug=tests", "web_studio_tests_tour", login="admin", timeout=140
        )
