...
from odoo.tools import sql, ustr
...
class WebStudioController(http.Controller):
    ...
    def is_activity_allowed(self, model):
        ...
        """
        Model = request.env[model]
        return Model._custom or isinstance(
            Model, type(request.env["mail.activity.mixin"])
        )
    ...
    def get_studio_action(self, action_name, model, view_id=None, view_type=None):
        view_type = (
            "tree" if view_type == "list" else view_type
        )  # list is stored as tree in db
        model = request.env["ir.model"]._get(model)

        action = None
        if hasattr(self, "_get_studio_action_" + action_name):
            action = getattr(self, "_get_studio_action_" + action_name)(
                model, view_id=view_id, view_type=view_type
            )
    ...
    def _get_studio_action_translations(self, model, **kwargs):
        ...
        action = {
            ...
            "view_mode": "tree",
            "views": [
                [request.env.ref("base.view_translation_dialog_tree").id, "list"]
            ],
            "target": "current",
            "domain": domain,
        }
    ...
    def create_new_app(
        ...
        if model_choice == "existing" and model_id:
            ...
            _logger.info(
                "using existing model %s as main model, all main views duplicated for use in the new app",
                model.model,
            )
        elif model_choice == "new" and menu_name:
            # create a new model
            (model, extra_models) = request.env["ir.model"].studio_model_create(
                menu_name, options=model_options
            )
            _logger.info(
                "created new model %s as main model with the following options: %s",
                model.model,
                ",".join(model_options),
            )
        else:
            _logger.error(
                "inconsistent parameters: model_choice is %s, model_id is %s and menu_name(model) is %s",
                model_choice,
                model_id,
                menu_name,
            )
            raise UserError(
                _(
                    "If you don't want to create a new model, an existing model should be selected."
                )
            )
        ...
        if extra_models:
            ...
            for extra_model in extra_models:
                ...
                configuration_menu_vals["child_id"].append(
                    (
                        0,
                        0,
                        {
                            "name": extra_model.name,
                            "action": "ir.actions.act_window," + str(extra_action.id),
                        },
                    )
                )
        ...
        )  # allows to create a menu without action
        new_menu = (
            request.env["ir.ui.menu"].with_context(new_context).create(menu_values)
        )
    ...
    def create_new_menu(
        ...
        sequence = 10
        if parent_menu_id:
            menu = request.env["ir.ui.menu"].search_read(
        ...
        if model_choice == "existing" and model_id:
            ...
            extra_models = request.env["ir.model"]
            _logger.info(
                "using existing model %s, all main views duplicated for use in the new app",
                model.model,
            )
        elif model_choice == "new" and menu_name:
            # create a new model
            ...
            _logger.info(
                "created new model %s with the following options: %s",
                model.model,
                ",".join(model_options),
            )
    ...
    def create_new_field(self, values):
        ...
        if not table_kind or table_kind == "v":
            raise UserError(
                _("The model %s doesn't support adding fields.", Model._name)
            )
        ...
        if values.get("relation_id"):
            values["relation"] = (
                request.env["ir.model"].browse(values.pop("relation_id")).model
            )
        ...
        if values.get("ttype") == "many2many":
            ...
            values["relation_table"] = request.env[
                "ir.model.fields"
            ]._get_next_relation(model_name, values.get("relation"))
    ...
    def edit_action(self, action_type, action_id, args):
        ...
        if action_id:
            ...
            if "view_mode" in args:
                args["view_mode"] = args["view_mode"].replace(
                    "list", "tree"
                )  # list is stored as tree in db
                ...
                if action_id.view_id or action_id.view_ids:
                    ...
                    missing_view_modes = [
                        x
                        for x in view_modes
                        if x not in [y.view_mode for y in action_id.view_ids]
                    ]
    ...
    def get_studio_view_arch(self, model, view_type, view_id=False):
        ...
        if not view_id:
            ...
            view_id = request.env["ir.ui.view"].default_view(
                request.env[model]._name, view_type
            )
    ...
    def _return_view(self, view, studio_view):
        ...
        fields_view = ViewModel.with_context(studio=True).fields_view_get(
            view.id, view.type
        )
        ...
        for op in operations:
            if op["type"] not in OPERATIONS_WHITELIST:
                raise ValidationError(
                    _('The operation  type "%s" is not supported', op["type"])
                )
            ...
            char_op["node"]["field_description"].update(
                {
                    ...
                    "type": "char",
                    "field_description": _(
                        "Filename for %s", op["node"]["field_description"]["name"]
                    ),
                }
            )
            ...
            if "node" in op:
                if op["node"].get("tag") == "field" and op["node"].get(
                    ...
                    if op["node"]["field_description"].get("special") == "lines":
                        field = (
                            request.env["ir.model"]._get(model)._setup_one2many_lines()
                        )
                    else:
                ...
                if (
                    op["node"].get("tag") == "filter"
                    and op["target"]["tag"] == "group"
                    and op["node"]["attrs"].get("create_group")
                ):
    ...
    def rename_field(self, studio_view_id, studio_view_arch, model, old_name, new_name):
        ...
        if field_id.ttype == "binary" and not field_id.related:
            ...
            filename_field_id = request.env["ir.model.fields"]._get(
                model, old_name + "_filename"
            )
    ...
    def edit_field(self, model_name, field_name, values, force_edit=False):
        ...
        if field.ttype == "selection" and "selection" in values:
            selection_values = [False] + [
                x[0] for x in literal_eval(values["selection"])
            ]
...
Are you sure you want to remove the selection values of those records?"""
        ...
        if field.ttype == "selection":
            current_default = request.env["ir.default"].get(
                model_name, field_name, company_id=True
            )
            if current_default:
                selection_values = literal_eval(field.selection)
                if current_default not in [x[0] for x in selection_values]:
                    request.env["ir.default"].discard_values(
                        model_name, field_name, [current_default]
                    )
    ...
    def _get_default_view(self, view_type, attrs):
        ...
        return etree.tostring(
            arch, encoding="unicode", pretty_print=True, method="html"
        )
    ...
    def _node_to_expr(self, node):
        if node.get("xpath_info") and not node.get("subview_xpath"):
            ...
            expr = "".join(
                [
                    "/%s[%s]" % (parent["tag"], parent["indice"])
                    for parent in node.get("xpath_info")
                ]
            )
        else:
    ...
    def _get_xpath_node(self, arch, operation):
        ...
        return etree.SubElement(arch, "xpath", {"expr": expr, "position": position})
    ...
    def _operation_remove(self, arch, operation, model=None):
        ...
        etree.SubElement(arch, "xpath", {"expr": expr, "position": "replace"})
    ...
    def _operation_replace(self, arch, operation, model):
        ...
        replace_xpath = etree.SubElement(
            arch, "xpath", {"expr": target_expr, "position": "replace"}
        )
    ...
    def _operation_add(self, arch, operation, model):
        ...
        def add_columns(xml_node):
            ...
            xml_node_page_left = etree.SubElement(
                xml_node_group, "group", {"name": name + "_left"}
            )
            xml_node_page_right = etree.SubElement(
                xml_node_group, "group", {"name": name + "_right"}
            )
        ...
        if node["tag"] == "notebook":
            ...
            xml_node_field = etree.Element(
                "field",
                {
                    "widget": "statinfo",
                    "name": button_count_field.name,
                    "string": node["string"] or button_count_field.field_description,
                },
            )
    ...
    def _operation_enable_approval(self, arch, operation, model):
        ...
        rule_domain = request.env["studio.approval.rule"]._get_rule_domain(
            model, method, action
        )
        ...
        matching_buttons = base_arch.findall(
            ".//button[@type='%s'][@name='%s']" % (btn_type, btn_name)
        )
        for idx, btn in enumerate(matching_buttons):
            ...
            xpath_node = etree.SubElement(
                ...
                "xpath",
                {
                    "expr": "(//button[@type='%s'][@name='%s'])[%s]"
                    % (btn_type, btn_name, idx + 1),
                    "position": "attributes",
                },
            )
    ...
    def _get_or_create_fields_for_button(self, model, field, button_name):
        ...
        if (
            field.ttype != "many2one" and field.ttype != "many2many"
        ) or field.relation != model:
            raise UserError(
                _("The related field of a button has to be a many2one to %s.", model)
            )
        ...
        button_count_field_name = (
            "x_%s_%s_count" % (field.name, field.model.replace(".", "_"))[0:63]
        )
            ...
            button_count_field = request.env["ir.model.fields"].create(
                {
                    ...
                    "compute": compute_function.replace(
                        "    ", ""
                    ),  # remove indentation for safe_eval
                }
            )
        ...
        button_action_context = "{'search_default_%s': active_id,'default_%s': %s}" % (
            field.name,
            field.name,
            active_id,
        )
    ...
    def _operation_attributes(self, arch, operation, model=None):
        ...
        if "groups" in new_attrs:
            ...
            for many2many_value in new_attrs["groups"]:
                group_xmlid = ir_model_data.search(
                    [("model", "=", "res.groups"), ("res_id", "=", many2many_value)]
                )
                if not group_xmlid:
                    raise UserError(
                        _(
        ...
        for key, new_attr in new_attrs.items():
            ...
            if key == "string" and operation.get("node", {}).get("tag") == "field":
                ...
                if (
                    field_name.startswith("x_")
                    and field_id
                    and field_id.field_description != new_attr
                ):
    ...
    def _operation_buttonbox(self, arch, operation, model=None):
        ...
        if arch.find("sheet"):
            ...
            if list(sheet_node):
                ...
                xpath_node = etree.SubElement(
                    studio_view_arch, "xpath", {"expr": "//sheet", "position": "inside"}
                )
            # Create and insert the buttonbox node inside the xpath node
            buttonbox_node = etree.Element(
    ...
    def _operation_chatter(self, arch, operation, model=None):
        ...
        if operation["remove_follower_ids"]:
            self._operation_remove(
                arch, _get_remove_field_op(arch, "message_follower_ids")
            )
    ...
    def _operation_kanban_dropdown(self, arch, operation, model):
        ...
        if not request.env["ir.model.fields"].search(
            [
                ("model_id", "=", model_id),
    ...
    def _operation_kanban_image(self, arch, operation, model):
        ...
        field_id = request.env["ir.model.fields"].search(
            [("model", "=", model), ("name", "=", operation["field"])]
        )
        if not field_id:
        ...
        if has_br_container:
            ...
            ).append(etree.fromstring(img_elem))
        else:
            etree.SubElement(
                arch,
                "xpath",
                ...
                field_id = request.env["ir.model.fields"].create(
                    {
                        ...
                        "ttype": "many2one",
                        "domain": '[("res_model", "=", "%s"), ("res_id", "=", "%s"), ("mimetype", "ilike", "image")]'
                        % (ir_model.model, ir_model.id),
                    }
                )
    ...
    def _operation_avatar_image(self, arch, operation, model):
        ...
        hasFlexH1 = (
            arch.xpath("//div[hasclass('oe_title')]//h1[hasclass('d-flex')]") and True
        )
        if hasFlexH1:
            attrs.update(
                {
    ...
    def _operation_statusbar(self, arch, operation, model=None):
        ...
        xpath_node = etree.SubElement(
            arch, "xpath", {"expr": "//form/*[1]", "position": "before"}
        )
    ...
    def get_email_alias(self, model_name):
        ...
        result = {
            "alias_domain": request.env["ir.config_parameter"].get_param(
    ...
    def set_email_alias(self, model_name, value):
        ...
        if model_id:
            email_alias = request.env["mail.alias"].search(
                [("alias_model_id", "=", model_id)], limit=1
            )
            if email_alias:
                if value:
    ...
    def get_default_value(self, model_name, field_name):
        ...
        return {
            "default_value": request.env["ir.default"].get(
                model_name, field_name, company_id=True
            )
        }
    ...
    def set_default_value(self, model_name, field_name, value):
        ...
        request.env["ir.default"].with_context(studio=True).set(
            model_name, field_name, value, company_id=True
        )

    @http.route("/web_studio/create_inline_view", type="json", auth="user")
    def create_inline_view(
        self, model, view_id, field_name, subview_type, subview_xpath
    ):
        view = request.env["ir.ui.view"].browse(view_id)
        studio_view = self._get_studio_view(view)
        if not studio_view:
        ...
        if xpath_node is None:  # bool(node) == False if node has no children
            xpath_node = etree.SubElement(
                arch, "xpath", {"expr": expr, "position": position}
            )
