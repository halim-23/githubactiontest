...
class WebStudioReportController(main.WebStudioController):
        ...
        view_document = request.env["ir.ui.view"].create(
            {
                ...
                "type": "qweb",
                "arch": etree.tostring(
                    arch_document, encoding="utf-8", pretty_print=True
                ),
            }
        )
    ...
    def get_widgets_available_options(self):
        ...
        for record in records:
            fields[record.model[14:]] = request.env[
                record.model
    ...
    def get_report_views(self, report_name, record_id=None):
        ...
        def load_arch(view_name):
            ...
            element = request.env["ir.qweb"]._get_template(
                view.id, {"full_branding": True}
            )[0]
        ...
        for group_view_id in groups:
            ...
            if view.key in request.env["ir.ui.view"].TEMPLATE_VIEWS_BLACKLIST:
                raise ValidationError(
                    _("You cannot modify this view, it is part of the generic layout")
                )
            arch = etree.fromstring(
                report_views[group_view_id]["studio_arch"], parser=parser
            )
    ...
    def edit_report_test_load_css(self):
        ...
        link_ids = [
            int(link.replace("/web/assets/", "").split("-", 1)[0]) for link in links
        ]
