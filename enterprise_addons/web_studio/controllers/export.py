...
def generate_module(module, data):
    ...
    for model in MODELS_TO_EXPORT:
        ...
        for record in records:
            ...
            if record._name == "ir.model.fields" and record.ttype == "monetary":
                ...
                rel_record = record._get(record.model, "currency_id") or record._get(
    ...
    if skipped:
        ...
        for xmlid, field, value in skipped:
            ...
            content.append(
                "Value: %s (%s)"
                % (value, ", ".join("%r" % v.display_name for v in value))
            )
            content.append("")
        yield ("warning.txt", "\n".join(content))
...
def get_relations(record, field):
    ...
    if field.model_name == "ir.model.fields":
        ...
        if field.name in ("depends", "related"):
            ...
            return record.search(
                [("model", "=", record.relation), ("name", "=", record.relation_field)]
            )
    ...
    if field.model_name == "ir.actions.act_window" and field.name in (
        "res_model",
        "binding_model",
    ):
...
def generate_field(record, field, get_xmlid):
    ...
    if field.type == "boolean":
        ...
        return E.field(
            name=field.name,
            eval="[(6, 0, [%s])]"
            % ", ".join("ref('%s')" % get_xmlid(v) for v in value),
        )
    else:
        if not value:
            return E.field(name=field.name, eval="False")
...
class MissingXMLID(Exception):
    def __init__(self, record):
        super(MissingXMLID, self).__init__(
            "Missing XMLID: %s (%s)" % (record, record.display_name)
        )
