...
class AccountReconciliation(models.AbstractModel):
    ...
    def process_bank_statement_line(self, st_line_ids, data):
        ...
        for st_line, datum in zip(st_lines, data):
            ...
            st_line.with_context(ctx).reconcile(
                datum.get("lines_vals_list", []), to_check=datum.get("to_check", False)
            )
    ...
    def get_move_lines_for_bank_statement_line(
        ...
        if mode == "rp":
            (
                query,
                params,
            ) = self._get_query_reconciliation_widget_customer_vendor_matching_lines(
            ...
            (
                query,
                params,
            ) = self._get_query_reconciliation_widget_miscellaneous_matching_lines(
                statement_line, domain=domain
            )

        trailing_query, trailing_params = self._get_trailing_query(
            statement_line, limit=limit
        )
        ...
        for line in move_lines:
            js_vals_list.append(
                self._prepare_js_reconciliation_widget_move_line(
                    statement_line, line, recs_count=recs_count
                )
            )
        return js_vals_list
    ...
    def _get_bank_statement_line_partners(self, st_lines):
        ...
        from_clause, where_clause, where_clause_params = ir_rules_query.get_sql()
        if where_clause:
            where_partner = re.sub(
                r"(?<! FROM \")\bres_partner\b", "p3", ("AND %s" % where_clause)
            )
            params += where_clause_params
        else:
            where_partner = ""
            ...
            if matching_amls[line.id].get("status") == "reconciled":
                ...
                line_vals = {
                    ...
                    "reconciliation_proposition": [
                        self._prepare_js_reconciliation_widget_move_line(line, aml)
                        for aml in amls
                    ],
                ...
                if matching_amls[line.id].get("status") == "write_off":
                    line_vals["write_off_vals"] = matching_amls[line.id][
                        "write_off_vals"
                    ]
                    self._complete_write_off_vals_for_widget(
                        line_vals["write_off_vals"]
                    )
    ...
    @api.model
    def _get_query_reconciliation_widget_liquidity_lines(
        self, statement_line, domain=[]
    ):
        ...
        tables, where_clause, where_params = self._prepare_reconciliation_widget_query(
            statement_line, domain=domain
        )
    ...
    @api.model
    def _get_query_reconciliation_widget_receivable_payable_lines(
        self, statement_line, domain=[]
    ):
        domain = domain + [
    ...
    @api.model
    def _get_query_reconciliation_widget_customer_vendor_matching_lines(
        self, statement_line, domain=[]
    ):
        ...
        (
            query_2,
            params_2,
        ) = self._get_query_reconciliation_widget_receivable_payable_lines(
            statement_line, domain=domain
        )
    ...
    @api.model
    def _get_query_reconciliation_widget_miscellaneous_matching_lines(
        self, statement_line, domain=[]
    ):
    ...
    def _prepare_js_reconciliation_widget_move_line(
        ...
        # Residual amounts.
        rec_vals_residual = statement_line._prepare_counterpart_move_line_vals(
            {}, move_line=line
        )
        if rec_vals_residual["currency_id"] != statement_line.company_currency_id.id:
            ...
            amount_currency_str = formatLang(
                self.env, abs(amount_currency), currency_obj=line.company_currency_id
            )
            total_amount_currency_str = formatLang(
                self.env,
                abs(rec_vals["debit"] - rec_vals["credit"]),
                currency_obj=line.company_currency_id,
            )
            total_amount_str = formatLang(
                self.env, abs(rec_vals["amount_currency"]), currency_obj=currency
            )
        else:
            balance = rec_vals_residual["debit"] - rec_vals_residual["credit"]
            amount_currency = 0.0
            amount_str = formatLang(
                self.env, abs(balance), currency_obj=line.company_currency_id
            )
            amount_currency_str = ""
    ...
    def _domain_move_lines_for_manual_reconciliation(
        self, account_id, partner_id=False, excluded_ids=None, search_str=""
    ):
    ...
    def _get_statement_line(self, st_line):
        ...
        if st_line.foreign_currency_id:
            ...
            amount_currency_str = formatLang(
                self.env, abs(amount_currency), currency_obj=st_line.currency_id
            )
        else:
        ...
        amount_str = formatLang(
            self.env,
            abs(amount),
            currency_obj=st_line.foreign_currency_id or st_line.currency_id,
        )
    ...
    def _prepare_writeoff_moves(self, move_lines, vals):
        ...
        currencies = {line.currency_id for line in move_lines}
        ...
        if "debit" not in vals and "credit" not in vals:
            balance = -vals.get("balance", 0.0) or sum(
                move_lines.mapped("amount_residual")
            )
        else:
        ...
        if currency == company_currency:
            ...
            if "amount_currency" in vals:
                ...
                line_vals["amount_currency"] = sum(
                    move_lines.mapped("amount_residual_currency")
                )
    ...
    def _process_move_lines(self, move_line_ids, new_mv_line_dicts):
        ...
        if len(new_mv_line_dicts) > 0:
            move_vals_list = [
                self._prepare_writeoff_moves(move_lines, vals)
                for vals in new_mv_line_dicts
            ]
    ...
    @api.model
    def get_reconciliation_dict_from_model(
        self, model_id, st_line, residual_balance, widget_partner_id
    ):
        ...
        new_aml_dicts = model._get_write_off_move_lines_dict(
            st_line, residual_balance, widget_partner_id
        )
        ...
        for line in new_aml_dicts:
            ...
            for m2o_name in (
                ...
                if line.get(m2o_name) and not isinstance(line[m2o_name], dict):
                    m2o_record = self.env[
                        self.env["account.move.line"]._fields[m2o_name].comodel_name
                    ].browse(line[m2o_name])
                    line[m2o_name] = {
                        "display_name": m2o_record.display_name,
                        "id": m2o_record.id,
                    }
            ...
            for x2m_name in ("analytic_tag_ids", "tax_ids", "tax_tag_ids"):
                if line.get(x2m_name) and not isinstance(line[x2m_name][0], dict):
                    x2m_records = self.env[
                        self.env["account.move.line"]._fields[x2m_name].comodel_name
                    ].browse(line[x2m_name][0][2])
                    line[x2m_name] = [
                        {"display_name": r.display_name, "id": r.id}
                        for r in x2m_records
                    ]
            if "reconcile_model_id" in line:
                line["to_check"] = (
                    self.env["account.reconcile.model"]
                    .browse(line["reconcile_model_id"])
                    .to_check
                )

    @api.model
    def open_rec_model_creation_widget(
        self, rec_propositions, st_line_amount, to_check
    ):
            ...
            line_vals = [
                (
                    ...
                    0,
                    {
                        ...
                        "amount_string": str(
                            round(100 * proposition["amount"] / st_line_amount, 5)
                        ),
        ...
        view_id = self.env["ir.model.data"]._xmlid_to_res_id(
            "account_accountant.view_account_reconcile_model_widget_wizard"
        )
        return {
