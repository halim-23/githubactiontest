...
class AccountMoveLine(models.Model):
    ...
    def _compute_attachment(self):
        for record in self:
            record.move_attachment_ids = self.env["ir.attachment"].search(
    ...
    def action_reconcile(self):
        ...
        if (
            len(all_accounts) == 2
            and "payable" in account_types
            and "receivable" in account_types
        ):

            if len(all_partners) != 1:
                raise UserError(
                    _(
                        "You cannot reconcile the payable and receivable accounts of multiple partners together at the same time."
                    )
                )
            ...
            for account in all_accounts:
                account_total = abs(
                    sum(
                        line.balance
                        for line in self.filtered(lambda x: x.account_id == account)
                    )
                )
