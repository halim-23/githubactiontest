...
class TestReconciliationWidget(TestAccountReconciliationCommon):
    def _get_st_widget_suggestions(self, st_line, mode="rp"):
        suggestions = self.env[
            "account.reconciliation.widget"
        ].get_move_lines_for_bank_statement_line(st_line.id, mode=mode)
    ...
    def test_reconciliation_process_move_lines_with_mixed_currencies(self):
        ...
        writeoff_line = self.env["account.move.line"].search(
            [("name", "=", "writeoff"), ("company_id", "=", self.company.id)]
        )
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic(self):
        ...
        inv2 = self.create_invoice(
            move_type="out_refund", invoice_amount=140, currency_id=self.currency_usd_id
        )
    ...
    def test_get_reconciliation_dict_with_tag_ids(self):
        ...
        res = self.env[
            "account.reconciliation.widget"
        ].get_reconciliation_dict_from_model(
            reconciliation_model.id,
            bank_stmt_line.id,
            -bank_stmt_line.amount,
            bank_stmt_line.partner_id,
        )
    ...
    def test_prepare_writeoff_moves_multi_currency(self):
        for invoice_type in ("out_invoice", "in_invoice"):
            ...
            account = invoice.line_ids.filtered(
            ...
            write_off_vals = self.env[
                "account.reconciliation.widget"
            ]._prepare_writeoff_moves(
                lines,
                {
            ...
            self.assertRecordValues(
                write_off.line_ids.sorted("balance"),
                [
                    {
                        "partner_id": self.partner_a.id,
                        "currency_id": self.currency_data["currency"].id,
            ...
            # Reconcile.
            all_lines = (invoice + payment.move_id + write_off).line_ids.filtered(
                lambda line: line.account_id == account
            )
            all_lines.reconcile()
    ...
    def test_st_line_suggestion_reconcile_account_on_bank_journal(self):
        ...
        suggested_amls = self._get_st_widget_suggestions(
            statement.line_ids, mode="misc"
        )
        self.assertRecordValues(
            suggested_amls,
            [
    ...
    def test_manual_writeoff_with_tax_opw2689002(self):
        ...
        widget.process_bank_statement_line(
        ...
        bank_statement_move_line = bank_statement.move_line_ids.search(
            [("debit", "=", 200.0)]
        )
                ...
                "new_mv_line_dicts": [
                    {
                        ...
                        if "journal_id" in line
                        else False,
                        "name": line["name"],
                        "reconcile_model_id": line["reconcile_model_id"],
                        "tax_ids": [
                            (6, 0, [x["id"] for x in line["tax_ids"]])
                            if "tax_ids" in line
                            else None
                        ],
                        "tax_repartition_line_id": line.get(
                    ...
                    for line in widget.get_reconciliation_dict_from_model(
                        ...
                        None,
                        residual_balance=42,
                        widget_partner_id=self.partner_agrolait.id,
                    )
                ],
            }
        ]
