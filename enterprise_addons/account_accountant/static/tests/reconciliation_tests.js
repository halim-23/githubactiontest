      ...
      json_friendly_compute_all: function (args) {
        ...
        for (let i = 0; i < tax.invoice_repartition_line_ids.length; i++) {
          ...
          if (rep_ln.repartition_type == "base") {
            tax_base =
              (tax.price_include ? (amount * 100) / (100 + tax.amount) : amount) *
    ...
    "account.reconciliation.widget": {
      ...
      get_move_lines_for_bank_statement_line: function (args, kwargs) {
        ...
        var key = JSON.stringify({
          st_line_id: kwargs.st_line_id,
          search_str: kwargs.search_str,
        });
          ...
          return [
            {
              ...
              account_id: {
                id: 285,
                display_name: "101120 Stock Interim Account (Received)",
              },
              tax_ids: [{id: 6, display_name: "Tax 20.00%"}],
            ...
            },
            {
              ...
              is_tax: true,
              account_code: "101300",
              account_id: {id: 288, display_name: "101300 Tax Paid"},
              date: today,
              name: "ATOS Banque Tax 20.00%",
              credit: 229.13,
              debit: 0,
              balance: -229.13,
              __focus: false,
            ...
            },
            {
              ...
              account_id: {
                id: 286,
                display_name: "101130 Stock Interim Account (Delivered)",
              },
              tax_ids: [{id: 7, display_name: "Tax 10.00% include"}],
            ...
            },
            {
              ...
              account_id: {id: 288, display_name: "101300 Tax Paid"},
              date: today,
              name: "ATOS Frais Tax 10.00% include",
              credit: 2.68,
              debit: 0,
              balance: -2.68,
              __focus: false,
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("Reconciliation basic data", async function (assert) {
        ...
        assert.strictEqual(
          clientAction.widgets[1].$('.mv_line .cell_right:contains(".")').length,
          3,
          "should display only the credit account move lines (hide the debit)"
        );
        assert.containsN(
        ...
        // Load more
        assert.ok(
          clientAction.widgets[1].$(".match:first div.load-more a:visible").length,
          "should display the 'load more' button"
        );
        assert.equal(
          clientAction.widgets[1].$(".match:first div.load-more span").text(),
          6,
          "should display 6 items remaining"
        );
        await testUtils.dom.click(
        ...
        assert.ok(
          clientAction.widgets[0].$("caption button.btn-secondary:visible").length,
        ...
        assert.ok(
          clientAction.widgets[2].$("caption button.btn-primary:visible").length,
      ...
      QUnit.test("Reconciliation partial [REQUIRE FOCUS]", async function (assert) {
        ...
        await testUtils.dom.click(widget.$(".accounting_view thead td:first"));
        assert.strictEqual(
          widget.$(".match:first .mv_line[data-line-id=114] .cell_account_code:first()")
            .length,
          1,
          "Should have line"
        );
        await testUtils.dom.click(
          widget.$(".match:first .mv_line[data-line-id=114] .cell_account_code")
        );
        ...
        await testUtils.fields.editAndTrigger(widget.$(".o_input_dropdown input"), "", [
          "keyup",
          "blur",
        ]);
        ...
        assert.strictEqual(
          widget.$(".accounting_view .cell_left .edit_amount_input").length,
          1,
        ...
        await testUtils.fields.editAndTrigger(
          widget.$(".accounting_view .cell_left .edit_amount_input:not(.d-none)"),
          "10000",
          ["blur"]
        );
        assert.strictEqual(
      ...
      QUnit.test(
        "Reconciliation with rounding different representation (but same value) is not partial",
        async function (assert) {
          ...
          this.params.data_widget = [
            {
              st_line: {
                ...
                journal_id: 7,
                statement_id: 2,
                account_id: [40, "101402 Bank"],
                account_code: "101402",
                account_name: "Bank",
                partner_name: "Deco Addict",
                communication_partner_name: false,
                amount_currency_str: "",
                amount_currency: 956.06,
                has_no_partner: false,
                company_id: 1,
                open_balance_account_id: 6,
              },
              reconciliation_proposition: [
                {
                  id: 36,
          ...
          this.params.move_lines_for_manual_reconciliation = {
            '[283,null,"",0]': [
              {
                id: 26,
                name: "INV/2021/08/0001",
                ref: "",
                date: "08/25/2021",
                date_maturity: "08/25/2021",
                account_id: [6, "121000 Account Receivable"],
                account_code: "121000",
                account_name: "Account Receivable",
                account_type: "receivable",
                ...
                debit: 365125,
                credit: 0,
                amount_str: "$ 365,125.00",
                amount_currency: 0,
                amount_currency_str: "",
                total_amount_currency_str: "",
                total_amount_str: "$ 365,125.00",
                recs_count: 3,
              },
              {
                id: 20,
                name: "INV/2021/09/0002",
                ref: "",
                date: "09/08/2021",
                ...
                debit: 169625,
                credit: 0,
                amount_str: "$ 169,625.00",
                amount_currency: 0,
                amount_currency_str: "",
                total_amount_currency_str: "",
                total_amount_str: "$ 169,625.00",
                recs_count: 3,
              },
              {
                id: 23,
                name: "INV/2021/09/0003",
                ref: "",
                date: "09/08/2021",
                date_maturity: "10/08/2021",
                account_id: [6, "121000 Account Receivable"],
                ...
                debit: 143750,
                credit: 0,
                amount_str: "$ 143,750.00",
                amount_currency: 0,
                amount_currency_str: "",
                total_amount_currency_str: "",
                total_amount_str: "$ 143,750.00",
                recs_count: 3,
              },
            ],
          };
          ...
          assert.strictEqual(
            widget.$('.accounting_view div[name="partner_id"] input').val(),
            "Deco Addict",
            "Display the partner in input"
          );
          assert.strictEqual(
            widget.$(".accounting_view thead .cell_left").text(),
            "$ 956.06",
            "Display the amount on left."
          );
          assert.strictEqual(
            widget
              .$(".accounting_view thead")
              .text()
              .replace(/[\s\n]+/g, " ")
              .trim(),
            "101402 09/09/2021 test $ 956.06",
            "Display the line to reconcile"
          );
          assert.strictEqual(
            widget.$(".accounting_view .strike_amount").length,
            0,
            "Should not display the proposal as partial reconcile."
          );
          assert.strictEqual(
            widget
              ...
              .replace(/[\s\n]+/g, " ")
              .trim(),
            "121000​ 10/09/2021 INV/2021/09/0004 $ 956.06",
            "Display the line proposal to reconcile"
          );
      ...
      QUnit.test("Reconciliation create line", async function (assert) {
        ...
        testUtils.mock.intercept(clientAction, "do_action", function (event) {
          assert.strictEqual(
            ...
            JSON.stringify(self.params.test_rec_model_wizard_action),
      ...
      QUnit.test("Reconciliation fetch correct reconciliation models", async function (
        assert
      ) {
        assert.expect(1);
      ...
      QUnit.test("Reconciliation manual", async function (assert) {
        ...
        assert.strictEqual(
          clientAction
            ...
            .data("content"),
          "10,222.00 €",
          "should display the monetary information in €"
        );
        ...
        await testUtils.dom.click(
          clientAction.$('.o_reconciliation_line:eq(1) [data-line-id="21"] .cell_label')
        );
        await testUtils.dom.click(
          clientAction.$('.o_reconciliation_line:eq(1) [data-line-id="22"] .cell_label')
        );
        ...
        assert.strictEqual(
          clientAction.$(".o_reconciliation_line:eq(1) .o_reconcile:visible").length,
          1,
        ...
        await testUtils.dom.click(
          clientAction.$('.o_reconciliation_line:eq(1) [data-line-id="23"] .cell_label')
        );
        await testUtils.dom.click(
          clientAction.$('.o_reconciliation_line:eq(1) [data-line-id="24"] .cell_label')
        );
        ...
        assert.strictEqual(
          clientAction.$(".o_reconciliation_line:eq(1) .o_validate:visible").length,
          1,
      ...
      QUnit.test("Reconciliation: Payment < inv1 + inv2(partial)", async function (
        assert
      ) {
        assert.expect(3);
        ...
        var indexModif = _.findIndex(
          this.params.mv_lines['{"st_line_id":5,"search_str":""}'],
          function (line) {
            return line.id === 112;
          }
        );
      ...
      QUnit.test("Reconciliation: payment and 2 partials", async function (assert) {
        ...
        assert.strictEqual(
          widget
            .$(".accounting_view .cell_right .line_amount")
            .text()
            .replace(/[\n\r\s]+/g, " "),
          " $ 1200.00 $ 1175.00 ",
        ...
        await testUtils.dom.click(
          widget.$(".accounting_view .cell_right .edit_amount")
        );
        await testUtils.fields.editAndTrigger(
        ...
        await testUtils.dom.click(
          writeOffCreate.find(".create_account_id input.ui-autocomplete-input")
        );
      ...
      QUnit.test("Tax on account receivable", async function (assert) {
        ...
        await testUtils.mock.addMockEnvironment(clientAction, {
          ...
          mockRPC: function (route, args) {
            ...
            if (args.method === "process_move_lines") {
              ...
              var idx = _.has(args.args[0][0].new_mv_line_dicts[0], "journal_id")
                ? 0
                : 1;
              assert.deepEqual(
                _.pick(
        ...
        await testUtils.dom.click($reconcileForm.find(".create_journal_id input"), {
          allowInvisible: true,
        });
        ...
        var idx =
          $($($newLines[0]).find("td")[3]).text().trim() === "dummy text" ? 0 : 1;
        ...
        await testUtilsMock.addMockEnvironment(clientAction, {
          ...
          mockRPC: function (route, args) {
            if (args.method === "process_bank_statement_line") {
              ...
              assert.deepEqual(lines_vals_list[0].analytic_tag_ids, [
                [6, null, [1, 2]],
              ]);
