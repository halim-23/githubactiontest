  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("No preview on small devices", async function (assert) {
        ...
        assert.containsOnce(list, ".o_move_line_list_view", "the class should be set");
        assert.containsNone(
          list,
      ...
      QUnit.test("Fetch and preview of attachments on big devices", async function (
        assert
      ) {
        assert.expect(21);
        ...
        assert.containsOnce(
          list,
          ".o_attachment_preview .o_move_line_empty",
        ...
        assert.containsOnce(
          list,
          ".o_attachment_preview .o_move_line_without_attachment",
        ...
        assert.hasAttrValue(
          list.$(".o_attachment_preview iframe"),
          "data-src",
      ...
      QUnit.test("group buttons are toggled when hovering the group", async function (
        assert
      ) {
        assert.expect(12);

        this.data["account.move.line"].fields.attachment_id = {
          type: "many2one",
