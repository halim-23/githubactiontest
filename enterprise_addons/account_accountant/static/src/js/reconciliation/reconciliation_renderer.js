odoo.define("account.ReconciliationRenderer", function (require) {
  ...
  } = require("@account_accountant/js/reconciliation/reconciliation_rainbowman_component");
  ...
  var LineRenderer = Widget.extend(FieldManagerMixin, {
    ...
    update: function (state) {
      ...
      this.$('.o_notebook li a[href*="notebook_page_' + state.mode + '"]').addClass(
        "active"
      );
      this.$(
        '.o_notebook .tab-content .tab-pane[id*="notebook_page_' + state.mode + '"]'
      ).addClass("active");
      ...
      _.each(props, function (line) {
        ...
        if (!isNaN(line.id)) {
          $('<span class="line_info_button fa fa-info-circle"/>')
            ...
            .attr(
              "data-content",
              qweb.render("reconciliation.line.mv_line.details", {line: line})
            );
        }
        $props.append($line);
      });
      ...
      for (let i = 0; i < matching_modes.length; i++) {
        ...
        var $mv_lines = this.$(
          'div[id*="notebook_page_' + matching_modes[i] + '"] .match table tbody'
        ).empty();
        this.$('.o_notebook li a[href*="notebook_page_' + matching_modes[i] + '"]')
          .parent()
          .toggleClass(
            "d-none",
        ...
        _.each(stateMvLines, function (line) {
          var $line = $(
            qweb.render("reconciliation.line.mv_line", {line: line, state: state})
          );
          if (!isNaN(line.id)) {
            $('<span class="line_info_button fa fa-info-circle"/>')
              .appendTo($line.find(".cell_info_popover"))
        ...
        this.$(
          'div[id*="notebook_page_' + matching_modes[i] + '"] .match div.load-more'
        ).toggle(remaining > 0);
        this.$(
          'div[id*="notebook_page_' + matching_modes[i] + '"] .match div.load-more span'
        ).text(remaining);
      }
              ...
              _.each(state.createForm.analytic_tag_ids, function (tag) {
                defs.push(
                  self.model.notifyChanges(self.handleCreateRecord, {
                    analytic_tag_ids: {operation: "ADD_M2M", ids: tag},
                  })
                );
              });
              return Promise.all(defs);
    ...
    _renderCreate: function (state) {
      ...
      return this.model
        ...
        .then(function (recordID) {
          ...
          self.fields.name = new basic_fields.FieldChar(self, "name", record, {
            mode: "edit",
          });
    ...
    _onCreateReconcileModel: function (event) {
      ...
      for (var i = 0; i < this._initialState.reconciliation_proposition.length; i++) {
        ...
        if (Number.isInteger(rec_proposition.id)) {
          ...
          !rec_proposition.tax_repartition_line_id &&
          rec_proposition.account_id
        ) {
          propositions_for_model.push({
            ...
            tax_ids:
              (rec_proposition.tax_ids && rec_proposition.tax_ids.map((x) => x.id)) ||
              [],
            label: rec_proposition.name,
          });
        }
      ...
      var proposition_base_amount =
        st_line_vals.amount != undefined ? st_line_vals.amount : base_lines_balance;

      return this._rpc({
        model: "account.reconciliation.widget",
        method: "open_rec_model_creation_widget",
        args: [
          propositions_for_model,
          Math.abs(proposition_base_amount),
          this._initialState.to_check,
        ],
      }).then(function (rslt) {
        self.do_action(rslt, {
          on_close: function () {
    ...
    _onQuickCreateProposition: function (event) {
      ...
      this.trigger_up("quick_create_proposition", {
        data: $(event.target).closest("button").data("reconcile-model-id"),
      });
    },
    /**
     * @private
    ...
    _onCreateProposition: function () {
      ...
      if (invalid.length) {
        this.displayNotification({
          title: _t("Some fields are undefined"),
