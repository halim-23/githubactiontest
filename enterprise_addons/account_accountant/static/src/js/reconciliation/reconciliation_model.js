odoo.define("account.ReconciliationModel", function (require) {
  ...
  var StatementModel = BasicModel.extend({
    ...
    addProposition: function (handle, mv_line_id) {
      ...
      if (!prop.is_liquidity_line) {
        ...
        if (
          balance > 0.0 &&
          this._amountCompare(prop_amount, balance, line.st_line.currency_id) > 0
        ) {
    ...
    changeMode: function (handle, mode) {
      ...
      if (mode === "next") {
        ...
        mode =
          available_modes[
            (available_modes.indexOf(line.mode) + 1) % available_modes.length
          ];
      }
      line.mode = mode;
    ...
    __reload: function () {
      ...
      var def_statement = this._rpc({
        ...
        self.bank_statement_id = statement.statement_id
          ? {id: statement.statement_id, display_name: statement.statement_name}
          : false;
    ...
    getPartialReconcileAmount: function (handle, data) {
      ...
      if (prop) {
        ...
        if (
          Math.abs(line.balance.amount) < Math.abs(amount) &&
          partial <= Math.abs(prop.amount) &&
          partial >= 0
        ) {
          amount = partial;
        }
        ...
        _.each(self.lines, function (other_line) {
          if (other_line != line) {
            var filtered_prop = other_line.reconciliation_proposition.filter(
              (p) => !line.reconciliation_proposition.map((l) => l.id).includes(p.id)
            );
    ...
    _addProposition: function (line, prop) {
      _.each(
        this.modes.filter((x) => x.startsWith("match")),
        (mode) => {
          line["mv_lines_" + mode] = line["mv_lines_" + mode].filter(
            (p) => p.id != prop.id
          );
        }
      );
    ...
    _computeLine: function (line) {
      ...
      _.each(line.reconciliation_proposition, function (prop) {
        ...
        if (prop.tax_repartition_line_id) {
          if (
            !_.find(line.reconciliation_proposition, {id: prop.link}).__tax_to_recompute
          ) {
      ...
      return Promise.all(tax_defs).then(function () {
        ...
        var isOtherCurrencyId = _.uniq(
          _.pluck(_.reject(reconciliation_proposition, "invalid"), "currency_id")
        );
        isOtherCurrencyId =
        ...
        _.each(reconciliation_proposition, function (prop) {
          if (!prop.invalid) {
            ...
            if (isOtherCurrencyId) {
              amount_currency -=
                (prop.amount < 0 ? -1 : 1) * Math.abs(prop.amount_currency);
            }
          }
        });
    ...
    _formatMany2ManyTagsTax: function (value) {
      ...
      for (var i = 0; i < value.length; i++) {
        res.push({
          id: value[i],
          display_name: this.taxes[value[i]] ? this.taxes[value[i]].display_name : "",
        });
      }
      return res;
    },
            ...
            .then(function () {
              if (
                ...
                var hasDifferentPartners = function (prop) {
                  return (
                    !prop.partner_id ||
                    prop.partner_id != line.reconciliation_proposition[0].partner_id
                  );
                };
    ...
    _refresh_partial_rec_preview: function (line) {
      ...
      _.each(line.reconciliation_proposition, (proposition) => {
        var prop_amount =
          st_line_balance_left < 0 ? proposition.credit : proposition.debit;

        // Invoice matching reconciliation models may have defined some write off rules as well
        var write_off_balances = (line.write_off_vals || []).map(function (elem) {
          return elem.balance;
        });
        var write_off_amount = write_off_balances.reduce(function (prev, cur) {
          return prev + cur;
        }, 0);

        var prop_impact = Math.min(
          Math.abs(st_line_balance_left + write_off_amount),
          prop_amount
        );
        ...
        if (
          prop_impact > 0 &&
          this._amountCompare(prop_impact, prop_amount, line.st_line.currency_id) !== 0
        ) {
          // Then it'll be a partial reconciliation
          proposition.partial_amount = signed_impact;
          proposition.partial_amount_str = field_utils.format.monetary(
            Math.abs(proposition.partial_amount),
            {},
            {currency_id: line.st_line.currency_id}
          );
        } else {
    ...
    _formatMoveLine: function (handle, mode, mv_lines) {
      ...
      line["mv_lines_" + mode] = _.uniq(
        [].concat(line["mv_lines_" + mode] || [], mv_lines),
        (l) => l.id
      );
      if (mv_lines[0]) {
     ...
     * @returns -1, 0 or 1: if ``value1`` is (resp.) lower than,
    ...
    _formatQuickCreate: function (line, values) {
      ...
      if (prop.base_amount) {
        ...
        var amount = _.unescape(
          field_utils.format.monetary(Math.abs(prop.base_amount), {}, formatOptions)
        );
    ...
    _isValid: function (prop) {
      return (
        !isNaN(prop.id) ||
        (prop.account_id && prop.amount && prop.name && Boolean(prop.name.length))
      );
    },
    /**
  ...
  var ManualModel = StatementModel.extend({
    ...
    __load: function (context) {
      ...
      var company_ids =
        (context && context.company_ids) || session_allowed_company_ids.slice(0, 1);
      if (company_ids) {
    ...
    _performMoveLine: function (handle, mode, limit) {
      ...
      var line = this.getLine(handle);
      var excluded_ids = _.map(
        _.union(line.reconciliation_proposition, line["mv_lines_" + mode]),
        function (prop) {
          return _.isNumber(prop.id) ? prop.id : null;
        }
      ).filter((id) => id != null);
    ...
    _getDefaultMode: function (handle) {
      ...
      if (
        line.balance.amount === 0 &&
        (!line.st_line.mv_lines_match || line.st_line.mv_lines_match.length === 0)
      ) {
        return "inactive";
      }
    ...
    _formatMoveLine: function (handle, mode, mv_lines) {
      ...
      line.mv_lines_match = _.uniq(
        [].concat(line.mv_lines_match || [], mv_lines),
        (l) => l.id
      );
      if (mv_lines[0]) {
