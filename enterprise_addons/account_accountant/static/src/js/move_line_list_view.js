odoo.define("account_accountant.MoveLineListView", function (require) {
  ...
  var AttachmentViewer = require("@mail_enterprise/js/attachment_viewer")[
    Symbol.for("default")
  ];
  ...
  var AccountMoveListController = ListController.extend({
    ...
    init: function () {
      ...
      this.hide_attachment = Boolean(this.initialState.context.hide_attachment);
    ...
    _renderAttachmentPreview: function (recordId) {
      var self = this;
      if (
        _.filter(this.model.localData, function (value, key, object) {
          return value.groupData == self.last_selected;
        }).length
      ) {
        recordId = _.filter(this.model.localData, function (value, key, object) {
          return value.groupData == self.last_selected;
        })[0].data[0];
      }
      if (!recordId) {
      ...
      var prom;
      if (
        !_.isEqual(_.pluck(this.currentAttachments, "id"), _.pluck(attachments, "id"))
      ) {
        ...
        prom = this.attachmentViewer
          .appendTo(this.$attachmentPreview.empty())
          .then(function () {
    ...
    _onRowSelected: function (ev) {
      if (config.device.size_class >= config.device.SIZES.XXL) {
        ...
        if (this.last_selected.includes("line")) {
          // If it comes from _onToggleGroup, this._update is triggered but not if it comes from _selectRow
  ...
  var AccountMoveListRenderer = ListRenderer.extend({
    ...
    _renderGroupRow: function (group, groupLevel) {
      ...
      if (this.state.context.name_groupby) {
        ...
        text_node.nodeValue = _.escape(text_node.nodeValue)
          .replace(/(\*\*)(.*)\1/g, "<strong>$2</strong>")
          .replace(/\s+\([0-9]+\)/, ""); // We only change the value of the text and not eh html to keep the listeners on the buttons
    ...
    _onToggleGroup: function (ev) {
      ...
      if (
        group.model === "account.move.line" &&
        group.groupData &&
        group.groupData.model === "account.move"
      ) {
        this.trigger_up("row_selected", {
    ...
    _onToggleGroupButton: function (ev) {
      ...
      let groupHeader;
      if (
        tr.classList.contains("o_group_header") &&
        tr.querySelector(".o_group_buttons")
      ) {
