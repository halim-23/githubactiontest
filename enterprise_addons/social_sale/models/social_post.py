...
class SocialPost(models.Model):
    ...
    def _compute_sale_quotation_count(self):
        ...
        if has_so_access and self.utm_source_id.ids:
            ...
            mapped_data = {
                datum["source_id"][0]: datum["source_id_count"]
    ...
    def _compute_sale_invoiced_amount(self):
        has_account_move_access = self.env["account.move"].check_access_rights(
            "read", raise_exception=False
        )
        if has_account_move_access and self.utm_source_id.ids:
            query = """SELECT move.source_id as source_id, -SUM(line.balance) as price_subtotal
                        ...
                        """
            self._cr.execute(
                query, [tuple(self.utm_source_id.ids), tuple(self.env.companies.ids)]
            )
            query_res = self._cr.dictfetchall()
            mapped_data = {
                datum["source_id"]: datum["price_subtotal"] for datum in query_res
            }
