...
class TestHelpdeskAccount(common.HelpdeskCommon):
    ...
    def test_helpdesk_account_1(self):
        ...
        partner = self.env["res.partner"].create({"name": "Customer Credee"})
        ...
        credit_note_form = Form(
            ...
            view=self.env.ref(
                "helpdesk_account.view_account_move_reversal_inherit_helpdesk_account"
            ),
        )
        ...
        self.assertEqual(
            refund.ref,
            "Reversal of: %s, %s" % (invoice.name, credit_note_form.reason),
            "The reference is wrong",
        )
        self.assertEqual(
            len(ticket.invoice_ids), 1, "The ticket should be linked to a credit note"
        )
        self.assertEqual(
            ticket.invoices_count, 1, "The ticket should be linked to a credit note"
        )
        self.assertEqual(
            refund[0].id,
