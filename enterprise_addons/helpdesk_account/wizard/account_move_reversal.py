...
class AccountMoveReversal(models.TransientModel):
    ...
    helpdesk_sale_order_id = fields.Many2one(
        "sale.order", related="helpdesk_ticket_id.sale_order_id", string="Sales Order"
    )
    suitable_move_ids = fields.Many2many(
        "account.move", compute="_compute_suitable_moves"
    )

    @api.depends(
        "helpdesk_ticket_id.sale_order_id.invoice_ids",
        "helpdesk_ticket_id.partner_id.commercial_partner_id",
    )
    def _compute_suitable_moves(self):
        for r in self:
            ...
            if r.helpdesk_ticket_id.sale_order_id:
                domain.append(
                    ("id", "in", r.helpdesk_ticket_id.sale_order_id.invoice_ids.ids)
                )
                ...
                domain.append(
                    (
                        "partner_id",
                        "child_of",
                        r.helpdesk_ticket_id.partner_id.commercial_partner_id.id,
                    )
                )
