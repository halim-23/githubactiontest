...
class L10nArAfipWsConsult(models.TransientModel):
    ...
    consult_type = fields.Selection(
        [("specific", "Specific Invoice Number"), ("last", "Get Last Invoice")],
        required=True,
        default="specific",
        string="Type",
        help="* Specific Invoice Number: consult all the invoice information in AFIP"
    ...
    def onchange_journal(self):
        self.document_type_id = (
            self.available_document_type_ids[0]
            if self.available_document_type_ids
            else False
        )
    ...
    def _compute_available_document_types(self):
        ...
        for rec in with_journal:
            ...
            rec.available_document_type_ids = (
                self.env["l10n_latam.document.type"].search([("code", "in", codes)])
                if codes
                else False
            )
    ...
    def onchange_last_invoice(self):
        ...
        if self.consult_type == "last":
            ...
            self.number = self.journal_id._l10n_ar_get_afip_last_invoice_number(
                self.document_type_id
            )
        else:
            self.number = 0
