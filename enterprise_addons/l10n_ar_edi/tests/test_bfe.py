...
@tagged(
    "bfe", "ri", "-at_install", "external_l10n", "post_install", "-standard", "external"
)
class TestBfe(common.TestEdi):
    ...
    def _test_case(
        ...
        if "_b" in document_type:
            forced_values.update(
                {
                    "partner": self.partner_mipyme,
                    "document_type": self.document_type[document_type],
                }
            )
        return super()._test_case(
            document_type,
            concept,
    ...
    def _test_case_credit_note(
        ...
        return super()._test_case_credit_note(
            document_type, invoice, data=data, expected_result=expected_result
        )
    ...
    def test_01_consult_invoice(self):
        ...
        self.assertTrue(
            invoice.message_ids.filtered(
                lambda x: "AFIP Validation Observation: 13" in x.body
            )
        )
    ...
    def test_20_iibb_sales_ars(self):
        ...
        invoice.invoice_line_ids.filtered(lambda x: x.tax_ids).tax_ids = [
            (4, iibb_tax.id)
        ]
        self.assertIn(
            iibb_tax.name, invoice.invoice_line_ids.mapped("tax_ids").mapped("name")
        )
