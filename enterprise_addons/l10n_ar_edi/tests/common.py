...
class TestEdi(TestAr):
    ...
    def _create_afip_connections(self, company, afip_ws, cert_file):
        ...
        company.l10n_ar_afip_ws_crt = base64.b64encode(
            file_open("l10n_ar_edi/tests/" + cert_file, "rb").read()
        )
        company.l10n_ar_afip_ws_key = base64.b64encode(
            file_open("l10n_ar_edi/tests/private_key.pem", "rb").read()
        )
        _logger.log(
            25, "Setting homologation private key to company %s" % (company.name)
        )
        ...
        while not checked_certificate_token:
            try:
                ...
                company.l10n_ar_afip_ws_crt = base64.b64encode(
                    file_open(
                        "l10n_ar_edi/tests/test_cert%d.crt" % new_cert_number, "rb"
                    ).read()
                )
                _logger.log(
                    25,
                    "Setting demo certificate from %s to %s in %s company"
    ...
    def _prepare_multicurrency_values(self):
        ...
        _date, value = USD.with_context(
            l10n_ar_invoice_skip_commit=True
        )._l10n_ar_get_afip_ws_currency_rate()
    ...
    def _test_connection(self):
        ...
        with self.assertRaisesRegex(
            UserError,
            '"Check Available AFIP PoS" is not implemented in testing mode for webservice',
        ):
            self.journal.with_context(
                l10n_ar_invoice_skip_commit=True
            ).l10n_ar_check_afip_pos_number()
    ...
    def _test_consult_invoice(self, expected_result=None):
        ...
        last = invoice.journal_id._l10n_ar_get_afip_last_invoice_number(
            invoice.l10n_latam_document_type_id
        )
        document_parts = invoice._l10n_ar_get_document_number_parts(
        ...
        with self.assertRaisesRegex(
            UserError, "(CodAutorizacion|Cae).*%s" % invoice.l10n_ar_afip_auth_code
        ):
    ...
    def _test_case(
        ...
        create_invoice = {
            "product": self._create_invoice_product,
        ...
        if "mipyme" in document_type:
            values.update(
                {"document_type": expected_document, "lines": [{"price_unit": 150000}]}
            )
        ...
        self.assertEqual(
            ...
            expected_document.display_name,
            "The document should be %s" % expected_document.display_name,
        )
        self._validate_and_review(invoice, expected_result=expected_result)
        return invoice

    def _test_case_credit_note(
        self, document_type, invoice, data=None, expected_result=None
    ):
        refund = self._create_credit_note(invoice, data=data)
        expected_document = self.document_type[document_type]
        self.assertEqual(
            refund.l10n_latam_document_type_id.display_name,
            expected_document.display_name,
            "The document should be %s" % expected_document.display_name,
        )
    ...
    def _test_case_debit_note(
        self, document_type, invoice, data=None, expected_result="A"
    ):
        debit_note = self._create_debit_note(invoice, data=data)
        expected_document = self.document_type[document_type]
        self.assertEqual(
            debit_note.l10n_latam_document_type_id.display_name,
            expected_document.display_name,
            "The document should be %s" % expected_document.display_name,
        )
    ...
    def _handler_afip_internal_error(self):
        try:
            ...
            error_msg = repr(exc)
            if (
                "Code 500" in error_msg
                or "Code 501" in error_msg
                or "Code 502" in error_msg
            ):
                self.skipTest(
                    "We receive an internal error from AFIP so skip this test"
                )
            else:
                raise
    ...
    def _validate_and_review(self, invoice, expected_result=None, error_msg=None):
        ...
        error_msg = (
            error_msg
            or "This test return a result different from the expteced (%s)"
            % expected_result
        )
        self._post(invoice)
