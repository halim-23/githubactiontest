...
@tagged(
    "fe", "ri", "external_l10n", "-at_install", "post_install", "-standard", "external"
)
class TestFe(common.TestEdi):
    ...
    def test_26_iibb_sales_ars(self):
        ...
        invoice = self._create_invoice(
            data={
                "lines": [
                    {"product": product_27, "price_unit": 100.0, "quantity": 8},
        ...
        invoice.invoice_line_ids.filtered(
            lambda x: x.product_id == product_27
        ).tax_ids = [(4, iibb_tax.id)]
        invoice.invoice_line_ids.filtered(
            lambda x: x.product_id == product_exento
        ).tax_ids = [(4, iibb_tax.id)]
    ...
    def test_28_vendor_bill_verify(self):
        ...
        bill = self._create_invoice(
            {
                "lines": [{"price_unit": invoice.amount_total}],
        ...
        self.assertEqual(
            bill.l10n_ar_afip_verification_result, "R", bill.message_ids[0].body
        )
