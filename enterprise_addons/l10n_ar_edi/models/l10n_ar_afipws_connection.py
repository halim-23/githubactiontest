...
class ARTransport(transports.Transport):
    def __init__(self, *args, **kwargs):
        ...
        self.session.mount(
            "https://", L10nArHTTPAdapter()
        )  # block DH ciphers for AFIP servers
    ...
    def post(self, address, message, headers):
        ...
        self.xml_request = etree.tostring(
            etree.fromstring(message), pretty_print=True
        ).decode("utf-8")
...
class L10nArAfipwsConnection(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", required=True, index=True, auto_join=True
    )
    ...
    type = fields.Selection(
        ...
        readonly=True,
        required=True,
        help="This field is not configure by the user, is extracted from the environment configured in"
    ...
    l10n_ar_afip_ws = fields.Selection(
        selection="_get_l10n_ar_afip_ws", string="AFIP WS", required=True
    )

    def _get_l10n_ar_afip_ws(self):
        """Return the list of values of the selection field."""
        return [("wscdc", _("Verification of Invoices (WSCDC)"))] + self.env[
            "account.journal"
    ...
    def _get_client(self, return_transport=False):
        ...
        try:
            ...
            self._l10n_ar_process_connection_error(
                error, self.type, self.l10n_ar_afip_ws
            )
        if return_transport:
    ...
    def _l10n_ar_process_connection_error(self, error, env_type, afip_ws):
        ...
        error_msg = _(
            "There was a problem with the connection to the %s webservice: %s"
        ) % (afip_ws, error_name)
        ...
        data = {
            ...
            "No se puede decodificar el BASE64": _(
    ...
    def _l10n_ar_get_token_data(self, company, afip_ws):
        ...
        try:
            _logger.info(
                "Connect to AFIP to get token: %s %s %s"
                % (afip_ws, company.l10n_ar_afip_ws_crt_fname, company.name)
            )
            transport = ARTransport(operation_timeout=60, timeout=60)
            client = zeep.Client(wsdl, transport=transport)
            ...
            return self._l10n_ar_process_connection_error(
                error, environment_type, afip_ws
            )
