...
class ResCurrency(models.Model):
    ...
    def _l10n_ar_get_afip_ws_currency_rate(self, afip_ws="wsfe"):
        ...
        if afip_ws == "wsfe":
            response = client.service.FEParamGetCotizacion(
                auth, MonId=self.l10n_ar_afip_code
            )
            if response.Errors:
                raise UserError(
                    _("The was an error obtaining the rate:\n\n * Code %s -  %s")
            ...
            response = client.service.FEXGetPARAM_Ctz(
                auth, Mon_id=self.l10n_ar_afip_code
            )
            date = response.FEXResultGet.Mon_fecha
            rate = response.FEXResultGet.Mon_ctz
        else:
            raise UserError(
                _("Get AFIP currency rate not implemented for webservice %s", afip_ws)
            )
        return date, rate
