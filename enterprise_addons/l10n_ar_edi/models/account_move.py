...
class AccountMove(models.Model):
    ...
    l10n_ar_afip_xml_request = fields.Text(
        string="AFIP XML Request", copy=False, readonly=True, groups="base.group_system"
    )
    ...
    l10n_ar_afip_result = fields.Selection(
        [("A", "Accepted in AFIP"), ("O", "Accepted in AFIP with Observations")],
        "Result",
        readonly=True,
        copy=False,
        states={"draft": [("readonly", False)]},
    ...
    l10n_ar_afip_verification_result = fields.Selection(
        [("A", "Approved"), ("O", "Observed"), ("R", "Rejected")],
        string="AFIP Verification result",
        copy=False,
        readonly=True,
    )
    ...
    l10n_ar_fce_transmission_type = fields.Selection(
        ...
        string="FCE: Transmission Option",
    ...
    def _compute_l10n_ar_fce_transmission_type(self):
        ...
        mipyme_fce_docs = self.filtered(
            lambda x: x._is_mipyme_fce() or x._is_mipyme_fce_refund()
        )
        for rec in mipyme_fce_docs:
            rec.l10n_ar_fce_transmission_type = (
                rec.company_id.l10n_ar_fce_transmission_type
            )
    ...
    def _compute_l10n_ar_afip_qr_code(self):
        ...
        with_qr_code = self.filtered(
            lambda x: x.l10n_ar_afip_auth_mode in ["CAE", "CAEA"]
        ...
        for rec in with_qr_code:
            data = {
                ...
                "nroCmp": int(
                    self._l10n_ar_get_document_number_parts(
                        rec.l10n_latam_document_number,
                        rec.l10n_latam_document_type_id.code,
                    )["invoice_number"]
                ),
                "importe": float_round(
                    rec.amount_total, precision_digits=2, rounding_method="DOWN"
                ),
            ...
            if (
                ...
                and commercial_partner_id.country_id.code != "AR"
            ):
                nro_doc_rec = int(
                    commercial_partner_id.country_id.l10n_ar_legal_entity_vat
                    if commercial_partner_id.is_company
                    else commercial_partner_id.country_id.l10n_ar_natural_vat
                )
            else:
                nro_doc_rec = (
                    commercial_partner_id._get_id_number_sanitize()
                    if commercial_partner_id.vat
                    else False
                )
            ...
            if commercial_partner_id.l10n_latam_identification_type_id:
                data.update(
                    {"tipoDocRec": int(rec._get_partner_code_id(commercial_partner_id))}
                )
            # For more info go to https://www.afip.gob.ar/fe/qr/especificaciones.asp
            rec.l10n_ar_afip_qr_code = (
                "https://www.afip.gob.ar/fe/qr/?p=%s"
                % base64.b64encode(json.dumps(data).encode()).decode("ascii")
            )
    ...
    def _compute_l10n_ar_afip_verification_type(self):
        ...
        available_to_verify = self.filtered(
            ...
            and x.l10n_latam_document_type_id.code in verify_codes
        )
        for rec in available_to_verify:
            rec.l10n_ar_afip_verification_type = (
                rec.company_id.l10n_ar_afip_verification_type
            )
    ...
    def _is_dummy_afip_validation(self):
        self.ensure_one()
        return (
            self.company_id._get_environment_type() == "testing"
            and not self.company_id.sudo().l10n_ar_afip_ws_crt
            or not self.company_id.sudo().l10n_ar_afip_ws_key
        )
    ...
    def _post(self, soft=True):
        ...
        sale_ar_invoices = ar_invoices.filtered(
            lambda x: x.move_type in ["out_invoice", "out_refund"]
        )
        sale_ar_edi_invoices = sale_ar_invoices.filtered(
            lambda x: x.journal_id.l10n_ar_afip_ws
        )
        ...
        for inv in sale_ar_edi_invoices:
            ...
            client, auth, transport = inv.company_id._l10n_ar_get_connection(
                inv.journal_id.l10n_ar_afip_ws
        ...
        if error_invoice:
            if error_invoice.exists():
                msg = _(
                    'We couldn\'t validate the invoice "%s" (Draft Invoice *%s) in AFIP'
...
""",
    ...
    def l10n_ar_verify_on_afip(self):
        ...
        for inv in self:
            if not inv.l10n_ar_afip_auth_mode or not inv.l10n_ar_afip_auth_code:
                raise UserError(
                    _("Please set AFIP Authorization Mode and Code to continue!")
                )

            # get Issuer and Receptor depending on the document type
            issuer, receptor = (
                (inv.commercial_partner_id, inv.company_id.partner_id)
                if inv.move_type in ["in_invoice", "in_refund"]
                else (inv.company_id.partner_id, inv.commercial_partner_id)
            )
            issuer_vat = issuer.ensure_vat()

            receptor_identification_code = (
                receptor.l10n_latam_identification_type_id.l10n_ar_afip_code or "99"
            )
            receptor_id_number = receptor_identification_code and receptor.vat or "0"

            if (
                inv.l10n_latam_document_type_id.l10n_ar_letter in ["A", "M"]
                and receptor_identification_code != "80"
                or not receptor_id_number
            ):
                raise UserError(
                    _(
                        "For type A and M documents the receiver identification is mandatory and should be VAT"
                    )
                )

            document_parts = self._l10n_ar_get_document_number_parts(
                inv.l10n_latam_document_number, inv.l10n_latam_document_type_id.code
            )
            if (
                not document_parts["point_of_sale"]
                or not document_parts["invoice_number"]
            ):
            ...
            if response.Observaciones or response.Errors:
                inv.message_post(
                    body=_(
                        "AFIP authorization verification result: %s%s",
    ...
    def _l10n_ar_do_afip_ws_request_cae(self, client, auth, transport):
        ...
        * Result (Approved, Aproved with Observations)
        ...
        for inv in self.filtered(
            lambda x: x.journal_id.l10n_ar_afip_ws and not x.l10n_ar_afip_auth_code
        ):
            ...
            if afip_ws == "wsfe":
                ...
                if response.FeDetResp:
                    ...
                    if result.Resultado == "A":
                        values = {
                            ...
                            and str(result.CAE)
                            or "",
                            "l10n_ar_afip_auth_code_due": datetime.strptime(
                                result.CAEFchVto, "%Y%m%d"
                            ).date(),
                ...
                if response.FEXErr.ErrCode != 0 or response.FEXErr.ErrMsg != "OK":
                    errors = "\n* Code %s: %s" % (
                        response.FEXErr.ErrCode,
                        response.FEXErr.ErrMsg,
                    )
                    return_codes += [str(response.FEXErr.ErrCode)]
                if (
                    response.FEXEvents.EventCode != 0
                    or response.FEXEvents.EventMsg != "Ok"
                ):
                    events = "\n* Code %s: %s" % (
                        response.FEXEvents.EventCode,
                        response.FEXEvents.EventMsg,
                    )
                ...
                if response.BFEEvents.EventCode != 0 or response.BFEEvents.EventMsg:
                    events = "\n* Code %s: %s" % (
                        response.BFEEvents.EventCode,
                        response.BFEEvents.EventMsg,
                    )
                if result.Obs:
                    obs = result.Obs
    ...
    def _dummy_afip_validation(self):
        ...
        self.message_post(
            body=_(
                "Invoice validated locally because it is in a testing environment without testing certificate/keys"
            )
        )
    ...
    def _l10n_ar_check_afip_auth_verify_required(self):
        ...
        verification_missing = self.filtered(
            ...
            and x.l10n_ar_afip_verification_type == "required"
            and x.l10n_latam_document_type_id.country_id.code == "AR"
            and x.l10n_ar_afip_verification_result not in ["A", "O"]
        )
        try:
        ...
        still_missing = verification_missing.filtered(
            lambda x: x.l10n_ar_afip_verification_result not in ["A", "O"]
        )
        if still_missing:
            text = (
                "these vendor bills" if len(still_missing) > 1 else "this vendor bill"
            )
            raise UserError(
                _(
                    "We can not post %s in Odoo because the AFIP verification fail: %s\nPlease verify in AFIP manually"
                    " and review the bill chatter for more information"
                )
    ...
    def _is_mipyme_fce_refund(self):
        """True of False if the invoice is a mipyme document"""
        self.ensure_one()
        return int(self.l10n_latam_document_type_id.code) in [
            202,
            203,
            207,
            208,
    ...
    def _due_payment_date(self):
        ...
        if (
            self.l10n_ar_afip_concept != "1"
            and not self._is_mipyme_fce_refund()
            or self._is_mipyme_fce()
        ):
            ...
            base_imp = sum(
                self.invoice_line_ids.filtered(
                    lambda x: x.tax_ids.filtered(
                        ...
                        == tribute.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code
                    )
                ).mapped("price_subtotal")
            )
            res.append(
                {
                    "Id": tribute.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code,
                    "Alic": 0,
        ...
        for line in self.invoice_line_ids.filtered(
            ...
            if afip_ws == "wsbfe":
                if not line.product_id.uom_id.l10n_ar_afip_code:
                    raise UserError(
                        _("No AFIP code in %s UOM", line.product_id.uom_id.name)
                    )

                vat_tax = line.tax_ids.filtered(
                    lambda x: x.tax_group_id.l10n_ar_vat_afip_code
                )
                vat_taxes_amounts = vat_tax.with_context(
                    force_sign=line.move_id._get_tax_force_sign()
                ).compute_all(
                    line.price_unit,
                    self.currency_id,
                    line.quantity,
                ...
                values.update(
                    {
                        ...
                        "Pro_total_item": float_repr(
                            line.price_subtotal, precision_digits=2
                        ),
    ...
    def _get_optionals_data(self):
        ...
        if self._is_mipyme_fce_refund():
            optionals.append(
                {
                    "Id": 22,
                    "Valor": self.l10n_ar_afip_fce_is_cancellation and "S" or "N",
                }
            )
    ...
    def wsfex_get_cae_request(self, last_id, client):
        if not self.commercial_partner_id.country_id:
            raise UserError(
                _(
                    ...
                    self.journal_id.l10n_ar_afip_ws,
                )
            )
        elif not self.commercial_partner_id.country_id.code:
            raise UserError(
                _(
                    'For WS "%s" country code is mandatory country: %s',
                    self.journal_id.l10n_ar_afip_ws,
                    self.commercial_partner_id.country_id.name,
                )
            )
        elif not self.commercial_partner_id.country_id.l10n_ar_afip_code:
            hint_msg = afip_errors._hint_msg(
                "country_afip_code", self.journal_id.l10n_ar_afip_ws
            )
            msg = _(
                'For "%s" WS the afip code country is mandatory: %s',
                self.journal_id.l10n_ar_afip_ws,
                self.commercial_partner_id.country_id.name,
            )
            if hint_msg:
            ...
            raise RedirectWarning(
                msg,
                self.env.ref("l10n_ar_edi.action_help_afip").id,
                _("Go to AFIP page"),
            )
        ...
        if self.commercial_partner_id.country_id.code != "AR":
            vat_country = (
                self.commercial_partner_id.country_id.l10n_ar_legal_entity_vat
        ...
        res = {
            ...
            "Forma_pago": self.invoice_payment_term_id.name
            if self.invoice_payment_term_id
            else None,
            "Idioma_cbte": 1,  # invoice language: spanish / español
            "Incoterms": self.invoice_incoterm_id.code
            if self.invoice_incoterm_id
            else None,
            # incoterms_ds only admit max 20 characters admite
            "Incoterms_Ds": self.invoice_incoterm_id.name[:20]
            if self.invoice_incoterm_id and self.invoice_incoterm_id.name
            else None,
            ...
            "Permiso_existente": "N"
            if int(self.l10n_latam_document_type_id.code) == 19
            and int(self.l10n_ar_afip_concept) == 1
            else "",
            "Items": ArrayOfItem(self._get_line_details()),
            "Cmps_asoc": ArrayOfCmp_asoc([related_invoices])
            if related_invoices
            else None,
        }
        ...
        payment_date = (
            datetime.strftime(self.invoice_date_due, WS_DATE_FORMAT["wsfex"])
            if int(self.l10n_latam_document_type_id.code) == 19
            and int(self.l10n_ar_afip_concept) in [2, 4]
    ...
    def wsbfe_get_cae_request(self, last_id, client=None):
        ...
        if self.l10n_latam_document_type_id.code in ["201", "206"]:  # WS4900
            res.update(
                {
                    "Fecha_vto_pago": self._due_payment_date().strftime(
                        WS_DATE_FORMAT["wsbfe"]
                    )
                }
            )
            optionals = self._get_optionals_data()
    ...
    def _is_argentina_electronic_invoice(self):
        return bool(
            ...
            and self.env.company.account_fiscal_country_id.code == "AR"
            and self.journal_id.l10n_ar_afip_ws
        )
    ...
    def _get_last_sequence_from_afip(self):
        ...
        last_number = (
            0
            if self._is_dummy_afip_validation() or self.l10n_latam_document_number
            else self.journal_id._l10n_ar_get_afip_last_invoice_number(
    ...
    def _get_last_sequence(self, relaxed=False, with_prefix=None):
        ...
        if (
            not res
            and self._is_argentina_electronic_invoice()
