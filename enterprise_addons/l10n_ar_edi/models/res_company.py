...
try:
    ...
    _logger.warning(
        "OpenSSL library not found. If you plan to use l10n_ar_edi, please install the library from https://pypi.python.org/pypi/pyOpenSSL"
    )
...
class ResCompany(models.Model):
    ...
    l10n_ar_afip_verification_type = fields.Selection(
        ...
        string="AFIP Invoice Verification",
        help="It adds an option on invoices to"
    ...
    l10n_ar_connection_ids = fields.One2many(
        "l10n_ar.afipws.connection", "company_id", "Connections"
    )
    ...
    l10n_ar_afip_ws_key = fields.Binary(
        "Private Key",
    ...
    l10n_ar_afip_ws_crt = fields.Binary(
        "Certificate",
        groups="base.group_system",
    ...
    l10n_ar_afip_ws_crt_fname = fields.Char(
        "Certificate name", compute="_compute_l10n_ar_afip_ws_crt_fname", store=True
    )
        ...
        for rec in self.filtered("l10n_ar_afip_ws_crt"):
            ...
            try:
                content = base64.decodebytes(
                    rec.with_context(bin_size=False).l10n_ar_afip_ws_crt
                ).decode("ascii")
                crypto.load_certificate(crypto.FILETYPE_PEM, content)
            except Exception as exc:
                if "Expecting: CERTIFICATE" in repr(
                    exc
                ) or "('PEM routines', 'get_name', 'no start line')" in repr(exc):
                    error = _(
    ...
    def _l10n_ar_check_afip_private_key(self):
        ...
        for rec in self.filtered("l10n_ar_afip_ws_key"):
            ...
            try:
                content = (
                    base64.decodebytes(
                        rec.with_context(bin_size=False).l10n_ar_afip_ws_key
                    )
                    .decode("ascii")
                    .strip()
                )
    ...
    def _l10n_ar_is_afip_crt_expire(self):
        ...
        if expire_date and expire_date < fields.Date.today():
            raise UserError(
                _("The AFIP certificate is expired, please renew in order to continue")
            )
    ...
    def _get_environment_type(self):
        ...
        if not self.l10n_ar_afip_ws_environment:
            raise UserError(
                _(
                    'AFIP environment not configured for company "%s", please check accounting settings'
                )
                % (self.name)
            )
    ...
    def _l10n_ar_get_connection(self, afip_ws):
        ...
        connection = self.l10n_ar_connection_ids.search(
            [
                ("type", "=", env_type),
                ("l10n_ar_afip_ws", "=", afip_ws),
                ("company_id", "=", self.id),
            ],
            limit=1,
        )
        ...
        if connection:
            ...
            _logger.info(
                "Connection created for company %s %s (%s)"
                % (self.name, afip_ws, env_type)
            )
    ...
    def _get_key_and_certificate(self):
        ...
        self.ensure_one()
        pkey = (
            base64.decodebytes(self.with_context(bin_size=False).l10n_ar_afip_ws_key)
            if self.l10n_ar_afip_ws_key
            else ""
        )
        cert = (
            base64.decodebytes(self.l10n_ar_afip_ws_crt)
    ...
    def _l10n_ar_create_certificate_request(self):
        ...
        req_subject.serialNumber = ("CUIT %s" % self.partner_id.ensure_vat()).encode(
            "ascii", "ignore"
        )
    ...
    def set_demo_random_cert(self):
        ...
        for rec in self:
            ...
            cert_file = get_module_resource(
                "l10n_ar_edi", "demo", "cert%d.crt" % random.randint(1, 10)
            )
