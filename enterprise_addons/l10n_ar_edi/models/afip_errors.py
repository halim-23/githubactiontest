...
WSFE_ERRORS = {
    "501": _lt(
        "This is an internal error in AFIP. Please wait a couple of minutes and try again"
    ),
    "600": _lt(
    ...
    "10040": _lt(
        "You are trying to create a debit or credit note but the document type you selected does not match with the original one"
    ),
    "10048": _lt(
        "This error is usually given if the decimal precision of product prices is greater than currency precision. The alternatives are: a) use the lowest decimal precision for product price. b) raise the decimal precision of the currency in question.\n\nAfter updating the precisions you must change the quantities or prices on all invoice lines in order to recomputed the amounts, save and then reset"
    ),
    ...
    "10154": _lt(
        ...
        '\n\t * "OK - Comprobante electrónico autorizado, NO rechazado por el Comprador" it means that the "FCE: Is cancellation" should be set.'
        '\n\t * "OK - Comprobante electrónico autorizado pero fue rechazado por el comprador" it means that the "FCE: Is cancellation" should NOT be set.'
    ),
    "10153": _lt(
        'If the invoice is a a FCE, Debit Note or Credit Note it is mandatory to inform related invoices. You are probably doing an CN/DN and the "Source" field (in the Other Information Tab) is not defined or has a wrong value. You must indicate in that field the original invoice number without any prefix or suffix, eg "0001-00000001"'
    ),
    "10181": _lt(
        "As the message mentions, if you are making an CN/DN, it must be with the same currency as the original invoice.\n\nThe only exception is the exchange rate adjustments that need to be done in ARS but taking into account that the orginal invoice must already be accepted (or rejected by the customer).\n\nWe recommend you to verify if the customer has already accepted/rejected the original invoice"
    ),
    "10184": _lt(
        'You are probably wanting to cancel an invoice through an CN but the invoice has already been accepted for the client. Keep in mind that acceptance may have been "express" or "tacit" (that is , automatically accepted after 30 calendar days without the client having rejected it).'
    ),
    "10051": _lt(
        "Could happen that the VAT is calculated without the discount, then it throws another amount."
    ),
...
WBSFE_ERRORS = {
    "501": _lt(
        "This is an internal error in AFIP. Contact to your Odoo provider in order to let it know that is happening and to investigate what is going on with the AFIP service"
    ),
    "1014": _lt(
