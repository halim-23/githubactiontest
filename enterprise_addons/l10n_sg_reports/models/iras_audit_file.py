...
class IrasAuditFileWizard(models.TransientModel):
    ...
    def generate_iras(self):
        options = {"date_from": self.date_from, "date_to": self.date_to}
...
class IrasAuditFile(models.AbstractModel):
        ...
        for invoice in invoice_ids:
            ...
            for lines in invoice.invoice_line_ids:
                ...
                if not invoice.partner_id.l10n_sg_unique_entity_number:
                    raise UserError(
                        _("Your partner (%s) must have a UEN.", invoice.partner_id.name)
                    )
                purchases_lines.append(
                    {
                        "SupplierName": (invoice.partner_id.name or "")[:100],
                        "SupplierUEN": (
                            invoice.partner_id.l10n_sg_unique_entity_number or ""
                        )[:16],
                        "InvoiceDate": fields.Date.to_string(
                            invoice.l10n_sg_permit_number_date
                            if invoice.l10n_sg_permit_number
                            and invoice.l10n_sg_permit_number_date
                            else invoice.invoice_date
                        ),
                        "InvoiceNo": (invoice.name or "")[:50],
                        "PermitNo": invoice.l10n_sg_permit_number[:20]
                        if invoice.l10n_sg_permit_number
                        else False,
                        ...
                        "ProductDescription": (
                            ...
                            else lines.product_id.name or ""
                        )[:250],
                        "PurchaseValueSGD": float_repr(lines.balance, IRAS_DIGITS),
                        "GSTValueSGD": float_repr(
                            (lines.price_total - lines.price_subtotal)
                            / (lines.quantity or 1),
                            IRAS_DIGITS,
                        ),
                        "TaxCode": (lines.tax_ids and lines.tax_ids[0].name or " ")[
                            :20
                        ],
                        "FCYCode": (
                        ...
                        "PurchaseFCY": float_repr(lines.price_subtotal, IRAS_DIGITS)
                        if invoice.currency_id.name != "SGD"
                        else "0",
                        "GSTFCY": float_repr(sign * tax_amount_company, IRAS_DIGITS)
                        if invoice.currency_id.name != "SGD"
                        else "0",
                    }
                )
                ...
                supply_lines.append(
                    {
                        ...
                        "Country": invoice.partner_id.commercial_partner_id.country_id.code
                        if invoice.invoice_origin
                        and invoice.partner_id.commercial_partner_id.country_id.code
                        != "SG"
                        else False,
                        "FCYCode": (
                            invoice.currency_id.name
                            if lines.currency_id and lines.currency_id.name != "SGD"
                            else "XXX"
                        )[:3],
                        "SupplyFCY": float_repr(lines.price_subtotal, IRAS_DIGITS)
                        if invoice.currency_id.name != "SGD"
                        else "0",
                        "GSTFCY": float_repr(sign * tax_amount_company, IRAS_DIGITS)
                        if invoice.currency_id.name != "SGD"
    ...
    def _get_gldata(self, date_from, date_to):
        ...
        for account in all_accounts:
            initial_bal = dict(accounts_results).get(
                account.id,
                {
                    "initial_balance": {
                        "balance": 0,
            ...
            for move_line_id in move_line_ids:
                if move_line_id.account_id.code == account.code:
                    balance = company_id.currency_id.round(
                    ...
                    gldata_lines.append(
                        {
                            ...
                            "Name": move_line_id.partner_id.name
                            if move_line_id.partner_id
                            else False,
                            "TransactionID": move_line_id.move_id.name,
                            "SourceDocumentID": move_line_id.move_id.invoice_origin
    ...
    def _txt_purchases_infos(self, values):
        ...
        node += (
            "PurcDataEnd|"
            + values["PurchaseTotalSGD"]
            + "|"
            + values["GSTTotalSGD"]
            + "|"
            + values["TransactionCountTotal"]
            + "|\n\n"
        )
        return node
