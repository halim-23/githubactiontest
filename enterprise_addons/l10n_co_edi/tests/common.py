...
class TestCoEdiCommon(AccountEdiTestCommon):
    ...
    def mock_carvajal(self):
        ...
        try:
            with freeze_time(self.frozen_today), patch(
                "odoo.addons.l10n_co_edi.models.carvajal_request.CarvajalRequest.upload",
                new=Mock(return_value=return_value_upload),
            ), patch(
                "odoo.addons.l10n_co_edi.models.carvajal_request.CarvajalRequest.check_status",
                new=Mock(return_value=return_value_check),
            ), patch(
                "odoo.addons.l10n_co_edi.models.carvajal_request.CarvajalRequest.client",
    ...
    def setUpClass(
        cls,
        chart_template_ref="l10n_co.l10n_co_chart_template_generic",
        ...
        cls.frozen_today = datetime.datetime(
            year=2020,
            month=8,
            day=27,
            hour=0,
        ...
        cls.company_data["company"].partner_id.write(
            {
                ...
                "l10n_co_edi_obligation_type_ids": [
                    (6, 0, [cls.env.ref("l10n_co_edi.obligation_type_1").id])
                ],
        ...
        cls.tax.write(
            {"amount": 15, "l10n_co_edi_type": cls.env.ref("l10n_co_edi.tax_type_0").id}
        )
        ...
        cls.expected_invoice_xml = misc.file_open(
            "l10n_co_edi/tests/accepted_invoice.xml", "rb"
        ).read()

        cls.expected_credit_note_xml = misc.file_open(
            "l10n_co_edi/tests/accepted_credit_note.xml", "rb"
        ).read()
