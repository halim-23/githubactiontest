...
class TestColombianInvoice(TestCoEdiCommon):
    def l10n_co_assert_generated_file_equal(
        self, invoice, expected_values, applied_xpath=None
    ):
    ...
    def test_invoice(self):
        ...
        with self.mock_carvajal():
            self.l10n_co_assert_generated_file_equal(
                self.invoice, self.expected_invoice_xml
            )
            ...
            self.l10n_co_assert_generated_file_equal(
                credit_note, self.expected_credit_note_xml
            )
    ...
    def test_invoice_with_attachment_url(self):
        with self.mock_carvajal():
            ...
            """
            self.l10n_co_assert_generated_file_equal(
                self.invoice, self.expected_invoice_xml, applied_xpath
            )
