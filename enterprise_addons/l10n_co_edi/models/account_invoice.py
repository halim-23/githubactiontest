...
class AccountMove(models.Model):
    ...
    l10n_co_edi_attachment_url = fields.Char(
        "Electronic Invoice Attachment URL",
    ...
    l10n_co_edi_transaction = fields.Char(
        "Transaction ID (CO)",
    ...
    l10n_co_edi_cufe_cude_ref = fields.Char(
        string="CUFE/CUDE",
        copy=False,
        help="Unique ID received by the government when the invoice is signed.",
    )
    l10n_co_edi_payment_option_id = fields.Many2one(
        "l10n_co_edi.payment.option",
        string="Payment Option",
        default=lambda self: self.env.ref(
            "l10n_co_edi.payment_option_1", raise_if_not_found=False
        ),
    )
    l10n_co_edi_is_direct_payment = fields.Boolean(
        "Direct Payment from Colombia", compute="_compute_l10n_co_edi_is_direct_payment"
    )
    l10n_co_edi_description_code_credit = fields.Selection(
        DESCRIPTION_CREDIT_CODE, string="Concepto Nota de Credito"
    )
    l10n_co_edi_description_code_debit = fields.Selection(
        DESCRIPTION_DEBIT_CODE, string="Concepto Nota de Débito"
    )
        ...
        for rec in self:
            ...
            if rec.move_type == "out_refund":
                ...
                if rec.l10n_co_edi_debit_note:
                    state = rec._get_edi_document(
                        self.env.ref("l10n_co_edi.edi_carvajal")
                    ).state
    ...
    def _compute_l10n_co_edi_is_direct_payment(self):
        for rec in self:
            ...
            ) and rec.company_id.account_fiscal_country_id.code == "CO"
    ...
    def _l10n_co_edi_get_electronic_invoice_type_info(self):
        if self.move_type == "out_invoice":
            return (
                ...
                if self.l10n_co_edi_debit_note
                else "DIAN 2.1: Factura Electrónica de Venta"
            )
    ...
    def _retry_edi_documents_error_hook(self):
        ...
        self.filtered(
            lambda m: m._get_edi_document(carvajal).blocking_level == "error"
        ).l10n_co_edi_transaction = None
        ...
        if self.product_id:
            if self.move_id.l10n_co_edi_type == "2":
                if not self.product_id.l10n_co_edi_customs_code:
                    raise UserError(
                        _(
                            "Exportation invoices require custom code in all the products, please fill in this information before validating the invoice"
                        )
                    )
                return (
                    self.product_id.l10n_co_edi_customs_code,
                    "020",
                    "Partida Alanceraria",
                )
