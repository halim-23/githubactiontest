...
class CarvajalUsernameToken(UsernameToken):
    def _create_password_digest(self):
        ...
        res[0].attrib["Type"] = (
            res[0].attrib["Type"].replace("PasswordDigest", "PasswordText")
        )
        return res
...
class CarvajalRequest:
    def __init__(self, company):
        ...
        self.wsdl = (
            company.env["ir.config_parameter"]
            .sudo()
            .get_param("l10n_edi_carvajal_wsdl")
        )
        if self.wsdl:
            self.wsdl = self.wsdl % ("-stage" if company.l10n_co_edi_test_mode else "")
        else:  # for old users, keep using the old URL
            self.wsdl = "https://wscenf%s.cen.biz/isows/InvoiceService?wsdl" % (
                "lab" if self.test_mode else ""
            )
    ...
    def client(self):
        if not hasattr(self, "_client"):
            ...
            self._client = Client(
                self.wsdl, plugins=[CarvajalPlugin()], wsse=token, transport=transport
            )
        return self._client
    ...
    def _handle_exception(self, e):
        ...
        * blocking_level: Info, warning, error.
        ...
        if isinstance(e, socket.timeout):
            return {
                "error": _("Connection to Carvajal timed out."),
                "blocking_level": "warning",
            }
        elif isinstance(e, requests.HTTPError) and 499 < e.response.status_code < 600:
            return {
                "error": _("Carvajal service not available."),
                "blocking_level": "warning",
            }
            ...
            return {
                "error": ("Electronic invoice submission to Carvajal failed."),
                "blocking_level": "warning",
            }

    def _create_wsse_header(self, username, password):
        created = datetime.now()
        token = CarvajalUsernameToken(
            username=username,
    ...
    def upload(self, filename, xml):
        ...
        * blocking_level: Info, warning, error.
        """
        try:
            response = self.client.service.Upload(
                fileName=filename,
                fileData=base64.b64encode(xml).decode(),
                companyId=self.co_id_company,
    ...
    def _download(self, invoice):
        ...
        * blocking_level:            Info, warning, error.
        ...
        if invoice.move_type == "out_refund":
            ...
            if (
                invoice.journal_id.l10n_co_edi_debit_note
                or invoice.l10n_co_edi_operation_type in ["30", "32", "33"]
            ):
                carvajal_type = "ND"
            else:
        ...
        if (
            ...
            and invoice.journal_id.l10n_co_edi_debit_note
        ):
            prefix = "ND"
        ...
        try:
            response = self.client.service.Download(
                ...
                documentType=carvajal_type,
                resourceType="PDF,SIGNED_XML",
                companyId=self.co_id_company,
                accountId=self.account,
            )
        except Exception as e:
            ...
            if xml_filenames:
                ...
                return {
                    ...
                    "message": _(
                        "The invoice was succesfully signed. <br/>Message from Carvajal: %s",
                        html_escape(response["status"]),
                    ),
                    "l10n_co_edi_cufe_cude_ref": ref_elem.text,
                }
            return {
                "error": _(
                    "The invoice was accepted by Carvajal but unexpected response was received."
                ),
                "blocking_level": "warning",
            }
    ...
    def check_status(self, invoice):
        ...
        * blocking_level:            Info, warning, error.
        """
        try:
            response = self.client.service.DocumentStatus(
                transactionId=invoice.l10n_co_edi_transaction,
                companyId=self.co_id_company,
                accountId=self.account,
            )
        except Exception as e:
        ...
        if response.processStatus == "PROCESSING":
            return {
                "error": _("The invoice is still processing by Carvajal."),
                "blocking_level": "info",
            }
        legalStatus = (
            response.legalStatus if hasattr(response, "legalStatus") else "REJECTED"
        )
        ...
        msg = (
            _(
                "The invoice was rejected by Carvajal: %s",
                html_escape(response["errorMessage"]).replace("\n", "<br/>"),
            )
            if hasattr(response, "errorMessage") and response["errorMessage"]
            else _(
                "The invoice was rejected by Carvajal but no error message was received."
            )
        )
