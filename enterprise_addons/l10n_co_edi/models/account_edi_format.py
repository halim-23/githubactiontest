...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_co_edi_generate_electronic_invoice_filename(self, invoice):
        ...
        if not invoice_number:
            IrSequence.sudo().create(
                {
                    "name": "Colombian electronic invoicing sequence for company %s"
        ...
        return "face_{}{:0>10}{:010x}.xml".format(
            invoice._l10n_co_edi_get_electronic_invoice_type(),
    ...
    def _l10n_co_edi_prepare_tim_sections(
        self, taxes_dict, invoice_currency, retention, tax_details=None
    ):
        ...
        def _get_conversion_rate(from_currency, to_currency, company, date):
            ...
            return currency_rates.get(to_currency.id) / currency_rates.get(
                from_currency.id
            )
            ...
            for rec in values:
                imp = {
                    ...
                    "IMP_2": (
                        ...
                        if rec.tax_line_id.l10n_co_edi_type.code == "05"
                        else _convert(
                ...
                if rec.tax_line_id.amount_type == "fixed":
                    imp.update(
                        {
                            ...
                            "IMP_7": 1,
                            "IMP_8": "BO"
                            if rec.tax_line_id.l10n_co_edi_type.code == "22"
                            else "94",
                    ...
                    imp.update(
                        {
                            "IMP_6": 15.0
                            if rec.tax_line_id.l10n_co_edi_type.code == "05"
                            else abs(rec.tax_line_id.amount),
                            "IMP_7": "",
                            "IMP_8": "",
                            "IMP_9": "",
                            "IMP_10": "",
                    ...
                    tim["TIM_4"] += float_round(
                        (imp["IMP_6"] / 100.0 * imp["IMP_2"]) - imp["IMP_4"], 2
                    )
                tim["TIM_2"] += imp["IMP_4"]
                tim["IMPS"].append(imp)
    ...
    def _l10n_co_edi_generate_xml(self, invoice):
        ...
        def get_notas():
            ...
            lines = invoice.invoice_line_ids.filtered(
                lambda line: line.product_uom_id.category_id
                == self.env.ref("uom.product_uom_categ_vol")
            )
            liters = sum(
                line.product_uom_id._compute_quantity(
                    line.quantity, self.env.ref("uom.product_uom_litre")
                )
                for line in lines
            )
            ...
            kg = sum(
                line.product_uom_id._compute_quantity(
                    line.quantity, self.env.ref("uom.product_uom_kgm")
                )
                for line in lines
            )
            total_weight = int(kg)
            ...
            lines = invoice.invoice_line_ids.filtered(
                lambda line: line.product_uom_id.category_id
                == self.env.ref("uom.product_uom_categ_unit")
            )
            units = sum(
                line.product_uom_id._compute_quantity(
                    line.quantity, self.env.ref("uom.product_uom_unit")
                )
                for line in lines
            )
            total_units = int(units)

            withholding_amount = invoice.amount_untaxed + sum(
                invoice.line_ids.filtered(
                    lambda line: line.tax_line_id
                    and not line.tax_line_id.l10n_co_edi_type.retention
                ).mapped("price_total")
            )
            amount_in_words = invoice.currency_id.with_context(
                lang=invoice.partner_id.lang or "es_ES"
            ).amount_to_text(withholding_amount)
            shipping_partner = self.env["res.partner"].browse(
                invoice._get_invoice_delivery_partner_id()
            )
            notas = [
                "1.-%s|%s|%s|%s|%s|%s"
                % (
                    invoice.company_id.l10n_co_edi_header_gran_contribuyente or "",
                ...
                "2.-%s"
                % (
                    invoice.company_id.l10n_co_edi_header_bank_information or ""
                ).replace("\n", "|"),
                "3.- %s" % (html2plaintext(invoice.narration or "N/A")),
                "6.- %s|%s"
                % (
                    html2plaintext(invoice.invoice_payment_term_id.note),
                    amount_in_words,
                ),
                "7.- %s" % (invoice.company_id.website),
                "8.-%s|%s|%s"
                % (
                    invoice.partner_id.commercial_partner_id._get_vat_without_verification_code()
                    or "",
                    shipping_partner.phone or "",
        ...
        move_lines_with_tax_type = invoice.line_ids.filtered(
            "tax_line_id.l10n_co_edi_type"
        )

        ovt_tax_codes = ("01C", "02C", "03C")
        ovt_taxes = move_lines_with_tax_type.filtered(
            lambda move: move.tax_line_id.l10n_co_edi_type.code in ovt_tax_codes
        ).tax_line_id
        ...
        retention_taxes = [
            ...
            if detail["l10n_co_edi_type"].retention
        ]
        regular_taxes = [
            (group, detail)
            for group, detail in tax_details["tax_details"].items()
            if not detail["l10n_co_edi_type"].retention
        ]

        exempt_tax_dict = {}
        tax_group_covered_goods = self.env.ref(
            "l10n_co.tax_group_covered_goods", raise_if_not_found=False
        )
        for line in invoice.invoice_line_ids:
            if tax_group_covered_goods and tax_group_covered_goods in line.mapped(
        ...
        for line in retention_lines:
            retention_lines_listdict[line.tax_line_id.l10n_co_edi_type.code].append(
                line
            )
            ...
            for tax, detail in tax_detail.get("tax_details").items():
                if not detail.get("tax_amount"):
                    for grouped_tax in detail.get("group_tax_details"):
                        ...
                        zero_tax_details[tax.l10n_co_edi_type.code] += abs(
                            grouped_tax.get("base_amount")
                        )
        retention_taxes_new = self._l10n_co_edi_prepare_tim_sections(
            retention_lines_listdict, invoice.currency_id, True
        )
        regular_taxes_new = self._l10n_co_edi_prepare_tim_sections(
            regular_lines_listdict, invoice.currency_id, False, zero_tax_details
        )
        # The rate should indicate how many pesos is one foreign currency
        currency_rate = "%.2f" % (
            tax_details["base_amount"] / tax_details["base_amount_currency"]
        )

        withholding_amount = "%.2f" % (
            invoice.amount_untaxed
            + sum(
                invoice.line_ids.filtered(
                    lambda move: move.tax_line_id
                    and not move.tax_line_id.l10n_co_edi_type.retention
                ).mapped("price_total")
            )
        )
        ...
        if invoice.move_type == "out_refund":
            ...
            edi_type = "{:0=2d}".format(int(invoice.l10n_co_edi_type))
        ...
        validation_time = (
            validation_time.strftime(DEFAULT_SERVER_TIME_FORMAT) + "-05:00"
        )
        ...
        description = (
            dict(invoice._fields[description_field].selection).get(description_code)
            if description_code
            else None
        )
    ...
    def _l10n_co_post_invoice_step_1(self, invoice):
        ...
        attachment = self.env["ir.attachment"].create(
            {
                ...
                "description": _(
                    "Colombian invoice UBL generated for the %s document.", invoice.name
                ),
            }
        )
        ...
        if "error" not in response:
            ...
            invoice.with_context(no_new_invoice=True).message_post(
                body=_(
                    "Electronic invoice submission succeeded. Message from Carvajal:<br/>%s",
                    response["message"],
                ),
    ...
    def _l10n_co_post_invoice_step_2(self, invoice):
        ...
        if not response.get("error"):
                ...
                response["attachment"] = self.env["ir.attachment"].create(
                    {
                        ...
                        "mimetype": "application/xml",
                        "description": _(
                            "Colombian invoice UBL generated for the %s document.",
                            invoice.name,
                        ),
                    }
                )
            ...
            # == Chatter ==
            invoice.with_context(no_new_invoice=True).message_post(
                body=response["message"], attachments=response["attachments"]
            )
    ...
    def _check_move_configuration(self, move):
        ...
        if (
            ...
            edi_result.append(
                _(
                    "Carvajal credentials are not set on the company, please go to Accounting Settings and set the credentials."
                )
            )
        if (
            ...
            edi_result.append(
                _(
                    "'Resolución DIAN' fields must be set on the journal %s",
        ...
        if not move.partner_id.vat:
            edi_result.append(
                _(
                    "You can not validate an invoice that has a partner without VAT number."
                )
            )
        if not move.company_id.partner_id.l10n_co_edi_obligation_type_ids:
            edi_result.append(
                _(
                    "'Obligaciones y Responsabilidades' on the Customer Fiscal Data section needs to be set for the partner %s.",
        ...
        if move.l10n_co_edi_type == "2" and any(
            l.product_id and not l.product_id.l10n_co_edi_customs_code
            for l in move.invoice_line_ids
        ):
            edi_result.append(_("Every exportation product must have a customs code."))
        elif move.invoice_date and not (
            oldest_date <= fields.Datetime.to_datetime(move.invoice_date) <= newest_date
        ):
            move.message_post(
                body=_(
            ...
            l.product_id
            and not l.product_id.default_code
            and not l.product_id.barcode
            and not l.product_id.unspsc_code_id
            for l in move.invoice_line_ids
        ):
            edi_result.append(
                _(
                    "Every product on a line should at least have a product code (barcode, internal, UNSPSC) set."
                )
            )

        if (
            not move.company_id.partner_id.l10n_latam_identification_type_id.l10n_co_document_code
        ):
            edi_result.append(
                _(
                    "The Identification Number Type on the company's partner should be 'NIT'."
                )
            )
        if (
            not move.partner_id.commercial_partner_id.l10n_latam_identification_type_id.l10n_co_document_code
        ):
            edi_result.append(
                _(
                    "The Identification Number Type on the customer's partner should be 'NIT'."
                )
            )
    ...
    def _post_invoice_edi(self, invoices):
        ...
        invoice = (
            invoices  # No batching ensures that only one invoice is given as parameter
        )
