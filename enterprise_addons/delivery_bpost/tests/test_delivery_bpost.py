...
class TestDeliveryBpost(TransactionCase):
    def setUp(self):
        ...
        self.odooinc = self.env["res.partner"].create(
            {
                "name": "Odoo Inc.",
                "is_company": True,
    ...
    def test_01_bpost_basic_be_domestic_flow(self):
            ...
            self.env["choose.delivery.carrier"].with_context(
                {
                    ...
                    "default_carrier_id": self.env.ref(
                        "delivery_bpost.delivery_carrier_bpost_domestic"
                    ).id,
                }
            )
        )
        ...
        self.assertGreater(
            ...
            0.0,
            "bpost delivery cost for this SO has not been correctly estimated.",
        )
        ...
        move_lines_to_pack = picking.move_line_ids.filtered(
            lambda line: line.product_id.id in [self.product2.id, self.product3.id]
        )
        ...
        try:
            ...
            if (
                exc.args[0]
                == "The BPost shipping service is unresponsive, please retry later."
            ):
        ...
        self.assertIsNot(
            ...
            False,
            "bpost did not return any tracking number",
        )
        self.assertGreater(
            picking.carrier_price, 0.0, "bpost carrying price is probably incorrect"
        )
        n_tracking_ref = len(picking.carrier_tracking_ref.split(TRACKING_REF_DELIM))
        self.assertEqual(
            n_tracking_ref,
            2,
            "should have 1 ref for the packed, 1 ref for the unpacked",
        )
            ...
            self.env["choose.delivery.carrier"].with_context(
                {
                    "default_order_id": sale_order.id,
                    "default_carrier_id": self.env.ref(
                        "delivery_bpost.delivery_carrier_bpost_inter"
                    ).id,
                }
            )
        )
    ...
    def test_02b_bpost_real_invoice_policy(self):
        ...
        self.assertEqual(
            len(delivery_line), 1, "The delivery line is not present on the SO."
        )
        ...
        self.assertEqual(
            ...
            "The delivery cost is not updated",
        )
