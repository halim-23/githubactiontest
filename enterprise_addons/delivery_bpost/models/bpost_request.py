...
class BpostRequest:
    ...
    def check_required_value(
        self, recipient, delivery_nature, shipper, order=False, picking=False
    ):
        ...
        if recipient.country_id.code == "BE" and delivery_nature == "International":
            return _(
                "bpost International is used only to ship outside Belgium. Please change the delivery method into bpost Domestic."
            )
        if recipient.country_id.code != "BE" and delivery_nature == "Domestic":
            return _(
                "bpost Domestic is used only to ship inside Belgium. Please change the delivery method into bpost International."
            )
        ...
        if shipper.country_id.code != "BE":
            return _(
                "Your company/warehouse address must be in Belgium to ship with bpost"
            )

        if order:
            if order.order_line and all(
                order.order_line.mapped(lambda l: l.product_id.type == "service")
            ):
                return _(
                    "The estimated shipping price cannot be computed because all your products are service."
                )
    ...
    def rate(self, order, carrier):
        ...
        return self._get_rate(
            carrier, _grams(weight_in_kg), order.partner_shipping_id.country_id
        )
    ...
    def _get_rate(self, carrier, weight, country):
        ...
        bpost_delivery_type = (
            carrier.bpost_domestic_deliver_type
            if carrier.bpost_delivery_nature == "Domestic"
            else carrier.bpost_international_deliver_type
        )
        for delivery_method in xml_response.findall(
            'ns1:deliveryMethod/[@name="home or office"]/ns1:product/[@name="%s"]/ns1:price'
            % bpost_delivery_type,
            ns,
        ):
            if delivery_method.attrib["countryIso2Code"] == country.code:
                price = float(self._get_price_by_weight(weight, delivery_method)) / 100
                sale_price_digits = carrier.env["decimal.precision"].precision_get(
                    "Product Price"
                )
                price = float_round(price, precision_digits=sale_price_digits)
        if not price:
            raise UserError(
                _("bpost did not return prices for this destination country.")
            )

        # If delivery on saturday is enabled, there are additional fees
        additional_fees = 0.0
        if carrier.bpost_saturday is True:
            for option_price in xml_response.findall(
                'ns1:deliveryMethod/[@name="home or office"]/ns1:product/[@name="%s"]/ns1:option/[@name="Saturday"]'
                % bpost_delivery_type,
                ns,
            ):
    ...
    def send_shipping(self, picking, carrier, with_return_label, is_return_label=False):
        ...
        if is_return_label:
            ...
            receiver_company = (
                receiver.commercial_partner_id.name
                if receiver.commercial_partner_id != receiver
                else ""
            )
        ...
        if receiver.state_id:
            receiver_locality = "%s, %s" % (
                receiver_locality,
                picking.partner_id.state_id.display_name,
            )

        values = {
            "accountId": carrier.sudo().bpost_account_number,
            ...
            # domestic
            "product": "bpack Easy Retour"
            if is_return_label
            else carrier.bpost_domestic_deliver_type,
        ...
        xml = carrier.env["ir.qweb"]._render(
            "delivery_bpost.bpost_shipping_request", values
        )
        ...
        code, response2 = self._send_request(
            "label",
            None,
            carrier,
            reference=reference_id,
            with_return_label=with_return_label,
        )
        ...
        for labels in root.findall("ns1:label", ns):
            if with_return_label:
                ...
                main_label = {
                    "tracking_codes": [
                        label.text for label in labels.findall("ns1:barcode", ns)
                    ],
                    "label": a2b_base64(labels.find("ns1:bytes", ns).text),
                }
                return_label = False
        return {"price": price, "main_label": main_label, "return_label": return_label}
    ...
    def _send_request(
        self, action, xml, carrier, reference=None, with_return_label=False
    ):
        supercarrier = carrier.sudo()
        passphrase = supercarrier._bpost_passphrase()
        METHODS = {"rate": "GET", "send": "POST", "label": "GET"}
        HEADERS = {
            "rate": {
                "authorization": "Basic %s" % passphrase,
            ...
            "label": {
                "authorization": "Basic %s" % passphrase,
                "accept": "application/vnd.bpost.shm-label-%s-v3+XML"
                % ("pdf" if carrier.bpost_label_format == "PDF" else "image"),
        ...
        label_url = url_join(
            ...
            "%s/orders/%s/labels/%s"
            % (
                supercarrier.bpost_account_number,
                reference,
                carrier.bpost_label_stock_type,
            ),
        )
        if with_return_label:
        ...
        URLS = {
            "rate": url_join(
                self.base_url, "%s/productconfig" % supercarrier.bpost_account_number
            ),
            "send": url_join(
                self.base_url, "%s/orders" % supercarrier.bpost_account_number
            ),
            "label": label_url,
        }
        self.debug_logger(
            "%s\n%s\n%s" % (URLS[action], HEADERS[action], xml if xml else None),
            "bpost_request_%s" % action,
        )
        try:
            ...
            raise UserError(
                _("The BPost shipping service is unresponsive, please retry later.")
            )
        self.debug_logger(
            "%s\n%s" % (response.status_code, response.text),
            "bpost_response_%s" % action,
        )
    ...
    def _compute_boxes(self, picking, carrier):
        ...
        for package in picking.package_ids:
            package_lines = picking.move_line_ids.filtered(
                lambda sml: sml.result_package_id.id == package.id
            )
            ...
            boxes.append(
                {
                    ...
                    "contentDescription": " ".join(
                        [
                            ...
                            % (
                                line.qty_done,
                                re.sub(r"[\W_]+", " ", line.product_id.name or ""),
                            )
        ...
        lines_without_package = picking.move_line_ids.filtered(
            lambda sml: not sml.result_package_id
        )
        if lines_without_package:
            parcel_value = sum(sml.sale_price for sml in lines_without_package)
            weight_in_kg = carrier._bpost_convert_weight(
                sum(
                    sml.qty_done * sml.product_id.weight
    ...
    def _compute_return_boxes(self, picking, carrier):
        weight = sum(
            move.product_qty * move.product_id.weight for move in picking.move_lines
        )
        weight_in_kg = carrier._bpost_convert_weight(weight)
        parcel_value = sum(
            move.product_qty * move.product_id.lst_price for move in picking.move_lines
        )
        boxes = [
            {
                "weight": str(_grams(weight_in_kg)),
