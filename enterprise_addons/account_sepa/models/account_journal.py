...
class AccountJournal(models.Model):
    ...
    has_sepa_ct_payment_method = fields.Boolean(
        compute="_compute_has_sepa_ct_payment_method"
    )
    ...
    def _compute_sepa_pain_version(self):
        ...
        for rec in self:
            ...
            if rec.bank_acc_number and re.match(
                "^[A-Z]{2}[0-9]{2}.*", rec.bank_acc_number
            ):
            ...
            rec.sepa_pain_version = pains_by_country.get(
                country_code, "pain.001.001.03"
            )
    ...
    def create_iso20022_credit_transfer(
        self, payments, batch_booking=False, sct_generic=False
    ):
        """
        ...
        for payment in payments:
            ...
            required_payment_date = (
                payment["payment_date"] if payment["payment_date"] > today else today
            )
            payments_date_instr_wise[(required_payment_date, local_instrument)].append(
                payment
            )
        count = 0
        for (
            payment_date,
            local_instrument,
        ), payments_list in payments_date_instr_wise.items():
            count += 1
            ...
            if (
                len(PmtTpInf) != 0
            ):  # Boolean conversion from etree element triggers a deprecation warning ; this is the proper way
            ...
            if (
                pain_version == "pain.001.001.03.se"
                and not self.bank_account_id.bank_bic
            ):
                raise UserError(
                    _(
                        "Bank account %s 's bank does not have any BIC number associated. Please define one."
                    )
            ...
            for payment in payments_list:
                PmtInf.append(
                    self._get_CdtTrfTxInf(
                        PmtInfId, payment, sct_generic, pain_version, local_instrument
                    )
                )
    ...
    def _get_InitgPty(self, pain_version, sct_generic=False):
        ...
        if pain_version == "pain.001.001.03.se":
            InitgPty.extend(
                self._get_company_PartyIdentification32(
                    sct_generic,
                    org_id=True,
            ...
            InitgPty.extend(
                self._get_company_PartyIdentification32(
                    sct_generic, org_id=True, postal_address=False, issr=False
                )
            )
        else:
            InitgPty.extend(
                self._get_company_PartyIdentification32(
                    sct_generic, org_id=True, postal_address=False, issr=True
                )
            )
        return InitgPty

    def _get_company_PartyIdentification32(
        self,
        sct_generic=False,
        org_id=True,
        postal_address=True,
    ...
    def _get_Dbtr(self, pain_version, sct_generic=False):
        ...
        if pain_version == "pain.001.001.03.se":
            ...
            Dbtr.extend(
                self._get_company_PartyIdentification32(
                    sct_generic, org_id=not sct_generic, postal_address=True
                )
            )
        return Dbtr
    ...
    def _get_PstlAdr(self, partner_id):
        if not partner_id.country_id.code:
            raise ValidationError(
                _("Partner %s has no country code defined.", partner_id.name)
            )
        ...
        if partner_id.zip and partner_id.city:
            AdrLine = etree.SubElement(PstlAdr, "AdrLine")
            AdrLine.text = sanitize_communication(
                (partner_id.zip + " " + partner_id.city)[:70]
            )
        return PstlAdr

    def _get_CdtTrfTxInf(
        self, PmtInfId, payment, sct_generic, pain_version, local_instrument=None
    ):
        ...
        currency_id = self.env["res.currency"].search(
            [("id", "=", payment["currency_id"])], limit=1
        )
        journal_id = self.env["account.journal"].search(
            [("id", "=", payment["journal_id"])], limit=1
        )
        val_Ccy = (
            currency_id and currency_id.name or journal_id.company_id.currency_id.name
        )
        ...
        if local_instrument != "CH01" and not self.env.context.get("skip_bic", False):
            CdtTrfTxInf.append(
                self._get_CdtrAgt(partner_bank, sct_generic, pain_version)
            )
        ...
        Nm.text = sanitize_communication(
            (
                ...
                or partner.name
                or partner.commercial_partner_id.name
                or "/"
            )[:70]
        )
        if partner.country_id.code and (
            partner.city or pain_version == "pain.001.001.03.se"
        ):  # For Sweden, country is enough
    ...
    def _get_CdtrAgt(self, bank_account, sct_generic, pain_version):
        ...
        if bank_account.bank_bic:
            ...
            if pain_version == "pain.001.001.03.austrian.004":
                ...
                raise UserError(
                    _(
                        "The bank defined on account %s (from partner %s) has no BIC. Please first set one.",
                        bank_account.acc_number,
                        bank_account.partner_id.name,
                    )
                )
    ...
    def _get_CdtrAcct(self, bank_account, sct_generic):
        if not sct_generic and (
            not bank_account.acc_type or not bank_account.acc_type == "iban"
        ):
            raise UserError(
                _(
                    "The account %s, linked to partner '%s', is not of type IBAN.\nA valid IBAN account is required to use SEPA features."
                )
                % (bank_account.acc_number, bank_account.partner_id.name)
            )
        ...
        if sct_generic and bank_account.acc_type != "iban":
            ...
            if (
                " " in bank_account.sanitized_acc_number
                and " " in bank_account.acc_number
            ):
    ...
    def _is_qr_iban(self, payment_dict):
        ...
        different.
        """
        partner_bank = self.env["res.partner.bank"].browse(
            payment_dict["partner_bank_id"]
        )
        company = (
            self.env["account.journal"].browse(payment_dict["journal_id"]).company_id
        )
