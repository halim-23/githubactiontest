...
class AccountBatchPayment(models.Model):
    ...
    sct_generic = fields.Boolean(
        ...
        "not identified by an IBAN account number.",
    )

    @api.depends("payment_ids", "journal_id")
    def _compute_sct_generic(self):
        for record in self:
            record.sct_generic = bool(record._get_sct_genericity_warnings()) or any(
                payment.company_id.account_fiscal_country_id.code == "CH"
        ...
        for batch in self.filtered(lambda x: x.payment_method_code == "sepa_ct"):
            if batch.journal_id.bank_account_id.acc_type != "iban":
                raise UserError(
                    ...
                    % (
                        batch.journal_id.bank_account_id.acc_number,
    ...
    def _get_sct_genericity_warnings(self):
        ...
        if no_iban_payments:
            rslt.append(
                {
                    "title": _(
                        "Some payments are not made on an IBAN recipient account. This batch might not be accepted by certain banks because of that."
                    ),
        ...
        if no_eur_payments:
            rslt.append(
                {
                    "title": _(
                        "Some payments were instructed in another currency than Euro. This batch might not be accepted by certain banks because of that."
                    ),
    ...
    def check_payments_for_errors(self):
        ...
        for payment in self.payment_ids.filtered(lambda x: x.state == "posted"):
            ...
            if (
                len(float_repr(payment.amount, 2)) >= max_digits
        ...
        if no_bank_acc_payments:
            rslt.append(
                {
                    "title": _("Some payments have no recipient bank account set."),
                    "records": no_bank_acc_payments,
                }
            )
            ...
            rslt.append(
                {
                    ...
                    "help": _(
                        "Maximum amount is %s for payments in Euros, %s for other currencies.",
                        8 * "9" + ".99",
                        12 * "9" + ".99",
                    ),
                }
    ...
    def _generate_export_file(self):
        if self.payment_method_code == "sepa_ct":
            ...
            xml_doc = self.journal_id.create_iso20022_credit_transfer(
                payment_dicts, self.sct_batch_booking, self.sct_generic
            )
            return {
    ...
    def _generate_payment_template(self, payments):
        ...
        for payment in payments:
            ...
            payment_dict = {
                ...
                + (
                    payment.ref
                    or "SCT-" + self.journal_id.code + "-" + str(fields.Date.today())
                ),
