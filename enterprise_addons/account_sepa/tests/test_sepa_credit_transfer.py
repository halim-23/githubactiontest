...
class TestSEPACreditTransfer(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.sepa_ct = cls.bank_journal.outbound_payment_method_line_ids.filtered(
        ...
        schema_file_path = get_module_resource(
            "account_sepa", "schemas", "pain.001.001.03.xsd"
        )
    ...
    def testStandardSEPA(self):
        for bic in ["BBRUBEBB", False]:
            ...
            batch = self.env["account.batch.payment"].create(
                {
                    ...
                    "payment_ids": [
                        (4, payment.id, None) for payment in (payment_1 | payment_2)
                    ],
            ...
            self.assertTrue(
                wizard_action, "Validation wizard should have returned an action"
            )
            self.assertEqual(
                wizard_action.get("res_model"),
                "account.batch.download.wizard",
                "The action returned at validation should target a download wizard",
            )

            download_wizard = self.env["account.batch.download.wizard"].browse(
                batch.export_batch_payment()["res_id"]
            )
            sct_doc = etree.fromstring(base64.b64decode(download_wizard.export_file))
            self.assertTrue(
                self.xmlschema.validate(sct_doc), self.xmlschema.error_log.last_error
            )
    ...
    def testGenericSEPA(self):
        for bic in ["BBRUBEBB", False]:
            ...
            self.assertEqual(
                ...
                "The action returned at validation should target an error wizard",
            )

            error_wizard = self.env["account.batch.error.wizard"].browse(
                wizard_action["res_id"]
            )
            self.assertTrue(
                len(error_wizard.warning_line_ids) > 0,
                "Using generic SEPA should raise warnings",
            )
            self.assertTrue(
                len(error_wizard.error_line_ids) == 0,
                "Error wizard should not list any error",
            )

            download_wizard = self.env["account.batch.download.wizard"].browse(
                error_wizard.proceed_with_validation()["res_id"]
            )
            sct_doc = etree.fromstring(base64.b64decode(download_wizard.export_file))
            self.assertTrue(
                self.xmlschema.validate(sct_doc), self.xmlschema.error_log.last_error
    ...
    def testSEPAPainVersion(self):
        ...
        self.env.company.country_id = self.env.company.country_id = self.env.ref(
            "base.se"
        )
