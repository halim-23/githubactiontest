...
class PlanningSend(models.TransientModel):
    ...
    note = fields.Text(
        "Extra Message",
        help="Additional message displayed in the email sent to employees",
    )
    employee_ids = fields.Many2many(
        "hr.employee",
        string="Resources",
        help="Employees who will receive planning by email if you click on publish & send.",
        compute="_compute_slots_data",
        inverse="_inverse_employee_ids",
        store=True,
    )
    slot_ids = fields.Many2many(
        "planning.slot", compute="_compute_slots_data", store=True
    )
    ...
    def _compute_slots_data(self):
        for wiz in self:
            wiz.slot_ids = (
                ...
                .search(
                    [
                        ("start_datetime", ">=", wiz.start_datetime),
                        ("end_datetime", "<=", wiz.end_datetime),
                    ]
                )
            )
            wiz.employee_ids = wiz.slot_ids.filtered(
                lambda s: s.resource_type == "user"
            ).mapped("employee_id")
    ...
    def get_employees_without_work_email(self):
        ...
        if not self.employee_ids.check_access_rights("write", raise_exception=False):
            return None
        employee_ids_without_work_email = self.employee_ids.filtered(
    ...
    def action_send(self):
        if not self.employee_ids:
            raise UserError(
                _("Select the employees you would like to send the planning to.")
            )
        ...
        if not slot_to_send:
            raise UserError(
                _(
                    "This action is not allowed as there are no shifts planned for the selected time period."
                )
            )
        # create the planning
        ...
        open_slots = slot_to_send.filtered(
            lambda s: not s.employee_id and not s.is_past
        )
        ...
        for employee in self.employee_ids:
            if employee in slot_employees:
                ...
                for slot in open_slots:
                    if (
                        not employee.planning_role_ids
                        or not slot.role_id
                        or slot.role_id in employee.planning_role_ids
                    ):
                        employees_to_send |= employee
        ...
        if res:
            return {
                ...
                "params": {
                    "type": "success",
                    "message": _(
                        "The schedule was successfully sent to your employees."
                    ),
    ...
    def action_publish(self):
        ...
        slot_to_publish.write({"state": "published", "publication_warning": False})
        return True
