...
class TestPlanningHr(TestCommonPlanning):
    ...
    def test_change_default_planning_role(self):
        self.assertFalse(
            self.employee_joseph.default_planning_role_id,
            "Joseph should have no default planning role",
        )
        self.assertFalse(
            self.employee_joseph.planning_role_ids,
            "Joseph should have no planning roles",
        )

        role_a = self.env["planning.role"].create({"name": "role a"})
        role_b = self.env["planning.role"].create({"name": "role b"})
        ...
        self.assertEqual(
            self.employee_joseph.default_planning_role_id,
            role_a,
            "Joseph should have role a as default role",
        )
        self.assertTrue(
            role_a in self.employee_joseph.planning_role_ids,
            "role a should be added to his planning roles",
        )

        self.employee_joseph.write({"planning_role_ids": [(5, 0, 0)]})
        self.assertTrue(
            role_a in self.employee_joseph.planning_role_ids,
            "role a should be automatically added to his planning roles",
        )

        self.employee_joseph.default_planning_role_id = role_b
        self.assertTrue(
            role_a in self.employee_joseph.planning_role_ids,
            "role a should still be in planning roles",
        )
        self.assertTrue(
            role_b in self.employee_joseph.planning_role_ids,
            "role b should be added to planning roles",
        )
    ...
    def test_hr_employee_view_planning(self):
        ...
        self.assertEqual(
            action["context"]["default_resource_id"], self.resource_bert.id
        )
    ...
    def test_employee_contract_validity_per_period(self):
        ...
        calendars_validity_within_period = (
            self.employee_joseph.resource_id._get_calendars_validity_within_period(
                start, end, default_company=self.employee_joseph.company_id
            )
        )

        self.assertEqual(
            len(calendars_validity_within_period[self.employee_joseph.resource_id.id]),
            1,
            "There should exist 1 calendar within the period",
        )
        interval_calendar_joseph = Intervals(
            [(start, end, self.env["resource.calendar.attendance"])]
        )
        computed_interval = calendars_validity_within_period[
            self.employee_joseph.resource_id.id
        ][self.employee_joseph.resource_calendar_id]
        self.assertFalse(
            computed_interval - interval_calendar_joseph,
            "The interval of validity for the 40h calendar must be from 2015-11-16 to 2015-11-21, not more",
        )
        self.assertFalse(
            interval_calendar_joseph - computed_interval,
            "The interval of validity for the 40h calendar must be from 2015-11-16 to 2015-11-21, not less",
        )

    def test_employee_work_intervals(self):
        start = datetime(2015, 11, 8, 00, 00, 00, tzinfo=pytz.UTC)
        end = datetime(2015, 11, 21, 23, 59, 59, tzinfo=pytz.UTC)
        work_intervals = self.employee_joseph.resource_id._get_work_intervals_batch(
            start, end
        )
        ...
        self.assertEqual(
            16,
            sum_work_intervals,
            "Sum of the work intervals for the employee Joseph should be 8h+8h = 16h",
        )
    ...
    def test_employee_work_planning_hours_info(self):
        ...
        planning_hours_info = joseph_resource_id.get_planning_hours_info(
            "2015-11-08 00:00:00", "2015-11-28 23:59:59"
        )
        self.assertEqual(
            24,
            planning_hours_info[joseph_resource_id.id]["work_hours"],
            "Work hours for the employee Jules should be 8h+8h+8h = 24h",
        )
        self.assertEqual(
            39,
            planning_hours_info[joseph_resource_id.id]["planned_hours"],
            "Planned hours for the employee Jules should be 3*9h+8h+4h = 39h",
        )

        planning_hours_info = joseph_resource_id.get_planning_hours_info(
            "2015-11-12 00:00:00", "2015-11-12 23:59:59"
        )
        self.assertEqual(
            8,
            planning_hours_info[joseph_resource_id.id]["work_hours"],
        ...
        planning_hours_info = joseph_resource_id.get_planning_hours_info(
            "2015-11-26 00:00:00", "2015-11-26 23:59:59"
        )
        self.assertEqual(
            8,
            planning_hours_info[joseph_resource_id.id]["work_hours"],
