...
class TestRecurrencySlotGeneration(TestCommonPlanning):
    ...
    def test_repeat_until(self):
        ...
        with self._patch_now("2019-06-27 08:00:00"):
            ...
            first_generated_slots_dates = set(
                map(lambda slot: slot.start_datetime, generated_slots)
            )
            expected_slot_dates = {
            ...
            self.assertTrue(
                first_generated_slots_dates == expected_slot_dates,
                "Initial run should have created expected slots",
            )
        ...
        with self._patch_now("2019-07-11 08:00:00"):
            ...
            self.assertTrue(
                all_slots_dates == new_expected_slots_dates,
                "first cron run should have generated 2 more slots",
            )
    ...
    def test_repeat_until_no_repeat(self):
        ...
        with self._patch_now("2019-06-27 08:00:00"):
            ...
            self.assertEqual(
                ...
                1,
                "first run should only have created 1 slot since repeat until is set at 1 week",
            )
    ...
    def test_repeat_until_cron_idempotent(self):
        ...
        with self._patch_now("2019-06-27 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                2,
                "initial run should have generated 2 slots",
            )
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                2,
                "running the cron right after should not generate new slots",
            )
    ...
    def test_repeat_until_cron_generation(self):
        ...
        with self._patch_now("2019-08-31 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                5,
                "first run should have generated 5 slots",
            )
        ...
        with self._patch_now("2019-09-14 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                7,
                "first cron should have generated 2 more slots",
            )
        ...
        with self._patch_now("2019-09-16 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                7,
                "second cron should not generate any slots",
            )
    ...
    def test_repeat_until_long_limit(self):
        ...
        with self._patch_now("2019-06-01 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                27,
                "first run has generated 27 slots",
            )
        ...
        with self._patch_now("2019-06-08 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                28,
                "second cron should only generate 1 more slot",
            )
    ...
    def test_repeat_forever(self):
        ...
        with self._patch_now("2019-05-16 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                6,
                "first run should generate 7 slots",
            )
        ...
        with self._patch_now("2019-05-24 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                7,
                "first cron should generate one more slot",
            )
        ...
        with self._patch_now("2019-05-31 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                7,
                "second run should not generate any slots",
            )
    ...
    def kkktest_slot_remove_all(self):
        with self._patch_now("2019-06-01 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                0,
                "calling remove after on any slot from the recurrency remove all slots linked to the recurrency",
            )
    ...
    def test_recurring_slot_company(self):
        with self._patch_now("2019-06-01 08:00:00"):
            ...
            self.assertEqual(
                len(self.get_by_employee(self.employee_bert)),
                18,
                "initial company's span is two month, so 2 * 9 slots",
            )
            self.assertEqual(
                len(self.get_by_employee(self.employee_joseph)),
                5,
                "other company's span is one month, so only 5 slots",
            )

            self.assertEqual(
                slot1.company_id,
                slot1.recurrency_id.company_id,
                "Recurrence and slots (1) must have the same company",
            )
            self.assertEqual(
                slot1.recurrency_id.company_id,
                slot1.recurrency_id.slot_ids.mapped("company_id"),
                "All slots in the same recurrence (1) must have the same company",
            )
            self.assertEqual(
                slot2.company_id,
                slot2.recurrency_id.company_id,
                "Recurrence and slots (2) must have the same company",
            )
            self.assertEqual(
                slot2.recurrency_id.company_id,
                slot2.recurrency_id.slot_ids.mapped("company_id"),
                "All slots in the same recurrence (1) must have the same company",
            )
    ...
    def test_slot_detach_if_some_fields_change(self):
        ...
        with self._patch_now("2019-06-27 08:00:00"):
            ...
            self.assertEqual(
                len(joseph_slots), 5, "the recurrency should generate 5 slots"
            )
            self.assertEqual(
                len(joseph_slots),
                len(recurrence.slot_ids),
                "all the slots generated should belong to the original employee",
            )
            ...
            self.assertEqual(
                len(recurrence.slot_ids),
                4,
                "writing on the slot should detach it from the recurrency",
            )
        ...
        with self._patch_now("2020-06-27 08:00:00"):
            ...
            self.assertEqual(
                len(
                    list(
                        filter(
                            lambda recu: recu.id == empty_recurrency_id,
                            self.env["planning.recurrency"].search([]),
                        )
                    )
                ),
                1,
                "the empty recurrency we created should be in the db",
            )
            self.env["planning.recurrency"]._cron_schedule_next()
            ...
            self.assertEqual(
                ...
                0,
                "the empty recurrency we created should not be in the db anymore",
            )
            recurrencies = self.env["planning.recurrency"].search([])
            self.assertFalse(
                len(list(filter(lambda recu: len(recu.slot_ids) == 0, recurrencies))),
                "cron with no slot gets deleted (there is no original slot to copy from)",
            )
    ...
    def test_recurrency_change_date(self):
        with self._patch_now("2020-01-01 08:00:00"):
            ...
            self.assertEqual(
                slot.recurrency_id.repeat_type,
                "until",
                "Changing the date should not change the repeat_type",
            )

            slot.update({"repeat_type": "forever"})
            self.assertEqual(
                slot.recurrency_id.repeat_until,
                False,
                "Repeat forever should not have a date",
            )
