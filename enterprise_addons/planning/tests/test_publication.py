...
class TestPlanningPublishing(TestCommonPlanning):
    ...
    def setUpClass(cls):
        ...
        cls.employee_dirk_no_mail = cls.env["hr.employee"].create(
            {"user_id": False, "name": "Dirk", "work_email": False, "tz": "UTC"}
        )
    ...
    def test_planning_publication(self):
        self.shift.write({"allocated_hours": 10})
        ...
        self.assertEqual(
            self.shift.state,
            "published",
            "planning is published when we call its action_send",
        )

        shift_mails = set(Mails.search([])) ^ set(before_mails)
        self.assertEqual(
            len(shift_mails), 1, "only one mail is created when publishing planning"
        )
    ...
    def test_sending_planning_do_not_create_mail_if_employee_has_no_email(self):
        ...
        self.assertEqual(
            len(shift_mails),
            0,
            "no mail should be sent when the employee has no work email",
        )
