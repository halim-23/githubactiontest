...
class TestPlanningForm(TestCommonPlanning):
    ...
    def setUpClass(cls):
        ...
        cls.setUpEmployees()
        cls.employee_janice.resource_calendar_id = (
            cls.resource_calendar_38_hours_per_week.id
        )
    ...
    def test_planning_no_employee_no_company(self):
        ...
        with Form(self.env["planning.slot"]) as slot:
            ...
            self.assertEqual(
                slot.allocated_hours, (end - start).total_seconds() / (60 * 60)
            )
    ...
    def planning_form(self, timezone, start, end, expected_start, expected_end):
        ...
        with Form(
            self.env["planning.slot"].with_user(self.test_user).with_context(context)
        ) as slot:
            tz = pytz.timezone(self.employee_janice.resource_calendar_id.tz)
            start_decimal_time = (
                slot.start_datetime.astimezone(tz).hour
                + slot.start_datetime.astimezone(tz).minute / 60
            )
            self.assertEqual(
                start_decimal_time,
                expected_start,
            ...
            end_decimal_time = (
                slot.end_datetime.astimezone(tz).hour
                + slot.end_datetime.astimezone(tz).minute / 60
            )
            self.assertEqual(
                end_decimal_time,
                expected_end,
