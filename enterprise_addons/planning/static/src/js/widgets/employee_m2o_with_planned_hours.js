...
const EmployeeWithPlannedHours = StandaloneM2OAvatarEmployee.extend({
  ...
  _updateTemplateFromCacheData() {
    const plannedHours = fieldUtils.format.float(this.cachePlannedHours, {
      digits: [false, 0],
    });
    this.$templateHtml = $(
      qweb._render(this.hoursTemplate, {
        width: Math.min(
          (this.cachePlannedHours /
            (this.cacheWorkHours || this.cachePlannedHours || 1)) *
            100,
          100
        ), // avoid NaN
        ...
        no_work_hours_title: _.str.sprintf(
          _t(
            "This employee isn't expected to work during this period. Planned hours : %s hours"
          ),
          plannedHours
        ),
      })
