  ...
  _openDialog: function (resID, context) {
    ...
    dialog.on("execute_action", this, async function (e) {
      ...
      if (action_name === "action_unschedule") {
        ...
        if (
          initialState.data.template_creation != state.data.template_creation &&
          state.data.template_creation
        ) {
  ...
  async _onCopyWeekClicked(ev) {
    ...
    if (result) {
      ...
      this.displayNotification({
        type: "danger",
        message: _t(
          "There are no shifts to copy or the previous shifts were already copied."
        ),
      });
    }
    this.reload();
