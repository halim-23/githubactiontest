...
const PlanningGanttRow = HrGanttRow.extend({
  ...
  init(parent, pillsInfo, viewInfo, options) {
    ...
    this.employeeID =
      (pillsInfo.planningHoursInfo && pillsInfo.planningHoursInfo.employee_id) || false;
  ...
  _getPopoverContext: function () {
    ...
    data.allocatedPercentageFormatted = fieldUtils.format.float(
  ...
  async _preloadAvatarWidget() {
    ...
    this.avatarWidget = new EmployeeWithPlannedHours(
      this,
      employee,
      this.planningHoursInfo
    );
