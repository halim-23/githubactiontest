...
var PlanningCalendarPopover = CalendarPopover.extend({
  ...
  init() {
    ...
    this.allocated_hours = fieldUtils.format.float_time(
      this.event.extendedProps.record.allocated_hours
    );
    this.allocated_percentage = fieldUtils.format.float(
      this.event.extendedProps.record.allocated_percentage
    );
  },

  willStart: function () {
    const self = this;
    const check_group = this.getSession()
      .user_has_group("planning.group_planning_manager")
  ...
  _processFields: function () {
    ...
    _.each(this.displayFields, function (def, field) {
      if (
        self.event.extendedProps &&
        self.event.extendedProps.record &&
        !self.event.extendedProps.record[field]
      ) {
