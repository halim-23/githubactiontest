odoo.define("planning.PlanningGanttRenderer", function (require) {
  ...
  const PlanningGanttRenderer = HrGanttRenderer.extend({
    ...
    sampleDataTargets: [".o_gantt_row[data-from-server=true]"],
    async _renderView() {
    ...
    _generatePillLabels: function (pills, scale) {
      ...
      const yearlessDateFormat = dateFormat
        .replace(/Y/gi, "")
        .replace(/(\W)\1+/g, "$1")
        .replace(/^\W|\W$/, "");

      pills
        .filter((pill) => !pill.consolidated)
        .forEach((pill) => {
          const localStartDateTime = (pill.start_datetime || pill.startDate)
            .clone()
            .local();
          ...
          if (!spanAccrossDays && ["week", "month"].includes(scale)) {
            labelElements.push(
              ...
              localEndDateTime.format("LT") +
                " (" +
                fieldUtils.format
                  .float_time(pill.allocated_hours, {}, {noLeadingZeroHour: true})
                  .replace(/(:00|:)/g, "h") +
                ")"
            );
          }
          ...
          pill.label = labelElements.filter((el) => Boolean(el)).join(" - ");
        });
    },
    /**
