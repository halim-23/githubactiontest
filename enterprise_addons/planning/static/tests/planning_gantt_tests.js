odoo.define("planning.planning_gantt_tests.js", function (require) {
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("empty gantt view with sample data: send schedule", async function (
        assert
      ) {
        assert.expect(3);
      ...
      QUnit.test('add record in empty gantt with sample="1"', async function (assert) {
        ...
        await testUtils.dom.triggerMouseEvent(
          gantt.$(`.o_gantt_row:first .o_gantt_cell:first .o_gantt_cell_add`),
          "click"
        );
      ...
      QUnit.test(
        ...
        async function (assert) {
          ...
          assert.doesNotHaveClass(getRow(0), "open", "'Open Shift' Group Collapsed");
          await testUtils.dom.click(getRow(0));
          assert.hasClass(getRow(0), "open", "'Open Shift' Group Expanded");
          assert.strictEqual(
            getRow(2).innerText,
            "Open Shifts",
            'should contain "Open Shifts" as a first group header for grouped by "Employee"'
          ...
          await testUtils.dom.click(getRow(1));
          assert.hasClass(getRow(1), "open", "'Undefined Role' Sub Group Expanded");
          assert.strictEqual(
            getRow(2).innerText,
            "Open Shifts",
            'should contain "Open Shifts" as a first group header for grouped by "Employee"'
