...
class Planning(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        string="Company",
        required=True,
        compute="_compute_planning_slot_company_id",
        store=True,
        readonly=False,
    )
    ...
    was_copied = fields.Boolean(
        "This Shift Was Copied From Previous Week", default=False, readonly=True
    )
    ...
    allow_self_unassign = fields.Boolean(
        "Let Employee Unassign Themselves",
    ...
    unassign_deadline = fields.Datetime(
        "Deadline for unassignment", compute="_compute_unassign_deadline"
    )
    is_unassign_deadline_passed = fields.Boolean(
        "Is unassignement deadline not past",
        compute="_compute_is_unassign_deadline_passed",
    )
    is_assigned_to_me = fields.Boolean(
        "Is This Shift Assigned To The Current User",
        compute="_compute_is_assigned_to_me",
    )
    conflicting_slot_ids = fields.Many2many(
        "planning.slot", compute="_compute_overlap_slot_count"
    )
    overlap_slot_count = fields.Integer(
        "Overlapping Slots",
        compute="_compute_overlap_slot_count",
        search="_search_overlap_slot_count",
    )
    is_past = fields.Boolean(
        "Is This Shift In The Past?", compute="_compute_past_shift"
    )
    ...
    working_days_count = fields.Float(
        "Number of Working Days", compute="_compute_working_days_count", store=True
    )
    ...
    template_creation = fields.Boolean(
        "Save as Template", store=False, inverse="_inverse_template_creation"
    )
    template_autocomplete_ids = fields.Many2many(
    ...
    allow_template_creation = fields.Boolean(
        string="Allow Template Creation", compute="_compute_allow_template_creation"
    )
    ...
    repeat = fields.Boolean(
        "Repeat", compute="_compute_repeat", inverse="_inverse_repeat"
    )
    ...
    confirm_delete = fields.Boolean(
        "Confirm Slots Deletion", compute="_compute_confirm_delete"
    )
    ...
    _sql_constraints = [
        (
            "check_start_date_lower_end_date",
            "CHECK(end_datetime > start_datetime)",
            "Shift end date should be greater than its start date",
        ),
        (
            "check_allocated_hours_positive",
            "CHECK(allocated_hours >= 0)",
            "You cannot have negative shift",
        ),
    ]
    ...
    def _compute_confirm_delete(self):
        for slot in self:
            if slot.recurrency_id and slot.repeat_until:
                slot.confirm_delete = (
                    fields.Date.to_date(slot.recurrency_id.repeat_until)
                    > slot.repeat_until
                    if slot.recurrency_id.repeat_until
                    else True
                )
            else:
    ...
    def _check_repeat_until(self):
        if any(
            [
                slot.repeat_until and slot.repeat_until < slot.start_datetime.date()
                for slot in self
            ]
        ):
            raise UserError(
                _("The recurrence until date should be after the shift start date")
            )
    ...
    def _compute_planning_slot_company_id(self):
        for slot in self:
            slot.company_id = (
                slot.resource_id.company_id or slot.company_id or slot.env.company
            )
    ...
    def _compute_employee_id(self):
        for slot in self:
            slot.employee_id = (
                slot.resource_id.employee_id if slot.resource_type == "user" else False
            )
    ...
    def _compute_role_id(self):
        for slot in self:
            ...
            if slot.template_id:
                ...
                and slot.previous_template_id.role_id == slot.role_id
            ):
                slot.role_id = False
    ...
    def _compute_allocation_type(self):
        for slot in self:
            if (
                ...
                and slot._get_slot_duration() < 24
            ):
    ...
    def _compute_allocated_percentage(self):
        ...
        for slot in self:
            if (
                slot.start_datetime
                and slot.end_datetime
                and slot.start_datetime != slot.end_datetime
            ):
                if slot.allocation_type == "planning":
                    slot.allocated_percentage = (
                        100 * slot.allocated_hours / slot._get_slot_duration()
                    )
                else:
                    if slot.employee_id:
                        work_hours = slot.employee_id._get_work_days_data_batch(
                            slot.start_datetime, slot.end_datetime, compute_leaves=True
                        )[slot.employee_id.id]["hours"]
                        slot.allocated_percentage = (
                            100 * slot.allocated_hours / work_hours
                            if work_hours
                            else 100
                        )
                    else:
    ...
    def _compute_allocated_hours(self):
        ...
        if forecast_slots:
            ...
            start_utc = pytz.utc.localize(
                min(planned_forecast_slots.mapped("start_datetime"))
            )
            end_utc = pytz.utc.localize(
                max(planned_forecast_slots.mapped("end_datetime"))
            )
            # work intervals per resource are retrieved with a batch
            work_intervals_per_resource = (
                forecast_slots.resource_id._get_work_intervals_batch(start_utc, end_utc)
            )
            for slot in planned_forecast_slots:
                ...
                if not slot.resource_id:
                    ...
                    work_intervals = (
                        interval & work_intervals_per_resource[slot.resource_id.id]
                    )
    ...
    def _search_overlap_slot_count(self, operator, value):
        if operator not in ["=", ">"] or not isinstance(value, int) or value != 0:
            raise NotImplementedError(
                _(
                    "Operation not supported, you should always compare overlap_slot_count to 0 value with = or > operator."
                )
            )
    ...
    def _get_domain_template_slots(self):
        ...
        if self.resource_type == "material":
            ...
            domain += [
                "|",
                ("role_id", "in", self.employee_id.sudo().planning_role_ids.ids),
                ("role_id", "=", False),
            ]
        return domain
    ...
    def _compute_template_autocomplete_ids(self):
        ...
        templates = self.env["planning.slot.template"].search(
            domain, order="start_time", limit=10
        )
        self.template_autocomplete_ids = templates + self.template_id

    @api.depends(
        "employee_id", "role_id", "start_datetime", "end_datetime", "allocated_hours"
    )
        ...
        for template_field, slot_field in template_fields:
            if self.template_id[template_field] or not check_empty:
                if template_field == "start_time":
                    ...
                    slot_time = self[slot_field].astimezone(
                        pytz.timezone(self._get_tz())
                    )
    ...
    @api.depends(
        "template_id", "role_id", "allocated_hours", "start_datetime", "end_datetime"
    )
    def _compute_allow_template_creation(self):
        for slot in self:
            ...
            existing_templates = self.env["planning.slot.template"].search(
                domain, limit=1
            )
            slot.allow_template_creation = (
                not existing_templates
                and slot._different_than_template(check_empty=False)
            )
    ...
    def _inverse_repeat(self):
        for slot in self:
            if slot.repeat and not slot.recurrency_id.id:  # create the recurrence
                ...
                if slot.repeat_type == "until":
                    repeat_until = datetime.combine(
                        fields.Date.to_date(slot.repeat_until), datetime.max.time()
                    )
                    repeat_until = (
                        repeat_until.replace(
                            tzinfo=pytz.timezone(
                                slot.company_id.resource_calendar_id.tz or "UTC"
                            )
                        )
                        .astimezone(pytz.utc)
                        .replace(tzinfo=None)
            ...
            # here we also check that we don't delete by mistake a slot of which the repeat parameters have been changed
                ...
                not slot.repeat
                and slot.recurrency_id.id
                and (
                    slot.repeat_type == slot.recurrency_id.repeat_type
                    and slot.repeat_until == slot.recurrency_id.repeat_until
                    and slot.repeat_interval == slot.recurrency_id.repeat_interval
                )
            ):
    ...
    def _inverse_template_creation(self):
        ...
        for slot in self.filtered(lambda s: s.template_creation):
            ...
            if not existing_templates:
                ...
                slot.write(
                    {"template_id": template.id, "previous_template_id": template.id}
                )
            else:
                slot.write({"template_id": existing_templates.id})

    @api.model
    def _calculate_start_end_dates(
        self,
        start_datetime,
        end_datetime,
        resource_id,
        ...
        employee = (
            resource_id.employee_id if resource_id.resource_type == "user" else False
        )
        ...
        start = (
            convert_datetime_timezone(start_datetime, user_tz)
            if start_datetime
            else user_tz.localize(self._default_start_datetime())
        )
        end = (
            convert_datetime_timezone(end_datetime, user_tz)
            if end_datetime
            else user_tz.localize(self._default_end_datetime())
        )
        ...
        if resource:
            work_interval_start, work_interval_end = resource._adjust_to_calendar(
                start.replace(tzinfo=pytz.timezone(resource.tz)),
                end.replace(tzinfo=pytz.timezone(resource.tz)),
            )[resource]
        ...
        if template_id and start_datetime:
            ...
            start = pytz.utc.localize(start_datetime).astimezone(
                pytz.timezone(resource.tz) if resource else user_tz
            )
    ...
    def _compute_datetime(self):
        for slot in self.filtered(lambda s: s.template_id):
            slot.start_datetime, slot.end_datetime = self._calculate_start_end_dates(
                slot.start_datetime,
                slot.end_datetime,
                slot.resource_id,
                slot.template_id,
    ...
    def _compute_unassign_deadline(self):
        ...
        for slot in slots_with_date:
            slot.unassign_deadline = fields.Datetime.subtract(
                slot.start_datetime, days=slot.self_unassign_days_before
            )
    ...
    def _compute_is_unassign_deadline_passed(self):
        ...
        for slot in slots_with_date:
            slot.is_unassign_deadline_passed = (
                slot.unassign_deadline < fields.Datetime.now()
            )
    ...
    def read_group(
        ...
        null_fields = [
            f
            for f in self._read_group_fields_nullify()
            if any(f2.startswith(f) for f2 in fields)
        ]
        if null_fields:
            for r in res:
    ...
    def default_get(self, fields_list):
        ...
        if res.get("resource_id"):
            ...
            template_id, previous_template_id = [
                res.get(key) for key in ["template_id", "previous_template_id"]
            ]
            template_id = template_id and self.env["planning.slot.template"].browse(
                template_id
            )
            previous_template_id = template_id and self.env[
                "planning.slot.template"
            ].browse(previous_template_id)
            (
                res["start_datetime"],
                res["end_datetime"],
            ) = self._calculate_start_end_dates(
                res.get("start_datetime"),
            ...
            if "start_datetime" in fields_list:
                start_datetime = (
                    fields.Datetime.from_string(res.get("start_datetime"))
                    if res.get("start_datetime")
                    else self._default_start_datetime()
                )
                end_datetime = (
                    fields.Datetime.from_string(res.get("end_datetime"))
                    if res.get("end_datetime")
                    else self._default_end_datetime()
                )
    ...
    def name_get(self):
        ...
        for slot in self:
            ...
            name_values = [
                ...
                if fname != "resource_id"
                else slot.resource_id.name
    ...
    def write(self, values):
        ...
        if any(
            fname in values.keys() for fname in self._get_fields_breaking_recurrency()
        ) and not values.get("recurrency_id"):
            values.update({"recurrency_id": False})
        # warning on published shifts
        if "publication_warning" not in values and (
            set(values.keys()) & set(self._get_fields_breaking_publication())
        ):
            values["publication_warning"] = True
        result = super(Planning, self).write(values)
        # recurrence
        if any(
            key in ("repeat", "repeat_type", "repeat_until", "repeat_interval")
            for key in values
        ):
            ...
            for slot in self:
                if slot.recurrency_id and values.get("repeat") is None:
                    repeat_type = (
                        values.get("repeat_type") or slot.recurrency_id.repeat_type
                    )
                    repeat_until = (
                        values.get("repeat_until") or slot.recurrency_id.repeat_until
                    )
                    if repeat_type == "until":
                        repeat_until = datetime.combine(
                            fields.Date.to_date(repeat_until), datetime.max.time()
                        )
                        repeat_until = (
                            repeat_until.replace(
                                tzinfo=pytz.timezone(
                    ...
                    slot.recurrency_id._delete_slot(
                        recurrency_values.get("repeat_until")
                    )
                    slot.recurrency_id._repeat_slot()
        return result
    ...
    def action_see_overlaping_slots(self):
        ...
        return {
            ...
            "context": {
                "initialDate": min(
                    [slot.start_datetime for slot in self.search(domain_map[self.id])]
                )
            },
        }
    ...
    def action_self_assign(self):
        ...
        if self.resource_id:
            raise UserError(
                _("You can not assign yourself to an already assigned shift.")
            )
        return self.sudo().write(
            {
                "resource_id": self.env.user.employee_id.resource_id.id
    ...
    def gantt_unavailability(
        ...
        leaves_mapping = resources._get_unavailable_intervals(
        ...
        cell_dt = (
            timedelta(hours=1) if scale in ["day", "week"] else timedelta(hours=12)
        )
        ...
        def inject_unavailability(row):
            ...
            if row.get("resource_id"):
                resource_id = self.env["resource.resource"].browse(
                    row.get("resource_id")
                )
                if resource_id:
            ...
            notable_intervals = filter(
                lambda interval: interval[1] - interval[0] >= cell_dt, calendar
            )
    ...
    def action_copy_previous_week(self, date_start_week, view_domain):
        date_end_copy = datetime.strptime(
            date_start_week, DEFAULT_SERVER_DATETIME_FORMAT
        )
        date_start_copy = date_end_copy - relativedelta(days=7)
        domain = [("recurrency_id", "=", False), ("was_copied", "=", False)]
        ...
        new_slot_values = []
        new_slot_values = slots_to_copy._copy_slots(
            date_start_copy, date_end_copy, relativedelta(days=7)
        )
    ...
    def get_employees_without_work_email(self):
        ...
        employee_ids_without_work_email = employees.filtered(
            lambda employee: not employee.work_email
        ).ids
                ...
                domain = expression.AND(
                    [
                        ...
                        [
                            ...
                            ("planning_role_ids", "in", self.role_id.id),
                        ],
                    ]
                )
    ...
    def _get_half_day_datetime(self, dt, end=False):
        ...
        return (
            midday
            if midday < localized_dt
            else localized_dt.replace(hour=0, minute=0, second=0)
        )
    ...
    def _update_remaining_hours_to_plan_and_values(
        self, remaining_hours_to_plan, values
    ):
        """
    ...
    def _get_split_slot_values(
        self, values, intervals, remaining_hours_to_plan, unassign=False
    ):
        """
            ...
            new_slot_vals = {
                **values,
                ...
                "allocated_hours": float_utils.float_round(
                    (
                        ((end_inter - start_inter).total_seconds() / 3600.0)
                        * (self.allocated_percentage / 100.0)
                    ),
                    precision_digits=2,
                ),
            }
            if not self._update_remaining_hours_to_plan_and_values(
                remaining_hours_to_plan, new_slot_vals
            ):
    ...
    def _copy_slots(self, start_dt, end_dt, delta):
        ...
        attendance_intervals_per_resource = defaultdict(
            Intervals
        )  # key: resource, values: attendance intervals
        unavailable_intervals_per_resource = defaultdict(
            Intervals
        )  # key: resource, values: unavailable intervals
        attendance_intervals_per_calendar = defaultdict(
            Intervals
        )  # key: calendar, values: attendance intervals (used for company calendars)
        leave_intervals_per_calendar = defaultdict(
            Intervals
        )  # key: calendar, values: leave intervals (used for company calendars)
        new_slot_values = []
        # date utils variable
        ...
        resource_calendar_validity_intervals = (
            self.resource_id.sudo()._get_calendars_validity_within_period(
                start_dt_delta_utc, end_dt_delta_utc
            )
        )
        for slot in self:
            if slot.resource_id:
                for calendar in resource_calendar_validity_intervals[
                    slot.resource_id.id
                ].keys():
                    resource_per_calendar[calendar] |= slot.resource_id
            company_calendar_id = slot.company_id.resource_calendar_id
            resource_per_calendar[company_calendar_id] |= self.env[
                "resource.resource"
            ]  # ensures the company_calendar will be in resource_per_calendar keys.
        # 3)
        ...
        for slot in self:
            ...
            if not values.get("start_datetime") or not values.get("end_datetime"):
                continue
            values["start_datetime"] = slot._add_delta_with_dst(
                values["start_datetime"], delta
            )
            values["end_datetime"] = slot._add_delta_with_dst(
                values["end_datetime"], delta
            )
            interval = Intervals(
                [
                    (
            ...
            attendance_resource = (
                attendance_intervals_per_resource[slot.resource_id]
                if slot.resource_id
                else attendance_intervals_per_calendar[company_calendar]
            )
            ...
            attendance_interval_company = (
                interval & attendance_intervals_per_calendar[company_calendar]
            )
            # Check if interval is contained in the company leaves interval
            unavailable_interval_company = (
                interval & leave_intervals_per_calendar[company_calendar]
            )
            if (
                slot.allocation_type == "planning"
                and not unavailable_interval_company
                and not attendance_interval_resource
            ):
                ...
                interval = slot._get_half_day_interval(
                    values
                )  # Get the afternoon and/or the morning whole interval where the planning slot exists.
                attendance_interval_resource = interval & attendance_resource
                attendance_interval_company = (
                    interval & attendance_intervals_per_calendar[company_calendar]
                )
            ...
            unavailable_interval_resource = (
                unavailable_interval_company
                if not slot.resource_id
                else (interval & unavailable_intervals_per_resource[slot.resource_id])
            )
            if (attendance_interval_resource - unavailable_interval_company) or (
                ...
                if slot.allocation_type == "planning":
                    ...
                    if (
                        unavailable_interval_resource
                        or not attendance_interval_resource
                    ):
                    ...
                    if not slot._update_remaining_hours_to_plan_and_values(
                        remaining_hours_to_plan, values
                    ):
                    ...
                    if attendance_interval_resource:
                        ...
                        work_interval_employee = (
                            attendance_interval_resource - unavailable_interval_resource
                        )
                        unassigned_interval = (
                            unavailable_interval_resource - unavailable_interval_company
                        )
                        split_slot_values = slot._get_split_slot_values(
                            values, work_interval_employee, remaining_hours_to_plan
                        )
                        if slot.resource_type == "user":
                            split_slot_values += slot._get_split_slot_values(
                                values,
                    ...
                    new_slot_values += self._merge_slots_values(
                        split_slot_values, unassigned_interval
                    )
    ...
    def _get_overlap_domain(self):
        ...
        for slot in self:
            ...
            domain_mapping[slot.id] = [
                ("id", "in", slot.conflicting_slot_ids.ids + [slot.id])
            ]
    ...
    def _prepare_template_values(self):
        ...
        start_datetime = pytz.utc.localize(self.start_datetime).astimezone(
            destination_tz
        )
    ...
    def _read_group_resource_id(self, resources, domain, order):
        ...
        if resource_ids:
            return self.env["resource.resource"].search(
                [("id", "in", resource_ids)], order=order
            )
        if (
            self.env.context.get("planning_expand_resource")
            and ("start_datetime", "<=") in dom_tuples
            and ("end_datetime", ">=") in dom_tuples
        ):
            if ("resource_id", "=") in dom_tuples or (
                "resource_id",
                "ilike",
            ) in dom_tuples:
    ...
    def _read_group_role_id(self, roles, domain, order):
        ...
        if (
            ...
            filters = expression.AND(
                [[("role_id.active", "=", True)], self._expand_domain_dates(domain)]
            )
        ...
        for dom in domain:
            if len(dom) == 3 and dom[0] == "start_datetime" and dom[1] == "<=":
                ...
                max_date = (
                    ...
                    else datetime.strptime(max_date, "%Y-%m-%d %H:%M:%S")
                )
                ...
                min_date = (
                    ...
                    else datetime.strptime(min_date, "%Y-%m-%d %H:%M:%S")
                )
    ...
    @api.model
    def _format_datetime_to_user_tz(
        self, datetime_without_tz, record_env, tz=None, lang_code=False
    ):
    ...
    def _send_slot(
        ...
        employee_with_backend = employee_ids.filtered(
            lambda e: e.user_id and e.user_id.has_group("planning.group_planning_user")
        )
        ...
        employee_url_map = {
            **employee_without_backend.sudo()._planning_get_url(planning),
            **employee_with_backend._slot_get_url(self),
        }
        ...
        if self.employee_id:
            ...
            if self.allow_self_unassign:
                if employee_ids.filtered(
                    lambda e: e.user_id
                    and e.user_id.has_group("planning.group_planning_user")
                ):
                    unavailable_link = "/planning/unassign/%s/%s" % (
                        self.employee_id.sudo().employee_token,
                        self.id,
                    )
                else:
                    unavailable_link = "/planning/%s/%s/unassign/%s?message=1" % (
        ...
        for employee in employee_ids.filtered(lambda e: e.work_email):
            if not self.employee_id and employee in employee_with_backend:
                view_context.update(
                    {
                        "available_link": "/planning/assign/%s/%s"
                        % (employee.sudo().employee_token, self.id)
                    }
                )
            elif not self.employee_id:
                view_context.update(
                    {
                        "available_link": "/planning/%s/%s/assign/%s?message=1"
                        % (
                            planning.access_token,
                            employee.sudo().employee_token,
                            self.id,
                        )
                    }
                )
            start_datetime = self._format_datetime_to_user_tz(
                self.start_datetime,
                employee.env,
                tz=employee.tz,
                lang_code=employee.user_partner_id.lang,
            )
            end_datetime = self._format_datetime_to_user_tz(
                self.end_datetime,
                employee.env,
                tz=employee.tz,
                lang_code=employee.user_partner_id.lang,
            )
            unassign_deadline = self._format_datetime_to_user_tz(
                self.unassign_deadline,
                employee.env,
                tz=employee.tz,
                lang_code=employee.user_partner_id.lang,
            )
...
class PlanningPlanning(models.Model):
    ...
    self_unassign_days_before = fields.Integer(
        "Days before shift for unassignment",
        related="company_id.planning_self_unassign_days_before",
        help="Deadline in days for shift unassignment",
    )
    ...
    def _compute_dates(self):
        ...
        for planning in self:
            planning.date_start = (
                pytz.utc.localize(planning.start_datetime)
                .astimezone(tz)
                .replace(tzinfo=None)
            )
            planning.date_end = (
                pytz.utc.localize(planning.end_datetime)
                .astimezone(tz)
                .replace(tzinfo=None)
            )
    ...
    def _send_planning(self, message=None, employees=False):
        ...
        for planning in self:
            ...
            slots_open = (
                slots.filtered(lambda slot: not slot.resource_id)
                if planning.include_unassigned
                else 0
            )
            ...
            template = self.env.ref(
                "planning.email_template_planning_planning", raise_if_not_found=False
            )
            template_context = {
                ...
                for employee in self.env["hr.employee.public"].browse(employees.ids):
                    if employee.work_email:
                        ...
                        template_context["assigned_new_shift"] = bool(
                            slots.filtered(
                                lambda slot: slot.employee_id.id == employee.id
                            )
                        )
            ...
            sent_slots |= slots
        # mark as sent
        sent_slots.write({"state": "published", "publication_warning": False})
        return True
