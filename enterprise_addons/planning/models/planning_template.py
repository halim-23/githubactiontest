...
class PlanningTemplate(models.Model):
    ...
    _sql_constraints = [
        (
            "check_start_time_lower_than_24",
            "CHECK(start_time <= 24)",
            "You cannot have a start hour greater than 24",
        ),
        (
            "check_start_time_positive",
            "CHECK(start_time >= 0)",
            "Start hour must be a positive number",
        ),
        (
    ...
    def _compute_name(self):
        for shift_template in self:
            ...
            duration = timedelta(
                hours=int(shift_template.duration),
                minutes=round(math.modf(shift_template.duration)[0] / (1 / 60.0)),
            )
            ...
            shift_template.name = "%s - %s %s" % (
                ...
                format_time(
                    shift_template.env, end_time.time(), time_format="short"
                ).replace(":00 ", " "),
    ...
    def read_group(
        ...
        res = []
        for data in super(PlanningTemplate, self).read_group(
