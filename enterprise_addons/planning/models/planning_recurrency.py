...
class PlanningRecurrency(models.Model):
    ...
    slot_ids = fields.One2many(
        "planning.slot", "recurrency_id", string="Related Planning Entries"
    )
    repeat_interval = fields.Integer("Repeat Every", default=1, required=True)
    repeat_type = fields.Selection(
        [("forever", "Forever"), ("until", "Until")], string="Weeks", default="forever"
    )
    repeat_until = fields.Datetime(
        string="Repeat Until", help="Up to which date should the plannings be repeated"
    )
    ...
    _sql_constraints = [
        (
            "check_repeat_interval_positive",
            "CHECK(repeat_interval >= 1)",
            "Recurrency repeat interval should be at least 1",
        ),
        (
            "check_until_limit",
    ...
    def _check_multi_company(self):
        for recurrency in self:
            if any(
                ...
                for planning in recurrency.slot_ids
            ):
    ...
    def name_get(self):
        ...
        for recurrency in self:
            if recurrency.repeat_type == "forever":
                ...
                name = _("Every %s week(s) until %s") % (
                    recurrency.repeat_interval,
