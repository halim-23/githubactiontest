...
class Employee(models.Model):
    ...
    default_planning_role_id = fields.Many2one(
        "planning.role", string="Default Planning Role", groups="hr.group_hr_user"
    )
    ...
    employee_token = fields.Char(
        "Security Token",
        default=_default_employee_token,
        copy=False,
        groups="hr.group_hr_user",
        readonly=True,
    )
    ...
    def _init_column(self, column_name):
        ...
        if column_name == "employee_token":
            ...
            self.env.cr.execute(
                "SELECT id FROM %s WHERE employee_token IS NULL" % self._table
            )
            acc_ids = self.env.cr.dictfetchall()
            query_list = [
                {"id": acc_id["id"], "employee_token": self._default_employee_token()}
                for acc_id in acc_ids
            ]
            query = (
                "UPDATE "
                + self._table
                + " SET employee_token = %(employee_token)s WHERE id = %(id)s;"
            )
    ...
    def _planning_get_url(self, planning):
        ...
        for employee in self:
            if employee.user_id and employee.user_id.has_group(
                "planning.group_planning_user"
            ):
                result[employee.id] = (
                    "/web?date_start=%s&date_end=%s#action=planning.planning_action_open_shift&menu_id="
                    % (planning.date_start, planning.date_end)
                )
            else:
                result[employee.id] = "/planning/%s/%s" % (
    ...
    def _slot_get_url(self, slot):
        ...
        link = (
            "/web?date_start=%s&date_end=%s#action=%s&model=planning.slot&menu_id=%s&db=%s"
            % (start_date, end_date, action_id, menu_id, dbname[0])
        )
    ...
    def _compute_planning_role_ids(self):
        ...
        for employee in self.filtered(lambda s: s.default_planning_role_id):
            if (
                employee.default_planning_role_id
                and employee.default_planning_role_id not in employee.planning_role_ids
            ):
