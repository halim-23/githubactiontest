...
class ResourceResource(models.Model):
    ...
    def get_formview_id(self, access_uid=None):
        if self.env.context.get("from_planning"):
            return self.env.ref(
                "planning.resource_resource_with_employee_form_view_inherit",
    ...
    def create(self, vals_list):
        ...
        if self.env.context.get("from_planning"):
            ...
            self.env["hr.employee"].sudo().with_context(from_planning=False).create(
                create_vals
            )
        return resources
        ...
        if self.env.context.get("planning_gantt_view"):
            for resource in self:
                ...
                if employee.job_title:
                    result.append(
                        (resource.id, "%s (%s)" % (employee.name, employee.job_title))
                    )
                else:
    ...
    def get_planning_hours_info(self, date_start_str, date_stop_str):
        ...
        work_intervals_batch = self.sudo()._get_work_intervals_batch(
            start_utc, stop_utc
        )
        for slot in planning_slots:
            if slot.start_datetime >= date_start and slot.end_datetime <= date_stop:
                ...
                if slot.allocation_type == "planning":
                    ...
                    work_intervals = (
                        interval & work_intervals_batch[slot.resource_id.id]
                    )
                    planned_hours_mapped[slot.resource_id.id] += (
                        sum(
    ...
    def _get_calendars_validity_within_period(self, start, end, default_company=None):
        ...
        if not self:
            ...
            calendars_within_period_per_resource[False][default_calendar] = Intervals(
                [(start, end, self.env["resource.calendar.attendance"])]
            )
        for resource in self:
            calendar = (
            ...
            calendars_within_period_per_resource[resource.id][calendar] = Intervals(
                [(start, end, self.env["resource.calendar.attendance"])]
            )
    ...
    def _get_work_intervals_batch(self, start, end):
        ...
        resource_calendar_validity_intervals = (
            self._get_calendars_validity_within_period(start, end)
        )
        ...
        for calendar in resource_per_calendar.keys():
            ...
            work_intervals_batch = calendar._work_intervals_batch(
                start, end, resources=resource_per_calendar[calendar]
            )
            for resource_id in resource_per_calendar[calendar]:
                # Make the conjonction between work intervals and calendar validity
                work_intervals_per_resource[resource_id.id] |= (
                    work_intervals_batch[resource_id.id]
                    & resource_calendar_validity_intervals[resource_id.id][calendar]
                )
