  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("render basic hirarchy kanban", async function (assert) {
        ...
        var $parentRecords = form.$(
          ".o_ma_hierarchy_container .o_kanban_view > .o_kanban_record"
        );
        assert.strictEqual($parentRecords.length, 2, "There should be 2 parent");
        assert.containsOnce(
          $($parentRecords[0]),
          "> .o_hierarchy_children > .o_kanban_record",
          "First parent should have 1 child"
        );
        assert.containsN(
          $($parentRecords[1]),
          "> .o_hierarchy_children > .o_kanban_record",
          2,
          "Second parent should have 2 child"
        );
        assert.containsOnce(
          $($parentRecords[1]),
          ".o_hierarchy_children .o_hierarchy_children > .o_kanban_record",
          "2nd parent's 2nd Child should have 1 child"
        );

        // Checking titles of kanban to verify proper values
        assert.strictEqual(
          $($parentRecords[0]).find("> .o_title").text(),
          "Parent 1",
          "Title of 1st parent"
        );
        assert.strictEqual(
          $($parentRecords[1]).find("> .o_title").text(),
          "Parent 2",
          "Title of 1st parent"
        );
        assert.strictEqual(
          $($parentRecords[0])
            .find("> .o_hierarchy_children > .o_kanban_record > .o_title")
            .text(),
          "Parent 1 > Child 1",
        ...
        assert.strictEqual(
          $($parentRecords[1])
            .find("> .o_hierarchy_children > .o_kanban_record:first > .o_title")
            .text(),
          "Parent 2 > Child 1",
        ...
        assert.strictEqual(
          $($parentRecords[1])
            .find("> .o_hierarchy_children > .o_kanban_record:last > .o_title")
            .text(),
          "Parent 2 > Child 2",
        ...
        assert.strictEqual(
          $($parentRecords[1])
            .find(
              "> .o_hierarchy_children .o_hierarchy_children > .o_kanban_record:last > .o_title"
            )
            .text(),
          "Parent 2 > Child 2 > Child 1",
          "Title of 2ed parent's 2ed child's 1st child"
        );
        form.destroy();
      });
    }
