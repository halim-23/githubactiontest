...
class MarketingCampaign(models.Model):
    ...
    utm_campaign_id = fields.Many2one(
        "utm.campaign", "UTM Campaign", ondelete="cascade", required=True
    )
    ...
    unique_field_id = fields.Many2one(
        "ir.model.fields",
        string="Unique Field",
        compute="_compute_unique_field_id",
        readonly=False,
        store=True,
    ...
    marketing_activity_ids = fields.One2many(
        "marketing.activity", "campaign_id", string="Activities", copy=False
    )
    mass_mailing_count = fields.Integer(
        "# Mailings", compute="_compute_mass_mailing_count"
    )
    link_tracker_click_count = fields.Integer(
        "# Clicks", compute="_compute_link_tracker_click_count"
    )
    last_sync_date = fields.Datetime(string="Last activities synchronization")
    require_sync = fields.Boolean(
        string="Sync of participants is required", compute="_compute_require_sync"
    )
    # participants
    participant_ids = fields.One2many(
        "marketing.participant", "campaign_id", string="Participants", copy=False
    )
    running_participant_count = fields.Integer(
        string="# of active participants", compute="_compute_participants"
    )
    completed_participant_count = fields.Integer(
        string="# of completed participants", compute="_compute_participants"
    )
    total_participant_count = fields.Integer(
        string="# of active and completed participants", compute="_compute_participants"
    )
    test_participant_count = fields.Integer(
        string="# of test participants", compute="_compute_participants"
    )
    ...
    def _compute_require_sync(self):
        for campaign in self:
            if campaign.last_sync_date and campaign.state == "running":
                activities_changed = campaign.marketing_activity_ids.filtered(
    ...
    def _compute_mass_mailing_count(self):
        ...
        for campaign in self:
            campaign.mass_mailing_count = len(
                campaign.mapped("marketing_activity_ids.mass_mailing_id").filtered(
                    lambda mailing: mailing.mailing_type == "mail"
                )
            )
    ...
    def _compute_link_tracker_click_count(self):
            ...
            .read_group(
                [
                    (
                        "mass_mailing_id",
                        "in",
                        self.mapped("marketing_activity_ids.mass_mailing_id").ids,
                    )
                ],
                ["mass_mailing_id", "ip"],
                ["mass_mailing_id"],
        ...
        mapped_data = {
            data["mass_mailing_id"][0]: data["mass_mailing_id_count"]
            for data in click_data
        }
        for campaign in self:
            campaign.link_tracker_click_count = sum(
                mapped_data.get(mailing_id, 0)
                for mailing_id in campaign.mapped(
                    "marketing_activity_ids.mass_mailing_id"
                ).ids
            )
    ...
    def _compute_participants(self):
        ...
        for campaign in self:
            ...
            campaign.total_participant_count = (
                ...
                + campaign.running_participant_count
            )
        ...
        if any(campaign.marketing_activity_ids for campaign in self):
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "Switching Target Model invalidates the existing activities. "
        ...
        for campaign in self:
            ...
            for trace in traces_to_reschedule:
                trace_offset = relativedelta(
                    **{
                        trace.activity_id.interval_type: trace.activity_id.interval_number
                    }
                )
                trigger_type = trace.activity_id.trigger_type
                if trigger_type == "begin":
                    trace.schedule_date = (
                        Datetime.from_string(trace.participant_id.create_date)
                        + trace_offset
                    )
                elif (
                    trigger_type
                    in ["activity", "mail_not_open", "mail_not_click", "mail_not_reply"]
                    and trace.parent_id
                ):
                    trace.schedule_date = (
                        Datetime.from_string(trace.parent_id.schedule_date)
                        + trace_offset
                    )
                elif trace.parent_id:
                    process_dt = trace.parent_id.mailing_trace_ids.state_update
                    trace.schedule_date = (
                        Datetime.from_string(process_dt) + trace_offset
                    )
            ...
            for activity in created_activities:
                ...
                if activity.trigger_type == "begin":
                    ...
                    for participant in participants:
                        schedule_date = (
                            Datetime.from_string(Datetime.now()) + activity_offset
                        )
                    ...
                    if activity.trigger_type in [
                        ...
                        valid_parent_traces = (
                            valid_parent_traces - brother_traces.mapped("parent_id")
                        )
    ...
    def action_start_campaign(self):
        if any(not campaign.marketing_activity_ids for campaign in self):
            raise ValidationError(
                _("You must set up at least one activity to start this campaign.")
            )
    ...
    def action_view_mailings(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "marketing_automation.mail_mass_mailing_action_marketing_automation"
        )
        action["domain"] = [
            "&",
    ...
    def action_view_tracker_statistics(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "marketing_automation.link_tracker_action_marketing_campaign"
        )
        action["domain"] = [
            (
                "mass_mailing_id",
    ...
    def sync_participants(self):
        ...
        for campaign in self.filtered(lambda c: c.marketing_activity_ids):
            ...
            existing_rec_ids = _uniquify_list(
                [live_participant["res_id"] for live_participant in participants_data]
            )
