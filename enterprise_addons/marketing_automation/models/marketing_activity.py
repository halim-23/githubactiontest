...
class MarketingActivity(models.Model):
    ...
    activity_type = fields.Selection(
        [("email", "Email"), ("action", "Server Action")],
    ...
    mass_mailing_id = fields.Many2one(
        ...
        compute="_compute_mass_mailing_id",
        readonly=False,
        store=True,
    )
    mass_mailing_id_mailing_type = fields.Selection(
        [("mail", "Email")],
        string="Mailing Type",
        compute="_compute_mass_mailing_id_mailing_type",
        readonly=True,
        store=True,
        help="Technical field doing the mapping of activity type and mailing type.",
    )
    ...
    interval_standardized = fields.Integer(
        "Send after (in hours)",
        compute="_compute_interval_standardized",
        store=True,
        readonly=True,
    )
    # validity
    validity_duration = fields.Boolean(
        "Validity Duration",
    ...
    domain = fields.Char(
        ...
        compute="_compute_inherited_domain",
        recursive=True,
        store=True,
        readonly=True,
    )
    ...
    model_id = fields.Many2one(
        "ir.model", related="campaign_id.model_id", string="Model", readonly=True
    )
    ...
    allowed_parent_ids = fields.Many2many(
        ...
        help="All activities which can be the parent of this one",
        compute="_compute_allowed_parent_ids",
    )
    child_ids = fields.One2many(
        "marketing.activity", "parent_id", string="Child Activities"
    )
    ...
    trigger_category = fields.Selection(
        [("email", "Mail")], compute="_compute_trigger_category"
    )
    # cron / updates
    require_sync = fields.Boolean("Require trace sync", copy=False)
    # For trace
    trace_ids = fields.One2many(
        "marketing.trace", "activity_id", string="Traces", copy=False
    )
    ...
    def _check_consistency_in_activities(self):
        ...
        for activity in self:
            if (
                activity.parent_id or activity.allowed_parent_ids
            ) and activity.parent_id not in activity.allowed_parent_ids:
                trigger_string = dict(
                    activity._fields["trigger_type"]._description_selection(self.env)
                ...
                raise ValidationError(
                    ...
                    % (
                        activity.parent_id.name,
                        activity.parent_id.activity_type,
                        activity.name,
                        trigger_string,
                    )
                )
    ...
    def _compute_mass_mailing_id(self):
        for activity in self:
            if (
                activity.mass_mailing_id_mailing_type
                != activity.mass_mailing_id.mailing_type
            ):
    ...
    def _compute_inherited_domain(self):
        for activity in self:
            ...
            while ancestor:
                domain = expression.AND(
                    [domain, literal_eval(ancestor.activity_domain)]
                )
    ...
    def _compute_interval_standardized(self):
        factors = {"hours": 1, "days": 24, "weeks": 168, "months": 720}
        for activity in self:
    ...
    def _compute_parent_id(self):
        for activity in self:
            if not activity.parent_id or (
                activity.parent_id and activity.trigger_type == "begin"
            ):
    ...
    def _compute_statistics_graph_data(self):
        if not self.ids:
            ...
            date_range.reverse()
            default_values = [
                {"x": date_item.strftime("%d %b"), "y": 0} for date_item in date_range
            ]
            self.statistics_graph_data = json.dumps(
                [
                    {
                        "points": default_values,
                        "label": _("Success"),
                        "color": "#21B799",
                    },
                    {
    ...
    def _check_parent_id(self):
        if not self._check_recursion():
            raise ValidationError(
                _("Error! You can't create recursive hierarchy of Activity.")
            )
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            values["require_sync"] = (
                self.env["marketing.campaign"].browse(campaign_id).state == "running"
            )
        return super().create(vals_list)

    def write(self, values):
        if any(
            field in values.keys() for field in ("interval_number", "interval_type")
        ):
    ...
    def _get_graph_statistics(self):
        ...
        past_date = (
            Datetime.from_string(Datetime.now()) + timedelta(days=-14)
        ...
        for activity in self:
            ...
            for i in date_range:
                ...
                rejected.append(
                    {"x": x, "y": stat_map.get((activity._origin.id, i, "rejected"), 0)}
                )
    ...
    def execute(self, domain=None):
        ...
        for activity, traces in trace_to_activities.items():
            for traces_batch in (
                traces[i : i + BATCH_SIZE] for i in range(0, len(traces), BATCH_SIZE)
            ):
    ...
    def execute_on_traces(self, traces):
        ...
        if self.domain:
            rec_domain = expression.AND(
                [literal_eval(self.campaign_id.domain), literal_eval(self.domain)]
            )
        else:
        ...
        if rec_domain:
            ...
            traces_rejected = traces.filtered(
                lambda trace: trace.res_id not in rec_ids_domain and not trace.is_test
    ...
    def _execute_action(self, traces):
        ...
        for trace in traces:
            ...
            try:
                ...
                _logger.warning(
                    "Marketing Automation: activity <%s> encountered server action issue %s",
                    self.id,
                    str(e),
                    exc_info=True,
                )
    ...
    def _execute_email(self, traces):
        ...
        if not self.env.is_superuser() and not self.user_has_groups(
            ...
            raise AccessError(
                _(
                    "To use this feature you should be an administrator or belong to the marketing automation group."
                )
            )
        try:
            ...
            error_doc_ids = [
                ...
                if stat.trace_status in ("error", "bounce")
            ]
            cancel_doc_ids = [
    ...
    def _generate_children_traces(self, traces):
        ...
        for activity in self.child_ids:
            ...
            for trace in traces:
                ...
                if activity.trigger_type in [
                    ...
                    schedule_date = (
                        Datetime.from_string(trace.schedule_date) + activity_offset
                    )
        ...
        if cron_trigger_dates:
            ...
            cron = self.env.ref(
                "marketing_automation.ir_cron_campaign_execute_activities"
            )
    ...
    def _action_view_documents_filtered(self, view_filter):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "marketing_automation.marketing_participants_action_mail"
        )

        if view_filter in ("reply", "bounce"):
            found_traces = self.trace_ids.filtered(
                lambda trace: trace.mailing_trace_status == view_filter
            )
        elif view_filter == "sent":
            found_traces = self.trace_ids.filtered(
                lambda trace: trace.mailing_trace_ids.sent_datetime
            )
        elif view_filter == "click":
            found_traces = self.trace_ids.filtered(
                lambda trace: trace.mailing_trace_ids.links_click_datetime
            )
        else:
