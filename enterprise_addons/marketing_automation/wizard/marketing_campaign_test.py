...
class MarketingCampaignTest(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if "res_id" in default_fields and not defaults.get("res_id"):
            ...
            if not model_name and defaults.get("campaign_id"):
                model_name = (
                    self.env["marketing.campaign"]
                    .browse(defaults["campaign_id"])
                    .model_name
                )
            if model_name:
    ...
    model_id = fields.Many2one(
        "ir.model", string="Model", related="campaign_id.model_id", readonly=True
    )
    model_name = fields.Char(
        "Record model", related="campaign_id.model_id.model", readonly=True
    )
