...
class TestFsmFlowStock(TestFsmFlowSaleCommon):
    ...
    def test_fsm_flow(self):
        ...
        self.assertNotEqual(
            ...
            self.lot_id2,
            "Lot automatically added on move lines is not the same as asked. (By default, it's the first lot available)",
        )
        self.task.with_user(self.project_user).action_fsm_validate()
        self.assertEqual(
            move.move_line_ids.lot_id,
            self.lot_id2,
            "Asked lots are added on move lines.",
        )
        self.assertEqual(
            move.move_line_ids.qty_done,
            3,
            "We deliver 3 (even they are only 2 in stock)",
        )

        self.assertEqual(
            self.task.sale_order_id.picking_ids.mapped("state"),
            ["done", "done", "done"],
    ...
    def test_fsm_flow_with_default_warehouses(self):
        ...
        self.assertEqual(
            self.project_user.property_warehouse_id.id,
            self.task.sale_order_id.warehouse_id.id,
        )
    ...
    def test_fsm_stock_already_validated_picking(self):
        ...
        wizard = self.product_lot.with_context(
            {"fsm_task_id": self.task.id}
        ).action_assign_serial()
        wizard_id = self.env["fsm.stock.tracking"].browse(wizard["res_id"])
        self.assertFalse(
            wizard_id.tracking_validated_line_ids, "There aren't validated line"
        )
        self.assertEqual(
            wizard_id.tracking_line_ids.product_id,
            self.product_lot,
            "There are one line with the right product",
        )
        self.assertEqual(
            wizard_id.tracking_line_ids.lot_id, self.lot_id2, "The line has lot_id2"
        )
        ...
        self.assertEqual(
            picking_ids.mapped("state"), ["done"], "Pickings should be set as done"
        )
        self.assertEqual(
            move.move_line_ids.lot_id, self.lot_id2, "The line has lot_id2"
        )
        ...
        self.assertEqual(
            wizard_id.tracking_validated_line_ids.product_id,
            self.product_lot,
            "There are one line with the right product",
        )
        self.assertEqual(
            wizard_id.tracking_validated_line_ids.lot_id,
            self.lot_id2,
            "The line has lot_id2 (the lot choosed at the beginning)",
        )
        ...
        self.task.with_user(self.project_user).action_fsm_validate()
        order_line_ids = self.task.sale_order_id.order_line.filtered(
        ...
        self.assertEqual(
            move.move_line_ids.lot_id,
            self.lot_id2 + self.lot_id3,
            "Lots stay the same.",
        )
        self.assertEqual(
            sum(move.move_line_ids.mapped("qty_done")), 4, "We deliver 4 (1+3)"
        )

        self.assertEqual(
            self.task.sale_order_id.picking_ids.mapped("state"),
            ["done", "done"],
            "The 2 pickings should be set as done",
        )
    ...
    def test_fsm_stock_validate_half_SOL_manually(self):
        ...
        self.assertEqual(
            wizard_id.tracking_line_ids.product_id,
            self.product_lot,
            "There are one (non validated) line with the right product",
        )
        self.assertEqual(
            wizard_id.tracking_line_ids.lot_id,
            self.lot_id3,
            "The line has lot_id3, (the lot choosed at the beginning in the wizard)",
        )
        self.assertEqual(
            wizard_id.tracking_line_ids.quantity,
            2,
            "Quantity is 2 (5 from the beginning in the wizard - 3 already delivered)",
        )
        self.assertEqual(
            wizard_id.tracking_validated_line_ids.product_id,
            self.product_lot,
            "There are one validated line with the right product",
        )
        self.assertEqual(
            wizard_id.tracking_validated_line_ids.lot_id,
            self.lot_id2,
            "The line has lot_id2, (not the lot choosed at the beginning, but the lot put in picking)",
        )
        self.assertEqual(
            wizard_id.tracking_validated_line_ids.quantity,
            3,
            "Quantity is 3, chosen in the picking",
        )
        ...
        self.assertEqual(
            ...
            7,
            "Quantity on SOL is 7 (3 already delivered and 4 set in wizard)",
        )
        self.assertEqual(
            order_line_ids.qty_delivered,
            3,
            "Quantity already delivered is 3, chosen in the picking",
        )
        ...
        self.assertEqual(
            len(order_line_ids),
            1,
            "There are 1 order lines, delivered in 2 times (first manually, second with fsm task validation).",
        )
        self.assertEqual(
            move.move_line_ids.lot_id, self.lot_id2 + self.lot_id3, "Lot stay the same."
        )
        self.assertEqual(
            sum(move.move_line_ids.mapped("qty_done")), 7, "We deliver 7 (4+3)"
        )
    ...
    def test_action_quantity_set(self):
        ...
        self.assertEqual(
            ...
            "It should redirect to the tracking wizard",
        )
        self.assertEqual(
    ...
    def test_set_quantity_with_done_so(self):
        ...
        validate_form = Form(
            self.env[validate_form_data["res_model"]].with_context(
                validate_form_data["context"]
            )
        ).save()
