...
class FsmStockTracking(models.TransientModel):
    ...
    tracking_line_ids = fields.One2many(
        "fsm.stock.tracking.line", "wizard_tracking_line"
    )
    tracking_validated_line_ids = fields.One2many(
        "fsm.stock.tracking.line", "wizard_tracking_line_valided"
    )
...
class FsmStockTrackingLine(models.TransientModel):
    ...
    wizard_tracking_line_valided = fields.Many2one(
        "fsm.stock.tracking", string="Validated Tracking Line"
    )
