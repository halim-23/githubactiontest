...
class ProductProduct(models.Model):
    ...
    def _compute_serial_missing(self):
        ...
        sale_lines = (
            ...
            .sudo()
            .search(
                [("order_id", "=", task.sale_order_id.id), ("task_id", "=", task.id)]
            )
        )
        for product in self:
            if product.tracking != "none":
                sale_product = sale_lines.filtered(
                    lambda sale: sale.product_id == product
                )
                product.serial_missing = sale_product.filtered(
                    lambda p: not p.fsm_lot_id
                    and p.product_uom_qty > 0
                    and not p.qty_delivered
                )
            else:
    ...
    def write(self, vals):
        ...
        if new_fsm_quantity and any(
            not product.quantity_decreasable and product.fsm_quantity > new_fsm_quantity
            for product in self
        ):
            raise UserError(
                _(
                    "You can no longer decrease the delivered quantity of a product once the task is marked as done. Please, create a return in your Inventory instead."
                )
            )
    ...
    def action_assign_serial(self):
        ...
        tracking_line_ids = [
            ...
            for line in sale_lines.filtered(
                lambda sl: sl.product_uom_qty - sl.qty_delivered and not task.fsm_done
            )
        ]

        lot_done_dict = defaultdict(int)
        for move_line in sale_lines.move_ids.filtered(
            lambda m: m.state == "done"
        ).move_line_ids:
            lot_done_dict[move_line.lot_id.id] += move_line.qty_done

        tracking_validated_line_ids = [
            (
                0,
                0,
                {
