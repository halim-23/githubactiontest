...
class Task(models.Model):
            ...
            for order_line in sale_line:
                ...
                if to_log:
                    ...
                    documents = {
                        ...
                        if k[0].state not in ["cancel", "done"]
                    }
    ...
    def _validate_stock(self):
        ...
        for so_line in self.sale_order_id.order_line:
            if not (
                so_line.task_id.is_fsm
                or so_line.project_id.is_fsm
                or so_line.fsm_lot_id
            ):
                continue
        ...
        def is_fsm_material_picking(picking, task):
            for move in picking.move_lines:
                ...
                if sol.fsm_lot_id:
                    continue
                if not (
                    sol.product_id != task.project_id.timesheet_product_id
        ...
        pickings_to_do = self.sale_order_id.picking_ids.filtered(
            lambda p: p.state not in ["done", "cancel"]
            and is_fsm_material_picking(p, self)
        )
        ...
        pickings_to_do.with_context(
            skip_sms=True, cancel_backorder=True
        ).button_validate()
