...
class ResCompany(models.Model):
    ...
    def set_special_defaults_on_install(self):
        ...
        for company in all_companies:
            company.currency_provider = currency_providers.get(
                company.country_id.code, "ecb"
            )
    ...
    def update_currency_rates(self):
        ...
        for (currency_provider, companies) in self._group_by_provider().items():
            ...
            if parse_results == False:
                ...
                _logger.warning(
                    "Unable to connect to the online exchange rate platform %s. The web service may be temporary down.",
                    currency_provider,
                )
                rslt = False
            else:
    ...
    def _generate_currency_rates(self, parsed_data):
        ...
        for company in self:
            ...
            if not rate_info:
                raise UserError(
                    _(
                        "Your main currency (%s) is not supported by this exchange rate provider. Please choose another one.",
    ...
    def _parse_fta_data(self, available_currencies):
        ...
        Python-usable form."""
        request_url = (
            "https://www.backend-rates.ezv.admin.ch/api/xmldaily?d=today&locale=en"
        )
        try:
    ...
    def _parse_boc_data(self, available_currencies):
        ...
        request_url = (
            "http://www.bankofcanada.ca/valet/observations/group/FX_RATES_DAILY/json"
        )
        try:
        ...
        last_obs = [
            obs for obs in data["observations"] if obs["d"] == last_observation_date
        ][0]
    ...
    def _parse_banxico_data(self, available_currencies):
        ...
        * The webservice returns the following currency rates:
        ...
        try:
            ...
            yesterday = (date_mx - datetime.timedelta(days=1)).strftime(
                DEFAULT_SERVER_DATE_FORMAT
            )
            res = requests.get(
                url % (",".join(foreigns), yesterday, today, token), timeout=30
            )
            res.raise_for_status()
            series = res.json()["bmx"]["series"]
            series = {
                serie["idSerie"]: {
                    dato["fecha"]: dato["dato"] for dato in serie["datos"]
                }
                for serie in series
                if "datos" in serie
            }
    ...
    def _parse_xe_com_data(self, available_currencies):
        ...
        try:
            fetched_data = requests.request(
                "GET", url_format % {"currency_code": "USD"}
            )
        except:
            return False
            ...
            for x in rates_node["children"]:
                if x["attrs"]["currency"] in available_currency_names:
                    rslt[x["attrs"]["currency"]] = (
                        float(x["attrs"].get("multiplier", "1"))
                        / float(x["children"][0]),
                        rate_date,
                    )
    ...
    def _parse_bcrp_data(self, available_currencies):
        ...
        result["PEN"] = (
            1.0,
            fields.Date.context_today(self.with_context(tz="America/Lima")),
        )
        ...
        date_pe = self.mapped("currency_next_execution_date")[
            0
        ...
        first_pe_str = (date_pe - datetime.timedelta(days=7)).strftime(
            bcrp_date_format_url
        )
    ...
    def _parse_mindicador_data(self, available_currencies):
        ...
        * The webservice returns the following currency rates:
        ...
        available_currency_names = available_currencies.mapped("name")
        logger.debug(
            "mindicador: available currency names: %s", available_currency_names
        )
        ...
        if records:
            ...
            for record in records:
                if record.currency_interval_unit == "daily":
                    ...
                    continue
                record.currency_next_execution_date = (
