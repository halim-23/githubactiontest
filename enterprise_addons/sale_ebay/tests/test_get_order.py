...
class TestEbay(TransactionCase):
    ...
    def test_synchronize_order(self):
        ...
        with patch(
            "odoo.addons.sale_ebay.models.product.ProductTemplate.ebay_execute",
            new=self.fake_execute_data(EBAY_ANSWER_1),
        ), patch(
            "odoo.addons.sale_ebay.models.product.ProductTemplate._ebay_configured",
            new=lambda d: True,
        ):
            number_of_sos = []
            ...
            number_of_ebay_loggings = self.env["ir.logging"].search_count(
                [("name", "=", "eBay")]
            )
