...
class EbayController(Controller):
    ...
    def _verify_signature(self):
        ...
        if not serialization:
            _logger.error(
                "Couldn't load cryptography lib, all ebay notifications will be discarded"
            )
            return False
        ...
        if not (
            signature_details["alg"] == "ecdsa"
            and signature_details["digest"] == "SHA1"
        ):
            _logger.error(
                "Ebay: unsupported algorithm for notification signatures (algo: %s, digest:%s)",
                signature_details["alg"],
                signature_details["digest"],
            )
            return False
        ...
        try:
            ...
            _logger.error(
                "Ebay: Public key (PEM) data’s structure could not be decoded successfully %s",
                e,
            )
            return False

        try:
            public_key.verify(decoded_signature, payload, ec.ECDSA(hashes.SHA1()))
            return True
    ...
    def _fetch_public_key(self, key_id):
        ...
        try:
            ...
            _logger.error(
                "Couldn't fetch public key from ebay (key id: %s):\n%s", key_id, e
            )
            return None
    ...
    def _format_public_key(self, key):
        ...
        :rtype: str
        """
        key = re.findall("-----BEGIN PUBLIC KEY-----(.+)-----END PUBLIC KEY-----", key)[
            0
        ]
