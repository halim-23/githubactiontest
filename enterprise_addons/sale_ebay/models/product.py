...
class ProductTemplate(models.Model):
    ...
    ebay_listing_status = fields.Char(
        "eBay Status", default="Unlisted", readonly=True, copy=False
    )
    ebay_title = fields.Char(
        "Title", size=80, help="The title is restricted to 80 characters"
    )
    ebay_subtitle = fields.Char(
        "Subtitle",
        size=55,
    ...
    ebay_item_condition_id = fields.Many2one(
        "ebay.item.condition", string="Item Condition"
    )
    ebay_category_id = fields.Many2one(
        "ebay.category",
        string="Category",
        domain=[("category_type", "=", "ebay"), ("leaf_category", "=", True)],
    )
    ebay_category_2_id = fields.Many2one(
        "ebay.category",
    ...
    ebay_store_category_id = fields.Many2one(
        "ebay.category",
        string="Store Category (Optional)",
        domain=[("category_type", "=", "store"), ("leaf_category", "=", True)],
    )
    ebay_store_category_2_id = fields.Many2one(
        "ebay.category",
        string="Store Category 2 (Optional)",
        domain=[("category_type", "=", "store"), ("leaf_category", "=", True)],
    )
    ...
    ebay_listing_type = fields.Selection(
        [("Chinese", "Auction"), ("FixedPriceItem", "Fixed price")],
        string="Listing Type",
        default="Chinese",
    )
    ...
    ebay_seller_payment_policy_id = fields.Many2one(
        "ebay.policy", string="Payment Policy", domain=[("policy_type", "=", "PAYMENT")]
    )
    ...
    ebay_quantity_sold = fields.Integer(
        related="product_variant_ids.ebay_quantity_sold",
        store=True,
        readonly=False,
        copy=False,
    )
    ebay_fixed_price = fields.Float(
        related="product_variant_ids.ebay_fixed_price", store=True, readonly=False
    )
    ebay_quantity = fields.Integer(
        related="product_variant_ids.ebay_quantity", store=True, readonly=False
    )
    ebay_last_sync = fields.Datetime(string="Last update", copy=False)
    ebay_template_id = fields.Many2one(
        "mail.template",
    ...
    def _prepare_item_dict(self):
        ...
        item = {
            "Item": {
                ...
                "StartPrice": comp_currency._convert(
                    self.ebay_price, currency, self.env.company, fields.Date.today()
                )
                if self.ebay_listing_type == "Chinese"
                else comp_currency._convert(
                    self.ebay_fixed_price,
                    currency,
                    self.env.company,
                    fields.Date.today(),
                ),
                ...
                "PostalCode": self.env["ir.config_parameter"]
                .sudo()
                .get_param("ebay_zip_code"),
                "Location": self.env["ir.config_parameter"]
                .sudo()
                .get_param("ebay_location"),
        ...
        if self.ebay_description and self.ebay_template_id:
            description = self.ebay_template_id._render_field("body_html", self.ids)[
                self.id
            ]
        ...
        if self.ebay_listing_type == "Chinese" and self.ebay_buy_it_now_price:
            item["Item"]["BuyItNowPrice"] = comp_currency._convert(
                self.ebay_buy_it_now_price,
                currency,
                self.env.company,
                fields.Date.today(),
            )
        ...
        if len(variant) == 1 and self.ebay_listing_type == "FixedPriceItem":
            if self.ebay_sync_stock:
        ...
        if self.attribute_line_ids:
            for attribute in self.attribute_line_ids:
                if len(attribute.value_ids) == 1:
                    ...
                    if attr_name == "Brand":
                        item["Item"]["ProductListingDetails"]["BrandMPN"][
                            "Brand"
                        ] = attr_value
                    elif attr_name == "MPN":
                        item["Item"]["ProductListingDetails"]["BrandMPN"][
        ...
        if self.ebay_category_2_id:
            item["Item"]["SecondaryCategory"] = {
                "CategoryID": self.ebay_category_2_id.category_id
            }
        if self.ebay_store_category_id:
            item["Item"]["Storefront"] = {
                "StoreCategoryID": self.ebay_store_category_id.category_id,
                "StoreCategoryName": self._ebay_encode(
                    self.ebay_store_category_id.name
                ),
            }
            if self.ebay_store_category_2_id:
                item["Item"]["Storefront"][
                    "StoreCategory2ID"
                ] = self.ebay_store_category_2_id.category_id
                item["Item"]["Storefront"]["StoreCategory2Name"] = self._ebay_encode(
                    self.ebay_store_category_2_id.name
                )
        return item
    ...
    def check_encoding(self, barcode, encoding):
        if encoding == "ean13":
            return (
                ...
                and self.ean_checksum(barcode) == int(barcode[-1])
            )
        elif encoding == "upc":
            return (
                len(barcode) == 12
    ...
    def _prepare_variant_dict(self):
        ...
        for variant in self.product_variant_ids:
            ...
            if (
                variant.ebay_use
                and not variant.ebay_quantity
                and not self.env["ir.config_parameter"]
                .sudo()
                .get_param("ebay_out_of_stock")
            ):
                raise UserError(
                    _(
                        "All the quantities must be greater than 0 or you need to enable the Out Of Stock option."
                    )
                )
            ...
            ean = "Does not apply"
            if variant.barcode:
                if len(variant.barcode) == 12 and self.check_encoding(
                    variant.barcode, "upc"
                ):
                    upc = variant.barcode
                elif len(variant.barcode) == 13 and self.check_encoding(
    ...
    def _set_variant_url(self, item_id):
        ...
        if len(variants) > 1 and self.ebay_listing_type == "FixedPriceItem":
            for variant in variants:
                ...
                variant.ebay_variant_url = item.dict()["Item"]["ListingDetails"][
                    "ViewItemURL"
                ]
    ...
    def get_ebay_api(self):
        ...
        if not params:
            ...
            raise RedirectWarning(
                _("One parameter is missing."),
                action.id,
                _("Configure The eBay Integrator Now"),
            )
    ...
    def ebay_execute(self, verb, data=None, list_nodes=[], verb_attrs=None, files=None):
        ...
        try:
            ...
            if error["ErrorCode"] == "21916884":
                error_message += _(
                    "Or the condition is not compatible with the category."
                )
    ...
    def get_ebay_images_attachments(self):
        ...
        def is_good_image(att):
            try:
                ...
                good_image = (
                    max(img.size) >= 500
                    and sum(img.size) <= 12000
                    and data.getbuffer().nbytes <= 12e6
                    and (not img.format == "JPEG" or img.mode == "RGB")
                )
                return good_image
            except Exception:
                return False
    ...
    def _create_picture_url(self):
        ...
        for att in attachments:
            ...
            pictureData = {"WarningLevel": "High", "PictureName": self.name}
            response = self.ebay_execute(
                "UploadSiteHostedPictures", pictureData, files=files
            )
    ...
    def _update_ebay_data(self, response):
        ...
        qty = int(item["Item"]["Quantity"]) - int(
            item["Item"]["SellingStatus"]["QuantitySold"]
        )
        for product in self:
            product.write(
                {
    ...
    def push_product_ebay(self):
        for product in self:
            if product.ebay_listing_status != "Active":
                ...
                response = self.ebay_execute(
                    "AddItem"
                    if product.ebay_listing_type == "Chinese"
                    else "AddFixedPriceItem",
                    item_dict,
                )
    ...
    def end_listing_product_ebay(self):
        for product in self:
            call_data = {"ItemID": product.ebay_id, "EndingReason": "NotAvailable"}
            self.ebay_execute(
                "EndItem"
                if product.ebay_listing_type == "Chinese"
                else "EndFixedPriceItem",
                call_data,
            )
    ...
    def synchronize_orders_from_last_sync(self, test_mode=False):
        ...
        now = datetime.now()
        last_sync_str = (
            self.env["ir.config_parameter"].sudo().get_param("ebay_last_sync")
        )
    ...
    def _synchronize_orders_ranged(self, date_from, date_to, page=1):
        ...
        if not date_to - date_from <= _30DAYS:
            raise ValidationError(
                "This function should not be called with a range of more than 30 days, "
        ...
        call_data = {
            ...
            "Pagination": {
                "EntriesPerPage": 100,  # max allowed by ebay
                "PageNumber": page,
            },
        }
        try:
            ...
            if (
                int(order_dict["ReturnedOrderCountActual"]) > 0
            ):  # order_dict.get('OrderArray'):
        ...
        if self.ebay_sync_stock:
            if self.ebay_listing_status in ["Active", "Error"]:
                ...
                if (
                    ...
                    and self.ebay_quantity != self.virtual_available
                ):
                    if (
                        self.env["ir.config_parameter"]
                        .sudo()
            ...
            "NameValueList": [
                {
                    ...
                    "Value": [
                        self._ebay_encode(ptav.product_attribute_value_id.name)
    ...
    def _get_variant_from_ebay_specs(self, specs):
        ...
        domain = expression.OR(
            [
                [
                    ("attribute_id.name", "=", spec["Name"]),
                    ("product_attribute_value_id.name", "=", spec["Value"]),
                ]
                for spec in specs
            ]
        )
        combination = self.env["product.template.attribute.value"].search(
            [("product_tmpl_id", "=", self.id)] + domain
        )
...
class ProductProduct(models.Model):
    ...
    ebay_listing_type = fields.Selection(
        related="product_tmpl_id.ebay_listing_type", readonly=False
    )
    ...
    def _get_ebay_variation_specifics(self):
        ...
        return {
            "NameValueList": [
                {
                    "Name": self.env["product.template"]._ebay_encode(
                        ptav.attribute_id.name
                    ),
                    "Value": self.env["product.template"]._ebay_encode(
