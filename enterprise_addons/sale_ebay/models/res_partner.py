...
class ResPartner(models.Model):
    ...
    def _compare_addresses(self, address_data):
        ...
        def zip_normalize(string):
            return (
                string.split("-")[0]
                if self.country_id.name == "United States"
                else default_normalize(string)
            )
    ...
    def _handle_ebay_account_deletion_notification(self):
        ...
        try:
            ...
            _logger.error(
                "eBay account deletion request: couldn't assign activity to admin user: %s",
                e,
            )
        ...
        try:
            ...
            _logger.error(
                "eBay: Couldn't anonymise partners (ids: %s): %s", self.ids, e
            )
    ...
    def _anonymize_ebay_partner(self, archive=False):
        ...
        delivery_children = self.child_ids.filtered(
            lambda p: p.type == "delivery" and p.id not in self.ids
        )
