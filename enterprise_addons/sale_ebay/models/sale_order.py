...
class SaleOrder(models.Model):
    ...
    def _process_order_new(self, order, transaction):
        ...
        fp = self.env["account.fiscal.position"].get_fiscal_position(
            partner.id, delivery_id=shipping_partner.id
        )
        if fp:
    ...
    def _process_order_new_find_partners(self, order):
        ...
        partner = self.env["res.partner"].search(
            [("ebay_id", "=", buyer_ebay_id)], limit=1
        )
        if not partner:
            partner = self.env["res.partner"].create(
                {"name": buyer_ebay_id, "ebay_id": buyer_ebay_id}
            )
        partner_data = {
            "name": infos.get("Name"),
        ...
        shipping_data["country_id"] = (
            ...
            .search([("code", "=", infos["Country"])], limit=1)
            .id
        )
    ...
    def _process_all_taxes(self, tax_dict, price_unit):
        ...
        """
        tax_commands = []
        tax_list_or_dict = tax_dict.get(
            "TaxDetails", []
        )  # returns a list if it contains more than one tax, directly returns the tax dict
        for tax in (
            [tax_list_or_dict]
            if isinstance(tax_list_or_dict, dict)
            else tax_list_or_dict
        ):
            tax_amount = float(tax["TaxAmount"]["value"])
            tax_rate = (
                100 * tax_amount / (price_unit - tax_amount)
                if price_unit > tax_amount > 0
                else 0
            )
            tax_description = tax.get("TaxDescription", "")
            tax_id = self._handle_taxes(
                tax_amount, tax_rate, description=tax_description
            )
            if tax_id:
    ...
    def _process_order_shipping(self, order):
        ...
        if "ShippingServiceSelected" in order:
            ...
            shipping_currency = (
                ...
                .with_context(active_test=False)
                .search([("name", "=", shipping_cost_dict["_currencyID"])], limit=1)
            )
            ...
            price_unit = shipping_currency._convert(
                shipping_amount,
                self.currency_id,
                self.company_id,
    ...
    def _process_transaction_product(self, transaction):
        ...
        if not product:
            ...
            product.message_post(
                body=_(
                    "Product created from eBay transaction %s",
        ...
        if product.product_variant_count > 1:
            if "Variation" in transaction:
                variant = product.product_variant_ids.filtered(
                    lambda l: l.ebay_use
                    and l.ebay_variant_url.split("vti", 1)[1]
                    == transaction["Variation"]["VariationViewItemURL"].split("vti", 1)[
                        1
                    ]
                )
            # If multiple variants but only one listed on eBay as Item Specific
                ...
                variant = product._get_variant_from_ebay_specs(
                    [n for n in name_value_list if n["Source"] == "ItemSpecific"]
                )
        else:
            variant = product.product_variant_ids[0]
        variant.ebay_quantity_sold = variant.ebay_quantity_sold + int(
            transaction["QuantityPurchased"]
        )
        if not product.ebay_sync_stock:
            variant.ebay_quantity = variant.ebay_quantity - int(
                transaction["QuantityPurchased"]
            )
            variant_qty = 0
    ...
    def _process_order_new_transaction(self, transaction):
        ...
        price_unit = transaction_currency._convert(
            price_unit,
            self.currency_id,
            self.company_id,
            self.date_order or datetime.now(),
    ...
    def _process_order_update(self, order):
        ...
        can_be_invoiced = "order" in product_lines.mapped(
            "product_id.invoice_policy"
        ) and "to invoice" in product_lines.mapped("invoice_status")

        no_confirm = (
            self.env.context.get("ebay_no_confirm", False)
            or not are_all_products_listed
        )
        try:
