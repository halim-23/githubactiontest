    ...
    def _compute_full_name(self):
        for rec in self:
            ...
            while parent_id != "0":
                ...
                parent_id = (
                    parent.category_parent_id if parent.category_parent_id else "0"
                )
            rec.full_name = name
        ...
        for category in categories:
            ...
            if category["CategoryLevel"] == "1":
                ...
                response = self.env["product.template"].ebay_execute(
                    "GetCategoryFeatures", call_data
                )
                if "ConditionValues" in response.dict()["Category"]:
                    conditions = response.dict()["Category"]["ConditionValues"][
                        "Condition"
                    ]
                    if not isinstance(conditions, list):
                        conditions = [conditions]
                    for condition in conditions:
                        if not self.env["ebay.item.condition"].search(
                            [("code", "=", condition["ID"])]
                        ):
...
class EbayPolicy(models.Model):
    ...
    def sync_policies(self):
        response = self.env["product.template"].ebay_execute(
            "GetUserPreferences", {"ShowSellerProfilePreferences": True}
        )
        if (
            "SellerProfilePreferences" not in response.dict()
            or not response.dict()["SellerProfilePreferences"][
                "SupportedSellerProfiles"
            ]
        ):
            raise UserError(_("No Business Policies"))
        policies = response.dict()["SellerProfilePreferences"][
            "SupportedSellerProfiles"
            ...
            record.write(
                {
                    ...
                    "short_summary": policy["ShortSummary"]
                    if "ShortSummary" in policy
                    else " ",
                }
            )
