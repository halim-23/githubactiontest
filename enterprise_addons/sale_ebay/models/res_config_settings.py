...
class ResConfigSettings(models.TransientModel):
    ...
    ebay_dev_id = fields.Char(
        "Developer Key", default="", config_parameter="ebay_dev_id"
    )
    ebay_sandbox_token = fields.Char(
        "Sandbox Token", default="", config_parameter="ebay_sandbox_token"
    )
    ebay_sandbox_app_id = fields.Char(
        "Sandbox App Key", default="", config_parameter="ebay_sandbox_app_id"
    )
    ebay_sandbox_cert_id = fields.Char(
        "Sandbox Cert Key", default="", config_parameter="ebay_sandbox_cert_id"
    )

    ebay_prod_token = fields.Char(
        "Production Token", default="", config_parameter="ebay_prod_token"
    )
    ebay_prod_app_id = fields.Char(
        "Production App Key", default="", config_parameter="ebay_prod_app_id"
    )
    ebay_prod_cert_id = fields.Char(
        "Production Cert Key", default="", config_parameter="ebay_prod_cert_id"
    )
    ebay_domain = fields.Selection(
        ...
        string="eBay Environment",
        default="sand",
        required=True,
        config_parameter="ebay_domain",
    )
    ebay_currency = fields.Many2one(
        "res.currency",
        string="ebay Currency",
        domain=[("ebay_available", "=", True)],
        required=True,
        default=lambda self: self.env["res.currency"]
        .search([("ebay_available", "=", True)], limit=1)
        .id,
        config_parameter="ebay_currency",
    )
    ebay_country = fields.Many2one(
        "res.country",
        domain=[("ebay_available", "=", True)],
        string="Country",
    ...
    ebay_site = fields.Many2one(
        "ebay.site",
        string="eBay Website",
        default=lambda self: self.env["ebay.site"]
        .search([("ebay_id", "=", "0")], limit=1)
        .id,
        config_parameter="ebay_site",
    )
    ebay_zip_code = fields.Char(
        string="Zip", default="", config_parameter="ebay_zip_code"
    )
    ...
    ebay_sales_team = fields.Many2one(
        "crm.team", string="ebay Sales Team", config_parameter="ebay_sales_team"
    )
    ebay_gallery_plus = fields.Boolean(
        "Gallery Plus", default="", config_parameter="ebay_gallery_plus"
    )
    ...
    def action_reset_token(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "sale_ebay.acc_deletion_token", uuid.uuid4()
        )
    ...
    def set_values(self):
        ...
        if out_of_stock != str2bool(
            self.env["ir.config_parameter"].get_param("ebay_out_of_stock")
        ):
            ...
            if self.ebay_domain == "sand":
                if (
                    self.ebay_sandbox_token
                    and self.ebay_sandbox_cert_id
                    and self.ebay_sandbox_app_id
                ):
                    ebay_api = Trading(
                ...
                if (
                    self.ebay_prod_token
                    and self.ebay_prod_cert_id
                    and self.ebay_prod_app_id
                ):
                    ebay_api = Trading(
                        domain="api.ebay.com",
                        config_file=None,
    ...
    def get_values(self):
        ...
        get_param = (
            self.env["ir.config_parameter"].sudo().get_param("ebay_out_of_stock")
        )
        res.update(
        ...
        for site in response.dict()["SiteDetails"]:
            ...
            if not record:
                record = self.env["ebay.site"].create(
                    {"name": site["Site"], "ebay_id": site["SiteID"]}
                )
            else:
