...
class HrReferralReward(models.Model):
    ...
    def _group_hr_referral_domain(self):
        group = self.env.ref(
            "hr_referral.group_hr_recruitment_referral_user", raise_if_not_found=False
        )
    ...
    gift_manager_id = fields.Many2one(
        "res.users",
        string="Gift Responsible",
        domain=_group_hr_referral_domain,
        help="User responsible of this gift.",
    )
    ...
    def _compute_awarded_employees(self):
        data = {
            d["hr_referral_reward_id"][0]: d["__count"]
            for d in self.env["hr.referral.points"].read_group(
    ...
    def _compute_points_missing(self):
        ...
        for item in self.env["hr.referral.points"].read_group(
            [("ref_user_id", "=", self.env.user.id)],
            ["company_id", "points"],
            ["company_id"],
        ):
            available_points_company[item["company_id"][0]] = item["points"]
        for item in self:
            item.points_missing = item.cost - (
                available_points_company[item.company_id.id]
                if item.company_id.id in available_points_company
                else 0
            )
    ...
    def buy(self):
        ...
        if available_points < self.cost:
            raise UserError(
                _(
                    "You do not have enough points in this company to buy this product. In this company, you have %s points.",
                    available_points,
                )
            )
    ...
    def action_get_employee_awarded(self):
        points_ids = (
            self.env["hr.referral.points"]
            .search([("hr_referral_reward_id", "in", self.ids)])
            .ids
        )
        return {
            ...
            "views": [
                [self.env.ref("hr_referral.view_hr_referral_gift_tree").id, "tree"],
                [False, "form"],
            ],
            "view_mode": "tree",
    ...
    def create(self, values):
        if "gift_manager_id" in values:
            reward_responsible_group = self.env.ref(
                "hr_referral.group_hr_referral_reward_responsible_user",
                raise_if_not_found=False,
            )
            if reward_responsible_group and values["gift_manager_id"]:
                reward_responsible_group.sudo().write(
                    {"users": [(4, values["gift_manager_id"])]}
                )
    ...
    def write(self, values):
        ...
        if "gift_manager_id" in values:
            ...
            if (
                reward_responsible_group
                and gift_manager
                and not gift_manager.has_group(
                    "hr_referral.group_hr_referral_reward_responsible_user"
                )
            ):
                reward_responsible_group.sudo().write(
