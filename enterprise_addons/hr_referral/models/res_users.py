...
class ResUsers(models.Model):
    ...
    hr_referral_level_id = fields.Many2one(
        "hr.referral.level", groups="hr.group_hr_user"
    )
    ...
    utm_source_id = fields.Many2one(
        "utm.source", "Source", ondelete="cascade", groups="hr.group_hr_user"
    )
    ...
    def _clean_responsibles(self):
        ...
        responsibles_to_remove_ids = set(self.ids) - {
            x["gift_manager_id"][0] for x in res
        }
        reward_responsible_group.sudo().write(
            {
                "users": [
                    (3, responsible_id) for responsible_id in responsibles_to_remove_ids
                ]
            }
        )
