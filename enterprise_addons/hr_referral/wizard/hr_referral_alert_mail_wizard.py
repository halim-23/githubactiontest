...
class HrReferralAlertMailWizard(models.TransientModel):
    ...
    def _get_user_domain(self):
        return [
            ...
            (
                "groups_id",
                "in",
                self.env.ref("hr_referral.group_hr_recruitment_referral_user").id,
            ),
        ]
    ...
    def _default_body(self):
        url = "/web#" + url_encode(
            {"action": "hr_referral.action_hr_referral_welcome_screen"}
        )
        return _(
            "A new alert has been added to the Referrals app! Check your <a href=%(url)s>dashboard</a> now!",
            url=url,
        )
    ...
    subject = fields.Char(
        required=True, default=lambda self: _("New Alert In Referrals App")
    )
