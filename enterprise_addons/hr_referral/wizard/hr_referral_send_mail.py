...
class HrReferralSendMail(models.TransientModel):
    ...
    body_html = fields.Html(
        "Body", compute="_compute_body_html", store=True, readonly=False
    )
    ...
    def _compute_url(self):
        self.ensure_one()
        self.url = (
            self.env["hr.referral.link.to.share"]
            .create(
                {
    ...
    def _compute_body_html(self):
        for wizard in self:
            if not wizard.job_id:
                wizard.body_html = _(
                    'Hello,<br><br>There are some amazing job offers in my company! Have a look, they  can be interesting for you<br><a href="%s">See Job Offers</a>'
                ) % (wizard.url)
            else:
                wizard.body_html = _(
                    'Hello,<br><br>There is an amazing job offer for %s in my company! It will be a fit for you<br><a href="%s">See Job Offer</a>'
                ) % (wizard.job_id.name, wizard.url)
