...
class HrReferralLinkToShare(models.TransientModel):
    ...
    def _compute_url(self):
        ...
        if not self.env.user.utm_source_id:
            ...
            self.env.user.utm_source_id = (
                self.env["utm.source"].create({"name": utm_name}).id
            )

        if self.job_id and not self.job_id.utm_campaign_id:
            self.job_id.utm_campaign_id = (
                self.env["utm.campaign"].create({"name": self.job_id.name}).id
            )
        ...
        if self.channel == "direct":
            ...
            self.url = (
                ...
                % link_tracker.short_url
            )
        elif self.channel == "twitter":
            self.url = (
                "https://twitter.com/intent/tweet?tw_p=tweetbutton&text=Amazing job offer for %s! Check it live: %s"
                % (self.job_id.name, link_tracker.short_url)
            )
