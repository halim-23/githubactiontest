...
class Task(models.Model):
    ...
    planned_date_begin = fields.Datetime(
        "Start date", tracking=True, task_dependency_tracking=True
    )
    planned_date_end = fields.Datetime(
        "End date", tracking=True, task_dependency_tracking=True
    )
    ...
    display_warning_dependency_in_gantt = fields.Boolean(
        compute="_compute_display_warning_dependency_in_gantt"
    )
    planning_overlap = fields.Integer(
        compute="_compute_planning_overlap", search="_search_planning_overlap"
    )
    ...
    def default_get(self, fields_list):
        ...
        if (
            planned_date_begin
            and planned_date_end
            and not self.env.context.get("fsm_mode", False)
        ):
            user_id = result.get("user_id", None)
            planned_date_begin, planned_date_end = self._calculate_planned_dates(
                planned_date_begin, planned_date_end, user_id
            )
            result.update(
                planned_date_begin=planned_date_begin, planned_date_end=planned_date_end
            )
        return result
    ...
    def _compute_display_warning_dependency_in_gantt(self):
        for task in self:
            task.display_warning_dependency_in_gantt = not (
                task.stage_id.is_closed or task.stage_id.fold
            )
    ...
    def _search_planning_overlap(self, operator, value):
        if operator not in ["=", ">"] or not isinstance(value, int) or value != 0:
            raise NotImplementedError(
                _(
                    "Operation not supported, you should always compare planning_overlap to 0 value with = or > operator."
                )
            )
    ...
    def _calculate_planned_dates(
        ...
        start = (
            list_intervals[0][0].astimezone(utc).replace(tzinfo=None)
        )  # We take the first date in the interval list
        stop = (
            list_intervals[-1][1].astimezone(utc).replace(tzinfo=None)
    ...
    def _is_auto_shift_candidate(self):
        ...
        return (
            self.project_id
            and self.project_id.allow_task_dependencies
            and not task_is_done
            and self.planned_date_begin
            and self.planned_date_end
            and self.planned_date_begin > datetime.now()
        )
    ...
    def _get_calendars_and_resources_key(self):
        ...
        return (
            ...
            or self.project_id.company_id.resource_calendar_id
        )
            ...
            self._get_calendars_and_resources_key(): [
                {
                    ...
                    "calendar_id": self.company_id.resource_calendar_id
                    or self.project_id.company_id.resource_calendar_id,
                    "resource_id": False,
                }
            ]
    ...
    def write(self, vals):
        ...
        if (
            not self.env.context.get("fsm_mode", False)
            and "planned_date_begin" in vals
            and "planned_date_end" in vals
            ...
            compute_default_planned_dates = self.filtered(
                lambda task: not task.planned_date_begin and not task.planned_date_end
            )
        ...
        if compute_default_planned_dates:
            ...
            tasks_by_resource_calendar_dict = (
                compute_default_planned_dates._get_tasks_by_resource_calendar_dict()
            )
            for (calendar, tasks) in tasks_by_resource_calendar_dict.items():
                date_start, date_stop = self._calculate_planned_dates(
                    planned_date_begin, planned_date_end, calendar=calendar
                )
                tasks.write(
                    {
        ...
        write_planned_date_fields = any(
            field
            for field in ["planned_date_begin", "planned_date_end"]
            if field in vals
        )
    ...
    def _write_planned_dates_if_in_future(
        self, new_planned_date_begin, new_planned_date_end
    ):
        self.ensure_one()
        if (
            new_planned_date_begin
            and new_planned_date_end
            and new_planned_date_begin >= datetime.now(tz=utc)
        ):
            context_update = {
                DATE_AUTO_SHIFT_IDS_TO_EXCLUDE_KEY: self.env.context.get(
                    DATE_AUTO_SHIFT_IDS_TO_EXCLUDE_KEY, set()
                )
                | {self.id}
            }
            self.with_context(**context_update).write(
                {
                    "planned_date_begin": new_planned_date_begin.astimezone(
                        utc
                    ).replace(tzinfo=None),
                    "planned_date_end": new_planned_date_end.astimezone(utc).replace(
                        tzinfo=None
                    ),
                }
            )
    ...
    @api.model
    def _get_cache_info(
        self, key, task_calendars, intervals_cache, date_time, search_forward
    ):
        match_index = 0
        for index, item in enumerate(
            task_calendars[key] if search_forward else reversed(task_calendars[key]), 1
        ):
        ...
        cache_key = (
            task_calendars[key][match_index - 1]["calendar_id"],
            task_calendars[key][match_index - 1]["resource_id"],
        )
        intervals_cache_entry = intervals_cache[cache_key]
        in_memory_start = (
            intervals_cache_entry._items[0][0].astimezone(utc)
            if len(intervals_cache_entry) > 0
            else datetime(9999, 12, 31, 23, 59, 59, 999999, tzinfo=utc)
        )
        in_memory_end = (
            intervals_cache_entry._items[-1][1].astimezone(utc)
            if len(intervals_cache_entry) > 0
    ...
    def _get_first_work_interval(
        self, intervals_cache, date_time, task_calendars, search_forward=True
    ):
        """
        ...
        if key not in task_calendars:
            task_calendars = self._get_calendars_and_resources(
                date_time - relativedelta(months=1), date_time + relativedelta(months=1)
            )

        intervals_cache_entry, in_memory_start, in_memory_end = self._get_cache_info(
            key, task_calendars, intervals_cache, date_time, search_forward
        )
        ...
        if (search_forward and in_memory_start <= date_time < in_memory_end) or (
            ...
            interval = self._get_interval(
                date_time, intervals_cache_entry, search_forward
            )
        else:
            ...
            while not interval:
                ...
                if date_to_search <= in_memory_start:
                    if date_to_search < task_calendars[key][0]["date_start"]:
                        new_task_calendars = self._get_calendars_and_resources(
                            date_to_search, task_calendars[key][0]["date_start"]
                        )
                        task_calendars[key][0:0] = new_task_calendars[key]

                        (
                            intervals_cache_entry,
                            in_memory_start,
                            in_memory_end,
                    ...
                    for entry in filter(
                        lambda calendar_interval: calendar_interval["date_start"]
                        <= in_memory_start
                        and calendar_interval["date_end"] >= date_to_search,
                        task_calendars[key]
                        if search_forward
                        else reversed(task_calendars[key]),
                    ):
                        new_interval = entry["calendar_id"]._work_intervals_batch(
                            max(date_to_search, entry["date_start"]),
                            min(in_memory_start, entry["date_end"]),
                            entry["resource_id"],
                        )[entry["resource_id"] and entry["resource_id"].id]
                        intervals |= new_interval
                        intervals_cache[
                            entry["calendar_id"], entry["resource_id"]
                        ] |= new_interval
                        interval_pool |= intervals_cache[
                            entry["calendar_id"], entry["resource_id"]
                        ]
                    ...
                    if date_to_search > task_calendars[key][-1]["date_end"]:
                        new_task_calendars = self._get_calendars_and_resources(
                            task_calendars[key][-1]["date_end"], date_to_search
                        )
                    ...
                    for entry in filter(
                        lambda calendar_interval: calendar_interval["date_start"]
                        <= date_to_search
                        and calendar_interval["date_end"] >= in_memory_end,
                        task_calendars[key]
                        if search_forward
                        else reversed(task_calendars[key]),
                    ):
                        new_interval = entry["calendar_id"]._work_intervals_batch(
                            max(in_memory_end, entry["date_start"]),
                            min(date_to_search, entry["date_end"]),
                            entry["resource_id"],
                        )[entry["resource_id"] and entry["resource_id"].id]
                        intervals |= new_interval
                        intervals_cache[
    ...
    def _get_interval(self, date_time, intervals, search_forward):
        ...
        for interval in intervals:
            if (search_forward and interval[1] > date_time) or (
                not search_forward and interval[0] < date_time
            ):
                return interval
        return ()

    @api.model
    def _get_date_for_planned_hours(
        self,
        intervals,
        planned_hours_target,
        planned_hours,
        searched_date,
        search_forward=True,
    ):
        ...
        for interval in enumeration:
            ...
            if search_forward:
                if interval[1] > searched_date:
                    ...
                    delta = min(
                        planned_hours_target - planned_hours, interval[1] - target_date
                    )
                ...
                if interval[0] < searched_date:
                    ...
                    delta = min(
                        planned_hours_target - planned_hours, target_date - interval[0]
                    )
                    searched_date = target_date - delta
    ...
    def _action_auto_shift(self):
        ...
        (
            ...
            mapped_depend_on,
            mapped_dependent,
        ) = self._action_auto_shift_get_candidates()
        ...
        for task in tasks_to_auto_shift:
            ...
            if task.planned_hours:
                (
                    new_planned_date_begin,
                    new_planned_date_end,
                    intervals_cache,
                ) = task.sudo()._action_auto_shift_with_planned_hours(
                ...
                (
                    ...
                    new_planned_date_end,
                    intervals_cache,
                ) = task.sudo()._action_auto_shift_without_planned_hours(
                    mapped_dependent, mapped_depend_on, intervals_cache
                )

            task._write_planned_dates_if_in_future(
    ...
    def _action_auto_shift_get_candidates(self):
        ...
        auto_shift_direction = self.env.context.get(
            DATE_AUTO_SHIFT_DIRECTION_CONTEXT_KEY, "none"
        )
        auto_shift_forward = auto_shift_direction in (
            DATE_AUTO_SHIFT_FORWARD,
            DATE_AUTO_SHIFT_BOTH_DIRECTIONS,
        )
        auto_shift_backward = auto_shift_direction in (
            DATE_AUTO_SHIFT_BACKWARD,
            DATE_AUTO_SHIFT_BOTH_DIRECTIONS,
        )
        ...
        date_auto_shift_ids_to_exclude = self.env.context.get(
            DATE_AUTO_SHIFT_IDS_TO_EXCLUDE_KEY, set()
        ) - set(self.ids)
        auto_shift_candidates = self.env["project.task"].browse(
            auto_shift_candidate_ids - date_auto_shift_ids_to_exclude
        )
        auto_shift_candidate_set = {
            auto_shift_candidate.id
            for auto_shift_candidate in auto_shift_candidates
            if auto_shift_candidate._is_auto_shift_candidate()
        }
        ...
        for task in self:
            ...
            if task.id in auto_shift_candidate_set:
                for depend_on_task in task.depend_on_ids:
                    if (
                        depend_on_task.id in auto_shift_candidate_set
                        and depend_on_task.project_id == task.project_id
                        and (
                            depend_on_task.planned_date_end > task.planned_date_begin
                            or auto_shift_forward
                        )
                    ):
                        if (
                            not mapped_depend_on[depend_on_task]
                            or mapped_depend_on[depend_on_task].planned_date_begin
                ...
                for dependent_task in task.dependent_ids:
                    if (
                        dependent_task.id in auto_shift_candidate_set
                        and dependent_task.project_id == task.project_id
                        and (
                            dependent_task.planned_date_begin < task.planned_date_end
                            or auto_shift_backward
                        )
                    ):
                        if (
                            not mapped_depend_on[dependent_task]
                            and not mapped_dependent[dependent_task]
                        ) or mapped_dependent[
                            dependent_task
    ...
    def _action_auto_shift_with_planned_hours(
        self, mapped_dependent, mapped_depend_on, intervals_cache
    ):
        ...
        if mapped_dependent[task]:
            ...
            (
                planned_date_begin_interval,
                intervals_cache,
                task_calendars,
            ) = self._get_first_work_interval(
                intervals_cache, date_candidate, task_calendars
            )
            searched_date = new_planned_date_begin = max(
                date_candidate, planned_date_begin_interval[0]
            )
        else:
            ...
            date_candidate = mapped_depend_on[task].planned_date_begin.replace(
                tzinfo=utc
            )
            # We arbitrary get the calendars info for the period from a month before the date and a month after.
            task_calendars = self._get_calendars_and_resources(
            ...
            (
                planned_date_end_interval,
                intervals_cache,
                task_calendars,
            ) = self._get_first_work_interval(
                intervals_cache, date_candidate, task_calendars, search_forward=False
            )
            searched_date = new_planned_date_end = min(
                date_candidate, planned_date_end_interval[1]
            )
        ...
        while new_task_hours < planned_hours:
            ...
            calendar_intervals_to_search = max(
                planned_hours - new_task_hours, timedelta(weeks=min_numb_of_weeks)
            )
            ...
            for calendar in (
                ...
                if calendar["date_start"] <= stop and calendar["date_end"] >= start:
                    intervals_cache_entry = intervals_cache[
                        calendar["calendar_id"], calendar["resource_id"]
                    ]
                    ...
                    task_interval = Intervals(
                        [
                            (
                                max(
                                    min(start, calendar_intervals_end),
                                    calendar["date_start"],
                                ),
                                min(
                                    max(stop, calendar_intervals_start),
                                    calendar["date_end"],
                                ),
                                self.env["resource.calendar.attendance"],
                            )
                    ...
                    if not search_intervals or first[intervals_cache_entry]:
                        ...
                        (
                            new_task_hours,
                            searched_date,
                        ) = self._get_date_for_planned_hours(
                            intervals_cache_entry,
                            planned_hours,
                            new_task_hours,
                            searched_date,
                        ...
                        for search_interval in search_intervals:
                            intervals = calendar["calendar_id"]._work_intervals_batch(
                                search_interval[0],
                                search_interval[1],
                                calendar["resource_id"],
                            )[calendar["resource_id"] and calendar["resource_id"].id]
                            if not intervals:
                                min_numb_of_weeks += 1
                                break
                            else:
    ...
    def _action_auto_shift_without_planned_hours(
        self, mapped_dependent, mapped_depend_on, intervals_cache
    ):
    ...
    def action_fsm_view_overlapping_tasks(self):
        ...
        if "views" in action:
            ...
            action["views"] = [(gantt_view.id, "gantt"), (map_view.id, "map")] + [
                (state, view)
                for state, view in action["views"]
                if view not in ["gantt", "map"]
            ]
        action.update(
            {
    ...
    def gantt_unavailability(
        ...
        resources = (
            ...
            .browse(user_ids)
            .mapped("resource_ids")
            .filtered(lambda r: r.company_id.id == self.env.company.id)
        )
        ...
        user_resource_mapping = {
            resource.user_id.id: resource.id
            for resource in resources.sorted("create_date", True)
        }
        ...
        company_leaves = self.env.company.resource_calendar_id._unavailable_intervals(
            start_datetime.replace(tzinfo=utc), end_datetime.replace(tzinfo=utc)
        )
    ...
    def action_reschedule(self, direction, master_task_id, slave_task_id):
        ...
        # Mapping of tasks with the related tasks dependent tasks
        mapped_dependent = defaultdict(lambda: self.env["project.task"])
        master_task, slave_task = self.env["project.task"].browse(
            [master_task_id, slave_task_id]
        )
        is_dependency_constraint_met = (
            master_task.planned_date_end <= slave_task.planned_date_begin
        )
        ...
        if direction == "forward":
            auto_shift_task_write_context[
                DATE_AUTO_SHIFT_DIRECTION_CONTEXT_KEY
            ] = DATE_AUTO_SHIFT_FORWARD
        ...
        if trigger_task.planned_hours:
            (
                ...
                new_planned_date_end,
                intervals_cache,
            ) = trigger_task.sudo()._action_auto_shift_with_planned_hours(
                mapped_dependent, mapped_depend_on, intervals_cache
            )
        else:
            (
                ...
                new_planned_date_end,
                intervals_cache,
            ) = trigger_task.sudo()._action_auto_shift_without_planned_hours(
                mapped_dependent, mapped_depend_on, intervals_cache
            )

        if not trigger_task._write_planned_dates_if_in_future(
            new_planned_date_begin, new_planned_date_end
        ):
            return {
                ...
                "params": {
                    "type": "warning",
                    "message": _(
                        "You cannot reschedule tasks in the past. Please, change their dates manually instead."
                    ),
                },
            }
