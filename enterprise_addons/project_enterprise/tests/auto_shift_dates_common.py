...
class AutoShiftDatesCommon(TestProjectCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.env = cls.env(
            context=dict(cls.env.context, date_auto_shift=True, mail_create_nolog=True)
        )
        ...
        cls.task_1_no_date_auto_shift_trigger = {
            "planned_date_begin": cls.task_1.planned_date_begin
            + overlapping_delta
            - timedelta(hours=1),
            "planned_date_end": cls.task_1.planned_date_end
            + overlapping_delta
            - timedelta(hours=1),
        }
