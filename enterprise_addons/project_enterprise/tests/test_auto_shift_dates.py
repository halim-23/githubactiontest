...
class TestTaskDependencies(AutoShiftDatesCommon):
    def test_auto_shift_date_not_active(self):
        ...
        * When the calculated planned_start_date is prior to now
        ...
        def test_task_3_dates_unchanged(
            task_1_new_planned_dates, failed_message, domain_force=False, **context
        ):
            task_1 = (
                self.task_1.with_context(context)
                if domain_force
                else self.task_1.with_context(**context)
            )
            task_1.write(task_1_new_planned_dates)
            self.assertEqual(
                self.task_3_planned_date_begin,
                self.task_3.planned_date_begin,
                failed_message,
            )
        ...
        test_task_3_dates_unchanged(
            self.task_1_date_auto_shift_trigger,
            failed_message,
            True,
        ...
        test_task_3_dates_unchanged(
            self.task_1_date_auto_shift_trigger, failed_message, date_auto_shift=False
        )
        ...
        self.task_3.write({"project_id": False})
        test_task_3_dates_unchanged(self.task_1_date_auto_shift_trigger, failed_message)
        self.task_3.write({"project_id": project_id.id})

        # Checks that no date shift is made when the new planned_date is prior to the current datetime.
        with freeze_time(
            self.task_1_no_date_auto_shift_trigger["planned_date_begin"]
            ...
            test_task_3_dates_unchanged(
                self.task_1_date_auto_shift_trigger, failed_message
            )
    ...
    def test_auto_shift_dependent_task(self):
        ...
        failed_message = (
            "The auto shift date feature should move forward a dependent tasks."
        )
        self.assertTrue(
            self.task_1.planned_date_end <= self.task_3.planned_date_begin,
            failed_message,
        )
    ...
    def test_auto_shift_depend_on_task(self):
        ...
        self.assertTrue(
            self.task_3.planned_date_begin >= self.task_1.planned_date_end,
            failed_message,
        )
    ...
    def test_auto_shift_on_dependent_task_with_planned_hours(self):
        ...
        failed_message = (
            "The auto shift date feature should take the planned_hours into account and update the"
            "planned_date_end accordingly when moving a task forward."
        )
        self.assertEqual(
            self.task_3.planned_date_end,
            self.task_3_planned_date_end + relativedelta(days=1, hour=9),
            failed_message,
        )
    ...
    def test_auto_shift_on_task_depending_on_with_planned_hours(self):
        ...
        self.task_3.write(
            {
                ...
                "planned_date_end": new_task_3_begin_date
                + (self.task_3_planned_date_end - self.task_3_planned_date_begin),
            }
        )
        failed_message = (
            "The auto shift date feature should take the planned_hours into account and update the"
        ...
        self.assertEqual(
            self.task_1.planned_date_begin,
            self.task_1_planned_date_begin + relativedelta(days=-1, hour=16),
            failed_message,
        )
    ...
    def test_auto_shift_on_dependent_task_without_planned_hours(self):
        ...
        failed_message = (
            "When planned_hours=0, the auto shift date feature should preserve the time interval between"
            "planned_date_begin and planned_date_end when moving a task forward."
        )
        self.assertEqual(
            self.task_3.planned_date_end - self.task_3.planned_date_begin,
            self.task_3_planned_date_end - self.task_3_planned_date_begin,
            failed_message,
        )
    ...
    def test_auto_shift_on_task_depending_on_without_planned_hours(self):
        ...
        self.assertEqual(
            self.task_1.planned_date_end - self.task_1.planned_date_begin,
            self.task_1_planned_date_end - self.task_1_planned_date_begin,
            failed_message,
        )
    ...
    def test_auto_shift_next_work_time_with_planned_hours(self):
        ...
        """
        new_task_1_planned_date_begin = self.task_3_planned_date_begin + timedelta(
            hours=1
        )
        self.task_1.write(
            {
        ...
        self.assertEqual(
            self.task_3.planned_date_begin,
            self.task_3_planned_date_begin + relativedelta(days=1, hour=8),
            failed_message,
        )
    ...
    def test_auto_shift_previous_work_time_with_planned_hours(self):
        ...
        failed_message = "The auto shift date feature should take the user company resource_calendar into account."
        self.assertEqual(
            self.task_1.planned_date_end,
            self.task_1_planned_date_end + relativedelta(days=-1, hour=17),
            failed_message,
        )
        ...
        self.task_3.write(
            {
                ...
                "planned_date_end": self.task_4_planned_date_begin
                + (self.task_3_planned_date_end - self.task_3_planned_date_begin),
            }
        )
        failed_message = (
            "The auto shift date feature should take the user company resource_calendar into account and"
            "works also for long periods (requiring extending the search interval period)."
        )
        self.assertEqual(
            self.task_4.planned_date_begin,
            self.task_4_planned_date_begin + relativedelta(month=8, day=2, hour=8),
            failed_message,
        )
    ...
    def test_auto_shift_previous_work_time_long_leaves(self):
        ...
        self.assertEqual(
            self.task_5.planned_date_end,
            self.task_5_planned_date_end + relativedelta(month=6, day=30, hour=17),
            failed_message,
        )
    ...
    def test_auto_shift_cascading_forward(self):
        ...
        """
        new_task_3_planned_date_begin = self.task_4_planned_date_begin - timedelta(
            hours=1
        )
        self.task_3.write(
            {
                "planned_date_begin": new_task_3_planned_date_begin,
                "planned_date_end": new_task_3_planned_date_begin
                + (self.task_3_planned_date_end - self.task_3_planned_date_begin),
            }
        )
        failed_message = (
            "The auto shift date feature should handle correctly dependencies cascades."
        )
        self.assertEqual(
            self.task_4.planned_date_begin, self.task_3.planned_date_end, failed_message
        )
        self.assertEqual(
            self.task_4.planned_date_end,
            self.task_4_planned_date_begin + relativedelta(month=8, day=2, hour=9),
            failed_message,
        )
        self.assertEqual(self.task_5.planned_date_begin, self.task_4.planned_date_end)
        self.assertEqual(
            self.task_5.planned_date_end,
            self.task_5_planned_date_end + relativedelta(days=1, hour=9),
            failed_message,
        )
    ...
    def test_auto_shift_cascading_backward(self):
        ...
        """
        new_task_6_planned_date_begin = self.task_5_planned_date_begin + timedelta(
            hours=1
        )
        self.task_6.write(
            {
                "planned_date_begin": new_task_6_planned_date_begin,
                "planned_date_end": new_task_6_planned_date_begin
                + (self.task_6_planned_date_end - self.task_6_planned_date_begin),
            }
        )
        failed_message = (
        ...
        self.assertEqual(
            self.task_5.planned_date_end, new_task_6_planned_date_begin, failed_message
        )
        self.assertEqual(
            self.task_5.planned_date_begin,
            datetime(year=2021, month=6, day=29, hour=9),
            failed_message,
        )
        self.assertEqual(self.task_4.planned_date_end, self.task_5.planned_date_begin)
        self.assertEqual(
            self.task_4.planned_date_begin,
            datetime(year=2021, month=6, day=28, hour=16),
            failed_message,
        )
    ...
    def test_auto_shift_project_user(self):
        ...
        self.task_6.with_user(self.user_projectuser).with_context(
