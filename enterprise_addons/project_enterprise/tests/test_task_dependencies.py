...
class TestTaskDependencies(TestProjectCommon):
    def test_task_dependencies_display_warning_dependency_in_gantt(self):
        ...
        self.assertTrue(
            self.task_1.display_warning_dependency_in_gantt,
            "display_warning_dependency_in_gantt should be True if the task stage is neither closed or fold",
        )
        self.task_1.write({"stage_id": close_staged.id})
        self.assertFalse(
            self.task_1.display_warning_dependency_in_gantt,
            "display_warning_dependency_in_gantt should be False if the task stage is closed",
        )
        self.task_1.write({"stage_id": fold_stage.id})
        self.assertFalse(
            self.task_1.display_warning_dependency_in_gantt,
            "display_warning_dependency_in_gantt should be False if the task stage is fold",
        )
