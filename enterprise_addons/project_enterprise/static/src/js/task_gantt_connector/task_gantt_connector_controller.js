...
const TaskGanttConnectorController = TaskGanttController.extend({
  custom_events: Object.assign({}, TaskGanttController.prototype.custom_events, {
  ...
  _onDisplayMilestonePopover: function (ev) {
    Object.assign(ev.data.popoverData, {
  ...
  async _onRescheduleTask(ev) {
    ev.stopPropagation();
    const result = await this.model.rescheduleTask(
      ev.data.direction,
      ev.data.masterTaskId,
      ev.data.slaveTaskId
    );
