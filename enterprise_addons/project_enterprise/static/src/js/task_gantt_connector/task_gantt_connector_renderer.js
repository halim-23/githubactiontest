...
const TaskGanttConnectorRenderer = TaskGanttRenderer.extend(WidgetAdapterMixin, {
  ...
  async start() {
    ...
    this._connectorContainerComponent = new ComponentWrapper(
      this,
      ConnectorContainer,
      this._getConnectorContainerProps()
    );
    this._throttledReRender = _.throttle(async () => {
      if (this._shouldRenderConnectors()) {
        await this._connectorContainerComponent.update(
          this._generateAndGetConnectorContainerProps()
        );
      }
    }, 100);
  },
  ...
  _generateConnectors() {
    ...
    for (const row of this.state.rows) {
      ...
      const rowElementSelector = `${
        this._connectorsCssSelectors.groupByNoGroup
      }[data-row-id="${row.id.replace(/["\\]/g, "\\$&")}"]`;
    ...
    for (const record of this.state.records) {
      const taskConnectors = this._generateConnectorsForTask(
        record,
    ...
    for (const masterTaskId of task.depend_on_ids) {
      if (masterTaskId in this._rowsAndRecordsDict.records) {
        ...
        for (const taskRowId in this._rowsAndRecordsDict.records[task.id].rowsInfo) {
          for (const masterTaskRowId in this._rowsAndRecordsDict.records[masterTaskId]
            .rowsInfo) {
            /**
             *   Having:
             *      * B dependent on A
            ...
            if (
              ...
              Object.keys(this._rowsAndRecordsDict.records[task.id].rowsInfo).every(
                (rowId) =>
                  masterTaskRowId !== rowId &&
                  masterTaskId in this._rowsAndRecordsDict.rows[rowId].records
              ) ||
              Object.keys(
                this._rowsAndRecordsDict.records[masterTaskId].rowsInfo
              ).every(
                (rowId) =>
                  taskRowId !== rowId &&
                  task.id in this._rowsAndRecordsDict.rows[rowId].records
              )
            ) {
              connectors.push(
  ...
  _generateConnector(
    ...
    const masterTaskPill = this._rowsAndRecordsDict.rows[masterTaskRowId].records[
      masterTask.id
    ].pillElement;
    const taskPill = this._rowsAndRecordsDict.rows[taskRowId].records[task.id]
      .pillElement;
    let source = this._connectorContainerComponent.componentRef.comp.getAnchorsPositions(
      masterTaskPill
    );
    let target = this._connectorContainerComponent.componentRef.comp.getAnchorsPositions(
      taskPill
    );
    ...
    let specialColors;
    if (
      masterTask.display_warning_dependency_in_gantt &&
  ...
  _getConnectorCreatorInfo(element) {
    ...
    if (element.matches(this._connectorsCssSelectors.pill)) {
      pill = element;
      connectorCreators = Array.from(
        element.parentElement.querySelectorAll(this._connectorsCssSelectors.wrapper)
      );
  ...
  _getPillForTaskId(taskId) {
    return this.el.querySelector(
      `${this._connectorsCssSelectors.pill}[data-id="${taskId}"]`
    );
  },
  /**
  ...
  _isConnectorCreatorDragged(connectorCreatorInfo) {
    return (
      this._connectorInCreation &&
      this._connectorInCreation.data.sourceElement.dataset.id ===
        connectorCreatorInfo.pill.dataset.id
    );
  },
  /**
   * @override
  ...
  _triggerConnectorHighlighting(connector, highlighted) {
    this.trigger_up("on_connector_highlight", {
      connector: connector,
  ...
  _triggerPillHighlighting(element, highlighted) {
    this.trigger_up("on_pill_highlight", {
      element: element,
      highlighted: highlighted,
    });
  },
  ...
  toggleConnectorHighlighting(connector, highlighted) {
    const masterTaskPill = this._rowsAndRecordsDict.rows[connector.data.masterTaskRowId]
      .records[connector.data.masterTaskId].pillElement;
    const taskPill = this._rowsAndRecordsDict.rows[connector.data.taskRowId].records[
      connector.data.taskId
    ].pillElement;
  ...
  async togglePillHighlighting(element, highlighted) {
    ...
    if (connectorCreatorInfo.pill.dataset.id != 0) {
      const connectedConnectors = Object.values(this._connectors).filter(
        (connector) => {
          const ids = [connector.data.taskId, connector.data.masterTaskId];
          return ids.includes(parseInt(connectorCreatorInfo.pill.dataset.id));
        }
      );
  ...
  async _onConnectorCreationAbort(ev) {
    ...
    const connectorCreatorInfo = this._getConnectorCreatorInfo(
      ev.data.data.sourceElement
    );
  ...
  async _onConnectorCreationDone(ev) {
    ev.stopPropagation();
    this._connectorInCreation = null;
    const connectorSourceCreatorInfo = this._getConnectorCreatorInfo(
      ev.data.data.sourceElement
    );
    const connectorTargetCreatorInfo = this._getConnectorCreatorInfo(
      ev.data.data.targetElement
    );
    this.trigger_up("on_connector_end_drag");
  ...
  async _onConnectorRemoveButtonClick(ev) {
    ...
    this.trigger_up("on_remove_connector", {
  ...
  onConnectorRescheduleLaterButtonClick(ev) {
    ev.stopPropagation();
    const payload = ev.data;
    this.trigger_up("on_reschedule_task", {
      direction: "forward",
      masterTaskId: payload.data.masterTaskId,
      slaveTaskId: payload.data.taskId,
    });
