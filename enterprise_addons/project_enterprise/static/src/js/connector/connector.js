...
class Connector extends Component {
  ...
  _refreshPropertiesFromProps(props) {
    ...
    const pathInfo = this._getPathInfo(
      props.source,
      props.target,
  ...
  _getPathInfo(source, target, slackness) {
    ...
    const xDelta =
      100 + (directionFactor * (target.left - source.left) * slackness) / 10;
    const yDelta =
      Math.abs(source.top - target.top) < 16 && source.left > target.left
        ? 15 + 0.001 * (source.left - target.left) * slackness
        : 0;
