...
class ConnectorContainer extends Component {
  ...
  setup() {
    ...
    if ("defaultStyle" in this.props) {
      this.newConnector = deepMerge(this.newConnector, {
        style: this.props.defaultStyle,
      });
    }
    if ("newConnectorStyle" in this.props) {
      this.newConnector = deepMerge(this.newConnector, {
        style: this.props.newConnectorStyle,
      });
    }
  ...
  _addParentListeners() {
    ...
    this.parentElement.addEventListener(
      "mousemove",
      this._throttledOnParentMouseOverHandler
    );
  ...
  _createsParentListenerHandlers() {
    this._throttledOnParentMouseOverHandler = _.throttle(
      (ev) => this._onParentMouseOver(ev),
      50
    );
  ...
  _getElementPosition(element) {
    ...
    for (
      ...
      el != null && el !== this.el.parentElement;
      el = el.offsetParent
    ) {
  ...
  _refreshPropertiesFromProps(props) {
    ...
    if (this.props.defaultStyle) {
      Object.keys(this.connectors).forEach((key) => {
        this.connectors[key].style = deepMerge(
          this.props.defaultStyle,
          this.connectors[key].style
        );
    ...
    const isHoveredConnectorPartOfProps =
      isHoveredConnectorSet &&
      this.mouseEventsInfo.hoveredConnector.dataset.id in this.connectors;
  ...
  _updateConnectorHoverState(id, hovered) {
    this.connectors[id].hovered =
      hovered &&
      !(this.props.preventHoverEffect || this.mouseEventsInfo.isParentDragging);
    if (hovered) {
  ...
  _onParentMouseUp(ev) {
    if (this.mouseEventsInfo.isParentDragging === true) {
      ev.stopPropagation();
      ev.preventDefault();
      const connector_target = ev.target.closest(
        this.props.targetQuerySelector || this.props.sourceQuerySelector
      );
      if (connector_target) {
        this.newConnector = deepMerge(this.newConnector, {
          data: {
  ...
  _onMouseOut(ev) {
    ...
    this._updateConnectorHoverState(
      this.mouseEventsInfo.hoveredConnector.dataset.id,
      false
    );
    this.trigger(
      "connector-mouseout",
      this.connectors[this.mouseEventsInfo.hoveredConnector.dataset.id]
    );
    delete this.mouseEventsInfo.hoveredConnector;
  }
  ...
  _onMouseOver(ev) {
    ...
    if (
      !(target.dataset.id in this.connectors) ||
      Object.is(this.connectors[target.dataset.id], this.newConnector)
    ) {
