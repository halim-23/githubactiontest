...
function getConnectorsDict(gantt) {
  ...
  for (const connector of Object.values(gantt.renderer._connectors)) {
    ...
    const masterTaskUserId =
      JSON.parse(connector_data.masterTaskRowId)[0].user_ids[0] || 0;
...
QUnit.test("Connectors are correctly computed and rendered.", async function (assert) {
  ...
  for (const [test_key, colorCode] of Object.entries(tests)) {
    ...
    assert.ok(
      test_key in connectorsDict,
      `Connector between task ${masterTaskId} from group user ${masterTaskUserId} and task ${taskId} from group user ${taskUserId} should be present.`
    );
    ...
    if (colorCode === "n") {
      ...
      connectorPropsColorMatch =
        !connectorsDict[test_key].style ||
    ...
    const connector_stroke = connectorContainer.querySelector(
      `${CSS.SELECTOR.CONNECTOR}[data-id="${connectorsDict[test_key].id}"] ${CSS.SELECTOR.CONNECTOR_STROKE}`
    );
  ...
  assert.notOk(
    Object.keys(connectorsDictCopy).length,
    "There should not be more connectors than expected."
  );
  assert.equal(
    gantt.el.querySelectorAll(CSS.SELECTOR.CONNECTOR).length,
    Object.keys(tests).length,
    "All connectors should be rendered."
  );
});
...
QUnit.test(
  ...
  async function (assert) {
    ...
    const connectorContainer = gantt.el.querySelector(CSS.SELECTOR.CONNECTOR_CONTAINER);
    let connector = connectorContainer.querySelector(
      `${CSS.SELECTOR.CONNECTOR}[data-id="1"]`
    );
    let connector_stroke = connector.querySelector(CSS.SELECTOR.CONNECTOR_STROKE);

    assert.notOk(
      connector.classList.contains(CSS.CLASS.CONNECTOR_HOVERED),
      "Connectors that are not hovered don't contain the o_connector_hovered class."
    );
    assert.equal(
      connector_stroke.getAttribute("stroke"),
      gantt.renderer._connectorsStrokeColors.stroke
    );
    await testUtils.dom.triggerMouseEvent(connector_stroke, "mouseover");
    await testUtils.nextTick();
    await testUtils.returnAfterNextAnimationFrame();
    connector = connectorContainer.querySelector(
      `${CSS.SELECTOR.CONNECTOR}[data-id="1"]`
    );
    connector_stroke = connector.querySelector(CSS.SELECTOR.CONNECTOR_STROKE);
    assert.ok(
      connector.classList.contains(CSS.CLASS.CONNECTOR_HOVERED),
      "Hovered connectors contain the o_connector_hovered class"
    );
    assert.equal(
      connector_stroke.getAttribute("stroke"),
      gantt.renderer._connectorsStrokeColors.hoveredStroke
    );
  }
);
...
QUnit.test("Buttons are displayed when hovering a connector.", async function (assert) {
  ...
  const connectorContainer = gantt.el.querySelector(CSS.SELECTOR.CONNECTOR_CONTAINER);
  const connector = connectorContainer.querySelector(
    `${CSS.SELECTOR.CONNECTOR}[data-id="1"]`
  );
  const connector_stroke = connector.querySelector(CSS.SELECTOR.CONNECTOR_STROKE);

  assert.ok(
    connector.querySelector(CSS.SELECTOR.CONNECTOR_STROKE_BUTTON) === null,
  ...
  ganttViewParams.mockRPCHook = (route, args) => {
    if (args.model === "project.task") {
      if (args.method === "action_reschedule" || args.method === "write") {
        ...
        assert.step(
          `${args.method}|${rpc_arg1}|${JSON.stringify(rpc_arg2)}${
            rpc_arg3 ? `|${rpc_arg3}` : ""
          }`
        );
...
QUnit.test("Correct RPC is called on connector buttons click.", async function (
  assert
) {
...
QUnit.test(
  "Hovering a task pill, all the pills of the same task, and their related connectors are highlighted.",
  async function (assert) {
    /**
     * This test checks that:
    ...
    for (const taskPill of taskPills) {
      ...
      assert.notOk(
        taskPill.classList.contains(CSS.CLASS.PILL_HIGHLIGHT),
        "Pills should not be highlighted by default."
      );
      // Check that connector creators (little pills antennas) are not displayed.
      assert.ok(
        taskPill.parentElement.querySelectorAll(
          `${CSS.SELECTOR.CONNECTOR_CREATOR_WRAPPER}${CSS.SELECTOR.INVISIBLE}`
        ).length === 2,
        "Connector creators should be hidden by default."
      );
    }
    // Check that all connectors are not in hover state.
    for (const test_key in tests) {
      const connector_stroke = connectorContainer.querySelector(
        `${CSS.SELECTOR.CONNECTOR}[data-id="${connectorsDict[test_key].id}"]`
      );
      assert.notOk(
        connector_stroke.classList.contains(CSS.CLASS.CONNECTOR_HOVERED),
        "Connectors should not be in hovered state by default"
      );
    }
    ...
    for (const taskPill of taskPills) {
      ...
      assert.ok(
        taskPill.classList.contains(CSS.CLASS.PILL_HIGHLIGHT),
        "Pills should be highlighted when hovered (or when pill of the same id is hovered (m2m grouping))."
      );
      // Check that connector creators (little pills antennas) are displayed (and only displayed) on the hovered pills.
      const querySelector = `${CSS.SELECTOR.CONNECTOR_CREATOR_WRAPPER}${
        taskPill != taskPills[0]
          ? CSS.SELECTOR.INVISIBLE
          : `:not(${CSS.SELECTOR.INVISIBLE})`
      }`;
      assert.ok(
        taskPill.parentElement.querySelectorAll(querySelector).length === 2,
        "Connector creators should be displayed on the hovered pills and not on the others."
      );
    }
    ...
    for (const [test_key, hoverEffectExpected] of Object.entries(tests)) {
      ...
      if (hoverEffectExpected === "y") {
        assert.ok(
          connector.classList.contains(CSS.CLASS.CONNECTOR_HOVERED),
          "Connectors that are connected to an highlighted pill should be in a hover state."
        );
      } else {
        assert.notOk(
          connector.classList.contains(CSS.CLASS.CONNECTOR_HOVERED),
          "Connectors that are not connected to an highlighted pill should not be in a hover state."
        );
      }
      assert.ok(
        connector.querySelector(CSS.SELECTOR.CONNECTOR_STROKE_BUTTON) === null,
        "Connectors that are not hovered don't display buttons, even if they are highlighted."
      );
    }
  }
);
...
QUnit.test(
  ...
  async function (assert) {
    ...
    const connector_stroke = gantt.el.querySelector(
      `${CSS.SELECTOR.CONNECTOR}[data-id="1"] ${CSS.SELECTOR.CONNECTOR_STROKE}`
    );
    let taskPills = gantt.el.querySelectorAll(
      `${CSS.SELECTOR.PILL}:not(.${CSS.CLASS.PILL_HIGHLIGHT})`
    );
    ...
    taskPills = gantt.el.querySelectorAll(
      `${CSS.SELECTOR.PILL}.${CSS.CLASS.PILL_HIGHLIGHT}`
    );
    assert.ok(
      taskPills.length === 2,
      "Pills should be highlighted when linked connector is hovered."
    );
    // Check that connector creators (little pills antennas) are displayed (and only displayed) on the hovered pills.
    const querySelector = `${CSS.SELECTOR.CONNECTOR_CREATOR_WRAPPER}:not(${CSS.SELECTOR.INVISIBLE})`;
    assert.ok(
      gantt.el.querySelectorAll(querySelector).length === 0,
      "Connector creators should not be displayed if the pill is not hovered."
    );
  }
);

QUnit.test("Connectors are displayed behind pills, except on hover.", async function (
  assert
) {
  assert.expect(2);
...
QUnit.test("Create a connector from the gantt view.", async function (assert) {
  ...
  const connectorCreator = taskPill.parentElement.querySelector(
    `${CSS.SELECTOR.CONNECTOR_CREATOR_WRAPPER}:not(${CSS.SELECTOR.INVISIBLE}) ${CSS.SELECTOR.CONNECTOR_CREATOR_BULLET}`
  );
  ...
  assert.verifySteps(
    ['write|2|{"depend_on_ids":[[4,1,false]]}'],
    "Connector ui creation from task 1 to task 2 should result in an rpc call on project.task write."
  );
});
