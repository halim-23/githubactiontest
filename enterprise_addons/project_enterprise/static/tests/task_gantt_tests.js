...
QUnit.test(
  "user_ids grouped: specific empty group added, even if no records",
  async (assert) => {
    assert.expect(1);
...
QUnit.test("[user_ids, ...] grouped", async (assert) => {
  ...
  const gantt = await createView({
    ...ganttViewParams,
    groupBy: ["user_ids", "stuff_id"],
  });
  registerCleanup(gantt.destroy);

  assert.deepEqual(
    [
      ...gantt.el.querySelectorAll(".o_gantt_row_container .o_gantt_row_sidebar"),
...
QUnit.test('Empty groupby "Assigned To" and "Project" can be rendered', async function (
  assert
) {
  ganttViewParams.data = {
    task: {
