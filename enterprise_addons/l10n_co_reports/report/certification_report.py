...
class CertificationReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        current_date = (
            fields.Datetime.to_datetime(data["wizard_values"].get("declaration_date"))
            or datetime.now()
        )
        return {
            "docs": docs,
            ...
            "current_year": self.env.company.compute_fiscalyear_dates(current_date)[
                "date_from"
            ].year,
        }
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for aml in amls:
            if previous_partner_id != aml.partner_id:
                partner_lines = self._generate_lines_for_partner(
                    previous_partner_id, lines_per_group, options
                )
                if partner_lines:
                    lines += partner_lines
                    lines_per_group = {}
        ...
        lines += self._generate_lines_for_partner(
            previous_partner_id, lines_per_group, options
        )
...
class ReportCertificationReportIca(models.AbstractModel):
    ...
    def _get_report_name(self):
        return "Retención en ICA"

    def _get_columns_name(self, options):
        return [
            {"name": "Nombre"},
            {"name": "Bimestre"},
            {"name": "Monto del pago sujeto a retención", "class": "number"},
...
class ReportCertificationReportIva(models.AbstractModel):
    ...
    def _get_report_name(self):
        return "Retención en IVA"

    def _get_columns_name(self, options):
        return [
            {"name": "Nombre"},
            {"name": "Bimestre"},
            {"name": "Monto Total Operación", "class": "number"},
            {"name": "Monto del Pago Sujeto Retención", "class": "number"},
