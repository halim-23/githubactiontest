...
QUnit.module("Views", (hooks) => {
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      assert.strictEqual(
        tooltipInfo.name,
        "my_field",
        "the help value should be `my_field`"
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        tooltipInfo.help,
        'test " 3',
        'the help value should be `test " 3`'
      );
    }
  );
  ...
  QUnit.test("basic rendering of a pie chart widget", async function (assert) {
    ...
    const chartTitle = dashboard.el.querySelector(
  ...
  QUnit.test("rendering of a pie chart widget and comparison active", async function (
    assert
  ) {
    // Pie Chart is rendered asynchronously.
    // concurrency.delay is a fragile way that we use to wait until the
  ...
  QUnit.test("basic rendering of a graph tag", async function (assert) {
    ...
    assert.containsNone(
      dashboard,
      ".o_subview .o_graph_view .o_control_panel .o_cp_top_left"
    );
    assert.containsNone(
      dashboard,
      ".o_subview .o_graph_view .o_control_panel .o_cp_top_right"
    );
    assert.containsOnce(
      dashboard,
  ...
  QUnit.test("basic rendering of a pivot tag", async function (assert) {
    ...
    assert.containsOnce(
      dashboard,
      ".o_subview .o_pivot",
      "should have rendered a pivot view"
    );
    ...
    async function (assert) {
      ...
      patchWithCleanup(session, {
        ...
        user_companies: Object.assign({}, session.user_companies, {
          allowed_companies: Object.assign(
            {},
  ...
  QUnit.test("rendering of an aggregate with value label", async function (assert) {
    serverData.models.test_report.fields.days = {
  ...
  QUnit.test("rendering of formula with widget attribute (formatter)", async function (
    assert
  ) {
    const dashboard = await makeView({
      type: "dashboard",
  ...
  QUnit.test("rendering of aggregates with domain attribute", async function (assert) {
    ...
    assert.strictEqual(
      dashboard.el
        .querySelector(".o_aggregate[name=untaxed] .o_value")
        .textContent.trim(),
      "30.00",
      "should correctly display the aggregate's value"
    );
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      assert.strictEqual(
        dashboard.el
          .querySelector(".o_aggregate[name=sold] .o_value")
          .textContent.trim(),
        "8.00",
        "should correctly display the aggregate's value"
      );
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      serverData.models.test_report.records.forEach(
  ...
  QUnit.test("open a graph view fullscreen", async function (assert) {
    ...
    await click(
      webClient.el.querySelector(".o-web-dashboard-view-wrapper--switch-button")
    );
    await nextTick();
    assert.strictEqual(
  ...
  QUnit.test("interact with a graph view and open it fullscreen", async function (
    ...
    // open graph in fullscreen
    await click(
      dashboard.el.querySelector(".o-web-dashboard-view-wrapper--switch-button")
    );
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      await toggleMenu(
        dashboard.el.querySelector(".o_subview[type='graph']"),
        "Measures"
      );
      assert.containsOnce(
        dashboard,
      ...
      await toggleMenu(
        dashboard.el.querySelector(".o_subview[type='pivot']"),
        "Measures"
      );
      assert.containsOnce(
        dashboard,
  ...
  QUnit.test("open subview fullscreen, update domain and come back", async function (
    assert
  ) {
    assert.expect(8);
  ...
  QUnit.test("getContext correctly returns graph subview context", async function (
    assert
  ) {
    assert.expect(2);
  ...
  QUnit.test("getContext correctly returns pivot subview context", async function (
    assert
  ) {
    assert.expect(2);
  ...
  QUnit.test("getContext correctly returns cohort subview context", async function (
    assert
  ) {
    assert.expect(2);
  ...
  QUnit.test("correctly uses graph_ keys from the context (at reload)", async function (
    assert
  ) {
    assert.expect(5);
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      await click(
        dashboard.el.querySelectorAll(".o_aggregate")[1].querySelector(".o_value")
      );
      await toggleMenu(
        dashboard.el.querySelector(".o_subview[type='graph']"),
        "Measures"
  ...
  QUnit.test("rendering of aggregate with widget attribute (widget)", async function (
    assert
  ) {
    assert.expect(3);
  ...
  QUnit.test(
    "basic rendering of a date aggregate and empty result set",
    async function (assert) {
      assert.expect(1);
  ...
  QUnit.test(
    ...
    async function (assert) {
      ...
      const tooltipInfo = JSON.parse(agg.dataset.tooltipInfo);
      assert.strictEqual(
        serverData.models.test_time_range.fields.categ_id.type,
        "many2one"
      );
  ...
  QUnit.test("non empty dashboard view with sub views and sample data", async function (
    assert
  ) {
    assert.expect(9);
  ...
  QUnit.test(
    "dashboard statistic support wowl field with comparison",
    async (assert) => {
      assert.expect(31);
