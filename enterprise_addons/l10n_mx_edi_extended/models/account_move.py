...
class AccountMove(models.Model):
    ...
    def _get_l10n_mx_edi_issued_address(self):
        ...
        self.ensure_one()
        return (
            self.journal_id.l10n_mx_address_issued_id
            or super()._get_l10n_mx_edi_issued_address()
        )
        ...
        vals.update(
            {
                ...
                "ext_trade_certificate_key": external_trade_node.get(
                    "ClaveDePedimento", ""
                ),
                "ext_trade_certificate_source": external_trade_node.get(
                    "CertificadoOrigen", ""
                )
                .replace("0", "No")
                .replace("1", "Si"),
                "ext_trade_nb_certificate_origin": external_trade_node.get(
                    "CertificadoOrigen", ""
                ),
                "ext_trade_certificate_origin": external_trade_node.get(
                    "NumCertificadoOrigen", ""
                ),
                "ext_trade_operation_type": external_trade_node.get(
                    "TipoOperacion", ""
                ).replace("2", "Exportación"),
                "ext_trade_subdivision": external_trade_node.get("Subdivision", ""),
                "ext_trade_nb_reliable_exporter": external_trade_node.get(
    ...
    def _compute_l10n_mx_edi_external_trade(self):
        for move in self:
            if move.l10n_mx_edi_cfdi_request == "on_invoice":
                move.l10n_mx_edi_external_trade = (
                    move.partner_id.l10n_mx_edi_external_trade
                )
            else:
...
class AccountMoveLine(models.Model):
    ...
    def _auto_init(self):
        if not column_exists(
            self.env.cr, "account_move_line", "l10n_mx_edi_umt_aduana_id"
        ):
            create_column(
                self.env.cr, "account_move_line", "l10n_mx_edi_umt_aduana_id", "int4"
            )
    ...
    def _compute_l10n_mx_edi_qty_umt(self):
        for line in self:
            ...
            if product_aduana_code == uom_aduana_code:
                ...
                line.l10n_mx_edi_qty_umt = round(
                    line.product_id.weight * line.quantity, 3
                )
            else:
    ...
    def _compute_l10n_mx_edi_price_unit_umt(self):
        for line in self:
            if line.l10n_mx_edi_qty_umt:
                line.l10n_mx_edi_price_unit_umt = round(
                    line.quantity * line.price_unit / line.l10n_mx_edi_qty_umt, 2
                )
            else:
    ...
    def _check_l10n_mx_edi_customs_number(self):
        ...
        for line in self:
            ...
            if any(
                not CUSTOM_NUMBERS_PATTERN.match(custom_number)
        ...
        raise ValidationError(
            _(
                ...
                invalid_message="\n".join(
                    "%s (id=%s)" % (line.l10n_mx_edi_customs_number, line.id)
