...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_get_invoice_cfdi_values(self, invoice):
        ...
        for line_vals in vals["invoice_line_vals_list"]:
            # Custom number
            line_vals["custom_numbers"] = line_vals[
                "line"
            ]._l10n_mx_edi_get_custom_numbers()
        ...
        if invoice.l10n_mx_edi_external_trade:
            ...
            vals["ext_trade_rate_usd_mxn"] = usd._convert(
                1.0, mxn, invoice.company_id, invoice.date
            )

            invoice_lines_gb_products = {}
            for line_vals in vals["invoice_line_vals_list"]:
                invoice_lines_gb_products.setdefault(line_vals["line"].product_id, [])
                invoice_lines_gb_products[line_vals["line"].product_id].append(
                    line_vals
                )
            ...
            for product, line_vals_list in invoice_lines_gb_products.items():
                price_unit_usd = invoice.currency_id._convert(
                    sum(
                        line_vals["line"].l10n_mx_edi_price_unit_umt
                ...
                ext_trade_goods_details.append(
                    {
                        "product": product,
                        "quantity_aduana": sum(
                            line_vals["line"].l10n_mx_edi_qty_umt
                            for line_vals in line_vals_list
                        ),
            ...
            vals.update(
                {
                    ...
                    "ext_trade_delivery_partner": self.env["res.partner"].browse(
