...
class L10nMXEdiTariffFraction(models.Model):
    ...
    active = fields.Boolean(
        help="If the tariff fraction has expired it could be disabled to do not allow select the record.",
        default=True,
    )
    ...
    def name_get(self):
        # OVERRIDE
        return [
            (tariff.id, "%s %s" % (tariff.code, tariff.name or "")) for tariff in self
        ]
