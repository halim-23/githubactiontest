...
def post_init_hook(cr, registry):
        ...
        .search(
            [
                (
                    "l10n_mx_edi_code",
                    "in",
                    [v["l10n_mx_edi_code"] for v in res_city_vals_list],
                )
            ]
        )
    ...
    res_city_vals_list = [
        city
        for city in res_city_vals_list
        if city["l10n_mx_edi_code"] not in existing_codes
    ]
    ...
    if not env["l10n_mx_edi.res.locality"].search_count([]):
        csv_path = join(
            dirname(realpath(__file__)), "data", "l10n_mx_edi.res.locality.csv"
        )
    ...
    if not env["l10n_mx_edi.tariff.fraction"].search_count([]):
        csv_path = join(
            dirname(realpath(__file__)), "data", "l10n_mx_edi.tariff.fraction.csv"
        )
        tariff_fraction_vals_list = []
        with open(csv_path, "r") as csv_file:
            for row in csv.DictReader(
                csv_file, delimiter="|", fieldnames=["code", "name", "uom_code"]
            ):
                tariff_fraction_vals_list.append(row)

        tariff_fractions = env["l10n_mx_edi.tariff.fraction"].create(
            tariff_fraction_vals_list
        )
