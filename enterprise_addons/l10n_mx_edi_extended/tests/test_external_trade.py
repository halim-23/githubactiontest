...
class TestExternalTrade(TestMxExtendedEdiCommon):
    def test_invoice_cfdi_external_trade(self):
        with freeze_time(self.frozen_today):
            ...
            self.invoice.write(
                {
                    ...
                    "invoice_line_ids": [
                        (
                            0,
                            0,
                            {  # same as first line but different price and quantity and no discount, see TestMxExtendedEdiCommon.
                                "product_id": self.product.id,
                                "price_unit": 1000.0,
                                "quantity": 2,
                                "discount": 0,
                                "tax_ids": [
                                    (6, 0, (self.tax_16 + self.tax_10_negative).ids)
                                ],
                            },
                        ),
                        (
            ...
            self.assertEqual(
                values["ext_trade_goods_details"][0]["line_total_usd"], 24000
            )  # 1000*2 + 2000*5 wo discount and converted to USD (*4/2)
            self.assertEqual(
                values["ext_trade_goods_details"][1]["line_total_usd"], 1000
            )  # 250*2 wo discount and converted to USD (*4/2)
