odoo.define("data_cleaning.ListView", function (require) {
  ...
  var DataCleaningListController = DataCommonListController.extend({
    ...
    renderButtons: function ($node) {
      ...
      this.$buttons.on(
        "click",
        ".o_data_cleaning_validate_button",
        this._onValidateClick.bind(this)
      );
      this.$buttons.on(
        "click",
        ".o_data_cleaning_unselect_button",
    ...
    _onValidateClick: async function (ev) {
      ...
      if (this.isDomainSelected) {
        record_ids = await this._domainToResIds(
          state.getDomain(),
          session.active_ids_limit
        );
      } else {
