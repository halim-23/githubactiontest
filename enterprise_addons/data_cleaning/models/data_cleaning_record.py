...
class DataCleaningRecord(models.Model):
    ...
    rule_ids = fields.Many2many(
        "data_cleaning.rule", string="Rule", required=True, ondelete="cascade"
    )
    ...
    res_model_id = fields.Many2one(
        related="cleaning_model_id.res_model_id", store=True, readonly=True
    )
    res_model_name = fields.Char(
        related="cleaning_model_id.res_model_name", store=True, readonly=True
    )

    current_value = fields.Char("Current", compute="_compute_values", compute_sudo=True)
    suggested_value = fields.Char(
        "Suggested Value", compute="_compute_values", compute_sudo=True
    )
    suggested_value_display = fields.Char(
        "Suggested", compute="_compute_values", compute_sudo=True
    )
    ...
    def _render_value(self, current_value):
        ...
        render = [
            rule_id._action_to_python()
            for rule_id in self.rule_ids.sorted(key=lambda r: r.sequence)
        ]
        ...
        for record in self:
            ...
            if original_record:
                ...
                if any(rule_id.action == "trim" for rule_id in record.rule_ids):
                    ...
                    suggested_value_display = suggested_value_display.replace(
                        " ", "\u00a0"
                    ).replace("\t", "\u00a0")
                record.update(
                    {
    ...
    def action_validate(self):
        ...
        for record in self:
            ...
            original_record[0].update({field_name: record.suggested_value})
        records_done.unlink()
