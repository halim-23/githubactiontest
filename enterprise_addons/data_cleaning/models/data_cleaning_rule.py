...
class DataCleaningRule(models.Model):
    ...
    res_model_name = fields.Char(
        related="cleaning_model_id.res_model_name",
        string="Model Name",
        readonly=True,
        store=True,
    )
    ...
    def _compute_action(self):
        for rule in self:
            ...
            if action == "trim":
                ...
                action_display = "%s (%s)" % (
                    action_display,
                    dict(ACTIONS_TRIM).get(rule.action_trim),
                )
            elif action == "case":
                action = "%s_%s" % (action, rule.action_case)
                action_display = "%s (%s)" % (
                    action_display,
                    dict(ACTIONS_CASE).get(rule.action_case),
                )
    ...
    def _action_to_sql(self):
        ...
        for rule in self:
            ...
            if not operator or operator != "<>":
                ...
                field_actions.setdefault(
                    rule.name,
                    dict(rule_ids=[], field_id=rule.field_id.id, operator=operator),
                )
        ...
        if self.action == "phone":
            try:
                ...
                raise ValidationError(
                    _(
                        "The Python module `phonenumbers` is not installed. Format phone will not work."
                    )
                )
