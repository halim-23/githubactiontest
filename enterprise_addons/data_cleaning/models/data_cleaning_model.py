...
class DataCleaningModel(models.Model):
    ...
    rule_ids = fields.One2many(
        "data_cleaning.rule", "cleaning_model_id", string="Rules"
    )
    records_to_clean_count = fields.Integer(
        "Records To Clean", compute="_compute_records_to_clean"
    )
    ...
    def _compute_name(self):
        for cm_model in self:
            if not cm_model.name:
                cm_model.name = (
                    cm_model.res_model_id.name if cm_model.res_model_id else ""
                )
    ...
    def _compute_records_to_clean(self):
        ...
        counts = {
            cd["cleaning_model_id"][0]: cd["cleaning_model_id_count"]
            for cd in count_data
        }
        for cm_model in self:
            cm_model.records_to_clean_count = (
                counts[cm_model.id] if cm_model.id in counts else 0
            )
    ...
    def _clean_records_format_phone(self, actions, field):
        ...
        for record in records:
            ...
            formatted = self.env["data_cleaning.record"]._phone_format(
                record[field], record_country
            )
            if (
                (record.id, rule_ids[0]) not in existing_rows
                and formatted
                and record[field] != formatted
            ):
                result.append(
                    {
                        "res_id": record["id"],
        ...
        for cleaning_model in self:
            ...
            for field in actions:
                ...
                if cleaner:
                    ...
                    active_cond = (
                        sql.SQL("AND {}").format(sql.Identifier(active_name))
                        if active_name
                        else sql.SQL("")
                    )
                    ...
                    """
                        ...
                        table=sql.Identifier(
                            self.env[cleaning_model.res_model_name]._table
                        ),
                        ...
                        cleaning_record_table=sql.Identifier(
                            self.env["data_cleaning.record"]._table
                        ),
            ...
            if cleaning_model.cleaning_mode == "automatic":
                for records_to_create_batch in split_every(
                    ...
                    self.env["data_cleaning.record"].create(
    ...
    def _notify_records_to_clean(self):
        for cleaning_model in self.search([("cleaning_mode", "=", "manual")]):
            if (
                not cleaning_model.notify_user_ids
                or not cleaning_model.notify_frequency
            ):
                continue
            ...
            if (
                not cleaning_model.last_notification
                or (cleaning_model.last_notification + delta) < fields.Datetime.now()
            ):
    ...
    def _send_notification(self, delta):
        ...
        records_count = self.env["data_cleaning.record"].search_count(
            [("cleaning_model_id", "=", self.id), ("create_date", ">=", last_date)]
        )
    ...
    def write(self, vals):
        if "active" in vals and not vals["active"]:
            self.env["data_cleaning.record"].search(
                [("cleaning_model_id", "in", self.ids)]
            ).unlink()
    ...
    def open_records(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "data_cleaning.action_data_cleaning_record"
        )
        action["context"] = dict(
            ast.literal_eval(action.get("context")),
            searchpanel_default_cleaning_model_id=self.id,
        )
        return action
