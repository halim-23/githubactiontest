...
class IntrastatReport(models.AbstractModel):
    ...
    def get_csv(self, options):
        ...
        line_map = {
            l.id: l
            for l in self.env["account.move.line"].browse(
                res["id"] for res in query_res
            )
        }
        ...
        software_version = (
            software_version.replace("saas~", "").replace("+e", "").replace("alpha", "")
        )
        ...
        if fields.Date.to_date(date_to) < date(2022, 1, 1):
            if all(len(str(res["transaction_code"])) == 1 for res in query_res):
                ...
                all(len(str(res["transaction_code"])) == 2 for res in query_res)
        ...
        file_content = (
            "".join(
                [
                    ...
                    vat
                    and vat[2:].replace(" ", "").ljust(12)
                    or "".ljust(12),  # VAT number            length=12
                    ...
                    company.phone
                    and company.phone.replace(" ", "")[:15].ljust(15)
                    or "".ljust(15),  # Telephone number      length=15
        ...
        for res in query_res:
            ...
            inv = line.move_id
            country_dest_code = (
                inv.partner_id.country_id and inv.partner_id.country_id.code or ""
            )
            country_origin_code = (
                inv.intrastat_country_id and inv.intrastat_country_id.code or ""
            )
            country = (
                country_origin_code if res["type"] == "Arrival" else country_dest_code
            )
            ...
            transaction_period = str(inv.invoice_date.year) + str(
            ...
            file_content += (
                "".join(
                    [
                        ...
                        "6"
                        if res["type"] == "Arrival"
                        else "7",  # Commodity flow        length=1
                        vat
                        and vat[2:].replace(" ", "").ljust(12)
                        or "".ljust(12),  # VAT number            length=12
                        ...
                        res["invoice_transport"]
                        or "3",  # Mode of transport     length=1
                        "0",  # Container             length=1
                        "00",  # Traffic region/port   length=2
                        "00",  # Statistical procedure length=2
                        " "
                        if new_codes
                        else res["transaction_code"]
                        or "1",  # Transaction (old)     length=1
                        (res["commodity_code"] or "")[:8].ljust(
                            8
                        ),  # Commodity code        length=8
                        "00",  # Taric                 length=2
                        mass >= 0 and "+" or "-",  # Mass sign             length=1
                        str(int(abs(mass))).zfill(
                            10
                        ),  # Mass                  length=10
                        "+",  # Supplementary sign    length=1
                        "0000000000",  # Supplementary unit    length=10
                        inv.move_type in ["in_invoice", "out_invoice"]
                        and "+"
                        ...
                        (inv.name or "")[-10:].ljust(
                            10
                        ),  # Administration number length=10
                        "".ljust(3),  # Reserve               length=3
                        " ",  # Correction items      length=1
                        "000",  # Preference            length=3
                        "".ljust(7),  # Reserve               length=7
                        res["transaction_code"] or "11"
                        if new_codes
                        else "",  # Transaction (new)     length=2
                        (res.get("partner_vat") or "QV999999999999").ljust(17)
                        if new_codes
                        else "",  # PartnerID (VAT No.)   length=17
                    ]
                )
                + "\n"
            )
