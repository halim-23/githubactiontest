...
class ProviderUPS(models.Model):
    ...
    ups_shipper_number = fields.Char(
        string="UPS Shipper Number", groups="base.group_system"
    )
    ups_access_number = fields.Char(string="UPS Access Key", groups="base.group_system")
    ups_default_package_type_id = fields.Many2one(
        "stock.package.type", string="UPS Package Type"
    )
    ups_default_service_type = fields.Selection(
        _get_ups_service_types, string="UPS Service Type", default="03"
    )
    ups_duty_payment = fields.Selection(
        [("SENDER", "Sender"), ("RECIPIENT", "Recipient")],
        required=True,
        default="RECIPIENT",
    )
    ups_package_weight_unit = fields.Selection(
        [("LBS", "Pounds"), ("KGS", "Kilograms")], default="LBS"
    )
    ups_package_dimension_unit = fields.Selection(
        [("IN", "Inches"), ("CM", "Centimeters")],
        string="Package Size Unit",
        default="IN",
    )
    ups_label_file_type = fields.Selection(
        [("GIF", "PDF"), ("ZPL", "ZPL"), ("EPL", "EPL"), ("SPL", "SPL")],
        string="UPS Label File Type",
        default="GIF",
    )
    ...
    ups_cod = fields.Boolean(
        string="Collect on Delivery",
    ...
    def ups_send_shipping(self, pickings):
        ...
        for picking in pickings:
                ...
                for package in picking.package_ids:
                    packages.append(
                        Package(
                            ...
                            quant_pack=package.package_type_id,
                            name=package.name,
                        )
                    )
            ...
            shipment_info = {
                ...
                "ilt_monetary_value": "%d"
                % sum(sml.sale_price for sml in picking.move_line_ids),
            ...
            if self.ups_bill_my_account:
                ups_carrier_account = picking.partner_id.with_company(
                    picking.company_id
            ...
            package_type = (
                ...
                or self.ups_default_package_type_id.shipper_package_code
            )
            srm.send_shipping(
                shipment_info=shipment_info,
                packages=packages,
    ...
    def ups_get_tracking_link(self, picking):
        return (
            ...
            % picking.carrier_tracking_ref.replace("+", "%0D%0A")
        )
