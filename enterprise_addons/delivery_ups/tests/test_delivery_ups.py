...
class TestDeliveryUPS(TransactionCase):
    ...
    def wiz_put_in_pack(self, picking):
        ...
        self.assertEqual(
            wiz_action["res_model"], "choose.delivery.package", "Wrong wizard returned"
        )
        wiz = (
    ...
    def test_01_ups_basic_flow(self):
        ...
        carrier.write(
            {"ups_default_service_type": "08", "ups_package_dimension_unit": "IN"}
        )
        carrier.ups_default_package_type_id.write(
            {"height": "3", "width": "3", "packaging_length": "3"}
        )

        so_vals = {"partner_id": self.agrolait.id, "order_line": [(0, None, sol_vals)]}

        sale_order = SaleOrder.create(so_vals)
        # I add delivery cost in Sales order
        delivery_wizard = Form(
            self.env["choose.delivery.carrier"].with_context(
                {"default_order_id": sale_order.id, "default_carrier_id": carrier.id}
            )
        )
        choose_delivery_carrier = delivery_wizard.save()
        choose_delivery_carrier.update_price()
        self.assertGreater(
            choose_delivery_carrier.delivery_price,
            0.0,
            "UPS delivery cost for this SO has not been correctly estimated.",
        )
        ...
        self.assertEqual(
            ...
            1,
            "The Sales Order did not generate a picking.",
        )
        ...
        self.assertEqual(
            ...
            "Carrier is not the same on Picking and on SO.",
        )

        picking.move_lines[0].quantity_done = 1.0
        self.assertGreater(
            picking.shipping_weight, 0.0, "Picking weight should be positive."
        )
        ...
        self.assertIsNot(
            ...
            False,
            "UPS did not return any tracking number",
        )
        self.assertGreater(
            picking.carrier_price, 0.0, "UPS carrying price is probably incorrect"
        )

        picking.cancel_shipment()
        self.assertFalse(
            picking.carrier_tracking_ref,
            "Carrier Tracking code has not been properly deleted",
        )
        self.assertEqual(
            picking.carrier_price, 0.0, "Carrier price has not been properly deleted"
        )
    ...
    def test_02_ups_multipackage_flow(self):
        ...
        carrier.write(
            {
                "ups_default_package_type_id": self.env.ref(
                    "delivery_ups.ups_packaging_30"
                ).id,
        ...
        self.assertEqual(
            ...
            2,
            "2 packages should have been created at this point",
        )
        self.assertGreater(
            picking.shipping_weight, 0.0, "Picking weight should be positive."
        )
    ...
    def test_03_ups_flow_from_delivery_order(self):
        ...
        self.assertEqual(
            delivery_order.state, "draft", "Shipment state should be draft."
        )
        ...
        self.assertEqual(
            ...
            "confirmed",
            "Shipment state should be waiting(confirmed).",
        )
        ...
        self.assertEqual(
            delivery_order.state,
            "assigned",
            "Shipment state should be ready(assigned).",
        )
    ...
    def test_04_backorder_and_track_number(self):
        ...
        carrier.ups_default_package_type_id.write(
            {"height": "1", "width": "1", "packaging_length": "1"}
        )
        ...
        delivery_wizard = Form(
            self.env["choose.delivery.carrier"].with_context(
                {"default_order_id": so.id, "default_carrier_id": carrier.id}
            )
        )
        choose_delivery_carrier = delivery_wizard.save()
        choose_delivery_carrier.update_price()
        ...
        self.assertEqual(
            pick01.carrier_tracking_ref,
            out01.carrier_tracking_ref + "," + out02.carrier_tracking_ref,
        )
