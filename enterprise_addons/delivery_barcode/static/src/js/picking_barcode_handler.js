odoo.define("delivery_barcode.PickingBarcodeHandler", function (require) {
  ...
  StockPickingBarcodeHandler.include({
    pre_onchange_hook: function (barcode) {
      var self = this;
      return this.try_put_in_pack_delivery(barcode).then(
        function () {
    ...
    try_put_in_pack_delivery: function (barcode) {
      ...
      var package_type_records =
        package_type_field.viewmanager.active_view.controller.records.records;
      var pack = _.find(package_type_records, function (pck) {
        return pck.get("barcode") === barcode;
      });
      if (pack) {
