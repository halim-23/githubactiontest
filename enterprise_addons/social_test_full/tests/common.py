...
class SocialTestFullCase(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        with patch.object(
            SocialAccountFacebook, "_compute_statistics", lambda x: None
        ), patch.object(
            SocialAccountTwitter, "_compute_statistics", lambda x: None
        ), patch.object(
            SocialAccountLinkedin, "_compute_statistics", lambda x: None
        ), patch.object(
            SocialAccountYoutube, "_compute_statistics", lambda x: None
        ), patch.object(
            ...
            "_create_default_stream_linkedin",
            lambda *args, **kwargs: None,
        ), patch.object(
            SocialAccountYoutube,
            ...
            cls.accounts = cls.env["social.account"].create(
                [
                    {"media_id": cls.env.ref(media_ref).id, "name": "Account"}
