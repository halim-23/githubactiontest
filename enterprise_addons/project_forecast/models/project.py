...
class Project(models.Model):
    ...
    total_forecast_time = fields.Integer(
        compute="_compute_total_forecast_time",
    ...
    def _compute_total_forecast_time(self):
        ...
        shifts_per_project = {
            res["project_id"][0]: int(round(res["allocated_hours"]))
            for res in shifts_read_group
        }
        for project in self:
    ...
    def _unlink_except_contains_plannings(self):
        if (
            ...
            .sudo()
            .search_count([("project_id", "in", self.ids)])
            > 0
        ):
            raise UserError(
                _(
                    "You cannot delete a project containing plannings. You can either delete all the project's forecasts and then delete the project or simply deactivate the project."
                )
            )
    ...
    def action_project_forecast_from_project(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "project_forecast.project_forecast_action_from_project"
        )
...
class Task(models.Model):
    ...
    allow_forecast = fields.Boolean(
        "Allow Planning",
        readonly=True,
        related="project_id.allow_forecast",
        store=False,
    )
    ...
    def _compute_forecast_hours(self):
        ...
        mapped_data = {
            f["task_id"][0]: f["allocated_hours"] for f in forecast_data
        }
        for task in self:
            hours = mapped_data.get(task.id, 0) + sum(
                mapped_data.get(child_task.id, 0)
    ...
    def _unlink_except_contains_plannings(self):
        if (
            self.env["planning.slot"].sudo().search_count([("task_id", "in", self.ids)])
            > 0
        ):
            raise UserError(
                _(
                    "You cannot delete a task containing plannings. You can either delete all the task's plannings and then delete the task or simply deactivate the task."
                )
            )
