...
class PlanningShift(models.Model):
    ...
    project_id = fields.Many2one(
        ...
        copy=True,
        check_company=True,
        group_expand="_read_group_project_id",
    ...
    def _read_group_project_id(self, projects, domain, order):
        ...
        if (
            self._context.get("planning_expand_project")
            and ("start_datetime", "<=") in dom_tuples
            and ("end_datetime", ">=") in dom_tuples
        ):
            if ("project_id", "=") in dom_tuples or (
                "project_id",
                "ilike",
            ) in dom_tuples:
            ...
            filters = expression.AND(
                [[("project_id.active", "=", True)], self._expand_domain_dates(domain)]
            )
    ...
    def _prepare_template_values(self):
        ...
        return {"project_id": self.project_id.id, "task_id": self.task_id.id, **result}
    ...
    def _compute_resource_id(self):
        for slot in self.filtered(
            lambda slot: not slot.resource_id and slot.task_id and slot.start_datetime
        ):
            slot.resource_id = slot.task_id.user_ids[:1].resource_ids.filtered(
                lambda r: r.company_id.id == slot.company_id.id
            )[:1]
