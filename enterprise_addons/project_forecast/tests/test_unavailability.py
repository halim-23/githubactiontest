...
class TestUnavailabilityForForecasts(TestCommonForecast):
    ...
    def setUpClass(cls):
        ...
        cls.employee_lionel = cls.env["hr.employee"].create(
            {"name": "lionel", "work_email": "lionel@a.be", "tz": "UTC"}
        )
    ...
    def test_gantt_unavailability_correctly_update_gantt_object_single_group_by(self):
        ...
        for resource in [
            self.resource_bert,
            self.resource_lionel,
            self.resource_joseph,
        ]:
            generated[resource.id] = self.env["planning.slot"].create(
        ...
        expected_unavailabilities = [
            {
                "start": datetime(2019, 1, 1, 17, 0, tzinfo=pytz.utc),
                "stop": datetime(2019, 1, 2, 8, 0, tzinfo=pytz.utc),
            },
            {
                "start": datetime(2019, 1, 2, 17, 0, tzinfo=pytz.utc),
                "stop": datetime(2019, 1, 3, 8, 0, tzinfo=pytz.utc),
            },
            {
                "start": datetime(2019, 1, 3, 17, 0, tzinfo=pytz.utc),
                "stop": datetime(2019, 1, 4, 8, 0, tzinfo=pytz.utc),
            },
            {
        ...
        self.assertEqual(
            bert_unavailabilities,
            expected_unavailabilities,
            "the gantt object was tranformed for bert",
        )
        self.assertEqual(
            lionel_unavailabilities,
            expected_unavailabilities,
            "the gantt object was tranformed for lionel",
        )
