...
class TestForecastAccessRights(TestCommonForecast):
    ...
    def setUpClass(cls):
        ...
        cls.employee_joseph.write({"user_id": cls.user_projectuser_joseph.id})
        # manager user
        ...
        cls.employee_bert.write({"user_id": cls.user_projectmanager_bert.id})
        ...
        cls.project_tinyhouse_forecast = cls.env["planning.slot"].create(
            {"project_id": cls.project_tinyhouse.id, **forecast_values}
        )
        cls.project_opera_forecast = cls.env["planning.slot"].create(
            {"project_id": cls.project_opera.id, **forecast_values}
        )
        cls.project_horizon_forecast = cls.env["planning.slot"].create(
            {"project_id": cls.project_horizon.id, **forecast_values}
        )
    ...
    def test_public_user_access_rights(self):
        ...
        # update
        with self.assertRaises(AccessError):
            self.project_opera_forecast.with_user(self.user_public.id).write(
                {"allocated_hours": 6}
            )
        # delete
        with self.assertRaises(AccessError):
    ...
    def test_portal_user_access_right(self):
        ...
        # update
        with self.assertRaises(AccessError):
            self.project_opera_forecast.with_user(self.user_portal.id).write(
                {"allocated_hours": 6}
            )
        # delete
        with self.assertRaises(AccessError):
    ...
    def test_regular_user_access_rights(self):
        ...
        # update
        with self.assertRaises(AccessError):
            self.project_opera_forecast.with_user(
                self.employee_joseph.user_id.id
            ).write({"allocated_hours": 6})
        # delete
        with self.assertRaises(AccessError):
            self.project_opera_forecast.with_user(
                self.employee_joseph.user_id.id
        ...
        with self.assertRaises(AccessError):
            self.project_tinyhouse_forecast.with_user(
                self.employee_joseph.user_id
            ).read()
    ...
    def test_manager_access_rights(self):
        ...
        # update
        self.project_opera_forecast.with_user(self.employee_bert.user_id.id).write(
            {"allocated_hours": 6}
        )
        # delete
