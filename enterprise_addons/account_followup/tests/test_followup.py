...
class TestAccountFollowupReports(TestAccountReportsCommon):
    ...
    def assertPartnerFollowup(self, partner, status, level):
        ...
        self.assertEqual(
            res.get(partner.id, {}).get("followup_level"), level.id if level else None
        )
    ...
    def test_followup_report(self):
        ...
        with freeze_time("2016-01-15"):
            ...
            self.assertPartnerFollowup(
                self.partner_a, "in_need_of_action", self.first_followup_level
            )
        ...
        sent_attachments = (
            ...
            .search([("partner_ids", "=", self.partner_a.id)])
            .attachment_ids
        )
        ...
        with freeze_time("2016-01-20"):
            self.assertPartnerFollowup(
                self.partner_a, "with_overdue_invoices", self.second_followup_level
            )
