...
Best Regards,
    ...
    email_subject = fields.Char(
        translate=True,
        default=lambda s: _("%(company_name)s Payment Reminder - %(partner_name)s"),
    )
    send_email = fields.Boolean(
        "Send an Email", help="When processing, it will send an email", default=True
    )
    print_letter = fields.Boolean(
        "Print a Letter", help="When processing, it will print a PDF", default=True
    )
    send_sms = fields.Boolean(
        "Send an SMS Message",
        help="When processing, it will send an sms text message",
        default=False,
    )
    ...
    manual_action_type_id = fields.Many2one(
        "mail.activity.type", "Manual Action Type", default=False
    )
    ...
    _sql_constraints = [
        (
            ...
            "Days of the follow-up levels must be different per company",
        ),
        (
            "uniq_name",
    ...
    def copy_data(self, default=None):
        ...
        if not default or "delay" not in default:
            ...
            higher_delay = (
                self.search(
                    [("company_id", "=", company_id)], order="delay desc", limit=1
                )[:1].delay
                or 0
            )
        ...
        for line in self:
            if line.email_subject:
                try:
                    ...
                    raise Warning(
                        _(
                            "Your email subject is invalid, use the right legend or %% if you want to use the percent character."
                        )
                    )
        ...
        for line in self:
            if line.sms_description:
                try:
                    ...
                    raise Warning(
                        _(
                            "Your sms description is invalid, use the right legend or %% if you want to use the percent character."
                        )
                    )
    ...
    def _get_next_date(self):
        self.ensure_one()
        next_followup = self.env["account_followup.followup.line"].search(
...
class AccountMoveLine(models.Model):
    ...
    followup_line_id = fields.Many2one(
        "account_followup.followup.line", "Follow-up Level", copy=False
    )
