...
class AccountFollowupReport(models.AbstractModel):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                date_due = format_date(
                    self.env, aml.date_maturity or aml.date, lang_code=lang_code
                )
                ...
                line_num += 1
                expected_pay_date = (
                    format_date(self.env, aml.expected_pay_date, lang_code=lang_code)
                    if aml.expected_pay_date
                    else ""
                )
            ...
            lines.append(
                {
                    ...
                    "columns": [
                        {"name": v}
                        for v in [""] * (3 if self.env.context.get("print_mode") else 5)
    ...
    def _get_html_render_values(self, options, report_manager):
        # OVERRIDE
        res = super(AccountFollowupReport, self)._get_html_render_values(
            options, report_manager
        )
            ...
            if summary:
                try:
                    summary = summary % {
                        ...
                        "date": format_date(
                            self.env, fields.Date.today(), lang_code=partner.lang
                        ),
                        "user_signature": html2plaintext(self.env.user.signature or ""),
                        "company_name": self.env.company.name,
                        "amount_due": formatLang(
                            self.env,
                            partner.total_due,
                ...
                return ""
        raise UserError(
            _("You need a least one follow-up level in order to process your follow-up")
        )
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        additional_context["today"] = fields.date.today().strftime(
            DEFAULT_SERVER_DATE_FORMAT
        )
        return super(AccountFollowupReport, self).get_html(
    ...
    def get_followup_informations(self, partner_id, options):
        ...
        if (
            followup_line
            and followup_line._amount_due_in_description()
            and options.get("total_due", -1) != partner.total_due
        ):
        ...
        if partner.followup_level:
            ...
            options["followup_level"] = (
                partner.followup_level.id,
                partner.followup_level.delay,
            )
        return infos
    ...
    def send_email(self, options):
        ...
        if email and email.strip():
            ...
            start_index = body_html.find(
                "<span>", body_html.find('<div class="o_account_reports_summary">')
            )
            end_index = (
                start_index > -1 and body_html.find("</span>", start_index) or -1
            )
            if end_index > -1:
        ...
        raise UserError(
            _(
                "Could not send mail to partner %s because it does not have any email address defined",
    ...
    def print_followups(self, records):
        ...
        """
        res_ids = (
            records["ids"] if "ids" in records else records.ids
        )  # records come from either JS or server.action
        action = self.env.ref("account_followup.action_report_followup").report_action(
            res_ids
        )
