odoo.define("accountReports.FollowupTests", function (require) {
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("followup form view get_followup_informations", async function (
        assert
      ) {
        assert.expect(2);
          ...
          mockRPC: function (route, args) {
            if (args.method === "get_followup_informations") {
              assert.strictEqual(
                args.args[0],
                9,
                "followup form view should call 'get_followup_informations' with id=9"
              );
              return Promise.resolve({
                ...
                html:
                  '<div class="o_account_reports_body"><div class="o_account_reports_page o_account_reports_no_print"></div></div>',
              });
            }
      ...
      QUnit.test("followup form view do it later", async function (assert) {
          ...
          mockRPC: function (route, args) {
            if (args.method === "update_next_action") {
              assert.strictEqual(
                args.args[0][0],
                9,
                "followup form view should call 'update_next_action' with id=9"
              );
              assert.strictEqual(
                args.args[1].next_action_type,
                "auto",
                "followup form view should call 'update_next_action' with next_action_type = auto"
              );
              return Promise.resolve();
            }
            if (args.method === "get_followup_informations") {
        ...
        var $buttonDoItLater = $buttons.find(
          "button.o_account_followup_do_it_later_button"
        );
        ...
        assert.strictEqual(
          $buttonDone.css("display"),
          "none",
          "Done button should be invisible"
        );
        assert.isVisible(
          followupFormView.$searchview,
          "The custom searchview should be visible on the form"
        );
        assert.strictEqual(
          followupFormView.$searchview
            .find("span.o_account-progress-bar-content")
            .text(),
          "0/1",
      ...
      QUnit.test("followup form print letter", async function (assert) {
        ...
        var followupFormView = await createView({
          ...
          mockRPC: function (route, args) {
            ...
            if (args.method === "update_next_action") {
              assert.ok(
                true,
                "Should call 'update_next_action' route when click on done"
              );
              return Promise.resolve({});
            }
            return this._super.apply(this, arguments);
        ...
        var $buttonPrintLetter = followupFormView.$buttons.find(
          "button.o_account_followup_print_letter_button"
        );
        assert.strictEqual(
          followupFormView.$buttons
            .find("button.o_account_followup_done_button")
            .css("display"),
          "none",
          "Done button should be invisible"
        );
        ...
        await testUtils.dom.click(
          followupFormView.$buttons.find("button.o_account_followup_done_button")
        );
      ...
      QUnit.test("followup form view multiple records", async function (assert) {
        ...
        assert.strictEqual(
          testUtils.controlPanel.getPagerValue(followupFormView),
          "1",
          "pager value should be 1"
        );
        assert.strictEqual(
          testUtils.controlPanel.getPagerSize(followupFormView),
          "4",
          "pager limit should be 4"
        );
        ...
        var $buttonDoItLater = followupFormView.$buttons.find(
          "button.o_account_followup_do_it_later_button"
        );
        await testUtils.dom.click($buttonDoItLater);
