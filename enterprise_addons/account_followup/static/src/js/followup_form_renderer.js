odoo.define("account_followup.FollowupFormRenderer", function (require) {
  ...
  var FollowupFormRenderer = FormRenderer.extend({
    events: _.extend({}, FormRenderer.prototype.events, {
      ...
      "click .o_account_reports_no_print .o_account_reports_email_subject":
        "_onEditEmailSubject",
    ...
    renderMailAlert: function () {
      this.$(".o_form_sheet_bg div.o_account_reports_page").prepend(
        QWeb.render("CustomerStatements.send_mail")
      );
    },
    /**
    ...
    renderSMSAlert: function () {
      this.$("div.o_account_reports_page").prepend(
        QWeb.render("CustomerStatements.send_sms")
      );
    },
    /**
    ...
    renderTrust: function (newTrust) {
      ...
      this.$("i.oe-account_followup-trust").attr(
        "style",
    ...
    _renderTagFollowup: function () {
      ...
      this.nextActionDatePicker
        .appendTo($element.find("div.o_account_reports_next_action_date_picker"))
        .then(function () {
          self.nextActionDatePicker.setValue(
            new moment(self.state.data.next_action_date)
          );
        });
    ...
    _onSaveEmailSubject: function (event) {
      var text = $(event.target)
        .siblings('input[name="email_subject"]')
        .val()
        .replace(/[ \t]+/g, " ");
