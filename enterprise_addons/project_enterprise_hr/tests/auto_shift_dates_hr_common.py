...
from odoo.addons.project_enterprise.tests.auto_shift_dates_common import (
    AutoShiftDatesCommon,
)
...
class AutoShiftDatesHRCommon(AutoShiftDatesCommon):
    ...
    def setUpClass(cls):
        ...
        cls.armande_employee_create_date = (
            cls.task_3_planned_date_begin
            - relativedelta(months=1, hour=12, minute=0, second=0, microsecond=0)
        )
        cls.env.cr.execute(
        ...
        cls.armande_departure_date = cls.task_1_planned_date_end.date() + relativedelta(
            day=29
        )  # 2021 06 25
