odoo.define("web_grid.grid_mobile_tests", function (require) {
  ...
  const GridView = require("web_grid.GridView");
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("basic grid view, range button in mobile", async function (assert) {
        ...
        const btnCal = grid.$buttons.find(
          ".btn-group.o_grid_range > button.btn.fa-calendar"
        );
        ...
        const btnRange = grid.$buttons.find(
          ".btn-group.o_grid_range button.grid_arrow_range"
        );
        assert.equal(
          btnRange.length,
          2,
          "should have two range buttons (Day and Week)"
        );
