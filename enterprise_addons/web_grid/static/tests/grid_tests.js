  ...
  QUnit.module(
    "Views",
    {
      beforeEach: function () {
        ...
        this.arch =
          ...
          "</grid>";
        this.archs = {
          "analytic.line,23,form":
            '<form string="Add a line"><group><group>' +
    ...
    function () {
      ...
      QUnit.test("basic grid view", async function (assert) {
        ...
        assert.containsN(grid, "div.o_grid_cell_container", 14, "should have 14 cells");
        assert.strictEqual(
          grid.$("div.o_grid_input:contains(2:30)").length,
          1,
        ...
        assert.strictEqual(
          grid.$("div.o_grid_input:contains(0:00)").length,
          12,
          "should have correctly parsed another float_time"
        );

        assert.ok(
          grid.$buttons.find("button.grid_arrow_previous").is(":visible"),
          "previous button should be visible"
        );
        assert.hasClass(
          grid.$buttons.find('button.grid_arrow_range[data-name="week"]'),
          "active",
          "current range is shown as active"
        );

        assert.strictEqual(
          grid.$("tfoot td:contains(2:30)").length,
          1,
          "should display total in a column"
        );
        assert.strictEqual(
          grid.$("tfoot td.o_grid_cell_null:contains(0:00)").length,
          5,
          "should display totals, even if 0"
        );

        await testUtils.dom.click(grid.$buttons.find("button.grid_arrow_next"));
        assert.ok(
          grid.$("div.o_grid_cell_container").length,
          "should not have any cells"
        );
        assert.ok(
          grid.$("th div:contains(P1)").length,
          "should have rendered a cell with project name"
        );
        assert.ok(
          grid.$("th div:contains(BS task)").length,
          "should have rendered a cell with task name"
        );
        assert.notOk(
          grid.$(".o_grid_nocontent_container p:contains(Add projects and tasks)")
            .length,
          "should not have rendered a no content helper"
        );
        assert.hasClass(
          grid.$("tbody tr:eq(0) td:eq(1) .o_grid_input"),
          "text-danger",
          "should have text-danger class on negative value cell"
        );
        assert.hasClass(
          grid.$("tbody tr:eq(0) td.o_grid_total"),
          "text-danger",
          "should have text-danger class on total column"
        );
        assert.hasClass(
          grid.$("tfoot tr:eq(0) td:eq(2)"),
          "text-danger",
          "should have text-danger class on footer total"
        );

        assert.notOk(
          grid.$("td.o_grid_current").length,
          "should not have any cell marked current"
        );
        await testUtils.dom.click(grid.$buttons.find("button.grid_arrow_next"));

        assert.notOk(
          grid.$("div.o_grid_cell_container").length,
          "should not have any cell"
        );
      ...
      QUnit.test("basic grouped grid view", async function (assert) {
        ...
        var grid = await createView({
          ...
          mockRPC: function (route, args) {
            if (route === "some-image") {
              ...
              assert.strictEqual(
                args.kwargs.groupby.length,
                1,
                "should read group on the section (project_id)"
              );
              assert.strictEqual(
                args.kwargs.groupby[0],
                "project_id",
                "should read group on the section (project_id)"
              );
              nbReadGroup++;
        ...
        assert.containsN(
          ...
          ".o_grid_section",
          2,
          "should have one section by project"
        );

        // First section
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) th:contains(P1)").length,
          1,
          "first section should be for project P1"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) div.o_grid_cell_container").length,
          14,
          "first section should have 14 cells"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) th:contains(Undefined)").length,
          1,
          "first section should have a row without task"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) th:contains(BS task)").length,
          1,
          "first section should have a row for BS task"
        );

        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) tr:eq(2) div.o_grid_input:contains(2:30)")
            .length,
          1,
          "should have correctly parsed a float_time for cell without task"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) div.o_grid_input:contains(0:00)").length,
          12,
          "should have correctly parsed another float_time"
        );
        ...
        // Second section
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) th:contains(Webocalypse Now)").length,
          1,
          "second section should be for project Webocalypse Now"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) th:contains(Another BS task)").length,
          0,
          "first section should be empty"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) div.o_grid_cell_container").length,
          0,
        ...
        assert.strictEqual(
          nbReadGridDomain,
          2,
          "should have read the grid domain again"
        );
      ...
      QUnit.test("load and reload a grid view with favorite search", async function (
        assert
      ) {
        assert.expect(4);
        ...
        var grid = await createView({
          ...
          data: this.data,
          arch:
            '<grid string="Timesheet By Project">' +
            '<field name="project_id" type="row" section="1"/>' +
            '<field name="task_id" type="row"/>' +
            '<field name="date" type="col">' +
            '<range name="week" string="Week" span="week" step="day"/>' +
        ...
        assert.strictEqual(
          grid.$("tr:eq(1) th").text(),
          "Another BS taskWebocalypse Now",
      ...
      QUnit.test("Removing groupBy defaults to initial groupings", async function (
        assert
      ) {
        assert.expect(6);
        ...
        assert.strictEqual(
          grid.$("tr:eq(3) th").text(),
          "Undefined",
          "Should be grouped by default (Project > Task)."
        );
        assert.strictEqual(
          grid.$("tr:eq(4) th").text(),
          "Webocalypse Now",
          "Should be grouped by default (Project > Task)."
        );
      ...
      QUnit.test("create analytic lines", async function (assert) {
        ...
        assert.strictEqual(
          grid.$("td.o_grid_total:contains(4:00)").length,
          1,
      ...
      QUnit.test("switching active range", async function (assert) {
        ...
        assert.strictEqual(
          grid.$("thead th:not(.o_grid_title_header)").length,
          8,
        ...
        assert.doesNotHaveClass(
          grid.$buttons.find('button.grid_arrow_range[data-name="month"]'),
          "active",
      ...
      QUnit.test(
        "clicking on the info icon on a cell triggers a do_action",
        async function (assert) {
          ...
          testUtils.mock.intercept(grid, "do_action", function (event) {
            ...
            assert.deepEqual(
              action.domain,
              domain,
              "should trigger a do_action with correct values"
            );
            assert.strictEqual(
              action.name,
              "P1 - BS task",
            ...
            assert.strictEqual(
              ...
              31,
              "should pass project_id in context when click on info icon on cell"
            );
            assert.strictEqual(
              action.context.default_task_id,
              1,
              "should pass task_id in context when click on info icon on cell"
            );
          });
      ...
      QUnit.test("editing a value [REQUIRE FOCUS]", async function (assert) {
        ...
        assert.strictEqual($(selection.focusNode)[0], $cell[0], "the cell is focused");
        assert.strictEqual(
      ...
      QUnit.test("editing a value does not glitch [REQUIRE FOCUS]", async function (
        assert
      ) {
        assert.expect(3);
      ...
      QUnit.test("changing timestamp removes errors [REQUIRE FOCUS]", async function (
        assert
      ) {
        assert.expect(2);
        ...
        await testUtils.dom.triggerEvent($input, "blur");
        assert.strictEqual(
          grid.$(".o_grid_input.o_has_error").length,
          1,
      ...
      QUnit.test("basic grid view, hide range button", async function (assert) {
        ...
        assert.containsN(
          grid.$buttons,
          "button.grid_arrow_range",
          2,
          "should have only one range button displayed"
        );
        assert.containsOnce(
          grid.$buttons,
          "button[data-name='week']",
          "should have week button displayed"
        );
        assert.containsOnce(
          grid.$buttons,
          "button[data-name='year']",
          "should have year button displayed"
        );
      ...
      QUnit.test("basic grid view, hide column/row total", async function (assert) {
        ...
        assert.doesNotHaveClass(
          grid.$("tbody"),
          ".o_grid_total",
          "body grid total should not be there"
        );

        grid.destroy();
      });

      QUnit.test("basic grid view, displayed footer as a barchart", async function (
        assert
      ) {
        assert.expect(3);
        ...
        assert.containsN(
          ...
          ".o_grid_total_bar",
          7,
          "should have a 7 div in the footer"
        );

        // Max height value is 90%
        assert.strictEqual(
          grid.el.querySelectorAll("tfoot .o_grid_total_bar")[2].style.height,
          "90%",
        ...
        assert.strictEqual(
          window.getComputedStyle(
            grid.el.querySelector("tfoot .o_grid_cell_null > .o_grid_total_bar")
          ).height,
          "0px",
      ...
      QUnit.test("compute correct cell value of barchart total", async function (
        assert
      ) {
        assert.expect(2);
        ...
        let cell = grid.el.querySelector(
          "tbody tr:nth-child(2) td:nth-child(2) .o_grid_cell_container"
        );
        ...
        await testUtils.dom.triggerEvent(input, "blur");
        assert.strictEqual(
          grid.el.querySelector("tfoot .o_grid_total_bar").style.height,
          "45%",
          "height of first total bar should be correctly computed"
        );

        cell = grid.el.querySelector(
          "tbody tr:nth-child(2) td:nth-child(5) .o_grid_cell_container"
        );
        ...
        await testUtils.dom.triggerEvent(input, "blur");
        assert.strictEqual(
          grid.el.querySelector("tfoot td:nth-child(5) .o_grid_total_bar").style.height,
          "30%",
      ...
      QUnit.test("row and column are highlighted when hovering a cell", async function (
        assert
      ) {
        assert.expect(29);
        ...
        await testUtils.dom.triggerEvents(
          grid.el.querySelectorAll("thead th")[2],
          "mouseover"
        );
        ...
        await testUtils.dom.triggerEvents(
          grid.el.querySelectorAll("thead th")[8],
          "mouseover"
        );
        assert.containsN(grid, "tbody .o_cell_hover", 5);
        for (let i = 0; i < 5; i++) {
        ...
        await testUtils.dom.triggerEvent(div, "focus");
        assert.strictEqual(
          cell.querySelector("input.o_grid_input").value,
          "0:00",
          "in non edit mode there not be leading zero in hours"
        );
        ...
        await testUtils.dom.triggerEvents(
          grid.el.querySelector("tbody tr:nth-child(1) td:nth-child(2)"),
          "mouseover"
        );
        assert.containsN(grid, ".o_cell_hover", 7);
        await testUtils.dom.triggerEvents(
        ...
        await testUtils.dom.triggerEvents(
          grid.el.querySelector("tfoot tr td:nth-child(2)"),
          "mouseover"
        );
        assert.containsN(grid, ".o_cell_hover", 7);
        await testUtils.dom.triggerEvents(
        ...
        await testUtils.dom.triggerEvents(
          grid.el.querySelector("tbody tr:nth-child(1) td.o_grid_total"),
          "mouseover"
        );
        assert.containsN(grid, ".o_cell_hover", 7);
        await testUtils.dom.triggerEvents(
        ...
        await testUtils.dom.triggerEvents(
          grid.el.querySelector("tfoot tr td:nth-child(9)"),
          "mouseover"
        );
        assert.containsN(grid, ".o_cell_hover", 7);
        await testUtils.dom.triggerEvents(
        ...
        assert.strictEqual(
          window.getComputedStyle(grid.el.querySelector(".o_grid_cell_null > div"))
            .opacity,
          "0.2"
        );
        ...
        assert.strictEqual(
          grid.el.querySelector("tbody .o_grid_cell_readonly").innerText,
          "4:00"
        );
      ...
      QUnit.test('grid view with type="readonly" field', async function (assert) {
        ...
        return concurrency.delay(0).then(function () {
          assert.strictEqual(
            grid.$(".o_grid_cell_container:not(.o_grid_cell_empty)").length,
            3,
            "should have 3 columns which has timesheet value"
          );
          assert.strictEqual(
            grid.$(
              ".o_grid_cell_container:not(.o_grid_cell_readonly, .o_grid_cell_empty)"
            ).length,
            2,
            "should have 2 cells which are not readonly"
          );
          assert.strictEqual(
            grid.$(".o_grid_cell_readonly").length,
            1,
            "should have 1 cell which is readonly"
          );
          assert.doesNotHaveClass(
            grid.$(".o_grid_cell_readonly div"),
            "o_grid_input",
      ...
      QUnit.test("grid_anchor is properly transferred in context", async function (
        assert
      ) {
        assert.expect(1);
        ...
        var grid = await createView({
          ...
          mockRPC: function (route, args) {
            if (args.method === "adjust_grid") {
              assert.strictEqual(
                args.kwargs.context.grid_anchor,
                "2017-01-24",
      ...
      QUnit.test("grid_anchor stays when navigating", async function (assert) {
        ...
        assert.strictEqual(
          $(webClient.el).find(".o_view_grid th:eq(2)").text(),
          "Tue,Jan 31",
          "The first day of the span should be the 31st of January"
        );

        // Move to previous week, and check first column header
        await testUtils.dom.click(
          $(webClient.el).find(".o_control_panel .grid_arrow_previous")
        );
        assert.strictEqual(
          $(webClient.el).find(".o_view_grid th:eq(2)").text(),
          "Tue,Jan 24",
          "The first day of the span should be the 24st of January, as we check the previous week"
        );
        ...
        assert.strictEqual(
          $(webClient.el).find(".o_view_grid th:eq(2)").text(),
          "Tue,Jan 24",
          "The first day of the span should STILL be the 24st of January, even we resetting search"
        );
      });

      QUnit.test("grid with two tasks with same name, and widget", async function (
        assert
      ) {
        assert.expect(2);
        ...
        assert.strictEqual(
          grid.$(".o_view_grid tbody tr th").text().trim(),
          "Awesome taskAwesome task"
        );
      ...
      QUnit.test('"create_inline is truthy', async function (assert) {
        ...
        this.arch =
          '<grid string="Timesheet By Project" adjustment="object" adjust_name="adjust_grid" create_inline="1">' +
          '<field name="project_id" type="row" section="1"/>' +
          '<field name="task_id" type="row"/>' +
          '<field name="date" type="col">' +
          '<range name="week" string="Week" span="week" step="day"/>' +
        ...
        assert.ok(
          grid.$buttons.find(".o_grid_button_add").is(":hidden"),
          "'Add a line' control panel button should not be visible"
        );
        const $addLineRow = grid.$(
          'tr.o_grid_add_line_row th div div a[role="button"]:contains("Add a line")'
        );
        assert.strictEqual($addLineRow.length, 1, "'Add a line' row should be visible");
        await testUtils.dom.click($addLineRow);
        assert.ok($(".modal").length, "should have opened a modal");
        await testUtils.dom.click(
          $(".modal .modal-footer button.o_form_button_cancel")
        );
        grid.destroy();
      });
      ...
      QUnit.test('"create_inline is truthy but create is falsy', async function (
        assert
      ) {
        assert.expect(2);

        this.arch =
          '<grid string="Timesheet By Project" adjustment="object" adjust_name="adjust_grid" create_inline="1" create="0">' +
          '<field name="project_id" type="row" section="1"/>' +
          '<field name="task_id" type="row"/>' +
          '<field name="date" type="col">' +
          '<range name="week" string="Week" span="week" step="day"/>' +
        ...
        assert.strictEqual(
          grid.$buttons.find(".o_grid_button_add").length,
          0,
          "'Add a line' control panel button should not be rendered"
        );
        assert.strictEqual(
          grid.$(
            'tr.o_grid_add_line_row th div div a[role="button"]:contains("Add a line")'
          ...
          0,
          "'Add a line' row should not be visible"
        );
        grid.destroy();
      });

      QUnit.test(
        '"create_inline is truthy with no data and display_empty is truthy',
        async function (assert) {
          assert.expect(3);

          this.arch =
            '<grid string="Timesheet By Project" adjustment="object" adjust_name="adjust_grid" create_inline="1" display_empty="1">' +
            '<field name="project_id" type="row" section="1"/>' +
            '<field name="task_id" type="row"/>' +
            '<field name="date" type="col">' +
            '<range name="week" string="Week" span="week" step="day"/>' +
          ...
          assert.strictEqual(
            $addLineRow.length,
            1,
            "'Add a line' row should be visible"
          );
          await testUtils.dom.click($addLineRow);
          assert.ok($(".modal").length, "should have opened a modal");
          await testUtils.dom.click(
      ...
      QUnit.test("float_toggle component", async function (assert) {
        ...
        var grid = await createView({
          ...
          async mockRPC(route, args) {
            if (args.method === "adjust_grid") {
              if (args.args[3] == "2017-02-01/2017-02-02") {
                // Use case "clicking on empty cell button"
                assert.strictEqual(
                  args.args[5],
                  4,
                  "saving 0.5 on float_toggle button will register 4 in database (0.5 / 0.125)"
                );
              }
              if (args.args[3] == "2017-01-30/2017-01-31") {
                // Use case "clicking on non empty cell button"
                assert.strictEqual(
                  args.args[5],
                  -10,
                  "saving 0.00 on float_toggle button will send a negative delta of 1.25 / 0.125"
                );
              }
            }
            return this._super.apply(this, arguments);
        ...
        // First section
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) button.o_grid_float_toggle").length,
          7,
          "first section should be for project P1"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) button.o_grid_float_toggle:contains(1.25)")
            .length,
          1,
          "one button contains the value 1.25 (timesheet line #4)"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) button.o_grid_float_toggle:contains(0.00)")
            .length,
          6,
          "Other button are filled with 0.00"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(0) .o_grid_cell_container:eq(0) button").text(),
          "1.25",
        ...
        // Second section
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) button.o_grid_float_toggle").length,
          7,
          "first section should be for project Webocalypse Now"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) button.o_grid_float_toggle:contains(-0.44)")
            .length,
          1,
          "one button contains the value -0.44 (timesheet line #5)"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) button.o_grid_float_toggle:contains(0.00)")
            .length,
          6,
          "Other button are filled with 0.00"
        );
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) .o_grid_cell_container:eq(1) button").text(),
          "-0.44",
          "the second cell of the second section is the timesheet #5"
        );
        assert.hasClass(
          grid.$(".o_grid_section:eq(1) tr:eq(0) td:eq(1)"),
          "text-danger",
          "the second cell of the second section has text-danger class as it has negative value"
        );
        assert.hasClass(
          grid.$(".o_grid_section:eq(1) tr:eq(1) td:eq(1) button.o_grid_float_toggle"),
          "text-danger",
        ...
        assert.strictEqual(
          grid.$(".o_grid_section:eq(1) .o_grid_cell_container:eq(2) button").text(),
          "0.00",
          "0.00 before we click on it"
        );
        var $button = grid.$(
          ".o_grid_section:eq(1) .o_grid_cell_container:eq(2) button"
        );
        $button.focus();
        ...
        var $button = grid.$(
          ".o_grid_section:eq(0) .o_grid_cell_container:eq(0) button"
        );
        ...
        assert.strictEqual(
          grid.$("tfoot tr td:eq(6) div").text(),
          "0.00",
          "The sixth cell of the footer should contain 0.00"
        );
        assert.strictEqual(
          grid.$("tbody tr:eq(1) td.o_grid_total").text(),
          "0.00",
          "The total of the BS task should be 0.00"
        );
        var $button = grid.$(
          ".o_grid_section:eq(0) .o_grid_cell_container:eq(5) button"
        );
        $button.focus();
        await testUtils.dom.click($button);
        $button.blur();
        ...
        assert.strictEqual(
          grid.$("tfoot tr td:eq(4) div").text(),
          "0.00",
        ...
        var $button = grid.$(
          ".o_grid_section:eq(0) .o_grid_cell_container:eq(4) button"
        );
        $button.focus();
        await testUtils.dom.click($button);
        $button.blur();
        assert.strictEqual(
          grid.$("tfoot tr td:eq(5) div").text(),
          "0.50",
      ...
      QUnit.test("button context not polluted by previous click", async function (
        assert
      ) {
        assert.expect(4);
      ...
      QUnit.test("grid view in day range ", async function (assert) {
        ...
        assert.containsOnce(
          grid,
          "thead th:not(.o_grid_title_header)",
          "Single Day column should be shown"
        );
        var grid_anchor = grid
          .$("thead tr")
          .text()
          .split(",\n")
          .join()
        ...
        assert.equal(
          grid_anchor,
          "Mon,Jan30",
          "grid should start with grid_anchor date as the default date."
        );
      ...
      QUnit.test("basic grouped grid view in day range", async function (assert) {
        ...
        assert.containsOnce(
          grid,
          ".o_grid_section:eq(0) th:contains(P1)",
          "first section should be for project P1"
        );
        assert.containsN(
          grid,
          ".o_grid_section:eq(0) div.o_grid_cell_container",
          1,
          "first section should have 1 cells"
        );
        assert.containsNone(
          grid,
          ".o_grid_section:eq(0) th:contains(Undefined)",
          "first section should'nt have a row without task"
        );
        assert.containsOnce(
          grid,
          ".o_grid_section:eq(0) th:contains(BS task)",
          "first section should have a row for BS task"
        );
      ...
      QUnit.test("work well with selection fields", async function (assert) {
        ...
        assert.strictEqual(
          grid.$("th div").first()[0].textContent,
          `P1A`,
      ...
      QUnit.test("button disabled after click", async function (assert) {
        ...
        const $button = grid.$(
          ".o_grid_section:eq(1) .o_grid_cell_container:eq(0) button"
        );
        $button.focus();
        ...
        assert.ok(
          $button.is(":disabled"),
          "The button is disabled while the RPC call is not done"
        );
        ...
        assert.notOk(
          $button.is(":disabled"),
          "The button is enabled when the RPC call is done"
        );
        grid.destroy();
      });

      QUnit.test("each button's value is correct after RPC call", async function (
        assert
      ) {
        /*
         * OPW 2121906
      ...
      QUnit.test("create/edit disabled for readonly grid view", async function (
        assert
      ) {
        assert.expect(4);
        this.data["analytic.line"].fields.validated = {
        ...
        assert.containsNone(
          grid,
          ".o_grid_cell_container:eq(3):not(.o_grid_cell_empty) .fa-search-plus",
          "should not have magnifying glass icon"
        );
        assert.containsOnce(
          grid,
          ".o_grid_cell_container:eq(2):not(.o_grid_cell_empty) .fa-search-plus",
        ...
        await testUtils.dom.click(
          grid.$("div.o_grid_cell_container:eq(2) .o_grid_cell_information")
        );
      ...
      QUnit.test(
        "concurrent reloads of a grid view (from 2 to 1 to 2 groupbys quickly)",
        async function (assert) {
          assert.expect(12);
          ...
          assert.deepEqual(
            grid.model.groupedBy,
            ["project_id", "task_id"],
            "two groupbys"
          );
          assert.deepEqual(
            grid.model._gridData.groupBy,
            ["project_id", "task_id"],
            "two groupbys"
          );
          ...
          assert.deepEqual(
            grid.model._gridData.groupBy,
            ["project_id", "task_id"],
            "_gridData has not been modified yet"
          );
          ...
          assert.deepEqual(
            grid.model.groupedBy,
            ["project_id", "task_id"],
            "the model state has not been corrupted"
          );
          assert.deepEqual(
            grid.model._gridData.groupBy,
            ["project_id", "task_id"],
      ...
      QUnit.test("Unavailable day is greyed", async function (assert) {
        ...
        assert.containsN(
          ...
          ".o_grid_unavailable",
          4,
          "should have 4 unavailable elements"
        );
        grid.destroy();
      });

      QUnit.test(
        "adjustment of type action should execute given action",
        async function (assert) {
          assert.expect(9);
            ...
            intercepts: {
              execute_action: function (event) {
                ...
                assert.strictEqual(
                  ...
                  "123",
                  "should call correct action id"
                );
                assert.ok(
                  context.grid_adjust.row_domain,
                  "row_domain should be in action context"
                );
                assert.ok(
                  context.grid_adjust.column_field,
                  "column_field should be in action context"
                );
                assert.ok(
                  context.grid_adjust.column_value,
                  "column_value should be in action context"
                );
                assert.ok(
                  context.grid_adjust.cell_field,
                  "cell_field should be in action context"
                );
                assert.ok(
                  context.grid_adjust.change,
                  "change should be in action context"
                );
              },
            },
          });
