odoo.define("web_grid.MockServer", function (require) {
  ...
  MockServer.include({
    ...
    _mockReadGrid: function (model, kwargs) {
      ...
      var start = gridAnchor
        .clone()
        .startOf(span === "day" ? "day" : span === "week" ? "isoWeek" : "month");
      var end = gridAnchor
        .clone()
        .endOf(span === "day" ? "day" : span === "week" ? "isoWeek" : "month");
      var nextAnchor = gridAnchor
        .clone()
        .add(1, span === "day" ? "day" : span === "week" ? "weeks" : "month")
        .format("YYYY-MM-DD");
      var prevAnchor = gridAnchor
        .clone()
        .subtract(1, span === "day" ? "day" : span === "week" ? "weeks" : "month")
        .format("YYYY-MM-DD");
      ...
      while (!current.isSame(end, "days")) {
        ...
        columns.push({
          ...
          values: {
            date: [dayStr + "/" + nextDayStr, current.format("ddd,\nMMM\u00a0DD")],
          },
        });
      }
      ...
      _.each(rows, function (row) {
        ...
        _.each(columns, function (col) {
          ...
          var read_fields = kwargs.readonly_field
            ? [kwargs.cell_field, kwargs.readonly_field]
            : [kwargs.cell_field];
