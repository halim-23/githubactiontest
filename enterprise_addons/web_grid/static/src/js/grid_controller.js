  ...
  var GridController = AbstractController.extend({
    ...
    renderButtons: function ($node) {
      ...
      this.$buttons.on(
        "click",
        ".grid_arrow_previous",
        this._onPaginationChange.bind(this, "prev")
      );
      this.$buttons.on(
        "click",
        ".grid_button_initial",
        this._onPaginationChange.bind(this, "initial")
      );
      this.$buttons.on(
        "click",
        ".grid_arrow_next",
        this._onPaginationChange.bind(this, "next")
      );
    ...
    updateButtons: function () {
      ...
      this.$buttons
        .find(".o_grid_button_add")
        .toggleClass(
          "d-none",
          this.createInline && (Boolean(state.data[0].rows.length) || this.displayEmpty)
        );
      this.$buttons
        .find(".grid_arrow_previous")
        .toggleClass("d-none", !state.data[0].prev);
      this.$buttons.find(".grid_arrow_next").toggleClass("d-none", !state.data[0].next);
      this.$buttons
        .find(".grid_button_initial")
        .toggleClass("d-none", !state.data[0].initial);
      this.$buttons.find(".grid_arrow_range").removeClass("active");
      this.$buttons
        .find(".grid_arrow_range[data-name=" + this.currentRange + "]")
        .addClass("active");
    },
     ...
     */
    _update: function () {
      return this._super.apply(this, arguments).then(this.updateButtons.bind(this));
    },
    ...
    _onOpenCellInformation: function (ev) {
      ...
      if (
        ...
        state.groupBy &&
        state.groupBy[0] === this.model.sectionField
      ) {
