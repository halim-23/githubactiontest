odoo.define("web_grid.GridModel", function (require) {
  ...
  const GridModel = AbstractModel.extend({
    ...
    __load: function (params) {
      ...
      var groupedBy =
        params.groupedBy && params.groupedBy.length ? params.groupedBy : this.rowFields;
    ...
    __reload: function (handle, params) {
      ...
      if ("context" in params) {
        ...
        this.context = params.context;
        if (
          old_context.grid_anchor !== undefined ||
          params.context.grid_anchor !== undefined
        ) {
          this.context.grid_anchor =
            old_context.grid_anchor || params.context.grid_anchor;
        }
      }
      if ("domain" in params) {
        this.domain = params.domain;
    ...
    _getRowInfo(row, grouped) {
      ...
      for (let i = 0; i < groupBy.length; i++) {
        ...
        const id =
          value && ["many2one", "selection"].includes(fieldType) ? value[0] : value;
        value =
    ...
    _fetchGroupedData: async function (groupBy) {
      ...
      const groups = await this.dp.add(
        this._rpc({
          model: this.modelName,
          method: "read_group",
          kwargs: {
      ...
      if (!groups.length) {
        ...
        prom = Promise.all([
          this._fetchSectionGrid(groupBy, {
      ...
      if (
        !(
          _.isEmpty(results) ||
          _.reduce(results, function (m, it) {
            return _.isEqual(m.cols, it.cols) && m;
          })
        )
      ) {
        throw new Error(
          _t("The sectioned grid view can't handle groups with different columns sets")
        );
      }
    ...
    _fetchUngroupedData: async function (groupBy) {
      const result = await this.dp.add(
        this._rpc({
          model: this.modelName,
          method: "read_grid",
          kwargs: {
