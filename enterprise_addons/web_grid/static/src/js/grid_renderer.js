odoo.define("web_grid.GridRenderer", function (require) {
  ...
  class GridRenderer extends AbstractRenderer {
    ...
    get showNoContentHelp() {
      const stateRow = Array.isArray(this.props.data)
        ? this.props.data.find((data) => data.rows[0])
        : this.props.data.rows[0];
      return stateRow === undefined && Boolean(this.props.noContentHelp);
    }
    ...
    _format(value) {
      ...
      return fieldUtils.format[this.formatType](
        value,
        cellField,
        this.props.cellComponentOptions
      );
    }
    /**
     * @private
    ...
    _onMouseEnter(ev) {
      ...
      this.el
        .querySelectorAll(`td:nth-child(${index}), th:nth-child(${index})`)
        .forEach((el) => {
