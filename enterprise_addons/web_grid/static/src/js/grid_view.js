odoo.define("web_grid.GridView", function (require) {
  ...
  var GridView = AbstractView.extend({
    ...
    init: function (viewInfo, params) {
      ...
      var contextRange =
        contextRangeName && _.findWhere(ranges, {name: contextRangeName});
      ...
      this.rendererParams.editableCells = Boolean(this.controllerParams.activeActions.edit && arch.attrs.adjustment);
      this.rendererParams.cellComponent = cellComponent;
      this.rendererParams.cellComponentOptions = cellComponentOptions;
      this.rendererParams.hideLineTotal = Boolean(JSON.parse(
        arch.attrs.hide_line_total || "0"
      ));
      this.rendererParams.hideColumnTotal = Boolean(JSON.parse(
        arch.attrs.hide_column_total || "0"
      ));
      this.rendererParams.hasBarChartTotal = Boolean(JSON.parse(
        arch.attrs.barchart_total || "0"
      ));
      this.rendererParams.createInline = Boolean(JSON.parse(
        arch.attrs.create_inline || "false"
      ));
      this.rendererParams.displayEmpty = Boolean(JSON.parse(
        arch.attrs.display_empty || "false"
      ));
      this.rendererParams.noContentHelp =
        (!this.rendererParams.displayEmpty && this.rendererParams.noContentHelp) || "";
    ...
    _extract_ranges: function (col_node, context) {
      ...
      _.each(_.pluck(col_node.children, "attrs"), function (range) {
        if (
          range.invisible &&
          pyUtils.py_eval(range.invisible, {context: pyevalContext})
        ) {
          return;
        }
        ranges.push(range);
