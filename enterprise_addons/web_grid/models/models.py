...
class Base(models.AbstractModel):
    ...
    def read_grid(
        ...
        if readonly_field:
            ...
            if (
                ...
                raise UserError(
                    _(
                        "The field used as readonly type must have a group_operator attribute."
                    )
                )
        ...
        for group in groups:
            ...
            cell_map[row][col] = self._grid_format_cell(
                group, cell_field, readonly_field
            )
    ...
    def _grid_column_info(self, name, range):
        ...
        if field.type == "selection":
                ...
                values=[
                    {
                        ...
                        "is_unavailable": self._grid_datetime_is_unavailable(
                            field, span, step, d
                        ),
                    }
                ...
                values=[
                    {
                        ...
                        "is_current": self._grid_datetime_is_current(
                            field, span, step, d
                        ),
                        "is_unavailable": self._grid_datetime_is_unavailable(
                            field, span, step, d
            ...
            raise ValueError(
                _("Can not use fields of type %s as grid columns", field.type)
            )
    ...
    def _get_date_column_label(self, date, field, span, step):
        ...
        return (
            "%s/%s"
            % (field.to_string(date), field.to_string(date + self._grid_step_by(step))),
            _labelize(_date_tz),
        )
    ...
    def _get_date_formatter(self, step, field, locale):
        ...
        if step == "week":
            ...
            def _week_format(date):
                if field.type == "date":
                    weeknumber = babel.dates.format_date(
                        date, format=FORMAT[step], locale=locale
                    )
                elif field.type == "datetime":
        ...
        if hasattr(babel.dates, "format_skeleton"):
            ...
            def _format(
                d, _fmt=babel.dates.format_skeleton, _sk=SKELETONS[step], _l=locale
            ):
    ...
    def _grid_pagination(self, field, span, step, anchor):
        if field.type in ["date", "datetime"]:
            ...
            return {
                "prev": {
                    "grid_anchor": period_prev,
                    "default_%s" % field.name: period_prev,
                },
                "next": {
                    "grid_anchor": period_next,
                    "default_%s" % field.name: period_next,
                },
                "initial": {
    ...
    def _get_start_of(self, span):
        if span == "week":
            ...
            return relativedelta(
                weekday=week_start_map.get(user_lang.week_start, MO(-1))
            )
    ...
    def _get_start_of_week(self, span):
        ...
        week_start_delta = relativedelta(
            weekday=week_start_map.get(user_lang.week_start, MO(-1))
        )
        if span == "week":
    ...
    def _get_end_of_week(self, span):
        ...
        week_end_delta = relativedelta(
            weekday=week_end_map.get(user_lang.week_start, SU)
        )
        if span == "week":
...
class datetime_range(object):
    ...
    @property
    def start_utc(self):
        return self._user_tz.localize(self._start.replace(tzinfo=None)).astimezone(
            pytz.utc
        )

    @property
    def end_utc(self):
        return self._user_tz.localize(self._end.replace(tzinfo=None)).astimezone(
            pytz.utc
        )
