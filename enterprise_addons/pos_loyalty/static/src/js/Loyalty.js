    ...
    get_won_points: function () {
      ...
      for (var line of this.get_orderlines()) {
        ...
        this.pos.loyalty.rules.forEach(function (rule) {
          var rule_points = 0;
          if (
            rule.valid_product_ids.find(function (product_id) {
          ...
          if (Math.abs(rule_points) > Math.abs(line_points)) line_points = rule_points;
        });
    ...
    get_available_rewards: function () {
      ...
      for (var i = 0; i < this.pos.loyalty.rewards.length; i++) {
        ...
        if (reward.minimum_points > self.get_spendable_points()) {
          ...
          reward.point_cost > self.get_spendable_points()
        ) {
          continue;
        } else if (
          ...
          reward.reward_type === "discount" &&
          reward.discount_apply_on === "specific_products"
        ) {
          var found = false;
          ...
          self.get_total_with_tax() < reward.minimum_amount
        ) {
          continue;
        }
        rewards.push(reward);
      ...
      if (!client) {
        ...
        if (reward.discount_type === "percentage") {
          if (reward.discount_apply_on === "on_order") {
            discount += round_pr(
              order_total * (reward.discount_percentage / 100),
              crounding
            );
          }
          ...
          if (reward.discount_apply_on === "specific_products") {
            for (var prod of reward.discount_specific_product_ids) {
              ...
              if (!specific_products) return;

              for (var line of this.get_orderlines()) {
                if (line.product.id === specific_products.id)
                  discount += round_pr(
                    line.get_price_with_tax() * (reward.discount_percentage / 100),
                    crounding
                  );
              }
            }
          ...
          if (reward.discount_apply_on === "cheapest_product") {
            ...
            for (var line of this.get_orderlines()) {
              if (
                (!price || price > line.get_unit_price()) &&
        ...
        if (reward.discount_type == "fixed_amount") {
          const discount_fixed_amount = reward.discount_fixed_amount;
          const point_cost = reward.point_cost;
          let quantity_to_apply = Math.floor(spendable / point_cost);
          const amount_discounted = discount_fixed_amount * quantity_to_apply;
