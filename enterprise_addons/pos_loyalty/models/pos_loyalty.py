...
class LoyaltyProgram(models.Model):
    ...
    reward_ids = fields.One2many(
        "loyalty.reward", "loyalty_program_id", string="Rewards"
    )
...
class LoyaltyRule(models.Model):
    ...
    valid_product_ids = fields.One2many(
        "product.product", compute="_compute_valid_product_ids"
    )
...
class LoyaltyReward(models.Model):
    ...
    reward_type = fields.Selection(
        [("gift", "Free Product"), ("discount", "Discount")],
        required=True,
        help="The type of the reward",
        default="gift",
    )
    gift_product_id = fields.Many2one(
        "product.product", string="Gift Product", help="The product given as a reward"
    )
    point_cost = fields.Float(
        string="Reward Cost",
        help="If the reward is a gift, that's the cost of the gift in points. If the reward type is a discount that's the cost in point per currency (e.g. 1 point per $)",
    )
    ...
    def _check_gift_product(self):
        if self.filtered(
            lambda reward: reward.reward_type == "gift" and not reward.gift_product_id
        ):
            raise ValidationError(
                _("The gift product field is mandatory for gift rewards")
            )
    ...
    def _check_discount_product(self):
        if self.filtered(
            ...
            and not reward.discount_product_id
        ):
            raise ValidationError(
                _("The discount product field is mandatory for discount rewards")
            )
