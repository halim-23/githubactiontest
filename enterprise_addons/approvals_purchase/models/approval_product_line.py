...
class ApprovalProductLine(models.Model):
    ...
    def _check_products_vendor(self):
        ...
        product_lines_without_seller = self.filtered(
            lambda line: not line._get_seller_id()
        )
        if product_lines_without_seller:
            product_names = product_lines_without_seller.product_id.mapped(
                "display_name"
            )
            raise UserError(
