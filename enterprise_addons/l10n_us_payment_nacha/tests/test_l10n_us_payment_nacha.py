...
class TestNacha(AccountTestInvoicingCommon):
    ...
    def testGenerateNachaFile(self):
        ...
        expected = [
            # header
            "101IMM_DESTINIMM_ORIGIN2011301945A094101DESTINATION            company_1_data         {:8d}".format(
                batch.id
            ),
        ...
        for generated, expected in zip(
            batch._generate_nacha_file().splitlines(), expected
        ):
            self.assertEqual(
                generated,
                expected,
                "Generated line in NACHA file does not match expected.",
            )
