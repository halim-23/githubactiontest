...
class AccountMove(models.Model):
    ...
    @api.depends(
        "invoice_line_ids.can_be_paid", "force_release_to_pay", "payment_state"
    )
    def _compute_release_to_pay(self):
        ...
        if self.env.context.get("module") == "account_3way_match":
            ...
            records = records.filtered(
                lambda r: r.payment_state != "paid"
                and r.move_type in ("in_invoice", "in_refund")
            )
...
class AccountMoveLine(models.Model):
            ...
            if po_line:
                ...
                if (
                    order_currency.compare_amounts(
                        po_line.price_unit, invoice_converted_price
                    )
                    != 0
                ):
                ...
                if (
                    po_line.product_id.purchase_method == "purchase"
                ):  # 'on ordered quantities'
                    invoice_line._can_be_paid_ordered_qty(
                        invoiced_qty, received_qty, ordered_qty, precision
                    )
                else:  # 'on received quantities'
                    invoice_line._can_be_paid_received_qty(
                        invoiced_qty, received_qty, ordered_qty, precision
                    )
    ...
    def _can_be_paid_ordered_qty(
        self, invoiced_qty, received_qty, ordered_qty, precision
    ):
        """
        ...
        if (
            float_compare(
                invoiced_qty - self.quantity, ordered_qty, precision_digits=precision
            )
            >= 0
        ):
            self.can_be_paid = "no"
    ...
    def _can_be_paid_received_qty(
        ...
        if float_compare(invoiced_qty, received_qty, precision_digits=precision) <= 0:
            ...
            received_qty == 0
            and float_compare(invoiced_qty, ordered_qty, precision_digits=precision)
            <= 0
