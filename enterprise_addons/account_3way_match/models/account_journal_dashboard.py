...
class AccountJournal(models.Model):
    ...
    def open_action(self):
        ...
        if view and action["id"] == view.id:
            account_purchase_filter = self.env.ref(
                "account_3way_match.account_invoice_filter_inherit_account_3way_match",
                False,
            )
            action["search_view_id"] = (
                account_purchase_filter
                and [account_purchase_filter.id, account_purchase_filter.name]
                or False
            )
        return action
    ...
    def _get_open_bills_to_pay_query(self):
        ...
        if self.type == "purchase":
            return (
                """SELECT state, (CASE WHEN move_type = 'in_refund' THEN -1 ELSE 1 END) *
