...
class HmrcSendWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if "obligation_id" in fields_list:
            obligations = self.env["l10n_uk.vat.obligation"].search(
                [("status", "=", "open")]
            )
            if not obligations:
            ...
            date_from = fields.Date.from_string(
                self.env.context["options"]["date"]["date_from"]
            )
            date_to = fields.Date.from_string(
                self.env.context["options"]["date"]["date_to"]
            )
    ...
    )  # Show message if no obligation corresponds to report options
    accept_legal = fields.Boolean(
        "Accept Legal Statement"
    )  # A checkbox to warn the user that what he sends is legally binding
