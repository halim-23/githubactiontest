...
class HmrcService(models.AbstractModel):
    ...
    def _login(self):
        ...
        if user.l10n_uk_user_token:
            if (
                not user.l10n_uk_hmrc_vat_token
                or user.l10n_uk_hmrc_vat_token_expiration_time
                ...
                try:
                    ...
                    data = json.dumps(
                        {
                            "params": {
                                "user_token": self.env.user.l10n_uk_user_token,
                                "dbuuid": dbuuid,
                            }
                        }
                    )
    ...
    def _get_fraud_prevention_info(self):
        ...
        try:
            ...
            if enterprise_code:
                hashed_license = hmac.new(
                    enterprise_code.encode(), db_secret.encode(), sha256
                ).hexdigest()
            else:
                hashed_license = ""
            ...
            if remote_needed:  # no need when on a private network
                ...
                gov_dict["Gov-Client-Public-Port"] = urls.url_quote(
                    str(environ.get("REMOTE_PORT"))
                )
            gov_dict["Gov-Client-Timezone"] = utc_offset
            gov_dict["Gov-Client-Browser-JS-User-Agent"] = urls.url_quote(
                environ.get("HTTP_USER_AGENT")
            )
            gov_dict["Gov-Vendor-Version"] = "Odoo=" + urls.url_quote(
                gov_vendor_version
            )
            gov_dict["Gov-Client-User-IDs"] = "os=" + urls.url_quote(self.env.user.name)
            gov_dict["Gov-Client-Browser-Do-Not-Track"] = (
                "true" if headers.get("DNT") == "1" else "false"
            )
            ...
            _logger.warning(
                "Could not construct fraud prevention headers", exc_info=True
            )
        return gov_dict
