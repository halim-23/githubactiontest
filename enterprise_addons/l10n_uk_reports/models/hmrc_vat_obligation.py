...
class HmrcVatObligation(models.Model):
    ...
    def name_get(self):
        return [
            (o.id, "%s (%s - %s)" % (o.date_due, o.date_start, o.date_end))
            for o in self
        ]
    ...
    def retrieve_vat_obligations(self, vat, from_date, to_date, status=""):
        ...
        if not match(r"^[0-9]{9}$", vat or ""):
            raise UserError(
                _(
                    "VAT numbers of UK companies should have exactly 9 figures. Please check the settings of the current company."
                )
            )
        ...
        url = self.env["hmrc.service"]._get_endpoint_url(
            "/organisations/vat/%s/obligations" % vat
        )
        params = {
            "from": from_date,
            "to": to_date,
        ...
        if error_code == "VRN_INVALID":
            error_message = _(
                "Invalid Company VAT number. Please fill in the correct VAT on the company form. "
            )
        elif error_code in (
            "INVALID_DATE_FROM",
            "INVALID_DATE_TO",
    ...
    def import_vat_obligations(self):
        ...
        for new_obligation in obligations:
            obligation = self.env["l10n_uk.vat.obligation"].search(
                [
                    ("period_key", "=", new_obligation.get("periodKey")),
            ...
            if not obligation:
                self.sudo().create(
                    {
                        "date_start": new_obligation["start"],
                ...
                or obligation.date_received != new_obligation.get("received_date")
            ):
                obligation.sudo().write(
                    {
                        "status": status,
                        "date_received": new_obligation.get("received_date"),
                    }
    ...
    def _fetch_values_from_report(self, lines):
        ...
        values = {}
        for line in lines:
            line_id = self.env["account.generic.tax.report"]._parse_line_id(line["id"])[
                -1
            ][-1]
            if reverse_table.get(line_id):
                ...
                if reverse_table[line_id] == "netVatDue":
                    values[reverse_table[line_id]] = abs(
                        round(line["columns"][0].get("balance", 0.0), 2)
                    )
                elif reverse_table[line_id] in (
                    "totalValueSalesExVAT",
                    "totalValuePurchasesExVAT",
                    "totalValueGoodsSuppliedExVAT",
                    "totalAcquisitionsExVAT",
                ):
                    values[reverse_table[line_id]] = round(
                        line["columns"][0].get("balance", 0.0)
                    )
                else:
                    values[reverse_table[line_id]] = round(
                        line["columns"][0].get("balance", 0.0), 2
                    )
        return values
    ...
    def action_submit_vat_return(self):
        ...
        options["date"].update(
            {
                "date_from": fields.Date.to_string(self.date_start),
        ...
        url = self.env["hmrc.service"]._get_endpoint_url(
            "/organisations/vat/%s/returns" % vat
        )
        data = values.copy()
        data.update({"periodKey": self.period_key, "finalised": True})
        ...
        if r.status_code == 201:  # Successful post
            ...
            if response.get("paymentIndicator"):
                msg += (
                    ...
                    + _("Payment Indicator")
                    + ": </b>"
                    + response["paymentIndicator"]
                    + "<br/>"
                )
            msg += (
                "<b>"
                + _("Form Bundle Number")
                + ": </b>"
