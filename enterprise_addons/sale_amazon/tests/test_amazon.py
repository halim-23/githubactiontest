...
class TestAmazon(TestAmazonCommon):
    ...
    def test_update_marketplaces_no_change(self):
        ...
        with patch(
            ...
            self.assertEqual(
                self.account.available_marketplace_ids.ids, self.marketplaces.ids
            )
            self.assertEqual(
    ...
    def test_update_marketplaces_remove(self):
        ...
        with patch(
            ...
            self.account.action_update_available_marketplaces()
            self.assertEqual(
                self.account.available_marketplace_ids.ids, self.marketplaces.ids[:1]
            )
            self.assertEqual(
                self.account.active_marketplace_ids.ids,
                self.marketplaces.ids[:1],
    ...
    def test_sync_orders_full(self):
        ...
        def _get_product_mock(
            _self, _product_code, _default_xmlid, _default_name, _default_type
        ):
        ...
        with patch(
            "odoo.addons.sale_amazon.models.mws_connector.get_api_connector",
            new=lambda *args, **kwargs: None,
        ), patch(
            "odoo.addons.sale_amazon.models.mws_connector.get_orders_data",
            new=_get_orders_data_mock,
        ), patch(
            "odoo.addons.sale_amazon.models.mws_connector.get_items_data",
            ...
            self.assertEqual(
                len(order), 1, "there should have been exactly one order created"
            )
            ...
            self.assertEqual(
                len(order_lines),
                4,
                "there should have been four order lines created: one for "
            ...
            self.assertEqual(
                ...
                50.0,
                "the unitary price should be the quotient of the "
    ...
    def test_sync_orders_partial(self):
        ...
        def _get_orders_data_mock(*_args, **_kwargs):
            ...
            return (
                [
                    BASE_ORDER_DATA,
                    dict(BASE_ORDER_DATA, AmazonOrderId={"value": "987654321"}),
                ],
                datetime(2020, 1, 1),
                None,
                False,
        ...
        def _get_items_data_mock(
            _orders_api, _amazon_order_ref, _error_message, _next_token=None
        ):
    ...
    def test_sync_orders_fba(self):
        ...
        with patch(
            ...
            self.assertEqual(
                len(moves),
                len(products),
                "FBA orders should generate one stock move "
    ...
    def test_sync_orders_europe(self):
        ...
        def _get_orders_data_mock(*_args, **_kwargs):
            """Return a one-order batch of test order data without calling MWS API."""
            return (
                [dict(BASE_ORDER_DATA, MarketplaceId={"value": "A13V1IB3VIYZZH"})],
                datetime(2020, 1, 1),
                None,
                False,
            )
        ...
        def _recompute_subtotal(
            _self, _subtotal, _tax_amount, _taxes, _currency, _fiscal_pos=None
        ):
    ...
    def test_sync_orders_cancel(self):
        ...
        def _get_orders_data_mock(*_args, **_kwargs):
            ...
            return (
                [dict(BASE_ORDER_DATA, OrderStatus={"value": _order_status})],
                datetime(2020, 1, 1),
                None,
                False,
            )
        ...
        with patch(
            ...
            self.assertEqual(
                order.state,
                "cancel",
                "cancellation of orders should be synchronized from Amazon",
            )
    ...
    def test_sync_orders_cancel_abort(self):
        ...
        with patch(
            ...
            "odoo.addons.sale_amazon.models.mws_connector.submit_feed",
    ...
    def test_sync_pickings(self):
        ...
        with patch(
            ...
            picking = self.env["stock.picking"].search([("sale_id", "=", order.id)])
            self.assertEqual(
                len(picking), 1, "FBM orders should generate exactly one picking"
            )
            picking.carrier_id = self.carrier
            picking.carrier_tracking_ref = "dummy tracking ref"
            picking._action_done()
            ...
            self.assertEqual(
                mock.call_count,
                1,
                "an order fulfillment feed should be sent to "
    ...
    def test_get_product_use_fallback(self):
        ...
        self.assertTrue(
            self.account._get_product("INCORRECT_CODE", "test_xmlid", None, None)
        )
    ...
    def test_get_product_regen_fallback(self):
        ...
        product = self.account._get_product(
    ...
    def test_get_product_no_fallback(self):
        ...
        self.assertFalse(
            self.account._get_product(
                "INCORRECT_CODE", "test_xmlid", "Default Name", "consu", fallback=False
            )
        )
        self.assertFalse(
            self.env.ref("sale_amazon.test_xmlid", raise_if_not_found=False)
        )
        ...
        self.env["res.partner"].create(
            {
                ...
                .search([("country_id", "=", country_id), ("code", "=", "CA")], limit=1)
                .id,
    ...
    def test_get_partners_no_creation_different_partners(self):
        ...
        contact = self.env["res.partner"].create(
            dict(new_partner_vals, name="Gederic Frilson")
        )
        self.env["res.partner"].create(
            dict(
                new_partner_vals,
        ...
        contact, delivery = self.account._get_partners(
            dict(
                ...
                ShippingAddress=dict(
                    ...
                    Name={"value": "Gederic Frilson Delivery"},
                ),
            ),
            "123456789",
        )
    ...
    def test_get_partners_arbitrary_fields(self):
        ...
        contact, _delivery = self.account._get_partners(
            dict(
                BASE_ORDER_DATA,
                ShippingAddress=dict(
                    BASE_ORDER_DATA["ShippingAddress"],
    ...
    def test_get_amazon_offer_search(self):
        ...
        marketplace = self.env["amazon.marketplace"].search(
            [("api_ref", "=", "ATVPDKIKX0DER")]
        )
        self.env["amazon.offer"].create(
            {
                "account_id": self.account.id,
                "marketplace_id": marketplace.id,
                "product_id": self.account._get_product(
