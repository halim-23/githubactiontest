...
class AmazonOffer(models.Model):
    ...
    def _default_marketplace(self):
        ...
        if account_id:
            marketplaces = (
                self.env["amazon.account"].browse([account_id]).active_marketplace_ids
            )
    ...
    def action_view_online(self):
        ...
        url = "https://sellercentral.%s/skucentral?mSku=%s" % (
            self.marketplace_id.domain.lower(),
            self.sku,
        )
        return {
