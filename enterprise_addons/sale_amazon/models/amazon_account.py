...
class AmazonAccount(models.Model):
    ...
    name = fields.Char(
        "Name", help="A user-defined name for the account", required=True
    )
    ...
    seller_key = fields.Char(
        "Seller ID",
        help="The Merchant ID of the Amazon Seller Central account",
        required=True,
    ...
    available_marketplace_ids = fields.Many2many(
        "amazon.marketplace",
        "amazon_account_marketplace_rel",
    ...
    active_marketplace_ids = fields.Many2many(
        "amazon.marketplace",
        "amazon_account_active_marketplace_rel",
    ...
    def _compute_offer_count(self):
        ...
        accounts_data = {
            offer_data["account_id"][0]: offer_data["account_id_count"]
        ...
        if self._origin.id:
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        "If the date is set in the past, orders placed on this Amazon "
        ...
        for account in self:
            if account.active_marketplace_ids.filtered(
                ...
                raise exceptions.ValidationError(
                    _("Only available marketplaces can be selected")
                )
    ...
    def create(self, vals):
        ...
        base_marketplace = self.env["amazon.marketplace"].browse(
            [vals["base_marketplace_id"]]
        )
    ...
    def action_view_orders(self):
        ...
        order_lines = self.env["sale.order.line"].search(
            [
                ...
                ("amazon_offer_id.account_id", "=", self.id),
            ]
        )
        return {
            "type": "ir.actions.act_window",
    ...
    def action_check_credentials(self, seller_key=None, auth_token=None):
        ...
        for account in self:
            error_message = _(
                "An error was encountered when preparing the connection to Amazon."
            )
            ...
            error_message = _(
                "The authentication to the Amazon Marketplace Web Service failed. "
                "Please verify your credentials."
            )
            if mwsc.do_account_credentials_check(sellers_api, error_message):
                _logger.warning(
                    "rate limit reached when checking credentials for "
                    "amazon.account with id %s" % account.id
                )
                raise exceptions.UserError(
                    _(
                        "The communication with Amazon is overloaded, "
    ...
    def _build_get_api_connector_kwargs(self):
        ...
        return {
            ...
            "db_enterprise_code": IrConfigParam_sudo.get_param(
    ...
    def action_update_available_marketplaces(self):
        ...
        for account in self:
            (
                ...
                rate_limit_reached,
            ) = self._get_available_marketplaces(
                account.seller_key,
                account.auth_token,
                account.base_marketplace_id,
                False,
            )
            if not rate_limit_reached:
                new_marketplaces = (
                    available_marketplaces - account.available_marketplace_ids
                )
                account.write(
                    {"available_marketplace_ids": [(6, 0, available_marketplaces.ids)]}
                )
        ...
        return {
            "effect": {
                "type": "rainbow_man",
                "message": _(
                    "Successfully updated the marketplaces available to this account !"
                ),
            }
        }
    ...
    def _sync_orders(self, auto_commit=True):
        ...
        for account in self or self.search([]):
            ...
            marketplace_api_refs = [
                marketplace.api_ref for marketplace in account.active_marketplace_ids
            ]
            ...
            error_message = _(
                "An error was encountered when synchronizing Amazon orders."
            )
            while has_next and not rate_limit_reached:
                (
                    ...
                    next_token,
                    rate_limit_reached,
                ) = mwsc.get_orders_data(
                    orders_api,
                    marketplace_api_refs,
                    updated_after.isoformat(sep="T"),
                    error_message,
                    next_token,
                )
                ...
                if rate_limit_reached:
                    _logger.warning(
                        "rate limit reached when synchronizing orders of "
                        "amazon.account with id %s" % account.id
                    )
                ...
                for order_data in orders_data_batch:
                    if not rate_limit_reached:
                        (
                            ...
                            rate_limit_reached,
                            sync_failure,
                        ) = account._process_order(order_data, orders_api)
                        if sync_failure:
                        ...
                        if rate_limit_reached:
                            _logger.warning(
                                ...
                                "for amazon.account with id %s"
                                % (amazon_order_ref, account.id)
                            )
                            continue
                        account.last_orders_sync = mwsc.get_date_value(
                            order_data, "LastUpdateDate"
                        )
                        if auto_commit:
                            self.env.cr.commit()
            if not rate_limit_reached:
    ...
    def _get_available_marketplaces(
        ...
        (
            ...
            rate_limit_reached,
        ) = mwsc.get_available_marketplace_api_refs(sellers_api, error_message)
        if rate_limit_reached:
            _logger.warning(
            ...
            if raise_if_rate_limit_reached:
                raise exceptions.UserError(
                    _(
                        "The communication with Amazon is overloaded, please try again later."
                    )
                )
        else:  # Don't update marketplaces if they could not all be fetched
            domain = expression.OR(
                [
                    [("api_ref", "ilike", marketplace_api_ref)]
    ...
    def _process_order(self, order_data, orders_api):
        ...
        sync_failure = False
        error_message = _(
            "An error was encountered when synchronizing Amazon order items."
        )
        while has_next and not rate_limit_reached:
        ...
        if not rate_limit_reached:
            try:
                ...
                _logger.warning(
                    "error (%(error)s) while syncing sale.order with amazon_order_ref %(order_ref)s for "
                ...
                if (
                    ...
                    _logger.info(
                        "synchronized sale.order with amazon_order_ref %s for "
                        "amazon.account with id %s" % (amazon_order_ref, self.id)
                    )
                elif order_found:  # Order already sync
                    _logger.info(
                        "ignored already sync sale.order with amazon_order_ref %s for "
                        "amazon.account with id %s" % (amazon_order_ref, self.id)
                    )
                else:  # Combination of status and fulfillment channel not handled
                    _logger.info(
                        "ignored %s amazon order with reference %s for amazon.account "
                        "with id %s"
                        % (amazon_status.lower(), amazon_order_ref, self.id)
                    )
    ...
    def _get_order(self, order_data, items_data, amazon_order_ref):
        ...
        if not order_found and (
            ...
            contact_partner, delivery_partner = self._get_partners(
                order_data, amazon_order_ref
            )
            fiscal_position = (
    ...
    def _process_order_lines(
        ...
        def _get_order_line_vals(**kwargs):
            ...
            return {
                ...
                "discount": (kwargs.get("discount", 0) / _subtotal) * 100
                if _subtotal
                else 0,
        ...
        for item_data in items_data:
            ...
            subtotal = (
                sales_price - tax_amount if marketplace.tax_included else sales_price
            )
            ...
            description_fields = (
                (sku, mwsc.get_string_value(item_data, "Title", unescape_html=True))
                if not main_condition or main_condition.lower() == "new"
                else (
                    sku,
                    mwsc.get_string_value(item_data, "Title", unescape_html=True),
                    main_condition,
                    sub_condition,
                )
            )
            new_order_lines_vals.append(
                _get_order_line_vals(
            ...
            if mwsc.get_string_value(item_data, "IsGift", "false") == "true":
                ...
                if gift_wrap_code and gift_wrap_price != 0:
                    ...
                    gift_wrap_tax_amount = mwsc.get_amount_value(
                        item_data, "GiftWrapTax"
                    )
                    gift_wrap_subtotal = (
                        gift_wrap_price - gift_wrap_tax_amount
                        if marketplace.tax_included
                ...
                gift_message = mwsc.get_string_value(
                    item_data, "GiftMessageText", unescape_html=True
                )
                if gift_message:
                    new_order_lines_vals.append(
                        _get_order_line_vals(
            ...
            if shipping_code:
                ...
                shipping_subtotal = (
                    shipping_price - shipping_tax_amount
                    if marketplace.tax_included
                    else shipping_price
                )
    ...
    def _get_product(
        self, product_code, default_xmlid, default_name, default_type, fallback=True
    ):
        """
    ...
    def _get_partners(self, order_data, amazon_order_ref):
        ...
        buyer_name = mwsc.get_string_value(
            order_data, "BuyerName", False, unescape_html=True
        )
        shipping_address_name = mwsc.get_string_value(
            order_data, ("ShippingAddress", "Name"), unescape_html=True
        )
        street = mwsc.get_string_value(
            order_data, ("ShippingAddress", "AddressLine1"), unescape_html=True
        )
        address_line2 = mwsc.get_string_value(
            order_data, ("ShippingAddress", "AddressLine2"), unescape_html=True
        )
        address_line3 = mwsc.get_string_value(
            order_data, ("ShippingAddress", "AddressLine3"), unescape_html=True
        )
        street2 = (
            "%s %s" % (address_line2, address_line3)
            if address_line2 or address_line3
            else None
        )
        zip_code = mwsc.get_string_value(order_data, ("ShippingAddress", "PostalCode"))
        city = mwsc.get_string_value(
            order_data, ("ShippingAddress", "City"), unescape_html=True
        )
        country_code = mwsc.get_string_value(
            order_data, ("ShippingAddress", "CountryCode")
        )
        state_code = mwsc.get_string_value(
            order_data, ("ShippingAddress", "StateOrRegion")
        )
        phone = mwsc.get_string_value(order_data, ("ShippingAddress", "Phone"), None)
        is_company = (
            mwsc.get_string_value(
                order_data, ("ShippingAddress", "AddressType"), "Residential"
            )
            == "Commercial"
        )
    ...
    def _recompute_subtotal(
        self, subtotal, tax_amount, taxes, currency, _fiscal_pos=None
    ):
        """
    ...
    def _handle_order_sync_failure(self, amazon_order_ref):
        ...
        mail_template = self.env.ref(
            "sale_amazon.order_sync_failure", raise_if_not_found=False
        )
        if not mail_template:
            _logger.warning(
                "the mail template with xmlid sale_amazon.order_sync_failure has "
                "been deleted."
            )
        else:
            responsible_emails = {
