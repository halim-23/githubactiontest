...
def get_api_connector(
    api_class, seller_key, auth_token, marketplace_code, error_message, **kwargs
):
...
def _generate_feed_base(seller_key, message_type):
    ...
    ElementTree.SubElement(message, "MessageID").text = str(
...
def generate_order_fulfillment_feed(
    ...
    ElementTree.SubElement(
        order_fulfillment, "FulfillmentDate"
...
def get_available_marketplace_api_refs(sellers_api, error_message):
    ...
    while has_next and not rate_limit_reached:
        request_response, rate_limit_reached = _send_request(
            sellers_api.list_marketplace_participations,
            "Sellers",
            error_message,
        ...
        if request_response:
            if not request_response.response.ok:
                ...
                marketplaces_data = get_raw_data(
                    parsed_data, ("ListMarketplaces", "Marketplace")
                )
                if marketplaces_data:
                    ...
                    marketplace_api_refs += [
                        get_string_value(marketplace_data, "MarketplaceId")
...
def get_orders_data(
    orders_api, marketplace_api_refs, updated_after, error_message, next_token=None
):
    ...
    request_response, rate_limit_reached = _send_request(
        ...
        orderstatus=("Unshipped", "PartiallyShipped", "Shipped", "Canceled"),
        next_token=next_token,
    )
    if request_response:
        if not request_response.response.ok:
            ...
            if orders_parsed_data and isinstance(
                orders_parsed_data, dict
            ):  # Single element in response
                orders_data.append(orders_parsed_data)
            elif orders_parsed_data and isinstance(
                orders_parsed_data, list
            ):  # Multiple elements in response
...
def _is_sent_by_amazon(mws_error):
    ...
    if 700 <= mws_error.response.status_code <= 799:  # Custom Odoo proxy error
        ...
        if mws_error.response.status_code == 730:  # Forbidden
            ...
            raise exceptions.UserError(
                _("The Odoo proxy received a malformed request.")
            )
        elif mws_error.response.status_code == 750:  # Internal Error
            raise exceptions.UserError(
                _("The Odoo proxy encountered an internal server error.")
            )
        else:  # This is unexpected, we should probably have a custom message for this code
            raise exceptions.UserError(
                _("The Odoo proxy encountered an unhandled error.")
            )
...
def get_float_value(parsed_data, key_path=(), default_value=0.0):
...
def get_date_value(parsed_data, key_path=(), default_value=None):
    ...
    string_value = get_string_value(parsed_data, key_path)
    date_value = string_value and dateutil.parser.parse(string_value).replace(
        tzinfo=None
    )
    return date_value or default_value


def get_amount_value(parsed_data, key_path=(), default_value=0.0):
