...
class QualityPoint(models.Model):
    ...
    def _get_default_team_id(self):
        company_id = self.company_id.id or self.env.context.get(
    ...
    def create(self, vals):
        if "name" not in vals or vals["name"] == _("New"):
            vals["name"] = self.env["ir.sequence"].next_by_code("quality.point") or _(
                "New"
            )
    ...
    def _get_domain(self, product_ids, picking_type_id, measure_on="operation"):
        ...
        domain_in_products_or_categs = [
            ...
            ("product_category_ids", "parent_of", product_ids.categ_id.ids),
        ]
...
class QualityAlertTeam(models.Model):
    ...
    def _compute_check_count(self):
        ...
        check_result = {
            data["team_id"][0]: data["team_id_count"] for data in check_data
        }
        for team in self:
    ...
    def _compute_alert_count(self):
        ...
        alert_result = {
            data["team_id"][0]: data["team_id_count"] for data in alert_data
        }
        for team in self:
...
class QualityTag(models.Model):
    ...
    color = fields.Integer(
        "Color Index", help="Used in the kanban view"
    )  # TDE: should be default value
...
class QualityCheck(models.Model):
    ...
    quality_state = fields.Selection(
        [("none", "To do"), ("pass", "Passed"), ("fail", "Failed")],
        string="Status",
        tracking=True,
        default="none",
        copy=False,
    ...
    test_type_id = fields.Many2one(
        "quality.point.test_type", "Test Type", required=True
    )
    ...
    def _compute_alert_count(self):
        alert_data = self.env["quality.alert"].read_group(
            [("check_id", "in", self.ids)], ["check_id"], ["check_id"]
        )
        alert_result = {
            data["check_id"][0]: data["check_id_count"] for data in alert_data
        }
        for check in self:
    ...
    def create(self, vals):
        if "name" not in vals or vals["name"] == _("New"):
            vals["name"] = self.env["ir.sequence"].next_by_code("quality.check") or _(
                "New"
            )
        if "point_id" in vals and not vals.get("test_type_id"):
            vals["test_type_id"] = (
                self.env["quality.point"].browse(vals["point_id"]).test_type_id.id
            )
...
class QualityAlert(models.Model):
    ...
    def _get_default_stage_id(self):
        ...
        if (
            not team_id
            and self.env.context.get("active_model") == "quality.alert.team"
    ...
    def create(self, vals):
        if "name" not in vals or vals["name"] == _("New"):
            vals["name"] = self.env["ir.sequence"].next_by_code("quality.alert") or _(
                "New"
            )
    ...
    def onchange_product_tmpl_id(self):
        self.product_id = (
            ...
            and self.product_tmpl_id.product_variant_ids.ids[0]
        )
