...
class L10nBe273S(models.Model):
    ...
    def _get_rendering_data(self):
        ...
        employees = payslips.mapped("employee_id").filtered(
            lambda e: not e._is_niss_valid()
        )
        if employees:
            raise UserError(
                _(
        ...
        for payslip in payslips.sudo():
            ...
            if ip_amount:
                ...
                mapped_ip[payslip.employee_id][1] += line_values["IP.DED"][payslip.id][
                    "total"
                ]
        ...
        return {
            ...
            "declaration": {
                "gross_amount": round(gross_amount / 2, 2)
                * 2,  # To avoid 1 cent diff (eg: 286079.05)
                "deductable_costs": {
    ...
    def action_generate_pdf(self):
        self.ensure_one()
        export_273S_pdf, dummy = (
            self.env.ref("l10n_be_hr_payroll.action_report_ip_273S")
            .sudo()
            ._render_qweb_pdf(res_ids=self.ids, data=self._get_rendering_data())
        )
    ...
    def action_generate_xml(self):
        ...
        xml_str = self.env.ref("l10n_be_hr_payroll.273S_xml_report")._render(
