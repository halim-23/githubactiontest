...
class Payslip(models.Model):
    ...
    l10n_be_max_seizable_amount = fields.Float(
        compute="_compute_l10n_be_max_seizable_amount"
    )
    l10n_be_max_seizable_warning = fields.Char(
        compute="_compute_l10n_be_max_seizable_amount"
    )
        ...
        for slip in self:
            ...
            if slip.struct_id.code == "CP200WARRANT":
                months = relativedelta(
                    date_utils.add(slip.date_to, days=1), slip.date_from
                ).months
                if slip.employee_id.id in self.env.context.get(
                    "commission_real_values", {}
                ):
                    warrant_value = self.env.context["commission_real_values"][
                        slip.employee_id.id
                    ]
                else:
                    warrant_value = slip.contract_id.commission_on_target * months
                warrant_type = self.env.ref(
                    "l10n_be_hr_payroll.cp200_other_input_warrant"
                )
                lines_to_remove = slip.input_line_ids.filtered(
                    lambda x: x.input_type_id == warrant_type
                )
                            ...
                            "input_line_ids": [
                                (
                                    ...
                                    0,
                                    {
                                        ...
                                        "amount": to_recover,
                                        "input_type_id": self.env.ref(
                                            "l10n_be_hr_payroll.input_double_holiday_european_leave_deduction"
                                        ).id,
                                    },
                                )
                            ]
    ...
    @api.depends(
        ...
        "worked_days_line_ids.is_credit_time",
    )
    ...
    def _compute_work_entry_dependent_benefits(self):
        if self.env.context.get("salary_simulation"):
            ...
            work_entries_benefits_rights = self.env[
            ...
            for payslip in self:
                ...
                calendar = (
                    ...
                    if not contract.time_credit
                    else contract.standard_calendar_id
                )
                mapped_resources[
                    (calendar, payslip.date_from, payslip.date_to)
            ...
            for payslip in self:
                ...
                for work_entries_benefits_right in (
                    ...
                    and payslip.employee_id.id
                    == work_entries_benefits_right["employee_id"][0]
                ):
                ...
                nb_of_days_to_work = len(
                    {dt_from.date(): True for (dt_from, dt_to, attendance) in intervals}
                )
                payslip.private_car_missing_days = nb_of_days_to_work - (
                    benefits["private_car"] if "private_car" in benefits else 0
                )
                payslip.representation_fees_missing_days = nb_of_days_to_work - (
                    benefits["representation_fees"]
                    if "representation_fees" in benefits
                    else 0
                )
    ...
    def _compute_l10n_be_max_seizable_amount(self):
        ...
        all_payslips = self.env["hr.payslip"].search(
            [("employee_id", "in", self.employee_id.ids), ("state", "!=", "cancel")]
        )
        ...
        for payslip in self:
            ...
            rates = self.env["hr.rule.parameter"]._get_parameter_from_code(
                "cp200_seizable_percentages", payslip.date_to, raise_if_not_found=False
            )
            child_increase = self.env["hr.rule.parameter"]._get_parameter_from_code(
                "cp200_seizable_amount_child", payslip.date_to, raise_if_not_found=False
            )
            ...
            period_payslips = all_payslips.filtered(
                lambda p: p.employee_id == payslip.employee_id
                and p.date_from == payslip.date_from
                and p.date_to == payslip.date_to
            )
            net_amount = sum(
                [payslip_values["NET"][p.id]["total"] for p in period_payslips]
            )
            seized_amount = sum(
                [
                    period_payslips._get_input_line_amount(code)
                    for code in ["ATTACH_SALARY", "ASSIG_SALARY", "CHILD_SUPPORT"]
                ]
            )
            ...
            dependent_children = (
                payslip.employee_id.l10n_be_dependent_children_attachment
            )
            ...
            if max_seizable_amount and seized_amount > max_seizable_amount:
                payslip.l10n_be_max_seizable_warning = _(
                    "The seized amount (%s€) is above the belgian ceilings. Given a global net salary of %s€ for the pay period and %s dependent children, the maximum seizable amount is equal to %s€",
                    round(seized_amount, 2),
                    round(net_amount, 2),
                    round(dependent_children, 2),
                    round(max_seizable_amount, 2),
                )
            else:
                payslip.l10n_be_max_seizable_warning = False
    ...
    def _get_worked_day_lines_values(self, domain=None):
        ...
        # be separated
        work_hours = self.contract_id._get_work_hours_split_half(
        ...
        if (
            ...
            if self.contract_id.employee_id.contract_ids.filtered(
                lambda c: c.state in ["open", "close"]
                and c.date_start > self.contract_id.date_end
            ):
                return res
            public_holiday_type = self.env.ref(
                "l10n_be_hr_payroll.work_entry_type_bank_holiday"
            )
            public_leaves = (
                self.contract_id.resource_calendar_id.global_leave_ids.filtered(
            ...
            public_leaves = public_leaves.filtered(
                lambda l: (
                    l.date_from.date() - self.employee_id.first_contract_date
                ).days
                >= 15
            )
            ...
            public_leaves = public_leaves.filtered(
                lambda l: 0
                < (l.date_from.date() - self.contract_id.date_end).days
                <= (
                    30
                    if self.employee_id.first_contract_date + relativedelta(months=1)
                    <= self.contract_id.date_end
                    else 15
                )
            )
            if public_leaves:
                input_type_id = self.env.ref(
                    "l10n_be_hr_payroll.cp200_other_input_after_contract_public_holidays"
                ).id
        ...
        if self._get_last_year_average_variable_revenues():
            we_types_ids = (
                self.env.ref("l10n_be_hr_payroll.work_entry_type_bank_holiday")
            ...
            if any(
                line_vals["work_entry_type_id"] in we_types_ids for line_vals in res
            ):
                we_type = self.env.ref(
                    "l10n_be_hr_payroll.work_entry_type_simple_holiday_pay_variable_salary"
                )
                res.append(
                    {
    ...
    def _get_last_year_average_variable_revenues(self):
        ...
        if not complete_payslips:
            return 0
        total_amount = complete_payslips._get_line_values(
            ["COMMISSION"], compute_sum=True
        )["COMMISSION"]["sum"]["total"]
    ...
    def _get_last_year_average_warrant_revenues(self):
        ...
        total_amount = warrant_payslips._get_line_values(["BASIC"], compute_sum=True)[
            "BASIC"
        ]["sum"]["total"]
        first_contract_date = self.employee_id.first_contract_date
        if not first_contract_date:
    ...
    def _get_credit_time_lines(self):
        lines_vals = self._get_worked_day_lines(
            domain=[("is_credit_time", "=", True)], check_out_of_contract=False
        )
    ...
    def _get_new_worked_days_lines(self):
        ...
        if self.struct_id.use_worked_day_lines:
            ...
            return [(5, 0, 0)] + [
                (0, 0, vals)
                for vals in worked_days_line_values + credit_time_line_values
            ]
    ...
    def _compute_number_complete_months_of_work(self, date_from, date_to, contracts):
        ...
        for contract in contracts:
            ...
            work_days = {
                int(d)
                for d in contract.resource_calendar_id._get_global_attendances().mapped(
                    "dayofweek"
                )
            }

            previous_week_start = max(
                contract_start + relativedelta(weeks=-1, weekday=MO(-1)), date_from
            )
            next_week_end = min(
                contract.date_end + relativedelta(weeks=+1, weekday=SU(+1))
                if contract.date_end
                else date.max,
                date_to,
            )
            days_to_check = rrule.rrule(
                rrule.DAILY, dtstart=previous_week_start, until=next_week_end
            )
    ...
    def _compute_presence_prorata(self, date_from, date_to, contracts):
        ...
        paid_work_entry_types = (
            self.env["hr.work.entry.type"].search([]) - unpaid_work_entry_types
        )
        ...
        unpaid_hours = sum(
            v for k, v in hours.items() if k in unpaid_work_entry_types.ids
        )
        return (
            paid_hours / (paid_hours + unpaid_hours)
            if paid_hours or unpaid_hours
            else 0
        )
    ...
    def _get_paid_amount_13th_month(self):
        ...
        contracts = self.employee_id.contract_ids.filtered(
            lambda c: c.state not in ["draft", "cancel"]
            and c.structure_type_id == self.struct_id.type_id
        )
        first_contract_date = self.contract_id.employee_id._get_first_contract_date(
            no_gap=False
        )
        ...
        if force_months:
            ...
            n_months = min(
                12,
                self._compute_number_complete_months_of_work(
                    date_from, date_to, contracts
                ),
            )
            # 2. Deduct absences
            presence_prorata = self._compute_presence_prorata(
                date_from, date_to, contracts
            )
        ...
        force_avg_variable_revenues = self.input_line_ids.filtered(
            lambda l: l.code == "VARIABLE"
        )
    ...
    def _get_paid_amount_warrant(self):
        self.ensure_one()
        warrant_input_type = self.env.ref(
            "l10n_be_hr_payroll.cp200_other_input_warrant"
        )
        return sum(
            self.input_line_ids.filtered(
                lambda a: a.input_type_id == warrant_input_type
            ).mapped("amount")
        )
    ...
    def _get_paid_double_holiday(self):
        ...
        basic = self.contract_id._get_contract_wage()
        force_months = self.input_line_ids.filtered(lambda l: l.code == "MONTHS")
        if force_months or self.contract_id.first_contract_date > date(
            self.date_from.year - 1, 1, 1
        ):
            ...
            if force_months:
                ...
                # 1. Number of months
                n_months = self._compute_number_complete_months_of_work(
                    date_from, date_to, contracts
                )
                # 2. Deduct absences
                presence_prorata = self._compute_presence_prorata(
    ...
    def _is_active_belgian_languages(self):
        ...
        return any(
            l in active_langs
            for l in ["fr_BE", "fr_FR", "nl_BE", "nl_NL", "de_BE", "de_DE"]
        )
    ...
    def _get_sum_european_time_off_days(self, check=False):
        ...
        european_time_off_amount = two_years_payslips._get_worked_days_line_amount(
            "LEAVE216"
        )
        already_recovered_amount = two_years_payslips._get_line_values(
            ["EU.LEAVE.DEDUC"], compute_sum=True
        )["EU.LEAVE.DEDUC"]["sum"]["total"]
    ...
    def _is_invalid(self):
        ...
        if not invalid and self._is_active_belgian_languages():
            ...
            if country.code == "BE" and lang_employee not in [
                "fr_BE",
                "fr_FR",
                "nl_BE",
                "nl_NL",
                ...
                return _(
                    "This document is a translation. This is not a legal document."
                )
        return invalid
    ...
    def action_payslip_done(self):
        regular_pay = self.env.ref(
            "l10n_be_hr_payroll.hr_payroll_structure_cp200_employee_salary"
        )
        invalid_payslips = self.filtered(
            lambda p: p.struct_id == regular_pay and not p.worked_days_line_ids
        )
        if invalid_payslips:
            raise UserError(
                _(
                    "The regular pay for these employees don't have any worked day lines:\n%s"
                ),
                "\n".join(invalid_payslips.mapped("employee_id.name")),
            )
        if self._is_active_belgian_languages():
            bad_language_slips = self.filtered(
                lambda p: p.struct_id.country_id.code == "BE"
                and p.employee_id.sudo().address_home_id.lang
                not in ["fr_BE", "fr_FR", "nl_BE", "nl_NL", "de_BE", "de_DE"]
            )
            if bad_language_slips:
                action = self.env["ir.actions.act_window"]._for_xml_id(
                    "l10n_be_hr_payroll.l10n_be_hr_payroll_employee_lang_wizard_action"
                )
    ...
    def _get_pdf_reports(self):
        ...
        report_n = self.env.ref(
            "l10n_be_hr_payroll.action_report_termination_holidays_n"
        )
        report_n1 = self.env.ref(
            "l10n_be_hr_payroll.action_report_termination_holidays_n1"
        )
        for payslip in self:
...
def compute_termination_withholding_rate(payslip, categories, worked_days, inputs):
    ...
    if (
        children
        and annual_taxable <= children_reduction.get(children, children_reduction[5])[1]
    ):
        pp_rate *= (
            1 - children_reduction.get(children, children_reduction[5])[0] / 100.0
        )
    return pp_rate
...
def compute_withholding_taxes(payslip, categories, worked_days, inputs):
    def compute_basic_bareme(value):
        ...
        rates = [
            (limit or float("inf"), rate) for limit, rate in rates
        ]  # float('inf') because limit equals None for last level
        rates = sorted(rates)
    ...
    def convert_to_month(value):
        return float_round(
            value / 12.0, precision_rounding=0.01, rounding_method="DOWN"
        )
    ...
    if payslip.contract_id.transport_mode_private_car:
        ...
        )._get_private_car_reimbursed_amount(contract.km_home_work)
    ...
    if yearly_gross_revenue <= payslip.rule_parameter(
        "yearly_gross_revenue_bound_expense"
    ):
        yearly_net_taxable_revenue = yearly_gross_revenue * (1.0 - 0.3)
    else:
        yearly_net_taxable_revenue = yearly_gross_revenue - payslip.rule_parameter(
            "expense_deduction"
        )
    ...
    if employee.resident_bool:
        ...
        if employee.marital in ["divorced", "single", "widower"] or (
            employee.marital in ["married", "cohabitant"]
            and employee.spouse_fiscal_status != "without_income"
        ):
            basic_bareme = max(
                compute_basic_bareme(yearly_net_taxable_revenue)
                - payslip.rule_parameter("deduct_single_with_income"),
                0.0,
            )
            withholding_tax_amount = convert_to_month(basic_bareme)
        ...
        if (
            ...
            yearly_net_taxable_revenue_for_spouse = min(
                yearly_net_taxable_revenue * 0.3,
                payslip.rule_parameter("max_spouse_income"),
            )
            basic_bareme_1 = compute_basic_bareme(yearly_net_taxable_revenue_for_spouse)
            basic_bareme_2 = compute_basic_bareme(
                yearly_net_taxable_revenue - yearly_net_taxable_revenue_for_spouse
            )
            withholding_tax_amount = convert_to_month(
                max(
                    basic_bareme_1
                    + basic_bareme_2
                    - 2 * payslip.rule_parameter("deduct_single_with_income"),
                    0,
                )
            )

    # Reduction for other family charges
    if (employee.children and employee.dependent_children) or (
        employee.other_dependent_people
        and (employee.dependent_seniors or employee.dependent_juniors)
    ):
        if employee.marital in ["divorced", "single", "widower"] or (
            employee.spouse_fiscal_status != "without_income"
        ):

            # if employee.marital in ['divorced', 'single', 'widower']:
            #     withholding_tax_amount -= payslip.rule_parameter('isolated_deduction')
            if (
                employee.marital in ["divorced", "single", "widower"]
                and employee.dependent_children
            ):
                withholding_tax_amount -= payslip.rule_parameter(
                    "disabled_dependent_deduction"
                )
            ...
            if employee.other_dependent_people and employee.dependent_seniors:
                withholding_tax_amount -= (
                    payslip.rule_parameter("dependent_senior_deduction")
                    * employee.dependent_seniors
                )
            if employee.other_dependent_people and employee.dependent_juniors:
                withholding_tax_amount -= (
                    payslip.rule_parameter("disabled_dependent_deduction")
                    * employee.dependent_juniors
                )
            if (
                employee.marital in ["married", "cohabitant"]
                and employee.spouse_fiscal_status == "low_income"
            ):
                withholding_tax_amount -= payslip.rule_parameter(
                    "spouse_low_income_deduction"
                )
            if (
                employee.marital in ["married", "cohabitant"]
                and employee.spouse_fiscal_status == "low_pension"
            ):
                withholding_tax_amount -= payslip.rule_parameter(
                    "spouse_other_income_deduction"
                )
        if (
            employee.marital in ["married", "cohabitant"]
            and employee.spouse_fiscal_status == "without_income"
    ...
    if n_children > 0:
        children_deduction = payslip.rule_parameter(
            "dependent_basic_children_deduction"
        )
        if n_children <= 8:
            withholding_tax_amount -= children_deduction.get(n_children, 0.0)
        if n_children > 8:
            withholding_tax_amount -= children_deduction.get(8, 0.0) + (
                n_children - 8
            ) * payslip.rule_parameter("dependent_children_deduction")

    if payslip.contract_id.fiscal_voluntarism:
        voluntary_amount = (
            categories.GROSS * payslip.contract_id.fiscal_voluntary_rate / 100
        )
...
def compute_special_social_cotisations(payslip, categories, worked_days, inputs):
    ...
    if employee.resident_bool:
        result = 0.0
    elif employee.marital in ["divorced", "single", "widower"] or (
        employee.marital in ["married", "cohabitant"]
        and employee.spouse_fiscal_status == "without_income"
    ):
...
def compute_employment_bonus_employees(payslip, categories, worked_days, inputs):
    ...
    if not payslip.dict.worked_days_line_ids and not payslip.env.context.get(
        "salary_simulation"
    ):
        return 0
    ...
    if payslip.env.context.get("salary_simulation"):
        ...
        worked_days = payslip.dict.worked_days_line_ids.filtered(
            lambda wd: wd.code not in ["LEAVE300", "LEAVE301"]
        )
        paid_hours = sum(
            worked_days.filtered(lambda wd: wd.amount).mapped("number_of_hours")
        )  # H
...
def compute_double_holiday_withholding_taxes(payslip, categories, worked_days, inputs):
    ...
    if contract.transport_mode_car:
        if "vehicle_id" in payslip.dict:
            yearly_revenue += (
                payslip.dict.vehicle_id._get_car_atn(date=payslip.dict.date_from) * 12.0
            )
        else:
    ...
    if children > 0 and yearly_revenue <= children_exoneration.get(
        children, children_exoneration[12]
    ):
        yearly_revenue -= (
            children_exoneration.get(children, children_exoneration[12])
            - yearly_revenue
        )
    ...
    if (
        children > 0
        and yearly_revenue <= children_reduction.get(children, children_reduction[5])[1]
    ):
        withholding_tax_amount = (
            gross
            * find_rates(yearly_revenue, rates)
            * (1 - children_reduction.get(children, children_reduction[5])[0] / 100.0)
        )
    else:
...
def compute_impulsion_plan_amount(payslip, categories, worked_days, inputs):
    ...
    numerator = sum(
        wd.number_of_hours for wd in payslip.dict.worked_days_line_ids if wd.amount > 0
    )
...
def compute_onss_restructuring(payslip, categories, worked_days, inputs):
    ...
    paid_hours = sum(
        payslip.worked_days_line_ids.filtered(lambda wd: wd.amount).mapped(
            "number_of_hours"
        )
    )
...
def compute_representation_fees(payslip, categories, worked_days, inputs):
    ...
    if not categories.BASIC:
        ...
        incapacity_attendances = calendar.attendance_ids.filtered(
            lambda a: a.work_entry_type_id.code == "LEAVE281"
        )
        if incapacity_attendances:
            incapacity_hours = sum(
                (attendance.hour_to - attendance.hour_from)
                for attendance in incapacity_attendances
            )
            incapacity_hours = (
                incapacity_hours / 2
                if calendar.two_weeks_calendar
                else incapacity_hours
            )
            incapacity_rate = (
                (1 - incapacity_hours / calendar.hours_per_week)
                if calendar.hours_per_week
                else 0
            )
            work_time_rate = (
                contract.resource_calendar_id.work_time_rate * incapacity_rate
            )
        else:
            work_time_rate = contract.resource_calendar_id.work_time_rate

        threshold = (
            0 if (worked_days.OUT and worked_days.OUT.number_of_hours) else 279.31
        )
        if days_per_week and contract.representation_fees > threshold:
            ...
            if (
                ...
                and work_time_rate
                and work_time_rate < 100
                and days_per_week == 5
            ):
                total_amount = (
                    threshold
                    + (contract.representation_fees - threshold) * work_time_rate / 100
                )
            ...
            if total_amount > threshold:
                ...
                result = max(
                    0,
                    total_amount
                    - daily_amount * payslip.representation_fees_missing_days,
                )
        elif days_per_week:
...
def compute_serious_representation_fees(payslip, categories, worked_days, inputs):
    return min(
        compute_representation_fees(payslip, categories, worked_days, inputs), 279.31
    )
