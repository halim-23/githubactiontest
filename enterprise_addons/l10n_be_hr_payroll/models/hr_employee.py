...
class HrEmployee(models.Model):
    ...
    spouse_fiscal_status = fields.Selection(
        ...
        groups="hr.group_hr_user",
        default="without_income",
        required=False,
    )
    spouse_fiscal_status_explanation = fields.Char(
        compute="_compute_spouse_fiscal_status_explanation"
    )
    ...
    l10n_be_dependent_children_attachment = fields.Integer(
        string="# dependent children for salary attachement",
        groups="hr.group_hr_user",
...
Source: Opinion on the indexation of the amounts set in Article 1, paragraph 4, of the Royal Decree of 27 December 2004 implementing Articles 1409, § 1, paragraph 4, and 1409, § 1 bis, paragraph 4 , of the Judicial Code relating to the limitation of seizure when there are dependent children, MB, December 13, 2019.""",
    ...
    start_notice_period = fields.Date(
        "Start notice period", groups="hr.group_hr_user", copy=False, tracking=True
    )
    end_notice_period = fields.Date(
        "End notice period", groups="hr.group_hr_user", copy=False, tracking=True
    )
    first_contract_in_company = fields.Date(
        "First contract in company", groups="hr.group_hr_user", copy=False
    )

    has_bicycle = fields.Boolean(
        string="Bicycle to work",
        default=False,
        groups="hr.group_hr_user",
        help="Use a bicycle as a transport mode to go to work",
    )
    certificate = fields.Selection(
        selection_add=[("civil_engineer", "Master: Civil Engineering")]
    )
    l10n_be_scale_seniority = fields.Integer(
        string="Seniority at Hiring", groups="hr.group_hr_user", tracking=True
    )
    ...
    def _compute_spouse_fiscal_status_explanation(self):
        low_income_threshold = (
            ...
            .sudo()
            ._get_parameter_from_code("spouse_low_income_threshold")
        )
        other_income_threshold = (
            self.env["hr.rule.parameter"]
            .sudo()
            ._get_parameter_from_code("spouse_other_income_threshold")
        )
        for employee in self:
            employee.spouse_fiscal_status_explanation = _(
    ...
    def _compute_dependent_children(self):
        for employee in self:
            if employee.disabled_children_bool:
                employee.dependent_children = (
                    employee.children + employee.disabled_children_number
                )
            else:
