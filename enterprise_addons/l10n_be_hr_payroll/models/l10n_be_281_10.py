...
class L10nBe28110(models.Model):
    ...
    state = fields.Selection(
        [("generate", "generate"), ("get", "get")], default="generate"
    )
    ...
    error_message = fields.Char(
        "Error Message", compute="_compute_validation_state", store=True
    )
    ...
    def _compute_validation_state(self):
        ...
        no_xml_file_records.update(
            {"xml_validation_state": "normal", "error_message": False}
        )
    ...
    def _check_employees_configuration(self, employees):
        invalid_employees = employees.filtered(
            lambda e: not (
                e.company_id
                and e.company_id.street
                and e.company_id.zip
                and e.company_id.city
                and e.company_id.phone
                and e.company_id.vat
            )
        )
        ...
        invalid_employees = employees.filtered(
            lambda e: not e.address_home_id
            or not e.address_home_id.street
            or not e.address_home_id.zip
            or not e.address_home_id.city
            or not e.address_home_id.country_id
        )
        if invalid_employees:
            raise UserError(
                _(
        ...
        invalid_country_codes = employees.address_home_id.country_id.filtered(
            lambda c: c.code not in COUNTRY_CODES
        )
        if invalid_country_codes:
            raise UserError(
                _(
                    "Unsupported country code %s. Please contact an administrator.",
                    ", ".join(invalid_country_codes.mapped("code")),
                )
            )
    ...
    def _get_atn_nature(self, payslips):
        result = ""
        if any(
            payslip.vehicle_id or payslip.contract_id.car_id for payslip in payslips
        ):
            result += "F"
    ...
    def _get_rendering_data(self):
        ...
        if len(phone) > 12:
            raise UserError(
                _("The company phone number shouldn't exceed 12 characters")
            )
        ...
        main_data = {
            ...
            "v0022_taalcode": self._get_lang_code(
                self.env.user.employee_id.address_home_id.lang
            ),
        ...
        for employee in employee_payslips:
            ...
            warrant_gross = sum(
                all_line_values["GROSS"][p.id]["total"]
                for p in payslips
                if p.struct_id == warrant_structure
            )
            holiday_gross = sum(
                all_line_values["GROSS"][p.id]["total"]
                for p in payslips
                if p.struct_id in holiday_n_structure + holiday_n1_structure
            )
            ...
            if len(postcode) > 4 or not postcode.isdecimal():
                raise UserError(
                    _(
                        "The belgian postcode length shouldn't exceed 4 characters and should contain only numbers for employee %s",
                        employee.name,
                    )
                )
            ...
            if len(first_name) > 30:
                raise UserError(
                    _(
                        "The employee first name shouldn't exceed 30 characters for employee %s",
                        employee.name,
                    )
                )
            ...
            sheet_values = {
                ...
                "f2018_landwoonplaats": "150"
                if is_belgium
                else self._get_country_code(employee.address_home_id.country_id),
                ...
                "f2112_buitenlandspostnummer": employee.address_home_id.zip
                if not is_belgium
                else "0",
                ...
                "f10_2055_datumvanindienstt": employee.first_contract_date.strftime(
                    "%d-%m-%Y"
                )
                if employee.first_contract_date
                else "",
                "f10_2056_datumvanvertrek": employee.end_notice_period.strftime(
                    "%d-%m-%Y"
                )
                if employee.end_notice_period
                else "",
                ...
                "f10_2074_bedrijfsvoorheffing": _to_eurocent(
                    round(mapped_total["PPTOTAL"], 2)
                ),  # 2.074 = 2.131 + 2.133. YTI Is it ok to include PROF_TAX / should include Double holidays?
                "f10_2075_bijzonderbijdrage": _to_eurocent(
                    round(-mapped_total["M.ONSS"], 2)
                ),
                "f10_2076_voordelenaardbedrag": _to_eurocent(
                    round(
                        sum(
                            mapped_total[code]
                            for code in ["ATN.INT", "ATN.MOB", "ATN.LAP", "ATN.CAR"]
                        ),
                        2,
                    )
                ),
                ...
                "f10_2078_compensationamountwithoutstandards": _to_eurocent(
                    round(mapped_total["REP.FEES"], 2)
                ),
                ...
                "f10_2086_openbaargemeenschap": _to_eurocent(
                    round(mapped_total["PUB.TRANS"], 2)
                ),
                "f10_2087_bedrag": 0,
                ...
                "f10_2131_bedrijfsvoorheffingvanwerkgever": _to_eurocent(
                    round(mapped_total["PPTOTAL"], 2)
                ),
            ...
            sheet_values["f10_2062_totaal"] = sum(
                sheet_values[code]
                for code in [
            ...
            sheet_values["f10_2077_totaal"] = sum(
                sheet_values[code]
                for code in [
            ...
            sheet_values["f10_2059_totaalcontrole"] = sum(
                sheet_values[code]
                for code in [
                    "f10_2060_gewonebezoldiginge",
        ...
        sum_2009 = sum(
            sheet_values["f2009_volgnummer"] for sheet_values in employees_data
        )
        sum_2059 = sum(
            sheet_values["f10_2059_totaalcontrole"] for sheet_values in employees_data
        )
        sum_2074 = sum(
            sheet_values["f10_2074_bedrijfsvoorheffing"]
    ...
    def _action_generate_pdf(self, post_process=False):
        ...
        template_sudo = self.env.ref(
            "l10n_be_hr_payroll.action_report_employee_281_10"
        ).sudo()
        ...
        for sheet in rendering_data["employees_data"]:
            ...
            counter += 1
            sheet_filename = "%s-%s-281_10" % (
                sheet["f2002_inkomstenjaar"],
                sheet["f2013_naam"],
            )
            sheet_file, dummy = template_sudo._render_qweb_pdf(
                sheet["employee_id"], data={**sheet, **rendering_data["data"]}
            )
            pdf_files.append((sheet["employee"], sheet_filename, sheet_file))

        if pdf_files:
            filename, binary = self._process_files(
                pdf_files,
                default_filename="281.10 PDF - %s.zip" % fields.Date.today(),
    ...
    def action_generate_xml(self):
        ...
        xml_str = self.env.ref("l10n_be_hr_payroll.281_10_xml_report")._render(
