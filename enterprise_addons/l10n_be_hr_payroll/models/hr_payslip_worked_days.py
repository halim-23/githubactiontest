...
class HrPayslipWorkedDays(models.Model):
    ...
    @api.depends(
        ...
        "number_of_hours",
        "payslip_id",
        "payslip_id.normal_wage",
        "payslip_id.sum_worked_hours",
    )
    def _compute_amount(self):
        monthly_self = self.filtered(
            lambda wd: not wd.payslip_id.edited and wd.payslip_id.wage_type == "monthly"
        )

        credit_time_days = monthly_self.filtered(
            lambda worked_day: worked_day.is_credit_time
        )
        ...
        for wd in variable_salary_wd:
            ...
            if not float_compare(
                wd.payslip_id.contract_id.resource_calendar_id.work_time_rate,
                100,
                precision_digits=2,
            ):
                amount *= 1.2
        ...
        if paid_be_wds:
            for be_wd in paid_be_wds:
                ...
                calendar = (
                    ...
                    or payslip.employee_id.resource_calendar_id
                )
                ...
                paid_hours = sum(
                    be_wd.payslip_id.worked_days_line_ids.filtered(
                        lambda wd: wd.is_paid
                        and wd.work_entry_type_id.code
                        not in ["OUT", "LEAVE300", "LEAVE301"]
                    ).mapped("number_of_hours")
                )
                if (
                    paid_hours < hours_per_week
                    and be_wd.work_entry_type_id.code
                    not in ["OUT", "LEAVE300", "LEAVE301"]
                ):
                    be_wd.amount = (
                        wage * 3 / (13 * hours_per_week) * be_wd.number_of_hours
                    )
                    continue

                # If out of contract, we use the hourly formula to deduct the real wage
                out_be_wd = be_wd.payslip_id.worked_days_line_ids.filtered(
                    lambda wd: wd.code == "OUT"
                )
                if out_be_wd:
                    ...
                    if payslip.contract_id.time_credit:
                        if contract.date_start > payslip.date_from:
                            ...
                            credit_time_attendances = payslip.contract_id.resource_calendar_id._attendance_intervals_batch(
                                start_dt, end_dt
                            )[
                                False
                            ]
                            standard_attendances = payslip.contract_id.standard_calendar_id._attendance_intervals_batch(
                                start_dt, end_dt
                            )[
                                False
                            ]
                            out_hours -= sum(
                                [
                                    ...
                                    for start, stop, dummy in standard_attendances
                                    - credit_time_attendances
                                ]
                            )
                    ...
                    out_ratio = (
                        1 - 3 / (13 * hours_per_week) * out_hours
                        if hours_per_week
                        else 1
                    )
                else:
                ...
                max_hours_per_week = (
                    ...
                    or contract.resource_calendar_id.hours_per_week
                )
                training_ratio = (
                    3 / (13 * max_hours_per_week) if max_hours_per_week else 0
                )
                training_wds = paid_be_wds.filtered(
                ...
                if training_wds:
                    ...
                    training_threshold = (
                        ...
                        ._get_parameter_from_code(
                            "training_time_off_threshold",
                            payslip.date_to,
                            raise_if_not_found=False,
                        )
                    )
                    if wage > training_threshold:
                        hourly_wage_to_deduct = (
                            wage - training_threshold
                        ) * training_ratio
                ...
                if main_worked_day not in be_wd.payslip_id.worked_days_line_ids.mapped(
                    "code"
                ):
                    main_worked_day = be_wd.payslip_id.worked_days_line_ids.filtered(
                        lambda wd: wd.is_paid
                        and wd.code not in ["LEAVE300", "LEAVE301"]
                    )
                    main_worked_day = (
                        main_worked_day[0].code if main_worked_day else False
                    )
                ...
                if be_wd.code == "OUT":
                    ...
                    worked_day_amount = (
                        min(wage, training_threshold) * training_ratio * training_hours
                    )
                elif be_wd.code == main_worked_day:  # WORK100 (Generally)
                    # Case with half days mixed with full days
                    work100_wds = be_wd.payslip_id.worked_days_line_ids.filtered(
                        lambda wd: wd.code == "WORK100"
                    )
                    number_of_hours = sum(
                        [
                            wd.number_of_hours
                    ...
                    if len(work100_wds) > 1:
                        ...
                        if (
                            len(
                                list(
                                    set(
                                        be_wd.payslip_id.worked_days_line_ids.mapped(
                                            "code"
                                        )
                                    )
                                )
                            ...
                            ratio = (
                                (
                                    out_ratio
                                    - 3 / (13 * hours_per_week) * number_of_hours
                                )
                                if hours_per_week
                                else 0
                            )
                            ...
                            if float_compare(
                                be_wd.number_of_hours,
                                max(work100_wds.mapped("number_of_hours")),
                                2,
                            ):  # lowest lines
                                ...
                                if be_wd not in work100_wds:
                                    ...
                                    ratio = (
                                        3
                                        / (13 * hours_per_week)
                                        * (work100_wds - be_wd).number_of_hours
                                        if hours_per_week
                                        else 0
                                    )
                                ...
                                ratio = (
                                    3 / (13 * hours_per_week) * be_wd.number_of_hours
                                    if hours_per_week
                                    else 0
                                )
                        ...
                        ratio = (
                            (out_ratio - 3 / (13 * hours_per_week) * number_of_hours)
                            if hours_per_week
                            else 0
                        )
        ...
        super(
            HrPayslipWorkedDays,
            self - credit_time_days - paid_be_wds - variable_salary_wd,
        )._compute_amount()
