...
class HolidaysRequest(models.Model):
    ...
    def action_validate(self):
        ...
        for leave in self:
            if (
                leave.employee_id.company_id.country_id.code == "BE"
                and leave.holiday_status_id.work_entry_type_id.code
                in self._get_drs_work_entry_type_codes()
            ):
                ...
                leave.activity_schedule(
                    ...
                    note=_(
                        "%s is in %s. Fill in the appropriate eDRS here: %s",
                        leave.employee_id.name,
                        leave.holiday_status_id.name,
                        drs_link,
                    ),
                    user_id=leave.holiday_status_id.responsible_id.id
                    or self.env.user.id,
                )
        return res
