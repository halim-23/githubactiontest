...
class ResConfigSettings(models.TransientModel):
    ...
    default_meal_voucher_amount = fields.Float(
        string="Meal Vouchers", default_model="hr.contract"
    )
    default_eco_checks = fields.Float(
        string="Eco Vouchers", default_model="hr.contract"
    )
    onss_company_id = fields.Char(related="company_id.onss_company_id", readonly=False)
    onss_registration_number = fields.Char(
        related="company_id.onss_registration_number", readonly=False
    )
    dmfa_employer_class = fields.Char(
        related="company_id.dmfa_employer_class", readonly=False
    )
    l10n_be_company_number = fields.Char(
        "Company Number", related="company_id.l10n_be_company_number", readonly=False
    )
    l10n_be_revenue_code = fields.Char(
        "Revenue Code", related="company_id.l10n_be_revenue_code", readonly=False
    )
    ...
    l10n_be_ffe_employer_type = fields.Selection(
        related="company_id.l10n_be_ffe_employer_type", readonly=False
    )
