    ...
    def apply_directly(self):
        for record in self:
            ...
            if record.contract_id.state in ["draft", "open"]:
                number_of_days = self.env[
                    "l10n_be.hr.payroll.schedule.change.wizard"
                ]._compute_new_allocation(
                    record.leave_allocation_id,
                    record.current_resource_calendar_id,
                ...
                record.leave_allocation_id._message_log(
                    body=_(
                        "New working schedule on %(contract_name)s.<br/>"
                        "New total : %(days)s"
                    )
                    % {"contract_name": record.contract_id.name, "days": number_of_days}
                )
