...
class L10nBe274XX(models.Model):
    ...
    month = fields.Selection(
        ...
        default=lambda self: str(
            (fields.Date.today() + relativedelta(months=-1)).month
        ),
    )
    date_start = fields.Date(
        "Start Period", store=True, readonly=False, compute="_compute_dates"
    )
    date_end = fields.Date(
        "End Period", store=True, readonly=False, compute="_compute_dates"
    )
    ...
    state = fields.Selection(
        [("draft", "Draft"), ("waiting", "Waiting"), ("done", "Done")], default="draft"
    )
    sheet_274_10 = fields.Binary("274.10 Sheet", readonly=True, attachment=False)
    sheet_274_10_filename = fields.Char()
    pp_amount = fields.Monetary(
        "Withholding Taxes", compute="_compute_line_ids", compute_sudo=True
    )
    ...
    taxable_amount = fields.Monetary(
        "Taxable Amount", compute="_compute_line_ids", compute_sudo=True
    )
    ...
    deducted_amount = fields.Monetary(
        "Exempted Amount", compute="_compute_line_ids", compute_sudo=True
    )
    ...
    capped_amount_34 = fields.Monetary(
        "Capped Amount", compute="_compute_line_ids", compute_sudo=True
    )
            ...
            record.update(
                {
                    ...
                    "date_end": date(record.year, int(record.month), 1)
                ...
                [
                    ...
                    [
                        (
                            "employee_id",
                            "in",
                            self.env.context["wizard_274xx_force_employee_ids"],
                        )
                    ],
                ]
            )
        ...
        for company in self.mapped("company_id"):
            if not company.l10n_be_company_number or not company.l10n_be_revenue_code:
                raise ValidationError(
                    _(
                        "Please configure the 'Company Number' and the 'Revenue Code' on the Payroll Settings."
                    )
                )
    ...
    def _compute_line_ids(self):
        ...
        for sheet in self:
            ...
            line_values = payslips._get_line_values(
                ["GROSS", "PPTOTAL", "DOUBLE.DECEMBER.GROSS", "DOUBLE.DECEMBER.P.P"],
                compute_sum=True,
            )

            payslips = payslips.filtered(
                lambda p: line_values["PPTOTAL"][p.id]["total"]
            )

            # Total
            sheet.taxable_amount = (
                line_values["GROSS"]["sum"]["total"]
                + line_values["DOUBLE.DECEMBER.GROSS"]["sum"]["total"]
            )
            sheet.pp_amount = (
                line_values["PPTOTAL"]["sum"]["total"]
                + line_values["DOUBLE.DECEMBER.P.P"]["sum"]["total"]
            )
            # Valid payslips for exemption
            payslips = payslips.filtered(
                lambda p: p.contract_id.rd_percentage and p.struct_id == monthly_pay
            )
            # 32 : Civil Engineers / Doctors
            payslips_32 = payslips.filtered(
                lambda p: p.employee_id.certificate in ["doctor", "civil_engineer"]
            )
            sheet.taxable_amount_32 = sum(
                line_values["GROSS"][p.id]["total"] for p in payslips_32
            )
            sheet.pp_amount_32 = sum(
                line_values["PPTOTAL"][p.id]["total"] for p in payslips_32
            )

            # 33 : Masters
            payslips_33 = payslips.filtered(
                lambda p: p.employee_id.certificate in ["master"]
            )
            sheet.taxable_amount_33 = sum(
                line_values["GROSS"][p.id]["total"] for p in payslips_33
            )
            sheet.pp_amount_33 = sum(
                line_values["PPTOTAL"][p.id]["total"] for p in payslips_33
            )
            # 33 : Bachelors
            payslips_34 = payslips.filtered(
                lambda p: p.employee_id.certificate in ["bachelor"]
            )
            sheet.taxable_amount_34 = sum(
                line_values["GROSS"][p.id]["total"] for p in payslips_34
            )
            sheet.pp_amount_34 = sum(
                line_values["PPTOTAL"][p.id]["total"] for p in payslips_34
            )
            sheet.write(
                {
                    "deducted_amount": 0,
            ...
            for payslip in payslips:
                if payslip.contract_id.rd_percentage:
                    deducted_amount = (
                        payslip.contract_id.rd_percentage
                        / 100
                        * 0.8
                    ...
                    mapped_pp[payslip.employee_id] += (
                        payslip.contract_id.rd_percentage
                        / 100
                        * 0.8
                        * line_values["PPTOTAL"][payslip.id]["total"]
                    )
                    mapped_taxable_amount[payslip.employee_id] += line_values["GROSS"][
                        payslip.id
                    ]["total"]
    ...
    def _get_rendering_data(self):
        ...
        invalid_payslips = payslips.filtered(
            lambda p: p.date_from != date_from or p.date_to != date_to
        )
        if invalid_payslips:
            raise UserError(
                _(
        ...
        for payslip in payslips:
            ...
            if pp_total:
                ...
                taxable_eurocent = (
                    line_values["GROSS"][payslip.id]["total"]
                    + line_values["DOUBLE.DECEMBER.GROSS"][payslip.id]["total"]
                )
        ...
        for declaration in [
            ...
            declaration["taxable_revenue"] = _to_eurocent(
                declaration["taxable_revenue"]
            )
    ...
    def action_generate_xls(self):
        ...
        certificate_selection_vals = {
            elem[0]: elem[1]
            for elem in self.env["hr.employee"]
            ._fields["certificate"]
        ...
        for line in self.line_ids:
            ...
            rows.append(
                (
                    ...
                    or employee.contract_id.department_id.name,
                    employee.first_contract_date.strftime("%d-%m-%Y")
                    if employee.first_contract_date
                    else "",
                    employee.departure_date.strftime("%d-%m-%Y")
