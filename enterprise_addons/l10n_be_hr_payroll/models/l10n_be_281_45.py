...
class L10nBe28145(models.Model):
    ...
    def _get_rendering_data(self):
        ...
        for employee in employee_payslips:
            ...
            sheet_values = {
                ...
                "f45_2061_forfaitairekosten": _to_eurocent(
                    round(mapped_total["IP"] / 2.0, 2)
                ),
                "f45_2062_werkelijkekosten": 0,
                "f45_2063_roerendevoorheffing": _to_eurocent(
                    round(-mapped_total["IP.DED"], 2)
                ),
                "f45_2099_comment": "",
            ...
            sheet_values["f45_2059_totaalcontrole"] = sum(
                sheet_values[code]
                for code in [
        ...
        sum_2059 = sum(
            sheet_values["f45_2059_totaalcontrole"] for sheet_values in employees_data
        )
        sum_2063 = sum(
            sheet_values["f45_2063_roerendevoorheffing"]
    ...
    def _action_generate_pdf(self, post_process=False):
        ...
        template_sudo = self.env.ref(
            "l10n_be_hr_payroll.action_report_employee_281_45"
        ).sudo()

        pdf_files = []
        for sheet in rendering_data["employees_data"]:
            sheet_filename = "%s-%s-281_45" % (
        ...
        if pdf_files:
            filename, binary = self._process_files(
                pdf_files,
                default_filename="281.45 PDF - %s.zip" % fields.Date.today(),
    ...
    def action_generate_xml(self):
        ...
        xml_str = self.env.ref("l10n_be_hr_payroll.281_45_xml_report")._render(
