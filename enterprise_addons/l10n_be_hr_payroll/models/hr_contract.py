...
class HrContract(models.Model):
    ...
    train_transport_employee_amount = fields.Monetary(
        "Train transport paid by the employee (Monthly)"
    )
    public_transport_employee_amount = fields.Monetary(
        "Public transport paid by the employee (Monthly)"
    )
    warrant_value_employee = fields.Monetary(
        compute="_compute_commission_cost",
        string="Warrant monthly value for the employee",
    )

    meal_voucher_paid_by_employer = fields.Monetary(
        compute="_compute_meal_voucher_info", string="Meal Voucher Paid by Employer"
    )
    meal_voucher_paid_monthly_by_employer = fields.Monetary(
        compute="_compute_meal_voucher_info"
    )
    company_car_total_depreciated_cost = fields.Monetary()
    private_car_reimbursed_amount = fields.Monetary(
        compute="_compute_private_car_reimbursed_amount"
    )
    km_home_work = fields.Integer(
        related="employee_id.km_home_work", related_sudo=True, readonly=False
    )
    ...
    warrants_cost = fields.Monetary(
        compute="_compute_commission_cost",
        string="Warrant monthly cost for the employer",
    )
    ...
    ip_wage_rate = fields.Float(
        string="IP percentage", help="Should be between 0 and 100 %"
    )
    ip_value = fields.Float(compute="_compute_ip_value")
    # Please stop making this field readonly
    time_credit = fields.Boolean(
        "Part Time", readonly=False, help="This is a part time contract."
    )
    ...
    time_credit_full_time_wage = fields.Monetary(
        ...
        compute="_compute_time_credit_full_time_wage",
        store=True,
        readonly=False,
    )
    ...
    time_credit_type_id = fields.Many2one(
        "hr.work.entry.type",
        string="Part Time Work Entry Type",
    ...
    fiscal_voluntary_rate = fields.Float(
        string="Fiscal Voluntary Rate", help="Should be between 0 and 100 %"
    )
    ...
    l10n_be_impulsion_plan = fields.Selection(
        ...
        string="Impulsion Plan",
    )
    l10n_be_onss_restructuring = fields.Boolean(
        string="Allow ONSS Reduction for Restructuring"
    )

    _sql_constraints = [
        (
            "check_percentage_ip_rate",
            "CHECK(ip_wage_rate >= 0 AND ip_wage_rate <= 100)",
            "The IP rate on wage should be between 0 and 100.",
        ),
        (
            "check_percentage_fiscal_voluntary_rate",
            "CHECK(fiscal_voluntary_rate >= 0 AND fiscal_voluntary_rate <= 100)",
            "The Fiscal Voluntary rate on wage should be between 0 and 100.",
        ),
    ]

    has_hospital_insurance = fields.Boolean(
        string="Has Hospital Insurance",
        groups="hr_contract.group_hr_contract_manager",
        tracking=True,
    )
    insured_relative_children = fields.Integer(
        string="# Insured Children < 19 y/o",
        groups="hr_contract.group_hr_contract_manager",
        tracking=True,
    )
    insured_relative_adults = fields.Integer(
        string="# Insured Children >= 19 y/o",
        groups="hr_contract.group_hr_contract_manager",
        tracking=True,
    )
    insured_relative_spouse = fields.Boolean(
        string="Insured Spouse",
        groups="hr_contract.group_hr_contract_manager",
        tracking=True,
    )
    hospital_insurance_amount_per_child = fields.Float(
        ...
        default=lambda self: float(
            ...
            .sudo()
            .get_param(
                "hr_contract_salary.hospital_insurance_amount_child", default=7.2
            )
        ),
    )
    hospital_insurance_amount_per_adult = fields.Float(
        string="Amount per Adult",
        groups="hr_contract.group_hr_contract_manager",
        default=lambda self: float(
            self.env["ir.config_parameter"]
            .sudo()
            .get_param(
                "hr_contract_salary.hospital_insurance_amount_adult", default=20.5
            )
        ),
    )
    ...
    insured_relative_adults_total = fields.Integer(
        compute="_compute_insured_relative_adults_total",
        groups="hr_contract.group_hr_contract_manager",
    )

    l10n_be_is_below_scale = fields.Boolean(
        string="Is below CP200 salary scale",
        compute="_compute_l10n_be_is_below_scale",
        search="_search_l10n_be_is_below_scale",
    )
    ...
    @api.depends(
        ...
        "standard_calendar_id.hours_per_week",
    )
        ...
        for contract in self:
            if contract.time_credit:
                contract.work_time_rate = (
                    ...
                    / contract.standard_calendar_id.hours_per_week
                )
            else:
                contract.work_time_rate = (
                    contract.resource_calendar_id.hours_per_week
                    / contract.company_id.resource_calendar_id.hours_per_week
                )

    @api.depends(
        "wage",
        "state",
        "employee_id.l10n_be_scale_seniority",
        "job_id.l10n_be_scale_category",
        "work_time_rate",
        "time_credit",
        "resource_calendar_id.work_time_rate",
    )
    def _compute_l10n_be_is_below_scale(self):
        ...
        student_stucture_type = self.env.ref(
            "hr_contract.structure_type_employee_cp200"
        )
        ...
        (self - open_contracts).write(
            {"l10n_be_is_below_scale_warning": False, "l10n_be_is_below_scale": False}
        )
        category_mapping = {
            "A": 0,
            "B": 1,
        ...
        for contract in open_contracts:
            company_seniority = relativedelta(
                fields.Date.today(), contract.first_contract_date
            ).years
            if not company_seniority:
                scales = self.env["hr.rule.parameter"]._get_parameter_from_code(
                    "cp200_salary_scale_first_year", raise_if_not_found=False
                )
            else:
                scales = self.env["hr.rule.parameter"]._get_parameter_from_code(
                    "cp200_salary_scale", raise_if_not_found=False
                )
            if not scales:
            ...
            category_index = category_mapping.get(
                contract.job_id.l10n_be_scale_category, 2
            )
            ...
            if contract._get_contract_wage() < min_wage:
                ...
                contract.l10n_be_is_below_scale_warning = _(
    ...
    def _search_l10n_be_is_below_scale(self, operator, value):
        ...
        below_contracts = (
            ...
            .search([("state", "in", ["draft", "open"])])
            .filtered(
                lambda c: c.company_id.country_id.code == "BE"
                and c.l10n_be_is_below_scale
            )
        )
    ...
    @api.depends(
        "has_hospital_insurance", "insured_relative_adults", "insured_relative_spouse"
    )
    ...
    @api.model
    def _get_insurance_amount(
        self, child_amount, child_count, adult_amount, adult_count
    ):
        return child_amount * child_count + adult_amount * adult_count

    @api.depends(
        "insured_relative_children",
        "insured_relative_adults_total",
        "hospital_insurance_amount_per_child",
        "hospital_insurance_amount_per_adult",
    )
    ...
    def _check_discount_percentage(self):
        if self.filtered(lambda c: c.rd_percentage < 0 or c.rd_percentage > 100):
            raise ValidationError(
                _("The time Percentage in R&D should be between 1-100")
            )
        for contract in self:
            if contract.rd_percentage and contract.employee_id.certificate not in [
                "civil_engineer",
                "doctor",
                "master",
                "bachelor",
                ...
                raise ValidationError(
                    _(
                        "Only employeers with a Bachelor/Master/Doctor/Civil Engineer degree can benefit from the withholding taxes exemption."
                    )
                )
    ...
    def _compute_commission_cost(self):
        for contract in self:
            ...
            contract.yearly_commission_cost = warrant_commission + cash_commission * (
                1 + EMPLOYER_ONSS
            )
            contract.yearly_commission = warrant_commission + cash_commission
            contract.warrant_value_employee = (
                contract.commission_on_target * 1.326 * (1.00 - 0.535)
            )
    ...
    def _compute_meal_voucher_info(self):
        ...
        for contract in self:
            ...
            contract.meal_voucher_paid_monthly_by_employer = (
                contract.meal_voucher_paid_by_employer * monthly_nb_meal_voucher
            )
            contract.meal_voucher_average_monthly_amount = (
                contract.meal_voucher_amount * monthly_nb_meal_voucher
            )

    @api.depends("train_transport_employee_amount")
    def _compute_train_transport_reimbursed_amount(self):
        for contract in self:
            contract.train_transport_reimbursed_amount = (
                contract._get_train_transport_reimbursed_amount(
                    contract.train_transport_employee_amount
                )
            )
    ...
    def _compute_public_transport_reimbursed_amount(self):
        for contract in self:
            contract.public_transport_reimbursed_amount = (
                contract._get_public_transport_reimbursed_amount(
    ...
    def _compute_employee_age(self):
        for contract in self:
            if not contract.employee_id or not contract.employee_id.birthday:
                ...
                contract.employee_age = relativedelta(
                    fields.Date.today(), contract.employee_id.birthday
                ).years
    ...
    def _get_private_car_reimbursed_amount(self, distance):
        ...
        private_car_reimbursement_scale = (
            ...
            ._get_parameter_from_code(
                "private_car_reimbursement_scale",
                date=self.env.context.get("payslip_date"),
    ...
    def update_state(self):
        ...
        for contract in nearly_expired_contracts.filtered(
            lambda contract: contract.hr_responsible_id
        ):
    ...
    def _get_work_hours_split_half(self, date_from, date_to, domain=None):
        ...
        number_of_hours_full_day = self.resource_calendar_id._get_max_number_of_hours(
            date_from, date_to
        )
        ...
        for day_data in work_entries:
            work_entry_type_id = (
                day_data["work_entry_type_id"][0]
                if day_data["work_entry_type_id"]
                else False
            )
            duration = day_data["hours"]
            if float_compare(day_data["hours"], number_of_hours_full_day, 2) != -1:
                if number_of_hours_full_day:
                    number_of_days = float_round(
                        duration / number_of_hours_full_day,
                        precision_rounding=1,
                        rounding_method="HALF-UP",
                    )
                else:
                    number_of_days = (
                        1  # If not supposed to work in calendar attendances, then there
                    )
                    # are not time offs
        ...
        for work_entry in work_entries:
            ...
            if work_entry.work_entry_type_id.is_leave:
                ...
                if (
                    float_compare(
                        contract_data.get("hours", 0), number_of_hours_full_day, 2
                    )
                    != -1
                ):
                ...
                work_data[("half", work_entry.work_entry_type_id.id)] += (
    ...
    def _get_leave_work_entry_type_dates(self, leave, date_from, date_to, employee):
        result = super()._get_leave_work_entry_type_dates(
            leave, date_from, date_to, employee
        )
        ...
        if result.code == "LEAVE500":
            ...
            if unemployed_less_than_14_days_before:
                ...
                for offset in range(15):
                    ...
                    if all(
                        l.date_from > day or l.date_to < day
                        for l in unemployed_less_than_14_days_before
                    ):
                        is_unemployed = False
                if is_unemployed:
                    return unemployed_less_than_14_days_before[
                        0
            ...
            absent_less_than_X_days_before = self.env["hr.leave"].search(
                [
                    ...
                    (
                        "holiday_status_id.work_entry_type_id.code",
                        "in",
                        ["LEAVE210", "LEAVE220", "LEAVE230", "LEAVE115", "LEAVE281"],
                    ),
                    ("state", "=", "validate"),
                ],
                order="date_from asc",
            ...
            if absent_less_than_X_days_before:
                ...
                if self.time_credit:
                    if not self.work_time_rate:
                        return absent_less_than_X_days_before[
                            0
                        ].holiday_status_id.work_entry_type_id
                    duration_start = self._get_occupation_dates()[0][1]
                    duration_stop = leave.date_from.date()
                    number_of_months = (
                        duration_stop.year - duration_start.year
                    ) * 12 + (duration_stop.month - duration_start.month)
                    ...
                    if number_of_months < 3:
                        paid_duration = 14
                        absent_less_than_X_days_before = (
                            absent_less_than_X_days_before.filtered_domain(
                                [
                                    (
                                        "date_to",
                                        ">=",
                                        leave.date_from
                                        + relativedelta(days=-paid_duration),
                                    )
                                ]
                            )
                        )
                for offset in range(paid_duration):
                    ...
                    if all(
                        l.date_from > day or l.date_to < day
                        for l in absent_less_than_X_days_before
                    ):
                        is_absent = False
                if is_absent:
                    return absent_less_than_X_days_before[
        ...
        if result.code == "LEAVE110":
            sick_work_entry_type = self.env.ref(
                "hr_work_entry_contract.work_entry_type_sick_leave"
            )
            partial_sick_work_entry_type = self.env.ref(
                "l10n_be_hr_payroll.work_entry_type_part_sick"
            )
            long_sick_work_entry_type = self.env.ref(
                "l10n_be_hr_payroll.work_entry_type_long_sick"
            )
            ...
            total_sick_days = sum(
                [
                    (l.date_to - l.date_from).days + 1
                    for l in sick_less_than_30days_before
                ]
            )
            this_leave_current_duration = (
                date_from - leave.holiday_id.date_from
            ).days + 1
    ...
    def _is_same_occupation(self, contract):
        ...
        return (
            res
            and time_credit == contract.time_credit
            and (not time_credit or (time_credit_type == contract.time_credit_type_id))
        )

    def _create_credit_time_next_activity(self):
        self.ensure_one()
        part_time_link = (
            "https://www.socialsecurity.be/site_fr/employer/applics/elo/index.htm"
        )
        part_time_link = '<a href="%s" target="_blank">%s</a>' % (
            part_time_link,
            part_time_link,
        )
        self.activity_schedule(
            "mail.mail_activity_data_todo",
            note=_(
                "Part Time of %s must be stated at %s.",
    ...
    def _create_dimona_next_activity(self):
        ...
        self.activity_schedule(
            "mail.mail_activity_data_todo",
            note=_(
                "State the Dimona at %s to declare the arrival of %s.",
                dimona_link,
                self.employee_id.name,
            ),
            user_id=self.hr_responsible_id.id or self.env.user.id,
    ...
    def _trigger_l10n_be_next_activities(self):
        ...
        employees_already_started = (
            self.env["hr.contract"]
            .search(employees_with_contract_domain)
            .mapped("employee_id")
        )
        for contract in self.filtered(
            lambda c: c.structure_type_id
            and c.structure_type_id.country_id.code == "BE"
        ):
            if contract.time_credit:
    ...
    def _get_fields_that_recompute_payslip(self):
        ...
        return super()._get_fields_that_recompute_payslip() + [
            ...
            "km_home_work",
            "has_laptop",
            "meal_voucher_amount" "work_time_rate",
            "fiscal_voluntarism",
