...
class ContractHistory(models.Model):
    ...
    time_credit = fields.Boolean(
        "Credit time", readonly=True, help="This is a credit time contract."
    )
    ...
    time_credit_full_time_wage = fields.Monetary(
        "Credit Time Full Time Wage", readonly=True
    )
    ...
    def _compute_l10n_be_is_below_scale(self):
        for history in self:
            ...
            history.l10n_be_is_below_scale_warning = (
                history.contract_id.l10n_be_is_below_scale_warning
            )
    ...
    def _search_l10n_be_is_below_scale(self, operator, value):
        ...
        below_histories = (
            ...
            .filtered(
                lambda h: h.company_id.country_id.code == "BE"
                and h.contract_id.l10n_be_is_below_scale
            )
        )
    ...
    def _compute_reference_data(self):
        non_credit_time_contracts_history = self.filtered(
            lambda contract_history: not contract_history.time_credit
        )
        credit_time_contracts_history = self.filtered(
            lambda contract_history: contract_history.time_credit
        )

        super(
            ContractHistory, non_credit_time_contracts_history
        )._compute_reference_data()

        mapped_employee_contract = defaultdict(
            lambda: self.env["hr.contract"],
            [
                (c.employee_id, c)
                for c in credit_time_contracts_history.mapped("contract_id")
            ],
        )
        for history in credit_time_contracts_history:
            history.reference_monthly_wage = mapped_employee_contract[
                history.employee_id
            ].time_credit_full_time_wage
            history.reference_yearly_cost = mapped_employee_contract[
                history.employee_id
            ].final_yearly_costs
    ...
    def _compute_has_valid_schedule_change_contract(self):
        for history in self:
            history.has_valid_schedule_change_contract = bool(
                self.contract_ids.filtered(lambda c: c.state in ("draft", "open"))
            )

    def action_work_schedule_change_wizard(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id(
            "l10n_be_hr_payroll.schedule_change_wizard_action"
        )
        # Use the valid contract starting on the furthest date
        valid_contracts = self.contract_ids.filtered(
