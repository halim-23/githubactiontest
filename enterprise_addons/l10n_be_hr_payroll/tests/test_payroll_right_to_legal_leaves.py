...
@tagged(
    "post_install_l10n", "post_install", "-at_install", "payroll_right_to_legal_leaves"
)
class TestPayrollRightToLegalLeaves(TestPayrollCommon):
    ...
    def setUpClass(cls):
        ...
        cls.resource_calendar_24_hours_per_week_5_days_per_week = (
            cls.resource_calendar.copy(
                {
        ...
        cls.resource_calendar_24_hours_per_week_4_days_per_week = (
            cls.resource_calendar.copy(
                {
                    "name": "Calendar 24 Hours/Week",
                    "full_time_required_hours": 38,
    ...
    def test_credit_time_for_employee_test_example1(self):
        ...
        wizard = self.env["hr.payroll.alloc.paid.leave"].new(
            {"year": 2017, "holiday_status_id": self.paid_time_off_type.id}
        )
        wizard._onchange_struct_id()
        wizard.alloc_employee_ids = wizard.alloc_employee_ids.filtered(
            lambda alloc_employee: alloc_employee.employee_id.id
            == self.employee_test.id
        )
        self.assertEqual(
            wizard.alloc_employee_ids.paid_time_off,
            20,
            "Employee Test should have 20 days for 2018",
        )
        ...
        wizard = (
            ...
            .with_context(
                ...
                active_id=employee_test_current_contract.id,
            )
            .new(
                {
        ...
        ]._cron_update_allocation_from_new_schedule(date(2018, 4, 1))
    ...
    def test_credit_time_for_employee_test_example2(self):
        ...
        self.assertEqual(
            ...
            16.5,
            "15 days left becomes 11.5 (15 * .78, rounded down) + 5 for the days already taken.",
        )
    ...
    def test_credit_time_for_employee_test_example3(self):
        ...
        employee_test_first_contract.write(
            {"date_end": date(2017, 5, 31), "state": "close"}
        )
        ...
        self.assertEqual(
            wizard.alloc_employee_ids.paid_time_off_to_allocate,
            10,
            "10 days is equal to 20 half days",
        )
        ...
        self.assertEqual(
            wizard.time_off_allocation, 10 + allocation.leaves_taken
        )  # Effectively allocation.number_of_days * 24/20 + leaves_taken
        view = wizard.with_context(force_schedule=True).action_validate()
        self.env[
            "l10n_be.schedule.change.allocation"
        ]._cron_update_allocation_from_new_schedule(date(2018, 4, 1))
        self.assertEqual(
            allocation.number_of_days,
            10 + allocation.leaves_taken,
            "10 days allocated by the credit + the number of leaves taken",
        )
