...
class TestPayrollCreditTime(TestPayrollCommon):
    def setUp(self):
        ...
        self.wizard.alloc_employee_ids = self.wizard.alloc_employee_ids.filtered(
            ...
            in [self.employee_georges.id, self.employee_john.id, self.employee_a.id]
        )
    ...
    def test_credit_time_for_georges(self):
        ...
        georges_allocation = self.allocations.filtered(
        ...
        wizard = (
            self.env["l10n_be.hr.payroll.schedule.change.wizard"]
            .with_context(
                allowed_company_ids=self.belgian_company.ids,
                active_id=georges_current_contract.id,
            )
            .new(
                {
        ...
        ]._cron_update_allocation_from_new_schedule(date(current_year, 2, 1))
        ...
        full_time_contract = (
            ...
            .search(view["domain"])
            .filtered(
                lambda contract: not contract.time_credit
                and contract.id != georges_current_contract.id
            )
        )
        self.env[
            "l10n_be.schedule.change.allocation"
        ]._cron_update_allocation_from_new_schedule(full_time_contract.date_start)
    ...
    def test_credit_time_for_john_doe(self):
        ...
        john_allocation = self.allocations.filtered(
        ...
        wizard = (
            ...
            .with_context(
                allowed_company_ids=self.belgian_company.ids,
                active_id=john_current_contract.id,
            )
            .new(
                {
                    "date_start": date(current_year, 2, 1),
        ...
        self.assertEqual(
            wizard.time_off_allocation, 18
        )  # 4*4*1 full days + 4*1*.5 half days
        ...
        continuation_contract = (
            ...
            .search(view["domain"])
            .filtered(
                lambda contract: contract.date_start == continuation_date
                and contract.id != john_current_contract.id
            )
        )
        self.env[
            "l10n_be.schedule.change.allocation"
        ]._cron_update_allocation_from_new_schedule(continuation_contract.date_start)
    ...
    def test_credit_time_for_a(self):
        ...
        a_allocation = self.allocations.filtered(
            lambda alloc: alloc.employee_id.id == self.employee_a.id
        )
        ...
        wizard = (
            ...
            .with_context(
                allowed_company_ids=self.belgian_company.ids,
                active_id=a_current_contract.id,
            )
            .new(
                {
                    "date_start": date(current_year, 2, 1),
        ...
        full_time_contract = (
            ...
            .filtered(
                lambda contract: not contract.time_credit
                and contract.id != a_current_contract.id
            )
        )
        self.env[
            "l10n_be.schedule.change.allocation"
    ...
    def test_remaining_leaves_with_credit_time(self):
        ...
        self.env[
            "l10n_be.schedule.change.allocation"
        ]._cron_update_allocation_from_new_schedule(date(today.year, 6, 1))
        ...
        self.assertEqual(full_time_contract.time_credit, False)
        self.assertEqual(
            a_allocation.number_of_days,
            18,
            "6 remained paid time offs and 12 days has been taken by the employee this current year",
        )
        ...
        wizard = (
            ...
            .with_context(
                allowed_company_ids=self.belgian_company.ids,
                active_id=full_time_contract.id,
            )
            .new(
                {
        ...
        self.env[
            "l10n_be.schedule.change.allocation"
        ]._cron_update_allocation_from_new_schedule(date(today.year, 9, 2))
