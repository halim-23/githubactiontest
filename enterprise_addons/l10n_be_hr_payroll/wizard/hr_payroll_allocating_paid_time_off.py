...
class HrPayrollAllocPaidLeave(models.TransientModel):
    ...
    year = fields.Selection(
        ...
        required=True,
        help="Year of the period to consider",
        default=lambda self: fields.Date.today().year,
    )
    structure_type_id = fields.Many2one(
        "hr.payroll.structure.type", string="Structure Type"
    )

    alloc_employee_ids = fields.One2many(
        "hr.payroll.alloc.employee", "alloc_paid_leave_id"
    )
    ...
    def _onchange_struct_id(self):
        ...
        max_leaves_count = (
            20 * calendar_of_company.hours_per_day
        )  # 20 days * (number of hours per day) for belgian company

        period_work_days_count = len(
            list(
                rrule(
                    DAILY,
                    dtstart=period_start,
                    until=period_end,
                    byweekday=[0, 1, 2, 3, 4, 5],
                )
            )
        )
        ...
        """.format(
            where_structure=structure,
            where_employee_in_department=employee_in_department,
        )
        ...
        alloc_employees = defaultdict(
            lambda: (0, None)
        )  # key = employee_id and value contains paid_time_off and contract_id in Tuple
        for vals in self.env.cr.dictfetchall():
            ...
            work_days_ratio = (
                work_days_count / period_work_days_count
            )  # In case the employee didn't work over the whole period
        ...
        for employee_id, value in alloc_employees.items():
            ...
            if contract_next_period is None:
                ...
                contract_next_period = self.env["hr.contract"].search(
                    domains, limit=1, order="date_start desc"
                )
            else:
                contract_next_period = self.env["hr.contract"].browse(
                    contract_next_period
                )
            ...
            if contract_next_period.id:
                ...
                allocation_hours = min(
                    max_hours_to_allocate, 4 * calendar.hours_per_week
                )
                paid_time_off_to_allocate = (
                    allocation_hours / calendar_of_company.hours_per_day
                    if calendar_of_company.hours_per_day
                    else 0
                )
                ...
                max_days_for_calendar = (
                    (len(calendar.attendance_ids) * 4) / 2
                ...
                if paid_time_off_to_allocate > max_days_for_calendar:
                    ...
                    paid_time_off_to_allocate = float_round(
                        paid_time_off_to_allocate, precision_rounding=0.5
                    )

            paid_time_off = float_round(
                max_hours_to_allocate / calendar_of_company.hours_per_day
                if calendar_of_company.hours_per_day
                else 0,
                0,
            )

            alloc_employee_ids.append(
                (
                    0,
                    0,
                    {
    ...
    def generate_allocation(self):
        allocation_values = []
        calendar_of_company = self.company_id.resource_calendar_id
        for alloc in self.alloc_employee_ids.filtered(
            lambda alloc: alloc.paid_time_off_to_allocate
        ):
            max_leaves_allocated = (
                alloc.paid_time_off * calendar_of_company.hours_per_day
            )
            number_of_days = alloc.paid_time_off_to_allocate

            if (
                alloc.paid_time_off_to_allocate
                * alloc.resource_calendar_id.hours_per_day
                > max_leaves_allocated
            ):
                number_of_days = alloc.paid_time_off

            number_of_days = (
                round(number_of_days * 2) / 2
            )  # round the paid time off until x.5
            if number_of_days:
...
class HrPayrollAllocEmployee(models.TransientModel):
    ...
    paid_time_off = fields.Float(
        ...
        required=True,
        help="1 day is 7 hours and 36 minutes",
    )
    paid_time_off_to_allocate = fields.Float(
        "Paid Time Off To Allocate",
        required=True,
        help="1 day is the number of the amount of hours per day in the working schedule",
    )
    contract_next_year_id = fields.Many2one(
        "hr.contract", string="Contract Active Next Year"
    )
    resource_calendar_id = fields.Many2one(
        related="contract_next_year_id.resource_calendar_id",
        string="Current Working Schedule",
        readonly=True,
    )
