...
class HrPayslipEmployeeDepartureHoliday(models.TransientModel):
    ...
    time_off_taken = fields.Float(
        ...
        compute="_compute_time_off_taken",
        store=True,
        readonly=False,
    )
    time_off_allocated = fields.Float(
        "Time off allocated during current year",
        compute="_compute_time_off_allocated",
        store=True,
        readonly=False,
    )

    unpaid_time_off_n = fields.Float(
        "Days Unpaid time off current year",
        help="Number of days of unpaid time off taken during current year",
    )
    unpaid_time_off_n1 = fields.Float(
        "Days Unpaid time off previous year",
        help="Number of days of unpaid time off taken during previous year",
    )

    unpaid_average_remunaration_n = fields.Monetary(
        "Average remuneration by month current year",
        help="Average remuneration for the 12 months preceding unpaid leave",
    )
    unpaid_average_remunaration_n1 = fields.Monetary(
        "Average remuneration by month previous year",
        help="Average remuneration for the 12 months preceding unpaid leave",
    )

    fictitious_remuneration_n = fields.Monetary(
        "Remuneration fictitious current year",
        compute="_compute_fictitious_remuneration_n",
    )
    fictitious_remuneration_n1 = fields.Monetary(
        "Remuneration fictitious previous year",
        compute="_compute_fictitious_remuneration_n1",
    )
    ...
    def _compute_history(self):
        for record in self:
            if record.employee_id and (
                not record.employee_id.start_notice_period
                or not record.employee_id.end_notice_period
            ):
                raise UserError(
                    _(
            ...
            if not record.employee_id:
                ...
                payslip_n_ids = self.env["hr.payslip"].search(
                    [
                        ...
                        (
                            ...
                            (
                                structure_warrant
                                + structure_double_holidays
                                + structure_termination
                                + structure_holidays_n
                                + structure_holidays_n1
                            ).ids,
                        ),
                    ]
                )
                ...
                work_entry_type_legal_leave = self.env.ref(
                    "hr_work_entry_contract.work_entry_type_legal_leave"
                )
                ...
                time_off_n_ids = self.env["hr.leave"].search(
                    [
                        ...
                        (
                            ...
                            "=",
                            work_entry_type_legal_leave.id,
                        ),
                    ]
                )
                ...
                record.time_off_allocation_n_ids = [
                    (4, t.id) for t in time_off_allocation_n_ids
                ]
    ...
    def _compute_net_n(self):
        for wizard in self:
            if wizard.payslip_n_ids:
                wizard.net_n = wizard.payslip_n_ids._origin._get_line_values(
                    ["SALARY"], compute_sum=True
                )["SALARY"]["sum"]["total"]
            else:
                wizard.net_n = 0
    ...
    def _compute_net_n1(self):
        for wizard in self:
            if wizard.payslip_n1_ids:
                wizard.net_n1 = wizard.payslip_n1_ids._origin._get_line_values(
                    ["SALARY"], compute_sum=True
                )["SALARY"]["sum"]["total"]
            else:
                wizard.net_n1 = 0
    ...
    def _compute_time_off_taken(self):
        for wizard in self:
            if wizard.time_off_n_ids:
                wizard.time_off_taken = sum(
                    time_off.number_of_days for time_off in wizard.time_off_n_ids
                )
            else:
    ...
    def _compute_time_off_allocated(self):
        for wizard in self:
            if wizard.time_off_allocation_n_ids:
                wizard.time_off_allocated = sum(
                    allocation.number_of_days
                    for allocation in wizard.time_off_allocation_n_ids
                )
            else:
    ...
    def _compute_fictitious_remuneration_n(self):
        for attest in self:
            attest.fictitious_remuneration_n = (
                attest.unpaid_time_off_n
                * attest.unpaid_average_remunaration_n
                * 3
                / (13 * 5)
            )
    ...
    def _compute_fictitious_remuneration_n1(self):
        for attest in self:
            attest.fictitious_remuneration_n1 = (
                attest.unpaid_time_off_n1
                * attest.unpaid_average_remunaration_n1
                * 3
                / (13 * 5)
            )

    def compute_termination_holidays(self):
        struct_n1_id = self.env.ref(
            "l10n_be_hr_payroll.hr_payroll_structure_cp200_employee_departure_n1_holidays"
        )
        struct_n_id = self.env.ref(
            "l10n_be_hr_payroll.hr_payroll_structure_cp200_employee_departure_n_holidays"
        )
        ...
        termination_payslip_n = self.env["hr.payslip"].create(
            {
                "name": "%s - %s"
                % (struct_n_id.payslip_name, self.employee_id.display_name),
        ...
        monthly_payslips = (
            ...
            .filtered(lambda p: "OUT" not in p.worked_days_line_ids.mapped("code"))
        )

        if monthly_payslips:
            slip = monthly_payslips[0]
            annual_gross = (
                slip._get_line_values(["GROSS"])["GROSS"][slip.id]["total"] * 12
            )
        else:
            annual_gross = 0
            ...
            [
                {
                    ...
                    "input_type_id": self.env.ref(
                        "l10n_be_hr_payroll.cp200_other_input_gross_ref"
                    ).id,
                ...
                },
                {
                    ...
                    "sequence": 3,
                    "input_type_id": self.env.ref(
                        "l10n_be_hr_payroll.cp200_other_input_allocation"
                    ).id,
                    "amount": 0,
                    "contract_id": termination_payslip_n.contract_id.id,
                },
                {
                    ...
                    "sequence": 4,
                    "input_type_id": self.env.ref(
                        "l10n_be_hr_payroll.cp200_other_input_time_off_taken"
                    ).id,
                    "amount": 0,
                    "contract_id": termination_payslip_n.contract_id.id,
                },
                {
                    ...
                    "sequence": 5,
                    "input_type_id": self.env.ref(
                        "l10n_be_hr_payroll.cp200_other_input_annual_taxable_amount"
                    ).id,
                    "amount": annual_gross,
                    "contract_id": termination_payslip_n.contract_id.id,
                },
        ...
        termination_payslip_n.name = "%s - %s" % (
            struct_n_id.payslip_name,
        ...
        termination_payslip_n1 = self.env["hr.payslip"].create(
            {
                "name": "%s - %s"
                % (struct_n1_id.payslip_name, self.employee_id.display_name),
                "employee_id": self.employee_id.id,
            }
        )
        ...
        double_amount_n = double_holiday_n._get_line_values(
        ...
        european_wds = payslip_n1.mapped("worked_days_line_ids").filtered(
            lambda wd: wd.code == "LEAVE216"
        )
        ...
        self.env["hr.payslip.input"].create(
            [
                ...
                },
                {
                    ...
                    "sequence": 6,
                    "input_type_id": self.env.ref(
                        "l10n_be_hr_payroll.cp200_other_input_european_leave"
                    ).id,
                ...
                },
                {
                    ...
                    "sequence": 7,
                    "input_type_id": self.env.ref(
                        "l10n_be_hr_payroll.cp200_other_input_european_leave_days"
                    ).id,
        ...
        termination_payslip_n1.name = "%s - %s" % (
            struct_n1_id.payslip_name,
            self.employee_id.display_name,
        )
        ...
        return {
            "name": _("Termination"),
            "domain": [
                ("id", "in", [termination_payslip_n.id, termination_payslip_n1.id])
            ],
