...
class L10nBeSocialBalanceSheet(models.TransientModel):
    ...
    social_balance_sheet = fields.Binary(
        "Social Balance Sheet", readonly=True, attachment=False
    )
    ...
    def print_report(self):
        ...
        invalid_employees = contracts.employee_id.filtered(
            lambda e: e.gender not in ["male", "female"]
        )
        ...
        for period, employee_payslips in mapped_payslips.items():
            if len(employee_payslips) > 1:
                ...
                payslip = employee_payslips.sorted(
                    lambda p: p.contract_id.date_start, reverse=True
                )[-1]
            else:
        ...
        report_data.update(
            {
                ...
                "1001_total": round(
                    (workers_data["male"]["full"] + workers_data["female"]["full"])
                    / 12.0,
                    2,
                ),
                ...
                "1002_total": round(
                    (workers_data["male"]["part"] + workers_data["female"]["part"])
                    / 12.0,
                    2,
                ),
                ...
                "1003_total": round(
                    (workers_data["male"]["fte"] + workers_data["female"]["fte"])
                    / 12.0,
                    2,
                ),
            }
        ...
        for payslip in payslips:
            gender = payslip.employee_id.gender
            lines = payslip.worked_days_line_ids.filtered(
                lambda l: l.work_entry_type_id in attendances
            )
            if lines:
        ...
        workers_data = collections.defaultdict(
            lambda: collections.defaultdict(lambda: dict(full=0, part=0))
        )
        meal_voucher = dict(male=0, female=0, total=0)

        line_values = payslips._get_line_values(
            ["GROSS", "CAR.PRIV", "ONSSEMPLOYER", "MEAL_V_EMP"],
            vals_list=["total", "quantity"],
        )
        for payslip in payslips:
            gender = payslip.employee_id.gender
            calendar = payslip.contract_id.resource_calendar_id
            ...
            workers_data["total"][gender][contract_type] = (
                gross + private_car + onss_employer
            )

            employer_amount = payslip.contract_id.meal_voucher_paid_by_employer
            meal_voucher[gender] += round(
                employer_amount * line_values["MEAL_V_EMP"][payslip.id]["quantity"], 2
            )
        ...
        end_contracts = self.env["hr.employee"]._get_all_contracts(
            self.date_to, self.date_to, states=["open", "close"]
        )
        ...
        cip = self.env.ref(
            "l10n_be_hr_payroll.l10n_be_contract_type_cip", raise_if_not_found=False
        )
        replacement = self.env.ref("l10n_be_hr_payroll.l10n_be_contract_type_cdd")
        defined_work = self.env.ref(
            "l10n_be_hr_payroll.l10n_be_contract_type_clearly_defined_work"
        )
        mapped_types = {
            cdi: "110",
            cdd: "111",
        ...
        for contract in end_contracts:
            ...
            if contract.contract_type_id not in mapped_types:
                raise UserError(
                    _(
                        "The contract %s for %s is not of one the following types: CDI, CDD. Replacement, For a clearly defined work",
                        contract.name,
            ...
            if (gender, contract.employee_id.certificate) not in mapped_certificates:
                raise UserError(
                    _(
                        "The employee %s doens't have a specified certificate",
                        contract.employee_id.name,
                    )
                )
            ...
            gender_certificate_code = mapped_certificates[
                (gender, contract.employee_id.certificate)
            ]
            workers_data[gender_certificate_code][contract_time] += 1
            workers_data[gender_certificate_code]["fte"] += (
                1 * calendar.work_time_rate / 100.0
            )
            ...
            if structure_type not in mapped_categories:
                raise UserError(
                    _(
                        "The contract %s for %s is not of one the following types: CP200 Employees or Student",
                        contract.name,
                        contract.employee_id.name,
                    )
                )
        ...
        for period, employee_payslips in mapped_payslips.items():
            ...
            if (
                employee not in in_employees
                and employee.first_contract_date
                and (date_from <= employee.first_contract_date <= date_to)
            ):
                ...
                workers_data[contract_type][contract_time] += 1
                workers_data[contract_type]["fte"] += (
                    1 * calendar.work_time_rate / 100.0
                )
            departure_date = employee.end_notice_period or employee.departure_date
            if (
                departure_date
                and employee not in out_employees
                and (date_from <= departure_date <= date_to)
            ):
        ...
        training_payslips = payslips.filtered(
            lambda p: training_code
            in p.worked_days_line_ids.mapped("work_entry_type_id.code")
        )
        male_payslips = training_payslips.filtered(
            lambda p: p.employee_id.gender == "male"
        )
        ...
        report_data["5802"] = male_payslips._get_worked_days_line_number_of_hours(
            training_code
        )
        report_data["5812"] = female_payslips._get_worked_days_line_number_of_hours(
            training_code
        )

        report_data["58031"] = male_payslips._get_worked_days_line_amount(training_code)
        report_data["58131"] = female_payslips._get_worked_days_line_amount(
            training_code
        )

        line_values = (male_payslips + female_payslips)._get_line_values(
            ["SALARY", "ONSSTOTAL"]
        )
        report_data["58032"] = sum(
            p._get_worked_days_line_amount(training_code)
            / line_values["SALARY"][p.id]["total"]
            * line_values["ONSSTOTAL"][p.id]["total"]
            for p in male_payslips
        )
        report_data["58132"] = sum(
            p._get_worked_days_line_amount(training_code)
            / line_values["SALARY"][p.id]["total"]
            * line_values["ONSSTOTAL"][p.id]["total"]
            for p in female_payslips
        )
        ...
        export_274_sheet_pdf, dummy = (
            self.env.ref("l10n_be_hr_payroll.action_report_social_balance")
            .sudo()
            ._render_qweb_pdf(res_ids=self.ids, data=report_data)
        )
    ...
    def action_validate(self):
        ...
        if self.social_balance_sheet:
            self._post_process_generated_file(
                self.social_balance_sheet, self.social_balance_filename
            )
