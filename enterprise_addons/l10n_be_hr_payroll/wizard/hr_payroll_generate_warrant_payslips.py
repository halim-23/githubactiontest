...
class HrPayrollGenerateCommissionPayslips(models.TransientModel):
    ...
    date_start = fields.Date(
        ...
        default=lambda self: fields.Date.to_string(
            (datetime.now() + relativedelta(months=-2, day=1)).date()
        ),
    )
    ...
    state = fields.Selection(
        [
            ...
            ("import", "Import the employee file"),
        ],
        default="draft",
    )
    ...
    def import_employee_file(self):
        ...
        try:
            values = [(5, 0, 0)]
            for line in (
                base64.decodebytes(self.import_file).decode("utf-8").split("\r\n")[1:]
            ):
                line = line.split(",")
                if len(line) != 3:
                    continue
