...
class HrPayslipEmployeeDepartureNotice(models.TransientModel):
    ...
    leaving_type_id = fields.Many2one(
        "hr.departure.reason", string="Leaving Type", required=True
    )
    departure_reason_code = fields.Integer(related="leaving_type_id.reason_code")

    start_notice_period = fields.Date(
        "Start notice period", required=True, default=fields.Date.context_today
    )
    ...
    oldest_contract_id = fields.Many2one(
        "hr.contract", string="Oldest Contract", compute="_compute_oldest_contract_id"
    )
    ...
    salary_december_2013 = fields.Selection(
        ...
        string="Gross Annual Salary as of December 31, 2013",
        required=True,
        default="superior",
    )
    salary_visibility = fields.Boolean("Salary as of December 2013")
    notice_duration_month_before_2014 = fields.Integer(
        "Notice Duration in month", compute="_notice_duration"
    )
    notice_duration_week_after_2014 = fields.Integer(
        "Notice Duration in weeks", compute="_notice_duration"
    )
    ...
    notice_respect = fields.Selection(
        ...
        string="Respect of the notice period",
        required=True,
        default="with",
    )
    ...
    @api.onchange(
        "notice_duration_month_before_2014",
        "notice_duration_week_after_2014",
        "start_notice_period",
        "notice_respect",
    )
    def _onchange_notice_duration(self):
        if self.notice_respect == "without":
            ...
            self.end_notice_period = self.start_notice_period + timedelta(
                weeks=months_to_weeks + self.notice_duration_week_after_2014
            )

    @api.depends(
        "first_contract",
        "leaving_type_id",
        "salary_december_2013",
        "start_notice_period",
    )
    def _notice_duration(self):
        first_2014 = datetime(2014, 1, 1)
        departure_reasons = self.env[
            "hr.departure.reason"
        ]._get_default_departure_reasons()
        for notice in self:
            ...
            period_since_2014 = relativedelta(
                notice.start_notice_period, first_day_since_2014
            )
            difference_in_years = notice._get_years(
                relativedelta(datetime(2013, 12, 31), notice.first_contract)
            )
            if notice.leaving_type_id == departure_reasons["fired"]:
                ...
                if difference_in_years > 0:
                    ...
                    if notice.salary_december_2013 == "inferior":
                        notice.notice_duration_month_before_2014 = int(
                            math.ceil(difference_in_years / 5.0) * 3.0
                        )
                    else:
                        notice.notice_duration_month_before_2014 = max(
                            int(math.ceil(difference_in_years)), 3
                        )
                else:
                ...
                if difference_in_years > 0:
                    ...
                    notice.notice_duration_month_before_2014 = 0
                    notice.notice_duration_week_after_2014 = notice._find_week(
                        period_since_2014.months + period_since_2014.years * 12,
                        "resigned",
                    )
    ...
    def _get_input_type(self, name, cp="cp200"):
        input_type = self.env.ref(
            "l10n_be_hr_payroll.%s_other_input_%s" % (cp, name),
    ...
    def compute_termination_fee(self):
        ...
        last_contract = self.env["hr.contract"].search(
            [("employee_id", "=", self.employee_id.id), ("state", "!=", "cancel")],
        ...
        termination_payslip = self.env["hr.payslip"].create(
            {
                "name": "%s - %s"
                % (struct_id.payslip_name, self.employee_id.display_name),
        ...
        self._create_input(
            payslip_id, 1, "months", self.notice_duration_month_before_2014, contract.id
        )
        self._create_input(
            payslip_id, 2, "weeks", self.notice_duration_week_after_2014, contract.id
        )
        ...
        self._create_input(
            payslip_id, 10, "yearend_bonus", contract._get_contract_wage(), contract.id
        )
        ...
        self._create_input(
            ...
            13,
            "variable_salary",
            termination_payslip._get_last_year_average_variable_revenues() * 12,
            contract.id,
        )
        ...
        self._create_input(
            ...
            15,
            "hospital_insurance",
            contract._get_contract_insurance_amount("hospital"),
            contract.id,
        )
        self._create_input(
            payslip_id,
            ...
            contract._get_contract_insurance_amount("ambulatory"),
            contract.id,
        )
        self._create_input(
            payslip_id,
            16,
            "group_insurance",
            contract._get_contract_insurance_amount("group"),
            contract.id,
        )
        self._create_input(
            payslip_id,
            17,
            "stock_option",
            termination_payslip._get_last_year_average_warrant_revenues(),
            contract.id,
        )
