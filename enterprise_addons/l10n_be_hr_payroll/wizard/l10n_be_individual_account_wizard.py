...
class L10nBeIndividualAccountWizard(models.TransientModel):
    ...
    def _get_rendering_data(self, year, employee_ids):
        ...
        payslips = self.env["hr.payslip"].search(
            [
                ...
                (
                    "date_from",
                    "<=",
                    Datetime.now().replace(month=12, day=31, year=year),
                ),
                "|",
        ...
        lines = payslips.line_ids.filtered(
            lambda l: l.salary_rule_id.appears_on_payslip
        )
                ...
                "rules": OrderedDict(
                    (
                        rule[0],
                        {
                            ...
                            "month": {
                                m: {"name": False, "total": 0} for m in range(12)
                            },
                            "quarter": {
                                q: {"name": False, "total": 0} for q in range(4)
                            },
                        },
                    )
    ...
    def _generate_files(self, company):
        self.ensure_one()
        employee_ids = (
            self.env["hr.employee"].search([("company_id", "=", company.id)]).ids
        )
        rendering_data = self._get_rendering_data(int(self.year), employee_ids)
        template_sudo = self.env.ref(
            "l10n_be_hr_payroll.action_report_individual_account"
        ).sudo()
        ...
        for employee, employee_data in rendering_data.items():
            _logger.info(
                "Printing Individual Account Sheet (%s/%s)", counter, sheet_count
            )
            counter += 1
            ...
            sheet_file, dummy = template_sudo.with_context(
                lang=employee_lang
            )._render_qweb_pdf(
                employee.id,
                data={
    ...
    def _process_files(
        self, files, default_filename="Individual Account", post_process=False
    ):
