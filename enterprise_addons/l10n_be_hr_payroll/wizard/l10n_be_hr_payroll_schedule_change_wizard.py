...
class L10nBeHrPayrollScheduleChange(models.TransientModel):
    ...
    structure_type_id = fields.Many2one(
        related="contract_id.structure_type_id", readonly=True
    )
    date_start = fields.Date(
        "Start Date", help="Start date of the new contract.", required=True
    )
    date_end = fields.Date("End Date", help="End date of the new contract.")
    work_time_rate = fields.Float(
        related="resource_calendar_id.work_time_rate", readonly=True
    )
    full_wage = fields.Monetary(
        "Full Time Equivalent Wage", compute="_compute_wages", store=True, readonly=True
    )
    ...
    presence_work_entry_type_id = fields.Many2one(
        "hr.work.entry.type", related="structure_type_id.default_work_entry_type_id"
    )
    absence_work_entry_type_id = fields.Many2one(
        "hr.work.entry.type",
        string="Absence Work Entry Type",
    ...
    full_time_off_allocation = fields.Float(
        compute="_compute_full_time_off_allocation", readonly=True
    )
    time_off_allocation = fields.Float(
        compute="_compute_time_off_allocation",
        store=True,
    ...
    leave_allocation_id = fields.Many2one(
        "hr.leave.allocation", compute="_compute_leave_allocation_id"
    )
    ...
    def _compute_wages(self):
        for wizard in self:
            ...
            wizard.full_wage = wizard.current_wage / (
                wizard.current_resource_calendar_id.work_time_rate / 100 or 1
            )
    ...
    def _compute_new_allocation(self, leave_allocation, current_calendar, new_calendar):
        ...
        new_allocation = float_round(
            remaining_days
            * (new_calendar.work_time_rate / current_calendar.work_time_rate),
            precision_rounding=0.25,
        )
        new_allocation = float_round(
            new_allocation, precision_rounding=0.5, rounding_method="DOWN"
        )
        ...
        max_allocation = (
            (len(new_calendar.attendance_ids) * 4) / 2
    ...
    def _compute_time_off_allocation(self):
        for wizard in self:
            wizard.time_off_allocation = self._compute_new_allocation(
                wizard.leave_allocation_id,
                wizard.current_resource_calendar_id,
    ...
    def _compute_requires_new_contract(self):
        ...
        requires_new_contract = self.filtered(
            lambda w: (
                not w.date_start
                or not w.contract_id
                or w.date_start <= w.contract_id.date_start
            )
        )
    ...
    def _update_allocation_or_schedule(self, date, contract, current, new):
        ...
        if date > fields.Date.today() or self.env.context.get("force_schedule", False):
            ...
            new_total = (
                self.time_off_allocation
                if new != self.contract_id.resource_calendar_id
                else self.initial_time_off_allocation
            )
            ...
            self.leave_allocation_id._message_log(
                body=_(
                    "New working schedule on %(contract_name)s.<br/>"
                    "New total : %(days)s"
                )
                % {"contract_name": contract.name, "days": new_total}
            )
    ...
    def action_validate(self):
        ...
        if self.contract_id.resource_calendar_id == self.resource_calendar_id:
            raise ValidationError(
                _(
                    "Working schedule would stay unchanged by this action. Please select another working schedule."
                )
            )
        ...
        if self.date_start < self.contract_id.date_start:
            raise ValidationError(
                _("Start date must be later than the current contract's start date.")
            )
        if (
            self.contract_id.date_end
            and self.date_end
            and self.contract_id.date_end < self.date_end
        ):
            raise ValidationError(
                _("Current contract is finished before the end of the new contract.")
            )
        ...
        if self.part_time:
            name = _("%s - Part Time %.0f%%") % (
                self.employee_id.name,
                self.work_time_rate,
            )
        else:
            name = _("%s - %.0f%% Work Schedule") % (
                self.employee_id.name,
                self.work_time_rate,
            )
        ...
        new_contracts = self.contract_id.copy(
            {
                ...
                "work_time_rate": self.work_time_rate / 100
                if self.part_time
                else False,
                "state": "draft",
                "time_credit_type_id": self.absence_work_entry_type_id.id
                if self.part_time
                else None,
            }
        )
