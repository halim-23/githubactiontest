...
class L10nBeDoublePayRecoveryWizard(models.TransientModel):
    ...
    line_ids = fields.One2many(
        ...
        "wizard_id",
        string="Occupation Lines",
        compute="_compute_line_ids",
        readonly=False,
        store=True,
    )
    gross_salary = fields.Monetary(
        compute="_compute_gross_salary", store=True, readonly=False
    )
    company_calendar = fields.Many2one(related="company_id.resource_calendar_id")
    months_count = fields.Float(
        string="Current Occupation Duration (Months)", compute="_compute_months_count"
    )
    months_count_description = fields.Char(
        string="Current Occupation Duration (Description)",
        compute="_compute_months_count",
    )

    classic_holiday_pay = fields.Monetary(
        compute="_compute_amounts_to_recover", store=True, readonly=False
    )
    threshold = fields.Monetary(
        compute="_compute_amounts_to_recover", store=True, readonly=False
    )
    double_pay_to_recover = fields.Monetary(
        compute="_compute_amounts_to_recover", store=True, readonly=False
    )
    ...
    def _compute_months_count(self):
        for wizard in self:
            ...
            date_to = (
                wizard.payslip_id.date_to
                + relativedelta(years=-1, month=12, day=31)
                + relativedelta(days=1)
            )
            wizard.months_count = (date_to.year - date_from.year) * 12 + (
                date_to.month - date_from.month
            )
            if wizard.months_count > 12:
                wizard.months_count_description = _(
                    "The employee is occupied from the %s to the %s. There is nothing to recover as the employee is there for more than 12 months",
                    format_date(self.env, date_from),
                    format_date(self.env, wizard.payslip_id.date_to),
                )
            else:
                wizard.months_count_description = _(
                    "The employee is occupied from the %s to the %s.",
                    format_date(self.env, date_from),
                    format_date(self.env, wizard.payslip_id.date_to),
                )
    ...
    def _compute_line_ids(self):
        ...
        fields_to_copy = self.env["l10n.be.double.pay.recovery.line"]._fields.keys() - {
            "employee_id",
            "company_id",
        }
        for wizard in self:
            wizard.write(
                {
                    ...
                    + [
                        fields.Command.create(read_result)
                        for read_result in wizard.employee_id.double_pay_line_ids.read(
    ...
    @api.depends(
        "gross_salary",
        "months_count",
        "line_ids.months_count",
        "line_ids.occupation_rate",
    )
    def _compute_amounts_to_recover(self):
        for wizard in self:
            wizard.double_pay_to_recover = 0

            total_occupation = wizard.months_count + sum(
                [l.months_count for l in wizard.line_ids]
            )
            # Step 1:
                    ...
                    (
                        ...
                        0,
                        {
                            "input_type_id": self.env.ref(
                                "l10n_be_hr_payroll.input_double_holiday_recovery"
                            ).id,
...
class L10nBeDoublePayRecoveryLineWizard(models.TransientModel):
    ...
    amount = fields.Monetary(
        string="Amount",
        required=True,
        help="Holiday pay amount on the holiday attest from the previous employer",
    )
