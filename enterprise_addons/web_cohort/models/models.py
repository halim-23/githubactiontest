...
class Base(models.AbstractModel):
    ...
    @api.model
    def get_cohort_data(
        self, date_start, date_stop, measure, interval, domain, mode, timeline
    ):
        """
        ...
        measure_is_many2one = (
            self._fields.get(measure) and self._fields.get(measure).type == "many2one"
        )
        field_measure = (
        ...
        for group in row_groups:
            ...
            for col_index, col in enumerate(col_range):
                ...
                significative_period = col_start_date.strftime(
                    DISPLAY_FORMATS[interval]
                )
                        ...
                        [
                            ...
                            [
                                ...
                                (
                                    date_stop,
                                    ">=",
                                    fields.Datetime.to_string(col_start_date),
                                ),
                            ],
                        ]
                    )
                    col_group = self._read_group_raw(
                        domain=outside_timeline_domain, fields=field_measure, groupby=[]
                    )
                    initial_value = float(col_group[0][measure] or 0.0)
                    initial_churn_value = value - initial_value

                previous_col_remaining_value = (
                    initial_value if col_index == 0 else columns[-1]["value"]
                )
                ...
                if interval == "week":
                    period = "%s - %s" % (
                        col_start_date.strftime("%d %b"),
                        (col_end_date - relativedelta(days=1)).strftime("%d %b"),
                    )
                else:
                ...
                if mode == "churn":
                    domain = [
                        (
                            date_stop,
                            "<",
                            col_end_date.strftime(DEFAULT_SERVER_DATE_FORMAT),
                        ),
                    ]
                else:
                    domain = [
                ...
                columns.append(
                    {
                        ...
                        "churn_value": col_value
                        + (
                            columns[-1]["churn_value"]
                            if col_index > 0
                            else initial_churn_value
                        ),
