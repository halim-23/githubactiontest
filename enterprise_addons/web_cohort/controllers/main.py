    ...
    def export_xls(self, data, **kw):
        ...
        def write_data(report, row, col):
            ...
            worksheet.merge_range(
                ...
                columns_length + 1,
                _("%s - By %s")
                % (result["date_stop_string"], result["interval_string"]),
                style_highlight,
            )
            row += 1
            ...
            col += 1
            worksheet.write(
                row, col, "%.1f" % result[report]["avg"]["avg_value"], style_highlight
            )
            col += 1
            ...
            for n in range(columns_length):
                if total[str(n)]["count"]:
                    worksheet.write(
                        ...
                        col,
                        "%.1f"
                        % float(total[str(n)]["percentage"] / total[str(n)]["count"])
                        + "%",
                        style_highlight,
                    )
                else:
