...
QUnit.module("Views", (hooks) => {
  ...
  QUnit.test("correctly set measure and interval after changed", async function (
    ...
    await testUtils.dom.click(
      cohort.el.querySelector(".dropdown-menu span:not(.selected)")
    );
    assert.equal(
      cohort.el.querySelector(".dropdown-menu span.selected").textContent,
      "Count",
    ...
    await testUtils.dom.click(
      cohort.el.querySelectorAll(".o_cohort_interval_button")[2]
    );
    assert.equal(
      cohort.el.querySelector(".o_cohort_interval_button.active").textContent,
      "Month",
  ...
  QUnit.test("test backward timeline", async function (assert) {
    ...
    assert.equal(
      values[5].textContent.trim(),
      "40%",
      "col 6 should display 40 percent"
    );
    assert.equal(
      values[7].textContent.trim(),
      "80%",
      "col 8 should display 80 percent"
    );
    assert.equal(
      values[14].textContent.trim(),
      "100%",
