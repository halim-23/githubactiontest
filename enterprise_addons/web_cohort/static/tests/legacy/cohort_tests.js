  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("simple cohort rendering", async function (assert) {
        ...
        assert.ok(
          cohort.$(".table thead tr:first th:first:contains(Start)").length,
          'should contain "Start" in header of first column'
        );
        assert.ok(
          cohort.$(".table thead tr:first th:nth-child(3):contains(Stop - By Day)")
            .length,
          'should contain "Stop - By Day" in title'
        );
        assert.ok(
          cohort.$(".table thead tr:nth-child(2) th:first:contains(+0)").length,
          "interval should start with 0"
        );
        assert.ok(
          cohort.$(".table thead tr:nth-child(2) th:nth-child(16):contains(+15)")
            .length,
          "interval should end with 15"
        );

        assert.strictEqual(
          cohort.$buttons.find(".o_cohort_measures_list").length,
          1,
          "should have list of measures"
        );
        assert.strictEqual(
          cohort.$buttons.find(".o_cohort_interval_button").length,
          4,
      ...
      QUnit.test("no content helper after update", async function (assert) {
        ...
        var cohort = await createView({
          ...
          data: this.data,
          arch:
            '<cohort string="Subscription" date_start="start" date_stop="stop" measure="recurring"/>',
        });
      ...
      QUnit.test("correctly set by default measure and interval", async function (
        assert
      ) {
        assert.expect(4);
        ...
        assert.hasClass(
          cohort.$buttons.find(".o_cohort_measures_list [data-field=__count]"),
          "selected",
          "count should by default for measure"
        );
        assert.hasClass(
          cohort.$buttons.find(".o_cohort_interval_button[data-interval=day]"),
          "active",
          "day should by default for interval"
        );

        assert.ok(
          cohort.$(".table thead tr:first th:nth-child(2):contains(Count)").length,
      ...
      QUnit.test("correctly sort measure items", async function (assert) {
        ...
        const measureButtonEls = buttonsEls[0].querySelectorAll(
          ".o_cohort_measures_list > button"
        );
        assert.deepEqual(
      ...
      QUnit.test("correctly set measure and interval after changed", async function (
        assert
      ) {
        assert.expect(8);
        ...
        assert.hasClass(
          cohort.$buttons.find(".o_cohort_measures_list [data-field=recurring]"),
          "selected",
          "should recurring for measure"
        );
        assert.hasClass(
          cohort.$buttons.find(".o_cohort_interval_button[data-interval=week]"),
          "active",
          "should week for interval"
        );

        assert.ok(
          cohort.$(".table thead tr:first th:nth-child(2):contains(Recurring Price)")
            .length,
          'should contain "Recurring Price" in header of second column'
        );
        assert.ok(
          cohort.$(".table thead tr:first th:nth-child(3):contains(Stop - By Week)")
            .length,
          'should contain "Stop - By Week" in title'
        );

        await testUtils.dom.click(
          cohort.$buttons.find(".dropdown-toggle:contains(Measures)")
        );
        await testUtils.dom.click(
          cohort.$buttons.find(".o_cohort_measures_list [data-field=__count]")
        );
        ...
        await testUtils.dom.click(
          cohort.$buttons.find(".o_cohort_interval_button[data-interval=month]")
        );
        assert.hasClass(
          cohort.$buttons.find(".o_cohort_interval_button[data-interval=month]"),
          "active",
        ...
        assert.ok(
          cohort.$(".table thead tr:first th:nth-child(3):contains(Stop - By Month)")
            .length,
      ...
      QUnit.test("cohort view without attribute invisible on field", async function (
        assert
      ) {
        assert.expect(3);
        ...
        assert.containsOnce(
          cohort,
          '.o_cohort_measures_list button[data-field="recurring"]'
        );
        assert.containsOnce(
          cohort,
          '.o_cohort_measures_list button[data-field="__count"]'
        );

        cohort.destroy();
      });

      QUnit.test("cohort view with attribute invisible on field", async function (
        assert
      ) {
        assert.expect(2);
        ...
        async function (assert) {
          ...
          const views = {
            ...
            "subscription,my_list_view,list":
              "<tree>" + '<field name="start"/>' + '<field name="stop"/>' + "</tree>",
            "subscription,my_form_view,form":
              "<form>" + '<field name="start"/>' + '<field name="stop"/>' + "</form>",
          ...
          assert.strictEqual(
            $(webClient.el).find(".o_list_view th:nth(1)").text(),
            "Start",
            "First field in the list view should be start"
          );
          assert.strictEqual(
            $(webClient.el).find(".o_list_view th:nth(2)").text(),
            "Stop",
          ...
          await testUtils.dom.click(
            $(webClient.el).find("td div.o_cohort_value:first")
          );
          await nextTick();
          assert.strictEqual(
            $(webClient.el).find(".o_list_view th:nth(1)").text(),
          ...
          assert.hasAttrValue(
            $(webClient.el).find(".o_form_view span:first"),
            "name",
            "start",
            "First field in the form view should be start"
          );
          assert.hasAttrValue(
            $(webClient.el).find(".o_form_view span:nth(1)"),
            "name",
            "stop",
      ...
      QUnit.test("test mode churn", async function (assert) {
        ...
        assert.strictEqual(
          cohort.$("td .o_cohort_value:first").text().trim(),
          "0%",
          "first col should display 0 percent"
        );
        assert.strictEqual(
          cohort.$("td .o_cohort_value:nth(4)").text().trim(),
          "100%",
      ...
      QUnit.test("test backward timeline", async function (assert) {
        ...
        assert.ok(
          cohort.$(".table thead tr:nth-child(2) th:first:contains(-15)").length,
          "interval should start with -15"
        );
        assert.ok(
          cohort.$(".table thead tr:nth-child(2) th:nth-child(16):contains(0)").length,
          "interval should end with 0"
        );
        assert.strictEqual(
          cohort.$("td .o_cohort_value:first").text().trim(),
          "20%",
          "first col should display 20 percent"
        );
        assert.strictEqual(
          cohort.$("td .o_cohort_value:nth(5)").text().trim(),
          "40%",
          "col 6 should display 40 percent"
        );
        assert.strictEqual(
          cohort.$("td .o_cohort_value:nth(7)").text().trim(),
          "80%",
          "col 8 should display 80 percent"
        );
        assert.strictEqual(
          cohort.$("td .o_cohort_value:nth(14)").text().trim(),
          "100%",
          "col 15 should display 100 percent"
        );
      ...
      QUnit.test("rendering of a cohort view with comparison", async function (assert) {
        ...
        function verifyContents(results) {
          ...
          assert.strictEqual(
            $tables.length,
            results.length,
            "There should be " + results.length + " tables"
          );
          var result;
          $tables.each(function () {
            ...
            if (rowCount) {
              assert.strictEqual(
                rowCount,
                result,
                "the table should contain " + result + " rows"
              );
            } else {
              assert.strictEqual(
