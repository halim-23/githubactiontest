  ...
  MockServer.include({
    ...
    _mockGetCohortData: function (model, kwargs) {
      ...
      _.each(groups, function (group) {
        ...
        var cohortStartDate = moment(
          group[kwargs.date_start + ":" + kwargs.interval],
          format
        );
        ...
        for (var column = 0; column <= 15; column++) {
          ...
          var colRecords = _.filter(records.records, function (record) {
            return (
              ...
              moment(record[kwargs.date_stop], "YYYY-MM-DD").format(
          ...
          if (kwargs.timeline === "backward" && column === 0) {
            colRecords = _.filter(records.records, function (record) {
              return (
                record[kwargs.date_stop] &&
                moment(record[kwargs.date_stop], "YYYY-MM-DD") >= colStartDate
              );
            });
            if (kwargs.measure === "__count") {
          ...
          var previousValue =
            column === 0 ? initialValue : columns[column - 1].value;
          var remainingValue = previousValue - colValue;
          var previousChurnValue =
