odoo.define("web_cohort.CohortController", function (require) {
  ...
  var CohortController = AbstractController.extend({
    ...
    renderButtons: function ($node) {
      this.$buttons = $(
        qweb.render("CohortView.buttons", {
    ...
    updateButtons: function () {
      ...
      var noData =
        ...
        (!data.comparisonReport || !data.comparisonReport.rows.length);
      this.$buttons.find(".o_cohort_download_button").toggleClass("d-none", noData);
      if (config.device.isMobile) {
        var $activeInterval = this.$buttons.find(
          '.o_cohort_interval_button[data-interval="' + data.interval + '"]'
        );
