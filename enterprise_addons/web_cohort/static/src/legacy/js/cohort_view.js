odoo.define("web_cohort.CohortView", function (require) {
  ...
  var CohortView = AbstractView.extend({
    ...
    init: function (viewInfo, params) {
      ...
      this.controllerParams.views = [_findViewID("list"), _findViewID("form")];
      function _findViewID(viewType) {
        ...
        var contextID =
          viewType === "list"
            ? action.context.list_view_id
            : action.context.form_view_id;
        if (contextID) {
          return [contextID, viewType];
        }
          const result = action._views
            ? action._views.find((v) => v[1] === viewType)
            : undefined;
    ...
    _updateMVCParams: function () {
      ...
      this.rendererParams.dateStartString = this.fields[
        this.loadParams.dateStart
      ].string;
