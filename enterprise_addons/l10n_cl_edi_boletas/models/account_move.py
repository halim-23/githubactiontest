...
class AccountMove(models.Model):
    ...
    def _l10n_cl_edi_post_validation(self):
        if self.l10n_latam_document_type_id.code == "39":
            ...
            if daily_sales_book:
                raise UserError(
                    _(
                        "The Daily Sales Book for this ticket has already been sent. "
    ...
    def l10n_cl_send_dte_to_sii(self, retry_send=True):
        ...
        self.l10n_cl_dte_status = (
            "ask_for_status" if sii_response_status == "REC" else "rejected"
        )
        self.message_post(
    ...
    def _l10n_cl_get_verify_status_msg_rest(self, data):
        ...
        if self.l10n_cl_dte_status in ["rejected", "objected"]:
            ...
            if detail:
                msg += "<br/><li><b>ESTADO</b>: %s</li>" % html_escape(
                    detail[0]["estado"]
                )
                errors = detail[0].get("error", [])
                for error in errors:
                    msg += "<br/><li><b>ERROR</b>: %s %s</li>" % (
                        html_escape(error["descripcion"]),
                        html_escape(error["detalle"]) or "",
                    )
                return msg
