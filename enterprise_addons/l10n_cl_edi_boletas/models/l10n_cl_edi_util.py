...
class L10nClEdiUtilMixin(models.AbstractModel):
    ...
    def _get_token_rest(self, mode, digital_signature):
        ...
        if not seed:
            return self._connection_exception(
                "exception", _("Not possible to get a seed")
            )
        ...
        response = requests.post(
            url, headers=headers, data=signed_token, timeout=TIMEOUT_REST
        )
        if not response.ok:
            return None
    ...
    def _send_xml_to_sii_rest(
        self, mode, company_vat, file_name, xml_message, digital_signature
    ):
        ...
        if response.status_code == 401:
            ...
            return self._send_xml_to_sii_rest(
                mode, company_vat, file_name, xml_message, digital_signature
            )
        try:
    ...
    def _get_send_status_rest(self, mode, track_id, company_vat, digital_signature):
        ...
        if token is None:
            ...
            return False
        url = url_join(
            REST_SERVER_URL[mode]["OTHER"],
            "boleta.electronica.envio/%s-%s-%s"
            % (company_vat[:8], company_vat[-1], track_id),
        )
        try:
            response = requests.get(
                url, headers={"Cookie": "TOKEN={}".format(token)}, timeout=TIMEOUT_REST
            )
        except (
            requests.exceptions.Timeout,
            requests.exceptions.RequestException,
        ...
        if response.status_code == 401:
            ...
            digital_signature.last_rest_token = False
            return self._get_send_status_rest(
                mode, track_id, company_vat, digital_signature
            )
        try:
            response.raise_for_status()
        except requests.exceptions.HTTPError as error:
    ...
    def _analyze_sii_result_rest(self, message):
        ...
        for doc in detail_rep_rech:
            ...
            matching_tipo = doc.get("tipo") == int(
