...
class L10nClDailySalesBook(models.Model):
    ...
    l10n_cl_sii_send_ident = fields.Text(
        string="SII Send Identification", copy=False, readonly=True
    )
    ...
    l10n_cl_dte_status = fields.Selection(
        ...
        string="SII Daily Sales Book Status",
        default="not_sent",
        copy=False,
        tracking=True,
    ...
    def _unlink_except_already_accepted(self):
        if self.filtered(
            lambda x: x.l10n_cl_dte_status in ("accepted", "objected", "ask_for_status")
        ):
    ...
    def _get_summary(self):
        ...
        for document_type in sorted(
            self.move_ids.mapped("l10n_latam_document_type_id.code")
        ):
            values = {"document_type": document_type}
            # Get amounts
            move_ids = self.move_ids.filtered(
                lambda x: x.l10n_latam_document_type_id.code == document_type
            )
            vat_taxes = move_ids.line_ids.filtered(
                lambda x: x.tax_line_id.l10n_cl_sii_code == 14
            )
            lines_with_taxes = move_ids.invoice_line_ids.filtered(lambda x: x.tax_ids)
            lines_without_taxes = move_ids.invoice_line_ids.filtered(
                lambda x: not x.tax_ids
            )
            values.update(
                {
                    "vat_amount": float_repr(
                        sum(vat_taxes.mapped("price_subtotal")), 0
                    ),
                    # Sum of the subtotal amount affected by tax
                    "subtotal_amount_taxable": float_repr(
                        sum(lines_with_taxes.mapped("price_subtotal")), 0
                    ),
                    # Sum of the subtotal amount not affected by tax
                    "subtotal_amount_exempt": float_repr(
                        sum(lines_without_taxes.mapped("price_subtotal")), 0
                    ),
            ...
            document_numbers = [
                int(doc_number)
                for doc_number in move_ids.mapped("l10n_latam_document_number")
            ]
            values.update(
                {
    ...
    def _create_month_calendar_report_check(self):
        if (
            ...
            .search([("l10n_latam_document_type_id.code", "in", ["39", "41"])])
            .mapped("company_id")
            .filtered(lambda x: x.l10n_cl_dte_service_provider)
        ):
    ...
    def _get_move_ids_without_daily_sales_book_by_date(self, date):
        ...
        final_book_set = daily_sales_book_candidate_moves.filtered(
            lambda x: x.l10n_latam_document_type_id.code in ["39", "41"]
            or x.l10n_latam_document_type_id.code == "61"
            and x.reversed_entry_id.l10n_latam_document_type_id.code in ["39", "41"]
        ).ids
    ...
    def _update_report(self):
        ...
        move_ids = (
            self.move_ids.ids
            + self._get_move_ids_without_daily_sales_book_by_date(self.date)
        )
        self.write(
            {"send_sequence": self.send_sequence + 1, "move_ids": [(6, 0, move_ids)]}
        )
        self._create_dte()
    ...
    def _create_dte(self):
        ...
        dte = self.env["l10n_cl.edi.util"]._sign_full_xml(
            xml_book.decode(), digital_signature, doc_id, "consu"
        )
        ...
        self.write(
            {"l10n_cl_dte_status": "not_sent", "l10n_cl_sii_send_file": attachment.id}
        )
        self.message_post(
            body=_("DTE has been created"), attachment_ids=[attachment.id]
        )
    ...
    def l10n_cl_send_dte_to_sii(self, retry_send=True):
        if not self.company_id.l10n_cl_dte_service_provider:
            self.message_post(
                body=_(
                    "Cannot send Sales Book Report to SII due to the service provider is not set in "
    ...
    def l10n_cl_verify_dte_status(self):
        ...
        try:
            self.l10n_cl_dte_status = self._analyze_sii_sales_book_result(
                response_parsed
            )
    ...
    def l10n_cl_retry_daily_sales_book_report(self):
        if self.l10n_cl_dte_status != "rejected":
            raise UserError(
                _("You cannot retry a daily sales book with status other than Rejected")
            )
        self._update_report()
    ...
    def _cron_run_sii_sales_book_report_process(self):
        for company in self.env["res.company"].search(
            ...
            self_skip = self.with_company(company=company.id).with_context(
