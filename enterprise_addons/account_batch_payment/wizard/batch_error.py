...
class BatchErrorWizard(models.TransientModel):
    ...
    batch_payment_id = fields.Many2one(
        ...
        required=True,
        help="The batch payment generating the errors and warnings displayed in this wizard.",
    )
    error_line_ids = fields.One2many(
        comodel_name="account.batch.error.wizard.line", inverse_name="error_wizard_id"
    )
    warning_line_ids = fields.One2many(
        comodel_name="account.batch.error.wizard.line", inverse_name="warning_wizard_id"
    )
    show_remove_options = fields.Boolean(
        required=True,
        help="True if and only if the options to remove the payments causing the errors or warnings from the batch should be shown",
    )

    @api.model
    def create_from_errors_list(self, batch, errors_list, warnings_list):
        rslt = self.create(
            {"batch_payment_id": batch.id, "show_remove_options": batch.export_file}
        )
...
class BatchErrorWizardLine(models.TransientModel):
    ...
    description = fields.Char(
        string="Description", required=True, help="Description of the error"
    )
    help_message = fields.Char(
        string="Help", help="Additional help message about the error"
    )
    payment_ids = fields.Many2many(
        string="Payments",
        comodel_name="account.payment",
        required=True,
        help="Payments causing this error",
    )
    ...
    show_remove_button = fields.Boolean(
        compute="_compute_show_remove_button",
        help="Whether or not this line should display a button allowing to remove its related payments from the batch",
    )

    def _compute_show_remove_button(self):
        for record in self:
            record.show_remove_button = (
                record.error_wizard_id.show_remove_options
                or record.warning_wizard_id.show_remove_options
            )
