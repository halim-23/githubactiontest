...
class AccountBatchPayment(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", compute="_compute_currency", store=True, readonly=True
    )
    ...
    export_file_create_date = fields.Date(
        string="Generation Date",
        default=fields.Date.today,
        readonly=True,
        help="Creation date of the related export file.",
        copy=False,
    )
    ...
    export_filename = fields.Char(
        string="File Name",
        help="Name of the export file generated for this batch",
        store=True,
        copy=False,
    )

    file_generation_enabled = fields.Boolean(
        help="Whether or not this batch payment should display the 'Generate File' button instead of 'Print' in form view.",
        compute="_compute_file_generation_enabled",
    )
    ...
    def _compute_payment_method_id(self):
        ...
        for batch in self:
            if batch.payment_ids:
                batch.payment_method_id = batch.payment_ids.payment_method_line_id[
                    0
                ].payment_method_id
                continue
            ...
            available_payment_method_lines = (
                batch.journal_id._get_available_payment_method_lines(batch.batch_type)
            )

            batch.available_payment_method_ids = available_payment_method_lines.mapped(
                "payment_method_id"
            )
    ...
    def _compute_state(self):
        for batch in self:
            ...
            elif batch.payment_ids and all(
                pay.is_move_sent for pay in batch.payment_ids
            ):
                batch.state = "sent"
            else:
                batch.state = "draft"
    ...
    def _compute_file_generation_enabled(self):
        for record in self:
            record.file_generation_enabled = (
                record.payment_method_id.code in record._get_methods_generating_files()
            )
    ...
    def _compute_currency(self):
        for batch in self:
            if batch.journal_id:
                batch.currency_id = (
                    batch.journal_id.currency_id
                    or batch.journal_id.company_id.currency_id
                )
            else:
    ...
    def _compute_amount(self):
        for batch in self:
            currency = (
                batch.currency_id
                or batch.journal_id.currency_id
        ...
        for record in self:
            ...
            if len(all_companies) > 1:
                raise ValidationError(
                    _("All payments in the batch must belong to the same company.")
                )
            all_journals = set(record.payment_ids.mapped("journal_id"))
            if len(all_journals) > 1 or (
                record.payment_ids
                and record.payment_ids[:1].journal_id != record.journal_id
            ):
                raise ValidationError(
                    _(
            ...
            if all_types and record.batch_type not in all_types:
                raise ValidationError(
                    _("The batch must have the same type as the payments it contains.")
                )
            all_payment_methods = record.payment_ids.payment_method_id
            if len(all_payment_methods) > 1:
                raise ValidationError(
                    _("All payments in the batch must share the same payment method.")
                )
            if (
                all_payment_methods
                and record.payment_method_id not in all_payment_methods
            ):
                raise ValidationError(
                    _(
    ...
    def create(self, vals):
        vals["name"] = self._get_batch_name(
            vals.get("batch_type"),
            vals.get("date", fields.Date.context_today(self)),
            vals,
        )
    ...
    def write(self, vals):
        if "batch_type" in vals:
            vals["name"] = self.with_context(
                default_journal_id=self.journal_id.id
            )._get_batch_name(vals["batch_type"], self.date, vals)
    ...
    def validate_batch(self):
        ...
        if not self.payment_ids:
            raise UserError(
                _(
                    "Cannot validate an empty batch. Please add some payments to it first."
                )
            )

        errors = (
            not self.export_file and self.check_payments_for_errors() or []
        )  # We don't re-check for errors if we are regenerating the file (we know there aren't any)
        ...
        if errors or warnings:
            return {
                ...
                .create_from_errors_list(self, errors, warnings)
                .id,
            }
    ...
    def check_payments_for_errors(self):
        ...
        rslt = self._check_and_post_draft_payments(
            self.payment_ids.filtered(lambda x: x.state == "draft")
        )
        ...
        no_statement_reconciliation = self.journal_id.default_account_id == (
            pmls.payment_account_id[:1] or default_payment_account
        )
        ...
        if bank_reconciled_payments and not no_statement_reconciliation:
            rslt.append(
                {
                    "title": _(
                        "Some payments have already been matched with a bank statement."
                    ),
    ...
    def _check_and_post_draft_payments(self, draft_payments):
        ...
        return [
            {"title": error, "records": pmts}
            for error, pmts in exceptions_mapping.items()
        ]
    ...
    def print_batch_payment(self):
        return self.env.ref(
            "account_batch_payment.action_print_batch_payment"
