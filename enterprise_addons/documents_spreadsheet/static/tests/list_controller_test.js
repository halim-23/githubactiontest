...
QUnit.module("documents_spreadsheet > list_controller", {}, () => {
  ...
  QUnit.test("Can remove a list with undo after editing a cell", async function (
    assert
  ) {
    assert.expect(4);
    const {model} = await createSpreadsheetFromList();
  ...
  QUnit.test("can select a List from cell formula within a formula", async function (
    assert
  ) {
    assert.expect(1);
    const {model} = await createSpreadsheetFromList();
  ...
  QUnit.test("Verify absence of pivot properties on non-pivot cell", async function (
    assert
  ) {
    assert.expect(1);
