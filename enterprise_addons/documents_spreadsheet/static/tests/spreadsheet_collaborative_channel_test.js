...
QUnit.test("sending a message forward it to the registered listener", function (
  assert
) {
  assert.expect(3);
  ...
  channel.onNewMessage("anId", (message) => {
    ...
    assert.strictEqual(
      message.message,
      "hello",
      "It should have the correct message content"
    );
  });
...
QUnit.test("previous messages are forwarded when registering a listener", function (
  assert
) {
  assert.expect(3);
  const channel = new SpreadsheetCollaborativeChannel(this.env, 5);
