  ...
  function () {
    ...
    test("spreadsheet with generic untitled name is styled", async function (assert) {
      ...
      assert.hasClass(
        ...
        "It should be styled as untitled"
      );
    ...
    QUnit.test("Display with an existing 'Relation' global filter", async function (
      assert
    ) {
      assert.expect(8);
      ...
      await dom.click(items[0].querySelector(".o_side_panel_filter_icon"));
      assert.ok(
        webClient.el.querySelectorAll(".o_spreadsheet_filter_editor_side_panel")
      );
