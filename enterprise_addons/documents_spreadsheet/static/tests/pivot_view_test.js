...
test("Add pivot: date grouping", async (assert) => {
  ...
  assert.equal(
    ...
    `=PIVOT.HEADER("1","date:month","04/2016")`
  );
  assert.equal(
    getCellFormula(model, "A4"),
  ...
  assert.equal(
    getCellFormula(model, "A6"),
    `=PIVOT.HEADER("1","date:month","10/2016")`
  );
  assert.equal(
    getCellFormula(model, "A7"),
...
test("Add pivot: no date groupings", async (assert) => {
  ...
  assert.equal(
    getCellFormula(model, "A4"),
    `=PIVOT.HEADER("1","product_id","37","foo","12")`
  );
  assert.equal(getCellFormula(model, "A5"), `=PIVOT.HEADER("1","product_id","41")`);
  assert.equal(
    getCellFormula(model, "A6"),
    `=PIVOT.HEADER("1","product_id","41","foo","1")`
  );
  assert.equal(
    getCellFormula(model, "A7"),
    `=PIVOT.HEADER("1","product_id","41","foo","2")`
  );
  assert.equal(
    getCellFormula(model, "A8"),
    `=PIVOT.HEADER("1","product_id","41","foo","17")`
  );
});
...
test("pivot with count as measure", async (assert) => {
  ...
  assert.strictEqual(
    getCellFormula(model, "C2"),
    '=PIVOT.HEADER("1","measure","__count")'
  );
...
test("groupby week is sorted", async (assert) => {
  ...
  assert.strictEqual(
    getCellFormula(model, "A3"),
    `=PIVOT.HEADER("1","date:week","15/2016")`
  );
  assert.strictEqual(
    getCellFormula(model, "A4"),
    `=PIVOT.HEADER("1","date:week","43/2016")`
  );
  assert.strictEqual(
    getCellFormula(model, "A5"),
    `=PIVOT.HEADER("1","date:week","49/2016")`
  );
  assert.strictEqual(
    getCellFormula(model, "A6"),
    `=PIVOT.HEADER("1","date:week","50/2016")`
  );
});
...
test("Can reopen a sheet after see records", async function (assert) {
  ...
  await window.o_spreadsheet.__DEBUG__.model.waitForIdle();
  ...
  assert.strictEqual(
    window.o_spreadsheet.__DEBUG__.model.getters.getPivotIds().length,
    2
  );
});
