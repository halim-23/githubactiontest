...
export default class PivotDataSource extends BasicDataSource {
  ...
  async _fetchLabel(field, value) {
    ...
    try {
      ...
      label = new Error(
        _.str.sprintf(_t("Unable to fetch the label of %s of model %s"), value, model)
      );
    }
    if (this.data) {
  ...
  async _createCache(readGroups) {
    ...
    for (const fieldName of fieldNames) {
      orderedMeasureIds[fieldName] = orderedValues[fieldName]
        ? orderedValues[fieldName].map((value) => [
            value,
            groupBys[fieldName][value] || [],
          ])
        : [];
    }
    ...
    const cols = this._createCols(
      this.definition.colGroupBys,
      orderedMeasureIds,
      measures
    );
  ...
  async _orderValues(values, fieldName, field, model, context) {
    ...
    values = ["boolean", "many2one", "many2many", "integer", "float"].includes(
      field.type
    )
  ...
  _fillColumns(
    ...
    for (let [id, vals] of values[field] || []) {
      ...
      const newValues = shouldFilterChildValues[field]
