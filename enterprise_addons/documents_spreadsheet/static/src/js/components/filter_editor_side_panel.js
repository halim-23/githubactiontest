...
odoo.define("documents_spreadsheet.filter_editor_side_panel", function (require) {
  ...
  } = require("@documents_spreadsheet/js/widgets/standalone_many2one_field");
  const {
    TagSelectorWidget,
  ...
  class FilterEditorSidePanel extends owl.Component {
    ...
    async onModelSelected(ev) {
      ...
      for (const pivotId of this.pivotIds) {
        const [field, fieldDesc] = this._findRelation(
          this.getters.getPivotFields(pivotId)
        );
      ...
      for (const listId of this.listIds) {
        const [field, fieldDesc] = this._findRelation(
          this.getters.getListFields(listId)
        );
    ...
    onValuesSelected(ev) {
      ...
      this.state.relation.displayNames = ev.detail.value.map(
