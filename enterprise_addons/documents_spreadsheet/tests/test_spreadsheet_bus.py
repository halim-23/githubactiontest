...
from odoo.addons.documents_spreadsheet.controllers.bus import (
    SpreadsheetCollaborationController,
)
...
class TestSpreadsheetBus(SpreadsheetTestCommon, MailCase):
    def poll(self, *channels, last=0, options=None):
        ...
        channels = SpreadsheetCollaborationController._add_spreadsheet_collaborative_bus_channels(
            self.env, channels
        )
    ...
    def poll_spreadsheet(self, spreadsheet_id):
        ...
        return [m["message"]["payload"] for m in notifs]
