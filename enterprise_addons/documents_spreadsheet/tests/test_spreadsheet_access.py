...
class SpreadsheetAccessTest(SpreadsheetTestCommon):
    ...
    def test_name_get_access_01(self):
        ...
        with self.assertRaises(
            AccessError,
            msg="spreadsheet user cannot access ir.model of 'ir.model.fields'",
        ):
        ...
        with self.assertRaises(
            AccessError,
            msg="portal user cannot read ir.model related to accessible model 'res.partner'",
        ):
            IrModel.with_user(self.portal_user).browse(
                ir_model_res_partner_id
            ).name_get()

        self.assertIsInstance(
            IrModel.with_user(self.tech_user)
            .browse(ir_model_ir_model_fields_id)
            .name_get(),
            list,
    ...
    def test_name_search_access_02(self):
        ...
        self.assertIsInstance(
            SpreadsheetIrModel.name_search(
                name="res.p", args=[("model", "in", ["res.partner"])]
            ),
            list,
        ...
        self.assertIsInstance(
            SpreadsheetIrModel.name_search(
                name="res.p", args=[("model", "=", "res.partner")]
            ),
            list,
            "allow read on allowed domain",
        )
        ...
        with self.assertRaises(
            ...
            IrModel.with_user(self.portal_user).name_search(
                name="res.p", args=[("model", "in", ["res.partner"])]
            )
        ...
        with self.assertRaises(
            ...
            SpreadsheetIrModel.name_search(
                name="%", args=["!", ("model", "=", "res.partner.bank")]
            )
    ...
    def test_search_read_access_03(self):
        ...
        self.assertIsInstance(
            SpreadsheetIrModel.search_read(
                domain=[("model", "=", "res.partner")], fields=["name", "id", "model"]
            ),
            list,
            "allow read on allowed domain",
        )
        ...
        with self.assertRaises(
            AccessError,
            msg="spreadsheet user cannot read on valid domain and invalid fields",
        ):
            SpreadsheetIrModel.search_read(
                domain=[("model", "in", ["res.partner"])],
        ...
        with self.assertRaises(
            ...
            SpreadsheetIrModel.search_read(
                domain=[("model", "in", ["res.partner"])], fields=None
            )

        # Invalid Domain
        with self.assertRaises(
            AccessError,
            msg="spreadsheet user cannot read on invalid domain and valid fields",
        ):
            SpreadsheetIrModel.search_read(
        ...
        with self.assertRaises(
            AccessError,
            msg="spreadsheet user cannot read on invalid domain and invalid fields",
        ):
            SpreadsheetIrModel.search_read(
                domain=[("model", "in", ["ir.model.fields"])],
                fields=["name", "id", "model", "state"],
        ...
        with self.assertRaises(
            AccessError,
            msg="spreadsheet user cannot read on valid domain and empty fields",
        ):
            SpreadsheetIrModel.search_read(
                domain=[("model", "in", ["ir.model.fields"])], fields=None
            )

        with self.assertRaises(
            AccessError, msg="spreadsheet user cannot read on forbidden models "
        ):
            SpreadsheetIrModel.search_read(domain=[], fields=["name", "id", "model"])

        self.assertIsInstance(
            IrModel.with_user(self.tech_user).search_read(domain=[], fields=["state"]),
            list,
