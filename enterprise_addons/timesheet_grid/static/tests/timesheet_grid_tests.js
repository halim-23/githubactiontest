odoo.define("timesheet_grid.timesheet_tests", function (require) {
  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("timesheet with employee section - no groupby", async function (
        assert
      ) {
        assert.expect(2);
        ...
        assert.containsN(
          grid,
          ".o_grid_section > tr > th > .o_standalone_avatar_employee",
          4,
          "should have 4 employee avatars"
        );
      ...
      QUnit.test(
        "timesheet with employee section - groupby employee>task",
      ...
      QUnit.test(
        "timesheet with employee section - groupby task>employee",
        async function (assert) {
          assert.expect(2);
      ...
      QUnit.test(
        "timesheet avatar widget should display hours in gray if in the view show the current period",
        async function (assert) {
          assert.expect(1);
          ...
          const numberOfTimesheetHoursAvatarWidgetWithoutHoursRedAlert = grid.$(
            ".o_grid_section_subtext:not(.o_grid_section_subtext_not_enough_hours)"
          ).length;

          assert.ok(
            numberOfAvatarWidget ===
              numberOfTimesheetHoursAvatarWidgetWithoutHoursRedAlert,
      ...
      QUnit.test(
        ...
        async function (assert) {
          ...
          assert.ok(
            numberOfSpanWhereAllHoursCompleted > 0,
            "There must be at least one element or this test is useless"
          );

          const numberOfSpanWhereAllHoursCompletedYetAreRed = grid.$(
            `span.o_grid_section_subtext_overtime_indication.d-none`
          ).length;
          assert.ok(
            numberOfSpanWhereAllHoursCompletedYetAreRed === 0,
            "Completed hours should not red"
          );
      ...
      QUnit.test(
        "timesheet avatar widget should display hours in red if all the hours were not performed and we are in the past",
        async function (assert) {
          assert.expect(2);
          ...
          const numberOfSpanWhereAllHoursCompleted = grid.$("span:contains('+04:00')")
            .length;
          assert.ok(
            numberOfSpanWhereAllHoursCompleted > 0,
            "There must be at least one element or this test is useless"
          ...
          const numberOfSpanWhereAllHoursCompletedYetAreGreen = grid.$(
            "span.o_grid_section_subtext_overtime:contains('+04:00')"
          ).length;
          assert.strictEqual(
            numberOfSpanWhereAllHoursCompletedYetAreGreen,
            numberOfSpanWhereAllHoursCompleted,
            "The employee has done too much hours, thus we have an overtime."
          );
      ...
      QUnit.test(
        "timesheet avatar widget should display days in red if all the days were not performed and we are in the past",
        async function (assert) {
          assert.expect(2);
          ...
          const numberOfSpanWhereAllHoursCompleted = grid.$(
            "span:contains('-1.00 days')"
          ).length;
          assert.ok(
            numberOfSpanWhereAllHoursCompleted > 0,
            "There must be at least one element or this test is useless"
          ...
          const numberOfSpanWhereAllDaysAreNotDoneYetAreRed = grid.$(
            "span.o_grid_section_subtext_not_enough_hours:contains('-1.00 days')"
          ).length;
          assert.strictEqual(
            numberOfSpanWhereAllDaysAreNotDoneYetAreRed,
            numberOfSpanWhereAllHoursCompleted,
            "The employee has not done enough days, thus we display the number of days in red."
          );
