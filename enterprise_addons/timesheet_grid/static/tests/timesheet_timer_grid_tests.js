  ...
  QUnit.module(
    ...
    function () {
      ...
      QUnit.test("basic timesheet timer grid view", async function (assert) {
        ...
        assert.ok(
          grid.$(".timesheet-timer").length,
          "should have rendered the timer header"
        );
        assert.ok(
          grid.$(".btn_start_timer").length,
          "should have rendered the start timer button"
        );
        assert.strictEqual(
          grid.$("button.btn_timer_line").length,
          2,
        ...
        assert.notOk(
          grid.$(".btn_start_timer").length,
          "start timer button must disappear"
        );
        assert.ok(grid.$(".input_timer").length, "should have rendered the timer");
        assert.ok(
          grid.$(".btn_stop_timer").length,
          "should have rendered the stop timer button"
        );
        assert.ok(
          document.activeElement === grid.$(".btn_stop_timer").get(0),
          "stop button focused"
        );
        assert.ok(
          grid.$(".timer_project_id").length,
          "should have rendered the project input"
        );
        assert.notOk(
          grid.$(".timer_project_id.o_field_invalid").length,
          "project input must be show as valid"
        );
        assert.ok(
          grid.$(".timer_task_id").length,
          "should have rendered the task input"
        );
        ...
        assert.ok(
          grid.$(".timer_project_id.o_field_invalid").length,
          "project input must be show as invalid"
        );
      ...
      QUnit.test("Timer already running", async function (assert) {
        ...
        assert.hasClass(
          grid.$(".btn_timer_line").eq(0),
          "fa-play",
          "should have rendered the play timer button on first line"
        );
        assert.doesNotHaveClass(
          grid.$(".btn_timer_line").eq(1),
          "fa-play",
          "should have rendered the button on second line"
        );
        assert.strictEqual(
          grid.$(".btn_timer_line").length,
          2,
          "should have rendered the button on each line"
        );
        assert.strictEqual(
          grid.$(".timer_project_id input").get(0).value,
          "P1",
          "project is set"
        );
        assert.strictEqual(
          grid.$(".timer_task_id input").get(0).value,
          "BS task",
          "task is set"
        );
        assert.strictEqual(
          grid.$(".input_description input").get(0).value,
          "Description",
          "description is set"
        );
        await testUtils.nextTick();
      ...
      QUnit.test("stop running timer then restart new one", async function (assert) {
        ...
        await testUtils.dom.click(grid.$(".btn_stop_timer"));
        assert.notOk(
          grid.$(".btn_timer_line.fa-play").length,
          "No timer button line should be in play mode"
        );
        assert.ok(
          document.activeElement === grid.$(".btn_start_timer").get(0),
      ...
      QUnit.test("drop running timer then restart new one", async function (assert) {
        ...
        await testUtils.dom.triggerEvent(grid.el, "keydown", {
          key: "Escape",
          target: {tagName: "div"},
        });
        assert.notOk(
      ...
      QUnit.test("Start buttons with groupBy", async function (assert) {
        ...
        assert.notOk(
          grid.$("button.btn_timer_line").length,
          "shouldn't have rendered a start button before each line"
        );
      ...
      QUnit.test("Start button with shift", async function (assert) {
        ...
        assert.strictEqual(
          grid.$(".timesheet-timer").text().indexOf("30"),
          -1,
          "should have rendered classic description"
        );
        ...
        assert.ok(
          grid.$(".timesheet-timer").text().indexOf("30"),
          "should have rendered the text about time added"
        );
        ...
        await testUtils.dom.triggerEvent(grid.$(".btn_start_timer"), "keydown", {
          key: "b",
          which: "66",
        });
        await testUtils.dom.triggerEvent(grid.$(".btn_start_timer"), "keydown", {
        ...
        await testUtils.nextTick();
        assert.strictEqual(
          grid.$("tbody tr:nth(1) td:nth(2)").text(),
          "1:30",
          "Time is correctly added"
        );
        ...
        await testUtils.dom.click(grid.$(".btn_start_timer"));
        await testUtils.nextTick();
        await testUtils.dom.triggerEvent(grid.$(".btn_stop_timer"), "keydown", {
          key: "Shift",
        });
        characters = grid.$("button.btn_timer_line").text();
        assert.strictEqual(
      ...
      QUnit.test("Start timer from button line", async function (assert) {
        ...
        assert.hasClass(
          grid.$(".btn_timer_line").eq(1),
          "fa-play",
          "should have rendered the play timer button on second line"
        );
        assert.doesNotHaveClass(
          grid.$(".btn_timer_line").eq(0),
          "fa-play",
          "should have rendered the button on first line"
        );
        assert.strictEqual(
          grid.$(".timer_project_id input").get(0).value,
          "P1",
        ...
        assert.notOk(
          grid.$(".btn_timer_line.red.fa-play").length,
          "no play button on lines (as no timer is running)"
        );
        assert.strictEqual(
          grid.$(".btn_timer_line").length,
          2,
      ...
      QUnit.test("Change description running timer", async function (assert) {
        ...
        await testUtils.nextTick();
        const descriptionInput = grid.$(".input_description input").get(0);
      ...
      QUnit.test("Edit timer manually", async function (assert) {
        ...
        const timerInput = grid.$("input.input_manual_time").get(0);
        assert.ok(timerInput, "Input timer must be rendered");
        assert.strictEqual(
          timerInput.value,
          "00:15",
          "should be the time returned by rounded function"
        );
        await testUtils.fields.editInput(timerInput, "abc");
        assert.hasClass(
          timerInput,
          "o_field_invalid",
          "'abc' is not a valid time, the input must be invalid"
        );
        await testUtils.fields.editInput(timerInput, "0:23");
        assert.doesNotHaveClass(
          timerInput,
          "o_field_invalid",
          "'0:23' is a valid time, the input must not be invalid"
        );
        await testUtils.dom.triggerEvent(timerInput, "focusout");
        assert.notOk(
          grid.$("input.input_manual_time").length,
          "Input timer must disappear"
        );
        assert.ok(grid.$("#display_timer").length, "Span timer must be rendered");
        await testUtils.nextTick();
        assert.ok(
          grid.$("#display_timer").text().includes("00:23:0"),
          "the timer must run from the value entered"
        );
      ...
      QUnit.test("Timer with favorite project", async function (assert) {
          ...
          mockRPC: function (route, args) {
            if (args.method === "get_timer_data") {
              ...
              assert.deepEqual(
                args.args,
                [[24], 31, 1],
                "check timesheet_id, project_id and task_id"
              );
              return Promise.resolve(24);
            }
            return this._super.apply(this, arguments);
        ...
        assert.hasClass(
          grid.$(".btn_timer_line").eq(0),
          "fa-play",
          "should have rendered the play timer button on line"
        );
        assert.doesNotHaveClass(
          grid.$(".btn_timer_line").eq(1),
          "fa-play",
      ...
      QUnit.test("Edit cell manually with shift key pressed", async function (assert) {
        ...
        await testUtils.dom.triggerEvent($(window)[0].$("body"), "keydown", {
          key: "Shift",
        });
        ...
        assert.strictEqual($input.val(), "04", "val should be 04");
        ...
        assert.strictEqual($input.val(), "04:30", "val should be 04:30");
