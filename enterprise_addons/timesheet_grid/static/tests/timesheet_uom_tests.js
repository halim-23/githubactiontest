  ...
  QUnit.module("Timesheet UOM Widgets", function (hooks) {
    ...
    QUnit.module("GridView timesheet_uom", function (hooks) {
      QUnit.module("fieldRegistry", async function (hooks) {
        QUnit.test(
          ...
          async function (assert) {
            ...
            assert.ok(
              gridComponentRegistry.get("timesheet_uom") ==
                GridTimesheetUOM.FloatFactorComponentTimesheet,
              "FloatFactorComponentTimesheet is rendered when company uom has float_factor as timesheet_widget"
            );
            ...
            assert.ok(
              gridComponentRegistry.get("timesheet_uom") ==
                GridTimesheetUOM.FloatToggleComponentTimesheet,
              "FloatToggleComponentTimesheet is rendered when company uom has float_toggle as timesheet_widget"
            );
          }
        );
      });
      QUnit.module("nodeOptions factor", function (hooks) {
        QUnit.test(
          "the Grid timesheet factor nodeOptions are company related",
          async function (assert) {
            assert.expect(4);
            ...
            let $renderedTotalData = grid.$(
              '.o_view_grid tbody tr:nth-of-type(1) td:nth-of-type(2) :contains("8.00")'
            );
            assert.ok(
              $renderedTotalData.length,
              "The GridView FloatFactorComponentTimesheet widget is taking the timesheet_uom_factor into account"
            );
            grid.destroy();
            ...
            $renderedTotalData = grid.$(
              '.o_view_grid tbody tr:nth-of-type(1) td:nth-of-type(2) :contains("8.00")'
            );
            assert.ok(
              $renderedTotalData.length,
              "The GridView FloatToggleComponentTimesheet widget is taking the timesheet_uom_factor into account"
            );
            grid.destroy();
            ...
            grid = await env.createView(options);
            $renderedTotalData = grid.$(
              '.o_view_grid tbody tr:nth-of-type(1) td:nth-of-type(2) :contains("1.00")'
            );
            assert.ok(
              $renderedTotalData.length,
              "The GridView FloatFactorComponentTimesheet widget is taking the timesheet_uom_factor into account"
      ...
      QUnit.module("fieldRegistry", function (hooks) {
        ...
        hooks.beforeEach(function (assert) {
          ...
          TimerTimesheetUOM.FieldTimesheetTimeTimer.include({
            _render: function () {
              const $widgetIdentification = $("<div>").addClass(
                "i_am_a_timesheet_timer_widget"
              );
          ...
          TimesheetUOM.FieldTimesheetToggle.include({
            _render: function () {
              const $widgetIdentification = $("<div>").addClass(
                "i_am_a_timesheet_toggle_widget"
              );
              this.$el.append($widgetIdentification);
            },
          });
        ...
        QUnit.test(
          "the timesheet_uom_timer widget added to the fieldRegistry is company related",
          async function (assert) {
            assert.expect(2);
            ...
            assert.ok(
              view.$(".i_am_a_timesheet_timer_widget").length,
              "FieldTimesheetTimeTimer is rendered when company uom is hour and timesheet_uom_timer widget is used"
            );
            view.destroy();
            ...
            assert.ok(
              view.$(".i_am_a_timesheet_toggle_widget").length,
              "FieldTimesheetTimeTimer is rendered when company uom is hour and timesheet_uom_timer widget is used"
            );
            view.destroy();
          }
