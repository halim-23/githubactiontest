...
QUnit.test("Check progress bar values", async (assert) => {
  ...
  ganttViewParams.mockRPC = function (route, args) {
    if (route === "/web/dataset/search_read") {
      assert.strictEqual(args.model, "task", "should read on the correct model");
  ...
  assert.strictEqual(
    pills[0].querySelector("span").dataset["progress"],
    "0%;",
    "The first task should have no progress"
  );
  assert.strictEqual(
    pills[0].querySelector("span").getAttribute("style"),
    "width:0%;",
    "The style should reflect the data-progress value"
  );
  assert.strictEqual(
    pills[1].querySelector("span").dataset["progress"],
    "50%;",
    "The second task should have 50% progress"
  );
  assert.strictEqual(
    pills[1].querySelector("span").getAttribute("style"),
    "width:50%;",
    "The style should reflect the data-progress value"
  );
  gantt.destroy();
