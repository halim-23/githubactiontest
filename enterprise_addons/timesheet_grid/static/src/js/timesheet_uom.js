odoo.define("timesheet_grid.timesheet_uom", function (require) {
  ...
  const TimesheetUOMMultiCompanyMixin = (component) =>
    class extends component {
      willStart() {
        ...
        this.currentCompanyTimesheetUOMFactor =
          currentCompany.timesheet_uom_factor || 1;
      }
    };
   ...
   * conversion.
   **/
  class FloatFactorComponentTimesheet extends TimesheetUOMMultiCompanyMixin(
    gridComponent.FloatFactorComponent
  ) {
    // ----------------------------------------------------------------------
    // Getters
    // ----------------------------------------------------------------------
  ...
  class FloatToggleComponentTimesheet extends TimesheetUOMMultiCompanyMixin(
    gridComponent.FloatToggleComponent
  ) {
    // ----------------------------------------------------------------------
    // Getters
    // ----------------------------------------------------------------------
    ...
    get fieldOptions() {
      ...
      if (!hasRange) {
        fieldOptions.range = [0.0, 0.5, 1.0];
      }
      return fieldOptions;
    }
  }
  ...
  const timesheetUomGridService = {
    ...
    start(env, {timesheet_uom}) {
      ...
      if (widgetName === "float_toggle") {
        ...
        FieldTimesheetUom =
          gridComponentRegistry.get(widgetName) || FloatFactorComponentTimesheet;
      }
