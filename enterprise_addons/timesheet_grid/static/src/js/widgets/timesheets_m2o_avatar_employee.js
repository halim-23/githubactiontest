...
const TimesheetM2OAvatarEmployee = StandaloneM2OAvatarEmployee.extend({
  ...
  init(parent, value, rowIndex, rangeContext, timeBoundariesContext, workingHoursData) {
    ...
    if (workingHoursData) {
      ...
      if (this.cacheUnit === "days") {
        this.title = _lt(
          "Difference between the number of days recorded and the number of days the employee was supposed to work according to his contract."
        );
      }
    } else {
      this.hasAllTheRequiredData = false;
      console.error(
        "For some reason, the working hours of the employee couldn't be loaded..."
      );
    }
  },
  ...
  _shouldShowHoursInRed() {
    return (
      this.cacheWorkedHours < this.cacheHours && moment(this.timeContext.end) < moment()
    );
  },
  ...
  _shouldShowHours() {
    return (
      this.cacheWorkedHours !== undefined &&
      this.cacheWorkedHours != null &&
      this.cacheWorkedHours - this.cacheHours != 0 &&
      moment(this.timeContext.end) < moment()
    );
  },
  ...
  _displayOvertimeIndication() {
    ...
    if (this.cacheUnit === "days") {
      ...
      // format in hours
      overtimeIndication += field_utils.format.float_time(
        this.cacheWorkedHours - this.cacheHours
      );
    }
  ...
  _updateTemplateFromCacheData() {
    this.$templateHtml = $(
      qweb._render(this.hoursTemplate, {
