odoo.define("timesheet_grid.TimesheetConfigQRCodeMixin", function (require) {
  ...
  const TimesheetConfigQRCodeMixin = {
    ...
    _onClickAppStoreIcon(ev) {
      ev.preventDefault();
      const googleUrl =
        "https://play.google.com/store/apps/details?id=com.odoo.OdooTimesheets";
      const appleUrl = "https://apps.apple.com/be/app/awesome-timesheet/id1078657549";
      const url = ev.target.classList.contains("o_config_play_store")
        ? googleUrl
        : appleUrl;
      ...
      if (!this.env.device.isMobile) {
        ...
        this.trigger("do-action", {
          action: Object.assign(actionDesktop, {params: {url: url}}),
        });
      } else {
