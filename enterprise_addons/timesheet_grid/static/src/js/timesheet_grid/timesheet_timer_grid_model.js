odoo.define("timesheet_grid.TimerGridModel", function (require) {
  ...
  const TimerGridModel = GridModel.extend({
    ...
    async actionTimer(state) {
      ...
      while (this._gridData.data.hasOwnProperty(i)) {
        array.some((el, index) => {
          if (_.isEqual(el.__label, this._gridData.data[i].__label)) {
            ...
            if (
              this._checkRowsSameOrder(
                state.data[index].rows,
                this._gridData.data[i].rows,
                fields
              )
            ) {
              // Then same order
      ...
      this._gridData.totals = this._computeTotals(
        _.flatten(_.pluck(this._gridData.data, "grid"), true)
      );
