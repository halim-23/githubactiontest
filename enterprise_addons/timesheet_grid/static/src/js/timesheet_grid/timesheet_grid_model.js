odoo.define("timesheet_grid.GridModel", function (require) {
  ...
  const TimesheetGridModel = GridModel.extend({
    ...
    async __load(params) {
      ...
      this._gridData.workingHoursData = await this.fetchAllTimesheetM2OAvatarData(
        this.getEmployeeGridValues(),
        this._gridData.timeBoundariesContext.start,
        this._gridData.timeBoundariesContext.end
      );
    ...
    getTimeContext() {
      ...
      const endOfTheRange = this._gridData.data[0].cols[
        this._gridData.data[0].cols.length - 1
      ].domain[1][2];
