odoo.define("timesheet_grid.TimerGridRenderer", function (require) {
  ...
  class TimerGridRenderer extends GridRenderer {
    constructor(parent, props) {
      ...
      this._onChangeProjectTaskDebounce = _.debounce(
        this._setProjectTask.bind(this),
        500
      );
    }
    mounted() {
    ...
    get showTimerButton() {
      return (
        this.formatType === "float_time" && this.props.groupBy.includes("project_id")
      );
    }
    /**
    ...
    _match_line(new_grid) {
      ...
      for (let i = 0; i < grid.length; i++) {
        ...
        if (
          current_value.project_id &&
          current_value.project_id[0] === this.stateTimer.projectId &&
          ((!current_value.task_id && !this.stateTimer.taskId) ||
            (current_value.task_id &&
              current_value.task_id[0] === this.stateTimer.taskId))
        ) {
    ...
    async _stop_timer() {
      ...
      const timesheetId = this.timesheetId;
      this.timesheetId = false;
