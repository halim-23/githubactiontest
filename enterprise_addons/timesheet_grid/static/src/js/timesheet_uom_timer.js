odoo.define("thimesheet_grid.timesheet_uom_timer", function (require) {
  ...
  const TimesheetUomDisplayTimer = TimesheetUom.FieldTimesheetTime.extend({
    ...
    _startTimeCounter() {
      ...
      if (
        this.recordData.timer_start &&
        !this.recordData.timer_pause &&
        !this.rendererIsSample
      ) {
        this.time = Timer.createTimer(
          this.recordData.unit_amount,
          this.recordData.timer_start,
          this.serverTime
        );
        if (this.time) {
  ...
  const FieldTimesheetTimeTimer = TimesheetUomDisplayTimer.extend({
    ...
    willstart() {
      ...
      return Promise.all([this._super(...arguments), timePromise]);
    },
  ...
  const timesheetUomTimerService = {
    ...
    start(env, {timesheet_uom}) {
      ...
      if (widgetName === "float_toggle") {
        ...
        FieldTimesheetUom =
          (fieldRegistry.get(widgetName) && fieldRegistry.get(widgetName).extend({})) ||
  ...
  const FieldTimesheetHours = TimesheetUomDisplayTimer.extend({
    ...
    async _render() {
      ...
      $timer.before(
        this._makeButton(_lt("Decrease Time"), "action_timer_decrease", "fa-minus")
      );
      $timer.after(
        this._makeButton(_lt("Increase Time"), "action_timer_increase", "fa-plus")
      );
    },
    /**
     * @private
