...
class TestTimesheetValidation(TestCommonTimesheet):
    ...
    def _test_next_date(self, now, result, delay, interval):
        ...
        self.assertEqual(
            result, self.user_manager.company_id.timesheet_mail_manager_nextdate
        )
    ...
    def test_minutes_computing_after_timer_stop(self):
        ...
        timesheet_1.with_user(
            self.user_employee
        ).user_timer_id.timer_start = now - timedelta(minutes=1, seconds=28)
        timesheet_1.with_user(self.user_employee).action_timer_stop()

        self.assertGreater(
            timesheet_1.unit_amount,
            1,
            "unit_amount should be greated than his last value",
        )
    ...
    def test_add_time_from_wizard(self):
        ...
        self.assertEqual(
            wizard_min.save_timesheet().unit_amount,
            1,
            "The timesheet's duration should be 1h (Minimum Duration = 60').",
        )
        self.assertEqual(
            wizard_round.save_timesheet().unit_amount,
            1.25,
            "The timesheet's duration should be 1h15 (Rounding = 15').",
        )
    ...
    def test_action_add_time_to_timer_multi_company(self):
        ...
        timesheet = (
            ...
            .create({"name": "coucou", "project_id": self.project_customer.id})
        )
    ...
    def test_working_hours_for_employees(self):
        ...
        working_hours = employee.get_timesheet_and_working_hours_for_employees(
            employees_grid_data, "2021-12-01", "2021-12-31"
        )
        self.assertEqual(
            working_hours[employee.id]["units_to_work"],
            184.0,
            "Number of hours should be 23d * 8h/d = 184h",
        )
