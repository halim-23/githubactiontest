...
class TestTimesheetMerge(TestCommonTimesheet):
    ...
    def test_merge_timesheets(self):
        ctx = {
            "active_ids": [self.timesheet1.id, self.timesheet2.id, self.timesheet3.id]
        }
        wizard = Form(self.env["hr_timesheet.merge.wizard"].with_context(ctx)).save()

        self.assertEqual(
            wizard.unit_amount, 3.0, "should not consider the validated timesheet"
        )
        self.assertEqual(
            wizard.task_id, self.task1, "should take the task of the 1st timesheet"
        )
        wizard.action_merge()
