...
class MergeTimesheets(models.TransientModel):
    ...
    name = fields.Char(
        "Description", compute="_compute_name", readonly=False, store=True
    )
    ...
    unit_amount = fields.Float(
        "Quantity", compute="_compute_unit_amount", readonly=False, store=True
    )
    ...
    def default_get(self, fields_list):
        ...
        if "timesheet_ids" in fields_list and active_ids:
            ...
            timesheets = timesheets.filtered(
                lambda l: l.is_timesheet and not l.validated
            )
            if timesheets:
    ...
    def _compute_name(self):
        for entry in self:
            ...
            entry.name = " / ".join(
                [name for name in sorted(all_names) if name and name != "/"]
            )
