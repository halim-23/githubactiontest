...
class Employee(models.Model):
    ...
    def _get_timesheet_manager_id_domain(self):
        group = self.env.ref(
            "hr_timesheet.group_hr_timesheet_approver", raise_if_not_found=False
        )
    ...
    def _compute_timesheet_manager(self):
        for employee in self:
            ...
            if (
                manager
                and manager.has_group("hr_timesheet.group_hr_timesheet_approver")
                and (
                    employee.timesheet_manager_id == previous_manager
                    or not employee.timesheet_manager_id
                )
            ):
    ...
    def get_timesheet_and_working_hours(self, date_start, date_stop):
        ...
        for data_row in self.env.cr.dictfetchall():
            result[data_row["employee_id"]]["timesheet_hours"] = float_round(
                data_row["amount_sum"], 2
            )
        ...
        employees_work_days_data = employees._get_work_days_data_batch(
            datetime_min, datetime_max, compute_leaves=False
        )
    ...
    @api.model
    def get_timesheet_and_working_hours_for_employees(
        self, employees_grid_data, date_start, date_stop
    ):
        """
        ...
        for employee in employees:
            ...
            if uom == "days":
                ...
                rounding = len(
                    str(self.env.company.timesheet_encode_uom_id.rounding).split(".")[1]
                )
