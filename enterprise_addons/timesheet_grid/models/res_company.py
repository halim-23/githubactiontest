...
class Company(models.Model):
    ...
    timesheet_mail_employee_allow = fields.Boolean(
        "Employee Reminder",
        default=True,
    ...
    timesheet_mail_employee_interval = fields.Selection(
        ...
        string="Employee Frequency",
        required=True,
        default="weeks",
    )
    timesheet_mail_employee_nextdate = fields.Datetime(
        "Next scheduled date for employee reminder", readonly=True
    )

    # reminder for manager
    timesheet_mail_manager_allow = fields.Boolean(
        "Manager Reminder",
        default=True,
    ...
    timesheet_mail_manager_interval = fields.Selection(
        ...
        string="Manager Reminder Frequency",
        required=True,
        default="weeks",
    )
    timesheet_mail_manager_nextdate = fields.Datetime(
        "Next scheduled date for manager reminder", readonly=True
    )
    ...
    def _timesheet_postprocess(self, values):
        ...
        if any(
            field_name in values
            for field_name in [
                "timesheet_mail_manager_delay",
                "timesheet_mail_manager_interval",
            ]
        ):
    ...
    def _calculate_timesheet_mail_employee_nextdate(self):
        for company in self:
            ...
            company.timesheet_mail_employee_nextdate = fields.Datetime.to_string(
                nextdate
            )
    ...
    def _calculate_timesheet_mail_manager_nextdate(self):
        for company in self:
            ...
            if company.timesheet_mail_manager_interval == "weeks":
                ...
                nextdate = self._calculate_next_month_date(delay)
            company.timesheet_mail_manager_nextdate = fields.Datetime.to_string(
                nextdate
            )
    ...
    def _cron_timesheet_reminder_employee(self):
        ...
        companies = self.search(
            [
                ...
                "|",
                "&",
                ("timesheet_mail_employee_nextdate", "<", today_max),
        ...
        for company in companies:
            if company.timesheet_mail_employee_nextdate < fields.Datetime.today():
                _logger.warning(
                    'The cron "Timesheet: Employees Email Reminder" should have run on %s'
                    % company.timesheet_mail_employee_nextdate
                )
            ...
            if company.timesheet_mail_employee_interval == "months":
                date_start = (
                    date.today() - timedelta(days=company.timesheet_mail_employee_delay)
                ) + relativedelta(day=1)
                date_stop = date_start + relativedelta(months=1, days=-1)
            else:
                date_start = date.today() - timedelta(
                    weeks=1, days=company.timesheet_mail_employee_delay - 1
                )
            ...
            work_hours_struct = employees.get_timesheet_and_working_hours(
            ...
            for employee in employees:
                if (
                    employee.user_id
                    and work_hours_struct[employee.id]["timesheet_hours"]
                    < work_hours_struct[employee.id]["working_hours"]
                ):
    ...
    def _cron_timesheet_reminder_manager(self):
        ...
        companies = self.search(
            [
                ("timesheet_mail_manager_allow", "=", True),
                ("timesheet_mail_manager_nextdate", "<", today_max),
                ("timesheet_mail_manager_nextdate", ">=", today_min),
            ]
        )
        for company in companies:
            # calculate the period
            if company.timesheet_mail_manager_interval == "months":
                date_start = (
                    date.today() - timedelta(days=company.timesheet_mail_manager_delay)
                ) + relativedelta(day=1)
                date_stop = date_start + relativedelta(months=1, days=-1)
            else:
                date_start = date.today() - timedelta(
                    weeks=1, days=company.timesheet_mail_manager_delay - 1
                )
                date_stop = date_start + timedelta(days=6)
    ...
    @api.model
    def _cron_timesheet_send_reminder(
        self, employees, template_xmlid, action_xmlid, additionnal_values=None
    ):
