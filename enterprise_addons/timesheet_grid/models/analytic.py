...
class AnalyticLine(models.Model):
    ...
    validated = fields.Boolean(
        "Validated line", group_operator="bool_and", store=True, copy=False
    )
    ...
    user_can_validate = fields.Boolean(
    ...
    duration_unit_amount = fields.Float(
        related="unit_amount", readonly=True, string="Timesheet Init Amount"
    )
    unit_amount_validate = fields.Float(
        related="unit_amount", readonly=True, string="Timesheet Unit Time"
    )
    ...
    def _compute_display_timer(self):
        other_employee_lines = self.filtered(
            lambda l: l.employee_id not in self.env.user.employee_ids
        )
        ...
        for analytic_line in self - validated_lines - other_employee_lines:
            analytic_line.display_timer = (
                analytic_line.encoding_uom_id == uom_hour
                and self.env.company.timesheet_encode_uom_id == uom_hour
            )
    ...
    def read_grid(
        ...
        result = super(AnalyticLine, self).read_grid(
            row_fields, col_field, cell_field, domain, range, readonly_field, orderby
        )
        ...
        last_week = (
            fields.Datetime.from_string(grid_anchor) - timedelta(days=7)
        ).date()
        domain_search = [
            ("project_id", "!=", False),
            "|",
        ...
        def read_row_value(row_field, timesheet):
            field_name = row_field.split(":")[
                0
            ]  # remove all groupby operator e.g. "date:quarter"
            return timesheets._fields[field_name].convert_to_read(
                timesheet[field_name], timesheet
            )
        ...
        for timesheet in timesheets:
            ...
            if k not in seen:  # check if it's not a duplicated row
                ...
                if not any(record == row for row in res_rows):
                    rows.append(
                        {"values": record, "domain": [("id", "=", timesheet.id)]}
                    )
        ...
        if "project_id" in domain_project_task:
            project_ids = self.env["project.project"].search(
                domain_project_task["project_id"]
            )
        ...
        grid = [
            [
                {
                    **self._grid_make_empty_cell(r["domain"], c["domain"], domain),
                    "is_current": c.get("is_current", False),
                    "is_unavailable": c.get("is_unavailable", False),
                }
    ...
    def _get_unavailable_dates(self, start_date, end_date):
        ...
        items = self.env.company.resource_calendar_id._work_intervals_batch(
        ...
        for line in self:
            if is_manager or (
                ...
                and (
                    ...
                    or line.employee_id.parent_id.user_id.id == self.env.user.id
                    or line.project_id.user_id.id == self.env.user.id
                    or line.user_id == self.env.user.id
                )
            ):
    ...
    def action_validate_timesheet(self):
        ...
        if not self.user_has_groups("hr_timesheet.group_hr_timesheet_approver"):
            notification["params"].update(
                {
                    "title": _(
                        "You can only validate the timesheets of employees of whom you are the manager or the timesheet approver."
                    ),
                    "type": "danger",
                }
            )
        ...
        analytic_lines = self.filtered_domain(
            self._get_domain_for_validation_timesheets()
        )
        if not analytic_lines:
            notification["params"].update(
                {
                    "title": _(
                        "You cannot validate the timesheets from employees that are not part of your team or there are no timesheets to validate."
                    ),
                    "type": "danger",
                }
            )
    ...
    def action_invalidate_timesheet(self):
        ...
        if not self.user_has_groups("hr_timesheet.group_hr_timesheet_approver"):
            raise AccessError(
                _(
                    "You can only reset to draft the timesheets of employees of whom you are the manager or the timesheet approver."
                )
            )
        ...
        if not analytic_lines:
            notification["params"].update(
                {
                    "title": _(
                        "There are no timesheet entries to reset to draft or they have already been invoiced."
                    ),
                    "type": "warning",
                }
            )
    ...
    def create(self, vals_list):
        ...
        if not (
            ...
            line.is_timesheet and line.user_id.id != self.env.user.id
    ...
    def write(self, vals):
        if not self.user_has_groups("hr_timesheet.group_hr_timesheet_approver"):
            if "validated" in vals:
                ...
                raise AccessError(
                    _(
                        "Only a Timesheets Approver or Manager is allowed to modify a validated entry."
                    )
                )
    ...
    def _unlink_if_manager(self):
        if not self.user_has_groups(
            "hr_timesheet.group_hr_timesheet_approver"
        ) and self.filtered(lambda r: r.is_timesheet and r.validated):
            raise AccessError(
                _(
                    "You cannot delete a validated entry. Please, contact your manager or your timesheet approver."
                )
            )
    ...
    def _apply_timesheet_label(self, view_arch, view_type="form"):
        ...
        for node in doc.xpath(
            "//field[@name='unit_amount'][@widget='timesheet_uom' or @widget='timesheet_uom_timer'][not(@string)]"
        ):
            if view_type == "grid":
    ...
    def adjust_grid(self, row_domain, column_field, column_value, cell_field, change):
        ...
        if len(line) > 1:  # copy the last line as adjustment
            ...
            line.write({cell_field: line[cell_field] + change})
        else:  # create new one
    ...
    def _group_expand_project_ids(self, projects, domain, order):
            ...
            if len(rule) == 3 and rule[0] == "date":
                ...
                domain_search.append(
                    (
                        name,
                        operator,
                        "1-1-2250" if operator in ["<", "<="] else "1-1-1970",
                    )
                )
            else:
                domain_search.append(rule)
        ...
        domain_search = expression.AND(
            [[("date", ">=", last_week), ("date", "<=", grid_anchor)], domain_search]
        )
    ...
    def get_running_timer(self):
        ...
        running_seconds = (
            fields.Datetime.now() - timer.timer_start
        ).total_seconds() + timesheet.unit_amount * 3600
        values = {
            "id": timer.res_id,
    ...
    def get_timer_data(self):
        ...
        return {
            "step_timer": int(
                self.env["ir.config_parameter"]
                .sudo()
                .get_param("timesheet_grid.timesheet_min_duration", 15)
            ),
    ...
    def action_add_time_to_timesheet(self, project, task, seconds):
        ...
        timesheet_id = self.create(
            {"project_id": project, "task_id": task, "unit_amount": seconds / 3600}
        )
    ...
    def action_add_time_to_timer(self, time):
        ...
        if not self.user_id.employee_ids:
            raise UserError(
                _("An employee must be linked to your user to record time.")
            )
        timer = self.user_timer_id
        if not timer:
            self.action_timer_start()
            timer = self.user_timer_id
        timer.timer_start = min(
            timer.timer_start - timedelta(0, time), fields.Datetime.now()
        )
    ...
    def _action_open_to_validate_timesheet_view(self, type_view="week"):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "hr_timesheet.act_hr_timesheet_report"
        )
        ...
        action_validate_id = self.env.ref(
            "timesheet_grid.timesheet_grid_to_validate_action", raise_if_not_found=False
        )
        ...
        if type_view == "week":
            pivot_view_id = self.env.ref(
                "timesheet_grid.timesheet_grid_pivot_view_weekly_validate"
            ).id
        else:
        ...
        views = [
            ...
            [
                self.env.ref(
                    "timesheet_grid.timesheet_view_grid_by_employee_validation"
                ).id,
                "grid",
            ],
            ...
            [
                self.env.ref("hr_timesheet.view_kanban_account_analytic_line").id,
                "kanban",
            ],
        ]
        views.sort(
            key=lambda v: view_order.index(v[1]) if v[1] in view_order else 10000
        )
        action.update(
            {
                ...
                "search_view_id": [
                    self.env.ref("timesheet_grid.timesheet_view_search").id,
                    "search",
                ],
                "context": context,
        ...
        if not self.user_has_groups("hr_timesheet.group_timesheet_manager"):
            return expression.AND(
                [
                    ...
                    [
                        "|",
                        ("employee_id.timesheet_manager_id", "=", self.env.user.id),
                        "|",
                        ("employee_id.parent_id.user_id", "=", self.env.user.id),
                        "|",
                        ("project_id.user_id", "=", self.env.user.id),
                        ("user_id", "=", self.env.user.id),
                    ],
                ]
            )
    ...
    def action_merge_timesheets(self):
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref("timesheet_grid.timesheet_merge_wizard_view_form").id,
                    "form",
                )
            ],
