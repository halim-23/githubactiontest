...
from odoo import _, _lt, fields, models, tools
...
class safedict(dict):
    def __init__(self, *args, return_val=None, **kwargs):
        self.__return_val = (
            return_val if return_val is not None else _("Wrong CODA code")
        )
...
card_scheme = safedict(
    {
        "1": _lt("Bancontact/Mister Cash"),
        "2": _lt("Maestro"),
        "3": _lt("Private"),
        "5": _lt("TINA"),
        "9": _lt("Other"),
...
transaction_type = safedict(
    {
        "0": _lt("cumulative"),
        "1": _lt("withdrawal"),
        "2": _lt("cumulative on network"),
        "4": _lt("reversal of purchases"),
        "5": _lt("POS others"),
        "7": _lt("distribution sector"),
        "8": _lt("teledata"),
        "9": _lt("fuel"),
    }
)
product_code = safedict(
    {
        "00": _lt("unset"),
        "01": _lt("premium with lead substitute"),
        "02": _lt("europremium"),
        "03": _lt("diesel"),
        "04": _lt("LPG"),
        "06": _lt("premium plus 98 oct"),
        "07": _lt("regular unleaded"),
        "08": _lt("domestic fuel oil"),
        "09": _lt("lubricants"),
        "10": _lt("petrol"),
        "11": _lt("premium 99+"),
        "12": _lt("Avgas"),
...
type_direct_debit = safedict(
    {
        "0": _lt("unspecified"),
        "1": _lt("recurrent"),
        "2": _lt("one-off"),
        "3": _lt("1-st (recurrent)"),
        "4": _lt("last (recurrent)"),
    }
)
direct_debit_scheme = safedict(
    {"0": _lt("unspecified"), "1": _lt("SEPA core"), "2": _lt("SEPA B2B")}
)
payment_reason = safedict(
    {
        "0": _lt("paid"),
        "1": _lt("technical problem"),
        "2": _lt("reason not specified"),
        "3": _lt("debtor disagrees"),
        "4": _lt("debtor’s account problem"),
    }
)
sepa_type = safedict(
    {
...
default_transaction_code = safedict(
    {
        ...
        "41": _lt("Codes proper to each bank"),
        "42": _lt("Codes proper to each bank"),
        "43": _lt("Codes proper to each bank"),
        "44": _lt("Codes proper to each bank"),
        "45": _lt("Codes proper to each bank"),
        "46": _lt("Codes proper to each bank"),
        "47": _lt("Codes proper to each bank"),
        "48": _lt("Codes proper to each bank"),
        "49": _lt("Cancellation or correction"),
        "87": _lt("Reimbursement of costs"),
        "90": _lt("Codes proper to each bank"),
        "91": _lt("Codes proper to each bank"),
        "92": _lt("Codes proper to each bank"),
        "93": _lt("Codes proper to each bank"),
        "94": _lt("Codes proper to each bank"),
        "95": _lt("Codes proper to each bank"),
        "96": _lt("Codes proper to each bank"),
        "97": _lt("Codes proper to each bank"),
        "98": _lt("Codes proper to each bank"),
...
transaction_code = safedict(
    **{
        "return_val": ("", {}),
        "01": (
            _lt("Domestic or local SEPA credit transfers"),
            {
        ...
        "02": (
            _lt("Instant SEPA credit transfer"),
            {
                "01": _lt("Individual transfer order"),
                "02": _lt("Individual transfer order initiated by the bank"),
                "03": _lt("Standing order"),
        ...
        "35": (
            _lt("Closing (periodical settlements for interest, costs,...)"),
            {
                "01": _lt("Closing"),
                "37": _lt("Costs"),
                "50": _lt("Closing"),
        ...
        "41": (
            _lt("International credit transfers - non-SEPA credit transfers"),
            {
                "01": _lt("Transfer"),
                "03": _lt("Standing order"),
        ...
        "80": (
            _lt("Separately charged costs and provisions"),
            {
...
class AccountBankStatementImport(models.TransientModel):
    ...
    def _check_coda(self, data_file):
        ...
        return re.match(br"0{5}\d{9}05[ D] +", data_file) is not None
    ...
    def _parse_file(self, data_file):
        ...
        def parse_terminal(s):
            return _("Name: {name}, Town: {city}").format(
                name=rmspaces(s[:16]), city=rmspaces(s[16:])
            )
        ...
        def parse_operation(type, family, operation, category):
            return "{type}: {family} ({operation})".format(
                ...
                operation=transaction_code[family][1].get(
                    operation, default_transaction_code.get(operation, _("undefined"))
                ),
            )
        ...
        def parse_structured_communication(type, communication):
            ...
            if type == "100":  # RF Creditor Reference
                ...
                "101",
                "102",
            ):  # Credit transfer or cash payment with structured format communication or with reconstituted structured format communication
                structured_com = (
                    "+++"
                    + communication[:3]
                    + "/"
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("Gross amount in the currency of the account")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                p_idx += 15
                note.append(
                    _("Detail")
                    + ": "
                    + _("Gross amount in the original currency")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                ...
                note.append(
                    ...
                    + _("Rate")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 8)
                )
                o_idx = p_idx
                p_idx += 3
                note.append(
                    ...
                    + _("Currency")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                p_idx += 12
                note.append(
                    _("Detail")
                    + ": "
                    + _("Structured format communication")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("Country code of the principal")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("equivalent in the currency of the account")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("amount on which % is calculated")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                ...
                note.append(
                    ...
                    + _("minimum")
                    + ": "
                    + minimum[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 15
                note.append(
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("interest rates, calculation basis")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("period from {} to {}").format(
                        parsedate(communication[o_idx : o_idx + 6]),
                        parsedate(communication[o_idx + 6 : o_idx + 12]),
                    )
                )
                ...
                note.append(
                    ...
                    + _("card scheme")
                    + ": "
                    + card_scheme[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 6
                note.append(
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("sequence number of first transaction")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("date of first transaction")
                    + ": "
                    + parsedate(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                p_idx += 6
                note.append(
                    _("Detail")
                    + ": "
                    + _("sequence number of last transaction")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    ...
                    + _("transaction type")
                    + ": "
                    + transaction_type[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 26
                note.append(
                    _("Detail")
                    + ": "
                    + _("identification of terminal")
                    + ": "
                    + parse_terminal(communication[o_idx:p_idx])
                )
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("sequence number of transaction")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("hour of transaction")
                    + ": "
                    + parsehour(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                p_idx += 1
                note.append(
                ...
                note.append(
                    ...
                    + _("volume")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 2)
                )
                o_idx = p_idx
                p_idx += 2
                note.append(
                    ...
                    + _("product code")
                    + ": "
                    + product_code[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 5
                note.append(
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("sequence number of validation")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("original amount (given by the customer)")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("basic amount of the calculation")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("guarantee number (no. allocated by the bank)")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("issuing institution")
                    + ": "
                    + issuing_institution[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 12
                note.append(
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("account number of the credit")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("extension zone of account number of the credit")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("amount (equivalent in foreign currency)")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 3)
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("nominal interest rate or rate of charge")
                    + ": "
                    + parsefloat(communication[o_idx:p_idx], 8)
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("reference of transaction on credit account")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                ...
                note.append(
                    ...
                    + _("Type Direct Debit")
                    + ": "
                    + type_direct_debit[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 1
                note.append(
                    _("Detail")
                    + ": "
                    + _("Direct Debit scheme")
                    + ": "
                    + direct_debit_scheme[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 1
                note.append(
                    _("Detail")
                    + ": "
                    + _("Paid or reason for refused payment")
                    + ": "
                    + payment_reason[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 35
                note.append(
                    _("Detail")
                    + ": "
                    + _("Creditor’s identification code")
                    + ": "
                    + rmspaces(communication[o_idx:p_idx])
                )
                o_idx = p_idx
                ...
                note.append(
                    _("Detail")
                    + ": "
                    + _("Type of R transaction")
                    + ": "
                    + sepa_type[communication[o_idx:p_idx]]
                )
                o_idx = p_idx
                p_idx += 4
                note.append(
                ...
                structured_com = (
                    _("Type of structured communication not supported: ") + type
                )
        ...
        for line in recordlist:
            if not line:
                ...
                if statement["version"] == "1":
                    ...
                    if line[1] == "0":  # Belgian bank account BBAN structure
                        ...
                        statement["acc_number"] = (
                            "BE%02d"
                            % (98 - int(statement["acc_number"] + "111400") % 97)
                            + statement["acc_number"]
                        )
                        statement["currency"] = rmspaces(line[18:21])
                ...
                statement["balance_start_date"] = (
                    time.strftime(
                        tools.DEFAULT_SERVER_DATE_FORMAT,
                        time.strptime(rmspaces(line[58:64]), "%d%m%y"),
                    )
                    if rmspaces(line[58:64]) != "000000"
                    else statement["date"]
                )
                ...
                if line[1] == "1":
                    ...
                    statementLine["entryDate"] = time.strftime(
                        tools.DEFAULT_SERVER_DATE_FORMAT,
                        time.strptime(rmspaces(line[115:121]), "%d%m%y"),
                    )
                    statementLine["type"] = "normal"
                    statementLine["globalisation"] = int(line[124])
                    if statementLine["globalisation"] > 0:
                        if (
                            statementLine["ref_move"]
                            in statement["globalisation_stack"]
                        ):
                            statement["globalisation_stack"].remove(
                                statementLine["ref_move"]
                            )
                        else:
                    ...
                    if not statementLine.get("communication"):
                        statementLine["communication"] = globalisation_comm.get(
                            statementLine["ref_move"], ""
                        )
                    ...
                    if statement["lines"][-1]["ref"][0:4] != line[2:6]:
                        raise UserError(
                            ...
                            + "R2004: "
                            + _(
                                "CODA parsing error on movement data record 2.2, seq nr %s! Please report this issue via your Odoo support channel.",
                                line[2:10],
                            )
                        )
                    ...
                    if statement["lines"][-1]["ref"][0:4] != line[2:6]:
                        raise UserError(
                            ...
                            + "R2005: "
                            + _(
                                "CODA parsing error on movement data record 2.3, seq nr %s! Please report this issue via your Odoo support channel.",
                                line[2:10],
                            )
                        )
                    if statement["version"] == "1":
                        statement["lines"][-1]["counterpartyNumber"] = rmspaces(
                            line[10:22]
                        )
                        statement["lines"][-1]["counterpartyName"] = rmspaces(
                            line[47:73]
                        )
                ...
                if line[1] == "1":
                    ...
                    if infoLine["ref"] != rmspaces(line[2:10]):
                        raise UserError(
                            ...
                            + "R3004: "
                            + _(
                                "CODA parsing error on information data record 3.2, seq nr %s! Please report this issue via your Odoo support channel.",
                                line[2:10],
                            )
                        )
                    ...
                    if infoLine["ref"] != rmspaces(line[2:10]):
                        raise UserError(
                            ...
                            + "R3005: "
                            + _(
                                "CODA parsing error on information data record 3.3, seq nr %s! Please report this issue via your Odoo support channel.",
                                line[2:10],
                            )
                        )
                ...
                if not statement.get("balance_end_real"):
                    statement["balance_end_real"] = (
                        ...
                        + statement["balancePlus"]
                        - statement["balanceMin"]
                    )
        ret_statements = []
        for i, statement in enumerate(statements):
            ...
            for line in statement["lines"]:
                to_add = (
                    statement_line
                    and statement_line[-1]["ref"][:4] == line.get("ref_move")
                    and statement_line[-1]
                    or temp_data
                )
                if line["type"] == "information":
                    if line["communication_struct"]:
                        to_add["narration"] = "\n".join(
                            [to_add.get("narration", ""), "Communication: "]
                            + parse_structured_communication(
                                line["communication_type"], line["communication"]
                            )[1]
                        )
                    else:
                        to_add["narration"] = "\n".join(
                            [to_add.get("narration", ""), line["communication"]]
                        )
                ...
                elif line["type"] == "normal" or (
                    line["type"] == "globalisation"
                    and line["ref_move"] in statement["globalisation_stack"]
                    and line["transaction_type"] in [1, 2]
                ):
                    note = []
                    if line.get("counterpartyName"):
                        note.append(
                            _("Counter Party") + ": " + line["counterpartyName"]
                        )
                    else:
                    ...
                    if line.get("counterpartyAddress"):
                        note.append(
                            _("Counter Party Address")
                            + ": "
                            + line["counterpartyAddress"]
                        )
                    structured_com = False
                    if line["communication_struct"]:
                        structured_com, extend_notes = parse_structured_communication(
                            line["communication_type"], line["communication"]
                        )
                        note.extend(extend_notes)
                    elif line.get("communication"):
                        note.append(
                            _("Communication") + ": " + rmspaces(line["communication"])
                        )
                    if (
                        not self.split_transactions
                        and statement_line
                        and line["ref_move"] == statement_line[-1]["ref"][:4]
                    ):
                        to_add["amount"] = to_add.get("amount", 0) + line["amount"]
                        to_add["narration"] = (
                            to_add.get("narration", "") + "\n" + "\n".join(note)
                        )
                    else:
                        line_data = {
                            "payment_ref": structured_com
                            or line.get("communication", "")
                            or "/",
                            "narration": "\n".join(note),
                            "transaction_type": parse_operation(
                                line["transaction_type"],
                                line["transaction_family"],
                                line["transaction_code"],
                                line["transaction_category"],
                            ),
                            "date": line["entryDate"],
                            "amount": line["amount"],
                            "account_number": line.get("counterpartyNumber", None),
                            "partner_name": line["counterpartyName"],
                            "ref": self.split_transactions
                            and line["ref"]
                            or line["ref_move"],
                            "sequence": line["sequence"],
                            "unique_import_id": str(statement["codaSeqNumber"])
                            + "-"
                            + str(statement["date"])
                            + "-"
                            + str(line["ref"]),
