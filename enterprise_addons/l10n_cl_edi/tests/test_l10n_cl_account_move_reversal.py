...
class TestL10AccountMoveReversal(TestL10nClEdiCommon):
    ...
    def test_l10n_cl_account_move_reversal_partial_refund(self):
        ...
        self.assertEqual(
            refund.ref,
            "Reversal of: %s, %s" % (self.invoice.name, refund_wizard.reason),
        )
        self.assertEqual(
            refund.invoice_line_ids.mapped("name"),
        ...
        self.assertEqual(
            refund.invoice_line_ids.name,
            "Where it says: Test Original Text should say: Test Corrected Text",
        )
