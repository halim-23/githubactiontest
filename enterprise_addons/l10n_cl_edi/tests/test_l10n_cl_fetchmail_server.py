...
class TestFetchmailServer(TestL10nClEdiCommon):
    ...
    def test_get_dte_recipient_company_incoming_supplier_document(self):
        incoming_supplier_dte = misc.file_open(
            os.path.join(
                "l10n_cl_edi", "tests", "fetchmail_dtes", "incoming_supplier_dte.xml"
            )
        ).read()
        xml_content = etree.fromstring(incoming_supplier_dte)
        self.assertEqual(
            self.env["fetchmail.server"]._get_dte_recipient_company(
                xml_content, "incoming_supplier_document"
            ),
            self.company_data["company"],
        )

    def test_get_dte_recipient_company_incoming_sii_dte_result(self):
        incoming_sii_dte_result = misc.file_open(
            os.path.join(
                "l10n_cl_edi", "tests", "fetchmail_dtes", "incoming_sii_dte_result.xml"
            )
        ).read()
        xml_content = etree.fromstring(incoming_sii_dte_result)
        self.assertEqual(
            self.env["fetchmail.server"]._get_dte_recipient_company(
                xml_content, "incoming_sii_dte_result"
            ),
            self.company_data["company"],
        )

    def test_get_dte_recipient_company_incoming_acknowledge(self):
        incoming_acknowledge = misc.file_open(
            os.path.join(
                "l10n_cl_edi", "tests", "fetchmail_dtes", "incoming_acknowledge.xml"
            )
        ).read()
        xml_content = etree.fromstring(incoming_acknowledge)
        self.assertEqual(
            self.env["fetchmail.server"]._get_dte_recipient_company(
                xml_content, "incoming_acknowledge"
            ),
            self.company_data["company"],
        )

    def test_get_dte_recipient_company_incoming_commercial_accept(self):
        incoming_commercial_accept = misc.file_open(
            os.path.join(
                "l10n_cl_edi",
                "tests",
                "fetchmail_dtes",
                "incoming_commercial_accept.xml",
            )
        ).read()
        xml_content = etree.fromstring(incoming_commercial_accept)
        self.assertEqual(
            self.env["fetchmail.server"]._get_dte_recipient_company(
                xml_content, "incoming_commercial_accept"
            ),
            self.company_data["company"],
        )

    def test_get_dte_recipient_company_incoming_commercial_reject(self):
        incoming_commercial_reject = misc.file_open(
            os.path.join(
                "l10n_cl_edi",
                "tests",
                "fetchmail_dtes",
                "incoming_commercial_reject.xml",
            )
        ).read()
        xml_content = etree.fromstring(incoming_commercial_reject)
        self.assertEqual(
            self.env["fetchmail.server"]._get_dte_recipient_company(
                xml_content, "incoming_commercial_reject"
            ),
            self.company_data["company"],
        )
    ...
    @patch(
        ...
        return_value=fields.Date.from_string("2019-11-23"),
    )
    def test_create_invoice_33_from_attachment(self, context_today):
        ...
        att_content = misc.file_open(
            os.path.join("l10n_cl_edi", "tests", "fetchmail_dtes", att_name)
        ).read()
    ...
    def test_create_invoice_33_from_attachment_with_sending_partner_defined_on_other_company(
        self,
    ):
        ...
        from_address = "incoming_dte@test.com"
        att_content = misc.file_open(
            "l10n_cl_edi/tests/fetchmail_dtes/{}".format(att_name)
        ).read()
    ...
    # Patch out the VAT check since the VAT number from the sender is invalid
    @patch("odoo.addons.base_vat.models.res_partner.ResPartner.check_vat", MagicMock())
    def test_create_invoice_33_from_attachment_with_sending_partner_defined_on_two_companies(
        self,
    ):
    ...
    def test_create_invoice_34_from_attachment(self):
        ...
        self.assertEqual(
            move.l10n_cl_reference_ids.l10n_cl_reference_doc_type_selection, "52"
        )
    ...
    @patch(
        "odoo.addons.l10n_cl_edi.models.fetchmail_server.FetchmailServer._get_dte_lines"
    )
    ...
    def test_process_incoming_customer_claim_move_not_found(self):
        ...
        with patch("logging.Logger.error") as logger:
            self.env["fetchmail.server"]._process_incoming_customer_claim(
                ...
                att_content,
                att_name,
                origin_type="incoming_acknowledge",
            )
    ...
    def test_process_incoming_customer_claim_accepted(self):
        ...
        self.env["fetchmail.server"]._process_incoming_customer_claim(
            ...
            att_content,
            att_name,
            origin_type="incoming_commercial_accept",
        )
