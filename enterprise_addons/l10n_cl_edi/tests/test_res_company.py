...
class TestResCompany(TransactionCase):
            ...
            .create(
                {
                    ...
                    "private_key": misc.file_open(
                        os.path.join("l10n_cl_edi", "tests", "private_key_test.key")
                    ).read(),
                    "certificate": misc.file_open(
                        os.path.join("l10n_cl_edi", "tests", "cert_test.cert")
                    ).read(),
                    "cert_expiration": fields.Datetime.now()
    ...
    def test_get_digital_signature_none(self):
        with self.assertRaises(
            UserError,
            msg="There is not a valid certificate for the company: %s"
            % self.company_cl.name,
        ):
    ...
    def test_get_digital_signature_not_valid_signature(self):
        ...
        self.company_cl.write(
            {"l10n_cl_certificate_ids": [(6, 0, [self.certificate.id])]}
        )
    ...
    def test_get_digital_signature_user_not_valid_certificate(self):
        ...
        self.company_cl.write(
            {
                "l10n_cl_certificate_ids": [
                    (6, 0, [self.certificate.id, user_certificate.id])
                ]
            }
        )

        self.assertEqual(
            self.company_cl._get_digital_signature(user_id=user.id), self.certificate
        )
    ...
    def test_get_digital_signature_user_valid_certificate(self):
        ...
        self.assertEqual(
            self.company_cl._get_digital_signature(user_id=user.id), user_certificate
        )
