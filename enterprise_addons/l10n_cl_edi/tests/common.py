...
class TestL10nClEdiCommon(AccountEdiTestCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_cl.cl_chart_template"):
        ...
        cls.company_data["company"].write(
            {
                ...
                "l10n_cl_company_activity_ids": [
                    (6, 0, [cls.env.ref("l10n_cl_edi.eco_new_acti_620200").id])
                ],
        ...
        caf_file_template = misc.file_open(
            os.path.join("l10n_cl_edi", "tests", "template", "caf_file_template.xml")
        ).read()

        caf34_file = (
            caf_file_template.replace("<TD></TD>", "<TD>34</TD>")
            .replace(
                "<RS>Blanco Martin Asociados EIRL</RS>",
                "<RS>Blanco Martin &amp; Asociados EIRL</RS>",
            )
            .replace(
        ...
        cls.caf_factura_afecta = (
            cls.env["l10n_cl.dte.caf"]
            .sudo()
            .create(
                {
