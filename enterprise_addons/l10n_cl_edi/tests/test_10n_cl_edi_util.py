...
class TestL10nClDte(TestL10nClEdiCommon):
    @patch(
        "odoo.addons.l10n_cl_edi.models.l10n_cl_edi_util.L10nClEdiUtilMixin._get_seed_ws"
    )
    ...
    def test_get_seed_ok(self, get_seed_ws):
        get_seed_ws.return_value = """<?xml version="1.0" encoding="UTF-8" ?>
        ...
        self.assertEqual(
            self.env["l10n_cl.edi.util"]._get_seed("SIITEST"), "00000000064"
        )
    ...
    @patch(
        "odoo.addons.l10n_cl_edi.models.l10n_cl_edi_util.L10nClEdiUtilMixin._get_signed_token"
    )
    @patch(
        "odoo.addons.l10n_cl_edi.models.l10n_cl_edi_util.L10nClEdiUtilMixin._get_seed_ws"
    )
    ...
    @patch(
        "odoo.addons.l10n_cl_edi.models.l10n_cl_edi_util.L10nClEdiUtilMixin._get_seed"
    )
    def get_token_certificate_no_exists(self, get_seed, get_signed_token, mock_client):
        get_seed.return_value = "1234456"
        get_signed_token.return_value = "test"
        mock_client.return_value.service.getToken.return_value = """<SII:RESPUESTA xmlns:SII="http://www.sii.cl/xxx">
                <SII:RESP_HDR>
                    <ESTADO>11</ESTADO>
    ...
    def test_get_token_ok(self):
        seed = "023071972740"
        xml_expected_dte = misc.file_open(
            os.path.join("l10n_cl_edi", "tests", "expected_xml", "token.xml")
        ).read()

        self.assertXmlTreeEqual(
            self.get_xml_tree_from_string(
                self.env["l10n_cl.edi.util"]
                ._get_signed_token(self.certificate, seed)
                .encode()
            ),
    ...
    def test_analyze_sii_result_rsc(self):
        ...
        self.assertEqual(
            self.env["l10n_cl.edi.util"]._analyze_sii_result(response_parsed),
            "rejected",
        )
