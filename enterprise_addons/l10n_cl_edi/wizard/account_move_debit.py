...
class AccountDebitNote(models.TransientModel):
    ...
    l10n_cl_original_text = fields.Char(
        "Original Text", help="This is the text that is intended to be changed"
    )
    l10n_cl_corrected_text = fields.Char(
        "New Corrected Text", help="This is the text that should say"
    )

    def _l10n_cl_get_reverse_doc_type(self, move):
        if (
            move.partner_id.l10n_cl_sii_taxpayer_type == "4"
            or move.partner_id.country_id.code != "CL"
        ):
            return self.env["l10n_latam.document.type"].search(
                [("code", "=", "111"), ("country_id.code", "=", "CL")], limit=1
            )
        return self.env["l10n_latam.document.type"].search(
            [("code", "=", "56"), ("country_id.code", "=", "CL")], limit=1
        )
        ...
        return (
            len(
                self.env["account.tax.repartition.line"].search(
                    [["account_id", "=", account.id], ["repartition_type", "=", "tax"]]
                )
            )
            > 0
        )
    ...
    def _prepare_default_values(self, move):
        ...
        if (
            move.company_id.account_fiscal_country_id.code != "CL"
            or not move.journal_id.l10n_latam_use_documents
        ):
            return default_values
        reverse_move_latam_doc_type = self._l10n_cl_get_reverse_doc_type(move)
        default_values["invoice_origin"] = "%s %s" % (
        ...
        default_values["l10n_cl_reference_ids"] = [
            [
                0,
                0,
                {
        ...
        if self.l10n_cl_edi_reference_doc_code == "1":
            ...
            tax_amount = sum(
                [
                    (
                        taxes.price_unit
                        if taxes.move_id.move_type in ["out_refund", "in_refund"]
                        else -taxes.price_unit
                    )
                    for taxes in move.line_ids.filtered(
            ...
            for line in move.line_ids.filtered(lambda x: not x.display_type):
                if self._is_tax(line.account_id):
                    ...
                    continue
                price_unit = (
                    abs(tax_amount) - abs(line.price_unit)
                    if line.account_id.user_type_id.type in ["receivable", "payable"]
                    else line.price_unit
                )
                default_values["line_ids"].append(
                    [
                        0,
                        0,
                        {
                            ...
                            "exclude_from_invoice_tab": line.move_id.is_invoice()
                            and (
                                line.account_id.user_type_id.type
                                in ["receivable", "payable"]
                                or self._is_tax(line.account_id)
                            ),
    ...
    def create_debit(self):
        for move in self.move_ids.filtered(
            ...
            and r.l10n_cl_journal_point_of_sale_type == "online"
            and r.l10n_cl_dte_status not in ["accepted", "objected"]
        ):
            raise UserError(
                _(
