...
class AccountMove(models.Model):
    ...
    l10n_cl_claim_description = fields.Char(
        string="Claim Detail", readonly=True, copy=False
    )
    l10n_cl_sii_send_file = fields.Many2one(
        "ir.attachment", string="SII Send file", copy=False
    )
    ...
    l10n_cl_journal_point_of_sale_type = fields.Selection(
        related="journal_id.l10n_cl_point_of_sale_type"
    )
    ...
    def button_cancel(self):
        for record in self.filtered(lambda x: x.company_id.country_id.code == "CL"):
            ...
            if record.l10n_cl_dte_status in ["accepted", "objected"]:
                l10n_cl_dte_status = dict(
                    record._fields["l10n_cl_dte_status"]._description_selection(
                        self.env
                    )
                ).get(record.l10n_cl_dte_status)
                raise UserError(
                    _(
                        "This %s is in SII status: %s. It cannot be cancelled. "
                ...
                raise UserError(
                    _(
                        "This %s is in the intermediate state: 'Ask for Status in the SII'. "
    ...
    def button_draft(self):
        for record in self.filtered(lambda x: x.company_id.country_id.code == "CL"):
            ...
            if record.l10n_cl_dte_status in ["accepted", "objected"]:
                ...
                raise UserError(
                    _(
                        "This %s is in SII status %s. It cannot be reset to draft state. "
                        "Instead you should revert it."
                    )
                    % (record.l10n_latam_document_type_id.name, l10n_cl_dte_status)
                )
    ...
    def _post(self, soft=True):
        ...
        for move in self.filtered(
            lambda x: x.company_id.account_fiscal_country_id.code == "CL"
            and x.company_id.l10n_cl_dte_service_provider in ["SII", "SIITEST"]
            ...
            if (
                ...
                raise UserError(
                    _(
                        "Invoice %s has the currency %s inactive. Please activate the currency and try again."
                    )
            ...
            if (
                ...
                move.move_type in ["in_invoice", "in_refund"]
                and move.l10n_latam_document_type_id._is_doc_type_vendor()
            ):
                move._l10n_cl_edi_post_validation()
                if (
                    move.journal_id.l10n_cl_point_of_sale_type != "online"
                    and not move.l10n_latam_document_type_id._is_doc_type_vendor()
                ):
                ...
                move.with_context(no_new_invoice=True).message_post(
                    body=_("DTE has been created"), attachment_ids=attachment.ids
                )
        return res
    ...
    def action_reverse(self):
        for record in self.filtered(
            ...
            if record.l10n_cl_dte_status == "rejected":
                raise UserError(
                    _(
                        "This %s is rejected by SII. Instead of creating a reverse, you should set it to "
    ...
    def _l10n_cl_send_dte_reception_status(self, status_type):
        ...
        response_id = (
            ...
            .browse(self.env.ref("l10n_cl_edi.response_sequence").id)
            .next_by_id()
        )
        response = self.env.ref("l10n_cl_edi.response_dte")._render(
            {
                ...
                "dte_glosa_status": "DTE Rechazado"
                if status_type == "claimed"
                else "DTE Aceptado OK",
                "code_rejected": "-1" if status_type == "claimed" else None,
                "__keep_empty_lines": True,
            }
        )
        response = unescape(response)
        digital_signature = self.company_id._get_digital_signature(
        ...
        email_template = (
            self.env.ref("l10n_cl_edi.email_template_claimed_ack")
            if status_type == "claimed"
            else self.env.ref("l10n_cl_edi.email_template_receipt_commercial_accept")
        )
        email_template.send_mail(
            self.id,
            force_send=True,
            email_values={"attachment_ids": [dte_attachment.id]},
        )
    ...
    def l10n_cl_send_dte_to_sii(self, retry_send=True):
        ...
        if sii_response_status == "5":
            ...
            _logger.error(
                "The response status is %s. Clearing the token."
            ...
            self.l10n_cl_dte_status = (
                "ask_for_status" if sii_response_status == "0" else "rejected"
            )
        self.message_post(
            body=_("DTE has been sent to SII with response: %s.")
            % self._l10n_cl_get_sii_reception_status_message(sii_response_status)
        )
    ...
    def l10n_cl_verify_dte_status(self, send_dte_to_partner=True):
        ...
        if response_parsed.findtext("{http://www.sii.cl/XMLSchema}RESP_HDR/ESTADO") in [
            "001",
            "002",
            "003",
        ]:
        ...
        self.message_post(
            ...
            % (
                response_parsed.findtext(
                    "{http://www.sii.cl/XMLSchema}RESP_HDR/ESTADO"
                ),
                response_parsed.findtext("{http://www.sii.cl/XMLSchema}RESP_HDR/GLOSA"),
                response_parsed.findtext(
                    "{http://www.sii.cl/XMLSchema}RESP_HDR/NUM_ATENCION"
                ),
            )
        )
    ...
    def l10n_cl_verify_claim_status(self):
        if self.company_id.l10n_cl_dte_service_provider == "SIITEST":
            raise UserError(
                _("This feature is not available in certification/test mode")
            )
        ...
        try:
            ...
            if not self.env.context.get("cron_skip_connection_errs"):
                self.message_post(
                    body=_("Asking for claim status with response:")
                    + "<br/>: %s <br/>" % response
                    + _("failed due to:")
                    + "<br/> %s" % error
                )
            ...
            self.message_post(
                body=_("Asking for claim status with response:") + "<br/> %s" % response
            )
    ...
    def l10n_cl_reprocess_acknowledge(self):
        if not self.partner_id:
            raise UserError(
                _("Please assign a partner before sending the acknowledgement")
            )
        try:
    ...
    def _l10n_cl_send_receipt_acknowledgment(self):
        ...
        xml_ack_template = self.env.ref("l10n_cl_edi.ack_template")._render(
            {
                ...
                "digest_value": xml_content.findtext(
                    ".//ns1:DigestValue",
                    namespaces={"ns1": "http://www.w3.org/2000/09/xmldsig#"},
                ),
                "__keep_empty_lines": True,
            }
        )
        xml_ack_template = (
            xml_ack_template.replace("&lt;", "<")
            .replace("&gt;", ">")
            .replace("&quot;", '"')
        ...
        try:
            ...
            raise Exception(
                _(
                    "There is no signature available to send acknowledge or acceptation of this DTE. "
                    "Please setup your digital signature"
                )
            )
        xml_ack = self._sign_full_xml(
            xml_ack_template,
            digital_signature,
            str(response_id),
            "env_resp",
        ...
        self.env.ref("l10n_cl_edi.email_template_receipt_ack").send_mail(
            self.id, force_send=True, email_values={"attachment_ids": attachment}
        )
    ...
    def l10n_cl_accept_document(self):
        if not self.l10n_latam_document_type_id._is_doc_type_acceptance():
            raise UserError(
                _("The document type with code %s cannot be accepted")
                % self.l10n_latam_document_type_id.code
            )
        if self.company_id.l10n_cl_dte_service_provider == "SIITEST":
            ...
            self.message_post(
                body=_(
                    "Claim status was not sending to SII. This feature is not available in "
        ...
        try:
            ...
            _logger.error(error)
            self.message_post(
                body=_("Exception error parsing the response: %s") % response
            )
            return None
        if cod_response in [0, 1]:
            ...
            self._l10n_cl_send_dte_reception_status("accepted")
            msg = (
                _("Document acceptance was accepted with response:")
                + "<br/> %s" % response
            )
        else:
    ...
    def l10n_cl_claim_document(self):
        if not self.l10n_latam_document_type_id._is_doc_type_acceptance():
            raise UserError(
                _("The document type with code %s cannot be claimed")
                % self.l10n_latam_document_type_id.code
            )
        if self.company_id.l10n_cl_dte_service_provider == "SIITEST":
            ...
            self.message_post(
                body=_(
                    "The claim status was not sent to SII as this feature does not work "
        ...
        try:
            ...
            _logger.error(error)
            self.message_post(
                body="Exception error parsing the response: %s" % response
            )
        else:
            if cod_response in [0, 1]:
                self.write(
    ...
    def _l10n_cl_create_dte_envelope(self, receiver_rut="60803000-K"):
        ...
        template = (
            self.l10n_latam_document_type_id._is_doc_type_voucher()
            and self.env.ref("l10n_cl_edi.envio_boleta")
            or self.env.ref("l10n_cl_edi.envio_dte")
        )
        dte_rendered = template._render(
            {
                ...
                "dte": base64.b64decode(self.l10n_cl_dte_file.datas).decode(
                    "ISO-8859-1"
                ),
                "__keep_empty_lines": True,
            }
        )
        dte_rendered = unescape(dte_rendered).replace(
    ...
    def _l10n_cl_send_dte_to_partner(self):
        ...
        self.env.ref("l10n_cl_edi.l10n_cl_edi_email_template_invoice").send_mail(
            self.id,
            force_send=True,
            email_values={"attachment_ids": [dte_partner_attachment.id]},
        )
    ...
    def _l10n_cl_edi_currency_validation(self):
        if (
            ...
            and self.l10n_cl_journal_point_of_sale_type == "online"
        ):
    ...
    def _l10n_cl_edi_post_validation(self):
        ...
        if (
            ...
            and not (
                self.partner_id.l10n_cl_dte_email and self.company_id.l10n_cl_dte_email
            )
            ...
            raise UserError(
                _(
                    "The %s has not a DTE email defined. This is mandatory for electronic invoicing."
                )
                % (
                    _("partner")
        ...
        if (
            self.l10n_latam_document_type_id.code
            not in ["39", "41", "110", "111", "112"]
            and not self.partner_id.l10n_cl_activity_description
        ):
            raise UserError(
                _(
        ...
        if (
            ...
            raise UserError(
                _(
                    "It seems that you are using items with taxes in exempt documents in invoice %s."
                    " You must either:\n"
        ...
        if (
            ...
            raise UserError(
                _(
                    "All the items you are billing in invoice %s, have no taxes.\n"
    ...
    def _l10n_cl_get_amounts(self):
        ...
        """
        self.ensure_one()
        vat_taxes = self.line_ids.filtered(
            lambda x: x.tax_line_id.l10n_cl_sii_code == 14
        )
        ...
        values = {
            "vat_amount": self.currency_id.round(
                sum(vat_taxes.mapped("price_subtotal"))
            ),
            # Sum of the subtotal amount affected by tax
            "subtotal_amount_taxable": sum(lines_with_taxes.mapped("price_subtotal"))
            if (
                lines_with_taxes
                and (
                    self.l10n_latam_document_type_id.code == "39"
                    or not self.l10n_latam_document_type_id._is_doc_type_voucher()
                )
            )
            else False,
            # Sum of the subtotal amount not affected by tax
            "subtotal_amount_exempt": sum(lines_without_taxes.mapped("price_subtotal"))
            ...
            "vat_percent": (
                ...
                and not self.l10n_latam_document_type_id._is_doc_type_exempt()
                else False
            ),
        ...
        if (
            self.currency_id != self.company_id.currency_id
            and self.l10n_latam_document_type_id._is_doc_type_export()
        ):
            ...
            values["second_currency"].update(
                {
                    "subtotal_amount_taxable": sum(
                        lines_with_taxes.mapped("price_subtotal")
                    )
                    / rate
                    if lines_with_taxes
                    else False,
                    "subtotal_amount_exempt": sum(
                        lines_without_taxes.mapped("price_subtotal")
                    )
                    / rate
                    if lines_without_taxes
                    else False,
                    "vat_amount": sum(lines_with_taxes.mapped("price_subtotal")) / rate
                    if lines_with_taxes
                    else False,
        ...
        return [
            {
                ...
                "tax_amount": self.currency_id.round(abs(line.price_subtotal)),
            }
    ...
    def _l10n_cl_get_dte_barcode_xml(self):
        ...
        ted = self.env.ref("l10n_cl_edi.ted_template")._render(
            {
                "dd": dd,
                "frmt": self._sign_message(
                    dd.encode("ISO-8859-1"), caf_file.findtext("RSASK")
                ),
        ...
        return {
            ...
            "barcode": etree.tostring(
                etree.fromstring(
                    re.sub(r"<TmstFirma>.*</TmstFirma>", "", ted.replace("&", "&amp;")),
    ...
    def _l10n_cl_get_reverse_doc_type(self):
        if (
            ...
            or self.partner_id.country_id.code != "CL"
        ):
    ...
    def _l10n_cl_get_comuna_recep(self):
        if self.partner_id._l10n_cl_is_foreign():
            return self._format_length(
                ...
                or self.commercial_partner_id.state_id.name
                or "N-A",
                20,
            )
    ...
    def _l10n_cl_get_set_dte_id(self, xml_content):
        set_dte = xml_content.find(
            ".//ns0:SetDTE", namespaces={"ns0": "http://www.sii.cl/SiiDte"}
        )
    ...
    def _l10n_cl_ask_claim_status(self):
        for move in self.search(
            [
                ("l10n_cl_dte_acceptation_status", "in", ["accepted", "claimed"]),
...
class AccountMoveLine(models.Model):
    ...
    def _l10n_cl_get_line_amounts(self):
        ...
        """
        values = {
            "price_item": float(self.price_total / self.quantity)
            if self.move_id.l10n_latam_document_type_id._is_doc_type_voucher()
            and not self.l10n_latam_document_type_id._is_doc_type_electronic_ticket()
            else self.price_unit,
            "total_discount": "{:.0f}".format(
        ...
        if self.move_id.currency_id != self.move_id.company_id.currency_id:
            ...
            )._get_rates(self.move_id.company_id, self.move_id.date).get(
                self.move_id.currency_id.id
            ) or 1
            second_currency_values = {
                "price": self.price_unit
                if not self.move_id.l10n_latam_document_type_id._is_doc_type_export()
                else "{:.4f}".format(self.price_unit / rate),
                "conversion_rate": "{:.4f}".format(
                    (self.currency_id + self.company_id.currency_id)
                    ._get_rates(self.company_id, self.move_id.date)
                    .get(self.currency_id.id)
                )
                if self.move_id.l10n_latam_document_type_id._is_doc_type_export()
                else False,
                "total_amount": "{:.4f}".format(self.price_subtotal / rate)
                if self.move_id.l10n_latam_document_type_id._is_doc_type_export()
                else False,
            }
