    ...
    def _load_xsd_sii_multi(self):
        ...
        for validator_type, values in validation_types.items():
            ...
            if attachment:
                return
            _logger.info(
                "Downloading file from sii: %s, (%s)"
                % (values["file_url"], values["description"])
            )
            try:
                ...
                _logger.warning(
                    "ConnectionError: %s with the given URL: %s" % (error, url)
                )
                return
            ...
            if values["file_url"].split(".")[1] == "zip":
                try:
                    content = self._extract_sii_xsd_from_zip(
                        url, response, values["file_name"]
                    )
                except:
                    _logger.warning(
                        "UNZIP for %s failed from URL: %s" % (values["file_name"], url)
                    )
            else:
            ...
            file = join(
                tools.config.filestore(self.env.cr.dbname), attachment.store_fname
            )
            files.append(file)
