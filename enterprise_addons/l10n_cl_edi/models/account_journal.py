...
class AccountJournal(models.Model):
    ...
    l10n_cl_point_of_sale_number = fields.Integer(
        ...
        help="This number is needed only if provided by SII.",
        copy=False,
    )
    l10n_cl_point_of_sale_name = fields.Char(
        "Point Of Sale Name",
        help="This is the name that you want to assign to your point of sale. It is not mandatory.",
        copy=False,
    )

    @api.onchange("type")
    def _onchange_type(self):
        self.l10n_cl_point_of_sale_type = (
            "online"
            if self.type == "sale"
            and self.l10n_latam_use_documents
            and self.country_code == "CL"
            else False
        )
