...
class L10nLatamDocumentType(models.Model):
    ...
    l10n_cl_dte_caf_ids = fields.One2many(
        "l10n_cl.dte.caf", "l10n_latam_document_type_id", string="DTE Caf"
    )
    ...
    def _get_caf_file(self, company_id, folio):
        ...
        if not caf:
            raise UserError(
                _(
                    "There are no CAFs available for folio %s in the sequence of %s. "
    ...
    def _get_start_number(self):
        caf = self.sudo().l10n_cl_dte_caf_ids.filtered(
            lambda x: x.status == "in_use" and x.l10n_latam_document_type_id == self.id
        )
        if not caf:
            raise UserError(
                _(
                    "There are no CAFs available. Please upload a CAF file or ask for a new one at www.sii.cl website"
                )
            )
        return caf.start_nb
