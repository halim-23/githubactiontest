...
class L10nClDteCaf(models.Model):
    ...
    caf_file = fields.Binary(
        string="CAF XML File", help="Upload the CAF XML File in this holder"
    )
    ...
    def action_enable(self):
        ...
        if not self.company_id.vat:
            raise UserError(
                _(
                    "The VAT of your company has not been configured. "
        ...
        if self._l10n_cl_format_vat(rut_n) != self._l10n_cl_format_vat(
            ...
            raise UserError(
                _("Caf vat %s should be the same that assigned company's vat: %s!")
