...
class FetchmailServer(models.Model):
    ...
    l10n_cl_is_dte = fields.Boolean(
        "DTE server",
        help="By checking this option, this email account will be used to receive the electronic\n"
    ...
    def fetch_mail(self):
        for server in self.filtered(lambda s: s.l10n_cl_is_dte):
            _logger.info(
                "Start checking for new emails on %s IMAP server %s",
                server.server_type,
                server.name,
            )
            ...
            try:
                ...
                result, data = imap_server.uid(
                    "search", None, "(UID %s:*)" % server.l10n_cl_last_uid
                )
        ...
        return super(
            FetchmailServer, self.filtered(lambda s: not s.l10n_cl_is_dte)
        ).fetch_mail()
    ...
    def _process_incoming_email(self, msg_txt):
        ...
        for attachment in attachments:
            ...
            if attachment_ext.lower() != ".xml" or not self._is_dte_email(
                format_content
            ):
                _logger.info(
                    "Attachment %s has been discarded! It is not a xml file or is not a DTE email"
                    % attachment.fname
                )
                continue
            ...
            if origin_type == "not_classified":
                _logger.info(
                    "Attachment %s has been discarded! Origin type: %s"
                    % (attachment.fname, origin_type)
                )
                continue
            ...
            if not company or not self._is_dte_enabled_company(company):
                _logger.info(
                    "Attachment %s has been discarded! It is not a valid company (id: %s)"
                    % (attachment.fname, company.id)
                )
                continue
            self._process_attachment_content(
                format_content, attachment.fname, from_address, origin_type, company.id
            )

    def _process_attachment_content(
        self, att_content, att_name, from_address, origin_type, company_id
    ):
        """
        This could be called from a button if there is a need to be processed manually
        """
        if origin_type == "incoming_supplier_document":
            for move in self._create_invoice_from_attachment(
                att_content, att_name, from_address, company_id
            ):
                if move.partner_id:
                    try:
            ...
            "incoming_acknowledge",
            "incoming_commercial_accept",
            "incoming_commercial_reject",
        ]:
            self._process_incoming_customer_claim(
                company_id, att_content, att_name, origin_type
            )
    ...
    def _process_incoming_sii_dte_result(self, att_content):
        ...
        if error_status is not None:
            msg = _(
                ...
                status,
                error_status,
                xml_content.findtext("REVISIONENVIO/REVISIONDTE/DETALLE"),
            )
        else:
    ...
    def _process_incoming_customer_claim(
        self, company_id, att_content, att_name, origin_type
    ):
        dte_tag = (
            "RecepcionDTE" if origin_type == "incoming_acknowledge" else "ResultadoDTE"
        )
        xml_content = etree.fromstring(att_content)
        for dte in xml_content.xpath("//ns0:%s" % dte_tag, namespaces=XML_NAMESPACES):
            ...
            if not partner:
                _logger.error(
                    "Partner for incoming customer claim has not been found for %s"
                    % issuer_vat
                )
                continue
            ...
            name = "{} {}".format(
                document_type.doc_code_prefix, document_number.zfill(zfill)
            )
            move = (
            ...
            if not move:
                _logger.error(
                    "Move not found with partner: %s, name: %s, l10n_latam_document_type: %s, "
                    "company_id: %s" % (partner.id, name, document_type.id, company_id)
                )
                continue
            status = {
                "incoming_acknowledge": "received",
                "incoming_commercial_accept": "accepted",
            }.get(origin_type, "claimed")
            move.write({"l10n_cl_dte_acceptation_status": status})
            move.with_context(no_new_invoice=True).message_post(
                body=_(
                    "DTE reception status established as <b>%s</b> by incoming email"
                )
                % status,
                attachments=[(att_name, att_content)],
            )

    def _check_document_number_exists(
        self, partner_id, document_number, document_type, company_id
    ):
        to_check_documents = (
            self.env["account.move"]
            .sudo()
        ...
        return (
            len(
                to_check_documents.filtered(
                    lambda x: x.l10n_latam_document_type_id.code == document_type.code
                    and x.l10n_latam_document_number.lstrip("0")
                    == document_number.lstrip("0")
                )
            )
            > 0
        )

    def _check_document_number_exists_no_partner(
        self, document_number, document_type, company_id, vat
    ):
    ...
    def _create_invoice_from_attachment(
        self, att_content, att_name, from_address, company_id
    ):
        moves = []
        xml_content = etree.fromstring(att_content)
        for dte_xml in xml_content.xpath("//ns0:DTE", namespaces=XML_NAMESPACES):
            ...
            if not document_type:
                _logger.info(
                    "DTE has been discarded! Document type %s not found"
                    % document_type_code
                )
                continue
            if document_type and document_type.internal_type not in [
                "invoice",
                "debit_note",
                "credit_note",
            ]:
                _logger.info(
                    "DTE has been discarded! The document type %s is not a vendor bill"
                    % document_type_code
                )
                continue
            ...
            if (
                partner
                and self._check_document_number_exists(
                    partner.id, document_number, document_type, company_id
                )
                or (
                    not partner
                    and self._check_document_number_exists_no_partner(
                        document_number, document_type, company_id, issuer_vat
                    )
                )
            ):
            ...
            default_move_type = (
                "in_invoice" if document_type_code != "61" else "in_refund"
            )
            msgs = []
            try:
            ...
            msg = (
                _("Vendor Bill DTE has been generated for the following vendor: </br>")
                if partner
                else _(
                    "Vendor not found: You can generate this vendor manually with the following information: </br>"
                )
            )
            move.with_context(no_new_invoice=True).message_post(
                body=msg
                ...
                % {
                    ...
                    "address": self._get_dte_issuer_address(xml_content) or "",
                },
            ...
            moves.append(move)
            _logger.info(
                _("New move has been created from DTE %s with id: %s")
                % (att_name, move.id)
            )
        return moves
    ...
    def _is_dte_email(self, attachment_content):
        return (
            b"http://www.sii.cl/SiiDte" in attachment_content
            or b"<RESULTADO_ENVIO>" in attachment_content
        )
    ...
    def _get_dte_recipient_company(self, xml_content, origin_type):
        ...
        if not receiver_rut:
            return None
        return (
            self.env["res.company"].sudo().search([("vat", "=", receiver_rut[0].text)])
        )
    ...
    def _get_xml_origin_type(self, xml_content):
        ...
        if tag == "RespuestaDTE":
            if (
                xml_content.findtext(".//ns0:EstadoRecepDTE", namespaces=XML_NAMESPACES)
                == "0"
            ):
                return "incoming_acknowledge"
            if (
                xml_content.findtext(".//ns0:EstadoDTE", namespaces=XML_NAMESPACES)
                == "0"
            ):
    ...
    def _get_dte_issuer_vat(self, xml_content):
        return (
            ...
            or xml_content.findtext(
                ".//ns0:RutEmisor", namespaces=XML_NAMESPACES
            ).upper()
        )

    def _get_dte_receptor_vat(self, xml_content):
        return (
            xml_content.findtext(".//ns0:RUTRecep", namespaces=XML_NAMESPACES).upper()
            or xml_content.findtext(
                ".//ns0:RutReceptor", namespaces=XML_NAMESPACES
            ).upper()
        )
    ...
    def _get_dte_issuer_address(self, xml_content):
        return xml_content.findtext(
            ".//ns0:DirOrigen", default="", namespaces=XML_NAMESPACES
        )
    ...
    def _get_doc_number_padding(self, company_id):
        ...
        move = (
            ...
            .sudo()
            .search(
                [("company_id", "=", company_id), ("name", "not in", (False, "/", ""))],
    ...
    def _get_withholding_taxes(self, company_id, dte_line):
        ...
        tax_codes = [
            int(element.text)
            for element in dte_line.findall(
                ".//ns0:CodImpAdic", namespaces=XML_NAMESPACES
            )
        ]
        return set(
    ...
    def _get_dte_currency(self, dte_xml):
        ...
        ):  # If the currency of the DTE is CLP then the tag doesn't exist
            currency_name = "CLP"
        return (
    ...
    def _get_dte_lines(self, dte_xml, company_id, partner_id):
        ...
        """
        gross_amount = (
            dte_xml.findtext(".//ns0:MntBruto", namespaces=XML_NAMESPACES) is not None
        )
        ...
        for dte_line in dte_xml.findall(".//ns0:Detalle", namespaces=XML_NAMESPACES):
            product_code = dte_line.findtext(
                ".//ns0:VlrCodigo", namespaces=XML_NAMESPACES
            )
            product_name = dte_line.findtext(
                ".//ns0:NmbItem", namespaces=XML_NAMESPACES
            )
            product = self._get_vendor_product(
                product_code, product_name, company_id, partner_id
            )
            ...
            quantity = float(
                dte_line.findtext(
                    ".//ns0:QtyItem", default=1, namespaces=XML_NAMESPACES
                )
            )
            ...
            price_unit = float(
                dte_line.findtext(
                    ".//ns0:PrcItem",
                    default=dte_line.findtext(
                        ".//ns0:MontoItem", namespaces=XML_NAMESPACES
                    ),
            ...
            values = {
                ...
                "name": product.name
                if product
                else dte_line.findtext(".//ns0:NmbItem", namespaces=XML_NAMESPACES),
                "quantity": quantity,
                "price_unit": price_unit,
                "discount": float(
                    dte_line.findtext(
                        ".//ns0:DescuentoPct", default=0, namespaces=XML_NAMESPACES
                    )
                ),
                "default_tax": False,
            }
            if (
                dte_xml.findtext(".//ns0:TasaIVA", namespaces=XML_NAMESPACES)
                is not None
                and dte_line.findtext(".//ns0:IndExe", namespaces=XML_NAMESPACES)
                is None
            ):
            ...
            if gross_amount:
                ...
                ).compute_all(price_unit, currency)["total_excluded"]
            invoice_lines.append(values)

        for desc_rcg_global in dte_xml.findall(
            ".//ns0:DscRcgGlobal", namespaces=XML_NAMESPACES
        ):
            line_type = desc_rcg_global.findtext(
                ".//ns0:TpoMov", namespaces=XML_NAMESPACES
            )
            price_type = desc_rcg_global.findtext(
                ".//ns0:TpoValor", namespaces=XML_NAMESPACES
            )
            valor_dr = desc_rcg_global.findtext(
                ".//ns0:ValorDROtrMnda", namespaces=XML_NAMESPACES
            ) or desc_rcg_global.findtext(".//ns0:ValorDR", namespaces=XML_NAMESPACES)
            values = {
            ...
            if price_type == "%":
                inde_exe_dr = desc_rcg_global.findtext(
                    ".//ns0:IndExeDR", namespaces=XML_NAMESPACES
                )
                if inde_exe_dr is None:  # Applied to items with tax
                    dte_amount_tag = dte_xml.findtext(
                        ".//ns0:MntNetoOtrMnda", namespaces=XML_NAMESPACES
                    ) or dte_xml.findtext(".//ns0:MntNeto", namespaces=XML_NAMESPACES)
                    ...
                    values["price_unit"] = (
                        -price_unit_multiplier * dte_amount_before_discount * percent_dr
                    )
                    values["default_tax"] = self._use_default_tax(dte_xml)
                elif inde_exe_dr == "2":  # Applied to items not billable
                    dte_amount_tag = dte_xml.findtext(
                        ".//ns0:MontoNF", namespaces=XML_NAMESPACES
                    )
                    dte_amount = dte_amount_tag is not None and int(dte_amount_tag) or 0
                    values["price_unit"] = (
                        round(dte_amount - (int(dte_amount) / (1 - amount_dr / 100)))
                        * price_unit_multiplier
                    )
                elif inde_exe_dr == "1":  # Applied to items without taxes
                    dte_amount_tag = dte_xml.findtext(
                        ".//ns0:MntExeOtrMnda", namespaces=XML_NAMESPACES
                    ) or dte_xml.findtext(".//ns0:MntExe", namespaces=XML_NAMESPACES)
                    dte_amount = dte_amount_tag is not None and int(dte_amount_tag) or 0
                    values["price_unit"] = (
                        round(dte_amount - (int(dte_amount) / (1 - amount_dr / 100)))
                        * price_unit_multiplier
                ...
                if gross_amount:
                    amount_dr = default_purchase_tax.with_context(
                        force_price_include=True
                    ).compute_all(amount_dr, currency)["total_excluded"]
                values["price_unit"] = amount_dr * -1 * price_unit_multiplier
                if desc_rcg_global.findtext(
                    ".//ns0:IndExeDR", namespaces=XML_NAMESPACES
                ) not in ["1", "2"]:
                    values["default_tax"] = self._use_default_tax(dte_xml)
            invoice_lines.append(values)
    ...
    def _is_valid_reference_doc_type(self, reference_doc_type):
        reference_codes = [
            item[0]
            for item in self.env["l10n_cl.account.invoice.reference"]
            ._fields["l10n_cl_reference_doc_type_selection"]
            .selection
        ]
    ...
    def _get_invoice_references(self, dte_xml):
        ...
        for reference in dte_xml.findall(
            ".//ns0:Referencia", namespaces=XML_NAMESPACES
        ):
            invoice_reference_ids.append(
                {
                    "origin_doc_number": reference.findtext(
                        ".//ns0:FolioRef", namespaces=XML_NAMESPACES
                    ),
                    "reference_doc_code": reference.findtext(
                        ".//ns0:CodRef", namespaces=XML_NAMESPACES
                    ),
                    "l10n_cl_reference_doc_type_selection": reference.findtext(
                        ".//ns0:TpoDocRef", namespaces=XML_NAMESPACES
                    ),
                    "reason": reference.findtext(
                        ".//ns0:RazonRef", namespaces=XML_NAMESPACES
                    ),
                    "date": reference.findtext(
                        ".//ns0:FchRef", namespaces=XML_NAMESPACES
                    ),
                }
            )
