        ...
        def wrapper_retry(self, *args):
            ...
            while retries > 0:
                try:
                    ...
                    break
            msg = (
                _("- It was not possible to get a seed after %s retries.") % max_retries
            )
            if custom_msg is not None:
                msg = custom_msg + msg
...
class L10nClEdiUtilMixin(models.AbstractModel):
    ...
    def _analyze_sii_result(self, xml_message):
        ...
        for key, values in result_dict.items():
            if status is not None and status.text in values:
                return key
        reject = xml_message.findtext(
            "{http://www.sii.cl/XMLSchema}RESP_BODY/RECHAZADOS"
        )
        if reject and int(reject) >= 1:
            return "rejected"
        accepted = xml_message.findtext(
            "{http://www.sii.cl/XMLSchema}RESP_BODY/ACEPTADOS"
        )
        informed = xml_message.findtext(
            "{http://www.sii.cl/XMLSchema}RESP_BODY/INFORMADOS"
        )
        objected = xml_message.findtext(
            "{http://www.sii.cl/XMLSchema}RESP_BODY/REPAROS"
        )
    ...
    def _xml_validator(
        self, xml_to_validate, validation_type, is_doc_type_voucher=False
    ):
        """
        ...
        if validation_type in ("token", "bol") or (
            validation_type == "doc" and is_doc_type_voucher
        ):
            return True
        ...
        try:
            ...
            _logger.warning(
                _(
                    'The XSD validation files from SII has not been found, please run manually the cron: "Download XSD"'
                )
            )
            return True

    def _sign_full_xml(
        self, message, digital_signature, uri, xml_type, is_doc_type_voucher=False
    ):
        """
        ...
        if xml_type in ["doc", "recep", "token"]:
            ...
            signed_info_template = self.env.ref(
                "l10n_cl_edi.signed_info_template_with_xsi"
            )
        signed_info = signed_info_template._render(
            {
                "uri": "#{}".format(uri),
                "digest_value": base64.b64encode(
                    self._get_sha1_digest(
                        etree.tostring(
                            etree.fromstring(digest_value_tree), method="c14n"
                        )
                    )
                ).decode(),
            }
        ...
        signature = etree.tostring(
            etree.fromstring(unescape(signature)), encoding="unicode"
        )
    ...
    def _get_seed_ws(self, mode):
        ...
        return Client(
            wsdl=SERVER_URL[mode] + "CrSeed.jws?WSDL", transport=transport
        ).service.getSeed()
    ...
    def _get_seed(self, mode):
        ...
        if response is None:
            self._report_connection_err(
                _("Token cannot be generated. Please try again")
            )
            return False
    ...
    def _get_token_ws(self, mode, signed_token):
        transport = Transport(operation_timeout=TIMEOUT)
        return Client(
            wsdl=SERVER_URL[mode] + "GetTokenFromSeed.jws?WSDL", transport=transport
        ).service.getToken(signed_token)

    def _send_xml_to_sii(
        self,
        mode,
        company_website,
        company_vat,
    ...
    application/ms-excel, application/msword, */*",
        ...
        try:
            ...
            self._report_connection_err(
                _("Sending DTE to SII failed due to:") + "<br /> %s" % error
            )
    ...
    def _get_token(self, mode, digital_signature):
        ...
        if not seed:
            return self._connection_exception(
                "exception", _("No possible to get a seed")
            )
        signed_token = self._get_signed_token(digital_signature, seed)
        response = self._get_token_ws(
            mode,
            etree.tostring(
                etree.fromstring(signed_token), pretty_print=True, encoding="ISO-8859-1"
            ).decode(),
        )
        try:
            response_parsed = etree.fromstring(response.encode("utf-8"))
        ...
        status = response_parsed.findtext(
            "{http://www.sii.cl/XMLSchema}RESP_HDR/ESTADO"
        )
        if status is None or status in ["-07", "12", "11"]:
            error = (
                _("No response trying to get a token")
                if status is None
                else response_parsed.findtext(
    ...
    def _get_send_status_ws(self, mode, company_vat, track_id, token):
        transport = Transport(operation_timeout=TIMEOUT)
        return Client(
            SERVER_URL[mode] + "QueryEstUp.jws?WSDL", transport=transport
        ).service.getEstUp(company_vat[:-2], company_vat[-1], track_id, token)
    ...
    @l10n_cl_edi_retry(
        logger=_logger, custom_msg=_("Asking for claim status failed due to:")
    )
    def _get_dte_claim_ws(
        self, mode, settings, company_vat, document_type_code, document_number
    ):
        transport = Transport(operation_timeout=TIMEOUT)
        return Client(
            CLAIM_URL[mode] + "?wsdl", settings=settings, transport=transport
        ).service.listarEventosHistDoc(
    ...
    def _get_dte_claim(
        self, mode, company_vat, digital_signature, document_type_code, document_number
    ):
        token = self._get_token(mode, digital_signature)
        if token is None:
            self._report_connection_err(
            ...
            return False
        settings = Settings(
            strict=False, extra_http_headers={"Cookie": "TOKEN=" + token}
        )
        return self._get_dte_claim_ws(
            mode, settings, company_vat, document_type_code, document_number
        )

    @l10n_cl_edi_retry(
        logger=_logger,
        custom_msg=_("Document acceptance or claim failed due to:") + "<br/> ",
    )
    def _send_sii_claim_response_ws(
        self,
        mode,
        settings,
        company_vat,
        ...
        return Client(
            CLAIM_URL[mode] + "?wsdl", settings=settings, transport=transport
        ).service.ingresarAceptacionReclamoDoc(
            self._l10n_cl_format_vat(company_vat)[:-2],
            self._l10n_cl_format_vat(company_vat)[-1],
            str(document_type_code),
            str(document_number),
    ...
    def _send_sii_claim_response(
        ...
        return self._send_sii_claim_response_ws(
            mode, settings, company_vat, document_type_code, document_number, claim_type
        )
