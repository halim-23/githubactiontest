...
class ResCompany(models.Model):
    ...
    l10n_cl_dte_email = fields.Char(
        "DTE Email", related="partner_id.l10n_cl_dte_email", readonly=False
    )
    l10n_cl_dte_service_provider = fields.Selection(
        [("SIITEST", "SII - Test"), ("SII", "SII - Production")],
    ...
    l10n_cl_sii_taxpayer_type = fields.Selection(
        related="partner_id.l10n_cl_sii_taxpayer_type",
        index=True,
        readonly=False,
    ...
    def _get_digital_signature(self, user_id=None):
        ...
        if user_id is not None:
            user_certificates = self.sudo().l10n_cl_certificate_ids.filtered(
                lambda x: x._is_valid_certificate()
                and x.user_id.id == user_id
                and x.company_id.id == self.id
            )
        ...
        if not shared_certificates:
            raise UserError(
                _("There is not a valid certificate for the company: %s") % self.name
            )
