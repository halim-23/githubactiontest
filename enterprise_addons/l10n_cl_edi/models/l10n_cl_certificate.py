...
class Certificate(models.Model):
    ...
    signature_key_file = fields.Binary(
        string="Certificate Key", help="Certificate Key in PFX format", required=True
    )
    signature_pass_phrase = fields.Char(
        string="Certificate Passkey",
        help="Passphrase for the certificate key",
        copy=False,
        required=True,
    )
    private_key = fields.Text(
        compute="_check_credentials",
        string="Private Key",
        store=True,
        copy=False,
    ...
    certificate = fields.Text(
        compute="_check_credentials", string="Certificate", store=True, copy=False
    )
    ...
    def _get_data(self):
        ...
        try:
            p12 = crypto.load_pkcs12(
                base64.b64decode(self.signature_key_file), self.signature_pass_phrase
            )
    ...
    def _is_valid_certificate(self):
        ...
        return (
            len(
                self.filtered(
                    lambda x: chilean_current_dt
                    <= fields.Datetime.context_timestamp(
                        x.with_context(tz="America/Santiago"), x.cert_expiration
                    )
                )
            )
            >= 1
    ...
    def _check_serial_number(self):
        ...
        for record in self:
            ...
            try:
                ...
                raise UserError(
                    _("The certificate signature_key_file is invalid: %s.") % e
                )
        ...
        for record in self:
            ...
            try:
                ...
                cert_expiration = chilean_tz.localize(
                    datetime.strptime(
                        certificate[1].get_notAfter().decode("utf-8"), date_format
                    )
                )
            except Exception as e:
                raise UserError(
            ...
            record.cert_expiration = cert_expiration.strftime(
            ...
            if chilean_current_dt > cert_expiration:
                raise UserError(
                    _("The certificate is expired since %s") % record.cert_expiration
                )
    ...
    def _get_private_key_modulus(self):
        key = load_pem_private_key(
            self.private_key.encode("ascii"), password=None, backend=default_backend()
        )
        return base64.b64encode(
            self._int_to_bytes(key.public_key().public_numbers().n)
        ).decode("utf-8")
    ...
    def _get_private_key_exponent(self):
        ...
        return base64.b64encode(
            self._int_to_bytes(key.public_key().public_numbers().e)
        ).decode("utf-8")
