...
class MrpWorkcenter(models.Model):
    ...
    def _get_unavailability_intervals(self, start_datetime, end_datetime):
        res = super(MrpWorkcenter, self)._get_unavailability_intervals(
        ...
        self.env.cr.execute(
            ...
            [
                tuple(self.ids),
                fields.Datetime.to_string(start_datetime.astimezone()),
        ...
        for wc_id in self.ids:
            intervals_previous_list = [
                (s.timestamp(), e.timestamp(), self.env["maintenance.request"])
                for s, e in res[wc_id]
            ]
            intervals_maintenances_list = [
                (m[0].timestamp(), m[1].timestamp(), self.env["maintenance.request"])
                for m in res_maintenance[wc_id]
            ]
            final_intervals_wc = Intervals(
                intervals_previous_list + intervals_maintenances_list
            )
            res[wc_id] = [
                (datetime.fromtimestamp(s), datetime.fromtimestamp(e))
                for s, e, _ in final_intervals_wc
            ]
        return res
        ...
        for equipment in self:
            ...
            for maintenance in maintenance_requests:
                if maintenance.stage_id.done and maintenance.close_date:
                    mttr_days += (
                        maintenance.close_date - maintenance.request_date
                    ).days
            equipment.mttr = (
            ...
            if len(maintenance) >= 1:
                equipment.mtbf = (
                    maintenance[-1].request_date - equipment.effective_date
                ).days / len(maintenance)
            equipment.latest_failure_date = (
                maintenance and maintenance[-1].request_date or False
            )
            if equipment.mtbf:
...
class MaintenanceRequest(models.Model):
    ...
    production_company_id = fields.Many2one(
        string="Production Company", related="production_id.company_id"
    )
...
class MrpProduction(models.Model):
    ...
    maintenance_count = fields.Integer(
        compute="_compute_maintenance_count", string="Number of maintenance requests"
    )
    ...
    def open_maintenance_request_mo(self):
        ...
        if self.maintenance_count == 1:
            production = self.env["maintenance.request"].search(
