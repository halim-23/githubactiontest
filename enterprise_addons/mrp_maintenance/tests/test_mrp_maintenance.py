...
class TestMrpMaintenance(common.TransactionCase):
    def setUp(self):
        ...
        self.maintenance_team_id = self.env.ref(
        ...
        self.manager = self.ResUsers.create(
            {
                ...
                "groups_id": [
                    (6, 0, [self.ref("maintenance.group_equipment_manager")])
                ],
            }
        )
    ...
    def test_00_mrp_maintenance(self):
        ...
        self.assertEqual(
            equipment_01.next_action_date,
            datetime.now().date() + timedelta(days=10),
            "Wrong next preventive maintenance date.",
        )

        # Create a maintenance request
        maintenance_request_01 = self._create_request(
            name="Display not working",
            request_date=datetime.now().date() + timedelta(days=10),
        ...
        self.assertEqual(
            equipment_01.next_action_date,
            datetime.now().date() + timedelta(days=10),
            "Wrong next preventive maintenance calculated when there is maintenance todo",
        )

        # check maintenance request date.
        self.assertEqual(
            maintenance_request_01.request_date,
            datetime.now().date() + timedelta(days=10),
            "maintenance request_date is wrong",
        )
        ...
        self.assertEqual(
            ...
            "Wrong next preventive maintenance calculated when there is maintenance done",
        )

        # Create another request which would be in maintenance todo stage
        maintenance_request_02 = self._create_request(
            name="Display not working",
            request_date=datetime.now().date() + timedelta(days=25),
            equipment_id=equipment_01,
            maintenance_type="preventive",
        )
        ...
        self.assertEqual(
            ...
            "Wrong next preventive maintenance calculated when there is one maintenance is done and another todo",
        )
    ...
    def test_01_mrp_maintenance(self):
        ...
        maintenance_request_01 = self._create_request(
            ...
            request_date=datetime(2017, 5, 3).date(),
            equipment_id=equipment_01,
            maintenance_type="corrective",
        )
        maintenance_request_02 = self._create_request(
            name="Touchpad not working",
            request_date=datetime(2017, 5, 23).date(),
            equipment_id=equipment_01,
            maintenance_type="corrective",
        )
        maintenance_request_03 = self._create_request(
            name="Battery drains fast",
            request_date=datetime(2017, 6, 11).date(),
            equipment_id=equipment_01,
            maintenance_type="corrective",
        )
        ...
        maintenance_request_01.write({"stage_id": self.stage_repaired_id})
        maintenance_request_01.close_date = datetime(2017, 5, 3).date() + timedelta(
            days=10
        )
        self.assertEqual(
            maintenance_request_01.close_date,
            datetime(2017, 5, 13).date(),
            "Wrong close date on maintenance request.",
        )

        # maintenance_request_02 write stage_id and close_date.
        maintenance_request_02.write({"stage_id": self.stage_repaired_id})
        maintenance_request_02.close_date = datetime(2017, 5, 23).date() + timedelta(
            days=5
        )
        self.assertEqual(
            maintenance_request_02.close_date,
            datetime(2017, 5, 28).date(),
            "Wrong close date on maintenance request.",
        )
        ...
        self.assertEqual(
            maintenance_request_03.close_date,
            datetime(2017, 6, 11).date(),
            "Wrong close date on maintenance request.",
        )
        ...
        self.assertEqual(
            equipment_01.mttr,
            5,
            "Maintenance Equipment MTTR(Mean Time To Repair) should be 5 days",
        )
        ...
        self.assertEqual(
            equipment_01.mtbf,
            19,
            "Maintenance Equipment MTBF(Mean Time Between Failure) should be 19 days",
        )

        # Check calculation of latest failure date (should be 11-06-2017)
        latest_failure_date = equipment_01.latest_failure_date
        self.assertEqual(
            maintenance_request_03.request_date,
            datetime(2017, 6, 11).date(),
            "Wrong request_date on maintenance request.",
        )
        self.assertEqual(
            latest_failure_date,
            maintenance_request_03.request_date,
            "Wrong latest_failure_date on maintenance request.",
        )

        # Check calculation of estimated next failure (should be 30-06-2017)
        # Step-1: latest failure date + MTBF
        estimated_next_failure = equipment_01.latest_failure_date + timedelta(
            days=equipment_01.mtbf
        )
        self.assertEqual(
    ...
    def test_02_mrp_maintenance(self):
        ...
        self.assertEqual(
            ...
            1,
            "Cron job execution failure or request is not created.",
        )
        ...
        self.assertEqual(
            ...
            equipment_01.next_action_date,
            "wrong request_date in maintenence request",
        )
        ...
        self.assertEqual(
            equipment_01.next_action_date,
            datetime.now().date() + timedelta(days=10),
            "Wrong next preventive maintenance date after cron job execution.",
        )
