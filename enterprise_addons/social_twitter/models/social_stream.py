...
class SocialStreamTwitter(models.Model):
    ...
    def _apply_default_name(self):
        ...
        for stream in twitter_streams:
            ...
            if (
                stream.stream_type_id.stream_type in ["twitter_follow", "twitter_likes"]
                and stream.twitter_followed_account_id
            ):
                name = "%s: %s" % (
                    stream.stream_type_id.name,
                    stream.twitter_followed_account_id.name,
                )
            elif (
                stream.stream_type_id.stream_type == "twitter_user_mentions"
                and stream.account_id
            ):
                name = "%s: %s" % (stream.stream_type_id.name, stream.account_id.name)
            elif (
                stream.stream_type_id.stream_type == "twitter_keyword"
                and stream.twitter_searched_keyword
            ):
                name = "%s: %s" % (
                    stream.stream_type_id.name,
                    stream.twitter_searched_keyword,
                )
    ...
    def _fetch_stream_data(self):
        ...
        if self.stream_type_id.stream_type == "twitter_user_mentions":
            ...
            return self._fetch_tweets(
                ...
                {"user_id": self.twitter_followed_account_id.twitter_id},
            )
            ...
            return self._fetch_tweets(
                "search/tweets",
                {
                    "q": self.twitter_searched_keyword + " -filter:retweets",
                    "result_type": "recent",
                },
            )
    ...
    def _fetch_tweets(self, endpoint_name, extra_params={}):
        ...
        query_params = {"tweet_mode": "extended", "count": 100}
        query_params.update(extra_params)
        tweets_endpoint_url = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT, "/1.1/%s.json" % endpoint_name
        )
        ...
        headers = self.account_id._get_twitter_oauth_header(
            tweets_endpoint_url, params=query_params, method="GET"
        )
        result = requests.get(
            tweets_endpoint_url, params=query_params, headers=headers, timeout=5
        )
        ...
        if not result.ok:
            ...
            if "Not authorized" in result.get("error", ""):
                ...
                ERROR_MESSAGES = {
                    195: _(
                        "The keyword you've typed in does not look valid. Please try again with other words."
                    ),
                    88: _(
                        "Looks like you've made too many requests. Please wait a few minutes before giving it another try."
                    ),
                }
        ...
        result_tweets = (
            result.json()
            if endpoint_name != "search/tweets"
            else result.json().get("statuses")
        )
        if (
            isinstance(result_tweets, dict)
            and result_tweets.get("errors")
            or result_tweets is None
        ):
        ...
        existing_tweets = self.env["social.stream.post"].search(
            [("stream_id", "=", self.id), ("twitter_tweet_id", "in", tweets_ids)]
        )
        ...
        favorites_by_id = self._lookup_tweets(
            [tweet.get("id_str") for tweet in result_tweets]
        )
        ...
        for tweet in result_tweets:
            values = {
                ...
                "published_date": dateutil.parser.parse(
                    tweet.get("created_at"), ignoretz=True
                ),
                "twitter_likes_count": tweet.get("favorite_count"),
                "twitter_user_likes": favorites_by_id.get(
                    tweet.get("id_str"), {"favorited": False}
                )["favorited"],
                ...
                "twitter_profile_image_url": tweet.get("user").get(
    ...
    def _lookup_tweets(self, tweet_ids):
        ...
        page = 1
        lookup_endpoint_ul = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT, "/1.1/statuses/lookup.json"
        )
        favorites_by_id = {}
        while len(tweet_ids) >= ((page - 1) * 100):
            start = (page - 1) * 100
            end = start + 100
            params = {"id": ",".join(tweet_ids[start:end])}
            headers = self.account_id._get_twitter_oauth_header(
                lookup_endpoint_ul, params=params
            )
            response = requests.post(
                lookup_endpoint_ul, data=params, headers=headers, timeout=5
            )
            try:
