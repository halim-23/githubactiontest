...
class SocialStreamPostTwitter(models.Model):
    ...
    def _compute_author_link(self):
        ...
        for post in twitter_posts:
            post.author_link = (
                "https://twitter.com/intent/user?user_id=%s" % post.twitter_author_id
            )
    ...
    def _compute_post_link(self):
        ...
        for post in twitter_posts:
            post.post_link = "https://www.twitter.com/%s/statuses/%s" % (
                post.twitter_author_id,
                post.twitter_tweet_id,
            )
    ...
    def _twitter_comment_add(self, stream, comment_id, message):
        ...
        if attachment:
            images_attachments_ids = stream.account_id._format_bytes_to_images_twitter(
                attachment
            )
            if images_attachments_ids:
                params["media_ids"] = ",".join(images_attachments_ids)

        post_endpoint_url = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT, "/1.1/statuses/update.json"
        )
        headers = stream.account_id._get_twitter_oauth_header(
            post_endpoint_url, params=params
        )
        result = requests.post(
            post_endpoint_url, data=params, headers=headers, timeout=5
        )
    ...
    def _twitter_comment_fetch(self, page=1):
        ...
        tweets_endpoint_url = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT, "/1.1/search/tweets.json"
        )
        query_params = {
            "tweet_mode": "extended",
            "result_type": "recent",
        ...
        answer_results = self._accumulate_tweets(
            tweets_endpoint_url, query_params, search_query
        )
        ...
        self_tweets = self._accumulate_tweets(
            tweets_endpoint_url, query_params, search_query
        )
        self_tweets = [
            tweet
            ...
            if tweet.get("id_str")
            not in [answer_tweet.get("id_str") for answer_tweet in answer_results]
        ]
            ...
            if tweet.get("in_reply_to_status_id_str") == self.twitter_tweet_id:
                ...
                for i in range(len(filtered_tweets)):
                    ...
                    if filtered_tweets[i].get("comments"):
                        tested_against += [
                            answer_tweet["id"]
                            for answer_tweet in filtered_tweets[i]["comments"]["data"]
                        ]
                    if tweet.get("in_reply_to_status_id_str") in tested_against:
                        filtered_tweets[i]["comments"] = filtered_tweets[i].get(
                            "comments", {"data": []}
                        )
                        filtered_tweets[i]["comments"]["data"] += [
                            self.env["social.media"]._format_tweet(tweet)
                        ]

        filtered_tweets = self._add_comments_favorites(filtered_tweets)

        return {"comments": list(reversed(filtered_tweets))}
    ...
    def _twitter_tweet_delete(self, tweet_id):
        ...
        headers = self.stream_id.account_id._get_twitter_oauth_header(delete_endpoint)
        requests.post(delete_endpoint, headers=headers, timeout=5)

        return True

    def _twitter_tweet_like(self, stream, tweet_id, like):
        favorites_endpoint = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT,
            ("/1.1/favorites/create.json" if like else "/1.1/favorites/destroy.json"),
        )
        headers = stream.account_id._get_twitter_oauth_header(
            favorites_endpoint, params={"id": tweet_id}
        )
        requests.post(
            favorites_endpoint, data={"id": tweet_id}, headers=headers, timeout=5
        )
    ...
    def _add_comments_favorites(self, filtered_tweets):
        ...
        for tweet in filtered_tweets:
            ...
            if "comments" in tweet:
                all_tweets_ids += [
                    answer_tweet["id"] for answer_tweet in tweet["comments"]["data"]
                ]

        favorites_by_id = self.stream_id._lookup_tweets(all_tweets_ids)

        for i in range(len(filtered_tweets)):
            looked_up_tweet = favorites_by_id.get(
                filtered_tweets[i]["id"], {"favorited": False}
            )
            ...
            if "comments" in filtered_tweets[i]:
                for j in range(len(filtered_tweets[i]["comments"]["data"])):
                    looked_up_tweet = favorites_by_id.get(
                        filtered_tweets[i]["comments"]["data"][j]["id"],
                        {"favorited": False},
                    )
                    filtered_tweets[i]["comments"]["data"][j][
                        "user_likes"
                    ] = looked_up_tweet["favorited"]

        return filtered_tweets

    def _accumulate_tweets(
        self, endpoint_url, query_params, search_query, query_count=1, force_max_id=None
    ):
        self.ensure_one()
        ...
        if "max_id" in copied_search_query and int(copied_search_query["max_id"]) < int(
            copied_search_query["since_id"]
        ):
        ...
        headers = self.stream_id.account_id._get_twitter_oauth_header(
            endpoint_url, params=query_params, method="GET"
        )
        result = requests.get(
            endpoint_url, params=query_params, headers=headers, timeout=5
        )
