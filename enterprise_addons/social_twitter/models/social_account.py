...
class SocialAccountTwitter(models.Model):
    ...
    def _compute_stats_link(self):
        ...
        for account in twitter_accounts:
            account.stats_link = (
                "https://analytics.twitter.com/user/%s" % account.twitter_screen_name
            )
    ...
    def create(self, vals_list):
        ...
        res.filtered(
            lambda account: account.media_type == "twitter"
        )._create_default_stream_twitter()
        return res
    ...
    def twitter_search_users(self, query):
        ...
        user_search_endpoint = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT, "/1.1/users/search.json"
        )
        params = {"count": 5, "q": query}
        headers = self._get_twitter_oauth_header(
            user_search_endpoint, params=params, method="GET"
        )
        result = requests.get(
            user_search_endpoint, params=params, headers=headers, timeout=5
        )
        return result.json()
    ...
    def _create_default_stream_twitter(self):
        ...
        own_tweets_stream_type_id = self.env.ref(
    ...
    def _get_account_stats(self):
        ...
        twitter_account_info_url = url_join(
            self.env["social.media"]._TWITTER_ENDPOINT, "/1.1/users/show.json"
        )
        headers = self._get_twitter_oauth_header(
    ...
    def _get_last_tweets_stats(self):
        ...
        params = {"count": 200, "user_id": self.twitter_user_id}
        headers = self._get_twitter_oauth_header(
            tweets_endpoint_url, params=params, method="GET"
        )
        result = requests.get(tweets_endpoint_url, params, headers=headers)

        if isinstance(result.json(), dict) and result.json().get("errors"):
            self.sudo().write({"is_media_disconnected": True})
            return False

        last_tweets_stats = {"engagement": 0, "stories": 0}
    ...
    def _get_twitter_oauth_header(self, url, headers={}, params={}, method="POST"):
        ...
        return self.media_id._get_twitter_oauth_header(
    ...
    def _init_twitter_upload(self, image):
        ...
        headers = self._get_twitter_oauth_header(
            TWITTER_IMAGES_UPLOAD_ENDPOINT, params=data
        )
        result = requests.post(
            TWITTER_IMAGES_UPLOAD_ENDPOINT, data=data, headers=headers, timeout=5
        )
        if not result.ok:
    ...
    def _process_twitter_upload(self, image, media_id):
        ...
        headers = self._get_twitter_oauth_header(
            TWITTER_IMAGES_UPLOAD_ENDPOINT, params=params
        )
        requests.post(
