...
class SocialTwitterCase(SocialCase):
    ...
    def setUpClass(cls):
        with patch.object(
            ...
            cls.social_accounts.write({"twitter_oauth_token_secret": "ABCD"})
    ...
    def _test_post(self, success=True):
        ...
        with patch.object(
            SocialAccountTwitter,
            "_format_images_twitter",
            lambda *args, **kwargs: ["media1", "media2"],
    ...
    def test_remove_mentions(self):
        ...
        for message, expected in assert_results:
            self.assertEqual(
                self.env["social.live.post"]._remove_mentions(message), expected
            )
