...
class TestCleaning(test_common.TestCommon):
    def test_cleaning_action_trim_all(self):
        jean_claude = self._create_record(
            "data_cleaning.test.model", name="jean claude"
        )
        ...
        records_found = self.Record.search(
            [("cleaning_model_id", "=", self.TestDCModel.id)]
        )
        self.assertEqual(len(records_found), 1, "Should find 1 record to clean")

        self.assertEqual(
            records_found.res_id, jean_claude.id, "Should clean jean claude"
        )
        self.assertEqual(
            records_found.suggested_value,
            "jeanclaude",
            "Should remove all spaces in jean claude",
        )

        records_found.action_validate()
        self.assertEqual(
            jean_claude.name, "jeanclaude", "should update the name of jeanclaude"
        )
    ...
    def test_cleaning_action_trim_superfluous(self):
        ...
        self.assertEqual(
            records_found.suggested_value,
            "jean claude",
            "Should remove spaces in jean claude",
        )

        records_found.action_validate()
        self.assertEqual(
            jean_claude.name, "jean claude", "should update the name to jean claude"
        )
    ...
    def test_cleaning_action_case_first(self):
        ...
        self.assertEqual(
            records_found.suggested_value,
            "Bernard",
            "Should change to all lowercase with B in caps",
        )
    ...
    def test_cleaning_action_case_upper(self):
        ...
        self.assertEqual(
            records_found.suggested_value,
            "JEAN CLAUDE",
            "Should change to all uppercase",
        )
    ...
    def test_cleaning_action_case_lower(self):
        ...
        self.assertEqual(
            records_found.mapped("res_id"),
            [jean_claude.id, bernard.id],
            "Should be Jean Claude and BERNARD!",
        )
    ...
    def test_cleaning_action_phone(self):
        ...
        self.assertEqual(
            records_found.suggested_value,
            "+32 81 12 34 00",
            "It should add Belgium's country code",
        )
    ...
    def test_cleaning_action_html(self):
        note_html = (
            "h<b>e</b><i>l</i><marquee>lo</marquee> <script>alert('pownez')</script>"
        )
        note_txt = "h*e* l lo  alert('pownez')"
        rec = self._create_record("data_cleaning.test.model", note=note_html)
        self._create_record(
            "data_cleaning.test.model", note="1 is < than 2 and 2 is > than 1"
        )
        ...
        self.assertEqual(
            records_found.suggested_value, note_txt, "It should strip HTML"
        )
    ...
    def test_cleaning_multiple_actions(self):
        ...
        self.assertEqual(
            records_found.suggested_value, "JeanClaude", "should be JeanClaude"
        )
    ...
    def test_cleaning_action_phone_country(self):
        ...
        company = self.env["res.company"].create(
            {"name": "be company", "country_id": country_be.id}
        )
        ...
        self.assertEqual(
            jc.country_id, self.env["res.country"], "no country should be set"
        )
        ...
        self.assertEqual(
            records_found.country_id,
            country_be,
            "It should take the country of the company",
        )
        self.assertEqual(
            records_found.suggested_value,
            "+32 81 12 34 00",
    ...
    def test_automatic_cleaning(self):
        ...
        bernard = self._create_record(
            "data_cleaning.test.model", name="B E r N A    RD"
        )
