...
class CL8ColumnsReport(models.AbstractModel):
            ...
            lines.append(
                {
                    ...
                    "level": 3,
                    "columns": [
                        {"name": self.format_value(subtotals[key])}
            ...
            lines.append(
                {
                    ...
                    "level": 2,
                    "columns": [
                        {"name": self.format_value(totals[key])}
    ...
    def _calculate_exercise_result(self, subtotal_line):
        ...
        if subtotal_line["ganancia"] >= subtotal_line["perdida"]:
            exercise_result["perdida"] = (
                subtotal_line["ganancia"] - subtotal_line["perdida"]
            )
            exercise_result["pasivo"] = exercise_result["perdida"]
        else:
            exercise_result["ganancia"] = (
                subtotal_line["perdida"] - subtotal_line["ganancia"]
            )
    ...
    def _calculate_totals(self, subtotal_line, exercise_result_line):
        totals = OrderedDict(
            [
                ("debe", subtotal_line["debe"]),
                ("haber", subtotal_line["haber"]),
                ("deudor", subtotal_line["deudor"]),
                ("acreedor", subtotal_line["acreedor"]),
                ...
                (
                    "ganancia",
                    subtotal_line["ganancia"] + exercise_result_line["ganancia"],
                ),
            ]
        )
        return totals
