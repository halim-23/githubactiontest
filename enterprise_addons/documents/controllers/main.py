...
class ShareRoute(http.Controller):
    ...
    def binary_content(
        ...
        if share_id:
            ...
            record = share._get_documents_and_check_access(
                share_token, [int(id)], operation="read"
            )
        ...
        if record.type == "url" and record.url:
            ...
            status, content, filename, mimetype, filehash = env[
                "ir.http"
    ...
    @http.route(
        "/documents/upload_attachment", type="http", methods=["POST"], auth="user"
    )
    def upload_document(
        self,
        folder_id,
    ...
    def share_download_all(self, access_token=None, share_id=None):
        ...
        try:
            ...
            documents = share._get_documents_and_check_access(
                access_token, operation="read"
            )
            if documents:
                return self._make_zip(
                    (share.name or "unnamed-link") + ".zip", documents
                )
            else:
                return request.not_found()
        except Exception:
    ...
    @http.route(
        ["/document/avatar/<int:share_id>/<access_token>"], type="http", auth="public"
    )
    def get_avatar(self, access_token=None, share_id=None):
        ...
        try:
            ...
            share = env["documents.share"].sudo().browse(share_id)
            if (
                share._get_documents_and_check_access(
                    access_token, document_ids=[], operation="read"
                )
                is not False
            ):
    ...
    @http.route(
        ["/document/thumbnail/<int:share_id>/<access_token>/<int:id>"],
        type="http",
        auth="public",
    )
    def get_thumbnail(self, id=None, access_token=None, share_id=None):
        ...
        """
        try:
            thumbnail = self._get_file_response(
                id, share_id=share_id, share_token=access_token, field="thumbnail"
            )
            return thumbnail
        except Exception:
    ...
    @http.route(
        ["/document/download/<int:share_id>/<access_token>/<int:id>"],
        type="http",
        auth="public",
    )
    ...
    @http.route(
        [
            "/document/upload/<int:share_id>/<token>/",
    ...
    def upload_attachment(self, share_id, token, document_id=None, **kwargs):
        ...
        if not share.can_upload or (
            not document_id and share.action != "downloadupload"
        ):
        ...
        if document_id and available_documents:
            ...
            try:
                ...
                available_documents.with_context(
                    binary_field_real_user=http.request.env.user
                ).write(write_vals)
            ...
            try:
                for file in request.httprequest.files.getlist("files"):
                    ...
                    document = (
                        request.env["documents.document"]
                        .with_user(share.create_uid)
                        .with_context(binary_field_real_user=http.request.env.user)
                        .create(document_dict)
                    )
    ...
    def share_portal(self, share_id=None, token=None):
        ...
        try:
            ...
            if share.type == "ids" and len(available_documents) == 1:
                ...
                options.update(
                    all_button="binary"
                    in [document.type for document in available_documents],
