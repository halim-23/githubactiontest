odoo.define("documents.DocumentViewer", function (require) {
  ...
  const DocumentsDocumentViewer = DocumentViewer.extend(WidgetAdapterMixin, {
    ...
    init(parent, attachments, activeAttachmentID, {openPdfManager, rules} = {}) {
      ...
      this._workflowRules = rules.filter(
        (rule) => rule.create_model !== "link.to.record"
      );
    },
    ...
    async start() {
      ...
      const isPdfOnly = this._documents.every(
        (record) => record.mimetype === "application/pdf"
      );
    ...
    destroy() {
      if (this._newDocumentIds.length) {
        this.trigger_up("document_viewer_attachment_changed", {
          documentIds: this._newDocumentIds,
        });
      }
    ...
    async _renderPdfManager(documents) {
      const $documentViewerElements = this.$(
        ".o_viewer_content, .move_previous, .move_next"
      );
