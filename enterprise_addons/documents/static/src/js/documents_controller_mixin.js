odoo.define("documents.controllerMixin", function (require) {
  ...
  const DocumentsControllerMixin = Object.assign({}, fileUploadMixin, {
     ...
     * @returns {Object[]} rules that are common to the given documents.
     */
    _getRules(documents) {
    ...
    async _triggerRule(ruleId, recordIds, {preventReload} = {}) {
      ...
      if (_.isObject(result)) {
        if (result.hasOwnProperty("warning")) {
          const documents = Markup(
            result.warning.documents.map((d) => Markup`<li>${d}</li>`).join("")
          );
    ...
    async _updateHelper(searchDomain) {
      var self = this;
      const rpcDomain = searchDomain.filter(
        (elem) => Array.isArray(elem) && elem.includes("folder_id")
      );
      ...
      if (rpcDomain.length && this._hasShareReadAccessRights) {
        return await this._rpc({
          ...
          if (shares.length) {
            const shareAliases = _.map(_.pluck(shares, "alias_id"), (vals) =>
              _.object(["alias_id", "aliasName"], vals)
            );
    ...
    _onClickDocumentsShareDomain() {
      ...
      this._shareDocuments({
        ...
        document_ids: this._selectedRecordIds.length
          ? [[6, 0, this._selectedRecordIds]]
          : false,
      });
    },
    /**
    ...
    _onDragleaveDocumentsView(ev) {
      if ($(ev.target).closest(this.renderer.$el) && ev.target !== this.renderer.el) {
        return;
      }
      this.renderer.$el.removeClass("o_documents_drop_over");
      this.$(".o_documents_upload_text").remove();
    ...
    _onDragstartDocumentDraggable(ev) {
      ...
      const unlockedRecordIds = this.model
        ...
        .data.filter(
          (record) => !record.data.lock_uid || record.data.lock_uid === session.uid
        )
    ...
    _onEditModel: function (ev) {
      ...
      this._rpc({
        model: "ir.model",
        method: "search_read",
        domain: [["model", "=", document.res_model]],
        fields: ["id"],
        limit: 1,
      }).then((models) => {
        const action = {
