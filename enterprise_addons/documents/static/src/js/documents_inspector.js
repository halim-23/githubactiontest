odoo.define("documents.DocumentsInspector", function (require) {
  ...
  const DocumentsInspector = Widget.extend({
    ...
    init: function (parent, params) {
      ...
      for (const resId of params.recordIds) {
        ...
        if (record) {
          ...
          if (record.data.url && record.data.url.length) {
             ...
             */
            youtubeUrlMatch = record.data.url.match(
          ...
          this.recordsData[record.id] = {
            ...
            isYouTubeVideo: Boolean(youtubeToken),
            youtubeToken,
          };
      ...
      const ruleIdsByRecord = this.records.map(
        (record) => record.data.available_rule_ids.res_ids
      );
      ...
      this._rules = commonRuleIds.map((ruleId) => {
        const rule = record.data.available_rule_ids.data.find(
      ...
      this._isLocked = this.records.some(
        (record) => record.data.lock_uid && record.data.lock_uid.res_id !== session.uid
      );
      this.isPdfOnly = this.records.every(
        (record) => record.data.mimetype === "application/pdf"
      );
    },
    /**
     * @override
    ...
    _renderField: function (fieldName, options) {
      ...
      // Render the label
      const $label = $(
        qweb.render("documents.DocumentsInspector.fieldLabel", {
          icon: options.icon,
      ...
      const prom = fieldWidget
        .appendTo($row.find(".o_inspector_value"))
        .then(function () {
    ...
    async _renderHistory() {
      const attachment_ids =
        this.records.length === 1
          ? this.records[0].data.previous_attachment_ids.res_ids
          : [];
      if (!attachment_ids) {
        return;
      }
    ...
    _renderModel: function () {
      ...
      this._rpc({
        ...
        .then((res) => {
          options.is_editable_attachment =
            res && this.records[0].data.is_editable_attachment && !this._isLocked;
          $modelContainer.append(
            qweb.render("documents.DocumentsInspector.resModel", options)
          );
        })
        .catch(() => {
          $modelContainer.append(
    ...
    _renderTags: function () {
      ...
      if (this.tags.length > this.commonTagIds.length) {
        ...
        const disabled =
          this._isLocked || (this.records.length === 1 && !this.records[0].data.active);
    ...
    _search: function (value) {
      ...
      const allSearchResults = tags.filter((tag) =>
        tag.label.toLowerCase().includes(lowerValue)
      );
    ...
    _updateButtons: function () {
      ...
      if (
        !binary &&
        (this.records.length > 1 ||
          (this.records.length && this.records[0].data.type === "empty"))
      ) {
    ...
    _onMouseoverRule: function (ev) {
      $(ev.currentTarget)
        .closest(".o_inspector_trigger_hover_target")
        .addClass("o_inspector_hover");
    },
    /**
    ...
    _onOpenPreview: function (ev) {
      ev.preventDefault();
      ev.stopPropagation();
      if (this.pendingSavingRequests > 0) return;
