odoo.define("documents.DocumentsListController", function (require) {
  ...
  const DocumentsListController = ListController.extend(DocumentsControllerMixin, {
    ...
    custom_events: Object.assign(
      {},
      ListController.prototype.custom_events,
    ...
    _onDocumentsViewRendered() {
      this.$(".o_data_row")
        .attr("draggable", true)
        .addClass("o_document_draggable o_document_list_record");
      ...
      for (const key in localData) {
        ...
        if (lockLocalId) {
          ...
          $record.find(".o_data_cell:nth(0)").append(
            qweb.render("documents.lockIcon", {
    ...
    async _onToggleAll(ev) {
      ...
      this._selectedRecordIds = ev.data.checked
        ? state.data.map((record) => record.res_id)
        : [];
      await Promise.all([
