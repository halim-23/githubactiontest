      ...
      function () {
        QUnit.module("DocumentsKanbanViewMobile", function () {
          QUnit.test("basic rendering on mobile", async function (assert) {
            ...
            // Open search panel
            await dom.click(
              dom.find(document.body, ".o_search_panel_current_selection")
            );
            // Select global view
            await dom.click(
              dom.find(
                document.body,
                ".o_search_panel_category_value:first-child header"
              )
            );
            // Close search panel
            ...
            await dom.click(
              dom.find(
                document.body,
                ".o_search_panel_category_value:nth-child(2) header"
              )
            );
            // Close search panel
            await dom.click(dom.find(document.body, ".o_mobile_search_footer"));
          ...
          QUnit.test("toggle inspector based on selection", async function (assert) {
            ...
            assert.isNotVisible(
              kanban.$(".o_documents_mobile_inspector"),
            ...
            assert.isVisible(
              kanban.$(toggleInspectorSelector),
              "toggle inspector's button should be displayed when selection is not empty"
            );
            assert.strictEqual(
              kanban.$(toggleInspectorSelector).text().replace(/\s+/g, " ").trim(),
              "1 document selected"
            );
        ...
        QUnit.module("DocumentsListViewMobile", function () {
          QUnit.test("basic rendering on mobile", async function (assert) {
            ...
            await dom.click(dom.find(document.body, ".o_mobile_search_footer"));
            assert.ok(
              list.$buttons.find(".o_documents_kanban_upload").is(":disabled"),
              "the upload button should be disabled on global view"
            );
            assert.ok(
              list.$buttons.find(".o_documents_kanban_url").is(":disabled"),
              "the upload url button should be disabled on global view"
            );
            assert.ok(
              list.$buttons.find(".o_documents_kanban_request").is(":disabled"),
              "the request button should be disabled on global view"
            );
            assert.ok(
              list.$buttons.find(".o_documents_kanban_share_domain").is(":disabled"),
              "the share button should be disabled on global view"
            );
            ...
            await dom.click(dom.find(document.body, ".o_mobile_search_footer"));
            assert.ok(
              list.$buttons.find(".o_documents_kanban_upload").not(":disabled"),
              "the upload button should be enabled when a folder is selected"
            );
            assert.ok(
              list.$buttons.find(".o_documents_kanban_url").not(":disabled"),
              "the upload url button should be enabled when a folder is selected"
            );
            assert.ok(
              list.$buttons.find(".o_documents_kanban_request").not(":disabled"),
              "the request button should be enabled when a folder is selected"
            );
            assert.ok(
              list.$buttons.find(".o_documents_kanban_share_domain").not(":disabled"),
              "the share button should be enabled when a folder is selected"
            );
          ...
          QUnit.test("toggle inspector based on selection", async function (assert) {
            ...
            assert.isNotVisible(
              list.$(".o_documents_mobile_inspector"),
              "inspector should be hidden when selection is empty"
            );
            assert.containsN(
              list,
              ".o_document_list_record",
              2,
              "should have 2 records in the renderer"
            );
            ...
            // Select a first record
            await dom.click(
              list.$(".o_document_list_record:first .o_list_record_selector input")
            );
            await nextTick();
            assert.containsOnce(
              list,
              ".o_document_list_record .o_list_record_selector input:checked",
              "should have 1 record selected"
            );
            const toggleInspectorSelector =
              ".o_documents_mobile_inspector > .o_documents_toggle_inspector";
            assert.isVisible(
              list.$(toggleInspectorSelector),
              "toggle inspector's button should be displayed when selection is not empty"
            );
            assert.strictEqual(
              list.$(toggleInspectorSelector).text().replace(/\s+/g, " ").trim(),
              "1 document selected"
            );
            ...
            // Select a second record
            await dom.click(
              list.$(".o_document_list_record:eq(1) .o_list_record_selector input")
            );
            await nextTick();
            assert.containsN(
              list,
