    ...
    QUnit.module(
      ...
      () => {
        QUnit.test("Pdf Manager basic rendering", async function (assert) {
          ...
          assert.containsOnce(
            $(pdfManager.el),
            ".o_pdf_manager_button:contains(Split)",
            "There should be one Split button"
          );
          assert.containsOnce(
            $(pdfManager.el),
            ".o_pdf_manager_button:contains(Add File)",
          ...
          assert.containsN(pdfManager, ".o_pdf_page", 12, "There should be 12 pages");
          assert.containsN(
            pdfManager,
        ...
        QUnit.test("Pdf Manager: page interactions", async function (assert) {
          ...
          await testUtils.dom.click(
            $(pdfManager.el).find(".o_page_splitter_wrapper:nth(1)")
          );
          await testUtils.nextTick();
          ...
          assert.containsN(
            ...
            ".o_pdf_page_selected",
            12,
            "There should be 5 selected pages"
          );
          await testUtils.dom.click(
            $(pdfManager.el).find(".o_documents_pdf_page_selector:nth(3)")
          );
          assert.containsN(
            pdfManager,
            ".o_pdf_page_selected",
        ...
        QUnit.test("Pdf Manager: drag & drop", async function (assert) {
          ...
          assert.containsOnce(
            $(pdfManager.el).find(".o_documents_pdf_page_frame:nth(2)"),
            ".o_pdf_name_display",
          ...
          $(pdfManager.el)
            .find(".o_documents_pdf_canvas_wrapper:nth(5)")[0]
            .dispatchEvent(startEvent);

          const endEvent = new Event("drop", {bubbles: true});
          endEvent.dataTransfer = dataTransfer;
          $(pdfManager.el)
            .find(".o_documents_pdf_canvas_wrapper:nth(1)")[0]
