...
class TestTags(TransactionCase):
    def test_create_tag(self):
        ...
        self.assertEqual(
            tag.facet_id.id, marketing_assets, "should have the right facet"
        )
    ...
    def test_get_tags(self):
        ...
        facet_assets = self.env["documents.facet"].browse(
            self.ref("documents.documents_marketing_assets")
        )
        tag_assets_ads = self.env["documents.tag"].browse(
            self.ref("documents.documents_marketing_assets_ads")
        )
        tag_assets_videos = self.env["documents.tag"].browse(
            self.ref("documents.documents_marketing_assets_Videos")
        )

        domain = [("folder_id", "=", folder_id)]
        folder_ids = (
            self.env["documents.folder"]
            .search([("parent_folder_id", "parent_of", folder_id)])
            .ids
        )
        tags = self.env["documents.tag"]._get_tags(domain, folder_ids)
        processed_tags = self.env["documents.document"]._get_processed_tags(
            domain, folder_ids
        )
        self.assertEqual(
            processed_tags[0]["group_hex_color"],
            "#F06050",
            "first record should have the right color",
        )
    ...
    def test_get_tags_reordered(self):
        ...
        tag_assets_images = self.env["documents.tag"].browse(
            self.ref("documents.documents_marketing_assets_images")
        )
        tag_assets_videos = self.env["documents.tag"].browse(
            self.ref("documents.documents_marketing_assets_Videos")
        )
    ...
    def test_get_tags_empty_folder(self):
        ...
        folder_ids = (
            self.env["documents.folder"]
            .search([("parent_folder_id", "parent_of", empty_folder_id)])
            .ids
        )
        tags = self.env["documents.tag"]._get_tags(domain, folder_ids)
