...
class TestCaseSecurity(TransactionCase):
    ...
    def test_documents_access_default(self):
        ...
        basic_user_doc_d_read_result = document_d.with_user(self.basic_user).read(
            ["name"]
        )
        self.assertEqual(
            basic_user_doc_d_read_result,
        ...
        document_d.with_user(self.document_manager).write(
            {"name": "document_manager_write"}
        )
    ...
    def test_documents_access_manager_read_write(self):
        ...
        document_a.with_user(self.document_manager).write(
            {"name": "nameChangedManagerA"}
        )
        self.assertEqual(
            document_a.name,
            "nameChangedManagerA",
    ...
    def test_documents_access_arbitrary_readonly(self):
        ...
        test_group_user_document_b_name = document_b.with_user(
            self.test_group_user
        ).read(["name"])
        self.assertEqual(
            test_group_user_document_b_name,
            [{"id": document_b.id, "name": "document B"}],
    ...
    def test_documents_arbitrary_read_write(self):
        ...
        document_c.with_user(self.document_manager).write(
            {"name": "nameChangedManager"}
        )
        self.assertEqual(
            document_c.name,
            "nameChangedManager",
    ...
    def test_documents_access(self):
        ...
        with self.assertRaises(AccessError):
            document_not_owner.with_user(self.basic_user).write(
                {"name": "nameChangedA"}
            )
        with self.assertRaises(AccessError):
            document_not_owner.with_user(self.document_user).write(
                {"name": "nameChangedA"}
            )
        ...
        self.assertEqual(
            name_from_read_owner,
            document_read_owner.name,
            "test_group2_user should be able to read his own document",
        )

        document_owner.with_user(self.test_group_user).write(
            {"name": "nameChangedOwner"}
        )
        self.assertEqual(
            document_owner.name,
            "nameChangedOwner",
        ...
        document_from_user = (
            ...
            .with_user(self.test_group_user)
            .browse(document_not_owner.id)
        )
        self.assertEqual(
            document_from_user.name,
            "document not owner",
            "test_group_user should be able to read document_not_owner as he is in the write group",
        )
        document_not_owner.with_user(self.test_group_user).write(
            {"name": "nameChangedA"}
        )
        self.assertEqual(
            document_not_owner.name,
            "nameChangedA",
        ...
        with self.assertRaises(AccessError):
            document_read_owner.with_user(test_group2_user).write(
                {"name": "nameChange"}
            )
        # the first user is in the write groups but is not the owner of the document
        with self.assertRaises(AccessError):
            document_read_owner.with_user(self.test_group_user).write(
                {"name": "nameChange"}
            )
    ...
    def test_share_link_access(self):
        ...
        available_documents = test_share._get_documents_and_check_access(
            test_share.access_token, operation="read"
        )
        self.assertEqual(
    ...
    def test_share_link_dynamic_access(self):
        ...
        folder_share = self.env["documents.folder"].create(
            {
                "name": "folder share",
                "read_group_ids": [
                    (6, 0, [self.ref("documents.group_documents_user")])
                ],
            }
        )
        document_a = self.env["documents.document"].create(
            ...
            .create(
                {
                    ...
                    "type": "ids",
                    "document_ids": [
                        (6, 0, [document_a.id, document_b.id, document_c.id])
                    ],
                }
            )
        )
        ...
        self.assertEqual(
            len(available_documents), 3, "there should be 3 available documents"
        )
        ...
        self.assertEqual(
            len(available_documents), 1, "there should be 1 available document"
        )
        self.assertEqual(
            available_documents.name, "filec.gif", "the document C should be available"
        )
