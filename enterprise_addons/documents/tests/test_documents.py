...
class TestCaseDocuments(TransactionCase):
        ...
        self.worflow_rule = self.env["documents.workflow.rule"].create(
            {
                ...
                "activity_type_id": self.env.ref(
                    "documents.mail_documents_activity_data_Inbox"
                ).id,
    ...
    def test_documents_rules(self):
        ...
        self.assertTrue(
            self.tag_b.id in self.document_gif.tag_ids.ids,
            "failed at workflow rule add tag id",
        )
        self.assertTrue(
            self.tag_b.id in self.document_txt.tag_ids.ids,
            "failed at workflow rule add tag id txt",
        )
        self.assertEqual(
            len(self.document_gif.tag_ids.ids), 1, "failed at workflow rule add tag len"
        )
        ...
        self.assertTrue(
            activity_gif.exists(), "failed at workflow rule activity exists"
        )
        self.assertEqual(
            activity_gif.summary,
            "test workflow rule activity summary",
            "failed at activity data summary from workflow create activity",
        )
        self.assertEqual(
            activity_gif.note,
            "<p>activity test note</p>",
            "failed at activity data note from workflow create activity",
        )
        self.assertEqual(
            activity_gif.activity_type_id.id,
            self.env.ref("documents.mail_documents_activity_data_Inbox").id,
            "failed at activity data note from workflow create activity",
        )

        self.assertEqual(
            self.document_gif.folder_id.id,
            self.folder_b.id,
            "failed at workflow rule set folder gif",
        )
        self.assertEqual(
            self.document_txt.folder_id.id,
            self.folder_b.id,
            "failed at workflow rule set folder txt",
        )
    ...
    def test_documents_rules_link_to_record(self):
        ...
        user_admin_doc = new_test_user(
            self.env,
            login="Test admin documents",
            groups="documents.group_documents_manager,base.group_partner_manager",
        )
        ...
        record = {
            "res_model": res_model,
            "res_model_id": self.env["ir.model"].name_search(
                res_model, operator="=", limit=1
            )[0],
            "res_id": self.env[res_model].search([], limit=1).id,
        }
        link_to_record_ctx = workflow_rule_link.apply_actions(
            [doc.id for doc in documents_to_link]
        )["context"]
        link_to_record_wizard = (
            self.env["documents.link_to_record_wizard"]
            .with_user(user_admin_doc)
            .with_context(link_to_record_ctx)
            .create({})
        )
        # Link record to document_gif and document_txt
        link_to_record_wizard.model_id = record["res_model_id"]
        link_to_record_wizard.resource_ref = "%s,%s" % (
            record["res_model"],
            record["res_id"],
        )
        ...
        for doc in documents_to_link:
            self.assertEqual(
                doc.res_model, record["res_model"], "bad model linked to the document"
            )
            self.assertEqual(
                doc.res_id, record["res_id"], "bad record linked to the document"
            )

        # Removes the link between document_gif and record
        workflow_rule_link.unlink_record([self.document_gif.id])
        self.assertNotEqual(
            self.document_gif.res_model,
            record["res_model"],
            "the link between document_gif and its record was not correctly removed",
        )
        self.assertNotEqual(
            self.document_gif.res_id,
            record["res_id"],
            "the link between document_gif and its record was not correctly removed",
        )
    ...
    def test_documents_rule_display(self):
        ...
        self.assertFalse(
            self.workflow_rule_criteria.limited_to_single_record,
        ...
        self.assertTrue(
            self.workflow_rule_criteria.id
            not in self.document_txt_criteria_a.available_rule_ids.ids,
        ...
        self.assertTrue(
            self.workflow_rule_criteria.id
            not in self.document_txt_criteria_b.available_rule_ids.ids,
            "failed at documents_workflow_rule unavailable rule",
        )
        ...
        self.assertTrue(
            self.workflow_rule_criteria.id
            in self.document_txt_criteria_c.available_rule_ids.ids,
        ...
        self.assertTrue(
            self.workflow_rule_criteria.id
            not in self.document_txt_criteria_d.available_rule_ids.ids,
            "failed at documents_workflow_rule unavailable rule",
        )
    ...
    def test_documents_share_links(self):
        ...
        self.assertEqual(
            self.documents_share_links_a.type,
            "domain",
            "failed at share link type domain",
        )
        ...
        self.assertTrue(
            self.share_folder_with_upload.exists(), "failed at upload folder creation"
        )
        self.assertEqual(
            self.share_folder_with_upload.activity_type_id.name,
            "To validate",
            "failed at activity type for upload documents",
        )
        self.assertEqual(
            self.share_folder_with_upload.state, "live", "failed at share_link live"
        )
        ...
        # Expiration date
        self.assertEqual(
            self.result_share_documents_act.state,
            "expired",
            "failed at share_link expired",
        )
    ...
    def test_documents_share_popup(self):
        ...
        self.assertEqual(
            action_context["default_owner_id"],
            self.env.uid,
            "the action should open a view with the current user as default owner",
        )
        self.assertEqual(
            action_context["default_folder_id"],
            share_folder.id,
            "the action should open a view with the right default folder",
        )
        self.assertEqual(
            action_context["default_tag_ids"],
            [[6, 0, [share_tag.id]]],
            "the action should open a view with the right default tags",
        )
        self.assertEqual(
            action_context["default_type"],
            "domain",
            "the action should open a view with the right default type",
        )
        self.assertEqual(
            action_context["default_domain"],
            domain,
            "the action should open a view with the right default domain",
        )
    ...
    def test_recurring_document_request(self):
        ...
        self.assertFalse(
            activity.exists(), "the activity should be removed after file upload"
        )
        ...
        document_2 = self.env["documents.document"].search(
            [("request_activity_id", "=", activity_2.id), ("type", "=", "empty")]
        )
    ...
    def test_versioning(self):
        ...
        self.assertEqual(
            len(document.previous_attachment_ids.ids),
            1,
            "There should be 1 attachment in history",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(document.previous_attachment_ids.ids),
            1,
            "there should still be 1 attachment in history",
        )
        self.assertEqual(
            document.attachment_id.id,
            old_attachment.id,
            "the history should contain the old attachment",
        )
        self.assertEqual(
            document.previous_attachment_ids[0].id,
            new_attachment.id,
    ...
    def test_create_from_message(self):
        ...
        partner = self.env["res.partner"].create({"name": "Luke Skywalker"})
        ...
        message._message_post_after_hook({}, {"attachment_ids": [(4, attachment.id)]})
        self.assertEqual(
            message.active, False, "Document created for the message should be inactive"
        )
        ...
        self.assertEqual(
            attachment_document.owner_id.id,
            self.doc_user.id,
            "Should assign owner from share",
        )
        self.assertEqual(
            attachment_document.partner_id.id,
            partner.id,
            "Should assign partner from share",
        )
        self.assertEqual(
            attachment_document.tag_ids.ids,
            [self.tag_b.id],
            "Should assign tags from share",
        )
