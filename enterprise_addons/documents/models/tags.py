...
class TagsCategories(models.Model):
    ...
    folder_id = fields.Many2one(
        "documents.folder", string="Workspace", ondelete="cascade"
    )
    name = fields.Char(required=True, translate=True)
    tag_ids = fields.One2many("documents.tag", "facet_id")
    tooltip = fields.Char(
        help="Text shown when hovering on this tag category or its tags",
        string="Tooltip",
    )
    ...
    _sql_constraints = [
        (
            ...
            "Facet already exists in this folder",
        ),
    ]
...
class Tags(models.Model):
    ...
    folder_id = fields.Many2one(
        "documents.folder",
        string="Workspace",
        related="facet_id.folder_id",
        store=True,
        readonly=False,
    )
    facet_id = fields.Many2one(
        "documents.facet", string="Category", ondelete="cascade", required=True
    )
    name = fields.Char(required=True, translate=True)
    sequence = fields.Integer("Sequence", default=10)
    ...
    _sql_constraints = [
        (
            "facet_name_unique",
            "unique (facet_id, name)",
            "Tag already exists for this facet",
        ),
    ]
    ...
    def _get_tags(self, domain, folder_id):
        ...
        params = [
            ...
            list(
                documents.ids
            ),  # using Postgresql's ANY() with a list to prevent empty list of documents
        ]
