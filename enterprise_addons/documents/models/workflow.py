...
class WorkflowActionRule(models.Model):
    ...
    domain_folder_id = fields.Many2one(
        "documents.folder",
        string="Related Workspace",
        required=True,
        ondelete="cascade",
    )
    ...
    required_tag_ids = fields.Many2many(
        "documents.tag", "required_tag_ids_rule_table", string="Required Tags"
    )
    excluded_tag_ids = fields.Many2many(
        "documents.tag", "excluded_tag_ids_rule_table", string="Excluded Tags"
    )
    limited_to_single_record = fields.Boolean(
        string="One record limit", compute="_compute_limited_to_single_record"
    )

    # Actions
    partner_id = fields.Many2one("res.partner", string="Set Contact")
    user_id = fields.Many2one("res.users", string="Set Owner")
    tag_action_ids = fields.One2many(
        "documents.workflow.action", "workflow_rule_id", string="Set Tags"
    )
    folder_id = fields.Many2one("documents.folder", string="Move to Workspace")
    create_model = fields.Selection(
        [("link.to.record", "Link to record")], string="Create"
    )
    link_model = fields.Many2one(
        "ir.model",
        string="Specific Model Linked",
    ...
    def link_to_record(self, documents=None):
        ...
        documents_link_record = [
            d for d in documents if (d.res_model != "documents.document")
        ]
    ...
    def apply_actions(self, document_ids):
        ...
        if self.create_model:
            return self.with_company(documents.company_id).create_record(
                documents=documents
            )
...
class WorkflowTagAction(models.Model):
    ...
    def execute_tag_action(self, document):
        if self.action == "add" and self.tag_id.id:
            ...
            faceted_tags = self.env["documents.tag"].search(
                [("facet_id", "=", self.facet_id.id)]
            )
            if faceted_tags.ids:
