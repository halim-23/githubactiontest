...
class Document(models.Model):
    ...
    attachment_id = fields.Many2one(
        "ir.attachment", ondelete="cascade", auto_join=True, copy=False
    )
    attachment_name = fields.Char(
        "Attachment Name", related="attachment_id.name", readonly=False
    )
    attachment_type = fields.Selection(
        string="Attachment Type", related="attachment_id.type", readonly=False
    )
    is_editable_attachment = fields.Boolean(
        default=False, help="True if we can edit the link attachment."
    )
    datas = fields.Binary(
        related="attachment_id.datas", related_sudo=True, readonly=False
    )
    ...
    description = fields.Text(
        "Attachment Description", related="attachment_id.description", readonly=False
    )
    ...
    # Document
    name = fields.Char(
        "Name", copy=True, store=True, compute="_compute_name", inverse="_inverse_name"
    )
    active = fields.Boolean(default=True, string="Active")
    thumbnail = fields.Binary(
        readonly=1, store=True, attachment=True, compute="_compute_thumbnail"
    )
    ...
    type = fields.Selection(
        [("url", "URL"), ("binary", "File"), ("empty", "Request")],
        string="Type",
        required=True,
        store=True,
        default="empty",
    ...
    create_share_id = fields.Many2one(
        "documents.share", help="Share used to create this document"
    )
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", related="folder_id.company_id", readonly=True
    )
    ...
    def _onchange_url(self):
        if self.url:
            is_youtube = re.match(
                r"^(https?\:\/\/)?(www\.)?(youtube\.com|youtu\.?be)\/.+$", self.url
            )
    ...
    def _compute_thumbnail(self):
        for record in self:
            try:
                record.thumbnail = image_process(
                    record.datas, size=(80, 80), crop="center"
                )
            except UserError:
    ...
    def _get_models(self, domain):
        ...
        for group in groups:
            ...
            if not res_model:
                ...
                models.append(
                    {
                        "id": res_model,
                        "display_name": self.env["ir.model"]
                        ._get(res_model)
                        .display_name,
                        "__count": group["res_model_count"],
                    }
        ...
        return (
            sorted(models, key=lambda m: m["display_name"]) + not_attached + not_a_file
        )
    ...
    def _compute_available_rules(self):
        ...
        rule_domain = (
            [("domain_folder_id", "parent_of", folder_ids)] if folder_ids else []
        )
            ...
            if rule.condition_type == "domain":
                ...
                if rule.criteria_partner_id:
                    domain = expression.AND(
                        [[["partner_id", "=", rule.criteria_partner_id.id]], domain]
                    )
                if rule.criteria_owner_id:
                    domain = expression.AND(
                        [[["owner_id", "=", rule.criteria_owner_id.id]], domain]
                    )
                if rule.create_model:
                    domain = expression.AND([[["type", "=", "binary"]], domain])
                if rule.required_tag_ids:
                    domain = expression.AND(
                        [[["tag_ids", "in", rule.required_tag_ids.ids]], domain]
                    )
                if rule.excluded_tag_ids:
                    domain = expression.AND(
                        [[["tag_ids", "not in", rule.excluded_tag_ids.ids]], domain]
                    )
    ...
    def documents_set_activity(self, settings_record=None):
        ...
        if settings_record and settings_record.activity_type_id:
            for record in self:
                ...
                if (
                    settings_record._fields.get("has_owner_activity")
                    and settings_record.has_owner_activity
                    and record.owner_id
                ):
                    user = record.owner_id
                elif (
                    settings_record._fields.get("activity_user_id")
                    and settings_record.activity_user_id
                ):
        ...
        self.sudo().write(
            {
                "favorited_ids": [
                    (
                        3 if self.env.user in self[0].favorited_ids else 4,
                        self.env.user.id,
                    )
                ]
            }
    ...
    def toggle_lock(self):
        ...
        if self.lock_uid:
            if (
                self.env.user == self.lock_uid
                or self.env.is_admin()
                or self.user_has_groups("documents.group_document_manager")
            ):
                self.lock_uid = False
        else:
    ...
    def create(self, vals):
        keys = [
            key
            for key in vals
            if self._fields[key].related
            and self._fields[key].related.split(".")[0] == "attachment_id"
        ]
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if record.type == "empty" and ("datas" in vals or "url" in vals):
                body = _("Document Request: %s Uploaded by: %s") % (
                    record.name,
                    self.env.user.name,
                )
            ...
            if record.attachment_id:
                ...
                if (
                    res_model
                    and res_model != "documents.document"
                    and not self.env[res_model].browse(res_id).exists()
                ):
        ...
        attachment_dict = {
            key: vals.pop(key) for key in ["datas", "mimetype"] if key in vals
        }
    ...
    def _process_activities(self, attachment_id):
        ...
        if attachment_id and self.request_activity_id:
            feedback = _("Document Request: %s Uploaded by: %s") % (
                self.name,
                self.env.user.name,
            )
            self.request_activity_id.action_feedback(
                feedback=feedback, attachment_ids=[attachment_id]
            )
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name == "folder_id":
            ...
            folder_domain = expression.OR(
                [
                    [("parent_folder_id", "parent_of", available_folders.ids)],
                    [("id", "in", available_folders.ids)],
                ]
            )
            ...
            if enable_counters:
                ...
                domain_image = self._search_panel_domain_image(
                    field_name, model_domain, enable_counters
                )
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        if field_name == "tag_ids":
            ...
            if filter_domain:
                ...
                model_count = {
                    model["id"]: model["__count"] for model in self._get_models(domain)
                }
        ...
        return super(Document, self).search_panel_select_multi_range(
            field_name, **kwargs
        )
