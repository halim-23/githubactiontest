...
class MailActivity(models.Model):
        ...
        if attachment_ids:
            for record in self:
                document = self.env["documents.document"].search(
                    [("request_activity_id", "=", record.id)], limit=1
                )
    ...
    def create(self, vals_list):
        ...
        for activity in activities:
            ...
            if (
                activity_type.category == "upload_file"
                and activity.res_model != "documents.document"
                and activity_type.folder_id
            ):
                doc_vals.append(
                    {
                ...
                existing_doc_req = self.env["documents.document"].browse(
                    activity.res_id
                )
