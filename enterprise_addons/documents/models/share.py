...
class DocumentShare(models.Model):
    ...
    def _get_documents_and_check_access(
        self, access_token, document_ids=None, operation="write"
    ):
        """
    ...
    def _compute_can_upload(self):
        for record in self:
            ...
            in_write_group = set(folder.group_ids.ids) & set(
                record.create_uid.groups_id.ids
            )
    ...
    def _compute_full_url(self):
        for record in self:
            record.full_url = "%s/document/share/%s/%s" % (
                record.get_base_url(),
                record.id,
                record.access_token,
            )
