...
class DocumentFolder(models.Model):
    ...
    _sql_constraints = [
        (
            "check_user_specific",
            "CHECK(not ((NOT user_specific OR user_specific IS NULL) and user_specific_write))",
    ...
    def default_get(self, fields):
        ...
        if (
            "parent_folder_id" in fields
            and self._context.get("folder_id")
            and not res.get("parent_folder_id")
        ):
    ...
    def name_get(self):
        ...
        for record in self:
            if hierarchical_naming and record.parent_folder_id:
                name_array.append(
                    (record.id, "%s / %s" % (record.parent_folder_id.name, record.name))
                )
            else:
    ...
    children_folder_ids = fields.One2many(
        "documents.folder", "parent_folder_id", string="Sub workspaces"
    )
    document_ids = fields.One2many(
        "documents.document", "folder_id", string="Documents"
    )
    sequence = fields.Integer("Sequence", default=10)
    share_link_ids = fields.One2many(
        "documents.share", "folder_id", string="Share Links"
    )
    facet_ids = fields.One2many(
        "documents.facet",
        "folder_id",
    ...
    group_ids = fields.Many2many(
        "res.groups",
        string="Write Groups",
        help="Groups able to see the workspace and read/create/edit its documents.",
    )
    read_group_ids = fields.Many2many(
        "res.groups",
        "documents_folder_read_groups",
        string="Read Groups",
        help="Groups able to see the workspace and read its documents without create/edit rights.",
    )
    ...
    user_specific_write = fields.Boolean(
        string="Own Documents Only (Write)",
        compute="_compute_user_specific_write",
        store=True,
        readonly=False,
    ...
    def _compute_action_count(self):
        ...
        action_count_dict = {
            d["domain_folder_id"][0]: d["domain_folder_id_count"]
            for d in read_group_var
        }
        for record in self:
    ...
    def _compute_document_count(self):
        read_group_var = self.env["documents.document"].read_group(
            [("folder_id", "in", self.ids)], fields=["folder_id"], groupby=["folder_id"]
        )

        document_count_dict = {
            d["folder_id"][0]: d["folder_id_count"] for d in read_group_var
        }
        for record in self:
