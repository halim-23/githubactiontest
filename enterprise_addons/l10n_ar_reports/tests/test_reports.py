...
class TestReports(TestAr, TestAccountReportsCommon):
    def _create_test_credit_notes_like_demo(self):
        ...
        for key, values in credit_notes.items():
            move_reversal = refund_wizard.with_context(
                active_model="account.move", active_ids=values.pop("move_ids").ids
            ).create(
                {
    ...
    def _create_test_vendor_bill_invoice_demo(self):
        ...
        vendor_bills = {
            ...
            "demo_despacho_1": {
                ...
                "invoice_line_ids": [
                    {
                        "product_id": self.service_wo_tax,
                    ...
                    {
                        "product_id": self.service_iva_no_gravado,
        ...
        for key, values in vendor_bills.items():
            with Form(
                ...
                invoice_form.l10n_latam_document_number = values[
    ...
    def setUpClass(cls, chart_template_ref="l10n_ar.l10nar_ri_chart_template"):
        ...
        cls.journal = cls._create_journal(
            cls, "preprinted", data={"l10n_ar_afip_pos_number": 37928}
        )
        cls.maxDiff = None
        company_ids = cls.company_ri.ids
        cls.report = cls.env["l10n_ar.vat.book"].with_context(
    ...
    def _test_txt_file(self, filename):
        ...
        out_txt = self.report._get_txt_files(self.options)[filetype].decode(
            "ISO-8859-1"
        )
        res_file = (
            file_open("l10n_ar_reports/tests/" + filename, "rb")
            .read()
            .decode("ISO-8859-1")
        )
