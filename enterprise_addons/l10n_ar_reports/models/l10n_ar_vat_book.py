...
class L10nARVatBook(models.AbstractModel):
    ...
    def print_pdf(self, options):
        options.update({"journal_type": self.env.context.get("journal_type")})
    ...
    def _get_dynamic_columns(self, options):
        ...
        if self.env["account.tax"].search(
            [
                ("type_tax_use", "=", options.get("journal_type")),
                ("tax_group_id.l10n_ar_vat_afip_code", "=", "9"),
            ]
        ):
        ...
        if self.env["account.tax"].search(
            [
                ("type_tax_use", "=", options.get("journal_type")),
                ("tax_group_id.l10n_ar_vat_afip_code", "=", "8"),
            ]
        ):
    ...
    def _get_columns_name(self, options):
        ...
        dynamic_columns = [
            item.get("name") for item in self._get_dynamic_columns(options)
        ]
        return (
            [
    ...
    def _get_lines(self, options, line_id=None):
        ...
        dynamic_columns = [
            item.get("sql_var") for item in self._get_dynamic_columns(options)
        ]
        totals = {}.fromkeys(
            ["taxed", "not_taxed"]
            + dynamic_columns
            + ["vat_10", "vat_21", "vat_27", "vat_per", "other_taxes", "total"],
            0,
        )
            ...
            lines.append(
                {
                    ...
                    "class": "date"
                    + (" text-muted" if rec["state"] != "posted" else ""),
                    "level": 2,
                    ...
                    + [
                        {"name": self.format_value(sign * rec[item])}
                        for item in dynamic_columns
                    ]
                    + [
        ...
        lines.append(
            {
                ...
                + [
                    {"name": self.format_value(sign * totals[item])}
                    for item in dynamic_columns
                ]
                + [
    ...
    def get_report_filename(self, options):
        ...
        journal_type = options.get("journal_type")
        filename = {"sale": "Libro_IVA_Ventas", "purchase": "Libro_IVA_Compras"}.get(
            journal_type, "Libro_IVA"
        )
    ...
    def _get_txt_files(self, options):
        ...
        vouchers_data = "\r\n".join(
            self._get_REGINFO_CV_CBTE(options, aliquots)
        ).encode("ISO-8859-1")
        return vouchers_data, aliquots_data

    def get_zip(self, options):
        txt_types = (
            ["purchases", "goods_import", "used_goods"]
        ...
        with zipfile.ZipFile(stream, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for txt_type in txt_types:
                ...
                if aliquots_data:
                    zf.writestr(
                        filenames.get("%s_aliquots" % txt_type) + ".txt", aliquots_data
                    )
    ...
    def _get_lines_domain(self, options):
        ...
        domain = [
            ("journal_id.type", "=", options.get("journal_type")),
    ...
    def _get_partner_document_code_and_number(self, partner):
        ...
        if partner.l10n_ar_afip_responsibility_type_id.code == "5" or (
            partner.l10n_ar_afip_responsibility_type_id.code == "10"
            and not partner.commercial_partner_id.is_company
        ):
            ...
            doc_number = partner.l10n_ar_vat or (
                commercial_partner.country_id.l10n_ar_legal_entity_vat
    ...
    def _get_txt_invoices(self, options):
        ...
        ] + self._get_lines_domain(options)
        txt_type = options.get("txt_type")
        if txt_type == "purchases":
            domain += [
                ("l10n_latam_document_type_id.code", "not in", ["66", "30", "32"])
            ]
    ...
    def _get_tax_row(self, invoice, base, code, tax_amount, options):
        ...
        if journal_type == "sale":
            row = [
                "{:0>3d}".format(
                    int(inv.l10n_latam_document_type_id.code)
                ),  # Field 1: Tipo de Comprobante
            ...
            row = [
                (inv.l10n_latam_document_number or inv.name or "").rjust(
                    16, "0"
                ),  # Field 1: Despacho de importación.
    ...
    def _get_REGINFO_CV_CBTE(self, options, aliquots):
        ...
        for inv in invoices:
            ...
            if vat_exempt_base_amount:
                if (
                    inv.partner_id.l10n_ar_afip_responsibility_type_id.code == "10"
                ):  # free zone operation
                    operation_code = "Z"
                elif (
                    inv.l10n_latam_document_type_id.l10n_ar_letter == "E"
            ...
            row += [
                ...
                inv.commercial_partner_id.name.ljust(30, " ")[
                    :30
                ],  # Field 8: Apellido y Nombre del comprador.
                self._format_amount(
                    inv.amount_total
                ),  # Field 9: Importe Total de la Operación.
                self._format_amount(
                    vat_untaxed_base_amount
                ),  # Field 10: Importe total de conceptos que no integran el precio neto gravado
            ]
            ...
            if journal_type == "sale":
                row += [
                    ...
                    self._format_amount(
                        vat_exempt_base_amount
                    ),  # Field 12: Importe de operaciones exentas
                    self._format_amount(
                        perc_imp_nacionales_amount + vat_perc_amount
                    ),  # Field 13: Importe de percepciones o pagos a cuenta de impuestos Nacionales
                ]
            else:
                row += [
                    self._format_amount(
                        vat_exempt_base_amount
                    ),  # Field 11: Importe de operaciones exentas
                    self._format_amount(
                        vat_perc_amount
                    ),  # Field 12: Importe de percepciones o pagos a cuenta del Impuesto al Valor Agregado
                    self._format_amount(
                        perc_imp_nacionales_amount
                    ),  # Field 13: Importe de percepciones o pagos a cuenta otros impuestos nacionales
                ]

            row += [
                self._format_amount(
                    iibb_perc_amount
                ),  # Field 14: Importe de percepciones de ingresos brutos
                self._format_amount(
                    mun_perc_amount
                ),  # Field 15: Importe de percepciones de impuestos municipales
                self._format_amount(
                    intern_tax_amount
                ),  # Field 16: Importe de impuestos internos
                str(currency_code),  # Field 17: Código de Moneda
                self._format_amount(
                    currency_rate, padding=10, decimals=6
                ),  # Field 18: Tipo de Cambio
            ...
            if journal_type == "sale":
                ...
                row += [
                    ...
                    inv.l10n_latam_document_type_id.code in document_codes
                    and "00000000"
                ...
                row.append(
                    self._format_amount(
                        0.0
                        if inv.company_id.l10n_ar_computable_tax_credit == "global"
                        else vat_amount
                    )
                )  # Field 21: Crédito Fiscal Computable
                ...
                row += [
                    ...
                    (liquido_type and inv.company_id.name or "").ljust(30, " ")[
                        :30
                    ],  # Field 24: Denominación Emisor / Corredor
