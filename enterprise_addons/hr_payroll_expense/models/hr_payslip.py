...
class HrPayslip(models.Model):
    ...
    def _compute_expenses_count(self):
        for payslip in self:
            payslip.expenses_count = len(
                payslip.mapped("expense_sheet_ids.expense_line_ids")
            )
    ...
    def create(self, vals_list):
        ...
        for slip in draft_slips:
            payslip_sheets = sheets.filtered(
                lambda s: s.employee_id == slip.employee_id
            )
            slip.expense_sheet_ids = [(5, 0, 0)] + [
                (4, s.id, False) for s in payslip_sheets
            ]
        return payslips
    ...
    def _compute_expense_input_line_ids(self):
        expense_type = self.env.ref(
            "hr_payroll_expense.expense_other_input", raise_if_not_found=False
        )
        for payslip in self:
            ...
            input_lines_vals.append(
                (0, 0, {"amount": total, "input_type_id": expense_type.id})
            )
    ...
    def _update_expense_sheets(self):
        ...
        for payslip in self:
            if not payslip.input_line_ids.filtered(
                lambda line: line.input_type_id == expense_type
            ):
    ...
    def open_expenses(self):
        ...
        return {
            ...
            "view_mode": "tree,form",
            "domain": [
                ("id", "in", self.mapped("expense_sheet_ids.expense_line_ids").ids)
            ],
        }
