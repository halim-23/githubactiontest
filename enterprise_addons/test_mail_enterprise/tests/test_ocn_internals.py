...
class TestPushNotification(MailCommon):
    ...
    def test_push_notifications(self, jsonrpc):
        ...
        self.assertEqual(
            jsonrpc.call_args[1]["params"]["data"]["model"], "mail.test.simple"
        )
        self.assertEqual(
            jsonrpc.call_args[1]["params"]["data"]["res_id"], self.record_simple.id
        )
        self.assertEqual(
            jsonrpc.call_args[1]["params"]["data"]["author_name"], self.user_email.name
        )
        self.assertIn(
        ...
        fake_email = self.env["mail.message"].create(
            {
                ...
                % (test_record.id, socket.gethostname()),
            }
        )
