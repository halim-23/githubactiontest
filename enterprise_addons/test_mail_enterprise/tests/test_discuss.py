...
class TestMailMobile(test_mail_common.TestMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.original_domain = (
            cls.env["ir.config_parameter"].sudo().get_param("web.base.url")
        )
        cls.env["ir.config_parameter"].sudo().set_param(
            "web.base.url", "http://yourcompany.odoo.com"
        )

    @classmethod
    def tearDownClass(cls):
        cls.env["ir.config_parameter"].sudo().set_param(
            "web.base.url", cls.original_domain
        )
        super(TestMailMobile, cls).tearDownClass()

    def test_override_url_in_mail(self):
        url = self.env["mail.thread"]._notify_get_action_link(
            "view", model="mail.activity", res_id=1
        )
        original_expected_link = (
            "http://yourcompany.odoo.com/mail/view?model=mail.activity&res_id=1"
        )
    ...
    def test_blacklist_not_override_url_in_mail(self):
        url = self.env["mail.thread"]._notify_get_action_link(
            "view", model="mail.activity", res_id=1, access_token="test"
        )
