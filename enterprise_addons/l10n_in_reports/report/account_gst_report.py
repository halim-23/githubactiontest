...
class L10nInReportAccount(models.AbstractModel):
                ...
                columns_name.append(
                    {
                        ...
                        "class": model_field.get("class", "")
                        + " o_account_reports_level0",
                    }
                )
            return columns_name
        return [
            {"name": _("Section"), "class": "o_account_reports_level0"},
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if gst_section:
            model_domain = self.get_gst_section_model_domain(
                gst_return_type, gst_section
            )
            fields_values = (
                self.env[model_domain.get("model")]
                .search(filter_domain + model_domain.get("domain"))
                .union()
                .read()
            )
            lines += self.set_gst_section_lines(
                gst_return_type, gst_section, fields_values
            )
        else:
            for gst_section, gst_section_name in self.get_gst_section(
                gst_return_type
            ).items():
                ...
                for read_data in (
                    ...
                    .search(domain)
                    .union()
                    .read(model_domain.get("sum_fields"))
                ):
                ...
                lines.append(
                    {
                        ...
                        "colspan": 0,
                        "columns": [
                            {"name": v.get("value"), "class": v.get("class", "")}
                            for v in columns
                        ],
                        "action_id": self.env.ref(
                            "l10n_in_reports.action_l10n_in_report_account"
                        ).id,
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        return super(L10nInReportAccount, self.with_context(options or {})).get_html(
    ...
    def set_gst_section_lines(self, gst_return_type, gst_section, fields_values):
        ...
        if gst_return_type == "gstr1":
            gst_section_fields = self.get_gst_section_fields(
                gst_return_type, gst_section
            )
                    ...
                    gst_section_lines.append(
                        {
                            ...
                            "columns": self.set_columns(
                                gst_section_fields, fields_value
                            ),
                            "style": "font-weight: normal;",
                        }
                    )
    ...
    def set_columns(self, gst_section_fields, fields_value):
        ...
        for section_field in gst_section_fields:
            ...
            if isinstance(field_value, bool) and not field_value:
                field_value = ""
            columns.append(
                {"name": field_value, "class": section_field.get("class", "")}
            )
        return columns
    ...
    def group_report_lines(self, group_fields, fields_values, fields):
        ...
        fields_values = sorted(
            fields_values, key=lambda s: [s.get(g, "") for g in group_fields]
        )
        for _, grouped_values in groupby(
            fields_values, lambda x: [x.get(g, "") for g in group_fields]
        ):
            vals = {}
            first_grouped_value = {}
    ...
    def view_sub_type(self, options, params):
        ...
        [action_read] = (
            ...
            .browse(int(params.get("actionId")))
            .sudo()
            .read()
        )
            ...
            if gst_section == "b2b":
                ...
                domain += [
                    (
                        ...
                        "in",
                        ["overseas", "special_economic_zone"],
                    ),
                    ("move_type", "not in", ("out_refund", "in_refund")),
                ]
            elif gst_section == "at":
                model = "l10n_in.advances.payment.report"
                domain = [("amount", ">", 0), ("payment_type", "=", "inbound")]
        ...
        if gst_return_type == "gstr1":
            if gst_section == "b2b":
                ...
                section_fields = [
                    ...
                    {
                        "name": "gst_format_refund_date",
                        "label": "Invoice/Advance Receipt date",
                        "class": "print_only",
                    },
                    {
                        "name": "refund_invoice_type",
                        "label": "Document Type",
                        "class": "print_only",
                    },
                    {"name": "place_of_supply", "label": "Place Of Supply"},
                    ...
                    {
                        "name": "total",
                        "label": "Note/Refund Voucher Value",
                        "class": "number",
                    },
                    {
                        "name": "price_total",
                ...
                section_fields = [
                    ...
                    {
                        "name": "gst_format_shipping_bill_date",
    ...
    def get_csv(self, options):
        ...
        for row in self.get_header(options):
            ...
            for columns in row:
                if not header:
                    header += "%s" % columns.get("name")
                else:
                    header += ",%s" % columns.get("name")
            headers.append(header)
        lines = headers
        for line in self.with_context(
            self._set_context(options), print_mode=True
        )._get_lines(options):
            csv_line = ""
            for columns in line.get("columns"):
                if not csv_line:
                    csv_line += '"%s"' % columns.get("name")
                else:
                    csv_line += ',"%s"' % columns.get("name")
            if csv_line:
                lines.append('"%s",' % line.get("name") + csv_line)
            else:
                lines.append('"%s"' % line.get("name"))
