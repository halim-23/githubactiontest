...
class TestReports(TestAccountReportsCommon):
    ...
    def test_gstr1_b2b_summary(self):
        ...
        self.assertListEqual(
            expected,
            lines[0]["columns"],
            "Wrong values for Indian GSTR-1 B2B summary report.",
        )
    ...
    def test_gstr1_b2b_detailed_report(self):
        ...
        expected = [
            {
                "id": min(self.invoice.line_ids.filtered(lambda l: l.tax_line_id).ids),
        ...
        self.assertListEqual(
            expected, lines, "Wrong values for Indian GSTR-1 B2B detailed report."
        )
