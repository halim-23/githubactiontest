odoo.define("delivery_iot.iot_widgets", function (require) {
  ...
  var FieldMany2OneIoTScale = FieldMany2One.extend(iot_widgets.IoTValueFieldMixin, {
    ...
    _reset: function () {
      ...
      if (
        !this.iot_device ||
        this.iot_device._identifier != this.recordData.iot_device_identifier
      ) {
        this._stopListening();
        this._getDeviceInfo();
    ...
    _getDeviceInfo: function () {
      ...
      this.manual_measurement = this.recordData[
        this.attrs.options.manual_measurement_field
      ];
    },
    ...
    _onNotification: function (notifications) {
      for (const {payload, type} of notifications) {
        if (type === "iot_print_documents" && this.call("bus_service", "isMasterTab")) {
          this._printDocuments(
            payload.iot_device_identifier,
            payload.iot_ip,
            payload.documents
          );
        }
  ...
  core.serviceRegistry.add(
    "delivery_iot_notification_service",
    DeliveryIoTNotificationManager
  );
