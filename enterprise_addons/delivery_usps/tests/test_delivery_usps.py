...
class TestDeliveryUSPS(TransactionCase):
    def setUp(self):
        ...
        self.montreal = self.env["res.partner"].create(
            {
                "name": "Vieux-Port de Montreal",
    ...
    def test_01_usps_basic_us_domestic_flow(self):
            ...
            self.env["choose.delivery.carrier"].with_context(
                {
                    ...
                    "default_carrier_id": self.env.ref(
                        "delivery_usps.delivery_carrier_usps_domestic"
                    ).id,
                }
            )
        )
        ...
        self.assertGreater(
            ...
            0.0,
            "USPS delivery cost for this SO has not been correctly estimated.",
        )
        ...
        self.assertIsNot(
            ...
            False,
            "USPS did not return any tracking number",
        )
        self.assertGreater(
            picking.carrier_price, 0.0, "USPS carrying price is probably incorrect"
        )
            ...
            self.env["choose.delivery.carrier"].with_context(
                {
                    "default_order_id": sale_order.id,
                    "default_carrier_id": self.env.ref(
                        "delivery_usps.delivery_carrier_usps_international"
                    ).id,
                }
            )
        )
    ...
    def test_03_usps_ship_to_canada_flow(self):
        ...
        so_vals = {"partner_id": self.montreal.id, "order_line": [(0, None, sol_vals)]}
        ...
        sale_order.action_confirm()
        self.assertEqual(
            len(sale_order.picking_ids), 1, "The Sale Order did not generate a picking."
        )
