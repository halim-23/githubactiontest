...
class USPSRequest:
    ...
    def check_required_value(
        ...
        if shipper.country_id.code != "US":
            return _(
                "Please set country U.S.A in your company address, Service is only available for U.S.A"
            )
        ...
        if not self._convert_phone_number(shipper.phone):
            return _(
                "Company phone number is invalid. Please insert a US phone number."
            )
        ...
        if recipient.country_id.code == "US" and delivery_nature == "international":
            return _(
                "USPS International is used only to ship outside of the U.S.A. Please change the delivery method into USPS Domestic."
            )
        if recipient.country_id.code != "US" and delivery_nature == "domestic":
            return _(
                "USPS Domestic is used only to ship inside of the U.S.A. Please change the delivery method into USPS International."
            )
        if order:
            ...
            weight_uom_id = order.env[
                "product.template"
            ]._get_weight_uom_id_from_ir_config_parameter()
            weight_in_pounds = weight_uom_id._compute_quantity(
                tot_weight, order.env.ref("uom.product_uom_lb")
            )
            if (
                weight_in_pounds > 4 and order.carrier_id.usps_service == "First Class"
            ):  # max weight of FirstClass Service
                return _(
    ...
    def _usps_request_data(self, carrier, order):
        currency = carrier.env["res.currency"].search(
            [("name", "=", "USD")], limit=1
        )  # USPS Works in USDollars
        tot_weight = order._get_estimated_weight()
        ...
        rate_detail = {
            "api": "RateV4"
            if carrier.usps_delivery_nature == "domestic"
            else "IntlRateV2",
    ...
    def usps_rate_request(self, order, carrier):
        ...
        request_text = carrier.env["ir.qweb"]._render(
            "delivery_usps.usps_price_request", request_detail
        )
        ...
        try:
            ...
            dict_response[
                "error_message"
            ] = "USPS Server Not Found - Check your connectivity"
            return dict_response
        root = etree.fromstring(response_text)
        errors_return = root.findall(".//Description")
        errors_number = root.findall(".//Number")
        if errors_return:
            dict_response["error_message"] = self._error_message(
                errors_number[0].text if errors_number else "", errors_return[0].text
            )
            return dict_response
        # Domestic Rate
    ...
    def _item_data(self, line, weight, price):
        return {
            ...
            "Quantity": max(
                int(line.product_uom_qty), 1
            ),  # the USPS API does not accept 1.0 but 1
            "Value": price,
    ...
    def _usps_shipping_data(self, picking, is_return=False):
        ...
        for line in picking.move_lines:
            ...
            if shipper_currency.name == USD.name:
                ...
                quote_currency = picking.env["res.currency"].search(
                    [("name", "=", shipper_currency.name)], limit=1
                )
                amount = line.product_id.lst_price * line.product_uom_qty
                price = quote_currency._convert(
                    amount, USD, company, order.date_order or fields.Date.today()
                )
            weight = carrier._usps_convert_weight(
        ...
        shipping_detail = {
            ...
            "GrossOunces": int(
                round(gross_weight["ounce"], 0)
            ),  # API want 1 and no 1.0
            "MailType": carrier.usps_mail_type,
            ...
            "AltReturnAddress3": carrier.usps_redirect_partner_id.zip
            + " "
            + carrier.usps_redirect_partner_id.city
            if carrier.usps_redirect_partner_id
            else "",
            ...
            shipping_detail.update(
                {
                    ...
                    "picking_warehouse_partner_phone": self._convert_phone_number(
                        picking.picking_type_id.warehouse_id.partner_id.phone
                    ),
                    "picking_partner": picking.partner_id,
                    "picking_partner_phone": self._convert_phone_number(
                        picking.partner_id.phone or picking.partner_id.mobile or ""
                    ),
                }
            )
        else:
    ...
    def usps_request(self, picking, delivery_nature, service, is_return=False):
        ...
        request_text = picking.env["ir.qweb"]._render(
            "delivery_usps.usps_shipping_common", ship_detail
        )
        ...
        if errors_return:
            dict_response["error_message"] = self._error_message(
                errors_number[0].text, errors_return[0].text
            )
            return dict_response
        else:
    ...
    def cancel_shipment(self, picking, account_validated):
        ...
        request_text = picking.env["ir.qweb"]._render(
            "delivery_usps.usps_cancel_request", cancel_detail
        )
    ...
    def _error_message(self, error_number, api_error_message):
        if error_number == "-2147219401":
            api_error_message += _(
                "Recipient address cannot be found. Please check the address exists."
            )
