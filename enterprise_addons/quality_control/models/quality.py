...
class QualityPoint(models.Model):
    ...
    measure_on = fields.Selection(
        ...
        default="operation",
        required=True,
        help="Defines if the Quality Check is done at the Operation level or at a more granular Lot/SN level",
    )
    measure_frequency_type = fields.Selection(
        [("all", "All"), ("random", "Randomly"), ("periodical", "Periodically")],
        string="Control Frequency",
        default="all",
        required=True,
    )
    measure_frequency_value = fields.Float("Percentage")  # TDE RENAME ?
    measure_frequency_unit_value = fields.Integer(
        "Frequency Unit Value"
    )  # TDE RENAME ?
    measure_frequency_unit = fields.Selection(
        [("day", "Days"), ("week", "Weeks"), ("month", "Months")], default="day"
    )  # TDE RENAME ?
    is_lot_tested_fractionally = fields.Boolean(
        string="Lot Tested Fractionally",
        help="Determines if only a fraction of the lot should be tested",
    )
    testing_percentage_within_lot = fields.Float(
        help="Defines the percentage within a lot that should be tested"
    )
            ...
            checks = self.env["quality.check"].search(
                [
                    ...
                    (
                        "create_date",
                        ">=",
                        date_previous.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    ),
                ],
                limit=1,
            )
    ...
    def action_see_quality_checks(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "quality_control.quality_check_action_main"
        )
    ...
    def action_see_spc_control(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id(
            "quality_control.quality_check_action_spc"
        )
...
class QualityCheck(models.Model):
    ...
    measure = fields.Float(
        "Measure", default=0.0, digits="Quality Tests", tracking=True
    )
    measure_success = fields.Selection(
        [("none", "No measure"), ("pass", "Pass"), ("fail", "Fail")],
        string="Measure Success",
        compute="_compute_measure_success",
        readonly=True,
        store=True,
    )
    tolerance_min = fields.Float(
        "Min Tolerance", related="point_id.tolerance_min", readonly=True
    )
    tolerance_max = fields.Float(
        "Max Tolerance", related="point_id.tolerance_max", readonly=True
    )
    ...
    qty_to_test = fields.Float(
        compute="_compute_qty_to_test",
        string="Quantity to Test",
        help="Quantity of product to test within the lot",
    )
    qty_tested = fields.Float(
        string="Quantity Tested", help="Quantity of product tested within the lot"
    )
    measure_on = fields.Selection(
        [("operation", "Operation"), ("product", "Product")],
        string="Control per",
    ...
    move_line_id = fields.Many2one(
        ...
        "Stock Move Line",
        check_company=True,
        help="In case of Quality Check by Lot/SN, Move Line on which the Quality Check applies",
    )
    lot_name = fields.Char("Lot/Serial Number Name")
    lot_line_id = fields.Many2one(
        "stock.production.lot", store=True, compute="_compute_lot_line_id"
    )
    qty_line = fields.Float(compute="_compute_qty_line", string="Quantity")
    uom_id = fields.Many2one(
        related="product_id.uom_id", string="Product Unit of Measure"
    )
    show_lot_text = fields.Boolean(compute="_compute_show_lot_text")
    is_lot_tested_fractionally = fields.Boolean(
        related="point_id.is_lot_tested_fractionally"
    )
    testing_percentage_within_lot = fields.Float(
        related="point_id.testing_percentage_within_lot"
    )
        ...
        for rec in self:
            if rec.measure_success == "fail":
                rec.warning_message = _(
                    "You measured %.2f %s and it should be between %.2f and %.2f %s."
                ) % (
                    rec.measure,
                    rec.norm_unit,
                    rec.point_id.tolerance_min,
                    rec.point_id.tolerance_max,
                    rec.norm_unit,
                )
            else:
                rec.warning_message = ""
    ...
    def _compute_measure_success(self):
        for rec in self:
            if rec.point_id.test_type == "passfail":
                ...
                if (
                    rec.measure < rec.point_id.tolerance_min
                    or rec.measure > rec.point_id.tolerance_max
                ):
    ...
    @api.depends(
        "qty_line", "testing_percentage_within_lot", "is_lot_tested_fractionally"
    )
    def _compute_qty_to_test(self):
        for qc in self:
            if qc.is_lot_tested_fractionally:
                qc.qty_to_test = float_round(
                    qc.qty_line * qc.testing_percentage_within_lot / 100,
    ...
    def do_measure(self):
        self.ensure_one()
        if (
            self.measure < self.point_id.tolerance_min
            or self.measure > self.point_id.tolerance_max
        ):
            return self.do_fail()
        else:
            return self.do_pass()
    ...
    def do_alert(self):
        ...
        return {
            ...
            "views": [
                (self.env.ref("quality_control.quality_alert_view_form").id, "form")
            ],
            "res_id": alert.id,
    ...
    def action_open_quality_check_wizard(self, current_check_id=None):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "quality_control.action_quality_check_wizard"
        )
...
class QualityAlert(models.Model):
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        custom_values["name"] = self.env["ir.sequence"].next_by_code(
            "quality.alert"
        ) or _("New")
...
class ProductTemplate(models.Model):
    ...
    quality_control_point_qty = fields.Integer(
        compute="_compute_quality_check_qty", groups="quality.group_quality_user"
    )
    quality_pass_qty = fields.Integer(
        compute="_compute_quality_check_qty", groups="quality.group_quality_user"
    )
    quality_fail_qty = fields.Integer(
        compute="_compute_quality_check_qty", groups="quality.group_quality_user"
    )
    ...
    def _compute_quality_check_qty(self):
        for product_tmpl in self:
            (
                product_tmpl.quality_fail_qty,
                product_tmpl.quality_pass_qty,
            ) = product_tmpl.product_variant_ids._count_quality_checks()
            product_tmpl.quality_control_point_qty = product_tmpl.with_context(
                active_test=product_tmpl.active
            ).product_variant_ids._count_quality_points()

    def action_see_quality_control_points(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id(
            "quality_control.quality_point_action"
        )
        action["context"] = dict(
            self.env.context, default_product_ids=self.product_variant_ids.ids
        )

        domain_in_products_or_categs = [
            "|",
            ("product_ids", "in", self.product_variant_ids.ids),
            ("product_category_ids", "parent_of", self.categ_id.ids),
        ]
        domain_no_products_and_categs = [
            ("product_ids", "=", False),
            ("product_category_ids", "=", False),
        ]
        action["domain"] = OR(
            [domain_in_products_or_categs, domain_no_products_and_categs]
        )
        return action
...
class ProductProduct(models.Model):
    ...
    def _compute_quality_check_qty(self):
        for product in self:
            (
                product.quality_fail_qty,
                product.quality_pass_qty,
            ) = product._count_quality_checks()
    ...
    def _count_quality_points(self):
        ...
        parent_category_ids = (
            [int(parent_id) for parent_id in self.categ_id.parent_path.split("/")[:-1]]
            if self.categ_id
            else []
        )
    ...
    def action_see_quality_checks(self):
        ...
        action["context"] = dict(
            self.env.context, default_product_id=self.id, create=False
        )
