...
class StockMove(models.Model):
    ...
    def _create_quality_checks(self):
        ...
        for picking, moves in pick_moves.items():
            quality_points_domain = self.env["quality.point"]._get_domain(
                moves.product_id, picking.picking_type_id, measure_on="operation"
            )
            quality_points = (
                self.env["quality.point"].sudo().search(quality_points_domain)
            )

            if not quality_points:
                continue
            picking_check_vals_list = quality_points._get_checks_values(
                moves.product_id,
    ...
    def _action_cancel(self):
        ...
        self.picking_id.sudo().mapped("check_ids").filtered(
