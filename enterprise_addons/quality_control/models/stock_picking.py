...
class StockPicking(models.Model):
        ...
        for picking in self:
            ...
            for check in picking.check_ids:
                if (
                    ...
                    and check.product_id in checkable_products
                ):
                    todo = True
    ...
    def _action_done(self):
        ...
        if self.mapped("check_ids").filtered(
            lambda x: x.quality_state == "none" and x.product_id in product_to_check
        ):
    ...
    def _pre_action_done_hook(self):
        ...
        if res is True:
            ...
            if pickings_to_check_quality:
                return (
                    pickings_to_check_quality[0]
                    .with_context(
                        pickings_to_check_quality=pickings_to_check_quality.ids
                    )
                    .check_quality()
                )
        return res
    ...
    def _check_for_quality_checks(self):
        ...
        for picking in self:
            product_to_check = (
                picking.mapped("move_line_ids")
                .filtered(lambda ml: ml.qty_done != 0)
                .mapped("product_id")
            )
            if picking.mapped("check_ids").filtered(
                lambda qc: qc.quality_state == "none"
                and qc.product_id in product_to_check
            ):
