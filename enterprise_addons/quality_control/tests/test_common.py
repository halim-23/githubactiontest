...
class TestQualityCommon(common.TransactionCase):
    def setUp(self):
        ...
        self.product_category_1 = self.env["product.category"].create(
            {"name": "Office furnitures", "parent_id": self.product_category_base.id}
        )
        self.product = self.env["product.product"].create(
            {"name": "Office Chair", "categ_id": self.product_category_1.id}
        )
        ...
        self.product_4 = self.env["product.product"].create(
            {"name": "Saleable Product", "categ_id": self.product_category_base.id}
        )
