...
class QualityCheckWizard(models.TransientModel):
    ...
    additional_note = fields.Text(
        related="current_check_id.additional_note", readonly=False
    )
    is_lot_tested_fractionally = fields.Boolean(
        related="current_check_id.is_lot_tested_fractionally"
    )
    testing_percentage_within_lot = fields.Float(
        related="current_check_id.testing_percentage_within_lot"
    )
    ...
    def _compute_position(self):
        for wz in self:
            wz.position_current_check = (
                wz.check_ids.ids.index(wz.current_check_id.id) + 1
            )
    ...
    def do_measure(self):
        ...
        if (
            ...
            and self.current_check_id._is_pass_fail_applicable()
            and (self.failure_message or self.warning_message)
        ):
    ...
    def show_failure_message(self):
        return {
            ...
            "views": [
                (
                    self.env.ref(
                        "quality_control.quality_check_wizard_form_failure"
                    ).id,
                    "form",
                )
            ],
