...
class TestProjectLeaves(common.TransactionCase):
    ...
    def test_simple_employee_leave(self):
        ...
        self.assertNotEqual(
            task_1.with_context(include_past=True).leave_warning,
            False,
        ...
        self.assertNotEqual(
            task_1.with_context(include_past=True).leave_warning.startswith(
                "Test HrUser is on time off on that day"
            ),
            "single day task, should show date",
        )

        self.assertEqual(
            task_2.with_context(include_past=True).leave_warning,
            False,
    ...
    def test_multiple_leaves(self):
        ...
        self.assertTrue(
            task_1.leave_warning.startswith(
        ...
        self.assertEqual(
            task_3.with_context(include_past=True).leave_warning,
            "Test HrUser is on time off from the 01/06/2020 to the 01/10/2020. \n",
