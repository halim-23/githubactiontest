...
class SaleSubscription(models.Model):
    ...
    referrer_id = fields.Many2one(
        "res.partner", "Referrer", domain=[("grade_id", "!=", False)]
    )
    ...
    commission = fields.Monetary(
        string="Referrer Commission", compute="_compute_commission"
    )
    ...
    commission_plan_assignation = fields.Selection(
        [("auto", "Based On Referrer"), ("fixed", "Manual")],
        default="auto",
    ...
    def _compute_commission_plan_assignation(self):
        for sub in self:
            sub.commission_plan_assignation = (
                "fixed" if sub.commission_plan_frozen else "auto"
            )
        ...
        for sub in self:
            if not sub.referrer_id or not sub.commission_plan_id:
                ...
                for line in self.recurring_invoice_line_ids:
                    rule = sub.commission_plan_id._match_rules(
                        line.product_id, template_id, sub.pricelist_id.id
                    )
                    if rule:
                        commission = sub.currency_id.round(
    ...
    def _set_commission_plan(self):
        for sub in self:
            if (
                not sub.commission_plan_frozen
                and sub.referrer_id
                and sub.referrer_id.commission_plan_id != sub.commission_plan_id
            ):
    ...
    def _prepare_renewal_order_values(
        ...
        vals = super(SaleSubscription, self)._prepare_renewal_order_values(
            discard_product_ids, new_lines_ids
        )
