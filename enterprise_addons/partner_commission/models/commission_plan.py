...
class CommissionPlan(models.Model):
    ...
    commission_rule_ids = fields.One2many(
        "commission.rule", "plan_id", "Rules", copy=True
    )
    ...
    def _match_rules(self, product, template, pricelist):
        ...
        rule = self.env["commission.rule"].search(
            ...
            limit=1,
            order="product_id, template_id, pricelist_id",
        )
...
class CommissionRule(models.Model):
    ...
    plan_id = fields.Many2one(
        "commission.plan", "Commission Plan", required=True, ondelete="cascade"
    )
    category_id = fields.Many2one(
        "product.category", "Product Category", required=True, ondelete="cascade"
    )
    product_id = fields.Many2one(
        "product.product",
        "Product",
    ...
    template_id = fields.Many2one(
        "sale.subscription.template", "Subscription Template", ondelete="cascade"
    )
    ...
    is_capped = fields.Boolean(
        "Capped", required=True, default=False, help="Whether the commission is capped."
    )
    max_commission = fields.Float(
        "Max Commission",
        help="Maximum amount, specified in the currency of the pricelist, if given.",
    )
        ...
        for rule in self:
            if rule.product_id and rule.product_id.categ_id != rule.category_id:
                raise ValidationError(
                    _("Product %s does not belong to category %s")
                    % (rule.product_id.code, rule.category_id.name)
                )
    ...
    def _auto_init(self):
        ...
        tools.create_unique_index(
            ...
            [
                ...
                "COALESCE(template_id, -1)",
                "COALESCE(pricelist_id, -1)",
            ],
        )
        return result
