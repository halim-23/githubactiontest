...
class PurchaseOrder(models.Model):
    ...
    def _compute_source_invoice_count(self):
        for purchase_order in self:
            purchase_order.invoice_commission_count = self.env[
                "account.move"
            ].search_count([("commission_po_line_id.order_id", "=", purchase_order.id)])

    def action_view_customer_invoices(self):
        self.ensure_one()

        res = self.env["ir.actions.act_window"]._for_xml_id(
            "partner_commission.action_view_customer_invoices"
        )
        res.update(
            {
    ...
    def _cron_confirm_purchase_orders(self):
        ...
        for company in companies:
            ...
            for po in purchases.filtered(
                lambda p: (
                    p.invoice_commission_count > 0
                    and p.currency_id._convert(
                        p.amount_total, company.currency_id, company, today
                    )
                    >= minimum_amount_total
                )
            ):
                try:
                    po.button_confirm()
                    ...
                    _logger.exception(
                        "_cron_confirm_purchase_orders: PO (id=%s) could not be confirmed",
                        po.id,
                    )
