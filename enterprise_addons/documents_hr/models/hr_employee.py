...
class HrEmployee(models.Model):
    ...
    def _get_document_folder(self):
        return (
            self.company_id.documents_hr_folder
            if self.company_id.documents_hr_settings
            else False
        )
    ...
    def _check_create_documents(self):
        return (
            self.company_id.documents_hr_settings and super()._check_create_documents()
        )
    ...
    def _get_employee_document_domain(self):
        ...
        if self.user_id:
            user_domain = expression.OR(
                [user_domain, [("owner_id", "=", self.user_id.id)]]
            )
        return user_domain
    ...
    def action_open_documents(self):
        ...
        if not self.address_home_id:
            ...
            raise ValidationError(
                _("You must set an address on the employee to use Documents features.")
            )
