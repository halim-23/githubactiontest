odoo.define("pos_l10n_se.PosBlackboxSweden", function (require) {
  ...
  models.PosModel = models.PosModel.extend({
    useBlackBoxSweden: function () {
      return Boolean(this.config.iface_sweden_fiscal_data_module);
    },
    disallowLineQuantityChange() {
      const result = posModelSuper.disallowLineQuantityChange.bind(this)();
    ...
    push_order_to_blackbox: async function (order) {
      const fdm = this.iot_device_proxies.fiscal_data_module;
      const date = order.creation_date.toJSON();
      const data = {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        const action_result = await fdm.action({
    ...
    add_product: async function (product, options) {
      if (this.pos.useBlackBoxSweden() && product.taxes_id.length === 0) {
        ...
        await Gui.showPopup("ErrorPopup", {
          title: _t("POS error"),
          body: _t(
            "You can only make positive or negative order. You cannot mix both."
          ),
        });
      } else {
