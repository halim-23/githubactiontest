...
class AccountPayment(models.Model):
            ...
            if rec.payment_method_id == aba_payment_method:
                ...
                if bank_acc.acc_type != "aba" or not bank_acc.aba_bsb:
                    raise ValidationError(
                        _(
                            "Journal '%s' requires a proper ABA account. Please configure it first.",
                            rec.journal_id.name,
                        )
                    )

                if (
                    not rec.journal_id.aba_user_spec
                    or not rec.journal_id.aba_fic
                    or not rec.journal_id.aba_user_number
                ):
                    raise ValidationError(
                        _(
                            "Please fill in the ABA data of account %s (journal %s) before using it to generate ABA payments."
                        )
    ...
    def _check_partner_bank_account(self):
        ...
        for rec in self:
            if rec.payment_method_id == aba_payment_method:
                if (
                    rec.partner_bank_id.acc_type != "aba"
                    or not rec.partner_bank_id.aba_bsb
                ):
                    raise ValidationError(
                        _(
                            "The partner requires a bank account with a valid BSB and account number. Please configure it first."
                        )
                    )
