...
class AccountBatchPayment(models.Model):
        ...
        if self.payment_method_code == "aba_ct":
            ...
            if bank_account.acc_type != "aba" or not bank_account.aba_bsb:
                raise UserError(
                    ...
                    % (bank_account.acc_number, self.journal_id.name)
                )

            if (
                not self.journal_id.aba_fic
                or not self.journal_id.aba_user_spec
                or not self.journal_id.aba_user_number
            ):
                raise UserError(
                    _(
            ...
            for payment in self.payment_ids:
                if (
                    payment.partner_bank_id.acc_type != "aba"
                    or not payment.partner_bank_id.aba_bsb
                ):
                    raise UserError(
                        _(
                            "Bank account for payment '%s' has an invalid BSB or account number.",
                            payment.name,
                        )
                    )
    ...
    def _create_aba_document(self):
        ...
        def to_fixed_width(string, length, fill=" ", right=False):
            return (
                right
                and string[0:length].rjust(length, fill)
                or string[0:length].ljust(length, fill)
            )
        ...
        def append_detail(detail_summary, detail_record, credit, debit):
            ...
            if len(detail_summary["detail_records"]) > 999997:
                raise UserError(
                    _(
                        "Too many transactions for one ABA file - Please split in to multiple transfers"
                    )
                )
            detail_summary["credit_total"] += credit
            detail_summary["debit_total"] += debit
            if (
                detail_summary["credit_total"] > 99999999.99
                or detail_summary["debit_total"] > 99999999.99
            ):
                raise UserError(
                    _(
                        "The value of transactions is too high for one ABA file - Please split in to multiple transfers"
                    )
                )
        ...
        for payment in self.payment_ids:
            ...
            detail_record = (
                ...
                + "50"
                + to_fixed_width(
                    str(round(aud_currency.round(credit) * 100)), 10, "0", right=True
                )
                + to_fixed_width(
                    payment.partner_bank_id.acc_holder_name or payment.partner_id.name,
                    32,
                )
                ...
                + to_fixed_width(
                    bank_account.acc_holder_name or self.journal_id.company_id.name, 16
                )
                + ("0" * 8)
            )
        ...
        if self.journal_id.aba_self_balancing:
            ...
            detail_record = (
                ...
                + "13"
                + to_fixed_width(
                    str(round(aud_currency.round(debit) * 100)),
                    10,
                    fill="0",
                    right=True,
                )
                + to_fixed_width(
                    bank_account.acc_holder_name or self.journal_id.company_id.name, 32
                )
        ...
        footer_record = (
            ...
            + to_fixed_width(
                str(
                    round(
                        aud_currency.round(
                            abs(
                                detail_summary["credit_total"]
                                - detail_summary["debit_total"]
                            )
                        )
                        * 100
                    )
            ...
            + to_fixed_width(
                str(round(aud_currency.round(detail_summary["credit_total"]) * 100)),
                10,
                fill="0",
                right=True,
            )
            + to_fixed_width(
                str(round(aud_currency.round(detail_summary["debit_total"]) * 100)),
                10,
                fill="0",
                right=True,
            )
            + (" " * 24)
            + to_fixed_width(
                str(len(detail_summary["detail_records"])), 6, fill="0", right=True
            )
            + (" " * 40)
        )
        ...
        return (
            header_record
            + SEPARATOR
            + SEPARATOR.join(detail_summary["detail_records"])
            + SEPARATOR
            + footer_record
            + SEPARATOR
        )
