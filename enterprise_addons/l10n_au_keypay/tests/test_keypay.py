...
@skipIf(
    not os.getenv("KEYPAY_BUSINESS_ID" or not os.getenv("KEYPAY_API_KEY")),
    "no keypay credentials",
)
class TestKeypay(TransactionCase):
        ...
        cls.config = cls.env["res.config.settings"].create(
            {
                ...
                "l10n_au_kp_journal_id": cls.env["account.journal"]
                .search(
    ...
    def test_01_keypay_fetch_payrun(self):
        ...
        moves = self.env["account.move"].search(
            [("l10n_au_kp_payrun_identifier", "!=", False)]
        )
        ...
        self.assertEqual(
            moves[-2].line_ids.mapped("credit"), [1996.0, 0.0, 884.0, 0.0, 273.6]
        )
        self.assertEqual(
            moves[-1].line_ids.mapped("credit"), [635.0, 0.0, 85.0, 0.0, 68.4]
        )
        self.assertEqual(
            moves[-2].line_ids.mapped("debit"), [0.0, 2880.0, 0.0, 273.6, 0.0]
        )
        self.assertEqual(
            moves[-1].line_ids.mapped("debit"), [0.0, 720.0, 0.0, 68.4, 0.0]
        )
        # verify that bis account are used
        self.assertEqual(
            moves[-1].line_ids.mapped("account_id.name"),
            ["Test 2 bis", "Test 1 bis", "Test 4", "Test 6", "Test 7"],
        )
        self.assertEqual(
            moves[-2].line_ids.mapped("account_id.name"),
            ["Test 2 bis", "Test 1 bis", "Test 4", "Test 6", "Test 7"],
        )
