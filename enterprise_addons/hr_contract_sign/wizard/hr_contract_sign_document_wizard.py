...
class HrContractSignDocumentWizard(models.TransientModel):
    ...
    def _group_hr_contract_domain(self):
        group = self.env.ref(
            "hr_contract.group_hr_contract_manager", raise_if_not_found=False
        )
    ...
    def _get_sign_template_ids(self):
        ...
        for template in self.env["sign.template"].search([]):
            distinct_responsible_count = len(
    ...
    def _default_get_template_warning(self):
        return not bool(self._get_sign_template_ids()) and _(
            "No appropriate template could be found, please make sure you configured them properly."
        )
    ...
    def default_get(self, fields_list):
        ...
        if (
            "responsible_id" in fields_list
            and not defaults.get("responsible_id")
            and defaults.get("contract_id")
        ):
    ...
    responsible_id = fields.Many2one(
        "res.users", string="Responsible", domain=_group_hr_contract_domain
    )
    ...
    def _compute_has_both_template(self):
        for wizard in self:
            wizard.has_both_template = bool(
                wizard.sign_template_ids.filtered(
                    lambda t: len(t.sign_item_ids.mapped("responsible_id")) == 2
                )
            )
    ...
    def validate_signature(self):
        ...
        for sign_template_id in sign_templates_both_ids:
            second_role = set(
                sign_template_id.sign_item_ids.mapped("responsible_id").ids
            )
        ...
        if self.responsible_id and sign_templates_both_ids:
            signatories_text = _("%s and %s are the signatories.") % (
