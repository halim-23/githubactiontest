...
class MrpProductionWorkcenterLine(models.Model):
    ...
    identifier = fields.Char(
        related="current_quality_check_id.point_id.device_id.identifier"
    )
    boxes = fields.Char(compute="_compute_boxes")
    device_name = fields.Char(
        related="current_quality_check_id.point_id.device_id.name",
        size=30,
        string="Device Name: ",
    )
    ...
    def _compute_boxes(self):
        for wo in self:
            ...
            for trigger in triggers:
                ...
                box_dict[box].append(
                    [trigger.device_id.identifier, trigger.key, trigger.action]
                )
...
class IotTrigger(models.Model):
    ...
    device_id = fields.Many2one(
        "iot.device", "Device", required=True, domain="[('type', '=', 'keyboard')]"
    )
    key = fields.Char("Key")
