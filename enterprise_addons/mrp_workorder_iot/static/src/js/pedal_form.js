  ...
  var PedalRenderer = PDFViewerNoReloadRenderer.extend({
    ...
    _updatePedalStatusButton: function () {
      ...
      if (this.show_pedal_button) {
        var button = $("<button>", {
          class:
            "btn o_pedal_status_button " +
            (self.pedal_connect ? " btn-primary o_active " : " btn-warning"),
      ...
      return this._super.apply(this, arguments).then(function () {
        ...
        for (var box in boxes) {
          for (var device in boxes[box]) {
            var device_proxy = new DeviceProxy(self, {
              identifier: boxes[box][device][0],
              iot_ip: box,
            });
    ...
    _onValueChange: function (data) {
      ...
      if (data.owner && data.owner !== data.session_id) {
        ...
        for (var box in boxes) {
          for (var device in boxes[box]) {
            if (
              data.device_identifier === boxes[box][device][0] &&
              data.value.toUpperCase() === boxes[box][device][1].toUpperCase()
            ) {
              this.$(
                "button[barcode_trigger='" + boxes[box][device][2] + "']:visible"
              ).click();
            }
          }
        }
