...
CATALOG52 = [
    ...
    (
        "2001",
        "BIENES TRANSFERIDOS EN LA AMAZONÍA REGIÓN SELVAPARA SER CONSUMIDOS EN LA MISMA",
    ),
    (
        "2002",
    ...
    (
        "2008",
        "VENTA EXONERADA DEL IGV-ISC-IPM. PROHIBIDA LA VENTA FUERA DE LA ZONA COMERCIAL DE TACNA",
    ),
    (
        "2009",
...
class AccountMove(models.Model):
    ...
    l10n_pe_edi_is_required = fields.Boolean(
        string="Is the Peruvian EDI needed", compute="_compute_l10n_pe_edi_is_required"
    )
    l10n_pe_edi_cancel_cdr_number = fields.Char(
        copy=False, help="Reference from webservice to consult afterwards."
    )
    ...
    l10n_pe_edi_operation_type = fields.Selection(
        selection=[
            ...
            (
                "0112",
                "[0112] Internal Sale - Sustains Natural Person Deductible Expenses",
            ),
            ("0113", "[0113] Internal Sale-NRUS"),
            ("0200", "[0200] Export of Goods"),
            (
                "0201",
                "[0201] Exportation of Services - Provision of services performed entirely in the country",
            ),
            (
                "0202",
            ...
            (
                "0204",
                "[0204] Exportation of Services - Services to foreign-flagged ships and aircraft",
            ),
            (
                "0205",
            ...
            (
                "0207",
                "[0207] Exportation of Services - Supply of electric power in favor of subjects domiciled in ZED",
            ),
            (
                "0208",
                "[0208] Exportation of Services - Provision of services partially carried out abroad",
            ),
            (
                "0301",
                "[0301] Operations with air waybill (issued in the national scope)",
            ),
            ("0302", "[0302] Passenger rail transport operations"),
            ("0303", "[0303] Oil royalty Pay Operations"),
            ("0401", "[0401] Non-domiciled sales that do not qualify as an export"),
            ("1001", "[1001] Operation Subject to Detraction"),
            (
                "1002",
                "[1002] Operation Subject to Detraction - Hydrobiological Resources",
            ),
            (
                "1003",
            ...
            (
                "1004",
                "[1004] Operation Subject to Drawdown - Cargo Transportation Services",
            ),
    ...
    l10n_pe_edi_legend = fields.Selection(selection=CATALOG52, string="Legend Code")
    l10n_pe_edi_legend_value = fields.Char(
        string="Legend",
        store=True,
        readonly=False,
        compute="_compute_l10n_pe_edi_legend_value",
    ...
    def _compute_l10n_pe_edi_is_required(self):
        for move in self:
            move.l10n_pe_edi_is_required = (
                move.country_code == "PE"
    ...
    def _compute_l10n_pe_edi_operation_type(self):
        for move in self:
            move.l10n_pe_edi_operation_type = (
                "0101"
                if move.country_code == "PE" and move.is_sale_document()
                else False
            )
    ...
    def _compute_l10n_pe_edi_legend_value(self):
        for move in self:
            if move.l10n_pe_edi_legend:
                matched_elements = [
                    element
                    for element in CATALOG52
                    if element[0] == move.l10n_pe_edi_legend
                ]
    ...
    def _get_starting_sequence(self):
        ...
        if self.l10n_pe_edi_is_required and self.l10n_latam_document_type_id:
            ...
            middle_code = doc_mapping.get(
                self.l10n_latam_document_type_id.code, self.journal_id.code
            )
    ...
    def _l10n_pe_edi_get_serie_folio(self):
        ...
        serie = (
            self.name[: number_match[-1].start()].replace("-", "").replace(" ", "")
            or None
        )
    ...
    def _l10n_pe_edi_get_spot(self):
        max_percent = max(
            self.invoice_line_ids.mapped("product_id.l10n_pe_withhold_percentage"),
            default=0,
        )
        if (
            not max_percent
            or self.amount_total_signed < 700
            or not self.l10n_pe_edi_operation_type in ["1001", "1002", "1003", "1004"]
        ):
            return {}
        line = self.invoice_line_ids.filtered(
            lambda r: r.product_id.l10n_pe_withhold_percentage == max_percent
        )[0]
        national_bank = self.env.ref(
            "l10n_pe_edi.peruvian_national_bank", raise_if_not_found=False
        )
        national_bank_account_number = False
        if national_bank:
            national_bank_account = self.company_id.bank_ids.filtered(
        ...
        return {
            ...
            "spot_amount": float_round(
                self.amount_total * (max_percent / 100.0), precision_rounding=2
            ),
            "Amount": float_repr(
                float_round(
                    self.amount_total_signed * (max_percent / 100.0),
    ...
    def _l10n_pe_edi_get_extra_report_values(self):
        ...
        edi_attachment_zipped = self._get_edi_attachment(
            self.env.ref("l10n_pe_edi.edi_pe_ubl_2_1")
        )
        if not edi_attachment_zipped:
            return {}
        edi_attachment_str = self.env[
            "account.edi.format"
        ]._l10n_pe_edi_unzip_edi_document(
            base64.decodebytes(edi_attachment_zipped.with_context(bin_size=False).datas)
        )
        edi_tree = etree.fromstring(edi_attachment_str)

        # Qr-code
        signature_hash = edi_tree.xpath(
            "//ds:DigestValue", namespaces={"ds": "http://www.w3.org/2000/09/xmldsig#"}
        )[0].text
        igv_tax_amount = ""
        nsmap = {k: v for k, v in edi_tree.nsmap.items() if k}
        for tax_element in edi_tree.xpath("//cac:TaxSubtotal", namespaces=nsmap):
            tax_name_elements = tax_element.xpath(
                ".//cac:TaxScheme/cbc:Name", namespaces=nsmap
            )
            tax_amount_elements = tax_element.xpath(
                ".//cbc:TaxAmount", namespaces=nsmap
            )
            if (
                tax_name_elements
                and tax_amount_elements
                and tax_name_elements[0].text == "IGV"
            ):
    ...
    def button_cancel_posted_moves(self):
        ...
        if pe_invoices:
            credit_notes_needed = pe_invoices.filtered(
                lambda move: move.l10n_latam_document_type_id.code == "03"
            )
            ...
            cancel_reason_needed = pe_invoices.filtered(
                lambda move: not move.l10n_pe_edi_cancel_reason
            )
            if cancel_reason_needed:
                return (
                    self.env.ref("l10n_pe_edi.action_l10n_pe_edi_cancel")
                    .sudo()
                    .read()[0]
                )
