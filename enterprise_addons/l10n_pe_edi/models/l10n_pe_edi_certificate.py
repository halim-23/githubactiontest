...
class Certificate(models.Model):
    ...
    def _decode_certificate(self):
        ...
        """
        self.ensure_one()
        decrypted_content = crypto.load_pkcs12(
    ...
    def create(self, vals):
        ...
        try:
            ...
            cert_date_start = peruvian_tz.localize(
                datetime.strptime(certificate.get_notBefore().decode(), date_format)
            )
            cert_date_end = peruvian_tz.localize(
                datetime.strptime(certificate.get_notAfter().decode(), date_format)
            )
    ...
    def _sign(self, edi_tree):
        ...
        edi_tree_c14n_str = etree.tostring(
            edi_tree_copy, method="c14n", exclusive=True, with_comments=False
        )
        ...
        signed_info_element = signature_tree.xpath(
            ".//ds:SignedInfo", namespaces=namespaces
        )[0]
        signature = etree.tostring(
            signed_info_element, method="c14n", exclusive=True, with_comments=False
        )
        ...
        signature_tree.xpath(".//ds:SignatureValue", namespaces=namespaces)[
            0
        ].text = signature_b64_hash
        signature_tree.xpath(".//ds:X509Certificate", namespaces=namespaces)[
            0
        ].text = pem_certificate
        signed_edi_tree = deepcopy(edi_tree)
        signature_element = signed_edi_tree.xpath(
            ".//ds:Signature", namespaces=namespaces
        )[0]
