...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_pe_edi_unzip_all_edi_documents(self, zip_bytes):
        ...
        with io.BytesIO(base64.b64decode(zip_bytes)) as buffer:
            ...
            for content_name in zip_bytes.namelist():
                ...
                application_response = etree.fromstring(xml_bytes).xpath(
                    "//applicationResponse"
                )
                ...
                if not application_response:
                    ...
                    unzipped_cdr = self._l10n_pe_edi_unzip_edi_document(
                        base64.b64decode(application_response[0].text)
                    )
    ...
    def _l10n_pe_edi_get_general_error_messages(self):
        return {
            "L10NPE02": _lt(
                "The zip file is corrupted, please check that the zip we are reading is the one you need."
            ),
            "L10NPE03": _lt(
                "The XML inside of the zip file is corrupted or has been altered, please review the XML "
                "inside of the XML we are reading."
            ),
            "L10NPE07": _lt(
                "Trying to send the invoice to the OSE the webservice returned a controlled error, please "
                "try again later, the error is on their side not here."
            ),
            "L10NPE08": _lt(
                "Check your firewall parameters, it is not being possible to connect with server to sign "
                "invoices."
            ),
            "L10NPE10": _lt(
                "The URL provided to connect to the OSE is wrong, please check your implementation."
            ),
            "L10NPE11": _lt("The XML generated is not valid."),
            "L10NPE16": _lt(
                "There was an error while establishing the connection to the server, try again and "
                "if it fails check the URL in the parameter l10n_pe_edi.endpoint."
            ),
            "L10NPE17": _lt(
                "There are problems with the connection to the IAP server. "
                "Please try again in a few minutes."
            ),
            "L10NPE18": _lt(
                "The URL provided for the IAP server is wrong, please go to  Settings --> System "
    ...
    def _l10n_pe_edi_get_cdr_error_messages(self):
        ...
        return {
            "2800": _lt(
                "The type of identity document used for the client is not valid. Review the type of document "
            ...
            "2801": _lt(
                "The VAT you use for the customer is a DNI type, to be a valid DNI it must be the exact length "
                "of 8 digits."
            ),
            "2315": _lt(
                "The cancellation reason field should not be empty when canceling the invoice, you must return "
            ...
            "4332": _lt(
                "One or more products do not have the UNSPSC code configured, to avoid this warning you must "
                "configure a code for this product. This warning does not invalidate the document."
            ),
            "2017": _lt(
                "For invoices, the customer's identity document must be RUC. Check that the client has a valid "
                "RUC and the type of document is RUC."
            ),
            "3206": _lt(
                "The type of operation is not valid for the type of document you are trying to create. The "
                "document must return to Draft state and change the type of operation."
            ),
            "2022": _lt(
                "The name of the Partner must be changed to at least 2 characters to meet the proper standard."
            ),
            "151": _lt(
                "The name of the file depends on the sequence in the journal, please go to the journal and "
                "configure the shortcode as LLL- (three (3) letters plus a dash and the 3 letters must be UPPERCASE.)"
            ),
            "156": _lt(
                "The zip file is corrupted, check again if the file trying to access is not damaged."
            ),
            "2119": _lt(
                "The invoice related to this Credit Note has not been reported, go to the invoice related and "
                "sign it in order to generate this Credit Note."
            ),
            "2120": _lt(
                "The invoice related to this Credit Note has been canceled, set this document to draft and "
                "cancel it."
            ),
            "2209": _lt(
                "The invoice related to this Debit Note has not been reported, go to the invoice related and "
                "sign it in order to generate this Debit Note"
            ),
            "2207": _lt(
                "The invoice related to this Debit Note has been canceled, set this document to draft and "
                "cancel it."
            ),
            "001": _lt(
                "This invoice has been validated by the OSE and we can not allow set it to draft, please try "
                "to revert it with a credit not or cancel it and create a new one instead."
            ),
            "1033": _lt(
                "This document already exists on the OSE side.  Check if you gave a proper unique name to your "
                "document. "
            ),
            "1034": _lt(
                "Check that the VAT set in the company is correct, this error generally happen when you did "
            ...
            "2204": _lt(
                "The document type of the invoice related is not the same of this document. Check the "
            ...
            "3034": _lt(
                "You need to configure the account for 'Banco de la Nación'. go to the Other Info setting"
            ...
            "3128": _lt(
                "As you have a document that must be detracted (withheld) which mean a document over 700 "
    ...
    def _l10n_pe_edi_decode_cdr(self, cdr_str):
        ...
        if code:
            ...
            error_message = "%s<br/><br/><b>%s</b><br/>%s|%s" % (
                error_messages_map.get(
                    code, _("We got an error response from the OSE. ")
                ),
                _("Original message:"),
                html_escape(code),
                html_escape(message),
    ...
    def _l10n_pe_edi_get_edi_values(self, invoice):
        ...
        for line_vals in values["invoice_line_vals_list"]:
            line = line_vals["line"]
            line_vals["price_unit_type_code"] = (
                "01"
                if not line.currency_id.is_zero(line_vals["price_unit_after_discount"])
                else "02"
            )
    ...
    def _l10n_pe_edi_get_iap_buy_credits_message(self, company):
        ...
        url = self.env["iap.account"].get_credits_url(
            service_name="l10n_pe_edi", base_url=base_url
        )
        return """<p><b>%s</b></p><p>%s</p>""" % (
            _("You have insufficient credits to sign or verify this document!"),
            _(
                'Please proceed to buy more credits <a href="%s">here.</a>',
                html_escape(url),
            ),
        )
    ...
    def _l10n_pe_edi_sign_invoices_iap(self, invoice, edi_filename, edi_str):
        ...
        signature_element = edi_tree_copy.xpath(
            ".//ds:Signature", namespaces=namespaces
        )[0]
        signature_str = self.env.ref("l10n_pe_edi.pe_ubl_2_1_signature")._render(
            {"digest_value": ""}
        )
        signature_element.getparent().replace(
            signature_element, objectify.fromstring(signature_str)
        )

        error = self.env["ir.attachment"]._l10n_pe_edi_check_with_xsd(
            edi_tree_copy, invoice.l10n_latam_document_type_id.code
        )
        if error:
            return {
                "error": "<b>%s</b><br/>%s"
                % (_("XSD validation failed:"), html_escape(error)),
                "blocking_level": "error",
            }

        dbuuid, iap_server_url, iap_token = self._l10n_pe_edi_get_iap_params(
            invoice.company_id
        )
        ...
        try:
            result = iap_jsonrpc(
                iap_server_url + "/iap/l10n_pe_edi/1/send_bill",
                params=rpc_params,
                timeout=1500,
            )
        except InvalidSchema:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE16"],
                "blocking_level": "error",
            }
        except AccessError:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE17"],
                "blocking_level": "warning",
            }
        except InvalidURL:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE18"],
                "blocking_level": "error",
            }

        if result.get("message"):
            if result["message"] == "no-credit":
                error_message = self._l10n_pe_edi_get_iap_buy_credits_message(
                    invoice.company_id
                )
            else:
        ...
        if cdr_decoded.get("error"):
            ...
            cdr_tree = etree.fromstring(cdr_str)
            message_element, response_code = self._l10n_pe_edi_response_code_digiflow(
                cdr_tree
            )
            if response_code == "1033":
                cdr_status = self._l10n_pe_edi_get_status_cdr_iap(invoice)
                if cdr_status.get("error"):
                    error_msg = "%s<br/>%s<br/>%s" % (
                        cdr_decoded["error"],
                        _("Error requesting CDR status:"),
                        cdr_status["error"],
                    )
                ...
                # Status code 0004 means the CDR already exists.
                    ...
                    invoice.message_post(
                        body=_(
                            "The invoice already exists on SUNAT. CDR successfully retrieved."
                        )
                    )
        ...
        xml_document = result.get("signed") and self._l10n_pe_edi_unzip_edi_document(
            base64.b64decode(result["signed"])
        )
    ...
    def _l10n_pe_edi_get_status_cdr_iap(self, invoice):
        ...
        try:
            result = iap_jsonrpc(
                iap_server_url + "/iap/l10n_pe_edi/1/get_status_cdr",
                params=rpc_params,
                timeout=1500,
            )
        except InvalidSchema:
        ...
        cdr_tree = etree.fromstring(cdr_str)
        status_code = (
            cdr_tree.find(".//{*}statusCode").text
            if cdr_tree.find(".//{*}statusCode") is not None
            else ""
        )
        status_message = (
            cdr_tree.find(".//{*}statusMessage").text
            if cdr_tree.find(".//{*}statusMessage") is not None
            else ""
        )
    ...
    def _l10n_pe_edi_cancel_invoices_step_1_iap(
        self, company, invoices, void_filename, void_str
    ):
        self.ensure_one()
        ...
        try:
            result = iap_jsonrpc(
                iap_server_url + "/iap/l10n_pe_edi/1/send_summary",
                params=rpc_params,
                timeout=15,
            )
        except Fault:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE07"],
                "blocking_level": "warning",
            }
        except AccessError:
            return {
    ...
    def _l10n_pe_edi_cancel_invoices_step_2_iap(self, company, edi_values, cdr_number):
        ...
        try:
            result = iap_jsonrpc(
                iap_server_url + "/iap/l10n_pe_edi/1/get_status",
                params=rpc_params,
                timeout=15,
            )
        except Fault:
    ...
    def _l10n_pe_edi_post_invoice_web_service(self, invoice, edi_filename, edi_str):
        ...
        res = getattr(self, "_l10n_pe_edi_sign_invoices_%s" % provider)(
            invoice, edi_filename, edi_str
        )
    ...
    def _l10n_pe_edi_sign_invoices_sunat_digiflow_common(
        self, invoice, edi_filename, edi_str, credentials
    ):
        self.ensure_one()
        ...
        if not invoice.company_id.l10n_pe_edi_certificate_id:
            return {
                "error": _(
                    "No valid certificate found for %s company.",
        ...
        error = self.env["ir.attachment"]._l10n_pe_edi_check_with_xsd(
            edi_tree, invoice.l10n_latam_document_type_id.code
        )
        if error:
            return {
                "error": _("XSD validation failed: %s", error),
                "blocking_level": "error",
            }
        edi_str = etree.tostring(edi_tree, xml_declaration=True, encoding="ISO-8859-1")

        zip_edi_str = self._l10n_pe_edi_zip_edi_document(
            [("%s.xml" % edi_filename, edi_str)]
        )
        ...
        try:
            ...
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE08"],
                "blocking_level": "warning",
            }
        except HTTPError:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE10"],
                "blocking_level": "warning",
            }
        except TypeError:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE11"],
                "blocking_level": "error",
            }
        except ReadTimeout:
            return {
                "error": self._l10n_pe_edi_get_general_error_messages()["L10NPE12"],
                "blocking_level": "warning",
            }
    ...
    def _l10n_pe_edi_sign_invoices_sunat(self, invoice, edi_filename, edi_str):
        ...
        return self._l10n_pe_edi_sign_invoices_sunat_digiflow_common(
            invoice, edi_filename, edi_str, credentials
        )
    ...
    def _l10n_pe_edi_cancel_invoices_step_1_sunat_digiflow_common(
        ...
        void_str = etree.tostring(
            void_tree, xml_declaration=True, encoding="ISO-8859-1"
        )
        zip_void_str = self._l10n_pe_edi_zip_edi_document(
            [("%s.xml" % void_filename, void_str)]
        )
        transport = Transport(operation_timeout=15, timeout=15)
    ...
    def _l10n_pe_edi_cancel_invoices_step_1_sunat(
        self, company, invoices, void_filename, void_str
    ):
    ...
    def _l10n_pe_edi_cancel_invoices_step_1_digiflow(
        self, company, invoices, void_filename, void_str
    ):
    ...
    def _l10n_pe_edi_cancel_invoices_step_2_sunat_digiflow_common(
        self, company, edi_values, cdr_number, credentials
    ):
        self.ensure_one()
    ...
    def _l10n_pe_edi_cancel_invoices_step_2_sunat(
        self, company, edi_values, cdr_number
    ):
        credentials = self._l10n_pe_edi_get_sunat_credentials(company)
    ...
    def _l10n_pe_edi_cancel_invoices_step_2_digiflow(
        self, company, edi_values, cdr_number
    ):
        credentials = self._l10n_pe_edi_get_digiflow_credentials(company)
        return self._l10n_pe_edi_cancel_invoices_step_2_sunat_digiflow_common(
    ...
    def _check_move_configuration(self, move):
        ...
        if not move.commercial_partner_id.vat:
            res.append(
                _("VAT number is missing on partner %s")
        ...
        for line in lines:
            ...
            if (
                len(taxes) > 1
                and len(
                    taxes.filtered(lambda t: t.tax_group_id.l10n_pe_edi_code == "IGV")
                )
                > 1
            ):
                res.append(
                    _(
                        "You can't have more than one IGV tax per line to generate a legal invoice in Peru"
                    )
                )
        if any(
            not line.tax_ids for line in move.invoice_line_ids if not line.display_type
        ):
    ...
    def _post_invoice_edi(self, invoices):
        ...
        edi_values = self._l10n_pe_edi_get_edi_values(invoice)
        edi_str = (
            self.env.ref("l10n_pe_edi.%s" % latam_invoice_type)
            ._render(edi_values)
            .encode()
        )
    ...
    def _l10n_pe_edi_cancel_invoice_edi_step_1(self, invoices):
        self.ensure_one()
        certificate_date = (
            self.env["l10n_pe_edi.certificate"]._get_pe_current_datetime().date()
        )
        company = invoices[
            0
        ].company_id  # documents are always batched by company in account_edi.
        provider = company.l10n_pe_edi_provider

        # Prepare the void documents to void all invoices at once.
        void_number = self.env["ir.sequence"].next_by_code(
            "l10n_pe_edi.summary.sequence"
        )
        void_values = {
        ...
        void_str = (
            self.env.ref("l10n_pe_edi.pe_ubl_2_1_void_documents")
            ._render(void_values)
            .encode()
        )
        void_filename = "%s-%s" % (company.vat, void_number)

        res = getattr(self, "_l10n_pe_edi_cancel_invoices_step_1_%s" % provider)(
            company, invoices, void_filename, void_str
        )
        ...
        if not res.get("cdr_number"):
            error = _(
                "The EDI document failed to be cancelled because the cancellation CDR number is missing."
            )
            return {invoice: {"error": error} for invoice in invoices}

        # Chatter.
        message = _(
            "Cancellation is in progress in the government side (CDR number: %s).",
            html_escape(res["cdr_number"]),
        )
        if res.get("xml_document"):
        ...
        return {
            invoice: {"error": message, "blocking_level": "info"}
            for invoice in invoices
        }

    def _l10n_pe_edi_cancel_invoice_edi_step_2(
        self, invoices, edi_attachments, cdr_number
    ):
        self.ensure_one()
        company = invoices[
            0
        ...
        res = getattr(self, "_l10n_pe_edi_cancel_invoices_step_2_%s" % provider)(
            company, edi_values, cdr_number
        )
        ...
        # Chatter.
        message = _(
            "The EDI document was successfully cancelled by the government (CDR number: %s).",
            html_escape(cdr_number),
        )
    ...
    def _cancel_invoice_edi(self, invoices):
        ...
        if invoices_with_cdr:
            ...
            invoices_same_number = invoices_with_cdr.filtered(
                lambda move: move.l10n_pe_edi_cancel_cdr_number == cdr_number
            )
            ...
            if len(invoices_same_number) == len(all_invoices_same_number):
                ...
                for invoice in invoices_same_number:
                    edi_attachments |= invoice._get_edi_attachment(self)
                res.update(
                    self._l10n_pe_edi_cancel_invoice_edi_step_2(
                        invoices_same_number, edi_attachments, cdr_number
                    )
                )
            else:
                # Error.
