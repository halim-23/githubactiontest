...
def mocked_l10n_pe_edi_post_invoice_web_service(
    edi_format, invoice, edi_filename, edi_str
):
...
class TestPeEdiCommon(AccountEdiTestCommon):
    ...
    def setUpClass(
        cls,
        chart_template_ref="l10n_pe.pe_chart_template",
        edi_format_ref="l10n_pe_edi.edi_pe_ubl_2_1",
    ):
        super().setUpClass(
        ...
        resource_path = modules.get_resource_path(
            "l10n_pe_edi", "demo/certificates", "certificate.pfx"
        )
        cls.certificate = cls.env["l10n_pe_edi.certificate"].create(
            {
                "content": base64.encodebytes(
                    misc.file_open(resource_path, "rb").read()
                ),
                "password": "12345678a",
            }
        )
        ...
        for doc_type in ("CreditNote", "DebitNote", "Invoice"):
            xsd_attachment = cls.env.ref(
                "l10n_pe_edi.UBL-%s-2.1.xsd" % doc_type, raise_if_not_found=False
            )
            if xsd_attachment:
    ...
    def _action_process_edi_web_services(self, moves):
        docs = moves.edi_document_ids.filtered(
