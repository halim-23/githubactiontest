...
class TestEdiXmls(TestPeEdiCommon):
    def test_invoice_simple_case(self):
        with freeze_time(self.frozen_today), patch(
            ...
            expected_etree = self.get_xml_tree_from_string(
                self.expected_invoice_xml_values
            )
    ...
    def test_refund_simple_case(self):
        with freeze_time(self.frozen_today), patch(
            ...
            current_etree = self.get_xml_tree_from_string(edi_xml)
            expected_etree = self.get_xml_tree_from_string(
                self.expected_refund_xml_values
            )
            self.assertXmlTreeEqual(current_etree, expected_etree)
    ...
    def test_debit_note_simple_case(self):
        with freeze_time(self.frozen_today), patch(
            ...
            current_etree = self.get_xml_tree_from_string(edi_xml)
            expected_etree = self.get_xml_tree_from_string(
                self.expected_debit_note_xml_values
            )
            self.assertXmlTreeEqual(current_etree, expected_etree)
    ...
    def test_invoice_payment_term_detraction_case(self):
        ...
        with freeze_time(self.frozen_today), patch(
            ...
            invoice = self._create_invoice(**update_vals_dict).with_context(
                edi_test_mode=True
            )
            invoice.action_post()
