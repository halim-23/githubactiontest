...
class HrEmployee(models.Model):
    ...
    def _check_insz_or_bis_number(self):
        for emp in self:
            if emp.insz_or_bis_number and (
                len(emp.insz_or_bis_number) != 11
                or not emp.insz_or_bis_number.isdigit()
            ):
...
class PosSession(models.Model):
    ...
    def _build_order_data_list_for_user_report(self):
        sorted_orders = sorted(
            self.order_ids, key=lambda o: o.employee_id.id or o.user_id.id
        )
