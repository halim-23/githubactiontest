...
class IoTController(http.Controller):
                    ...
                    zip_list.append(
                        (
                            modules.get_resource_path(
                                module, "iot_handlers", directory, file
                            ),
    ...
    def update_box(self, **kwargs):
        ...
        box = (
            request.env["iot.box"]
            .sudo()
            .search([("identifier", "=", iot_box["identifier"])], limit=1)
        )
        if box:
            box = box[0]
            box.ip = iot_box["ip"]
        ...
        if box:
            previously_connected_iot_devices = (
                ...
                .sudo()
                .search([("iot_id", "=", box.id), ("connected", "=", True)])
            )
            connected_iot_devices = request.env["iot.device"].sudo()
            for device_identifier in devices:
                available_types = [
                    s[0] for s in request.env["iot.device"]._fields["type"].selection
                ]
                available_connections = [
                    s[0]
                    for s in request.env["iot.device"]._fields["connection"].selection
                ]

                data_device = devices[device_identifier]
                if (
                    data_device["type"] in available_types
                    and data_device["connection"] in available_connections
                ):
                    if data_device["connection"] == "network":
                        device = (
                            request.env["iot.device"]
                            .sudo()
                            .search([("identifier", "=", device_identifier)])
                        )
                    else:
                        device = (
                            request.env["iot.device"]
            ...
            (previously_connected_iot_devices - connected_iot_devices).write(
                {"connected": False}
            )
