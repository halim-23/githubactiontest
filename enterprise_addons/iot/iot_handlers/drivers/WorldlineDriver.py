...
class WorldlineDriver(Driver):
        ...
        if result == 1:
            ...
            if error_code not in IGNORE_ERRORS:
                error_msg = "%s (Error code: %s)" % (
                    TERMINAL_ERRORS.get(
                        error_code, "Transaction was not processed correctly"
                    ),
                    error_code,
                )
    ...
    def cancelTransaction(self):
        ...
        result = easyCTEP.abortTransaction(
            ctypes.byref(self.dev), ctypes.byref(error_code)
        )
        ...
        if result:
            ...
            error_code = error_code.value.decode("utf-8")
            error_msg = "%s (Error code: %s)" % (
                TERMINAL_ERRORS.get(error_code, "Transaction could not be cancelled"),
                error_code,
            )
