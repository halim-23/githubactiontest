    ...
    def _action_default(self, data):
        try:
            ...
            v4l2 = subprocess.Popen(
                ["v4l2-ctl", "--list-formats-ext"], stdout=subprocess.PIPE
            )
            all_sizes = subprocess.Popen(
                ["grep", "Size"], stdin=v4l2.stdout, stdout=subprocess.PIPE
            )
            all_resolutions = subprocess.Popen(
                ["awk", "{print $3}"], stdin=all_sizes.stdout, stdout=subprocess.PIPE
            )
            sorted_resolutions = subprocess.Popen(
                ["sort", "-rn"], stdin=all_resolutions.stdout, stdout=subprocess.PIPE
            )
            resolution = subprocess.check_output(
                ["awk", "NR==1"], stdin=sorted_resolutions.stdout
            ).decode("utf-8")
