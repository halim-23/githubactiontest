  ...
  var IoTDeviceFormController = FormController.extend({
    ...
    saveRecord: function () {
      ...
      if (["keyboard", "scanner"].indexOf(this.renderer.state.data.type) >= 0) {
        return this._updateKeyboardLayout().then(
          self._processResult.bind(self, _super)
        );
    ...
    _updateKeyboardLayout: function () {
      ...
      var iot_device = new DeviceProxy(this, {
        iot_ip: this.renderer.state.data.iot_ip,
        identifier: this.renderer.state.data.identifier,
      });
