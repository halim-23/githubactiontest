odoo.define("iot.IoTLongpolling", function (require) {
  ...
  var IoTLongpolling = BusService.extend(IoTConnectionMixin, {
    ...
    removeListener: function (iot_ip, device_identifier, listener_id) {
      if (
        this._listeners[iot_ip].devices[device_identifier].listener_id === listener_id
      ) {
    ...
    action: function (iot_ip, device_identifier, data) {
      ...
      var prom = new Promise(function (resolve, reject) {
        self
          ._rpcIoT(iot_ip, self.ACTION_ROUTE, data, options)
          .then(resolve)
          .fail(reject);
      });
      return prom;
    ...
    _onError: function () {
      ...
      this._delayedStartPolling(
        Math.min(this.RPC_DELAY * this._retries, this.MAX_RPC_DELAY)
      );
    },
  });
