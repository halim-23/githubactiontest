...
class AddIotBox(models.TransientModel):
    ...
    def _default_token(self):
        ...
        iot_token = self.env["ir.config_parameter"].search(
            [("key", "=", "iot_token")], limit=1
        )
        if iot_token:
        ...
        db_uuid = (
            ...
            .search([("key", "=", "database.uuid")], limit=1)
            .value
            or ""
        )
        enterprise_code = (
            self.env["ir.config_parameter"]
            .search([("key", "=", "database.enterprise_code")], limit=1)
            .value
            or ""
        )
    ...
    def box_pairing(self):
        data = {
            "params": {
                ...
                "database_url": self.env["ir.config_parameter"]
                .sudo()
                .get_param("web.base.url"),
                "enterprise_code": self.env["ir.config_parameter"]
                .sudo()
                .get_param("database.enterprise_code"),
        ...
        try:
            ...
            raise UserError(
                _("We had troubles pairing your IoT Box. Please try again later.")
            )
        ...
        if "error" in response:
            if response["error"]["code"] == 404:
                raise UserError(
                    _(
                        "The pairing code you provided was not found in our system. Please check that you entered it correctly."
                    )
                )
            else:
