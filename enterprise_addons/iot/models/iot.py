...
class IotBox(models.Model):
    ...
    device_ids = fields.One2many(
        "iot.device", "iot_id", string="Devices", readonly=True
    )
...
class IotDevice(models.Model):
    ...
    iot_id = fields.Many2one(
        "iot.box", string="IoT Box", required=True, ondelete="cascade"
    )
    ...
    connected = fields.Boolean(
        string="Status", help="If device is connected to the IoT Box", readonly=True
    )
    keyboard_layout = fields.Many2one("iot.keyboard.layout", string="Keyboard Layout")
    display_url = fields.Char(
        "Display URL",
        help="URL of the page that will be displayed by the device, leave empty to use the customer facing display of the POS.",
    )
