    ...
    QUnit.module("form_renderer", {}, function () {
      ...
      QUnit.test("Message list loads new messages on scroll", async function (assert) {
        ...
        await this.afterEvent({
          ...
          message:
            "should wait until partner 11 thread loaded more messages after scrolling to bottom a first time",
          predicate: ({hint, threadViewer}) => {
            return (
        ...
        const listRect = document
          .querySelector(".o_Chatter_scrollPanel")
          .getBoundingClientRect();
        assert.ok(
          lastMessageRect.top > listRect.top &&
            lastMessageRect.bottom < listRect.bottom,
        ...
        await this.afterEvent({
          ...
          message:
            "should wait until partner 11 thread loaded more messages after scrolling to bottom a second time",
          predicate: ({hint, threadViewer}) => {
            return (
              hint.type === "more-messages-loaded" &&
              threadViewer.thread.model === "res.partner" &&
      ...
      QUnit.test(
        "Message list is scrolled to new message after posting a message",
        ...
        `,
          ...
          await this.afterEvent({
            ...
            message:
              "should wait until partner 11 thread scrolled to bottom after doing it manually",
