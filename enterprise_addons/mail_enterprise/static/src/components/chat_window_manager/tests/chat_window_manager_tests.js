...
QUnit.module("mail_enterprise", {}, function () {
  QUnit.module("components", {}, function () {
    QUnit.module("chat_window_manager", {}, function () {
      ...
      QUnit.test("'backbutton' event should close chat window", async function (
        assert
      ) {
        assert.expect(1);
      ...
      QUnit.test(
        "[technical] chat window should properly override the back button",
        async function (assert) {
          ...
          await afterNextRender(() =>
            document
              .querySelector(
                `.o_MessagingMenu_dropdownMenu .o_NotificationList_preview`
              )
              .click()
          );
          assert.verifySteps(
