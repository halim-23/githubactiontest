  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("Attachment on side", async function (assert) {
        ...
        assert.containsOnce(
          form,
          ".o_attachment_preview_img > img",
          "There should be an image for attachment preview"
        );
        assert.containsOnce(
          form,
          ".o_form_sheet_bg > .o_FormRenderer_chatterContainer",
        ...
        assert.containsOnce(
          form,
          ".o_form_sheet_bg + .o_attachment_preview",
        ...
        assert.containsOnce(
          form,
          ".o_attachment_preview_container > iframe",
      ...
      QUnit.test("Attachment on side on new record", async function (assert) {
        ...
        assert.strictEqual(
          form.$(".o_form_sheet_bg .o_attachment_preview").children().length,
          0,
      ...
      QUnit.test("Attachment on side not displayed on smaller screens", async function (
        assert
      ) {
        assert.expect(2);
        ...
        assert.strictEqual(
          form.$(".o_attachment_preview").children().length,
          0,
      ...
      QUnit.test("Attachment triggers list resize", async function (assert) {
        ...
        this.data.partner.fields.yeses = {
          relation: "yes",
          string: "Yeses",
          type: "many2many",
        };
        ...
        assert.notEqual(
          form.el.querySelector("table th").style.width,
          "0px",
