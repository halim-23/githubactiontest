...
class HrContractSalaryAdvantage(models.Model):
    ...
    def _get_binary_field_domain(self):
        return [("model", "=", "hr.contract"), ("ttype", "=", "binary")]
    ...
    cost_field = fields.Char(
        related="cost_res_field_id.name",
        string="Cost Field Name",
        readonly=True,
        compute_sudo=True,
    )
    ...
    advantage_type_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref(
            "hr_contract_salary.l10n_be_monthly_benefit", raise_if_not_found=False
        ),
    )
    ...
    fold_field = fields.Char(
        related="fold_res_field_id.name", string="Fold Field Name", readonly=True
    )
    ...
    manual_field = fields.Char(
        related="manual_res_field_id.name", string="Manual Field Name", readonly=True
    )
    country_id = fields.Many2one("res.country")
    structure_type_id = fields.Many2one(
        "hr.payroll.structure.type", string="Salary Structure Type", required=True
    )
    icon = fields.Char()
    ...
    hide_description = fields.Boolean(
        help="Hide the description if the advantage is not taken."
    )
    requested_documents_field_ids = fields.Many2many(
        "ir.model.fields", domain=_get_binary_field_domain, string="Requested Documents"
    )
    requested_documents = fields.Char(
        compute="_compute_requested_documents",
        string="Requested Documents Fields",
        compute_sudo=True,
    )
    uom = fields.Selection(
        [("days", "Days"), ("percent", "Percent"), ("currency", "Currency")],
        string="Advantage Unit of Measure",
        default="currency",
    )
    ...
    activity_type_id = fields.Many2one(
        ...
        help="The type of activity that will be created automatically on the contract if this advantage is chosen by the employee.",
    )
    activity_creation = fields.Selection(
        [
            ("countersigned", "Contract is countersigned"),
            ("running", "Employee signs his contract"),
        ],
        default="countersigned",
        help="The benefit is created when the employee signs his contract at the end of the salary configurator or when the HR manager countersigns the contract.",
    )
    activity_creation_type = fields.Selection(
        [
            ("always", "When the advantage is set"),
            ("onchange", "When the advantage is modified"),
        ],
        default="always",
        help="Choose whether to create a next activity each time that the advantage is taken by the employee or on modification only.",
    )
    ...
    sign_copy_partner_id = fields.Many2one(
        "res.partner",
        string="Send a copy to",
        help="Email address to which to transfer the signature.",
    )
    sign_frenquency = fields.Selection(
        [
            ("onchange", "When the advantage is set"),
            ("always", "When the advantage is modified"),
        ],
        default="onchange",
    )
    ...
    def _compute_requested_documents(self):
        for advantage in self:
            advantage.requested_documents = ",".join(
                advantage.requested_documents_field_ids.mapped("name")
            )
    ...
    def _check_min_inferior_to_max(self):
        for record in self:
            if (
                record.display_type == "slider"
                and record.slider_min > record.slider_max
            ):
...
class HrContractSalaryAdvantageValue(models.Model):
    ...
    color = fields.Selection(
        selection=[("green", "Green"), ("red", "Red")], string="Color", default="green"
    )
    hide_description = fields.Boolean()
