...
class HrContractSalaryPersonalInfo(models.Model):
    ...
    res_field_id = fields.Many2one(
        ...
        domain="[('model', '=', res_model), ('ttype', 'not in', ('one2many', 'many2one', 'many2many'))]",
        required=True,
        ondelete="cascade",
    ...
    info_type_id = fields.Many2one(
        "hr.contract.salary.personal.info.type", required=True
    )
    ...
    value_ids = fields.One2many(
        "hr.contract.salary.personal.info.value", "personal_info_id"
    )
