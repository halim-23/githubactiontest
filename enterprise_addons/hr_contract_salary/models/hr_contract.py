...
class HrContract(models.Model):
    ...
    origin_contract_id = fields.Many2one(
        "hr.contract",
        string="Origin Contract",
        domain="[('company_id', '=', company_id)]",
        help="The contract from which this contract has been duplicated.",
    )
    is_origin_contract_template = fields.Boolean(
        compute="_compute_is_origin_contract_template",
        string="Is origin contract a contract template ?",
        readonly=True,
    )
    ...
    contract_reviews_count = fields.Integer(
        compute="_compute_contract_reviews_count", string="Proposed Contracts Count"
    )
    ...
    sign_template_id = fields.Many2one(
        "sign.template",
        string="New Contract Document Template",
    ...
    wage_with_holidays = fields.Monetary(
        ...
        tracking=True,
        string="Wage with Holidays",
    )
    wage_on_signature = fields.Monetary(
        string="Wage on Payroll", help="Wage on contract signature", tracking=True
    )
    ...
    def _get_contract_wage_field(self):
        if (len(self) == 1 and self.structure_type_id.country_id.code == "BE") or (
            not len(self) and self.env.company.country_id.code == "BE"
        ):
    ...
    def _compute_is_origin_contract_template(self):
        for contract in self:
            contract.is_origin_contract_template = (
                contract.origin_contract_id
                and not contract.origin_contract_id.employee_id
            )
    ...
    def _get_yearly_cost(self, inverse=False):
        ...
        if inverse:
            return (
                self._get_advantages_costs()
                + self._get_salary_costs_factor() * self.wage_with_holidays
                + fixed
            ) / ratio
            ...
            if contract._is_salary_sacrifice():
                if (
                    abs(
                        ...
                        - contract._get_yearly_cost(inverse=True)
                    )
                    <= 0.10
                ):
    ...
    def _get_advantage_description(self, advantage, new_value=None):
        ...
        if hasattr(self, "_get_description_%s" % advantage.field):
            description = getattr(self, "_get_description_%s" % advantage.field)(
                new_value
            )
        else:
    ...
    def _get_advantage_fields(self):
        ...
        advantage_fields = {
            ...
            and (field.store or not field.store and field.name in nonstored_whitelist)
        }
    ...
    def _advantage_black_list(self):
        return set(
            MAGIC_COLUMNS + ["wage_with_holidays", "wage_on_signature", "active"]
        )
    ...
    def _compute_final_yearly_costs(self):
        for contract in self:
            contract.final_yearly_costs = (
                contract._get_advantages_costs()
                + contract._get_salary_costs_factor() * contract.wage
            )
    ...
    def _get_advantages_costs(self):
        ...
        if not advantages:
            return 0
        monthly_advantages = advantages.filtered(
            lambda a: a.advantage_type_id.periodicity == "monthly"
        )
        monthly_cost = sum(
            self[advantage.cost_field] if advantage.cost_field in self else 0
            for advantage in monthly_advantages
        )
        yearly_cost = sum(
            self[advantage.cost_field] if advantage.cost_field in self else 0
            for advantage in advantages - monthly_advantages
        )
    ...
    def _compute_signatures_count(self):
        for contract in self:
            contract.signatures_count = max(
                contract.sign_request_ids.mapped("nb_closed") or [0]
            )

    @api.depends("origin_contract_id")
    def _compute_contract_reviews_count(self):
        for contract in self:
            contract.contract_reviews_count = self.with_context(
                active_test=False
            ).search_count([("origin_contract_id", "=", contract.id)])
    ...
    def _get_redundant_salary_data(self):
        employees = self.mapped("employee_id").filtered(
            lambda employee: not employee.active
        )
    ...
    def _clean_redundant_salary_data(self):
        ...
        for records in records_to_unlink:
            if not records:
                continue
            _logger.info(
                "Salary: About to unlink %s: %s" % (records._name, records.ids)
            )
