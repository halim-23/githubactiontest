odoo.define("hr_contract_salary", function (require) {
  ...
  publicWidget.registry.SalaryPackageWidget = publicWidget.Widget.extend({
    selector: "#hr_cs_form",
    xmlDependencies: ["/hr_contract_salary/static/src/xml/resume_sidebar.xml"],
    ...
    init(parent, options) {
      ...
      $("#hr_cs_configurator .hr_cs_control input.folded:checked")
        .closest("div")
        .find(".folded_content")
        .removeClass("d-none");
    },
    ...
    willStart() {
      return Promise.all([this._super(), this.updateGross()]);
    },
    ...
    async getPersonalDocuments() {
      const documentNames = $("input[type='file']")
        .toArray()
        .map((input) => ({
          name: input.name,
    ...
    updateGrossToNetModal(data) {
      const resumeSidebar = $(
        qweb.render("hr_contract_salary.salary_package_resume", {
    ...
    onchangeFolded(event) {
      const foldedContent = $(event.target.parentElement.parentElement).find(
        ".folded_content"
      );
      ...
      checked
        ? $(foldedContent).removeClass("d-none")
        : $(foldedContent).addClass("d-none");
    },

    onchangeSlider(event) {
      const advantageField = event.target.name.replace("_slider", "");
      $("input[name='" + advantageField + "']").val(event.target.value);
    },

    onchangeCountry(event) {
      const countryID = parseInt(
        $("select[name='country_id'][applies-on='address']").val()
      );
      $("select[name='state_id']")
        ...
        .forEach((option) => {
          const $option = $(option);
          const stateCountryID = $option.data("additional-info");
    ...
    checkFormValidity() {
      const requiredEmptyInput = $("input:required")
        .toArray()
        .find(
          (input) =>
            input.value === "" && input.name !== "" && input.type !== "checkbox"
        );
      const requiredEmptySelect = $("select:required")
        .toArray()
      ...
      groups.some((group, index) => {
        ...
        if (!group.some((el) => el.checked)) {
          ...
          document
            .querySelector("section#hr_cs_personal_information")
      ...
      if (requiredEmptyInput || requiredEmptySelect) {
        ...
        $("section#hr_cs_personal_information")[0].scrollIntoView({
          block: "start",
          behavior: "smooth",
        });
      }
    ...
    async getFormInfo() {
      ...
      advantages = {
        ...
        bank_account: Object.assign(
          advantages.bank_account,
          personalDocuments.bank_account
        ),
      };
      ...
      return {
        ...
        employee_contract_id:
          parseInt($("input[name='employee_contract_id']").val()) || false,
      ...
      if (this.checkFormValidity()) {
        ...
        if (data.error) {
          ...
          document.location.pathname =
            "/sign/document/" + data.request_id + "/" + data.token;
        }
      }
    },
