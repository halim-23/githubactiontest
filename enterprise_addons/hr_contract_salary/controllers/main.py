...
class SignContract(Sign):
    ...
    def sign(
        ...
        result = super(SignContract, self).sign(
            sign_request_id,
            token,
            sms_token=sms_token,
            signature=signature,
            new_sign_items=new_sign_items,
        )
        request_item = (
        ...
        contract = (
            request.env["hr.contract"]
            .sudo()
            .with_context(active_test=False)
            .search([("sign_request_ids", "in", request_item.sign_request_id.ids)])
        )
    ...
    def _create_activity_advantage(self, contract, contract_state):
        advantages = (
            request.env["hr.contract.salary.advantage"]
            .sudo()
            .search(
                [
        ...
        for advantage in advantages:
            ...
            if (
                (
                    advantage.activity_creation_type == "onchange"
                    and contract[field] != contract.origin_contract_id[field]
                )
                or advantage.activity_creation_type == "always"
                and value
            ):
                contract.activity_schedule(
                    activity_type_id=advantage.activity_type_id.id,
                    note="%s: %s"
                    % (advantage.name or advantage.res_field_id.name, value),
            ...
            if (
                ...
                res = sign_request.initialize_new(
                    ...
                    signers=[
                        ...
                        {
                            "role": request.env.ref(
                                "hr_contract_sign.sign_item_role_job_responsible"
                            ).id,
                    ...
                    reference=_(
                        "Signature Request - %s", advantage.name or contract.name
                    ),
                    subject=_(
                        "Signature Request - %s", advantage.name or contract.name
                    ),
                ...
                sign_request.message_subscribe(
                    partner_ids=advantage.sign_copy_partner_id.ids
                )
...
class HrContractSalary(http.Controller):
    def _check_access_rights(self, contract_id):
        ...
        if (
            not contract_sudo.employee_id
            or contract_sudo.employee_id.user_id == request.env.user
        ):
    ...
    @http.route(
        ["/salary_package/simulation/contract/<int:contract_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def salary_package(self, contract_id=None, **kw):
        ...
        if not request.env.user.has_group("hr_contract.group_hr_contract_manager"):
            if kw.get("applicant_id"):
                applicant = (
                    ...
                    .sudo()
                    .browse(int(kw.get("applicant_id")))
                )
                if (
                    not kw.get("token")
                    or not applicant.access_token
                    or not consteq(applicant.access_token, kw.get("token"))
                    or applicant.access_token_end_date < fields.Date.today()
                ):
                    return request.render(
                        "http_routing.http_error",
                        {
            ...
            if (
                ...
                and not contract.employee_id.user_id
                and not kw.get("applicant_id")
            ):
                return request.render(
                    "http_routing.http_error",
                    {
        ...
        if kw.get("employee_contract_id"):
            employee_contract = (
                request.env["hr.contract"]
                .sudo()
                .browse(int(kw.get("employee_contract_id")))
            )
            if (
                not request.env.user.has_group("hr_contract.group_hr_contract_manager")
                and employee_contract.employee_id
        ...
        values.update(
            {
                ...
                "default_mobile": request.env["ir.default"]
                .sudo()
                .get("hr.contract", "mobile"),
    ...
    @http.route(
        ["/salary_package/thank_you/<int:contract_id>"],
        type="http",
        auth="public",
        website=True,
    )
    def salary_package_thank_you(self, contract_id=None, **kw):
        contract = request.env["hr.contract"].sudo().browse(contract_id)
        return request.render(
            "hr_contract_salary.salary_package_thank_you",
            {
                "responsible_name": contract.hr_responsible_id.partner_id.name
                or contract.job_id.user_id.partner_id.name,
                "responsible_email": contract.hr_responsible_id.partner_id.email
                or contract.job_id.user_id.partner_id.email,
                "responsible_phone": contract.hr_responsible_id.partner_id.phone
                or contract.job_id.user_id.partner_id.phone,
            },
        )
    ...
    def _get_personal_infos(self, contract):
        ...
        # PERSONAL INFOS
        personal_infos = (
            request.env["hr.contract.salary.personal.info"]
            .sudo()
            .search(
                [
                    "|",
            ...
            .sorted(lambda info: (info.info_type_id.sequence, info.sequence))
        )
        ...
        for personal_info in personal_infos:
            if personal_info.position == "left":
                mapped_personal_infos[0][
                    personal_info.info_type_id.name
                ] |= personal_info
            else:
            ...
            if personal_info.display_type == "document":
                ...
                initial_values[personal_info.field] = (
                    target[personal_info.field] if personal_info.field in target else ""
                )
            ...
            if personal_info.display_type == "dropdown":
                ...
                if personal_info.dropdown_selection == "specific":
                    values = [
                        (value.value, value.name) for value in personal_info.value_ids
                    ]
                elif personal_info.dropdown_selection == "country":
                    values = [
                        (country.id, country.name)
                        for country in self._get_personal_infos_countries(
                            contract, personal_info
                        )
                    ]
                elif personal_info.dropdown_selection == "state":
                    values = [
                        (state.id, state.name, state.country_id.id)
                        for state in self._get_personal_infos_states(
                            contract, personal_info
                        )
                    ]
                    ...
                    values = [
                        (lang.code, lang.name)
                        for lang in self._get_personal_infos_langs(
                            contract, personal_info
                        )
                    ]
    ...
    def _get_advantages(self, contract):
        return (
            request.env["hr.contract.salary.advantage"]
            .sudo()
            .search([("structure_type_id", "=", contract.structure_type_id.id)])
        )
    ...
    def _get_advantages_values(self, contract):
        ...
        mapped_advantages = defaultdict(
            lambda: request.env["hr.contract.salary.advantage"]
        )
        for advantage in advantages:
            ...
            if advantage.folded:
                ...
                initial_values[fold_field] = (
                    contract[advantage_fold_field]
                    if advantage_fold_field and advantage_fold_field in contract
                    else 0
                )
            ...
            if advantage.display_type == "manual":
                ...
                initial_values[manual_field] = (
                    contract[field] if field and field in contract else 0
                )
            elif (
                advantage.display_type == "dropdown"
                or advantage.display_type == "dropdown-group"
            ):
                initial_values["select_%s" % field] = contract[field]

        dropdown_advantages = advantages.filtered(
        ...
        for dropdown_advantage in dropdown_advantages:
            dropdown_options[dropdown_advantage.field] = [
                ...
                for value in dropdown_advantage.value_ids.filtered(
                    lambda v: v.display_type == "line"
                )
            ]
        dropdown_group_advantages = advantages.filtered(
        ...
        advantage_types = sorted(
            advantages.mapped("advantage_type_id"), key=lambda x: x.sequence
        )
        return (
            mapped_advantages,
            advantage_types,
    ...
    def _get_salary_package_values(self, contract):
        (
            ...
            dropdown_options_1,
            initial_values_1,
        ) = self._get_personal_infos(contract)
        (
            mapped_advantages,
            advantage_types,
            dropdown_options_2,
            dropdown_group_options,
            initial_values_2,
        ) = self._get_advantages_values(contract)
        all_initial_values = {**initial_values_1, **initial_values_2}
        all_initial_values = {
            key: round(value, 2) if isinstance(value, float) else value
            for key, value in all_initial_values.items()
        }
        ...
        for advantage in contract_advantages:
            ...
            if hasattr(contract, "_get_advantage_values_%s" % (advantage.field)):
                contract_vals.update(
                    getattr(contract, "_get_advantage_values_%s" % (advantage.field))(
                        contract, advantages
                    )
                )
                continue
            if advantage.folded:
                contract_vals[advantage.fold_field or advantage.field] = advantages[
                    "fold_%s" % (advantage.field)
                ]
            if advantage.display_type == "dropdown":
                contract_vals[advantage.field] = advantages[advantage.field]
            if advantage.display_type == "manual":
                contract_vals[advantage.manual_field or advantage.field] = advantages[
                    "%s_manual" % (advantage.field)
                ]
            else:
                contract_vals[advantage.field] = advantages[advantage.field]
        return contract_vals

    def _update_personal_info(
        self, employee, contract, personal_infos_values, no_name_write=False
    ):
        def resolve_value(field_name, values):
            ...
            target = targets[personal_info.applies_on]
            if field_name in target and isinstance(
                target[field_name], models.BaseModel
            ):
        ...
        for personal_info in personal_infos:
            ...
            if personal_info.display_type == "document" and not employee_infos.get(
                field_name
            ):
                continue
            ...
            if field_name in employee_infos and personal_info.applies_on == "employee":
                ...
                field_name in bank_account_infos
                and personal_info.applies_on == "bank_account"
            ):
        ...
        existing_bank_account = (
            ...
            .sudo()
            .search([("acc_number", "=", bank_account_vals["acc_number"])], limit=1)
        )
        if existing_bank_account:
            bank_account = existing_bank_account
        else:
            bank_account = (
                request.env["res.partner.bank"].sudo().create(bank_account_vals)
            )
    ...
    def create_new_contract(self, contract, advantages, no_write=False, **kw):
        ...
        applicant = (
            request.env["hr.applicant"].sudo().browse(kw.get("applicant_id")).exists()
        )
        ...
        if not employee and applicant:
            existing_contract = (
                ...
                .with_context(active_test=False)
                .search(
                    [("applicant_id", "=", applicant.id), ("employee_id", "!=", False)],
                    limit=1,
                )
            )
        ...
        new_contract = (
            request.env["hr.contract"]
            .sudo()
            .new(self._get_new_contract_values(contract, employee, contract_values))
        )
    ...
    def _get_compute_results(self, new_contract):
        ...
        uoms = {
            "days": _("Days"),
            "percent": "%",
            "currency": new_contract.company_id.currency_id.symbol,
        }
        ...
        for resume_line in resume_lines - monthly_total_lines:
            ...
            if resume_line.value_type == "contract":
                value = (
                    new_contract[resume_line.code]
                    if resume_line.code in new_contract
                    else 0
                )
            if resume_line.value_type == "sum":
                resume_explanation = _(
                    "Equals to the sum of the following values:\n\n%s",
                    "\n+ ".join(
                        resume_line.advantage_ids.res_field_id.mapped(
                            "field_description"
                        )
                    ),
                )
                for advantage in resume_line.advantage_ids:
                    if not advantage.fold_field or (
                        advantage.fold_field and new_contract[advantage.fold_field]
                    ):
                        field = advantage.field
            ...
            if resume_line.impacts_monthly_total:
                monthly_total += (
                    value / 12.0
                    if resume_line.category_id.periodicity == "yearly"
                    else value
                )
            try:
                ...
                pass
            result["resume_lines_mapped"][resume_line.category_id.name][
                resume_line.code
            ] = (resume_line.name, value, uom, resume_explanation)
        for resume_line in monthly_total_lines:
            result["resume_lines_mapped"][resume_line.category_id.name][
                resume_line.code
            ] = (
                resume_line.name,
                round(float(monthly_total), 2),
                uoms["currency"],
                resume_explanation,
            )
        return result
    ...
    def onchange_advantage(self, advantage_field, new_value, contract_id, advantages):
        ...
        if hasattr(contract, "_get_description_%s" % advantage_field):
            description = getattr(contract, "_get_description_%s" % advantage_field)(
                new_value
            )
        else:
    ...
    def _get_email_info(self, contract, **kw):
        field_names = {
            model: {
                ...
                for field in request.env["ir.model.fields"]
                .sudo()
                .search([("model", "=", model)])
            }
            for model in [
        ...
        contract_info = {
            advantage_type.name: []
            for advantage_type in sorted(
                contract_advantages.mapped("advantage_type_id"),
        ...
        for advantage in contract_advantages:
            if advantage.folded and advantage.fold_field:
                ...
                contract_info[advantage.advantage_type_id.name].append(
                    (field_names["hr.contract"][advantage.fold_field], value)
                )
        ...
        personal_infos = {
            ...
            for personal_info_type in sorted(
    ...
    def send_email(self, contract, **kw):
        ...
        request.env[model].sudo().browse(res_id).message_post_with_view(
            "hr_contract_salary.hr_contract_salary_email_template", values=values
        )
        return contract.id
    ...
    def submit(self, contract_id=None, advantages=None, **kw):
        ...
        if kw.get("employee_contract_id", False):
            contract = (
                request.env["hr.contract"].sudo().browse(kw.get("employee_contract_id"))
            )
        ...
        new_contract = self.create_new_contract(
            contract, advantages, no_write=True, **kw
        )
        ...
        if applicant and kw.get("token", False):
            ...
            not applicant
            and contract.employee_id.user_id
            and contract.employee_id.user_id == request.env.user
            and kw.get("original_link", False)
        ):
            hash_token_access = hashlib.sha1(
                kw.get("original_link").encode("utf-8")
            ).hexdigest()
            existing_contract = (
                request.env["hr.contract"]
                .sudo()
        ...
        sign_template = (
            new_contract.contract_update_template_id
            if kw.get("employee_contract_id")
            else new_contract.sign_template_id
        )
        if not sign_template:
            return {
                "error": 1,
                "error_msg": _(
                    "No signature template defined on the contract. Please contact the HR responsible."
                ),
            }
        if not new_contract.hr_responsible_id:
            return {
                "error": 1,
                "error_msg": _(
                    "No HR responsible defined on the job position. Please contact an administrator."
                ),
            }
        ...
        items = (
            request.env["sign.item"]
            .sudo()
            .search([("template_id", "=", sign_template.id), ("name", "!=", "")])
        )
        for item in items:
            ...
            .search(
                [
                    ...
                    (
                        "role_id",
                        "=",
                        request.env.ref("sign.sign_item_role_employee").id,
                    ),
                ]
            )
            .access_token
