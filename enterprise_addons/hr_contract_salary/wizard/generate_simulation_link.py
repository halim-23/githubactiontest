...
class GenerateSimulationLink(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if not set(fields) & {
            "contract_id", "employee_id", "employee_contract_id", "applicant_id"
        }:
            return result
        ...
        if model == "hr.contract":
            ...
            if (
                not applicant.access_token
                or applicant.access_token_end_date < Date.today()
            ):
                applicant.access_token = uuid.uuid4().hex
                applicant.access_token_end_date = self.env[
                    "hr.applicant"
                ]._get_access_token_end_date()
        ...
        if (
            not result.get("applicant_id")
            and not contract.contract_update_template_id
            or result.get("applicant_id")
            and not contract.sign_template_id
        ):
    ...
    employee_contract_employee_id = fields.Many2one(
        related="employee_contract_id.employee_id", string="contract employee"
    )
    ...
    job_title = fields.Char(
        "Job Title", compute="_compute_from_contract_id", store=True, readonly=False
    )
    ...
    email_to = fields.Char(
        "Email To", compute="_compute_email_to", store=True, readonly=False
    )
    ...
    def _compute_url(self):
        for wizard in self:
            url = (
                wizard.contract_id.get_base_url()
                + "/salary_package/simulation/contract/%s?" % (wizard.contract_id.id)
            )
            params = {}
            for trigger in self._get_url_triggers():
                if wizard[trigger]:
                    params[trigger] = (
                        wizard[trigger].id
                        if isinstance(wizard[trigger], models.BaseModel)
                        else wizard[trigger]
                    )
    ...
    def _compute_from_contract_id(self):
        for wizard in self:
            ...
            if not wizard.job_title or (
                wizard.applicant_id
                and wizard.applicant_id.job_id
                and wizard.job_title != wizard.applicant_id.job_id.name
            ):
                wizard.job_title = (
                    wizard.contract_id.employee_id.job_title
                    or wizard.contract_id.job_id.name
                )
    ...
    def send_offer(self):
        try:
            ...
            template_id = False
        try:
            template_applicant_id = self.env.ref(
