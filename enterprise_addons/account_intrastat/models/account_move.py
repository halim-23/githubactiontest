...
class AccountMoveLine(models.Model):
    ...
    def _auto_init(self):
        if not column_exists(
            self.env.cr, "account_move_line", "intrastat_product_origin_country_id"
        ):
            create_column(
                self.env.cr,
                "account_move_line",
    ...
    intrastat_product_origin_country_id = fields.Many2one(
        ...
        compute="_compute_origin_country",
        store=True,
        readonly=False,
    )
    ...
    def _compute_origin_country(self):
        for line in self:
            line.intrastat_product_origin_country_id = (
                line.product_id.product_tmpl_id.intrastat_origin_country_id
            )
