...
class ProductCategory(models.Model):
    ...
    def search_intrastat_code(self):
        ...
        return (
            self.intrastat_id
            or (self.parent_id and self.parent_id.search_intrastat_code())
            or self.intrastat_id
        )
...
class ProductProduct(models.Model):
    ...
    intrastat_variant_id = fields.Many2one(
        comodel_name="account.intrastat.code",
        string="Variant commodity Code",
        domain="[('type', '=', 'commodity')]",
    )
    ...
    def _compute_intrastat_id(self):
        ...
        for product in self:
            product.intrastat_id = (
                product.intrastat_variant_id or product.product_tmpl_id.intrastat_id
            )
