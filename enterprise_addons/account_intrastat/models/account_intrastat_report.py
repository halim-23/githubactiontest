...
_merchandise_export_code = {"BE": "29", "FR": "21"}

_merchandise_import_code = {"BE": "19", "FR": "11"}
...
class IntrastatReport(models.AbstractModel):
    ...
    def _decode_options(self, options):
        ...
        return (
            ...
            incl_arrivals,
            incl_dispatches,
            options.get("intrastat_extended"),
            options.get("with_vat"),
        )

    @api.model
    def _prepare_query(
        self, date_from, date_to, journal_ids, invoice_types=None, with_vat=False
    ):
        query_blocks, params = self._build_query(
            date_from,
            date_to,
            journal_ids,
        ...
        query = (
            "SELECT %(select)s FROM %(from)s WHERE %(where)s ORDER BY %(order)s"
            % query_blocks
        )
        return query, params
    ...
    def _build_query(
        ...
        params = {
            ...
            "import_merchandise_code": _merchandise_import_code.get(
                self.env.company.country_id.code, "29"
            ),
            "export_merchandise_code": _merchandise_export_code.get(
                self.env.company.country_id.code, "19"
            ),
            "date_from": date_from,
            "date_to": date_to,
            "journal_ids": tuple(journal_ids),
            "weight_category_id": self.env["ir.model.data"]._xmlid_to_res_id(
    ...
    def _fill_missing_values(self, vals_list):
        ...
        category_ids = self.env["product.category"].browse(
            {vals["category_id"] for vals in vals_list}
        )
        self.env["product.category"].search(
            [("id", "parent_of", category_ids.ids)]
        ).read(["intrastat_id", "parent_id"])
        ...
        for vals in vals_list:
            ...
            if currency_id != company_currency_id:
                vals["value"] = currency_id._convert(
                    vals["value"],
                    company_currency_id,
                    self.env.company,
                    vals["invoice_date"],
