...
class StockPicking(models.Model):
    ...
    def _compute_state(self):
        ...
        tracked_pickings = self.filtered(
            lambda m: m.state == "done" and m.state != previous_states[m]
        )
        ticket_ids = (
            ...
            .search(
                [
                    ...
                    ("picking_ids", "in", tracked_pickings.ids),
                ]
            )
        )
        if ticket_ids:
            ...
            for ticket, pickings in mapped_data.items():
                ...
                body = "</br>".join(
                    (
                        '<a href="#" data-oe-model="stock.picking" data-oe-id="%s">%s</a>'
