...
class TestSocialMultiCompany(common.SocialCase):
    ...
    def test_allowed_company_ids(self):
        ...
        self.assertEqual(
            self.account_1 | self.account_2 | self.account_3, post_1.account_allowed_ids
        )
        ...
        with self.assertRaises(
            ValidationError,
            msg="Should not be able to add a social account of an other company",
        ):
    ...
    def test_social_account_acls(self):
        ...
        with self.assertRaises(
            AccessError,
            msg="Should not be able to read the account of an other company",
        ):
    ...
    def test_social_post_click_count(self):
        ...
        live_post_1 = post.live_post_ids.filtered(
            lambda l: l.company_id == self.company_1
        )
        live_post_2 = post.live_post_ids.filtered(
            lambda l: l.company_id == self.company_2
        )
        ...
        self.env["mail.render.mixin"].sudo()._shorten_links_text(
            post.message, live_post_1._get_utm_values()
        )
        link_tracker_1 = self.env["link.tracker"].search(
            [("url", "=", url), ("medium_id", "=", self.account_1.utm_medium_id.id)]
        )

        self.env["mail.render.mixin"].sudo()._shorten_links_text(
            post.message, live_post_2._get_utm_values()
        )
        link_tracker_2 = self.env["link.tracker"].search(
            [("url", "=", url), ("medium_id", "=", self.account_2.utm_medium_id.id)]
        )
        ...
        self.env["link.tracker.click"].sudo().create(
            [{"link_id": link_tracker_1.id} for _ in range(13)]
        )
        # 7 clicks on the first account, in the first company
        self.env["link.tracker.click"].sudo().create(
            [{"link_id": link_tracker_2.id} for _ in range(7)]
        )
        ...
        post.invalidate_cache()
        self.assertEqual(
            post.click_count, 20, "There should be 20 clicks across all the companies"
        )
        post.invalidate_cache()
        self.assertEqual(
            post.with_company(self.company_1).click_count,
            13,
            "There should be 13 clicks for a user who is only in the first company",
        )
        post.invalidate_cache()
        self.assertEqual(
            post.with_company(self.company_2).click_count,
            7,
            "There should be 7 clicks for a user who is only in the second company",
        )
    ...
    def test_social_stream_post_acls(self):
        ...
        result = (
            self.env["social.stream.post"].with_user(self.social_manager).search([])
        )
