...
class SocialCase(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        attachments = [
            {"name": "first.png", "datas": "ABCDEFG="},
            {"name": "second.png", "datas": "GFEDCBA="},
        ]
    ...
    def _get_social_account(cls):
        return cls.env["social.account"].create(
            {"media_id": cls._get_social_media().id, "name": "Social Account 1"}
        )

    @classmethod
    def _get_post_social_accounts(cls):
        return cls.social_account | cls.env["social.account"].create(
            {"media_id": cls._get_social_media().id, "name": "Social Account 2"}
        )

    def _checkPostedStatus(self, success):
        live_posts = self.env["social.live.post"].search(
        ...
        self.assertTrue(
            all(
                live_post.state == "posted" if success else "failed"
