odoo.define("social.social_stream_post_kanban_controller", function (require) {
  ...
  var StreamPostKanbanController = KanbanController.extend({
    events: _.extend({}, KanbanController.prototype.events, {
      "click .o_social_stream_post_image_more, .o_social_stream_post_image_click":
        "_onClickMoreImages",
    ...
    willStart: function () {
      ...
      var isSocialManagerPromise = this.getSession()
        .user_has_group("social.group_social_manager")
    ...
    renderButtons: function ($node) {
      this.$buttons = $(
        QWeb.render("StreamPostKanbanView.buttons", {
          hasAccounts: this.hasAccounts,
        })
      );
      this.$buttons.on(
        "click",
        ".o_stream_post_kanban_new_post",
        this._onNewPost.bind(this)
      );
      this.$buttons.on(
        "click",
        ".o_stream_post_kanban_new_stream",
        this._onNewStream.bind(this)
      );
      this.$buttons.on(
        "click",
        ".o_stream_post_kanban_refresh_now",
        this._onRefreshNow.bind(this)
      );
      this.updateButtons();
      if ($node) {
    ...
    _onNewStream: function (ev) {
      ...
      if (this.renderer.state.socialAccountsStats.length > 0 || this.isSocialManager) {
        ...
        this.displayNotification({
          message: _t(
            "No social accounts configured, please contact your administrator."
          ),
          type: "danger",
        });
      }
    ...
    _onRelinkAccount: function (ev) {
      ...
      if (this.isSocialManager) {
        ...
        this.displayNotification({
          message: _t(
            "Sorry, you're not allowed to re-link this account, please contact your administrator."
          ),
          type: "danger",
        });
      }
    ...
    _onRefreshNow: function () {
      ...
      this.$buttons
        .find(".o_stream_post_kanban_refresh_now .fa-circle-o-notch")
        .removeClass("d-none");
      Promise.all([
    ...
    _updateLikesCount: function ($target) {
      ...
      $target
        .find(".o_social_kanban_likes_count")
        .text(utils.insert_thousand_seps(likesCount.toString()));
    },
    ...
    _getCompanies: function () {
      ...
      return session.user_context.allowed_company_ids.map(
