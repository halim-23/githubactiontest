...
class UtmCampaign(models.Model):
    ...
    social_post_ids = fields.One2many(
        "social.post",
        "utm_campaign_id",
        string="All related social media posts",
        groups="social.group_social_user",
    )
    social_posts_count = fields.Integer(
        compute="_compute_social_posts_count",
        string="Social Media Posts",
        groups="social.group_social_user",
    )
    social_engagement = fields.Integer(
        compute="_compute_social_engagement",
        string="Number of interactions (likes, shares, comments ...) with the social posts",
        groups="social.group_social_user",
    )
    ...
    def _compute_social_engagement(self):
        ...
        posts_data = self.env["social.post"].search_read(
            [("utm_campaign_id", "in", self.ids)], ["utm_campaign_id", "engagement"]
        )
    ...
    def _compute_social_posts_count(self):
        domain = expression.AND(
            [self._get_social_posts_domain(), [("utm_campaign_id", "in", self.ids)]]
        )
        ...
        mapped_data = {
            datum["utm_campaign_id"][0]: datum["utm_campaign_id_count"]
    ...
    def action_create_new_post(self):
        ...
        action["context"] = {
            ...
            "default_account_ids": self.env["social.account"]
            .search(self._get_social_media_accounts_domain())
            .ids,
        }
        return action
