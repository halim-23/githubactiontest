    ...
    def _compute_message(self):
        ...
        for live_post in self:
            message = (
                ...
                .sudo()
                ._shorten_links_text(
                    live_post.post_id.message, live_post._get_utm_values()
                )
            )
            ...
            live_post.message = self.env["social.post"]._prepare_post_content(
                ...
                **{
                    ...
                    for field in self.env[
                        "social.post"
                    ]._get_post_message_modifying_fields()
                }
            )
    ...
    def name_get(self):
        ...
        return [
            (
                ...
                "%s: %s"
                % (
                    live_post.account_id.display_name,
                    state_description_values.get(live_post.state),
                ),
            )
            for live_post in self
        ]
