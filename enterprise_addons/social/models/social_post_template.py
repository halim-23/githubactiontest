...
class SocialPostTemplate(models.Model):
    ...
    has_active_accounts = fields.Boolean(
        "Are Accounts Available?", compute="_compute_has_active_accounts"
    )
    ...
    def _check_message_not_empty(self):
        for post in self:
            if not post.message:
                raise UserError(
                    _("The 'message' field is required for post ID %s", post.id)
                )
    ...
    def _compute_account_ids(self):
        ...
        for post in self:
            accounts = all_account_ids.filtered_domain(
                post._get_default_accounts_domain()
            )
    ...
    def _prepare_post_content(self, message, media_type, **kw):
        ...
        if media_type not in [
            key
            for (key, val) in self.env["social.media"].fields_get(["media_type"])[
                "media_type"
            ]["selection"]
        ]:
