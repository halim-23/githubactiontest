...
class SocialPost(models.Model):
    ...
    has_post_errors = fields.Boolean(
        "There are post errors on sub-posts", compute="_compute_has_post_errors"
    )
    account_ids = fields.Many2many(domain="[('id', 'in', account_allowed_ids)]")
    account_allowed_ids = fields.Many2many(
        "social.account",
    ...
    post_method = fields.Selection(
        [("now", "Send now"), ("scheduled", "Schedule later")],
        string="When",
        default="now",
        required=True,
    ...
    utm_source_id = fields.Many2one(
        "utm.source", string="UTM Source", readonly=True, required=True
    )
    # Statistics
    stream_posts_count = fields.Integer(
        "Feed Posts Count",
        compute="_compute_stream_posts_count",
        help="Number of linked Feed Posts",
    )
    engagement = fields.Integer(
        "Engagement",
        compute="_compute_post_engagement",
            ...
            if not (post.account_ids <= post.account_allowed_ids):
                raise ValidationError(
                    _(
                        ...
                        ",".join(
                            (post.account_ids - post.account_allowed_ids).mapped("name")
                        ),
    ...
    def _compute_post_engagement(self):
        ...
        engagement_per_post = {
            result["post_id"][0]: result["engagement_total"] for result in results
        }
        for post in self:
    ...
    def _compute_account_allowed_ids(self):
        ...
        for post in self:
            post.account_allowed_ids = all_account_allowed_ids.filtered_domain(
                post._get_company_domain()
            )

    @api.depends("live_post_ids.state")
    def _compute_has_post_errors(self):
        for post in self:
            post.has_post_errors = any(
                live_post.state == "failed" for live_post in post.live_post_ids
            )

    @api.depends("account_ids.media_id")
    def _compute_media_ids(self):
        for post in self:
            post.media_ids = post.with_context(active_test=False).account_ids.mapped(
                "media_id"
            )
    ...
    def _compute_calendar_date(self):
        for post in self:
            post.calendar_date = (
                post.published_date if post.state == "posted" else post.scheduled_date
            )
    ...
    def _compute_live_posts_by_media(self):
        ...
        for post in self:
            accounts_by_media = {
                media_id.id: list() for media_id in post.media_ids
            }
            for live_post in post.live_post_ids.filtered(
                lambda lp: lp.account_id.media_id.ids
            ):
                accounts_by_media[live_post.account_id.media_id.id].append(
    ...
    def _compute_click_count(self):
        ...
        if not self.utm_source_id.ids or not medium_ids.ids:
            ...
            self.env.cr.execute(
                query, [tuple(self.utm_source_id.ids), tuple(medium_ids.ids)]
            )
            click_data = self.env.cr.dictfetchall()
            mapped_data = {
                datum["source_id"]: datum["click_count"] for datum in click_data
            }
            for post in self:
    ...
    def default_get(self, fields):
        ...
        if default_calendar_date and (
            "post_method" in fields or "scheduled_date" in fields
        ):
            result.update(
                {"post_method": "scheduled", "scheduled_date": default_calendar_date}
            )
        return result
    ...
    def create(self, vals_list):
        ...
        if (
            ...
            and not self.user_has_groups("social.group_social_manager")
            and any(vals.get("state", "draft") != "draft" for vals in vals_list)
        ):
            raise AccessError(
                _(
                    'You are not allowed to create/update posts in a state other than "Draft".'
                )
            )
        ...
        cron_trigger_dates = {
            post.scheduled_date for post in res if post.scheduled_date
        }
    ...
    def write(self, vals):
        if (
            ...
            and (
                ...
                or any(post.state != "draft" for post in self)
            )
        ):
            raise AccessError(
                _(
    ...
    def social_stream_post_action_my(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "social.action_social_stream_post"
        )
    ...
    def _check_post_access(self):
        ...
        """
        if not self.env.is_admin() and not self.user_has_groups(
    ...
    def action_post(self):
        ...
        self.write({"post_method": "now", "scheduled_date": False})
    ...
    def _action_post(self):
        ...
        for post in self:
            ...
            failed_posts.write(
                {"state": "failed", "failure_reason": _("Unknown error")}
            )
