...
class SocialMedia(models.Model):
    ...
    can_link_accounts = fields.Boolean(
        "Can link accounts ?",
        default=True,
        readonly=True,
        required=True,
    ...
    stream_type_ids = fields.One2many(
        "social.stream.type", "media_id", string="Stream Types"
    )
    ...
    def _compute_csrf_token(self):
        for media in self:
            media.csrf_token = hmac(
                self.env(su=True), "social_social-account-csrf-token", media.id
            )
