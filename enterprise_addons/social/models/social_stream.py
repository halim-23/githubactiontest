...
class SocialStream(models.Model):
    ...
    media_image = fields.Binary(
        related="media_id.image", string="The related Social Media's image"
    )
    sequence = fields.Integer(
        "Sequence",
        help="Sequence used to order streams (mainly for the 'Feed' kanban view)",
    )
    account_id = fields.Many2one(
        "social.account", "Social Account", required=True, ondelete="cascade"
    )
    stream_type_id = fields.Many2one(
        "social.stream.type", string="Type", required=True, ondelete="cascade"
    )
    ...
    company_id = fields.Many2one(
        "res.company", "Company", related="account_id.company_id", store=True
    )
