  ...
  QUnit.module(
    ...
    function () {
      QUnit.test("Check accounts statistics", async function (assert) {
        ...
        assert.containsN(
          kanban,
          ".o_social_stream_stat_box small:contains('0%')",
          3,
        ...
        assert.containsOnce(
          kanban,
          ".o_social_stream_stat_box b:contains('519')",
          "Audience is correctly displayed."
        );

        assert.containsOnce(
          kanban,
          ".o_social_stream_stat_box small:contains('50%')",
      ...
      QUnit.test("Check messages display", async function (assert) {
        ...
        var kanban = await createView({
          ...
          mockRPC: function (route, params) {
            if (
              params.method === "refresh_all" ||
              params.method === "refresh_statistics"
            ) {
              return Promise.resolve({});
            } else if (route.startsWith("https://graph.facebook.com/")) {
              return Promise.resolve("");
      ...
      QUnit.test("Check comments behavior", async function (assert) {
        ...
        await testUtils.dom.click(
          kanban.$(".o_social_stream_post_facebook_stats:first .fa-comments")
        );
        ...
        await testUtils.fields.editInput(
          $(".o_social_write_reply:first .o_social_add_comment"),
          "New Comment"
        );
        ...
        await testUtils.fields.editInput(
          $(".o_social_comment:contains('Root Comment') .o_social_add_comment"),
          "New Reply"
        );
        await testUtils.fields.triggerKeydown(
          $(".o_social_comment:contains('Root Comment') .o_social_add_comment"),
