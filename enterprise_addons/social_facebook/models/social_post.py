...
class SocialPostFacebook(models.Model):
    ...
    def _get_stream_post_domain(self):
        ...
        facebook_post_ids = [
            facebook_post_id
            for facebook_post_id in self.live_post_ids.mapped("facebook_post_id")
            if facebook_post_id
        ]
        if facebook_post_ids:
            return expression.OR(
                [domain, [("facebook_post_id", "in", facebook_post_ids)]]
            )
        else:
            return domain
    ...
    def _format_images_facebook(self, facebook_account_id, facebook_access_token):
        ...
        for image in self.image_ids:
            facebook_photo_endpoint_url = url_join(
                self.env["social.media"]._FACEBOOK_ENDPOINT,
                "/v10.0/%s/photos" % facebook_account_id,
            )

            post_result = requests.request(
                "POST",
                facebook_photo_endpoint_url,
            ...
            if post_result.ok:
                ...
                generic_api_error = (
                    json.loads(post_result.text or "{}")
                    .get("error", {})
                    .get("message", "")
                )
                raise UserError(
