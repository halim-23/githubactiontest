...
class SocialAccountFacebook(models.Model):
    ...
    def _compute_stats_link(self):
        ...
        for account in facebook_accounts:
            account.stats_link = (
                "https://www.facebook.com/%s/insights" % account.facebook_account_id
            )
    ...
    def _compute_statistics(self):
        ...
        for account in facebook_accounts:
            insights_endpoint_url = url_join(
                ...
                "/v10.0/%s/insights" % account.facebook_account_id,
            )
            statistics_30d = account._compute_statistics_facebook(insights_endpoint_url)
            statistics_360d = account._compute_statistics_facebook_360d(
                insights_endpoint_url
            )

            page_global_stats = requests.get(
                url_join(
                    self.env["social.media"]._FACEBOOK_ENDPOINT,
                    "/v10.0/%s" % account.facebook_account_id,
                ),
                params={
                    "fields": "fan_count",
            ...
            account.write(
                {
                    ...
                    "audience_trend": self._compute_trend(
                        account.audience, statistics_30d["page_fans"]
                    ),
                    "engagement": statistics_360d["page_post_engagements"],
                    "engagement_trend": self._compute_trend(
                        account.engagement, statistics_30d["page_post_engagements"]
                    ),
                    "stories": statistics_360d["page_content_activity"],
                    "stories_trend": self._compute_trend(
                        account.stories, statistics_30d["page_content_activity"]
                    ),
                }
            )
    ...
    def _compute_statistics_facebook_360d(self, insights_endpoint_url):
        ...
        total_statistics = dict(
            page_post_engagements=0, page_content_activity=0, page_fans=0
        )
        for i in range(4):
            ...
            total_statistics["page_post_engagements"] += statistics_90d[
                "page_post_engagements"
            ]
            total_statistics["page_content_activity"] += statistics_90d[
    ...
    def _compute_statistics_facebook(
        self, endpoint_url, date_preset="last_30d", since=None, until=None
    ):
    ...
    def create(self, vals_list):
        ...
        res.filtered(
            lambda account: account.media_type == "facebook"
        )._create_default_stream_facebook()
        return res
