...
class SocialMediaFacebook(models.Model):
    ...
    def _action_add_account(self):
        ...
        facebook_app_id = (
            self.env["ir.config_parameter"].sudo().get_param("social.facebook_app_id")
        )
    ...
    def _add_facebook_accounts_from_iap(self):
        ...
        iap_add_accounts_url = requests.get(
            url_join(social_iap_endpoint, "api/social/facebook/1/add_accounts"),
            params={
                "returning_url": url_join(
                    self.get_base_url(), "social_facebook/callback"
                ),
