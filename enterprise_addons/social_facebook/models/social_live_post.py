...
class SocialLivePostFacebook(models.Model):
    ...
    def _refresh_statistics(self):
        ...
        for account in accounts:
            posts_endpoint_url = url_join(
                ...
                "/v10.0/%s/%s" % (account.facebook_account_id, "feed"),
            )
            result = requests.get(
                posts_endpoint_url,
                params={
            ...
            existing_live_posts = (
                ...
                .sudo()
                .search([("facebook_post_id", "in", facebook_post_ids)])
            )

            existing_live_posts_by_facebook_post_id = {
                live_post.facebook_post_id: live_post
                for live_post in existing_live_posts
            }

            for post in result_posts:
                existing_live_post = existing_live_posts_by_facebook_post_id.get(
                    post.get("id")
                )
    ...
    def _post(self):
        facebook_live_posts = self.filtered(
            lambda post: post.account_id.media_type == "facebook"
        )
    ...
    def _post_facebook(self, facebook_target_id):
        ...
        post_endpoint_url = url_join(
            self.env["social.media"]._FACEBOOK_ENDPOINT,
            "/v10.0/%s/feed" % facebook_target_id,
        )
        ...
        if post.image_ids and len(post.image_ids) == 1:
            ...
            result = requests.request(
                ...
                files={
                    "source": (
                        image.name,
                        image.with_context(bin_size=False).raw,
                        image.mimetype,
                    )
                },
            )
            if not result.ok:
                generic_api_error = (
                    json.loads(result.text or "{}").get("error", {}).get("message", "")
                )
                self.write(
                    {
                        "state": "failed",
                        "failure_reason": _(
                            "We could not upload your image, try reducing its size and posting it again (error: %s).",
                            generic_api_error,
                        ),
                    }
                )
            ...
            if post.image_ids:
                try:
                    images_attachments = post._format_images_facebook(
                        account.facebook_account_id, account.facebook_access_token
                    )
                except UserError as e:
                    self.write({"state": "failed", "failure_reason": e.name})
                    return
                images_attachments = post._format_images_facebook(
                    facebook_target_id, account.facebook_access_token
                )
        ...
        if result.status_code == 200:
            ...
            self.facebook_post_id = result_json.get(
                "post_id", result_json.get("id", False)
            )
            values = {"state": "posted", "failure_reason": False}
        else:
            values = {
                "state": "failed",
                "failure_reason": json.loads(result.text or "{}")
                .get("error", {})
                .get("message", ""),
            }
