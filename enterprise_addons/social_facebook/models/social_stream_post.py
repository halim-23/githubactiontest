...
class SocialStreamPostFacebook(models.Model):
    ...
    def _compute_author_link(self):
        ...
        for post in facebook_posts:
            post.author_link = "/social_facebook/redirect_to_profile/%s/%s?name=%s" % (
                post.account_id.id,
                post.facebook_author_id,
                urllib.parse.quote(post.author_name),
            )
    ...
    def _facebook_comment_delete(self, comment_id):
        requests.delete(
            url_join(
                self.env["social.media"]._FACEBOOK_ENDPOINT, "/v10.0/%s" % comment_id
            ),
    ...
    def _facebook_comment_fetch(self, next_records_token=False, count=20):
        ...
        comments_endpoint_url = url_join(
            self.env["social.media"]._FACEBOOK_ENDPOINT,
            "/v10.0/%s/comments" % (self.facebook_post_id),
        )
        params = {
        ...
        if not result.ok:
            _logger.error(
                "An error occurred while fetching the comment: %s" % result.text
            )
            ...
            if result_json.get("error"):
                ...
                if error_code == 100 and error_subcode == 33:
                    error_message = _(
                        "Post not found. It could be because the post has been deleted on the Social Platform."
                    )
        ...
        for comment in result_json.get("data"):
            comment["likes"] = {
                "summary": {"total_count": comment.get("like_count", 0)}
            }
            comment["formatted_created_time"] = self._format_facebook_published_date(
                comment
            )
            comment["message"] = self.stream_id._format_facebook_message(
                comment.get("message"), comment.get("message_tags")
            )
            ...
            for inner_comment in inner_comments:
                inner_comment["likes"] = {
                    "summary": {"total_count": inner_comment.get("like_count", 0)}
                }
                inner_comment[
                    "formatted_created_time"
                ] = self._format_facebook_published_date(inner_comment)
                inner_comment["message"] = self.stream_id._format_facebook_message(
                    inner_comment.get("message"), inner_comment.get("message_tags")
                )
        ...
        return {
            ...
            "nextRecordsToken": result_json.get("paging").get("cursors").get("after")
            if result_json.get("paging")
            else None,
        }
    ...
    def _facebook_comment_post(
        ...
        result = requests.post(
            endpoint_url,
            data=params,
            files={"source": ("source", attachment.read(), attachment.content_type)}
            if attachment
            else None,
    ...
    def _facebook_like(self, object_id, like):
        ...
        comments_like_endpoint_url = url_join(
            self.env["social.media"]._FACEBOOK_ENDPOINT, "/v10.0/%s/likes" % (object_id)
        )
        if like:
