odoo.define("mrp_mps.ClientAction", function (require) {
  ...
  var ClientAction = AbstractAction.extend({
    ...
    update_cp: async function () {
      this.$buttons = $(
        QWeb.render("mrp_mps_control_panel_buttons", {groups: this.groups})
      );
      ...
      this.$buttons
        .find(".o_mrp_mps_create")
        .on("click", this._onClickCreate.bind(this));
      ...
      this._controlPanelWrapper.$el
        .find(".o_mps_mps_show_line")
        .on("click", this._onChangeCompany.bind(this));
      return res;
    },
    ...
    _renderProductionSchedule: function (productionScheduleId) {
      var self = this;
      return this._getProductionScheduleState(productionScheduleId).then(function (
        states
      ) {
    ...
    _renderState: function (states) {
      for (var i = 0; i < states.length; i++) {
        ...
        var $table = $(
          QWeb.render("mrp_mps_production_schedule", {
        ...
        if ($tbody.length) {
          ...
          if ("warehouse_id" in state) {
            $warehouse = $(
              ".o_mps_content[data-warehouse_id=" + state.warehouse_id[0] + "]"
            );
          }
          if ($warehouse.length) {
    ...
    _saveForecast: function (productionScheduleId, dateIndex, forecastQty) {
      ...
      this.mutex.exec(function () {
        return self
          ...
          .then(function () {
            return self
              ._renderProductionSchedule(productionScheduleId)
              .then(function () {
                return self._focusNextInput(
    ...
    _onPagerChanged: function (ev) {
      ...
      currentMinimum -= 1;
      this.active_ids = this.recordsPager
        .slice(currentMinimum, currentMinimum + limit)
        .map((i) => i.id);
      this._reloadContent();
    },
