...
class Company(models.Model):
    ...
    manufacturing_period_to_display = fields.Integer(
        ...
        given period to display in Master Production Schedule",
        default=12,
    )
    ...
    mrp_mps_show_actual_demand = fields.Boolean("Display Actual Demand", default=False)
    mrp_mps_show_indirect_demand = fields.Boolean(
        "Display Indirect Demand", default=True
    )
    mrp_mps_show_to_replenish = fields.Boolean("Display To Replenish", default=True)
    mrp_mps_show_actual_replenishment = fields.Boolean(
        "Display Actual Replenishment", default=False
    )
    mrp_mps_show_safety_stock = fields.Boolean("Display Safety Stock", default=True)
    ...
    def _get_date_range(self, years=False):
        ...
        if not years:
            years = 0
        first_day = start_of(
            subtract(fields.Date.today(), years=years), self.manufacturing_period
        )
    ...
    def _date_range_to_str(self):
        ...
        for date_start, date_stop in date_range:
            if self.manufacturing_period == "month":
                dates_as_str.append(
                    format_date(self.env, date_start, date_format="MMM yyyy")
                )
            elif self.manufacturing_period == "week":
                dates_as_str.append(
                    _("Week %s", format_date(self.env, date_start, date_format="w"))
                )
            else:
                dates_as_str.append(
