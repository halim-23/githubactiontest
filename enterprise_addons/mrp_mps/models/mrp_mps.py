...
class MrpProductionSchedule(models.Model):
    ...
    forecast_ids = fields.One2many(
        "mrp.product.forecast", "production_schedule_id", "Forecasted quantity at date"
    )
    ...
    def action_open_actual_demand_details(self, date_str, date_start, date_stop):
        ...
        picking_ids = (
            self._filter_moves(moves_by_date, date_start, date_stop)
            .mapped("picking_id")
            .ids
        )
        return {
            ...
            "view_mode": "list,form",
            "name": _("Actual Demand %s %s (%s - %s)")
            % (self.product_id.display_name, date_str, date_start_str, date_stop_str),
            "target": "current",
    ...
    def action_open_actual_replenishment_details(self, date_str, date_start, date_stop):
        ...
        purchase_order_by_date = self._get_rfq_and_planned_date(
            rfq_domain, date_start, date_stop
        )
        purchase_order_line_ids = self._filter_rfq(
            purchase_order_by_date, date_start, date_stop
        ).ids
        name = _("Actual Replenishment %s %s (%s - %s)") % (
    ...
    def action_replenish(self, based_on_lead_time=False):
        ...
        if procurements:
            self.env["procurement.group"].with_context(skip_lead_time=True).run(
                procurements
            )
    ...
    def get_mps_view_state(self, domain=False):
        ...
        productions_schedules_states = (
            productions_schedules.get_production_schedule_view_state()
        )
                ...
                mps = self.search(
                    [
                        ...
                        (
                            "warehouse_id",
                            "=",
                            vals.get("warehouse_id", self._default_warehouse_id().id),
                        ),
                        (
                            "company_id",
                            "=",
    ...
    def get_production_schedule_view_state(self):
        ...
        schedules_to_compute = (
            self.env["mrp.production.schedule"].browse(self.get_impacted_schedule())
            | self
        )
        ...
        indirect_ratio_mps = schedules_to_compute._get_indirect_demand_ratio_mps(
        ...
        dummy, outgoing_qty_year_minus_1 = self._get_outgoing_qty(
            date_range_year_minus_1
        )
        dummy, outgoing_qty_year_minus_2 = self._get_outgoing_qty(
            date_range_year_minus_2
        )
        read_fields = [
            "forecast_target_qty",
        ...
        for production_schedule in indirect_demand_order:
            ...
            if production_schedule in self:
                ...
                precision_digits = max(
                    0, int(-(log10(production_schedule.product_uom_id.rounding)))
                )
            ...
            for index, (date_start, date_stop) in enumerate(date_range):
                ...
                if production_schedule in self:
                    ...
                    forecast_values["incoming_qty"] = float_round(
                        incoming_qty.get(key, 0.0) + incoming_qty_done.get(key, 0.0),
                        precision_rounding=rounding,
                    )
                    forecast_values["outgoing_qty"] = float_round(
                        outgoing_qty.get(key, 0.0) + outgoing_qty_done.get(key, 0.0),
                        precision_rounding=rounding,
                    )
                    forecast_values["outgoing_qty_year_minus_1"] = float_round(
                        outgoing_qty_year_minus_1.get(key_y_1, 0.0),
                        precision_rounding=rounding,
                    )
                    forecast_values["outgoing_qty_year_minus_2"] = float_round(
                        outgoing_qty_year_minus_2.get(key_y_2, 0.0),
                        precision_rounding=rounding,
                    )

                forecast_values["indirect_demand_qty"] = float_round(
                    indirect_demand_qty.get(key, 0.0), precision_rounding=rounding
                )
                ...
                for (product, ratio) in indirect_ratio_mps[
                    (production_schedule.warehouse_id, production_schedule.product_id)
                ].items():
                    related_date = max(
                        subtract(date_start, days=lead_time), fields.Date.today()
                    )
                    index = next(
                        i
                        for i, (dstart, dstop) in enumerate(date_range)
                        if related_date <= dstart
                        or (related_date >= dstart and related_date <= dstop)
                    )
                    related_key = (
                        date_range[index],
                        product,
                    ...
                    indirect_demand_qty[related_key] += (
                        ratio * forecast_values["replenish_qty"]
                    )
    ...
    def get_impacted_schedule(self, domain=False):
        ...
        def _used_in_bom(products, related_products):
            ...
            products = boms.mapped("product_id") | boms.mapped(
                ...
                [
                    ...
                    [
                        ...
                        (
                            ...
                            "in",
                            _used_in_bom(
                                self.mapped("product_id"), self.env["product.product"]
                            ).ids,
                        ),
                    ],
                ]
    ...
    def remove_replenish_qty(self, date_index):
        ...
        forecast_ids = self.forecast_ids.filtered(
    ...
    def set_forecast_qty(self, date_index, quantity):
        ...
        quantity = float_round(
            float(quantity), precision_rounding=self.product_uom_id.rounding
        )
        ...
        if existing_forecast:
            ...
            new_qty = float_round(
                new_qty, precision_rounding=self.product_uom_id.rounding
            )
    ...
    def set_replenish_qty(self, date_index, quantity):
        ...
        if existing_forecast:
            ...
            existing_forecast[0].write(
                {"replenish_qty": new_qty, "replenish_qty_updated": True}
            )
        else:
            existing_forecast.create(
                {
    ...
    def _filter_moves(self, moves_by_date, date_start, date_stop):
        return self.env["stock.move"].concat(
            *[m[0] for m in moves_by_date if m[1] >= date_start and m[1] <= date_stop]
        )
    ...
    def _filter_rfq(self, rfq_by_date_planned, date_start, date_stop):
        return self.env["purchase.order.line"].concat(
            *[
                pl[0]
                for pl in rfq_by_date_planned
                if pl[1] >= date_start and pl[1] <= date_stop
            ]
        )
    ...
    def _get_forecasts_state(
        self, production_schedule_states, date_range, procurement_date
    ):
        ...
        for production_schedule in self:
            forecast_values = production_schedule_states[production_schedule.id][
                "forecast_ids"
            ]
            ...
            for index, (date_start, date_stop) in enumerate(date_range):
                ...
                if incoming_qty < replenish_qty and procurement_launched:
                    ...
                    state = "to_correct"
                elif incoming_qty == replenish_qty and (
                    date_start <= procurement_date or procurement_launched
                ):
                    state = "launched"
                else:
                    state = "to_launch"
                ...
                if (
                    forecast_state["state"] not in ("launched", "to_correct")
                    and procurement_qty > 0
                ):
    ...
    def _get_incoming_qty(self, date_range):
        ...
        rfq_lines_date_planned = self._get_rfq_and_planned_date(
            rfq_domain, after_date, before_date, order="date_planned"
        )
        ...
        for (line, date_planned) in rfq_lines_date_planned:
            ...
            while not (
                date_range[index][0] <= date_planned
                and date_range[index][1] >= date_planned
            ):
                index += 1
                    ...
                    if child.product in other_mps_product_ids:
                        result[
                            (
                                warehouse_id,
                                node_indirect
                                and node_indirect.product
                                or current_node.product,
                            )
                        ][child.product] += (
                            ratio * child.ratio
                        )
                        if (
                            child.product in subtree_visited
                        ):  # Don't visit the same subtree twice
                            continue
                        ...
                        _dfs_ratio_search(
                            child, child.ratio * ratio, node_indirect=current_node
                        )
    ...
    def _get_indirect_demand_tree(self):
        ...
        def _get_product_tree(product, ratio):
            ...
            for line in product_bom.bom_line_ids:
                ...
                bom_qty = line.bom_id.product_uom_id._compute_quantity(
                    line.bom_id.product_qty, line.bom_id.product_tmpl_id.uom_id
                )
    ...
    def _get_moves_domain(self, date_start, date_stop, type):
        ...
        common_domain = [
            ...
            (
                location_dest,
                "child_of",
                self.mapped("warehouse_id.view_location_id").ids,
            ),
            ("is_inventory", "=", False),
            ("date", "<=", date_stop),
        ]
        ...
        for schedule in self:
            rules = schedule.product_id._get_rules_from_location(
                schedule.warehouse_id.lot_stock_id
            )
            delay, dummy = rules.filtered(
                lambda r: r.action not in ["buy", "manufacture"]
            )._get_lead_days(schedule.product_id)
    ...
    def _get_moves_and_date(self, moves_domain, order=False):
        ...
        for move in moves:
            if not move.move_dest_ids:
                ...
                delay = max(
                    move.move_dest_ids.mapped(lambda m: self._get_dest_moves_delay(m))
                )
        ...
        for delay in groupby_delay:
            ...
            specific_domain = [
                (
                    ...
                    "child_of",
                    warehouses.mapped("view_location_id").ids,
                ),
                ("product_id", "in", products.ids),
    ...
    def _get_rfq_and_planned_date(self, rfq_domain, date_start, date_stop, order=False):
        ...
        for line in purchase_lines:
            if not line.move_dest_ids:
                res_purchase_lines.append(
                    (line, fields.Date.to_date(line.date_planned))
                )
                continue
            delay = max(
                line.move_dest_ids.mapped(lambda m: self._get_dest_moves_delay(m))
            )
...
class MrpProductForecast(models.Model):
    ...
    production_schedule_id = fields.Many2one(
        "mrp.production.schedule", required=True, ondelete="cascade"
    )
