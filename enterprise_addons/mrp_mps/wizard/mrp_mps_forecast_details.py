...
class MrpMpsForecastDetails(models.TransientModel):
    ...
    moves_qty = fields.Integer(
        "Quantity from Incoming Moves", compute="_compute_quantity"
    )
    manufacture_qty = fields.Integer(
        "Quantity from Manufacturing Order", compute="_compute_quantity"
    )
    ...
    def _compute_quantity(self):
        for mps in self:
            mps.moves_qty = sum(
                mps.move_ids.filtered(lambda m: m.picking_id).mapped("product_qty")
            )
            mps.manufacture_qty = sum(
                mps.move_ids.filtered(lambda m: m.production_id).mapped("product_qty")
            )
            mps.rfq_qty = sum(
                [
                    l.product_uom._compute_quantity(l.product_qty, l.product_id.uom_id)
                    for l in mps.purchase_order_line_ids
                ]
            )
    ...
    def action_open_rfq_details(self):
        return {
            ...
            "domain": [
                ("id", "in", self.purchase_order_line_ids.mapped("order_id").ids)
            ],
        }
