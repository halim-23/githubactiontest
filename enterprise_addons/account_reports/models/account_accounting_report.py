...
HierarchyDetail = namedtuple(
    "HierarchyDetail", ["field", "foldable", "lazy", "section_total", "namespan"]
)
...
class AccountingReport(models.AbstractModel):
    ...
    def _get_sql(self):
        ...
        return """
            SELECT {}
             ...
             WHERE FALSE
        """.format(
            self._get_move_line_fields()
        )

    def _get_move_line_fields(self, aml_alias="account_move_line"):
        return ", ".join(
            "%s.%s" % (aml_alias, field) for field in self._depends["account.move.line"]
        )

    # COLUMN/CELL FORMATTING ###################################################
    # ##########################################################################
    def _field_column(
        self, field_name, sortable=False, name=None, ellipsis=False, blank_if_zero=False
    ):
        ...
        def getter(v):
            return self._fields[field_name].convert_to_cache(
                v.get(field_name, ""), self
            )
        ...
        return self._custom_column(
            name=name or IrModelFields._get(self._name, field_name).field_description,
            getter=getter,
            formatter=formatter,
            classes=classes,
            ellipsis=ellipsis,
    ...
    def _custom_column(
        self, name, getter, formatter=None, classes=None, sortable=False, ellipsis=False
    ):
        """Build custom column.
        ...
        return ColumnDetail(
            name=name, classes=" ".join(classes), getter=getter, formatter=formatter
        )
    ...
    def _get_columns_name(self, options):
        return [
            {"name": col.name, "class": col.classes}
            for col in self._get_column_details(options)
        ]
    ...
    def _hierarchy_level(
        self, field_name, foldable=False, lazy=False, section_total=False, namespan=1
    ):
    ...
    def _get_values(self, options, line_id):
        ...
        for i in range(len(groupby)):
                        ...
                        if len(current_groupby) == len(parsed) + 1:
                            unfolded_domain = expression.OR(
                                [
                                    ...
                                    [
                                        (field_name, "=", value)
                                        for field_name, model_name, value in parsed
                                    ],
                                ]
                            )
            ...
            if "id" in current_groupby:
                read = self.search_read(
                    domain, self._fields.keys(), offset=offset, limit=limit
                )
            else:
            ...
            for r in read:
                ...
                for j, gb in enumerate(current_groupby):
                    gb_model = (
                        self._fields[gb].comodel_name
                        if gb != "id"
                        else self._get_id_field_comodel()
                    )
    ...
    def _aggregate_values(self, destination, source):
        for field, value in source.items():
            ...
            if not destination.get(field):
                ...
                destination[field] = min(
                    destination[field] or value, value or destination[field]
                )
            elif self._fields[field].group_operator == "max":
                destination[field] = max(
                    destination[field] or value, value or destination[field]
                )
                ...
                raise NotImplementedError(
                    "%s operator not implemented for %s"
                    % (self._fields[field].group_operator, field)
                )
    ...
    def _append_grouped(
        ...
        line = self._format_line(
            line_dict["values"], value_getters, value_formatters, current, options
        )
        ...
        if self._show_line(line, line_dict["values"], current, options):
            ...
            hidden_lines[line["id"]] = (
                hidden_lines.get("parent_id") or line["parent_id"]
            )
        ...
        # Handle load more
        offset = line["offset"] = len(line_dict["children"]) + int(
            options.get("lines_offset", 0)
        )
        if (
            current
            and self._get_hierarchy_details(options)[len(current) - 1].lazy
            and len(line_dict["children"]) >= self.MAX_LINES
        ...
        if (
            current
            and self._get_hierarchy_details(options)[len(current) - 1].section_total
            and line_dict["children"]
            and lines[-1] != line
        ):
    ...
    def _get_load_more_line(
        self, line_dict, value_getters, value_formatters, current, options, offset
    ):
        load_more_line = self._format_line(
            line_dict["values"], value_getters, value_formatters, current, options
        )
        ...
        load_more_line["name"] = _("Load more... (%s remaining)") % (
            line_dict["values"].get("__count", 1) - offset
        )
    ...
    def _get_lines(self, options, line_id=None):
        ...
        self._append_grouped(
            ...
            value_formatters=[
                d.formatter for d in self._get_column_details(options)[1:]
            ],
            options=options,
            hidden_lines={},
        )
        ...
        if lines:
            ...
            return lines[1:] + (
                self.total_line and [{**lines[0], "name": _("Total")}] or []
            )
        return []
    ...
    def _show_line(self, report_dict, value_dict, current, options):
        ...
        """
        return (
            report_dict["parent_id"] is None
            or report_dict["parent_id"] == "total-None"
            or (
                report_dict["parent_id"] in options.get("unfolded_lines", [])
    ...
    # To override
    def _format_line(
        self, value_dict, value_getters, value_formatters, current, options, total=False
    ):
        ...
        res = {
            ...
            "unfolded": (
                (id in options.get("unfolded_lines", []))
                or options.get("unfold_all")
