...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates[
            "main_template"
        ] = "account_reports.main_template_with_filter_input_partner"
        return templates
    ...
    def _get_options_domain(self, options):
        ...
        exch_code = (
            ...
            .browse(self.env.context.get("company_ids"))
    ...
    @api.model
    def _get_lines_without_partner(
        self, options, expanded_partner=None, offset=0, limit=0
    ):
    ...
    def _do_query(self, options, expanded_partner=None):
        ...
        for row in self._cr.dictfetchall():
            ...
            if None not in groupby_partners and not (
                expanded_partner or unfold_all or options["unfolded_lines"]
            ):
        ...
        if None in groupby_partners:
            if "sum" not in groupby_partners[None]:
                groupby_partners[None].setdefault(
    ...
    def _get_report_line_partner(
        self, options, partner, initial_balance, debit, credit, balance
    ):
        company_currency = self.env.company.currency_id
        ...
        return {
            ...
            "unfoldable": not company_currency.is_zero(debit)
            or not company_currency.is_zero(credit),
            "unfolded": "partner_%s" % (partner.id if partner else 0)
    ...
    def _get_report_line_move_line(
        self, options, partner, aml, cumulated_init_balance, cumulated_balance
    ):
        if aml["payment_id"]:
        ...
        return {
            ...
            "class": "text"
            + aml.get("class", ""),  # do not format as date to prevent text centering
            "columns": columns,
    ...
    def _load_more_lines(self, options, line_id, offset, load_more_remaining, progress):
        ...
        for aml in self._cr.dictfetchall():
            ...
            lines.append(
                self._get_report_line_move_line(
                    options, expanded_partner, aml, cumulated_init_balance, progress
                )
            )
        ...
        for row in self._cr.dictfetchall():
            ...
            lines.append(
                self._get_report_line_move_line(
                    options, expanded_partner, row, cumulated_init_balance, progress
                )
            )
