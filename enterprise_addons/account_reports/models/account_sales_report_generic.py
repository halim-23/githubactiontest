...
class ECSalesReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        if self._get_report_country_code(
            options
        ) in self._get_non_generic_country_codes(options):
            ...
            lines.append(
                {
                    ...
                    "columns": [
                        {"name": c}
                        for c in [res["country_code"], res["partner_vat"], res["value"]]
                    ],
                    "level": 2,
                }
            )
        ...
        lines.append(
            {
                ...
                "level": 2,
                "columns": [
                    {"name": self.format_value(total_value), "no_format": total_value}
                ],
                "colspan": 3,
            }
        )
    ...
    def _prepare_query(self, options):
        ...
        tables, where_clause, where_params = self._query_get(
            options, [("move_id.move_type", "in", ("out_invoice", "out_refund"))]
        )
        where_params.append(
            tuple(self.env["account.sales.report"].get_ec_country_codes(options))
        )
        query = (
            """
