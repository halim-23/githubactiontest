...
class analytic_report(models.AbstractModel):
    ...
    def _get_amount_of_parents(self, group):
        return self.env["account.analytic.group"].search_count(
            [("id", "parent_of", group.id)]
        )
    ...
    def _get_balance_for_group(self, group, analytic_line_domain):
        ...
        analytic_lines = self.env["account.analytic.line"].read_group(
            analytic_line_domain_for_group, ["amount", "currency_id"], ["currency_id"]
        )
        balance = sum(
            [
                currency_obj.browse(row["currency_id"][0])._convert(
                    row["amount"],
                    company_currency,
                    self.env.company,
                    fields.Date.today(),
                )
                for row in analytic_lines
            ]
        )
        return balance

    def _generate_analytic_group_line(
        self, group, analytic_line_domain, unfolded=False
    ):
        LOWEST_LEVEL = 1
    ...
    def _get_lines(self, options, line_id=None):
        ...
        AccountAnalyticAccount = self.env["account.analytic.account"].with_context(
            from_date=date_from, to_date=date_to, active_test=False
        )
        ...
        if options.get("analytic_tags"):
            ...
            AccountAnalyticAccount = AccountAnalyticAccount.with_context(
                tag_ids=analytic_tag_ids
            )
        ...
        if not options["hierarchy"]:
            return self._generate_analytic_account_lines(
                AccountAnalyticAccount.search(analytic_account_domain)
            )
        ...
        if analytic_groups:
            analytic_groups = AccountAnalyticGroup.search(
                [("id", "parent_of", analytic_groups.ids)]
            )
        ...
        if line_id:
            parent_group = (
                ...
                else AccountAnalyticGroup.browse(int(line_id))
            )
            ...
            lines.append(
                self._generate_analytic_group_line(
                    parent_group, analytic_entries_domain, unfolded=True
                )
            )
            ...
            analytic_accounts = AccountAnalyticAccount.search(analytic_account_domain)
            lines += self._generate_analytic_account_lines(
                analytic_accounts,
        ...
        if line_id != self.DUMMY_GROUP_ID:
            for group in AccountAnalyticGroup.search(domain):
                if group.id in options.get("unfolded_lines") or options.get(
                    "unfold_all"
                ):
                    lines += self.with_context(
                        account_ids_to_not_display=account_ids_to_not_display
                    )._get_lines(options, line_id=str(group.id))
                else:
                    lines.append(
                        self._generate_analytic_group_line(
                            group, analytic_entries_domain
                        )
                    )
        ...
        if not line_id and any(not account.group_id for account in analytic_accounts):
            if self.DUMMY_GROUP_ID in options.get("unfolded_lines"):
                ...
                lines.append(
                    self._generate_analytic_group_line(
                        AccountAnalyticGroup, analytic_entries_domain
                    )
                )
