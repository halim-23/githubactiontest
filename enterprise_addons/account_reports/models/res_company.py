...
class ResCompany(models.Model):
    ...
    account_revaluation_expense_provision_account_id = fields.Many2one(
        "account.account", string="Expense Provision Account"
    )
    account_revaluation_income_provision_account_id = fields.Many2one(
        "account.account", string="Income Provision Account"
    )
    ...
    def get_available_tax_reports(self):
        ...
        vat_fiscal_positions = self.env["account.fiscal.position"].search(
            [("company_id", "in", self.ids), ("foreign_vat", "!=", False)]
        )
        ...
        return self.env["account.tax.report"].search(
            [("country_id", "in", available_countries.ids)]
        )
    ...
    def _update_tax_closing_after_periodicity_change(self):
        ...
        self._get_and_update_tax_closing_moves(
            fields.Date.today(), vat_fiscal_positions, include_domestic=True
        )

    def _get_and_update_tax_closing_moves(
        self, in_period_date, fiscal_positions=None, include_domestic=False
    ):
        ...
        tax_closing_activity_type_id = self.env["ir.model.data"]._xmlid_to_res_id(
            "account_reports.tax_closing_activity_type"
        )

        all_closing_moves = self.env["account.move"]
        for fpos in itertools.chain(
            fiscal_positions, [None] if include_domestic else []
        ):
            ...
            if len(tax_closing_move) > 1:
                if fpos:
                    ...
                    params = (
                        fpos.name,
                        period_start,
                        tax_closing_move.mapped("display_name"),
                    )
            ...
            ref = self._get_tax_closing_move_description(
                self.account_tax_periodicity, period_start, period_end, fpos
            )
    ...
    def _get_tax_closing_move_description(
        self, periodicity, period_start, period_end, fiscal_position
    ):
        ...
        if periodicity == "year":
            ...
            return _(
                "Tax return for %s%s",
                format_date(self.env, period_start, date_format="qqq"),
                region_string,
            )
        elif periodicity == "monthly":
            return _(
                "Tax return for %s%s",
                format_date(self.env, period_start, date_format="LLLL"),
                region_string,
            )
        else:
            return _(
                "Tax return from %s to %s%s",
    ...
    def _get_tax_closing_period_boundaries(self, date):
        ...
        period_number = (date.month // period_months) + (
            1 if date.month % period_months != 0 else 0
        )
        end_date = date_utils.end_of(
            datetime.date(date.year, period_number * period_months, 1), "month"
        )
