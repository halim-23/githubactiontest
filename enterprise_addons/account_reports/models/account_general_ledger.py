...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates[
            "line_template"
        ] = "account_reports.line_template_general_ledger_report"
        templates[
            "main_template"
        ] = "account_reports.main_template_with_filter_input_accounts"
        return templates
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if offset > 0:
            ...
            return self._load_more_lines(
                options, line_id, offset, remaining, balance_progress
            )
        else:
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            balance = account_sum.get("balance", 0.0) + account_un_earn.get(
                "balance", 0.0
            )
            ...
            if has_lines and (unfold_all or is_unfolded):
                ...
                cumulated_balance = account_init_bal.get(
                    "balance", 0.0
                ) + account_un_earn.get("balance", 0.0)

                lines.append(
                    self._get_initial_balance_line(
                        options,
                        account,
                        account_init_bal.get("amount_currency", 0.0)
                        + account_un_earn.get("amount_currency", 0.0),
                ...
                load_more_counter = (
                    self._context.get("print_mode")
                    and load_more_remaining
                    or self.MAX_LINES
                )
                ...
                for aml in amls:
                    ...
                    lines.append(
                        self._get_aml_line(
                            ...
                            account,
                            aml,
                            company_currency.round(cumulated_balance),
                        )
                    )
        ...
        if not line_id:
            ...
            if len(journal_options) == 1 and journal_options[0]["type"] in (
                "sale",
                "purchase",
            ):
    ...
    @api.model
    def _load_more_lines(
        self, options, line_id, offset, load_more_remaining, balance_progress
    ):
        ...
        amls_query, amls_params = self._get_query_amls(
            options, expanded_account, offset=offset, limit=load_more_counter
        )
        ...
        for aml in self._cr.dictfetchall():
            ...
            lines.append(
                self._get_aml_line(options, expanded_account, aml, balance_progress)
            )
    ...
    def _get_options_sum_balance(self, options):
        ...
        new_options["date"] = {
            "mode": "range",
            "date_from": fiscalyear_dates["date_from"].strftime(
    ...
    def _do_query(self, options_list, expanded_account=None, fetch_lines=True):
        ...
        query, params = self._get_query_sums(
            options_list, expanded_account=expanded_account
        )
        ...
        for res in self._cr.dictfetchall():
            ...
            if key == "sum":
                groupby_accounts.setdefault(
                    res["groupby"], [{} for n in range(len(options_list))]
                )
        ...
        if groupby_companies:
            ...
            for account in candidates_accounts:
                ...
                for i in range(len(options_list)):
                    ...
                    groupby_accounts.setdefault(
                        account.id, [{} for i in range(len(options_list))]
                    )
                    groupby_accounts[account.id][i][
                        "unaffected_earnings"
                    ] = unaffected_earnings
        ...
        if expanded_account:
            ...
            accounts = []
        accounts_results = [
            (account, groupby_accounts[account.id]) for account in accounts
        ]

        # Fetch as well the taxes.
        if groupby_taxes:
            taxes = self.env["account.tax"].search(
                [("id", "in", list(groupby_taxes.keys()))]
            )
        else:
            taxes = []
    ...
    def _get_account_title_line(
        ...
        has_foreign_currency = (
            account.currency_id
            and account.currency_id != account.company_id.currency_id
            or False
        )
        ...
        return {
            ...
            "unfolded": has_lines
            and "account_%d" % account.id in options.get("unfolded_lines")
            or unfold_all,
            "colspan": 4,
    ...
    def _get_tax_declaration_lines(self, options, journal_type, taxes_results):
        lines = [
            ...
            },
            {
                ...
                "name": _("Name"),
                "columns": [
                    {"name": v} for v in ["", _("Base Amount"), _("Tax Amount"), ""]
                ],
        ...
        journal = self.env["account.journal"].browse(
            self._get_options_journals(options)[0]["id"]
        )
        tax_report_lines = (
            self.env["account.generic.tax.report"]
            .with_company(journal.company_id)
            ._get_lines(tax_report_options)
        )

        for tax_line in tax_report_lines:
            if tax_line["id"] not in (
                "sale",
                "purchase",
            ):  # We want to exclude title lines here
                tax_line["columns"].append({"name": ""})
                tax_line["colspan"] = (
                    self.user_has_groups("base.group_multi_currency") and 5 or 4
                )
    ...
    def action_dropdown_audit_default_tax_report(self, options, data):
        ...
        ].action_dropdown_audit_default_tax_report(options, data)
