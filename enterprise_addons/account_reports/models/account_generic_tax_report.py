...
class AccountGenericTaxReport(models.AbstractModel):
    ...
    def _init_filter_tax_report(self, options, previous_options=None):
        ...
        if (
            not self._is_generic_report(options)
            and options["tax_report"] not in available_reports.ids
        ):
            ...
            options["tax_report"] = (
                available_reports and available_reports[0].id or "generic"
            )
    ...
    def _init_filter_multi_company(self, options, previous_options=None):
        ...
        if (
            previous_options
            and previous_options.get("tax_unit") in companies_authorized_tax_unit_opt
        ):
        ...
        if options["tax_unit"] != "company_only":
            ...
            options["multi_company"] = [
                ...
                for company in tax_unit.company_ids
            ]
    ...
    def _is_generic_report(self, options):
        return isinstance(options["tax_report"], str) and options[
            "tax_report"
        ].startswith("generic")
        ...
        for alias, field in groupby_fields:
            ...
            if alias == "src_tax":
                select_clause_list.append(
                    "%s.%s AS %s_%s" % ("tax", field, "tax", field)
                )
    ...
    @api.model
    def _default_tax_report_build_report_line(
        self, options, default_vals, groupby_key, value
    ):
        ...
        if groupby_key == "src_tax_type_tax_use":
            ...
            report_line["id"] = self._get_generic_line_id(
                None, None, markup=type_tax_use_option[0]
            )
    ...
    @api.model
    def _default_tax_report_populate_lines_recursively(
        self,
        options,
        lines,
        sorting_map_list,
        ...
        for key in sorted_keys:
            ...
            report_line = self._default_tax_report_build_report_line(
                options, default_vals, groupby_key, sorting_map[key][0]
            )
    ...
    def _get_lines_default_tax_report(self, options):
        ...
        tax_amount_hierarchy = self._read_default_tax_report_amounts(
            options_list, groupby_fields
        )
    ...
    def _compute_vat_closing_entry(self, company, options):
        ...
        for tg, result in zip(
            self.env["account.tax.group"].browse(tax_group_ids), results
        ):
        ...
        for tg, values in tax_groups.items():
            ...
            if (
                not tg.property_tax_receivable_account_id
                or not tg.property_tax_payable_account_id
            ):
                continue
            ...
            if not currency.is_zero(total):
                ...
                key = (
                    tg.property_advance_tax_payment_account_id.id or False,
            ...
            if not currency.is_zero(total):
                line_ids_vals.append(
                    (
                        ...
                        {
                            "name": total < 0
                            and _("Payable tax amount")
                            or _("Receivable tax amount"),
    ...
    def _generate_tax_closing_entries(self, options, closing_moves=False):
        ...
        """
        options_company_ids = [
            company_opt["id"] for company_opt in options.get("multi_company", [])
        ]
        companies = (
            self.env["res.company"].browse(options_company_ids)
            if options_company_ids
        ...
        if closing_moves:
            ...
            for company in companies:
                (
                    include_domestic,
                    fiscal_positions,
                ) = self._get_fpos_info_for_tax_closing(company, options)
        ...
        for company, company_closing_moves in closing_moves_by_company.items():
            ...
            if company.tax_lock_date and company.tax_lock_date >= end_date:
                raise UserError(
                    _("This period is already closed for company %s", company.name)
                )
            ...
            for move in company_closing_moves:
                ...
                move_vals["tax_report_control_error"] = bool(
                    move_options.get("tax_report_control_error")
                )
    ...
    def _redirect_to_misconfigured_tax_groups(self, company, countries):
        ...
        need_config_action = {
            ...
            "view_mode": "tree",
            "views": [[False, "list"]],
            "context": len(countries) == 1
            and {"search_default_country_id": countries.ids or {}},
            # More than 1 id into search_default isn't supported
        }

        raise RedirectWarning(
            _(
                "Some of your tax groups are missing information in company %s. Please complete their configuration.",
            ...
            additional_context={
                ...
                "force_account_company": company.id,
            },
        )
    ...
    def _compute_from_amls_grids(self, options, dict_to_fill, period_number):
        ...
        for account_tax_report_line_id, balance in self.env.cr.fetchall():
            if account_tax_report_line_id in dict_to_fill:
                dict_to_fill[account_tax_report_line_id][0]["periods"][period_number][
                    "balance"
                ] = balance
    ...
    def _get_lines_by_grid(self, options, line_id, grids):
        ...
        deferred_total_lines = (
            []
        )  # list of tuples (index where to add the total in lines, tax report line object)
        for current_line in report.get_lines_in_hierarchy():

            hierarchy_level = self._get_hierarchy_level(current_line)
            parent_line_id = (
                lines_mapping[current_line.parent_id.id]["id"]
                if current_line.parent_id.id
                else None
            )
            ...
            if current_line.formula:
                ...
                line = self._prepare_total_line(
                    current_line, parent_line_id, hierarchy_level
                )
                ...
                line = self._build_tax_grid_line(
                    grids[current_line.id][0], parent_line_id, hierarchy_level, options
                )
            else:
                # Then it's a title line
                line = self._build_tax_section_line(
                    current_line, parent_line_id, hierarchy_level
                )
            lines.append(line)
        ...
        for (index, total_line) in deferred_total_lines:
            ...
            total_period_number = 1 + (
                options["comparison"].get("periods")
                and options["comparison"]["number_period"]
                or 0
            )
            parent_line_id = (
                lines_mapping[total_line.parent_id.id]["id"]
                if total_line.parent_id.id
                else None
            )
    ...
    def _postprocess_lines(self, lines, options):
        ...
        def assign_active_section(col_nber):
            ...
            line_to_assign["columns"] = [
                ...
                for balance in total_balance_col
            ]
        ...
        for line in lines:
            while (
                active_sections_stack
                and line["level"] <= active_sections_stack[-1]["level"]
            ):
            ...
            if markup == "total":
                ...
                if line.get("line_code"):
                    balances_by_code[line["line_code"]] = [
                        col["balance"] for col in line["columns"]
                    ]
                ...
                if active_sections_stack:
                    for active_section in active_sections_stack:
                        ...
                        totals_by_line[active_section["id"]] = (
                            line_balances
                            if not rslt_balances
                            else [
                                line_balances[i] + rslt_balances[i]
                                for i in range(0, len(rslt_balances))
                            ]
                        )
    ...
    def compute_check(self, lines, options):
        ...
        for line in lines:
            if line.get("line_code"):
                ...
                carried_over[line["line_code"]] = line["columns"][0].get(
                    "carryover_bounds", False
                )

        for i, calc in enumerate(
            tax_report.get_checks_to_perform(amounts, carried_over)
        ):
            if calc[1]:
                ...
                html_lines.append(
                    "<tr><td>{name}</td><td>{amount}</td></tr>".format(
                        name=calc[0], amount=value
                    )
                )
        if controls:
            lines.extend(
                [
                    {
                        ...
                        "unfoldable": False,
                        "columns": [
                            {"name": "", "style": "white-space:nowrap;", "balance": ""}
                        ]
                        * col_nber,
                        "level": 0,
                        "line_code": False,
            ...
            ] = "<table width='100%'><tr><th>Control</th><th>Difference</th></tr>{}</table>".format(
                "".join(html_lines)
            )

    def _get_total_line_eval_dict(
        self, period_balances_by_code, period_date_from, period_date_to, options
    ):
    ...
    def _build_total_line(
        ...
        def expand_formula(formula):
            for word in re.split(r"\W+", formula):
                if formulas_dict.get(word):
                    formula = re.sub(
                        r"\b%s\b" % word,
                        "(%s)" % expand_formula(formulas_dict.get(word)),
                        formula,
                    )
            return formula
        ...
        for period_index in range(0, number_periods):
            period_balances_by_code = {
                code: balances[period_index]
                for code, balances in balances_by_code.items()
            }
            period_date_from = (
                (period_index == 0)
                and options["date"]["date_from"]
                or options["comparison"]["periods"][period_index - 1]["date_from"]
            )
            period_date_to = (
                (period_index == 0)
                and options["date"]["date_to"]
                or options["comparison"]["periods"][period_index - 1]["date_to"]
            )
            ...
            carryover_account_balance = self.get_carried_over_balance_before_date(
                report_line, options, period_index
            )
            column = {
            ...
            carryover_bounds = report_line._get_carryover_bounds(
                options, period_total, carryover_account_balance
            )
            if carryover_bounds:
    ...
    def _build_tax_grid_line(self, grid_data, parent_id, hierarchy_level, options):
        ...
        for i, period in enumerate(grid_data["periods"]):
            ...
            carryover_bounds = grid_data["obj"]._get_carryover_bounds(
                options, value, carryover_account_balance
            )
            if carryover_bounds:
                column["carryover_bounds"] = carryover_bounds
        ...
        line_id = self._get_generic_line_id(
            "account.tax.report.line", grid_data["obj"].id, parent_line_id=parent_id
        )
    ...
    def _format_lines_for_display(self, lines, options):
        ...
        if options.get("tax_report"):
            ...
            lines_without_controls = [
                line for line in lines if not line.get("is_control", False)
            ]
    ...
    def _format_column_after_carryover(self, line, column, period, options):
        ...
        line_balance, carryover_balance = self.get_amounts_after_carryover(
            tax_report_line, column["balance"], carryover_bounds, options, period
        )
        ...
        if not self.env.context.get("print_mode") and carryover_bounds is not None:
            popup_data = {}
            messages = self._get_popup_messages(
                line_balance, carryover_balance, options, tax_report_line
            )
            column_styles = self._get_column_styles(line)
            column_style = column_styles.get("base_style", "")

            if (
                carryover_bounds[0] is not None
                and column["balance"] < carryover_bounds[0]
            ):
                popup_data.update(messages["out_of_bounds"])
                column_style += column_styles.get("below_bound_style", "")
            elif (
                carryover_bounds[1] is not None
                and column["balance"] > carryover_bounds[1]
            ):
                popup_data.update(messages["out_of_bounds"])
    ...
    def _get_popup_messages(
        self, line_balance, carryover_balance, options, tax_report_line
    ):
        return {
            "positive": {
                "description1": _(
                    "This amount will be increased by the positive amount from"
                ),
                "description2": _(
                    " past period(s), previously stored on the corresponding tax line."
                ),
                "description3": _(
                    "The amount will be : %s", self.format_value(line_balance)
                ),
            },
            "negative": {
                "description1": _(
                    "This amount will be reduced by the negative amount from"
                ),
                "description2": _(
                    " past period(s), previously stored on the corresponding tax line."
                ),
            ...
            "out_of_bounds": {
                "description1": _(
                    "This amount will be set to %s.", self.format_value(line_balance)
                ),
                "description2": _(
    ...
    def get_amounts_after_carryover(
        self,
        tax_report_line,
        amount,
        carryover_bounds,
        ...
        if carryover_bounds is None:
            return amount, self.get_carried_over_balance_before_date(
                tax_report_line, options, period
            )
        ...
        carryover_balance = self.get_carried_over_balance_before_date(
            tax_report_line, options, period
        )
        ...
        # Amounts between the bounds are changed according to the current balance of the carryover
            ...
            if carryover_balance < 0:
                maximum_to_take = (
                    amount - carryover_bounds[0]
                    if carryover_bounds[0] is not None
                    else carryover_balance
                )
            elif carryover_balance > 0:
                maximum_to_take = (
                    carryover_bounds[1] - amount
                    if carryover_bounds[1] is not None
                    else carryover_balance
                )
    ...
    def _compute_tax_report_data(self, options):
        rslt = {}
        empty_data_dict = (
            {"net": 0, "tax": 0} if self._is_generic_layout(options) else {"balance": 0}
        )
        for record in (
            self.env["account.tax.report"].browse(options["tax_report"]).line_ids
        ):
            rslt[record.id] = defaultdict(
                lambda record=record: {
                    "obj": record,
                    "show": False,
                    "periods": [
                        empty_data_dict.copy()
                        for i in range(len(options["comparison"].get("periods")) + 1)
                    ],
                }
            )

        for period_number, period_options in enumerate(
            self._get_options_periods_list(options)
        ):
