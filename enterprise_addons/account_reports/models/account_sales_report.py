...
class ECSalesReport(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates[
            "main_template"
        ] = "account_reports.account_reports_sales_report_main_template"
        return templates
    ...
    def _get_options(self, previous_options=None):
        ...
        if self._get_report_country_code(options):
            ...
            options[
                "country_specific_report_label"
            ] = self.env.company.country_id.display_name
        else:
    ...
    def _init_filter_ec_sale_code(self, options, previous_options=None):
        ...
        if previous_options and previous_options.get("ec_sale_code"):
            for i in range(0, 3):
                options["ec_sale_code"][i]["selected"] = previous_options[
                    "ec_sale_code"
                ][i]["selected"]
    ...
    def _prepare_query(self, options):
        ...
        query = " ".join(
            [
                "SELECT",
                self._get_query_select(options),
                "FROM",
                tables,
                self._get_query_from(options),
                "WHERE",
                where_clause,
                "GROUP BY",
                self._get_query_group_by(options),
                "ORDER BY",
                self._get_query_order_by(options),
            ]
        )
    ...
    def _query_get(self, options, domain=None):
        tables, where_clause, where_params = super(ECSalesReport, self)._query_get(
            options, domain
        )
    ...
    def _get_options_domain(self, options):
        ...
        if options.get("selected_tag_ids"):
            domain.append(
                ("tax_tag_ids.tax_report_line_ids", "in", options["selected_tag_ids"])
            )
        return domain
    ...
    def _get_query_order_by(self, options):
        order_items = [
            "account_tax_report_line_tags_rel.account_tax_report_line_id = %s"
        ] * len(options.get("selected_tag_ids", []))
    ...
    def _process_query_result(self, options, query_result):
        ...
        for row in query_result:
            ...
            if amt:
                ...
                if (
                    row["same_country"]
                    or row["partner_country_code"] not in ec_country_to_check
                ):
                ...
                columns = [vat[:2], vat[2:], ec_sale_code, amt]
                if not self.env.context.get("no_format", False) and not options.get(
                    "get_file_data", False
                ):
                    currency_id = self.env.company.currency_id
                    columns[3] = formatLang(
                        self.env, columns[3], currency_obj=currency_id
                    )
