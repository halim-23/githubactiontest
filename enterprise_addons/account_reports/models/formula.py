...
class FormulaLocals(dict):
    ...
    def __getitem__(self, item):
        if item == "NDays":
            ...
            return self.solver._get_amls_results(self.financial_line)[item].get(
                self.key[0], 0
            )
        elif item in (
            "sum",
...
class FormulaSolver:
    ...
    def _eval_formula(self, financial_line, key):
        ...
        try:
            return safe_eval(
                financial_line.formulas,
                globals_dict=FormulaLocals(self, financial_line, key),
                nocopy=True,
            )
    ...
    def _get_line_by_code(self, line_code):
        ...
        if line_code in self.cache_line_by_code:
            ...
            # Fetch line.
            financial_line = self.env["account.financial.html.report.line"].search(
                [("code", "=", line_code)], limit=1
            )
    ...
    def _get_amls_results(self, financial_line):
        ...
        if "amls" not in self.cache_results_by_id[financial_line.id]:
            ...
            results = financial_line._compute_sum(
                self.options_list, self.financial_report
            )
            ...
            if financial_line.formulas and re.search(
                r"-\s*sum", financial_line.formulas
            ):
                results["sign"] = -1
            else:
                results["sign"] = 1
        ...
        class LeafResolver(ast.NodeTransformer):
            ...
            def visit_Name(self, node):
                if node.id in (
                    ...
                    if financial_line:
                        self.solver.cache_results_by_id[
                            self.financial_line.id
                        ].setdefault("sub_codes", set())
                        self.solver.cache_results_by_id[self.financial_line.id][
                            "sub_codes"
                        ].add(financial_line.code)
    ...
    def _get_balance_from_context(self, financial_line):
        ...
        """
        financial_report_line_values = self.options_list[0].get(
    ...
    def has_move_lines(self, financial_line):
        ...
        if not self.is_leaf(financial_line):
            return False
        total_count_rows = sum(
            self.cache_results_by_id[financial_line.id]["amls"]["count_rows"].values()
        )
    ...
    def get_formula_string(self, financial_line):
        ...
        if self.is_leaf(financial_line):
            ...
            formula = inject_in_formula(
                formula, "count_rows", results["amls"]["count_rows"].get(0, 0)
            )
        ...
        formula = inject_in_formula(
            formula,
            "from_context",
            self._get_balance_from_context(financial_line),
            is_monetary=True,
        )
        ...
        for code in results.get("sub_codes", []):
            ...
            formula = inject_in_formula(
                formula, other_line.code, balance, is_monetary=True
            )
    ...
    def _get_missing_control_domain(self, options, financial_line):
        ...
        has_missing = bool(
            self.env["account.move.line"].search(missing_domain, limit=1)
        )
    ...
    def _get_excess_control_domain(self, options, financial_line):
        ...
        for i, domain in enumerate(involved_domains):
            ...
            comparison_domain = (
                control_domain + ["!"] + normalize_domain(remaining_domains)
            )

            excess_domain = domain + ["!"] + normalize_domain(comparison_domain)
            has_excess = bool(
                self.env["account.move.line"].search(excess_domain, limit=1)
            )
            if has_excess:
