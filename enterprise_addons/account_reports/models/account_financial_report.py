...
from .formula import PROTECTED_KEYWORDS, FormulaSolver
...
class ReportAccountFinancialReport(models.Model):
    ...
    def filter_analytic(self):
        enable_filter_analytic_accounts = (
            self.env.user.id
            in self.env.ref("analytic.group_analytic_accounting").users.ids
        )
        enable_filter_analytic_tags = (
            self.env.user.id in self.env.ref("analytic.group_analytic_tags").users.ids
        )
        if (
            self.analytic
            and not enable_filter_analytic_accounts
            and not enable_filter_analytic_tags
        ):
            return None
    ...
    analytic = fields.Boolean(
        "Allow analytic filters", help="display the analytic filters"
    )
    ...
    tax_report = fields.Boolean(
        "Tax Report",
        help="Set to True to automatically filter out journal items that are not tax exigible.",
    )
    applicable_filters_ids = fields.Many2many(
        "ir.filters",
    ...
    country_id = fields.Many2one(
        ...
        help="The country this report is intended to.",
    )
    ...
    def _init_filter_ir_filters(self, options, previous_options=None):
        ...
        if previous_options and previous_options.get("ir_filters"):
            filters_map = {
                opt["id"]: opt["selected"] for opt in previous_options["ir_filters"]
            }
        else:
            filters_map = {}
        options["ir_filters"] = []
            ...
            options["ir_filters"].append(
                {
                    ...
                    "groupby": (
                        ir_filter.context and ast.literal_eval(ir_filter.context) or {}
                    ).get("group_by", []),
    ...
    def _get_options_groupby_fields(self, options):
        ...
        for option in options["ir_filters"]:
            ...
            for field in selected_fields:
                if field in AccountMoveLine._fields and self._is_allowed_groupby_field(
                    AccountMoveLine._fields[field]
                ):
    ...
    def _get_options(self, previous_options=None):
        # OVERRIDE
        options = super(ReportAccountFinancialReport, self)._get_options(
            previous_options
        )
        ...
        if self._context.get("financial_report_line_values"):
            options["financial_report_line_values"] = self.env.context[
    ...
    @api.model
    def _compute_growth_comparison_column(
    ...
    def _compute_debug_info_column(self, options, solver, financial_line):
        ...
        if financial_line.formulas:
            ...
            failed_control_domain = financial_line.id in options.get(
                "control_domain_missing_ids", []
            ) + options.get("control_domain_excess_ids", [])
            return {
                ...
                "popup_data": json.dumps(
                    {
                        ...
                        "formula_with_values": solver.get_formula_string(
                            financial_line
                        ),
                        "formula_balance": self._format_cell_value(
                            financial_line, sum(results["formula"].values())
                        ),
                        "domain": str(financial_line.domain)
                        if solver.is_leaf(financial_line) and financial_line.domain
                        else "",
                        "control_domain": failed_control_domain
                        and str(financial_line.control_domain),
    ...
    def _display_debug_info(self, options):
        ...
        """
        return (
            not self._context.get("print_mode")
            and self.user_has_groups("base.group_no_one")
            and (
                not options.get("comparison")
                or not options["comparison"].get("periods")
            )
        )
        ...
        for financial_line in financial_lines:
            ...
            if self.env.company.totals_below_sections and (
                ...
                financial_report_line[
                    "unfolded"
                ] = True  # enables adding "o_js_account_report_parent_row_unfolded" -> hides total amount in head line as it is displayed later in total line
    ...
    def _build_headers_hierarchy(self, options_list, groupby_keys):
        ...
        for groupby, ids_set in zip(groupby_list, keys_grouped_by_ids):
            ...
            if groupby_field.relational:
                ...
                sorted_records = (
                    self.env[groupby_field.comodel_name]
                    .with_context(active_test=False)
                    .search([("id", "in", tuple(ids_set))])
                )
                index = 0
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if model_name != "account.financial.html.report.line":
            raise Exception(
                "Error: trying to unfold a line which isn't a financial report line."
            )
        ...
        sorted_groupby_keys = [
            tuple(key) for key in options.get("sorted_groupby_keys", [(0,)])
        ]
        lines = self._build_lines_hierarchy(
    ...
    def _get_financial_line_report_line(
        ...
        report_line_id = self._get_generic_line_id(
            "account.financial.html.report.line", financial_line.id
        )
        ...
        if not has_something_to_unfold or financial_line.show_domain == "never":
            ...
            and report_line_id in options["unfolded_lines"]
        ):
            is_unfolded = True
        else:
            is_unfolded = False
        ...
        if self.user_has_groups("base.group_no_one"):
            ...
            if financial_line.control_domain:
                if not financial_line._check_control_domain(options, results, self):
                    ...
                    has_missing = solver._has_missing_control_domain(
                        options, financial_line
                    )
                    has_excess = solver._has_excess_control_domain(
                        options, financial_line
                    )
        ...
        if self._display_debug_info(options):
            columns.append(
                self._compute_debug_info_column(options, solver, financial_line)
            )
    ...
    def _get_financial_aml_report_line(
        ...
        groupby_model = (
            self.env["account.move.line"]._fields[financial_line.groupby].comodel_name
        )

        return {
            "id": self._get_generic_line_id(
                groupby_model, groupby_id, parent_line_id=financial_report_line_id
            ),
            "name": display_name,
    ...
    def _get_financial_total_section_report_line(self, options, financial_report_line):
        ...
        return {
            "id": self._get_generic_line_id(
                ...
                None,
                parent_line_id=financial_report_line["id"],
                markup="total",
            ),
    ...
    def copy(self, default=None):
        ...
        copied_report_id = super(ReportAccountFinancialReport, self).copy(
            default=default
        )
    ...
    def _create_action_and_menu(self, parent_id):
        ...
        for report in self:
            if not report.generated_menu_id:
                ...
                action_xmlid = "%s.%s" % (
                    module,
                    "account_financial_html_report_action_" + str(report.id),
                )
                ...
                menu_vals = {
                    "name": report._get_report_name(),
                    "parent_id": parent_id
                    or IMD._xmlid_to_res_id("account.menu_finance_reports"),
                    "action": "ir.actions.client,%s" % (action.id,),
                }
                menu_xmlid = "%s.%s" % (
                    module,
                    "account_financial_html_report_menu_" + str(report.id),
                )
    ...
    def _prepare_control_domain_action(self, options, missing=True):
        ...
        active_ids = (
            options.get("control_domain_missing_ids")
            if missing
            else options.get("control_domain_excess_ids")
        )
    ...
    def open_control_domain_missing(self, options, params=None):
        ...
        action["domain"] += expression.OR(
            [solver._get_missing_control_domain(options, line) for line in lines]
        )
    ...
    def open_control_domain_excess(self, options, params=None):
        ...
        lines, solver, action = self._prepare_control_domain_action(
            options, missing=False
        )
        action["domain"] += expression.OR(
            [solver._get_excess_control_domain(options, line) for line in lines]
        )
...
class AccountFinancialReportLine(models.Model):
    ...
    control_domain = fields.Char(
        default=None,
        help="Specify a control domain that will raise a warning if the report line is not computed correctly.",
    )
    formulas = fields.Char()
    groupby = fields.Char("Group by")
    figure_type = fields.Selection(
        [("float", "Float"), ("percents", "Percents"), ("no_unit", "No Unit")],
        "Type",
        default="float",
        required=True,
    )
    ...
    show_domain = fields.Selection(
        [("always", "Always"), ("never", "Never"), ("foldable", "Foldable")],
        default="foldable",
    )
        ...
        for rec in self:
            ...
            if rec.code:
                ...
                if rec.code and rec.code.strip().lower() in protected_codes:
                    raise ValidationError(
                        "The code '%s' is invalid on line with name '%s'"
                    ) % (rec.code, rec.name)

            # Check 'groupby' must be a valid field.
            if rec.groupby:
                groupby_field = AccountMoveLine._fields.get(rec.groupby)
                if not groupby_field or not self.env[
                    "account.financial.html.report"
                ]._is_allowed_groupby_field(groupby_field):
                    raise ValidationError(
                        _("Groupby field %s is invalid on line with name '%s'")
                        % (rec.groupby, rec.name)
                    )
            ...
            if rec.formulas:
                if (
                    any(
                        key in rec.formulas
                        for key in ("sum_if_pos_groupby", "sum_if_neg_groupby")
                    )
                    and not rec.groupby
                ):
                    raise ValidationError(
                        _(
                            "Please specify a Group by field when using '%s' in Formulas, on line with name '%s'"
                        )
    ...
    def _get_options_financial_line(
        self, options, calling_financial_report, parent_financial_report
    ):
        ...
        if (
            ...
            and calling_financial_report != parent_financial_report
        ):
    ...
    def _compute_amls_results(self, options_list, calling_financial_report, sign=1):
        ...
        horizontal_groupby_list = (
            AccountFinancialReportHtml._get_options_groupby_fields(options_list[0])
        )
    ...
    def _compute_control_domain(self, options_list, calling_financial_report):
        ...
        parent_financial_report = self._get_financial_report()
        groupby_list = parent_financial_report._get_options_groupby_fields(
            options_list[0]
        )
        groupby_clause = ",".join("account_move_line.%s" % gb for gb in groupby_list)
        ...
        for i, options in enumerate(options_list):
            ...
            control_domain = (
                self.control_domain
                and ast.literal_eval(ustr(self.control_domain))
                or []
            )

            tables, where_clause, where_params = parent_financial_report._query_get(
                new_options, domain=control_domain
            )
    ...
    def _check_control_domain(self, options, results, calling_financial_report):
        ...
        results_control = self._compute_control_domain(
            options_list, calling_financial_report
        )
    ...
    def _compute_sum(self, options_list, calling_financial_report):
        ...
        * (1,3): At the period 1 (first comparison), the results for 'partner_id = 3' are...
        ...
        groupby_in_formula = any(
            x in (self.formulas or "")
        ...
        groupby_clause = ",".join(
            "account_move_line.%s" % gb for gb in all_groupby_list
        )
    ...
    def _copy_hierarchy(
        self, report_id=None, copied_report_id=None, parent_id=None, code_mapping=None
    ):
        ...
        if (
            report_id
            and copied_report_id
            and self.financial_report_id.id == report_id.id
        ):
    ...
    def action_view_coa(self, options):
        ...
        amls = self.env["account.move.line"].search(
            expression.OR([missing_amls + excess_amls])
        )
