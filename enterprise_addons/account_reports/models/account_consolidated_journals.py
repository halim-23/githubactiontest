...
class report_account_consolidated_journal(models.AbstractModel):
    ...
    def _get_options(self, previous_options=None):
        options = super(report_account_consolidated_journal, self)._get_options(
    ...
    def _get_sum(self, results, lambda_filter):
        sum_debit = self.format_value(
            sum([r["debit"] for r in results if lambda_filter(r)])
        )
        sum_credit = self.format_value(
            sum([r["credit"] for r in results if lambda_filter(r)])
        )
        sum_balance = self.format_value(
            sum([r["balance"] for r in results if lambda_filter(r)])
        )
    ...
    def _get_journal_line(self, options, current_journal, results, record):
        return {
            ...
            "columns": [
                {"name": n}
                for n in self._get_sum(
                    results, lambda x: x["journal_id"] == current_journal
                )
            ],
            "unfoldable": True,
            "unfolded": self._need_to_unfold(
                "journal_%s" % (current_journal,), options
            ),
        }

    def _get_account_line(
        self, options, current_journal, current_account, results, record
    ):
        return {
            ...
            "columns": [
                ...
                for n in self._get_sum(
                    results,
                    lambda x: x["account_id"] == current_account
                    and x["journal_id"] == current_journal,
                )
            ],
            "unfoldable": True,
            "unfolded": self._need_to_unfold(
                "account_%s_%s" % (current_account, current_journal), options
            ),
        ...
        lines.append(
            {
                ...
                "columns": [
                    {"name": n}
                    for n in self._get_sum(
                        results, lambda x: x["company_id"] == current_company
                    )
                ],
            }
        )
        ...
        if dates:
            ...
            for date in sorted(dates):
                ...
                sum_debit = self.format_value(
                    sum(
                        [
                            r["debit"]
                            for r in results
                            if (r["month"] == month and r["yyyy"] == year)
                            and r["company_id"] == current_company
                        ]
                    )
                )
                sum_credit = self.format_value(
                ...
                vals = {
                    ...
                    "level": 2,
                    "columns": [
                        {"name": v} for v in [sum_debit, sum_credit, sum_balance]
                    ],
                }
                lines.append(vals)
        return lines
    ...
    def _get_lines(self, options, line_id=None):
        ...
        tables, where_clause, where_params = (
            self.env["account.move.line"].with_context(strict_range=True)._query_get()
        )
        line_model = None
        if line_id:
            ...
            where_clause += (
                line_model == "account"
                and " AND account_id = %s AND j.id = %s"
                or " AND j.id = %s"
            )
            ...
            if line_model == "account":
                where_params += [
                    str(split_line_id[2])
                ]  # We append the id of the parent journal in case of an account line
        ...
        current_journal = (
            line_model == "account" and results[0]["journal_id"] or None
        )  # If line_id points toward an account line, we don't want to regenerate the parent journal line
        for values in results:
            if values["journal_id"] != current_journal:
                current_journal = values["journal_id"]
                lines.append(
                    self._get_journal_line(options, current_journal, results, values)
                )

            if (
                self._need_to_unfold("journal_%s" % (current_journal,), options)
                and values["account_id"] != current_account
            ):
                current_account = values["account_id"]
                lines.append(
                    self._get_account_line(
                        options, current_journal, current_account, results, values
                    )
                )

            # If we need to unfold the line
            if self._need_to_unfold(
                "account_%s_%s" % (values["account_id"], values["journal_id"]), options
            ):
                vals = {
        ...
        if not line_id:
            lines.extend(
                self._get_line_total_per_month(options, values["company_id"], results)
            )
        return lines
