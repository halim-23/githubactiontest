...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        for move in self.filtered(
            lambda m: not m.posted_before and m.tax_closing_end_date
        ):
            # Make sure to use the company of the move for the carryover operations
            AccountGenericTaxReport = self.env[
                "account.generic.tax.report"
            ].with_company(move.journal_id.company_id)
            ...
            report_lines = AccountGenericTaxReport.with_context(new_context)._get_lines(
                options
            )
            ...
            for line in [
                ...
                if line["columns"][0].get("carryover_bounds", False)
            ]:
                ...
                country_id = (
                    self.env["account.tax.report"]
                    .browse(options["tax_report"])
                    .country_id
                )
                reports = self.env["account.tax.report"].search(
                ...
                for report in reports:
                    ...
                    old_carryover_balance = (
                        AccountGenericTaxReport.get_carried_over_balance_before_date(
                    ...
                    (
                        dummy,
                        carryover_balance,
                    ) = AccountGenericTaxReport.get_amounts_after_carryover(
                        carry_to_line,
                        line_balance,
                        carryover_bounds,
                        options,
                        0,
                        tax_line.is_carryover_persistent,
                    )
                    ...
                    # So we must make sure to set it back to 0.
                        ...
                        not tax_line.is_carryover_persistent
                        and not move.currency_id.is_zero(old_carryover_balance)
                    ):
                    ...
                    if not move.currency_id.is_zero(amount):
                        self.env["account.tax.carryover.line"].create(
                            {
                                "name": _(
                                    "Carryover for period %s to %s",
                                    new_context["date_from"],
                                    new_context["date_to"],
                                ),
                                "amount": amount,
