...
class AccountCashFlowReport(models.AbstractModel):
    ...
    def _get_liquidity_move_ids(self, options):
        ...
        tables, where_clause, where_params = self._query_get(
            new_options, [("account_id", "in", list(payment_account_ids))]
        )
    ...
    @api.model
    def _get_liquidity_move_report_lines(
        self, options, currency_table_query, payment_move_ids, payment_account_ids
    ):
        ...
        query = (
            ...
            + """ ON currency_table.company_id = credit_line.company_id
            ...
            LEFT JOIN """
            + currency_table_query
            + """ ON currency_table.company_id = debit_line.company_id
        ...
        for (
            ...
            reconciled_amount_per_account.setdefault(
                account_id,
        ...
        query = (
            ...
            LEFT JOIN """
            + currency_table_query
            + """ ON currency_table.company_id = line.company_id
        ...
        return [
            ...
            for k, v in reconciled_amount_per_account.items()
        ]

    @api.model
    def _get_reconciled_move_report_lines(
        self, options, currency_table_query, payment_move_ids, payment_account_ids
    ):
        ...
        self._cr.execute(
            query,
            [
                tuple(reconciled_percentage_per_move.keys()),
                tuple(reconciled_account_ids),
            ],
        )
        ...
        for (
            ...
            if (
                total_amount
                and account_id not in reconciled_percentage_per_move[move_id]
            ):
                ...
                not total_amount
                and account_id in reconciled_percentage_per_move[move_id]
            ):
            ...
            reconciled_amount_per_account.setdefault(
                account_id,
                [account_id, account_code, account_name, account_internal_type, 0.0],
            )
            reconciled_amount_per_account[account_id][4] += balance
    ...
    def _compute_liquidity_balance(
        self, options, currency_table_query, payment_account_ids
    ):
        ...
        new_options = self._get_options_current_period(options)
        tables, where_clause, where_params = self._query_get(
            new_options, domain=[("account_id", "in", payment_account_ids)]
        )
    ...
    def _get_lines(self, options, line_id=None):
        def _insert_at_index(index, account_id, account_code, account_name, amount):
            ...
            line["unfolded_lines"].setdefault(
                account_id,
                {
                    "id": account_id,
        ...
        def _dispatch_result(
            account_id, account_code, account_name, account_internal_type, amount
        ):
            ...
            if account_internal_type == "receivable":
                ...
                if tag_operating_id in tags_per_account.get(account_id, []):
                    ...
                    _insert_at_index(
                        11, account_id, account_code, account_name, -amount
                    )
                else:
                    # 'Cash in for unclassified activities'                 (index=14)
                    _insert_at_index(
                        14, account_id, account_code, account_name, -amount
                    )
            elif amount > 0:
                if tag_operating_id in tags_per_account.get(account_id, []):
                    ...
                    _insert_at_index(
                        12, account_id, account_code, account_name, -amount
                    )
                else:
                    # 'Cash out for unclassified activities'                (index=15)
                    _insert_at_index(
                        15, account_id, account_code, account_name, -amount
                    )

        self.flush()

        unfold_all = self._context.get("print_mode") or options.get("unfold_all")
        currency_table_query = self.env["res.currency"]._get_query_currency_table(
            options
        )
        ...
        for (
            ...
            account_name,
            balance,
        ) in self._compute_liquidity_balance(
            beginning_period_options, currency_table_query, payment_account_ids
        ):
        ...
        for (
            ...
            options, currency_table_query, payment_account_ids
        ):
            _insert_at_index(16, account_id, account_code, account_name, balance)

        # ==== Process liquidity moves ====
        res = self._get_liquidity_move_report_lines(
            options, currency_table_query, payment_move_ids, payment_account_ids
        )
        for (
            account_id,
            account_code,
        ...
        res = self._get_reconciled_move_report_lines(
            options, currency_table_query, payment_move_ids, payment_account_ids
        )
        for (
            account_id,
            ...
            _dispatch_result(
                account_id, account_code, account_name, account_internal_type, balance
            )
        ...
        for line in lines_to_compute:
            ...
            line["unfolded"] = line["unfoldable"] and (
                unfold_all or line["id"] in options["unfolded_lines"]
            )
            ...
            for sub_line in sub_lines:
                sub_line["columns"][0]["name"] = self.format_value(
                    sub_line["columns"][0]["name"]
                )
