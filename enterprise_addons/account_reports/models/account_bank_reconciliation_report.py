...
class AccountBankReconciliationReport(models.AbstractModel):
    ...
    def _apply_groups(self, columns):
        if self.user_has_groups("base.group_multi_currency") and self.user_has_groups(
            "base.group_no_one"
        ):
            return columns
    ...
    def _get_templates(self):
        ...
        templates[
            "main_template"
        ] = "account_reports.bank_reconciliation_report_main_template"
        return templates
    ...
    @api.model
    def _build_section_report_lines(
        self, options, journal, unfolded_lines, total, title, title_hover
    ):
    ...
    def _get_statement_report_lines(self, options, journal):
        ...
        tables, where_clause, where_params = self.with_company(
            journal.company_id
        )._query_get(
            options,
            domain=[
        ...
        """,
            [journal.suspense_account_id.id, journal.suspense_account_id.id]
            + where_params,
        )
        ...
        for res in self._cr.dictfetchall():
            ...
            is_parent_unfolded = (
                unfold_all or st_report_line["parent_id"] in options["unfolded_lines"]
            )
            if not is_parent_unfolded:
                st_report_line["style"] = "display: none;"

        return (
            self._build_section_report_lines(
                options,
                journal,
                plus_report_lines,
                plus_total,
                ...
                _(
                    "%s for Transactions(+) imported from your online bank account (dated today) that "
                    "are not yet reconciled in Odoo (Waiting the final reconciliation allowing finding the right "
                    "account)"
                )
                % journal.suspense_account_id.display_name,
            ),
            self._build_section_report_lines(
                options,
                journal,
        ...
        for res in self._cr.dictfetchall():
            ...
            if (
                ...
                if foreign_currency.is_zero(
                    amount_currency
                ) and company_currency.is_zero(balance):
                    continue
            ...
            if (
                res["account_id"]
                in journal._get_journal_inbound_outstanding_payment_accounts().ids
            ):
            ...
            is_parent_unfolded = (
                unfold_all or pay_report_line["parent_id"] in options["unfolded_lines"]
            )
            if not is_parent_unfolded:
    ...
    def _get_lines(self, options, line_id=None):
        ...
        options["unconsistent_statement_ids"] = self._get_unconsistent_statements(
            options, journal
        ).ids
        ...
        plus_st_lines, less_st_lines = self._get_statement_report_lines(
            options, journal
        )
        plus_pay_lines, less_pay_lines = self._get_payment_report_lines(
            options, journal
        )
