...
class AccountReportManager(models.Model):
    ...
    def add_footnote(self, text, line):
        return self.env["account.report.footnote"].create(
            {"line": line, "text": text, "manager_id": self.id}
        )
...
class AccountReport(models.AbstractModel):
    ...
    def _init_filter_journals(self, options, previous_options=None):
        ...
        if previous_options and previous_options.get("journals"):
            journal_map = {
                opt["id"]: opt["selected"]
                for opt in previous_options["journals"]
        ...
        if all_journal_groups:
            ...
            for group in all_journal_groups:
                ...
                options["journals"].append(
                    {"id": "group", "name": group.name, "ids": group_ids}
                )
        ...
        for journal in all_journals:
            if journal.company_id != previous_company:
                options["journals"].append(
                    {"id": "divider", "name": journal.company_id.name}
                )
        ...
        if group_selected:
            ...
            options["name_journal_group"] = ", ".join(
                jrnl["code"] for jrnl in journals_sel
            )
            ...
            options["name_journal_group"] = ", ".join(
                jrnl["code"] for jrnl in journals_sel
            ) + _(" and %s others", len(journals_sel) - 5)
    ...
    def _get_dates_period(
        self, options, date_from, date_to, mode, period_type=None, strict_range=False
    ):
    ...
    def _init_filter_date(self, options, previous_options=None):
        ...
        if previous_mode == "single" and options_mode == "range":
            ...
            if previous_filter == "custom":
                date_to = fields.Date.from_string(
                    previous_date_to or previous_date_from
                )
        ...
        if not date_from or not date_to:
            if options_filter == "today":
                ...
                date_from = (
                    fields.Date.from_string(custom_date_from)
                    if custom_date_from
                    else date_utils.get_month(date_to)[0]
                )
    ...
    def _init_filter_comparison(self, options, previous_options=None):
        ...
        if previous_filter == "custom":
            ...
            if options["date"]["mode"] == "range":
                ...
                date_from = False
                date_to = self.filter_comparison.get(
                    "date_to"
                ) or self.filter_comparison.get("date_from")
            number_period = 1
            options_filter = "custom"
        else:
            ...
            number_period = previous_comparison.get(
                "number_period"
            ) or self.filter_comparison.get("number_period", 1)
    ...
    def _init_filter_analytic(self, options, previous_options=None):
        ...
        enable_analytic_accounts = self.user_has_groups(
        ...
        if enable_analytic_accounts:
            previous_analytic_accounts = (previous_options or {}).get(
                "analytic_accounts", []
            )
        ...
        if enable_analytic_tags:
            ...
            selected_analytic_tags = self.env["account.analytic.tag"].search(
                [("id", "in", analytic_tag_ids)]
            )
            options["analytic_tags"] = selected_analytic_tags.ids
            options["selected_analytic_tag_names"] = selected_analytic_tags.mapped(
                "name"
            )
    ...
    def _init_filter_partner(self, options, previous_options=None):
        ...
        options["partner_ids"] = (
            previous_options and previous_options.get("partner_ids") or []
        )
        options["partner_categories"] = (
            previous_options and previous_options.get("partner_categories") or []
        )
        selected_partner_ids = [int(partner) for partner in options["partner_ids"]]
        selected_partners = (
            selected_partner_ids
            and self.env["res.partner"].browse(selected_partner_ids)
    ...
    def _init_order_selected_column(self, options, previous_options=None):
        if self.order_selected_column is not None:
            options["selected_column"] = (
                previous_options
                and previous_options.get("selected_column")
                or self.order_selected_column["default"]
            )
    ...
    def _create_hierarchy(self, lines, options):
        ...
        def add_to_hierarchy(lines, key, level, parent_id, hierarchy):
            ...
            if not self._context.get("print_mode") or unfolded:
                ...
                for child in sorted(val_dict["children_codes"]):
                    add_to_hierarchy(
                        children, child, level + 1, val_dict["id"], hierarchy
                    )
                ...
                lines.extend(
                    sorted(
                        val_dict["lines"] + children,
                        key=lambda k: k.get("account_code", "") + k["name"],
                    )
                )
        ...
        def compute_hierarchy(lines, level, parent_id):
            ...
            hierarchy = defaultdict(
                lambda: {
                    "totals": [None] * len(lines[0]["columns"]),
                    "lines": [],
                    "children_codes": set(),
                    "name": "",
                    "parent_id": None,
            ...
            for line in lines:
                account = self.env["account.account"].browse(
                    line.get(
                        "account_id", self._get_caret_option_target_id(line.get("id"))
                    )
                )
                codes = self.get_account_codes(account)  # id, name
                for code in codes:
                    hierarchy[code[0]]["id"] = self._get_generic_line_id(
                        "account.group", code[0], parent_line_id=line["id"]
                    )
        ...
        for line in lines:
            if not (
                line.get("caret_options") == "account.account" or line.get("account_id")
            ):
                # make the hierarchy with the lines we gathered, append it to the new lines and restart the gathering
                if account_lines:
                    new_lines.extend(
                        compute_hierarchy(account_lines, current_level + 1, parent_id)
                    )
                account_lines = []
    ...
    def _init_filter_fiscal_position(self, options, previous_options=None):
        ...
        vat_fpos_domain = [
            (
                "company_id",
                "in",
                [comp["id"] for comp in options.get("multi_company", self.env.company)],
            ),
            ("foreign_vat", "!=", False),
        ]
        ...
        if country:
            ...
            options["allow_domestic"] = (
                self.env.company.account_fiscal_country_id == country
            )
            ...
            if (
                previous_options
                and previous_options.get("fiscal_position") in accepted_prev_vals
            ):
    ...
    def _get_options(self, previous_options=None):
        ...
        options = {
            "unfolded_lines": previous_options
            and previous_options.get("unfolded_lines")
            or [],
        }
    ...
    def _get_forced_filter_init_sequence_map(self):
        ...
        """
        return {
            "filter_multi_company": 10,
            "filter_fiscal_position": 20,
            "filter_date": 30,
            "filter_comparison": 40,
        }
    ...
    def _build_line_id(self, current):
        ...
        return "|".join(
            "%s-%s-%s" % (markup, convert_none(model), convert_none(value))
        ...
        return (
            ...
            and [
                ...
                for markup, model, value in (
                    key.split("-") for key in line_id.split("|")
                )
            ]
            or []
        )
        ...
        if "selected_column" in options and self.order_selected_column:
            ...
            if "sortable" in selected_column.get("class", ""):
                selected_column["class"] = (
                    options["selected_column"] > 0 and "up " or "down "
                ) + selected_column["class"]
        return columns
    ...
    def execute_action(self, options, params=None):
        ...
        if action_type == "ir.actions.client":
            ...
            if action.tag == "account_report":
                ...
                action_read.update(
                    {"params": {"options": options, "ignore_session": "read"}}
                )
        if params.get("id"):
            ...
            context = (
                ...
                and ast.literal_eval(action_read["context"])
                or {}
            )
    ...
    def open_document(self, options, params=None):
        ...
        # Redirect
        view_id = self.env["ir.model.data"]._xmlid_lookup(
            "%s.%s" % (module, view_name)
        )[2]
        return {
    ...
    def periodic_vat_entries(self, options):
        ...
        moves = self.env["account.generic.tax.report"]._generate_tax_closing_entries(
            options
        )
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account.action_move_journal_line"
    ...
    def action_partner_reconcile(self, options, params):
        form = self.env.ref(
            "account_accountant.action_manual_reconciliation", False
        ).sudo()
        ctx = self.env.context.copy()
    ...
    def open_journal_items(self, options, params):
        ...
        if options:
            domain = expression.normalize_domain(
                ast.literal_eval(action.get("domain") or "[]")
            )
            if options.get("journals"):
                ...
                if len(selected_journals) == 1:
                    ...
                    selected_journals
                ):  # Otherwise, nothing is selected, so we want to display everything
                    domain = expression.AND(
                        [domain, [("journal_id", "in", selected_journals)]]
                    )
            ...
            if params.get("financial_group_line_id"):
                ...
                if not (
                    isinstance(params["financial_group_line_id"], str)
                    and "hierarchy_" in params["financial_group_line_id"]
                ):
                    parent_financial_report_line = self.env[
                        "account.financial.html.report.line"
                    ].browse(params["financial_group_line_id"])
                    domain = expression.AND(
                        [domain, ast.literal_eval(parent_financial_report_line.domain)]
                    )
    ...
    def _sort_lines(self, lines, options):
        ...
        def merge_tree(line):
            ...
            for l in sorted(
                tree[line["id"]],
                key=lambda k: selected_sign
                * k["columns"][selected_column - k.get("colspan", 1)]["no_format"],
            ):
                merge_tree(l)
        ...
        if "sortable" not in self._get_columns_name(options)[selected_column].get(
            "class", ""
        ):
    ...
    def _set_context(self, options):
        ...
        if options.get("date"):
            ctx["date_to"] = options["date"].get("date_to") or options["date"].get(
                "date"
            )
        ...
        if options.get("journals"):
            ctx["journal_ids"] = [
                j.get("id") for j in options.get("journals") if j.get("selected")
            ]
        if options.get("analytic_accounts"):
    ...
    def get_report_informations(self, options):
        ...
        )  # For multicompany, when allowed companies are changed by options (such as aggregare_tax_unit)
        ...
        if options.get("analytic_tags") is not None:
            options["selected_analytic_tag_names"] = [
                self.env["account.analytic.tag"].browse(int(tag)).name
                for tag in options["analytic_tags"]
            ]
        if options.get("partner"):
            options["selected_partner_ids"] = [
                self.env["res.partner"].browse(int(partner)).name
                for partner in options["partner_ids"]
            ]
            options["selected_partner_categories"] = [
                self.env["res.partner.category"].browse(int(category)).name
                for category in (options.get("partner_categories") or [])
            ]
        ...
        info = {
            ...
            "footnotes": [
                {"id": f.id, "line": f.line, "text": f.text}
            ...
            "searchview_html": self.env["ir.ui.view"]._render_template(
                self._get_templates().get(
                    "search_template", "account_report.search_template"
                ),
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the line_template
        ...
        if self.env.context.get("print_mode", False):
            ...
            html = html.replace(
                markupsafe.Markup('<div class="js_account_report_footnotes"></div>'),
                self.get_html_footnotes(footnotes_to_render),
            )
        return html

    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get(
            "footnotes_template", "account_reports.footnotes_template"
        )
    ...
    def _get_reports_buttons_in_sequence(self, options):
        return sorted(
            self._get_reports_buttons(options), key=lambda x: x.get("sequence", 9)
        )
    ...
    def open_report_export_wizard(self, options):
        ...
        new_wizard = (
            ...
            .env["account_reports.export.wizard"]
            .create({"report_model": self._name, "report_id": getattr(self, "id", 0)})
        )
    ...
    def _get_report_manager(self, options):
        ...
        domain = (
            (domain + [("financial_report_id", "=", self.id)])
            if "id" in dir(self)
            else domain
        )
