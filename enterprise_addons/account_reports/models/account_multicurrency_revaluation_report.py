...
from odoo.tools import classproperty, float_is_zero
...
class MulticurrencyRevaluationReport(models.Model):
    ...
    _order = (
        "report_include desc, currency_code desc, account_code asc, date desc, id desc"
    )
    ...
    def _get_templates(self):
        ...
        templates[
            "line_template"
        ] = "account_reports.line_template_multicurrency_report"
    ...
    def _get_options(self, previous_options=None):
        ...
        rates = (
            ...
            .search([("active", "=", True)])
            ._get_rates(self.env.company, options.get("date").get("date_to"))
        )
        ...
        if value_dict.get("report_currency_id"):
            res["columns"][0] = {
                "name": self.format_value(
                    value_dict["report_amount_currency"],
    ...
    def _format_report_currency_id_line(self, res, value_dict, options):
        res["name"] = "{for_cur} (1 {comp_cur} = {rate:.6} {for_cur})".format(
            ...
            rate=float(
                options["currency_rates"][str(value_dict.get("report_currency_id")[0])][
                    "rate"
                ]
            ),
        )
    ...
    def _format_report_include_line(self, res, value_dict, options):
        res["name"] = (
            _("Accounts to adjust")
            if value_dict.get("report_include")
            else _("Excluded Accounts")
        )
        res["columns"] = [{}, {}, {}, {}]
    ...
    def view_revaluation_wizard(self, context):
        ...
        form = self.env.ref(
            "account_reports.view_account_multicurrency_revaluation_wizard", False
        )
        return {
    ...
    def view_currency(self, options, params=None):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": _(
                "Currency Rates (%s)", self.env["res.currency"].browse(id).display_name
            ),
            "views": [(False, "list")],
