...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates[
            "main_template"
        ] = "account_reports.template_aged_partner_balance_report"
        return templates
    ...
    def _get_query_period_table(self, options):
        ...
        period_table = (
            "(VALUES %s) AS period_table(date_start, date_stop, period_index)"
        ...
        return self.env.cr.mogrify(period_table, params).decode(
    ...
    def _get_hierarchy_details(self, options):
        return [
            self._hierarchy_level(
                "partner_id",
                foldable=True,
                namespan=len(self._get_column_details(options)) - 7,
            ),
    ...
    def _show_line(self, report_dict, value_dict, current, options):
        ...
        return (
            super()._show_line(report_dict, value_dict, current, options)
            and not all_zero
        )

    def _format_partner_id_line(self, res, value_dict, options):
        res["name"] = (
            value_dict["partner_name"][:128]
            if value_dict["partner_name"]
    ...
    def _format_id_line(self, res, value_dict, options):
        ...
        res["caret_options"] = (
            "account.payment" if value_dict.get("payment_id") else "account.move"
        )
...
class ReportAccountAgedReceivable(models.Model):
    ...
    def _get_options(self, previous_options=None):
        # OVERRIDE
        options = super(ReportAccountAgedReceivable, self)._get_options(
    ...
    def _get_templates(self):
        ...
        templates[
            "line_template"
        ] = "account_reports.line_template_aged_receivable_report"
        return templates
