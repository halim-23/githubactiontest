...
class AccountMove(models.Model):
    ...
    tax_closing_end_date = fields.Date(
        help="Technical field used for VAT closing, containig the end date of the period this entry closes."
    )
    tax_report_control_error = fields.Boolean(
        help="technical field used to know if there was a failed control check"
    )

    def action_open_tax_report(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_reports.action_account_report_gt"
        )
    ...
    def refresh_tax_entry(self):
        for move in self.filtered(
            lambda m: m.tax_closing_end_date and m.state == "draft"
        ):
            ...
            move.env["account.generic.tax.report"].with_context(
                ctx
            )._generate_tax_closing_entries(options, closing_moves=move)
    ...
    def _get_report_options_from_tax_closing_entry(self):
        ...
        tax_report = self.env["account.tax.report"].search(
            [("country_id", "=", report_country.id)], limit=1
        )
    ...
    def _close_tax_entry(self):
        ...
        for move in self:
            ...
            attachments = (
                ...
                .with_context(ctx)
                ._get_vat_report_attachments(options)
            )

            # End activity
            activity = move.activity_ids.filtered(
                lambda m: m.activity_type_id.id == tax_closing_activity_type.id
            )
            if activity:
                activity.action_done()

            # Post the message with the PDF
            subject = _("Vat closing from %s to %s") % (
                format_date(self.env, options.get("date").get("date_from")),
                format_date(self.env, options.get("date").get("date_to")),
            )
            move.with_context(no_new_invoice=True).message_post(
                body=move.ref, subject=subject, attachments=attachments
            )
            ...
            move.company_id._get_and_update_tax_closing_moves(
                move.tax_closing_end_date + relativedelta(days=1), **next_closing_params
            )
