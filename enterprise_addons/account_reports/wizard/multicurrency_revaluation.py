...
class MulticurrencyRevaluationWizard(models.TransientModel):
    ...
    date = fields.Date(
        default=lambda self: self._context.get("date").get("date_to"), required=True
    )  # TODO change defult dates
    ...
    def default_get(self, default_fields):
        ...
        if "reversal_date" in default_fields:
            rec["reversal_date"] = fields.Date.to_date(
                self._context.get("date").get("date_to")
            ) + relativedelta(days=1)
        if (
            not self._context.get("revaluation_no_loop")
            and not self.with_context(revaluation_no_loop=True)._compute_move_vals()[
                "line_ids"
            ]
        ):
        ...
        for record in self:
            preview_vals = [
                self.env["account.move"]._move_dict_to_preview_vals(
                    self._compute_move_vals(), record.company_id.currency_id
                )
            ]
            preview_columns = [
    ...
    def _compute_move_vals(self):
        ...
        line_dict = self.env["account.multicurrency.revaluation"]._get_values(
            options=options, line_id="report_include--True"
        )["children"][("report_include", None, True)]
        value_getter = (
            self.env["account.multicurrency.revaluation"]
            ._get_column_details(options=options)[-1]
            .getter
        )
        move_lines = []
        if line_dict and line_dict["children"]:
            for (_key1, _key2, currency_id), account_info in line_dict[
                "children"
            ].items():
                for (_key1, _key2, account_id), values in account_info[
                    "children"
                ].items():
                    ...
                    if not float_is_zero(
                        ...
                        move_lines.append(
                            (
                                ...
                                {
                                    "name": (
                                        _("Expense Provision for {for_cur}")
                                        if balance < 0
                                        else _("Income Provision for {for_cur}")
                                    ).format(
                                        for_cur=self.env["res.currency"]
                                        .browse(currency_id)
                                        .display_name,
    ...
    def _inverse_expense_provision_account(self):
        for record in self:
            record.company_id.sudo().account_revaluation_expense_provision_account_id = (
                record.expense_provision_account_id
            )
