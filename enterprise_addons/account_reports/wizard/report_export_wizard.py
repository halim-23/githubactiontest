...
class ReportExportWizard(models.TransientModel):
    ...
    def create(self, vals):
        ...
        rslt.doc_name = (
            hasattr(report, "name") and report.name or report._description
        )  # account.financial.html.report objects have a name field, not account.report ones
        ...
        for button_dict in report._get_reports_buttons_in_sequence(
            self._context.get("account_report_generation_options", {})
        ):
    ...
    def _get_attachments_to_save(self):
        ...
        for format in self.export_format_ids:
            ...
            if report_action["type"] == "ir_actions_account_report_download":
                ...
                if (
                    mimetype is not False
                ):  # We let the option to set a None value for it
                    ...
                    file_content = base64.encodebytes(
                        generation_function(report_options)
                    )
            ...
            to_create_attachments.append(
                self.get_attachment_vals(
...
class ReportExortWizardOption(models.TransientModel):
    ...
    def apply_export(self, report_options):
        self.ensure_one()
        report = self.env[self.export_wizard_id.report_model].browse(
            self.export_wizard_id.report_id
        )
