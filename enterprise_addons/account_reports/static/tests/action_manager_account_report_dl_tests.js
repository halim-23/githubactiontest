odoo.define("account_reports.account_reports_tests", function (require) {
  ...
  QUnit.module("Account Reports", {}, function () {
    ...
    QUnit.test("Account report m2m filters", async function (assert) {
        ...
        mockRPC: function (route, args) {
          if (route === "/web/dataset/call_kw/account.report/get_report_informations") {
            ...
            if (count === 1) {
              ...
              reportOptions = args.args[1];
              assert.strictEqual(
                reportOptions.partner_categories[0],
                1,
                "pass correct partner_id to report"
              );
      ...
      assert.containsOnce(
        webClient,
        '.o_control_panel .o_field_many2manytags[name="partner_ids"]',
        "partner_ids m2m field added to filter"
      );

      // Search on partners m2m
      await testUtils.dom.click(
        $(webClient.el).find(".o_control_panel .o_search_options a.dropdown-toggle")
      );
      ...
      assert.containsOnce(
        webClient,
        '.o_control_panel .o_field_many2manytags[name="partner_categories"]',
      ...
      await testUtils.nextTick();
      await testUtils.fields.many2one.clickItem(
        "partner_categories",
