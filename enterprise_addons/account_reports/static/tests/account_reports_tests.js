odoo.define("account_reports/static/tests/account_reports_tests", function (require) {
  ...
  QUnit.module(
      ...
      beforeEach: function () {
        this.models = {
          partner: {
            ...
            records: [{id: 1, display_name: "Genda Swami"}],
          },
        };
        this.views = {
        ...
        this.mockRPC = function (route) {
          if (route === "/web/dataset/call_kw/account.report/get_report_informations") {
            ...
            route === "/web/dataset/call_kw/account.report/get_html_footnotes"
          ) {
            return Promise.resolve("");
          }
        };
    ...
    () => {
      QUnit.test(
        "mounted is called once when returning on 'Account Reports' from breadcrumb",
      ...
      QUnit.test(
        "recomputeHeader is unregistered when leaving the 'Account Reports' view",
        async function (assert) {
          assert.expect(1);
