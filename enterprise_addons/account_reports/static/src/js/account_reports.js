  ...
  var accountReportsWidget = AbstractAction.extend({
    ...
    init: function (parent, action) {
      ...
      if ((this.ignore_session === "read" || this.ignore_session === "both") !== true) {
        var persist_key =
          ...
          ":" +
          session.company_id;
        var company_reload_persist_key = this.get_persist_options_key_for_company_reload(
          session.user_context.allowed_company_ids
        );

        this.report_options =
          JSON.parse(sessionStorage.getItem(company_reload_persist_key)) ||
    ...
    start: async function () {
      ...
      if ("default_filter_accounts" in (this.odoo_context || {}))
        this.$(".o_account_reports_filter_input")
          .val(this.odoo_context.default_filter_accounts)
          .trigger("input");
    },
    ...
    persist_options: function () {
      if (
        (this.ignore_session === "write" || this.ignore_session === "both") !== true
      ) {
        var persist_key =
          "report:" +
          this.report_model +
    ...
    render: function () {
      ...
      this.batch_fold(
        this.$(".js_account_report_foldable").filter(function () {
    ...
    _init_line_popups: function () {
      ...
      _.each(this.$(".o_account_report_popup"), function (popup) {
        $(popup).popover({
          html: true,
          template:
            "<div class='popover' role='tooltip' style='max-width: 100%; margin-right:80px;'><div class='popover-body'></div></div>",
          placement: "left",
          trigger: "focus",
          container: "body",
            ...
            _.each($content.find(".js_popup_formula"), function (element) {
              $(element).on("mouseenter", function (event) {
                ...
                self
                  ...
                  .addClass("o_financial_report_hover_popup");
              });
    ...
    filter_accounts: function (e) {
      ...
      this.$(".o_account_searchable_line").each(function (index, el) {
        ...
        var line_id = $accountReportLineFoldable
          .find(".o_account_report_line")
          .data("id");
        var $childs = self.$(
          'tr[data-parent-id="' + $.escapeSelector(String(line_id)) + '"]'
        );

        // Only the direct text node, not text situated in other child nodes
        const lineContent = $accountReportLineFoldable
          .find(".account_report_line_name")
          .contents();
        const lineText =
          lineContent.length > 0 ? lineContent.get(0).nodeValue.trim() : "";
        ...
        $accountReportLineFoldable.toggleClass(
      ...
      if (self.report_options.selected_column !== undefined) {
        var col_number =
          Array.prototype.indexOf.call(
            ...
            e.currentTarget
          ) + 1; // We can't have negative 0 so lets start index at 1
        if (
          self.report_options.selected_column &&
    ...
    _onChangeExpectedDate: function (event) {
      ...
      paymentDatePicker.appendTo(
        $content.find("div.o_account_reports_payment_date_picker")
      );
      var save = function () {
        return this._rpc({
          model: "res.partner",
    ...
    render_searchview_buttons: function () {
      ...
      this.$searchview_buttons.find(".js_foldable_trigger").click(function (event) {
        ...
        self.$searchview_buttons
          .find('.o_foldable_menu[data-filter="' + $(this).data("filter") + '"]')
      ...
      _.each(this.$searchview_buttons.find(".js_account_report_bool_filter"), function (
        k
      ) {
        ...
        function (k) {
          $(k).toggleClass(
            ...
            _.filter(self.report_options[$(k).data("filter")], function (el) {
              return String(el.id) == String($(k).data("id")) && el.selected === true;
            }).length > 0
          );
        }
      );
      $(".js_account_report_group_choice_filter", this.$searchview_buttons).each(
        function (i, el) {
          var $el = $(el);
      ...
      $(".js_account_report_group_choice_filter", this.$searchview_buttons).click(
        function () {
          var option_value = $(this).data("filter");
      ...
      this.$searchview_buttons
        ...
        .click(function (event) {
          ...
          _.filter(self.report_options[option_value], function (el) {
            if (String(el.id) == String(option_id)) {
        ...
        .click(function (event) {
          ...
          if (option_value === "tax_unit") {
            ...
            if (self.report_options.tax_unit != "company_only") {
              ...
              var selected_unit = self.report_options.available_tax_units.filter(
    ...
    edit_summary: function (e) {
      var $textarea = $(e.target)
        .parents(".o_account_reports_body")
        .find('textarea[name="summary"]');
      var height = Math.max(
        $(e.target)
          .parents(".o_account_reports_body")
          .find(".o_account_report_summary")
          .height(),
        100
      ); // Compute the height that will be needed
      ...
      $(e.target)
        .parents(".o_account_reports_body")
        .find(".o_account_reports_summary_edit")
        .show();
      $(e.target)
        .parents(".o_account_reports_body")
        .find(".o_account_reports_summary")
        .hide();
      $(e.target)
        .parents(".o_account_reports_body")
        .find('textarea[name="summary"]')
    ...
    save_summary: function (e) {
      ...
      return this._rpc({
        ...
        return $(e.target)
          .parent()
          .siblings(".o_account_reports_summary")
          .find("> .o_account_report_summary")
          .html($content);
      });
    },
    render_footnotes: function () {
      ...
      var $dom_footnotes = self.$el.find(
        ".js_account_report_line_footnote:not(.folded)"
      );
      $dom_footnotes.html("");
      var number = 1;
      ...
      _.each($dom_footnotes, function (el) {
        ...
        var footnote = _.filter(self.footnotes, function (footnote) {
          return String(footnote.line) === String(line_id);
        });
        if (footnote.length !== 0) {
          $(el).html(
            '<sup><b class="o_account_reports_footnote_sup"><a href="#footnote' +
              number +
              '">' +
              number +
              "</a></b></sup>"
    ...
    add_edit_footnote: function (e) {
      ...
      var $content = $(
        QWeb.render("accountReports.footnote_dialog", {text: text, line: line_id})
      );
      var save = function () {
        var footnote_text = $(".js_account_reports_footnote_note")
          .val()
          .replace(/[ \t]+/g, " ");
        ...
        if (existing_footnote.length !== 0) {
          ...
          return this._rpc({
            ...
            method: "create",
            args: [
              {line: line_id, text: footnote_text, manager_id: self.report_manager_id},
            ],
            context: this.odoo_context,
          }).then(function (result) {
      ...
      new Dialog(this, {
        ...
        $content: $content,
        buttons: [
          {text: "Save", classes: "btn-primary", close: true, click: save},
    ...
    fold_unfold: function (e) {
      ...
      if (
        ...
        $(e.target).parents(".o_account_reports_footnote_sup").length > 0
      ) {
        return;
      }
      e.stopPropagation();
      ...
      var method =
        line[0].dataset.unfolded === "True" ? this.batch_fold(line) : this.unfold(line);
    ...
    fold: function (line) {
      ...
      line
        .find(".o_account_reports_caret_icon .fa-caret-down")
        .toggleClass("fa-caret-right fa-caret-down");
      line.toggleClass("folded");
      $(line).parent("tr").removeClass("o_js_account_report_parent_row_unfolded");
      var $lines_to_hide = this.$el.find(
        'tr[data-parent-id="' + $.escapeSelector(String(line_id)) + '"]'
      );
    ...
    unfold: function (line) {
      ...
      if ($lines_in_dom.length > 0) {
        ...
        $lines_in_dom.show();
        line
          .find(".o_account_reports_caret_icon .fa-caret-right")
          .toggleClass("fa-caret-right fa-caret-down");
