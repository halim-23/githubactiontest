...
class TestTaxReport(TestAccountReportsCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        tax_templates = cls.env["account.tax.template"].search(
            [
                (
                    "chart_template_id",
                    "=",
                    cls.company_data["company"].chart_template_id.id,
                )
            ]
        )
        ...
        cls.basic_tax_report = cls.env["account.tax.report"].create(
            {"name": "The Unseen Tax Report", "country_id": cls.fiscal_country.id}
        )
        ...
        cls.test_fpos_tax_purchase = cls._add_basic_tax_for_report(
            ...
            cls.tax_group_2,
            [
                (10, cls.tax_account_1, False),
                (60, cls.tax_account_1, True),
                (-5, cls.tax_account_2, True),
            ],
        )
    ...
    def _instantiate_basic_test_tax_group(cls):
        return cls.env["account.tax.group"].create(
            {
                "name": "Test tax group",
                "property_tax_receivable_account_id": cls.company_data[
                    "default_account_receivable"
                ]
                .copy()
                .id,
                "property_tax_payable_account_id": cls.company_data[
                    "default_account_payable"
                ]
                .copy()
                .id,
    ...
    def _add_basic_tax_for_report(
        ...
        for move_type_suffix in ("invoice", "refund"):
            ...
            base_tag = base_report_line.tag_ids.filtered(
                lambda x: x.tax_negate == tax_negate
            )
            ...
            for (factor_percent, account, use_in_tax_closing) in tax_repartition:
                ...
                tax_tag = tax_report_line.tag_ids.filtered(
                    lambda x: x.tax_negate == tax_negate
                )
    ...
    def test_vat_closing_single_fpos(self):
        ...
        options = self._init_options(
            ...
            {
                "tax_report": self.basic_tax_report.id,
            ...
            options,
            {
                self.foreign_vat_fpos: [
                    ...
                    # For sales operations
                    {
                        "debit": 0,
                        "credit": 180,
                        "account_id": self.tax_group_1.property_tax_payable_account_id.id,
                    },
                    # For purchase operations
                    {
                        "debit": 110,
                        "credit": 0,
                        "account_id": self.tax_group_2.property_tax_receivable_account_id.id,
                    },
                ]
            },
        )
    ...
    def test_vat_closing_generic(self):
        ...
        One closing move should then be generated per fiscal position.
        """
        for generic_report in (
            "generic",
            "generic_grouped_tax_account",
            "generic_grouped_account_tax",
        ):
            options = self._init_options(
                self.env["account.generic.tax.report"],
                fields.Date.from_string("2021-01-15"),
    ...
    def test_tax_report_single_fpos(self):
        ...
        new_tax_report = self.env["account.tax.report"].create(
            {"name": "", "country_id": new_country.id}
        )
    ...
    def test_tax_report_grid(self):
        ...
        tax_42_section = self._create_tax_report_line(
            "Tax 42%", tax_report, sequence=1, parent_line=tax_section, code="tax_42"
        )
        ...
        tax_neg_10_line = self._create_tax_report_line(
            "Tax -10%",
            tax_report,
            sequence=3,
            parent_line=tax_section,
        ...
        tax_difference_line = self._create_tax_report_line(
            "Tax difference (42%-11%)",
            tax_report,
            sequence=3,
            formula="tax_42 - tax_11",
        )
        ...
        tax_11 = tax_template_11._generate_tax(company)["tax_template_to_tax"][
            tax_template_11
        ]
        ...
        tax_42 = tax_template_42._generate_tax(company)["tax_template_to_tax"][
            tax_template_42
        ]
    ...
    def _create_caba_taxes_for_report_lines(self, report_lines_dict, company):
        ...
        for tax_type, report_line in report_lines_dict.items():
            ...
            rslt += tax_template._generate_tax(self.env.user.company_id)[
                "tax_template_to_tax"
            ][tax_template]
    ...
    def test_tax_report_grid_cash_basis_misc_pmt(self):
        ...
        def reconcile_with_misc_pmt(invoice):
            ...
            invoice_reconcilable_line = invoice.line_ids.filtered(
                lambda x: x.account_internal_type in ("payable", "receivable")
            )
    ...
    def test_caba_mixed_generic_report(self):
        ...
        report = self.env["account.generic.tax.report"]
        options = self._init_options(
            report, invoice.date, invoice.date, {"tax_report": "generic"}
        )
        self.assertLinesValues(
            report._get_lines(options),
            #   Name                         Net              Tax
    ...
    def test_tax_report_mixed_exigibility_affect_base_tags(self):
        ...
        # Create taxes
        tax_report = self.env["account.tax.report"].create(
            {"name": "Sokovia Accords", "country_id": self.fiscal_country.id}
        )
    ...
    def _run_check_suite_mixed_exigibility_affect_base(
        ...
        # Check the report
        report = self.env["account.generic.tax.report"]
        report_options = self._init_options(
            report, invoice.date, invoice.date, {"tax_report": tax_report_option}
        )
        self.assertLinesValues(
            report._get_lines(report_options), report_columns, vals_not_paid
        )
        ...
        self.assertLinesValues(
            report._get_lines(report_options), report_columns, vals_30_percent_paid
        )
        ...
        # Check the report
        self.assertLinesValues(
            report._get_lines(report_options), report_columns, vals_fully_paid
        )
    ...
    def test_caba_always_exigible(self):
        ...
        """
        tax_report = self.env["account.tax.report"].create(
            {"name": "Laplace's Box", "country_id": self.fiscal_country.id}
        )
        ...
        report = self.env["account.generic.tax.report"]
        report_options = self._init_options(
            report, move.date, move.date, {"tax_report": tax_report.id}
        )
        self.assertLinesValues(
    ...
    def test_fiscal_position_switch_all_option_flow(self):
        ...
        foreign_tax_report = self.env["account.tax.report"].create(
            {"name": "", "country_id": foreign_country.id}
        )
        foreign_vat_fpos = self.env["account.fiscal.position"].create(
            {
                "name": "Test fpos",
        ...
        to_check = report._get_options(
            {"fiscal_position": "all", "tax_report": self.basic_tax_report.id}
        )
        self.assertEqual(
        ...
        to_check = report._get_options(
            {"fiscal_position": "all", "tax_report": foreign_tax_report.id}
        )
        self.assertEqual(
            to_check["fiscal_position"],
            foreign_vat_fpos.id,
        ...
        to_check = report._get_options(
            {"fiscal_position": foreign_vat_fpos.id, "tax_report": "generic"}
        )
        self.assertEqual(
            to_check["fiscal_position"],
            "all",
    ...
    def test_tax_report_multi_inv_line_no_rep_account(self):
        ...
        report = self.env["account.generic.tax.report"]
        options = self._init_options(
            report, invoice_date, invoice_date, {"tax_report": "generic"}
        )
        self.assertLinesValues(
            report._get_lines(options),
            #   Name                         Net              Tax
    ...
    def test_tax_unit(self):
        ...
        company_data_3 = self.setup_company_data(
            "Company 3", chart_template=company_1.chart_template_id
        )
            ...
            tax_account = self.env["account.account"].create(
                {
                    ...
                    "code": "test_tax_unit",
                    "user_type_id": self.env["ir.model.data"]._xmlid_to_res_id(
        ...
        for active_companies in (
            ...
            self.assertLinesValues(
                ...
                [
                    # Company 1
                    (
                        "%s-invoice-base" % created_taxes[company_1].id,
                        100 if company_1 in active_companies else 0,
                    ),
                    (
                        "%s-invoice-100" % created_taxes[company_1].id,
                        42 if company_1 in active_companies else 0,
                    ),
                    ("%s-refund-base" % created_taxes[company_1].id, 0),
                    ("%s-refund-100" % created_taxes[company_1].id, 0),
                    # Company 2
                    (
                        "%s-invoice-base" % created_taxes[company_2].id,
                        200
                    ...
                    (
                        "%s-invoice-100" % created_taxes[company_2].id,
                        84
                        if active_companies == unit_companies
                        or active_companies[0] == company_2
                        else 0,
                    ...
                    (
                        "%s-invoice-base" % created_taxes[company_3].id,
                        300 if company_3 == active_companies[0] else 0,
                    ),
                    (
                        "%s-invoice-100" % created_taxes[company_3].id,
                        126 if company_3 == active_companies[0] else 0,
                    ),
        ...
        self.tax_group_1.with_company(company_2).write(
            {
                "property_tax_receivable_account_id": self.company_data_2[
                    "default_account_receivable"
                ]
                .copy()
                .id,
                "property_tax_payable_account_id": self.company_data_2[
                    "default_account_payable"
                ]
                .copy()
                .id,
            ...
            options,
            {
                (company_1, self.env["account.fiscal.position"]): [
                    ...
                    },
                    {
                        "debit": 0,
                        "credit": 42,
                        "account_id": self.tax_group_1.with_company(
                            company_1
                        ).property_tax_payable_account_id.id,
                    },
                ],
    ...
    def test_vat_unit_with_foreign_vat_fpos(self):
        ...
        for unit_companies in ((company_1 + company_2), (company_2 + company_1)):
            ...
            self.assertEqual(
                options["fiscal_position"],
                "all",
                "The provided fiscal position ('domestic') was inconsistent with the use of tax units, it should have been reinitialized",
            )
