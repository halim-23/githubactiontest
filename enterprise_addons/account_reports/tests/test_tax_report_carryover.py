...
class TestTaxReportCarryover(TestAccountReportsCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.tax_42_line = cls._create_tax_report_line(
            "Base 42%",
            cls.tax_report,
            sequence=1,
            tag_name="base_42",
        ...
        cls.tax_11_line = cls._create_tax_report_line(
            "Base 11%", cls.tax_report, sequence=2, tag_name="base_11"
        )

        cls.tax_22_line = cls._create_tax_report_line(
            "Base 22%",
            cls.tax_report,
            sequence=3,
            tag_name="base_22",
        ...
        cls.tax_27_line = cls._create_tax_report_line(
            "Base 27%",
            cls.tax_report,
            sequence=4,
            tag_name="base_27",
    ...
    def _trigger_carryover_line_creation(
        self, company_data, tax_lines, with_reversal=True
    ):
        ...
        report = self.env["account.generic.tax.report"].with_company(
    ...
    def _configure_tax_for_company(self, company_data, tax_lines):
            ...
            .create(
                {
                    ...
                    "property_tax_receivable_account_id": company_data[
                        "default_account_receivable"
                    ]
                    .copy()
                    .id,
                    "property_tax_payable_account_id": company_data[
        ...
        for line in tax_lines:
            ...
            self.env["ir.model.data"].create(
                {
                    "name": "account_reports.test_tax_report_tax_"
                    + amount
                    + "_"
                    + company.name,
    ...
    def _check_carryover_test_result(
        self, invoice, report, company_data, expected_value, tax_line
    ):
        options = self._init_options(report, invoice.date, invoice.date)
        ...
        with patch.object(
            ...
            domain = tax_line.with_company(
                company_data["company"]
            )._get_carryover_lines_domain(options)
    ...
    def test_tax_report_carry_over(self):
        report, taxes, invoice = self._trigger_carryover_line_creation(
            self.company_data, [self.tax_11_line, self.tax_42_line]
        )

        # ====== Test the current value, based on the invoice created earlier ======

        self._check_carryover_test_result(
            invoice, report, self.company_data, -42.0, self.tax_42_line
        )
        ...
        invoice.action_post()
        self._check_carryover_test_result(
            invoice, report, self.company_data, -21.0, self.tax_42_line
        )
        ...
        invoice.action_post()
        self._check_carryover_test_result(
            invoice, report, self.company_data, 0.0, self.tax_42_line
        )
    ...
    def test_tax_report_carry_over_multi_company(self):
        ...
        """
        report, _, invoice = self._trigger_carryover_line_creation(
            self.company_data_2, [self.tax_11_line, self.tax_42_line]
        )
        self._check_carryover_test_result(
            invoice, report, self.company_data_2, -42.0, self.tax_42_line
        )
        report, _, invoice = self._trigger_carryover_line_creation(
            self.company_data, [self.tax_11_line, self.tax_42_line]
        )
    ...
    def test_tax_report_carry_over_non_persistent_and_used_in_balance(self):
        ...
        report, taxes, invoice = self._trigger_carryover_line_creation(
            self.company_data, [self.tax_27_line], False
        )
        ...
        self._check_carryover_test_result(
            invoice, report, self.company_data, 27.0, self.tax_22_line
        )
        ...
        line_id = report._get_generic_line_id(
            "account.tax.report.line", self.tax_22_line.id
        )
        ...
        with patch.object(
            ...
            domain = self.tax_22_line.with_company(
