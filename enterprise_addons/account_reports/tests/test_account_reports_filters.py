...
class TestAccountReportsFilters(AccountTestInvoicingCommon):
    def _assert_filter_date(
        self, filter_date, expected_date_values, previous_options=None
    ):
    ...
    def _assert_filter_comparison(
        ...
        """
        report = self.env["account.report"]
        with patch.object(type(report), "filter_date", filter_date), patch.object(
            type(report), "filter_comparison", filter_comparison
        ):
            options = {}
            ...
            for i, expected_values in enumerate(expected_period_values):
                self.assertDictEqual(
                    options["comparison"]["periods"][i], expected_values
                )
    ...
    def test_filter_date_quarter_range(self):
        ...
        self._assert_filter_comparison(
            {"filter": "this_quarter", "mode": "range"},
            {"filter": "custom", "date_from": "2016-10-01", "date_to": "2016-12-31"},
            [
                {
                    "string": "Q4\N{NO-BREAK SPACE}2016",
                    "period_type": "quarter",
    ...
    def test_filter_date_fiscalyear_range_full_year(self):
        ...
        self._assert_filter_comparison(
            {"filter": "this_year", "mode": "range"},
            {"filter": "custom", "date_from": "2016-01-01", "date_to": "2016-12-31"},
            [
                {
                    "string": "2016",
                    "period_type": "fiscalyear",
    ...
    def test_filter_date_fiscalyear_range_custom_years(self):
        ...
        self._assert_filter_comparison(
            {"filter": "this_year", "mode": "range"},
            {"filter": "custom", "date_from": "2017-07-01", "date_to": "2017-09-30"},
            [
                {
                    "string": "custom 1",
                    "period_type": "fiscalyear",
