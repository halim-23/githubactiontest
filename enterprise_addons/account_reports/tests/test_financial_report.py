...
class TestFinancialReport(TestAccountReportsCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.partner_c = cls.env["res.partner"].create(
            {"name": "partner_c", "company_id": False}
        )
        ...
        cls.report_no_parent_id = cls.env["account.financial.html.report"].create(
            {"name": "Test report"}
        )
        ...
        invoice = self.env["account.move"].create(
            {
                ...
                "date": "2016-02-02",
                "invoice_line_ids": [
                    (0, 0, {"product_id": self.product_a.id, "price_unit": 110})
                ],
            }
        )
        invoice.action_post()
    ...
    def test_financial_report_single_company(self):
        line_id = self._build_generic_id_from_financial_line(
    ...
    def test_financial_report_custom_filters(self):
        ...
        options = self._update_multi_selector_filter(
            options, "ir_filters", self.filter.ids
        )
        options["unfolded_lines"] = [line_id]
    ...
    def test_financial_report_control_domain(self):
        ...
        with self.debug_mode(self.report):
            ...
            report_line.control_domain = OR(
                [
                    ast.literal_eval(ustr(line.domain))
                    for line in report_line.children_ids
                ]
            )
            lines = self.report._get_table(options)[1]
            check_missing_exceeding(lines, {}, {})
            ...
            report_line = self.env.ref(
                "account_reports.account_financial_unaffected_earnings0"
            )
            line_id_unaffected_earnings = self.env[
                "account.financial.html.report"
            ]._get_generic_line_id(
                "account.financial.html.report.line",
                report_line.id,
            ...
            report_line.control_domain = "[('account_id', '!=', False)]"
            lines = self.report._get_table(options)[1]
            check_missing_exceeding(
                lines,
                {line_id_current_assets, line_id_unaffected_earnings},
                {line_id_unaffected_earnings},
            )
