...
class DefaultBuilder(AbstractBuilder):
    ...
    def _get_params(self, period_ids: list, options: dict, line_id: str = None) -> dict:
        chart_ids = (
            self.env["consolidation.chart"]
            .search([("period_ids", "in", period_ids)])
            .ids
        )

        cols_amount = len(self.journals) + 1
        params = super()._get_params(period_ids, options, line_id)
        params.update({"chart_ids": chart_ids, "cols_amount": cols_amount})
        return params
    ...
    def _compute_account_totals(self, account_id: int, **kwargs) -> list:
        ...
        for journal in self.journals:
            ...
            groupby_res = JournalLine.read_group(
                domain, ["amount:sum", "journal_id"], ["journal_id"]
            )
            journal_total_balance = (
                groupby_res[0]["amount"] if len(groupby_res) > 0 else 0
            )
    ...
    def _format_account_line(
        ...
        for col, journal in zip(line["columns"], self.journals):
            ...
            journal_lines_amount = self.env["consolidation.journal.line"].search_count(
                domain
            )
    ...
    def _get_default_line_totals(self, options: dict, **kwargs) -> list:
        return kwargs.get("cols_amount", len(self.journals) if self.journals else 0) * [
            0.0
        ]
