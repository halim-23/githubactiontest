...
class ComparisonBuilder(AbstractBuilder):
    ...
    def _compute_account_totals(self, account_id: int, **kwargs) -> list:
        ...
        groupby = ("period_id",)
        total_lines = self.env["consolidation.journal.line"].read_group(
            domain, ("total:sum(amount)",), groupby
        )
    ...
    def _format_account_line(
        ...
        account_line = super()._format_account_line(
    ...
    def _build_section_line(self, section, level: int, options: dict, **kwargs):
        section_totals, section_lines = super()._build_section_line(
            section, level, options, **kwargs
        )
        if kwargs.get("include_percentage", False) and section_totals and section_lines:
            section_lines[0]["columns"].append(
                self._build_percentage_column(*section_totals)
            )
