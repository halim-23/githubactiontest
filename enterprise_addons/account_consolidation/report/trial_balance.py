...
class AccountConsolidationTrialBalanceReport(models.AbstractModel):
    ...
    def action_open_view_grid(self, options):
        ...
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "account_consolidation.view_trial_balance_report_grid"
                    ).id,
                    "grid",
                ],
                [
                    self.env.ref(
                        "account_consolidation.view_trial_balance_report_graph"
                    ).id,
                    "graph",
                ],
                [
                    self.env.ref(
                        "account_consolidation.consolidation_journal_line_form"
                    ).id,
                    "form",
                ],
            ],
    ...
    def action_open_audit(self, options, params=None):
        ...
        journal_lines = self.env["consolidation.journal.line"].search(
            [("account_id", "=", account_id), ("journal_id", "=", journal_id)]
        )
        if len(journal_lines) == 0:
            return None
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_consolidation.view_account_move_line_filter"
        )
        action.update(
            {
                "context": {
                    ...
                    "consolidation_rate": company_period.rate_consolidation
                    if company_period
                    else 0,
                    "currencies_are_different": company_period.currencies_are_different
                    if company_period
                    else False,
                    "currencies": {
                        "chart": company_period.currency_chart_id.symbol
                        if company_period
                        else None,
                        "company": company_period.currency_company_id.symbol
                        if company_period
                        else None,
                    },
                },
                "views": [
                    (
                        self.env.ref(
                            "account_consolidation.view_move_line_tree_grouped_general"
                        ).id,
                        "list",
                    )
                ],
    ...
    def _get_options(self, previous_options=None):
        ...
        for key, handler in handlers.items():
            ...
            current_options[key] = handler.handle(
                previous_handler_value, base_period, current_options
            )
        return current_options

    def _get_columns_name(self, options):
        AnalysisPeriod = self.env["consolidation.period"]
        all_period_ids = PeriodsHandler.get_selected_values(options) + [
            self._get_selected_period_id()
        ]
        ...
        if len(selected_periods) == 1:
            ...
            periods_columns = [
                {"name": period.display_name, "class": "number"}
    ...
    def _get_reports_buttons(self, options):
        ...
        if not ap_is_closed:
            buttons.append(
                {"name": _("Edit"), "sequence": 10, "action": "action_open_view_grid"}
            )
        return buttons
    ...
    def print_xlsx(self, options):
        options.update(
            {"force_periods": self._get_period_ids(options), "xlsx_mode": True}
        )
        return super().print_xlsx(options)

    def print_pdf(self, options):
        options.update({"force_periods": self._get_period_ids(options)})
    ...
    def _get_default_analysis_period(self):
        ...
        :rtype: int
        """
        return self.env["consolidation.period"].search_read(
    ...
    def _get_period_ids(self, options):
        ...
        return forced_periods or PeriodsHandler.get_selected_values(options) + [
            self._get_selected_period_id()
        ]
    ...
    def _get_selected_period_id(self):
        ...
        :rtype: int
        """
        default_analysis_period = self.env.context.get(
            "default_period_id", self.env.context.get("active_id", None)
        )
