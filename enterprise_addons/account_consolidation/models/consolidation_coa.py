...
class ConsolidationChart(models.Model):
    ...
    period_ids = fields.One2many(
        "consolidation.period", "chart_id", string="Analysis Periods"
    )
    period_ids_count = fields.Integer(
        compute="_compute_period_ids_count", string="# Periods"
    )
    account_ids = fields.One2many(
        "consolidation.account", "chart_id", "Consolidation Accounts", copy=True
    )
    account_ids_count = fields.Integer(
        compute="_compute_account_ids_count", string="# Accounts"
    )
    group_ids = fields.One2many("consolidation.group", "chart_id", "Account Groups")
    group_ids_count = fields.Integer(
        compute="_compute_group_ids_count", string="# Groups"
    )
    ...
    children_ids = fields.Many2many(
        "consolidation.chart",
        "account_consolidation_inner_rel",
        "children_ids",
        "parent_ids",
        string="Sub-consolidations",
    )
    ...
    def action_open_mapping(self):
        ...
        return {
            ...
            "views": [
                [
                    self.env.ref(
                        "account_consolidation.consolidation_account_tree_mapping"
                    ).id,
                    "list",
                ]
            ],
            ...
            "search_view_id": [
                self.env.ref(
                    "account_consolidation.consolidation_account_search_mapping"
                ).id,
                "search",
            ],
        }
    ...
    def setting_consolidation_action(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_consolidation.consolidation_chart_action_onboarding"
        )
    ...
    def setting_consolidated_chart_of_accounts_action(self):
        ...
        action.update(
            {
                "context": {
                    "default_chart_id": last_chart.id,
                    "search_default_chart_id": last_chart.id,
                },
                "views": [
                    (
                        self.env.ref(
                            "account_consolidation.consolidation_account_tree_onboarding"
                        ).id,
                        "list",
                    ),
                    (False, "form"),
    ...
    def setting_create_period_action(self):
        ...
        """
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_consolidation.consolidation_period_action_onboarding"
        )
        last_chart = self.search([], order="id desc", limit=1)
...
class ConsolidationAccount(models.Model):
    ...
    currency_mode = fields.Selection(
        [("end", "Closing Rate"), ("avg", "Average Rate"), ("hist", "Historical Rate")],
        required=True,
        default="end",
        string="Currency Conversion Method",
    )
    line_ids = fields.One2many(
        "consolidation.journal.line", "account_id", string="Account"
    )

    linked_chart_ids = fields.Many2many(
        "consolidation.chart", store=False, related="chart_id.children_ids"
    )
    company_ids = fields.Many2many(
        "res.company", store=False, related="chart_id.company_ids"
    )
    ...
    using_ids = fields.Many2many(
        ...
        "consolidation_accounts_rel",
        "used_in_ids",
        "using_ids",
        string="Consolidation Accounts",
    )
    used_in_ids = fields.Many2many(
        "consolidation.account",
        "consolidation_accounts_rel",
        "using_ids",
        "used_in_ids",
    ...
    filtered_used_in_ids = fields.Many2many(
        "consolidation.account",
        readonly=False,
    ...
    def write(self, vals):
        for account in self:
            ...
            if "account_ids" in vals and not vals.get("account_ids"):
                ...
                and vals["account_ids"][0][0] == 6
                and account.company_ids - active_companies
            ):
                next_accounts = self.env["account.account"].browse(
                    vals["account_ids"][0][2]
                )
                add_accounts = [
                    (4, account.id) for account in next_accounts - account.account_ids
                ]
                remove_accounts = [
                    (3, account.id) for account in account.account_ids - next_accounts
                ]
    ...
    def _compute_filtered_used_in_ids(self):
        ...
        for record in self:
            if chart_id:
                record.filtered_used_in_ids = record.used_in_ids.filtered(
    ...
    def _inverse_filtered_used_in_ids(self):
        ...
        for record in self:
            record.used_in_ids = (
                record.filtered_used_in_ids
                + record.used_in_ids.filtered(
                    lambda x: x.chart_id.id != (chart_id or False)
                )
            )
        ...
        if operator in ("!=", "=") and operand == False:
            ...
            if chart_id:
                domain = expression.AND(
                    [domain, [("used_in_ids.chart_id", "=", chart_id)]]
                )
            if operator == "=":
...
class ConsolidationGroup(models.Model):
    ...
    account_ids = fields.One2many(
        "consolidation.account", "group_id", "Consolidation Account"
    )
    line_ids = fields.One2many(
    ...
    def _check_unique_type_of_descendant(self):
        ...
        for record in self:
            if (
                record.child_ids
                and len(record.child_ids) > 0
                and record.account_ids
                and len(record.account_ids) > 0
            ):
                raise ValidationError(
                    _(
                        "An account group can only have accounts or other groups children but not both !"
                    )
                )
