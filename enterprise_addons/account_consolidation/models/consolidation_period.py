...
class ConsolidationPeriod(models.Model):
    ...
    def _get_default_date_analysis_begin(self):
        ...
        return (
            company.compute_fiscalyear_dates(datetime.date.today())["date_from"]
            if company
            else None
        )

    chart_id = fields.Many2one(
        "consolidation.chart", string="Consolidation", required=True, ondelete="cascade"
    )
    ...
    chart_account_ids_count = fields.Integer(
        related="chart_id.account_ids_count", string="# Accounts"
    )
    display_dates = fields.Char(compute="_compute_display_dates")
    date_analysis_begin = fields.Date(
        string="Start Date", required=True, default=_get_default_date_analysis_begin
    )
    ...
    using_composition_ids = fields.One2many(
        "consolidation.period.composition", "using_period_id"
    )
    used_in_composition_ids = fields.One2many(
        "consolidation.period.composition", "composed_period_id"
    )

    company_period_ids = fields.One2many(
        "consolidation.company_period",
        "period_id",
        string="Company Periods",
        copy=False,
    )
    ...
    journal_ids_count = fields.Integer(
        compute="_compute_journal_ids_count", string="# Journals"
    )
    color = fields.Integer(
        "Color Index",
        readonly=False,
        related="chart_id.color",
        help="Used in the kanban view",
    )
    dashboard_sections = fields.Text(compute="_compute_dashboard_sections")
    company_unmapped_accounts_counts = fields.Text(
        compute="_compute_company_unmapped_accounts_counts"
    )
    ...
    def _compute_dashboard_sections(self):
        ...
        for record in self:
            domain = [
                ("period_id.id", "=", record.id),
                ("group_id.show_on_dashboard", "=", True),
            ]
            rfields = ["group_id.id", "total:sum(amount)"]
            group_by = ["group_id"]
            grouped_res = self.env["consolidation.journal.line"].read_group(
                domain, rfields, group_by
            )
            results = [
                '["%s","%s"]'
                % (
                    Section.browse(value["group_id"][0]).name,
                    record._format_value(value["total"]),
                )
    ...
    def _compute_company_unmapped_accounts_counts(self):
        ...
        for record in self:
            ...
            values = Account.with_context(context).read_group(
                domain, ["amount:count(id)", "company_id.id"], ["company_id"]
            )
            ...
            results = [
                ...
                % (
                    val["company_id"][0],
                    Company.browse(val["company_id"][0]).name,
                    val["amount"],
                )
                for val in values
            ]
    ...
    def generate_guessed_company_periods(self):
        ...
        if not self.company_period_ids:
            if self.date_analysis_begin and self.date_analysis_end and self.chart_id:
                ...
                self.company_period_ids = [
                    (0, 0, value) for value in company_period_values
                ]
    ...
    def name_get(self):
        ...
        for record in self:
            result.append(
                (record.id, "%s (%s)" % (record.chart_name, record.display_dates))
            )
        return result

    def copy(self, default=None):
        default = dict(default or {})
        default["date_analysis_begin"] = self.date_analysis_end + datetime.timedelta(
            days=1
        )
    ...
    def action_generate_journals(self):
        ...
        for record in self:
            ...
            record.journal_ids.search(
                [("auto_generated", "=", True), ("period_id", "=", record.id)]
            ).unlink()
    ...
    def action_open_mapping(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_consolidation.account_mapping_action"
        )
        action.update(
            {
                ...
                "views": [
                    [
                        self.env.ref("account_consolidation.account_mapping_tree").id,
                        "list",
                    ]
                ],
                "context": {
                ...
                "display_name": _("Account Mapping: %s (for %s)")
                % (company.name, self.chart_id.name),
            }
        )
        return action
    ...
    def action_open_form(self):
        ...
        action.update({"views": [[False, "form"]], "res_id": self.id})
        return action
    ...
    def action_open_trial_balance_grid(self):
        ...
        return {
            ...
            "search_view_id": [
                self.env.ref("account_consolidation.trial_balance_grid_search").id,
                "search",
            ],
        }
    ...
    def action_open_chart_of_accounts(self):
        ...
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_consolidation.consolidation_account_action"
        )
        action["context"] = {
    ...
    def action_save_onboarding_create_step(self):
        ...
        """
        self.env.user.company_id.sudo().set_onboarding_step_done(
    ...
    def _get_company_periods_default_values(self):
        ...
        if not previous_analysis_period:
            company_period_values = (
                self._get_company_periods_default_values_from_chart()
            )
        # Generate from analysis periods linked to chart
        else:
            company_period_values = (
                self._get_company_periods_default_values_from_period(
                    previous_analysis_period
                )
            )
    ...
    def _get_previous_similiar_period(self):
        ...
        return (
            previous_analysis_periods[0]
            if len(previous_analysis_periods) > 0
            else False
        )

    def _get_similar_periods(
        self, extra_domain=None, limit=None, order="date_analysis_end desc"
    ):
        """
...
class ConsolidationPeriodComposition(models.Model):
    ...
    composed_chart_currency_id = fields.Many2one(
        "res.currency",
        string="Composed Consolidation Currency",
        related="composed_period_id.chart_id.currency_id",
    )
    composed_period_id = fields.Many2one(
        "consolidation.period",
        ondelete="cascade",
        string="Composed Analysis Period",
        required=True,
    )
    using_period_id = fields.Many2one(
        "consolidation.period",
        string="Analysis Period Using This",
        ondelete="cascade",
        required=True,
    )
    using_chart_currency_id = fields.Many2one(
        "res.currency", related="using_period_id.chart_id.currency_id"
    )
    rate_consolidation = fields.Float(
    ...
    currencies_are_different = fields.Boolean(
        compute="_compute_currencies_are_different", readonly=True
    )

    _sql_constraints = [
        (
            "_unique_composition",
            "unique (composed_period_id, using_period_id)",
    ...
    def get_journal_lines_values(self):
        ...
        for consolidation_account in self.using_period_id.chart_id.account_ids:
            ...
            journal_lines_values.append(
                {"account_id": consolidation_account.id, "amount": amount}
            )
    ...
    def _get_total_amount(self, consolidation_account):
        ...
        amounts = (
            self.env["consolidation.journal.line"]
            .sudo()
            .read_group(domain, ["amount:sum(amount)"], [])
        )
    ...
    def _compute_currencies_are_different(self):
        ...
        """
        for record in self:
            record.currencies_are_different = (
                record.composed_chart_currency_id != record.using_chart_currency_id
            )
    ...
    def _get_default_currency_rate(self):
        ...
        return self.env["res.currency"]._get_conversion_rate(
            from_currency, to_currency, company, datetime.datetime.now()
        )
...
class ConsolidationCompanyPeriod(models.Model):
    ...
    chart_id = fields.Many2one(
        "consolidation.chart", related="period_id.chart_id", string="Chart"
    )
    period_id = fields.Many2one(
        "consolidation.period", string="Period", required=True, ondelete="cascade"
    )
    ...
    currency_chart_id = fields.Many2one(
        "res.currency",
        related="period_id.chart_id.currency_id",
    ...
    currency_rate_avg = fields.Float(
        string="Average Currency Rate",
        default=1.0,
        digits=[12, 12],
        help="How many units of company currency is needed to get 1 unit of chart currency",
    )
    currency_rate_end = fields.Float(
        string="End Currency Rate",
        default=1.0,
        digits=[12, 12],
        help="How many units of company currency is needed to get 1 unit of chart currency",
    )
    ...
    rate_control = fields.Float(
        string="Rate Control",
        help="Should be between 0 and 100 %",
        default=100,
        required=True,
    )
    rate_ownership = fields.Float(
        string="Rate Ownership",
        help="Should be between 0 and 100 %",
        default=100,
        required=True,
    )
    ...
    def _compute_currencies_are_different(self):
        ...
        for record in self:
            record.currencies_are_different = (
                record.currency_chart_id != record.currency_company_id
            )
    ...
    def get_journal_lines_values(self):
        ...
        self.ensure_one()
        journal_lines_values = []
        historical_account_ids = self.period_id.chart_id.account_ids.filtered(
            lambda x: x.currency_mode == "hist"
        )
        non_hist_account_ids = (
            self.period_id.chart_id.account_ids - historical_account_ids
        )
        ...
        for consolidation_account in non_hist_account_ids:
            ...
            currency_amount, move_lines_ids = self._get_total_balance_and_audit_lines(
                consolidation_account
            )
    ...
    def _get_total_balance_and_audit_lines(self, consolidation_account):
        ...
        res = self.env["account.move.line"].read_group(
            domain, ["balance:sum", "id:array_agg"], []
        )
    ...
    def _apply_historical_rates(self, move_line):
        ...
        """
        self.ensure_one()
        rate = self.env["consolidation.rate"].get_rate_for(
            move_line.date, self.company_id.id, self.chart_id.id
        )
        if rate:
            ...
            if currency != self.currency_chart_id:
                amount = currency._convert(
                    amount, self.currency_chart_id, self.company_id, move_line.date
                )
        return self._apply_consolidation_rate(amount)
    ...
    def _get_display_name(self):
        ...
        if date_begin.month == date_end.month and date_begin.year == date_end.year:
            ...
            return "%s (%s-%s)" % (
                generic_name,
                date_begin.strftime("%b"),
                date_end.strftime("%b %Y"),
            )
        else:
            return "%s (%s-%s)" % (
                generic_name,
                date_begin.strftime("%b %Y"),
                date_end.strftime("%b %Y"),
            )
