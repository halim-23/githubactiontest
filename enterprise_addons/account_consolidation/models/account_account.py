...
class Account(models.Model):
    ...
    consolidation_account_chart_filtered_ids = fields.Many2many(
    ...
    consolidation_color = fields.Integer(
        "Color", related="company_id.consolidation_color", readonly=True
    )
    ...
    def _inverse_filtered_consolidation_account_ids(self):
        ...
        for record in self:
            ...
            if chart_id:
                from_other_charts = from_other_charts.filtered(
                    lambda x: x.chart_id.id != chart_id
                )
            record.consolidation_account_ids = (
                record.consolidation_account_chart_filtered_ids.union(from_other_charts)
            )
        ...
        if operator in ("!=", "=") and operand is False:
            ...
            if chart_id:
                domain = expression.AND(
                    [domain, [("consolidation_account_ids.chart_id", "=", chart_id)]]
                )
            if operator == "=":
