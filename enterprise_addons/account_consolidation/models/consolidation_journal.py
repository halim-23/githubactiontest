...
class ConsolidationJournal(models.Model):
    ...
    period_id = fields.Many2one(
        "consolidation.period", string="Analysis Period", ondelete="cascade"
    )
    chart_id = fields.Many2one("consolidation.chart", string="Chart", required=True)
    company_period_id = fields.Many2one(
        "consolidation.company_period",
        string="Company Period",
        required=False,
        copy=False,
    )
    ...
    currency_id = fields.Many2one(
        "res.currency", related="period_id.chart_id.currency_id", readonly=True
    )
    composition_id = fields.Many2one(
        "consolidation.period.composition",
        string="Consolidation Period",
        required=False,
        copy=False,
    )
    line_ids = fields.One2many(
        "consolidation.journal.line", "journal_id", "Consolidation items"
    )

    auto_generated = fields.Boolean(
        default=False, string="Automatically Generated", readonly=True, copy=False
    )
    balance = fields.Monetary(
        compute="_compute_balance", string="Balance", readonly=True
    )

    # Originating company period or composition fields
    originating_currency_id = fields.Many2one(
        "res.currency", compute="_compute_originating_fields", readonly=True
    )
    rate_consolidation = fields.Float(
        compute="_compute_originating_fields",
        ...
        for record in self:
            if record.company_period_id and record.composition_id:
                raise ValidationError(
                    _(
                        "A journal entry should only be linked to a company period OR to a analysis period of another consolidation !"
                    )
                )
    ...
    def _check_chart_id(self):
        for record in self:
            if record.period_id and record.chart_id != record.period_id.chart_id:
                raise ValidationError(
                    _(
                        "When setting a period on a consolidation journal, the selected consolidation chart for the journal cannot be different from the one of the chosen period."
                    )
                )
    ...
    def _compute_balance(self):
        ...
        journal_lines = JournalLine.read_group(
    ...
    def _compute_originating_fields(self):
        ...
        for record in self:
            if record.company_period_id:
                record.originating_currency_id = (
                    record.company_period_id.currency_company_id
                )
                record.rate_consolidation = record.company_period_id.rate_consolidation
            else:
                record.originating_currency_id = (
                    record.composition_id.using_chart_currency_id
                )
    ...
    def _compute_currencies_are_different(self):
        ...
        for record in self:
            record.currencies_are_different = (
                record.currency_id != record.originating_currency_id
            )
...
class ConsolidationJournalLine(models.Model):
    ...
    journal_id = fields.Many2one(
        "consolidation.journal", string="Journal", ondelete="cascade", required=True
    )
    account_id = fields.Many2one(
        "consolidation.account", string="Consolidated Account", required=True
    )
    group_id = fields.Many2one(
        "consolidation.group", related="account_id.group_id", string="Group", store=True
    )
    period_id = fields.Many2one(
        "consolidation.period",
        related="journal_id.period_id",
        string="Period",
        store=True,
    )
    auto_generated = fields.Boolean(related="journal_id.auto_generated")
    currency_amount = fields.Monetary(
        string="Currency Amount", currency_field="chart_currency_id"
    )
    journal_originating_currency_id = fields.Many2one(
        "res.currency", related="journal_id.originating_currency_id", readonly=True
    )
    amount = fields.Monetary(string="Amount", currency_field="chart_currency_id")
    chart_currency_id = fields.Many2one(
        "res.currency", related="account_id.chart_id.currency_id", readonly=True
    )
    ...
    def _check_conditional_unicity(self):
        ...
        for record in self:
            if (
                ...
                and record.journal_id.auto_generated
                and record.account_id.currency_mode != "hist"
            ):
                domain = [
                ...
                if (
                    existings.get((record.journal_id, record.account_id), False)
                    or record.search(domain).exists()
                ):
                    raise ValidationError(
    ...
    def _unlink_except_autogenerated(self):
        if (
            not self.env.context.get("allow_unlink", False)
            and self.journal_id
            and self.journal_id.auto_generated
        ):
    ...
    def _grid_make_empty_cell(self, row_domain, column_domain, view_domain):
        ...
        if len(column_domain) == 1:
            ...
            if domain_clause[0] == "journal_id":
                ...
                cell["readonly"] = (
                    self.env["consolidation.journal"]
                    .search(journal_domain, limit=1)
                    .auto_generated
                )
        return cell
