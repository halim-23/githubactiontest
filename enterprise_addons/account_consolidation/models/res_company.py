...
class Company(models.Model):
    ...
    consolidation_color = fields.Integer(
        "Accounts color",
        required=False,
        store=True,
        default=_default_consolidation_color,
    )
    ...
    consolidation_dashboard_onboarding_state = fields.Selection(
        ...
        string="State Of The Account Consolidation Dashboard Onboarding Panel",
        default="not_done",
    )
    ...
    consolidation_setup_consolidation_state = fields.Selection(
        ...
        string="State Of The Onboarding Consolidation Step",
        default="not_done",
    )
    consolidation_setup_ccoa_state = fields.Selection(
        [("not_done", "Not done"), ("just_done", "Just done"), ("done", "Done")],
        string="State Of The Onboarding Consolidated Chart Of Account Step",
        default="not_done",
    )
    consolidation_create_period_state = fields.Selection(
        [("not_done", "Not done"), ("just_done", "Just done"), ("done", "Done")],
        string="State Of The Onboarding Create Period Step",
        default="not_done",
    )
    ...
    def _compute_account_consolidation_currency_is_different(self):
        for record in self:
            record.account_consolidation_currency_is_different = (
                self._context.get("consolidation_currency_id") != record.currency_id.id
            )
    ...
    def action_open_rate_ranges(self):
        ...
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id(
            "account_consolidation.consolidation_rate_action"
        )
        action.update(
            {
                "context": {
