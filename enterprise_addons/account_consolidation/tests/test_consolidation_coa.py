...
class TestAccountConsolidationChart(AccountConsolidationTestCase):
    ...
    def test_account_ids_count(self):
        ...
        chart = self.env["consolidation.chart"].create(
            {"name": "bluh", "currency_id": 1}
        )
...
class TestAccountConsolidationAccount(AccountConsolidationTestCase):
    def setUp(self):
        ...
        self.mapped_account = self._create_consolidation_account(
            "I am mapped", chart=self.chart, section=None
        )

        self.not_mapped_account = self._create_consolidation_account(
            "I am not mapped", chart=self.chart, section=None
        )
        ...
        self.super_accounts = [
            self._create_consolidation_account(
                "I am the mapping one %s" % sc.id, chart=sc, section=None
            )
            for sc in self.super_charts
        ]
        self.mapped_account.write(
            {"used_in_ids": [(6, 0, [sa.id for sa in self.super_accounts])]}
        )

    # TESTS
    def test_filtered_used_in_ids_removal(self):
        context_chart = self.super_charts[0]
        ConsoAccountCtx = self.env["consolidation.account"].with_context(
            chart_id=context_chart.id
        )

        # Removed all mapped accounts with a chart in context
        ConsoAccountCtx.browse(self.mapped_account.id).write(
            {"filtered_used_in_ids": [(6, 0, [])]}
        )
    ...
    def test_filtered_used_in_ids_search(self):
        # TESTING THE SEARCH
        ConsoAccountCharts = [
            self.env["consolidation.account"].with_context(chart_id=sc.id)
            for sc in self.super_charts
        ]
    ...
    def test_filtered_used_in_ids(self):
        ...
        self.assertEqual(
            len(ConsoAccount.browse(self.mapped_account.id).used_in_ids), 2
        )

        for sc in self.super_charts:
            consolidation_account = ConsoAccount.with_context(chart_id=sc.id).browse(
                self.mapped_account.id
            )
            self.assertEqual(len(consolidation_account.filtered_used_in_ids), 1)
            self.assertEqual(
                len(consolidation_account.used_in_ids), len(self.super_accounts)
            )

        # TESTING THE WRITING
        new_super_account = self._create_consolidation_account(
            "New super account", chart=self.super_charts[0]
        )
        self.mapped_account.with_context(
            chart_id=self.super_charts[0].id
        ).filtered_used_in_ids += new_super_account
        ...
        self.assertEqual(
            len(
                self.mapped_account.with_context(
                    chart_id=self.super_charts[1].id
                ).filtered_used_in_ids
            ),
            1,
        ...
        second_super_chart_not_mappeds = self.mapped_account.with_context(
            chart_id=self.super_charts[1].id
        ).search(
            [("chart_id", "=", self.chart.id), ("filtered_used_in_ids", "=", False)]
        )
        second_super_chart_mappeds = self.mapped_account.with_context(
            chart_id=self.super_charts[1].id
        ).search(
            [("chart_id", "=", self.chart.id), ("filtered_used_in_ids", "!=", False)]
        )
        ...
        self.assertEqual(
            second_super_chart_not_mappeds[0].id, self.not_mapped_account.id
        )
    ...
    def test_filtered_consolidation_account_ids(self):
        ...
        mapped_account = self._create_account(
            "001", "RCV", company=self.default_company
        )
        account_not_mapped = self._create_account(
            "002", "RCV2", company=self.default_company
        )
        ...
        conso_accounts = [
            self._create_consolidation_account(
                "Chart 1 BLAH 1", chart=self.chart, section=None
            ),
            self._create_consolidation_account(
                "Chart 1 BLAH 2", chart=self.chart, section=None
            ),
            self._create_consolidation_account(
                "Chart 1 BLAH 3", chart=self.chart, section=None
            ),
            self._create_consolidation_account(
                "Chart 2 BLAH 1", chart=chart_2, section=None
            ),
            self._create_consolidation_account(
                "Chart 2 BLAH 2", chart=chart_2, section=None
            ),
        ]

        mapped_account.write(
            {"consolidation_account_ids": [(6, 0, [ca.id for ca in conso_accounts])]}
        )
        self.assertEqual(
            len(mapped_account.consolidation_account_ids), len(conso_accounts)
        )
        ...
        # TESTING THE WRITING
        new_conso_account = self._create_consolidation_account(
            "Chart 1 BLAH 1", chart=self.chart, section=None
        )
        account_c1.write(
        ...
        self.assertEqual(
            len(account_c1.consolidation_account_ids), len(conso_accounts) + 1
        )
        ...
        # ACCOUNT IS NOW CONSIDERED AS MAPPED FOR CHART 1
        not_mappeds = AccountChart1.search(
            [("consolidation_account_chart_filtered_ids", "=", False)]
        )
        self.assertEqual(len(not_mappeds), amount_of_unmapped_account - 1)
        mappeds = AccountChart1.search(
            [("consolidation_account_chart_filtered_ids", "!=", False)]
        )
        self.assertEqual(len(mappeds), 2)
        self.assertSetEqual(
            set(mappeds.ids), {account_not_mapped.id, mapped_account.id}
        )

        # NOTHING CHANGE FOR CHART 2
        not_mappeds = AccountChart2.search(
            [("consolidation_account_chart_filtered_ids", "=", False)]
        )
        self.assertEqual(len(not_mappeds), amount_of_unmapped_account)
        self.assertIn(account_not_mapped.id, not_mappeds.ids)
        mappeds = AccountChart2.search(
            [("consolidation_account_chart_filtered_ids", "!=", False)]
        )
        self.assertEqual(len(mappeds), 1)
        self.assertEqual(mappeds[0].id, mapped_account.id)
