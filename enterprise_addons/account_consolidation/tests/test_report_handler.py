...
class TestAbstractHandler(TransactionCase):
    ...
    @patch(
        "odoo.addons.account_consolidation.report.handler.abstract.AbstractHandler.get_selected_values"
    )
    def test__is_set(self, patched):
        combinations = (([], False), ([1], True), ([1, 1], True))
...
class TestJournalHandler(AccountConsolidationTestCase):
    ...
    @patch(
        "odoo.addons.account_consolidation.report.handler.periods.PeriodsHandler.is_set"
    )
    @patch(
        "odoo.addons.account_consolidation.report.handler.journals.JournalsHandler.get_option_values"
    )
    def test_handle(self, patched_options, patched_isset):
        patched_options.return_value = [
            {"id": 1, "selected": True},
            {"id": 2, "selected": False},
        ]
    ...
    def test__get_selected_values(self):
        ...
        options = {
            "consolidation_journals": [
                {"id": 1, "name": "BLAH", "selected": False},
                {"id": 2, "name": "BLUH", "selected": True},
            ]
        }
    ...
    def test_to_option_dict(self):
        selected = True
        exp = {"id": self.journal.id, "name": self.journal.name, "selected": selected}
...
class TestPeriodsHandler(AccountConsolidationTestCase):
    ...
    def test__get_selected_values(self):
        options = {
            "periods": [
                {"id": 1, "name": "bluh", "selected": True},
                {"id": 2, "name": "blah", "selected": False},
                {"id": 3, "name": "bleh", "selected": True},
            ]
        }
    ...
    @patch(
        "odoo.addons.account_consolidation.report.handler.periods.PeriodsHandler.get_selected_values",
        return_value=[1, 3],
    )
    ...
    def test_handle(self):
        base_period = self._create_analysis_period(
            start_date="2019-02-01", end_date="2019-02-28"
        )
        previous_period = self._create_analysis_period(
            start_date="2019-01-01", end_date="2019-01-31"
        )
        pprevious_period = self._create_analysis_period(
            start_date="2018-12-01", end_date="2018-12-31"
        )
        initial_client_state = None
        first_handle_result = self.periods_handler.handle(
            initial_client_state, base_period, {}
        )
        self.assertIn(
            self.periods_handler._to_option_dict(pprevious_period, []),
            first_handle_result,
        )
        self.assertIn(
            self.periods_handler._to_option_dict(previous_period, []),
            first_handle_result,
        )

        # Second handle result
        client_state = [
            self.periods_handler._to_option_dict(
                pprevious_period, [previous_period.id]
            ),
            self.periods_handler._to_option_dict(previous_period, [previous_period.id]),
        ]
        second_handle_result = self.periods_handler.handle(
            client_state, base_period, {}
        )
...
class TestShowZeroHandler(AccountConsolidationTestCase):
    @patch(
        "odoo.addons.account_consolidation.report.handler.show_zero.ShowZeroHandler._line_is_not_zero"
    )
    def test_account_line_should_be_added(self, patched_line_not_zero):
        ...
        self.assertTrue(
            ShowZeroHandler.account_line_should_be_added(
                will_be_ignored, options_enabled
            )
        )
        ...
        for patched_value in possible_values:
            ...
            self.assertEqual(
                ShowZeroHandler.account_line_should_be_added(will_be_ignored, None),
                patched_value,
            )
            self.assertEqual(
                ShowZeroHandler.account_line_should_be_added(will_be_ignored, {}),
                patched_value,
            )
            self.assertEqual(
                ShowZeroHandler.account_line_should_be_added(
                    will_be_ignored, options_disabled
                ),
                patched_value,
            )
    ...
    @patch(
        "odoo.addons.account_consolidation.report.handler.show_zero.ShowZeroHandler._section_line_is_not_zero"
    )
    @patch(
        "odoo.addons.account_consolidation.report.handler.show_zero.ShowZeroHandler._section_line_has_children"
    )
    def test_section_line_should_be_added(self, patched_children, patched_zero):
        ...
        options_disabled = {"show_zero_balance_accounts": False}
        self.assertTrue(
            ShowZeroHandler.section_line_should_be_added(will_be_ignored, None)
        )
        patched_children.assert_not_called()
        patched_zero.assert_not_called()
        self.assertTrue(
            ShowZeroHandler.section_line_should_be_added(
                will_be_ignored, options_enabled
            )
        )
        patched_children.assert_not_called()
        ...
        for pc_value, pz_value, exp_value in combination_values:
            ...
            self.assertEqual(
                ShowZeroHandler.section_line_should_be_added(will_be_ignored, {}),
                exp_value,
            )
            self.assertEqual(
                ShowZeroHandler.section_line_should_be_added(
    ...
    def test__line_is_not_zero(self):
        ...
        for line in test_lines:
            self.assertEqual(
                ShowZeroHandler._line_is_not_zero(line[0]),
                line[1],
    ...
    def test__section_line_has_children(self):
        ...
        self.assertFalse(
            ShowZeroHandler._section_line_has_children([{"id": "section-1"}])
        )
        self.assertTrue(
            ShowZeroHandler._section_line_has_children([{"id": "section-1"}, {"id": 1}])
        )
