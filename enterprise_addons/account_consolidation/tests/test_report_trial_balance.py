...
class TestTrialBalanceReport(AccountConsolidationTestCase):
    ...
    def test_plain_all_journals(self):
        ...
        self.assertListEqual(
            expected_matrix, matrix, "Report amounts are not all corrects"
        )
        for line in lines:
            line_id = line["id"]
            parent_id = line.get("parent_id", None)
            self.assertFalse(
                line.get("unfoldable", False), "Account line should not be unfoldable"
            )
            if line.get("class", None) != "total":
                ...
                if line_id != self.consolidation_accounts["alone in the dark"].id:
                    ...
                    self.assertIsNone(
                        line.get("parent_id", None),
                        "Account line alone in the dark should not have a parent_id but does (%s)"
                        % line,
                    )
    ...
    def test_hierarchy_all_journals(self):
        ...
        self.assertEqual(
            real_headers[0]["name"],
            report._get_journal_col(self.journals["be"][0], options)["name"],
            'First column should be the column of "BE company" journal',
        )
        self.assertEqual(
            real_headers[1]["name"],
            report._get_journal_col(self.journals["us"][0], options)["name"],
        ...
        self.assertEqual(
            lines[0]["id"], self.consolidation_accounts["alone in the dark"].id
        )
        ...
        for line in lines:
            ...
            if str(line_id).startswith("section"):
                section = self.env["consolidation.group"].browse(
                    int(line_id.split("-")[1])
                )
                ...
                self.assertFalse(
                    line.get("unfoldable", False),
                    "Account line should not be unfoldable",
                )
    ...
    def _report_lines_to_matrix(self, lines):
        matrix = []
        for line in lines:
            matrix_line = [line["name"]] + [
                col["no_format_name"] for col in line["columns"]
            ]
    ...
    def _create_chart_of_accounts(self, chart):
        ...
        chart = chart or self.chart
        bst = Section.create({"name": "Balance sheet", "chart_id": chart.id})
        pal = Section.create({"name": "Profit and loss", "chart_id": chart.id})
        exp = Section.create(
            {"name": "Expense", "parent_id": pal.id, "chart_id": chart.id}
        )
        inc = Section.create(
            {"name": "Income", "parent_id": pal.id, "chart_id": chart.id}
        )
        self.sections = [bst, pal, exp, inc]

        self.consolidation_accounts = {
            "revenue": self._create_consolidation_account(
                "Revenue", chart=chart, section=inc.id
            ),
            "finance income": self._create_consolidation_account(
                "Finance Income", chart=chart, section=inc.id
            ),
            "cost of sales": self._create_consolidation_account(
                "Cost of Sales", chart=chart, section=exp.id
            ),
            "alone in the dark": self._create_consolidation_account(
                "Alone in the dark", chart=chart, section=None
            ),
        }
