...
class TestAccountConsolidationJournal(AccountConsolidationTestCase):
    ...
    def test_balance(self):
        ...
        JournalLine.create(
            {
                "account_id": self._create_consolidation_account().id,
                "journal_id": self.env["consolidation.journal"]
                .create(
                    {
                        "name": "bluh",
                        "chart_id": self.chart.id,
        ...
        self.assertEqual(
            JournalLine.search_count([("journal_id", "=", journal.id)]), count
        )
    ...
    def test_action_generate_journal_lines_when_origin_is_company_period(
        self, patched_company_period_method, patched_conso_method
    ):
        ...
        cps = (
            self._create_company_period(ap, self.default_company),
    ...
    def test_action_generate_journal_lines_when_origin_is_composition(
        self, patched_company_period_method, patched_conso_method
    ):
        ...
        compo = self.env["consolidation.period.composition"].create(
            {"composed_period_id": cap.id, "using_period_id": uap.id}
        )
        journal = self.env["consolidation.journal"].create(
            {
                "name": "blah",
...
class TestAccountConsolidationJournalLine(AccountConsolidationTestCase):
    ...
    def test__check_conditional_unicity(self):
        ...
        editable_journal = self.env["consolidation.journal"].create(
            {"name": "BLAH", "chart_id": self.chart.id}
        )
        not_editable_journal = self.env["consolidation.journal"].create(
            {"name": "BLAH", "auto_generated": True, "chart_id": self.chart.id}
        )
    ...
    def test_adjust_grid(self):
        ...
        # THEN
        self.assertEqual(
            len(created_journal_line), 1, "A journal line has been created"
        )
        self.assertAlmostEqual(
            created_journal_line.amount,
            params["change"],
        ...
        # THEN
        self.assertAlmostEqual(
            journal_line.amount, amount_before, msg="Old journal line did not change"
        )

        # CANNOT CREATE AS JOURNAL LINKED TO COMPANY (no journal line created)
        # GIVEN
        journal = self.env["consolidation.journal"].create(
            {"name": "bluh", "auto_generated": True, "chart_id": self.chart.id}
        )
    ...
    def test_write(self):
        ...
        journal_line = self.env["consolidation.journal.line"].create(
            {"journal_id": journal.id, "account_id": account.id, "amount": 42}
        )
        with self.assertRaises(UserError):
    ...
    def test__grid_column_info(self):
        ...
        journal_in = Journal.create(
            {
                "name": "blah",
                "period_id": aperiod.id,
                "chart_id": self.chart.id,
            }
        )
        journal_out = Journal.create(
            {
                "name": "bluh",
                "chart_id": self.chart.id,
            }
        ...
        self.assertEqual(
            cinfo.values[0]["values"]["journal_id"], (journal_in.id, journal_in.name)
        )
        self.assertEqual(
            cinfo.values[0]["domain"],
            [("journal_id", "=", journal_in.id)],
        )
        ...
        for i, journal in enumerate((journal_in, journal_out)):
            self.assertEqual(
                cinfo.values[i]["values"]["journal_id"], (journal.id, journal.name)
            )
            self.assertEqual(
                cinfo.values[i]["domain"],
                [("journal_id", "=", journal.id)],
            )
    ...
    def test__grid_make_empty_cell(self):
        ...
        self.assertFalse(
            cell["readonly"],
            "Created empty cell in a column with no journal should not be readonly",
        )

        m = self.env["consolidation.journal"].create(
            {"name": "bluh", "chart_id": self.chart.id}
        )
        ...
        self.assertTrue(
            cell["readonly"],
            "Created empty cell in the column of a readonly journal should be readonly",
        )
    ...
    def test__journal_is_editable(self):
        ...
        journal_line = self.env["consolidation.journal.line"].create(
            {"journal_id": journal.id, "account_id": self.dummy_account.id}
        )
        params = {
            "domain": [("id", "=", journal_line.id)],
            "column_field": "journal_id",
