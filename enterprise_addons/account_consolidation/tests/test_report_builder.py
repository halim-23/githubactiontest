...
from odoo.addons.account_consolidation.report.builder.comparison import (
    ComparisonBuilder,
)
...
class TestAbstractBuilder(AccountConsolidationTestCase):
    ...
    def setUp(self):
        ...
        self.ap = self._create_analysis_period(
            start_date="2019-02-01", end_date="2019-02-28"
        )
        self.builder = ComparisonBuilder(self.env, self.ap._format_value)

    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._get_hierarchy"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._get_plain"
    )
    def test_get_lines(self, patched_get_plain, patched_get_hierarchy):
        ...
        self.assertListEqual(
            patched_get_hierarchy.return_value,
        ...
        self.assertListEqual(
            patched_get_hierarchy.return_value,
            self.builder.get_lines(period_ids, options, 1),
        )
        ...
        self.assertListEqual(
            patched_get_plain.return_value,
            self.builder.get_lines(period_ids, options, None),
        )
    ...
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._handle_accounts"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._build_total_line"
    )
    def test__get_plain(self, patched_total, patched_handle):
        ...
        patched_total.assert_called_once_with(
            patched_handle.return_value[0], options, **kwargs
        )

    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._handle_orphan_accounts"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._get_root_sections"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._handle_sections"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._build_total_line"
    )
    def test__get_hierarchy(
        self, patched_total, patched_sections, patched_root, patched_orphan
    ):
        patched_total.return_value = {
            "columns": [{"no_format_name": 4242}, {"no_format_name": 2424}]
        }
        patched_sections.return_value = (
            [42, 24],
            [
                {
                    "id": "rootsect1",
                    "name": "rootsect1",
                    "columns": [{"no_format_name": 40}, {"no_format_name": 20}],
                },
                {
                    "id": "sect2",
                    "name": "sect2",
                    "parent_id": "rootsect1",
                    "columns": [{"no_format_name": 2}, {"no_format_name": 4}],
                },
            ],
        )
        patched_root.return_value = [1]  # Need at least 1
        patched_orphan.return_value = (
            [4200, 2400],
            [
                {
                    "id": 1,
                    "name": "orphan1",
                    "columns": [{"no_format_name": 4200}, {"no_format_name": 2400}],
                }
            ],
        )
        chart_ids = []
        ...
        options = {}
        line_id = None
        res = self.builder._get_hierarchy(
            options,
            line_id,
            chart_ids=chart_ids,
            cols_amount=cols_amount,
        ...
        expected = (
            patched_orphan.return_value[1]
            + patched_sections.return_value[1]
            + [patched_total.return_value]
        )
    ...
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._build_section_line"
    )
    @patch(
    ...
    def test__handle_sections(self, patched_added, patched_build):
        ...
        patched_build.return_value = (
            ...
            [
                ...
                },
                {
                    "columns": [
                        {"no_format_name": -0.42},
                        {"no_format_name": -42.01},
                        {"no_format_name": 0.02},
                    ]
                },
                {
                    "columns": [
                        {"no_format_name": 4200.84},
                        {"no_format_name": 14.01},
                        {"no_format_name": -0.03},
                    ]
                },
            ],
        ...
        totals, lines = self.builder._handle_sections(
            sections,
            options={},
            level=2,
            cols_amount=3,
    ...
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._get_orphan_accounts"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._handle_accounts"
    )
    def test__handle_orphan_accounts(self, patched_handle_accounts, patched_get_orphan):
        ...
        patched_handle_accounts.assert_called_once_with(
            patched_get_orphan.return_value, options, level, **kwargs
        )

    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._compute_account_totals"
    )
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._format_account_line"
    )
    @patch(
        "odoo.addons.account_consolidation.report.handler.show_zero.ShowZeroHandler.account_line_should_be_added",
        return_value=True,
    )
    def test__handle_accounts(
        self, patched_account_added, patched_build_account, patched_compute
    ):
        ...
        period_ids = [0, 0]
        totals, lines = self.builder._handle_accounts(
            accounts,
            {},
            2,
            cols_amount=2,
    ...
    def test__get_root_sections(self):
        ...
        Section.create(
            {"name": "bluh", "parent_id": root_s.id, "chart_id": self.chart.id}
        )
        Section.create(
            {"name": "bluh", "parent_id": root_s.id, "chart_id": self.chart.id}
        )
        self.assertEqual(
            root_s, self.builder._get_root_sections({}, chart_ids=[self.chart.id])
        )
    ...
    def test__get_orphan_accounts(self):
        ...
        self.assertEqual(
            len(self.builder._get_orphan_accounts({}, chart_ids=[self.chart.id])), 0
        )
        orphan_account = self._create_consolidation_account()
        self.assertEqual(
            orphan_account,
            self.builder._get_orphan_accounts({}, chart_ids=[self.chart.id]),
        )
...
class TestComparisonBuilder(AccountConsolidationTestCase):
    ...
    def test__compute_account_totals(self):
        ...
        actual = self.builder._compute_account_totals(
            account.id, period_ids=[p.id for p in periods]
        )
    ...
    def test__build_section_line_and_format_account_line(
        self, patched_account_totals, patched_account_added, patched_section_added
    ):
        ...
        section = self.env["consolidation.group"].create(
            {
                ...
                "chart_id": self.chart.id,
                "child_ids": [
                    (0, 0, {"name": "BLUH CHILD", "chart_id": self.chart.id})
                ],
            }
        )
        ...
        expected = [
            ...
            },
            {
                ...
                "columns": [
                    {"name": "1,000.00 €", "no_format_name": 1000.0, "class": "number"},
                    {
                        "name": "-2,000.00 €",
        ...
        section_totals, section_line = self.builder._build_section_line(
            section, level, options, include_percentage=False
        )
    ...
    def test__build_section_line_no_children_no_accounts(self):
        ...
        options = {}
        section_totals, section_line = self.builder._build_section_line(
            section, level, options, cols_amount=2, include_percentage=False
        )
        expected = [
            {
                "id": "section-%s" % section.id,
    ...
    def test__build_percentage_column(self):
        ...
        for orig_value, now_value, exp_percent, exp_class in test_values:
            perc_column = ComparisonBuilder._build_percentage_column(
                orig_value, now_value
            )
    ...
    @patch(
        "odoo.addons.account_consolidation.report.builder.comparison.ComparisonBuilder._build_percentage_column",
    ...
    def test__build_total_line(self, patched_bpc):
        ...
        euro_exp = {
            ...
            "level": 1,
            "columns": [
                {"name": "0.00 €", "no_format_name": 0.0, "class": "number"},
                {
                    "name": "1,500,000.00 €",
                    "no_format_name": 1500000.0,
                    "class": "number text-danger",
                },
                {
                    "name": "-2,000.00 €",
                    "no_format_name": -2000.0,
        ...
        euro_total_line = self.builder._build_total_line(
            totals, {}, include_percentage=False
        )
        ...
        usd_total_line = us_builder._build_total_line(
            totals, [ap_usd], include_percentage=False
        )
        ...
        # WITH PERCENTAGE
        totals = [0.0, -2000.0]
        euro_prct_total_line = self.builder._build_total_line(
            totals, {}, include_percentage=True
        )
        euro_exp_prct = {
            "id": "grouped_accounts_total",
            "name": "Total",
...
class TestDefaultBuilder(AccountConsolidationTestCase):
    ...
    def test__format_account_line(self):
        level = 2
        totals = [12.0, 13.14]
        line = self.builder._format_account_line(
            self.consolidation_account, level, totals, {}
        )
        ...
        expected = {
            ...
            "level": level,
            "name": account_name,
            "title_hover": "%s (%s Currency Conversion Method)"
            % (account_name, account_currency_name),
            "columns": [
                {
