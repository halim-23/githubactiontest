...
from odoo.addons.account_consolidation.tests.account_consolidation_test_classes import (
    AccountConsolidationTestCase,
)
...
class TestConsolidationPeriod(AccountConsolidationTestCase):
    ...
    def test_unlink(self):
        ...
        composition = Composition.create(
            {"composed_period_id": ap.id, "using_period_id": ap2.id}
        )
        ...
        composition2 = Composition.create(
            {"composed_period_id": ap3.id, "using_period_id": ap2.id}
        )
    ...
    def test_action_generate_journals(self):
        ...
        self.assertDictEqual(
            states["first"]["amounts"],
            states["second"]["amounts"],
        ...
        self.assertEqual(
            0,
            Journal.search_count([("id", "in", states["first"]["ids"]["journal"])]),
            "Old journals have been deleted (and so new ones have been generated",
        )
        self.assertEqual(
            0,
            JournalLine.search_count(
                [("id", "in", states["first"]["ids"]["journal_line"])]
            ),
    ...
    def test__get_company_periods_default_values_to_chart(
        self, patched_get_company_values, patched_get_similar
    ):
        ap = self._create_analysis_period()
    ...
    def test__get_company_periods_default_values_to_period(
        self, patched_get_company_values, patched_get_similar
    ):
        ap = self._create_analysis_period()
    ...
    def test__get_company_periods_default_values_from_chart(self):
        ...
        for company_id in self.chart.company_ids.ids:
            ...
            self.assertNotEqual(
                index,
                -1,
                "Default values for company %s should be generated" % company_id,
            )
            company_values = values_list[index]
            self.assertEqual(company_values["chart_id"].id, self.chart.id)
            self.assertEqual(
                company_values["date_company_begin"], ap.date_analysis_begin
            )
    ...
    def test__dashboard_sections(self):
        ...
        for section in sections:
            accounts = (
                self.env["consolidation.account"].create(
                    {"name": "BLAH #" + str(i), "group_id": section.id}
                ),
                self.env["consolidation.account"].create(
                    {"name": "BLIH #" + str(i), "group_id": section.id}
                ),
            )
        ...
        excl_section = AccountSection.create(
            {"name": "Not listed section", "chart_id": self.chart.id}
        )
        journal = self.env["consolidation.journal"].create(
            {
                "name": "BLAH",
        ...
        self.env["consolidation.journal.line"].create(
            {
                "account_id": self.env["consolidation.account"]
                .create({"name": "Excluded account", "group_id": excl_section.id})
                .id,
                "journal_id": journal.id,
            }
        )
        ...
        self.assertEqual(
            list(map(lambda x: x.name, sections[:2])),
            [val[0] for val in dashboard_sections],
        )
    ...
    def test__display_dates(self):
        ap_same_month_and_year = self._create_analysis_period(
            start_date="2019-01-01", end_date="2019-01-31"
        )
        expected_str = ap_same_month_and_year.date_analysis_begin.strftime(
            "%b %Y"
        )  # Jan 2019
        self.assertEqual(
            ap_same_month_and_year.display_dates.lower(), expected_str.lower()
        )

        ap_same_month = self._create_analysis_period(
            start_date="2019-01-01", end_date="2020-01-31"
        )
        expected_str = "-".join(
            (
                ap_same_month.date_analysis_begin.strftime(
                    "%b %Y"
                ),  # Jan 2019-Jan 2020
        ...
        ap_same_year = self._create_analysis_period(
            start_date="2019-01-01", end_date="2019-12-31"
        )
        expected_str = "-".join(
            (
                ap_same_year.date_analysis_begin.strftime("%b"),  # Jan-Dec 2019
                ap_same_year.date_analysis_end.strftime("%b %Y"),
            )
        )
        self.assertEqual(ap_same_year.display_dates.lower(), expected_str.lower())

        ap_nothing_common = self._create_analysis_period(
            start_date="2019-01-01", end_date="2020-06-01"
        )
        expected_str = "-".join(
            (
                ap_nothing_common.date_analysis_begin.strftime(
                    "%b %Y"
                ),  # Jan 2019-Jun 2020
    ...
    def _generate_company_period(self, ap, nb, company):
        return self._create_company_period(
            period=ap, rate_consolidation=10 * nb, company=company
        )
...
class TestConsolidationPeriodComposition(AccountConsolidationTestCase):
    def setUp(self):
        ...
        usd_chart = self.env["consolidation.chart"].create(
            {"name": "USD chart", "currency_id": 2}
        )
        usd_ap = self._create_analysis_period(chart=usd_chart)
        # Children accounts
        children_accounts = [
            self._create_consolidation_account(
                "Child account 1", chart=self.chart, section=None
            ),
            self._create_consolidation_account(
                "Child account 2", chart=self.chart, section=None
            ),
        ]
        usd_children_accounts = [
            self._create_consolidation_account(
                "USD Child account 1", chart=usd_chart, section=None
            ),
            self._create_consolidation_account(
                "USD Child account 2", chart=usd_chart, section=None
            ),
        ]
        # Account in child conso but not mapped to an account in super conso
        other_account = self._create_consolidation_account(
            "Child account 2", chart=self.chart, section=None
        )

        # Parent/Super conso
        super_chart = self.env["consolidation.chart"].create(
            {"name": "Super chart", "currency_id": 1}
        )
        super_ap = self._create_analysis_period(chart=super_chart)

        # Super account (in super conso)
        self.super_account = self._create_consolidation_account(
            "Super account", chart=super_chart, section=None
        )

        # Link account to children accounts
        self.super_account.write(
            {
                "using_ids": [
                    (6, 0, [ca.id for ca in children_accounts + usd_children_accounts])
                ]
            }
        )
        ...
        self.env["consolidation.journal"].create(
            {
                ...
                "chart_id": self.chart.id,
                "line_ids": [
                    (0, 0, {"account_id": children_accounts[0].id, "amount": 42}),
                    (0, 0, {"account_id": children_accounts[1].id, "amount": 4200}),
                    (0, 0, {"account_id": other_account.id, "amount": 420000}),
                ],
            }
        )
        self.env["consolidation.journal"].create(
            {
                ...
                "line_ids": [
                    (0, 0, {"account_id": usd_children_accounts[0].id, "amount": 8000}),
                    (
                        0,
                        0,
                        {"account_id": usd_children_accounts[1].id, "amount": 12000},
                    ),
                ],
            }
        )
    ...
    def test_generate_journal(self, patched_ap_action_generate_journals):
        ...
        self.assertEqual(
            amount_of_journals,
            AccountJournal.search_count([]),
            "Old journal has been deleted",
        )
    ...
    def test__get_total_amount(self):
        ...
        self.assertAlmostEqual(
            self.usd_compo._get_total_amount(self.super_account), 1000.0
        )
    ...
    @patch(
        "odoo.addons.base.models.res_currency.Currency._get_conversion_rate",
        return_value=150.0,
    )
    def test__get_default_currency_rate(self, mocked_method):
        ...
        mocked_method.assert_called_once_with(
            self.usd_compo.composed_chart_currency_id,
            self.usd_compo.using_chart_currency_id,
            self.env.company,
            ANY,
        )
...
class TestConsolidationCompanyPeriod(AccountConsolidationTestCase):
    def test_get_display_name(self):
        ...
        cp = self._create_company_period(
            period=ap, start_date="2019-01-01", end_date="2019-01-31"
        )
        ...
        expected_str = (
            cp.company_name + " (" + cp.date_company_begin.strftime("%b %Y") + ")"
        )
        self.assertEqual(expected_str, cp._get_display_name())
        ...
        expected_str = (
            ...
            + "-".join(
                (
                    cp.date_company_begin.strftime("%b %Y"),  # Jan 2019-Jan 2020
        ...
        expected_str = (
            ...
            + "-".join(
                (
                    cp.date_company_begin.strftime("%b"),  # # Jan-Dec 2019
                    cp.date_company_end.strftime("%b %Y"),
                )
            )
            + ")"
        ...
        expected_str = (
            ...
            + "-".join(
                (
                    cp.date_company_begin.strftime("%b %Y"),  # Jan 2019-Jun 2020
                    cp.date_company_end.strftime("%b %Y"),
                )
            )
            + ")"
    ...
    def test_generate_journal(self, patch_apply_rates, patched_get_total_balance):
        ...
        ap = self._create_analysis_period()
        cp = self._create_company_period(
            period=ap, rate_consolidation=10, company=self.default_company
        )
        cp.generate_journal()
        journals = Journal.search([("company_period_id", "=", cp.id)])
        self.assertEqual(
            1, len(journals), "Company period should only have one Journal"
        )
        journal = journals[0]
        self.assertEqual(
            journal.period_id.id,
            ap.id,
            "Created journal should have the right analysis period",
        )

        journal_lines = JournalLine.search([("journal_id", "=", journal.id)])
        self.assertEqual(
            2, len(journal_lines), "Generated journal should have two journal lines"
        )
        self.assertNotEqual(
            journal_lines[0].account_id,
            journal_lines[1].account_id,
            "Generated journals lines should be linked to different accounts",
        )
        for journal_line in journal_lines:
            self.assertAlmostEqual(
                journal_line.currency_amount,
                patched_get_total_balance.return_value[0],
                msg="Generated journals should have the right currency amount",
            )
            self.assertAlmostEqual(
                journal_line.amount,
                patch_apply_rates.return_value,
    ...
    def test_get_journal_lines_values(self, patch_apply_rates, patch_get_total_balance):
        ...
        ap = self._create_analysis_period()
        cps = (
            self._create_company_period(
                period=ap, rate_consolidation=10.0, company=self.default_company
            ),
            self._create_company_period(
                period=ap, rate_consolidation=20.0, company=self.us_company
            ),
        )
        expected = [
            {
    ...
    def test__apply_historical_rates(self):
        ...
        cp = self._create_company_period(
            period=ap,
            rate_consolidation=consolidation_rate,
            company=self.us_company,
            start_date="2010-01-01",
            end_date="2024-12-31",
        )
        account_credit = self._create_account(
            "111", "Credit account", company=self.us_company
        )
    ...
    def test__apply_rates(self, patched_apply_consolidation_rate, patched_convert):
        cp = self._create_company_period(
            start_date="2019-01-01", end_date="2019-01-31", company=self.us_company
        )
        ...
        self.assertAlmostEqual(
            patched_apply_consolidation_rate.return_value, cp._apply_rates(amount, ca)
        )
        patched_convert.assert_called_once_with(amount, ca.currency_mode)
        patched_apply_consolidation_rate.assert_called_once_with(
            patched_convert.return_value
        )

    def test__get_total_balance_and_audit_lines(self):
        journals = {
            "good": self.env["account.journal"].create(
                {
                    "name": "Bank 123456",
                    "code": "BNK67",
                    "type": "bank",
            ...
            "ignored": self.env["account.journal"].create(
                {
                    "name": "Outlier",
                    "code": "OUT",
                    "type": "bank",
                    "bank_acc_number": "12345226",
                    "company_id": self.default_company.id,
                }
            ),
        }
        revenue_type = self.env.ref("account.data_account_type_revenue")
        dummy_account = self._create_account(
            "003", "NOTCONSIDERATED", company=self.default_company, type=revenue_type
        )
        not_ignored_accounts = [
            self._create_account(
                "001", "RCV", company=self.default_company, type=revenue_type
            ),
            self._create_account(
                "002", "RCV2", company=self.default_company, type=revenue_type
            ),
        ]
        ...
        self.env["account.move"].create(
            {
                ...
                "journal_id": journals["good"].id,
                "line_ids": [
                    (0, 0, {"account_id": dummy_account.id, "credit": 1100}),
                    (
                        0,
                        0,
                        {
        ...
        total_balance, associated_move_line_ids = cp._get_total_balance_and_audit_lines(
            conso_account
        )
        ...
        for move_line_id in associated_move_line_ids:
            self.assertEqual(
                self.env["account.move.line"].browse(move_line_id).move_id, right_move
            )
    ...
    def test__get_move_lines(self):
        ...
        include_initial_balance_type = self.env.ref(
        ...
        include_initial_account = self._create_account(
            "INC", "Normal account", type=include_initial_balance_type
        )
        include_consolidation_account.write(
            {"account_ids": [(4, include_initial_account.id)]}
        )

        cp = self._create_company_period(
            start_date="2019-01-01", end_date="2019-12-31", company=self.default_company
        )
        ...
        self._create_basic_move(
            9042,
            account_credit=normal_account,
            move_date=date(2012, 6, 30),
            account_debit=include_initial_account,
            journal=journal,
    ...
    def test__apply_consolidation_rate(self):
        cp = self._create_company_period(
            start_date="2019-01-01", end_date="2019-01-31", rate_consolidation=75.0
        )
