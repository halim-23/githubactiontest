    ...
    def write(self, vals):
        ...
        if "state" in vals and vals["state"] == "done":
            for appraisal in self:
                ...
                deleted_skills = []
                added_skills = []
                for employee_skill in employee_skills.filtered(
                    lambda s: s.skill_id in appraisal_skills.skill_id
                ):
                    appraisal_skill = appraisal_skills.filtered(
                        lambda a: a.skill_id == employee_skill.skill_id
                    )
                ...
                deleted_skills = employee_skills.filtered(
                    lambda s: s.skill_id not in appraisal_skills.skill_id
                ).mapped("skill_id.name")
                added_skills = appraisal_skills.filtered(
                    lambda a: a.skill_id not in employee_skills.skill_id
                ).mapped("skill_id.name")
                ...
                if len(updated_skills + added_skills + deleted_skills) > 0:
                    template = self.env.ref(
                        "hr_appraisal_skills.appraisal_skills_update_template",
...
class HrAppraisalSkill(models.Model):
    ...
    employee_id = fields.Many2one(
        "hr.employee", related="appraisal_id.employee_id", store=True
    )
    manager_ids = fields.Many2many(
        "hr.employee", compute="_compute_manager_ids", store=True
    )
