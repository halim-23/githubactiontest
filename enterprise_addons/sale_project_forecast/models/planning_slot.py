...
class PlanningSlot(models.Model):
    ...
    def _compute_project_id(self):
        ...
        for slot in self:
            if (
                not slot.project_id
                and slot.sale_line_id
                and (
                    slot.sale_line_id.project_id or slot.sale_line_id.task_id.project_id
                )
            ):
                slot.project_id = (
                    slot.sale_line_id.project_id or slot.sale_line_id.task_id.project_id
    ...
    def _compute_sale_line_id(self):
        ...
        for slot in self:
            if not slot.sale_line_id and slot.project_id:
                slot.sale_line_id = (
                    slot.task_id.sale_line_id or slot.project_id.sale_line_id
                )
    ...
    def _name_get_fields(self):
        ...
        name_get_fields = [
            item
            for item in super()._name_get_fields()
            if item not in ["sale_line_id", "project_id", "task_id"]
        ]
        ...
        def insert_after_or_append(preceding_priority, priority_to_insert):
            if preceding_priority in priority_list:
                priority_list.insert(
                    priority_list.index(preceding_priority) + 1, priority_to_insert
                )
            else:
    ...
    def _get_employee_per_priority(self, priority, employee_ids_to_exclude, cache):
        ...
        """
        employee_id = super()._get_employee_per_priority(
            priority, employee_ids_to_exclude, cache
        )
        if employee_id or priority in cache:
            return employee_id
        if (
            priority == "task_assignee"
            and self.task_id
            and len(self.task_id.user_ids) == 1
        ):
