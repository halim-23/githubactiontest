...
class WorksheetTemplate(models.Model):
    ...
    model_id = fields.Many2one(
        "ir.model", ondelete="cascade", readonly=True, domain=[("state", "=", "manual")]
    )
    ...
    report_view_id = fields.Many2one(
        "ir.ui.view", domain=[("type", "=", "qweb")], readonly=True
    )
    ...
    def _compute_worksheet_count(self):
        for record in self:
            record.worksheet_count = (
                record.model_id
                and self.env[record.model_id.model].search_count([])
                or 0
            )
    ...
    def _check_res_model_exists(self):
        ...
        ir_model_names = [
            res["model"]
            for res in self.env["ir.model"]
            .sudo()
            .search_read([("model", "in", res_models)], ["model"])
        ]
            ...
            for res_model, templates in template_dict.items():
                for model, name in self._get_models_to_check_dict()[res_model]:
                    records = self.env[model].search(
                        [("worksheet_template_id", "in", templates.ids)]
                    )
                    for record in records:
                        if (
                            record.company_id
                            not in record.worksheet_template_id.company_ids
                        ):
                            raise UserError(
                                _(
                                    "There is still %s linked to this template. Please unlink them first."
                                )
                                % name
                            )
        return res
    ...
    def unlink(self):
        ...
        self.env["ir.ui.view"].search(
            [("model", "in", self.mapped("model_id.model"))]
        ).unlink()  # backend views (form, pivot, ...)
        self.mapped("report_view_id").unlink()  # qweb templates
        self.env["ir.model.access"].search([("model_id", "in", models_ids)]).unlink()
        x_name_fields = self.env["ir.model.fields"].search(
            [("model_id", "in", models_ids), ("name", "=", "x_name")]
        )
        x_name_fields.write(
            {"related": False}
        )  # we need to manually remove relation to allow the deletion of fields
    ...
    def copy(self, default=None):
        ...
        template = super(
            WorksheetTemplate, self.with_context(worksheet_no_generation=True)
        ).copy(default)
        template._generate_worksheet_model()
        return template
    ...
    def _generate_worksheet_model(self):
        ...
        if not hasattr(self, "_get_%s_manager_group" % res_model):
            raise NotImplementedError(
                "Method _get_%s_manager_group not implemented on %s"
                % (res_model, res_model)
            )
        if not hasattr(self, "_get_%s_user_group" % res_model):
            raise NotImplementedError(
                "Method _get_%s_user_group not implemented on %s"
                % (res_model, res_model)
            )
        if not hasattr(self, "_get_%s_access_all_groups" % res_model):
            raise NotImplementedError(
                "Method _get_%s_access_all_groups not implemented on %s"
                % (res_model, res_model)
            )
        ...
        self.env["ir.rule"].sudo().create(
            {
                ...
                "groups": [
                    (6, 0, [getattr(self, "_get_%s_user_group" % res_model)().id])
                ],
            }
        )
        self.env["ir.rule"].sudo().create(
            {
                ...
                "domain_force": [(1, "=", 1)],
                "groups": [
                    (6, 0, getattr(self, "_get_%s_access_all_groups" % res_model)().ids)
                ],
            }
        )
        ...
        self.env["ir.model.data"].sudo().create(
            [action_xmlid_values, model_xmlid_values, view_xmlid_values]
        )
    ...
    def _prepare_default_fields_values(self):
        ...
        fields_func = getattr(
            self, "_default_%s_template_fields" % res_model_name, False
        )
        return [
            (
                0,
    ...
    def _prepare_default_form_view_values(self, model):
        ...
        res_model_name = self.res_model.replace(".", "_")
        form_arch_func = getattr(
            self, "_default_%s_worksheet_form_arch" % res_model_name, False
        )
        return {
            "type": "form",
        ...
        action.update(
            {
                ...
                "context": {
                    "default_x_%s_id"
                    % self.res_model.replace(
                        ".", "_"
                    ): True,  # to hide model_id from view
                    "form_view_initial_mode": "readonly",
                },  # to avoid edit mode at studio exit
            }
        )
        return action
    ...
    def _get_qweb_arch_omitted_fields(self):
        return [
            "x_%s_id" % self.res_model.replace(".", "_"),
            "x_name",  # redundant
        ]
        ...
        if field_name not in self._get_qweb_arch_omitted_fields():
            ...
            if widget:
                ...
                if is_signature or widget == "image":
                    field_node.attrib["t-options"] = "{'widget': '%s'}" % (
                        widget if not is_signature else "image"
                    )
            ...
            if field_info["type"] != "binary" or widget in ["image", "signature"]:
                ...
                if is_signature:
                    ...
                    field_node.tag = "i"
                    field_node.attrib["t-att-class"] = (
                        "'text-wrap col-8 fa ' + ('fa-check-square' if %s else 'fa-square-o')"
                        % field_name
                    )
                else:
                    field_node.tag = "div"
    ...
    def _get_qweb_arch(self, ir_model, qweb_template_name, form_view_id=False):
        ...
        """
        fields_view_get_result = self.env[ir_model.model].fields_view_get(
            view_id=form_view_id, view_type="form", toolbar=False, submenu=False
        )
        form_view_arch = fields_view_get_result["arch"]
        form_view_fields = fields_view_get_result["fields"]

        qweb_arch = etree.Element("div")
        for row_node in etree.fromstring(form_view_arch).xpath(
            "//group[not(ancestor::group)]|//field[not(ancestor::group)]"
        ):
            ...
            if (
                "name" in row_node.attrib
                and row_node.attrib["name"] not in self._get_qweb_arch_omitted_fields()
                and row_node.attrib["name"] in form_view_fields
            ):
                field_node = row_node
                new_container_col = self._add_field_node_to_container(
                    field_node, form_view_fields, container_col
                )
                ...
                if len(cols) > 0:
                    container_row = etree.Element(
                        "div", {"class": "row", "style": "page-break-inside: avoid"}
                    )
                    for col_node in cols:
                        container_col = etree.Element(
                            "div",
                            {"class": "col-6", "style": "page-break-inside: avoid"},
                        )
                ...
                self.env["ir.model.data"].create(
                    {
                        ...
                        "module": getattr(
                            self,
                            "_get_%s_module_name" % self.res_model.replace(".", "_"),
                        )(),
