...
class Picking(models.Model):
    ...
    l10n_mx_edi_content = fields.Binary(
        compute="_l10n_mx_edi_compute_edi_content", compute_sudo=True
    )
    ...
    l10n_mx_edi_des_lat = fields.Float(
        string="Destination Latitude", related="partner_id.partner_latitude"
    )
    l10n_mx_edi_des_lon = fields.Float(
        string="Destination Longitude", related="partner_id.partner_longitude"
    )
    ...
    l10n_mx_edi_cfdi_file_id = fields.Many2one(
        "ir.attachment", string="CFDI file", copy=False
    )
    ...
    def _l10n_mx_edi_compute_edi_content(self):
        for picking in self:
            picking.l10n_mx_edi_content = base64.b64encode(
                picking._l10n_mx_edi_create_delivery_guide()
            )
    ...
    def _l10n_mx_edi_check_comex_availability(self):
        ...
        if any(self.mapped("l10n_mx_edi_is_export")):
            raise UserError(
                _(
                    "The Delivery Guide is only available for shipping in MX. You might want to install comex features"
                )
            )
    ...
    def _l10n_mx_edi_check_required_data(self):
        for record in self:
            if not record.l10n_mx_edi_transport_type:
                raise UserError(
                    _("You must select a transport type to generate the delivery guide")
                )
            if record.move_line_ids.mapped("product_id").filtered(
                lambda product: not product.unspsc_code_id
            ):
                raise UserError(_("All products require a UNSPSC Code"))
            if (
                not record.company_id.l10n_mx_edi_certificate_ids.sudo().get_valid_certificate()
            ):
                raise UserError(_("A valid certificate was not found"))
            if (
                record.l10n_mx_edi_transport_type == "01"
                and not record.l10n_mx_edi_distance
            ):
                raise UserError(
                    _("Distance in KM must be specified when using federal transport")
                )
    ...
    def _l10n_mx_edi_create_delivery_guide(self):
        ...
        for record in self:
            ...
            mx_tz = self.env["account.move"]._l10n_mx_edi_get_cfdi_partner_timezone(
                ...
                or record.company_id.partner_id
            )
            date_fmt = "%Y-%m-%dT%H:%M:%S"
            warehouse_zip = (
                record.picking_type_id.warehouse_id.partner_id
                and record.picking_type_id.warehouse_id.partner_id.zip
                or record.company_id.zip
            )
            ...
            values = {
                ...
                "scheduled_date": record.scheduled_date.astimezone(mx_tz).strftime(
                    date_fmt
                ),
            ...
            certificate = (
                record.company_id.l10n_mx_edi_certificate_ids.sudo().get_valid_certificate()
            )
            if certificate:
    ...
    def _l10n_mx_edi_validate_with_xsd(self, xml_str, raise_error=False):
        ...
        if not result and raise_error:
            xmlschema.assertValid(
                xml_doc
            )  # if the document is invalid, raise the error for debugging
        return result
                ...
                .search(
                    [
                        (
                            "name",
                            "=",
                            ATTACHMENT_NAME.format(self.l10n_mx_edi_cfdi_uuid),
                        ),
    ...
    def _l10n_mx_edi_decode_cfdi(self, cfdi_data=None):
        ...
        return {
            ...
            "cadena": self.env["l10n_mx_edi.certificate"]._get_cadena_chain(
                cfdi_node, CFDI_XSLT_CADENA
            ),
    ...
    def l10n_mx_edi_action_set_partner_coordinates(self):
        mb_token = (
            ...
            .sudo()
            .get_param("web_map.token_map_box", False)
        )
        if not mb_token:
            raise UserError(_("Please configure MapBox to use this feature"))
        for record in self:
            src = (
                record.picking_type_id.warehouse_id.partner_id.contact_address_complete
            )
            dest = record.partner_id.contact_address_complete
            if not (src and dest):
                raise UserError(
                    _("The warehouse address and the delivery address are required")
                )
            src_address = url_quote(src)
            url = f"{MAPBOX_GEOCODE_URL}{src_address}.json?"
            fetched_data = record._l10n_mx_edi_request_mapbox(
                url, {"access_token": mb_token}
            )
            res = json.loads(fetched_data.content)
            if "features" in res:
                record.picking_type_id.warehouse_id.partner_id.partner_latitude = res[
                    "features"
                ][0]["geometry"]["coordinates"][0]
                record.picking_type_id.warehouse_id.partner_id.partner_longitude = res[
                    "features"
            ...
            res = json.loads(fetched_data.content)
            if "features" in res:
                record.partner_id.partner_latitude = res["features"][0]["geometry"][
                    "coordinates"
                ][0]
                record.partner_id.partner_longitude = res["features"][0]["geometry"][
                    "coordinates"
                ][1]
    ...
    def l10n_mx_edi_action_calculate_distance(self):
        ...
        for record in self:
            if (
                record.l10n_mx_edi_src_lat
                and record.l10n_mx_edi_src_lon
                and record.l10n_mx_edi_des_lat
                and record.l10n_mx_edi_des_lon
            ):
            ...
            record.write(
                {
                    ...
                    "l10n_mx_edi_status": "cancelled",
                    "l10n_mx_edi_origin": origin,
                }
            )
    ...
    def l10n_mx_edi_action_send_delivery_guide(self):
        ...
        for record in self:
            ...
            res = getattr(
                self.env["account.edi.format"],
                "_l10n_mx_edi_%s_sign_service" % pac_name,
            )(credentials, cfdi_str)
            if res.get("errors"):
                record.l10n_mx_edi_error = "\n".join(res["errors"])
                continue
            ...
            cfdi_signed = (
                base64.decodebytes(res["cfdi_signed"])
                if res["cfdi_encoding"] == "base64"
                else res["cfdi_signed"]
            )
            ...
            cfdi_attachment = self.env["ir.attachment"].create(
                {
                    ...
                    "description": _(
                        "Mexican Delivery Guide CFDI generated for the %s document.",
                        record.name,
                    ),
                }
            )
    ...
    def l10n_mx_edi_action_update_sat_status(self):
        ...
        for record in self:
            ...
            try:
                ...
                record._message_log(
                    body=_("Failure during update of the SAT status: %(msg)s", msg=e)
                )
                continue
