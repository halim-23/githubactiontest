...
class Vehicle(models.Model):
    ...
    vehicle_config = fields.Selection(
        selection=[
            ...
            (
                "C2R2",
                "Camión-Remolque (6 llantas en el camión y 8 llantas en remolque)",
            ),
            (
                "C3R2",
                "Camión-Remolque (10 llantas en el camión y 8 llantas en remolque)",
            ),
            (
                "C2R3",
                "Camión-Remolque (6 llantas en el camión y 12 llantas en remolque)",
            ),
            (
                "C3R3",
                "Camión-Remolque (10 llantas en el camión y 12 llantas en remolque)",
            ),
            (
                "T2S1",
    ...
    def name_get(self):
        return [
            (vehicle.id, "[%s] %s" % (vehicle.vehicle_licence, vehicle.name))
            for vehicle in self
        ]
        ...
        for vehicle in self:
            ...
            if not operators:
                raise ValidationError(
                    _(
                        "The vehicle intermediaries must contain at least one intermediary of type: Operator"
                    )
                )
    ...
    def _check_trailers(self):
        for vehicle in self:
            if len(vehicle.trailer_ids) > 2:
                raise ValidationError(
                    _("A maximum of 2 trailers are allowed per vehicle")
                )
