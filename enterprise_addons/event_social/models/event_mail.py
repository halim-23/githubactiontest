...
class EventMailScheduler(models.Model):
    ...
    def _selection_template_model(self):
        return super(EventMailScheduler, self)._selection_template_model() + [
            ("social.post.template", "Social")
        ]

    notification_type = fields.Selection(
        selection_add=[("social_post", "Social Post")],
        ondelete={"social_post": "set default"},
    )

    @api.depends("notification_type")
    def _compute_template_model_id(self):
        social_model = self.env["ir.model"]._get("social.post.template")
        social_mails = self.filtered(
            lambda mail: mail.notification_type == "social_post"
        )
    ...
    def _check_interval_type(self):
        ...
        for mail in self:
            if (
                mail.template_ref
                and mail.template_ref._name == "social.post.template"
                and mail.interval_type == "after_sub"
            ):
                raise UserError(
                    _(
                        "As social posts have no recipients, they cannot be triggered by registrations."
                    )
                )

    def execute(self):
        social_post_mails = self.filtered(
            lambda mail: mail.template_ref
